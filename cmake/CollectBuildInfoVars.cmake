# =============================================================================
# This script is included to set all build information variables in the calling
# scope. It does not use a function to avoid variable scope issues.
# ==============================================================================
...
# if(DEFINED CMAKE_BUILD_TYPE) string(TOUPPER ${CMAKE_BUILD_TYPE}
# CMAKE_BUILD_TYPE_UPPER) list(APPEND TEMP_CXX_FLAGS_LIST
# "${CMAKE_CXX_FLAGS_${CMAKE_BUILD_TYPE_UPPER}}") endif()
string(REPLACE ";" " " ABACUS_CXX_FLAGS "${TEMP_CXX_FLAGS_LIST}")
set(TEMP_LINKER_FLAGS_LIST "${CMAKE_EXE_LINKER_FLAGS}")
# if(DEFINED CMAKE_BUILD_TYPE) string(TOUPPER ${CMAKE_BUILD_TYPE}
# CMAKE_BUILD_TYPE_UPPER) list(APPEND TEMP_LINKER_FLAGS_LIST
# "${CMAKE_EXE_LINKER_FLAGS_${CMAKE_BUILD_TYPE_UPPER}}") endif()
...
if(ENABLE_MPI)
  ...
  if(MPI_VERSION_OUTPUT MATCHES "Intel")
    ...
    string(
      REGEX
        MATCH
        "Intel\\(R\\) oneAPI DPC\\+\\+/C\\+\\+ Compiler ([0-9]+\\.[0-9]+\\.[0-9]+)"
        _
        "${MPI_VERSION_OUTPUT}")
    if(CMAKE_MATCH_1)
      set(MPI_DETECTED_VERSION "${CMAKE_MATCH_1}")
    else()
      string(REGEX MATCH "icpc \\(ICC\\) ([0-9]+\\.[0-9]+\\.[0-9]+)" _
                   "${MPI_VERSION_OUTPUT}")
      if(CMAKE_MATCH_1)
        set(MPI_DETECTED_VERSION "${CMAKE_MATCH_1}")
      endif()
  ...
  if(NOT MPI_DETECTED_VERSION)
    ...
    if(MPIRUN_EXECUTABLE)
      execute_process(
        ...
        OUTPUT_VARIABLE MPIRUN_VERSION_OUTPUT
        OUTPUT_STRIP_TRAILING_WHITESPACE ERROR_QUIET)
      if(MPIRUN_VERSION_OUTPUT MATCHES "Open MPI")
        set(ABACUS_MPI_IMPLEMENTATION "OpenMPI")
        string(REGEX MATCH "Open MPI ([0-9]+\\.[0-9]+\\.[0-9]+)" _
      ...
      elseif(MPIRUN_VERSION_OUTPUT MATCHES "MPICH")
        set(ABACUS_MPI_IMPLEMENTATION "MPICH")
        string(REGEX MATCH "MPICH Version: ([0-9]+\\.[0-9]+\\.[0-9]+)" _
                     "${MPIRUN_VERSION_OUTPUT}")
        if(CMAKE_MATCH_1)
          set(MPI_DETECTED_VERSION "${CMAKE_MATCH_1}")
        endif()
  ...
  if(USE_CUDA)
    ...
    if(OMPI_INFO_EXECUTABLE)
      execute_process(
        COMMAND ${OMPI_INFO_EXECUTABLE} --parsable --all
        OUTPUT_VARIABLE OMPI_INFO_OUT
        OUTPUT_STRIP_TRAILING_WHITESPACE ERROR_QUIET)
...
if(ENABLE_LCAO AND USE_ELPA)
  ...
  if(ELPA_VERSION)
    ...
    find_program(
      ELPA_VERSION_EXE elpa2_print_version
      PATHS ${ELPA_DIR}/bin
      NO_DEFAULT_PATH)
    if(ELPA_VERSION_EXE)
      execute_process(
        COMMAND ${ELPA_VERSION_EXE}
        OUTPUT_VARIABLE ELPA_VER_RAW
        OUTPUT_STRIP_TRAILING_WHITESPACE ERROR_QUIET)
      if(ELPA_VER_RAW)
...
if(MKLROOT)
  ...
  find_path(
    MKL_VERSION_HEADER mkl_version.h
    PATHS ${MKL_INCLUDE}
    NO_DEFAULT_PATH)
  if(MKL_VERSION_HEADER)
    file(STRINGS "${MKL_VERSION_HEADER}" MKL_VERSION_LINE
         REGEX "^#define INTEL_MKL_VERSION [0-9]+")
    string(REGEX REPLACE "^#define INTEL_MKL_VERSION ([0-9]+)" "\\1"
                         MKL_VER_NUM "${MKL_VERSION_LINE}")
    if(MKL_VER_NUM)
...
elseif(NOT USE_SW AND NOT MKLROOT)
  if(FFTW3_INCLUDE_DIR AND EXISTS "${FFTW3_INCLUDE_DIR}/fftw3.h")
    file(STRINGS "${FFTW3_INCLUDE_DIR}/fftw3.h" _fftw_ver_line
         REGEX "^#define[\t ]+FFTW_VERSION[\t ]+\"[^\"]+\"")
    if(_fftw_ver_line)
      string(REGEX REPLACE "^#define[\t ]+FFTW_VERSION[\t ]+\"([^\"]+)\"" "\\1"
                           FFTW3_VERSION "${_fftw_ver_line}")
      set(ABACUS_FFTW_VERSION "yes (v${FFTW3_VERSION})")
    else()
...
if(ENABLE_LCAO)
  ...
  if(CEREAL_INCLUDE_DIR AND EXISTS "${CEREAL_INCLUDE_DIR}/cereal/version.hpp")
    file(STRINGS "${CEREAL_INCLUDE_DIR}/cereal/version.hpp" CEREAL_MAJOR_LINE
         REGEX "^#define CEREAL_VERSION_MAJOR")
    file(STRINGS "${CEREAL_INCLUDE_DIR}/cereal/version.hpp" CEREAL_MINOR_LINE
         REGEX "^#define CEREAL_VERSION_MINOR")
    file(STRINGS "${CEREAL_INCLUDE_DIR}/cereal/version.hpp" CEREAL_PATCH_LINE
         REGEX "^#define CEREAL_VERSION_PATCH")
    string(REGEX REPLACE "^#define CEREAL_VERSION_MAJOR +([0-9]+).*" "\\1"
                         CEREAL_MAJOR "${CEREAL_MAJOR_LINE}")
    string(REGEX REPLACE "^#define CEREAL_VERSION_MINOR +([0-9]+).*" "\\1"
                         CEREAL_MINOR "${CEREAL_MINOR_LINE}")
    string(REGEX REPLACE "^#define CEREAL_VERSION_PATCH +([0-9]+).*" "\\1"
                         CEREAL_PATCH "${CEREAL_PATCH_LINE}")
    if(CEREAL_MAJOR
       AND CEREAL_MINOR
       AND CEREAL_PATCH)
      set(ABACUS_CEREAL_VERSION
          "yes (v${CEREAL_MAJOR}.${CEREAL_MINOR}.${CEREAL_PATCH})")
    endif()
  endif()
  if(ABACUS_CEREAL_VERSION STREQUAL "yes (version unknown)"
     AND CEREAL_INCLUDE_DIR)
  ...
  if(HIP_VERSION)
    ...
    if(HIPCC_EXECUTABLE)
      execute_process(
        ...
        OUTPUT_STRIP_TRAILING_WHITESPACE ERROR_QUIET)
      string(REGEX MATCH "HIP version: ([0-9]+\\.[0-9]+\\.[0-9]+)" _
                   "${HIPCC_VERSION_OUTPUT}")
      if(CMAKE_MATCH_1)
...
if(ENABLE_LIBRI)
  ...
  if(EXISTS ${LIBRI_VERSION_HEADER})
    file(STRINGS "${LIBRI_VERSION_HEADER}" LIBRI_MAJOR_LINE
         REGEX "__LIBRI_VERSION_MAJOR")
    file(STRINGS "${LIBRI_VERSION_HEADER}" LIBRI_MINOR_LINE
         REGEX "__LIBRI_VERSION_MINOR")
    file(STRINGS "${LIBRI_VERSION_HEADER}" LIBRI_PATCH_LINE
         REGEX "__LIBRI_VERSION_PATCH")
    ...
    if(LIBRI_MAJOR
       AND LIBRI_MINOR
       AND LIBRI_PATCH)
      set(ABACUS_LIBRI_VERSION
          "yes (v${LIBRI_MAJOR}.${LIBRI_MINOR}.${LIBRI_PATCH})")
    endif()
  endif()
...
if(DEFINED DeePMD_DIR)
  ...
  if(EXISTS ${DEEPMD_VERSION_HEADER})
    file(STRINGS "${DEEPMD_VERSION_HEADER}" DEEPMD_VERSION_LINE
         REGEX "global_install_prefix")
    if(DEEPMD_VERSION_LINE)
      string(
        REGEX
          MATCH
          "global_install_prefix=\".*deepmd-kit-([0-9]+\\.[0-9]+\\.[0-9]+)\"" _
          "${DEEPMD_VERSION_LINE}")
      if(CMAKE_MATCH_1)
