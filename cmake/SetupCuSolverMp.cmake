...
function(abacus_setup_cusolvermp target_name)
  ...
  if(NOT CUSOLVERMP_LIBRARY OR NOT CUSOLVERMP_INCLUDE_DIR)
    message(
      FATAL_ERROR "cuSOLVERMp not found. Set CUSOLVERMP_PATH or NVHPC_ROOT_DIR."
    )
  endif()
  ...
  if(EXISTS "${CUSOLVERMP_VERSION_HEADER}")
    ...
    string(REGEX MATCH "([0-9]+)" CUSOLVERMP_VER_MINOR
                 "${CUSOLVERMP_MINOR_LINE}")
    string(REGEX MATCH "([0-9]+)" CUSOLVERMP_VER_PATCH
  ...
  # Auto-select communicator backend by cuSOLVERMp version. cuSOLVERMp < 0.7.0
  # -> CAL, otherwise -> NCCL.
  set(_use_cal OFF)
  ...
  # Backend selection: - _use_cal=ON  -> cal communicator backend - _use_cal=OFF
  # -> NCCL communicator backend
  if(_use_cal)
    ...
    if(NOT TARGET CAL::CAL)
      ...
      set_target_properties(
        CAL::CAL PROPERTIES INTERFACE_LINK_LIBRARIES "${CAL_LIBRARY}"
    ...
    if(NOT TARGET NCCL::NCCL)
      ...
      set_target_properties(
        NCCL::NCCL
        PROPERTIES INTERFACE_LINK_LIBRARIES "${NCCL_LIBRARY}"
  ...
  if(NOT TARGET cusolverMp::cusolverMp)
    ...
    set_target_properties(
      ...
      PROPERTIES INTERFACE_LINK_LIBRARIES "${CUSOLVERMP_LIBRARY}"
                 INTERFACE_INCLUDE_DIRECTORIES "${CUSOLVERMP_INCLUDE_DIR}")
  endif()

  # === Link libraries ===
  if(_use_cal)
    target_link_libraries(${target_name} CAL::CAL cusolverMp::cusolverMp)
  else()
    target_link_libraries(${target_name} NCCL::NCCL cusolverMp::cusolverMp)
  endif()
endfunction()
