...
void TwoCenterTable::build(const RadialCollection& bra, const RadialCollection& ket, const char op, const int nr, const double* const rgrid, const bool deriv)
{
    assert( nr > 0 && rgrid );
    cleanup();
        ...
        for (int l1 = 0; l1 <= bra.lmax(); ++l1)
        {
            for (const NumericalRadial** it1 = bra.cbegin(l1); it1 != bra.cend(l1); ++it1)
            {
                for (int l2 = std::abs(l1-l); l2 <= std::min(ket.lmax(), l + l1); l2 += 2)
                {
                    for (const NumericalRadial** it2 = ket.cbegin(l2); it2 != ket.cend(l2); ++it2)
                    {
                        (*it1)->radtab(op, **it2, l, table_.inner_most_ptr<double>(table_index(*it1, *it2, l)), 
                                nr_, rgrid_, deriv);
                    }
                }
            }
