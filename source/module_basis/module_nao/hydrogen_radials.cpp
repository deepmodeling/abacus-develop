...
{
    if (with_slater_screening && rank == 0)
    {
        printf("Build hydrogen_radials with Slater screening coefficients.\n");
    }
    cleanup();
    itype_ = itype;
    symbol_ = symbol;
                                                                      ...
                                                                      std::ofstream* ptr_log)
{
    ...
    double norm_factor = sqrt(4.0 * std::pow(screened_charge, 3)
                              * static_cast<double>(this->assoc_laguerre_.factorial(n - l - 1)) / std::pow(double(n), 4)
                              / static_cast<double>(this->assoc_laguerre_.factorial(n + l)) / std::pow(a0, 3));

    for (int ir = 0; ir != ngrid; ++ir)
    {
        // Bohr radius is 1.0
        double rho = 2.0 * rgrid[ir] * screened_charge / n / a0;
        rvalue[ir] = norm_factor * std::pow(rho, l) * exp(-rho / 2.0) * this->assoc_laguerre_.value(n, l, rho);
    }
    return rvalue;
}

double HydrogenRadials::radial_norm(const std::vector<double> rgrid, const std::vector<double> rvalue)
{
                                                        ...
                                                        std::ofstream* ptr_log)
{
    ...
    while ((std::fabs(delta_norm) > conv_thr))
    {
        ...
        std::vector<double> rvalue_segment
            = generate_hydrogen_radial_segment(charge, with_slater_screening, n, l, rmin_, rmax_, dr, rank, ptr_log);
...
std::vector<std::pair<int, int>> HydrogenRadials::unzip_strategy(const int nmax, const std::string strategy)
{
    ...
    else if (strategy.substr(0, 6) == "energy")
    {
        ...
        if (strategy == "energy-valence")
        {
            ...
            {
                if (nl_pair.first > real_nmax)
                    real_nmax = nl_pair.first;
            }
            for (auto it = nl_pairs.begin(); it != nl_pairs.end();)
            {
                if (it->first == real_nmax)
                    nmax_ls.push_back(it->second);
                else if (it->first == real_nmax - 1)
                    nmax_minus1_ls.push_back(it->second);
                else
                {
...
void HydrogenRadials::smooth(std::vector<double>& rgrid, std::vector<double>& rvalue, const double sigma)
{
...
std::map<std::pair<int, int>, std::pair<std::vector<double>, std::vector<double>>> HydrogenRadials::generate_orb(
    const double charge,
    const bool with_slater_screening,
    const int nmax,
    const double dr,
...
std::map<std::pair<int, int>, std::pair<int, int>> HydrogenRadials::mapping_nl_lzeta(const int nmax,
                                                                                     const std::string strategy)
{
                               ...
                               std::ofstream* ptr_log)
{
    ...
    for (auto orbital: orbitals)
    {
        ...
        chi_[index(l, lzeta)]
            .build(l, true, rgrid.size(), rgrid.data(), rvalue.data(), 0, lzeta, symbol_, itype_, false);
        chi_[index(l, lzeta)].normalize();
        //++ichi;
    }
}

double HydrogenRadials::slater_screening(const std::string symbol, const int n, const int l)
{
    atom_in atom_db;
    ...
    if (symbol == "H")
        return 0.0; // only one 1s electron, no screening by "other electrons"
    else if (symbol == "He")
        return 0.30; // only two 1s electrons, one screening the other
    else if (n == 1)
        return 0.30;
    for (int n_ = 1; n_ <= n; ++n_)
    {
