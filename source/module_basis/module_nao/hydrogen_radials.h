...
/*
    ...
    5. for each n, l pair, generate_hydrogen_radial_toconv() is called to generate the radial function for a given n, l
   pair.
    6. note that the "toconv" means each radial function should have a norm converged to 1 at a certain radius, this is
       also controlled by a convergence threshold (conv_thr), now set to 1e-6. Therefore each radial function will have
   different rmax.
    7. if the norm of radial function is not converged to 1 at the given radius, the radius will be increased by dr, and
   the radial function will be recalculated until the norm is converged to 1.
    8. the radial function is calculated by generate_hydrogen_radial_segment(), which is a wrapper of assoc_laguerre_.
    9. assoc_laguerre_ is a class that can calculate the radial function of hydrogen-like atom, with the help of
   associated Laguerre polynomials.
    10. the radial function is calculated from 0.0 to a radius where the norm of radial function is converged, and the
   radial function is stored in a pair of vectors, first vector is the radial grid, second vector is the radial
   function.
    11. after the first generation step, all radial functions are zero-padded to the same length, and stored in a map,
   with the key being the n, l pair, and the value being the pair of vectors.
    12. to store the radial functions, they must indiced by the l, zeta pairs, so mapping_nl_lzeta() is called to map
   the n, l pairs to the l, zeta pairs.
    13. finally, the radial functions are stored in NumericalRadials, with the key being the l, zeta pairs, and the
   value being the pair of vectors.

    User capable settings:
    1. charge of the nucleus charge according to pseudopotential, it is also a rescale of radius. Higher charge result
   in smaller radius.
    2. generation strategy. minimal will only generate 1 orbital per angular momentum, double will generate 2 orbitals
   per angular momentum, full will generate all orbitals per angular momentum up to nmax. Note: nmax is read from
   atom_database.
    3. conv_thr, the convergence threshold of the norm of radial function, if not reached, will continue to increase the
   radius. user use this to control the accuracy of radial function. Too large conv_thr will result in inaccurate
   spherical Bessel transformation results (in FFT-two_center_integrator)
*/
class HydrogenRadials : public RadialSet
{
  public:
    ...
    {
        return new HydrogenRadials(*this);
    ...
    /// @brief generate hydrogen-like radial functions for a given n, l, from 0.0 to a radius where the norm of radial
    /// function is converged
    /// @param charge charge of the nucleus
    /// @param n principal quantum number
    /// @param l angular momentum quantum number
    /// @param converge_threshold the threshold of norm of radial function, if not reached, will continue to increase
    /// the radius
    /// @param rank MPI rank
    ...
    /// @param strategy strategy string
    /// @param ptr_log pointer to the log ofstream
    std::map<std::pair<int, int>, std::pair<std::vector<double>, std::vector<double>>> generate_orb(
        const double charge = 1.0,
        const bool with_slater_screening = false,
        const int nmax = 0,
        const double dr = 0.01,
    ...
    std::map<std::pair<int, int>, std::pair<int, int>> mapping_nl_lzeta(const int nmax = 0,
                                                                        const std::string strategy = "minimal-valence");
    ...
    /// @note hoping to get a more accurate estimation of hydrogen-like atom radial function, by including many-electron
    /// effect in this way
    /// @details algorithm:
