...
template <typename T>
void PW_Basis::collect(const ModulePW::PW_Basis* pw_rho, T* in, T* out, const int irank)
{
    const int npwtot = pw_rho->npwtot;
    delete[] out;
    if(irank == 0)
    {
        out = new T[npwtot];
...
#ifdef __MPI
    // collect the data from all the cores
    const int npw = pw_rho->npw; // # of planewaves on current core
    for(int ig = 0; ig < npw; ++ig)
    {
        int isz = pw_rho->ig2isz[ig]; // map: from the local pw to find the stick
        int iz = isz % pw_rho->nz; // z of pw in the stick
        int is = isz / pw_rho->nz; // index of the stick
        int ixy = pw_rho->is2fftixy[is]; // map: from the stick to find the x-y coordinate
        int ixyz = iz + pw_rho->nz * ixy;
        if(pw_rho->fftixy2ip[ixy] == irank) // if the stick is on current core, otherwise -1
        {
            out[ixyz] = in[ig];
        }
    }
#else
    for(int ig = 0; ig < npwtot; ++ig)
    {
        out[ig] = in[ig];
    }
#endif
...
template <typename T>
void PW_Basis::collect(const ModulePW::PW_Basis* pw_rho, std::complex<T>* in, std::complex<T>* out, const int irank)
{
    const int npwtot = pw_rho->npwtot;
    delete[] out;
    if(irank == 0)
