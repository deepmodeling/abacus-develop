...
void
    Cal_MLKEDF_Descriptors::set_para (const int& nx,
                                      const double& nelec,
    ...
    for (int ik = 0; ik < this->nkernel; ++ik)
        {
            ...
            if (this->kernel_type[ik] == 3 || this->kernel_type[ik] == 4) // 3 for TKK Al, and 4 for TKK Si
                {
                    this->read_kernel (this->kernel_file[ik],
                                       this->kernel_scaling[ik],
                                       pw_rho,
                                       this->kernel[ik].data ());
                }
            else
...
double
    Cal_MLKEDF_Descriptors::MLkernel (double eta, double tf_weight, double vw_weight)
{
    if (eta < 0.)
        {
            return 0.;
...
double
    Cal_MLKEDF_Descriptors::MLkernel_yukawa (double eta, double alpha)
{
...
void
    Cal_MLKEDF_Descriptors::read_kernel (const std::string& fileName,
                                         ...
                                         double* kernel_)
{
    ...
    if (!ifs)
        {
            ModuleBase::WARNING_QUIT ("cal_mlkedf_descriptors.cpp", "The kernel file not found");
        }
    ...
    if (maxEta > maxEta_in)
        {
            ModuleBase::WARNING ("cal_mlkedf_descriptors.cpp",
                                 "Please increase the maximal eta value in KEDF kernel file");
        }
...
void
    Cal_MLKEDF_Descriptors::multiKernel (const int ikernel,
                                         double* pinput,
                                         const ModulePW::PW_Basis* pw_rho,
                                         double* routput)
{
...
void
    Cal_MLKEDF_Descriptors::Laplacian (double* pinput, const ModulePW::PW_Basis* pw_rho, double* routput)
{
...
void
    Cal_MLKEDF_Descriptors::divergence (double** pinput, const ModulePW::PW_Basis* pw_rho, double* routput)
{
    std::complex<double>* recipContainer = new std::complex<double>[pw_rho->npw];
...
void
    Cal_MLKEDF_Descriptors::tanh (std::vector<double>& pinput, std::vector<double>& routput, double chi)
{
...
double
    Cal_MLKEDF_Descriptors::dtanh (double tanhx, double chi)
{
    return (1. - tanhx * tanhx) * chi;
}

void
    Cal_MLKEDF_Descriptors::getGamma (const double* const* prho, std::vector<double>& rgamma)
{
...
void
    Cal_MLKEDF_Descriptors::getP (const double* const* prho,
                                  const ModulePW::PW_Basis* pw_rho,
...
void
    Cal_MLKEDF_Descriptors::getQ (const double* const* prho, const ModulePW::PW_Basis* pw_rho, std::vector<double>& rq)
{
    // get Laplacian rho
...
void
    Cal_MLKEDF_Descriptors::getGammanl (const int ikernel,
...
void
    Cal_MLKEDF_Descriptors::getPnl (const int ikernel,
...
void
    Cal_MLKEDF_Descriptors::getQnl (const int ikernel,
...
// xi = gammanl/gamma
void
    Cal_MLKEDF_Descriptors::getXi (std::vector<double>& pgamma, std::vector<double>& pgammanl, std::vector<double>& rxi)
{
    for (int ir = 0; ir < this->nx; ++ir)
        {
            if (pgamma[ir] == 0)
...
void
    Cal_MLKEDF_Descriptors::getTanhXi (const int ikernel,
                                       std::vector<double>& pgamma,
...
// tanh(p)
void
    Cal_MLKEDF_Descriptors::getTanhP (std::vector<double>& pp, std::vector<double>& rtanhp)
{
...
// tanh(q)
void
    Cal_MLKEDF_Descriptors::getTanhQ (std::vector<double>& pq, std::vector<double>& rtanhq)
{
...
// tanh(pnl)
void
    Cal_MLKEDF_Descriptors::getTanh_Pnl (const int ikernel, std::vector<double>& ppnl, std::vector<double>& rtanh_pnl)
{
...
// tanh(qnl)
void
    Cal_MLKEDF_Descriptors::getTanh_Qnl (const int ikernel, std::vector<double>& pqnl, std::vector<double>& rtanh_qnl)
{
...
// tanh(p)_nl
void
    Cal_MLKEDF_Descriptors::getTanhP_nl (const int ikernel,
...
// tanh(q)_nl
void
    Cal_MLKEDF_Descriptors::getTanhQ_nl (const int ikernel,
...
// (tanhxi)_nl
void
    Cal_MLKEDF_Descriptors::getTanhXi_nl (const int ikernel,
...
void
    Cal_MLKEDF_Descriptors::getF_KS (psi::Psi<std::complex<double>>* psi,
                    ...
                    if (ik == 0 && ibnd == 0)
                        {
                            ...
                            const long unsigned cshape[]
                                = {(long unsigned)this->nx}; // shape of container and containernl
                        }
                    ...
                    for (int j = 0; j < 3; ++j)
                        {
                            ...
                            for (int ig = 0; ig < npw; ig++)
                                {
                                    double fact = pw_psi->getgpluskcar (ik, ig)[j] * ucell.tpiba;
                            ...
                            for (int ir = 0; ir < this->nx; ++ir)
                                {
                                    pauliED[ir]
                                        += w1 * norm (wfcr[ir]); // actually, here should be w1/2 * norm(wfcr[ir]), but
                                                                 // we multiply 2 to convert Ha to Ry.
                                }
                        }
                }
        }
    ...
    for (int j = 0; j < 3; ++j)
        {
            for (int ir = 0; ir < this->nx; ++ir)
                {
                    pauliED[ir] -= nablaRho[j][ir] * nablaRho[j][ir] / (8. * pelec->charge->rho[0][ir])
                                   * 2.; // convert Ha to Ry.
                }
        }
...
void
    Cal_MLKEDF_Descriptors::getNablaRho (const double* const* prho,
                                         const ModulePW::PW_Basis* pw_rho,
