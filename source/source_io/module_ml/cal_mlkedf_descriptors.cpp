...
void Cal_MLKEDF_Descriptors::set_para(const int& nx,
                                      const double& nelec,
...
void Cal_MLKEDF_Descriptors::read_kernel(const std::string& fileName,
    ...
    if (maxEta > maxEta_in)
        ModuleBase::WARNING("cal_mlkedf_descriptors.cpp", "Please increase the maximal eta value in KEDF kernel file");
...
void Cal_MLKEDF_Descriptors::multiKernel(const int ikernel,
                                         double* pinput,
                                         const ModulePW::PW_Basis* pw_rho,
                                         double* routput) {
...
double Cal_MLKEDF_Descriptors::dtanh(double tanhx, double chi) { return (1. - tanhx * tanhx) * chi; }
...
void Cal_MLKEDF_Descriptors::getP(const double* const* prho,
                                  const ModulePW::PW_Basis* pw_rho,
...
void Cal_MLKEDF_Descriptors::getQ(const double* const* prho,
                                  const ModulePW::PW_Basis* pw_rho,
...
void Cal_MLKEDF_Descriptors::getGammanl(const int ikernel,
...
void Cal_MLKEDF_Descriptors::getPnl(const int ikernel,
...
void Cal_MLKEDF_Descriptors::getQnl(const int ikernel,
...
// xi = gammanl/gamma
void Cal_MLKEDF_Descriptors::getXi(std::vector<double>& pgamma,
...
void Cal_MLKEDF_Descriptors::getTanhXi(const int ikernel,
                                       std::vector<double>& pgamma,
                                       std::vector<double>& pgammanl,
...
// tanh(p)_nl
void Cal_MLKEDF_Descriptors::getTanhP_nl(const int ikernel,
...
// tanh(q)_nl
void Cal_MLKEDF_Descriptors::getTanhQ_nl(const int ikernel,
...
// (tanhxi)_nl
void Cal_MLKEDF_Descriptors::getTanhXi_nl(const int ikernel,
...
void Cal_MLKEDF_Descriptors::getF_KS(psi::Psi<std::complex<double>>* psi,
    ...
    for (int ik = 0; ik < psi->get_nk(); ++ik) {
        ...
        for (int ibnd = 0; ibnd < nbands; ++ibnd) {
            ...
            for (int j = 0; j < 3; ++j) {
                ...
                for (int ir = 0; ir < this->nx; ++ir) {
                    pauliED[ir] += w1 * norm(wfcr[ir]); // actually, here should be w1/2 * norm(wfcr[ir]), but we
                                                        // multiply 2 to convert Ha to Ry.
                }
            }
        }
    }
    ...
    for (int j = 0; j < 3; ++j) {
        for (int ir = 0; ir < this->nx; ++ir) {
            pauliED[ir] -=
                nablaRho[j][ir] * nablaRho[j][ir] / (8. * pelec->charge->rho[0][ir]) * 2.; // convert Ha to Ry.
        }
    }
...
void Cal_MLKEDF_Descriptors::getNablaRho(const double* const* prho,
                                         const ModulePW::PW_Basis* pw_rho,
