...
void
    wfc_nao_write2file (const std::string& name,
                        const double* ctot,
                        const int nlocal,
                        const int ik,
...
void
    wfc_nao_write2file_complex (const std::string& name,
    ...
    if (writeBinary)
        {
            ...
            if (!ofs)
                {
                    ModuleBase::WARNING ("ModuleIO::wfc_nao_write2file_complex",
                                         "Can't write local orbital wave functions.");
                }
            ofs << ik + 1;
            ofs << kvec_c.x;
            ofs << kvec_c.y;
...
template <typename T>
void
    write_wfc_nao (const int out_type,
            ...
            if (myid == 0)
                {
                    ...
                    std::string fn = filename_output (wfc_dir,
                                                      "wf",
                                                      "nao",
                                                      ik,
                                                      ik2iktot,
