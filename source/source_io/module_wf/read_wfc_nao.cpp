...
void
    ModuleIO::read_wfc_nao_one_data (std::ifstream& ifs, float& data)
{
    ifs >> data;
}

void
    ModuleIO::read_wfc_nao_one_data (std::ifstream& ifs, double& data)
{
    ifs >> data;
}

void
    ModuleIO::read_wfc_nao_one_data (std::ifstream& ifs, std::complex<double>& data)
{
    double a = 0.0;
    double b = 0.0;
    ifs >> a >> b;
...
void
    ModuleIO::read_wfc_nao_one_data (std::ifstream& ifs, std::complex<float>& data)
{
    float a = 0.0;
    float b = 0.0;
    ifs >> a >> b;
...
template <typename T>
bool
    ModuleIO::read_wfc_nao (const std::string& global_readin_dir,
                            ...
                            const int istep)
{
    ...
    auto read_one_file
        = [&] (const std::string& ss, std::stringstream& error_message, const int ik, std::vector<T>& ctot)
        {
            std::ifstream ifs;
            ifs.open (ss.c_str ());
            if (!ifs)
            ...
            if (!gamma_only)
                {
                    ...
                    if (ik_file != ik + 1)
                        {
                            error_message
                                << "The k index read in from file do not match the k index generated by ABACUS!\n";
            ...
            if (nbands > nbands_file)
                {
                    error_message << "The number of bands to be read exceeds the number of bands in the file generated "
                                     "by ABACUS!\n";
            ...
            for (int i = 0; i < skip_band + nbands; i++)
                {
                    ...
                    if (i + 1 != ib)
                        {
                            error_message
                                << "The band index read in from file do not match the global parameter band index!\n";
            ...
            if (myrank == 0)
                {
                    ...
                    std::string ss = ModuleIO::filename_output (readin_dir,
                                                                "wf",
                                                                "nao",
                                                                ik,
                                                                ik2iktot,
