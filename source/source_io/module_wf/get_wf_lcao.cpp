...
// For gamma_only
void
    Get_wf_lcao::begin (const UnitCell& ucell,
                        ...
                        std::ofstream& ofs_running)
{
    ...
    for (int is = 0; is < nspin; ++is)
        {
            psid->fix_k (is);
            ModuleGint::Gint_env_gamma gint_env (psid->get_pointer (),
                                                 &para_orb,
                                                 nbands,
                                                 nlocal,
                                                 pes_->charge->rho[is]);
                    ...
                    if (bands_picked_[ib])
                        {
                            ...
                            std::stringstream ss_info;
                            ss_info << "Wave func. " << ib + 1 << " spin " << ispin + 1 << " k-point " << ik0 + 1
                                    << " saved in";
                    ...
                    for (int ik = 0; ik < nks; ++ik)
                        {
                            ...
                            // Output real part
                            std::stringstream ss_real;
                            ss_real << global_out_dir << "wfi" << ib + 1 << "s" << ispin + 1 << "k" << ik0 + 1
                                    << "re.cube";
                            ...
                            // Output imaginary part
                            std::stringstream ss_imag;
                            ss_imag << global_out_dir << "wfi" << ib + 1 << "s" << ispin + 1 << "k" << ik0 + 1
                                    << "im.cube";
                            ModuleIO::write_vdata_palgrid (pgrid,
                                                           wfc_imag.data (),
                                                           ispin,
...
void
    Get_wf_lcao::select_bands (const std::vector<int>& out_wfc_kb, const int nbands, const int fermi_band)
{
    ...
    for (int value: out_wfc_kb)
        {
            if (value != 0 && value != 1)
                {
                    ModuleBase::WARNING_QUIT ("Get_wf_lcao::select_bands",
                                              "The elements of `out_wfc_norm` or `out_wfc_re_im` must be either 0 or "
...
// for each band
void
    Get_wf_lcao::set_pw_wfc (const ModulePW::PW_Basis_K* pw_wfc,
                             const int& ik,
                             const int& ib,
                             const int& nspin,
...
template <typename T>
int
    Get_wf_lcao::set_wfc_grid (const int naroc[2],
                               const int nb,
                               const int dim0,
                               const int dim1,
                               const int iprow,
...
template <typename T>
void
    Get_wf_lcao::wfc_2d_to_grid (const T* lowf_2d,
...
void
    Get_wf_lcao::prepare_get_wf (std::ofstream& ofs_running)
{
    ofs_running << "\n\n";
...
int
    Get_wf_lcao::globalIndex (int localindex, int nblk, int nprocs, int myproc) const
{
...
int
    Get_wf_lcao::localIndex (int globalindex, int nblk, int nprocs, int& myproc) const
{
