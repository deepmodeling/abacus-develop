...
void
    ModuleIO::read_wfc_pw (const std::string& filename,
    ...
    if (rank_in_pool == 0)
        {
            if (filetype == "txt")
                ...
                {
                    rfs >> size >> ikstot_in >> nkstot_in >> kvec[0] >> kvec[1] >> kvec[2] >> weight >> npwtot_in
                        >> nbands_in >> ecutwfc_in >> lat0_in >> tpiba_in >> size;
                }
        }
    ...
    if (rank_in_pool == 0)
        {
            if (filetype == "txt")
                ...
                {
                    rfs >> size >> G_in[0].x >> G_in[0].y >> G_in[0].z >> G_in[1].x >> G_in[1].y >> G_in[1].z
                        >> G_in[2].x >> G_in[2].y >> G_in[2].z >> size;
                }
        }
    ...
    if (rank_in_pool == 0)
        {
            ...
            if (nx <= 0 || ny <= 0 || nz <= 0 || nxyz_sz > static_cast<size_t> (std::numeric_limits<int>::max ()))
                {
