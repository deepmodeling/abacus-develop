    ...
    ReadInput::item_deepks ()
{
    ...
    // Please preserve this ordering when adding new parameters.
    {
        ...
        item.type = "Integer";
        item.description
            = R"(Print labels and descriptors for DeePKS in OUT.${suffix}. The names of these files start with "deepks".
* 0 : No output.
...
[NOTE] When deepks_out_labels equals 1, the path of a numerical descriptor (an orb file) is needed to be specified under the NUMERICAL_DESCRIPTOR tag in the STRU file. This is not needed when deepks_out_labels equals 2.)";
    ...
    }
    {
        ...
        item.category = "DeePKS";
        item.type = "Integer";
        item.description = "When deepks_out_freq_elec is greater than 0, print labels and descriptors for DeePKS in "
                           "OUT.${suffix}/DeePKS_Labels_Elec per deepks_out_freq_elec electronic iterations, with "
    ...
    }
    {
        ...
        item.type = "String";
        item.description
            = "Print labels and descriptors calculated by base functional ( determined by deepks_out_base ) and target "
              "functional ( determined by dft_functional ) for DeePKS in per deepks_out_freq_elec electronic "
              "iterations. The SCF process, labels and descriptors output of the target functional are all consistent "
              "with those when the target functional is used alone. The only additional output under this "
              "configuration is the labels of the base functional. Often used with deepks_out_labels equals 1.";
        ...
        item.check_value = [] (const Input_Item& item, const Parameter& para)
            {
                if (para.input.deepks_out_base != "none" && para.input.deepks_out_labels == 0)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput",
                                                  "to use deepks_out_base, please set deepks_out_labels > 0 ");
                    }
                if (para.input.deepks_out_base != "none" && para.input.deepks_bandgap > 0)
                    {
                        ModuleBase::WARNING_QUIT (
                            "ReadInput",
                            "outputting bandgap labels during electronic steps is not implemented yet ");
                    }
            };
        this->add_item (item);
    }
    ...
    }
    {
        ...
        item.type = "Integer";
        item.description
            = "the maximum angular momentum of the Bessel functions generated as the projectors in DeePKS - NOte: To "
              "generate such projectors, set calculation type to gen_bessel in ABACUS. See also calculation.";
...
* 3: Used for systems containing H atoms. Here HOMO is defined as the max occupation except H atoms and the bandgap label is the energy between HOMO and (HOMO + 1))";
    ...
    }
    {
        ...
        item.type = "Integer*2";
        item.description
            = R"(The first value should not be larger than the second one and the meaning differs in different cases below
...
* deepks_bandgap is 2: Bandgap labels are energies between HOMO and all states in range [LUMO + deepks_band_range[0], LUMO + deepks_band_range[1]] (Thus there are deepks_band_range[1] - deepks_band_range[0] + 1 bandgaps in total). If HOMO is included in the setting range, it will be ignored since it will always be zero and has no valuable messages (deepks_band_range[1] - deepks_band_range[0] bandgaps in this case). NOTICE: The set range can be greater than, less than, or include the value of HOMO. In the bandgap label, we always calculate the energy of the state in the set range minus the energy of HOMO state, so the bandgap can be negative if the state is lower than HOMO.)";
                ...
                if (para.input.deepks_bandgap == 1)
                    {
                        if (para.input.deepks_band_range[0] >= para.input.deepks_band_range[1])
                            {
                                ModuleBase::WARNING_QUIT ("ReadInput",
                                                          "deepks_band_range[0] must be smaller than "
                                                          "deepks_band_range[1] for deepks_bandgap = 1.");
                            }
                    }
                else if (para.input.deepks_bandgap == 2)
                    {
                        if (para.input.deepks_band_range[0] > para.input.deepks_band_range[1])
                            {
                                ModuleBase::WARNING_QUIT ("ReadInput",
                                                          "deepks_band_range[0] must be no more than "
                                                          "deepks_band_range[1] for deepks_bandgap = 2.");
                            }
                    }
                else
                    {
                        if (para.input.deepks_band_range[0] != -1 || para.input.deepks_band_range[1] != 0)
                            {
                                ModuleBase::WARNING ("ReadInput",
                                                     "deepks_band_range is used for deepks_bandgap = 1/2. Ignore its "
    ...
    }
    {
        ...
        item.annotation = "!=0 for v_delta/v_delta_R label. when output, 1 for vdpre, 2 for phialpha and grad_evdm "
                          "(can save memory )"
                          " -1 for vdrpre, -2 for phialpha_r and gevdm (can save memory)";
        item.category = "DeePKS";
        item.type = "Integer";
        item.description
            = R"(Include V_delta/V_delta_R (Hamiltonian in k/real space) label for DeePKS training. When deepks_out_labels is true and deepks_v_delta > 0 (k space), ABACUS will output deepks_hbase.npy, deepks_vdelta.npy and deepks_htot.npy(htot=hbase+vdelta). When deepks_out_labels is true and deepks_v_delta < 0 (real space), ABACUS will output deepks_hrtot.csr, deepks_hrdelta.csr. Some more files output for different settings. NOTICE: To match the unit Normally used in DeePKS, the unit of Hamiltonian in k space is Hartree. However, currently in R space the unit is still Ry.
