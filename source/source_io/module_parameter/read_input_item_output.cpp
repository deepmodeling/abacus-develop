...
[NOTE] In the 3.10-LTS version, the file names are data-0-H and data-0-S, etc.)";
    ...
    }
    {
        ...
        item.type = R"(Boolean \[Integer\](optional))";
        item.description =
            "Whether to print the matrix representation of the position matrix into files named rxrs1_nao.csr, "
            "ryrs1_nao.csr, rzrs1_nao.csr in the directory OUT.${suffix}. If calculation is set to get_s, the position "
            "matrix can be obtained without scf iterations. For more information, please refer to position_matrix.md."
    ...
    }
    {
        ...
        item.type = "Integer";
        item.description =
            "Whether to print files containing the derivatives of the Hamiltonian matrix. The format will be the same "
            "as the Hamiltonian matrix and overlap matrix as mentioned in out_mat_hs2. The name of the files will be "
            "dhrxs1_nao.csr, dhrys1_nao.csr, dhrzs1_nao.csr and so on. Also controled by out_freq_ion and out_app_flag."
    ...
    }
    {
        ...
        item.type = R"(Boolean \[Integer\](optional))";
        item.description =
            "Whether to print the exchange-correlation matrices in numerical orbital representation: in CSR format in "
            "the directory OUT.${suffix}. The name of the files will be vxcrs1_nao.csr and so on."
...
If EXX(exact exchange) is calculated (i.e. dft_fuctional==hse/hf/pbe0/scan0 or rpa==True), the Hexx(R) files for each processor will also be saved in the above folder, which can be read in EXX calculation with restart_load==True.)";
    ...
    }
    {
        ...
        item.type = "String";
        item.description =
            "Specifies the electronic states to calculate the real-space wave function modulus (norm, or known as the "
            "envelope function) with state index. The syntax and state selection rules are identical to out_pchg, but "
            "the output is the norm of the wave function. The outputs comprise multiple .cube files following the "
            "naming convention wfi[state]s[spin]k[kpoint].cube.";
        item.default_value = "none";
        item.unit = "";
    ...
    }
    {
        ...
        item.type = "String";
        item.description =
            "Specifies the electronic states to calculate the real and imaginary parts of the wave function with state "
            "index. The syntax and state selection rules are identical to out_pchg, but the output contains both the "
            "real and imaginary components of the wave function. The outputs comprise multiple .cube files following "
            "the naming convention wfi[state]s[spin]k[kpoint][re/im].cube.";
        item.default_value = "none";
        item.unit = "";
        item.availability = "For both PW and LCAO. When basis_type = lcao, used when calculation = get_wf.";
        item.read_value = [](const Input_Item& item, Parameter& para) {
    ...
    }
    {
        ...
        item.description =
            ...
            "them. Warning: Enabling symmetry may produce unwanted results due to reduced k-point weights and symmetry "
            "operations in real space. Therefore when calculating partial charge densities, if you are not sure what "
            "you want exactly, it is strongly recommended to set symmetry = -1. It is noteworthy that your symmetry "
            "setting should remain the same as that in the SCF procedure.";
        item.default_value = "false";
        item.unit = "";
...
* False: Do not output vector potential.)";
    ...
    }
    {
        ...
        item.type = R"(Integer \[Integer\](optional))";
        item.description =
            "Whether to print the matrix representation of symmetry operation to running log file. If the first value "
            "is given as 1, then all matrix representations will be printed. The second optional parameter controls "
