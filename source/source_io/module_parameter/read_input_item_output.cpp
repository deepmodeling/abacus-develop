    ...
    ReadInput::item_output ()
{
    ...
    // Please preserve this ordering when adding new parameters.
    {
        ...
        item.type = "Integer";
        item.description
            = "Controls the output interval in ionic steps. When set to a positive integer, information such as charge "
              "density, local potential, electrostatic potential, Hamiltonian matrix, overlap matrix, density matrix, "
              "and Mulliken population analysis is printed every n ionic steps."
              "\n\n[NOTE] In RT-TDDFT calculations, this parameter is inactive; output frequency is instead controlled "
              "by out_freq_td.";
    ...
    }
    {
        ...
        item.type = "Integer";
        item.description
            = "Controls the output interval in completed electronic evolution steps during RT-TDDFT calculations. When "
              "set to a positive integer n, detailed information (see out_freq_ion) is printed every n electron "
              "time-evolution steps (i.e., every STEP OF ELECTRON EVOLVE). For example, if you wish to output "
              "information once per ionic step, you should set out_freq_td equal to estep_per_md, since one ionic step "
              "corresponds to estep_per_md electronic evolution steps."
    ...
    }
    {
        ...
        item.category = "Output information";
        item.type = "Integer";
        item.description = "Output the charge density (only binary format, controlled by out_chg), wavefunction "
                           "(controlled by out_wfc_pw) per out_freq_elec electronic iterations. Note that they are "
                           "always output when converged or reach the maximum iterations scf_nmax.";
...
[NOTE] In the 3.10-LTS version, the file names are SPIN1_CHG.cube and SPIN1_CHG_INI.cube, etc.)";
        ...
        item.read_value = [] (const Input_Item& item, Parameter& para)
            {
                ...
                if (count < 1)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput", "out_chg needs at least 1 value");
                    }
                ...
                if (count >= 2)
                    {
                        try
                            {
                                para.input.out_chg[1] = std::stoi (item.str_values[1]);
                            }
    ...
    }
    {
        ...
        item.type = R"(Integer \[Integer\](optional))";
        item.description
            = R"(* 1: Output the total local potential (i.e., local pseudopotential + Hartree potential + XC potential + external electric field (if exists) + dipole correction potential (if exists) + ...) on real space grids (in Ry) into files in the folder OUT.{suffix}. The files are named as:
 * nspin = 1: pots1.cube;
...
[NOTE] In the 3.10-LTS version, the file names are SPIN1_POT.cube and SPIN1_POT_INI.cube, etc.)";
        ...
        item.read_value = [] (const Input_Item& item, Parameter& para)
            {
                ...
                if (count < 1)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput", "out_pot needs at least 1 value");
                    }
    ...
    }
    {
        ...
        item.description
            = R"(Whether to output the density matrix for each k-point into files in the folder OUT.${suffix}. For current develop versions, out_dmk writes *_nao.txt files and includes a g{istep} index in the file name:
    * For gamma only case:
        ...
        item.read_value = [] (const Input_Item& item, Parameter& para)
            {
                ...
                if (count < 1)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput", "out_dmk needs at least 1 value");
                    }
                ...
                if (count >= 2)
                    {
                        try
                            {
                                para.input.out_dmk[1] = std::stoi (item.str_values[1]);
                            }
                        catch (const std::invalid_argument&)
                            { /* do nothing */
                            }
        ...
        item.read_value = [] (const Input_Item& item, Parameter& para)
            {
                ...
                if (count < 1)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput", "out_dmr needs at least 1 value");
                    }
                ...
                if (count >= 2)
                    {
                        try
                            {
                                para.input.out_dmr[1] = std::stoi (item.str_values[1]);
                            }
                        catch (const std::invalid_argument&)
                            { /* do nothing */
                            }
...
[NOTE] In the 3.10-LTS version, the file names are WAVEFUNC1.dat, WAVEFUNC2.dat, etc.)";
        ...
        item.unit = "";
        item.availability
            = "Output electronic wave functions in plane wave basis, or transform the real-space electronic wave "
              "function into plane wave basis (see get_wf option in calculation with NAO basis)";
...
* 3: output the Fermi surface file (fermi.bxsf) in BXSF format that can be visualized by XCrySDen)";
    ...
    }
    {
        ...
        item.type = R"(Integer \[Integer\](optional))";
        item.description
            = R"(Whether to output the local density of states (LDOS), optionally output precision can be set by a second parameter, default is 3.
* 0: no output
...
* 3: output both two LDOS modes above.)";
    ...
    }
    {
        ...
        item.type = R"(Boolean \[Integer\](optional))";
        item.description
            = R"(Whether to output the eigenvalues of the Hamiltonian matrix (in eV) into the running log during electronic iterations and into a file at the end of calculations. The former can be used with the 'out_freq_elec' parameter while the latter option allows the output precision to be set via a second parameter, with a default value of 8. The output file names are:
 ...
 * For more information, refer to the band.md)";
    ...
    }
    {
        ...
        item.category = "Output information";
        item.type = "Boolean";
        item.description = "Whether to output structure files per ionic step in geometry relaxation calculations into "
                           "OUT.{istep}_D, where ${istep} is the ionic step.";
        item.default_value = "False";
        item.unit = "";
        item.availability = "";
        item.reset_value = [] (const Input_Item& item, Parameter& para)
...
* m: molecular dynamics level, which does not print some information for simplicity.)";
    ...
    }
    {
        ...
        item.type = R"(Boolean \[Integer\](optional))";
        item.description
            = R"(Whether to print the upper triangular part of the Hamiltonian matrices and overlap matrices for each k-point into files in the directory OUT.${suffix}. The second number controls precision. For more information, please refer to hs_matrix.md. Also controled by out_freq_ion and out_app_flag.
* For gamma only case:
...
[NOTE] In the 3.10-LTS version, the file names are data-0-H and data-0-S, etc.)";
        ...
        item.read_value = [] (const Input_Item& item, Parameter& para)
            {
                ...
                if (count < 1)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput", "out_mat_hs needs at least 1 value");
                    }
    ...
    }
    {
        ...
        item.category = "Output information";
        item.type = R"(Boolean \[Integer\](optional))";
        item.description = "Whether to print files containing the Hamiltonian matrix and overlap matrix into files in "
                           "the directory OUT.${suffix}. For more information, please refer to hs_matrix.md."
        ...
        item.read_value = [] (const Input_Item& item, Parameter& para)
            {
                ...
                if (count < 1)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput", "out_mat_hs2 needs at least 1 value");
                    }
                ...
                if (count >= 2)
                    {
                        try
                            {
                                para.input.out_mat_hs2[1] = std::stoi (item.str_values[1]);
                            }
                        catch (const std::invalid_argument&)
                            { /* do nothing */
                            }
        ...
        item.check_value = [] (const Input_Item& item, const Parameter& para)
            {
                if (para.input.out_mat_r[0] && para.sys.gamma_only_local)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput",
                                                  "out_mat_r is not available for gamma only calculations");
                    }
            };
    ...
    }
    {
        ...
        item.category = "Output information";
        item.type = R"(Boolean \[Integer\](optional))";
        item.description = "Whether to print the upper triangular part of the kinetic matrices for each k-point into "
                           "OUT.${suffix}/tks1ki_nao.txt, where i is the index of k points. One may optionally provide "
                           "a second parameter to specify the precision."
        ...
        item.read_value = [] (const Input_Item& item, Parameter& para)
            {
                ...
                if (count < 1)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput", "out_mat_tk needs at least 1 value");
                    }
    ...
    }
    {
        ...
        item.category = "Output information";
        item.type = R"(Boolean \[Integer\](optional))";
        item.description = "Whether to print the matrix representation of the position matrix into files named "
                           "rxrs1_nao.csr, ryrs1_nao.csr, rzrs1_nao.csr in the directory OUT.${suffix}. If calculation "
                           "is set to get_s, the position matrix can be obtained without scf iterations. For more "
                           "information, please refer to position_matrix.md."
        ...
        item.read_value = [] (const Input_Item& item, Parameter& para)
            {
                ...
                if (count < 1)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput", "out_mat_r needs at least 1 value");
                    }
                try
                    {
                        ...
                        if (count >= 2)
                            {
                                try
                                    {
                                        para.input.out_mat_r[1] = std::stoi (item.str_values[1]);
                                    }
                                catch (const std::invalid_argument& e)
                                    {
                                        ModuleBase::WARNING ("Input",
                                                             "out_mat_r precision must be an integer, using default 8");
                                    }
                            }
                    }
                catch (const std::invalid_argument& e)
    ...
    }
    {
        ...
        item.category = "Output information";
        item.type = R"(Boolean \[Integer\](optional))";
        item.description = "Generate files containing the kinetic energy matrix. The format will be the same as the "
                           "Hamiltonian matrix and overlap matrix as mentioned in out_mat_hs2. The name of the files "
        ...
        item.read_value = [] (const Input_Item& item, Parameter& para)
            {
                ...
                if (count < 1)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput", "out_mat_t needs at least 1 value");
                    }
                try
                    {
                        ...
                        if (count >= 2)
                            {
                                try
                                    {
                                        para.input.out_mat_t[1] = std::stoi (item.str_values[1]);
                                    }
                                catch (const std::invalid_argument& e)
                                    {
                                        ModuleBase::WARNING ("Input",
                                                             "out_mat_t precision must be an integer, using default 8");
                                    }
                            }
                    }
                catch (const std::invalid_argument& e)
    ...
    }
    {
        ...
        item.category = "Output information";
        item.type = "Integer";
        item.description = "Whether to print files containing the derivatives of the Hamiltonian matrix. The format "
                           "will be the same as the Hamiltonian matrix and overlap matrix as mentioned in out_mat_hs2. "
                           "The name of the files will be dhrxs1_nao.csr, dhrys1_nao.csr, dhrzs1_nao.csr and so on. "
                           "Also controled by out_freq_ion and out_app_flag."
        ...
        item.read_value = [] (const Input_Item& item, Parameter& para)
            {
                ...
                if (count < 1)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput", "out_mat_dh needs at least 1 value");
                    }
                try
                    {
                        ...
                        if (count >= 2)
                            {
                                try
                                    {
                                        para.input.out_mat_dh[1] = std::stoi (item.str_values[1]);
                                    }
                                catch (const std::invalid_argument& e)
                                    {
                                        ModuleBase::WARNING (
                                            "Input",
                                            "out_mat_dh precision must be an integer, using default 8");
                                    }
                            }
                    }
                catch (const std::invalid_argument& e)
    ...
    }
    {
        ...
        item.type = R"(Boolean \[Integer\](optional))";
        item.description
            = "Whether to print files containing the derivatives of the overlap matrix. The format will be the same as "
              "the overlap matrix as mentioned in out_mat_dh. The name of the files will be dsxrs1_nao.csr and so on. "
              "Also controled by out_freq_ion and out_app_flag. This feature can be used with calculation get_s."
        ...
        item.read_value = [] (const Input_Item& item, Parameter& para)
            {
                ...
                if (count < 1)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput", "out_mat_ds needs at least 1 value");
                    }
                try
                    {
                        ...
                        if (count >= 2)
                            {
                                try
                                    {
                                        para.input.out_mat_ds[1] = std::stoi (item.str_values[1]);
                                    }
                                catch (const std::invalid_argument& e)
                                    {
                                        ModuleBase::WARNING (
                                            "Input",
                                            "out_mat_ds precision must be an integer, using default 8");
                                    }
                            }
                    }
                catch (const std::invalid_argument& e)
    ...
    }
    {
        ...
        item.type = "Boolean";
        item.description
            = "Whether to print the upper triangular part of the exchange-correlation matrices in Kohn-Sham orbital "
              "representation: for each k point into files in the directory OUT.i_nao.txt, where {suffix}/vxc_out.dat. "
              "If EXX is calculated, the local and EXX part of band energy will also be printed in "
              "OUT.{suffix}/vxc_exx_out.dat, respectively. All the vxc_out.dat files contains 3 integers (nk, nspin, "
              "nband) followed by nk*nspin*nband lines of energy Hartree and eV."
    ...
    }
    {
        ...
        item.type = R"(Boolean \[Integer\](optional))";
        item.description
            = "Whether to print the exchange-correlation matrices in numerical orbital representation: in CSR format "
              "in the directory OUT.${suffix}. The name of the files will be vxcrs1_nao.csr and so on."
        ...
        item.read_value = [] (const Input_Item& item, Parameter& para)
            {
                ...
                if (count < 1)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput", "out_mat_xc2 needs at least 1 value");
                    }
                try
                    {
                        ...
                        if (count >= 2)
                            {
                                try
                                    {
                                        para.input.out_mat_xc2[1] = std::stoi (item.str_values[1]);
                                    }
                                catch (const std::invalid_argument& e)
                                    {
                                        ModuleBase::WARNING (
                                            "Input",
                                            "out_mat_xc2 precision must be an integer, using default 8");
                                    }
                            }
                    }
                catch (const std::invalid_argument& e)
    ...
    }
    {
        ...
        item.type = R"(Boolean \[Integer\](optional))";
        item.description
            = "Whether to print the expectation value of the angular momentum operator , , and in the basis of the "
              "localized atomic orbitals. The files are named OUT.{suffix}_Lx.dat, OUT.{suffix}_Ly.dat, and "
              "OUT.{suffix}_Lz.dat. The second integer controls the precision of the output.";
        item.default_value = "False 8";
        item.unit = "";
        item.availability = "Numerical atomic orbital (NAO) basis";
        item.read_value = [] (const Input_Item& item, Parameter& para)
            {
                ...
                if (count < 1)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput", "out_mat_l needs at least 1 value");
                    }
                ...
                if (count >= 2)
                    {
                        try
                            {
                                para.input.out_mat_l[1] = std::stoi (item.str_values[1]);
                            }
                        catch (const std::invalid_argument&)
                            { /* do nothing */
                            }
    ...
    }
    {
        ...
        item.type = R"(Integer \[Integer\](optional))";
        item.description
            = R"(The first integer controls whether to output the exchange-correlation (in Bohr^-3) on real space grids using Libxc to folder OUT.${suffix}:
...
The circle order of the charge density on real space grids is: x is the outer loop, then y and finally z (z is moving fastest).)";
    ...
    }
    {
        ...
        item.category = "Output information";
        item.type = "Boolean";
        item.description = "Whether to print the band energy terms separately in the file OUT.{term}_out.dat. The "
                           "terms include the kinetic, pseudopotential (local + nonlocal), Hartree and "
                           "exchange-correlation (including exact exchange if calculated).";
        item.default_value = "False";
        item.unit = "";
        item.availability = "Numerical atomic orbital basis";
    ...
    }
    {
        ...
        item.category = "Output information";
        item.type = "Boolean";
        item.description = "Whether to print the Mulliken population analysis result into OUT.${suffix}/mulliken.txt. "
                           "In molecular dynamics calculations, the output frequency is controlled by out_freq_ion.";
        item.default_value = "False";
        item.unit = "";
        item.availability = "Numerical atomic orbital basis";
    ...
    }
    {
        ...
        item.category = "Output information";
        item.type = "Boolean";
        item.description = "Whether to output r(R), H(R), S(R), T(R), dH(R), dS(R), and wfc matrices in an append "
                           "manner during molecular dynamics calculations. Check input parameters out_mat_r, "
    ...
    }
    {
        ...
        item.category = "Output information";
        item.type = "Integer";
        item.description = "Controls the length of decimal part of output data, such as charge density, Hamiltonian "
                           "matrix, Overlap matrix and so on.";
    ...
    }
    {
        ...
        item.category = "Output information";
        item.type = "Boolean";
        item.description = "Whether to print element information into files in the directory OUT.{element_label}, "
                           "including pseudopotential and orbital information of the element (in atomic Ryberg units).";
        item.default_value = "False";
        item.unit = "";
        item.availability = "";
    ...
    }
    {
        ...
        item.type = "Boolean";
        item.description
            = R"(Whether to save charge density files per ionic step, which are used to restart calculations. According to the value of read_file_dir:
...
If EXX(exact exchange) is calculated (i.e. dft_fuctional==hse/hf/pbe0/scan0 or rpa==True), the Hexx(R) files for each processor will also be saved in the above folder, which can be read in EXX calculation with restart_load==True.)";
    ...
    }
    {
        ...
        item.type = "String";
        item.description
            = "Specifies the electronic states to calculate the real-space wave function modulus (norm, or known as "
              "the envelope function) with state index. The syntax and state selection rules are identical to "
              "out_pchg, but the output is the norm of the wave function. The outputs comprise multiple .cube files "
              "following the naming convention wfi[state]s[spin]k[kpoint].cube.";
        item.default_value = "none";
        item.unit = "";
    ...
    }
    {
        ...
        item.type = "String";
        item.description
            = "Specifies the electronic states to calculate the real and imaginary parts of the wave function with "
              "state index. The syntax and state selection rules are identical to out_pchg, but the output contains "
              "both the real and imaginary components of the wave function. The outputs comprise multiple .cube files "
              "following the naming convention wfi[state]s[spin]k[kpoint][re/im].cube.";
        item.default_value = "none";
        item.unit = "";
        item.availability = "For both PW and LCAO. When basis_type = lcao, used when calculation = get_wf.";
        item.read_value = [] (const Input_Item& item, Parameter& para)
    ...
    }
    {
        ...
        item.type = "Boolean";
        item.description
            = "Specifies whether to write the partial charge densities for all k-points to individual files or merge "
              "them. Warning: Enabling symmetry may produce unwanted results due to reduced k-point weights and "
              "symmetry operations in real space. Therefore when calculating partial charge densities, if you are not "
              "sure what you want exactly, it is strongly recommended to set symmetry = -1. It is noteworthy that your "
              "symmetry setting should remain the same as that in the SCF procedure.";
        item.default_value = "false";
        item.unit = "";
        item.availability = "For both PW and LCAO. When basis_type = pw, used if out_pchg is set. When basis_type = "
                            "lcao, used only when calculation = get_pchg and gamma_only = 0.";
    ...
    }
    {
        ...
        item.type = R"(Integer \[Integer\](optional))";
        item.description
            = R"(Whether to output the electron localization function (ELF) in the folder `OUT.${suffix}`. The files are named as
* nspin = 1:
...
In molecular dynamics calculations, the output frequency is controlled by out_freq_ion.)";
        ...
        item.check_value = [] (const Input_Item& item, const Parameter& para)
            {
                if (para.input.out_elf[0] > 0 && para.input.esolver_type != "ksdft"
                    && para.input.esolver_type != "ofdft")
                    {
    ...
    }
    {
        ...
        item.type = "Integer";
        item.description
            = "This output is only intentively needed by the ABACUS numerical atomic orbital generation workflow. This "
              "parameter is used to control whether to output the overlap integrals between truncated spherical Bessel "
              "functions (TSBFs) and plane-wave basis expanded wavefunctions (named as OVERLAP_Q), and between TSBFs "
              "(named as OVERLAP_Sq), also their first order derivatives. The output files are named starting with "
              "orb_matrix. A value of 2 would enable the output.";
        item.default_value = "0";
        item.unit = "";
...
* False: Do not output vector potential.)";
    ...
    }
    {
        ...
        item.type = R"(Integer \[Integer\](optional))";
        item.description
            = "Whether to print the matrix representation of symmetry operation to running log file. If the first "
              "value is given as 1, then all matrix representations will be printed. The second optional parameter "
              "controls the precision (number of digits) to print, default is 3, which is enough for a quick check.";
        item.default_value = "1 3";
        item.unit = "";
        item.availability = "";
        item.read_value = [] (const Input_Item& item, Parameter& para)
