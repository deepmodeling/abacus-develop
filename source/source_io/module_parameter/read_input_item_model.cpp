...
[NOTE] Note: If you do not want any electric field, the parameter efield_amp should be set to zero. This should ONLY be used in a slab geometry for surface calculations, with the discontinuity FALLING IN THE EMPTY SPACE.)";
    ...
    }
    {
        ...
        item.type = "Integer";
        item.description =
            R"(The direction of the electric field or dipole correction is parallel to the reciprocal lattice vector, so the potential is constant in planes defined by FFT grid points, efield_dir can set to 0, 1 or 2.
...
* 2: parallel to the third reciprocal lattice vector)";
    ...
    }
    {
        ...
        item.category = "Electric field and dipole correction";
        item.type = "Real";
        item.description = "Position of the maximum of the saw-like potential along crystal axis efield_dir, within "
                           "the unit cell, 0 <= efield_pos_max < 1.";
    ...
    }
    {
        ...
        item.type = "Real";
        item.description =
            R"(Amplitude of the electric field. The saw-like potential increases with slope efield_amp in the region from efield_pos_max+efield_pos_dec-1) to (efield_pos_max), then decreases until (efield_pos_max+efield_pos_dec), in units of the crystal vector efield_dir.
...
* false: No potential barrier is added.)";
    ...
    }
    {
        ...
        item.type = "Real";
        item.description =
            "The effective surface tension parameter that describes the cavitation, the dispersion, and the repulsion "
            "interaction between the solute and the solvent which are not captured by the electrostatic terms";
...
[NOTE] ABACUS supports automatic setting of DFT-D3 parameters for common functionals. To benefit from this feature, please specify the parameter dft_functional explicitly, otherwise the autoset procedure will crash. If not satisfied with the built-in parameters, any manual setting on vdw_s6, vdw_s8, vdw_a1 and vdw_a2 will overwrite the automatic values.)";
    ...
    }
    {
        ...
        item.type = "String";
        item.description =
            "This scale factor is used to optimize the interaction energy deviations in van der Waals (vdW) corrected "
            "calculations. The recommended values of this parameter are dependent on the chosen vdW correction method "
            "and the DFT functional being used. For DFT-D2, the recommended values are 0.75 (PBE), 1.2 (BLYP), 1.05 "
            "(B-P86), 1.0 (TPSS), and 1.05 (B3LYP). If not set, will use values of PBE functional. For DFT-D3, "
            "recommended values with different DFT functionals can be found on the here. If not set, will search in "
            "ABACUS built-in dataset based on the dft_functional keywords. User set value will overwrite the searched "
            "value.";
        item.default_value = "";
        item.unit = "";
    ...
    }
    {
        ...
        item.category = "vdW correction";
        item.type = "String";
        item.description = "This scale factor is relevant for D3(0) and D3(BJ) van der Waals (vdW) correction methods. "
                           "The recommended values of this parameter with different DFT functionals can be found on "
                           "the webpage. If not set, will search in ABACUS built-in dataset based on the "
    ...
    }
    {
        ...
        item.type = "String";
        item.description =
            "This damping function parameter is relevant for D3(0) and D3(BJ) van der Waals (vdW) correction methods. "
            "The recommended values of this parameter with different DFT functionals can be found on the webpage. If "
            "not set, will search in ABACUS built-in dataset based on the dft_functional keywords. User set value will "
            "overwrite the searched value.";
        item.default_value = "";
        item.unit = "";
        item.availability = "vdw_method is set to d3_0 or d3_bj";
        item.reset_value = [](const Input_Item& item, Parameter& para) {
    ...
    }
    {
        ...
        item.type = "String";
        item.description =
            "This damping function parameter is only relevant for D3(0) and D3(BJ) van der Waals (vdW) correction "
            "methods. The recommended values of this parameter with different DFT functionals can be found on the "
            "webpage. If not set, will search in ABACUS built-in dataset based on the dft_functional keywords. User "
            "set value will overwrite the searched value.";
        item.default_value = "";
        item.unit = "";
        item.availability = "vdw_method is set to d3_0 or d3_bj";
        item.reset_value = [](const Input_Item& item, Parameter& para) {
...
* False: The three-body term is not included.)";
    ...
    }
    {
        ...
        item.type = "String";
        item.description =
            R"(Specifies the name of the file containing parameters for each element when using the D2 method. If not set, ABACUS uses the default parameters (Jnm6/mol) stored in the program. To manually set the parameters, provide a file containing the parameters. An example is given by:
...
* eVA (eV Angstrom))";
    ...
    }
    {
        ...
        item.type = "String";
        item.description =
            R"(Specifies the name of the file containing parameters for each element when using the D2 method. If not set, ABACUS uses the default parameters (Angstrom) stored in the program. To manually set the parameters, provide a file containing the parameters. An example is given by:
...
Namely, each line contains the element name and the corresponding parameter.)";
    ...
    }
    {
        ...
        item.type = "String";
        item.description =
            R"(Specifies the unit for the parameters in the D2 method when manually set by the user. Available options are:
* A (Angstrom)
* Bohr)";
    ...
    }
    {
        ...
        item.type = "String";
        item.description =
            R"(Determines the method used for specifying the cutoff radius in periodic systems when applying Van der Waals correction. Available options are:
...
* period: The extent of the supercell is explicitly specified using the vdw_cutoff_period keyword.)";
    ...
    }
    {
        ...
        item.category = "vdW correction";
        item.type = "String";
        item.description = "Defines the radius of the cutoff sphere when vdw_cutoff_type is set to radius. The default "
                           "values depend on the chosen vdw_method.";
        item.default_value = "";
...
* Bohr)";
    ...
    }
    {
        ...
        item.description = "The three integers supplied here explicitly specify the extent of the supercell in the "
                           "directions of the three basis lattice vectors.";
