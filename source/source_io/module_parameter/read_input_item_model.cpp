...
* false: No potential barrier is added.)";
    ...
    }
    {
        ...
        item.category = "Implicit solvation model";
        item.type = "Real";
        item.description = "The effective surface tension parameter that describes the cavitation, the dispersion, and "
                           "the repulsion interaction between the solute and the solvent which are not captured by the "
                           "electrostatic terms";
...
[NOTE] ABACUS supports automatic setting of DFT-D3 parameters for common functionals. To benefit from this feature, please specify the parameter dft_functional explicitly, otherwise the autoset procedure will crash. If not satisfied with the built-in parameters, any manual setting on vdw_s6, vdw_s8, vdw_a1 and vdw_a2 will overwrite the automatic values.)";
    ...
    }
    {
        ...
        item.type = "String";
        item.description
            = "This scale factor is used to optimize the interaction energy deviations in van der Waals (vdW) "
              "corrected calculations. The recommended values of this parameter are dependent on the chosen vdW "
              "correction method and the DFT functional being used. For DFT-D2, the recommended values are 0.75 (PBE), "
              "1.2 (BLYP), 1.05 (B-P86), 1.0 (TPSS), and 1.05 (B3LYP). If not set, will use values of PBE functional. "
              "For DFT-D3, recommended values with different DFT functionals can be found on the here. If not set, "
              "will search in ABACUS built-in dataset based on the dft_functional keywords. User set value will "
    ...
    }
    {
        ...
        item.type = "String";
        item.description
            = "This damping function parameter is relevant for D3(0) and D3(BJ) van der Waals (vdW) correction "
...
* Bohr)";
    ...
    }
    {
        ...
        bcastfuncs.push_back ([] (Parameter& para)
                                  { Parallel_Common::bcast_int ((int*)&para.input.vdw_cutoff_period, 3); });
#endif
        this->add_item (item);
    }
    {
