...
* -1: the seed is decided by time(NULL).)";
    ...
    }
    {
        ...
        item.type = "Integer";
        item.description =
            R"(Frequency (once each initsto_freq steps) to generate new stochastic orbitals when running md.
...
* 0: Never change stochastic orbitals.)";
    ...
    }
    {
        ...
        item.category = "Electronic structure (SDFT)";
        item.type = "Integer";
        item.description = "Make memory cost to 1/npart_sto times of the previous one when running the post process of "
                           "SDFT like DOS or conductivities.";
