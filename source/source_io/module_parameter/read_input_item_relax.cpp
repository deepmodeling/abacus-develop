 ...
 * Traditional approach with separate ionic and cell optimization steps)";
    ...
    }
    {
        ...
        item.category = "Geometry relaxation";
        item.type = "Integer";
        item.description = "The maximal number of ionic iteration steps. If set to 0, the code performs a quick \"dry "
                           "run\", stopping just after initialization. This is useful to check for input correctness "
    ...
    }
    {
        ...
        item.description
            = "Minimum allowed total displacement of all atoms. When the total atomic displacement falls below this "
              "value and force convergence is not achieved, the calculation will terminate. Note: This parameter is "
              "not used in the default BFGS algorithm (relax_method = bfgs 2 or bfgs).";
