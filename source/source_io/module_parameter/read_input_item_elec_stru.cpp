                        ...
                        if (std::find (lcao_solvers.begin (), lcao_solvers.end (), ks_solver) == lcao_solvers.end ())
                            {
                                const std::string warningstr
                                    = "For LCAO basis: " + nofound_str (lcao_solvers, "ks_solver");
                                ModuleBase::WARNING_QUIT ("ReadInput", warningstr);
                            }
...
#endif
#ifndef __CUDA
                                warningstr = "ks_solver is set to " + ks_solver
                                             + " but ABACUS is built with CPU only!\n"
...
#ifndef __CUSOLVERMP
                                        warningstr
                                            = "ks_solver is set to cusolvermp, but ABACUS is not built with cusolvermp "
                                              "support\n"
...
The other way is only available when compiling with LIBXC, and it allows for supplying exchange-correlation functionals as combinations of LIBXC keywords for functional components, joined by a plus sign, for example, dft_functional='LDA_X_1D_EXPONENTIAL+LDA_C_1D_CSC'.)";
    ...
    }
    {
        ...
        item.description
            = "Customized parameterization on the exchange part of XC functional. The first value should be the LibXC "
              "ID of the original functional, and latter values are external parameters. Default values are those of "
              "Perdew-Burke-Ernzerhof (PBE) functional. For more information on LibXC ID of functionals, please refer "
              "to LibXC. For parameters of functionals of interest, please refer to the source code of LibXC, such as "
              "PBE functional interface in LibXC: gga_x_pbe.c."
    ...
    }
    {
        ...
        item.type = "Integer followed by Real values";
        item.description
            = "Customized parameterization on the correlation part of XC functional. The first value should be the "
              "LibXC ID of the original functional, and latter values are external parameters. Default values are "
              "those of Perdew-Burke-Ernzerhof (PBE) functional. For more information on LibXC ID of functionals, "
              "please refer to LibXC. For parameters of functionals of interest, please refer to the source code of "
              "LibXC, such as PBE functional interface in LibXC: gga_c_pbe.c."
              "\n\n[NOTE] Solely setting this keyword will take no effect on XC functionals. One should also set "
              "dft_functional to the corresponding functional to apply the customized parameterization. "
...
Note: If gamma_only is set to 1, the KPT file will be overwritten. So make sure to turn off gamma_only for multi-k calculations.)";
                ...
                if (para.input.basis_type == "pw" && para.input.gamma_only)
                    {
                        ...
                        GlobalV::ofs_warning << " WARNING : gamma_only has not been implemented for pw yet"
                                             << std::endl;
                        ...
                        GlobalV::ofs_warning << " and a new KPT is generated with gamma point as the only k point"
                                             << std::endl;
                        GlobalV::ofs_warning << " Auto generating k-points file: " << para.input.kpoint_file
                                             << std::endl;
    ...
    }
    {
        ...
        item.description
            = "It's the density threshold for electronic iteration. It represents the charge density error between two "
              "sequential densities from electronic iterations. This criterion is always enabled. If scf_ene_thr is "
              "set, the total-energy criterion (scf_ene_thr) is additionally checked only after the first SCF "
              "iteration and only when the charge-density criterion (scf_thr) has already been satisfied. For "
              "local-orbital calculations, 1e-6 is usually accurate enough.";
    ...
    }
    {
        ...
        item.type = "Real";
        item.description
            = "It's the energy threshold for electronic iteration. The compared quantity is the total-energy "
              "difference evaluated from the charge densities before and after the Hpsi operation in one SCF step. It "
              "is not the same as the screen-output EDIFF, which is the energy difference before Hpsi and after charge "
              "mixing (i.e., across both Hpsi and charge-mixing operations).";
        ...
        item.check_value = [] (const Input_Item& item, const Parameter& para)
            {
                ...
                if (para.input.use_k_continuity && para.input.nspin == 2)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput",
                                                  "use_k_continuity cannot work for spin-polarized calculation");
                    }
