...
void ReadInput::item_exx() {
    ...
    // EXX
    {
        ...
        item.unit = "";
        item.availability = "";
        item.read_value = [](const Input_Item& item, Parameter& para) { para.input.exx_fock_alpha = item.str_values; };
    ...
    }
    {
        ...
        item.type = "Real";
        item.description =
            R"(Fraction of short-ranged Fock exchange $\mathrm{erfc}(\omega r)/r$ in range-separated hybrid functionals.)";
        item.default_value = "see hybrid_func_params";
        item.unit = "";
        item.availability = "";
        item.read_value = [](const Input_Item& item, Parameter& para) { para.input.exx_erfc_alpha = item.str_values; };
        item.reset_value = [](const Input_Item& item, Parameter& para) {
    ...
    }
    {
        ...
        item.type = "Real";
        item.description =
            R"(Range-separation parameter $\omega$ in the short-ranged Fock term $\mathrm{erfc}(\omega r)/r$.)";
        item.default_value = "see hybrid_func_params";
        item.unit = "";
        item.availability = "";
        item.read_value = [](const Input_Item& item, Parameter& para) { para.input.exx_erfc_omega = item.str_values; };
        item.reset_value = [](const Input_Item& item, Parameter& para) {
            if (para.input.exx_erfc_omega.size() == 1 && para.input.exx_erfc_omega[0] == "default") {
                std::string& dft_functional = para.input.dft_functional;
                std::string dft_functional_lower = dft_functional;
                std::transform(dft_functional.begin(), dft_functional.end(), dft_functional_lower.begin(), tolower);
                if (dft_functional_lower == "hse" || dft_functional_lower == "cwp22" ||
                    dft_functional_lower == "wp22") {
...
* True: A two-step method is employed, i.e. in the inner iterations, density matrix is updated, while in the outer iterations, is calculated based on density matrix that converges in the inner iteration.)";
        ...
        item.reset_value = [](const Input_Item& item, Parameter& para) {
            if (para.input.esolver_type == "tddft" && para.input.exx_separate_loop) {
                GlobalV::ofs_running << "For RT-TDDFT with hybrid functionals, only exx_separate_loop = 0 is supported"
                                     << std::endl;
    ...
    }
    {
        ...
        item.type = "Real";
        item.description = "It is used to compensate for divergence points at G=0 in the evaluation of Fock exchange "
                           "using lcao_in_pw method.";
        item.default_value = "0.3";
        item.unit = "";
        item.availability = "basis_type==lcao_in_pw";
        item.read_value = [](const Input_Item& item, Parameter& para) { para.input.exx_fock_lambda = item.str_values; };
        item.reset_value = [](const Input_Item& item, Parameter& para) {
    ...
    }
    {
        ...
        item.type = "Real";
        item.description =
            "To accelerate the evaluation of four-center integrals (), the product of atomic orbitals are expanded in "
            "the basis of auxiliary basis functions (ABF): . The size of the ABF (i.e. number of ) is reduced using "
            "principal component analysis. When a large PCA threshold is used, the number of ABF will be reduced, "
            "hence the calculation becomes faster. However, this comes at the cost of computational accuracy. A "
            "relatively safe choice of the value is 1e-4.";
    ...
    }
    {
        ...
        item.type = "Real";
        item.description =
            "See also the entry exx_pca_threshold. Smaller components (less than exx_c_threshold) of the matrix are "
            "neglected to accelerate calculation. The larger the threshold is, the faster the calculation and the "
            "lower the accuracy. A relatively safe choice of the value is 1e-4.";
        item.default_value = "1E-4";
        item.unit = "";
        item.availability = "";
    ...
    }
    {
        ...
        item.type = "Real";
        item.description =
            "By default, the Coulomb matrix inversion required for obtaining LRI coefficients is performed using LU "
            "decomposition. However, this approach may suffer from numerical instabilities when a large set of "
            "auxiliary basis functions (ABFs) is employed. When exx_cs_inv_thr > 0, the inversion is instead carried "
            "out via matrix diagonalization. Eigenvalues smaller than exx_cs_inv_thr are discarded to improve "
            "numerical stability. A relatively safe and commonly recommended value is 1e-5.";
    ...
    }
    {
        ...
        item.type = "Real";
        item.description =
            "See also the entry exx_pca_threshold. With the approximation , the four-center integral in Fock exchange "
            "is expressed as , where is a double-center integral. Smaller values of the V matrix can be truncated to "
            "accelerate calculation. The larger the threshold is, the faster the calculation and the lower the "
            "accuracy. A relatively safe choice of the value is 0, i.e. no truncation.";
    ...
    }
    {
        ...
        item.type = "Real";
        item.description =
            "The Fock exchange can be expressed as where D is the density matrix. Smaller values of the density matrix "
            "can be truncated to accelerate calculation. The larger the threshold is, the faster the calculation and "
    ...
    }
    {
        ...
        item.type = "Real";
        item.description =
            "See also the entry exx_pca_threshold. is used in force. Smaller components (less than "
            "exx_c_grad_threshold) of the matrix are neglected to accelerate calculation. The larger the threshold is, "
            "the faster the calculation and the lower the accuracy. A relatively safe choice of the value is 1e-4.";
        item.default_value = "1E-4";
        item.unit = "";
        item.availability = "";
    ...
    }
    {
        ...
        item.description =
            "See also the entry exx_pca_threshold. With the approximation , the four-center integral in Fock exchange "
            "is expressed as , where is a double-center integral. is used in force. Smaller values of the V matrix can "
            "be truncated to accelerate calculation. The larger the threshold is, the faster the calculation and the "
            "lower the accuracy. A relatively safe choice of the value is 0, i.e. no truncation.";
        item.default_value = "1E-1";
        item.unit = "";
        item.availability = "";
    ...
    }
    {
        ...
        item.type = "Real";
        item.description =
            "See also the entry exx_pca_threshold. is used in stress. Smaller components (less than "
            "exx_c_grad_r_threshold) of the matrix are neglected to accelerate calculation. The larger the threshold "
            "is, the faster the calculation and the lower the accuracy. A relatively safe choice of the value is 1e-4.";
        item.default_value = "1E-4";
        item.unit = "";
        item.availability = "";
    ...
    }
    {
        ...
        item.description =
            "See also the entry exx_pca_threshold. With the approximation , the four-center integral in Fock exchange "
            "is expressed as , where is a double-center integral. is used in force and stress. Smaller values of the V "
            "matrix can be truncated to accelerate calculation. The larger the threshold is, the faster the "
            "calculation and the lower the accuracy. A relatively safe choice of the value is 0, i.e. no truncation.";
        item.default_value = "1E-1";
        item.unit = "";
        item.availability = "";
    ...
    }
    {
        ...
        item.type = "String";
        item.description =
            "This parameter determines how many times larger the radial mesh required for calculating Columb potential "
            "is to that of atomic orbitals. The value should be larger than 0. Reducing this value can effectively "
            "increase the speed of self-consistent calculations using hybrid functionals.";
        item.default_value = "";
        item.unit = "";
        item.availability = "";
        read_sync_string(input.exx_ccp_rmesh_times);
        item.reset_value = [](const Input_Item& item, Parameter& para) {
            if (para.input.exx_ccp_rmesh_times == "default") { // to run through here, the default value of
                                                               // para.input.exx_ccp_rmesh_times should be "default"
                std::string& dft_functional = para.input.dft_functional;
                std::string dft_functional_lower = dft_functional;
                std::transform(dft_functional.begin(), dft_functional.end(), dft_functional_lower.begin(), tolower);
    ...
    }
    {
        ...
        item.category = "Exact Exchange (LCAO)";
        item.type = "Integer";
        item.description = "The maximum l of the spherical Bessel functions, when the radial part of opt-ABFs are "
                           "generated as linear combinations of spherical Bessel functions. A reasonable choice is 2.";
    ...
    }
    {
        ...
        item.category = "Exact Exchange (LCAO)";
        item.type = "Real";
        item.description = "The cut-off of plane wave expansion, when the plane wave basis is used to optimize the "
                           "radial ABFs. A reasonable choice is 60.";
        item.default_value = "0";
        item.unit = "Ry";
        item.availability = "calculation==gen_opt_abfs";
...
* False: Enforce LibRI to use complex data type. Setting it to True can effectively improve the speed of self-consistent calculations with hybrid functionals.)";
        ...
        item.reset_value = [](const Input_Item& item, Parameter& para) {
            if (para.input.exx_real_number ==
                "default") { // to run through here, the default value of para.input.exx_real_number should be "default"
...
* revised_spencer: see Phys. Rev. Mater. 5, 013807 (2021). Set the scheme of Coulomb singularity correction.)";
        ...
        item.reset_value = [](const Input_Item& item, Parameter& para) {
            if (para.input.exx_singularity_correction == "default") {
                ...
                if (dft_functional_lower == "hf" || dft_functional_lower == "pbe0" || dft_functional_lower == "b3lyp" ||
                    dft_functional_lower == "scan0" || dft_functional_lower == "muller" ||
                    dft_functional_lower == "power" || dft_functional_lower == "wp22" ||
    ...
    }
    {
        ...
        item.category = "Exact Exchange (LCAO)";
        item.type = "Real";
        item.description = "How many times larger the radial mesh required is to that of atomic orbitals in the "
                           "postprocess calculation of the bare Coulomb matrix for RPA, GW, etc.";
    ...
    }
    {
        ...
        item.type = "Boolean";
        item.description =
            R"(* False: only rotate k-space density matrix D(k) from irreducible k-points to accelerate diagonalization
* True: rotate both D(k) and Hexx(R) to accelerate both diagonalization and EXX calculation)";
    ...
    }
    {
        ...
        item.category = "Exact Exchange (LCAO)";
        item.type = "Boolean";
        item.description = "Whether to output the coefficient tensor C(R) and ABFs-representation Coulomb matrix V(R) "
                           "for each atom pair and cell in real space.";
...
void ReadInput::item_dftu() {
    ...
    // dft+u
    {
        ...
        item.type = "Integer";
        item.description =
            R"(Determines whether to calculate the plus U correction, which is especially important for correlated electrons.
...
* 0: Do not calculate plus U correction.)";
    ...
    }
    {
        ...
        item.description =
            R"(Specifies which orbits need plus U correction for each atom type ( for atom type 1, 2, 3, respectively).
...
* 3: For f-electron orbits, the plus U correction is needed.)";
    ...
    }
    {
        ...
        item.description =
            R"(Specifies the Hubbard Coulomb interaction parameter U (eV) in plus U correction, which should be specified for each atom unless the Yukawa potential is used.
...
[NOTE] Note: Since only the simplified scheme by Duradev is implemented, the 'U' here is actually U-effective, which is given by Hubbard U minus Hund J.)";
    ...
    }
    {
        ...
        item.type = "Boolean";
        item.description =
            R"(Determines whether to use the local screen Coulomb potential method to calculate the values of U and J.
...
* False: hubbard_u does need to be specified.)";
    ...
    }
    {
        ...
        item.category = "DFT+U correction";
        item.type = "Real";
        item.description = "The screen length of Yukawa potential. If left to default, the screen length will be "
                           "calculated as an average of the entire system. It's better to stick to the default setting "
    ...
    }
    {
        ...
        item.category = "DFT+U correction";
        item.type = "Real";
        item.description = "Once uramping > 0.15 eV. DFT+U calculations will start SCF with U = 0 eV, namely normal "
                           "LDA/PBE calculations. Once SCF restarts when drho<mixing_restart, U value will increase by "
                           "uramping eV. SCF will repeat above calcuations until U values reach target defined in "
                           "hubbard_u. As for uramping=1.0 eV, the recommendations of mixing_restart is around 5e-4.";
        item.default_value = "-1.0.";
        item.unit = "eV";
...
[NOTE] The easiest way to create initial_onsite.dm is to run a DFT+U calculation, look for a file named onsite.dm in the OUT.prefix directory, and make replacements there. The format of the file is rather straight-forward.)";
    ...
    }
    {
        ...
        item.type = "Real";
        item.description =
            R"(* The onsite_radius parameter facilitates modulation of the single-zeta portion of numerical atomic orbitals used for DFT+U projections.
