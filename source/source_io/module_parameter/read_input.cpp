...
std::string
    longstring (const std::vector<std::string>& words)
{
    return FmtCore::join (" ", words);
}

bool
    assume_as_boolean (const std::string& val)
{
...
void
    read_information (std::stringstream& ifs, std::vector<std::string>& output, const std::string& delimiters)
{
    std::string line;
    getline (ifs, line);
...
bool
    filter_nonascii_and_comment (std::ifstream& ifs, std::stringstream& out_ascii_stream)
{
    if (!ifs.is_open ())
        {
            if (!ifs)
...
void
    ReadInput::read_parameters (Parameter& param, const std::string& filename_in)
{
...
void
    ReadInput::create_directory (const Parameter& param)
{
    ...
    if (!param.input.out_app_flag
        && (param.input.out_mat_hs2[0] || param.input.out_mat_r[0] || param.input.out_mat_t[0]
...
void
    ReadInput::write_parameters (const Parameter& param, const std::string& filename_out)
{
    if (this->rank == 0)
        {
...
void
    ReadInput::read_txt_input (Parameter& param, const std::string& filename)
{
    ...
    while (ascii_stream.good ())
        {
            ...
            auto it = std::find_if (input_lists.begin (),
                                    input_lists.end (),
                                    [&word] (const std::pair<std::string, Input_Item>& item)
            ...
            if (ascii_stream.eof ())
                ...
                {
                    ModuleBase::WARNING_QUIT ("Input", " Bad input parameters. ", 1);
                }
            else if (ascii_stream.fail ())
                {
                    ascii_stream.clear ();
                    ModuleBase::WARNING_QUIT ("Input", " fail to read parameters. ", 1);
                }
        }
...
void
    ReadInput::write_txt_input (const Parameter& param, const std::string& filename)
{
...
void
    ReadInput::check_ntype (const std::string& fn, int& param_ntype)
{
...
int
    ReadInput::current_md_step (const std::string& file_dir)
{
...
void
    ReadInput::add_item (const Input_Item& item)
{
...
std::string
    nofound_str (std::vector<std::string> init_chgs, const std::string& str)
{
