...
void ReadInput::item_system()
{
    ...
    }
    {
        ...
        item.read_value = [](const Input_Item& item, Parameter& para) {
            ...
            {
                ModuleBase::WARNING_QUIT("ReadInput", "replicate requires exactly three integers.");
            }
            for (int i = 0; i < 3; ++i) para.input.replicate[static_cast<std::size_t>(i)] = std::stoi(item.str_values[static_cast<std::size_t>(i)]);
        };
        item.check_value = [](const Input_Item&, const Parameter& para) {
            for (int i = 0; i < 3; ++i)
            {
                if (para.inp.replicate[static_cast<std::size_t>(i)] <= 0)
                {
                    ModuleBase::WARNING_QUIT("ReadInput", "replicate values must all be positive.");
                }
            }
        };
        sync_intvec(input.replicate, 3, 1);
        this->add_item(item);
    }
    {
