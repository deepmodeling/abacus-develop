        ...
        item.check_value = [] (const Input_Item& item, const Parameter& para)
            {
                ...
                if (std::find (esolver_types.begin (), esolver_types.end (), para.input.esolver_type)
                    == esolver_types.end ())
                    {
                ...
                if (para.input.symmetry == "default")
                    {
                        if (para.input.gamma_only || para.input.calculation == "nscf"
                            || para.input.calculation == "get_s" || para.input.calculation == "get_pchg"
        ...
        item.check_value = [] (const Input_Item& item, const Parameter& para)
            {
                ...
                if (kspacing_zero_num > 0 && kspacing_zero_num < 3)
                    {
                        std::cout << "kspacing: " << kspacing[0] << " " << kspacing[1] << " " << kspacing[2]
                                  << std::endl;
                        ModuleBase::WARNING_QUIT ("ReadInput", "kspacing must > 0");
                    }
            };
        ...
        item.check_value = [] (const Input_Item& item, const Parameter& para)
            {
                ...
                if (para.inp.gint_precision != "double" && para.inp.nspin == 4)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput",
                                                  "gint_precision = single or mix is not supported for nspin = 4 "
        ...
        item.check_value = [] (const Input_Item& item, const Parameter& para)
            {
                ...
                if (para.input.dm_to_rho && para.inp.gamma_only)
                    {
                        ModuleBase::WARNING_QUIT ("ReadInput",
                                                  "dm_to_rho is not available for gamma_only calculations");
                    }
