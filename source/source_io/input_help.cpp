 ...
 */
std::string
    word_wrap (const std::string& text, size_t width, size_t indent, size_t first_indent)
{
    if (text.empty ())
        {
            return "";
 ...
 */
std::string
    word_wrap (const std::string& text, size_t width, size_t indent)
{
 ...
 */
std::string
    format_structured_description (const std::string& desc)
{
    ...
    while (pos < desc.length ())
        {
            ...
            if ((pos + 3 < desc.length () && desc[pos] == '\n' && desc[pos + 1] == ' ' && desc[pos + 2] == ' '
                 && desc[pos + 3] == '*')
                || (at_line_start && pos + 2 < desc.length () && desc[pos] == ' ' && desc[pos + 1] == ' '
                    && desc[pos + 2] == '*'))
                {
                    // Flush current text
                    if (!current_text.empty ())
 ...
 */
void
    emit_block_scalar (std::ostream& os, const std::string& text, int indent)
{
    os << "|\n";
 ...
 */
std::string
    yaml_quote_if_needed (const std::string& value)
{
    if (value.empty ())
        {
            return "\"\"";
    ...
    if (lower == "true" || lower == "false" || lower == "yes" || lower == "no" || lower == "on" || lower == "off"
        || lower == "null" || lower == "~" || lower == ".inf" || lower == "-.inf" || lower == ".nan")
        {
            return "\"" + value + "\"";
        }
        ...
        for (char c: value)
            {
                if (!std::isdigit (static_cast<unsigned char> (c)) && c != '.' && c != '-' && c != '+' && c != 'e'
                    && c != 'E' && c != 'x' && c != 'X' && c != 'o' && c != 'O' && c != 'a' && c != 'b' && c != 'c'
                    && c != 'd' && c != 'f' && c != 'A' && c != 'B' && c != 'C' && c != 'D' && c != 'F')
                    {
...
bool
    ParameterHelp::show_parameter_help (const std::string& key, std::ostream& os)
{
    initialize ();
...
std::vector<std::string>
    ParameterHelp::search_parameters (const std::string& query)
{
    initialize ();
...
void
    ParameterHelp::show_general_help (std::ostream& os)
{
    os << "\n";
...
void
    ParameterHelp::generate_yaml (std::ostream& os)
{
    // Create a ReadInput instance to access Input_Item documentation
    // Use rank -1 to indicate help-system mode (no MPI operations)
    ReadInput reader (-1);
...
ParameterMetadata
    ParameterHelp::get_metadata (const std::string& key)
{
    initialize ();
...
std::string
    ParameterHelp::to_lowercase (const std::string& str)
{
...
int
    ParameterHelp::levenshtein_distance (const std::string& s1, const std::string& s2)
{
...
std::vector<std::string>
    ParameterHelp::find_similar_parameters (const std::string& query, int max_suggestions, int max_distance)
{
    initialize ();
