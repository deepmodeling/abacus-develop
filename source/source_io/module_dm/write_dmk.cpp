...
std::string
    ModuleIO::dmk_gen_fname (const bool gamma_only, const int ispin, const int nspin, const int ik, const int istep)
{
...
void
    ModuleIO::dmk_readData (std::ifstream& ifs, double& data)
{
    ifs >> data;
}

void
    ModuleIO::dmk_readData (std::ifstream& ifs, std::complex<double>& data)
{
    ...
    if (complex_str.front () == '(' && complex_str.back () == ')' && comma_pos != std::string::npos)
        ...
        {
            ModuleBase::WARNING_QUIT ("ModuleIO::dmk_readData", "Invalid complex number format in dmk: " + complex_str);
        }
}

template <typename T>
bool
    ModuleIO::read_dmk (const int nspin,
                        const int nk,
                        const K_Vectors& kv,
                    ...
                    for (int ik = 0; ik < nk; ik++)
                        {
                            ...
                            if (!ifs)
                                {
                                    ofs_running << " Cannot find density matrix file " << fn << " for k-point "
                                                << ik + 1 << std::endl;
                                    ModuleBase::WARNING ("ModuleIO::read_dmk",
                                                         "Can't open density matrix (k) file < " + fn + " >.");
                                    read_success = false;
                                    break;
                                }
                            else
                                {
                                    ofs_running << " Read density matrix file " << fn << " for k-point " << ik + 1
                                                << std::endl;
                                }
...
template <typename T>
void
    ModuleIO::write_dmk (const std::vector<std::vector<T>>& dmk,
                         const K_Vectors& kv,
                         const int precision,
            ...
            for (int ik = 0; ik < nk; ik++)
                {
                    ...
                    if (my_rank == 0)
                        {
                            std::string fn
                                = PARAM.globalv.global_out_dir + dmk_gen_fname (gamma_only, ispin, nspin, ik, istep);
                            ...
                            if (!ofs)
                                {
                                    ModuleBase::WARNING ("ModuleIO::write_dmk",
                                                         "Can't create DENSITY MATRIX File < " + fn + " >.");
                                    continue;
                                }
                            else
                                {
                            ...
                            ofs << " " << kv.get_nkstot () << " # total k points after symmetrized (if open) "
                                << std::endl;
