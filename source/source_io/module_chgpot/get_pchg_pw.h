...
void
    get_pchg_pw (const std::vector<int>& out_pchg,
                 const int nbands,
                 const int nspin,
                 const int nxyz,
                 const int chr_ngmc,
                            ...
                            for (int ik = 0; ik < nks; ++ik)
                                {
                                    ...
                                    const int k_number
                                        = ikstot % (nkstot / nspin) + 1; // k-point number, starting from 1
                                    ...
                                    // To ensure the normalization of charge density in multi-k calculation (if
                                    // if_separate_k is true)
                                    double wg_sum_k = 0.0;
                                    if (nspin == 1)
                                        {
                                            wg_sum_k = 2.0;
                                    ...
                                    ssc << global_out_dir << "pchgi" << ib + 1 << "s" << spin_index + 1 << "k"
                                        << k_number << ".cube";
