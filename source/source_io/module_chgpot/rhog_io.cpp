...
bool
    ModuleIO::read_rhog (const std::string& filename, const ModulePW::PW_Basis* pw_rhod, std::complex<double>** rhog)
{
            ...
            for (int i = 0; i < npwtot_in; ++i)
                {
                    ...
                    if (ix <= -int ((nx + 1) / 2) || ix >= int (nx / 2) + 1 || iy <= -int ((ny + 1) / 2)
                        || iy >= int (ny / 2) + 1 || iz <= -int ((nz + 1) / 2) || iz >= int (nz / 2) + 1)
                        {
...
bool
    ModuleIO::write_rhog (const std::string& fchg,
                    ...
                    for (int ig = 0; ig < pw_rho->npw; ++ig)
                        {
                            const ModuleBase::Vector3<double> g
                                = pw_rho->gdirect[ig]; // g direct is (ix, iy, iz), miller index (integer), centered at
                                                       // (0, 0, 0)
