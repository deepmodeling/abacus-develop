...
void
    ModuleIO::write_libxc_r (const int order,
                             ...
                             const ModulePW::PW_Basis& pw_rhod)
{
    ...
    if (is_gga)
        {
            switch (order)
                ...
                case 0:
                    break;
                default:
                    throw std::domain_error ("order =" + std::to_string (order) + " unfinished in "
                ...
                default:
                    {
                        throw std::domain_error ("func.info->family =" + std::to_string (func.info->family)
                                                 + " unfinished in " + std::string (__FILE__) + " line "
                                                 + std::to_string (__LINE__));
                        break;
                    }
    ...
    auto write_data =
        [&pw_big, &pw_rhod, nspin] (const std::string data_name, const std::vector<double>& data, const int number_spin)
        {
            for (int is = 0; is < number_spin; ++is)
                {
                    ...
                    if (GlobalV::MY_RANK == 0)
                        {
                            const std::string file_name = PARAM.globalv.global_out_dir + "xc_" + data_name + "_s"
                                                          + std::to_string (is + 1) + ".cube";
                            ofs.open (file_name);
...
void
    ModuleIO::write_cube_core (std::ofstream& ofs_cube,
                               const int bz,
                               const int nbz,
                               const int nplane,
