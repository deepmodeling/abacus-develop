...
// For gamma_only
void
    Get_pchg_lcao::begin (double** rho,
...
#else
                    ModuleBase::WARNING_QUIT ("Get_pchg_lcao::begin",
                                              "The `pchg` calculation is only available for MPI now!");
#endif
                            ...
                            for (int ik = 0; ik < kv.get_nks () / nspin; ++ik)
                                {
                                    ...
                                    for (int is = 0; is < nspin; ++is)
                                        {
                                            ModuleBase::GlobalFunc::DCOPY (rho[is],
                                                                           rho_save[is].data (),
                                                                           rhopw_nrxx); // Copy data
                                        }
                                    ...
                                    for (int is = 0; is < nspin; ++is)
                                        {
                                            // ssc should be inside the inner loop to reset the string stream each time
                                            std::stringstream ssc;
                                            ssc << global_out_dir << "pchgi" << ib + 1 << "s" << is + 1 << "k" << ik + 1
                                                << ".cube";
...
void
    Get_pchg_lcao::select_bands (const std::vector<int>& out_pchg, const int nbands, const int fermi_band)
{
...
// For gamma_only
void
    Get_pchg_lcao::idmatrix (const int& ib,
                             const int nspin,
                             const double& nelec,
                             const ModuleBase::matrix& wg,
    ...
    for (int is = 0; is < nspin; ++is)
        {
            ...
            for (int ir = 0; ir < wg_wfc.get_nbands (); ++ir)
                {
                    BlasConnector::scal (wg_wfc.get_nbasis (),
                                         wg_local[ir],
                                         wg_wfc.get_pointer () + ir * wg_wfc.get_nbasis (),
                                         1);
                }
...
void
    Get_pchg_lcao::prepare_get_pchg (std::ofstream& ofs_running)
{
    ofs_running << "\n\n";
