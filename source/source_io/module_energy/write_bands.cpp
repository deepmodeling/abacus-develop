...
void
    ModuleIO::write_bands (const Input_para& inp, const ModuleBase::matrix& ekb, const K_Vectors& kv)
{
...
void
    ModuleIO::nscf_bands (const int& is,
                    ...
                    if (GlobalV::RANK_IN_POOL == 0)
                        {
                            ...
                            for (int ib = 0; ib < nband; ib++)
                                {
                                    ofs << FmtCore::format (fmtstr.c_str (),
                                                            (ekb (ik_now + is * nks_np, ib) - fermie)
                                                                * ModuleBase::Ry_to_eV);
                                }
                            ofs << std::endl;
                            ofs.close ();
