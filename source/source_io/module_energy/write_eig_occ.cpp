...
void
    ModuleIO::write_eig_iter (const ModuleBase::matrix& ekb, const ModuleBase::matrix& wg, const K_Vectors& kv)
{
                    ...
                    if (GlobalV::MY_POOL == ip && ip_flag)
                        {
                            ...
                            for (int ik = start_ik; ik < end_ik; ++ik)
                                {
                                    GlobalV::ofs_running << " spin=" << is + 1
                                                         << " k-point=" << kv.ik2iktot[ik] + 1 - is * nkstot_np
                                                         << std::endl;

                                    GlobalV::ofs_running << std::setw (8) << "Index" << std::setw (18)
                                                         << "Eigenvalues(eV)" << std::setw (18) << "Occupations"
                                                         << std::endl;

                                    for (int ib = 0; ib < ekb.nc; ib++)
                                        {
                                            GlobalV::ofs_running << std::setw (8) << ib + 1 << std::setw (18)
                                    ...
                                    for (int ik = 0; ik < recv_nks_np; ++ik)
                                        {
                                            GlobalV::ofs_running
                                                << " spin=" << is + 1
                                                << " k-point=" << recv_ik2iktot[ik] + 1 - is * nkstot_np << std::endl;
                                            ...
                                            for (int ib = 0; ib < recv_nbands; ib++)
                                                {
                                                    GlobalV::ofs_running << std::setw (8) << ib + 1 << std::setw (18)
                                                                         << recv_ekb (ik, ib) * ModuleBase::Ry_to_eV
                                                                         << std::setw (18) << recv_wg (ik, ib)
                                                                         << std::endl;
                                                }
                                            GlobalV::ofs_running << std::endl;
                                        }
...
void
    ModuleIO::write_eig_file (const ModuleBase::matrix& ekb,
                            ...
                            for (int ik = start_ik; ik < end_ik; ++ik)
                                {
                                    ofs_eig << " spin=" << is + 1 << " k-point=" << kv.ik2iktot[ik] + 1 - is * nkstot_np
                                            << "/" << nkstot_np << " Cartesian=" << kv.kvec_c[ik].x << " "
                                            << kv.kvec_c[ik].y << " " << kv.kvec_c[ik].z << " (" << ngk_tot[ik]
                                            << " plane wave)" << std::endl;
