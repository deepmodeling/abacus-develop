        ...
        for (int ip = 0; ip < kpar; ++ip) {
            ...
            if (GlobalV::MY_POOL == ip && ip_flag) {
                ...
                for (int ik = start_ik; ik < end_ik; ++ik) {
                    GlobalV::ofs_running << " spin=" << is + 1 << " k-point=" << kv.ik2iktot[ik] + 1 - is * nkstot_np
                                         << std::endl;

                    GlobalV::ofs_running << std::setw(8) << "Index" << std::setw(18) << "Eigenvalues(eV)"
                                         << std::setw(18) << "Occupations" << std::endl;

                    for (int ib = 0; ib < ekb.nc; ib++) {
                        GlobalV::ofs_running << std::setw(8) << ib + 1 << std::setw(18)
                                             << ekb(ik, ib) * ModuleBase::Ry_to_eV << std::setw(18) << wg(ik, ib)
                                             << std::endl;
                    }
                ...
                if (GlobalV::MY_RANK == 0) {
                    ...
                    for (int ik = 0; ik < recv_nks_np; ++ik) {
                        ...
                        for (int ib = 0; ib < recv_nbands; ib++) {
                            GlobalV::ofs_running << std::setw(8) << ib + 1 << std::setw(18)
                                                 << recv_ekb(ik, ib) * ModuleBase::Ry_to_eV << std::setw(18)
                                                 << recv_wg(ik, ib) << std::endl;
                        }
                        GlobalV::ofs_running << std::endl;
                    }
            ...
            if (GlobalV::MY_POOL == ip && ip_flag) {
                ...
                for (int ik = start_ik; ik < end_ik; ++ik) {
                    ...
                    for (int ib = 0; ib < ekb.nc; ib++) {
                        ...
                        ofs_eig << " " << ib + 1 << " " << ekb(ik, ib) * ModuleBase::Ry_to_eV << " " << occupation
                                << std::endl;
                    }
                    ofs_eig << std::endl;
                }
