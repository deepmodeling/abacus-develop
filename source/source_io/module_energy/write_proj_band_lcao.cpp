...
template <>
void ModuleIO::write_proj_band_lcao(const psi::Psi<std::complex<double>>* psi,
                                    const Parallel_Orbitals& pv,
                                    const elecstate::ElecState* pelec,
                                    const K_Vectors& kv,
                                    const UnitCell& ucell,
    ...
    for (int is = 0; is < nspin0; is++) {
        ...
        for (int ik = 0; ik < kv.get_nks(); ik++) {
            if (is == kv.isk[ik]) {
                ...
                // calculate Mulk
                psi->fix_k(ik);
                psi::Psi<std::complex<double>> Dwfc(1, psi->get_nbands(), psi->get_nbasis(), psi->get_nbasis(), true);
