...
template <>
void
    ModuleIO::write_proj_band_lcao (const psi::Psi<double>* psi,
            ...
            if (GlobalV::MY_RANK == 0)
                {
                    ...
                    if (PARAM.inp.nspin == 4)
                        {
                            out << "<norbitals>" << std::setw (2) << PARAM.globalv.nlocal / 2 << "</norbitals>"
                                << std::endl;
                        }
                    else
                        {
                            out << "<norbitals>" << std::setw (2) << PARAM.globalv.nlocal << "</norbitals>"
                                << std::endl;
                        }
                    out << "<band_structure nkpoints=\"" << nks << "\" nbands=\"" << PARAM.inp.nbands
                        << "\" units=\"eV\">" << std::endl;
                            ...
                            for (int j = 0; j < atom1->nw; ++j)
                                {
                                    ...
                                    out << std::setw (5) << "atom_index=\"" << std::setw (40) << i + 1 << "\""
                                        << std::endl;
                                            ...
                                            else if (PARAM.inp.nspin == 4)
                                                {
                                                    ...
                                                    out << std::setw (13)
                                                        << weight (is, ib * PARAM.globalv.nlocal + s0 + 2 * w0)
                                                               + weight (is,
                                                                         ib * PARAM.globalv.nlocal + s0 + 2 * w0 + 1);
                                                }
                                        }
                                    out << std::endl;
                                    out << "</data>" << std::endl;
...
template <>
void
    ModuleIO::write_proj_band_lcao (const psi::Psi<std::complex<double>>* psi,
                                    const Parallel_Orbitals& pv,
                                    const elecstate::ElecState* pelec,
                                    const K_Vectors& kv,
                                    const UnitCell& ucell,
                    ...
                    if (is == kv.isk[ik])
                        {
                            ...
                            if (PARAM.inp.nspin == 4)
                                {
                                    dynamic_cast<hamilt::HamiltLCAO<std::complex<double>, std::complex<double>>*> (
                                        p_ham)
                                        ->updateSk (ik, 1);
                                    sk = dynamic_cast<
                                             const hamilt::HamiltLCAO<std::complex<double>, std::complex<double>>*> (
                                             p_ham)
                                             ->getSk ();
                                }
                            else
                                            ...
                                            if (pv.in_this_processor (j, i))
                                                {
                                                    ...
                                                    weightk (ik, i * PARAM.globalv.nlocal + j)
                                            ...
                                            for (int ib = 0; ib < PARAM.inp.nbands; ib++)
                                                {
                                                    if (PARAM.inp.nspin == 1)
                                                        {
                                                            out << std::setw (13)
                                                                << weight (ik, ib * PARAM.globalv.nlocal + w);
                                                        }
                                                    else if (PARAM.inp.nspin == 2)
                                                        {
                                                            out << std::setw (13)
                                                                << weight (ik + nks * is,
                                                                           ib * PARAM.globalv.nlocal + w);
                                                        }
                                                    else if (PARAM.inp.nspin == 4)
                                                        {
                                                            int w0 = w - s0;
                                                            out << std::setw (13)
                                                                << weight (ik, ib * PARAM.globalv.nlocal + s0 + 2 * w0)
                                                                       + weight (ik,
                                                                                 ib * PARAM.globalv.nlocal + s0 + 2 * w0
                                                                                     + 1);
                                                        }
                                                }
                                            out << std::endl;
