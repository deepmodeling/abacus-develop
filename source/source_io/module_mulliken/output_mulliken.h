    ...
    if (PARAM.inp.out_mul)
        {
            ...
            auto mulp
                = ModuleIO::Output_Mulliken<TK> (&(out_s_k), &(out_dm_k), pv, &cell_index, kv.isk, PARAM.inp.nspin);
    ...
    if (PARAM.inp.onsite_radius > 0)
        {
            ...
            if (PARAM.inp.nspin == 2)
                ...
                {
                    auto sc_lambda
                        = new hamilt::DeltaSpin<hamilt::OperatorLCAO<std::complex<double>, std::complex<double>>> (
                            nullptr,
                            kv.kvec_d,
