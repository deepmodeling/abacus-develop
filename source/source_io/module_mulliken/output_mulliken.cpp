...
Output_Mulliken<TK>::Output_Mulliken (Output_Sk<TK>* output_sk,
                                      ...
                                      int nspin)
    : output_sk_ (output_sk), output_dmk_ (output_dmk), ParaV_ (ParaV), cell_index_ (cell_index), isk_ (isk),
      nspin_ (nspin)
{
...
template <typename TK>
void
    Output_Mulliken<TK>::write (int istep, std::string out_dir)
{
...
template <typename TK>
void
    Output_Mulliken<TK>::write_mulliken_nspin1 (int istep,
    ...
    for (int iat = 0; iat < this->cell_index_->get_nat (); ++iat)
        {
            ...
            os << " ------------------" << std::endl;
            os << FmtCore::format ("%14s", std::string ("zeta")) << FmtCore::format ("%10s", std::string ("spin1"))
               << std::endl;
            ...
            for (int L = 0; L <= this->cell_index_->get_maxL (iat); L++)
                {
                    ...
                    os << fmt_of_sum.format (std::string (" sum mz")) << fmt_of_chg.format (sum_over_m_and_z[0])
                       << std::endl;
                    os << std::endl;
                }
            os << fmt_of_sum.format (std::string (" sum lmz")) << fmt_of_chg.format (atom_chg[iat][0]) << std::endl;
            os << std::endl;
            os << std::left << " total charge    on atom " << iat + 1 << " " << fmt_of_chg.format (atom_chg[iat][0])
               << std::endl;
            os << std::endl;
        }
}
...
template <typename TK>
void
    Output_Mulliken<TK>::write_mulliken_nspin2 (int istep,
                                                const std::vector<double>& tot_chg,
                                                const std::vector<std::vector<double>>& atom_chg,
                                                std::map<std::vector<int>, double> orb_chg,
                                                std::ofstream& os)
    ...
    for (int iat = 0; iat < this->cell_index_->get_nat (); ++iat)
        {
            ...
            os << " ------------------" << std::endl;
            os << FmtCore::format ("%14s", std::string ("zeta")) << FmtCore::format ("%10s", std::string ("spin1"))
               << FmtCore::format ("%10s", std::string ("spin2")) << FmtCore::format ("%10s", std::string ("sum"))
            ...
            os << fmt_of_sum.format (std::string (" sum lmz")) << fmt_of_chg.format (atom_chg[iat][0])
               << fmt_of_chg.format (atom_chg[iat][1]) << fmt_of_chg.format (atom_chg[iat][0] + atom_chg[iat][1])
...
template <typename TK>
void
    Output_Mulliken<TK>::write_mulliken_nspin4 (int istep,
                                                const std::vector<double>& tot_chg,
                                                const std::vector<std::vector<double>>& atom_chg,
                                                std::map<std::vector<int>, double> orb_chg,
                                                std::ofstream& os)
    ...
    for (int iat = 0; iat < this->cell_index_->get_nat (); ++iat)
        {
            ...
            os << " ------------------" << std::endl;
            os << FmtCore::format ("%14s", std::string ("zeta")) << FmtCore::format ("%10s", std::string ("spin1"))
               << FmtCore::format ("%10s", std::string ("spin2")) << FmtCore::format ("%10s", std::string ("spin3"))
                            ...
                            if (L > 0)
                                {
                                    os << fmt_of_sum.format (std::string (" sum m"))
                                       << fmt_of_chg.format (sum_over_m[0]) << fmt_of_chg.format (sum_over_m[1])
                                       << fmt_of_chg.format (sum_over_m[2]) << fmt_of_chg.format (sum_over_m[3])
                                       << std::endl;
                                }
                        }
                    os << fmt_of_sum.format (std::string (" sum mz")) << fmt_of_chg.format (sum_over_m_and_z[0])
                       << fmt_of_chg.format (sum_over_m_and_z[1]) << fmt_of_chg.format (sum_over_m_and_z[2])
            ...
            os << std::left << " total charge    on atom " << iat + 1 << " " << fmt_of_chg.format (atom_chg[iat][0])
               << std::endl;
            os << std::left << " total magnetism on atom " << iat + 1 << " " << fmt_of_chg.format (atom_chg[iat][1])
               << " " << fmt_of_chg.format (atom_chg[iat][2]) << " " << fmt_of_chg.format (atom_chg[iat][3])
               << std::endl;
            os << std::endl;
        }
}
...
template <typename TK>
void
    Output_Mulliken<TK>::set_nspin (int nspin_in)
{
...
template <typename TK>
void
    Output_Mulliken<TK>::set_ParaV (Parallel_Orbitals* ParaV_in)
{
...
template <typename TK>
std::vector<double>
    Output_Mulliken<TK>::get_tot_chg ()
{
...
std::vector<std::vector<double>>
    Output_Mulliken<TK>::get_atom_chg ()
{
...
std::map<std::vector<int>, double>
    Output_Mulliken<TK>::get_orb_chg ()
{
    int nat = this->cell_index_->get_nat ();
    std::map<std::vector<int>, double> orb_chg;
    for (int is = 0; is < this->nspin_; is++)
                                    ...
                                    for (int M = 0; M < (2 * L + 1); M++)
                                        {
                                            orb_chg[std::vector<int>{iat, is, L, Z, M}]
                                                = std::abs (this->orbMulP_ (is, num)) < 1e-10
                                                      ? 0.0
                                                      : this->orbMulP_ (is, num);
                                            num++;
                                        }
                                }
                        }
...
template <typename TK>
void
    Output_Mulliken<TK>::collect_MW (ModuleBase::matrix& MecMulP, const ModuleBase::ComplexMatrix& mud, int nw, int isk)
{
...
template <typename TK>
void
    Output_Mulliken<TK>::print_atom_mag (const std::vector<std::vector<double>>& atom_chg, std::ostream& os)
{
    int nat = this->cell_index_->get_nat ();
    ...
    if (this->nspin_ == 2)
    ...
    else if (this->nspin_ == 4)
        {
            ...
            for (int iat = 0; iat < nat; ++iat)
                {
                    ...
                    magnitude[iat]
                        = std::sqrt (mag_x[iat] * mag_x[iat] + mag_y[iat] * mag_y[iat] + mag_z[iat] * mag_z[iat]);
            ...
            const std::vector<std::string> title_polar
...
template <typename TK>
std::vector<std::vector<double>>
    Output_Mulliken<TK>::get_atom_mulliken (std::vector<std::vector<double>>& atom_chg)
{
    int nat = this->cell_index_->get_nat ();
...
template <>
void
    Output_Mulliken<std::complex<double>>::cal_orbMulP ()
{
...
template <>
void
    Output_Mulliken<double>::cal_orbMulP ()
{
