...
template <>
double*
    Output_Sk<double>::get_Sk (int ik)
{
...
template <>
std::complex<double>*
    Output_Sk<std::complex<double>>::get_Sk (int ik)
{
    if (ik < 0 || ik >= this->nks_)
        {
            ModuleBase::WARNING_QUIT ("Output_Sk::get_sk", "ik out of range");
    ...
    if (this->nspin_ == 4)
        {
            dynamic_cast<hamilt::HamiltLCAO<std::complex<double>, std::complex<double>>*> (this->p_hamilt_)
                ->updateSk (ik, 1);
            return dynamic_cast<hamilt::HamiltLCAO<std::complex<double>, std::complex<double>>*> (this->p_hamilt_)
                ->getSk ();
        }
    else
        {
