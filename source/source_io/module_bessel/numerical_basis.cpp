...
//                                 PARAM.inp.bessel_nao_smooth, PARAM.inp.bessel_nao_sigma,
//                                 PARAM.globalv.bessel_nao_rcut, PARAM.inp.bessel_nao_tolerence, ucell);
...
void
    Numerical_Basis::output_overlap (const psi::Psi<std::complex<double>>& psi,
                    ...
                    if (PARAM.inp.out_spillage == 2)
                        {
                            ...
                            overlap_Sq[ik] = this->cal_overlap_Sq (ik,
                                                                   npw,
                            ...
                            overlap_Sq[ik] = NumericalBasis::cal_overlap_Sq (type,
                                                                             ucell.lmaxmax,
                                                                             this->bessel_basis.get_ecut_number (),
                                                                             bessel_nao_rcut,
                                                                             tau_cart,
...
ModuleBase::ComplexArray
    Numerical_Basis::cal_overlap_Q (const int& ik,
                                    const int& np,
                                    const ModulePW::PW_Basis_K* wfcpw,
                    ...
                    for (int L = 0; L < ucell.atoms[T].nwl + 1; L++)
                        {
                            GlobalV::ofs_running << " " << std::setw (5) << ik + 1 << std::setw (8)
                                                 << ucell.atoms[T].label << std::setw (8) << I + 1 << std::setw (8) << L
                                                 << std::endl;
                            // OUT("l",l);
                            std::complex<double> lphase
                                = normalization
                                  * pow (ModuleBase::IMAG_UNIT, -L); // Peize Lin add normalization 2015-12-29
                                                    ...
                                                    for (int ig = 0; ig < np; ig++)
                                                        {
                                                            ...
                                                            //                              * ylm(lm, ig) * flq[ig];
                                                            const std::complex<double> local_tmp
                                                                = lphase * sk[ig] * ylm (lm, ig) * flq (L, ie, ig)
                                                                  * gpow[ig]; // Peize Lin add for dpsi 2020.04.23
                                                            overlap_tmp += conj (local_tmp)
                                                                           * psi (ib, ig); // psi is bloch orbitals
                                                        }
...
ModuleBase::ComplexArray
    Numerical_Basis::cal_overlap_Sq (const int& ik,
                                     const int& np,
                                     const double derivative_order,
                                     const Structure_Factor& sf,
                                     const ModulePW::PW_Basis_K* wfcpw,
                                     const UnitCell& ucell) const
{
    ...
    GlobalV::ofs_running << "\n " << std::setw (5) << "ik" << std::setw (8) << "Type1" << std::setw (8) << "Atom1"
                         << std::setw (8) << "L1" << std::setw (8) << "Type2" << std::setw (8) << "Atom2"
                         << std::setw (8) << "L2" << std::endl;
                                    ...
                                    for (int l1 = 0; l1 < ucell.atoms[T1].nwl + 1; l1++) // 1.3
                                        {
                                            const std::complex<double> lphase1
                                                = normalization
                                                  * pow (ModuleBase::IMAG_UNIT,
                                                         l1); // Peize Lin add normalization 2015-12-29
                                            for (int l2 = 0; l2 < ucell.atoms[T2].nwl + 1; l2++) // 2.3
                                                {
                                                    GlobalV::ofs_running
                                                        << " " << std::setw (5) << ik + 1 << std::setw (8)
                                                        << ucell.atoms[T1].label << std::setw (8) << I1 + 1
                                                        << std::setw (8) << l1 << std::setw (8) << ucell.atoms[T2].label
                                                        << std::setw (8) << I2 + 1 << std::setw (8) << l2
                                                        << std::setw (8) << std::endl;

                                                    const std::complex<double> lphase2
                                                        = pow (ModuleBase::IMAG_UNIT, l2);
                                                                            ...
                                                                            for (int ig = 0; ig < np; ig++)
                                                                                {
                                                                                    about_ig1[ig]
                                                                                        = conj (lphase1 * sk1[ig]
                                                                                                * ylm (lm1, ig))
                                                                                          * gpow[ig]; // Peize Lin add
                                                                                                      // for dpsi
                                                                                                      // 2020.04.23
                                                                                }

                                                                            for (int m2 = 0; m2 < 2 * l2 + 1;
                                                                                 m2++) // 2.6
                                                                                {
                                                                                    ...
                                                                                    /* same as:
                                                                                    for (int ig=0; ig<np; ig++)
                                                                                        for (int ie1=0; ie1 < enumber;
                                                                                    ie1++) for (int ie2=0; ie2 <
                                                                                    enumber; ie2++) overlap_Sq( iwt1,
                                                                                    iwt2, ie1, ie2) += about_ig2[ig] *
                                                                                    flq(l1,ie1,ig) * flq(l2,ie2,ig);
                                                                                    */

                                                                                    ModuleBase::ComplexMatrix
                                                                                        about_ig3_1 (enumber, np);
                                                                                    std::copy (&flq (l1, 0, 0),
                                                                                               &flq (l1, 0, 0)
                                                                                                   + enumber * np,
                                                                                               about_ig3_1.c);
                                                                                    ...
                                                                                    ModuleBase::ComplexMatrix
                                                                                        about_ig3_2 (enumber, np);
                                                                                    for (int ie2 = 0; ie2 < enumber;
                                                                                         ++ie2)
                                                                                        {
                                                                                            std::transform (
                                                                                                &flq (l2, ie2, 0),
...
ModuleBase::matrix
    Numerical_Basis::cal_overlap_V (const ModulePW::PW_Basis_K* wfcpw,
                                    const psi::Psi<std::complex<double>>& psi,
                                    const double derivative_order,
                                    const K_Vectors& kv,
                                    const double tpiba)
...
ModuleBase::realArray
    Numerical_Basis::cal_flq (const std::vector<ModuleBase::Vector3<double>>& gk, const int ucell_lmax) const
{
...
ModuleBase::matrix
    Numerical_Basis::cal_ylm (const std::vector<ModuleBase::Vector3<double>>& gk, const int ucell_lmax)
{
...
std::vector<double>
    Numerical_Basis::cal_gpow (const std::vector<ModuleBase::Vector3<double>>& gk, const double derivative_order)
{
...
std::vector<ModuleBase::IntArray>
    Numerical_Basis::init_mu_index (const UnitCell& ucell)
{
    ...
    for (int it = 0; it < ucell.ntype; it++)
        {
            mu_index_[it].create (ucell.atoms[it].na,
...
void
    Numerical_Basis::numerical_atomic_wfc (const int& ik,
                                           const ModulePW::PW_Basis_K* wfcpw,
                            ...
                            for (int ic = 0; ic < ucell.atoms[it].l_nchi[l]; ic++)
                                {
                                    ...
                                    for (int ig = 0; ig < np; ig++)
                                        {
                                            flq[ig] = this->bessel_basis.Polynomial_Interpolation (it,
                                                                                                   l,
                                                                                                   ic,
                                                                                                   gk[ig].norm ()
                                                                                                       * ucell.tpiba);
...
void
    Numerical_Basis::output_info (std::ofstream& ofs,
            ...
            for (int it = 0; it < ucell.ntype; it++)
                {
                    ...
                    for (int ia = 0; ia < ucell.atoms[it].na; ia++)
                        {
                            ofs << ucell.atoms[it].tau[ia].x << " " << ucell.atoms[it].tau[ia].y << " "
...
void
    Numerical_Basis::output_k (std::ofstream& ofs, const K_Vectors& kv)
{
    // (1)
    if (GlobalV::MY_RANK == 0)
        {
...
void
    Numerical_Basis::output_overlap_Q (std::ofstream& ofs,
                                       ...
                                       const K_Vectors& kv)
{
    ...
    */
    ...
    for (int ik = 0; ik < kv.get_nks (); ++ik)
        {
            std::memcpy (overlap_Q_k.ptr + ik * overlap_Q[ik].getSize (),
                         overlap_Q[ik].ptr,
    ...
    for (int ik = 0; ik < nkstot; ik++)
        {
            ModuleBase::ComplexArray Qtmp (overlap_Q[ik].getBound1 (),
                                           overlap_Q[ik].getBound2 (),
                                           overlap_Q[ik].getBound3 ());
            Qtmp.zero_out ();
...
void
    Numerical_Basis::output_overlap_Sq (const std::string& name,
                                        std::ofstream& ofs,
                                    ...
                                    for (int i = 0; i < size; i++)
                                        {
                                            ...
                                            ofs << " " << overlap_Sq[ik_now].ptr[i].real () << " "
                                                << overlap_Sq[ik_now].ptr[i].imag ();
                                            ++count;
                                        }
                                    ofs.flush ();
                                    ofs.close ();
...
// Peize Lin add 2020.04.23
void
    Numerical_Basis::output_overlap_V (std::ofstream& ofs, const ModuleBase::matrix& overlap_V)
{
    if (GlobalV::MY_RANK == 0)
        {
            ofs << "\n<OVERLAP_V>" << std::endl;
