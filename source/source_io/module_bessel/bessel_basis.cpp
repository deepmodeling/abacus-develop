...
void
    Bessel_Basis::init (const bool start_from_file,
...
double
    Bessel_Basis::Polynomial_Interpolation2 (const int& l, const int& ie, const double& gnorm) const
{
    ...
    */
    ...
    const double y
        = this->TableOne (l, ie, iq) * x1 * x2 * x3 / 6.0 + this->TableOne (l, ie, iq + 1) * x0 * x2 * x3 / 2.0
          - this->TableOne (l, ie, iq + 2) * x1 * x0 * x3 / 2.0 + this->TableOne (l, ie, iq + 3) * x1 * x2 * x0 / 6.0;
    return y;
}

double
    Bessel_Basis::Polynomial_Interpolation (const int& it, const int& l, const int& ic, const double& gnorm) const
{
    const double position = gnorm / this->Dk;
    const int iq = static_cast<int> (position);
    assert (iq < kmesh - 4);
    ...
    const double x3 = 3.0 - x0;
    const double y
        = this->Faln (it, l, ic, iq) * x1 * x2 * x3 / 6.0 + this->Faln (it, l, ic, iq + 1) * x0 * x2 * x3 / 2.0
          - this->Faln (it, l, ic, iq + 2) * x1 * x0 * x3 / 2.0 + this->Faln (it, l, ic, iq + 3) * x1 * x2 * x0 / 6.0;
    return y;
}

void
    Bessel_Basis::init_Faln (const int& ntype,
                             const int& lmax,
                             const int& nmax,
                            ...
                            for (int ie = 0; ie < ecut_number; ie++)
                                {
                                    for (int ik = 0; ik < this->kmesh; ik++)
                                        {
                                            this->Faln (it, il, in, ik)
                                                += this->C4 (it, il, in, ie) * this->TableOne (il, ie, ik);
                                        }
                                }
                            nwfc += 2 * il + 1;
                        }
...
void
    Bessel_Basis::init_TableOne (const bool smooth_in,   // mohan add 2009-08-28
    ...
    for (int l = 0; l < lmax + 1; l++)
        {
            switch (l)
                {
                case 0:
                    ofs << std::setiosflags (std::ios::left) << std::setw (28) << "Number of Sorbitals-->"
                        << ecut_number << std::endl;
                    break;
                case 1:
                    ofs << std::setiosflags (std::ios::left) << std::setw (28) << "Number of Porbitals-->"
                        << ecut_number << std::endl;
                    break;
                case 2:
                    ofs << std::setiosflags (std::ios::left) << std::setw (28) << "Number of Dorbitals-->"
                        << ecut_number << std::endl;
                    break;
                case 3:
                    ofs << std::setiosflags (std::ios::left) << std::setw (28) << "Number of Forbitals-->"
                        << ecut_number << std::endl;
                    break;
                default:
                    ofs << std::setiosflags (std::ios::left) << std::setw (28) << "Number of Gorbitals-->"
                        << ecut_number << std::endl;
                }
        }
    ofs << "---------------------------------------------------------------------------" << std::endl;
            ...
            for (int ie = 0; ie < ecut_number; ie++)
                {
                    ...
                    // caoyu add 2021-3-10
                    //=========output .orb format=============
                    ofs << std::setiosflags (std::ios::right) << std::setw (20) << "Type" << std::setw (20) << "L"
                        << std::setw (20) << "N" << std::endl;
                    ofs << std::setiosflags (std::ios::right) << std::setw (20) << "0" << std::setw (20) << l
                        << std::setw (20) << ie << std::endl;
                    for (int ir = 0; ir < rmesh; ir++)
                        {
                            ofs << std::setiosflags (std::ios::scientific) << std::setprecision (12) << jle[ir] << " ";
                            if ((ir + 1) % 4 == 0)
                                {
                                    ofs << std::endl;
                                }
                    ...
                    //			for(int ir=0; ir<rmesh; ir++) ofs << r[ir] << " " << jle[ir] << " " << jle[ir]*g[ir] <<
                    // std::endl;
                    ...
                    for (int ik = 0; ik < kmesh; ik++)
                        {
                            ...
                            // make table value
                            ModuleBase::Integral::Simpson_Integral (rmesh,
                                                                    function.data (),
                                                                    rab.data (),
                                                                    this->TableOne (l, ie, ik));
                        }
...
void
    Bessel_Basis::readin_C4 (const std::string& name,
                             const int& ntype,
                             const int& ecut,
                             const int& rcut,
                             const int& ecut_number,
                    ...
                    for (int il = 0; il < ucell.atoms[it].nwl + 1; il++)
                        {
                            for (int in = 0; in < ucell.atoms[it].l_nchi[il]; in++)
                                {
                                    // for tests
                                    // std::cout << "\n" << std::setw(5) << it << std::setw(5) << il << std::setw(5) <<
                                    // in; std::cout << "\n file=" << filec4;
                                            ...
                                            for (int ichi = 0; ichi < total_nchi; ichi++)
                                                {
                                                    ...
                                                    // std::cout << "\n Find T=" << tmp_type << " L=" << tmp_l << " N="
                                                    // << tmp_n;

                                                    if (tmp_l == il && tmp_n == in)
                                                        // if(tmp_type == it && tmp_l == il && tmp_n == in) // mohan
                                                        // modify 2009-11-29
                                                        {
                                                            find = true;
                                                            for (int ie = 0; ie < ecut_number; ie++)
                                                                {
                                                                    ...
                                                                    // for tests
                                                                    // std::cout << "\n" << std::setw(5) << ie <<
                                                                    // std::setw(25) << this->C4(it, il, in, ie);
                                                                }
                                                        }
                                                    else
                                                        {
                                    ...
                                    if (!find)
                                        {
                                            std::cout << "\n T=" << it << " L=" << il << " N=" << in;
                                            ModuleBase::WARNING_QUIT ("Bessel_Basis::readin_C4",
                                                                      "Can't find needed c4!");
                                        }
                                    inc4.close ();
                                }
                        }
...
void
    Bessel_Basis::allocate_C4 (const int& ntype,
                               const int& lmax,
                               const int& nmax,
                               const int& ecut_number,
                               const UnitCell& ucell)
