...
class Bessel_Basis
{
  public:
    ...
    /// @details Used for a specific group of C4 coefficients. 2021-01-04, mohan added a new input parameter lmax_in, if
    /// we only generate numerical atomic orbitals based on spherical Bessel functions, lmax_in = ucell.lmax. However,
    /// if we want to generate Spherical Bessel functions (SBF) for descriptor, then the lmax_in is controlled by user.
    ...
    /// @param smooth whether smooth SBFs when perform integration to calculate value of matrix element of TableOne. For
    /// details, see J. Phys.: Condens. Matter 22 (2010) 445501
    ...
    void init (const bool start_from_file,
    ...
    /// @details atomic orbital is constructed always with not only one set of SBFs. For different sets, they are marked
    /// with different `chi`(s), similar with concept of contracted GTOs. For one `chi`, it is 'q' the summation index,
    /// and q is in SBFs like: j_l(q*r), where l is the order of SBF.
    /// @return number of SBFs
    const int&
        get_ecut_number () const
    {
        return Ecut_number;
    }
  ...
  private:
    ...
    /// @brief whether smooth SBFs around cutoff radius, resulting in non-zero values. For importance of smooth of SBFs,
    /// see J. Phys.: Condens. Matter 22 (2010) 445501, eqn 6. (mohan add 2009-01-18)
    bool smooth = false;
    ...
    void
        allocate_C4 (const int& ntype, const int& lmax, const int& nmax, const int& ecut_number, const UnitCell& ucell);

    /// @brief Read C4 from external file. Presently an O(N^2) search algorithm is used. A HTML parser is needed in the
    ...
    void readin_C4 (const std::string& name,
                    const int& ntype,
                    const int& ecut,
                    const int& rcut,
    ...
    /// @brief calculate F_{aln}(it, il, in, ik) = sum_{ie}{C4(it, il, in, ie)*TableOne(il, ie, ik)}, where TableOne is
    /// overlap integral between two spherical bessel functions (jle(r) and jlk(r))
    ...
    /// @details (be called in Bessel_Basis::init(), used for outputing overlap Q matrix) initialize the table whose
    /// matrix element is the result of integral int{dr r^2 jle(r)*jlk(r)}, TableOne has three subscript (l, ie, ik),
    /// the first runs over orbitals' angular momentum and ie, ik run over ecut_number and kmesh SBFs
    ...
    void init_TableOne (const bool smooth_in,
