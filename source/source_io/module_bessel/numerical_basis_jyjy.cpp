...
std::vector<index_t>
    indexgen (const std::vector<int>& natom, const std::vector<int>& lmax)
{
#ifdef __DEBUG
...
ModuleBase::ComplexArray
    cal_overlap_Sq (const char type,
                    const int lmax,
                    const int nbes,
                    const double rcut,
                                    ...
                                    for (int zeta2 = 0; zeta2 < nbes; ++zeta2)
                                        {
                                            ...
                                            overlap_Sq (it1 - mu_index.cbegin (),
                                                        it2 - mu_index.cbegin (),
                                                        zeta1,
                                                        zeta2) += elem;
                                        }
...
std::vector<Vec3>
    neighbor_vec (const Vec3& d0, const ModuleBase::Matrix3& latvec, const double r)
{
