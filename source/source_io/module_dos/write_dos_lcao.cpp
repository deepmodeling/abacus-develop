...
template <typename T>
void write_dos_lcao(const psi::Psi<T>* psi,
    ...
    prepare_dos(ofs_running, energy_fermi, ekb, kv.get_nks(), nbands, dos_edelta_ev, dos_scale, emax, emin);
    ...
    if (PARAM.inp.out_dos == 2) {
        cal_pdos(psi, p_ham, pv, ucell, kv, nspin0, nbands, ekb, emax, emin, dos_edelta_ev, bcoeff);
    }
...
template void write_dos_lcao(const psi::Psi<double>* psi,
...
template void write_dos_lcao(const psi::Psi<std::complex<double>>* psi,
