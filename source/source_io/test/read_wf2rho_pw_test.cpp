...
Symmetry_rho::~Symmetry_rho () {}
void
    Symmetry_rho::begin (const int& spin_now,
                         const Charge& CHR,
...
void
    cal_ik2iktot (std::vector<int>& ik2iktot, const int& nks, const int& nkstot)
{
    if (PARAM.inp.kpar == 1)
        {
    ...
    if (GlobalV::MY_RANK == 0)
        {
            ...
            for (int ik = 0; ik < nkstot; ++ik)
                {
                    ofs << " spin=" << is + 1 << " k-point=" << ik + 1 << "/" << nkstot
                        << " Cartesian=" << kv->kvec_c[ik].x << " " << kv->kvec_c[ik].y << " " << kv->kvec_c[ik].z
                        << " (" << kv->ngk[ik] << " plane wave)" << std::endl;
    ...
    if (GlobalV::NPROC == 1)
    ...
    else if (GlobalV::NPROC == 4)
        {
            const std::vector<double> ref
                = {8207.849135313403, 35.34776105132742, 8207.849135313403, 35.34776105132742};
