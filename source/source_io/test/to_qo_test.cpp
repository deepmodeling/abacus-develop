...
#endif
void
    output::printM3 (std::ofstream& ofs, const std::string& description, const ModuleBase::Matrix3& m)
{
}

void
    define_fcc_cell (UnitCell& ucell)
{
    ...
    ucell.latvec.e11 = 8.0;
    ucell.latvec.e12 = 8.0;
    ucell.latvec.e13 = 0.0;
    ucell.latvec.e21 = 8.0;
    ucell.latvec.e22 = 0.0;
    ucell.latvec.e23 = 8.0;
    ucell.latvec.e31 = 0.0;
    ucell.latvec.e32 = 8.0;
    ucell.latvec.e33 = 8.0;
...
TEST_F (toQOTest, Constructor)
{
    ...
    toQO tqo ("hydrogen", {"minimal-nodeless", "minimal-nodeless"}, PARAM.input.qo_thr, PARAM.input.qo_screening_coeff);
...
TEST_F (toQOTest, RadialCollectionIndexing)
{
    ...
    tqo.build_ao (ucell.ntype, "./", ucell.pseudo_fn.data (), {}, PARAM.input.qo_thr, GlobalV::ofs_running, 0);
...
TEST_F (toQOTest, ScanSupercellSC1)
{
    ...
    toQO tqo ("hydrogen", {"minimal-nodeless"}, PARAM.input.qo_thr, PARAM.input.qo_screening_coeff);
    std::vector<ModuleBase::Vector3<double>> kvecs_d;
    kvecs_d.push_back (ModuleBase::Vector3<double> (0.0, 0.0, 0.0)); // Gamma point
    tqo.read_structures (&ucell, kvecs_d, 0, 1);
    tqo.build_nao (ucell.ntype, "./", ucell.orbital_fn.data (), 0);
...
TEST_F (toQOTest, Initialize)
{
    ...
    std::vector<ModuleBase::Vector3<double>> kvecs_d;
    kvecs_d.push_back (ModuleBase::Vector3<double> (0.0, 0.0, 0.0)); // Gamma point
    tqo.initialize (PARAM.sys.global_out_dir, "", "", &ucell, kvecs_d, GlobalV::ofs_running, 0, 1);
}
...
TEST_F (toQOTest, CalculateSelfOvlpRMinimal)
{
    ...
    for (int i = 0; i < tqo.nphi (); i++)
        {
            EXPECT_NEAR (tqo.ovlpR (i, i),
                         1.0,
                         1e-3); // this is too tight for 1s orbital, which fluctuates a lot in narrow region
        }
...
TEST_F (toQOTest, BuildHydrogenFull)
{
    define_fcc_cell (ucell);
    PARAM.input.qo_thr = 1e-10;
    toQO tqo ("hydrogen", {"full", "full"}, PARAM.input.qo_thr, PARAM.input.qo_screening_coeff);
    std::vector<ModuleBase::Vector3<double>> kvecs_d;
    kvecs_d.push_back (ModuleBase::Vector3<double> (0.0, 0.0, 0.0)); // Gamma point
    tqo.read_structures (&ucell, kvecs_d, 0, 1);
    PARAM.input.qo_thr = 1e-10;
    ...
    for (int i = 0; i < tqo.nphi (); i++)
        {
            EXPECT_NEAR (tqo.ovlpR (i, i),
                         1.0,
                         5e-4); // this is too tight for 1s orbital, which fluctuates a lot in narrow region
        }
...
TEST_F (toQOTest, OrbitalFilterOut)
{
    ...
    toQO tqo2 ("pswfc", {"s", "s"}, PARAM.input.qo_thr, PARAM.input.qo_screening_coeff);
                    ...
                    for (int z = 0; z < 100; z++)
                        {
                            if (l == 0)
                                {
                                    EXPECT_FALSE (tqo2.orbital_filter_out (it, l, z));
                                }
                            else
                                {
                                    EXPECT_TRUE (tqo2.orbital_filter_out (it, l, z));
                                }
                        }
                }
        }
    // next test with random ordered arranged names of subshell
    toQO tqo3 ("pswfc", {"sfdp", "pdf"}, PARAM.input.qo_thr, PARAM.input.qo_screening_coeff);
    for (int l = 0; l < 100; l++)
        {
            for (int z = 0; z < 100; z++)
                {
    ...
    toQO tqo4 ("pswfc", {"all", "p"}, PARAM.input.qo_thr, PARAM.input.qo_screening_coeff);
    for (int l = 0; l < 100; l++)
        {
            for (int z = 0; z < 100; z++)
                {
                    if (l == 1)
                        {
                            EXPECT_FALSE (tqo4.orbital_filter_out (1, l, z));
                        }
                    else
                        {
                            EXPECT_TRUE (tqo4.orbital_filter_out (1, l, z));
                        }
                    EXPECT_FALSE (tqo4.orbital_filter_out (0, l, z)); // do not filter out anything
                }
        }
    // test szv, which controls both l and zeta
    toQO tqo5 ("szv", {"sdp", "spdfg"}, PARAM.input.qo_thr, PARAM.input.qo_screening_coeff);
...
{
    define_fcc_cell (ucell);
    toQO tqo ("pswfc", {"s", "s"}, PARAM.input.qo_thr, PARAM.input.qo_screening_coeff);
    std::vector<ModuleBase::Vector3<double>> kvecs_d;
    kvecs_d.push_back (ModuleBase::Vector3<double> (0.0, 0.0, 0.0)); // Gamma point
    tqo.read_structures (&ucell, kvecs_d, 0, 1);
...
{
    define_fcc_cell (ucell);
    toQO tqo ("pswfc", {"ps", "s"}, PARAM.input.qo_thr, PARAM.input.qo_screening_coeff);
    std::vector<ModuleBase::Vector3<double>> kvecs_d;
    kvecs_d.push_back (ModuleBase::Vector3<double> (0.0, 0.0, 0.0)); // Gamma point
    tqo.read_structures (&ucell, kvecs_d, 0, 1);
    tqo.build_ao (ucell.ntype,
...
{
    define_fcc_cell (ucell);
    toQO tqo ("pswfc", {"all", "p"}, PARAM.input.qo_thr, PARAM.input.qo_screening_coeff);
    std::vector<ModuleBase::Vector3<double>> kvecs_d;
    kvecs_d.push_back (ModuleBase::Vector3<double> (0.0, 0.0, 0.0)); // Gamma point
    tqo.read_structures (&ucell, kvecs_d, 0, 1);
    tqo.build_ao (ucell.ntype,
...
TEST_F (toQOTest, BuildPswfcAll)
{
    define_fcc_cell (ucell);
    PARAM.input.qo_thr = 1e-10;
    toQO tqo ("pswfc", {"all", "all"}, PARAM.input.qo_thr, PARAM.input.qo_screening_coeff);
    std::vector<ModuleBase::Vector3<double>> kvecs_d;
    kvecs_d.push_back (ModuleBase::Vector3<double> (0.0, 0.0, 0.0)); // Gamma point
    tqo.read_structures (&ucell, kvecs_d, 0, 1);
    tqo.build_ao (ucell.ntype,
...
{
    define_sc_cell (ucell);
    toQO tqo ("pswfc", {"all"}, PARAM.input.qo_thr, PARAM.input.qo_screening_coeff);
    std::vector<ModuleBase::Vector3<double>> kvecs_d;
    kvecs_d.push_back (ModuleBase::Vector3<double> (0.0, 0.0, 0.0)); // Gamma point
    tqo.read_structures (&ucell, kvecs_d, 0, 1);
    tqo.build_nao (ucell.ntype, "./", ucell.orbital_fn.data (), 0);
...
TEST_F (toQOTest, ScanSupercellSC3)
{
    ...
    tqo.scan_supercell (0, 1);
    EXPECT_EQ (tqo.nR (),
               57); // 5*5*5 - 12(edge center) - 8*(8-1)(corner) = 5*5*5 - 12(edge center) - 8*(2*2*2-1)(corner)
...
TEST_F (toQOTest, CalculateOvlpKSlaterGamma)
{
    ...
    PARAM.input.qo_thr = 1e-10;
    PARAM.input.qo_screening_coeff = {0.1};
    toQO tqo ("hydrogen", {"energy-full", "energy-full"}, PARAM.input.qo_thr, PARAM.input.qo_screening_coeff);
    std::vector<ModuleBase::Vector3<double>> kvecs_d;
    kvecs_d.push_back (ModuleBase::Vector3<double> (0.0, 0.0, 0.0)); // Gamma point
    tqo.initialize (PARAM.sys.global_out_dir, "", "", &ucell, kvecs_d, GlobalV::ofs_running, 0, 1);
    tqo.calculate_ovlpk (0);
