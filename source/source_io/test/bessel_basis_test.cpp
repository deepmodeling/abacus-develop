...
double
    SimpsonIntegral (const std::vector<double>& x, const std::vector<double>& y)
{
    double result = 0.0;
...
double
    SphericalBessel (int l, double x)
{
...
std::vector<double>
    CalculateSphericalBessel (int l, double q, const std::vector<double>& r)
{
...
std::vector<double>
    GetSphericalBesselZeros (int order, int number)
{
...
/// @details first solve the equation j_l(x) = 0, therefore get the table (l, k) -> x, where l is the order of SBF and k
/// is the k-th zero of j_l(x). Then let x = q*rcut, therefore q = x/rcut, return it.
/// @attention this function itself is a COMPLETE version, while the function it called GetSphericalBesselZeros may be
/// INCOMPLETE, due to limited support of numerical table.
/// @param order the angular momentum of Spherical Bessel functions
/// @param number number of q to be returned
/// @param rcut 'cutoff radius' of Spherical Bessel functions. When r=rcut, Spherical Bessel functions are zero, and for
/// r>rcut, they are zero as required by concept of constructing localized atomic orbital.
/// @return a vector of q, the q is from j_l(qr)
std::vector<double>
    GetqList (int order, int number, double rcut)
{
...
/// @return `std::vector<std::vector<std::vector<double>>>` TableOne[angular momentum][index of Spherical Bessel
/// function][Arbitrary wave vector k]
std::vector<std::vector<std::vector<double>>>
    GenerateTableOne (const bool smooth,
                      const double sigma,
...
/// @brief Improved version of source_io/bessel_basis::readin_C4 and allocate_C4 functions, for generating C4 matrix but
/// now with a higher speed on accessing elements
/// @attention function will read in total number of chi-s both from file and from input, and assert that they are the
/// same. It is also just a INCOMPLETE version, for a complete version, HTML parser library will be included and the
/// other parameter, NumAtomType, will also be used for calibrating data.
...
std::unordered_map<std::string, double>
    ReadinC4 (const std::string& FileName,
                    ...
                    for (int indexchi = 0; indexchi < TotalNumChi; indexchi++)
                        {
                            ...
                            for (int indexNumBesselFunction = 0; indexNumBesselFunction < NumBesselFunction;
                                 indexNumBesselFunction++)
                                {
...
/// @param NumChi number of contracted spherical bessel functions to fit one atomic orbital
/// @param NumBesselFunction number of spherical bessel functions in one contracted spherical bessel function
/// @param vvv_d_TableOne Integral table, has subscripts (l, q, k), whose element is the result of integral int{dr r^2
/// jle(r)*jlk(r)}. l runs over angular momentum, q runs over all spherical bessel functions, k runs over k points
/// sampled controlled by ecutwfc and dk by sqrt(ecutwfc)/dk + 1 + 4.
/// @return F_{alpha, l, chi, k} matrix, whose element is the result of sum_{q}{C4(alpha, l, chi, q)*TableOne(l, q, k)}
std::vector<std::vector<std::vector<std::vector<double>>>>
    GenerateFaln (const std::string& FileName,
                  const int& NumAtomType,
                  const int& lmax,
                  const int& NumChi,
                  const int& NumBesselFunction,
...
{
    std::unordered_map<std::string, double> umap_str_d_C4Map
        = ReadinC4 (FileName, NumAtomType, lmax, NumChi, NumBesselFunction);
                    ...
                    for (int indexChi = 0; indexChi < NumChi; indexChi++)
                        {
                            ...
                            for (int indexk = 0; indexk < vvv_d_TableOne[0][0].size (); indexk++)
                                {
                                    double Faln = 0.0;
                                    for (int indexBesselFunction = 0; indexBesselFunction < NumBesselFunction;
                                         indexBesselFunction++)
                                        {
                                            std::string key = std::to_string (indexAtomType) + " "
                                                              + std::to_string (indexl) + " "
                                                              + std::to_string (indexChi) + " "
                                                              + std::to_string (indexBesselFunction);
                                            Faln += umap_str_d_C4Map[key]
                                                    * vvv_d_TableOne[indexl][indexBesselFunction][indexk];
                                        }
...
/*
 * - Tested Functions:
 *   - InitTest: Bessel_Basis::init(start_from_file, ecutwfc, ntype, lmax_in, smooth, sigma, rcut_in, tol_in, ucell, dk,
 * dr)
 *     - initialize Bessel_Basis class object with following steps:
 *     - call init_TableOne(...): calculate TableOne (l, ie, ik) = int{dr r^2 jle(r)*jlk(r)} (jle(r) and jlk(r) are
 ...
 *   - PolynomialInterpolation2Test: Bessel_Basis::Polynomial_Interpolation2(const int &l, const int &ie, const double
 * &gnorm)
 *     - return (cubic spline) interpolated element value of TableOne 3d-matrix
 *   - PolynomialInterpolationTest: Bessel_Basis::Polynomial_Interpolation(const int &it, const int &l, const int &ic,
 * const double &gnorm)
...
TEST_F (TestBesselBasis, PolynomialInterpolation2Test)
{
    ...
    std::vector<std::vector<std::vector<double>>> vvv_d_TableOne
        = GenerateTableOne (b_Smooth, d_SmoothSigma, d_EnergyCutoff, d_CutoffRadius, i_Lmax, d_dr, d_dk);
...
*/
    ...
    std::vector<std::vector<std::vector<double>>> vvv_d_TableOne
        = GenerateTableOne (b_Smooth, d_SmoothSigma, d_EnergyCutoff, d_CutoffRadius, i_Lmax, d_dr, d_dk);
    std::vector<std::vector<std::vector<std::vector<double>>>> vvvv_d_Faln
        = GenerateFaln ("./support/BesselBasis_UnitTest_C4_AtomType0.html", i_Ntype, i_Lmax, 1, i_Nq, vvv_d_TableOne);
