...
class toQO
{
  public:
    ...
    toQO (const std::string& qo_basis,                //< basis of QO, hydrogen or pswfc
          ...
          const std::vector<double>&
              screening_coeffs); //< screening coefficients for pseudowavefunction or Slater screening
    ~toQO ();
    ...
    void build_ao (const int ntype,                             //< number of atom types
                   ...
                   = std::vector<double> (),                    //< screening coefficients of pseudopotentials
    ...
    void build_pswfc (const int ntype,                      //< number of atom types
                      ...
                      const double* const screening_coeffs, //< screening coefficients of pseudopotentials, appears like
                                                            // a factor (exp[-s*r]) scaling the pswfc
                      const double qo_thr,                  //< threshold for QO
                      const int rank);                      //< rank of present processor
    void build_szv ();
    ...
    void read_ovlp (const std::string& dir,   //< directory of output files
                    ...
                    const bool& is_R = false, //< whether it is in real space
                    const int& imat = 0);     //< index of matrix
    /// @brief build bidirectional map indexing for one single RadialCollection object, which is an axis of
    /// two-center-integral table.
    /// @details from (it,ia,l,zeta,m) to index and vice versa
    void radialcollection_indexing (
        const RadialCollection&,                              //< [in] instance of RadialCollection
    ...
    ModuleBase::Vector3<double>
        cal_two_center_vector (ModuleBase::Vector3<double> rij, //< vector connecting atom i and atom j
    ...
    std::vector<ModuleBase::Vector3<int>> scan_supercell_for_atom (int it, //< type of atom i
                                                                   ...
                                                                   int start_it
                                                                   = 0, //< starting scan index of atom type
    ...
    std::string
        strategy (const int itype) const
    {
        return strategies_[itype];
    }
    UnitCell*
        p_ucell () const
    {
        return const_cast<UnitCell*> (p_ucell_);
    }
    RadialCollection*
        p_nao () const
    {
    ...
    double
        ovlpR (const int i, const int j) const
    {
        return ovlpR_[i * nchi_ + j];
    }
    ...
    std::complex<double>
        ovlpk (const int i, const int j) const
    {
        return ovlpk_[i * nchi_ + j];
    }
    std::vector<std::string>
        symbols () const
    {
  ...
  private:
    ...
    std::vector<std::complex<double>>
        ovlpk_; //< overlap between atomic orbitals and numerical atomic orbitals, in k space
    //< indices
