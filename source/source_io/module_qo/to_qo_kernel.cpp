...
void
    toQO::initialize (const std::string& out_dir,
...
void
    toQO::build_nao (const int ntype,
                     const std::string orbital_dir,
    ...
    if (rank == 0)
        {
            ...
            nao_build_info
                += "Orbitals arrange in sequence of (it, ia, l, zeta, m), m in order of 0, 1, -1, 2, -2, ...\n";
            printf ("%s", nao_build_info.c_str ());
        }
}

bool
    toQO::orbital_filter_out (const int& itype, const int& l, const int& izeta)
{
...
void
    toQO::build_hydrogen (const int ntype,
...
void
    toQO::build_pswfc (const int ntype,
                       const std::string pseudo_dir,
                    ...
                    const int rank)
{
    if (qo_basis_ == "hydrogen")
        {
            bool with_slater_screening = std::find_if (screening_coeffs.begin (),
                                                       screening_coeffs.end (),
                                                       [] (double sc) { return sc > 1e-10; })
                                         != screening_coeffs.end ();
...
void
    toQO::calculate_ovlpR (const int iR)
{
            ...
            for (int icol = 0; icol < nphi_; icol++)
                {
                    ...
                    Rij.x = rij.x + double (R.x) * p_ucell_->a1.x + double (R.y) * p_ucell_->a2.x
                            + double (R.z) * p_ucell_->a3.x;
                    Rij.y = rij.y + double (R.x) * p_ucell_->a1.y + double (R.y) * p_ucell_->a2.y
                            + double (R.z) * p_ucell_->a3.y;
                    Rij.z = rij.z + double (R.x) * p_ucell_->a1.z + double (R.y) * p_ucell_->a2.z
                            + double (R.z) * p_ucell_->a3.z;
                    Rij *= p_ucell_->lat0; // convert to Bohr
                    overlap_calculator_
                        ->calculate (it, li, izeta, mi, jt, lj, jzeta, mj, Rij, &ovlpR_[irow * nphi_ + icol]);
                }
        }
}

void
    toQO::calculate_ovlpk (int ik)
{
...
void
    toQO::append_ovlpR_eiRk (int ik, int iR)
{
...
void
    toQO::allocate_ovlp (const bool& is_R)
{
    if (is_R)
        {
...
void
    toQO::deallocate_ovlp (const bool& is_R)
{
    if (is_R)
        {
            ovlpR_.clear ();
...
void
    toQO::zero_out_ovlps (const bool& is_R)
{
    if (is_R)
        {
...
void
    toQO::radialcollection_indexing (const RadialCollection& radcol,
...
template <typename T>
void
    toQO::write_ovlp (const std::string& dir,
    ...
    if (is_R)
        {
            ofs << "SUPERCELL_COORDINATE: " << std::setw (5) << std::right << supercells_[i].x << " " << std::setw (5)
                << std::right << supercells_[i].y << " " << std::setw (5) << std::right << supercells_[i].z
                << std::endl;
        }
    else
        {
    ...
    for (int irow = 0; irow < nrows; irow++)
        {
            for (int icol = 0; icol < ncols; icol++)
                {
                    ofs << std::setw (22) << std::setprecision (14) << std::right << std::scientific
                        << ovlp[irow * ncols + icol] << " ";
                }
            ofs << std::endl;
        }
    ofs.close ();
...
std::complex<double>
    str2complex (const std::string& str)
{
...
void
    toQO::read_ovlp (const std::string& dir, const int& nrows, const int& ncols, const bool& is_R, const int& ik)
{
    zero_out_ovlps (is_R); // clear the ovlp vector before reading
    assert (nrows * ncols == nchi_ * nphi_);
    std::string filename
        = is_R ? "QO_ovlpR_" + std::to_string (ik) + ".dat" : "QO_ovlp_" + std::to_string (ik) + ".dat";
