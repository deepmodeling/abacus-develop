...
#endif
void
    toQO::read_structures (const UnitCell* p_ucell,
                           ...
                           const int& nranks)
{
    ...
    std::for_each (p_ucell->atoms,
                   p_ucell->atoms + p_ucell->ntype,
                   [this] (Atom& atom)
                       {
    ...
    for (int itype = 0; itype < ntype_; itype++)
        {
            ...
            nmax_[itype] = (strategies_[itype].substr (0, 6) != "energy")
                               ? atom_database_.principle_quantum_number[symbols_[itype]]
                               : atom_database_.atom_Z[symbols_[itype]];
            ...
            for (int i = 0; i < nranks; i++)
                {
                    ...
                    if ((i >= nks_remain) && (nks_remain > 0))
                        nks_this_rank_indices.push_back (-1);
    ...
    for (int i = 0; i < nranks; i++)
        {
            int nks_dim = 0;
            if (iproc_ == 0)
                nks_dim = nks_divided[i].size ();
            Parallel_Common::bcast_int (nks_dim);
            if (iproc_ != 0)
                nks_divided[i].resize (nks_dim);
    ...
    for (int i = 0; i < nks_divided[rank].size (); i++)
        {
            if (nks_divided[rank][i] != -1)
                iks_.push_back (nks_divided[rank][i]); // we want it is explicitly -1
            else
                iks_.push_back (-1);
        }
    nks_ = iks_.size ();
...
#endif
    if (rank == 0)
        printf ("toQO KPOINTS parallelization: calculation of S(k) will be parallelized on %d processes\n", nranks);
#ifdef __MPI
...
template <typename T>
void
    toQO::eliminate_duplicate_vector3 (std::vector<ModuleBase::Vector3<T>>& v)
{
...
std::vector<ModuleBase::Vector3<int>>
    toQO::scan_supercell_for_atom (int it, int ia, int start_it, int start_ia)
{
            ...
            for (int iatom = start_ia; iatom < p_ucell_->atoms[itype].na; iatom++)
                {
                    ...
                    ModuleBase::Vector3<double> rij
                        = p_ucell_->atoms[itype].tau[iatom] - p_ucell_->atoms[it].tau[ia]; // in unit lat0?
                    int n1 = 0;
                    int n2 = 0;
                    int n3 = 0;
...
double
    cosine_between_vector3 (ModuleBase::Vector3<double> v1, ModuleBase::Vector3<double> v2)
{
    double f = v1 * v2;
    f /= v1.norm ();
    f /= v2.norm ();
...
std::vector<int>
    toQO::rcut_to_supercell_index (double rcut,
...
double
    toQO::norm2_rij_supercell (ModuleBase::Vector3<double> rij, int n1, int n2, int n3)
{
    double f = rij * rij;
...
void
    toQO::scan_supercell (const int& rank, const int& nranks)
{
    if (rank == 0)
        {
    ...
    if (rank == 0)
        {
            // divide nR into std::vector of std::vector<int>, each std::vector<int> is a chunk of indices of
            // supercells_
            int nRs = nR_;
    ...
    for (int i = 0; i < nranks; i++)
        {
            int nR_dim = 0;
            if (rank == 0)
                nR_dim = nR_divided[i].size ();
            Parallel_Common::bcast_int (nR_dim);
            if (rank != 0)
                nR_divided[i].resize (nR_dim);
            Parallel_Common::bcast_int (nR_divided[i].data (), nR_dim);
        }
    iRs_.clear ();
    for (int i = 0; i < nR_divided[rank].size (); i++)
        iRs_.push_back (nR_divided[rank][i]);
    nR_ = iRs_.size ();

    MPI_Barrier (MPI_COMM_WORLD);
#endif
    if (rank == 0)
        printf ("toQO SUPERCELLS parallelization: calculation of S(R) will be parallelized on %d processes\n", nranks);
#ifdef __MPI
    MPI_Barrier (MPI_COMM_WORLD);
    int last_iR_ = nR_ - 1;
...
ModuleBase::Vector3<double>
    toQO::cal_two_center_vector (ModuleBase::Vector3<double> rij, ModuleBase::Vector3<int> R)
{
    ModuleBase::Vector3<double> Rij;
    Rij.x = rij.x + R.x * p_ucell_->a1.x + R.y * p_ucell_->a2.x + R.z * p_ucell_->a3.x;
    Rij.y = rij.y + R.x * p_ucell_->a1.y + R.y * p_ucell_->a2.y + R.z * p_ucell_->a3.y;
    Rij.z = rij.z + R.x * p_ucell_->a1.z + R.y * p_ucell_->a2.z + R.z * p_ucell_->a3.z;
    return Rij;
}

void
    toQO::write_supercells ()
{
    ...
    for (int i = 0; i < supercells_.size (); i++)
        {
            ofs << std::setw (5) << std::right << supercells_[i].x << " " << std::setw (5) << std::right
                << supercells_[i].y << " " << std::setw (5) << std::right << supercells_[i].z << std::endl;
        }
    ofs.close ();
}
