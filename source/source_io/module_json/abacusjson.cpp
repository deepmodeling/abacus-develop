...
void
    AbacusJson::add_nested_member (std::vector<jsonKeyNode>::iterator begin,
                            ...
                            if (parent[key].IsArray ())
                            ...
                            else
                                {
                                    ...
                                    if (parent[key].IsObject ())
                                        {
                                            std::cout
                                                << "Warning: write to json, key " << (*begin).key
                    ...
                    if (keyNode.key.empty () && parent.IsArray ())
                        {
                            ...
                            if (index >= 0)
                                ...
                                {
                                    int arr_size = parent.Size ();
                                    add_nested_member (begin + 1,
                                                       end,
                                                       val,
                                                       parent[arr_size + index],
                                                       allocator,
                                                       IsArray);
                                }
                        }
                            ...
                            if (!parent[key].IsObject () && !parent[key].IsArray ())
                                {
                                    std::cout << "Warning: write to json, key " << (*begin).key
                                              << " exist and is not an object or array, and abacus will add it as a "
                                                 "middle node."
                                              << std::endl;
                                }
...
void
    AbacusJson::write_to_json (std::string filename)
{
...
template <>
void
    AbacusJson::add_json (std::vector<jsonKeyNode> keys, const std::string& value, bool IsArray)
{
    if (!doc.IsObject ())
        {
            doc.SetObject ();
...
template <>
void
    AbacusJson::add_json (std::vector<jsonKeyNode> keys, const rapidjson::Value& value, bool IsArray)
{
