        ...
        const int& precision)
{
    ...
    double eps = 1.0e-5; // suppress the numerical instability in LCAO (Ref: Acta Phys. -Chim. Sin. 2011, 27(12),
                         // 2786-2792. doi: 10.3866/PKU.WHXB20112786)
    for (int is = 0; is < nspin_eff; ++is)
        {
            for (int ir = 0; ir < rho_basis->nrxx; ++ir)
                {
    ...
    if (nspin == 1 || nspin == 4)
    ...
    else if (nspin == 2)
        {
            ...
            for (int ir = 0; ir < rho_basis->nrxx; ++ir)
                {
                    elf_tot[ir]
                        = (tau[0][ir] + tau[1][ir] - tau_vw[0][ir] - tau_vw[1][ir]) / (tau_TF[0][ir] + tau_TF[1][ir]);
