...
void
    cal_r_overlap_R::initialize_orb_table (const UnitCell& ucell, const LCAO_Orbitals& orb)
{
...
void
    cal_r_overlap_R::construct_orbs_and_orb_r (const UnitCell& ucell, const LCAO_Orbitals& orb)
{
    int orb_r_ntype = 0;
                                            ...
                                            for (int NB = 0; NB < orb.Phi[TB].getNchi (LB); ++NB)
                                                {
                                                    center2_orb11[TA][TB][LA][NA][LB].insert (
                                                        std::make_pair (NB,
                                                                        Center2_Orb::Orb11 (orbs[TA][LA][NA],
                                                                                            orbs[TB][LB][NB],
                                                                                            psb_,
                                                                                            MGT)));
                                                }
                                            ...
                                            for (int NB = 0; NB < orb.Phi[TB].getNchi (LB); ++NB)
                                                {
                                                    center2_orb21_r[TA][TB][LA][NA][LB].insert (
                                                        std::make_pair (NB,
                                                                        Center2_Orb::Orb21 (orbs[TA][LA][NA],
                                                                                            orb_r,
                                                                                            orbs[TB][LB][NB],
                                                                                            psb_,
                                                                                            MGT)));
...
void
    cal_r_overlap_R::construct_orbs_and_nonlocal_and_orb_r (const UnitCell& ucell, const LCAO_Orbitals& orb)
{
            ...
            for (int ip = 0; ip < nproj; ip++)
                {
                    ...
                    int nr_uniform = static_cast<int> ((infoNL_.Beta[T].Proj[ip].getRadial (nr - 1)
                                                        - infoNL_.Beta[T].Proj[ip].getRadial (0))
                                                       / dr_uniform)
                                     + 1;
                    ...
                    if (infoNL_.Beta[T].Proj[ip].getRadial (0) > 1e-10)
                        {
                            double slope
                                = (infoNL_.Beta[T].Proj[ip].getBeta_r (1) - infoNL_.Beta[T].Proj[ip].getBeta_r (0))
                                  / (infoNL_.Beta[T].Proj[ip].getRadial (1) - infoNL_.Beta[T].Proj[ip].getRadial (0));
                            Beta_r_uniform[0] = infoNL_.Beta[T].Proj[ip].getBeta_r (0)
                                                - slope * infoNL_.Beta[T].Proj[ip].getRadial (0);
                        }

                    // Here, the operation beta_r / r is performed. To avoid divergence at r=0, beta_r(0) is set to
                    // beta_r(1). However, this may introduce issues, so caution is needed.
                    ...
                    orbs_nonlocal[T][ip].set_orbital_info (
                        infoNL_.Beta[T].getLabel (),
...
void
    cal_r_overlap_R::init (const UnitCell& ucell, const Parallel_Orbitals& pv, const LCAO_Orbitals& orb)
{
...
void
    cal_r_overlap_R::init_nonlocal (const UnitCell& ucell, const Parallel_Orbitals& pv, const LCAO_Orbitals& orb)
{
...
ModuleBase::Vector3<double>
    cal_r_overlap_R::get_psi_r_psi (const ModuleBase::Vector3<double>& R1,
                                    const int& T1,
                                    const int& L1,
                                    const int& m1,
                                    const int& N1,
...
void
    cal_r_overlap_R::get_psi_r_beta (const UnitCell& ucell,
...
void
    cal_r_overlap_R::out_rR (const UnitCell& ucell, const Grid_Driver& gd, const int& istep)
{
                                    ...
                                    if (ic >= 0)
                                        {
                                            ...
                                            // The off-diagonal term in SOC calculaiton is zero, and the two diagonal
                                            // terms are the same
                                            int new_index
                                                = iw1 - PARAM.globalv.npol * orb_index_row
                                            ...
                                            if (new_index == 0 || new_index == 3)
                                                {
                                                    ...
                                                    ModuleBase::Vector3<double> r_distance
                                                        = (ucell.atoms[it2].tau[ia2] - ucell.atoms[it1].tau[ia1]
                                                           + R_car)
                                                          * ucell.lat0;

                                                    double overlap_o
                                                        = center2_orb11[it1][it2][iL1][iN1][iL2].at (iN2).cal_overlap (
                                                            origin_point,
                                                            r_distance,
                                                            im1,
                                                            im2);
                                                    ...
                                                    double overlap_x = -1 * factor
                                                                       * center2_orb21_r[it1][it2][iL1][iN1][iL2]
                                                                             .at (iN2)
                                                                             .cal_overlap (origin_point,
                                                                                           r_distance,
                                                                                           im1,
                                                                                           1,
                                                                                           im2); // m =  1
                            ...
                            if (GlobalV::DRANK == 0)
                                {
                                    if (binary)
                                        {
                                            ofs_tem1.write (reinterpret_cast<char*> (&rR_nonzero_num[direction]),
                                                            sizeof (int));
                                        }
                                    else
                                        {
...
void
    cal_r_overlap_R::out_rR_other (const UnitCell& ucell,
                                   const int& istep,
