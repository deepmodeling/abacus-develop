...
void cal_r_overlap_R::construct_orbs_and_nonlocal_and_orb_r(const UnitCell& ucell, const LCAO_Orbitals& orb) {
    ...
    for (int T = 0; T < orb.get_ntype(); ++T) {
        ...
        for (int ip = 0; ip < nproj; ip++) {
            ...
            int nr_uniform =
                static_cast<int>((infoNL_.Beta[T].Proj[ip].getRadial(nr - 1) - infoNL_.Beta[T].Proj[ip].getRadial(0)) /
                                 dr_uniform) +
                1;
            ...
            if (infoNL_.Beta[T].Proj[ip].getRadial(0) > 1e-10) {
                ...
                Beta_r_uniform[0] =
                    infoNL_.Beta[T].Proj[ip].getBeta_r(0) - slope * infoNL_.Beta[T].Proj[ip].getRadial(0);
            }
...
ModuleBase::Vector3<double> cal_r_overlap_R::get_psi_r_psi(const ModuleBase::Vector3<double>& R1,
    ...
    double overlap_z = factor * center2_orb21_r[T1][T2][L1][N1][L2].at(N2).cal_overlap(origin_point,
                                                                                       distance,
                                                                                       m1,
                                                                                       0,
                                                                                       m2); // m =  0
...
void cal_r_overlap_R::get_psi_r_beta(const UnitCell& ucell,
    ...
    for (int ip = 0; ip < nproj; ip++) {
        ...
        for (int m2 = 0; m2 < 2 * L2 + 1; m2++) {
            ...
            double overlap_z = factor * center2_orb21_r_nonlocal[T1][T2][L1][N1].at(ip).cal_overlap(origin_point,
                                                                                                    distance,
                                                                                                    m1,
                                                                                                    0,
                                                                                                    m2); // m =  0
                ...
                for (int iw2 = 0; iw2 < PARAM.globalv.nlocal; iw2++) {
                    ...
                    if (ic >= 0) {
                        ...
                        if (new_index == 0 || new_index == 3) {
                            ...
                            double overlap_z =
                                factor * center2_orb21_r[it1][it2][iL1][iN1][iL2].at(iN2).cal_overlap(origin_point,
                                                                                                      r_distance,
                                                                                                      im1,
                                                                                                      0,
                                                                                                      im2); // m =  0
