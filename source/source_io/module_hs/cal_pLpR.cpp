...
double
    _lambda_plus (const int l, const int m)
{
...
// L-ylm = sqrt((l+m)(l-m+1))ylm-1, return the sqrt((l+m)(l-m+1))
double
    _lambda_minus (const int l, const int m)
{
    return std::sqrt ((l + m) * (l - m + 1)); // NOTE: complex spherical harmonics
}
...
std::complex<double>
    ModuleIO::cal_LzijR (const std::unique_ptr<TwoCenterIntegrator>& calculator,
                         const int it,
                         const int ia,
                         const int il,
                         const int iz,
...
std::complex<double>
    ModuleIO::cal_LxijR (const std::unique_ptr<TwoCenterIntegrator>& calculator,
                         const int it,
                         const int ia,
                         const int il,
                         const int iz,
...
std::complex<double>
    ModuleIO::cal_LyijR (const std::unique_ptr<TwoCenterIntegrator>& calculator,
                         const int it,
                         const int ia,
                         const int il,
                         const int iz,
...
void
    ModuleIO::AngularMomentumCalculator::kernel (std::ofstream* ofs,
                                                 const UnitCell& ucell,
                                                 const char dir,
                                                 const int precision)
{
                    ...
                    for (int ia_adj = 0; ia_adj < adjinfo.adj_num + 1; ia_adj++) // "+1" is to include itself
                        {
                            ...
                            dtau = ucell.cal_dtau (ucell.itia2iat (it, ia),
                                                   ...
                                                   iR)
                                   * ucell.lat0; // convert to unit of Bohr
...
void
    ModuleIO::AngularMomentumCalculator::calculate (const std::string& prefix,
