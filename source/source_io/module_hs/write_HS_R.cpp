...
template <typename T>
hamilt::HContainer<T>*
sparse_map_to_hcontainer(const std::map<Abfs::Vector3_Order<int>, std::map<size_t, std::map<size_t, T>>>& sparse_map,
...
std::string ModuleIO::hsr_gen_fname(const std::string& prefix, const int ispin, const bool append, const int istep) {
    if (!append && istep >= 0) {
...
std::string ModuleIO::dhr_gen_fname(const std::string& prefix, const int ispin, const bool append, const int istep) {
