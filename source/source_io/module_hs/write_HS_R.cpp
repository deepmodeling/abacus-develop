...
void
    ModuleIO::output_dSR (const int& istep,
...
void
    ModuleIO::output_dHR (const int& istep,
...
void
    ModuleIO::output_SR (Parallel_Orbitals& pv,
...
void
    ModuleIO::output_TR (const int istep,
                         const UnitCell& ucell,
                         const Parallel_Orbitals& pv,
                         LCAO_HS_Arrays& HS_Arrays,
                         const Grid_Driver& grid,
...
std::string
    ModuleIO::hsr_gen_fname (const std::string& prefix, const int ispin, const bool append, const int istep)
{
    if (!append && istep >= 0)
        {
...
std::string
    ModuleIO::dhr_gen_fname (const std::string& prefix, const int ispin, const bool append, const int istep)
{
...
void
    ModuleIO::write_hcontainer_csr (const std::string& fname,
                                    const UnitCell* ucell,
                                    const int precision,
...
template <typename TR>
void
    ModuleIO::write_hsr (const std::vector<hamilt::HContainer<TR>*>& hr_vec,
                         ...
                         const int istep)
{
    ...
    for (int ispin = 0; ispin < nspin; ispin++)
        {
            ...
            if (GlobalV::MY_RANK == 0)
                {
                    std::string fname = PARAM.globalv.global_out_dir + hsr_gen_fname ("hrs", ispin, append, istep);
    ...
    // Output SR (single file)
    {
        ...
        if (GlobalV::MY_RANK == 0)
            {
                std::string fname = PARAM.globalv.global_out_dir + hsr_gen_fname ("srs", 0, append, istep);
...
template void ModuleIO::write_hcontainer_csr<double> (const std::string&,
                                                      const UnitCell*,
                                                      const int,
...
template void ModuleIO::write_hsr<double> (const std::vector<hamilt::HContainer<double>*>&,
...
template <typename TR>
void
    ModuleIO::write_matrix_r (const std::string& matrix_label,
...
template void ModuleIO::write_matrix_r<double> (const std::string&,
                                                const std::string&,
...
template void
    ModuleIO::write_matrix_r<std::complex<double>> (const std::string&,
                                                    const std::string&,
