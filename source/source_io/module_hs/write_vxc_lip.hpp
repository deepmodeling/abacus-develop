...
inline FPTYPE
    get_real (const std::complex<FPTYPE>& c)
{
    return c.real ();
}
template <typename FPTYPE>
inline FPTYPE
    get_real (const FPTYPE& d)
{
    return d;
}
...
template <typename T>
std::vector<T>
    cVc (T* const V, T* const c, int nbasis, int nbands)
{
...
template <typename FPTYPE>
std::vector<std::complex<FPTYPE>>
    psi_Hpsi (std::complex<FPTYPE>* const psi, std::complex<FPTYPE>* const hpsi, const int nbasis, const int nbands)
{
...
template <typename FPTYPE>
std::vector<FPTYPE>
    orbital_energy (const int ik, const int nbands, const std::vector<std::complex<FPTYPE>>& mat_mo)
{
#ifdef __DEBUG
    assert (nbands >= 0);
#endif
...
template <typename FPTYPE>
FPTYPE
    all_band_energy (const int ik, const std::vector<FPTYPE>& orbital_energy, const ModuleBase::matrix& wg)
{
    FPTYPE e = 0.0;
    ...
    for (int ik = 0; ik < kv.get_nks (); ++ik)
        {
            ...
            //     vxcs_op_pw->act(1, psi_pw.get_current_nbas(), psi_pw.npol, psi_single_band.get_pointer(),
            //     hpsi_single_band.get_pointer(), psi_pw.get_ngk(ik));
            // }
            vxcs_op_pw->act (psi_pw.get_nbands (),
                             psi_pw.get_nbasis (),
                             psi_pw.get_npol (),
                             &psi_pw (ik, 0, 0),
                             &hpsi_localxc (ik, 0, 0),
                             psi_pw.get_ngk (ik));
            delete vxcs_op_pw;
            std::vector<T> vxc_local_k_mo
                = psi_Hpsi (&psi_pw (ik, 0, 0), &hpsi_localxc (ik, 0, 0), psi_pw.get_nbasis (), psi_pw.get_nbands ());
            ...
            if (GlobalC::exx_info.info_global.cal_exx)
                {
                    ...
                    // ======test=======
                    // std::cout << "exx_energy from matrix:" << all_band_energy(ik, nbands, vexx_k_mo, wg) <<
                    // std::endl; std::cout << "exx_energy from orbitals: " << all_band_energy(ik, e_orb_exx.at(ik), wg)
                    // << std::endl; std::cout << "exx_energy from exx_lip: " <<
                    // GlobalC::exx_info.info_global.hybrid_alpha * exx_lip.get_exx_energy() << std::endl;
                    // ======test=======
                    container::BlasConnector::axpy (nbands * nbands,
                                                    1.0,
                                                    vexx_k_mo.data (),
                                                    1,
                                                    vxc_tot_k_mo.data (),
