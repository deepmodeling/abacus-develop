...
std::set<Abfs::Vector3_Order<int>>
    get_R_range (const hamilt::HContainer<TR>& hR)
{
...
template <typename T>
std::map<Abfs::Vector3_Order<int>, std::map<size_t, std::map<size_t, T>>>
    cal_HR_sparse (const hamilt::HContainer<T>& hR, const double sparse_thr)
{
