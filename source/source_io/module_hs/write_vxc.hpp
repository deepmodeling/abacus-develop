...
inline void
    set_para2d_MO (const Parallel_Orbitals& pv, const int nbands, Parallel_2D& p2d)
{
    std::ofstream ofs;
#ifdef __MPI
...
template <typename T>
inline std::vector<T>
    cVc (T* V, T* c, const int nbasis, const int nbands, const Parallel_Orbitals& pv, const Parallel_2D& p2d)
{
...
inline double
    get_real (const std::complex<double>& c)
{
    return c.real ();
}

inline double
    get_real (const double& d)
{
    return d;
}
...
template <typename T>
double
    all_band_energy (const int ik, const std::vector<T>& mat_mo, const Parallel_2D& p2d, const ModuleBase::matrix& wg)
{
    double e = 0.0;
...
template <typename T>
std::vector<double>
    orbital_energy (const int ik, const int nbands, const std::vector<T>& mat_mo, const Parallel_2D& p2d)
{
#ifdef __DEBUG
    assert (nbands >= 0);
#endif
...
inline void
    write_orb_energy (const K_Vectors& kv,
                      const int nspin0,
                      const int nbands,
            ...
            if (GlobalC::exx_info.info_global.cal_exx)
                {
                    ...
                    std::vector<TK> vexx_k_mo
                        = cVc (vexxonly_k_ao.get_hk (), &psi (ik, 0, 0), nbasis, nbands, *pv, p2d);
