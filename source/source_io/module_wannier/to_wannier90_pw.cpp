...
void
    toWannier90_PW::calculate (const UnitCell& ucell,
    ...
    if (PARAM.inp.nspin == 2)
        {
            if (wannier_spin == "up")
                ...
                {
                    ModuleBase::WARNING_QUIT ("toWannier90::calculate",
                                              "Error wannier_spin set,is not \"up\" or \"down\" ");
                }
        }
...
void
    toWannier90_PW::set_tpiba_omega (const double& tpiba, const double& omega)
{
    this->tpiba = &tpiba;
    this->omega = &omega;
}
...
void
    toWannier90_PW::cal_Mmn (const psi::Psi<std::complex<double>>& psi_pw, const ModulePW::PW_Basis_K* wfcpw)
{
    ...
    if (GlobalV::MY_RANK == 0)
        {
            ...
            mmn_file << std::setw (12) << num_bands << std::setw (12) << cal_num_kpts << std::setw (12) << nntot
                     << std::endl;
        }
                    ...
                    if (GlobalV::MY_RANK == 0)
                        {
                            mmn_file << std::setw (5) << ik + 1 << std::setw (5) << ikb + 1 << std::setw (5)
                                     << int (phase_G.x) << std::setw (5) << int (phase_G.y) << std::setw (5)
                                     << int (phase_G.z) << std::endl;

                            for (int n = 0; n < num_bands; n++)
                                {
                                    for (int m = 0; m < num_bands; m++)
                                        {
                                            mmn_file << std::setw (18) << std::setprecision (12) << std::showpoint
                                                     << std::fixed << Mmn (m, n).real () << std::setw (18)
                                                     << std::setprecision (12) << std::showpoint << std::fixed
                                                     << Mmn (m, n).imag ()
                                                     // jingan test
...
void
    toWannier90_PW::cal_Amn (const psi::Psi<std::complex<double>>& psi_pw, const ModulePW::PW_Basis_K* wfcpw)
{
                    ...
                    for (int iw = 0; iw < num_wannier; iw++)
                        {
                            for (int ib_w = 0; ib_w < num_bands; ib_w++)
                                {
                                    Amn_file
                                        << std::setw (5) << ib_w + 1 << std::setw (5) << iw + 1 << std::setw (5)
                                        << ik + 1 - start_k_index << std::setw (18) << std::showpoint << std::fixed
                                        << std::setprecision (12) << Amn (ib_w, iw).real () << std::setw (18)
...
void
    toWannier90_PW::out_unk (const psi::Psi<std::complex<double>>& psi_pw,
                             const ModulePW::PW_Basis_K* wfcpw,
                             const ModulePW::PW_Basis_Big* bigpw)
{
                    ...
                    if (GlobalV::RANK_IN_POOL == 0)
                        {
                            ...
                            if (PARAM.inp.nspin == 1 || PARAM.inp.nspin == 4)
                                ...
                                {
                                    if (wannier_spin == "up")
                                        name << PARAM.globalv.global_out_dir << "UNK" << std::setw (5)
                                             << std::setfill ('0') << ik + 1 - start_k_index << ".1";
                                    else if (wannier_spin == "down")
                                        name << PARAM.globalv.global_out_dir << "UNK" << std::setw (5)
                                             << std::setfill ('0') << ik + 1 - start_k_index << ".2";
                                }
                            if (wvfn_formatted)
                                {
                                    unkfile.open (name.str (), std::ios::out);
                                    unkfile << std::setw (12) << wfcpw->nx << std::setw (12) << wfcpw->ny
                                            << std::setw (12) << wfcpw->nz << std::setw (12) << ik + 1 << std::setw (12)
                                            << num_bands << std::endl;
                                }
                            else
                                {
                                    unkfile_b.open (name.str (), "w");
                                    unkfile_b << int (20) << wfcpw->nx << wfcpw->ny << wfcpw->nz << ik + 1 << num_bands
                                              << 20;
                                }
                        }
                            ...
                            if (GlobalV::RANK_IN_POOL == 0)
                                {
                                    if (!wvfn_formatted)
                                        {
                                            unkfile_b
                                                << wfcpw->nz * wfcpw->ny * wfcpw->nx * 8 * 2; // sizeof(double) = 8
                                        }
                                }
                                    ...
                                    if (which_ip[iz] == 0 && GlobalV::RANK_IN_POOL == 0)
                                        {
                                            for (int ir = 0; ir < nxy; ir++)
                                                {
                                                    zpiece[ir]
                                                        = porter[ir * wfcpw->nplane + iz - wfcpw->startz_current];
                                                }
                                        }
...
void
    toWannier90_PW::unkdotkb (const psi::Psi<std::complex<double>>& psi_pw,
                              const ModulePW::PW_Basis_K* wfcpw,
                              const int& cal_ik,
                              const int& cal_ikb,
...
void
    toWannier90_PW::gen_radial_function_in_q (std::vector<ModuleBase::matrix>& radial_in_q)
{
            ...
            if (rvalue[wannier_index] == 2)
                {
                    for (int ir = 0; ir < mesh_r; ir++)
                        {
                            psi[ir]
                                = 1.0 / sqrt (8.0) * alfa32 * (2.0 - alfa_new * r[ir]) * exp (-alfa_new * r[ir] * 0.5);
                        }
                }
...
void
    toWannier90_PW::produce_trial_in_pw (const psi::Psi<std::complex<double>>& psi_pw,
                                         const int& ik,
                                         const ModulePW::PW_Basis_K* wfcpw,
                            ...
                            for (int ig = 0; ig < npw; ig++)
                                {
                                    ...
                                    trial_orbitals_k (wannier_index, ig)
                                        = sf[ig] * (bs2 * orb_s[ig] + tmp_bs2 * orb_px[ig]);
                                }
                                    ...
                                    for (int ig = 0; ig < npw; ig++)
                                        {
                                            ...
                                            trial_orbitals_k (wannier_index, ig)
                                                = sf[ig] * (bs3 * orb_s[ig] + 2.0 * bs6 * orb_px[ig]);
                                        }
                            ...
                            for (int ig = 0; ig < npw; ig++)
                                {
                                    ...
                                    trial_orbitals_k (wannier_index, ig)
                                        = sf[ig] * 0.5
                                          * (orb_s[ig] + m_px * orb_px[ig] + m_py * orb_py[ig] + m_pz * orb_pz[ig]);
                                }
                                    ...
                                    for (int ig = 0; ig < npw; ig++)
                                        {
                                            ...
                                            trial_orbitals_k (wannier_index, ig)
                                                = sf[ig] * bs2 * (m_pz * orb_pz[ig] + orb_dz2[ig]);
                                        }
                                    ...
                                    for (int ig = 0; ig < npw; ig++)
                                        {
                                            ...
                                            trial_orbitals_k (wannier_index, ig)
                                                = sf[ig]
                                                  * (bs6 * orb_s[ig] + tmp_bs2 * orb_px[ig] + tmp_bs12 * orb_dz2[ig]
                                                     + tmp_d * orb_dx2_y2[ig]);
                                        }
                                    ...
                                    for (int ig = 0; ig < npw; ig++)
                                        {
                                            ...
                                            trial_orbitals_k (wannier_index, ig)
                                                = sf[ig]
                                                  * (bs6 * orb_s[ig] + tmp_bs2 * orb_py[ig] + tmp_bs12 * orb_dz2[ig]
                                                     + tmp_d * orb_dx2_y2[ig]);
                                        }
                                    ...
                                    for (int ig = 0; ig < npw; ig++)
                                        {
                                            ...
                                            trial_orbitals_k (wannier_index, ig)
                                                = sf[ig]
                                                  * (bs6 * orb_s[ig] + tmp_pz * bs2 * orb_pz[ig] + bs3 * orb_dz2[ig]);
                                        }
...
void
    toWannier90_PW::get_trial_orbitals_lm_k (const int& orbital_L,
                                             const int& orbital_m,
...
void
    toWannier90_PW::integral (const int meshr,
                              const double* psir,
                              const double* r,
                              const double* rab,
                              const int& l,
...
void
    toWannier90_PW::unkdotW_A (const psi::Psi<std::complex<double>>& psi_pw,
                               const ModulePW::PW_Basis_K* wfcpw,
                               const int& ik,
                               const std::vector<ModuleBase::matrix>& radial_in_q,
                               ModuleBase::ComplexMatrix& Amn)
                    ...
                    if (PARAM.inp.nspin != 4)
                    ...
                    else
                        {
                            for (int ig = 0; ig < npw; ig++)
                                {
                                    Amn (ib_w, iw)
                                        += up_con[iw] * conj (psi_pw (ik, ib, ig)) * trial_orbitals (iw, ig)
