...
void
    toWannier90_LCAO_IN_PW::calculate (UnitCell& ucell,
...
psi::Psi<std::complex<double>>*
    toWannier90_LCAO_IN_PW::get_unk_from_lcao (const UnitCell& ucell,
                                               ...
                                               const K_Vectors& kv)
{
    ...
    psi::Psi<std::complex<double>>* unk_inLcao
        = new psi::Psi<std::complex<double>> (num_kpts, num_bands, npwx * PARAM.globalv.npol, kv.ngk, true);
    unk_inLcao->zero_out ();
                            ...
                            for (int ig = 0; ig < npw; ig++)
                                {
                                    for (int iw = 0; iw < PARAM.globalv.nlocal; iw++)
                                        {
                                            unk_inLcao[0](ik, ib, ig)
                                                += lcao_wfc_global (ib, iw) * orbital_in_G (iw, ig);
                                        }
                                }
                    ...
                    for (int ib = 0; ib < num_bands; ib++)
                        {
                            ...
                            for (int ig = 0; ig < npw; ig++)
                                {
                                    ...
                                    for (int iw = 0; iw < basis_num; iw++)
                                        {
                                            unk_inLcao[0](ik, ib, ig)
                                                += lcao_wfc_global (ib, 2 * iw) * orbital_in_G (iw, ig);
                                            unk_inLcao[0](ik, ib, ig + npwx)
                                                += lcao_wfc_global (ib, 2 * iw + 1) * orbital_in_G (iw, ig);
                                        }
                                }
                            ...
                            for (int ig = 0; ig < npw; ig++)
                                {
                                    anorm
                                        = anorm + conj (unk_inLcao[0](ik, ib, ig)) * unk_inLcao[0](ik, ib, ig)
                                          + conj (unk_inLcao[0](ik, ib, ig + npwx)) * unk_inLcao[0](ik, ib, ig + npwx);
                                }
...
void
    toWannier90_LCAO_IN_PW::nao_G_expansion (const int& ik,
                                             const ModulePW::PW_Basis_K* wfcpw,
...
void
    toWannier90_LCAO_IN_PW::get_lcao_wfc_global_ik (const int ik,
                                                    const psi::Psi<std::complex<double>>& psi_in,
                                                    ModuleBase::ComplexMatrix& lcao_wfc_global)
{
