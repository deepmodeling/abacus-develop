...
template <typename T>
void
    FR_overlap<T>::set_parameters (fr_ptr fr_in,
...
template <typename T>
void
    FR_overlap<T>::initialize_FR (const Grid_Driver* GridD, const Parallel_Orbitals* paraV)
{
...
template <typename T>
void
    FR_overlap<T>::cal_FR_IJR (const int& iat1,
                               const int& iat2,
                               const Parallel_Orbitals* paraV,
                            ...
                            for (int iw2l = 0; iw2l < col_indexes.size (); iw2l += npol)
                                {
                                    ...
                                    grid_2[count * col_num + icol2] = psi_value2[std::make_pair (L2, N2)]
                                                                      * rly2[L2 * L2 + m2] * weights_angular
                                                                      * fr (r_coor + tau_1);
                                }
                        }
                }
...
std::map<std::pair<int, int>, double>
    FR_overlap<T>::psi_inter (const int& T1, const std::set<std::pair<int, int>>& LN_pair1, const double& r_norm)
{
...
template <typename T>
double
    FR_overlap<T>::Polynomial_Interpolation (const double* psi_r, const int& mesh_r, const double& dr, const double& x)
{
    ...
    const double y
        = t1 * x1 * x2 * x3 / 6.0 + t2 * x0 * x2 * x3 / 2.0 - t3 * x1 * x0 * x3 / 2.0 + t4 * x1 * x2 * x0 / 6.0;
