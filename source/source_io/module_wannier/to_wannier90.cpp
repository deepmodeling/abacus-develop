                          ...
                          const std::string& wannier_spin)
{
    ...
    if (PARAM.inp.bndpar != 1)
        {
            ModuleBase::WARNING_QUIT ("toWannier90",
                                      "The wannier90 interface does not currently support bndpar groups");
        }
}
...
void
    toWannier90::calculate ()
{
}

void
    toWannier90::read_nnkp (const UnitCell& ucell, const K_Vectors& kv)
{
    // read *.nnkp file
...
void
    toWannier90::out_eig (const ModuleBase::matrix& ekb)
{
#ifdef __MPI
    if (GlobalV::MY_RANK == 0)
#endif
            ...
            for (int ik = start_k_index; ik < (cal_num_kpts + start_k_index); ik++)
                {
                    for (int ib = 0; ib < num_bands; ib++)
                        {
                            eig_file << std::setw (5) << ib + 1 << std::setw (5) << ik + 1 - start_k_index
...
bool
    toWannier90::try_read_nnkp (const UnitCell& ucell, const K_Vectors& kv)
{
    ...
    if (ModuleBase::GlobalFunc::SCAN_BEGIN (nnkp_read, "real_lattice"))
        {
            ...
            nnkp_read >> real_lattice_nnkp.e11 >> real_lattice_nnkp.e12 >> real_lattice_nnkp.e13
                >> real_lattice_nnkp.e21 >> real_lattice_nnkp.e22 >> real_lattice_nnkp.e23 >> real_lattice_nnkp.e31
                >> real_lattice_nnkp.e32 >> real_lattice_nnkp.e33;
            ...
            if (PARAM.inp.nspin == 1 || PARAM.inp.nspin == 2)
                {
                    if (ModuleBase::GlobalFunc::SCAN_BEGIN (nnkp_read, "projections"))
                            ...
                            if (num_wannier < 0)
                                {
                                    ModuleBase::WARNING_QUIT ("toWannier90::read_nnkp",
                                                              "wannier number is lower than 0");
                                }
                        ...
                        {
                            ModuleBase::WARNING_QUIT ("toWannier90::read_nnkp",
                                                      "Cannot find projections in *.nnkp file");
                        }
                }
            else if (PARAM.inp.nspin == 4)
                {
                    if (ModuleBase::GlobalFunc::SCAN_BEGIN (nnkp_read, "spinor_projections"))
                        ...
                        {
                            ModuleBase::WARNING_QUIT ("toWannier90::read_nnkp",
                                                      "Cannot find spinor_projections in *.nnkp file");
                        }
                }
            ...
            for (int i = 0; i < num_wannier; i++)
                {
                    if (L[i] < -5 || L[i] > 3)
                        {
                            ModuleBase::WARNING_QUIT ("toWannier90::read_nnkp",
                                                      "L angular momentum is wrong, please check !!!");
                        }
                    ...
                    if (L[i] >= 0)
                        {
                            if (m[i] < 0 || m[i] > 2 * L[i])
                                {
                                    ModuleBase::WARNING_QUIT ("toWannier90::read_nnkp",
                                                              "m momentum is wrong, please check !!!");
                                }
                        }
                    else
                        {
                            ...
                            if (!spin_z_pos && !spin_z_neg)
                                {
                                    if (spin_eig[i] == 1)
                                        {
                                            up_con[i]
                                                = (1.0 / std::sqrt (1.0 + spin_qaxis[i].z)) * (spin_qaxis[i].z + 1.0);
