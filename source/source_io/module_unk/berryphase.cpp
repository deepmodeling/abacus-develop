...
berryphase::berryphase () { GDIR = PARAM.inp.gdir; }

#ifdef __LCAO
berryphase::berryphase (const Parallel_Orbitals* paraV_in) : paraV (paraV_in) { GDIR = PARAM.inp.gdir; }
#endif

berryphase::~berryphase () {}

void
    berryphase::get_occupation_bands ()
{
...
#ifdef __LCAO
void
    berryphase::lcao_init (const UnitCell& ucell, const Grid_Driver& gd, const K_Vectors& kv, const LCAO_Orbitals& orb)
{
...
void
    berryphase::set_kpoints (const K_Vectors& kv, const int direction)
{
...
double
    berryphase::stringPhase (const UnitCell& ucell,
                             int index_str,
                             int nbands,
                             const int npwx,
                                            ...
                                            if (k_start == (nppstr - 2))
                                                {
                                                    ...
                                                    mat (nb, mb)
                                                        = pw_method
                                                              .unkdotp_G0 (rhopw, wfcpw, ik_1, ik_2, nb, mb, psi_in, G);
                                                }
                                            else
                                                {
                                                    mat (nb, mb)
                                                        = pw_method.unkdotp_G (wfcpw, ik_1, ik_2, nb, mb, psi_in);
                                                }
                                        }
                                    else
                                        {
                                            if (k_start == (nppstr - 2))
                                                {
                                                    ...
                                                    mat (nb, mb) = pw_method.unkdotp_soc_G0 (rhopw,
                                                                                             wfcpw,
                                                                                             ik_1,
                                                                                             ik_2,
                                                                                             nb,
                                            ...
                                            else
                                                {
                                                    mat (nb, mb)
                                                        = pw_method
                                                              .unkdotp_soc_G (wfcpw, ik_1, ik_2, nb, mb, npwx, psi_in);
                                                }
                                        }
                    ...
                    if (PARAM.inp.nspin != 4)
                        {
                            zeta = zeta
                                   * lcao_method
                                         .det_berryphase (ucell, ik_1, ik_2, dk, nbands, *(this->paraV), psi_in, kv);
                        }
                    else
                        {
                        }
...
void
    berryphase::Berry_Phase (const UnitCell& ucell,
                             int nbands,
                             double& pdl_elec_tot,
                             int& mod_elec_tot,
                             const int npwx,
...
void
    berryphase::Macroscopic_polarization (const UnitCell& ucell,
                                          const int npwx,
                                          const psi::Psi<std::complex<double>>* psi_in,
                                          const ModulePW::PW_Basis* rhopw,
                                          const ModulePW::PW_Basis_K* wfcpw,
        ...
        case 1:
            {
                ...
                GlobalV::ofs_running << "\n"
                                     << " P = "
                                     << outFormat (unit1 * total_polarization,
                                                   unit1 * modulus,
                                                   unit1 * polarization_xyz)
                                     << "(e/Omega).bohr" << std::endl;
                GlobalV::ofs_running << "\n"
                                     << " P = "
                                     << outFormat (unit2 * total_polarization,
                                                   unit2 * modulus,
                                                   unit2 * polarization_xyz)
                                     << "e/bohr^2" << std::endl;
                GlobalV::ofs_running << "\n"
                                     << " P = "
                                     << outFormat (unit3 * total_polarization,
                                                   unit3 * modulus,
                                                   unit3 * polarization_xyz)
                                     << "C/m^2" << std::endl;
                GlobalV::ofs_running << std::endl;
...
std::string
    berryphase::outFormat (const double polarization,
