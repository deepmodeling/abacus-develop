...
unkOverlap_lcao::unkOverlap_lcao () { allocate_flag = false; }
...
void
    unkOverlap_lcao::init (const UnitCell& ucell, const int nkstot, const LCAO_Orbitals& orb)
{
...
int
    unkOverlap_lcao::iw2it (const UnitCell& ucell, int iw)
{
    int ic = 0;
    int type = 0;
...
int
    unkOverlap_lcao::iw2ia (const UnitCell& ucell, int iw)
{
    int ic = 0;
    int na = 0;
    for (int it = 0; it < ucell.ntype; it++)
...
int
    unkOverlap_lcao::iw2iL (const UnitCell& ucell, int iw)
{
    int ic = 0;
    int iL = 0;
    for (int it = 0; it < ucell.ntype; it++)
...
int
    unkOverlap_lcao::iw2iN (const UnitCell& ucell, int iw)
{
    int ic, iN;
    ic = 0;
    iN = 0;
...
int
    unkOverlap_lcao::iw2im (const UnitCell& ucell, int iw)
{
    int ic = 0;
    int im = 0;
    for (int it = 0; it < ucell.ntype; it++)
...
void
    unkOverlap_lcao::cal_R_number (const UnitCell& ucell, const Grid_Driver& gd)
{
                            ...
                            if (distance < rcut - 1.0e-15)
                                {
                                    ...
                                    ModuleBase::Vector3<double> R_car
                                        = R_direct_x * ucell.a1 + R_direct_y * ucell.a2 + R_direct_z * ucell.a3;
                                            ...
                                            for (int iw2 = 0; iw2 < atom2->nw; iw2++)
                                                {
                                                    ...
                                                    orb1_orb2_R[orb_index_in_NLOCAL_1][orb_index_in_NLOCAL_2]
                                                        .push_back (R_car);
                                                } // end iw2
...
void
    unkOverlap_lcao::cal_orb_overlap (const UnitCell& ucell)
{
...
void
    unkOverlap_lcao::prepare_midmatrix_pblas (const UnitCell& ucell,
                                              const int ik_L,
                                              const int ik_R,
                    ...
                    if (ir >= 0 && ic >= 0)
                        {
                            ...
                            ModuleBase::Vector3<double> tau1
                                = ucell.atoms[iw2it (ucell, iw_row)].tau[iw2ia (ucell, iw_row)];
                            for (int iR = 0; iR < orb1_orb2_R[iw_row][iw_col].size (); iR++)
                                {
                                    double kRn = (kv.kvec_c[ik_R] * orb1_orb2_R[iw_row][iw_col][iR] - dk * tau1)
                                                 * ModuleBase::TWO_PI;
                                    std::complex<double> kRn_phase (cos (kRn), sin (kRn));
                                    std::complex<double> orb_overlap (
                                        psi_psi[iw_row][iw_col][iR],
...
std::complex<double>
    unkOverlap_lcao::det_berryphase (const UnitCell& ucell,
                                     const int ik_L,
                                     const int ik_R,
                                     const ModuleBase::Vector3<double> dk,
                                     const int occ_bands,
