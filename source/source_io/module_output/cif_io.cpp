...
void
    _build_chem_formula (const int natom,
                         ...
                         std::string& structural)
{
    ...
    std::transform (kinds.begin (),
                    kinds.end (),
                    counts.begin (),
                    [&labels] (const std::string& kind) { return std::count (labels.begin (), labels.end (), kind); });
...
void
    vec_to_abc_angles (const double* vec, double* abc_angles)
{
...
void
    abc_angles_to_vec (const double* abc_angles, double* vec)
{
...
double
    vec_to_volume (const double* vec)
{
...
double
    abc_angles_to_volume (const double* abc_angles)
{
...
std::vector<std::string>
    _split_loop_block (const std::string& block)
{
    std::vector<std::string> out;
...
std::map<std::string, std::vector<std::string>>
    _build_table (const std::vector<std::string>& keys, const std::vector<std::string>& values)
{
...
std::map<std::string, std::vector<std::string>>
    _build_block_data (const std::vector<std::string>& block)
{
    ...
    auto it = std::find_if (block.begin (),
                            block.end (),
                            [] (const std::string& s) { return FmtCore::startswith (s, "_"); });
    if (it != block.begin ())
        {
...
void
    bcast_cifmap (std::map<std::string, std::vector<std::string>>& map, // the map to be broadcasted
...
void
    ModuleIO::CifParser::_unpack_ucell (const UnitCell& ucell,
    ...
    for (int i = 0; i < natom; ++i)
        {
            ...
            atom_site_labels[i]
                = atom_site_labels[i].empty () ? ucell.atom_label[ucell.iat2it[i]] : atom_site_labels[i];
            atom_site_labels[i]
                = atom_site_labels[i].empty () ? ucell.atoms[ucell.iat2it[i]].label : atom_site_labels[i];
...
void
    ModuleIO::CifParser::write (const std::string& fcif,
...
void
    ModuleIO::CifParser::read (const std::string& fcif,
                               std::map<std::string, std::vector<std::string>>& out,
                               const int rank)
{
    ...
    if (myrank == rank) // only the rank assigned to read the cif file will read the file
        {
            ...
            blocks.erase (
                std::remove_if (blocks.begin (), blocks.end (), [] (const std::string& s) { return s.empty (); }),
                blocks.end ());
...
std::vector<std::string>
    ModuleIO::CifParser::get (const std::string& key)
{
