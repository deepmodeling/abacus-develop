...
bool
    ModuleIO::read_vdata_palgrid (const Parallel_Grid& pgrid,
                                  const int my_rank,
    ...
    if (my_rank == 0)
        {
            ...
            if (nx == nx_read && ny == ny_read && nz == nz_read)
                ...
                {
                    trilinear_interpolate (data_read.data (),
                                           nx_read,
                                           ny_read,
                                           nz_read,
                                           nx,
...
void
    ModuleIO::trilinear_interpolate (const double* const data_in,
                                     const int& nx_read,
                                     const int& ny_read,
                                     const int& nz_read,
                                     const int& nx,
                    ...
                    for (int iz = 0; iz < nz; iz++)
                        {
                            ...
                            data_out[(ix * ny + iy) * nz + iz]
                                = result; // x > y > z order, consistent with the cube file
                        }
                }
        }
...
bool
    ModuleIO::read_cube (const std::string& file,
                         std::vector<std::string>& comment,
                         int& natom,
                         std::vector<double>& origin,
                         int& nx,
