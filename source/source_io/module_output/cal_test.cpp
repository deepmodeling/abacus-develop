...
void
    Cal_Test::test_memory (const int nat,
                           const int ntype,
                           ...
                           const int chr_mixing_ndim)
{
    ...
    meigts123 = ModuleBase::Memory::calculate_mem (nat * (2 * rhopw->nx + 1 + 2 * rhopw->ny + 1 + 2 * rhopw->nz + 1),
                                                   "cdouble");
    ...
    std::cout << " Memory for H,S matrix ( " << PARAM.globalv.nlocal << ", " << PARAM.globalv.nlocal << ") = " << mhs
              << " MB" << std::endl;

    //(4) Memory for wave functions.
    std::cout << " Memory for wave functions ( " << PARAM.globalv.nlocal << ", " << PARAM.inp.nbands << ") = " << mwf
              << " MB" << std::endl;
...
int
    Cal_Test::cal_np (const ModuleBase::Matrix3& GGT, const double& ggcut, const int& n1, const int& n2, const int& n3)
{
...
void
    Cal_Test::print_mem (const int& nproc)
{
    std::cout << " ========================: " << std::endl;
    mtot = 0.0;

    mtot += mporter + mrho + mrho_save + mrho_core + mRrho + mdRrho + mdrho + mrho_save2 + mvltot + mvr + mvrs + mvrs1
            + mvnew + mrhog + mrhog_save + mrhog_core + mgvec + mgg + mig2fftw + mig2fftc + mig123 + mstrucFac
            + meigts123;
    mtot += mwf + mhs;
    ...
    std::cout << " MEMORY FOR nonzero      : " << std::setw (15)
              << (double)PARAM.globalv.nlocal * (PARAM.globalv.nlocal + 1) / 1028 / 1028 / 2.0 / nproc << " MB"
              << std::endl;
}
