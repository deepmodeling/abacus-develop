...
void
    print_parameters (const UnitCell& ucell, K_Vectors& kv, const Input_para& inp)
{
    ...
    if (inp.calculation == "scf" || inp.calculation == "relax" || inp.calculation == "cell-relax"
        ...
        || inp.calculation == "md")
        {
            ...
            std::cout << std::setw (12) << GlobalV::NPROC << std::setw (14) << PARAM.globalv.nthread_per_proc
...
void
    print_time (time_t& time_start, time_t& time_finish)
{
...
void
    print_rhofft (ModulePW::PW_Basis* pw_rhod,
...
void
    print_wfcfft (const Input_para& inp, ModulePW::PW_Basis_K& pw_wfc, std::ofstream& ofs)
{
    ofs << "\n\n";
    ofs << " >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" << std::endl;
    ofs << " |                                                                    |" << std::endl;
...
void
    print_screen (const int& stress_step, const int& force_step, const int& istep)
{
    std::cout << "\n ================================================================" << std::endl;
...
void
    print_kpar (const int& nks, const int& kpar_lcao)
{
    assert (nks > 0);
    assert (kpar_lcao > 0);
            ...
            if (nks % kpar_lcao != 0)
                {
                    ...
                    std::cout << "\n%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
                                 ...
                                 "%%%%%%%%%%%%%%%%%%%%%%%%%%"
                              << std::endl;
                    std::cout << " Warning: nks (" << nks << ") is not divisible by kpar (" << kpar_lcao << ")."
                              << std::endl;
