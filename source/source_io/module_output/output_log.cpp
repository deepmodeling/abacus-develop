...
{
void
    output_convergence_after_scf (const bool& convergence, double& energy, std::ofstream& ofs_running)
{
    if (convergence)
        {
            ...
            //      << std::endl;
            ofs_running << " #TOTAL ENERGY# " << std::setprecision (11) << energy * ModuleBase::Ry_to_eV << " eV"
                        << std::endl;
        }
    else
        {
...
void
    output_after_relax (bool conv_ion, bool conv_esolver, std::ofstream& ofs_running)
{
...
void
    output_efermi (const bool& convergence, double& efermi, std::ofstream& ofs_running)
{
    if (convergence && PARAM.inp.out_level != "m")
        {
            // mohan comment out 2025-06-22
            //        ofs_running << std::setprecision(16);
            //        ofs_running << " EFERMI = " << std::setprecision(11) << efermi * ModuleBase::Ry_to_eV << " eV" <<
            //        std::endl;
        }
}

void
    output_vacuum_level (const UnitCell* ucell,
...
void
    print_force (std::ofstream& ofs_running,
                 const UnitCell& cell,
...
void
    print_stress (const std::string& name,
                  ...
                  std::ofstream& ofs)
{
    ...
    if (name == "TOTAL-STRESS" && PARAM.inp.calculation != "md")
        {
            ofs << " #TOTAL-PRESSURE# (EXCLUDE KINETIC PART OF IONS): " << std::fixed << std::setprecision (6)
            ...
            if (name == "TOTAL-STRESS")
                {
                    std::cout << " TOTAL-PRESSURE (EXCLUDE KINETIC PART OF IONS): " << std::fixed
                              << std::setprecision (6) << pressure << unit << std::endl;
                }
        }
    return;
}
...
void
    write_head (std::ofstream& ofs, const int& istep, const int& iter, const std::string& basisname)
{
    ofs << std::right;
    ofs << "\n";
    ofs << " ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
...
void
    write_head_td (std::ofstream& ofs,
                   const int& istep,
                   const int& estep,
                   const int& iter,
                   const std::string& basisname)
{
    ...
    ofs << "\n";
    ofs << " ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
    ofs << " --> #ION MOVE#" << std::setw (10) << istep + 1 << " #ELEC PROP#" << std::setw (10) << estep + 1
