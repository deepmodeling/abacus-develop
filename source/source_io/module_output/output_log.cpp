...
void print_stress(const std::string& name,
    ...
    if (name == "TOTAL-STRESS" && PARAM.inp.calculation != "md") {
        ofs << " #TOTAL-PRESSURE# (EXCLUDE KINETIC PART OF IONS): " << std::fixed << std::setprecision(6) << pressure
            << unit << std::endl;
    }
    if (screen) {
        std::cout << table;
        if (name == "TOTAL-STRESS") {
            std::cout << " TOTAL-PRESSURE (EXCLUDE KINETIC PART OF IONS): " << std::fixed << std::setprecision(6)
...
void write_head_td(std::ofstream& ofs,
                   const int& istep,
                   const int& estep,
                   const int& iter,
