...
class output
{
  public:
    ...
    //===============================
    static void
        printrm (std::ofstream& ofs, const std::string& s, const ModuleBase::matrix& m, const double& limit = 1.0e-15);
    ...
    static void
        printcm_real_limit_hermit (const std::string& s, const ModuleBase::ComplexMatrix& m, const double& limit);
    ...
    static void printcm_norm (std::ofstream& ofs,
                              const std::string& s,
  ...
  public:
    template <class T>
    static void
        printr1_d (std::ofstream& ofs, const std::string& s, T* u, int n1)
    {
        ofs << "\n\n " << s << "  n1 = " << n1;
        if (n1 <= 0)
            {
    ...
    template <class T>
    static void
        printr1_d (const std::string& s, T* u, const int n1)
    {
        std::cout << "\n " << s << "  Dimension = " << n1;
        if (n1 <= 0)
            {
    ...
    template <class T>
    static void
        printV3 (std::ofstream& ofs, const ModuleBase::Vector3<T> v)
    {
        ofs << " ";
    ...
    template <class T>
    static void
        printV3 (const ModuleBase::Vector3<T> v)
    {
        std::cout << " ";
    ...
    template <class T>
    static void
        printv31_d (std::ofstream& ofs, const std::string& s, ModuleBase::Vector3<T>* u, int n1)
    {
    ...
    template <class T>
    static void
        printv31_d (const std::string& s, ModuleBase::Vector3<T>* u, int n1)
    {
        std::cout << "\n " << s << "  dimension = " << n1;
        if (n1 <= 0)
            {
