...
void
    output::printrm (std::ofstream& ofs, const std::string& s, const ModuleBase::matrix& m, const double& limit)
{
    const int b1 = m.nr;
    const int b2 = m.nc;
    ofs << "\n " << s << " nr=" << b1 << " nc=" << b2;
...
void
    output::printrm (const std::string& s, const ModuleBase::matrix& m, const double& limit)
{
    const int b1 = m.nr;
    const int b2 = m.nc;
    std::cout << "\n " << s << " nr=" << b1 << " nc=" << b2;
...
void
    output::printr3_d (std::ofstream& ofs, const std::string& s, const ModuleBase::realArray& u)
{
    ...
    output::printM3 (std::ofstream& ofs, const std::string& description, const ModuleBase::Matrix3& m)
{
    ...
    ofs << " " << std::setw (20) << m.e11 << std::setw (20) << m.e12 << std::setw (20) << m.e13 << "\n "
        ...
        << m.e31 << std::setw (20) << m.e32 << std::setw (20) << m.e33 << std::endl;
    ofs << std::resetiosflags (std::ios::showpos);
    return;
}

void
    output::printM3 (const std::string& description, const ModuleBase::Matrix3& m)
{
