...
void
    read_Hexxs_csr (const std::string& file_name,
                    ...
                    for (auto& ijv: matrix.getElements ())
                        {
                            ...
                            Hexxs.at (is)
                                .at (ucell.iwt2iat[i])
                                .at ({ucell.iwt2iat[j], {R[0], R[1], R[2]}}) (ucell.iwt2iw[i] / npol,
                                                                              ucell.iwt2iw[j] / npol) = ijv.second;
                        }
                }
        }
}
...
template <typename Tdata>
void
    read_Hexxs_cereal (const std::string& file_name,
                       std::vector<std::map<int, std::map<TAC, RI::Tensor<Tdata>>>>& Hexxs)
{
            ...
            for (auto& a2R_data: a1_a2R_data.second)
                {
                    ...
                    int start1
                        = ucell.atoms[ucell.iat2it[iat1]].stapos_wf / ucell.get_npol () + ucell.iat2ia[iat1] * nw1;
                    int start2
                        = ucell.atoms[ucell.iat2it[iat2]].stapos_wf / ucell.get_npol () + ucell.iat2ia[iat2] * nw2;
                            ...
                            for (int j = 0; j < nw2; ++j)
                                {
                                    target[dR][start1 + i][start2 + j]
                                        = ((std::abs (matrix (i, j)) > sparse_threshold) ? matrix (i, j)
                                                                                         : static_cast<Tdata> (0));
                                }
                        }
                }
...
template <typename Tdata>
void
    write_Hexxs_csr (const std::string& file_name,
                     const UnitCell& ucell,
    ...
    for (int is = 0; is < Hexxs.size (); ++is)
        {
            ...
            ModuleIO::save_sparse (calculate_RI_Tensor_sparse (sparse_threshold, Hexxs[is], ucell),
                                   all_R_coor,
                                   sparse_threshold,
                                   false, // binary
