        ...
        for (int iat1 = 0; iat1 < ucell.nat; ++iat1) {
            for (int iat2 = 0; iat2 < ucell.nat; ++iat2) {
                for (int iR = 0; iR < nR; ++iR) {
                    ...
                    TC dR({R[0], R[1], R[2]});
                    Hexxs[is][iat1][{iat2, dR}] =
                        RI::Tensor<Tdata>({static_cast<size_t>(ucell.atoms[ucell.iat2it[iat1]].nw),
...
#pragma omp for schedule(dynamic)
        ...
        for (int iR = 0; iR < nR; ++iR) {
            ...
            for (auto& ijv: matrix.getElements()) {
                ...
                Hexxs.at(is)
                    .at(ucell.iwt2iat[i])
                    .at({ucell.iwt2iat[j], {R[0], R[1], R[2]}})(ucell.iwt2iw[i] / npol, ucell.iwt2iw[j] / npol) =
                    ijv.second;
            }
        }
    }
