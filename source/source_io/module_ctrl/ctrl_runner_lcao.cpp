                      ...
                      surchem& solvent) // solvent model
{
    ...
    // 2) out ldos
    if (inp.out_ldos[0]) {
        ModuleIO::Cal_ldos<TK>::cal_ldos_lcao(pelec->eferm, chr, dmat, kv, pelec->ekb, pelec->wg, psi[0], pgrid, ucell);
    }
...
template void ctrl_runner_lcao<std::complex<double>, double>(
...
template void ctrl_runner_lcao<std::complex<double>, std::complex<double>>(
    UnitCell& ucell,                                                          // unitcell
    const Input_para& inp,                                                    // input
    K_Vectors& kv,                                                            // k-point
    elecstate::ElecState* pelec,                                              // electronic info
