...
void
    ModuleIO::ctrl_iter_pw (const int istep,
                            const int iter,
...
void
    ModuleIO::ctrl_scf_pw (const int istep,
                           UnitCell& ucell,
                           ...
                           const Input_para& inp)
{
    ...
    if (inp.onsite_radius > 0)
        { // float type has not been implemented
            ...
            onsite_p->cal_occupations (
                reinterpret_cast<psi::Psi<std::complex<double>, Device>*> (stp.template get_psi_t<T, Device> ()),
                pelec->wg);
        }
...
template <typename T, typename Device>
void
    ModuleIO::ctrl_runner_pw (UnitCell& ucell,
                              elecstate::ElecState* pelec,
                    ...
                    for (int i = 0; i < inp.bessel_nao_rcuts.size (); i++)
                        {
                            if (GlobalV::MY_RANK == 0)
                                {
                                    std::cout << "update value: bessel_nao_rcut <- " << std::fixed
                                              << inp.bessel_nao_rcuts[i] << " a.u." << std::endl;
                                }
...
// complex<double> + CPU
template void
    ModuleIO::ctrl_scf_pw<std::complex<double>, base_device::DEVICE_CPU> (const int nstep,
                                                                          UnitCell& ucell,
                                                                          elecstate::ElecState* pelec,
                                                                          const Charge& chr,
                                                                          const K_Vectors& kv,
...
// complex<double> + GPU
template void
    ModuleIO::ctrl_scf_pw<std::complex<double>, base_device::DEVICE_GPU> (const int nstep,
                                                                          UnitCell& ucell,
                                                                          elecstate::ElecState* pelec,
                                                                          const Charge& chr,
                                                                          const K_Vectors& kv,
