...
void ModuleIO::ctrl_scf_pw(const int istep,
    ...
    //------------------------------------------------------------------
    if (inp.onsite_radius > 0) { // float type has not been implemented
...
void ModuleIO::ctrl_runner_pw(UnitCell& ucell,
        ...
        if (inp.out_spillage <= 2) {
            for (int i = 0; i < inp.bessel_nao_rcuts.size(); i++) {
                if (GlobalV::MY_RANK == 0) {
                    std::cout << "update value: bessel_nao_rcut <- " << std::fixed << inp.bessel_nao_rcuts[i] << " a.u."
                              << std::endl;
                }
    ...
    if (inp.cal_cond) {
        ...
        elec_cond
            .KG(inp.cond_smear, inp.cond_fwhm, inp.cond_wcut, inp.cond_dw, inp.cond_dt, inp.cond_nonlocal, pelec->wg);
    }
