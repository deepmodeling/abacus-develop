...
void
    ModuleIO::init_from_hR (const hamilt::HContainer<TR>* hR, hamilt::HContainer<TA>* aimR)
{
...
void
    ModuleIO::init_from_adj (const UnitCell& ucell,
            ...
            for (int ad1 = 0; ad1 < adjs.adj_num + 1; ++ad1)
                {
                    ...
                    // but the calculated value is not zero due to the numerical error, which would lead to result
                    // changes.
...
void
    ModuleIO::set_rR_from_hR (const UnitCell& ucell,
                              const Grid_Driver& GridD,
                              const LCAO_Orbitals& orb,
                              const Parallel_Orbitals* pv,
            ...
            for (int ad = 0; ad < adjs.adj_num + 1; ++ad)
                {
                    ...
                    // std::cout << "tau1: " << tau1 << " tau2: " << GlobalC::ucell.get_tau(iat2) << " r_index: " <<
                    // r_index
                    //           << std::endl;
                                    ...
                                    for (size_t i_alpha = 0; i_alpha != 3; ++i_alpha)
                                        {
                                            hamilt::BaseMatrix<double>* HlocR
                                                = rR[i_alpha]->find_matrix (iat1, iat2, r_index);
                                            if (HlocR != nullptr)
                                                HlocR->add_element (iw1, iw2, tmp_r[i_alpha]);
                                            // if (i_alpha == 2)
                                            // {
                                            //     std::cout << "iw1: " << iw1 << " iw2: " << iw2 << " i_alpha: " <<
                                            //     i_alpha
...
void
    ModuleIO::sum_HR (const UnitCell& ucell,
                      const Parallel_Orbitals& pv,
                      const K_Vectors& kv,
            ...
            if (use_cell_nearest)
                {
                    ...
                    for (int iat = 0; iat < ucell.nat; ++iat)
                        {
                            atoms_pos[iat]
                                = RI_Util::Vector3_to_array3 (ucell.atoms[ucell.iat2it[iat]].tau[ucell.iat2ia[iat]]);
                        }
    ...
    if (PARAM.inp.td_stype == 1)
        {
            ...
            const hamilt::HContainer<std::complex<double>>* velocity_hR
                = TD_info::td_vel_op->get_velocity_HR_pointer ();
...
void
    ModuleIO::add_HR (const hamilt::HContainer<Tadd>* hR, hamilt::HContainer<Tfull>* full_hR)
{
    ...
    for (int ipair = 0; ipair < hR->size_atom_pairs (); ++ipair)
        {
            ...
            for (int iR = 0; iR < atom_ij.get_R_size (); iR++)
                {
                    const ModuleBase::Vector3<int> r_index = atom_ij.get_R_index (iR);
                    hamilt::BaseMatrix<Tfull>* full_HlocR
                        = full_hR->find_matrix (iat1, iat2, r_index.x, r_index.y, r_index.z);
                    const hamilt::BaseMatrix<Tadd>* HlocR
                        = hR->find_matrix (iat1, iat2, r_index.x, r_index.y, r_index.z);
...
template <typename TR>
void
    ModuleIO::cal_velocity_basis_k (const UnitCell& ucell,
                                    const LCAO_Orbitals& orb,
                                    const Parallel_Orbitals* pv,
                                    const K_Vectors& kv,
            ...
            for (size_t i_alpha = 0; i_alpha != 3; ++i_alpha)
                {
                    ...
                    //                       << " iw2: " << GlobalC::ucell.iwt2iw[iwt2] << " rk: " << rk[irc] <<
                    //                       std::endl;
                    //         }
                    //     }
                    // }
                    // 4. calculate <\vu,k|v_a|\mu,k> = partial_Hk + IMAG_UNIT * (Hk * Sk_inv * rk) - IMAG_UNIT * (rk *
                    // Sk_inv * Hk) - Hk * Sk_inv * partial_Sk 4.1.1 Hk * Sk_inv (note 2.)
                    ...
                    //             const int iat2 = GlobalC::ucell.iwt2iat[iwt2];
                    //             const int irc=ic*pv->nrow + ir;
                    //             std::cout<<"ik: "<<ik<<" i_alpha: "<<i_alpha<<" iat1:"<<iat1<<" iat2:"<<iat2<<"
                    //             v_basis_k:
...
void
    ModuleIO::cal_velocity_matrix (const psi::Psi<std::complex<double>>* psi,
                                   const Parallel_Orbitals* pv,
                                   const K_Vectors& kv,
                                    ...
                                    if (pv->in_this_processor (ir, ic))
                                        {
                                            ...
                                            // if (i_alpha == 0)
                                            // {
                                            //     std::cout<<"ik: "<<ik<<" i_alpha: "<<i_alpha<<" iat1:"<<iat1<<"
                                            //     iat2:"<<iat2<<" vk:
                                            //     "<<velocity_basis_k[ik][i_alpha][irc]<<std::endl;
                                            // }
                                        }
...
template <typename TR>
void
    ModuleIO::cal_current_comm_k (const UnitCell& ucell,
                                  const Grid_Driver& GridD,
                                  const LCAO_Orbitals& orb,
                                  const Parallel_Orbitals* pv,
                                  const K_Vectors& kv,
        ...
        for (size_t i_alpha = 0; i_alpha != 3; ++i_alpha)
            {
                for (size_t ib = 0; ib != PARAM.inp.nbands; ++ib)
                    current_k[ik][i_alpha]
                        -= pelec->wg (ik, ib) * velocity_k[ik][i_alpha](ib, ib).real () / 2.0; // for unit
            }
    for (size_t i_alpha = 0; i_alpha < 3; ++i_alpha)
        {
            delete rR[i_alpha];
...
template <typename TR>
void
    ModuleIO::write_current (const UnitCell& ucell,
                             const Grid_Driver& GridD,
                             const int istep,
                             const psi::Psi<std::complex<double>>* psi,
                             const elecstate::ElecState* pelec,
                    ...
                    if (is == kv.isk[ik])
                        {
                            if (GlobalV::MY_RANK == 0 && TD_info::out_current_k)
                                {
                                    std::string filename = PARAM.globalv.global_out_dir + "currents"
                                                           + std::to_string (is) + "k" + std::to_string (ik)
                                                           + "comm.txt";
                                    std::ofstream fout;
