...
#ifdef __LCAO
void
    ModuleIO::cal_tmp_DM (const UnitCell& ucell,
                                ...
                                for (int nu = 0; nu < pv->get_col_size (iat2); ++nu)
                                    {
                                        ...
                                        //  std::cout<<"dm2d1_real: "<< dm2d1_real << " dm2d1_imag: "<< dm2d1_imag <<
                                        //  std::endl;
    ...
    if (GlobalV::MY_RANK == 0)
        {
            ...
            fout << std::scientific;
            fout << istep + 1 << " " << current_total[0] / omega << " " << current_total[1] / omega << " "
...
void
    ModuleIO::cal_tmp_DM_k (const UnitCell& ucell,
                            elecstate::DensityMatrix<std::complex<double>, double>& DM_real,
                            elecstate::DensityMatrix<std::complex<double>, double>& DM_imag,
                            const int ik,
                            const int nspin,
                                    ...
                                    for (int icol = 0; icol < tmp_ap_real.get_col_size (); icol += 2)
                                        {
                                            ...
                                            target_DMR_real[icol + step_trace[2]]
                                                += -tmp[1].imag ()
                                                   + tmp[2].imag (); // (i * (rho_updown - rho_downup)).real()
                                            ...
                                            target_DMR_imag[icol + step_trace[2]]
                                                += tmp[1].real ()
                                                   - tmp[2].real (); // (i * (rho_updown - rho_downup)).real()
...
template <typename TR>
void
    ModuleIO::write_current_eachk (const UnitCell& ucell,
                                   const int istep,
                                   const psi::Psi<std::complex<double>>* psi,
                                   const elecstate::ElecState* pelec,
                                   const K_Vectors& kv,
                                ...
                                for (int cb = 0; cb < ra.na_each[iat]; ++cb)
                                    {
                                        ...
                                        double Ry = ra.info[iat][cb][1];
                                        double Rz = ra.info[iat][cb][2];
                                        // std::cout<< "iat1: " << iat1 << " iat2: " << iat2 << " Rx: " << Rx << " Ry: "
                                        // << Ry << " Rz:" << Rz << std::endl;
                                        //   get BaseMatrix
                                        hamilt::BaseMatrix<double>* tmp_matrix_real
                                                ...
                                                for (int nu = 0; nu < pv->get_col_size (iat2); ++nu)
                                                    {
                                                        ...
                                                        // std::cout<<"mu: "<< mu <<" nu: "<< nu << std::endl;
                                                        // std::cout<<"dm2d1_real: "<< dm2d1_real << " dm2d1_imag: "<<
                                                        // dm2d1_imag << std::endl; std::cout<<"rvz: "<< rvz.real() << "
                                                        // " << rvz.imag() << std::endl;
                                                        local_current_ik[0]
                                                            -= dm2d1_real * rvx.real () - dm2d1_imag * rvx.imag ();
                                                        local_current_ik[1]
                                                            -= dm2d1_real * rvy.real () - dm2d1_imag * rvy.imag ();
                                                        local_current_ik[2]
                                                            -= dm2d1_real * rvz.real () - dm2d1_imag * rvz.imag ();
                                                    } // end kk
                                            } // end jj
                                    } // end cb
                            } // end iat
...
#pragma omp critical(cal_current_k_reduce)
                    ...
                    if (GlobalV::MY_RANK == 0 && TD_info::out_current_k)
                        {
                            std::string filename = PARAM.globalv.global_out_dir + "current_s" + std::to_string (is)
                                                   + "k" + std::to_string (ik + 1) + ".txt";
                            std::ofstream fout;
                            fout.open (filename, std::ios::app);
                            fout << std::setprecision (16);
                            fout << std::scientific;
                            fout << istep + 1 << " " << current_ik[0] / omega << " " << current_ik[1] / omega << " "
...
template void ModuleIO::write_current_eachk<double> (const UnitCell& ucell,
                                                     const int istep,
                                                     const psi::Psi<std::complex<double>>* psi,
                                                     const elecstate::ElecState* pelec,
                                                     const K_Vectors& kv,
                                                     ...
                                                     Record_adj& ra);
template void ModuleIO::write_current_eachk<std::complex<double>> (const UnitCell& ucell,
                                                                   const int istep,
                                                                   const psi::Psi<std::complex<double>>* psi,
                                                                   const elecstate::ElecState* pelec,
                                                                   const K_Vectors& kv,
