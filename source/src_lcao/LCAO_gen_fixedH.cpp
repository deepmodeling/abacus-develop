							...
							for (int ad0=0; ad0 < GlobalC::GridD.getAdjacentNum()+1 ; ad0++)
							{
								...
								if(distance1 < rcut_10 && distance2 < rcut_20)
								{
									...
									GlobalC::UOT.snap_psibeta(
											...
											atom2->iw2m[ k0 ], // m2
											atom2->iw2n[ k0 ], // n2
											tau0, T0, GlobalC::ucell.atoms[T0].atom_pp.dion, GlobalV::NSPIN,
											GlobalC::ucell.atoms[T0].atom_pp.d_so, // mohan  add 2021-05-07
											GlobalC::ucell.atoms[T0].atom_pp.non_zero_count_soc[0], // 0 stands for spin
											GlobalC::ucell.atoms[T0].atom_pp.index1_soc[0],
											GlobalC::ucell.atoms[T0].atom_pp.index2_soc[0],
											GlobalC::ucell.atoms[T0].atom_pp.nproj_soc
											);
							...
							for (int k=0; k<atom2->nw*GlobalV::NPOL; k++)
							{
								...
								if(!calc_deri)
								{
									...
									if(GlobalV::NSPIN==4)
									{
										...
										for (int no = 0; no < GlobalC::ucell.atoms[T0].atom_pp.non_zero_count_soc[is0]; no++)
										{
											const int p1 = GlobalC::ucell.atoms[T0].atom_pp.index1_soc[is0][no];
											const int p2 = GlobalC::ucell.atoms[T0].atom_pp.index2_soc[is0][no];
											nlm_tmp += nlm_1[p1] * nlm_2[p2] * GlobalC::ucell.atoms[T0].atom_pp.d_so(is0, p2, p1);
										}
									...
									else
									{
										...
										for (int nb = 0; nb < nproj; nb++)
										{
											...
											for(int m=0;m<2*L0+1;m++)
											{
												...
												{
													nlm_tmp += nlm_1[ib]*nlm_2[ib]*GlobalC::ucell.atoms[T0].atom_pp.dion(nb,nb);
												}
												ib+=1;
											}
										}
									...
									if(GlobalV::GAMMA_ONLY_LOCAL)
									{
										...
										for (int nb = 0; nb < nproj; nb++)
										{
											...
											for(int m=0;m<2*L0+1;m++)
											{
												...
												{
													nlm[ir] += nlm_2[ir][ib]*nlm_1[ib]*GlobalC::ucell.atoms[T0].atom_pp.dion(nb,nb);
												}
												ib+=1;
											}
										}
								...
								if(distance1 < rcut1 && distance2 < rcut2)
								{
									...
									if(!calc_deri)
									{
										...
										GlobalC::UOT.snap_psibeta(
												...
												tau0, T0, GlobalC::ucell.atoms[T0].atom_pp.dion, GlobalV::NSPIN,
												GlobalC::ucell.atoms[T0].atom_pp.d_so, // mohan  add 2021-05-07
												GlobalC::ucell.atoms[T0].atom_pp.non_zero_count_soc[is0], // index stands for spin
												GlobalC::ucell.atoms[T0].atom_pp.index1_soc[is0],
												GlobalC::ucell.atoms[T0].atom_pp.index2_soc[is0],
												GlobalC::ucell.atoms[T0].atom_pp.nproj_soc,
												nlm2, is0 //for soc
												);
                    ...
                    for(int iw1=0; iw1<nw1_tot; ++iw1)
                    {
                        ...
                        for(int iw2=0; iw2<nw2_tot; ++iw2)
                        {
                            ...
                            for(int nb = 0; nb < nproj; nb++)
                            {
                                ...
                                {
                                    #ifdef _OPENMP
                                        nlm_thread += nlm1[ib]*nlm2[ib]*GlobalC::ucell.atoms[T0].atom_pp.dion(nb,nb);
                                    #else 
                                        nlm += nlm1[ib]*nlm2[ib]*GlobalC::ucell.atoms[T0].atom_pp.dion(nb,nb);
                                    #endif
                                    ib+=1;
                                }
                            }
							...
							for (int iw2=0; iw2<nw2_tot; ++iw2)
							{
								...
								if(!calc_deri)
								{
									...
									GlobalC::UOT.snap_psibeta(
										...
										atom2->iw2m[ iw2_0 ], // m2
										atom2->iw2n[ iw2_0 ], // n2
										GlobalC::ucell.atoms[T0].tau[I0], T0, GlobalC::ucell.atoms[T0].atom_pp.dion, GlobalV::NSPIN,
