                    ...
                    for (int iw2=0; iw2<GlobalC::ucell.atoms[T2].nw; ++iw2)
                    {
						...
						for (int nb = 0; nb < nproj; nb++)
						{
							...
							for(int m=0;m<2*L0+1;m++)
							{
                                ...
                                {
                                    nlm[ir] += nlm2[ir][ib]*nlm1[ib]*GlobalC::ucell.atoms[T0].atom_pp.dion(nb,nb);
                                }
								ib+=1;
							}
						}
                        ...
                        if(isstress)
                        {
		    				...
		    				for (int nb = 0; nb < nproj; nb++)
				    		{
		    					...
		    					for(int m=0;m<2*L0+1;m++)
				    			{
                                   for(int ir=0;ir<3;ir++)
                                    {
                                        nlm_t[ir] += nlm2[ir][ib]*nlm1[ib]*GlobalC::ucell.atoms[T0].atom_pp.dion(nb,nb);
                                    }
					    			ib+=1;
					    		}
					    	}
                ...
                for (int jj = 0; jj < GlobalC::ucell.atoms[T1].nw; jj++)
                {
                    ...
                    for (int kk = 0; kk < GlobalC::ucell.atoms[T2].nw; kk++)
                    {
                        ...
                        GlobalC::UOT.snap_psibeta(
                            ...
                            atom2->iw2m[kk], // m1
                            atom2->iw2n[kk], // n1
                            tau0, it, GlobalC::ucell.atoms[it].atom_pp.dion, GlobalV::NSPIN,
							GlobalC::ucell.atoms[it].atom_pp.d_so,
							GlobalC::ucell.atoms[it].atom_pp.non_zero_count_soc[0], // index stands for spin
							GlobalC::ucell.atoms[it].atom_pp.index1_soc[0],
							GlobalC::ucell.atoms[it].atom_pp.index2_soc[0],
							GlobalC::ucell.atoms[it].atom_pp.nproj_soc
