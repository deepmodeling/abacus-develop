...
class HamiltTEST : public hamilt::Hamilt<T> {
  public:
    ...
    void constructHamilt(const int iter,
...
bool read_hs(std::string fname, T& matrix) {
    ...
    if (!inf.is_open()) {
        std::cout << "Error: open file " << fname << " failed, skip!"
                  << std::endl;
        return false;
    }
    inf >> ndim;
...
class DiagoLapackPrepare {
  public:
    DiagoLapackPrepare(int nlocal,
                       ...
                       std::string hfname,
                       std::string sfname)
        : nlocal(nlocal), nbands(nbands), nb2d(nb2d), sparsity(sparsity),
          hfname(hfname), sfname(sfname) {}
    ...
    void diago_lapack() {
        ...
        starttime = MPI_Wtime();
        for (int i = 0; i < REPEATRUN; i++)
            LCAO_DIAGO_TEST::lapack_diago(this->h.data(),
                                          this->s.data(),
                                          this->e_lapack.data(),
                                          nlocal);
        endtime = MPI_Wtime();
    ...
    bool compare_eigen(std::stringstream& out_info) {
        ...
        std::cout << "H/S matrix are read from " << hfname << ", " << sfname
                  << std::endl;
        std::cout << "solver=" << ks_solver << ", NLOCAL=" << nlocal
                  << ", nbands=" << nbands << ", nb2d=" << nb2d;
        std::cout << std::endl;
        out_info << "solver time: " << hsolver_time
                 << "s, LAPACK time(1 core):" << lapack_time << "s"
                 << std::endl;
        out_info << "Maximum difference between ks_hsolver and LAPACK is "
                 << maxerror << " (" << iindex
                 << "-th eigenvalue), the pass threshold is " << PASSTHRESHOLD
                 << std::endl;
...
class DiagoLapackGammaOnlyTest
    : public ::testing::TestWithParam<DiagoPrepare<double>> {};
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values( // int nlocal, int nbands, int nb2d, int sparsity,
                       // std::string ks_solver_in, std::string hfname,
                       // std::string sfname DiagoPrepare<double>(0, 0, 1, 0,
                       // "genelpa", "H-GammaOnly-Si2.dat",
                       // "S-GammaOnly-Si2.dat")
        DiagoPrepare<
            double>(0, 0, 1, 0, "H-GammaOnly-Si2.dat", "S-GammaOnly-Si2.dat"),
        DiagoPrepare<double>(0,
                             0,
                             32,
                             0,
...
class DiagoLapackKPointsTest
    : public ::testing::TestWithParam<DiagoPrepare<std::complex<double>>> {};
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values( // int nlocal, int nbands, int nb2d, int sparsity,
                       // std::string ks_solver_in, std::string hfname,
                       // std::string sfname
                       // DiagoPrepare<std::complex<double>>(800, 400, 32, 7,
                       // "genelpa", "", ""),
        DiagoPrepare<std::complex<double>>(0,
                                           0,
                                           1,
                                           0,
                                           "lapack",
