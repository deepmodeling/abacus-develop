...
{
    psi.fix_k(ik);
    if (!GlobalV::psi_initializer && !this->initialed_psi)
    {
...
typename HSolverPW<T, Device>::Real HSolverPW<T, Device>::set_diagethr(const int istep, const int iter, const Real drho)
{
    ...
    if (iter == 1)
    {
        ...
        if (this->calculation_type == "md" || this->calculation_type == "relax" || this->calculation_type == "cell-relax")
        {
