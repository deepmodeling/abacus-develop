...
std::vector<xc_func_type> XC_Functional_Libxc::init_func(const std::vector<int> &func_id, const int xc_polarized)
{
	...
	for(int id : func_id)
	{
		...
		else if( id == XC_GGA_X_ITYH ) // short-range of B88_X
		{
			...
			double parameter_omega[1] = {std::stod(PARAM.inp.exx_hse_omega)}; // GlobalC::exx_info.info_global.hse_omega
		...
		else if( id == XC_GGA_C_LYPR ) // short-range of LYP_C
		{
			...
			double parameter_lypr[7] = {0.04918, 0.132, 0.2533, 0.349, 0.35/2.29, 2.0/2.29, std::stod(PARAM.inp.exx_hse_omega)};
        ...
        else if( id == XC_HYB_GGA_XC_LC_PBEOP ) // LC version of PBE
		{
			...
			double parameter_lcpbe[3] = { std::stod(PARAM.inp.exx_hse_omega) }; //Range separation constant: 0.33
        ...
        else if( id == XC_HYB_GGA_XC_LC_WPBE ) // Long-range corrected PBE (LC-wPBE) by Vydrov and Scuseria
		{
			...
			double parameter_lcwpbe[3] = { std::stod(PARAM.inp.exx_hybrid_alpha),  //Fraction of Hartree-Fock exchange: 1.0
				std::stod(PARAM.inp.exx_hybrid_beta),  //Fraction of short-range exact exchange: -1.0
				std::stod(PARAM.inp.exx_hse_omega) }; //Range separation constant: 0.4
        ...
        else if( id == XC_HYB_GGA_XC_LRC_WPBE ) // Long-range corrected PBE (LRC-wPBE) by by Rohrdanz, Martins and Herbert
		{
			...
			double parameter_lrcwpbe[3] = { std::stod(PARAM.inp.exx_hybrid_alpha),  //Fraction of Hartree-Fock exchange: 1.0
				std::stod(PARAM.inp.exx_hybrid_beta),  //Fraction of short-range exact exchange: -1.0
				std::stod(PARAM.inp.exx_hse_omega) }; //Range separation constant: 0.3
        ...
        else if( id == XC_HYB_GGA_XC_LRC_WPBEH ) // Long-range corrected short-range hybrid PBE (LRC-wPBEh) by Rohrdanz, Martins and Herbert
		{
			...
			double parameter_lrcwpbeh[3] = { std::stod(PARAM.inp.exx_hybrid_alpha),  //Fraction of Hartree-Fock exchange: 1.0
				std::stod(PARAM.inp.exx_hybrid_beta),  //Fraction of short-range exact exchange: -0.8
				std::stod(PARAM.inp.exx_hse_omega) }; //Range separation constant: 0.2
        ...
        else if( id == XC_HYB_GGA_XC_CAM_PBEH ) // CAM hybrid screened exchange PBE version
		{
			...
			double parameter_campbeh[3] = { std::stod(PARAM.inp.exx_hybrid_alpha),  //Fraction of Hartree-Fock exchange: 0.2
				std::stod(PARAM.inp.exx_hybrid_beta),  //Fraction of short-range exact exchange: 0.8
				std::stod(PARAM.inp.exx_hse_omega) }; //Range separation constant: 0.7
