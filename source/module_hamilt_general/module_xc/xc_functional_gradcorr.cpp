	...
	std::vector<double> &stress_gga, const bool is_stress)
{
	...
	if(GlobalV::NSPIN==4&&(PARAM.globalv.domag||GlobalV::DOMAG_Z)) { nspin0 = 2;
}
...
#pragma omp parallel for schedule(static, 1024)
	...
	if(GlobalV::NSPIN == 4&&(PARAM.globalv.domag||GlobalV::DOMAG_Z))
	{
		rhotmp2 = new double[rhopw->nrxx];
		rhogsum2 = new std::complex<double>[rhopw->npw];
			...
			else
			{
				...
				if(rh > epsr)
				{
					...
					else
					{
						...
						if(GlobalV::NSPIN==4&&(PARAM.globalv.domag||GlobalV::DOMAG_Z)) { zeta = fabs(zeta) * neg[ir];
}
