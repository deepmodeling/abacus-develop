...
void H_Ewald_pw::compute_ewald(const UnitCell& cell, ModulePW::PW_Basis* rho_basis, Structure_Factor sf)
{
...
//----------------------------------------------------------
    ...
    for (int ig = 0; ig < rho_basis->npw; ig++)
    {
        ...
        for (int it=0; it<cell.ntype; it++)
        {
            rhon += static_cast<double>(cell.atoms[it].ncpp.zv) * conj(sf.strucFac(it, ig));
        }
