...
void Numerical_Basis::output_overlap(const psi::Psi<std::complex<double>>& psi, const K_Vectors* p_kv)
{
	...
	for(int derivative_order=0; derivative_order<=1; ++derivative_order)            // Peize Lin add 2020.04.23
	{
        ...
        // OVERLAP : < J_mu | Psi >
        std::vector<ModuleBase::ComplexArray> overlap_Q(p_kv->nks);

        // OVERLAP : < J_mu | J_nu >
        std::vector<ModuleBase::ComplexArray> overlap_Sq(p_kv->nks);

        ModuleBase::GlobalFunc::OUT(GlobalV::ofs_running, "number of k points", p_kv->nks);
        ...
        this->output_info(ofs, bessel_basis, p_kv);

        this->output_k(ofs, p_kv);

        this->output_overlap_Q(ofs, overlap_Q, p_kv);

        if (winput::out_spillage == 2)
        {
            this->output_overlap_Sq(ss.str(), ofs, overlap_Sq, p_kv);
        }
...
{
    ModuleBase::matrix overlap_V(p_kv->nks, GlobalV::NBANDS);
    for (int ik = 0; ik < p_kv->nks; ++ik)
    {
        std::vector<ModuleBase::Vector3<double>> gk(p_kv->ngk[ik]);
...
void Numerical_Basis::output_info(std::ofstream& ofs, const Bessel_Basis& bessel_basis, const K_Vectors* p_kv)
{
...
void Numerical_Basis::output_k(std::ofstream& ofs, const K_Vectors* p_kv)
{
    // (1)
    if (GlobalV::MY_RANK==0)
    {
    ...
    for (int ik=0; ik<nkstot; ik++)
    {
        ...
        if (GlobalV::RANK_IN_POOL==0)
        {
            ...
            else
            {
                ...
                {
                    MPI_Send(&p_kv->kvec_c[iknow].x, 1, MPI_DOUBLE, 0, ik * 4, MPI_COMM_WORLD);
                    MPI_Send(&p_kv->kvec_c[iknow].y, 1, MPI_DOUBLE, 0, ik * 4 + 1, MPI_COMM_WORLD);
                    MPI_Send(&p_kv->kvec_c[iknow].z, 1, MPI_DOUBLE, 0, ik * 4 + 2, MPI_COMM_WORLD);
                    MPI_Send(&p_kv->wk[iknow], 1, MPI_DOUBLE, 0, ik * 4 + 3, MPI_COMM_WORLD);
                }
            }
        }
                                       ...
                                       const K_Vectors* p_kv)
{
    ...
    */
    ...
    assert(p_kv->nks > 0);
    ModuleBase::ComplexArray overlap_Q_k(p_kv->nks,
