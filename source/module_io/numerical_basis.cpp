...
//============================================================
void Numerical_Basis::start_from_file_k(const int& ik, ModuleBase::ComplexMatrix& psi, const Structure_Factor* p_sf)
{
    ...
    this->numerical_atomic_wfc(ik, GlobalC::wfcpw, psi, p_sf);
}

// The function is called in run_fp.cpp.
void Numerical_Basis::output_overlap(const psi::Psi<std::complex<double>>& psi, const Structure_Factor* p_sf)
{
	...
	for(int derivative_order=0; derivative_order<=1; ++derivative_order)            // Peize Lin add 2020.04.23
	{
        ...
        for (int ik=0; ik<GlobalC::kv.nks; ik++)
        {
            ...
            psi.fix_k(ik);
            overlap_Q[ik]
                = this->cal_overlap_Q(ik, npw, GlobalC::wfcpw, psi, static_cast<double>(derivative_order), p_sf);
            ModuleBase::GlobalFunc::DONE(GlobalV::ofs_running,"cal_overlap_Q");

            // (2) generate Sq matrix if necessary.
            if (winput::out_spillage == 2)
            {
                overlap_Sq[ik] = this->cal_overlap_Sq( ik, npw, static_cast<double>(derivative_order), p_sf);
                                                        ...
                                                        const Structure_Factor* p_sf) const
{
    ...
    for (int T = 0; T < GlobalC::ucell.ntype; T++)
    {
        ...
        {
            //OUT("I",I);
            std::complex<double>* sk = p_sf->get_sk(ik, T, I, GlobalC::wfcpw);
                                                         ...
                                                         const Structure_Factor* p_sf) const
{
    ...
    for (int T1 = 0; T1 < GlobalC::ucell.ntype; T1++) // 1.1
    {
        ...
        {
            std::complex<double>* sk1 = p_sf->get_sk(ik, T1, I1, GlobalC::wfcpw);
            for (int T2=0; T2<GlobalC::ucell.ntype; T2++) // 2.1
            {
                for (int I2=0; I2<GlobalC::ucell.atoms[T2].na; I2++) // 2.2
                {
                    std::complex<double>* sk2 = p_sf->get_sk(ik, T2, I2, GlobalC::wfcpw);
                                           ...
                                           const Structure_Factor* p_sf)
{
    ...
    for (int it = 0; it < GlobalC::ucell.ntype; it++)
    {
        ...
        {
            //OUT("ia",ia);
            std::complex<double>* sk = p_sf->get_sk(ik, it, ia, GlobalC::wfcpw);
            for (int l = 0; l < GlobalC::ucell.atoms[it].nwl+1; l++)
            {
                //OUT("l",l);
