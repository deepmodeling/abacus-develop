...
)
{
    ...
    for(size_t ik = 0; ik != p_kv->nks; ++ik)
    {
        uhm.LM->zeros_HSk('S');
...
void ModuleIO::out_mulliken(const int& step, LCAO_Hamilt &uhm, Local_Orbital_Charge &loc, const K_Vectors* p_kv)
{
    ...
    if(GlobalV::GAMMA_ONLY_LOCAL)
        ...
        orbMulP = ModuleIO::cal_mulliken_k(loc.dm_k, uhm, p_kv);
