...
namespace ModuleIO
{
	...
	void write_psi_r_1(const psi::Psi<std::complex<double>> &wfc_g, const std::string &folder_name, const bool& square,const K_Vectors* p_kv)
	{
		...
		for(int ik=0; ik<wfc_g.get_nk(); ++ik)
		{
			...
			const int ik_out = (GlobalV::NSPIN!=2)
				...
				: ik - p_kv->nks/2*p_kv->isk[ik] + p_kv->nkstot/2*p_kv->isk[ik] + GlobalC::Pkpoints.startk_pool[GlobalV::MY_POOL];
