        ...
        item.checkvalue = [](const Input_Item& item, const Parameter& para) {
            ...
            if (!find_str(basis_types, para.input.basis_type)) {
                ModuleBase::WARNING_QUIT(
                    "ReadInput",
                    "basis_type should be pw, lcao_in_pw, or lcao");
            }
        };
        this->add_item(item);
    }
    {
        ...
        item.resetvalue = [](const Input_Item& item, Parameter& para) {
            ...
            if (basis_type == "pw" && gamma_only) // pengfei Li add 2015-1-31
            {
                ...
                GlobalV::ofs_warning
                    << " Auto generating k-points file: " << input.kpoint_file
                    << std::endl;
            ...
            if ((input.out_mat_r || input.out_mat_hs2 || input.out_mat_t
                 || input.out_mat_dh || input.out_hr_npz || input.out_dm_npz
                 || input.dm_to_rho)
    ...
    }
    {
        ...
        item.checkvalue = [](const Input_Item& item, const Parameter& para) {
            if (para.input.out_mat_dh && para.input.nspin == 4) {
                ModuleBase::WARNING_QUIT(
                    "ReadInput",
                    "out_mat_dh is not available for nspin = 4");
            }
        };
        this->add_item(item);
    }
    ...
    }
    {
        ...
        item.checkvalue = [](const Input_Item& item, const Parameter& para) {
            if (para.input.out_wfc_lcao < 0 || para.input.out_wfc_lcao > 2) {
                ModuleBase::WARNING_QUIT("ReadInput",
                                         "out_wfc_lcao should be 0, 1, or 2");
            }
            if (para.input.basis_type != "lcao"
                && para.input.out_wfc_lcao != 0) {
                ModuleBase::WARNING_QUIT(
                    "ReadInput",
                    "out_wfc_lcao is only available for basis_type = lcao");
            }
        };
        this->add_item(item);
    }
