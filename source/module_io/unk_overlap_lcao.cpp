...
void unkOverlap_lcao::init(const Grid_Technique& gt, std::complex<double>*** wfc_k_grid, const int nkstot)
{
    ...
    orb_r.set_orbital_info(GlobalC::ORB.Phi[T].PhiLN(0, 0).getLabel(),  // atom label
                           ...
                           GlobalC::ORB.Phi[T].PhiLN(0, 0).getNk(), GlobalC::ORB.Phi[T].PhiLN(0, 0).getDk(),
                           GlobalC::ORB.Phi[T].PhiLN(0, 0).getDruniform(), false, true, GlobalV::CAL_FORCE);
                    ...
                    for (int LB = 0; LB <= GlobalC::ORB.Phi[TB].getLmax(); ++LB)
                    {
                        for (int NB = 0; NB < GlobalC::ORB.Phi[TB].getNchi(LB); ++NB)
                        {
                            center2_orb11[TA][TB][LA][NA][LB].insert(
                                std::make_pair(NB, Center2_Orb::Orb11(GlobalC::ORB.Phi[TA].PhiLN(LA, NA),
                                                                      GlobalC::ORB.Phi[TB].PhiLN(LB, NB), MOT, MGT)));
                        }
                    }
                }
            }
                    ...
                    for (int LB = 0; LB <= GlobalC::ORB.Phi[TB].getLmax(); ++LB)
                    {
                        for (int NB = 0; NB < GlobalC::ORB.Phi[TB].getNchi(LB); ++NB)
                        {
                            center2_orb21_r[TA][TB][LA][NA][LB].insert(
                                std::make_pair(NB, Center2_Orb::Orb21(GlobalC::ORB.Phi[TA].PhiLN(LA, NA), orb_r,
                                                                      GlobalC::ORB.Phi[TB].PhiLN(LB, NB), MOT, MGT)));
                        }
                    }
                }
            ...
            for (int iR = 0; iR < orb1_orb2_R[iw1][iw2].size(); iR++)
            {
                ModuleBase::Vector3<double> r_distance
                    = (GlobalC::ucell.atoms[atomType2].tau[ia2] - GlobalC::ucell.atoms[atomType1].tau[ia1]
                      ...
                      * GlobalC::ucell.lat0;
                psi_psi[iw1][iw2].push_back(center2_orb11[atomType1][atomType2][L1][N1][L2].at(N2).cal_overlap(
                    origin_point, r_distance, m1, m2));

                double overlap_x = -1 * sqrt(ModuleBase::FOUR_PI / 3.0)
                                   * center2_orb21_r[atomType1][atomType2][L1][N1][L2].at(N2).cal_overlap(
                                       origin_point, r_distance, m1, 1, m2); // m = 1
                double overlap_y = -1 * sqrt(ModuleBase::FOUR_PI / 3.0)
                                   * center2_orb21_r[atomType1][atomType2][L1][N1][L2].at(N2).cal_overlap(
                                       origin_point, r_distance, m1, 2, m2); // m = -1
                double overlap_z = sqrt(ModuleBase::FOUR_PI / 3.0)
                                   * center2_orb21_r[atomType1][atomType2][L1][N1][L2].at(N2).cal_overlap(
                                       origin_point, r_distance, m1, 0, m2); // m =0
...
std::complex<double> unkOverlap_lcao::unkdotp_LCAO(const int ik_L, const int ik_R, const int iband_L, const int iband_R,
                                                   const ModuleBase::Vector3<double> dk, const K_Vectors& kv)
{
...
void unkOverlap_lcao::get_lcao_wfc_global_ik(const Grid_Technique& gt, std::complex<double>** ctot,
...
void unkOverlap_lcao::prepare_midmatrix_pblas(const int ik_L, const int ik_R, const ModuleBase::Vector3<double> dk,
                                              std::complex<double>*& midmatrix, const Parallel_Orbitals& pv,
                                              const K_Vectors& kv)
{
...
std::complex<double> unkOverlap_lcao::det_berryphase(const int ik_L, const int ik_R,
                                                     ...
                                                     const psi::Psi<std::complex<double>>* psi_in, const K_Vectors& kv)
{
    ...
    int one = 1;
#ifdef __MPI
    pzgemm_(&transa, &transb, &occBands, &nlocal, &nlocal, &alpha, &psi_in[0](ik_L, 0, 0), &one, &one, lowf.ParaV->desc,
            midmatrix, &one, &one, lowf.ParaV->desc, &beta, C_matrix, &one, &one, lowf.ParaV->desc);

    pzgemm_(&transb, &transb, &occBands, &occBands, &nlocal, &alpha, C_matrix, &one, &one, lowf.ParaV->desc,
            &psi_in[0](ik_R, 0, 0), &one, &one, lowf.ParaV->desc, &beta, out_matrix, &one, &one, lowf.ParaV->desc);
