...
void ModuleIO::write_libxc_r(const int order,
                             ...
                             const ModulePW::PW_Basis& pw_rhod)
{
    ...
    const int nspin
        = (PARAM.inp.nspin == 1 || (PARAM.inp.nspin == 4 && !PARAM.globalv.domag && !PARAM.globalv.domag_z)) ? 1 : 2;
    ...
    std::vector<xc_func_type> funcs
        = XC_Functional_Libxc::init_func(func_id, (1 == nspin) ? XC_UNPOLARIZED : XC_POLARIZED);
    ...
    {
    case 4:
        v4rho4.resize(nrxx * ((1 == nspin) ? 1 : 5));
    case 3:
        v3rho3.resize(nrxx * ((1 == nspin) ? 1 : 4));
    case 2:
        v2rho2.resize(nrxx * ((1 == nspin) ? 1 : 3));
    case 1:
        vrho.resize(nrxx * nspin);
    case 0:
        exc.resize(nrxx);
        break;
    default:
        throw std::domain_error("order =" + std::to_string(order) + " unfinished in " + std::string(__FILE__) + " line "
    ...
    if (is_gga)
    {
        ...
        {
        case 4:
            v4rho3sigma.resize(nrxx * ((1 == nspin) ? 1 : 12));
            v4rho2sigma2.resize(nrxx * ((1 == nspin) ? 1 : 15));
            v4rhosigma3.resize(nrxx * ((1 == nspin) ? 1 : 20));
            v4sigma4.resize(nrxx * ((1 == nspin) ? 1 : 15));
        case 3:
            v3rho2sigma.resize(nrxx * ((1 == nspin) ? 1 : 9));
            v3rhosigma2.resize(nrxx * ((1 == nspin) ? 1 : 12));
            v3sigma3.resize(nrxx * ((1 == nspin) ? 1 : 10));
        case 2:
            v2rhosigma.resize(nrxx * ((1 == nspin) ? 1 : 6));
            v2sigma2.resize(nrxx * ((1 == nspin) ? 1 : 6));
        case 1:
            vsigma.resize(nrxx * ((1 == nspin) ? 1 : 3));
        case 0:
            break;
        default:
            throw std::domain_error("order =" + std::to_string(order) + " unfinished in " + std::string(__FILE__)
    ...
    for (xc_func_type& func: funcs)
    {
        ...
        case XC_FAMILY_LDA: {
            ...
            {
            case 4:
                xc_lda_lxc(&func, nrxx, rho.data(), v4rho4.data());
            case 3:
                xc_lda_kxc(&func, nrxx, rho.data(), v3rho3.data());
            case 2:
                xc_lda_fxc(&func, nrxx, rho.data(), v2rho2.data());
            case 1:
                xc_lda_exc_vxc(&func, nrxx, rho.data(), exc.data(), vrho.data());
                break;
            case 0:
                xc_lda_exc(&func, nrxx, rho.data(), exc.data());
                break;
            default:
                throw std::domain_error("order =" + std::to_string(order) + " unfinished in " + std::string(__FILE__)
                                        + " line " + std::to_string(__LINE__));
        ...
        case XC_FAMILY_HYB_GGA: {
            ...
            case 1:
                xc_gga_exc_vxc(&func, nrxx, rho.data(), sigma.data(), exc.data(), vrho.data(), vsigma.data());
                break;
            case 0:
                xc_gga_exc(&func, nrxx, rho.data(), sigma.data(), exc.data());
                break;
            default:
                throw std::domain_error("order =" + std::to_string(order) + " unfinished in " + std::string(__FILE__)
                                        + " line " + std::to_string(__LINE__));
    ...
    auto write_data
        = [&pw_big, &pw_rhod](const std::string data_name, const std::vector<double>& data, const int number_spin) {
              for (int is = 0; is < number_spin; ++is)
              {
                  ...
                  if (GlobalV::MY_RANK == 0)
                  {
                      const std::string file_name
                          = PARAM.globalv.global_out_dir + "xc_" + data_name + "_s" + std::to_string(is + 1) + ".cube";
                      ofs.open(file_name);
                  ...
                  if (nspin != 1)
                  {
                      throw std::invalid_argument("nspin=" + std::to_string(nspin)
                                                      + " is invalid for ModuleIO::write_cube_core without MPI. see "
                                                      + std
                                                  : string(__FILE__) + " line " + std::to_string(__LINE__));
                  }
                  ModuleIO::write_cube_core(ofs, data.data() + is, pw_rhod.nx * pw_rhod.ny, pw_rhod.nz, pw_rhod.nz);
#endif
              }
          };
    ...
    {
    case 4:
        write_data("v4rho4", v4rho4, (1 == nspin) ? 1 : 5);
    case 3:
        write_data("v3rho3", v3rho3, (1 == nspin) ? 1 : 4);
    case 2:
        write_data("v2rho2", v2rho2, (1 == nspin) ? 1 : 3);
    case 1:
        write_data("vrho", vrho, nspin);
    case 0:
        write_data("exc", exc, 1);
        break;
    default:
        throw std::domain_error("order =" + std::to_string(order) + " unfinished in " + std::string(__FILE__) + " line "
                                + std::to_string(__LINE__));
    ...
    if (is_gga)
    {
        ...
        {
        case 4:
            write_data("v4rho3sigma", v4rho3sigma, (1 == nspin) ? 1 : 12);
...
void ModuleIO::write_cube_core(std::ofstream& ofs_cube,
                               const int bz,
                               const int nbz,
                               const int nplane,
