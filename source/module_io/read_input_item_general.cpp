...
void ReadInput::item_general()
{
    ...
    // bands_to_print                  #specify the bands to be calculated in the get_pchg calculation
    ...
    // each group bndpar                         1 #devide all processors into bndpar groups and bands will be
    // distributed among each group out_freq_elec                  0 #the frequency ( >= 0) of electronic iter to output
    // charge density and wavefunction. 0: output only when converged dft_plus_dmft                  0 #true:DFT+DMFT;
    // false: standard DFT calcullation(default) rpa                            0 #true:generate output files used in
    // rpa calculation; false:(default) printe                         100 #Print out energy for each band for every
    // printe steps mem_saver                      0 #Only for nscf calculations. if set to 1, then a memory saving
    // technique will be used for many k point calculations. diago_proc                     1 #the number of procs used
    // to do diagonalization nbspline                       -1 #the order of B-spline basis wannier_card none #input
    // card for wannier functions soc_lambda                     1 #The fraction of averaged SOC pseudopotential is
    // given by (1-soc_lambda) cal_force                      0 #if calculate the force at the end of the electronic
    // iteration out_freq_ion                   0 #the frequency ( >= 0 ) of ionic step to output charge density and
    // wavefunction. 0: output only when ion steps are finished elpa_num_thread                -1 #Number of threads
    // need to use in elpa device                         cpu #the computing device for ABACUS precision double #the
    // computing precision for ABACUS

    // #Parameters (2.PW)
    // ecutwfc                        20 ##energy cutoff for wave functions
    // ecutrho                        80 ##energy cutoff for charge density and potential
    ...
    // 'file' init_chg                       atomic #start charge is from 'atomic' or file chg_extrap atomic #atomic;
    // first-order; second-order; dm:coefficients of SIA out_chg                        0 #>0 output charge density for
    // selected electron steps out_pot                        0 #output realspace potential out_wfc_pw 0 #output wave
    // functions out_wfc_r                      0 #output wave functions in realspace out_dos                        0
    // #output energy and dos out_band                       0 #output energy and band structure (with precision 8)
    ...
    // emin_sto                       0 #trial energy to guess the lower bound of eigen energies of the Hamitonian
    // operator emax_sto                       0 #trial energy to guess the upper bound of eigen energies of the
    // Hamitonian operator seed_sto                       0 #the random seed to generate stochastic orbitals
    ...
    // deepks_out_unittest            0 #if set 1, prints intermediate quantities that shall be used for making unit
    // test deepks_model                    #file dir of traced pytorch model: 'model.ptg

    // #Parameters (5.LCAO)
    // basis_type                     pw #PW; LCAO in pw; LCAO
    // gamma_only                     0 #Only for localized orbitals set and gamma point. If set to 1, a fast algorithm
    // is used search_radius                  -1 #input search radius (Bohr) search_pbc                     1 #input
    // periodic boundary condition lcao_ecut                      0 #energy cutoff for LCAO lcao_dk 0.01 #delta k for 1D
    // integration in LCAO lcao_dr                        0.01 #delta r for 1D integration in LCAO lcao_rmax 30 #max R
    // for 1D two-center integration table out_mat_hs                     0 #output H and S matrix (with precision 8)
    ...
    // out_app_flag                   1 #whether output r(R), H(R), S(R), T(R), and dH(R) matrices in an append manner
    // during MD out_mat_t                      0 #output T(R) matrix out_element_info               0 #output
    // (projected) wavefunction of each element out_mat_r                      0 #output r(R) matrix out_wfc_lcao 0
    // #ouput LCAO wave functions, 0, no output 1: text, 2: binary bx                             1 #division of an
    // element grid in FFT grid along x by                             1 #division of an element grid in FFT grid along
    // y bz                             1 #division of an element grid in FFT grid along z num_stream 4 #the nstream in
    // compute the LCAO with CUDA
    ...
    // md_pfreq                       0 #oscillation frequency, used to determine qmass of thermostats coupled with
    // barostat dump_force                     1 #output atomic forces into the file MD_dump or not dump_vel 1 #output
    // atomic velocities into the file MD_dump or not dump_virial                    1 #output lattice virial into the
    // file MD_dump or not
    ...
    // efield_pos_max                 -1 #position of the maximum of the saw-like potential along crystal axis
    // efield_dir efield_pos_dec                 -1 #zone in the unit cell where the saw-like potential decreases
    ...
    // exx_separate_loop              1 #if 1, a two-step method is employed, else it will start with a GGA-Loop, and
    // then Hybrid-Loop exx_hybrid_step                100 #the maximal electronic iteration number in the evaluation of
    // Fock exchange exx_mixing_beta                1 #mixing_beta for outer-loop when exx_separate_loop=1 exx_lambda
    // 0.3 #used to compensate for divergence points at G=0 in the evaluation of Fock exchange using lcao_in_pw method
    ...
    // exx_ccp_rmesh_times            1 #how many times larger the radial mesh required for calculating Columb potential
    // is to that of atomic orbitals exx_opt_orb_lmax               0 #the maximum l of the spherical Bessel functions
    // for opt ABFs exx_opt_orb_ecut               0 #the cut-off of plane wave expansion for opt ABFs
    // exx_opt_orb_tolerence          0 #the threshold when solving for the zeros of spherical Bessel functions for opt
    // ABFs rpa_ccp_rmesh_times            10 #how many times larger the radial mesh required for calculating Columb
    ...
    // of_tole                        1e-06 #tolerance of the energy change (in Ry) for determining the convergence,
    // default=2e-6 Ry of_tolp                        1e-05 #tolerance of potential for determining the convergence,
    // default=1e-5 in a.u. of_tf_weight                   1 #weight of TF KEDF of_vw_weight                   1 #weight
    // of vW KEDF of_wt_alpha                    0.833333 #parameter alpha of WT KEDF of_wt_beta 0.833333 #parameter
    // beta of WT KEDF of_wt_rho0                     0 #the average density of system, used in WT KEDF, in Bohr^-3
    // of_hold_rho0                   0 #If set to 1, the rho0 will be fixed even if the volume of system has changed,
    // it will be set to 1 automaticly if of_wt_rho0 is not zero of_lkt_a                       1.3 #parameter a of LKT
    // KEDF of_full_pw                     1 #If set to 1, ecut will be ignored when collect planewaves, so that all
    // planewaves will be used of_full_pw_dim                 0 #If of_full_pw = true, dimention of FFT is testricted to
    // be (0) either odd or even; (1) odd only; (2) even only of_read_kernel                 0 #If set to 1, the kernel
    // of WT KEDF will be filled from file of_kernel_file, not from formula. Only usable for WT KEDF of_kernel_file
    // WTkernel.txt #The name of WT kernel file.
    ...
    // bessel_nao_ecut                20.000000 #energy cutoff for spherical bessel functions(Ry)
    ...
    // bessel_descriptor_ecut         20.000000 #energy cutoff for spherical bessel functions(Ry)
    ...
    // sc_file                        none #file name for parameters used in non-collinear spin-constrained DFT (json
    // format)

    // #Parameters (23.Quasiatomic Orbital analysis)
    // qo_switch                      0 #0: no QO analysis; 1: QO analysis
    // qo_basis                       szv #type of QO basis function: hydrogen: hydrogen-like basis, pswfc: read basis
    // from pseudopotential qo_thr                         1e-06 #accuracy for evaluating cutoff radius of QO basis
    // function
    ...
    // pexsi_row_ordering             1 #Row permutation strategy for factorization and selected inversion, 0:
    // NoRowPerm, 1: LargeDiag pexsi_nproc                    1 #Number of processors for parmetis pexsi_symm 1 #Matrix
    // symmetry pexsi_trans                    0 #Whether to transpose pexsi_method                   1 #pole expansion
    // method, 1: Cauchy Contour Integral, 2: Moussa optimized method pexsi_nproc_pole               1 #Number of
    // processes used by each pole pexsi_temp                     0.015 #Temperature, in the same unit as H pexsi_gap 0
    // #Spectral gap pexsi_delta_e                  20 #An upper bound for the spectral radius of \f$S^{-1} H\f$
    ...
    // pexsi_mu_thr                   0.05 #Stopping criterion in terms of the chemical potential for the inertia
    // counting procedure pexsi_mu_expand                0.3 #If the chemical potential is not in the initial interval,
    // the interval is expanded by muInertiaExpansion pexsi_mu_guard                 0.2 #Safe guard criterion in terms
    // of the chemical potential to reinvoke the inertia counting procedure pexsi_elec_thr                 0.001
    // #Stopping criterion of the PEXSI iteration in terms of the number of electrons compared to numElectronExact
    // pexsi_zero_thr                 1e-10 #if the absolute value of matrix element is less than ZERO_Limit, it will be
    // considered as 0

    {
        Input_Item item("suffix");
        item.annotation = "the name of main output directory";
        item.readvalue = [](const Input_Item& item, Parameter& para) { para.suffix = strvalue; };
        this->add_item(item);
    }
    {
        Input_Item item("latname");
        item.annotation = "the name of lattice name";
        item.readvalue = [](const Input_Item& item, Parameter& para) { para.latname = strvalue; };
        this->add_item(item);
    }
    {
        Input_Item item("stru_file");
        item.annotation = "the filename of file containing atom positions";
        item.readvalue = [](const Input_Item& item, Parameter& para) { para.stru_file = strvalue; };
        this->add_item(item);
    }
    {
        Input_Item item("kpoint_file");
        item.annotation = "the name of file containing k points";
        item.readvalue = [](const Input_Item& item, Parameter& para) { para.kpoint_file = strvalue; };
        this->add_item(item);
    }
    {
        Input_Item item("pseudo_dir");
        item.annotation = "the directory containing pseudo files";
        item.readvalue = [](const Input_Item& item, Parameter& para) { para.pseudo_dir = strvalue; };
        this->add_item(item);
    }
    {
        Input_Item item("orbital_dir");
        item.annotation = "the directory containing orbital files";
        item.readvalue = [](const Input_Item& item, Parameter& para) { para.orbital_dir = strvalue; };
        this->add_item(item);
    }
    {
