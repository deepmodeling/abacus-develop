...
/**
 * @brief Converts the string sa to a minor character and stores it in the
 * string sb
 * @param sa Pointer to the string to be converted
 * @param sb Store a pointer to the converted string, which needs to be
...
/**
 * @brief Reads the default parameters from the specified file and saves them to
 * the global variable default_parametes_type
 * @param fn Specifies the path to the file
 */
void default_parametes_reader(
    const std::string& fn,
    std::map<std::string, std::string>& default_parametes_type) {
    ...
    if (inputFile.is_open()) {
        ...
        while (inputFile >> word1 >> word2) {
            ...
            default_parametes_type.insert(
                std::pair<std::string, std::string>(word1, word2));
        }
        // Close file
        inputFile.close();
    } else {
...
/**
 * @brief This function is used to read the input parameter file and store it as
 * a key-value pair
 * @param fn Enter the path to the parameter file
 */
void input_parameters_get(const std::string& fn,
                          std::map<std::string, InputParameter>& input) {
    ...
    // If ierr is 0, the word "INPUT_PARAMETERS" is not found, and an error
    // message is printed with false
    if (ierr == 0) {
        ModuleBase::WARNING_QUIT("Input",
                                 "INPUT_PARAMETERS statement not found.");
    }
    ifs.rdstate();
    ...
    while (ifs.good()) {
        ...
        // main parameters
        //----------------------------------------------------------
        // The parameter values are read according to the parameter type and
    ...
    if (INPUT.ntype == 0) {
        ...
        GlobalV::ofs_running
            ...
            << INPUT.ntype << " according to STRU" << std::endl;
...
void input_parameters_set(
    std::map<std::string, InputParameter> input_parameters) {
    if (input_parameters.count("nupdown") != 0) {
        INPUT.nupdown
            = *static_cast<double*>(input_parameters["nupdown"].get());
    } else if (input_parameters.count("suffix") != 0) {
        INPUT.suffix
            = static_cast<SimpleString*>(input_parameters["suffix"].get())
                  ->c_str();
    } else if (input_parameters.count("stru_file") != 0) {
        INPUT.stru_file
            = static_cast<SimpleString*>(input_parameters["stru_file"].get())
                  ->c_str();
    } else if (input_parameters.count("pseudo_dir") != 0) {
        INPUT.pseudo_dir
            = static_cast<SimpleString*>(input_parameters["pseudo_dir"].get())
                  ->c_str();
    } else if (input_parameters.count("orbital_dir") != 0) {
        INPUT.orbital_dir
            = static_cast<SimpleString*>(input_parameters["orbital_dir"].get())
                  ->c_str();
    } else if (input_parameters.count("read_file_dir") != 0) {
        INPUT.read_file_dir = static_cast<SimpleString*>(
                                  input_parameters["read_file_dir"].get())
                                  ->c_str();
    } else if (input_parameters.count("kpoint_file") != 0) {
        INPUT.kpoint_file
            = static_cast<SimpleString*>(input_parameters["kpoint_file"].get())
                  ->c_str();
    } else if (input_parameters.count("wannier_card") != 0) {
        INPUT.wannier_card
            = static_cast<SimpleString*>(input_parameters["wannier_card"].get())
                  ->c_str();
    } else if (input_parameters.count("latname") != 0) {
        INPUT.latname
            = static_cast<SimpleString*>(input_parameters["latname"].get())
                  ->c_str();
    } else if (input_parameters.count("calculation") != 0) {
        INPUT.calculation
            = static_cast<SimpleString*>(input_parameters["calculation"].get())
                  ->c_str();
    } else if (input_parameters.count("esolver_type") != 0) {
        INPUT.esolver_type
            = static_cast<SimpleString*>(input_parameters["esolver_type"].get())
                  ->c_str();
    } else if (input_parameters.count("pseudo_rcut") != 0) {
        INPUT.pseudo_rcut
            = *static_cast<double*>(input_parameters["pseudo_rcut"].get());
    } else if (input_parameters.count("pseudo_mesh") != 0) {
        INPUT.pseudo_mesh
            = *static_cast<bool*>(input_parameters["pseudo_mesh"].get());
        ...
        INPUT.nbands_istate
            = *static_cast<int*>(input_parameters["nbands_istate"].get());
        ...
        INPUT.init_vel
            = *static_cast<bool*>(input_parameters["init_vel"].get());
    } else if (input_parameters.count("ref_cell_factor") != 0) {
        INPUT.ref_cell_factor
            = *static_cast<double*>(input_parameters["ref_cell_factor"].get());
        ...
        INPUT.symmetry_prec
            = *static_cast<double*>(input_parameters["symmetry_prec"].get());
        ...
        INPUT.berry_phase
            = *static_cast<bool*>(input_parameters["berry_phase"].get());
        ...
        SimpleVector<double> vec_D = *static_cast<SimpleVector<double>*>(
            input_parameters["kspacing"].get());
        ...
        INPUT.min_dist_coef
            = *static_cast<double*>(input_parameters["min_dist_coef"].get());
    } else if (input_parameters.count("towannier90") != 0) {
        INPUT.towannier90
            = *static_cast<bool*>(input_parameters["towannier90"].get());
    } else if (input_parameters.count("nnkpfile") != 0) {
        INPUT.nnkpfile
            = static_cast<SimpleString*>(input_parameters["nnkpfile"].get())
                  ->c_str();
    } else if (input_parameters.count("wannier_spin") != 0) {
        INPUT.wannier_spin
            = static_cast<SimpleString*>(input_parameters["wannier_spin"].get())
                  ->c_str();
    } else if (input_parameters.count("wannier_method") != 0) {
        INPUT.wannier_method
            = *static_cast<int*>(input_parameters["wannier_method"].get());
    } else if (input_parameters.count("out_wannier_mmn") != 0) {
        INPUT.out_wannier_mmn
            = *static_cast<bool*>(input_parameters["out_wannier_mmn"].get());
    } else if (input_parameters.count("out_wannier_amn") != 0) {
        INPUT.out_wannier_amn
            = *static_cast<bool*>(input_parameters["out_wannier_amn"].get());
    } else if (input_parameters.count("out_wannier_unk") != 0) {
        INPUT.out_wannier_unk
            = *static_cast<bool*>(input_parameters["out_wannier_unk"].get());
    } else if (input_parameters.count("out_wannier_eig") != 0) {
        INPUT.out_wannier_eig
            = *static_cast<bool*>(input_parameters["out_wannier_eig"].get());
    } else if (input_parameters.count("out_wannier_wvfn_formatted") != 0) {
        INPUT.out_wannier_wvfn_formatted = *static_cast<bool*>(
            input_parameters["out_wannier_wvfn_formatted"].get());
        ...
        INPUT.nbands_sto
            = *static_cast<int*>(input_parameters["nbands_sto"].get());
    } else if (input_parameters.count("nbndsto_str") != 0) {
        INPUT.nbndsto_str
            = static_cast<SimpleString*>(input_parameters["nbndsto_str"].get())
                  ->c_str();
        ...
        INPUT.initsto_ecut
            = *static_cast<double*>(input_parameters["initsto_ecut"].get());
    } else if (input_parameters.count("emax_sto") != 0) {
        INPUT.emax_sto
            = *static_cast<double*>(input_parameters["emax_sto"].get());
    } else if (input_parameters.count("emin_sto") != 0) {
        INPUT.emin_sto
            = *static_cast<double*>(input_parameters["emin_sto"].get());
        ...
        INPUT.initsto_freq
            = *static_cast<int*>(input_parameters["initsto_freq"].get());
    } else if (input_parameters.count("method_sto") != 0) {
        INPUT.method_sto
            = *static_cast<int*>(input_parameters["method_sto"].get());
    } else if (input_parameters.count("npart_sto") != 0) {
        INPUT.npart_sto
            = *static_cast<int*>(input_parameters["npart_sto"].get());
    } else if (input_parameters.count("cal_cond") != 0) {
        INPUT.cal_cond
            = *static_cast<bool*>(input_parameters["cal_cond"].get());
    } else if (input_parameters.count("cond_che_thr") != 0) {
        INPUT.cond_che_thr
            = *static_cast<double*>(input_parameters["cond_che_thr"].get());
    } else if (input_parameters.count("cond_dw") != 0) {
        INPUT.cond_dw
            = *static_cast<double*>(input_parameters["cond_dw"].get());
    } else if (input_parameters.count("cond_wcut") != 0) {
        INPUT.cond_wcut
            = *static_cast<double*>(input_parameters["cond_wcut"].get());
        ...
        INPUT.cond_dtbatch
            = *static_cast<int*>(input_parameters["cond_dtbatch"].get());
    } else if (input_parameters.count("cond_smear") != 0) {
        INPUT.cond_smear
            = *static_cast<int*>(input_parameters["cond_smear"].get());
    } else if (input_parameters.count("cond_fwhm") != 0) {
        INPUT.cond_fwhm
            = *static_cast<double*>(input_parameters["cond_fwhm"].get());
    } else if (input_parameters.count("cond_nonlocal") != 0) {
        INPUT.cond_nonlocal
            = *static_cast<bool*>(input_parameters["cond_nonlocal"].get());
    } else if (input_parameters.count("dft_functional") != 0) {
        INPUT.dft_functional = static_cast<SimpleString*>(
                                   input_parameters["dft_functional"].get())
                                   ->c_str();
    } else if (input_parameters.count("xc_temperature") != 0) {
        INPUT.xc_temperature
            = *static_cast<double*>(input_parameters["xc_temperature"].get());
        ...
        INPUT.basis_type
            = static_cast<SimpleString*>(input_parameters["basis_type"].get())
                  ->c_str();
    } else if (input_parameters.count("ks_solver") != 0) {
        INPUT.ks_solver
            = static_cast<SimpleString*>(input_parameters["ks_solver"].get())
                  ->c_str();
    } else if (input_parameters.count("cal_force") != 0) {
        INPUT.cal_force
            = *static_cast<bool*>(input_parameters["cal_force"].get());
    } else if (input_parameters.count("force_thr") != 0) {
        INPUT.force_thr
            = *static_cast<double*>(input_parameters["force_thr"].get());
    } else if (input_parameters.count("force_thr_ev2") != 0) {
        INPUT.force_thr_ev2
            = *static_cast<double*>(input_parameters["force_thr_ev2"].get());
    } else if (input_parameters.count("stress_thr") != 0) {
        INPUT.stress_thr
            = *static_cast<double*>(input_parameters["stress_thr"].get());
        ...
        INPUT.cal_stress
            = *static_cast<bool*>(input_parameters["cal_stress"].get());
    } else if (input_parameters.count("fixed_axes") != 0) {
        INPUT.fixed_axes
            = static_cast<SimpleString*>(input_parameters["fixed_axes"].get())
                  ->c_str();
    } else if (input_parameters.count("fixed_ibrav") != 0) {
        INPUT.fixed_ibrav
            = *static_cast<bool*>(input_parameters["fixed_ibrav"].get());
    } else if (input_parameters.count("fixed_atoms") != 0) {
        INPUT.fixed_atoms
            = *static_cast<bool*>(input_parameters["fixed_atoms"].get());
    } else if (input_parameters.count("relax_method") != 0) {
        INPUT.relax_method
            = static_cast<SimpleString*>(input_parameters["relax_method"].get())
                  ->c_str();
    } else if (input_parameters.count("relax_new") != 0) {
        INPUT.relax_new
            = *static_cast<bool*>(input_parameters["relax_new"].get());
    } else if (input_parameters.count("relax_cg_thr") != 0) {
        INPUT.relax_cg_thr
            = *static_cast<double*>(input_parameters["relax_cg_thr"].get());
    } else if (input_parameters.count("relax_bfgs_w1") != 0) {
        INPUT.relax_bfgs_w1
            = *static_cast<double*>(input_parameters["relax_bfgs_w1"].get());
    } else if (input_parameters.count("relax_bfgs_w2") != 0) {
        INPUT.relax_bfgs_w2
            = *static_cast<double*>(input_parameters["relax_bfgs_w2"].get());
    } else if (input_parameters.count("relax_bfgs_rmax") != 0) {
        INPUT.relax_bfgs_rmax
            = *static_cast<double*>(input_parameters["relax_bfgs_rmax"].get());
    } else if (input_parameters.count("relax_bfgs_rmin") != 0) {
        INPUT.relax_bfgs_rmin
            = *static_cast<double*>(input_parameters["relax_bfgs_rmin"].get());
    } else if (input_parameters.count("relax_bfgs_init") != 0) {
        INPUT.relax_bfgs_init
            = *static_cast<double*>(input_parameters["relax_bfgs_init"].get());
    } else if (input_parameters.count("relax_scale_force") != 0) {
        INPUT.relax_scale_force = *static_cast<double*>(
            input_parameters["relax_scale_force"].get());
    } else if (input_parameters.count("gamma_only") != 0) {
        INPUT.gamma_only
            = *static_cast<bool*>(input_parameters["gamma_only"].get());
    } else if (input_parameters.count("gamma_only_local") != 0) {
        INPUT.gamma_only_local
            = *static_cast<bool*>(input_parameters["gamma_only_local"].get());
        ...
        INPUT.ecutwfc
            = *static_cast<double*>(input_parameters["ecutwfc"].get());
    } else if (input_parameters.count("ecutrho") != 0) {
        INPUT.ecutrho
            = *static_cast<double*>(input_parameters["ecutrho"].get());
        ...
        INPUT.diago_proc
            = *static_cast<int*>(input_parameters["diago_proc"].get());
    } else if (input_parameters.count("pw_diag_nmax") != 0) {
        INPUT.pw_diag_nmax
            = *static_cast<int*>(input_parameters["pw_diag_nmax"].get());
    } else if (input_parameters.count("diago_cg_prec") != 0) {
        INPUT.diago_cg_prec
            = *static_cast<int*>(input_parameters["diago_cg_prec"].get());
    } else if (input_parameters.count("pw_diag_ndim") != 0) {
        INPUT.pw_diag_ndim
            = *static_cast<int*>(input_parameters["pw_diag_ndim"].get());
    } else if (input_parameters.count("diago_full_acc") != 0) {
        INPUT.diago_full_acc
            = *static_cast<int*>(input_parameters["diago_full_acc"].get());
    } else if (input_parameters.count("pw_diag_thr") != 0) {
        INPUT.pw_diag_thr
            = *static_cast<double*>(input_parameters["pw_diag_thr"].get());
        ...
        INPUT.vnl_in_h
            = *static_cast<bool*>(input_parameters["vnl_in_h"].get());
        ...
        INPUT.vion_in_h
            = *static_cast<bool*>(input_parameters["vion_in_h"].get());
    } else if (input_parameters.count("test_force") != 0) {
        INPUT.test_force
            = *static_cast<bool*>(input_parameters["test_force"].get());
    } else if (input_parameters.count("test_stress") != 0) {
        INPUT.test_stress
            = *static_cast<bool*>(input_parameters["test_stress"].get());
    } else if (input_parameters.count("scf_thr") != 0) {
        INPUT.scf_thr
            = *static_cast<double*>(input_parameters["scf_thr"].get());
    } else if (input_parameters.count("scf_thr_type") != 0) {
        INPUT.scf_thr_type
            = *static_cast<int*>(input_parameters["scf_thr_type"].get());
        ...
        INPUT.relax_nmax
            = *static_cast<int*>(input_parameters["relax_nmax"].get());
    } else if (input_parameters.count("out_stru") != 0) {
        INPUT.out_stru
            = *static_cast<bool*>(input_parameters["out_stru"].get());
    } else if (input_parameters.count("out_level") != 0) {
        INPUT.out_level
            = static_cast<SimpleString*>(input_parameters["out_level"].get())
                  ->c_str();
    } else if (input_parameters.count("out_md_control") != 0) {
        INPUT.out_md_control
            = *static_cast<bool*>(input_parameters["out_md_control"].get());
    } else if (input_parameters.count("occupations") != 0) {
        INPUT.occupations
            = static_cast<SimpleString*>(input_parameters["occupations"].get())
                  ->c_str();
    } else if (input_parameters.count("smearing_method") != 0) {
        INPUT.smearing_method = static_cast<SimpleString*>(
                                    input_parameters["smearing_method"].get())
                                    ->c_str();
    } else if (input_parameters.count("smearing_sigma") != 0) {
        INPUT.smearing_sigma
            = *static_cast<double*>(input_parameters["smearing_sigma"].get());
    } else if (input_parameters.count("mixing_mode") != 0) {
        INPUT.mixing_mode
            = static_cast<SimpleString*>(input_parameters["mixing_mode"].get())
                  ->c_str();
    } else if (input_parameters.count("mixing_beta") != 0) {
        INPUT.mixing_beta
            = *static_cast<double*>(input_parameters["mixing_beta"].get());
    } else if (input_parameters.count("mixing_ndim") != 0) {
        INPUT.mixing_ndim
            = *static_cast<int*>(input_parameters["mixing_ndim"].get());
    } else if (input_parameters.count("mixing_restart") != 0) {
        INPUT.mixing_restart
            = *static_cast<double*>(input_parameters["mixing_restart"].get());
    } else if (input_parameters.count("mixing_gg0") != 0) {
        INPUT.mixing_gg0
            = *static_cast<double*>(input_parameters["mixing_gg0"].get());
    } else if (input_parameters.count("mixing_beta_mag") != 0) {
        INPUT.mixing_beta_mag
            = *static_cast<double*>(input_parameters["mixing_beta_mag"].get());
    } else if (input_parameters.count("mixing_gg0_mag") != 0) {
        INPUT.mixing_gg0_mag
            = *static_cast<double*>(input_parameters["mixing_gg0_mag"].get());
    } else if (input_parameters.count("mixing_gg0_min") != 0) {
        INPUT.mixing_gg0_min
            = *static_cast<double*>(input_parameters["mixing_gg0_min"].get());
    } else if (input_parameters.count("mixing_angle") != 0) {
        INPUT.mixing_angle
            = *static_cast<double*>(input_parameters["mixing_angle"].get());
    } else if (input_parameters.count("mixing_tau") != 0) {
        INPUT.mixing_tau
            = *static_cast<bool*>(input_parameters["mixing_tau"].get());
    } else if (input_parameters.count("mixing_dftu") != 0) {
        INPUT.mixing_dftu
            = *static_cast<bool*>(input_parameters["mixing_dftu"].get());
    } else if (input_parameters.count("mixing_dmr") != 0) {
        INPUT.mixing_dmr
            = *static_cast<bool*>(input_parameters["mixing_dmr"].get());
    } else if (input_parameters.count("init_wfc") != 0) {
        INPUT.init_wfc
            = static_cast<SimpleString*>(input_parameters["init_wfc"].get())
                  ->c_str();
    } else if (input_parameters.count("init_chg") != 0) {
        INPUT.init_chg
            = static_cast<SimpleString*>(input_parameters["init_chg"].get())
                  ->c_str();
    } else if (input_parameters.count("chg_extrap") != 0) {
        INPUT.chg_extrap
            = static_cast<SimpleString*>(input_parameters["chg_extrap"].get())
                  ->c_str();
    } else if (input_parameters.count("mem_saver") != 0) {
        INPUT.mem_saver
            = *static_cast<int*>(input_parameters["mem_saver"].get());
        ...
        INPUT.out_freq_elec
            = *static_cast<int*>(input_parameters["out_freq_elec"].get());
    } else if (input_parameters.count("out_freq_ion") != 0) {
        INPUT.out_freq_ion
            = *static_cast<int*>(input_parameters["out_freq_ion"].get());
        ...
        INPUT.out_wfc_pw
            = *static_cast<int*>(input_parameters["out_wfc_pw"].get());
    } else if (input_parameters.count("out_wfc_r") != 0) {
        INPUT.out_wfc_r
            = *static_cast<bool*>(input_parameters["out_wfc_r"].get());
        ...
        INPUT.out_band = *static_cast<std::vector<int>*>(
            input_parameters["out_band"].get());
    } else if (input_parameters.count("out_proj_band") != 0) {
        INPUT.out_proj_band
            = *static_cast<bool*>(input_parameters["out_proj_band"].get());
    } else if (input_parameters.count("out_mat_hs") != 0) {
        INPUT.out_mat_hs = *static_cast<std::vector<int>*>(
            input_parameters["out_mat_hs"].get());
    } else if (input_parameters.count("out_mat_xc") != 0) {
        INPUT.out_mat_xc
            = *static_cast<bool*>(input_parameters["out_mat_xc"].get());
    } else if (input_parameters.count("out_hr_npz") != 0) {
        INPUT.out_hr_npz
            = *static_cast<bool*>(input_parameters["out_hr_npz"].get());
    } else if (input_parameters.count("out_dm_npz") != 0) {
        INPUT.out_dm_npz
            = *static_cast<bool*>(input_parameters["out_dm_npz"].get());
    } else if (input_parameters.count("dm_to_rho") != 0) {
        INPUT.dm_to_rho
            = *static_cast<bool*>(input_parameters["dm_to_rho"].get());
    } else if (input_parameters.count("cal_syns") != 0) {
        INPUT.cal_syns
            = *static_cast<bool*>(input_parameters["cal_syns"].get());
        ...
        INPUT.out_mat_hs2
            = *static_cast<bool*>(input_parameters["out_mat_hs2"].get());
    } else if (input_parameters.count("out_mat_dh") != 0) {
        INPUT.out_mat_dh
            = *static_cast<bool*>(input_parameters["out_mat_dh"].get());
    } else if (input_parameters.count("out_interval") != 0) {
        INPUT.out_interval
            = *static_cast<int*>(input_parameters["out_interval"].get());
    } else if (input_parameters.count("out_app_flag") != 0) {
        INPUT.out_app_flag
            = *static_cast<bool*>(input_parameters["out_app_flag"].get());
    } else if (input_parameters.count("out_mat_t") != 0) {
        INPUT.out_mat_t
            = *static_cast<bool*>(input_parameters["out_mat_t"].get());
    } else if (input_parameters.count("out_mat_r") != 0) {
        INPUT.out_mat_r
            = *static_cast<bool*>(input_parameters["out_mat_r"].get());
    } else if (input_parameters.count("out_wfc_lcao") != 0) {
        INPUT.out_wfc_lcao
            = *static_cast<bool*>(input_parameters["out_wfc_lcao"].get());
    } else if (input_parameters.count("out_alllog") != 0) {
        INPUT.out_alllog
            = *static_cast<bool*>(input_parameters["out_alllog"].get());
    } else if (input_parameters.count("out_element_info") != 0) {
        INPUT.out_element_info
            = *static_cast<bool*>(input_parameters["out_element_info"].get());
    } else if (input_parameters.count("out_bandgap") != 0) {
        INPUT.out_bandgap
            = *static_cast<bool*>(input_parameters["out_bandgap"].get());
    } else if (input_parameters.count("dos_emin_ev") != 0) {
        INPUT.dos_emin_ev
            = *static_cast<double*>(input_parameters["dos_emin_ev"].get());
    } else if (input_parameters.count("dos_emax_ev") != 0) {
        INPUT.dos_emax_ev
            = *static_cast<double*>(input_parameters["dos_emax_ev"].get());
    }
    if (input_parameters.count("dos_edelta_ev") != 0) {
        INPUT.dos_edelta_ev
            = *static_cast<double*>(input_parameters["dos_edelta_ev"].get());
    } else if (input_parameters.count("dos_scale") != 0) {
        INPUT.dos_scale
            = *static_cast<double*>(input_parameters["dos_scale"].get());
        ...
        INPUT.dos_setemin
            = *static_cast<bool*>(input_parameters["dos_setemin"].get());
    } else if (input_parameters.count("dos_setemax") != 0) {
        INPUT.dos_setemax
            = *static_cast<bool*>(input_parameters["dos_setemax"].get());
    } else if (input_parameters.count("dos_sigma") != 0) {
        INPUT.dos_sigma
            = *static_cast<double*>(input_parameters["dos_sigma"].get());
    } else if (input_parameters.count("lcao_ecut") != 0) {
        INPUT.lcao_ecut
            = *static_cast<double*>(input_parameters["lcao_ecut"].get());
    } else if (input_parameters.count("lcao_dk") != 0) {
        INPUT.lcao_dk
            = *static_cast<double*>(input_parameters["lcao_dk"].get());
    } else if (input_parameters.count("lcao_dr") != 0) {
        INPUT.lcao_dr
            = *static_cast<double*>(input_parameters["lcao_dr"].get());
    } else if (input_parameters.count("lcao_rmax") != 0) {
        INPUT.lcao_rmax
            = *static_cast<double*>(input_parameters["lcao_rmax"].get());
    } else if (input_parameters.count("search_radius") != 0) {
        INPUT.search_radius
            = *static_cast<double*>(input_parameters["search_radius"].get());
    } else if (input_parameters.count("search_pbc") != 0) {
        INPUT.search_pbc
            = *static_cast<bool*>(input_parameters["search_pbc"].get());
        ...
        INPUT.efield_flag
            = *static_cast<bool*>(input_parameters["efield_flag"].get());
    } else if (input_parameters.count("dip_cor_flag") != 0) {
        INPUT.dip_cor_flag
            = *static_cast<bool*>(input_parameters["dip_cor_flag"].get());
    } else if (input_parameters.count("efield_dir") != 0) {
        INPUT.efield_dir
            = *static_cast<int*>(input_parameters["efield_dir"].get());
    } else if (input_parameters.count("efield_pos_max") != 0) {
        INPUT.efield_pos_max
            = *static_cast<double*>(input_parameters["efield_pos_max"].get());
    } else if (input_parameters.count("efield_pos_dec") != 0) {
        INPUT.efield_pos_dec
            = *static_cast<double*>(input_parameters["efield_pos_dec"].get());
    } else if (input_parameters.count("efield_amp") != 0) {
        INPUT.efield_amp
            = *static_cast<double*>(input_parameters["efield_amp"].get());
    } else if (input_parameters.count("gate_flag") != 0) {
        INPUT.gate_flag
            = *static_cast<bool*>(input_parameters["gate_flag"].get());
        ...
        INPUT.block_down
            = *static_cast<double*>(input_parameters["block_down"].get());
    } else if (input_parameters.count("block_up") != 0) {
        INPUT.block_up
            = *static_cast<double*>(input_parameters["block_up"].get());
    } else if (input_parameters.count("block_height") != 0) {
        INPUT.block_height
            = *static_cast<double*>(input_parameters["block_height"].get());
    } else if (input_parameters.count("vdw_method") != 0) {
        INPUT.vdw_method
            = static_cast<SimpleString*>(input_parameters["vdw_method"].get())
                  ->c_str();
    } else if (input_parameters.count("vdw_s6") != 0) {
        INPUT.vdw_s6
            = static_cast<SimpleString*>(input_parameters["vdw_s6"].get())
                  ->c_str();
    } else if (input_parameters.count("vdw_s8") != 0) {
        INPUT.vdw_s8
            = static_cast<SimpleString*>(input_parameters["vdw_s8"].get())
                  ->c_str();
    } else if (input_parameters.count("vdw_a1") != 0) {
        INPUT.vdw_a1
            = static_cast<SimpleString*>(input_parameters["vdw_a1"].get())
                  ->c_str();
    } else if (input_parameters.count("vdw_a2") != 0) {
        INPUT.vdw_a2
            = static_cast<SimpleString*>(input_parameters["vdw_a2"].get())
                  ->c_str();
        ...
        INPUT.vdw_cutoff_radius
            = static_cast<SimpleString*>(
                  input_parameters["vdw_cutoff_radius"].get())
                  ->c_str();
    } else if (input_parameters.count("vdw_radius_unit") != 0) {
        INPUT.vdw_radius_unit = static_cast<SimpleString*>(
                                    input_parameters["vdw_radius_unit"].get())
                                    ->c_str();
    } else if (input_parameters.count("vdw_cn_thr") != 0) {
        INPUT.vdw_cn_thr
            = *static_cast<double*>(input_parameters["vdw_cn_thr"].get());
    } else if (input_parameters.count("vdw_cn_thr_unit") != 0) {
        INPUT.vdw_cn_thr_unit = static_cast<SimpleString*>(
                                    input_parameters["vdw_cn_thr_unit"].get())
                                    ->c_str();
    } else if (input_parameters.count("vdw_C6_file") != 0) {
        INPUT.vdw_C6_file
            = static_cast<SimpleString*>(input_parameters["vdw_C6_file"].get())
                  ->c_str();
    } else if (input_parameters.count("vdw_C6_unit") != 0) {
        INPUT.vdw_C6_unit
            = static_cast<SimpleString*>(input_parameters["vdw_C6_unit"].get())
                  ->c_str();
    } else if (input_parameters.count("vdw_R0_file") != 0) {
        INPUT.vdw_R0_file
            = static_cast<SimpleString*>(input_parameters["vdw_R0_file"].get())
                  ->c_str();
    } else if (input_parameters.count("vdw_R0_unit") != 0) {
        INPUT.vdw_R0_unit
            = static_cast<SimpleString*>(input_parameters["vdw_R0_unit"].get())
                  ->c_str();
    } else if (input_parameters.count("vdw_cutoff_type") != 0) {
        INPUT.vdw_cutoff_type = static_cast<SimpleString*>(
                                    input_parameters["vdw_cutoff_type"].get())
                                    ->c_str();
    } else if (input_parameters.count("vdw_cutoff_period") != 0) {
        // INPUT.vdw_cutoff_period =
        // static_cast<ModuleBase::Vector3<int>>(input_parameters["vdw_cutoff_period"].get());
        ...
        INPUT.ocp_set
            = static_cast<SimpleString*>(input_parameters["ocp_set"].get())
                  ->c_str();
        ...
        INPUT.noncolin
            = *static_cast<bool*>(input_parameters["noncolin"].get());
    } else if (input_parameters.count("lspinorb") != 0) {
        INPUT.lspinorb
            = *static_cast<bool*>(input_parameters["lspinorb"].get());
    } else if (input_parameters.count("soc_lambda") != 0) {
        INPUT.soc_lambda
            = *static_cast<double*>(input_parameters["soc_lambda"].get());
    } else if (input_parameters.count("exx_hybrid_alpha") != 0) {
        INPUT.exx_hybrid_alpha = static_cast<SimpleString*>(
                                     input_parameters["exx_hybrid_alpha"].get())
                                     ->c_str();
    } else if (input_parameters.count("exx_hse_omega") != 0) {
        INPUT.exx_hse_omega
            = *static_cast<double*>(input_parameters["exx_hse_omega"].get());
    } else if (input_parameters.count("exx_separate_loop") != 0) {
        INPUT.exx_separate_loop
            = *static_cast<bool*>(input_parameters["exx_separate_loop"].get());
    } else if (input_parameters.count("exx_hybrid_step") != 0) {
        INPUT.exx_hybrid_step
            = *static_cast<int*>(input_parameters["exx_hybrid_step"].get());
    } else if (input_parameters.count("exx_mixing_beta") != 0) {
        INPUT.exx_mixing_beta
            = *static_cast<double*>(input_parameters["exx_mixing_beta"].get());
    } else if (input_parameters.count("exx_lambda") != 0) {
        INPUT.exx_lambda
            = *static_cast<double*>(input_parameters["exx_lambda"].get());
    } else if (input_parameters.count("exx_real_number") != 0) {
        INPUT.exx_real_number = static_cast<SimpleString*>(
                                    input_parameters["exx_real_number"].get())
                                    ->c_str();
    } else if (input_parameters.count("exx_pca_threshold") != 0) {
        INPUT.exx_pca_threshold = *static_cast<double*>(
            input_parameters["exx_pca_threshold"].get());
    } else if (input_parameters.count("exx_c_threshold") != 0) {
        INPUT.exx_c_threshold
            = *static_cast<double*>(input_parameters["exx_c_threshold"].get());
    } else if (input_parameters.count("exx_v_threshold") != 0) {
        INPUT.exx_v_threshold
            = *static_cast<double*>(input_parameters["exx_v_threshold"].get());
    } else if (input_parameters.count("exx_dm_threshold") != 0) {
        INPUT.exx_dm_threshold
            = *static_cast<double*>(input_parameters["exx_dm_threshold"].get());
    } else if (input_parameters.count("exx_schwarz_threshold") != 0) {
        INPUT.exx_schwarz_threshold = *static_cast<double*>(
            input_parameters["exx_schwarz_threshold"].get());
    } else if (input_parameters.count("exx_cauchy_threshold") != 0) {
        INPUT.exx_cauchy_threshold = *static_cast<double*>(
            input_parameters["exx_cauchy_threshold"].get());
    } else if (input_parameters.count("exx_c_grad_threshold") != 0) {
        INPUT.exx_c_grad_threshold = *static_cast<double*>(
            input_parameters["exx_c_grad_threshold"].get());
    } else if (input_parameters.count("exx_v_grad_threshold") != 0) {
        INPUT.exx_v_grad_threshold = *static_cast<double*>(
            input_parameters["exx_v_grad_threshold"].get());
    } else if (input_parameters.count("exx_cauchy_force_threshold") != 0) {
        INPUT.exx_cauchy_force_threshold = *static_cast<double*>(
            input_parameters["exx_cauchy_force_threshold"].get());
    } else if (input_parameters.count("exx_cauchy_stress_threshold") != 0) {
        INPUT.exx_cauchy_stress_threshold = *static_cast<double*>(
            input_parameters["exx_cauchy_stress_threshold"].get());
    } else if (input_parameters.count("exx_ccp_threshold") != 0) {
        INPUT.exx_ccp_threshold = *static_cast<double*>(
            input_parameters["exx_ccp_threshold"].get());
    } else if (input_parameters.count("exx_ccp_rmesh_times") != 0) {
        INPUT.exx_ccp_rmesh_times
            = static_cast<SimpleString*>(
                  input_parameters["exx_ccp_rmesh_times"].get())
                  ->c_str();
    } else if (input_parameters.count("exx_distribute_type") != 0) {
        INPUT.exx_distribute_type
            = static_cast<SimpleString*>(
                  input_parameters["exx_distribute_type"].get())
                  ->c_str();
    } else if (input_parameters.count("exx_opt_orb_lmax") != 0) {
        INPUT.exx_opt_orb_lmax
            = *static_cast<int*>(input_parameters["exx_opt_orb_lmax"].get());
    } else if (input_parameters.count("exx_opt_orb_ecut") != 0) {
        INPUT.exx_opt_orb_ecut
            = *static_cast<double*>(input_parameters["exx_opt_orb_ecut"].get());
    } else if (input_parameters.count("exx_opt_orb_tolerence") != 0) {
        INPUT.exx_opt_orb_tolerence = *static_cast<double*>(
            input_parameters["exx_opt_orb_tolerence"].get());
    } else if (input_parameters.count("rpa_ccp_rmesh_times") != 0) {
        INPUT.rpa_ccp_rmesh_times = *static_cast<double*>(
            input_parameters["rpa_ccp_rmesh_times"].get());
    } else if (input_parameters.count("td_force_dt") != 0) {
        INPUT.td_force_dt
            = *static_cast<double*>(input_parameters["td_force_dt"].get());
        ...
        INPUT.td_vext_dire
            = static_cast<SimpleString*>(input_parameters["td_vext_dire"].get())
                  ->c_str();
    } else if (input_parameters.count("out_dipole") != 0) {
        INPUT.out_dipole
            = *static_cast<bool*>(input_parameters["out_dipole"].get());
    } else if (input_parameters.count("out_efield") != 0) {
        INPUT.out_efield
            = *static_cast<bool*>(input_parameters["out_efield"].get());
    } else if (input_parameters.count("out_current") != 0) {
        INPUT.out_current
            = *static_cast<bool*>(input_parameters["out_current"].get());
    } else if (input_parameters.count("out_current_k") != 0) {
        INPUT.out_current_k
            = *static_cast<bool*>(input_parameters["out_current_k"].get());
    } else if (input_parameters.count("td_print_eij") != 0) {
        INPUT.td_print_eij
            = *static_cast<double*>(input_parameters["td_print_eij"].get());
        ...
        INPUT.propagator
            = *static_cast<int*>(input_parameters["propagator"].get());
        ...
        INPUT.td_ttype
            = static_cast<SimpleString*>(input_parameters["td_ttype"].get())
                  ->c_str();
    } else if (input_parameters.count("td_tstart") != 0) {
        INPUT.td_tstart
            = *static_cast<int*>(input_parameters["td_tstart"].get());
        ...
        INPUT.td_lcut1
            = *static_cast<double*>(input_parameters["td_lcut1"].get());
    } else if (input_parameters.count("td_lcut2") != 0) {
        INPUT.td_lcut2
            = *static_cast<double*>(input_parameters["td_lcut2"].get());
    } else if (input_parameters.count("td_gauss_freq") != 0) {
        INPUT.td_gauss_freq = static_cast<SimpleString*>(
                                  input_parameters["td_gauss_freq"].get())
                                  ->c_str();
    } else if (input_parameters.count("td_gauss_phase") != 0) {
        INPUT.td_gauss_phase = static_cast<SimpleString*>(
                                   input_parameters["td_gauss_phase"].get())
                                   ->c_str();
    } else if (input_parameters.count("td_gauss_sigma") != 0) {
        INPUT.td_gauss_sigma = static_cast<SimpleString*>(
                                   input_parameters["td_gauss_sigma"].get())
                                   ->c_str();
    } else if (input_parameters.count("td_gauss_t0") != 0) {
        INPUT.td_gauss_t0
            = static_cast<SimpleString*>(input_parameters["td_gauss_t0"].get())
                  ->c_str();
    } else if (input_parameters.count("td_gauss_amp") != 0) {
        INPUT.td_gauss_amp
            = static_cast<SimpleString*>(input_parameters["td_gauss_amp"].get())
                  ->c_str();
    } else if (input_parameters.count("td_trape_freq") != 0) {
        INPUT.td_trape_freq = static_cast<SimpleString*>(
                                  input_parameters["td_trape_freq"].get())
                                  ->c_str();
    } else if (input_parameters.count("td_trape_phase") != 0) {
        INPUT.td_trape_phase = static_cast<SimpleString*>(
                                   input_parameters["td_trape_phase"].get())
                                   ->c_str();
    } else if (input_parameters.count("td_trape_t1") != 0) {
        INPUT.td_trape_t1
            = static_cast<SimpleString*>(input_parameters["td_trape_t1"].get())
                  ->c_str();
    } else if (input_parameters.count("td_trape_t2") != 0) {
        INPUT.td_trape_t2
            = static_cast<SimpleString*>(input_parameters["td_trape_t2"].get())
                  ->c_str();
    } else if (input_parameters.count("td_trape_t3") != 0) {
        INPUT.td_trape_t3
            = static_cast<SimpleString*>(input_parameters["td_trape_t3"].get())
                  ->c_str();
    } else if (input_parameters.count("td_trape_amp") != 0) {
        INPUT.td_trape_amp
            = static_cast<SimpleString*>(input_parameters["td_trape_amp"].get())
                  ->c_str();
    } else if (input_parameters.count("td_trigo_freq1") != 0) {
        INPUT.td_trigo_freq1 = static_cast<SimpleString*>(
                                   input_parameters["td_trigo_freq1"].get())
                                   ->c_str();
    } else if (input_parameters.count("td_trigo_freq2") != 0) {
        INPUT.td_trigo_freq2 = static_cast<SimpleString*>(
                                   input_parameters["td_trigo_freq2"].get())
                                   ->c_str();
    } else if (input_parameters.count("td_trigo_phase1") != 0) {
        INPUT.td_trigo_phase1 = static_cast<SimpleString*>(
                                    input_parameters["td_trigo_phase1"].get())
                                    ->c_str();
    } else if (input_parameters.count("td_trigo_phase2") != 0) {
        INPUT.td_trigo_phase2 = static_cast<SimpleString*>(
                                    input_parameters["td_trigo_phase2"].get())
                                    ->c_str();
    } else if (input_parameters.count("td_trigo_amp") != 0) {
        INPUT.td_trigo_amp
            = static_cast<SimpleString*>(input_parameters["td_trigo_amp"].get())
                  ->c_str();
    } else if (input_parameters.count("td_heavi_t0") != 0) {
        INPUT.td_heavi_t0
            = static_cast<SimpleString*>(input_parameters["td_heavi_t0"].get())
                  ->c_str();
    } else if (input_parameters.count("td_heavi_amp") != 0) {
        INPUT.td_heavi_amp
            = static_cast<SimpleString*>(input_parameters["td_heavi_amp"].get())
                  ->c_str();
    } else if (input_parameters.count("restart_save") != 0) {
        INPUT.restart_save
            = *static_cast<bool*>(input_parameters["restart_save"].get());
    } else if (input_parameters.count("restart_load") != 0) {
        INPUT.restart_load
            = *static_cast<bool*>(input_parameters["restart_load"].get());
    } else if (input_parameters.count("input_error") != 0) {
        INPUT.input_error
            = *static_cast<bool*>(input_parameters["input_error"].get());
    } else if (input_parameters.count("cell_factor") != 0) {
        INPUT.cell_factor
            = *static_cast<double*>(input_parameters["cell_factor"].get());
    } else if (input_parameters.count("dft_plus_u") != 0) {
        INPUT.dft_plus_u
            = *static_cast<int*>(input_parameters["dft_plus_u"].get());
    } else if (input_parameters.count("orbital_corr") != 0) {
        bool close_plus_u = true;
        bool dmft_flag = false;
        SimpleVector<double> vec_D = *static_cast<SimpleVector<double>*>(
            input_parameters["orbital_corr"].get());
        for (int i = 0; i < INPUT.ntype; i++) {
            INPUT.orbital_corr[i] = vec_D[i];
            if ((INPUT.orbital_corr[i] != -1) && (INPUT.orbital_corr[i] != 0)
                && (INPUT.orbital_corr[i] != 1) && (INPUT.orbital_corr[i] != 2)
                && (INPUT.orbital_corr[i] != 3)) {
        ...
        SimpleVector<double> vec_D = *static_cast<SimpleVector<double>*>(
            input_parameters["hubbard_u"].get());
        for (int i = 0; i < INPUT.ntype; i++) {
        ...
        INPUT.onsite_radius
            = *static_cast<double*>(input_parameters["onsite_radius"].get());
        ...
        INPUT.yukawa_potential
            = *static_cast<bool*>(input_parameters["yukawa_potential"].get());
    } else if (input_parameters.count("yukawa_lambda") != 0) {
        INPUT.yukawa_lambda
            = *static_cast<double*>(input_parameters["yukawa_lambda"].get());
    } else if (input_parameters.count("dft_plus_dmft") != 0) {
        INPUT.dft_plus_dmft
            = *static_cast<bool*>(input_parameters["dft_plus_dmft"].get());
        ...
        INPUT.coulomb_type
            = static_cast<SimpleString*>(input_parameters["coulomb_type"].get())
                  ->c_str();
    } else if (input_parameters.count("deepks_out_labels") != 0) {
        INPUT.deepks_out_labels
            = *static_cast<bool*>(input_parameters["deepks_out_labels"].get());
    } else if (input_parameters.count("deepks_scf") != 0) {
        INPUT.deepks_scf
            = *static_cast<bool*>(input_parameters["deepks_scf"].get());
    } else if (input_parameters.count("deepks_equiv") != 0) {
        INPUT.deepks_equiv
            = *static_cast<bool*>(input_parameters["deepks_equiv"].get());
    } else if (input_parameters.count("deepks_bandgap") != 0) {
        INPUT.deepks_bandgap
            = *static_cast<bool*>(input_parameters["deepks_bandgap"].get());
    } else if (input_parameters.count("deepks_out_unittest") != 0) {
        INPUT.deepks_out_unittest = *static_cast<bool*>(
            input_parameters["deepks_out_unittest"].get());
    } else if (input_parameters.count("deepks_model") != 0) {
        INPUT.deepks_model
            = static_cast<SimpleString*>(input_parameters["deepks_model"].get())
                  ->c_str();
        ...
        INPUT.sigma_k
            = *static_cast<double*>(input_parameters["sigma_k"].get());
        ...
        INPUT.of_kinetic
            = static_cast<SimpleString*>(input_parameters["of_kinetic"].get())
                  ->c_str();
    } else if (input_parameters.count("of_method") != 0) {
        INPUT.of_method
            = static_cast<SimpleString*>(input_parameters["of_method"].get())
                  ->c_str();
    } else if (input_parameters.count("of_conv") != 0) {
        INPUT.of_conv
            = static_cast<SimpleString*>(input_parameters["of_conv"].get())
                  ->c_str();
    } else if (input_parameters.count("of_tole") != 0) {
        INPUT.of_tole
            = *static_cast<double*>(input_parameters["of_tole"].get());
    } else if (input_parameters.count("of_tolp") != 0) {
        INPUT.of_tolp
            = *static_cast<double*>(input_parameters["of_tolp"].get());
    } else if (input_parameters.count("of_tf_weight") != 0) {
        INPUT.of_tf_weight
            = *static_cast<double*>(input_parameters["of_tf_weight"].get());
    } else if (input_parameters.count("of_vw_weight") != 0) {
        INPUT.of_vw_weight
            = *static_cast<double*>(input_parameters["of_vw_weight"].get());
    } else if (input_parameters.count("of_wt_alpha") != 0) {
        INPUT.of_wt_alpha
            = *static_cast<double*>(input_parameters["of_wt_alpha"].get());
    } else if (input_parameters.count("of_wt_beta") != 0) {
        INPUT.of_wt_beta
            = *static_cast<double*>(input_parameters["of_wt_beta"].get());
    } else if (input_parameters.count("of_wt_rho0") != 0) {
        INPUT.of_wt_rho0
            = *static_cast<double*>(input_parameters["of_wt_rho0"].get());
    } else if (input_parameters.count("of_hold_rho0") != 0) {
        INPUT.of_hold_rho0
            = *static_cast<bool*>(input_parameters["of_hold_rho0"].get());
    } else if (input_parameters.count("of_lkt_a") != 0) {
        INPUT.of_lkt_a
            = *static_cast<double*>(input_parameters["of_lkt_a"].get());
    } else if (input_parameters.count("of_full_pw") != 0) {
        INPUT.of_full_pw
            = *static_cast<bool*>(input_parameters["of_full_pw"].get());
    } else if (input_parameters.count("of_full_pw_dim") != 0) {
        INPUT.of_full_pw_dim
            = *static_cast<int*>(input_parameters["of_full_pw_dim"].get());
    } else if (input_parameters.count("of_read_kernel") != 0) {
        INPUT.of_read_kernel
            = *static_cast<bool*>(input_parameters["of_read_kernel"].get());
    } else if (input_parameters.count("of_kernel_file") != 0) {
        INPUT.of_kernel_file = static_cast<SimpleString*>(
                                   input_parameters["of_kernel_file"].get())
                                   ->c_str();
    } else if (input_parameters.count("bessel_nao_smooth") != 0) {
        INPUT.bessel_nao_smooth
            = *static_cast<bool*>(input_parameters["bessel_nao_smooth"].get());
    } else if (input_parameters.count("bessel_nao_sigma") != 0) {
        INPUT.bessel_nao_sigma
            = *static_cast<double*>(input_parameters["bessel_nao_sigma"].get());
    } else if (input_parameters.count("bessel_nao_ecut") != 0) {
        INPUT.bessel_nao_ecut = static_cast<SimpleString*>(
                                    input_parameters["bessel_nao_ecut"].get())
                                    ->c_str();
    } else if (input_parameters.count("bessel_nao_rcut") != 0) {
        INPUT.bessel_nao_rcut
            = *static_cast<double*>(input_parameters["bessel_nao_rcut"].get());
    } else if (input_parameters.count("bessel_nao_tolerence") != 0) {
        INPUT.bessel_nao_tolerence = *static_cast<double*>(
            input_parameters["bessel_nao_tolerence"].get());
    } else if (input_parameters.count("bessel_descriptor_lmax") != 0) {
        INPUT.bessel_descriptor_lmax = *static_cast<int*>(
            input_parameters["bessel_descriptor_lmax"].get());
    } else if (input_parameters.count("bessel_descriptor_smooth") != 0) {
        INPUT.bessel_descriptor_smooth = *static_cast<bool*>(
            input_parameters["bessel_descriptor_smooth"].get());
    } else if (input_parameters.count("bessel_descriptor_sigma") != 0) {
        INPUT.bessel_descriptor_sigma = *static_cast<double*>(
            input_parameters["bessel_descriptor_sigma"].get());
    } else if (input_parameters.count("bessel_descriptor_ecut") != 0) {
        INPUT.bessel_descriptor_ecut
            = static_cast<SimpleString*>(
                  input_parameters["bessel_descriptor_ecut"].get())
                  ->c_str();
    } else if (input_parameters.count("bessel_descriptor_rcut") != 0) {
        INPUT.bessel_descriptor_rcut = *static_cast<double*>(
            input_parameters["bessel_descriptor_rcut"].get());
    } else if (input_parameters.count("bessel_descriptor_tolerence") != 0) {
        INPUT.bessel_descriptor_tolerence = *static_cast<double*>(
            input_parameters["bessel_descriptor_tolerence"].get());
    } else if (input_parameters.count("device") != 0) {
        INPUT.device
            = static_cast<SimpleString*>(input_parameters["device"].get())
                  ->c_str();
    } else if (input_parameters.count("precision") != 0) {
        INPUT.precision
            = static_cast<SimpleString*>(input_parameters["precision"].get())
                  ->c_str();
    } else if (input_parameters.count("test_skip_ewald") != 0) {
        INPUT.test_skip_ewald
            = *static_cast<bool*>(input_parameters["test_skip_ewald"].get());
    }
}
