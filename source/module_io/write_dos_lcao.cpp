        ...
        for (int i = 0; i < GlobalV::NBANDS; ++i) {
            ...
            for (int j = 0; j < GlobalV::NLOCAL; ++j) {
                ...
                if (pv.in_this_processor(j, i)) {
                    ...
                    BlasConnector::axpy(np,
                                        x,
                                        Gauss,
                                        1,
                                        pdosk[is].c + j * pdosk[is].nc,
                                        1);
                }
            }
        } // ib
    ...
    if (GlobalV::MY_RANK == 0) {
        {
            ...
            if (GlobalV::NSPIN == 1 || GlobalV::NSPIN == 4) {
                ...
                for (int n = 0; n < npoints; ++n) {
                    ...
                    out << std::setw(20) << en << std::setw(30) << y
                        << std::endl;
                }
            ...
            for (int i = 0; i < GlobalC::ucell.nat; i++) {
                ...
                for (int j = 0; j < atom1->nw; ++j) {
                    ...
                    out << std::setw(5) << "atom_index=\"" << std::setw(40)
                        << i + 1 << "\"" << std::endl;
                    out << std::setw(8) << "species=\""
                        << GlobalC::ucell.atoms[t].label << "\"" << std::endl;
                    ...
                    if (GlobalV::NSPIN == 1) {
                        ...
                        for (int n = 0; n < npoints; ++n) {
                            out << std::setw(20) << pdos[0](w, n)
                                << std::setw(30) << pdos[1](w, n) << std::endl;
                        } // n
