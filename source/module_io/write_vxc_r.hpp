...
template <typename TR>
std::map<Abfs::Vector3_Order<int>, std::map<size_t, std::map<size_t, TR>>> cal_HR_sparse(
    const hamilt::HContainer<TR>& hR,
    const int current_spin,
    const double sparse_thr);

template <>
std::map<Abfs::Vector3_Order<int>, std::map<size_t, std::map<size_t, double>>> cal_HR_sparse(
...
template <>
std::map<Abfs::Vector3_Order<int>, std::map<size_t, std::map<size_t, std::complex<double>>>> cal_HR_sparse(
                 ...
                 const double sparse_thr = 1e-10)
{
    ...
    for (int is = 0; is < nspin0; ++is)
    {
        ...
        ModuleIO::save_sparse(cal_HR_sparse(vxcs_R_ao[is], is, sparse_thr),
                              all_R_coor,
                              sparse_thr,
                              false, // binary
