...
template <>
void ModuleIO::write_proj_band_lcao(const psi::Psi<double>* psi,
                                    LCAO_Matrix& lm,
    ...
    const double* sk
        = dynamic_cast<const hamilt::HamiltLCAO<double, double>*>(p_ham)
              ->getSk();
    ...
    for (int is = 0; is < nspin0; is++) {
        ...
        for (int i = 0; i < GlobalV::NBANDS; ++i) {
            ...
            for (int j = 0; j < GlobalV::NLOCAL; ++j) {
                ...
                if (pv->in_this_processor(j, i)) {
                    ...
                    weightk(is, i * GlobalV::NLOCAL + j)
                        = Mulk[0](ic, ir) * psi[0](ic, ir);
                }
            }
        } // ib
#ifdef __MPI
        MPI_Reduce(weightk.real().c,
                   weight.c,
                   NUM,
                   MPI_DOUBLE,
                   MPI_SUM,
        ...
        if (GlobalV::MY_RANK == 0) {
            ...
            GlobalV::ofs_running
                << "\n Output projected bands in file: " << ps2.str()
                << std::endl;
            ...
            if (GlobalV::NSPIN == 4)
                out << "<norbitals>" << std::setw(2) << GlobalV::NLOCAL / 2
                    << "</norbitals>" << std::endl;
            else
                out << "<norbitals>" << std::setw(2) << GlobalV::NLOCAL
                    << "</norbitals>" << std::endl;
            out << "<band_structure nkpoints=\"" << nks << "\" nbands=\""
                << GlobalV::NBANDS << "\" units=\"eV\">" << std::endl;
            ...
            for (int i = 0; i < ucell.nat; i++) {
                ...
                for (int j = 0; j < atom1->nw; ++j) {
                    ...
                    out << std::setw(6) << "index=\"" << std::setw(40) << w + 1
                        << "\"" << std::endl;
                    out << std::setw(5) << "atom_index=\"" << std::setw(40)
                        << i + 1 << "\"" << std::endl;
                    out << std::setw(8) << "species=\"" << ucell.atoms[t].label
                        << "\"" << std::endl;
                    out << std::setw(2) << "l=\"" << std::setw(40) << L1 << "\""
                        << std::endl;
                    out << std::setw(2) << "m=\"" << std::setw(40) << m1 << "\""
                        << std::endl;
                    out << std::setw(2) << "z=\"" << std::setw(40) << N1 + 1
                        << "\"" << std::endl;
                    out << ">" << std::endl;
                    out << "<data>" << std::endl;
                    for (int ib = 0; ib < GlobalV::NBANDS; ib++) {
                        if (GlobalV::NSPIN == 1 || GlobalV::NSPIN == 2)
                            out << std::setw(13)
                                << weight(is, ib * GlobalV::NLOCAL + w);
                        else if (GlobalV::NSPIN == 4) {
                            int w0 = w - s0;
                            out << std::setw(13)
                                << weight(is,
                                          ib * GlobalV::NLOCAL + s0 + 2 * w0)
                                       + weight(is,
                                                ib * GlobalV::NLOCAL + s0
                                                    + 2 * w0 + 1);
                        }
                    }
                    out << std::endl;
            ...
            if (is == kv.isk[ik]) {
                ...
                if (GlobalV::NSPIN == 4) {
                    dynamic_cast<hamilt::HamiltLCAO<std::complex<double>,
                        ...
                        ->updateSk(ik, 1);
                    sk = dynamic_cast<
                             const hamilt::HamiltLCAO<std::complex<double>,
                                                      std::complex<double>>*>(
                             p_ham)
                             ->getSk();
                } else {
