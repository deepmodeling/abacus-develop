    ...
    if (GlobalV::DRANK == 0)
    {
        if (binary)
        {
            ...
            for (int ispin = 0; ispin < spin_loop; ++ispin)
            {
                ...
                g1[ispin].write(reinterpret_cast<char*>(&nlocal), sizeof(int));
        ...
        else
        {
            for (int ispin = 0; ispin < spin_loop; ++ispin)
            {
                ...
                g1[ispin] << "Matrix Dimension of H(R): " << PARAM.globalv.nlocal << std::endl;
                              ...
                              const std::string& fileflag)
{
    ...
    if (PARAM.inp.calculation == "md" && !PARAM.inp.out_app_flag)
    {
        sshx[0] << PARAM.globalv.global_matrix_dir << "d" << fileflag << "rxs1g" << step << ".csr";
        sshx[1] << PARAM.globalv.global_matrix_dir << "d" << fileflag << "rxs2g" << step << ".csr";
        sshy[0] << PARAM.globalv.global_matrix_dir << "d" << fileflag << "rys1g" << step << ".csr";
        sshy[1] << PARAM.globalv.global_matrix_dir << "d" << fileflag << "rys2g" << step << ".csr";
        sshz[0] << PARAM.globalv.global_matrix_dir << "d" << fileflag << "rzs1g" << step << ".csr";
        sshz[1] << PARAM.globalv.global_matrix_dir << "d" << fileflag << "rzs2g" << step << ".csr";
    }
    else
    {
