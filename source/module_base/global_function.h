...
template <class T>
void OUTP(std::ofstream& ofs,
          const std::string& name,
          const T& a,
          const std::string& explanation = "") {
    ofs << std::setw(30) << name << " " << a << " #" << explanation
        << std::endl;
}
...
//==========================================================
void DONE(std::ofstream& ofs,
          const std::string& description,
          bool only_rank0 = false);
...
inline void ZEROS(std::complex<T>* u,
                  const TI n) // Peize Lin change int to TI at 2020.03.03
{
    assert(n >= 0);
    for (TI i = 0; i < n; i++) {
...
// ifwarn: whether to call GlobalV::ofs_warning when the TargetName is not
// found, used to avoid invalid warning. Mohan warning : the last term can't be
// written as const bool &restart, I don't know why.

void SCAN_END(std::ifstream& ifs,
              const std::string& TargetName,
              const bool ifwarn = true);
// ifwarn: whether to call GlobalV::ofs_warning when the TargetName is not
// found, used to avoid invalid warning.
...
template <>
inline void COPYARRAY(const std::complex<double>* a,
...
template <typename T_map, typename T_key1, typename... T_key_tail>
inline const void* MAP_EXIST(const T_map& ms,
                             const T_key1& key1,
...
static inline bool IS_COLUMN_MAJOR_KS_SOLVER() {
    return GlobalV::KS_SOLVER == "genelpa" || GlobalV::KS_SOLVER == "lapack"
