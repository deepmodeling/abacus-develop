...
void CubicSpline::sanity_check(const int n, const double* const x, const double* const y, BoundaryCondition bc_start, BoundaryCondition bc_end) {
        ...
        if ( bc_start == BoundaryCondition::periodic ) {
            ...
            switch (bc_end) {
                case BoundaryCondition::not_a_knot:
                    ...
                    s[n-1] =  (dd[n-2]*dx[n-3]*(dx[n-2]+2*(x[n-1]-x[n-3])) + dd[n-3]*dx[n-2]*dx[n-2]) / (x[n-1]-x[n-3]);
                    break;
                case BoundaryCondition::first_deriv:
                    diag[n-1] = 1.0 * dx[n-2];
                    subdiag[n-2] = 0.0;
...
void CubicSpline::interpolate(const int n, const double* const x, double* const y) {
    int p = 0;
    double w = 0.0;
...
void CubicSpline::solve_cyctri(const int n, double* const diag, double* const supdiag, double* const subdiag, double* const b) {
    ...
    double fac = ( beta*supdiag[n-1]*bp[0] + alpha*subdiag[n-1]*bp[n-1] ) / (1. + beta*supdiag[n-1]*bp[n] + alpha*subdiag[n-1]*bp[2*n-1] );
