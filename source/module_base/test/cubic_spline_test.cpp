...
TEST_F(CubicSplineTest, NotAKnot) {
    ...
    cubspl.interpolate(ni, x_interp_, y_interp_);
    for (int i = 0; i != ni; ++i) {
...
TEST_F(CubicSplineTest, FirstDeriv) {
    ...
    cubspl.build(n, x_, y_, CubicSpline::BoundaryCondition::first_deriv, CubicSpline::BoundaryCondition::first_deriv, -3, -0.5); 
...
TEST_F(CubicSplineTest, SecondDeriv) {
    ...
    cubspl.build(n, x_, y_, CubicSpline::BoundaryCondition::second_deriv, CubicSpline::BoundaryCondition::second_deriv, -1, -0.01); 
...
TEST_F(CubicSplineTest, TwoPoints) {
    ...
    // first derivative
    cubspl.build(2, x_, y_, CubicSpline::BoundaryCondition::first_deriv, CubicSpline::BoundaryCondition::first_deriv, 0.8, 1.5); 
    cubspl.interpolate(ni, x_interp_, y_interp_);

    EXPECT_NEAR(y_interp_[0], 2.33, tol);
    EXPECT_NEAR(y_interp_[1], 2.4373, tol);
    EXPECT_NEAR(y_interp_[2], 2.8487171, tol);
    EXPECT_NEAR(y_interp_[3], 4.1597, tol);
    EXPECT_NEAR(y_interp_[4], 4.33, tol);

    // second derivative
    cubspl.build(2, x_, y_, CubicSpline::BoundaryCondition::second_deriv, CubicSpline::BoundaryCondition::second_deriv, 0.8, 1.5); 
    cubspl.interpolate(ni, x_interp_, y_interp_);

    EXPECT_NEAR(y_interp_[0], 2.33, tol);
    EXPECT_NEAR(y_interp_[1], 2.48245, tol);
    EXPECT_NEAR(y_interp_[2], 2.86725265, tol);
    EXPECT_NEAR(y_interp_[3], 4.07405, tol);
    EXPECT_NEAR(y_interp_[4], 4.33, tol);
...
TEST_F(CubicSplineTest, ThreePoints) {
    ...
    EXPECT_NEAR(y_interp_[0], 1.2, tol);
    EXPECT_NEAR(y_interp_[1], 1.5075, tol);
    EXPECT_NEAR(y_interp_[2], 2.259025, tol);
    EXPECT_NEAR(y_interp_[3], 4.3875, tol);
    EXPECT_NEAR(y_interp_[4], 4.8, tol);
    ...
    EXPECT_NEAR(y_interp_[0], 1.2, tol);
    EXPECT_NEAR(y_interp_[1], 1.44375, tol);
    EXPECT_NEAR(y_interp_[2], 2.35383125, tol);
    EXPECT_NEAR(y_interp_[3], 1.2361111111111112, tol);
    EXPECT_NEAR(y_interp_[4], 1.2, tol);
}
