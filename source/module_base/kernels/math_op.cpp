...
#pragma omp parallel for
        ...
        for (int ig = 0; ig < ng; ig++) {
            ...
            // NAME : phi
            //----------------------------------------------------------
            const FPTYPE gmod = std::sqrt(g[ig * 3 + 0] * g[ig * 3 + 0] + g[ig * 3 + 1] * g[ig * 3 + 1] + g[ig * 3 + 2] * g[ig * 3 + 2]);
            FPTYPE cost = gmod < 1.0e-9 ? 0.0 : g[ig * 3 + 2] / gmod;
            FPTYPE phi;
            //  beware the arc tan, it is defined modulo pi
            if (g[ig * 3 + 0] > 1.0e-9) {
                phi = std::atan(g[ig * 3 + 1] / g[ig * 3 + 0]);
            }
            else if (g[ig * 3 + 0] < -1.e-9) {
                phi = std::atan(g[ig * 3 + 1] / g[ig * 3 + 0]) + PI;
            }
            else {
            ...
            for (int l = 0; l <= lmax; l++) {
                const FPTYPE c = std::sqrt((2 * l + 1) / FOUR_PI);
                if (l == 0) {
                    ...
                    p[l * (lmax + 1) * ng + l * ng + ig] = _semi_fact(l3) * pow(x2, static_cast<FPTYPE>(l) / 2.0);//mohan modify 2007-10-13
                    if (l % 2 == 1) {
                ...
                for (int m = 1; m <= l; m++) {
                    // Y_lm, m > 0
                    const FPTYPE same =
                            c * std::sqrt(_fact<FPTYPE>(l - m) /
                                     _fact<FPTYPE>(l + m)) * SQRT2;
                    FPTYPE sinp, cosp;
