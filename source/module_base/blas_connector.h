...
class BlasConnector
{
  public:
    ...
    static inline double nrm2(const int n, const std::complex<double>* X, const int incX)
    {
        return dznrm2_(&n, X, &incX);
    }

    // copies a into b
    static inline void copy(const long n, const double* a, const int incx, double* b, const int incy)
    {
        dcopy_(&n, a, &incx, b, &incy);
    }
    static inline void copy(const long n,
                            const std::complex<double>* a,
                            const int incx,
                            std::complex<double>* b,
                            const int incy)
...
/// The usage of col-major interface (and the defination of row/col major) can be referred to the [blas
/// documentation](https://www.intel.com/content/www/us/en/docs/onemkl/developer-reference-c/2023-0/cblas-gemm-001.html).
