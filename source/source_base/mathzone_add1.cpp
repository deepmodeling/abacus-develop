...
double
    Mathzone_Add1::factorial (const int& l)
{
    if (l == 0 || l == 1)
        return 1.0;
    else
...
double
    Mathzone_Add1::dualfac (const int& l)
{
    if (l == -1 || l == 0)
        return 1.0;
    else
...
void
    Mathzone_Add1::Cubic_Spline_Interpolation (const double* const rad,
    ...
    for (int m = 0; m < rsize; m++)
        {
            ...
            const double dy_tmp = (rad_f[khi] - rad_f[klo]) / h - (3.0 * a * a - 1.0) / 6.0 * h * y2[klo]
                                  + (3.0 * b * b - 1.0) / 6.0 * h * y2[khi];
            dy[m] = dy_tmp;
            const double y_tmp = a * rad_f[klo] + b * rad_f[khi]
...
double
    Mathzone_Add1::Uni_RadialF (const double* old_phi, const int& msh, const double& dr, const double& newr)
{
    ...
    if (newr < 0.0)
        {
            throw std::runtime_error ("newr should >= 0. " + std::string (__FILE__) + " line "
...
void
    Mathzone_Add1::Uni_Deriv_Phi (const double* radf, const int& mesh, const double& dr, const int& nd, double* phind)
{
