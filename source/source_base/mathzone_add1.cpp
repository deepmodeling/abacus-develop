...
void Mathzone_Add1::Cubic_Spline_Interpolation(const double* const rad,
...
#pragma omp parallel for schedule(static)
    ...
    for (int m = 0; m < rsize; m++) {
        ...
        dy[m] = dy_tmp;
        const double y_tmp =
            a * rad_f[klo] + b * rad_f[khi] + ((a * a * a - a) * y2[klo] + (b * b * b - b) * y2[khi]) * (h * h) / 6.0;
        y[m] = y_tmp;
...
double Mathzone_Add1::Uni_RadialF(const double* old_phi, const int& msh, const double& dr, const double& newr) {
...
void Mathzone_Add1::Uni_Deriv_Phi(const double* radf, const int& mesh, const double& dr, const int& nd, double* phind) {
