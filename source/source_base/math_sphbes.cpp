...
void
    Sphbes::BESSJY (double x, double xnu, double* rj, double* ry, double* rjp, double* ryp)
{
    const int XMIN = 2.0;
...
void
    Sphbes::BESCHB (double x, double* gam1, double* gam2, double* gampl, double* gammi)
{
    const int NUSE1 = 7;
    const int NUSE2 = 8;
    double xx = 0;
...
double
    Sphbes::CHEBEV (double a, double b, double c[], int m, double x)
{
    double d = 0.0;
    double dd = 0.0;
    double sv = 0.0;
...
double
    Sphbes::Spherical_Bessel_7 (const int n, const double& x)
{
    if (x == 0)
        {
            if (n != 0)
...
void
    Sphbes::Spherical_Bessel_Roots (const int& num,
                                    const int& l,
...
void
    Sphbes::Spherical_Bessel (const int& msh,  // number of grid points
                    ...
                    for (ir = ir0; ir < msh; ir++)
                        {
                            ...
                            jl[ir]
                                = (std::sin (x1) * (105.0 - 45.0 * x2 + x4) + std::cos (x1) * (10.0 * x3 - 105.0 * x1))
                            ...
                            if (x1 < 0.14)
                            ...
                            else
                                {
                                    ...
                                    jl[ir] = (-cx1 - (945.0 * cx1) / std::pow (x1, 4) + (105.0 * cx1) / (x1 * x1)
                                              + (945.0 * sx1) / std::pow (x1, 5) - (420.0 * sx1) / std::pow (x1, 3)
                                              + (15.0 * sx1) / x1)
                                             / x1;
                                }
                        }
...
void
    Sphbes::dSpherical_Bessel_dx (const int& msh,  // number of grid points
                                  const double* r, // radial grid
                                  const double& q, // wave std::vector
                                  const int& l,    // angular momentum
...
double
    Sphbes::_sphbesj_ascending_recurrence (int l, double x)
{
...
double
    Sphbes::_sphbesj_series (int l, double x)
{
...
double
    Sphbes::sphbesj (const int l, const double x)
{
    assert (l >= 0);
    assert (x >= 0);
    ...
    if (x < l)
    ...
    else
        {
            ...
            switch (l)
                ...
                // case 3:
                //   return ( std::sin(x) * (15.0 * invx - 6.0 * x) + std::cos(x) * (x * x - 15.0) ) * std::pow(invx,
                //   3);
                // case 4:
...
double
    Sphbes::dsphbesj (const int l, const double x)
{
    assert (l >= 0);
    assert (x >= 0);
...
void
    Sphbes::sphbesj (const int n, const double* const r, const double q, const int l, double* const jl)
{
    for (int i = 0; i != n; ++i)
        {
...
void
    Sphbes::dsphbesj (const int n, const double* const r, const double q, const int l, double* const djl)
{
    for (int i = 0; i != n; ++i)
        {
...
void
    Sphbes::sphbes_zeros (const int l, const int n, double* const zeros, const bool return_all)
{
    assert (n > 0);
    assert (l >= 0);
