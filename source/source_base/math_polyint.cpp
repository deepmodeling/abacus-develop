...
void PolyInt::Polynomial_Interpolation(const ModuleBase::realArray& table,
                                       const int& dim1,
                                       const int& dim2,
    ...
    y(dim1, dim2, dim_y) = table(dim1, dim2, iq) * x1 * x2 * x3 / 6.0 + table(dim1, dim2, iq + 1) * x0 * x2 * x3 / 2.0 -
...
double PolyInt::Polynomial_Interpolation(const ModuleBase::realArray& table,
                                         const int& dim1,
                                         const int& dim2,
                                         const int& table_length,
                                         const double& table_interval,
    ...
    const double x2 = 2.0 - x0;
    const double x3 = 3.0 - x0;
    const double y = table(dim1, dim2, iq) * x1 * x2 * x3 / 6.0 + table(dim1, dim2, iq + 1) * x0 * x2 * x3 / 2.0 -
                     table(dim1, dim2, iq + 2) * x1 * x0 * x3 / 2.0 + table(dim1, dim2, iq + 3) * x1 * x2 * x0 / 6.0;
...
double PolyInt::Polynomial_Interpolation // pengfei Li 2018-3-23
    ...
    const double x3 = 3.0 - x0;
    const double y =
        table(dim1, dim2, dim3, iq) * x1 * x2 * x3 / 6.0 + table(dim1, dim2, dim3, iq + 1) * x0 * x2 * x3 / 2.0 -
        table(dim1, dim2, dim3, iq + 2) * x1 * x0 * x3 / 2.0 + table(dim1, dim2, dim3, iq + 3) * x1 * x2 * x0 / 6.0;

    //    ModuleBase::timer::end("PolyInt","Poly_Interpo_3");
    return y;
}

double PolyInt::Polynomial_Interpolation(const double* table,
                                         const int& table_length,
                                         const double& table_interval,
                                         const double& x // input value
) {
...
double PolyInt::Polynomial_Interpolation_xy(const double* xpoint,
                                            const double* ypoint,
                                            const int table_length,
                                            const double& x // input value
) {
