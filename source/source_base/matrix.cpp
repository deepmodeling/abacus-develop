...
matrix::matrix (const int nrows, const int ncols, const bool flag_zero) : nr (nrows), nc (ncols), c (nullptr)
{
    if (nr && nc)
        {
...
matrix::matrix (const matrix& m_in) : nr (m_in.nr), nc (m_in.nc), c (nullptr)
{
    if (nr && nc)
        {
            auto handler_old = std::set_new_handler (matrixAlloc);
...
matrix::matrix (matrix&& m_in) : nr (m_in.nr), nc (m_in.nc)
{
    c = m_in.c;
    m_in.nr = m_in.nc = 0;
    m_in.c = nullptr;
...
void
    matrix::create (const int nrow, const int ncol, const bool flag_zero)
{
    if (nrow && ncol)
        {
            if (c)
...
/* Scale a matrix */
matrix
    operator* (const double& s, const matrix& m)
{
    matrix sm (m);
...
/* matrix * double */
matrix
    operator* (const matrix& m, const double& s)
{
    matrix sm (m);
    const int size = m.nr * m.nc;
    for (int i = 0; i < size; i++)
...
void
    matrix::fill_out (const double x)
{
    const int size = nr * nc;
    if (size == 0)
        {
...
void
    matrix::reshape (const int nr_new, const int nc_new, const bool flag_zero)
{
...
double
    trace_on (const matrix& A, const matrix& B)
{
    assert (A.nr == B.nc);
    assert (A.nc == B.nr);
...
double
    mdot (const matrix& A, const matrix& B)
{
    assert (A.nr == B.nr);
    assert (A.nc == B.nc);
...
std::ostream&
    matrix::print (std::ostream& os, const double threshold) const
{
