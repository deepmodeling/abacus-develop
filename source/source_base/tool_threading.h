...
inline void
    TASK_DIST_1D (int nworker, int iworker, T_task ntask, T_out& start, T_out& len)
{
    if (nworker == 1)
        {
            start = 0;
...
template <typename T_task, typename T_out>
inline void
    BLOCK_TASK_DIST_1D (int nworker, int iworker, T_task ntask, T_task block_size, T_out& start, T_out& len)
{
    if (nworker == 1)
        {
            start = 0;
...
inline void
    OMP_PARALLEL (const std::function<void (int, int)>& f)
{
#ifdef _OPENMP
#pragma omp parallel
    {
...
inline void
    TRY_OMP_PARALLEL (const std::function<void (int, int)>& f)
{
#ifdef _OPENMP
    if (!omp_in_parallel ())
        {
