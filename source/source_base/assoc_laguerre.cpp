...
void
    Assoc_Laguerre::generate (const int& n, const int& l, const double ns, double* const& s, double* L)
{
    for (int i = 0; i < ns; i++)
        {
...
void
    Assoc_Laguerre::generate (const int& n, const int& l, std::vector<double>& x, std::vector<double>& y)
{
...
double
    Assoc_Laguerre::laguerre (const int& n, const double x)
{
    if (n == 0)
        {
            return 1;
...
double
    Assoc_Laguerre::associate_laguerre (const int& n, const double x, const int& a)
{
    ...
    if (n == 0)
        ...
        {
            return -x * x * x / 6.0 + (a_ + 3) * x * x / 2.0 - (a_ + 2) * (a_ + 3) * x / 2.0
                   + (a_ + 1) * (a_ + 2) * (a_ + 3) / 6.0;
        }
    else if (n >= 4)
        {
...
int
    Assoc_Laguerre::factorial (const int& n)
{
    if (n == 0)
        {
            return 1;
...
double
    Assoc_Laguerre::value (const int& n, const int& l, const double& s)
{
    int k_ = 2 * l + 1;
    int n_ = n - l - 1;
    if (k_ < 0)
    ...
    for (int iq = 0; iq <= n_; iq++)
        {
            L += std::pow (-s, iq) * static_cast<double> (this->factorial (n_ + k_))
                 / static_cast<double> (this->factorial (n_ - iq)) / static_cast<double> (this->factorial (k_ + iq))
