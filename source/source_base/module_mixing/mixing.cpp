...
void
    Mixing::push_data (Mixing_Data& mdata,
...
void
    Mixing::mix_data (const Mixing_Data& mdata, double* data_mix)
{
    if (mdata.length <= 0)
        return;
...
void
    Mixing::mix_data (const Mixing_Data& mdata, std::complex<double>* data_mix)
{
    if (mdata.length <= 0)
        return;
