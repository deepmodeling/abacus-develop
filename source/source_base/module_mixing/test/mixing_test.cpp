...
double
    ext_inner_product_mock (double* x1, double* x2)
{
    return 0.0;
}
class Mixing_Test : public testing::Test
{
  protected:
    ...
    void
        solve_linear_eq (FPTYPE* x_in, FPTYPE* x_out, bool diff_beta = false)
    {
    ...
    template <typename FPTYPE>
    void
        Kerker_mock (FPTYPE* drho)
    {
    }

    double
        inner_product_mock (double* x1, double* x2)
    {
        double xnorm = 0.0;
    ...
    double
        inner_product_mock (std::complex<double>* x1, std::complex<double>* x2)
    {
        double xnorm = 0.0;
        for (int ir = 0; ir < 3; ++ir)
            {
