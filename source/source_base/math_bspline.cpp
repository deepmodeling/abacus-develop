...
Bspline::~Bspline () { delete[] bezier; }

void
    Bspline::init (int norderin, double Dxin, double xiin)
{
    this->xi = xiin;
    this->Dx = Dxin;
...
double
    Bspline::bezier_ele (int n)
{
    return this->bezier[n];
}

void
    Bspline::getbspline (double x)
{
    bezier[0] = 1.0;
            ...
            for (int n = k; n >= 1; --n)
                {
                    this->bezier[n] = ((x + n * this->Dx - this->xi) * this->bezier[n]
                                       + (this->xi + (k - n + 1) * Dx - x) * this->bezier[n - 1])
                                      / (k * this->Dx);
                }
