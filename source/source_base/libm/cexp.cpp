...
template <>
inline double __mexp(double x) {
    return __exp(x);
}
template <>
inline void __msincos(double x, double* s, double* c) {
    __sincos(x, s, c);
}
template <>
inline double __mhugeval() {
    return HUGE_VAL;
}

template <typename FLOAT>
inline std::complex<FLOAT> __cexp_impl(const std::complex<FLOAT>& x) {
...
std::complex<double> __cexp(const std::complex<double>& x) { return __cexp_impl(x); }
