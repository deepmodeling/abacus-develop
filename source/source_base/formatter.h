 ...
 * @brief
 *
 * In C++20, the std::format library is introduced. However, it is not supported under restriction of ABACUS development
 * that not later than C++11. Plus in ABACUS the formatting-output demands is not quite general but more specific,
 * therefore, a simple alternative is proposed here. To use:
 ...
 * The first way is more flexible while the second way is more efficient. The format string can be reset by reset()
 * function. If empty, the format string is empty, otherwise it will be updated.
 */
class FmtCore
{
  public:
    ...
    static inline std::string
        format (const char* fmt, const Ts&... args)
    {
    ...
    {
        return FmtCore::format (fmt_.c_str (), args...);
    }
    /**
    ...
    static bool
        startswith (const std::string& in, const std::string& prefix)
    {
        return (in.size () >= prefix.size ()) && (in.substr (0, prefix.size ()) == prefix);
    }
    static bool
        endswith (const std::string& in, const std::string& suffix)
    {
        return (in.size () >= suffix.size ()) && (in.substr (in.size () - suffix.size ()) == suffix);
    }
    static std::string
        strip (const std::string& in, const std::string& chars = " ")
    {
    ...
    static std::string
        center (const std::string& in, const size_t& width, const char& fillchar = ' ')
    {
        if (in.size () >= width)
            {
                return in;
    ...
    static std::string
        replace (const std::string& in, const std::string& old, const std::string& new_)
    {
        std::string dst = in;
    ...
    static std::string
        join (const std::string& delim, const std::vector<std::string>& src)
    {
        return (src.empty ()) ? ""
                              : std::accumulate (src.begin () + 1,
                                                 src.end (),
                                                 src[0],
                                                 [&delim] (const std::string& acc, const std::string& s)
                                                     { return acc + delim + s; });
    }

    static std::string
        upper (const std::string& in)
    {
        std::string dst = in;
  ...
  private:
    std::string fmt_;
    template <typename T>
    static typename std::enable_if<std::is_same<T, std::string>::value, const char*>::type
        filter (const T& s)
    {
        return s.c_str ();
    }
...
class FmtTable
{
  ...
  private:
    ...
    struct Alignments
    {
        Alignments (const Align& val = Align::RIGHT, const Align& title = Align::CENTER)
            : val_ (val), title_ (title) {};
        Align val_, title_; // value and title alignments
    } aligns_;
    struct Frames
    {
        Frames (char up = '-', char mid = '-', char dw = '-', char l = ' ', char r = ' ')
            : up_ (up), mid_ (mid), dw_ (dw), l_ (l), r_ (r) {};
        char up_, mid_, dw_, l_, r_; // up, middle, down, left, right frames. up: the frame above title, middle: the one
                                     // between title and data
  ...
  public:
     ...
     * @param aligns Alignments instance, for alignment of values and titles, e.g. {Align::LEFT, Align::RIGHT} for left
     * alignment of values and right alignment of titles
     * @param frames Frames instance, can be constructed with initializer_list<char> like {'-', '-', '-', ' ', ' '}, for
     * up, middle, down, left and right frames
     * @param delimiters Delimiters instance, can be constructed with initializer_list<char> like {'-', ' '}, for
     * horizontal and vertical delimiters
     */
        ...
        operator<< (const std::vector<T>& src)
    {
        ...
        for (size_t i = 0UL; i < src.size (); i++)
            {
                data[i] = core::format (fmts_[j_].c_str (), src[i]);
            }
    ...
    template <typename T>
    void
        set_value (const size_t& i, const size_t& j, const T& value)
    {
        data_ (i, j) = core::format (fmts_[j].c_str (), value);
    }
    /**
    ...
    std::vector<std::string>
        relax_col_width (
            ...
            const Align valign
            = Align::RIGHT, // because enum type would be the smallest integral type, so it is safe to pass by value
        ...
        for (size_t i = 0; i < col.size () + 1; i++)
            {
                ...
                switch ((i == 0) ? talign : valign)
                    {
                    case Align::RIGHT:
                        new_col[i] = std::string (nwhitespaces, ' ') + new_col[i];
                        break;
                    case Align::LEFT:
                        new_col[i] += std::string (nwhitespaces, ' ');
                        break;
                    case Align::CENTER:
                        new_col[i] = FmtCore::center (new_col[i], max_width);
                        break;
                    }
            }
        return new_col;
     ...
     */
    std::string
        concat_title (const std::vector<std::string>& titles) const
    {
        std::string dst = "";
        // first sum width of all titles
        size_t width = std::accumulate (titles.begin (),
                                        titles.end (),
                                        0,
                                        [] (const size_t& acc, const std::string& s) { return acc + s.size (); });
        ...
        dst += std::string (indent_, ' ')
               + std::string (1, frames_.l_); // second line: the left frame + titles + right frame
     ...
     */
    std::string
        concat_row (const std::vector<std::string>& row, const char& pos) const
    {
        std::string dst = "";
        // first sum width of all elements of the row
        size_t width = std::accumulate (row.begin (),
                                        row.end (),
                                        0,
                                        [] (const size_t& acc, const std::string& s) { return acc + s.size (); });
        // for the delimiters
  ...
  private:
    ...
    template <typename T>
    void
        set_value (const size_t& i, const size_t& j, const char& dir, const std::vector<T>& src)
    {
        if (dir == 'v')
            {
