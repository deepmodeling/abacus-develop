...
class FmtCore {
  ...
  private:
    std::string fmt_;
    template <typename T>
    static typename std::enable_if<std::is_same<T, std::string>::value, const char*>::type filter(const T& s) {
        return s.c_str();
    }
    template <typename T>
    static typename std::enable_if<!std::is_same<T, std::string>::value, const T&>::type filter(const T& s) {
        return s;
    }
};
...
class FmtTable {
  ...
  public:
     ...
     */
    std::vector<std::string> relax_col_width(
    ...
    std::string concat_title(const std::vector<std::string>& titles) const {
        ...
        size_t width = std::accumulate(titles.begin(), titles.end(), 0, [](const size_t& acc, const std::string& s) {
            return acc + s.size();
        });
    ...
    std::string concat_row(const std::vector<std::string>& row, const char& pos) const {
        ...
        size_t width = std::accumulate(row.begin(), row.end(), 0, [](const size_t& acc, const std::string& s) {
            return acc + s.size();
        });
        // for the delimiters
        width += row.size() - 1;
