...
TensorShape::TensorShape (std::initializer_list<int64_t> dims)
    : dims_ (dims), strides_ (std::move (get_strides_ (dims)))
{
}
...
int64_t
    TensorShape::dim_size (int dim) const
{
    return dims_[dim];
}
...
void
    TensorShape::set_dim_size (int dim, int64_t size)
{
    dims_[dim] = size;
    strides_ = std::move (get_strides_ (dims_));
}
...
void
    TensorShape::add_dim (int64_t size)
{
...
void
    TensorShape::remove_dim (int dim)
{
...
bool
    TensorShape::operator== (const TensorShape& other) const
{
...
std::vector<int64_t>
    TensorShape::get_strides_ (const std::vector<int64_t>& dim)
{
...
std::ostream&
    operator<< (std::ostream& os, const TensorShape& shape)
{
    os << "[";
