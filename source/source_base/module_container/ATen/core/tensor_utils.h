 ...
 */
__inline__ std::string
    removeTrailingZeros (std::string str)
{
...
template <typename T>
__inline__ int
    _get_digit_places (const T* arr, int size, int& integer_count, int& fraction_count)
{
    integer_count = 0;
    fraction_count = 0;
...
template <typename T>
__inline__ int
    _get_digit_places (const std::complex<T>* arr, int size, int& integer_count, int& fraction_count)
{
...
template <typename T>
__inline__ void
    _output_wrapper (std::ostream& os, const T data, const int& digit_width, const int& fraction_count)
{
...
template <>
__inline__ void
    _output_wrapper (std::ostream& os, const int data, const int& digit_width, const int& fraction_count)
{
...
template <typename T>
T
    extract (const container::Tensor& tensor)
{
    if (tensor.device_type () == DeviceType::CpuDevice)
    ...
    else
        {
            ...
            TEMPLATE_ALL_2 (
                tensor.data_type (),
                tensor.device_type (),
                kernels::synchronize_memory<T, DEVICE_CPU, DEVICE_> () (&result,
