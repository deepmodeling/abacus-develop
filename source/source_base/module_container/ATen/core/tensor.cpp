    ...
    : data_type_ (other.data_type_), shape_ (other.shape_), device_ (other.device_),
      buffer_ (new TensorBuffer (GetAllocator (device_), shape_.NumElements () * SizeOfType (data_type_))){
          TEMPLATE_ALL_2 (data_type_,
                          device_,
                          kernels::synchronize_memory<T_, DEVICE_, DEVICE_> () (this->data<T_> (),
                                                                                other.data<T_> (),
                                                                                this->NumElements ()))}
...
base::core::Allocator*
    Tensor::GetAllocator (DeviceType device)
{
...
void
    Tensor::resize (const TensorShape& new_shape)
{
    if (shape_ == new_shape)
        {
            return;
    ...
    if (buffer_ && buffer_->GetAllocatedBytes () < new_shape.NumElements () * SizeOfType (data_type_))
        {
            buffer_->unref ();
            this->buffer_
                = new TensorBuffer (GetAllocator (device_), new_shape.NumElements () * SizeOfType (data_type_));
        }
    shape_ = new_shape;
}
...
bool
    Tensor::CopyFrom (const Tensor& other, const TensorShape& shape)
{
...
bool
    Tensor::AllocateFrom (const Tensor& other, const TensorShape& shape)
{
