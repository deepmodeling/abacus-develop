...
class TensorAccessorBase
{
  public:
    ...
    AT_HOST_DEVICE index_t
        stride (index_t idx) const
    {
        return strides_[idx];
    }

    AT_HOST_DEVICE index_t
        size (index_t idx) const
    {
        return sizes_[idx];
    }

    AT_HOST_DEVICE PtrType
        data ()
    {
        return data_;
    }

    AT_HOST_DEVICE const PtrType
        data () const
    {
        return data_;
    }
...
class TensorAccessor : public TensorAccessorBase<T, N, index_t, PtrTraits>
{
  public:
    ...
    AT_HOST_DEVICE TensorAccessor<T, N - 1, index_t, PtrTraits>
    ...
    AT_HOST_DEVICE const TensorAccessor<T, N - 1, index_t, PtrTraits>
