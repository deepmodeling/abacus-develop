...
struct BCast
{
  public:
    ...
    static void
        reverse (std::vector<int64_t>& vec)
    {
...
static BCast
    prepare_bcast (const std::vector<int64_t>& x_, const std::vector<int64_t>& y_)
{
    ...
    for (int ii = 0; ii < lagest_batch_rank; ii++)
        {
            ...
            if (!output_dim_set)
            ...
            else if (x_current_is_one == x_prev_is_one && y_current_is_one == y_prev_is_one && set_one)
                {
                    ...
                    bcast.x_bcast_shape.back ()
                        = mul_dims (bcast.x_bcast_shape.back (), x_current_is_one ? output_dim : 1);
                    bcast.y_bcast_shape.back ()
                        = mul_dims (bcast.y_bcast_shape.back (), y_current_is_one ? output_dim : 1);
                }
            else
                {
...
static inline int64_t
    IPow (int64_t base, int64_t exponent)
{
    int64_t result = 1;
...
static inline bool
    CopyFrom (const Tensor& input, const TensorShape& shape, Tensor* output)
{
...
static inline bool
    CopyFromWithAllocate (const Tensor& input, const TensorShape& shape, Tensor* output)
{
...
static bool
    ReshapeToRank3 (Tensor& input, int batch_size, Tensor& output)
{
...
template <typename T>
static inline bool
    all_of (const std::vector<T>& vec, const std::function<bool (T)>& predicate)
{
...
static bool
    StrideOrInflateOperand (Tensor& input,
...
static void
    PermuteLabels (const std::vector<int>& permutation, std::vector<int>& labels)
{
...
static bool
    ShouldTranspose (const TensorShape& input_shape, const std::vector<int>& permutation)
{
...
static bool
    TransposeOperand (const Tensor& input, const std::vector<int>& permutation, Tensor& output)
{
...
static bool
    ShouldSwapFreeAndContract (const std::vector<int>& labels, const std::vector<EinsumDimensionType>& label_types)
{
...
static void
    InsertBroadcastLabels (int num_bcast_labels,
...
// dimensions.
static EinsumDimensionType
    GetDimensionType (bool is_removed, bool is_unique)
{
...
bool
    ValidateEinsumEquation (const std::string& equation,
...
bool
    ParseEinsumEquation (const std::string& equation,
...
bool
    RecordLabelToDimension (const int label,
                            const int axis,
                            const Tensor& input,
                            std::unordered_map<int, int64_t>& label_to_dim_sizes)
{
    ...
    if (label_dim != 0 && label_dim != input_dim)
        {
            throw std::invalid_argument ("Expected dimension " + std::to_string (label_to_dim_sizes[label])
                                         + " at axis " + std::to_string (axis) + " of the input shaped "
...
bool
    ProcessDimensions (const std::vector<const Tensor*>& inputs,
                       std::vector<EinsumDimensionType>& label_types,
                       std::vector<std::vector<int>>& input_labels,
                       std::vector<int>& output_labels,
                       std::vector<std::vector<int>>& input_label_counts,
    ...
    for (int ii = 0; ii < num_inputs; ii++)
        {
            ...
            if (!has_ellipsis)
                {
                    ...
                    if (num_dims != labels.size ())
                        {
                            throw std::invalid_argument ("Input " + std::to_string (ii) + " has "
                                                         + std::to_string (num_dims) + " dimensions but got"
            ...
            if (ellipsis_idx != -1)
                {
                    InsertBroadcastLabels (num_bcast_labels,
                                           num_labels,
                                           ellipsis_idx,
                                           input_labels[ii],
...
bool
    ReduceOperand (const Tensor& input,
                   ...
                   Tensor& output)
{
    ...
    if (reshape[EinsumDimensionType::kReduce] == 1)
...
bool
    ContractOperands (std::vector<Tensor>& inputs,
...
void
    ProcessOutput (const Tensor& input,
