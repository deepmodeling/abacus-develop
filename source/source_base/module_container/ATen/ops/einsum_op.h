...
struct EinsumOption
{
    ...
    EinsumOption (bool conj_x_ = false,
                  bool conj_y_ = false,
                  float alpha_ = 1.0,
                  float beta_ = 0.0,
                  Tensor* out_ = nullptr)
...
typename std::enable_if<std::is_same<typename std::common_type<Tensors...>::type, Tensor>::value, Tensor>::type
    einsum_impl (const std::string& equation, const EinsumOption& option, const Tensors&... tensors)
{
    ...
    einsum_utils::ParseEinsumEquation (equation,
                                       label_types,
                                       input_labels,
                                       output_labels,
                                       input_label_counts,
    ...
    einsum_utils::ProcessDimensions (inputs,
                                     label_types,
                                     input_labels,
                                     output_labels,
                                     input_label_counts,
    ...
    for (int ii = 0; ii < num_inputs; ++ii)
        {
            einsum_utils::ReduceOperand (*inputs[ii],
                                         label_types,
                                         input_labels[ii],
                                         input_label_counts[ii],
                                         free_labels[ii],
                                         swap_free_and_contract[ii],
                                         inputs_reduced[ii]);
        }
    ...
    einsum_utils::ContractOperands (inputs_reduced, swap_free_and_contract, option, contraction_output_reshaped);

    Tensor output;
    // Copy the batch labels from the contraction output. Recover the batch
    // shape, which may have been broadcasted.
    einsum_utils::ProcessOutput (contraction_output_reshaped,
                                 label_types,
                                 free_labels,
                                 label_to_dim_sizes,
                                 output_labels,
...
inline static Tensor
    einsum (const std::string& equation, const Tensor& A)
{
    const EinsumOption& option = {};
...
inline static Tensor
    einsum (const std::string& equation, const Tensor& A, const Tensor& B, const EinsumOption& option = {})
{
