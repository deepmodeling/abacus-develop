...
TYPED_TEST(LinalgTest, Mul) {
    ...
    Tensor expected = std::move(Tensor({static_cast<Type>(1.0),
                                        ...
                                        static_cast<Type>(49.0),
                                        static_cast<Type>(64.0),
                                        static_cast<Type>(81.0)})
                                    .to_device<Device>());
    Tensor result = Tensor(expected.data_type(), expected.device_type(), expected.shape());
    kernels::mul<Type, Device>()(A.NumElements(),
                                 static_cast<Type>(1.0),
                                 A.data<Type>(),
                                 B.data<Type>(),
                                 result.data<Type>());
...
TYPED_TEST(LinalgTest, Div) {
    ...
    Tensor result = Tensor(expected.data_type(), expected.device_type(), expected.shape());
    kernels::div<Type, Device>()(A.NumElements(),
                                 static_cast<Type>(1.0),
                                 A.data<Type>(),
                                 B.data<Type>(),
                                 result.data<Type>());
...
TYPED_TEST(LinalgTest, Fma) {
    ...
    Tensor B = std::move(Tensor({static_cast<Type>(1.0),
                                 ...
                                 static_cast<Type>(9.0)})
                             .to_device<Device>());
    Tensor C = std::move(Tensor({static_cast<Type>(1.0),
                                 static_cast<Type>(2.0),
                                 static_cast<Type>(3.0),
                                 static_cast<Type>(4.0),
                                 static_cast<Type>(5.0),
    ...
    Tensor expected = std::move(Tensor({static_cast<Type>(5.0),
                                        static_cast<Type>(14.0),
                                        static_cast<Type>(27.0),
                                        static_cast<Type>(44.0),
                                        static_cast<Type>(65.0),
                                        static_cast<Type>(90.0),
                                        static_cast<Type>(119.0),
                                        static_cast<Type>(152.0),
                                        static_cast<Type>(189.0)})
                                    .to_device<Device>());

    Tensor result = Tensor(expected.data_type(), expected.device_type(), expected.shape());
    kernels::fma<Type, Device>()(A.NumElements(),
                                 static_cast<Type>(2.0),
                                 A.data<Type>(),
                                 B.data<Type>(),
                                 static_cast<Type>(3.0),
...
TYPED_TEST(LinalgTest, Transpose) {
    ...
    kernels::transpose<Type, Device>()(perm,
                                       A.shape().dims(),
                                       A_transpose.shape().dims(),
                                       A.data<Type>(),
                                       A_transpose.data<Type>());
...
TYPED_TEST(LinalgTest, Stride) {
    ...
    A.reshape({-1});
    Tensor expected =
        std::move(Tensor({static_cast<Type>(1.0), static_cast<Type>(5.0), static_cast<Type>(9.0)}).to_device<Device>());
    expected.reshape({-1});
    Tensor A_stride = expected;
    A_stride.zero();
    std::vector<int64_t> stride = {4};

    kernels::stride<Type, Device>()(stride,
                                    A.shape().dims(),
                                    A_stride.shape().dims(),
                                    A.data<Type>(),
                                    A_stride.data<Type>());
...
TYPED_TEST(LinalgTest, Inflate) {
    ...
    kernels::inflate<Type, Device>()(inflate,
                                     A.shape().dims(),
                                     A_inflate.shape().dims(),
                                     A.data<Type>(),
                                     A_inflate.data<Type>());
...
TYPED_TEST(LinalgTest, Reduce) {
    ...
    kernels::reduce<Type, Device>()(A_reduce.NumElements(), inner_most_dim, A.data<Type>(), A_reduce.data<Type>());
