...
{
    void
        operator() (const int m, const int n, T* A, const int lda)
    {
        // Tensor or vector?
...
struct lapack_heevx<T, DEVICE_CPU>
{
    using Real = typename GetTypeReal<T>::type;
    void
        operator() (const int n, const int lda, const T* Mat, const int neig, Real* eigen_val, T* eigen_vec)
    {
        ...
        lapackConnector::heevx (jobz,
                                ...
                                0.0,
                                il,
                                iu, // vl, vu not used when range='I'
                                abstol,
                                found,
                                eigen_val,
                                eigen_vec,
...
struct lapack_hegvx<T, DEVICE_CPU>
{
    using Real = typename GetTypeReal<T>::type;
    void
        operator() (const int n, const int lda, T* Mat_A, T* Mat_B, const int m, Real* eigen_val, T* eigen_vec)
    {
        ...
        if (info > 0)
            {
                throw std::runtime_error ("hegvx failed to converge. Number of converged eigenvalues: "
                                          + std::to_string (info));
            }
    }
};
