...
struct blas_nrm2<T, DEVICE_CPU> {
    ...
    Real operator()(const int n, const T* x, const int incx) { return BlasConnector::nrm2(n, x, incx); }
};
...
struct blas_scal<T, DEVICE_CPU> {
    void operator()(const int& n, const T* alpha, T* x, const int& incx) { BlasConnector::scal(n, *alpha, x, incx); }
};
