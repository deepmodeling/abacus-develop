...
template <typename T>
__global__ void
    do_add_kernel (const int num_element, const T alpha, const T* x, const T beta, const T* y, T* z)
{
    ...
    for (int o_idx = threadIdx.x; o_idx < num_element; o_idx += blockDim.x)
        {
            // Assign the sum of the input Tensor elements at index 'o_idx' to the output Tensor element at index
            // 'o_idx'.
...
template <typename T>
__global__ void
    do_mul_kernel (const int num_element, const T alpha, const T* x, T* y)
{
    for (int o_idx = threadIdx.x; o_idx < num_element; o_idx += blockDim.x)
        {
            // Assign the sum of the input Tensor elements at index 'o_idx' to the output Tensor element at index
...
template <typename T>
__global__ void
    do_mul_kernel (const int num_element, const T alpha, const T* x, const T* y, T* z)
{
    for (int o_idx = threadIdx.x; o_idx < num_element; o_idx += blockDim.x)
        {
            // Assign the sum of the input Tensor elements at index 'o_idx' to the output Tensor element at index
...
template <typename T>
__global__ void
    do_div_kernel (const int num_element, const T alpha, const T* x, const T* y, T* z)
{
    for (int o_idx = threadIdx.x; o_idx < num_element; o_idx += blockDim.x)
        {
            // Assign the sum of the input Tensor elements at index 'o_idx' to the output Tensor element at index
...
template <typename T>
__global__ void
    do_fma_kernel (const int num_element, const T alpha, const T* x, const T* y, const T beta, const T* z, T* out)
{
    for (int o_idx = threadIdx.x; o_idx < num_element; o_idx += blockDim.x)
        {
            // Assign the sum of the input Tensor elements at index 'o_idx' to the output Tensor element at index
    ...
    for (int64_t o_idx = 0; o_idx < num_elements; o_idx++)
        {
            ...
            for (int ii = 0; ii < ndim; ++ii)
                {
                    ...
                    // Calculate the offset for the corresponding index position in the input Tensor and accumulate it
                    // in 'i_idx'.
            ...
            if (Conjugate)
                {
                    // Assign the conjugate value of the input Tensor element at index 'i_idx' to the output Tensor
                    // element at index 'o_idx'.
...
template <typename T>
__global__ void
    do_reduce_kernel (int64_t size, int64_t inner_most_dim, const T* p, T* q)
{
    for (int64_t o_idx = threadIdx.x; o_idx < size; o_idx += blockDim.x)
        {
            T sum = 0;
...
template <typename T>
static std::vector<T>
    compute_stride (const std::vector<T>& shape)
{
...
void
    add<T, Device>::operator() (const int& num_element, const T& alpha, const T* x, const T& beta, const T* y, T* z)
{
    ...
    do_add_kernel<Type><<<block, THREADS_PER_BLOCK>>> (num_element,
                                                       alpha_,
                                                       reinterpret_cast<const Type*> (x),
                                                       beta_,
                                                       reinterpret_cast<const Type*> (y),
                                                       reinterpret_cast<Type*> (z));
}

template <typename T, typename Device>
void
    mul<T, Device>::operator() (const int& num_element, const T& alpha, const T* x, T* y)
{
    using Type = typename GetTypeThrust<T>::type;
    auto alpha_ = *reinterpret_cast<const Type*> (&alpha);
    const int block = (num_element + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
    do_mul_kernel<Type><<<block, THREADS_PER_BLOCK>>> (num_element,
                                                       alpha_,
                                                       reinterpret_cast<const Type*> (x),
...
template <typename T, typename Device>
void
    mul<T, Device>::operator() (const int& num_element, const T& alpha, const T* x, const T* y, T* z)
{
    using Type = typename GetTypeThrust<T>::type;
    auto alpha_ = *reinterpret_cast<const Type*> (&alpha);
    const int block = (num_element + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
...
template <typename T, typename Device>
void
    div<T, Device>::operator() (const int& num_element, const T& alpha, const T* x, const T* y, T* z)
{
    using Type = typename GetTypeThrust<T>::type;
    auto alpha_ = *reinterpret_cast<const Type*> (&alpha);
    const int block = (num_element + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
    do_div_kernel<Type><<<block, THREADS_PER_BLOCK>>> (num_element,
                                                       alpha_,
                                                       reinterpret_cast<const Type*> (x),
                                                       reinterpret_cast<const Type*> (y),
                                                       reinterpret_cast<Type*> (z));
...
template <typename T, typename Device>
void
    fma<T, Device>::operator() (const int& num_element,
                                const T& alpha,
                                const T* x,
                                const T* y,
                                const T& beta,
                                ...
                                T* out)
{
    ...
    auto beta_ = *reinterpret_cast<const Type*> (&beta);
    const int block = (num_element + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
    do_fma_kernel<Type><<<block, THREADS_PER_BLOCK>>> (num_element,
                                                       alpha_,
                                                       reinterpret_cast<const Type*> (x),
                                                       reinterpret_cast<const Type*> (y),
                                                       beta_,
                                                       reinterpret_cast<const Type*> (z),
                                                       reinterpret_cast<Type*> (out));
}

template <typename T, typename Device, bool Conjugate>
void
    transpose<T, Device, Conjugate>::operator() (const std::vector<int>& perm,
                                                 ...
                                                 T* q)
{
    ...
    REQUIRES_OK (p_shape.size () == q_shape.size (), "transpose: p and q must have the same number of dimensions");
    ...
    kernels::synchronize_memory<int, DEVICE_GPU, DEVICE_CPU> () (t_perm.data<int> (), perm.data (), perm.size ());
    kernels::synchronize_memory<int64_t, DEVICE_GPU, DEVICE_CPU> () (t_in_strides.data<int64_t> (),
                                                                     in_strides.data (),
                                                                     in_strides.size ());
    kernels::synchronize_memory<int64_t, DEVICE_GPU, DEVICE_CPU> () (t_out_strides.data<int64_t> (),
                                                                     out_strides.data (),
                                                                     out_strides.size ());
    ...
    do_transpose_kernel<Type, Conjugate><<<block, THREADS_PER_BLOCK>>> (ndim,
                                                                        num_elements,
                                                                        p_,
                                                                        t_perm.data<int> (),
                                                                        t_in_strides.data<int64_t> (),
                                                                        t_out_strides.data<int64_t> (),
                                                                        q_);
}

template <typename T, typename Device>
void
    stride<T, Device>::operator() (const std::vector<int64_t>& stride,
                                   const std::vector<int64_t>& p_shape,
                                   const std::vector<int64_t>& q_shape,
                                   const T* p,
                                   T* q)
{
    ...
    kernels::synchronize_memory<int64_t, DEVICE_GPU, DEVICE_CPU> () (t_stride.data<int64_t> (),
                                                                     stride.data (),
                                                                     stride.size ());
    kernels::synchronize_memory<int64_t, DEVICE_GPU, DEVICE_CPU> () (t_in_strides.data<int64_t> (),
                                                                     in_strides.data (),
    ...
    const int block = (num_elements + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
    do_stride_kernel<Type><<<block, THREADS_PER_BLOCK>>> (ndim,
                                                          num_elements,
                                                          p_,
                                                          t_stride.data<int64_t> (),
                                                          t_in_strides.data<int64_t> (),
                                                          t_out_strides.data<int64_t> (),
                                                          q_);
}
...
template <typename T, typename Device>
void
    inflate<T, Device>::operator() (const std::vector<int64_t>& inflate,
                                    const std::vector<int64_t>& p_shape,
                                    const std::vector<int64_t>& q_shape,
                                    const T* p,
                                    T* q)
{
    ...
    const int block = (num_elements + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
    do_inflate_kernel<Type><<<block, THREADS_PER_BLOCK>>> (ndim,
                                                           num_elements,
                                                           p_,
                                                           t_stride.data<int64_t> (),
                                                           t_in_strides.data<int64_t> (),
...
template <typename T, typename Device>
void
    reduce<T, Device>::operator() (const int64_t& num_element, const int64_t& inner_most_dim, const T* p, T* q)
{
    using Type = typename GetTypeThrust<T>::type;
    ...
    do_reduce_kernel<Type><<<block, THREADS_PER_BLOCK>>> (num_element, inner_most_dim, p_, q_);
}
