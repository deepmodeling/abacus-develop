...
template <typename T>
static std::vector<T>
    ComputeStride (const std::vector<T>& shape)
{
    ...
    mul<T, Device>::operator() (const int& num_element, const T& alpha, const T* x, T* y)
{
    ...
    for (int o_idx = 0; o_idx < num_element; o_idx++)
        {
            // Assign the product of the input Tensor elements at index 'o_idx' to the output Tensor element at index
            // 'o_idx'.
    ...
    div<T, Device>::operator() (const int& num_element, const T& alpha, const T* x, const T* y, T* z)
{
    ...
    for (int o_idx = 0; o_idx < num_element; o_idx++)
        {
            // Assign the quotient of the input Tensor elements at index 'o_idx' to the output Tensor element at index
            // 'o_idx'.
            z[o_idx] = alpha * x[o_idx] / y[o_idx];
        }
}
                                ...
                                T* out)
{
    ...
    for (int o_idx = 0; o_idx < num_element; o_idx++)
        {
            // Assign the sum of the product of the input Tensor elements at index 'o_idx' and the corresponding
            // coefficients to the output Tensor element at index 'o_idx'.
...
void
    stride<T, Device>::operator() (const std::vector<int64_t>& stride,
                                   ...
                                   T* q)
{
    REQUIRES_OK (p_shape.size () == q_shape.size (), "stride: p and q must match the number of dimensions");
    const int ndim = static_cast<int> (p_shape.size ());
    auto in_strides = ComputeStride (p_shape);
    auto out_strides = ComputeStride (q_shape);
