...
struct blas_nrm2<T, DEVICE_GPU>
{
    ...
    Real
        operator() (const int n, const T* x, const int incx)
    {
        Real result;
...
{
    void
        operator() (const int n, const T* x, const int incx, T* y, const int incy)
    {
...
{
    void
        operator() (const int& n, const T* x, const int& incx, const T* y, const int& incy, T* result)
    {
...
{
    void
        operator() (const int& n, const T* alpha, T* x, const int& incx)
    {
...
{
    void
        operator() (const int& n, const T* alpha, const T* x, const int& incx, T* y, const int& incy)
    {
...
struct blas_gemv_batched_strided<T, DEVICE_GPU>
{
    void
        operator() (const char& trans,
                    ...
                    const int& batch_size)
    {
        cuBlasConnector::gemv_batched_strided (cublas_handle,
                                               trans,
                                               m,
                                               n,
                                               *alpha,
...
struct blas_gemm_batched<T, DEVICE_GPU>
{
    ...
    {
        cuBlasConnector::
            gemm_batched (cublas_handle, transa, transb, m, n, k, *alpha, A, lda, B, ldb, *beta, C, ldc, batch_size);
    }
};
...
struct blas_gemm_batched_strided<T, DEVICE_GPU>
{
    void
        operator() (const char& transa,
                    ...
                    const int& batch_size)
    {
        cuBlasConnector::gemm_batched_strided (cublas_handle,
                                               transa,
                                               transb,
                                               m,
                                               n,
