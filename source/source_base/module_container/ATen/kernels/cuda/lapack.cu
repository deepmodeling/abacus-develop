...
template <typename T>
__global__ void
    set_matrix_kernel (const char uplo, T* A, const int dim)
{
    int bid = blockIdx.x;
    int tid = threadIdx.x;
...
struct set_matrix<T, DEVICE_GPU>
{
    ...
    void
        operator() (const char& uplo, T* A, const int& dim)
    {
        set_matrix_kernel<Type><<<dim - 1, THREADS_PER_BLOCK>>> (uplo, reinterpret_cast<Type*> (A), dim);
...
{
    void
        operator() (const char& uplo, const char& diag, const int& dim, T* Mat, const int& lda)
    {
...
{
    void
        operator() (const char& uplo, const int& dim, T* Mat, const int& lda)
    {
...
{
    void
        operator() (const int& m, const int& n, T* Mat, const int& lda, int* ipiv)
    {
...
{
    void
        operator() (const int& n, T* Mat, const int& lda, const int* ipiv, T* work, const int& lwork)
    {
        throw std::runtime_error ("cuSOLVER does not provide LU-based matrix inversion interface (getri). To compute "
                                  "the inverse on GPU, use getrs instead.");
    }
};
...
{
    void
        operator() (const int m, const int n, T* d_A, const int lda)
    {
...
struct lapack_heevd<T, DEVICE_GPU>
{
    ...
    void
        operator() (const int dim, T* Mat, const int lda, Real* eigen_val)
    {
...
struct lapack_heevx<T, DEVICE_GPU>
{
    using Real = typename GetTypeReal<T>::type;
    void
        operator() (const int n, const int lda, const T* d_Mat, const int neig, Real* d_eigen_val, T* d_eigen_vec)
    {
        assert (n <= lda);
        ...
        cuSolverConnector::heevdx (cusolver_handle,
                                   n,
                                   lda,
                                   d_eigen_vec,
...
struct lapack_hegvd<T, DEVICE_GPU>
{
    using Real = typename GetTypeReal<T>::type;
    void
        operator() (const int dim, const int lda, T* Mat_A, T* Mat_B, Real* eigen_val, T* eigen_vec)
    {
        const int itype = 1;
        const char jobz = 'V';
        const char uplo = 'U';
        ...
        cuSolverConnector::hegvd (cusolver_handle, itype, jobz, uplo, dim, eigen_vec, lda, d_B_backup, lda, eigen_val);
...
struct lapack_hegvx<T, DEVICE_GPU>
{
    using Real = typename GetTypeReal<T>::type;
    void
        operator() (const int n, const int lda, T* A, T* B, const int m, Real* eigen_val, T* eigen_vec)
    {
        const int itype = 1;
        const char jobz = 'V';
        const char range = 'I';
