...
template <typename T>
__global__ void set_matrix_kernel(const char uplo, T* A, const int dim) {
    int bid = blockIdx.x;
    int tid = threadIdx.x;
...
struct lapack_geqrf_inplace<T, DEVICE_GPU> {
    void operator()(const int m, const int n, T* d_A, const int lda) {
...
struct lapack_heevx<T, DEVICE_GPU> {
    using Real = typename GetTypeReal<T>::type;
    void operator()(const int n, const int lda, const T* d_Mat, const int neig, Real* d_eigen_val, T* d_eigen_vec) {
        assert(n <= lda);
...
struct lapack_hegvx<T, DEVICE_GPU> {
    using Real = typename GetTypeReal<T>::type;
    void operator()(const int n, const int lda, T* A, T* B, const int m, Real* eigen_val, T* eigen_vec) {
        const int itype = 1;
        const char jobz = 'V';
        const char range = 'I';
        const char uplo = 'U';
