...
template <typename T>
__device__ static inline T conj(T& in) {
    return in;
}
...
template <typename T>
__global__ void do_add_kernel(const int num_element, const T alpha, const T* x, const T beta, const T* y, T* z) {
...
template <typename T>
__global__ void do_mul_kernel(const int num_element, const T alpha, const T* x, T* y) {
    for (int o_idx = threadIdx.x; o_idx < num_element; o_idx += blockDim.x) {
        // Assign the sum of the input Tensor elements at index 'o_idx' to the output Tensor element at index 'o_idx'.
...
template <typename T>
__global__ void do_mul_kernel(const int num_element, const T alpha, const T* x, const T* y, T* z) {
    for (int o_idx = threadIdx.x; o_idx < num_element; o_idx += blockDim.x) {
        // Assign the sum of the input Tensor elements at index 'o_idx' to the output Tensor element at index 'o_idx'.
...
template <typename T>
__global__ void do_div_kernel(const int num_element, const T alpha, const T* x, const T* y, T* z) {
    for (int o_idx = threadIdx.x; o_idx < num_element; o_idx += blockDim.x) {
        // Assign the sum of the input Tensor elements at index 'o_idx' to the output Tensor element at index 'o_idx'.
        z[o_idx] = alpha * x[o_idx] / y[o_idx];
    }
...
__global__ void do_transpose_kernel(int ndim,
    ...
    for (int64_t o_idx = 0; o_idx < num_elements; o_idx++) {
        ...
        for (int ii = 0; ii < ndim; ++ii) {
            ...
            // Calculate the offset for the corresponding index position in the input Tensor and accumulate it in
            // 'i_idx'.
            i_idx += ratio * in_strides[perm[ii]];
        }
        // Check if conjugation is needed.
        if (Conjugate) {
            // Assign the conjugate value of the input Tensor element at index 'i_idx' to the output Tensor element at
            // index 'o_idx'.
...
template <typename T>
__global__ void do_stride_kernel(int ndim,
                                 int64_t size,
                                 const T* p,
...
template <typename T>
__global__ void do_inflate_kernel(int ndim,
                                  int64_t size,
                                  const T* p,
                                  const int64_t* stride,
                                  const int64_t* in_strides,
...
template <typename T>
__global__ void do_reduce_kernel(int64_t size, int64_t inner_most_dim, const T* p, T* q) {
    for (int64_t o_idx = threadIdx.x; o_idx < size; o_idx += blockDim.x) {
        T sum = 0;
...
template <typename T, typename Device>
void fma<T, Device>::operator()(const int& num_element,
                                const T& alpha,
                                const T* x,
                                const T* y,
                                const T& beta,
...
void transpose<T, Device, Conjugate>::operator()(const std::vector<int>& perm,
...
template <typename T, typename Device>
void stride<T, Device>::operator()(const std::vector<int64_t>& stride,
                                   const std::vector<int64_t>& p_shape,
                                   const std::vector<int64_t>& q_shape,
                                   const T* p,
                                   T* q) {
...
template <typename T, typename Device>
void inflate<T, Device>::operator()(const std::vector<int64_t>& inflate,
                                    const std::vector<int64_t>& p_shape,
                                    const std::vector<int64_t>& q_shape,
                                    const T* p,
                                    T* q) {
...
template <typename T, typename Device>
void reduce<T, Device>::operator()(const int64_t& num_element, const int64_t& inner_most_dim, const T* p, T* q) {
    using Type = typename GetTypeThrust<T>::type;
