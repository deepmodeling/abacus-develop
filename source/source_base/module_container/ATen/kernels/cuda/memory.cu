...
template <typename T>
__global__ void
    do_set_memory (T* out, const T var, const size_t size)
{
...
__global__ void
    do_cast_memory (T_out* out, const T_in* in, const int size)
{
    unsigned int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= size)
        {
...
template <typename T_out, typename T_in>
__global__ void
    do_cast_memory (std::complex<T_out>* out, const std::complex<T_in>* in, const int size)
{
    unsigned int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= size)
        {
...
void
    resize_memory<T, Device>::operator() (T*& arr, const size_t& size, const char* record_in)
{
    if (arr != nullptr)
        {
...
template <typename T, typename Device>
void
    set_memory<T, Device>::operator() (T* arr, const T& var, const size_t& size)
{
...
template <typename T, typename Device>
void
    delete_memory<T, Device>::operator() (T* arr)
{
    CHECK_CUDA (cudaFree (arr));
}
