...
class array_ref final
{
  ...
  public:
    ...
    template <typename A>
    /* implicit */ array_ref (const std::vector<T, A>& vec) : data_ (vec.data ()), length_ (vec.size ())
    {
    ...
    constexpr bool
        equals (const array_ref<T>& rhs) const
    {
    ...
    template <typename U>
    typename std::enable_if<std::is_same<U, T>::value, array_ref<T>>::type& operator= (U&& Temporary) = delete;

    template <typename U>
    typename std::enable_if<std::is_same<U, T>::value, array_ref<T>>::type& operator= (std::initializer_list<U>)
        = delete;
...
template <typename T>
std::ostream&
    operator<< (std::ostream& out, array_ref<T> arr)
{
    int ii = 0;
    out << "[";
...
template <typename T>
bool
    operator== (array_ref<T> a1, array_ref<T> a2)
{
    return a1.equals (a2);
}
