...
static inline hipblasOperation_t
    GetHipblasOperation (const char& trans)
{
...
static inline hipblasFillMode_t
    hipblas_fill_mode (const char& uplo)
{
    if (uplo == 'U' || uplo == 'u')
...
static inline hipblasDiagType_t
    hipblas_diag_type (const char& diag)
{
    if (diag == 'U' || diag == 'u')
...
static inline hipsolverEigMode_t
    hipblas_eig_mode (const char& jobz)
{
    if (jobz == 'N' || jobz == 'n')
...
static inline hipsolverEigType_t
    hipblas_eig_type (const int& itype)
{
    if (itype == 1)
...
static inline hipsolverFillMode_t
    hipsolver_fill_mode (const char& uplo)
{
    if (uplo == 'U' || uplo == 'u')
...
static const char*
    hipsolverGetErrorEnum (hipsolverStatus_t error)
{
    switch (error)
        {
...
inline void
    hipsolverAssert (hipsolverStatus_t code, const char* file, int line, bool abort = true)
{
...
// hipSOLVER API errors
static const char*
    hipblasGetErrorEnum (hipblasStatus_t error)
{
    switch (error)
        {
...
inline void
    hipblasAssert (hipblasStatus_t code, const char* file, int line, bool abort = true)
{
