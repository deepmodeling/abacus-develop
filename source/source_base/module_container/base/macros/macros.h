...
#define CHECK_MSG(expr, ...)                                                                                           \
    (::base::utils::check_msg_impl("Expected " #expr " to be true, but got false.  " __VA_ARGS__ " "                   \
                                   "(Could this error message be improved?  If so, "                                   \
                                   "please report an enhancement request to Container)"))

#define REQUIRES_OK(expr, ...)                                                                                         \
    if (PREDICT_FALSE(!(expr))) {                                                                                      \
        ::base::utils::check_exit_impl(__func__,                                                                       \
                                       __FILE__,                                                                       \
...
#define CASE_2(TYPE, DEVICE, STMTS)                                                                                    \
    case (int(ct::DataTypeToEnum<TYPE>::value) * 10 + int(ct::DeviceTypeToEnum<DEVICE>::value)): {                     \
        typedef TYPE T_;                                                                                               \
...
#define TEMPLATE_ALL_LAMBDA_2(TYPE_ENUM, DEVICE_ENUM, ...)                                                             \
    CASES_ALL_LAMBDA_WITH_DEFAULT_2_GPU(                                                                               \
        TYPE_ENUM, DEVICE_ENUM, (__VA_ARGS__), std::cerr << "Unexpected type: " << TYPE_ENUM; exit(EXIT_FAILURE));

#define TEMPLATE_ALL_2(TYPE_ENUM, DEVICE_ENUM, ...)                                                                    \
    CASES_ALL_WITH_DEFAULT_2_GPU(TYPE_ENUM, DEVICE_ENUM, (__VA_ARGS__), std::cerr << "Unexpected type: " << TYPE_ENUM; \
                                 exit(EXIT_FAILURE));
...
#define TEMPLATE_ALL_CALC_2(TYPE_ENUM, DEVICE_ENUM, ...)                                                               \
    CASES_ALL_CALC_WITH_DEFAULT_2_GPU(                                                                                 \
        TYPE_ENUM, DEVICE_ENUM, (__VA_ARGS__), std::cerr << "Unexpected type: " << TYPE_ENUM; exit(EXIT_FAILURE));
#else
#define TEMPLATE_ALL_LAMBDA_2(TYPE_ENUM, DEVICE_ENUM, ...)                                                             \
...
#define TEMPLATE_ALL_2(TYPE_ENUM, DEVICE_ENUM, ...)                                                                    \
    CASES_ALL_WITH_DEFAULT_2(TYPE_ENUM, DEVICE_ENUM, (__VA_ARGS__), std::cerr << "Unexpected type: " << TYPE_ENUM;     \
                             exit(EXIT_FAILURE));

#define TEMPLATE_BLAS_2(TYPE_ENUM, DEVICE_ENUM, ...)                                                                   \
    CASES_BLAS_WITH_DEFAULT_2(TYPE_ENUM, DEVICE_ENUM, (__VA_ARGS__), std::cerr << "Unexpected type: " << TYPE_ENUM;    \
                              exit(EXIT_FAILURE));
...
#define TEMPLATE_CZ_2(TYPE_ENUM, DEVICE_ENUM, ...)                                                                     \
    CASES_CZ_WITH_DEFAULT_2(TYPE_ENUM, DEVICE_ENUM, (__VA_ARGS__), std::cerr << "Unexpected type: " << TYPE_ENUM;      \
                            exit(EXIT_FAILURE));
