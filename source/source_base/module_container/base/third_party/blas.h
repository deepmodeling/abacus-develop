...
static inline void axpy(const int& n,
                        const std::complex<float>& alpha,
                        const std::complex<float>* x,
                        const int& incx,
                        std::complex<float>* y,
...
// x=a*x
static inline void scal(const int& n, const float& alpha, float* x, const int& incx) { sscal_(&n, &alpha, x, &incx); }
static inline void scal(const int& n, const double& alpha, double* x, const int& incx) { dscal_(&n, &alpha, x, &incx); }
static inline void scal(const int& n, const std::complex<float>& alpha, std::complex<float>* x, const int& incx) {
    cscal_(&n, &alpha, x, &incx);
}
static inline void scal(const int& n, const std::complex<double>& alpha, std::complex<double>* x, const int& incx) {
    zscal_(&n, &alpha, x, &incx);
}

// Peize Lin add 2017-10-27
// d=x*y
static inline float dot(const int& n, const float* x, const int& incx, const float* y, const int& incy) {
    return sdot_(&n, x, &incx, y, &incy);
}
static inline double dot(const int& n, const double* x, const int& incx, const double* y, const int& incy) {
    return ddot_(&n, x, &incx, y, &incy);
}
// Denghui Lu add 2023-8-01
static inline std::complex<float>
dot(const int& n, const std::complex<float>* x, const int& incx, const std::complex<float>* y, const int& incy) {
...
dot(const int& n, const std::complex<double>* x, const int& incx, const std::complex<double>* y, const int& incy) {
...
// C = a * A.? * B.? + b * C
static inline void gemm(const char& transa,
                        const char& transb,
                        const int& m,
                        const int& n,
                        const int& k,
...
template <typename T>
static inline void gemm_batched(const char& transa,
                                const char& transb,
                                const int& m,
                                const int& n,
                                const int& k,
...
template <typename T>
static inline void gemm_batched_strided(const char& transa,
                                        const char& transb,
                                        const int& m,
                                        const int& n,
                                        const int& k,
...
static inline void gemv(const char& trans,
                        const int& m,
                        const int& n,
                        const float& alpha,
                        const float* A,
...
template <typename T>
static inline void gemv_batched(const char& trans,
                                const int& m,
                                const int& n,
                                const T& alpha,
                                T** A,
...
template <typename T>
static inline void gemv_batched_strided(const char& transa,
                                        const int& m,
                                        const int& n,
                                        const T& alpha,
                                        const T* A,
...
// out = ||x||_2
static inline float nrm2(const int n, const float* x, const int incx) { return snrm2_(&n, x, &incx); }
static inline double nrm2(const int n, const double* x, const int incx) { return dnrm2_(&n, x, &incx); }
static inline double nrm2(const int n, const std::complex<float>* x, const int incx) { return scnrm2_(&n, x, &incx); }
static inline double nrm2(const int n, const std::complex<double>* x, const int incx) { return dznrm2_(&n, x, &incx); }

// copies a into b
static inline void copy(const int n, const float* a, const int incx, float* b, const int incy) {
    scopy_(&n, a, &incx, b, &incy);
}
static inline void copy(const int n, const double* a, const int incx, double* b, const int incy)

{
    dcopy_(&n, a, &incx, b, &incy);
}
static inline void
copy(const int n, const std::complex<float>* a, const int incx, std::complex<float>* b, const int incy) {
    ccopy_(&n, a, &incx, b, &incy);
}
static inline void
copy(const int n, const std::complex<double>* a, const int incx, std::complex<double>* b, const int incy) {
