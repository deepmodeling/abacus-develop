...
static inline void
    potri (cusolverDnHandle_t& cusolver_handle,
           const char& uplo,
           const char& diag,
           const int& n,
           float* A,
...
static inline void
    heevd (cusolverDnHandle_t& cusolver_handle,
           const char& jobz,
           const char& uplo,
           const int& n,
           float* A,
...
// --- float ---
static inline void
    heevdx (cusolverDnHandle_t& cusolver_handle,
            const int n,
            const int lda,
            float* d_A,
            const char jobz,
...
static inline void
    hegvd (cusolverDnHandle_t& cusolver_handle,
           const int& itype,
           const char& jobz,
           const char& uplo,
           const int& n,
...
// --- float ---
static inline void
    hegvdx (cusolverDnHandle_t& cusolver_handle,
            ...
            std::complex<float>* d_eigen_vec)
{
    ...
    CHECK_CUDA (cudaMemcpy (d_B_copy,
                            reinterpret_cast<cuComplex*> (d_B),
                            sizeof (cuComplex) * n * lda,
                            cudaMemcpyDeviceToDevice));
    ...
    if (h_info < 0)
    ...
    else if (h_info > 0)
        {
            if (jobz_t == CUSOLVER_EIG_MODE_NOVECTOR && h_info <= n)
                ...
                {
                    throw std::runtime_error ("hegvdx (complex<float>): leading minor of order "
                                              + std::to_string (h_info - n) + " of B is not positive definite");
                }
        }
    ...
    if (h_info < 0)
    ...
    else if (h_info > 0)
        {
            if (jobz_t == CUSOLVER_EIG_MODE_NOVECTOR && h_info <= n)
                ...
                {
                    throw std::runtime_error ("hegvdx (complex<double>): leading minor of order "
                                              + std::to_string (h_info - n) + " of B is not positive definite");
                }
        }
...
static inline void
    getrf (cusolverDnHandle_t& cusolver_handle,
           const int& m,
           const int& n,
           std::complex<float>* A,
           const int& lda,
           int* ipiv)
{
    ...
    // Perform LU decomposition
    CHECK_CUSOLVER (
        cusolverDnCgetrf (cusolver_handle, m, n, reinterpret_cast<cuComplex*> (A), lda, d_work, ipiv, d_info));
...
static inline void
    getrs (cusolverDnHandle_t& cusolver_handle,
           const char& trans,
           const int& n,
           const int& nrhs,
           float* A,
           ...
           const int& ldb)
{
    ...
    CHECK_CUSOLVER (
        cusolverDnSgetrs (cusolver_handle, GetCublasOperation (trans), n, nrhs, A, lda, ipiv, B, ldb, d_info));
           ...
           const int& ldb)
{
    ...
    CHECK_CUSOLVER (
        cusolverDnDgetrs (cusolver_handle, GetCublasOperation (trans), n, nrhs, A, lda, ipiv, B, ldb, d_info));
...
// --- std::complex<float> ---
static inline void
    geqrf (cusolverDnHandle_t& cusolver_handle,
           const int m,
           const int n,
           std::complex<float>* d_A,
           const int lda,
...
// --- float ---
static inline void
    orgqr (cusolverDnHandle_t& cusolver_handle,
           const int m,
           const int n,
           const int k,
           float* d_A,
