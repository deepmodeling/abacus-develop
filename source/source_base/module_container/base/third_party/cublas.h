...
static inline void
copy(cublasHandle_t& handle, const int& n, const float* x, const int& incx, float* y, const int& incy) {
    CHECK_CUBLAS(cublasScopy(handle, n, x, incx, y, incy));
}
static inline void
copy(cublasHandle_t& handle, const int& n, const double* x, const int& incx, double* y, const int& incy) {
    CHECK_CUBLAS(cublasDcopy(handle, n, x, incx, y, incy));
}
static inline void copy(cublasHandle_t& handle,
                        const int& n,
...
static inline void copy(cublasHandle_t& handle,
                        ...
                        const int& incy) {
    CHECK_CUBLAS(cublasZcopy(handle,
                             n,
...
static inline void nrm2(cublasHandle_t& handle, const int& n, const float* x, const int& incx, float* result) {
    CHECK_CUBLAS(cublasSnrm2(handle, n, x, incx, result));
}
static inline void nrm2(cublasHandle_t& handle, const int& n, const double* x, const int& incx, double* result) {
    CHECK_CUBLAS(cublasDnrm2(handle, n, x, incx, result));
}
static inline void
nrm2(cublasHandle_t& handle, const int& n, const std::complex<float>* x, const int& incx, float* result) {
    CHECK_CUBLAS(cublasScnrm2(handle, n, reinterpret_cast<const cuComplex*>(x), incx, result));
}
static inline void
nrm2(cublasHandle_t& handle, const int& n, const std::complex<double>* x, const int& incx, double* result) {
    CHECK_CUBLAS(cublasDznrm2(handle, n, reinterpret_cast<const cuDoubleComplex*>(x), incx, result));
}

static inline void dot(cublasHandle_t& handle,
                       const int& n,
                       const float* x,
                       const int& incx,
                       const float* y,
...
static inline void dot(cublasHandle_t& handle,
    ...
    CHECK_CUBLAS(cublasCdotc(handle,
                             n,
...
static inline void dot(cublasHandle_t& handle,
    ...
    CHECK_CUBLAS(cublasZdotc(handle,
                             n,
                             reinterpret_cast<const cuDoubleComplex*>(x),
                             incx,
...
static inline void axpy(cublasHandle_t& handle,
                        const int& n,
                        const float& alpha,
                        const float* x,
                        const int& incx,
...
static inline void axpy(cublasHandle_t& handle,
                        ...
                        std::complex<float>* y,
                        const int& incy) {
    CHECK_CUBLAS(cublasCaxpy(handle,
                             n,
...
static inline void axpy(cublasHandle_t& handle,
                        ...
                        std::complex<double>* y,
                        const int& incy) {
    CHECK_CUBLAS(cublasZaxpy(handle,
                             n,
...
static inline void
scal(cublasHandle_t& handle, const int& n, const std::complex<float>& alpha, std::complex<float>* x, const int& incx) {
    CHECK_CUBLAS(
        cublasCscal(handle, n, reinterpret_cast<const cuComplex*>(&alpha), reinterpret_cast<cuComplex*>(x), incx));
}
static inline void scal(cublasHandle_t& handle,
                        const int& n,
                        const std::complex<double>& alpha,
                        std::complex<double>* x,
                        const int& incx) {
    CHECK_CUBLAS(cublasZscal(handle,
                             n,
                             reinterpret_cast<const cuDoubleComplex*>(&alpha),
...
static inline void gemv(cublasHandle_t& handle,
                        const char& trans,
                        const int& m,
                        const int& n,
                        const float& alpha,
...
static inline void gemv(cublasHandle_t& handle,
                        ...
                        std::complex<float>* y,
                        const int& incy) {
    CHECK_CUBLAS(cublasCgemv(handle,
...
static inline void gemv(cublasHandle_t& handle,
                        ...
                        std::complex<double>* y,
                        const int& incy) {
    CHECK_CUBLAS(cublasZgemv(handle,
                             GetCublasOperation(trans),
                             m,
                             n,
                             reinterpret_cast<const cuDoubleComplex*>(&alpha),
...
template <typename T>
static inline void gemv_batched(cublasHandle_t& handle,
                                const char& trans,
                                const int& m,
                                const int& n,
                                const T& alpha,
...
template <typename T>
static inline void gemv_batched_strided(cublasHandle_t& handle,
                                        const char& transa,
                                        const int& m,
                                        const int& n,
                                        const T& alpha,
...
static inline void gemm(cublasHandle_t& handle,
                        ...
                        std::complex<float>* C,
                        const int& ldc) {
    CHECK_CUBLAS(cublasCgemm(handle,
                             GetCublasOperation(transa),
                             GetCublasOperation(transb),
                             m,
                             n,
...
static inline void gemm_batched(cublasHandle_t& handle,
                                const char& transa,
                                const char& transb,
                                const int& m,
                                const int& n,
...
static inline void gemm_batched(cublasHandle_t& handle,
    ...
    CHECK_CUBLAS(cublasDgemmBatched(handle,
                                    GetCublasOperation(transa),
                                    GetCublasOperation(transb),
                                    m,
                                    n,
...
static inline void gemm_batched(cublasHandle_t& handle,
    ...
    CHECK_CUBLAS(cublasCgemmBatched(handle,
                                    GetCublasOperation(transa),
                                    GetCublasOperation(transb),
                                    m,
                                    n,
                                    ...
                                    ldb,
                                    reinterpret_cast<const cuComplex*>(&beta),
                                    reinterpret_cast<cuComplex**>(d_C),
                                    ldc,
                                    batch_size));
    CHECK_CUDA(cudaFree(d_A));
    CHECK_CUDA(cudaFree(d_B));
...
static inline void gemm_batched(cublasHandle_t& handle,
    ...
    CHECK_CUBLAS(cublasZgemmBatched(handle,
                                    GetCublasOperation(transa),
                                    GetCublasOperation(transb),
                                    m,
                                    n,
...
static inline void gemm_batched_strided(cublasHandle_t& handle,
                                        const char& transa,
                                        const char& transb,
                                        const int& m,
                                        const int& n,
...
static inline void gemm_batched_strided(cublasHandle_t& handle,
                                        ...
                                        const int& stride_c,
                                        const int& batch_size) {
    CHECK_CUBLAS(cublasCgemmStridedBatched(handle,
                                           GetCublasOperation(transa),
                                           GetCublasOperation(transb),
                                           m,
                                           n,
...
static inline void gemm_batched_strided(cublasHandle_t& handle,
                                        ...
                                        const int& stride_c,
                                        const int& batch_size) {
    CHECK_CUBLAS(cublasZgemmStridedBatched(handle,
                                           GetCublasOperation(transa),
                                           GetCublasOperation(transb),
                                           m,
                                           n,
