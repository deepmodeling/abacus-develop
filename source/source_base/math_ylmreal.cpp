...
void YlmReal::Ylm_Real2(const int lmax2,                      // lmax2 = (lmax+1)^2
                        const int ng,                         //
...
//==========================================================
// from ylmr2.f90
void YlmReal::Ylm_Real(const int lmax2,                      // lmax2 = (lmax+1)^2
                       const int ng,                         //
                       const ModuleBase::Vector3<double>* g, // g_cartesian_vec(x,y,z)
                       matrix& ylm                           // output
) {
...
#pragma omp parallel for
        ...
        for (int m = 1; m <= l; m++) {
            // Y_lm, m > 0
            const double same =
                c * sqrt(static_cast<double>(Fact(l - m)) / static_cast<double>(Fact(l + m))) * ModuleBase::SQRT2;
...
void YlmReal::grad_Ylm_Real(const int lmax2,
                            const int ng,
