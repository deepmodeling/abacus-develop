...
void
    YlmReal::Ylm_Real2 (const int lmax2,                      // lmax2 = (lmax+1)^2
                        const int ng,                         //
...
void
    YlmReal::Ylm_Real (Device* ctx, const int lmax2, const int ng, const FPTYPE* g, FPTYPE* ylm)
{
...
// from ylmr2.f90
void
    YlmReal::Ylm_Real (const int lmax2,                      // lmax2 = (lmax+1)^2
                       const int ng,                         //
                       const ModuleBase::Vector3<double>* g, // g_cartesian_vec(x,y,z)
                       matrix& ylm                           // output
    )
                    ...
                    for (int m = 0; m <= l2; m++) // do m = 0, l - 2//mohan modify 2007-10-13
                        {
                            for (int i = 0; i < ng; i++)
                                {
                                    p (m, l, i) = (cost[i] * l3 * p (m, l1, i) - (l1 + m) * p (m, l2, i)) / (l - m);
                                }
                        } // end do
#ifdef _OPENMP
#pragma omp parallel for
                    ...
                    for (int i = 0; i < ng; i++)
                        {
                            ...
                            x2 = std::max (0.0, x2);
                            p (l, l, i)
                                = Semi_Fact (l3) * pow (x2, static_cast<double> (l) / 2.0); // mohan modify 2007-10-13
                            if (l % 2 == 1)
                                {
                                    p (l, l, i) = -p (l, l, i);
                                }
            ...
            for (int m = 1; m <= l; m++)
                {
                    // Y_lm, m > 0
                    const double same = c
                                        * sqrt (static_cast<double> (Fact (l - m)) / static_cast<double> (Fact (l + m)))
                                        * ModuleBase::SQRT2;
...
void
    YlmReal::grad_Ylm_Real (const int lmax2,
                            const int ng,
                            ...
                            for (int im = 0; im < 2 * il + 1; ++im, ++lm)
                                {
                                    ...
                                    dylmx (lm, ig)
                                        = (tmpgylm[lm * 3] - il * rlylm * gg.x / pow (gmod, 2)) / pow (gmod, il);
                                    dylmy (lm, ig)
                                        = (tmpgylm[lm * 3 + 1] - il * rlylm * gg.y / pow (gmod, 2)) / pow (gmod, il);
                                    dylmz (lm, ig)
                                        = (tmpgylm[lm * 3 + 2] - il * rlylm * gg.z / pow (gmod, 2)) / pow (gmod, il);
                                }
                        }
                }
        }
...
template void
    YlmReal::Ylm_Real<float, base_device::DEVICE_CPU> (base_device::DEVICE_CPU*, int, int, const float*, float*);
template void
    YlmReal::Ylm_Real<double, base_device::DEVICE_CPU> (base_device::DEVICE_CPU*, int, int, const double*, double*);
#if ((defined __CUDA) || (defined __ROCM))
template void
    YlmReal::Ylm_Real<float, base_device::DEVICE_GPU> (base_device::DEVICE_GPU*, int, int, const float*, float*);
template void
    YlmReal::Ylm_Real<double, base_device::DEVICE_GPU> (base_device::DEVICE_GPU*, int, int, const double*, double*);
#endif
} // namespace ModuleBase
