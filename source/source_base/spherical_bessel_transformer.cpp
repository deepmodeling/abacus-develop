...
SphericalBesselTransformer::Impl::Impl (const bool cache_enabled)
    : // NOTE: For the current usage of this class, the performance gain
      ...
      fftw_plan_flag_ (FFTW_ESTIMATE), cache_enabled_ (cache_enabled)
{
}

void
    SphericalBesselTransformer::Impl::radrfft (const int l,
                                               const int ngrid,
                                               const double cutoff,
                                               const double* const in,
                                               double* const out,
...
void
    SphericalBesselTransformer::Impl::direct (const int l,
                                              const int ngrid_in,
                                              const double* const grid_in,
                                              const double* const in,
                                              const int ngrid_out,
                                              ...
                                              const int p)
{
    ...
    std::for_each (tmp, tmp + ngrid_in, [&] (double& x) { x *= std::pow (grid_in[&x - tmp], 2 - p); });
...
void
    SphericalBesselTransformer::Impl::_rfft_prepare (const int n)
{
    if (n != sz_planned_)
        {
            if (f_)
...
void
    SphericalBesselTransformer::Impl::_tabulate (const int l,
                                                 const int ngrid_in,
                                                 const double* const grid_in,
                                                 const int ngrid_out,
                                                 const double* const grid_out)
{
    const bool is_cached = cache_enabled_ && l == l_ && ngrid_in <= grid_in_.size () && ngrid_out <= grid_out_.size ()
...
void
    SphericalBesselTransformer::Impl::_rfft_clear ()
{
    if (rfft_plan_)
        {
            fftw_destroy_plan (rfft_plan_);
...
void
    SphericalBesselTransformer::Impl::_table_clear ()
{
...
size_t
    SphericalBesselTransformer::Impl::heap_usage () const
{
    return (grid_in_.capacity () + grid_out_.capacity () + jl_.capacity () + sz_planned_) * sizeof (double);
}

void
    SphericalBesselTransformer::Impl::clear ()
{
    _rfft_clear ();
    _table_clear ();
}
...
//                          Interface
//**********************************************************************
SphericalBesselTransformer::SphericalBesselTransformer (const bool cache_enabled) : impl_ (new Impl (cache_enabled)) {}

void
    SphericalBesselTransformer::radrfft (const int l,
                                         const int ngrid,
                                         const double cutoff,
                                         const double* const in,
                                         double* const out,
...
void
    SphericalBesselTransformer::direct (const int l,
                                        const int ngrid_in,
                                        const double* const grid_in,
                                        const double* const in,
                                        const int ngrid_out,
...
size_t
    SphericalBesselTransformer::heap_usage () const
{
    return impl_->heap_usage ();
}

void
    SphericalBesselTransformer::clear ()
{
    impl_->clear ();
}
