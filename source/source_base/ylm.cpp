...
void
    Ylm::get_ylm_real (const int& Lmax, const ModuleBase::Vector3<double>& vec, double ylmr[])
{
            ...
            if (l == 0)
            ...
            else
                {
                    ...
                    for (m = 0; m <= l2; m++) // do m = 0, l - 2//mohan modify 2007-10-13
                        {
                            p[m][l] = (cost * l3 * p[m][l1] - (l1 + m) * p[m][l2]) / (l - m);
                        } // end do
            ...
            for (m = 1; m <= l; m++)
                {
                    // Y_lm, m > 0
                    const double same
                        = c * sqrt (static_cast<double> (Ylm::Fact (l - m)) / static_cast<double> (Ylm::Fact (l + m)))
                          * ModuleBase::SQRT2;
...
void
    Ylm::get_ylm_real (const int& Lmax, const ModuleBase::Vector3<double>& vec, double ylmr[], double dylmdr[][3])
{
    // 1e-9 is too large
    const double cut0 = 1e-12;
    // allocate space.
 ...
 * *************************/
void
    Ylm::rlylm (const int Lmax, // max momentum of l + 1
                const double x,
                const double y,
                const double z,
                double rly[])
...
void
    Ylm::sph_harm (const int Lmax, // max momentum of l
                   const double xdr,
                   const double ydr,
                   const double zdr,
    ...
    for (int il = 6; il <= Lmax; il++)
        {
            ...
            rly[istart + 2 * il - 1] = (bl3 * rly[istart + 2 * il - 5] - bl2 * rly[istart2 + 2 * il - 5]
                                        - 2.0 * xdr * rly[istart1 + 2 * il - 3])
                                       / bl1;
            rly[istart + 2 * il] = (bl3 * rly[istart + 2 * il - 4] - bl2 * rly[istart2 + 2 * il - 4]
...
void
    Ylm::rl_sph_harm (const int Lmax, // max momentum of L
                      const double x,
                      const double y,
                      const double z,
                      std::vector<double>& rly)
    ...
    for (int il = 6; il <= Lmax; il++)
        {
            ...
            for (int im = 0; im < 2 * il - 1; im++)
                {
                    ...
                    rly[istart + im] = fac2 / sqrt ((double)istart - imm * imm)
                                       * (z * rly[istart1 + im]
                                          - sqrt ((double)istart1 - imm * imm) / fac4 * rly[istart2 + im] * radius2);
                }
            ...
            rly[istart + 2 * il - 1] = (bl3 * rly[istart + 2 * il - 5] - bl2 * rly[istart2 + 2 * il - 5] * radius2
                                        - 2.0 * x * rly[istart1 + 2 * il - 3])
                                       / bl1;
            rly[istart + 2 * il] = (bl3 * rly[istart + 2 * il - 4] - bl2 * rly[istart2 + 2 * il - 4] * radius2
                                    - 2.0 * x * rly[istart1 + 2 * il - 2])
                                   / bl1;
        }
...
void
    Ylm::grad_rl_sph_harm (const int Lmax, // max momentum of L
                           const double x,
                           const double y,
                           const double z,
                           double* rly,
                           double* grly_flat)
{
    ...
    ***************************/
    ...
    double tmp2 = ylmcoef[4] * x;
    rly[7] = ylmcoef[5] * rly[4] - ylmcoef[6] * rly[0] * radius2 - tmp2 * rly[2]; // l=2,m=2
    grly[7][0] = ylmcoef[5] * grly[4][0] - ylmcoef[6] * (rly[0] * tx + grly[0][0] * radius2)
    ...
    ***************************/
    ...
    double tmp5 = ylmcoef[14] * x;
    rly[14] = ylmcoef[12] * rly[10] - ylmcoef[13] * rly[2] * radius2 - tmp5 * rly[7]; // l=3,m=3
    grly[14][0] = ylmcoef[12] * grly[10][0] - ylmcoef[13] * (rly[2] * tx + grly[2][0] * radius2)
    ...
    rly[15] = ylmcoef[12] * rly[11] - ylmcoef[13] * rly[3] * radius2 - tmp5 * rly[8]; // l=3,m=-3
    grly[15][0] = ylmcoef[12] * grly[11][0] - ylmcoef[13] * (rly[3] * tx + grly[3][0] * radius2)
    ...
    ***************************/
    ...
    double tmp9 = ylmcoef[23] * x;
    rly[23] = ylmcoef[21] * rly[19] - ylmcoef[22] * rly[7] * radius2 - tmp9 * rly[14]; // l=4,m=4
    grly[23][0] = ylmcoef[21] * grly[19][0] - ylmcoef[22] * (rly[7] * tx + grly[7][0] * radius2)
    ...
    rly[24] = ylmcoef[21] * rly[20] - ylmcoef[22] * rly[8] * radius2 - tmp9 * rly[15]; // l=4,m=-4
    grly[24][0] = ylmcoef[21] * grly[20][0] - ylmcoef[22] * (rly[8] * tx + grly[8][0] * radius2)
    ...
    ***************************/
    ...
    double tmp14 = ylmcoef[35] * x;
    rly[34] = ylmcoef[33] * rly[30] - ylmcoef[34] * rly[14] * radius2 - tmp14 * rly[23]; // l=5,m=5
    grly[34][0] = ylmcoef[33] * grly[30][0] - ylmcoef[34] * (rly[14] * tx + grly[14][0] * radius2)
                  - ylmcoef[35] * (x * grly[23][0] + rly[23]);
    grly[34][1]
        = ylmcoef[33] * grly[30][1] - ylmcoef[34] * (rly[14] * ty + grly[14][1] * radius2) - tmp14 * grly[23][1];
    grly[34][2]
        = ylmcoef[33] * grly[30][2] - ylmcoef[34] * (rly[14] * tz + grly[14][2] * radius2) - tmp14 * grly[23][2];

    rly[35] = ylmcoef[33] * rly[31] - ylmcoef[34] * rly[15] * radius2 - tmp14 * rly[24]; // l=5,m=-5
    grly[35][0] = ylmcoef[33] * grly[31][0] - ylmcoef[34] * (rly[15] * tx + grly[15][0] * radius2)
                  - ylmcoef[35] * (x * grly[24][0] + rly[24]);
    grly[35][1]
        = ylmcoef[33] * grly[31][1] - ylmcoef[34] * (rly[15] * ty + grly[15][1] * radius2) - tmp14 * grly[24][1];
    grly[35][2]
        = ylmcoef[33] * grly[31][2] - ylmcoef[34] * (rly[15] * tz + grly[15][2] * radius2) - tmp14 * grly[24][2];
    ...
    for (int il = 6; il <= Lmax; il++)
        {
            ...
            for (int im = 0; im < 2 * il - 1; im++)
                {
                    ...
                    grly[istart + im][0] = var1
                                           * (z * grly[istart1 + im][0]
                                              - var2 * (rly[istart2 + im] * tx + grly[istart2 + im][0] * radius2));
                    grly[istart + im][1] = var1
                                           * (z * grly[istart1 + im][1]
                                              - var2 * (rly[istart2 + im] * ty + grly[istart2 + im][1] * radius2));
                    grly[istart + im][2] = var1
                                           * (z * grly[istart1 + im][2] + rly[istart1 + im]
                                              - var2 * (rly[istart2 + im] * tz + grly[istart2 + im][2] * radius2));
                }
            ...
            grly[id1][0] = (bl3 * grly[id2][0] - bl2 * (grly[id3][0] * radius2 + rly[id3] * tx)
                            - 2.0 * (rly[id4] + x * grly[id4][0]))
                           / bl1;
            grly[id1][1]
                = (bl3 * grly[id2][1] - bl2 * (grly[id3][1] * radius2 + rly[id3] * ty) - 2.0 * x * grly[id4][1]) / bl1;
            grly[id1][2]
                = (bl3 * grly[id2][2] - bl2 * (grly[id3][2] * radius2 + rly[id3] * tz) - 2.0 * x * grly[id4][2]) / bl1;

            rly[id1 + 1] = (bl3 * rly[id2 + 1] - bl2 * rly[id3 + 1] * radius2 - 2.0 * x * rly[id4 + 1]) / bl1;
            grly[id1 + 1][0] = (bl3 * grly[id2 + 1][0] - bl2 * (grly[id3 + 1][0] * radius2 + rly[id3 + 1] * tx)
                                - 2.0 * (rly[id4 + 1] + x * grly[id4 + 1][0]))
                               / bl1;
            grly[id1 + 1][1] = (bl3 * grly[id2 + 1][1] - bl2 * (grly[id3 + 1][1] * radius2 + rly[id3 + 1] * ty)
                                - 2.0 * x * grly[id4 + 1][1])
                               / bl1;
            grly[id1 + 1][2] = (bl3 * grly[id2 + 1][2] - bl2 * (grly[id3 + 1][2] * radius2 + rly[id3 + 1] * tz)
                                - 2.0 * x * grly[id4 + 1][2])
                               / bl1;
        }
...
void
    Ylm::hes_rl_sph_harm (const int Lmax, // max momentum of L
                          const double x,
                          const double y,
                          const double z,
                          std::vector<std::vector<double>>& hrly)
{
    ...
    ***************************/
    ...
    hrly[36][0] = (-150 * x * x * x * x - 180 * x * x * y * y + 1080 * x * x * z * z - 30 * y * y * y * y
                   + 360 * y * y * z * z - 240 * z * z * z * z)
                  * coeff;
    hrly[36][1] = (-120 * x * x * x * y - 120 * x * y * y * y + 720 * x * y * z * z) * coeff;
    hrly[36][2] = (720 * x * x * x * z + 720 * x * y * y * z - 960 * x * z * z * z) * coeff;
    hrly[36][3] = (-30 * x * x * x * x - 180 * x * x * y * y + 360 * x * x * z * z - 150 * y * y * y * y
                   + 1080 * y * y * z * z - 240 * z * z * z * z)
                  * coeff;
    hrly[36][4] = (720 * x * x * y * z + 720 * y * y * y * z - 960 * y * z * z * z) * coeff;
    hrly[36][5] = (180 * x * x * x * x + 360 * x * x * y * y - 1440 * x * x * z * z + 180 * y * y * y * y
                   - 1440 * y * y * z * z + 480 * z * z * z * z)
                  * coeff;
    ...
    hrly[37][2] = (25 * x * x * x * x + 30 * x * x * y * y - 180 * x * x * z * z + 5 * y * y * y * y
                   - 60 * y * y * z * z + 40 * z * z * z * z)
                  * coeff;
    ...
    hrly[38][4] = (5 * x * x * x * x + 30 * x * x * y * y - 60 * x * x * z * z + 25 * y * y * y * y
                   - 180 * y * y * z * z + 40 * z * z * z * z)
                  * coeff;
    ...
    hrly[39][0]
        = (30 * x * x * x * x + 12 * x * x * y * y - 192 * x * x * z * z - 2 * y * y * y * y + 32 * z * z * z * z)
          * coeff;
    hrly[39][1] = (8 * x * x * x * y - 8 * x * y * y * y) * coeff;
    hrly[39][2] = (-128 * x * x * x * z + 128 * x * z * z * z) * coeff;
    hrly[39][3]
        = (2 * x * x * x * x - 12 * x * x * y * y - 30 * y * y * y * y + 192 * y * y * z * z - 32 * z * z * z * z)
          * coeff;
    ...
    hrly[40][1] = (20 * x * x * x * x + 36 * x * x * y * y - 96 * x * x * z * z + 20 * y * y * y * y
                   - 96 * y * y * z * z + 32 * z * z * z * z)
                  * coeff;
    ...
    hrly[41][2]
        = (-30 * x * x * x * x + 36 * x * x * y * y + 72 * x * x * z * z + 18 * y * y * y * y - 72 * y * y * z * z)
          * coeff;
    ...
    hrly[42][4]
        = (-18 * x * x * x * x - 36 * x * x * y * y + 72 * x * x * z * z + 30 * y * y * y * y - 72 * y * y * z * z)
          * coeff;
    ...
    hrly[43][0]
        = (-30 * x * x * x * x + 60 * x * x * y * y + 120 * x * x * z * z + 10 * y * y * y * y - 120 * y * y * z * z)
          * coeff;
    hrly[43][1] = (40 * x * x * x * y + 40 * x * y * y * y - 240 * x * y * z * z) * coeff;
    hrly[43][2] = (80 * x * x * x * z - 240 * x * y * y * z) * coeff;
    hrly[43][3]
        = (10 * x * x * x * x + 60 * x * x * y * y - 120 * x * x * z * z - 30 * y * y * y * y + 120 * y * y * z * z)
          * coeff;
                    ...
                    for (int ip = 0; ip <= im; ip++)
                        {
                            ...
                            if (ip > 0)
                                {
                                    Gx_Am[im] += aux * ip * pow (x, ip - 1) * pow (y, im - ip)
                                                 * cos ((im - ip) * ModuleBase::PI / 2.0);
                                    Gx_Bm[im] += aux * ip * pow (x, ip - 1) * pow (y, im - ip)
                                                 * sin ((im - ip) * ModuleBase::PI / 2.0);
                                }

                            if (ip < im)
                                {
                                    Gy_Am[im] += aux * pow (x, ip) * (im - ip) * pow (y, im - ip - 1)
                                                 * cos ((im - ip) * ModuleBase::PI / 2.0);
                                    Gy_Bm[im] += aux * pow (x, ip) * (im - ip) * pow (y, im - ip - 1)
                                                 * sin ((im - ip) * ModuleBase::PI / 2.0);
                                }
                        }
                }
                            ...
                            for (int ik = 0; ik <= kmax; ik++)
                                {
                                    ...
                                    if (ik > 0)
                                        {
                                            Gx_dep[il][im]
                                                += (ik * pow (r2, ik - 1) * 2.0 * x) * pow (z, il - twok - im) * gamma;
                                            Gy_dep[il][im]
                                                += (ik * pow (r2, ik - 1) * 2.0 * y) * pow (z, il - twok - im) * gamma;
                                        }
                                    ...
                                    else
                                        {
                                            if (il - twok - im == 0)
                                            ...
                                            else
                                                {
                                                    Gz_dep[il][im]
                                                        += gamma
                                                           * (ik * pow (r2, ik - 1) * 2.0 * z * pow (z, il - twok - im)
                                                              + pow (r, twok) * (il - twok - im)
                                                                    * pow (z, il - twok - im - 1));
                                                }
                                        }
                                }
...
void
    Ylm::ZEROS (double u[], const int& n)
{
    for (int i = 0; i < n; i++)
        {
            u[i] = 0.0;
