...
void Ylm::get_ylm_real(const int& Lmax, const ModuleBase::Vector3<double>& vec, double ylmr[]) {
    ...
    for (int l = 0; l < Lmax; l++) {
        ...
        for (m = 1; m <= l; m++) {
            // Y_lm, m > 0
            const double same = c *
                                sqrt(static_cast<double>(Ylm::Fact(l - m)) / static_cast<double>(Ylm::Fact(l + m))) *
                                ModuleBase::SQRT2;
 ...
 * *************************/
void Ylm::rlylm(const int Lmax, // max momentum of l + 1
                const double x,
                const double y,
                const double z,
                double rly[]) {
...
void Ylm::sph_harm(const int Lmax, // max momentum of l
                   const double xdr,
                   const double ydr,
                   const double zdr,
...
void Ylm::rl_sph_harm(const int Lmax, // max momentum of L
                      const double x,
                      const double y,
                      const double z,
                      std::vector<double>& rly) {
...
void Ylm::grad_rl_sph_harm(const int Lmax, // max momentum of L
                           const double x,
                           const double y,
                           const double z,
                           double* rly,
    ...
    ***************************/
    ...
    for (int il = 6; il <= Lmax; il++) {
        ...
        for (int im = 0; im < 2 * il - 1; im++) {
            ...
            grly[istart + im][0] =
                var1 * (z * grly[istart1 + im][0] - var2 * (rly[istart2 + im] * tx + grly[istart2 + im][0] * radius2));
            grly[istart + im][1] =
                var1 * (z * grly[istart1 + im][1] - var2 * (rly[istart2 + im] * ty + grly[istart2 + im][1] * radius2));
            grly[istart + im][2] = var1 * (z * grly[istart1 + im][2] + rly[istart1 + im] -
...
void Ylm::hes_rl_sph_harm(const int Lmax, // max momentum of L
                          const double x,
                          const double y,
                          const double z,
    ...
    for (int im = 0; im < MaxL + 1; im++) {
        if (im == 0) {
            ...
            for (int ip = 0; ip <= im; ip++) {
                ...
                if (ip < im) {
                    Gy_Am[im] +=
                        aux * pow(x, ip) * (im - ip) * pow(y, im - ip - 1) * cos((im - ip) * ModuleBase::PI / 2.0);
                    Gy_Bm[im] +=
                        aux * pow(x, ip) * (im - ip) * pow(y, im - ip - 1) * sin((im - ip) * ModuleBase::PI / 2.0);
                }
            }
        }
    }
