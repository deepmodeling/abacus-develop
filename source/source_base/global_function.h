...
template <class T>
void
    OUT (std::ofstream& ofs, const std::string& name, const T& a)
{
...
template <class T>
void
    OUT (std::ofstream& ofs, const std::string& name, const T& x, const T& y)
{
...
template <class T>
void
    OUT (std::ofstream& ofs, const std::string& name, const T& x, const T& y, const T& z)
{
...
template <class T>
void
    OUTP (std::ofstream& ofs, const std::string& name, const T& a, const std::string& explanation = "")
{
...
template <class T>
void
    OUT (const std::string& name, const T& a)
{
...
template <class T>
void
    AUTO_SET (const std::string& name, const T& a)
{
...
inline void
    ZEROS (std::complex<T>* u, const TI n) // Peize Lin change int to TI at 2020.03.03
{
    assert (n >= 0);
    for (TI i = 0; i < n; i++)
        {
...
template <class T, class TI>
inline void
    ZEROS (T* u, const TI n) // Peize Lin change int to TI at 2020.03.03
{
    assert (n >= 0);
    for (TI i = 0; i < n; i++)
        {
...
template <class T>
static void
    READ_VALUE (std::ifstream& ifs, T& v)
{
    ifs >> v;
    std::string line;
    getline (ifs, line);
...
//-------------------------------------------------------------
bool
    SCAN_BEGIN (std::ifstream& ifs, const std::string& TargetName, const bool restart = true, const bool ifwarn = true);
...
template <class T>
static inline void
    DCOPY (const T& a, T& b, const int& dim)
{
    for (int i = 0; i < dim; ++i)
        {
            b[i] = a[i];
...
template <>
inline void
    COPYARRAY (const std::complex<double>* a, std::complex<double>* b, const int dim)
{
    const int one = 1;
...
template <>
inline void
    COPYARRAY (const double* a, double* b, const int dim)
{
    const int one = 1;
...
template <class T>
static inline T*
    VECTOR_TO_PTR (std::vector<T>& v)
{
    return &(v[0]);
}
template <class T>
static inline T*
    VECTOR_TO_PTR (std::valarray<T>& v)
{
    return &(v[0]);
}
...
static inline const T*
    VECTOR_TO_PTR (const std::vector<T>& v)
{
    return &(v[0]);
}
template <class T>
static inline const T*
    VECTOR_TO_PTR (const std::valarray<T>& v)
{
    return &(v[0]);
}
...
template <typename T>
std::string
    TO_STRING (const T& t, const int n = 20) // n=20 since LDBL_EPSILON is 1E-16 or 1E-19
{
...
inline void*
    MAP_EXIST (T_map& ms, const T_key1& key1)
{
...
inline void*
    MAP_EXIST (T_map& ms, const T_key1& key1, const T_key_tail&... key_tail)
{
    auto ms1 = ms.find (key1);
    if (ms1 == ms.end ())
        {
...
template <typename T_map, typename T_key1>
inline const void*
    MAP_EXIST (const T_map& ms, const T_key1& key1)
{
    auto ms1 = ms.find (key1);
    if (ms1 == ms.end ())
        {
...
template <typename T_map, typename T_key1, typename... T_key_tail>
inline const void*
    MAP_EXIST (const T_map& ms, const T_key1& key1, const T_key_tail&... key_tail)
{
    auto ms1 = ms.find (key1);
    if (ms1 == ms.end ())
        {
...
static inline void
    DELETE_MUL_PTR (T_element* v, const T_N_first N_first, const T_N_tail... N_tail)
{
...
template <typename T_element, typename T_N_first, typename... T_N_tail>
static inline void
    FREE_MUL_PTR (T_element* v, const T_N_first N_first, const T_N_tail... N_tail)
{
    for (T_N_first i = 0; i < N_first; ++i)
        {
...
//==========================================================
static inline bool
    IS_COLUMN_MAJOR_KS_SOLVER (std::string ks_solver)
{
