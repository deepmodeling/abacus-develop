 ...
 *   4. GatherIntAll:
 *       Tests gather_int_all() and reduce_min_int()
 *   5. GatherDoubleAll:
 ...
 *       Tests two variations of reduce_pool()
 *   12.GatherDoublePool:
 *       Tests reduce_min_double_pool() and reduce_max_double_pool()
 *
 *
 */
...
TEST_F(ParaReduce, GatherIntAll)
{
    ...
    Parallel_Reduce::reduce_min_int(min_number);
...
TEST_F(ParaReduce, GatherDoubleAll)
{
    ...
    Parallel_Reduce::reduce_min_double(min_number);
    // get maximum integer among all processes
    double max_number = local_number;
    Parallel_Reduce::reduce_max_double(max_number);
    for (int i = 0; i < nproc; i++)
    {
        EXPECT_LE(min_number, array[i]);
...
TEST_F(ParaReduce, GatherDoublePool)
{
    ...
    if (nproc == 4)
    {
        ...
        // get minimum integer among all processes
        double min_number = local_number;
        Parallel_Reduce::reduce_min_double_pool(mpiContext.nproc_in_pool, min_number);
        // get maximum integer among all processes
        double max_number = local_number;
        Parallel_Reduce::reduce_max_double_pool(mpiContext.nproc_in_pool, max_number);
