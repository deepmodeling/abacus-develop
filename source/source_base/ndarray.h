 ...
 * @brief under the restriction of C++11, a simple alternative to std::vector<T> + std::mdspan. In
 * source_base/module_container/ATen/tensor.h, tensor class provides a cross-device container, but std::string is not
 * supported. Therefore, this class is to provide a general (but CPU-only) container for multi-dimensional data. It can
 * easily convert to ontainer::Tensor.
 * @version 0.1
 * @date 2024-04-24
 *
 ...
 * source_base/module_container/ATen/tensor.h, tensor class provides a cross-device container, but std::string is not
 * supported. Therefore, this class is to provide a general (but CPU-only) container for multi-dimensional data. It can
 * easily convert to container::Tensor.
 *
 * @tparam T
 */
template <typename T>
class NDArray
{
  ...
  public:
    ...
    NDArray (std::initializer_list<size_t> il)
        : shape_ (il), data_ (std::accumulate (shape_.begin (), shape_.end (), 1, std::multiplies<size_t> ()))
    {
    }
    NDArray (std::initializer_list<int> il)
        : shape_ (il.begin (), il.end ()),
          data_ (std::accumulate (shape_.begin (), shape_.end (), 1, std::multiplies<size_t> ()))
    {
    }
    // variadic template constructor, (delegate constructor)
    template <typename... Args>
    NDArray (const size_t idx, Args... args) : NDArray ({idx, static_cast<size_t> (args)...})
    {
    }
    template <typename... Args>
    NDArray (const int& idx, Args... args) : NDArray ({idx, static_cast<int> (args)...})
    {
    } // not happy with this because size_t can have larger range
    // copy constructor
     ...
     */
    bool
        operator== (const NDArray& other) const
    {
    ...
    template <typename... Args>
    T&
        at (const size_t idx, Args... args)
    {
        return data_[index (idx, args...)];
    }
    template <typename... Args>
    const T&
        at (const size_t idx, Args... args) const
    {
        return data_[index (idx, args...)];
    }
    /**
    ...
    template <typename... Args>
    T&
        operator() (const size_t idx, Args... args)
    {
        return data_[index (idx, args...)];
    }
    template <typename... Args>
    const T&
        operator() (const size_t idx, Args... args) const
    {
        return data_[index (idx, args...)];
    }
    // front
    ...
    /**
     * @brief SFINAE (Substitution Failure Is Not An Error) to_tensor function, only if T is int, double, float,
     * std::complex<float>, std::complex<double>, otherwise there is no such function
     *
    ...
    template <typename... Args>
    size_t
        index (const size_t idx, Args... args) const
    {
        ...
        for (size_t i = 0; i < shape_.size (); ++i)
            {
                index += indices[i]
                         * std::accumulate (shape_.begin () + i + 1, shape_.end (), 1, std::multiplies<size_t> ());
            }
