...
// Copy constructor
ComplexMatrix::ComplexMatrix (const ComplexMatrix& m1) : nr (m1.nr), nc (m1.nc), size (m1.size), c (nullptr)
{
    if (size)
        {
            c = new std::complex<double>[size];
...
void
    ComplexMatrix::create (const int nr_in, const int nc_in, const bool flag_zero)
{
    if (nr_in && nc_in)
        {
            if (c)
            ...
            for (int j = 0; j < nc; j++)
                {
                    if (i == j)
                        {
                            c[nc * i + j] = std::complex<double> (1.0, 0.0);
                        }
                    else
                        {
                            c[nc * i + j] = std::complex<double> (0.0, 0.0);
                        }
                }
        }
    return;
...
// mohan add 2021-04-05
    ...
    BlasConnector::gemm ('N', 'N', m1.nr, m2.nc, m1.nc, 1, m1.c, m1.nc, m2.c, m2.nc, 0, mprod.c, mprod.nc);
#endif
...
std::ostream&
    ComplexMatrix::print (std::ostream& os, const double threshold_abs, const double threshold_imag) const
{
