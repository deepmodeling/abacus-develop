             ...
             std::vector<double>& alpha)
{
    ...
    for (int p = 0; p < nbes; ++p)
        {
            std::transform (r,
                            ...
                            tmp.begin (),
                            integrand.begin (),
                            [theta, p, rcut, l] (double r_i, double tmp_i)
    ...
    std::transform (w.begin (),
                    ...
                    z.begin (),
                    c.begin (),
                    [] (double w_p, double z_p) { return w_p * w_p / z_p; });
    double prefac = 1.0 / std::sqrt (std::accumulate (c.begin (), c.end (), 0.0));
    std::transform (w.begin (),
                    w.end (),
                    z.begin (),
                    c.begin (),
                    [prefac] (double w_p, double z_p) { return prefac * w_p / z_p; });
...
void
    smoothgen (const int nr, const double* r, const double* chi, const double rcut, std::vector<double>& alpha)
{
    ...
    auto smooth_sigma = [&] (double sigma_in)
        {
            ...
            std::transform (r,
                            ...
                            alpha.data (),
                            tmp.begin (),
                            [] (double r_i, double chi_i) { return r_i * r_i * chi_i; });
    ...
    auto overlap_dalpha_dchi = [&] ()
        {
            ...
            std::transform (r,
                            ...
                            dalpha.data (),
                            tmp.begin (),
                            [] (double r_i, double dalpha_i) { return r_i * r_i * dalpha_i; });
