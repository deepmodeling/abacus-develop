...
{
void
    ModuleBase::Global_File::make_dir_out (const std::string& suffix,
            ...
            if (rank == times)
                {
                    ...
                    if (ret == 0 || errno == EEXIST)
                        ...
                        {
                            std::cout << " PROC " << rank << " Please delete the file named by OUT.suffix !!! "
                                      << std::endl;
                            make_dir = 0;
                        }
                }
                    ...
                    if (rank == times)
                        {
                            ...
                            if (ret == 0 || errno == EEXIST)
                                {
                                    std::cout << " MAKE THE STRU DIR    : " << PARAM.globalv.global_stru_dir
                                              << std::endl;
                                    make_dir_stru = 1;
                                }
                            else
                    ...
                    if (rank == times)
                        {
                            ...
                            if (ret == 0 || errno == EEXIST)
                                {
                                    std::cout << " MAKE THE MATRIX DIR    : " << PARAM.globalv.global_matrix_dir
                                              << std::endl;
                                    make_dir_matrix = 1;
                                }
                            else
                    ...
                    if (rank == times)
                        {
                            ...
                            if (ret == 0 || errno == EEXIST)
                                {
                                    std::cout << " MAKE THE MLKEDF DESCRIPTOR DIR    : "
                                              << PARAM.globalv.global_mlkedf_descriptor_dir << std::endl;
                                    make_dir_descrip = 1;
                                }
                            else
                                {
                                    std::cout << " PROC " << rank << " CAN NOT MAKE THE MLKEDF DESCRIPTOR DIR !!! "
                                              << std::endl;
                                    make_dir_descrip = 0;
                                }
                        }
                    ...
                    if (rank == times)
                        {
                            ...
                            if (ret == 0 || errno == EEXIST)
                                ...
                                {
                                    std::cout << " PROC " << rank << " CAN NOT MAKE THE DEEPKS LABELS (ELEC) DIR !!! "
                                              << std::endl;
...
void
    ModuleBase::Global_File::make_dir_atom (const std::string& label)
{
    //----------------------------------------------------------
...
void
    ModuleBase::Global_File::open_log (std::ofstream& ofs,
                                       const std::string& fn,
                                       const std::string& calculation,
                                       const bool& restart)
{
...
void
    ModuleBase::Global_File::close_log (std::ofstream& ofs, const std::string& fn)
{
    if (ofs)
        {
            ofs.close ();
...
void
    ModuleBase::Global_File::close_all_log (const int rank, const bool out_alllog, const std::string& calculation)
{
    //----------------------------------------------------------
