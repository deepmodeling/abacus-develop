...
void CubicSpline::add(const double* y, const BoundaryCondition& bc_start, const BoundaryCondition& bc_end) {
...
void CubicSpline::multi_eval(int n_spline,
    ...
    if (y_interp) {
        ...
        for (int i = 0; i < n_spline; ++i) {
            ...
            y_interp[i] = wy0 * y_[offset] + wy1 * y_[offset + 1] + ws0 * y_[offset + n_] + ws1 * y_[offset + n_ + 1];
        }
    }
    ...
    if (dy_interp) {
        ...
        for (int i = 0; i < n_spline; ++i) {
            offset = i_spline[i] * 2 * n_ + p;
            dy_interp[i] = wy1 * (y_[offset + 1] - y_[offset]) + ws0 * y_[offset + n_] + ws1 * y_[offset + n_ + 1];
        }
    }
    ...
    if (d2y_interp) {
        ...
        for (int i = 0; i < n_spline; ++i) {
            offset = i_spline[i] * 2 * n_ + p;
            d2y_interp[i] = wy1 * (y_[offset + 1] - y_[offset]) + ws0 * y_[offset + n_] + ws1 * y_[offset + n_ + 1];
        }
    }
}

void CubicSpline::multi_eval(double x, double* y, double* dy, double* d2y) const {
...
void CubicSpline::eval(int n,
    ...
    // indices of the polynomial segments that contain x_interp
    std::vector<int> _ind(n_interp);
    std::transform(x_interp, x_interp + n_interp, _ind.begin(), [n, x0, dx](double x_i) {
        return _index(n, x0, dx, x_i);
    });
...
void CubicSpline::_validate_eval(int n,
    ...
    assert(std::all_of(x_interp, x_interp + n_interp, [xmin, xmax](double x_i) { return xmin <= x_i && x_i <= xmax; }));
}
    ...
    if (n == 2 && bc_start.type == BoundaryType::periodic) {
        ...
        if (bc_start.type == BoundaryType::periodic) {
            ...
            default: // BoundaryCondition::not_a_knot
                ...
                l[n - 2] = dx[n - 3] + dx[n - 2];
                dy[n - 1] =
                    (dd[n - 2] * dx[n - 3] * (dx[n - 2] + 2 * l[n - 2]) + dd[n - 3] * dx[n - 2] * dx[n - 2]) / l[n - 2];
            }
...
void CubicSpline::_solve_cyctri(int n, double* d, double* u, double* l, double* b) {
    ...
    std::transform(bp.begin(), bp.begin() + n, bp.begin() + n, b, [fac](double yi, double zi) {
        return yi - fac * zi;
    });
}
