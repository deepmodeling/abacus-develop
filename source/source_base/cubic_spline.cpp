...
CubicSpline::BoundaryCondition::BoundaryCondition (BoundaryType type) : type (type)
{
    assert (type == BoundaryType::periodic || type == BoundaryType::not_a_knot);
}

CubicSpline::BoundaryCondition::BoundaryCondition (BoundaryType type, double val) : type (type), val (val)
{
...
CubicSpline::CubicSpline (int n, const double* x) : n_spline_ (0), n_ (n), xmin_ (x[0]), xmax_ (x[n - 1]), x_ (x, x + n)
{
}
...
void
    CubicSpline::add (const double* y, const BoundaryCondition& bc_start, const BoundaryCondition& bc_end)
{
...
void
    CubicSpline::multi_eval (int n_spline,
                             const int* i_spline,
                             double x_interp,
                             double* y_interp,
                             double* dy_interp,
                             double* d2y_interp) const
{
    assert (std::all_of (i_spline, i_spline + n_spline, [this] (int i) { return 0 <= i && i < n_spline_; }));
    _validate_eval (n_, {xmin_, dx_}, x_.empty () ? nullptr : x_.data (), y_.data (), &y_[n_], 1, &x_interp);
            ...
            for (int i = 0; i < n_spline; ++i)
                {
                    ...
                    y_interp[i]
                        = wy0 * y_[offset] + wy1 * y_[offset + 1] + ws0 * y_[offset + n_] + ws1 * y_[offset + n_ + 1];
                }
        }
            ...
            for (int i = 0; i < n_spline; ++i)
                {
                    offset = i_spline[i] * 2 * n_ + p;
                    dy_interp[i]
                        = wy1 * (y_[offset + 1] - y_[offset]) + ws0 * y_[offset + n_] + ws1 * y_[offset + n_ + 1];
                }
        }
...
void
    CubicSpline::multi_eval (double x, double* y, double* dy, double* d2y) const
{
                       ...
                       double* d2y_interp)
{
    ...
    std::transform (x_interp, x_interp + n_interp, _ind.begin (), [n, x] (double x_i) { return _index (n, x, x_i); });
...
void
    CubicSpline::_validate_build (int n,
                                  const double* dx,
                                  const double* y,
                                  const BoundaryCondition& bc_start,
                                  const BoundaryCondition& bc_end)
{
    ...
    assert ((bc_start.type == BoundaryType::periodic) == (bc_end.type == BoundaryType::periodic));
    ...
    assert ((bc_start.type != BoundaryType::not_a_knot && bc_end.type != BoundaryType::not_a_knot) || n > 2);
...
void
    CubicSpline::_validate_eval (int n,
                                 const double (&u)[2],
                                 const double* x,
                                 const double* y,
                                 const double* dy,
                                 ...
                                 const double* x_interp)
{
    ...
    assert (
        std::all_of (x_interp, x_interp + n_interp, [xmin, xmax] (double x_i) { return xmin <= x_i && x_i <= xmax; }));
}

void
    CubicSpline::_build (int n,
                         const double* dx,
                         const double* y,
                         const BoundaryCondition& bc_start,
                         const BoundaryCondition& bc_end,
                         double* dy)
{
    ...
    else if (n == 3 && bc_start.type == BoundaryType::not_a_knot && bc_end.type == BoundaryType::not_a_knot)
        {
...
int
    CubicSpline::_index (int n, const double* knots, double x)
{
...
int
    CubicSpline::_index (int n, double x0, double dx, double x)
{
    int i = (x - x0) / dx;
    return i - (i == n - 1);
}
...
void
    CubicSpline::_solve_cyctri (int n, double* d, double* u, double* l, double* b)
{
