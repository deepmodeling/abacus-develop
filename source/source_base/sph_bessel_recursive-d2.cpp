...
void
    Sph_Bessel_Recursive::D2::set_dx (const double dx_in)
{
...
const std::vector<std::vector<std::vector<double>>>&
    Sph_Bessel_Recursive::D2::cal_jlx (const int lmax, const size_t ix1_size, const size_t ix2_size)
{
    if (lmax < 0)
        {
...
void
    Sph_Bessel_Recursive::D2::cal_jlx_0 (const int l_size, const size_t ix1_size, const size_t ix2_size)
{
...
void
    Sph_Bessel_Recursive::D2::cal_jlx_smallx (const int l_size, const size_t ix1_size, const size_t ix2_size)
{
    if (jlx.size () < static_cast<size_t> (l_size))
        {
            jlx.resize (l_size);
...
void
    Sph_Bessel_Recursive::D2::cal_jlx_recursive (const int l_size, const size_t ix1_size, const size_t ix2_size)
{
    if (jlx.size () < static_cast<size_t> (l_size))
        {
            jlx.resize (l_size);
            ...
            switch(l)
            {
                ...
                case 10:
                {
                    ...
                    for( size_t ix=max(ix_size_begin,ix_size_piecewise); ix<ix_size; ++ix )
                    {
                        ...
                        jlx[l][ix] = sin(x1)*( - 1.0/x1 + 1485.0/x3 - 315315.0/x5 + 18918900.0/x7 - 310134825.0/x9 +
654729075.0/x11 )
                ...
                case 11:
                {
                    ...
                    for( size_t ix=max(ix_size_begin,ix_size_piecewise); ix<ix_size; ++ix )
                    {
                        ...
                        jlx[l][ix] = sin(x1)*( - 66.0/x2 + 45045.0/x4 - 7567560.0/x6 + 413513100.0/x8 - 6547290750.0/x10
+ 13749310575.0/x12 )
                                   + cos(x1)*( 1.0/x1 - 2145.0/x3 + 675675.0/x5 - 64324260.0/x7 + 1964187225.0/x9 -
13749310575.0/x11 );
                    }
                    break;
                }
...
//						jlx[l][ix] = sin(x1)*( 1.0/x1 - 3003.0/x3 + 1351350.0/x5 - 192972780.0/x7 + 9820936125.0/x9 -
151242416325.0/x11 + 316234143225.0/x13 )
//						           + cos(x1)*( 78.0/x2 - 75075.0/x4 + 18378360.0/x6 - 1571349780.0/x8 +
45831035250.0/x10 - 316234143225.0/x12 ); jlx[l][ix] = sin(x1)*( (1.0 +(-3003.0 +(1351350.0/x5 +(-192972780.0
+(9820936125.0 +(-151242416325.0 +316234143225.0/x2)/x2)/x2)/x2)/x2)/x2)/x1 )
                                   + cos(x1)*( (78.0 +(-75075.0 +(18378360.0 +(-1571349780.0 +(45831035250.0
-316234143225.0/x2)/x2)/x2)/x2)/x2)/x2 );
                    }
                    break;
                }
                default:
...
//std::cout<<l<<"\t"<<(double)(t_end.tv_sec-t_start.tv_sec) +
