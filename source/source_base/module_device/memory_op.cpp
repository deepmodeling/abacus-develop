...
{
    void
        operator() (FPTYPE*& arr, const size_t size, const char* record_in)
    {
        if (arr != nullptr)
            {
                free (arr);
...
{
    void
        operator() (FPTYPE* arr, const int var, const size_t size)
    {
        ModuleBase::OMP_PARALLEL (
            [&] (int num_thread, int thread_id)
                {
                    int beg = 0, len = 0;
                    ModuleBase::BLOCK_TASK_DIST_1D (num_thread,
                                                    thread_id,
                                                    size,
                                                    (size_t)4096 / sizeof (FPTYPE),
                                                    beg,
                                                    len);
...
{
    void
        operator() (FPTYPE* arr, const size_t pitch, const int var, const size_t width, const size_t height)
    {
...
{
    void
        operator() (FPTYPE* arr_out, const FPTYPE* arr_in, const size_t size)
    {
        ModuleBase::OMP_PARALLEL (
            [&] (int num_thread, int thread_id)
                {
...
struct synchronize_memory_2d_op<FPTYPE, base_device::DEVICE_CPU, base_device::DEVICE_CPU>
{
                    ...
                    const size_t height)
    {
        for (int i = 0; i < height; i++)
            {
                synchronize_memory_op<FPTYPE, base_device::DEVICE_CPU, base_device::DEVICE_CPU> () (arr_out
                                                                                                        + i * dpitch,
                                                                                                    arr_in + i * spitch,
                                                                                                    width);
            }
...
{
    void
        operator() (FPTYPE*& arr, const size_t size, const char* record_in = nullptr)
    {
    }
};
...
template <typename FPTYPE>
void
    resize_memory (FPTYPE* arr, const size_t size, base_device::AbacusDevice_t device_type)
{
...
template <typename FPTYPE>
void
    set_memory (FPTYPE* arr, const int var, const size_t size, base_device::AbacusDevice_t device_type)
{
    if (device_type == base_device::AbacusDevice_t::CpuDevice)
        {
...
template <typename FPTYPE>
void
    synchronize_memory (FPTYPE* arr_out,
                        const FPTYPE* arr_in,
                        const size_t size,
                        base_device::AbacusDevice_t device_type_out,
                        base_device::AbacusDevice_t device_type_in)
{
    if (device_type_out == base_device::AbacusDevice_t::CpuDevice
        || device_type_in == base_device::AbacusDevice_t::CpuDevice)
        {
            synchronize_memory_op<FPTYPE, DEVICE_CPU, DEVICE_CPU> () (arr_out, arr_in, size);
        }
    else if (device_type_out == base_device::AbacusDevice_t::CpuDevice
             || device_type_in == base_device::AbacusDevice_t::GpuDevice)
        {
            synchronize_memory_op<FPTYPE, DEVICE_CPU, DEVICE_GPU> () (arr_out, arr_in, size);
        }
    else if (device_type_out == base_device::AbacusDevice_t::GpuDevice
             || device_type_in == base_device::AbacusDevice_t::CpuDevice)
        {
...
template <typename FPTYPE_out, typename FPTYPE_in>
void
    cast_memory (FPTYPE_out* arr_out,
...
template <typename FPTYPE>
void
    delete_memory (FPTYPE* arr, base_device::AbacusDevice_t device_type)
{
    if (device_type == base_device::AbacusDevice_t::CpuDevice)
        {
