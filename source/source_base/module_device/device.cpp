...
#if __MPI
int
    get_node_rank_with_mpi_shared (const MPI_Comm mpi_comm)
{
...
std::string
    get_device_flag (const std::string& device, const std::string& basis_type)
{
    // 1. Validate input string
    if (device != "cpu" && device != "gpu" && device != "auto")
        {
            ModuleBase::WARNING_QUIT ("device",
                                      "Parameter \"device\" can only be set to \"cpu\", \"gpu\", or \"auto\"!");
        }
    ...
    if (device == "gpu")
        {
            if (probe_gpu_availability ())
                ...
                {
                    ModuleBase::WARNING_QUIT ("device",
                                              "Device is set to 'gpu', but no available GPU was found. Please check "
                                              "your hardware/drivers or set 'device=cpu'.");
                }
        }
    else if (device == "auto")
        {
            if (probe_gpu_availability ())
            ...
            else
                {
                    ...
                    // std::endl; std::cout << "          To suppress this warning, please explicitly set 'device=cpu'
                    // in your input." << std::endl;
                }
        }
    else
        { // device == "cpu"
