...
template <typename T, typename Device>
void
    send_dev (const T* object, int count, int dest, int tag, MPI_Comm& comm, T* tmp_space = nullptr)
{
#ifdef __CUDA_MPI
...
template <typename T, typename Device>
void
    isend_dev (const T* object, int count, int dest, int tag, MPI_Comm& comm, MPI_Request* request, T* send_space)
{
#ifdef __CUDA_MPI
...
template <typename T, typename Device>
void
    recv_dev (T* object, int count, int source, int tag, MPI_Comm& comm, MPI_Status* status, T* tmp_space = nullptr)
{
#ifdef __CUDA_MPI
...
template <typename T, typename Device>
void
    bcast_dev (T* object, const int& n, const MPI_Comm& comm, int root = 0, T* tmp_space = nullptr)
{
#if defined(__NCCL_PARALLEL_DEVICE)
...
template <typename T, typename Device>
void
    reduce_dev (T* object, const int& n, const MPI_Comm& comm, T* tmp_space = nullptr)
{
#if defined(__NCCL_PARALLEL_DEVICE)
    if (std::is_same<Device, base_device::DEVICE_GPU>::value)
        {
