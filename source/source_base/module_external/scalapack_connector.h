...
struct block2d_data_type {
    constexpr static bool value = std::is_same<T, double>::value || std::is_same<T, std::complex<double>>::value ||
 ...
 */
template <typename T>
typename std::enable_if<block2d_data_type<T>::value, void>::type
Cpxgemr2d(int M, int N, T* A, int IA, int JA, int* DESCA, T* B, int IB, int JB, int* DESCB, int ICTXT) {
    ...
    if (std::is_same<T, float>::value)
        Cpsgemr2d(M, N, reinterpret_cast<float*>(A), IA, JA, DESCA, reinterpret_cast<float*>(B), IB, JB, DESCB, ICTXT);
    ...
    if (std::is_same<T, int>::value)
        Cpigemr2d(M, N, reinterpret_cast<int*>(A), IA, JA, DESCA, reinterpret_cast<int*>(B), IB, JB, DESCB, ICTXT);
};

class ScalapackConnector {
  public:
    static inline void geadd(const char transa,
                             const int m,
                             const int n,
                             const std::complex<double> alpha,
                             const std::complex<double>* a,
    ...
    static inline void
    dot(int n, double& dot, double* a, int ia, int ja, int inca, double* b, int ib, int jb, int incb, int* desc) {
    ...
    static inline void gemm(char transa,
                            char transb,
                            int M,
                            int N,
                            int K,
    ...
    static inline void
    symm(char side, char uplo, int m, int n, double alpha, double* a, double* b, double beta, double* c, int* desc) {
        int isrc = 1;
        pdsymm_(&side,
                &uplo,
                &m,
    ...
    static inline void getrf(const int M,
                             const int N,
                             std::complex<double>* A,
                             const int IA,
                             const int JA,
                             const int* DESCA,
                             int* ipiv,
                             int* info) // fix a bug: info is output and we must use int*
    {
    ...
    static inline void gesv(const int n,
                            const int nrhs,
                            const std::complex<double>* A,
                            const int ia,
                            const int ja,
    ...
    static inline void tranu(const int m,
                             const int n,
                             const std::complex<double> alpha,
                             std::complex<double>* a,
                             const int ia,
    ...
    static inline int potrf(char uplo, int na, double* U, int* desc) {
        int isrc = 1;
        int info;
    ...
    static inline int potrf(char uplo, int na, std::complex<double>* U, int* desc) {
        int isrc = 1;
        int info;
    ...
    static inline void
    trmm(char side, char uplo, char trans, char diag, int m, int n, double alpha, double* a, double* b, int* desc) {
        int isrc = 1;
        pdtrmm_(&side, &uplo, &trans, &diag, &m, &n, &alpha, a, &isrc, &isrc, desc, b, &isrc, &isrc, desc);
    }

    static inline void trmm(char side,
                            char uplo,
                            char trans,
                            char diag,
                            int m,
    ...
    static inline void hemm(char side,
                            char uplo,
                            int na,
                            std::complex<double> alpha,
                            std::complex<double>* a,
