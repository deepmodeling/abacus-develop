...
public:
	...
	static
	void ger_cm(int m, int n, double alpha, const double* x,
    	int incx, const double* y, const int incy, double a, int lda, base_device::AbacusDevice_t device_type = base_device::AbacusDevice_t::CpuDevice);
	...
	static
	void herk(char uplo, char trans, int n, int k, float alpha, const std::complex<float> *A, int lda, float beta, std::complex<float> *C, int ldc, base_device::AbacusDevice_t device_type = base_device::AbacusDevice_t::CpuDevice);
	static
	void herk(char uplo, char trans, int n, int k, double alpha, const std::complex<double> *A, int lda, double beta, std::complex<double> *C, int ldc, base_device::AbacusDevice_t device_type = base_device::AbacusDevice_t::CpuDevice);

	static
	void syrk(char uplo, char trans, int n, int k,
    	double alpha, const double* a, int lda, double beta, double* c, int ldc, base_device::AbacusDevice_t device_type = base_device::AbacusDevice_t::CpuDevice);
	
	// y = A*x + beta*y
	static
	void gemv_cm(const char trans, const int m, const int n,
...
namespace BlasUtils {
	static inline
	CBLAS_UPLO toCblasUplo(const char uplo)
	{
		if (uplo == 'U' || uplo == 'u')
			return CblasUpper;
			...
			throw std::invalid_argument("Invalid uplo argument");
	}

	static inline
	CBLAS_TRANSPOSE toCblasTrans(const char trans)
	{
		if (trans == 'N' || trans == 'n')
			return CblasNoTrans;
			...
			throw std::invalid_argument("Invalid trans augument");
	}
}
