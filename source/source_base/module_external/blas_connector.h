...
extern "C"
{
    ...
    void
        zcopy_ (const int* n, const std::complex<double>* a, const int* incx, std::complex<double>* b, const int* incy);
...
class BlasConnector
{
  public:
    ...
    static double dotu (const int n,
                        const double* const X,
                        const int incX,
                        const double* const Y,
                        const int incY,
    ...
    static std::complex<float> dotu (const int n,
    ...
    static std::complex<double> dotu (const int n,
    ...
    static double dotc (const int n,
                        const double* const X,
                        const int incX,
                        const double* const Y,
                        const int incY,
    ...
    static std::complex<float> dotc (const int n,
                                     const std::complex<float>* const X,
                                     const int incX,
                                     const std::complex<float>* const Y,
                                     const int incY,
    ...
    static std::complex<double> dotc (const int n,
                                      const std::complex<double>* const X,
                                      const int incX,
                                      const std::complex<double>* const Y,
                                      const int incY,
    ...
    static void gemm_cm (const char transa,
                         const char transb,
                         const int m,
                         const int n,
                         const int k,
    ...
    // side=='R': C = a * B * A + b * C.
    // 		A == A^T
    // Because you cannot pack symm or hemm into a row-major kernel by exchanging parameters, so only col-major
    // functions are provided.
    static void symm_cm (const char side,
                         const char uplo,
                         const int m,
                         const int n,
                         const float alpha,
    ...
    // side=='R': C = a * B * A + b * C.
    // 		A == A^H
    static void hemm_cm (const char side,
                         const char uplo,
                         const int m,
                         const int n,
                         const float alpha,
    ...
    // out = ||x||_2
    static float nrm2 (const int n,
                       const float* X,
                       const int,
                       base_device::AbacusDevice_t device_type = base_device::AbacusDevice_t::CpuDevice);

    static double nrm2 (const int n,
                        const double* X,
                        const int incX,
                        base_device::AbacusDevice_t device_type = base_device::AbacusDevice_t::CpuDevice);
    ...
    template <typename T>
    static void vector_mul_vector (const int& dim,
                                   T* result,
                                   const T* vector1,
                                   const T* vector2,
                                   base_device::AbacusDevice_t device_type = base_device::AbacusDevice_t::CpuDevice);
    ...
    template <typename T>
    static void vector_div_vector (const int& dim,
                                   T* result,
                                   const T* vector1,
                                   const T* vector2,
                                   base_device::AbacusDevice_t device_type = base_device::AbacusDevice_t::CpuDevice);
    ...
    // y = alpha * x + beta * y
    static void vector_add_vector (const int& dim,
                                   float* result,
                                   const float* vector1,
...
cublasOperation_t judge_trans (bool is_complex,
                               const char& trans,
                               const char* name); // Translate a normal transpose parameter to a cublas/hipblas type.
