...
void
    BlasConnector::axpy (const int n,
                         const float alpha,
                         const float* X,
                         const int incX,
                         float* Y,
                         ...
                         base_device::AbacusDevice_t device_type)
{
    if (device_type == base_device::AbacusDevice_t::CpuDevice)
        ...
        {
            throw std::invalid_argument ("device_type = " + std::to_string (device_type) + " in "
                         ...
                         base_device::AbacusDevice_t device_type)
{
    if (device_type == base_device::AbacusDevice_t::CpuDevice)
        ...
        {
            CHECK_CUBLAS (
                cublasCaxpy (BlasUtils::cublas_handle, n, (float2*)&alpha, (float2*)X, incX, (float2*)Y, incY));
        }
#endif
    else
        {
                         ...
                         base_device::AbacusDevice_t device_type)
{
    if (device_type == base_device::AbacusDevice_t::CpuDevice)
        ...
        {
            CHECK_CUBLAS (
                cublasZaxpy (BlasUtils::cublas_handle, n, (double2*)&alpha, (double2*)X, incX, (double2*)Y, incY));
        }
#endif
    else
        {
...
// x=a*x
void
    BlasConnector::scal (const int n,
                         const float alpha,
                         float* X,
                         const int incX,
                         base_device::AbacusDevice_t device_type)
...
// d=x*y
float
    BlasConnector::dotu (const int n,
                         const float* const X,
                         const int incX,
                         const float* const Y,
                         const int incY,
    ...
    if (device_type == base_device::AbacusDevice_t::CpuDevice)
        {
            ...
            return std::complex<float> (BlasConnector::dot (n, x, incX2, y, incY2, device_type)
                                            - dot (n, x + 1, incX2, y + 1, incY2, device_type),
                                        BlasConnector::dot (n, x, incX2, y + 1, incY2, device_type)
                                            + dot (n, x + 1, incX2, y, incY2, device_type));
        }
    else
        {
            throw std::invalid_argument ("device_type = " + std::to_string (device_type) + " in "
    ...
    if (device_type == base_device::AbacusDevice_t::CpuDevice)
        {
            ...
            // Re(result)=Re(x)*Re(y)-Im(x)*Im(y)
            // Im(result)=Re(x)*Im(y)+Im(x)*Re(y)
            return std::complex<double> (BlasConnector::dot (n, x, incX2, y, incY2, device_type)
                                             - dot (n, x + 1, incX2, y + 1, incY2, device_type),
                                         BlasConnector::dot (n, x, incX2, y + 1, incY2, device_type)
                                             + dot (n, x + 1, incX2, y, incY2, device_type));
        }
...
// d = x.conj() * Vy
float
    BlasConnector::dotc (const int n,
                         const float* const X,
                         const int incX,
                         const float* const Y,
                         const int incY,
...
// out = ||x||_2
float
    BlasConnector::nrm2 (const int n, const float* X, const int incX, base_device::AbacusDevice_t device_type)
{
    if (device_type == base_device::AbacusDevice_t::CpuDevice)
        {
...
double
    BlasConnector::nrm2 (const int n, const double* X, const int incX, base_device::AbacusDevice_t device_type)
{
    if (device_type == base_device::AbacusDevice_t::CpuDevice)
        {
...
double
    BlasConnector::nrm2 (const int n,
                         const std::complex<double>* X,
                         const int incX,
                         base_device::AbacusDevice_t device_type)
{
...
// copies a into b
void
    BlasConnector::copy (const int n,
                         const float* a,
                         const int incx,
                         float* b,
                         const int incy,
...
template <typename T>
void
    vector_mul_vector (const int& dim,
                       T* result,
                       const T* vector1,
                       const T* vector2,
                       base_device::AbacusDevice_t device_type)
...
template <typename T>
void
    vector_div_vector (const int& dim,
                       T* result,
                       const T* vector1,
                       const T* vector2,
                       base_device::AbacusDevice_t device_type)
...
void
    vector_add_vector (const int& dim,
                       float* result,
                       const float* vector1,
...
#pragma omp parallel for schedule(static)
        ...
        {
            ModuleBase::vector_add_vector_op<float, base_device::DEVICE_GPU> () (dim,
                                                                                 result,
                                                                                 vector1,
                                                                                 constant1,
                                                                                 vector2,
...
#pragma omp parallel for schedule(static)
        ...
        {
            ModuleBase::vector_add_vector_op<double, base_device::DEVICE_GPU> () (dim,
                                                                                  result,
                                                                                  vector1,
                                                                                  constant1,
                                                                                  vector2,
...
#pragma omp parallel for schedule(static)
        ...
        {
            ModuleBase::vector_add_vector_op<std::complex<float>, base_device::DEVICE_GPU> () (dim,
                                                                                               result,
                                                                                               vector1,
                                                                                               constant1,
                                                                                               vector2,
...
#pragma omp parallel for schedule(static)
        ...
        {
            ModuleBase::vector_add_vector_op<std::complex<double>, base_device::DEVICE_GPU> () (dim,
                                                                                                result,
                                                                                                vector1,
                                                                                                constant1,
                                                                                                vector2,
