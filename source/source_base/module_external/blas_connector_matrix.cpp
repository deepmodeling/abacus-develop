...
void
    BlasConnector::set_dsp_cluster_id (int id)
{
    dsp_cluster_id_ = id;
}
#endif
...
// Row-Major part
void
    BlasConnector::gemm (const char transa,
                         const char transb,
                         const int m,
                         const int n,
                         const int k,
                         ...
                         base_device::AbacusDevice_t device_type)
{
    if (device_type == base_device::AbacusDevice_t::CpuDevice)
    ...
    else if (device_type == base_device::AbacusDevice_t::GpuDevice)
        {
            ...
            CHECK_CUBLAS (cublasSgemm (BlasUtils::cublas_handle,
                                       cutransA,
                                       cutransB,
                                       n,
                                       m,
                         ...
                         base_device::AbacusDevice_t device_type)
{
    if (device_type == base_device::AbacusDevice_t::CpuDevice)
    ...
    else if (device_type == base_device::AbacusDevice_t::GpuDevice)
        {
            ...
            cublasOperation_t cutransA = BlasUtils::judge_trans (false, transa, "gemm_op");
            cublasOperation_t cutransB = BlasUtils::judge_trans (false, transb, "gemm_op");
            CHECK_CUBLAS (cublasDgemm (BlasUtils::cublas_handle,
                                       cutransA,
                                       cutransB,
                                       n,
                                       m,
...
// Col-Major part
void
    BlasConnector::gemm_cm (const char transa,
                            const char transb,
                            const int m,
                            const int n,
                            const int k,
...
void
    BlasConnector::symm_cm (const char side,
                            const char uplo,
                            const int m,
                            const int n,
                            const float alpha,
                            ...
                            base_device::AbacusDevice_t device_type)
{
    if (device_type == base_device::AbacusDevice_t::CpuDevice)
    ...
    else if (device_type == base_device::AbacusDevice_t::GpuDevice)
        {
            ...
            CHECK_CUBLAS (cublasSsymm (BlasUtils::cublas_handle,
                                       sideMode,
                                       fillMode,
                                       m,
                                       n,
                            ...
                            base_device::AbacusDevice_t device_type)
{
    if (device_type == base_device::AbacusDevice_t::CpuDevice)
    ...
    else if (device_type == base_device::AbacusDevice_t::GpuDevice)
        {
            cublasSideMode_t sideMode = BlasUtils::judge_side (side);
            cublasFillMode_t fillMode = BlasUtils::judge_fill (uplo);
            CHECK_CUBLAS (cublasDsymm (BlasUtils::cublas_handle,
                                       sideMode,
                                       fillMode,
                                       m,
                                       n,
...
void
    BlasConnector::hemm_cm (const char side,
                            const char uplo,
                            const int m,
                            const int n,
                            const float alpha,
...
void
    BlasConnector::hemm_cm (char side,
                            char uplo,
                            int m,
                            int n,
...
void
    BlasConnector::gemv (const char trans,
                         const int m,
                         const int n,
                         const float alpha,
                         const float* A,
                         ...
                         base_device::AbacusDevice_t device_type)
{
    if (device_type == base_device::AbacusDevice_t::CpuDevice)
    ...
    else if (device_type == base_device::AbacusDevice_t::GpuDevice)
        {
            ...
            CHECK_CUBLAS (
                cublasSgemv (BlasUtils::cublas_handle, cutransA, m, n, &alpha, A, lda, X, incx, &beta, Y, incy));
        }
#endif
    else
        {
                         ...
                         base_device::AbacusDevice_t device_type)
{
    if (device_type == base_device::AbacusDevice_t::CpuDevice)
    ...
    else if (device_type == base_device::AbacusDevice_t::GpuDevice)
        {
            cublasOperation_t cutransA = BlasUtils::judge_trans (false, trans, "gemv_op");
            CHECK_CUBLAS (
                cublasDgemv (BlasUtils::cublas_handle, cutransA, m, n, &alpha, A, lda, X, incx, &beta, Y, incy));
        }
#endif
    else
        {
