...
void BlasConnector::set_dsp_cluster_id(int id) { dsp_cluster_id_ = id; }
#endif
...
void BlasConnector::gemm(const char transa,
    ...
    if (device_type == base_device::AbacusDevice_t::CpuDevice) {
        ...
        CHECK_CUBLAS(
            cublasSgemm(BlasUtils::cublas_handle, cutransA, cutransB, n, m, k, &alpha, b, ldb, a, lda, &beta, c, ldc));
    }
#endif
    else {
...
void BlasConnector::gemm_cm(const char transa,
    ...
    if (device_type == base_device::AbacusDevice_t::CpuDevice) {
        ...
        cublasOperation_t cutransB = BlasUtils::judge_trans(false, transb, "gemm_op");
        CHECK_CUBLAS(
            cublasSgemm(BlasUtils::cublas_handle, cutransA, cutransB, m, n, k, &alpha, a, lda, b, ldb, &beta, c, ldc));
    }
#endif
    else {
        throw std::invalid_argument("device_type = " + std::to_string(device_type) + " in " + std::string(__FILE__) +
...
void BlasConnector::symm_cm(const char side,
    ...
    if (device_type == base_device::AbacusDevice_t::CpuDevice) {
        ...
        CHECK_CUBLAS(
            cublasSsymm(BlasUtils::cublas_handle, sideMode, fillMode, m, n, &alpha, a, lda, b, ldb, &beta, c, ldc));
    }
#endif
    else {
        throw std::invalid_argument("device_type = " + std::to_string(device_type) + " in " + std::string(__FILE__) +
...
void BlasConnector::symm_cm(const char side,
    ...
    if (device_type == base_device::AbacusDevice_t::CpuDevice) {
        ...
        cublasFillMode_t fillMode = BlasUtils::judge_fill(uplo);
        CHECK_CUBLAS(
            cublasDsymm(BlasUtils::cublas_handle, sideMode, fillMode, m, n, &alpha, a, lda, b, ldb, &beta, c, ldc));
    }
#endif
    else {
        throw std::invalid_argument("device_type = " + std::to_string(device_type) + " in " + std::string(__FILE__) +
...
void BlasConnector::symm_cm(const char side,
    ...
    if (device_type == base_device::AbacusDevice_t::CpuDevice) {
        ...
        cublasSideMode_t sideMode = BlasUtils::judge_side(side);
        cublasFillMode_t fillMode = BlasUtils::judge_fill(uplo);
        CHECK_CUBLAS(cublasCsymm(BlasUtils::cublas_handle,
                                 sideMode,
                                 fillMode,
                                 m,
                                 n,
...
void BlasConnector::symm_cm(const char side,
    ...
    if (device_type == base_device::AbacusDevice_t::CpuDevice) {
        ...
        cublasSideMode_t sideMode = BlasUtils::judge_side(side);
        cublasFillMode_t fillMode = BlasUtils::judge_fill(uplo);
        CHECK_CUBLAS(cublasZsymm(BlasUtils::cublas_handle,
                                 sideMode,
                                 fillMode,
                                 m,
                                 n,
...
void BlasConnector::hemm_cm(char side,
    ...
    if (device_type == base_device::AbacusDevice_t::CpuDevice) {
        ...
        cublasSideMode_t sideMode = BlasUtils::judge_side(side);
        cublasFillMode_t fillMode = BlasUtils::judge_fill(uplo);
        CHECK_CUBLAS(cublasChemm(BlasUtils::cublas_handle,
                                 sideMode,
                                 fillMode,
                                 m,
                                 n,
...
void BlasConnector::hemm_cm(char side,
    ...
    if (device_type == base_device::AbacusDevice_t::CpuDevice) {
        ...
        cublasSideMode_t sideMode = BlasUtils::judge_side(side);
        cublasFillMode_t fillMode = BlasUtils::judge_fill(uplo);
        CHECK_CUBLAS(cublasZhemm(BlasUtils::cublas_handle,
                                 sideMode,
                                 fillMode,
                                 m,
                                 n,
...
void BlasConnector::gemv(const char trans,
    ...
    if (device_type == base_device::AbacusDevice_t::CpuDevice) {
        ...
        mtfunc::sgemv_mth_(&trans, &m, &n, &alpha, A, &lda, X, &incx, &beta, Y, &incy, BlasConnector::dsp_cluster_id_);
    }
#endif
#ifdef __CUDA
    else if (device_type == base_device::AbacusDevice_t::GpuDevice) {
...
void BlasConnector::gemv(const char trans,
    ...
    if (device_type == base_device::AbacusDevice_t::CpuDevice) {
        ...
        mtfunc::dgemv_mth_(&trans, &m, &n, &alpha, A, &lda, X, &incx, &beta, Y, &incy, BlasConnector::dsp_cluster_id_);
    }
#endif
#ifdef __CUDA
    else if (device_type == base_device::AbacusDevice_t::GpuDevice) {
...
void BlasConnector::gemv(const char trans,
    ...
    if (device_type == base_device::AbacusDevice_t::CpuDevice) {
        ...
        mtfunc::cgemv_mth_(&trans, &m, &n, &alpha, A, &lda, X, &incx, &beta, Y, &incy, BlasConnector::dsp_cluster_id_);
    }
#endif
#ifdef __CUDA
    else if (device_type == base_device::AbacusDevice_t::GpuDevice) {
        ...
        CHECK_CUBLAS(cublasCgemv(BlasUtils::cublas_handle,
                                 cutransA,
                                 m,
                                 n,
                                 &alpha_cu,
...
void BlasConnector::gemv(const char trans,
    ...
    if (device_type == base_device::AbacusDevice_t::CpuDevice) {
        ...
        mtfunc::zgemv_mth_(&trans, &m, &n, &alpha, A, &lda, X, &incx, &beta, Y, &incy, BlasConnector::dsp_cluster_id_);
    }
#endif
#ifdef __CUDA
    else if (device_type == base_device::AbacusDevice_t::GpuDevice) {
        ...
        cublasOperation_t cutransA = BlasUtils::judge_trans(true, trans, "gemv_op");
        CHECK_CUBLAS(cublasZgemv(BlasUtils::cublas_handle,
                                 cutransA,
                                 m,
                                 n,
                                 &alpha_cu,
