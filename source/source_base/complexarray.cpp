...
void
    ComplexArray::init (const int size)
{
    assert (size >= 0);
    if (size > 0)
        {
...
void
    ComplexArray::create (const int bnd1, const int bnd2, const int bnd3, const int bnd4)
{
    delete[] ptr;
    bound1 = bnd1;
    bound2 = bnd2;
...
ComplexArray
    operator* (const double r, const ComplexArray& cd)
{
    ComplexArray cd2 (cd);
    const int size = cd.getSize ();
    for (int i = 0; i < size; i++)
...
ComplexArray
    operator* (const std::complex<double> c, const ComplexArray& cd)
{
    const int size = cd.getSize ();
...
bool
    ComplexArray::operator== (const ComplexArray& cd2) const
{
    ...
    for (int i = 0; i < size; i++)
        {
            dot12 += std::complex<double> (
                cd1.ptr[i].real () * cd2.ptr[i].real () + cd1.ptr[i].imag () * cd2.ptr[i].imag (),
...
void
    scale_accumulate (const std::complex<double> c, const ComplexArray& cd1, ComplexArray& cd2)
{
    assert (cd1.getSize () == cd2.getSize ());
    const int size = cd1.getSize ();
    for (int i = 0; i < size; i++)
...
void
    scaled_sum (double r1, const ComplexArray& cd1, double r2, const ComplexArray& cd2, ComplexArray& cd3)
{
    assert (cd1.getSize () == cd2.getSize ());
...
void
    scaled_sum (std::complex<double> c1,
    ...
    for (int i = 0; i < size; i++)
        {
            out.ptr[i] = std::complex<double> (
                in1.ptr[i].real () * in2.ptr[i].real () - in1.ptr[i].imag () * in2.ptr[i].imag (),
