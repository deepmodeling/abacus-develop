...
std::complex<double> ErrorFunc::erf(std::complex<double> z, double relerr) {
    ...
    */
    ...
    if (x >= 0) {
        ...
        return 1.0 - std::exp(mRe_z2) * (std::complex<double>(std::cos(mIm_z2), std::sin(mIm_z2)) *
        ...
        /* don't use complex exp function, since that will produce spurious NaN
           values when multiplying w in an overflow situation. */
        return std::exp(mRe_z2) * (std::complex<double>(std::cos(mIm_z2), std::sin(mIm_z2)) *
                                   scaled_w(std::complex<double>(y, -x), relerr)) -
               1.0;
    }
}
...
// erfi(x) = -i erf(ix)
double ErrorFunc::erfi(double x) { return x * x > 720 ? (x > 0 ? Inf : -Inf) : std::exp(x * x) * scaled_w_im(x); }
