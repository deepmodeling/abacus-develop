...
std::complex<double>
    ErrorFunc::scaled_w (std::complex<double> z, double relerr)
{
    if (std::real (z) == 0.0)
        {
                    ...
                    if (x + ya > 1e7)
                        { // nu == 1, w(z) = i/sqrt(pi) / z
                            ...
                            if (x > ya)
                                ...
                                {
                                    return ((std::isnan (x) || y < 0) ? std::complex<double> (NaN, NaN)
    ...
    return ret
           + std::complex<double> ((0.5 * c) * y * (sum2 + sum3), (0.5 * c) * copysign (sum5 - sum4, std::real (z)));
    ;
}

double
    ErrorFunc::scaled_w_im (double x)
{
    if (x >= 0)
        {
            if (x > 45)
...
std::complex<double>
    ErrorFunc::erf (std::complex<double> z, double relerr)
{
    ...
    if (x == 0)
        {                                   // handle separately for speed & handling of y = Inf or NaN
    ...
    auto taylor_erfi = [&] () -> std::complex<double>
        {
            ...
            return std::complex<double> (
                ...
                expy2
                    * (scaled_w_im (y)
                       - x2 * y
                             * (1.1283791670955125739 - x2 * (0.56418958354775628695 + 0.37612638903183752464 * y2))));
        };
...
// erfi(z) = -i erf(iz)
std::complex<double>
    ErrorFunc::erfi (std::complex<double> z, double relerr)
{
...
double
    ErrorFunc::erfcx (double x) // exp(z^2) erfc(z)
{
    return std::exp (x * x) * std::erfc (x);
}

std::complex<double>
    ErrorFunc::erfcx (std::complex<double> z, double relerr) // exp(z^2) erfc(z)
{
...
// erfc(z) = 1 - erf(z)
std::complex<double>
    ErrorFunc::erfc (std::complex<double> z, double relerr)
{
    double x = std::real (z), y = std::imag (z);
    ...
    if (x >= 0)
        ...
        {
            return 2.0
                   - std::exp (std::complex<double> (mRe_z2, mIm_z2)) * scaled_w (std::complex<double> (y, -x), relerr);
        }
}

double
    ErrorFunc::w_im_y100 (double y100, double x)
{
    switch ((int)y100)
        {
        case 0:
