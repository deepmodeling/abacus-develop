...
class ComplexMatrix
{
  ...
  public:
    ...
    //============
    std::complex<double>&
        operator() (const int ir, const int ic)
    {
        assert (ir >= 0);
        assert (ir < nr);
        assert (ic >= 0);
    ...
    const std::complex<double>&
        operator() (const int ir, const int ic) const
    {
        assert (ir >= 0);
        assert (ir < nr);
        assert (ic >= 0);
    ...
    void set_as_identity_matrix ();

    std::ostream& print (std::ostream& os,
                         const double threshold_abs = 0.0,
                         const double threshold_imag = 0.0) const; // Peize Lin add 2021.09.08

    // check if all the elements are real
    bool checkreal () const;
