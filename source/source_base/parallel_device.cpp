...
class NcclCommRegistry
{
  public:
        ...
        get (MPI_Comm comm)
    {
        ...
        std::pair<std::map<MPI_Fint, NcclCommContext>::iterator, bool> inserted
            = contexts_.insert (std::make_pair (key, ctx));
...
template <typename T>
void
    nccl_bcast_impl (T* object,
                     const int n,
                     MPI_Comm& comm,
                     ncclDataType_t datatype,
                     int root = 0,
                     const int count_scale = 1)
{
    ...
    CHECK_NCCL (
        ncclBroadcast (object, object, static_cast<size_t> (n) * count_scale, datatype, root, ctx.comm, ctx.stream));
    CHECK_CUDA (cudaStreamSynchronize (ctx.stream));
}

template <typename T>
void
    nccl_reduce_impl (T* object, const int n, MPI_Comm& comm, ncclDataType_t datatype, const int count_scale = 1)
{
    NcclCommContext& ctx = get_nccl_registry ().get (comm);
    if (ctx.size <= 1 || n <= 0)
        {
    ...
    CHECK_NCCL (
        ncclAllReduce (object, object, static_cast<size_t> (n) * count_scale, datatype, ncclSum, ctx.comm, ctx.stream));
    CHECK_CUDA (cudaStreamSynchronize (ctx.stream));
}

template <typename T>
void
    nccl_gatherv_impl (const T* sendbuf,
                       const int sendcount,
                       T* recvbuf,
...
void
    nccl_bcast_data (double* object, const int& n, MPI_Comm& comm, int root)
{
    nccl_bcast_impl (object, n, comm, ncclDouble, root);
}

void
    nccl_bcast_data (std::complex<double>* object, const int& n, MPI_Comm& comm, int root)
{
    nccl_bcast_impl (reinterpret_cast<double*> (object), n, comm, ncclDouble, root, 2);
}

void
    nccl_bcast_data (float* object, const int& n, MPI_Comm& comm, int root)
{
    nccl_bcast_impl (object, n, comm, ncclFloat, root);
}

void
    nccl_bcast_data (std::complex<float>* object, const int& n, MPI_Comm& comm, int root)
{
    nccl_bcast_impl (reinterpret_cast<float*> (object), n, comm, ncclFloat, root, 2);
}

void
    nccl_reduce_data (double* object, const int& n, MPI_Comm& comm)
{
    nccl_reduce_impl (object, n, comm, ncclDouble);
}

void
    nccl_reduce_data (std::complex<double>* object, const int& n, MPI_Comm& comm)
{
    nccl_reduce_impl (reinterpret_cast<double*> (object), n, comm, ncclDouble, 2);
}

void
    nccl_reduce_data (float* object, const int& n, MPI_Comm& comm)
{
    nccl_reduce_impl (object, n, comm, ncclFloat);
}

void
    nccl_reduce_data (std::complex<float>* object, const int& n, MPI_Comm& comm)
{
    nccl_reduce_impl (reinterpret_cast<float*> (object), n, comm, ncclFloat, 2);
}

void
    nccl_gatherv_data (const double* sendbuf,
                       int sendcount,
                       double* recvbuf,
                       const int* recvcounts,
                       const int* displs,
...
void
    nccl_gatherv_data (const std::complex<double>* sendbuf,
                       int sendcount,
...
void
    nccl_gatherv_data (const float* sendbuf,
                       int sendcount,
                       float* recvbuf,
                       const int* recvcounts,
                       const int* displs,
...
void
    nccl_gatherv_data (const std::complex<float>* sendbuf,
                       int sendcount,
...
void
    isend_data (const double* buf, int count, int dest, int tag, MPI_Comm& comm, MPI_Request* request)
{
    MPI_Isend (buf, count, MPI_DOUBLE, dest, tag, comm, request);
}
void
    isend_data (const std::complex<double>* buf, int count, int dest, int tag, MPI_Comm& comm, MPI_Request* request)
{
    MPI_Isend (buf, count, MPI_DOUBLE_COMPLEX, dest, tag, comm, request);
}
void
    isend_data (const float* buf, int count, int dest, int tag, MPI_Comm& comm, MPI_Request* request)
{
    MPI_Isend (buf, count, MPI_FLOAT, dest, tag, comm, request);
}
void
    isend_data (const std::complex<float>* buf, int count, int dest, int tag, MPI_Comm& comm, MPI_Request* request)
{
    MPI_Isend (buf, count, MPI_COMPLEX, dest, tag, comm, request);
}
void
    send_data (const double* buf, int count, int dest, int tag, MPI_Comm& comm)
{
    MPI_Send (buf, count, MPI_DOUBLE, dest, tag, comm);
}
void
    send_data (const std::complex<double>* buf, int count, int dest, int tag, MPI_Comm& comm)
{
    MPI_Send (buf, count, MPI_DOUBLE_COMPLEX, dest, tag, comm);
}
void
    send_data (const float* buf, int count, int dest, int tag, MPI_Comm& comm)
{
    MPI_Send (buf, count, MPI_FLOAT, dest, tag, comm);
}
void
    send_data (const std::complex<float>* buf, int count, int dest, int tag, MPI_Comm& comm)
{
    MPI_Send (buf, count, MPI_COMPLEX, dest, tag, comm);
}
void
    recv_data (double* buf, int count, int source, int tag, MPI_Comm& comm, MPI_Status* status)
{
    MPI_Recv (buf, count, MPI_DOUBLE, source, tag, comm, status);
}
void
    recv_data (std::complex<double>* buf, int count, int source, int tag, MPI_Comm& comm, MPI_Status* status)
{
    MPI_Recv (buf, count, MPI_DOUBLE_COMPLEX, source, tag, comm, status);
}
void
    recv_data (float* buf, int count, int source, int tag, MPI_Comm& comm, MPI_Status* status)
{
    MPI_Recv (buf, count, MPI_FLOAT, source, tag, comm, status);
}
void
    recv_data (std::complex<float>* buf, int count, int source, int tag, MPI_Comm& comm, MPI_Status* status)
{
    MPI_Recv (buf, count, MPI_COMPLEX, source, tag, comm, status);
}
void
    bcast_data (std::complex<double>* object, const int& n, const MPI_Comm& comm, int root)
{
    MPI_Bcast (object, n * 2, MPI_DOUBLE, root, comm);
}
void
    bcast_data (std::complex<float>* object, const int& n, const MPI_Comm& comm, int root)
{
    MPI_Bcast (object, n * 2, MPI_FLOAT, root, comm);
}
void
    bcast_data (double* object, const int& n, const MPI_Comm& comm, int root)
{
    MPI_Bcast (object, n, MPI_DOUBLE, root, comm);
}
void
    bcast_data (float* object, const int& n, const MPI_Comm& comm, int root)
{
    MPI_Bcast (object, n, MPI_FLOAT, root, comm);
}
void
    reduce_data (std::complex<double>* object, const int& n, const MPI_Comm& comm)
{
    MPI_Allreduce (MPI_IN_PLACE, object, n * 2, MPI_DOUBLE, MPI_SUM, comm);
}
void
    reduce_data (std::complex<float>* object, const int& n, const MPI_Comm& comm)
{
    MPI_Allreduce (MPI_IN_PLACE, object, n * 2, MPI_FLOAT, MPI_SUM, comm);
}
void
    reduce_data (double* object, const int& n, const MPI_Comm& comm)
{
    MPI_Allreduce (MPI_IN_PLACE, object, n, MPI_DOUBLE, MPI_SUM, comm);
}
void
    reduce_data (float* object, const int& n, const MPI_Comm& comm)
{
    MPI_Allreduce (MPI_IN_PLACE, object, n, MPI_FLOAT, MPI_SUM, comm);
}
void
    gatherv_data (const double* sendbuf,
                  int sendcount,
                  double* recvbuf,
                  const int* recvcounts,
                  const int* displs,
...
void
    gatherv_data (const std::complex<double>* sendbuf,
                  int sendcount,
                  std::complex<double>* recvbuf,
                  const int* recvcounts,
                  const int* displs,
...
void
    gatherv_data (const float* sendbuf,
                  int sendcount,
                  float* recvbuf,
                  const int* recvcounts,
                  const int* displs,
...
void
    gatherv_data (const std::complex<float>* sendbuf,
                  int sendcount,
                  std::complex<float>* recvbuf,
                  const int* recvcounts,
                  const int* displs,
...
struct object_cpu_point<T, base_device::DEVICE_GPU>
{
    bool alloc = false;
    T*
        get_buffer (const T* object, const int& n, T* tmp_space = nullptr)
    {
    ...
    T*
        get (const T* object, const int& n, T* tmp_space = nullptr)
    {
    ...
    void
        sync_h2d (T* object, const T* object_cpu, const int& n)
    {
    ...
    void
        sync_d2h (T* object_cpu, const T* object, const int& n)
    {
        base_device::memory::synchronize_memory_op<T, base_device::DEVICE_CPU, base_device::DEVICE_GPU> () (object_cpu,
                                                                                                            object,
                                                                                                            n);
