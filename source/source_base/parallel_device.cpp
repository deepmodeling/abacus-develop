...
void nccl_gatherv_impl(const T* sendbuf,
    ...
    if (ctx.size <= 1) {
        if (sendbuf != recvbuf && sendcount > 0) {
            CHECK_CUDA(
                cudaMemcpy(recvbuf, sendbuf, static_cast<size_t>(sendcount) * sizeof(T), cudaMemcpyDeviceToDevice));
        }
        return;
    }
...
void nccl_reduce_data(double* object, const int& n, MPI_Comm& comm) { nccl_reduce_impl(object, n, comm, ncclDouble); }
...
void nccl_reduce_data(float* object, const int& n, MPI_Comm& comm) { nccl_reduce_impl(object, n, comm, ncclFloat); }
...
void nccl_gatherv_data(const double* sendbuf,
                       int sendcount,
                       double* recvbuf,
                       const int* recvcounts,
                       const int* displs,
...
void nccl_gatherv_data(const float* sendbuf,
                       int sendcount,
                       float* recvbuf,
                       const int* recvcounts,
                       const int* displs,
...
void gatherv_data(const double* sendbuf,
                  int sendcount,
                  double* recvbuf,
                  const int* recvcounts,
                  const int* displs,
...
void gatherv_data(const std::complex<double>* sendbuf,
                  int sendcount,
                  std::complex<double>* recvbuf,
                  const int* recvcounts,
                  const int* displs,
...
void gatherv_data(const float* sendbuf,
                  int sendcount,
                  float* recvbuf,
                  const int* recvcounts,
                  const int* displs,
...
void gatherv_data(const std::complex<float>* sendbuf,
                  int sendcount,
                  std::complex<float>* recvbuf,
                  const int* recvcounts,
                  const int* displs,
...
struct object_cpu_point<T, base_device::DEVICE_CPU> {
    bool alloc = false;
    T* get_buffer(const T* object, const int& n, T* tmp_space = nullptr) { return const_cast<T*>(object); }
    T* get(const T* object, const int& n, T* tmp_space = nullptr) { return const_cast<T*>(object); }
    void sync_h2d(T* object, const T* object_cpu, const int& n) {}
    void sync_d2h(T* object_cpu, const T* object, const int& n) {}
    void del(T* object_cpu) {}
};
