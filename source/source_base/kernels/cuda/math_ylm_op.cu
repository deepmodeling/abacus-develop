                  ...
                  FPTYPE* ylm)
{
    ...
    //----------------------------------------------------------
    const FPTYPE gmod
        = sqrt (g[ig * 3 + 0] * g[ig * 3 + 0] + g[ig * 3 + 1] * g[ig * 3 + 1] + g[ig * 3 + 2] * g[ig * 3 + 2]);
    ...
    for (int l = 0; l <= lmax; l++)
        {
            ...
            if (l == 0)
                ...
                {
                    const int l1 = l - 1, l2 = l - 2, l3 = 2 * l - 1;
                    //  recursion on l for P(:,l,m)
                    for (int m = 0; m <= l2; m++)
                        { // do m = 0, l - 2//mohan modify 2007-10-13
                            p[m * (lmax + 1) * ng + l * ng + ig] = (cost * l3 * p[m * (lmax + 1) * ng + l1 * ng + ig]
                                                                    - (l1 + m) * p[m * (lmax + 1) * ng + l2 * ng + ig])
                                                                   / (l - m);
                        } // end do
                    p[l1 * (lmax + 1) * ng + l * ng + ig] = cost * l3 * p[l1 * (lmax + 1) * ng + l1 * ng + ig];
                    FPTYPE x2 = (1.0 - cost * cost) > 0.0 ? (1.0 - cost * cost) : 0.0;
                    p[l * (lmax + 1) * ng + l * ng + ig]
                        = __semi_fact (l3) * pow (x2, static_cast<FPTYPE> (l) / 2.0); // mohan modify 2007-10-13
                    if (l % 2 == 1)
                        {
            ...
            for (int m = 1; m <= l; m++)
                {
                    // Y_lm, m > 0
                    const FPTYPE same = c * sqrt (__fact<FPTYPE> (l - m) / __fact<FPTYPE> (l + m)) * SQRT2;
...
template <typename FPTYPE>
void
    cal_ylm_real_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* ctx,
                                                                  const int& ng,
                                                                  const int& lmax,
                                                                  const FPTYPE& SQRT2,
                                                                  const FPTYPE& PI,
                                                                  ...
                                                                  FPTYPE* ylm)
{
    ...
    cal_ylm_real<FPTYPE>
        <<<block, THREADS_PER_BLOCK>>> (ng, lmax, SQRT2, PI, PI_HALF, FOUR_PI, SQRT_INVERSE_FOUR_PI, g, p, ylm);
