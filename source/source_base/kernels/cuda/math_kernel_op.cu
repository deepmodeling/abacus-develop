...
template <typename T>
__global__ void
    matrix_transpose_kernel (const int row, const int col, const T* in, T* out)
{
...
template <typename T>
__global__ void
    matrix_copy_kernel (const int n1, const int n2, const T* A, const int LDA, T* B, const int LDB)
{
...
__global__ void
    matrix_multiply_vector_kernel (const int m,
                                   const int n,
                                   T* a,
                                   const int lda,
                                   const Real* b,
...
template <>
void
    gemv_op<float, base_device::DEVICE_GPU>::operator() (const char& trans,
                                                         const int& m,
                                                         const int& n,
                                                         const float* alpha,
                                                         const float* A,
                                                                       ...
                                                                       const int& incy)
{
    ...
    CHECK_CUBLAS (cublasCgemv (cublas_handle,
                               cutrans,
                               m,
                               n,
                               &alpha,
                                                                       ...
                                                                       const int& ldc)
{
    ...
    CHECK_CUBLAS (cublasCgemm (cublas_handle,
                               cutransA,
                               cutransB,
                               m,
                               n,
    ...
    if (row == col)
        {
            ...
            // use 'geam' API todo transpose.
            CHECK_CUBLAS (cublasCgeam (cublas_handle,
                                       CUBLAS_OP_T,
                                       CUBLAS_OP_N,
                                       col,
                                       row,
                                       reinterpret_cast<const float2*> (&ONE),
                                       (float2*)input_matrix,
                                       col,
                                       reinterpret_cast<const float2*> (&ZERO),
                                       (float2*)input_matrix,
                                       col,
                                       (float2*)device_temp,
                                       col));
    ...
    else
        {
            int thread = 1024;
            int block = (row + col + thread - 1) / thread;
            matrix_transpose_kernel<thrust::complex<float>><<<block, thread>>> (row,
                                                                                col,
    ...
    if (row == col)
        {
            ...
            // use 'geam' API todo transpose.
            CHECK_CUBLAS (cublasZgeam (cublas_handle,
                                       CUBLAS_OP_T,
                                       CUBLAS_OP_N,
                                       col,
                                       row,
                                       &ONE,
                                       (double2*)input_matrix,
                                       col,
                                       &ZERO,
                                       (double2*)input_matrix,
                                       col,
    ...
    else
        {
            int thread = 1024;
            int block = (row + col + thread - 1) / thread;
            matrix_transpose_kernel<thrust::complex<double>><<<block, thread>>> (row,
                                                                                 col,
                                                                          ...
                                                                          const int& LDB)
{
    const dim3 blockSize (16, 16);
    const dim3 gridSize ((n1 + blockSize.x - 1) / blockSize.x, (n2 + blockSize.y - 1) / blockSize.y);
    matrix_copy_kernel<thrust::complex<float>>
        <<<gridSize, blockSize>>> (n1,
                                   n2,
                                                                           ...
                                                                           const int& LDB)
{
    const dim3 blockSize (16, 16);
    const dim3 gridSize ((n1 + blockSize.x - 1) / blockSize.x, (n2 + blockSize.y - 1) / blockSize.y);
    matrix_copy_kernel<thrust::complex<double>>
        <<<gridSize, blockSize>>> (n1,
                                   n2,
                                                                       ...
                                                                       const int& ldc)
{
    ...
    matrix_multiply_vector_kernel<double, double><<<block, thread>>> (m, n, a, lda, b, alpha, c, ldc);
    CHECK_CUDA_SYNC ();
}
                                                                                    ...
                                                                                    const int& ldc)
{
    dim3 thread (16, 16, 1);
    dim3 block ((m + thread.x - 1) / thread.x, (n + thread.y - 1) / thread.y, 1);
    matrix_multiply_vector_kernel<thrust::complex<float>, float>
        <<<block, thread>>> (m,
                             n,
                                                                                     ...
                                                                                     const int& ldc)
{
    dim3 thread (16, 16, 1);
    dim3 block ((m + thread.x - 1) / thread.x, (n + thread.y - 1) / thread.y, 1);
    matrix_multiply_vector_kernel<thrust::complex<double>, double>
        <<<block, thread>>> (m,
                             n,
