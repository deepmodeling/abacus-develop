...
template <typename T>
__global__ void matrix_transpose_kernel(const int row, const int col, const T* in, T* out) {
...
__global__ void matrix_multiply_vector_kernel(const int m,
                                              const int n,
                                              T* a,
                                              const int lda,
                                              const Real* b,
...
void matrixTranspose_op<std::complex<float>, base_device::DEVICE_GPU>::operator()(
    ...
    if (row == col) {
        ...
        int thread = 1024;
        int block = (row + col + thread - 1) / thread;
        matrix_transpose_kernel<thrust::complex<float>>
            <<<block, thread>>>(row, col, (thrust::complex<float>*)input_matrix, (thrust::complex<float>*)device_temp);
