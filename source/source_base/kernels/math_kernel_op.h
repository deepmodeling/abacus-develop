...
//-----------------------------0. Tool Functions-----------------------------------
//---------------------------------------------------------------------------------
inline std::complex<double> set_real_tocomplex(const std::complex<double>& x) { return {x.real(), 0.0}; }

inline std::complex<float> set_real_tocomplex(const std::complex<float>& x) { return {x.real(), 0.0}; }

inline double set_real_tocomplex(const double& x) { return x; }

inline float set_real_tocomplex(const float& x) { return x; }

inline std::complex<double> get_conj(const std::complex<double>& x) { return {x.real(), -x.imag()}; }

inline std::complex<float> get_conj(const std::complex<float>& x) { return {x.real(), -x.imag()}; }
...
struct matrix_mul_vector_op {
    ...
    void
    operator()(const int& m, const int& n, T* a, const int& lda, const Real* b, const Real alpha, T* c, const int& ldc);
};
...
template <typename T>
struct matrixCopy<T, base_device::DEVICE_GPU> {
...
template <typename T>
struct matrix_mul_vector_op<T, base_device::DEVICE_GPU> {
    using Real = typename GetTypeReal<T>::type;
    void
    operator()(const int& m, const int& n, T* a, const int& lda, const Real* b, const Real alpha, T* c, const int& ldc);
};
