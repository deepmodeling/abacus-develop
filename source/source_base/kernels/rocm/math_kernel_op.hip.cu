...
void
    xdot_wrapper (const int& n, const float* x, const int& incx, const float* y, const int& incy, float& result)
{
    hipblasErrcheck (hipblasSdot (cublas_handle, n, x, incx, y, incy, &result));
}

void
    xdot_wrapper (const int& n, const double* x, const int& incx, const double* y, const int& incy, double& result)
{
...
template <>
void
    scal_op<float, base_device::DEVICE_GPU>::operator() (const int& N,
...
template <>
void
    scal_op<double, base_device::DEVICE_GPU>::operator() (const int& N,
...
template <>
void
    axpy_op<double, base_device::DEVICE_GPU>::operator() (const int& N,
                                                          const double* alpha,
                                                          const double* X,
                                                          const int& incX,
                                                          double* Y,
...
template <>
void
    axpy_op<std::complex<float>, base_device::DEVICE_GPU>::operator() (const int& N,
                                                                       const std::complex<float>* alpha,
...
template <>
void
    axpy_op<std::complex<double>, base_device::DEVICE_GPU>::operator() (const int& N,
                                                                        const std::complex<double>* alpha,
...
template <typename T>
__launch_bounds__ (1024) __global__ void matrix_transpose_kernel (const int row, const int col, const T* in, T* out)
{
...
__launch_bounds__ (1024) __global__ void matrix_multiply_vector_kernel (const int m,
                                                                        const int n,
                                                                        T* a,
                                                                        const int lda,
                                                                        const Real* b,
...
hipblasOperation_t
    judge_trans_op (bool is_complex, const char& trans, const char* name)
{
    if (trans == 'N')
        {
            return HIPBLAS_OP_N;
...
template <>
void
    gemv_op<double, base_device::DEVICE_GPU>::operator() (const char& trans,
                                                          const int& m,
                                                          const int& n,
                                                          const double* alpha,
                                                          const double* A,
...
template <>
void
    gemv_op<std::complex<float>, base_device::DEVICE_GPU>::operator() (const char& trans,
                                                                       const int& m,
                                                                       const int& n,
                                                                       const std::complex<float>* alpha,
...
template <>
void
    gemv_op<std::complex<double>, base_device::DEVICE_GPU>::operator() (const char& trans,
                                                                        const int& m,
                                                                        const int& n,
                                                                        const std::complex<double>* alpha,
...
template <>
void
    gemm_op<float, base_device::DEVICE_GPU>::operator() (const char& transa,
                                                         const char& transb,
                                                         const int& m,
                                                         const int& n,
                                                         const int& k,
...
template <>
void
    gemm_op<double, base_device::DEVICE_GPU>::operator() (const char& transa,
                                                          const char& transb,
                                                          const int& m,
                                                          const int& n,
                                                          const int& k,
...
template <>
void
    gemm_op<std::complex<float>, base_device::DEVICE_GPU>::operator() (const char& transa,
                                                                       const char& transb,
                                                                       const int& m,
                                                                       const int& n,
                                                                       const int& k,
...
template <>
void
    gemm_op<std::complex<double>, base_device::DEVICE_GPU>::operator() (const char& transa,
                                                                        const char& transb,
                                                                        const int& m,
                                                                        const int& n,
                                                                        const int& k,
...
template <>
void
    matrixTranspose_op<double, base_device::DEVICE_GPU>::operator() (const int& row,
                                                                     const int& col,
    ...
    if (row == col)
        {
            ...
            hipblasErrcheck (hipblasDgeam (cublas_handle,
                                           HIPBLAS_OP_T,
                                           HIPBLAS_OP_N,
                                           col,
                                           row,
    ...
    else
        {
            ...
            hipLaunchKernelGGL (HIP_KERNEL_NAME (matrix_transpose_kernel<double>),
                                dim3 (block),
                                dim3 (thread),
                                0,
                                0,
...
template <>
void
    matrixTranspose_op<std::complex<float>, base_device::DEVICE_GPU>::operator() (
        const int& row,
        const int& col,
    ...
    if (row == col)
        {
            ...
            // use 'geam' API todo transpose.
            hipblasErrcheck (hipblasCgeam (cublas_handle,
                                           HIPBLAS_OP_T,
                                           HIPBLAS_OP_N,
                                           col,
                                           row,
                                           reinterpret_cast<const hipblasComplex*> (&ONE),
                                           (hipblasComplex*)input_matrix,
                                           col,
                                           reinterpret_cast<const hipblasComplex*> (&ZERO),
                                           (hipblasComplex*)input_matrix,
                                           col,
                                           (hipblasComplex*)device_temp,
                                           col));
        }
    else
        {
            int thread = 1024;
            int block = (row + col + thread - 1) / thread;
            hipLaunchKernelGGL (HIP_KERNEL_NAME (matrix_transpose_kernel<thrust::complex<float>>),
                                dim3 (block),
                                dim3 (thread),
                                0,
                                0,
                                row,
                                col,
                                (thrust::complex<float>*)input_matrix,
                                (thrust::complex<float>*)device_temp);
            hipCheckOnDebug ();
        }
...
template <>
void
    matrixTranspose_op<std::complex<double>, base_device::DEVICE_GPU>::operator() (
        const int& row,
        const int& col,
    ...
    if (row == col)
        {
            ...
            // use 'geam' API todo transpose.
            hipblasErrcheck (hipblasZgeam (cublas_handle,
                                           HIPBLAS_OP_T,
                                           HIPBLAS_OP_N,
                                           col,
                                           row,
                                           &ONE,
                                           (hipblasDoubleComplex*)input_matrix,
                                           col,
                                           &ZERO,
                                           (hipblasDoubleComplex*)input_matrix,
                                           col,
                                           (hipblasDoubleComplex*)device_temp,
                                           col));
        }
    else
        {
            int thread = 1024;
            int block = (row + col + thread - 1) / thread;
            hipLaunchKernelGGL (HIP_KERNEL_NAME (matrix_transpose_kernel<thrust::complex<double>>),
                                dim3 (block),
                                dim3 (thread),
                                0,
                                0,
                                row,
                                col,
                                (thrust::complex<double>*)input_matrix,
                                (thrust::complex<double>*)device_temp);
            hipCheckOnDebug ();
        }
...
template <>
void
    matrixCopy<double, base_device::DEVICE_GPU>::operator() (const int& n1,
                                                             const int& n2,
                                                             const double* A,
                                                             const int& LDA,
                                                             double* B,
                                                             const int& LDB)
{
    ...
    hipLaunchKernelGGL (HIP_KERNEL_NAME (matrix_copy_kernel<double>),
                        gridSize,
                        blockSize,
                        0,
                        0,
...
template <>
void
    matrixCopy<std::complex<float>, base_device::DEVICE_GPU>::operator() (const int& n1,
                                                                          const int& n2,
                                                                          const std::complex<float>* A,
                                                                          const int& LDA,
                                                                          std::complex<float>* B,
                                                                          const int& LDB)
{
    ...
    hipLaunchKernelGGL (HIP_KERNEL_NAME (matrix_copy_kernel<thrust::complex<float>>),
                        gridSize,
                        blockSize,
                        0,
                        0,
...
template <>
void
    matrixCopy<std::complex<double>, base_device::DEVICE_GPU>::operator() (const int& n1,
                                                                           const int& n2,
                                                                           const std::complex<double>* A,
                                                                           const int& LDA,
                                                                           ...
                                                                           const int& LDB)
{
    ...
    hipLaunchKernelGGL (HIP_KERNEL_NAME (matrix_copy_kernel<thrust::complex<double>>),
                        gridSize,
                        blockSize,
                        0,
                        0,
...
template <>
void
    matrix_mul_vector_op<double, base_device::DEVICE_GPU>::operator() (const int& m,
                                                                       const int& n,
                                                                       double* a,
                                                                       const int& lda,
                                                                       const double* b,
                                                                       ...
                                                                       const int& ldc)
{
    ...
    hipLaunchKernelGGL (HIP_KERNEL_NAME (matrix_multiply_vector_kernel<double, double>),
                        dim3 (block, thread),
                        m,
                        n,
                        a,
...
template <>
void
    matrix_mul_vector_op<std::complex<float>, base_device::DEVICE_GPU>::operator() (const int& m,
                                                                                    const int& n,
                                                                                    std::complex<float>* a,
                                                                                    const int& lda,
                                                                                    const float* b,
                                                                                    ...
                                                                                    const int& ldc)
{
    dim3 thread (16, 16, 1);
    dim3 block ((m + thread.x - 1) / thread.x, (n + thread.y - 1) / thread.y, 1);
    hipLaunchKernelGGL (HIP_KERNEL_NAME (matrix_multiply_vector_kernel<thrust::complex<float>, float>),
                        dim3 (block, thread),
                        m,
                        n,
...
template <>
void
    matrix_mul_vector_op<std::complex<double>, base_device::DEVICE_GPU>::operator() (const int& m,
                                                                                     const int& n,
                                                                                     ...
                                                                                     const int& ldc)
{
    dim3 thread (16, 16, 1);
    dim3 block ((m + thread.x - 1) / thread.x, (n + thread.y - 1) / thread.y, 1);
    hipLaunchKernelGGL (HIP_KERNEL_NAME (matrix_multiply_vector_kernel<thrust::complex<double>, double>),
                        dim3 (block, thread),
                        m,
                        n,
