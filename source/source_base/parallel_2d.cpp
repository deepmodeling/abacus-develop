...
bool
    Parallel_2D::in_this_processor (const int iw1_all, const int iw2_all) const
{
    return global2local_row (iw1_all) != -1 && global2local_col (iw2_all) != -1;
}

int
    Parallel_2D::get_global_row_size () const
{
    if (!is_serial)
        {
#ifdef __MPI
...
int
    Parallel_2D::get_global_col_size () const
{
    if (!is_serial)
        {
#ifdef __MPI
...
void
    Parallel_2D::_init_proc_grid (const MPI_Comm comm, const bool mode)
{
...
void
    Parallel_2D::_set_dist_info (const int mg, const int ng, const int nb)
{
    this->nb = nb;
...
int
    Parallel_2D::init (const int mg, const int ng, const int nb, const MPI_Comm comm, const bool mode)
{
...
int
    Parallel_2D::set (const int mg, const int ng, const int nb, const int blacs_ctxt)
{
...
void
    Parallel_2D::set_serial (const int mg, const int ng)
{
    assert (mg > 0 && ng > 0);
