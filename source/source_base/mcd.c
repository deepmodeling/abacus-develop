...
void
    addChunk (void* p, int size, char* fun, char* file, int line)
{
#ifdef MCD_FASTFREE
...
int
    delChunk (void* p /*,char*fun,char*file,int line*/)
{
    Chunk* s;
#ifndef MCD_FASTFREE
    unsigned int bias, i;
...
#endif
    void
    phex (FILE* f, // print our 64bit id in a generic way
#ifdef WIN32
          LONGLONG v
#else
          long long v
...
void*
    MCD_malloc (int size, char* fun, char* file, int line)
{
#ifdef MCD_FASTFREE
...
#ifdef MCD_VERBOSE
    fprintf (RTL, "malloc\t[%X][%i bytes]\tfrom %s:%s, line %i [id ", (int)p + SOC, size, file, fun, line);
    phex (RTL, idbase);
#endif
...
void*
    MCD_calloc (int size, char* fun, char* file, int line)
{
#ifdef MCD_FASTFREE
...
#endif
#ifdef MCD_VERBOSE
    fprintf (RTL, "calloc\t[%X][%i bytes]\tfrom %s:%s, line %i [id ", (int)p + SOC, size, file, fun, line);
    phex (RTL, idbase);
#endif
...
void*
    MCD_realloc (void* p, int size, char* fun, char* file, int line)
{
#ifdef MCD_FASTFREE
    Chunk *New, *Old;
...
#ifdef MCD_VERBOSE
    fprintf (RTL,
             "realloc\t[%X]->[%X][%i bytes]\tfrom %s:%s, line %i [id ",
             (int)p + SOC,
             (int)n + SOC,
             size,
             file,
...
#ifdef MCD_VERBOSE
    fprintf (RTL,
             "realloc [%X]->[%X][%ib] from %s:%s, line %i [id ",
             (int)p + SOC,
             (int)n + SOC,
             size,
             file,
...
char*
    MCD_getcwd (char* p, int size, char* fun, char* file, int line)
{
...
#ifdef __GNUC__
char*
    MCD_strdup (const char* s, char* fun, char* file, int line)
#else
char*
    MCD_strdup (char* s, char* fun, char* file, int line)
#endif
{
    char* n = nullptr;
...
#endif
#ifdef MCD_VERBOSE
    fprintf (RTL, "strdup\t[%X][%ib]\t\tfrom %s:%s, line %i [id ", (int)n + SOC, (int)strlen (s), file, fun, line);
    phex (RTL, idbase);
#endif
...
#ifdef __GNUC__
char*
    MCD_strndup (const char* s, int z, char* fun, char* file, int line)
#else
char*
    MCD_strndup (char* s, int z, char* fun, char* file, int line)
#endif
{
    char* n = nullptr;
    int size;
...
#endif
#ifdef MCD_VERBOSE
    fprintf (RTL, "strndup\t[%X][%ib]\t\tfrom %s:%s, line %i [id ", (int)n + SOC, (int)size, file, fun, line);
    phex (RTL, idbase);
#endif
...
int
    MCD_asprintf (char** ptr, const char* fmt, char* fun, char* file, int line, ...)
{
    int retval;
    va_list argptr;
#ifdef MCD_FASTFREE
    ...
    if ((retval = vasprintf (ptr, fmt, argptr)) < 0)
        {
#ifdef MCD_VERBOSE
            fprintf (RTL, "asprintf failure %s:%s, line %i [id ", file, fun, line);
#endif
            return retval;
        }
    va_end (argptr);
...
#endif
#ifdef MCD_VERBOSE
    fprintf (RTL, "asprintf\t[%X][%ib]\t\tfrom %s:%s, line %i [id ", (int)*ptr + SOC, retval + 1, file, fun, line);
    phex (RTL, idbase);
#endif
    return retval;
}
...
int
    MCD_vasprintf (char** ptr, const char* fmt, va_list argptr, char* fun, char* file, int line)
{
    int retval;
#ifdef MCD_FASTFREE
    void* fc = nullptr;
    ...
    if ((retval = vasprintf (ptr, fmt, argptr)) < 0)
        {
#ifdef MCD_VERBOSE
            fprintf (RTL, "vasprintf failure %s:%s, line %i [id ", file, fun, line);
#endif
            return retval;
        }
#ifdef MCD_FASTFREE
...
#endif
#ifdef MCD_VERBOSE
    fprintf (RTL, "vasprintf\t[%X][%ib]\t\tfrom %s:%s, line %i [id ", (int)*ptr + SOC, retval + 1, file, fun, line);
    phex (RTL, idbase);
#endif
    return retval;
}
...
int
    MCD_scanf (const char* fmt, char* fun, char* file, int line, ...)
{
    int retval;
    va_list argptr;
...
int
    MCD_fscanf (FILE* stream, const char* fmt, char* fun, char* file, int line, ...)
{
    int retval;
    va_list argptr;
...
int
    MCD_sscanf (const char* str, const char* fmt, char* fun, char* file, int line, ...)
{
    int retval;
    va_list argptr;
...
void
    scan_args (const char* fmt, va_list argptr, char* fun, char* file, int line)
{
    char** ptr = nullptr;
...
void
    MCD_free (void* p, char* fun, char* file, int line)
{
    ...
    showMemStats ()
{
    ...
    while (c)
        {
            fprintf (o,
                     "%12i bytes allocated in [%s:%s], line %7i [%08X][id ",
                     c->size,
                     c->function,
                     c->file,
                     c->line,
