...
Gram_Schmidt_Orth<Func_Type, R_Type>::Gram_Schmidt_Orth (const std::vector<R_Type>& rab_in,
                                                         const Coordinate& coordinate_in)
    : rab (rab_in), coordinate (coordinate_in)
{
...
template <typename Func_Type, typename R_Type>
std::vector<std::vector<Func_Type>>
    Gram_Schmidt_Orth<Func_Type, R_Type>::cal_orth (const std::vector<std::vector<Func_Type>>& func,
    ...
    for (size_t if1 = 0; if1 != func.size (); ++if1)
        {
            ...
            for (int niter = 0; niter < 3; niter++)
                {
                    ...
                    for (size_t if_minus = 0; if_minus != func_new.size (); ++if_minus)
                        {
                            // (hn,ei)
                            const std::vector<Func_Type>&& mul_func
                                = Mathzone::Pointwise_Product (func_tmp, func_new[if_minus]);
                            const Func_Type in_product = cal_norm (mul_func);

                            // hn - (hn,ei)ei
                            BlasConnector::axpy (mul_func.size (),
                                                 -in_product,
                                                 ModuleBase::GlobalFunc::VECTOR_TO_PTR (func_new[if_minus]),
                                                 1,
                                                 ModuleBase::GlobalFunc::VECTOR_TO_PTR (func_try),
                                                 1);
                        }
                }
            ...
            if (norm >= norm_threshold)
                {
                    BlasConnector::scal (func_try.size (),
                                         1.0 / norm,
                                         ModuleBase::GlobalFunc::VECTOR_TO_PTR (func_try),
                                         1);
...
template <typename Func_Type, typename R_Type>
Func_Type
    Gram_Schmidt_Orth<Func_Type, R_Type>::cal_norm (const std::vector<Func_Type>& f)
{
    Func_Type norm = 0.0;
    switch (this->coordinate)
        ...
        case Coordinate::Cartesian:
            {
                Integral::Simpson_Integral (f.size (),
                                            ModuleBase::GlobalFunc::VECTOR_TO_PTR (f),
        ...
        case Coordinate::Sphere:
            {
                ...
                Integral::Simpson_Integral (f.size (),
                                            ModuleBase::GlobalFunc::VECTOR_TO_PTR (tmp_func),
                                            ModuleBase::GlobalFunc::VECTOR_TO_PTR (rab),
                                            norm);
                break;
            }
        default:
            {
                throw std::invalid_argument ("coordinate must be Cartesian or Sphere " + std::string (__FILE__)
