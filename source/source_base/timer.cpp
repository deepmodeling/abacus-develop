...
void
    timer::finish (std::ofstream& ofs, const bool print_flag, const bool check_end)
{
...
void
    timer::start (const std::string& class_name, const std::string& name)
{
    //----------------------------------------------------------
...
void
    timer::end (const std::string& class_name, const std::string& name)
{
    //----------------------------------------------------------
    // EXPLAIN : if timer is disabled , return
    //----------------------------------------------------------
...
void
    timer::write_to_json (std::string file_name)
{
#ifdef __MPI
            ...
            for (auto& timer_pool_B: timer_pool_A.second)
                {
                    ...
                    ofs << indent << indent << indent << indent << "\"cpu_second\": " << std::setprecision (15)
                        << timer_one.cpu_second << ",\n";
                    ofs << indent << indent << indent << indent << "\"calls\": " << timer_one.calls << ",\n";
                    ofs << indent << indent << indent << indent
                        << "\"cpu_second_per_call\": " << double_to_string (timer_one.cpu_second / timer_one.calls)
                        << ",\n";
...
void
    timer::print_all (std::ofstream& ofs, const bool check_end)
{
            ...
            for (auto& timer_pool_B: timer_pool_A.second)
                {
                    ...
                    // timer_pool_order[timer_one.order] = {{class_name, name}, timer_one}; //qianrui change it to make
                    // it compatible with old compiler version
                    timer_pool_order[timer_one.order] = std::pair<std::pair<std::string, std::string>, Timer_One>{
                        std::pair<std::string, std::string>{class_name, name},
                        timer_one};
                }
        }
