...
CubicSplineTest::CubicSplineTest()
    : n_max_(1000), spline_(3 * n_max_), x_(spline_.data()), y_(x_ + n_max_), dy_(y_ + n_max_), n_interp_max_(1000),
      interp_(7 * n_interp_max_), x_interp_(interp_.data()), y_interp_(x_interp_ + n_interp_max_),
      dy_interp_(y_interp_ + n_interp_max_), d2y_interp_(dy_interp_ + n_interp_max_),
      y_ref_(d2y_interp_ + n_interp_max_), dy_ref_(y_ref_ + n_interp_max_),
      d2y_ref_(dy_ref_ + n_interp_max_), f_{
                                             {
...
double CubicSplineTest::error_bound(int n, const double* x, const std::function<double(double)>& d4f, int d) const
{
                           ...
                           double* d2y_interp) const
{
    ...
    auto bc_parse = [](const std::string& bc) {
        ...
        {
            return BoundaryCondition(BoundaryType::first_deriv, std::stod(bc.substr(12, std::string::npos)));
        }
        if (bc.find("second_deriv") != std::string::npos)
        {
            return BoundaryCondition(BoundaryType::second_deriv, std::stod(bc.substr(13, std::string::npos)));
        }
        else
        {
            assert(false);
    ...
    for (int i = 0; i < 6; ++i)
    {
        std::getline(ifs, line);
        std::stringstream ss(line);
        data[i] = std::copy(std::istream_iterator<double>(ss), std::istream_iterator<double>(), data[i]);
    }
    n = data[0] - x;
...
TEST_F(CubicSplineTest, MultiEval)
{
    ...
    for (size_t i = 0; i < f_.size(); ++i)
    {
        ...
        cubspl.add(y_, {BoundaryType::first_deriv, f_[i][1](xmin)}, {BoundaryType::first_deriv, f_[i][1](xmax)});
    }
    ...
    std::for_each(x_interp_, x_interp_ + n_interp, [&](double& x) { x = (&x - x_interp_) * dx_interp + xmin; });
...
TEST_F(CubicSplineTest, ErrorBound)
{
    ...
    for (size_t i = 0; i < f_.size(); ++i)
    {
        ...
        CubicSpline::eval(n, x_, y_, dy_, n_interp, x_interp_, y_interp_, dy_interp_, d2y_interp_);

        double* diff[3] = {y_interp_, dy_interp_, d2y_interp_};
        for (int d = 0; d < 3; ++d)
        {
            std::transform(x_interp_, x_interp_ + n_interp, diff[d], diff[d], [&](double x, double y) {
                return std::abs(y - f_[i][d](x));
            });

            double err_bound = error_bound(n, x_, f_[i][4], d);
            EXPECT_TRUE(
                std::all_of(diff[d], diff[d] + n_interp, [err_bound](double diff) { return diff < err_bound; }));
        }
    }
}
...
TEST_F(CubicSplineTest, CrossCheck)
{
    ...
    for (const auto& fname: fnames)
    {
        ...
        for (int d = 0; d < 3; ++d)
        {
            ...
            EXPECT_TRUE(
                std::all_of(diff[d], diff[d] + n_interp, [this, d](double diff) { return std::abs(diff) < tol_[d]; }));
        }
    }
}
