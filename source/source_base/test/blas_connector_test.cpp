...
TEST(blas_connector, dcopy) {
    ...
    for (int i = 0; i < size; i++)
        answer[i] = x_const[i];
    BlasConnector::copy(size, x_const.data(), incx, result.data(), incy);
    for (int i = 0; i < size; i++)
        EXPECT_DOUBLE_EQ(answer[i], result[i]);
}

TEST(blas_connector, zcopy) {
    typedef std::complex<double> T;
    long const size = 8;
    int const incx = 1;
    int const incy = 1;
