...
cases = [
    Case(np.sin, np.logspace(-1, 0, 10), "not-a-knot", "sin_not_a_knot.dat"),
    Case(np.cos, [0, 0.1, 1, 1.5, 3, 2 * np.pi], "periodic", "cos_periodic.dat"),
    Case(
        np.exp,
        np.logspace(-1, 0, 10),
    ...
    Case(np.sqrt, np.logspace(0, 1, 10), ((1, 0.5), "not-a-knot"), "sqrt_mix_bc.dat"),
    ...
    Case(np.sin, [0, 0.3, 1], "not-a-knot", "three_points_not_a_knot.dat"),
]
