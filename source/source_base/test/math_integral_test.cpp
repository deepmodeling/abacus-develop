...
void
    sinspace (double start, double end, const int nums, double* xv, double* h)
{
...
TEST_F (SimpsonIntegralSinx, Constructor) { EXPECT_NO_THROW (ModuleBase::Integral intgral); }
            ...
            for (double end = 0.1; end < 10 * ModuleBase::PI; end += 0.5)
                {
                    ...
                    EXPECT_NEAR (std::cos (start) - std::cos (end),
                                 ModuleBase::Integral::simpson (ngrid, f, dx),
                                 std::max (tol, doublethreshold));
                }
        }
    delete[] f;
}
            ...
            for (double end = 0.1; end < 10 * ModuleBase::PI; end += 0.5)
                {
                    ...
                    // error bound of composite Simpson's rule (max|sin^(4)(x)| = 1)
                    double tol = ((ngrid - 4) * dx - start) * std::pow (dx, 4) / 180
                                 + (end - (ngrid - 4) * dx) * std::pow (dx, 4) / 80;
                    EXPECT_NEAR (std::cos (start) - std::cos (end),
                                 ModuleBase::Integral::simpson (ngrid, f, dx),
                                 std::max (tol, doublethreshold));
                }
            ...
            for (double w = 0.1; w < 10 * ModuleBase::PI; w += 0.5)
                {
                    ...
                    EXPECT_NEAR (std::cos (start) - std::cos (start + w),
                                 ModuleBase::Integral::simpson (ngrid, f, h),
                                 std::max (tol, doublethreshold));
...
TEST_F (SimpsonIntegralSinx, FourPoints)
{
    ...
    auto I = [&] (double x)
        {
            return A * std::pow (x - x0, 4) / 4 + B * std::pow (x - x0, 3) / 3 + C * std::pow (x - x0, 2) / 2
                   + D * (x - x0);
        };
    ...
    for (int ngrid = 5; ngrid <= ngrid_max; ngrid += 2)
        {
            ...
            EXPECT_NEAR (std::atan (end) - std::atan (start),
                         ModuleBase::Integral::simpson (ngrid, f, dx),
                         std::max (tol, doublethreshold));
    ...
    for (int ngrid = 5; ngrid <= ngrid_max; ngrid += 2)
        {
            ...
            EXPECT_NEAR (std::exp (end) - std::exp (start),
                         ModuleBase::Integral::simpson (ngrid, f, dx),
                         std::max (tol, doublethreshold));
            double err = std::abs (ModuleBase::Integral::simpson (ngrid, f, dx) - ref_val) / std::abs (ref_val);
            file_o.write (reinterpret_cast<char*> (&err), sizeof (double));
