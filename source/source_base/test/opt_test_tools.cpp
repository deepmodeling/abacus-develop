...
void
    LinearEqu::get_Ap (double** A, double* p, double* Ap, int nx, int ny)
{
    for (int i = 0; i < this->nx; ++i)
        {
            Ap[i] = 0;
...
// df(x)/dx = Ax - b
void
    LinearEqu::dfuncdx (double* x, double* gradient)
{
    double* Ax = new double[this->nx];
    this->get_Ap (A, x, Ax, this->nx, this->nx);
    for (int i = 0; i < this->nx; ++i)
...
double
    LinearEqu::dfuncdstp (double* x, double* p)
{
    double* Ax = new double[this->nx];
...
// df(x)/dx = Ax - b
void
    ESolver_OF::dfuncdx (double* x, double* gradient)
{
...
double
    ESolver_OF::dfuncdstp (double* x, double* p)
{
