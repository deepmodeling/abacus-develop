...
TEST_F(ToolQuitTest, warningquit) {
    testing::internal::CaptureStdout();
    EXPECT_EXIT(ModuleBase::WARNING_QUIT("INPUT", "bad input parameter"), ::testing::ExitedWithCode(1), "");
    output = testing::internal::GetCapturedStdout();
...
TEST_F(ToolQuitTest, warningquit_with_ret) {
    testing::internal::CaptureStdout();
    EXPECT_EXIT(ModuleBase::WARNING_QUIT("INPUT", "bad input parameter", 1), ::testing::ExitedWithCode(1), "");
    output = testing::internal::GetCapturedStdout();
    // test output on screening
    EXPECT_THAT(output, testing::HasSubstr("TIME STATISTICS"));
    GlobalV::ofs_warning.close();
