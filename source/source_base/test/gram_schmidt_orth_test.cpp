...
class GramSchmidtOrth
{
  public:
        ...
        inner_product (std::vector<double> a, std::vector<double> b)
    {
        ...
        ModuleBase::Integral::Simpson_Integral (mul_func1.size (),
                                                ModuleBase::GlobalFunc::VECTOR_TO_PTR (mul_func1),
    ...
    while (true)
        {
            ...
            niter += 1;
            old_basis = gso_sphere.cal_orth (new_basis);
            new_basis = old_basis;
        }

    // std::cout <<  "nbasis=" << gsot.nbasis << "niter=" << niter << " max_inner_product=" << std::setprecision(15) <<
    // maxip << std::endl;
    EXPECT_TRUE (pass) << "nbasis=" << gsot.nbasis << "niter=" << niter
                       << " max_inner_product=" << std::setprecision (15) << maxip;
}
