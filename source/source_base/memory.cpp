...
double
    Memory::calculate_mem (const long& n_in, const std::string& type)
{
...
double
    Memory::record (const std::string& class_name_in,
...
void
    Memory::record (const std::string& name_in, const long& n_in, const bool accumulate)
{
    if (!Memory::init_flag)
        {
            name = new std::string[n_memory];
...
double
    Memory::record_gpu (const std::string& class_name_in,
                        const std::string& name_in,
                        const long& n_in,
                        const std::string& type,
                        const bool accumulate)
...
void
    Memory::record_gpu (const std::string& name_in, const size_t& n_in, const bool accumulate)
{
    if (!Memory::init_flag_gpu)
        {
            name_gpu = new std::string[n_memory];
...
void
    Memory::finish (std::ofstream& ofs)
{
    print_all (ofs);
    if (init_flag)
        {
...
void
    Memory::print_all (std::ofstream& ofs)
{
    if (!init_flag)
        {
            return;
    ...
    for (int i = 0; i < n_memory; i++) // Xiaoyang fix memory record sum bug 2023/10/25
        {
            ...
            if (consume[k] < small)
                ...
                {
                    ofs << std::setw (30) << name[k] << std::setw (15) << consume[k] << std::endl;
                }
        }
    ...
    for (int i = 0; i < n_memory; i++) // Xiaoyang fix memory record sum bug 2023/10/25
        {
            ...
            if (consume_gpu[k] < small)
                ...
                {
                    ofs << std::setw (30) << name_gpu[k] << std::setw (15) << consume_gpu[k] << std::endl;
                }
        }
