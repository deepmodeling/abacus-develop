...
double Memory::record(const std::string& class_name_in,
...
void Memory::record(const std::string& name_in, const long& n_in, const bool accumulate) {
    if (!Memory::init_flag) {
        name = new std::string[n_memory];
        class_name = new std::string[n_memory];
        consume = new double[n_memory];
...
double Memory::record_gpu(const std::string& class_name_in,
                          const std::string& name_in,
                          const long& n_in,
                          const std::string& type,
                          const bool accumulate) {
...
void Memory::record_gpu(const std::string& name_in, const size_t& n_in, const bool accumulate) {
    if (!Memory::init_flag_gpu) {
        name_gpu = new std::string[n_memory];
        class_name_gpu = new std::string[n_memory];
        consume_gpu = new double[n_memory];
...
void Memory::print(const int find) {
    GlobalV::ofs_running << "\n Warning_Memory_Consuming allocated: " << " " << name[find] << " " << consume[find]
                         << " MB" << std::endl;
    return;
}
