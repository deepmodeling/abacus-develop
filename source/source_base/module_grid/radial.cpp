...
void
    baker (int nbase, double R, double* r, double* w, int mult)
{
...
void
    baker (int nbase, double R, std::vector<double>& r, std::vector<double>& w, int mult)
{
    int n = (nbase + 1) * mult - 1;
    r.resize (n);
    w.resize (n);
...
void
    murray (int n, double R, double* r, double* w)
{
    for (int i = 1; i <= n; ++i)
        {
            double x = static_cast<double> (i) / (n + 1);
            r[i - 1] = std::pow (x / (1.0 - x), 2) * R;
            w[i - 1] = 2.0 / (n + 1) * std::pow (R, 3) * std::pow (x, 5) / std::pow (1.0 - x, 7);
        }
}

void
    treutler_m4 (int n, double R, double* r, double* w, double alpha)
{
    ...
    for (int i = 1; i <= n; ++i)
        {
            ...
            w[i - 1] = pi / (n + 1) * std::pow (delta * R * inv_ln2, 3) * gamma * gamma * (beta - alpha / beta * gamma);
        }
}

void
    mura (int n, double R, double* r, double* w)
{
    for (int i = 1; i <= n; ++i)
        {
            double x = static_cast<double> (i) / (n + 1);
