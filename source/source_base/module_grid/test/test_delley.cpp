...
{
void
    WARNING_QUIT (const std::string&, const std::string&)
{
}
} // namespace ModuleBase
...
void
    DelleyTest::randgen (int lmax, std::vector<double>& coef)
{
    ...
    for (int grid_lmax = 17; grid_lmax < 60; grid_lmax += 6)
        {
            ...
            for (size_t i = 0; i < weight.size (); i++)
                {
                    ModuleBase::Ylm::sph_harm (func_lmax, grid[3 * i], grid[3 * i + 1], grid[3 * i + 2], ylm_real);
                    double tmp = 0.0;
