...
std::vector<std::pair<Func_t, double>> test_func_ref = {
    {[] (double r) { return std::exp (-0.3 * r * r) + std::exp (-3.0 * r * r); },
     0.25 * std::sqrt (pi) * (std::pow (0.3, -1.5) + std::pow (3.0, -1.5))},
    {[] (double r) { return r * (std::exp (-0.3 * r * r) + std::exp (-3.0 * r * r)); },
     0.5 / (0.3 * 0.3) + 0.5 / (3.0 * 3.0)},
    {[] (double r) { return r * r * (std::exp (-0.3 * r * r) + std::exp (-3.0 * r * r)); },
     0.375 * std::sqrt (pi) * (std::pow (0.3, -2.5) + std::pow (3.0, -2.5))},
};

double
    quadrature (const Func_t& f, int n, double* r, double* w)
{
    double res = 0.0;
    for (int i = 0; i < n; i++)
        {
