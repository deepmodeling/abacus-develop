...
bool
    is_same_octant (int ngrid, const double* grid)
{
    if (ngrid == 0)
        {
            return true;
    ...
    for (; grid != end; grid += 3)
        {
            if (is_positive_x != (grid[0] > 0) || is_positive_y != (grid[1] > 0) || is_positive_z != (grid[2] > 0))
                {
                    return false;
                }
        }
...
bool
    good_batch_size (const std::vector<int>& idx, const std::vector<int>& delim, int n_batch_thr)
{
...
TEST_F (BatchTest, MaxMinRandom)
{
    ...
    std::vector<int> delim = maxmin (grid_.data (), idx_.data (), idx_.size (), n_batch_rand_);
...
TEST_F (BatchTest, MaxMinOctant)
{
    ...
    gen_octant (n_batch_oct_, offset_x_, offset_y_, offset_z_, width_oct_, grid_, idx_);

    std::vector<int> delim = maxmin (grid_.data (), idx_.data (), idx_.size (), n_batch_oct_);
