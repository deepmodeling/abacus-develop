...
template <typename T, typename Device>
void
    Stochastic_WF<T, Device>::allocate_chiallorder (const int& norder)
{
...
template <typename T, typename Device>
void
    Stochastic_WF<T, Device>::clean_chiallorder ()
{
    delete[] chiallorder;
    chiallorder = nullptr;
}
template <typename T, typename Device>
void
    Stochastic_WF<T, Device>::init_sto_orbitals (const int seed_in)
{
    const unsigned int rank_seed_offset = 10000;
    if (seed_in == 0 || seed_in == -1)
        {
            srand (static_cast<unsigned int> (time (nullptr))
                   + GlobalV::MY_RANK * rank_seed_offset); // GlobalV global variables are reserved
        }
    else
        {
...
template <typename T, typename Device>
void
    Stochastic_WF<T, Device>::allocate_chi0 ()
{
...
template <typename T, typename Device>
void
    Stochastic_WF<T, Device>::update_sto_orbitals (const int seed_in)
{
    const int nchi = PARAM.inp.nbands_sto;
            ...
            for (int i = 0; i < this->chi0_cpu->size (); ++i)
                {
                    ...
                    this->chi0_cpu->get_pointer ()[i]
                        = std::complex<double> (cos (phi), sin (phi)) / sqrt (double (nchi));
                }
        }
    else
        {
...
template <typename T, typename Device>
void
    Stochastic_WF<T, Device>::init_com_orbitals ()
{
...
template <typename T, typename Device>
void
    Stochastic_WF<T, Device>::init_sto_orbitals_Ecut (const int seed_in,
                                                      const K_Vectors& kv,
...
template <typename T, typename Device>
void
    Stochastic_WF<T, Device>::sync_chi0 ()
{
    Device* ctx = {};
    if (base_device::get_device_type (ctx) == base_device::GpuDevice)
        {
