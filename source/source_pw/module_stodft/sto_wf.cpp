...
{
    unsigned int seed_scale=10000; // seed_scale is defined to avoid repetition
    if (seed_in == 0 || seed_in == -1)
    {
        srand(static_cast<unsigned>(time(nullptr)) + GlobalV::MY_RANK * seed_scale); // GlobalV global variables are reserved
    }
    else
    {
        srand(static_cast<unsigned>(std::abs(seed_in)) + (GlobalV::MY_BNDGROUP * GlobalV::NPROC_IN_BNDGROUP + GlobalV::RANK_IN_BPGROUP) * seed_scale);
    }
