...
template <typename T, typename Device>
void
    Stochastic_Iter<T, Device>::dot (const int& n,
                                     const Real* x,
                                     const int& incx,
                                     const Real* y,
                                     const int& incy,
...
template <typename T, typename Device>
void
    Stochastic_Iter<T, Device>::init (K_Vectors* pkv_in,
...
template <typename T, typename Device>
void
    Stochastic_Iter<T, Device>::orthog (const int& ik, psi::Psi<T, Device>& psi, Stochastic_WF<T, Device>& stowf)
{
...
template <typename T, typename Device>
void
    Stochastic_Iter<T, Device>::checkemm (const int& ik,
                                          const int istep,
                                          const int iter,
                                          Stochastic_WF<T, Device>& stowf)
{
            ...
            while (true)
                {
                    ...
                    auto hchi_norm = std::bind (&hamilt::HamiltSdftPW<T, Device>::hPsi_norm,
                                                ...
                                                std::placeholders::_2,
                                                std::placeholders::_3);
                    converge = p_che->checkconverge (hchi_norm,
                                                     pchi,
                                                     npw,
                                                     stowf.npwx,
                                                     *p_hamilt_sto->emax,
    ...
    if (ik == nks - 1)
        {
            ...
            if (change)
                {
                    GlobalV::ofs_running << "New Emax Ry" << *p_hamilt_sto->emax << " ; new Emin "
                                         << *p_hamilt_sto->emin << " Ry" << std::endl;
                }
            change = false;
        }
    ModuleBase::timer::end ("Stochastic_Iter", "checkemm");
...
template <typename T, typename Device>
void
    Stochastic_Iter<T, Device>::check_precision (const double ref, const double thr, const std::string info)
{
    //==============================
    // precision check
    //==============================
...
template <typename T, typename Device>
void
    Stochastic_Iter<T, Device>::itermu (const int iter, elecstate::ElecState* pes)
{
...
template <typename T, typename Device>
void
    Stochastic_Iter<T, Device>::calPn (const int& ik, Stochastic_WF<T, Device>& stowf)
{
    ...
    if (this->method == 1)
    ...
    else
        {
            ...
            ModuleBase::gemm_op<Real, Device> () (trans,
                                                  normal,
                                                  N,
                                                  N,
                                                  M,
...
template <typename T, typename Device>
double
    Stochastic_Iter<T, Device>::calne (elecstate::ElecState* pes)
{
...
template <typename T, typename Device>
void
    Stochastic_Iter<T, Device>::calHsqrtchi (Stochastic_WF<T, Device>& stowf)
{
...
template <typename T, typename Device>
void
    Stochastic_Iter<T, Device>::sum_stoeband (Stochastic_WF<T, Device>& stowf,
...
template <typename T, typename Device>
void
    Stochastic_Iter<T, Device>::cal_storho (const UnitCell& ucell,
                                            Stochastic_WF<T, Device>& stowf,
                                            elecstate::ElecStatePW<T, Device>* pes,
                                            ModulePW::PW_Basis_K* wfc_basis)
{
...
template <typename T, typename Device>
void
    Stochastic_Iter<T, Device>::calTnchi_ik (const int& ik, Stochastic_WF<T, Device>& stowf)
{
    const int npw = stowf.ngk[ik];
    const int npwx = stowf.npwx;
    stowf.shchi->fix_k (ik);
    ...
    if (this->method == 2)
        {
            ...
            // zgemv_(&transa, &M, &N, &one, stowf.chiallorder[ik].get_pointer(), &LDA, coef_real, &inc, &zero, out,
            // &inc);
