...
template <typename FPTYPE, typename Device>
void
    Sto_EleCond<FPTYPE, Device>::decide_nche (const FPTYPE dt,
...
template <typename FPTYPE, typename Device>
void
    Sto_EleCond<FPTYPE, Device>::cal_jmatrix (hamilt::HamiltSdftPW<std::complex<lowTYPE>, Device>* hamilt,
                                              ...
                                              const int bandinfo[6])
{
    ...
    righthchi_all = gatherchi_op<lowTYPE, Device> () (right_hchi,
                                                      hchi_all,
                                                      npwx,
                                                      sto_npwx->nrecv,
                                                      sto_npwx->displs,
    ...
    if (PARAM.inp.bndpar > 1 && rightfact != nullptr)
        {
            ...
            Parallel_Common::gatherv_data (rightfact,
                                           perbands_ks,
                                           rightf_all,
                                           ks_fact->nrecv,
                                           ks_fact->displs,
...
template <typename FPTYPE, typename Device>
void
    Sto_EleCond<FPTYPE, Device>::sKG (const int& smear_type,
                                      const double& fwhmin,
                                      const double& wcut,
                                      const double& dw_in,
                                      const double& dt_in,
    ...
    for (int ik = 0; ik < nk; ++ik)
        {
            ...
            if (this->nbands_ks > 0)
                {
                    ...
                    std::cout << "Emin_KS(" << cutib0 + 1 << "): " << Emin_KS * ModuleBase::Ry_to_eV
                              << " eV; Emax: " << emax * ModuleBase::Ry_to_eV
            ...
            if (PARAM.inp.bndpar > 1)
                {
                    ...
                    for (int ib = 0; ib < allbands_ks; ++ib)
                        {
                            castmem_lcomplex_op () (&psi_all (0, ib, 0),
                                                    &this->p_psi[0](this->nbands_ks - allbands_ks + ib, 0),
                                                    npw);
                        }
                    kspsi_all = &psi_all;
            ...
            ModuleBase::Memory::record ("SDFT::batchjpsi",
                                        3 * bsize_psi * ndim * npwx * sizeof (std::complex<lowTYPE>));
            ...
            if (nbatch > 1)
                {
                    ...
                    ModuleBase::Memory::record ("SDFT::poly_exptsfchi",
                                                sizeof (lcomplex) * cond_nche * perbands_sto * npwx);
            ...
            for (int it = 1; it < nt; ++it)
                {
                    ...
                    if (it - 1 == print_step)
                        {
                            ...
                            std::cout << "(Time left " << timeTaken * (double (nt - 1) / print_step * (nk - ik) - 1)
                                      << " s) " << std::endl;
                    ...
                    if (nbatch == 1)
                    ...
                    else
                        {
                            ...
                            if ((it - 1) % nbatch == 0)
                                {
                                    chet.calpolyvec_complex (hchi_norm_low,
                                                             stoexptsfchi,
                                                             tmppolyexptsfchi,
                                                             npw,
                                                             npwx,
                                    ...
                                    chet.calpolyvec_complex (hchi_norm_low,
                                                             ...
                                                             npwx,
                                                             perbands_sto);
                                    chemt.calpolyvec_complex (hchi_norm_low,
                                                              stoexpmtsfchi,
                                                              tmppolyexpmtsfchi,
                                                              npw,
                                                              npwx,
                    ...
                    ct11[it] += static_cast<double> (
                        ModuleBase::dot_real_op<lcomplex, Device> () (num_per,
