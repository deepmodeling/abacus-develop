...
template <typename FPTYPE, typename Device>
void
    Sto_DOS<FPTYPE, Device>::decide_param (const int& dos_nche,
...
template <typename FPTYPE, typename Device>
void
    Sto_DOS<FPTYPE, Device>::caldos (const double sigmain, const double de, const int npart)
{
    ...
    for (int ie = 0; ie < ndos; ++ie)
        {
            ...
            if (this->method_sto == 1)
                ...
                {
                    auto nroot_gauss
                        = std::bind (&Sto_Func<double>::nroot_gauss, &this->stofunc, std::placeholders::_1);
