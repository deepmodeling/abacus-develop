...
void
    KEDF_vW::set_para (double dV, double vw_weight)
{
    this->dV_ = dV;
 ...
 */
double
    KEDF_vW::get_energy (double** pphi, ModulePW::PW_Basis* pw_rho)
{
 ...
 */
double
    KEDF_vW::get_energy_density (double** pphi, int is, int ir, ModulePW::PW_Basis* pw_rho)
{
    // since pphi may contain minus element, we define tempPhi = std::abs(phi), which is true sqrt(rho)
    double** tempPhi = new double*[PARAM.inp.nspin];
    for (int is = 0; is < PARAM.inp.nspin; ++is)
 ...
 */
void
    KEDF_vW::tau_vw (const double* const* pphi, ModulePW::PW_Basis* pw_rho, double* rtau_vw)
{
            ...
            for (int j = 0; j < 3; ++j)
                {
                    ...
                    for (int ir = 0; ir < pw_rho->nrxx; ++ir)
                        {
                            rtau_vw[ir] += nabla_phi[j][ir] * nabla_phi[j][ir] / 2. * this->vw_weight_
                                           * 2.0; // convert Ha to Ry.
                        }
                }
        }
}
 ...
 */
void
    KEDF_vW::vw_potential (const double* const* pphi, ModulePW::PW_Basis* pw_rho, ModuleBase::matrix& rpotential)
{
            ...
            for (int ir = 0; ir < pw_rho->nrxx; ++ir)
                {
                    if (pphi[is][ir] >= 0)
                        {
                            rpotential (is, ir)
                                += LapPhi[is][ir] * this->vw_weight_ * 2.; // vw_weight * 2 to convert Hartree to Ry
                        }
                    else
                        {
                            rpotential (is, ir)
 ...
 */
void
    KEDF_vW::get_stress (const double* const* pphi, ModulePW::PW_Basis* pw_rho)
{
    // since pphi may contain minus element, we define tempPhi = std::abs(phi), which is true sqrt(rho)
    double** tempPhi = new double*[PARAM.inp.nspin];
    for (int is = 0; is < PARAM.inp.nspin; ++is)
                    ...
                    for (int is = 0; is < PARAM.inp.nspin; ++is)
                        {
                            for (int ik = 0; ik < pw_rho->npw; ++ik)
                                {
                                    ggrecipPhi[is][ik] = -recipPhi[is][ik] * pw_rho->gcar[ik][alpha]
                                                         * pw_rho->gcar[ik][beta] * pw_rho->tpiba2;
                                }
 ...
 */
void
    KEDF_vW::laplacian_phi (const double* const* pphi, double** rLapPhi, ModulePW::PW_Basis* pw_rho)
{
    std::complex<double>** recipPhi = new std::complex<double>*[PARAM.inp.nspin];
    for (int is = 0; is < PARAM.inp.nspin; ++is)
        {
