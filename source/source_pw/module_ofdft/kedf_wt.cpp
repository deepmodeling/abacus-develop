 ...
 */
double
    KEDF_WT::get_energy (const double* const* prho, ModulePW::PW_Basis* pw_rho)
{
 ...
 */
double
    KEDF_WT::get_energy_density (const double* const* prho, int is, int ir, ModulePW::PW_Basis* pw_rho)
{
    double** kernelRhoBeta = new double*[PARAM.inp.nspin];
    for (int is = 0; is < PARAM.inp.nspin; ++is)
        {
 ...
 */
void
    KEDF_WT::tau_wt (const double* const* prho, ModulePW::PW_Basis* pw_rho, double* rtau_wt)
{
    double** kernelRhoBeta = new double*[PARAM.inp.nspin];
    for (int is = 0; is < PARAM.inp.nspin; ++is)
        {
 ...
 */
void
    KEDF_WT::wt_potential (const double* const* prho, ModulePW::PW_Basis* pw_rho, ModuleBase::matrix& rpotential)
{
 ...
 */
void
    KEDF_WT::get_stress (const double* const* prho, ModulePW::PW_Basis* pw_rho, double vw_weight)
{
    double coef = 0.;
    double mult = 0.;
    if (this->hold_rho0_)
                    ...
                    for (int a = 0; a < 3; ++a)
                        {
                            for (int b = a; b < 3; ++b)
                                {
                                    this->stress (a, b)
                                        += -diff * pw_rho->gcar[ip][a] * pw_rho->gcar[ip][b] / pw_rho->gg[ip];
                                    if (a == b)
                                        {
 ...
 */
double
    KEDF_WT::wt_kernel (double eta, double tf_weight, double vw_weight)
{
    if (eta < 0.)
        {
            return 0.;
 ...
 */
double
    KEDF_WT::diff_linhard (double eta, double vw_weight)
{
    if (eta < 0.)
        {
            return 0.;
 ...
 */
void
    KEDF_WT::multi_kernel (const double* const* prho, double** rkernel_rho, double exponent, ModulePW::PW_Basis* pw_rho)
{
 ...
 */
void
    KEDF_WT::fill_kernel (double tf_weight, double vw_weight, ModulePW::PW_Basis* pw_rho)
{
    double eta = 0.;
 ...
 */
void
    KEDF_WT::read_kernel (std::string file_name, ModulePW::PW_Basis* pw_rho)
{
