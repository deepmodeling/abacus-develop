 ...
 */
double
    KEDF_XWM::get_energy (const double* const* prho, ModulePW::PW_Basis* pw_rho)
{
 ...
 */
double
    KEDF_XWM::get_energy_density (const double* const* prho, int is, int ir, ModulePW::PW_Basis* pw_rho)
{
    double** w1Rho5_6 = new double*[PARAM.inp.nspin];
    for (int is = 0; is < PARAM.inp.nspin; ++is)
        {
 ...
 */
void
    KEDF_XWM::tau_xwm (const double* const* prho, ModulePW::PW_Basis* pw_rho, double* rtau_xwm)
{
    double** w1Rho5_6 = new double*[PARAM.inp.nspin];
    for (int is = 0; is < PARAM.inp.nspin; ++is)
        {
 ...
 */
void
    KEDF_XWM::xwm_potential (const double* const* prho, ModulePW::PW_Basis* pw_rho, ModuleBase::matrix& rpotential)
{
 ...
 */
void
    KEDF_XWM::get_stress (const double* const* prho, ModulePW::PW_Basis* pw_rho, double vw_weight)
{
 ...
 */
void
    KEDF_XWM::multi_kernel (const double* const* prho,
                            const double* kernel,
                            double** rkernel_rho,
                            double exponent,
                            ModulePW::PW_Basis* pw_rho)
 ...
 */
void
    KEDF_XWM::fill_kernel (double tf_weight, double vw_weight, ModulePW::PW_Basis* pw_rho)
{
    double eta = 0.;
    ...
    for (int ig = 0; ig < pw_rho->npw; ++ig)
        {
            ...
            // Taylor expansion for high eta
            else if (eta > 3.65)
                {
                    ...
                    diff_lindhard
                        = ((eta2 + 1.) * 0.25 / eta2 * std::log (std::abs ((1. + eta) / (1. - eta))) - 0.5 / eta)
                              / std::pow (
                                  (0.5 + 0.25 * (1. - eta2) * std::log ((1. + eta) / std::abs (1. - eta)) / eta),
                                  2)
                          - 6. * eta * vw_weight;
                }
            else
