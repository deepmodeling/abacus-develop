...
#pragma omp parallel for
    ...
    for (int is = 0; is < PARAM.inp.nspin; ++is) {
        ...
        for (int ik = 0; ik < pw_rho->npw; ++ik) {
            recipCDPotential[ik] = recipCurrent_x[ik] * pw_rho->gcar[ik].x + recipCurrent_y[ik] * pw_rho->gcar[ik].y +
...
#pragma omp parallel for collapse(2)
    ...
    for (int is = 0; is < PARAM.inp.nspin; ++is) {
        for (int ir = 0; ir < pw_rho->nrxx; ++ir) {
            ...
            pphi_[is * nrxx + ir] +=
                1.0 / 6.0 *
                (K1[is * nrxx + ir] + 2.0 * K2[is * nrxx + ir] + 2.0 * K3[is * nrxx + ir] + K4[is * nrxx + ir]);
        }
    }
