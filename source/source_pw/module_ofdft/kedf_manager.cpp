 ...
 */
void
    KEDF_Manager::init (const Input_para& inp, ModulePW::PW_Basis* pw_rho, const double dV, const double nelec)
{
    ...
    if (this->of_kinetic_ == "tf" || this->of_kinetic_ == "tf+" || this->of_kinetic_ == "wt"
        || this->of_kinetic_ == "ext-wt" || this->of_kinetic_ == "ml" || this->of_kinetic_ == "xwm")
        {
            if (this->tf_ == nullptr)
                {
    ...
    if (this->of_kinetic_ == "vw" || this->of_kinetic_ == "tf+" || this->of_kinetic_ == "wt"
        || this->of_kinetic_ == "ext-wt" || this->of_kinetic_ == "xwm" || this->of_kinetic_ == "lkt"
        || this->of_kinetic_ == "ml")
        {
            if (this->vw_ == nullptr)
                {
 ...
 */
void
    KEDF_Manager::get_potential (const double* const* prho,
                                 ...
                                 ModuleBase::matrix& rpot)
{
    ...
    if (PARAM.inp.of_ml_local_test)
        this->ml_->localTest (prho, pw_rho);
#endif
            ...
            if (this->ml_->ml_energy >= this->tf_->tf_energy)
                {
                    ...
                    std::cout << "ML Term = " << this->ml_->ml_energy << " Ry, TF Term = " << this->tf_->tf_energy
                              << " Ry." << std::endl;
                }
        }
#endif
 ...
 */
void
    KEDF_Manager::get_energy_density (const double* const* prho,
                                      const double* const* pphi,
                                      ModulePW::PW_Basis* pw_rho,
                                      double** rtau)
{
 ...
 */
void
    KEDF_Manager::get_stress (const double omega,
...
void
    KEDF_Manager::record_energy (std::vector<std::string>& titles, std::vector<double>& energies_Ry)
{
    if (this->of_kinetic_ == "tf" || this->of_kinetic_ == "tf+" || this->of_kinetic_ == "wt"
        || this->of_kinetic_ == "ext-wt" || this->of_kinetic_ == "xwm")
        {
...
void
    KEDF_Manager::generate_ml_target (const double* const* prho, ModulePW::PW_Basis* pw_rho, const double* veff)
{
#ifdef __MLALGO
