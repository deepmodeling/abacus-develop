...
void
    ML_Base::set_device (std::string device_inpt)
{
    if (device_inpt == "cpu")
        {
...
void
    ML_Base::updateInput (const double* const* prho, const ModulePW::PW_Basis* pw_rho)
{
...
void
    ML_Base::NN_forward (const double* const* prho, const ModulePW::PW_Basis* pw_rho, bool cal_grad)
{
...
torch::Tensor
    ML_Base::get_data (std::string parameter, const int ikernel) const
{
...
void
    ML_Base::get_potential_ (const double* const* prho,
    ...
    for (int ir = 0; ir < this->nx; ++ir)
        {
            ...
            pauli_potential[ir]
                += factor
                   * (this->energy_exponent * this->enhancement_cpu_ptr[ir] + this->potGammaTerm (ir)
                      + this->potPTerm1 (ir) + this->potQTerm1 (ir) + this->potXiTerm1 (ir) + this->potTanhxiTerm1 (ir)
...
// IO tools
void
    ML_Base::loadVector (std::string filename, std::vector<double>& data)
{
...
void
    ML_Base::dumpVector (std::string filename, const std::vector<double>& data)
{
...
void
    ML_Base::dumpTensor (std::string filename, const torch::Tensor& data)
{
...
void
    ML_Base::dumpMatrix (std::string filename, const ModuleBase::matrix& data)
{
    std::cout << "Dumping " << filename << std::endl;
