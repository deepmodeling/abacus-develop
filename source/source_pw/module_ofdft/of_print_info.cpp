 ...
 */
void
    OFDFT::print_info (const int iter,
                       ...
                       const bool conv_esolver)
{
    ...
    std::cout << " " << std::setw (8) << iteration << std::setw (18) << std::scientific << std::setprecision (8)
              << energy_current * ModuleBase::Ry_to_eV << std::setw (18)
              << (energy_current - energy_last) * ModuleBase::Ry_to_eV << std::setw (13) << std::setprecision (4)
              << pelec->eferm.get_efval (0) * ModuleBase::Ry_to_eV << std::setw (13) << std::setprecision (4)
              << normdLdphi << std::setw (6) << std::fixed << std::setprecision (2) << duration << std::endl;
    ...
    if ((PARAM.inp.out_band[0] > 0
         && ((iter + 1) % PARAM.inp.out_band[0] == 0 || conv_esolver || iter == PARAM.inp.scf_nmax))
