...
void
    KEDF_ExtWT::update_rho0 (const double* const* prho, ModulePW::PW_Basis* pw_rho)
{
...
void
    KEDF_ExtWT::cal_kernel (double tf_weight, double vw_weight, double rho0, ModulePW::PW_Basis* pw_rho)
{
    this->rho0_ = rho0;
 ...
 */
void
    KEDF_ExtWT::update_dkernel_deta (const double& vw_weight, ModulePW::PW_Basis* pw_rho)
{
    ...
    for (int ig = 0; ig < pw_rho->npw; ++ig)
        {
            ...
            this->dkernel_deta_[ig] = -((this->alpha_ + this->beta_ - 5. / 3.) * this->kernel_[ig]
                                        + 1. / 3. * eta * this->diff_linhard (eta, vw_weight))
 ...
 */
double
    KEDF_ExtWT::get_energy (const double* const* prho, ModulePW::PW_Basis* pw_rho)
{
 ...
 */
double
    KEDF_ExtWT::get_energy_density (const double* const* prho, int is, int ir, ModulePW::PW_Basis* pw_rho)
{
    double** kernelRhoBeta = new double*[PARAM.inp.nspin];
    for (int is = 0; is < PARAM.inp.nspin; ++is)
        {
 ...
 */
void
    KEDF_ExtWT::tau_extwt (const double* const* prho, ModulePW::PW_Basis* pw_rho, double* rtau_extwt)
{
    double** kernelRhoBeta = new double*[PARAM.inp.nspin];
    for (int is = 0; is < PARAM.inp.nspin; ++is)
        {
 ...
 */
void
    KEDF_ExtWT::extwt_potential (const double* const* prho, ModulePW::PW_Basis* pw_rho, ModuleBase::matrix& rpotential)
{
            ...
            for (int ir = 0; ir < pw_rho->nrxx; ++ir)
                {
                    ...
                    if (second_term > 100 && count == 0)
                        {
                            count++;
                            std::cout << "Warning: second_term is too large: " << second_term << ", coef = " << coef
                                      << std::endl;
                        }
                    ...
                    rpotential (is, ir) += coef / this->sum_rho_kappa_
                                           * ((this->kappa_ + 1) * std::pow (prho[is][ir], this->kappa_ + 0.5)
                                              - this->kappa_ * second_term * this->rho0_)
                                           * 2.; // 2 is for 2 * phi
                }
        }
 ...
 */
void
    KEDF_ExtWT::get_stress (const double* const* prho, ModulePW::PW_Basis* pw_rho, double vw_weight)
{
 ...
 */
double
    KEDF_ExtWT::extwt_kernel (double eta, double tf_weight, double vw_weight)
{
    if (eta < 0.)
        {
            return 0.;
 ...
 */
double
    KEDF_ExtWT::diff_linhard (double eta, double vw_weight)
{
    if (eta < 0.)
        {
            return 0.;
 ...
 */
void
    KEDF_ExtWT::multi_kernel (const double* const* prho,
                              const double* kernel,
                              double** rkernel_rho,
                              double exponent,
                              ModulePW::PW_Basis* pw_rho)
 ...
 */
void
    KEDF_ExtWT::fill_kernel (double tf_weight, double vw_weight, ModulePW::PW_Basis* pw_rho)
{
    double eta = 0.;
    for (int ig = 0; ig < pw_rho->npw; ++ig)
        {
