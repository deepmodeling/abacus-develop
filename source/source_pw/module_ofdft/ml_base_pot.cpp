...
double
    ML_Base::potGammaTerm (int ir)
{
    return (this->ml_gamma)
               ? 1. / 3. * gamma[ir] * this->gradient_cpu_ptr[ir * this->ninput + this->descriptor2index["gamma"][0]]
               : 0.;
}
double
    ML_Base::potPTerm1 (int ir)
{
    return (this->ml_p) ? -8. / 3. * p[ir] * this->gradient_cpu_ptr[ir * this->ninput + this->descriptor2index["p"][0]]
                        : 0.;
}
double
    ML_Base::potQTerm1 (int ir)
{
    return (this->ml_q) ? -5. / 3. * q[ir] * this->gradient_cpu_ptr[ir * this->ninput + this->descriptor2index["q"][0]]
                        : 0.;
}
double
    ML_Base::potXiTerm1 (int ir)
{
    double result = 0.;
...
double
    ML_Base::potTanhpTerm1 (int ir)
{
    return (this->ml_tanhp) ? -8. / 3. * p[ir] * this->cal_tool->dtanh (this->tanhp[ir], this->chi_p)
                                  * this->gradient_cpu_ptr[ir * this->ninput + this->descriptor2index["tanhp"][0]]
                            : 0.;
}
double
    ML_Base::potTanhqTerm1 (int ir)
{
    return (this->ml_tanhq) ? -5. / 3. * q[ir] * this->cal_tool->dtanh (this->tanhq[ir], this->chi_q)
                                  * this->gradient_cpu_ptr[ir * this->ninput + this->descriptor2index["tanhq"][0]]
                            : 0.;
}

// Implementations of nl terms using energy_prefactor/exponent logic
void
    ML_Base::potGammanlTerm (const double* const* prho,
...
void
    ML_Base::potXinlTerm (const double* const* prho,
                          const std::vector<double>& tau_lda,
                          const ModulePW::PW_Basis* pw_rho,
    ...
    for (int ik = 0; ik < this->descriptor2kernel["xi"].size (); ++ik)
        {
            ...
            for (int ir = 0; ir < this->nx; ++ir)
                {
                    dFdxi[ir] = tau_lda[ir] / std::pow (prho[0][ir], 1. / 3.)
                                * this->gradient_cpu_ptr[ir * this->ninput + d2i];
                }
...
void
    ML_Base::potTanhxinlTerm (const double* const* prho,
                              const std::vector<double>& tau_lda,
                              const ModulePW::PW_Basis* pw_rho,
    ...
    for (int ik = 0; ik < this->descriptor2kernel["tanhxi"].size (); ++ik)
        {
            ...
            for (int ir = 0; ir < this->nx; ++ir)
                {
                    dFdtanhxi[ir] = tau_lda[ir] / std::pow (prho[0][ir], 1. / 3.)
                                    * this->cal_tool->dtanh (this->tanhxi[d2k][ir], this->chi_xi[d2k])
                                    * this->gradient_cpu_ptr[ir * this->ninput + d2i];
                }
...
void
    ML_Base::potTanhxi_nlTerm (const double* const* prho,
                               const std::vector<double>& tau_lda,
                               const ModulePW::PW_Basis* pw_rho,
    ...
    for (int ik = 0; ik < this->descriptor2kernel["tanhxi_nl"].size (); ++ik)
        {
            ...
            for (int ir = 0; ir < this->nx; ++ir)
                {
                    dFdtanhxi_nl[ir] *= this->cal_tool->dtanh (this->tanhxi[d2k][ir], this->chi_xi[d2k])
                                        / std::pow (prho[0][ir], 1. / 3.);
                }
...
void
    ML_Base::potPPnlTerm (const double* const* prho,
                          const std::vector<double>& tau_lda,
                          const ModulePW::PW_Basis* pw_rho,
            ...
            for (int ir = 0; ir < this->nx; ++ir)
                {
                    tempP[i][ir]
                        = (this->ml_p)
                              ? -this->pqcoef * 2.
                                    * this->gradient_cpu_ptr[ir * this->ninput + this->descriptor2index["p"][0]]
                                    * this->nablaRho[i][ir] * tau_lda[ir] / std::pow (prho[0][ir], 8. / 3.)
                              : 0.;
                    if (this->ml_pnl)
                        {
                            tempP[i][ir] += -this->pqcoef * 2. * this->nablaRho[i][ir] / std::pow (prho[0][ir], 8. / 3.)
                                            * dFdpnl_tot[ir];
                        }
                }
        }
...
void
    ML_Base::potQQnlTerm (const double* const* prho,
                          const std::vector<double>& tau_lda,
                          const ModulePW::PW_Basis* pw_rho,
                          std::vector<double>& rQQnlTerm)
{
    ...
    for (int ir = 0; ir < this->nx; ++ir)
        {
            tempQ[ir] = (this->ml_q)
                            ? this->pqcoef * this->gradient_cpu_ptr[ir * this->ninput + this->descriptor2index["q"][0]]
                                  * tau_lda[ir] / std::pow (prho[0][ir], 5. / 3.)
                            : 0.;
            if (this->ml_qnl)
                {
...
void
    ML_Base::potTanhpTanh_pnlTerm (const double* const* prho,
                                   const std::vector<double>& tau_lda,
                                   const ModulePW::PW_Basis* pw_rho,
            ...
            for (int ir = 0; ir < this->nx; ++ir)
                {
                    tempP[i][ir]
                        = (this->ml_tanhp)
                              ? -this->pqcoef * 2. * this->cal_tool->dtanh (this->tanhp[ir], this->chi_p)
                                    * this->gradient_cpu_ptr[ir * this->ninput + this->descriptor2index["tanhp"][0]]
                                    * this->nablaRho[i][ir] * tau_lda[ir] / std::pow (prho[0][ir], 8. / 3.)
                              : 0.;
                    if (this->ml_tanh_pnl)
...
void
    ML_Base::potTanhqTanh_qnlTerm (const double* const* prho,
                                   const std::vector<double>& tau_lda,
                                   const ModulePW::PW_Basis* pw_rho,
                                   std::vector<double>& rTanhqTanh_qnlTerm)
{
    ...
    for (int ir = 0; ir < this->nx; ++ir)
        {
            tempQ[ir] = (this->ml_tanhq)
                            ? this->pqcoef * this->cal_tool->dtanh (this->tanhq[ir], this->chi_q)
                                  * this->gradient_cpu_ptr[ir * this->ninput + this->descriptor2index["tanhq"][0]]
                                  * tau_lda[ir] / std::pow (prho[0][ir], 5. / 3.)
                            : 0.;
            if (this->ml_tanh_qnl)
...
// Note we assume that tanhp_nl and tanh_pnl will NOT be used together.
void
    ML_Base::potTanhpTanhp_nlTerm (const double* const* prho,
                                   const std::vector<double>& tau_lda,
                                   const ModulePW::PW_Basis* pw_rho,
...
void
    ML_Base::potTanhqTanhq_nlTerm (const double* const* prho,
                                   const std::vector<double>& tau_lda,
                                   const ModulePW::PW_Basis* pw_rho,
