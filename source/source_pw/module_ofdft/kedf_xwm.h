...
{
  public:
    KEDF_XWM () { this->stress.create (3, 3); }
    ~KEDF_XWM () {}
  ...
  private:
    void multi_kernel (const double* const* prho,
                       const double* kernel,
                       double** rkernel_rho,
                       double exponent,
                       ModulePW::PW_Basis* pw_rho);
    ...
    double tkf_ = 0.;     // 2 * kF
    double kappa_ = 0.;
    double c_kernel = std::pow (ModuleBase::PI, 4. / 3.) / std::pow (3., 1. / 3.)
                      * 2.;            // multiply by 2 to convert unit from Hartree to Ry
