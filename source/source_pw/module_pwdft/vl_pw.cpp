...
void
    pseudopot_cell_vl::init_vloc (const UnitCell& ucell, const ModulePW::PW_Basis* rho_basis)
{
    ...
    for (int it = 0; it < ucell.ntype; it++)
        {
            ...
            if (atom->coulomb_potential)
                ...
                {
                    this->vloc_of_g (atom->ncpp.msh, // after cutoff
                                     atom->ncpp.rab.data (),
                                     atom->ncpp.r.data (),
...
void
    pseudopot_cell_vl::allocate (const UnitCell& ucell, const int ngg)
{
...
void
    pseudopot_cell_vl::vloc_coulomb (const UnitCell& ucell,
                                     const double& zp_in,
                                     double* vloc_1d,
...
void
    pseudopot_cell_vl::vloc_of_g (const int& msh,
                                  const double* rab,
                                  const double* r,
...
void
    pseudopot_cell_vl::print_vloc (const UnitCell& ucell, const ModulePW::PW_Basis* rho_basis) const
{
            ...
            for (int it = 0; it < ucell.ntype; it++)
                {
                    ...
                    for (int ig = 0; ig < rho_basis->ngg; ig++)
                        {
                            ofs_vg << std::setw (15) << rho_basis->gg_uniq[ig] * ucell.tpiba2 << std::setw (15)
                                   << this->vloc (it, ig) << std::endl;
                        }
                    ofs_vg.close ();
                }
        }
