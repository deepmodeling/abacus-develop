...
void
    Parallel_Grid::init (const int& ncx_in,
                         const int& ncy_in,
                         const int& ncz_in,
                         const int& nczp_in,
                         const int& nrxx_in,
...
#ifdef __MPI
void
    Parallel_Grid::bcast (const double* const data_global, double* data_local, const int& rank) const
{
...
void
    Parallel_Grid::zpiece_to_all (double* zpiece, const int& iz, double* rho) const
{
    if (PARAM.inp.esolver_type == "sdft")
        {
...
#ifdef __MPI
void
    Parallel_Grid::zpiece_to_stogroup (double* zpiece, const int& iz, double* rho) const
{
    assert (allocate);
...
void
    Parallel_Grid::reduce (double* rhotot, const double* const rhoin, const bool reduce_all_pool) const
{
    ...
    for (int iz = 0; iz < this->ncz; iz++)
        {
            ...
            const int proc = this->whichpro[GlobalV::MY_POOL][iz];
            const int proc_loc
                = this->whichpro_loc[GlobalV::MY_POOL][iz]; // Obtain the local processor index in the pool
...
void
    Parallel_Grid::init_final_scf (const int& ncx_in,
                                   const int& ncy_in,
                                   const int& ncz_in,
                                   const int& nczp_in,
                                   const int& nrxx_in,
