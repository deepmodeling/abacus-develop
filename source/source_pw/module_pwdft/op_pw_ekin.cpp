...
Ekinetic<OperatorPW<T, Device>>::Ekinetic (Real tpiba2_in, const Real* gk2_in, const int gk2_row, const int gk2_col)
{
    ...
    if (this->tpiba2 < 1e-10 || this->gk2 == nullptr)
        {
            ModuleBase::WARNING_QUIT ("EkineticPW",
                                      "Constuctor of Operator::EkineticPW is failed, please check your code!");
        }
}
...
template <typename T, typename Device>
void
    Ekinetic<OperatorPW<T, Device>>::act (const int nbands,
                                          const int nbasis,
                                          const int npol,
                                          const T* tmpsi_in,
                                          T* tmhpsi,
...
hamilt::Ekinetic<OperatorPW<T, Device>>::Ekinetic (const Ekinetic<OperatorPW<T_in, Device_in>>* ekinetic)
{
    ...
    if (this->tpiba2 < 1e-10 || this->gk2 == nullptr)
        {
            ModuleBase::WARNING_QUIT ("EkineticPW",
                                      "Copy Constuctor of Operator::EkineticPW is failed, please check your code!");
        }
}
