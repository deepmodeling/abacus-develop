...
TEST (RadialProjectionTest, BuildSbtTabStabilityTest)
{
    ...
    std::transform (r.begin (),
                    r.end (),
                    aug1.begin (),
                    [sigma1] (double x) { return std::exp (-x * x / (2 * sigma1 * sigma1)); });
    std::transform (r.begin (),
                    r.end (),
                    aug2.begin (),
                    [sigma2] (double x) { return std::exp (-x * x / (2 * sigma2 * sigma2)); });
    std::transform (r.begin (),
                    r.end (),
                    aug3.begin (),
                    [sigma3] (double x) { return std::exp (-x * x / (2 * sigma3 * sigma3)); });
    ...
    std::for_each (q.begin (),
                   q.end (),
                   [&gen, &dis] (ModuleBase::Vector3<double>& x)
                       {
                           x[0] = dis (gen);
                           x[1] = dis (gen);
                           x[2] = dis (gen);
