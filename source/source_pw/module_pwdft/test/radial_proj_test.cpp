...
TEST(RadialProjectionTest, BuildBackwardMapTest) {
    const std::vector<std::vector<int>> it2iproj = {{0, 1}, {2, 3, 4, 5}, {6, 7, 8}};
...
TEST(RadialProjectionTest, BuildForwardMapTest) {
    ...
    const std::vector<std::vector<int>> it2ia = {{0, 1}, {0, 1, 3}, {12}};
    const std::vector<std::vector<int>> it2iproj = {{0, 1}, {2, 3, 4, 5}, {6, 7, 8}};
    const std::vector<int> iproj2l = {0, 1, 0, 1, 2, 3, 0, 1, 2};
...
TEST(RadialProjectionTest, BuildSbtTabStabilityTest) {
    ...
    std::transform(r.begin(), r.end(), aug1.begin(), [sigma1](double x) {
        return std::exp(-x * x / (2 * sigma1 * sigma1));
    });
    std::transform(r.begin(), r.end(), aug2.begin(), [sigma2](double x) {
        return std::exp(-x * x / (2 * sigma2 * sigma2));
    });
    std::transform(r.begin(), r.end(), aug3.begin(), [sigma3](double x) {
        return std::exp(-x * x / (2 * sigma3 * sigma3));
    });
    ...
    rp1._build_sbt_tab(r, std::vector<std::vector<double>>(1, mask), std::vector<int>(1, 0), 401, 0.01);
    ...
    rp2._build_sbt_tab(r, std::vector<std::vector<double>>(1, mask1), std::vector<int>(1, 1), 401, 0.01);
    ...
    rp3._build_sbt_tab(r, std::vector<std::vector<double>>(1, mask2), std::vector<int>(1, 2), 401, 0.01);
    ...
    rp4._build_sbt_tab(r, std::vector<std::vector<double>>(1, mask3), std::vector<int>(1, 3), 401, 0.01);
