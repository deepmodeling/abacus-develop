...
void
    EXPECT_COMPLEX_DOUBLE_EQ (const std::complex<double>& a, const std::complex<double>& b)
{
    ...
    for (int i = 1; i < 2 * l + 1; i += 2)
        {
            ...
            EXPECT_COMPLEX_DOUBLE_EQ (soc.p_rot[l2p1 * (i + 1) + n],
                                      std::complex<double> (0.0, -pow (-1.0, m) / sqrt (2)));
            n = l + m;
