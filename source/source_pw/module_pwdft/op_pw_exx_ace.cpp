...
void
    OperatorEXXPW<T, Device>::act_op_ace (const int nbands,
                                          const int nbasis,
                                          const int npol,
                                          const T* tmpsi_in,
                                          T* tmhpsi,
...
template <typename T, typename Device>
void
    OperatorEXXPW<T, Device>::construct_ace () const
{
                    ...
                    if (skip_ik)
                                    ...
                                    for (int m_iband = 0; m_iband < psi.get_nbands (); m_iband++)
                                        {
                                            ...
                                            MPI_Bcast (&wg_mqb,
                                                       1,
                                                       MPI_DOUBLE,
                                                       kv->para_k.get_startpro_pool (iq_pool),
                                                       MPI_COMM_WORLD);
#endif
                                            if (wg_mqb < 1e-12)
                                                continue;
...
#ifdef __MPI
                                            Parallel_Common::bcast_dev<T, Device> (psi_mq_real,
                                                                                   wfcpw->nrxx,
                                                                                   KP_WORLD,
                                                                                   iq_pool);
#endif
                    ...
                    else
                        {
                            *ik_ = ik;
                            ...
                            //         {
                            //             std::cout << L_ace[i * nbands + j].real() << "+" << L_ace[i * nbands +
                            //             j].imag() << "im ";
                            //         }
                            //         else
                            //         {
                            //             std::cout << L_ace[i * nbands + j].real() << L_ace[i * nbands + j].imag() <<
                            //             "im ";
                            //         }
                            //     }
                            //     std::cout << ";" << std::endl;
...
template <typename T, typename Device>
double
    OperatorEXXPW<T, Device>::cal_exx_energy_ace (psi::Psi<T, Device>* ppsi_) const
{
    double Eexx = 0;
    int nspin_fac = PARAM.inp.nspin == 2 ? 2 : 1;
