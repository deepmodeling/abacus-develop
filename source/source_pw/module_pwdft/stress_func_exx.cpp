...
#pragma omp parallel for reduction(+:div)
    ...
    for (int ik = 0; ik < nks; ik++)
    {
        for (int nband = 0; nband < d_psi_in->get_nbands(); nband++)
        {
            ...
            wfcpw->recip2real_impl<std::complex<FPTYPE>,base_device::DEVICE_CPU>(psi_nk, psi_nk_real, ik);
            ...
            for (int iq = 0; iq < nqs; iq++)
            {
                for (int mband = 0; mband < d_psi_in->get_nbands(); mband++)
                {
                    ...
                    wfcpw->recip2real_impl<std::complex<FPTYPE>,base_device::DEVICE_CPU>(psi_mq, psi_mq_real, iq);
