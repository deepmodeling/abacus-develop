                                    ...
                                    const bool nonlocal_in)
{
    if (wfcpw_in == nullptr || isk_in == nullptr || ppcell_in == nullptr || ucell_in == nullptr)
        {
            ModuleBase::WARNING_QUIT ("Velocity",
                                      "Constuctor of Operator::Velocity is failed, please check your code!");
        }
    this->wfcpw = wfcpw_in;
    this->isk = isk_in;
...
template <typename FPTYPE, typename Device>
void
    Velocity<FPTYPE, Device>::init (const int ik_in)
{
    this->ik = ik_in;
    // init G+K
...
template <typename FPTYPE, typename Device>
void
    Velocity<FPTYPE, Device>::act (const psi::Psi<std::complex<FPTYPE>, Device>* psi_in,
                                   const int n_npwx,
    ...
    if (n_npwx == 1)
        {
            ...
            ModuleBase::gemv_op<Complex,
                                Device> () ('C', npw, nkb, &one, vkb_d, max_npw, psi0, inc, &zero, becp1_, inc);
            ModuleBase::gemv_op<Complex,
                                Device> () ('C', npw, nkb3, &one, gradvkb_d, max_npw, psi0, inc, &zero, becp2_, inc);
        }
    else
        {
                                    ...
                                    for (int ip2 = 0; ip2 < nproj; ip2++)
                                        {
                                            for (int ib = 0; ib < n_npwx; ++ib)
                                                {
                                                    FPTYPE dij = static_cast<FPTYPE> (
                                                        this->ppcell->deeq (current_spin, iat, ip, ip2));
                                                    int sumip2 = sum + ip2;
                                                    int sumip = sum + ip;
                                                    ps1_cpu[sumip2 * n_npwx + ib] += dij * becp1_cpu[ib * nkb + sumip];
                                                    ps2_cpu[sumip2 * n_npwx + ib] += dij * becp2_cpu[ib * nkb3 + sumip];
                                                    ps2_cpu[(sumip2 + nkb) * n_npwx + ib]
                                                        += dij * becp2_cpu[ib * nkb3 + sumip + nkb];
                                                    ps2_cpu[(sumip2 + 2 * nkb) * n_npwx + ib]
                                                        += dij * becp2_cpu[ib * nkb3 + sumip + 2 * nkb];
                                                }
                                        }
                                }
                            sum += nproj;
