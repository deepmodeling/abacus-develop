...
template <typename FPTYPE, typename Device>
void
    FS_Kin_tools<FPTYPE, Device>::cal_gk (const int& ik)
{
            ...
            if (wfc_basis_->erf_height > 0)
                {
                    ...
                    kfac[i]
                        = 1.0 + wfc_basis_->erf_height / wfc_basis_->erf_sigma * twobysqrtpi * std::exp (-arg * arg);
                }
            else
                {
                    kfac[i] = 1.0;
...
template <typename FPTYPE, typename Device>
void
    FS_Kin_tools<FPTYPE, Device>::cal_stress_kin (const int& ik,
                                                  const int& npm,
                                                  const bool& occ,
...
template <typename FPTYPE, typename Device>
void
    FS_Kin_tools<FPTYPE, Device>::symmetrize_stress (ModuleSymmetry::Symmetry* p_symm, ModuleBase::matrix& sigma)
{
    for (int l = 0; l < 3; ++l)
        {
            for (int m = 0; m < l; ++m)
