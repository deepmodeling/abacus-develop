        ...
        for (int ig = 0; ig < rho_basis->npw; ig++) {
            ...
            for (int it = 0; it < ucell.ntype; it++) {
                for (int i = 0; i < ucell.atoms[it].na; i++) {
                    ...
                    rhostar =
                        rhostar + std::complex<FPTYPE>(ucell.atoms[it].ncpp.zv * cosp, ucell.atoms[it].ncpp.zv * sinp);
                }
            }
            rhostar /= ucell.omega;
            ...
            for (int l = 0; l < 3; l++) {
                for (int m = 0; m < l + 1; m++) {
                    local_sigma(l, m) +=
                        sewald * ucell.tpiba2 * 2.0 * rho_basis->gcar[ig][l] * rho_basis->gcar[ig][m] / g2 * (g2a + 1);
                }
            }
        }
            ...
            for (long long ijat = 0; ijat < ucell.nat * ucell.nat; ijat++) {
                ...
                if (ucell.atoms[it].na != 0 && ucell.atoms[jt].na != 0) {
                    ...
                    for (int nr = 0; nr < nrm; nr++) {
                        ...
                        fac = -ModuleBase::e2 / 2.0 / ucell.omega * pow(ucell.lat0, 2) * ucell.atoms[it].ncpp.zv *
                              ucell.atoms[jt].ncpp.zv / pow(rr, 3) *
                              (erfc(sqa * rr) + rr * sq8a_2pi * ModuleBase::libm::exp(-alpha * pow(rr, 2)));
