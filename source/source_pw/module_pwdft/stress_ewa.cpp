...
void
    Stress_Func<FPTYPE, Device>::stress_ewa (const UnitCell& ucell,
    ...
    do
        {
            ...
            upperbound = ModuleBase::e2 * pow (charge, 2) * sqrt (2 * alpha / (ModuleBase::TWO_PI))
                        ...
                        for (int i = 0; i < ucell.atoms[it].na; i++)
                            {
                                ...
                                rhostar = rhostar
                                          + std::complex<FPTYPE> (ucell.atoms[it].ncpp.zv * cosp,
                                                                  ucell.atoms[it].ncpp.zv * sinp);
                            }
                    }
                rhostar /= ucell.omega;
                sewald = fact * (ModuleBase::TWO_PI)*ModuleBase::e2 * ModuleBase::libm::exp (-g2a) / g2
                         * pow (std::abs (rhostar), 2);
                local_sdewald -= sewald;
                for (int l = 0; l < 3; l++)
                    {
                        for (int m = 0; m < l + 1; m++)
                            {
                                local_sigma (l, m) += sewald * ucell.tpiba2 * 2.0 * rho_basis->gcar[ig][l]
                                                      * rho_basis->gcar[ig][m] / g2 * (g2a + 1);
                            }
                    }
            }
                                ...
                                for (int nr = 0; nr < nrm; nr++)
                                    {
                                        ...
                                        fac = -ModuleBase::e2 / 2.0 / ucell.omega * pow (ucell.lat0, 2)
                                              * ucell.atoms[it].ncpp.zv * ucell.atoms[jt].ncpp.zv / pow (rr, 3)
                                              * (erfc (sqa * rr)
                                                 + rr * sq8a_2pi * ModuleBase::libm::exp (-alpha * pow (rr, 2)));
