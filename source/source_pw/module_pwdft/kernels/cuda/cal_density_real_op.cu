...
__global__ void
    cal_density_real_kernel (const thrust::complex<FPTYPE>* in1,
...
struct cal_density_real_op<std::complex<FPTYPE>, base_device::DEVICE_GPU>
{
    ...
    void
        operator() (const T* psi1, const T* psi2, T* out, double omega, int nrxx)
    {
        int threads_per_block = 256;
        int num_blocks = (nrxx + threads_per_block - 1) / threads_per_block;

        cal_density_real_kernel<FPTYPE>
            <<<num_blocks, threads_per_block>>> (reinterpret_cast<const thrust::complex<FPTYPE>*> (psi1),
        ...
        if (err != cudaSuccess)
            {
                throw std::runtime_error ("CUDA error in cal_density_real_kernel: "
