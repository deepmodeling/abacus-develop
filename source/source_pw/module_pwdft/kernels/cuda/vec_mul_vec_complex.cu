...
__global__ void
    vec_mul_vec_complex_kernel (const thrust::complex<FPTYPE>* vec1,
...
struct vec_mul_vec_complex_op<std::complex<FPTYPE>, base_device::DEVICE_GPU>
{
    ...
    void
        operator() (const T* a, const T* b, T* out, int size)
    {
        int threads_per_block = 256;
        int num_blocks = (size + threads_per_block - 1) / threads_per_block;

        vec_mul_vec_complex_kernel<FPTYPE>
            <<<num_blocks, threads_per_block>>> (reinterpret_cast<const thrust::complex<FPTYPE>*> (a),
        ...
        if (err != cudaSuccess)
            {
                throw std::runtime_error ("CUDA error in vec_mul_vec_kernel: "
