...
__global__ void
    mul_potential_kernel (const FPTYPE* pot_shifted, thrust::complex<FPTYPE>* density_recip, int npw)
{
...
struct mul_potential_op<std::complex<FPTYPE>, base_device::DEVICE_GPU>
{
    ...
    void
        operator() (const FPTYPE* pot, T* density_recip, int npw, int nks, int ik, int iq)
    {
        // #ifdef _OPENMP
        ...
        if (err != cudaSuccess)
            {
                throw std::runtime_error ("CUDA error in mul_potential_kernel: "
