...
template <typename FPTYPE>
__global__ void
    cal_force_nl (const bool nondiagonal,
                  const int ntype,
                  const int spin,
                  const int deeq_2,
                  const int deeq_3,
            ...
            for (int ip = threadIdx.x; ip < nproj; ip += blockDim.x)
                {
                    ...
                    for (int ipol = 0; ipol < 3; ipol++)
                        {
                            const FPTYPE dbb
                                = (conj (dbecp[ipol * nbands * nkb + ib * nkb + inkb]) * becp[ib * nkb + inkb]).real ();
...
template <typename FPTYPE>
void
    cal_vkb1_nl_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* ctx,
                                                                 const int& nkb,
                                                                 const int& npwx,
                                                                 const int& vkb_nc,
                                                                 const int& nbasis,
...
template <typename FPTYPE>
void
    cal_force_nl_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* ctx,
                    ...
                    for (int ip2 = 0; ip2 < nproj; ip2++)
                        {
                            ...
                            const thrust::complex<FPTYPE> ps0
                                = deeq_nc[((0 * deeq_2 + iat) * deeq_3 + ip) * deeq_4 + ip2] + ps_qq;
                            const thrust::complex<FPTYPE> ps1
                                = deeq_nc[((1 * deeq_2 + iat) * deeq_3 + ip) * deeq_4 + ip2];
                            const thrust::complex<FPTYPE> ps2
                                = deeq_nc[((2 * deeq_2 + iat) * deeq_3 + ip) * deeq_4 + ip2];
                            const thrust::complex<FPTYPE> ps3
                                = deeq_nc[((3 * deeq_2 + iat) * deeq_3 + ip) * deeq_4 + ip2] + ps_qq;
                            ...
                            for (int ipol = 0; ipol < 3; ipol++)
                                {
                                    ...
                                    const thrust::complex<FPTYPE> dbb3
                                        = conj (dbecp[index0 + nkb]) * becp[index1 + nkb];
                                    const FPTYPE tmp
                                        = -fac * (ps0 * dbb0 + ps1 * dbb1 + ps2 * dbb2 + ps3 * dbb3).real ();
...
{
    cal_force_nl<FPTYPE>
        <<<nbands_occ * ntype, THREADS_PER_BLOCK>>> (ntype,
                                                     deeq_2,
                                                     deeq_3,
                                                     deeq_4,
                                                     forcenl_nc,
                    ...
                    for (int ipol = 0; ipol < 3; ipol++)
                        {
                            ...
                            const FPTYPE tmp
                                = -fac * (ps[0] * dbb0 + ps[1] * dbb1 + ps[2] * dbb2 + ps[3] * dbb3).real ();
                            atomicAdd (force + iat * forcenl_nc + ipol, tmp);
                        }
                }
            ++iat;
...
template <typename FPTYPE>
__global__ void
    saveVkbValues_ (const int* gcar_zero_ptrs,
...
template <typename FPTYPE>
void
    saveVkbValues (const int* gcar_zero_ptrs,
...
template <typename FPTYPE>
__global__ void
    revertVkbValues_ (const int* gcar_zero_ptrs,
...
template <typename FPTYPE>
void
    revertVkbValues (const int* gcar_zero_ptrs,
...
template <typename FPTYPE>
__global__ void
    force_loc_kernel (const int nat,
                      const int npw,
    ...
    for (int ig = tid; ig < npw; ig += blockDim.x)
        {
            ...
            const FPTYPE phase = ModuleBase::TWO_PI * (gcar_x * tau_x + gcar_y * tau_y + gcar_z * tau_z);
            FPTYPE sinp, cosp;
...
template <typename FPTYPE>
void
    cal_force_loc_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const int nat,
                                                                   const int npw,
                                                                   const FPTYPE tpiba_omega,
                                                                   const int* iat2it,
                                                                   const int* ig2igg,
                                                                   ...
                                                                   FPTYPE* forcelc)
{
    force_loc_kernel<FPTYPE><<<nat, THREADS_PER_BLOCK>>> (nat,
                                                          npw,
                                                          tpiba_omega,
                                                          iat2it,
                                                          ig2igg,
...
template <typename FPTYPE>
void
    cal_force_ew_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const int nat,
                                                                  const int npw,
                                                                  const int ig_gge0,
                                                                  const int* iat2it,
                                                                  const FPTYPE* gcar,
                                                                  ...
                                                                  FPTYPE* forceion)
{
    force_ew_kernel<FPTYPE><<<nat, THREADS_PER_BLOCK>>> (nat,
                                                         npw,
                                                         ig_gge0,
                                                         iat2it,
                                                         gcar,
