...
template <typename T>
__device__ static inline thrust::complex<T>
    conj (thrust::complex<T>& in)
{
    return thrust::conj (in);
}
template <typename T>
__global__ void
    cal_stress_mgga (const int spin, const int nrxx, const T w1, const thrust::complex<T>* gradwfc, T* crosstaus)
{
...
template <typename FPTYPE>
__global__ void
    cal_dbecp_noevc_nl (const int ipol,
                        const int jpol,
                        const int npw,
                        const int npwx,
                        const int ik,
    ...
    for (int ig = threadIdx.x; ig < npw; ig += blockDim.x)
        {
            ...
            pvkb[ig] += 0.5 * qvec[ipol] * pvkb0j[ig] + 0.5 * qvec[jpol] * pvkb0i[ig];
            ...
            pdbecp_noevc[ig] -= 2.0 * pvkb[ig] * qvec[ipol] * qvec[jpol] * qm1 * tpiba;
        } // end ig
}

template <typename FPTYPE>
__global__ void
    cal_stress_nl (const bool nondiagonal,
                   const int ipol,
                   const int jpol,
                   const int nkb,
                   const int ntype,
...
template <typename FPTYPE>
void
    cal_dbecp_noevc_nl_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* ctx,
                                                                        const int& ipol,
                                                                        const int& jpol,
                                                                        const int& nkb,
                                                                        const int& npw,
...
{
    cal_dbecp_noevc_nl<FPTYPE><<<nkb, THREADS_PER_BLOCK>>> (ipol,
                                                            jpol,
                                                            npw,
                                                            npwx,
                                                            ik,
...
template <typename FPTYPE>
void
    cal_stress_nl_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* ctx,
            ...
            for (int ii = threadIdx.x; ii < nproj * nproj; ii += blockDim.x)
                {
                    ...
                    if (ekb_now != 0)
                        {
                            ps_qq
                                = thrust::complex<FPTYPE> (-ekb_now * qq_nt[it * deeq_3 * deeq_4 + ip1 * deeq_4 + ip2],
                                                           0.0);
                        }
                    ...
                    const thrust::complex<FPTYPE> dbb1
                        = conj (dbecp[ib2 * nkb + inkb1]) * becp[(ib2 + 1) * nkb + inkb2];
                    const thrust::complex<FPTYPE> dbb2
                        = conj (dbecp[(ib2 + 1) * nkb + inkb1]) * becp[ib2 * nkb + inkb2];
                    const thrust::complex<FPTYPE> dbb3
...
{
    cal_stress_nl<FPTYPE>
        <<<nbands_occ * ntype, THREADS_PER_BLOCK>>> (ipol,
                                                     jpol,
                                                     nkb,
                                                     ntype,
                                                     deeq_2,
...
template <typename FPTYPE>
FPTYPE
    cal_multi_dot_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const int& npw,
                                                                   const FPTYPE& fac,
                                                                   const FPTYPE* gk1,
                                                                   const FPTYPE* gk2,
                                                                   const FPTYPE* d_kfac,
                                                                   const std::complex<FPTYPE>* psi)
{
    ...
    cal_multi_dot<FPTYPE><<<block, THREADS_PER_BLOCK>>> (npw,
                                                         fac,
                                                         gk1,
                                                         gk2,
                                                         d_kfac,
...
void
    cal_stress_mgga_op<T, Device>::operator() (const int& spin,
                                               const int& nrxx,
                                               const Real& w1,
                                               const T* gradwfc,
                                               Real* crosstaus)
{
    ...
    cal_stress_mgga<Real><<<block, THREADS_PER_BLOCK>>> (spin, nrxx, w1, gradwfc_, crosstaus);
    ...
    if (idx < npw)
        {
            ...
            vkb_ptr[idx] -= (gk[ipol] * ylm_deri_ptr2[idx] + gk[jpol] * ylm_deri_ptr1[idx]) * vq_ptr[idx] * sk_in[idx]
                            * pref_in[ih];

            vkb_ptr[idx]
                -= 2.0 * ylm_ptr[idx] * vq_deri_ptr[idx] * sk_in[idx] * pref_in[ih] * gk[ipol] * gk[jpol] * gkn[idx];
        }
}

template <typename FPTYPE>
__global__ void
    cal_vq (const FPTYPE* tab,
            int it,
            const FPTYPE* gk,
            int npw,
            const int tab_2,
            ...
            FPTYPE* vq)
{
    ...
    if (idx < npw)
        vq_ptr[idx] = _polynomial_interpolation (tab, it, ib, tab_2, tab_3, table_interval, gnorm[idx]);
}

template <typename FPTYPE>
__global__ void
    cal_vq_deri (const FPTYPE* tab,
                 int it,
                 const FPTYPE* gk,
                 int npw,
                 const int tab_2,
                 ...
                 FPTYPE* vq)
{
    ...
    const FPTYPE* gnorm = &gk[3 * npw];
    if (idx < npw)
        vq_ptr[idx] = _polynomial_interpolation_nl (tab, it, ib, tab_2, tab_3, table_interval, gnorm[idx]);
}

template <typename FPTYPE>
__global__ void
    cal_stress_drhoc_aux0 (const FPTYPE* r,
                           const FPTYPE* rhoc,
                           const FPTYPE* gx_arr,
                           const FPTYPE* rab,
                           FPTYPE* drhocg,
...
template <typename FPTYPE>
__global__ void
    cal_stress_drhoc_aux1 (const FPTYPE* r,
                           const FPTYPE* rhoc,
                           const FPTYPE* gx_arr,
                           const FPTYPE* rab,
                           FPTYPE* drhocg,
...
template <typename FPTYPE>
__global__ void
    cal_stress_drhoc_aux2 (const FPTYPE* r,
                           const FPTYPE* rhoc,
                           const FPTYPE* gx_arr,
                           const FPTYPE* rab,
                           FPTYPE* drhocg,
...
template <typename FPTYPE>
__global__ void
    cal_stress_drhoc_aux3 (const FPTYPE* r,
                           const FPTYPE* rhoc,
                           const FPTYPE* gx_arr,
                           const FPTYPE* rab,
                           FPTYPE* drhocg,
...
template <typename FPTYPE>
__global__ void
    cal_force_npw (const thrust::complex<FPTYPE>* psiv,
                   const FPTYPE* gv,
    ...
    for (int ig = tid; ig < npw; ig += blockDim.x)
        {
            ...
            const FPTYPE arg
                = ModuleBase::TWO_PI * (gv[ig * 3] * pos_x + gv[ig * 3 + 1] * pos_y + gv[ig * 3 + 2] * pos_z);
            FPTYPE sinp, cosp;
...
template <typename FPTYPE>
void
    cal_vkb_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* ctx,
                                                             const int nh,
                                                             const int npw,
                                                             const int* indexes,
                                                             const FPTYPE* vqs_in,
                                                             ...
                                                             std::complex<FPTYPE>* vkbs_out)
{
    ...
    cal_vkb<FPTYPE><<<gridsize, THREADS_PER_BLOCK>>> (npw,
                                                      indexes,
                                                      vqs_in,
                                                      ylms_in,
...
template <typename FPTYPE>
void
    cal_vkb_deri_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* ctx,
                                                                  const int nh,
                                                                  const int npw,
                                                                  const int ipol,
                                                                  const int jpol,
                                                                  ...
                                                                  std::complex<FPTYPE>* vkbs_out)
{
    ...
    cal_vkb_deri<FPTYPE><<<gridsize, THREADS_PER_BLOCK>>> (npw,
                                                           ipol,
                                                           jpol,
                                                           indexes,
                                                           vqs_in,
...
template <typename FPTYPE>
void
    cal_vq_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* ctx,
                                                            const FPTYPE* tab,
                                                            int it,
                                                            const FPTYPE* gk,
                                                            int npw,
                                                            ...
                                                            FPTYPE* vq)
{
    ...
    cal_vq<FPTYPE><<<gridsize, THREADS_PER_BLOCK>>> (tab, it, gk, npw, tab_2, tab_3, table_interval, nbeta, vq);
}

template <typename FPTYPE>
void
    cal_vq_deri_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* ctx,
                                                                 const FPTYPE* tab,
                                                                 int it,
                                                                 const FPTYPE* gk,
                                                                 int npw,
                                                                 ...
                                                                 FPTYPE* vq)
{
    ...
    cal_vq_deri<FPTYPE><<<gridsize, THREADS_PER_BLOCK>>> (tab, it, gk, npw, tab_2, tab_3, table_interval, nbeta, vq);
...
template <typename FPTYPE>
void
    cal_stress_drhoc_aux_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const FPTYPE* r,
                                                                          const FPTYPE* rhoc,
                                                                          const FPTYPE* gx_arr,
                                                                          const FPTYPE* rab,
                                                                          FPTYPE* drhocg,
                                                                          ...
                                                                          int type)
{
    ...
    if (type == 0)
        {
            cal_stress_drhoc_aux0<FPTYPE>
                <<<block, THREADS_PER_BLOCK>>> (r, rhoc, gx_arr, rab, drhocg, mesh, igl0, ngg, omega);
        }
    else if (type == 1)
        {
            cal_stress_drhoc_aux1<FPTYPE>
                <<<block, THREADS_PER_BLOCK>>> (r, rhoc, gx_arr, rab, drhocg, mesh, igl0, ngg, omega);
        }
    else if (type == 2)
        {
            cal_stress_drhoc_aux2<FPTYPE>
                <<<block, THREADS_PER_BLOCK>>> (r, rhoc, gx_arr, rab, drhocg, mesh, igl0, ngg, omega);
        }
    else if (type == 3)
        {
            cal_stress_drhoc_aux3<FPTYPE>
                <<<block, THREADS_PER_BLOCK>>> (r, rhoc, gx_arr, rab, drhocg, mesh, igl0, ngg, omega);
        }
    return;
}
...
template <typename FPTYPE>
void
    cal_force_npw_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const std::complex<FPTYPE>* psiv,
                                                                   const FPTYPE* gv,
                                                                   const FPTYPE* rhocgigg_vec,
                                                                   FPTYPE* force,
                                                                   const FPTYPE* tau,
                                                                   ...
                                                                   const int na)
{
    cal_force_npw<<<na, THREADS_PER_BLOCK>>> (reinterpret_cast<const thrust::complex<FPTYPE>*> (psiv),
                                              gv,
                                              rhocgigg_vec,
                                              force,
                                              tau,
...
template <>
void
    pointer_array_malloc<base_device::DEVICE_GPU>::operator() (void** ptr, const int n)
{
...
template <>
void
    synchronize_ptrs<base_device::DEVICE_GPU>::operator() (void** ptr_out, const void** ptr_in, const int size)
{
...
template <typename FPTYPE>
__global__ void
    cal_stress_onsite (const int nkb,
                       const int ntype,
                       const int wg_nc,
                       const int ik,
                       const int* atom_nh,
...
void
    cal_stress_nl_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* ctx,
                                                                   ...
                                                                   FPTYPE* stress)
{
    cal_stress_onsite<FPTYPE>
        <<<nbands_occ * ntype, THREADS_PER_BLOCK>>> (nkb,
                                                     ntype,
                                                     wg_nc,
                                                     ik,
                                                     atom_nh,
