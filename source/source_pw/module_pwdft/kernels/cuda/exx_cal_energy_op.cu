...
__global__ void
    cal_vec_norm_kernel (const thrust::complex<FPTYPE>* den, const FPTYPE* pot, FPTYPE* result, int npw)
{
...
struct exx_cal_energy_op<std::complex<FPTYPE>, base_device::DEVICE_GPU>
{
    ...
    operator() (const T * den, const FPTYPE * pot, FPTYPE scalar, int npw)
    {
        ...
        // FPTYPE result = exx_cal_energy_op<std::complex<FPTYPE>, base_device::DEVICE_CPU>()(den_cpu, pot_cpu, scalar,
        // npw); delete[] den_cpu; delete[] pot_cpu; return result;
        FPTYPE result = 0.0;
        ...
        cal_vec_norm_kernel<FPTYPE>
            <<<num_blocks, threads_per_block>>> (reinterpret_cast<const thrust::complex<FPTYPE>*> (den),
                                                 pot,
                                                 d_result,
                                                 npw);
        ...
        if (err != cudaSuccess)
            {
                throw std::runtime_error ("CUDA error in cal_vec_norm_kernel: "
