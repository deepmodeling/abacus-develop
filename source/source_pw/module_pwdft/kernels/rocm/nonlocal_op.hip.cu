    ...
    for (int kk = threadIdx.x; kk < l3; kk += blockDim.x)
        {
            ...
            for (int xx = 0; xx < l3; xx++)
                {
                    res += deeq[((spin * deeq_x + iat + ii) * deeq_y + xx) * deeq_z + kk]
...
template <typename FPTYPE>
void
    hamilt::nonlocal_pw_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* dev,
                                                                         const int& l1,
                                                                         const int& l2,
                                                                         const int& l3,
                                                                         int& sum,
                                                                         ...
                                                                         const std::complex<FPTYPE>* becp)
{
    ...
    // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    hipLaunchKernelGGL (HIP_KERNEL_NAME (nonlocal_pw<FPTYPE>),
                        dim3 (l1 * l2),
