             ...
             const bool add)
{
    ...
    FPTYPE fact = (gcar[(ik * npwx + ig) * 3 + pol] + kvec_c[ik * 3 + pol]) * tpiba;
    if (add)
        {
...
template <typename FPTYPE>
void
    meta_pw_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* dev,
                                                             const int& ik,
                                                             const int& pol,
                                                             const int& npw,
                                                             const int& npwx,
                                                             ...
                                                             const bool add)
{
    ...
    hipLaunchKernelGGL (HIP_KERNEL_NAME (meta_pw<FPTYPE>),
                        dim3 (block),
