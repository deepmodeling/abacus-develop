    ...
    for (int ib = threadIdx.x; ib < nbands; ib += blockDim.x)
        {
            ...
            ps[psind + 1]
                += lambda_coeff[iat * 4 + 1] * becp[becpind] + lambda_coeff[iat * 4 + 3] * becp[becpind + tnp];
        }
}
...
template <typename FPTYPE>
void
    hamilt::onsite_ps_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* dev,
                                                                       const int& npm,
                                                                       const int npol,
                                                                       const int* ip_iat,
                                                                       const int& tnp,
                                                                       ...
                                                                       const std::complex<FPTYPE>* becp)
{
    ...
    // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    hipLaunchKernelGGL (HIP_KERNEL_NAME (onsite_op<FPTYPE>),
                        dim3 (tnp),
