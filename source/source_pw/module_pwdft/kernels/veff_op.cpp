...
{
    void
        operator() (const base_device::DEVICE_CPU* dev, const int& size, std::complex<FPTYPE>* out, const FPTYPE* in)
    {
#ifdef _OPENMP
        ...
        for (int ir = 0; ir < size; ir++)
            {
                auto sup = out[ir] * (in[0][ir] + in[3][ir])
                           + out1[ir] * (in[1][ir] - std::complex<FPTYPE> (0.0, 1.0) * in[2][ir]);
                auto sdown = out1[ir] * (in[0][ir] - in[3][ir])
                             + out[ir] * (in[1][ir] + std::complex<FPTYPE> (0.0, 1.0) * in[2][ir]);
                out[ir] = sup;
                out1[ir] = sdown;
            }
    }
