                            ...
                            for (int ip = 0; ip < nproj; ip++)
                                {
                                    ...
                                    for (int ipol = 0; ipol < 3; ipol++)
                                        {
#ifdef __SW
                                            ModuleBase::truncated_underflow (
                                                dbecp[ipol * nbands * nkb + ib * nkb + inkb]);
                                                    ...
                                                    if (ip != ip2)
                                                        {
                                                            ...
                                                            if (ekb_now != 0)
                                                                {
                                                                    ps_qq = -ekb_now
                                                                            * qq_nt[it * deeq_3 * deeq_4 + ip * deeq_4
                                                                                    + ip2];
                                                                }
                                                            FPTYPE ps
                                                                = deeq[((spin * deeq_2 + iat) * deeq_3 + ip) * deeq_4
                                                                       + ip2]
                                                                  + ps_qq;
                                                            for (int ipol = 0; ipol < 3; ipol++)
                                                                {
                                                                    const FPTYPE dbb = (conj (dbecp[ipol * nbands * nkb
                                                                                                    + ib * nkb + inkb])
                                                                                        * becp[ib * nkb + jnkb])
                                                                                           .real ();
                                                                    local_force[ipol] -= ps * fac * dbb;
                                        ...
                                        for (int ipol = 0; ipol < 3; ipol++)
                                            {
                                                ...
                                                const std::complex<FPTYPE> dbb1
                                                    = conj (dbecp[index0]) * becp[index1 + nkb];
                                                const std::complex<FPTYPE> dbb2
                                                    = conj (dbecp[index0 + nkb]) * becp[index1];
                                                const std::complex<FPTYPE> dbb3
                                                    = conj (dbecp[index0 + nkb]) * becp[index1 + nkb];

                                                local_force[ipol]
                                                    -= fac
                                                       * (ps0 * dbb0 + ps1 * dbb1 + ps2 * dbb2 + ps3 * dbb3).real ();
                                            }
                                    }
                            }
#ifdef _OPENMP
