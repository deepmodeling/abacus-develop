                ...
                for (int ig = 0; ig < npw; ig++)
                    {
                        ...
                        pdbecp_noevc[ig]
                            -= static_cast<FPTYPE> (2.0) * pvkb2[ig] * qvec[ipol] * qvec[jpol] * qm1 * tpiba;
                    } // end ig
            } // end nkb
    }
};
                                            ...
                                            for (int ip2 = 0; ip2 < nproj; ip2++)
                                                {
                                                    ...
                                                    if (ekb_now != 0)
                                                        {
                                                            ps_qq = -ekb_now
                                                                    * qq_nt[it * deeq_3 * deeq_4 + ip1 * deeq_4 + ip2];
                                                        }
                                                    FPTYPE ps
                                                        = deeq[((spin * deeq_2 + iat + ia) * deeq_3 + ip1) * deeq_4
                                                               + ip2]
                                                          + ps_qq;
...
#endif
                                                    const FPTYPE dbb
                                                        = (conj (dbecp[ib * nkb + inkb1]) * becp[ib * nkb + inkb2])
                                                              .real ();
                                            ...
                                            for (int ip2 = 0; ip2 < nproj; ip2++)
                                                {
                                                    ...
                                                    std::complex<FPTYPE> ps0
                                                        = deeq_nc[((iat + ia) * deeq_3 + ip1) * deeq_4 + ip2] + ps_qq;
                                                    std::complex<FPTYPE> ps1
                                                        = deeq_nc[((1 * deeq_2 + iat + ia) * deeq_3 + ip1) * deeq_4
                                                                  + ip2];
                                                    std::complex<FPTYPE> ps2
                                                        = deeq_nc[((2 * deeq_2 + iat + ia) * deeq_3 + ip1) * deeq_4
                                                                  + ip2];
                                                    std::complex<FPTYPE> ps3
                                                        = deeq_nc[((3 * deeq_2 + iat + ia) * deeq_3 + ip1) * deeq_4
                                                                  + ip2]
                                                          + ps_qq;
                                                    const int inkb1 = sum + ia * nproj + ip1;
                                                    const int inkb2 = sum + ia * nproj + ip2;
                                                    ...
                                                    const std::complex<FPTYPE> dbb1 = conj (dbecp[ib2 * nkb + inkb1])
                                                                                      * becp[(ib2 + 1) * nkb + inkb2];
                                                    const std::complex<FPTYPE> dbb2
                                                        = conj (dbecp[(ib2 + 1) * nkb + inkb1])
                                                          * becp[ib2 * nkb + inkb2];
                                                    ...
                                                    local_stress
                                                        -= fac
                                                           * (ps0 * dbb0 + ps1 * dbb1 + ps2 * dbb2 + ps3 * dbb3)
                                                                 .real ();
                                                }
                                        } // end ip
                                } // ia
                                        ...
                                        for (int ip2 = ip_begin; ip2 < ip_end; ip2++)
                                            {
                                                ...
                                                const std::complex<FPTYPE> dbb1
                                                    = conj (dbecp[inkb1]) * becp[nkb + inkb2];
                                                const std::complex<FPTYPE> dbb2
                                                    = conj (dbecp[nkb + inkb1]) * becp[inkb2];
                                                const std::complex<FPTYPE> dbb3
                                                    = conj (dbecp[nkb + inkb1]) * becp[nkb + inkb2];
                                                local_stress
                                                    -= fac
                                                       * (ps[0] * dbb0 + ps[1] * dbb1 + ps[2] * dbb2 + ps[3] * dbb3)
                                                             .real ();
                                            }
                                    } // end ip
                            } // ib
                                ...
                                for (int ip = 0; ip < nproj; ip++)
                                    {
                                        ...
                                        local_stress -= fac
                                                        * (coefficients0 * dbb0 + coefficients1 * dbb1
                                                           + coefficients2 * dbb2 + coefficients3 * dbb3)
                                                              .real ();
                                    } // end ip
                            } // ib
                    } // ia
        ...
        for (int ih = 0; ih < nh; ih++)
            {
                ...
                for (int ig = 0; ig < npw; ig++)
                    {
                        vkb_ptr[ig]
                            -= (gk_in[ig * 3 + ipol] * ylm_deri_ptr2[ig] + gk_in[ig * 3 + jpol] * ylm_deri_ptr1[ig])
        ...
        for (int nb = 0; nb < nbeta; nb++)
            {
                ...
                for (int ig = 0; ig < npw; ig++)
                    {
                        vq_ptr[ig]
                            = _polynomial_interpolation<FPTYPE> (tab, it, nb, tab_2, tab_3, table_interval, gnorm[ig]);
                    }
            }
    }
};
        ...
        for (int nb = 0; nb < nbeta; nb++)
            {
                ...
                for (int ig = 0; ig < npw; ig++)
                    {
                        vq_ptr[ig] = _polynomial_interpolation_nl<FPTYPE> (tab,
                                                                           it,
                                                                           nb,
                                                                           tab_2,
                                                                           tab_3,
...
template <typename FPTYPE>
void
    Simpson_Integral (const int mesh, FPTYPE* func, const FPTYPE* rab, FPTYPE& asum)
{
    assert (mesh & 1);
            ...
            for (int igl = 0; igl < ngg; igl++)
                {
                    ...
                    for (int ir = 0; ir < mesh; ir++)
                        {
                            if (type == 0)
                            ...
                            else if (type == 2)
                                {
                                    aux[ir] = r[ir] < 1.0e-8
                                                  ? rhoc[ir]
                                                  : rhoc[ir] * sin (gx_arr[igl] * r[ir]) / (gx_arr[igl] * r[ir]);
                                }
                            else if (type == 3)
                                {
                                    FPTYPE sinp, cosp;
                    ...
                    if (type == 0)
                    ...
                    else if (type == 3)
                        {
                            ...
                            rhocg1 += ModuleBase::FOUR_PI / omega * gx_arr[ngg] * ModuleBase::truncated_exp (-g2a)
                                      * (g2a + 1) / pow (gx_arr[igl] * gx_arr[igl], 2);
                            drhocg[igl] = rhocg1;
                        }
                }
#ifdef _OPENMP
...
template <>
void
    pointer_array_malloc<base_device::DEVICE_CPU>::operator() (void** ptr, const int n)
{
    return;
}
...
template <>
void
    synchronize_ptrs<base_device::DEVICE_CPU>::operator() (void** ptr_out, const void** ptr_in, const int size)
{
    return;
}
