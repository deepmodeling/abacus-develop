...
void
    Forces<FPTYPE, Device>::cal_force_cc (ModuleBase::matrix& forcecc,
                                          ...
                                          UnitCell& ucell_in)
{
    ...
    if (XC_Functional::get_ked_flag ())
        {
#ifdef USE_LIBXC
            const auto etxc_vtxc_v = XC_Functional_Libxc::v_xc_meta (XC_Functional::get_func_id (),
                                                                     rho_basis->nrxx,
                                                                     ucell_in.omega,
                                                                     ucell_in.tpiba,
                                                                     chr);
            ...
            if (ucell_in.atoms[it].ncpp.nlcc)
                {
                    ...
                    if (this->device == base_device::GpuDevice)
                        {
                            hamilt::cal_force_npw_op<FPTYPE, Device> () (psiv_d,
                                                                         gv_d,
                                                                         rhocgigg_vec_d,
                                                                         force_it_d,
                                                                         tau_it_d,
                                    ...
                                    for (int ig = 0; ig < rho_basis->npw; ig++)
                                        {
                                            ...
                                            const double arg = ModuleBase::TWO_PI
                                                               * (gv_h[ig * 3] * tau_h[iat * 3]
                                                                  + gv_h[ig * 3 + 1] * tau_h[iat * 3 + 1]
                                                                  + gv_h[ig * 3 + 2] * tau_h[iat * 3 + 2]);
                                            double sinp, cosp;
                                            ModuleBase::libm::sincos (arg, &sinp, &cosp);
                                            const std::complex<double> expiarg = std::complex<double> (sinp, cosp);

                                            const std::complex<double> tmp_var = psiv_conj * expiarg * ucell_in.tpiba
                                                                                 * ucell_in.omega * rhocgigg_vec[ig];
...
template <typename FPTYPE, typename Device>
void
    Forces<FPTYPE, Device>::deriv_drhoc (const bool& numeric,
                                         const int mesh,
                                         const FPTYPE* r,
                                         const FPTYPE* rab,
                                         const FPTYPE* rhoc,
