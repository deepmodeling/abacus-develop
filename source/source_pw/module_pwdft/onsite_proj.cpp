...
projectors::OnsiteProjector<T, Device>*
    projectors::OnsiteProjector<T, Device>::get_instance ()
{
...
template <typename T, typename Device>
void
    projectors::OnsiteProjector<T, Device>::init (
    ...
    if (!this->initialed)
        {
            ...
            this->init_proj (PARAM.inp.orbital_dir, orb_files, nproj, lproj, iproj, onsite_r);
            ...
            this->fs_tools = new hamilt::Onsite_Proj_tools<T, Device> (nproj,
                                                                       lproj,
                                                                       tab,
                                                                       nhtol,
                                                                       this->tab_atomic_,
...
template <typename T, typename Device>
void
    projectors::OnsiteProjector<T, Device>::init_proj (
        const std::string& orbital_dir,
...
template <typename T, typename Device>
void
    projectors::OnsiteProjector<T, Device>::tabulate_atomic (const int ik, const char grad)
{
    ...
    //         for(int ig = 0; ig < this->npw_; ++ig)
    //         {
    //             std::complex<double> deriv = (grad == 'n')? 1.0: ModuleBase::NEG_IMAG_UNIT; // because sk is
    //             exp(-iqtau) deriv = (grad == 'n')? 1.0: (grad == 'x')? deriv * q[ig].x: (grad == 'y')? deriv *
    //             q[ig].y: deriv * q[ig].z;
...
template <typename T, typename Device>
void
    projectors::OnsiteProjector<T, Device>::overlap_proj_psi (const int npm, const std::complex<double>* ppsi)
{
    ...
    this->fs_tools->cal_becp (ik_,
                              npm / npol,
                              this->becp,
                              ppsi); // in cal_becp, npm should be the one not multiplied by npol
    if (this->device == base_device::GpuDevice)
        {
...
template <typename T, typename Device>
void
    projectors::OnsiteProjector<T, Device>::read_abacus_orb (std::ifstream& ifs,
                                                             std::string& elem,
                                                             double& ecut,
                                                             int& nr,
                                                             double& dr,
...
template <typename T, typename Device>
void
    projectors::OnsiteProjector<T, Device>::cal_occupations (const psi::Psi<std::complex<T>, Device>* psi_in,
    ...
    for (int ik = 0; ik < psi_in->get_nk (); ik++)
        {
            ...
            this->overlap_proj_psi (nbands * psi_in->get_npol (), psi_in->get_pointer ());
