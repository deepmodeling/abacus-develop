...
{
double
    sphere_cut (double r, double r_out, double r_power)
{
    if (r <= 0 || r >= r_out)
        {
            return 0.0;
...
double
    shell_cut (double r, double r_in, double r_out, double r_power)
{
    if (r_in <= 0)
        {
...
void
    VSep::init_vsep (const ModulePW::PW_Basis& rho_basis, const Sep_Cell& sep_cell)
{
    ...
    for (int it = 0; it < ntype; ++it)
        {
            ...
            for (int ir = 0; ir < sep_pot->mesh; ++ir)
                {
                    shell_rv[ir] = shell_cut (r[ir], sep_pot->r_in, sep_pot->r_out, sep_pot->r_power) * rv[ir]
                                   * sep_pot->enhence_a;
...
void
    VSep::generate_vsep_r (const ModulePW::PW_Basis& rho_basis,
