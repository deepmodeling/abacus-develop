...
template <typename T, typename Device>
void Meta<OperatorPW<T, Device>>::act(const int nbands,
                                      const int nbasis,
                                      const int npol,
                                      const T* tmpsi_in,
                                      T* tmhpsi,
    ...
    for (int ib = 0; ib < nbands; ++ib) {
        for (int j = 0; j < 3; j++) {
            meta_op()(this->ctx,
                      ...
                      this->wfcpw->npwk_max,
                      this->tpiba,
                      wfcpw->get_gcar_data<Real>(),
                      wfcpw->get_kvec_c_data<Real>(),
                      tmpsi_in,
                      this->porter);
            wfcpw->recip_to_real(this->ctx, this->porter, this->porter, this->ik);

            if (this->vk_col != 0) {
                vector_mul_vector_op()(this->vk_col,
                                       this->porter,
                                       this->porter,
                                       this->vk + current_spin * this->vk_col);
            }
