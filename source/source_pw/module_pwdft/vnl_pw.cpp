...
void
    pseudopot_cell_vnl::release_memory ()
{
    ...
    if (this->use_gpu_)
    ...
    else
        {
            ...
            if (this->z_vkb != nullptr)
                {
                    base_device::memory::delete_memory_op_mt<std::complex<double>, base_device::DEVICE_CPU> () (
                        this->z_vkb);
                    this->z_vkb = nullptr;
                }
#endif
...
//-----------------------------------
void
    pseudopot_cell_vnl::init (const UnitCell& ucell,
...
void
    pseudopot_cell_vnl::getvnl (Device* ctx, const UnitCell& ucell, const int& ik, std::complex<FPTYPE>* vkb_in) const
{
...
void
    pseudopot_cell_vnl::init_vnl (UnitCell& cell, const ModulePW::PW_Basis* rho_basis)
{
                                            ...
                                            for (int is2 = 0; is2 < 2; ++is2)
                                                {
                                                    this->dvan_so (ijs, it, ip, ip2)
                                                        = cell.atoms[it].ncpp.dion (ir, is)
                                                          * soc.fcoef (it, is1, is2, ip, ip2);
                                                    ++ijs;
                                                    if (ir != is)
                                                        {
                                                            soc.fcoef (it, is1, is2, ip, ip2)
                    ...
                    for (int iq = 0; iq < PARAM.globalv.nqx; iq++)
                        {
                            ...
                            double vqint = 0.0;
                            ModuleBase::Integral::Simpson_Integral (kkbeta,
                                                                    aux,
                                                                    cell.atoms[it].ncpp.rab.data (),
                                                                    vqint);
...
void
    pseudopot_cell_vnl::compute_qrad (UnitCell& cell)
{
                                            ...
                                            for (int mb = nb; mb < nbeta; mb++)
                                                {
                                                    ...
                                                    if ((l >= std::abs (upf->lll[nb] - upf->lll[mb]))
                                                        && (l <= (upf->lll[nb] + upf->lll[mb]))
...
void
    pseudopot_cell_vnl::radial_fft_q (const int ng,
                                      const int ih,
                                      const int jh,
                                      const int itype,
                                      const double* qnorm,
...
template <typename FPTYPE, typename Device>
void
    pseudopot_cell_vnl::radial_fft_q (Device* ctx,
                                      const int ng,
                                      const int ih,
                                      const int jh,
                                      const int itype,
...
#ifdef __LCAO
std::complex<double>
    pseudopot_cell_vnl::Cal_C (int alpha, int lu, int mu, int L, int M) // pengfei Li  2018-3-23
{
...
double
    pseudopot_cell_vnl::CG (int l1, int m1, int l2, int m2, int L, int M) // pengfei Li 2018-3-23
{
    int dim = L * L + M;
    int dim1 = l1 * l1 + m1;
    int dim2 = l2 * l2 + m2;
...
void
    pseudopot_cell_vnl::init_vnl_alpha (const UnitCell& ucell) // pengfei Li 2018-3-23
{
    if (PARAM.inp.test_pp)
        {
                    ...
                    for (int L = 0; L <= lmaxkb + 1; L++)
                        {
                            for (int iq = 0; iq < PARAM.globalv.nqx; iq++)
                                {
                                    const double q = iq * PARAM.globalv.dq;
                                    ModuleBase::Sphbes::Spherical_Bessel (kkbeta,
...
void
    pseudopot_cell_vnl::print_vnl (std::ofstream& ofs)
{
...
// ----------------------------------------------------------------------
void
    pseudopot_cell_vnl::cal_effective_D (const ModuleBase::matrix& veff,
                                            ...
                                            if (PARAM.inp.lspinorb)
                                            ...
                                            else if (PARAM.inp.nspin == 4)
                                                {
                                                    if (is == 0)
                                                        ...
                                                        {
                                                            this->deeq_nc (is, iat, ih, jh)
                                                                = std::complex<double> (0.0, 0.0);
                                                            this->deeq_nc (is, iat, jh, ih)
                                                                = std::complex<double> (0.0, 0.0);
                                                        }
                                                    else if (is == 2)
                                                        {
                                            ...
                                            else
                                                {
                                                    ...
                                                    // in most of pseudopotential files, number of projections of one
                                                    // orbital is only one, which lead to diagonal matrix of dion when
                                                    // number larger than 1, non-diagonal dion should be calculated.
...
void
    pseudopot_cell_vnl::newq (const ModuleBase::matrix& veff, const ModulePW::PW_Basis* rho_basis, UnitCell& cell)
{
...
void
    pseudopot_cell_vnl::newd_so (const int& iat, UnitCell& cell)
{
                                                    ...
                                                    if (PARAM.globalv.domag)
                                                        {
                                                            deeq_nc (ijs, iat, ih, jh)
                                                                += deeq (0, iat, kh, lh)
                                                                       * (soc.fcoef (it, is1, 0, ih, kh)
                                                                              * soc.fcoef (it, 0, is2, lh, jh)
                                                                          + soc.fcoef (it, is1, 1, ih, kh)
                                                                                * soc.fcoef (it, 1, is2, lh, jh))
                                                                   + deeq (1, iat, kh, lh)
                                                                         * (soc.fcoef (it, is1, 0, ih, kh)
                                                                                * soc.fcoef (it, 1, is2, lh, jh)
                                                                            + soc.fcoef (it, is1, 1, ih, kh)
...
void
    pseudopot_cell_vnl::newd_nc (const int& iat, UnitCell& cell)
{
            ...
            for (int jh = 0; jh < upf->nh; jh++)
                {
                    if (PARAM.inp.lspinorb)
                        {
                            deeq_nc (0, iat, ih, jh)
                                = dvan_so (0, it, ih, jh) + deeq (0, iat, ih, jh) + deeq (3, iat, ih, jh);
                            deeq_nc (3, iat, ih, jh)
                                = dvan_so (3, it, ih, jh) + deeq (0, iat, ih, jh) - deeq (3, iat, ih, jh);
                        }
                    else
                        {
                            deeq_nc (0, iat, ih, jh)
                                = dvan (it, ih, jh) + deeq (0, iat, ih, jh) + deeq (3, iat, ih, jh);
                            deeq_nc (3, iat, ih, jh)
                                = dvan (it, ih, jh) + deeq (0, iat, ih, jh) - deeq (3, iat, ih, jh);
                        }
                    deeq_nc (1, iat, ih, jh)
                        = deeq (1, iat, ih, jh) + ModuleBase::NEG_IMAG_UNIT * deeq (2, iat, ih, jh);
...
void
    pseudopot_cell_vnl::rescale_vnl (const double& omega_in)
{
...
template <>
float*
    pseudopot_cell_vnl::get_nhtol_data () const
{
    return this->s_nhtol;
}
template <>
...
template <>
float*
    pseudopot_cell_vnl::get_nhtolm_data () const
{
...
template <>
float*
    pseudopot_cell_vnl::get_indv_data () const
{
    return this->s_indv;
}
template <>
...
template <>
float*
    pseudopot_cell_vnl::get_tab_data () const
{
    return this->s_tab;
}
template <>
...
template <>
float*
    pseudopot_cell_vnl::get_deeq_data () const
{
    return this->s_deeq;
}
template <>
...
template <>
float*
    pseudopot_cell_vnl::get_qq_nt_data () const
{
    return this->s_qq_nt;
}
template <>
...
template <>
std::complex<float>*
    pseudopot_cell_vnl::get_vkb_data () const
{
    return this->c_vkb;
}
template <>
...
template <>
std::complex<float>*
    pseudopot_cell_vnl::get_deeq_nc_data () const
{
...
template <>
std::complex<float>*
    pseudopot_cell_vnl::get_qq_so_data () const
{
    return this->c_qq_so;
}
template <>
