...
struct Exx_Helper : public Exx_HelperBase
{
  ...
  public:
    ...
    void
        set_firstiter (bool flag = true) override
    {
        first_iter = flag;
    }
    void
        set_wg (const ModuleBase::matrix* wg_) override
    {
        wg = wg_;
    }
    ...
    bool
        get_op_first_iter () const override
    {
        return op_exx ? op_exx->first_iter : false;
    }
    void
        set_op_first_iter (bool flag) override
    {
        if (op_exx)
            op_exx->first_iter = flag;
    }
    void
        set_op_exx (void* op) override
    {
        op_exx = reinterpret_cast<OperatorEXX*> (op);
    }
