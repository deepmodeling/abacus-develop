...
void
    get_exx_potential (const K_Vectors* kv,
            ...
            for (int ig = 0; ig < rhopw_dev->npw; ig++)
                {
                    ...
                    if (gamma_extrapolation)
                        {
                            // if isint(kqg_d[0] * nqs_half1) && isint(kqg_d[1] * nqs_half2) && isint(kqg_d[2] *
                            // nqs_half3)
                            auto isint = [] (double x)
                                {
                            ...
                            if (isint (kqg_d[0] * nqs_half1) && isint (kqg_d[1] * nqs_half2)
            ...
            for (int ig = 0; ig < rhopw_dev->npw; ig++)
                {
                    ...
                    // {
                    //     printf("k-q+G: %f %f %f\n", (k_c - q_c + rhopw_dev->gcar[ig])[0], (k_c - q_c +
                    //     rhopw_dev->gcar[ig])[1], (k_c - q_c + rhopw_dev->gcar[ig])[2]);
                    // }
                    // if (kqgcar2 > 1e-12) // vasp uses 1/40 of the smallest (k spacing)**2
                    if (gg >= 1e-8)
                        {
                            ...
                            if (!gamma_extrapolation)
                                {
                                    if (is_stress)
                                        ...
                                        pot_cpu[ig]
                                            += (exx_div - ModuleBase::PI * ModuleBase::e2 / erfc_omega2) * alpha;
                                }
                            else
                                {
                                    pot_cpu[ig] += exx_div * alpha;
...
template <typename Real, typename Device>
void
    get_exx_stress_potential (const K_Vectors* kv,
                              const ModulePW::PW_Basis_K* wfcpw,
                              ModulePW::PW_Basis* rhopw_dev,
                              Real* pot,
                              double tpiba,
...
double
    exx_divergence (Conv_Coulomb_Pot_K::Coulomb_Type coulomb_type,
                    double erfc_omega,
                    const K_Vectors* kv,
                    const ModulePW::PW_Basis_K* wfcpw,
                    ModulePW::PW_Basis* rhopw_dev,
