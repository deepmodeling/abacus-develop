...
void
    print_orb_chg (const UnitCell* ucell,
    ...
    for (int iat = 0; iat < ucell->nat; iat++)
        {
                    ...
                    if (ih == current_l * current_l - 1)
                        {
                            ...
                            sum[3] += charge_mag[3];
                            GlobalV::ofs_running
                                << FmtCore::format ("%20s", orb_names[current_l - 1])
            ...
            GlobalV::ofs_running << FmtCore::format ("%20s", std::string ("Sum")) << "" << fmt_of_chg.format (sum[0])
                                 << fmt_of_chg.format (sum[1]) << fmt_of_chg.format (sum[2])
                                 << fmt_of_chg.format (sum[3]) << std::endl;
        }
    GlobalV::ofs_running
        << "-------------------------------------------------------------------------------------------" << std::endl;
    GlobalV::ofs_running << std::endl;
...
void
    print_mag_table (const std::vector<std::string>& atom_labels,
...
void
    print_proj_status (int it, int nproj_it)
{
    if (nproj_it == 0)
        {
