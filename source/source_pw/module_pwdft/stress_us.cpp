...
void
    Stress_PW<FPTYPE, Device>::stress_us (ModuleBase::matrix& sigma,
                                    ...
                                    for (int is = 0; is < PARAM.inp.nspin; is++)
                                        {
                                            for (int ij = 0; ij < nij; ij++)
                                                {
                                                    tbecsum (is, ij)
                                                        = becsum[is * ucell.nat * nh_tot + iat * nh_tot + ij];
                                                }
                                        }
                                    ...
                                    for (int ig = 0; ig < npw; ig++)
                                        {
                                            ...
                                            std::complex<double> cfac
                                                = ucell.tpiba * ModuleBase::libm::exp (ci_tpi * arg);
                                            for (int ipol = 0; ipol < 3; ipol++)
                                                {
...
template <typename FPTYPE, typename Device>
void
    Stress_Func<FPTYPE, Device>::dqvan2 (const pseudopot_cell_vnl& nlpp,
                                         const int ih,
                                         const int jh,
                                         const int itype,
                                         const int ipol,
                    ...
                    if (std::abs (qnorm[ig] - qm1) > 1e-6)
                        {
                            work = ModuleBase::PolyInt::Polynomial_Interpolation (nlpp.qrad,
                                                                                  ...
                                                                                  PARAM.globalv.dq,
                                                                                  qnorm[ig]);
                            work1 = this->Polynomial_Interpolation_nl (nlpp.qrad,
                                                                       itype,
                                                                       l,
                                                                       ijv,
                                                                       PARAM.globalv.dq,
