...
void
    pseudopot_cell_vnl::initgradq_vnl (const UnitCell& cell)
{
    const int nbrx = 10;
                    ...
                    for (int iq = 0; iq < PARAM.globalv.nqx; iq++)
                        {
                            ...
                            for (int ir = 0; ir < kkbeta; ir++)
                                {
                                    aux[ir] = cell.atoms[it].ncpp.betar (ib, ir) * djl[ir]
                                              * pow (cell.atoms[it].ncpp.r[ir], 2);
                                }
                            double vqint = 0.0;
...
void
    pseudopot_cell_vnl::getgradq_vnl (const UnitCell& ucell, const int ik)
{
    ...
    // When the internal memory is large enough, it is better to make tmpgradvkb and tmpvkb be the number of
    // pseudopot_cell_vnl We only need to initialize them once as long as the cell is unchanged.
    ...
    ModuleBase::matrix* dylm = new ModuleBase::matrix[3]{ModuleBase::matrix (x1, npw),
                                                         ModuleBase::matrix (x1, npw),
                                                         ModuleBase::matrix (x1, npw)};
                            ...
                            for (int ig = 0; ig < npw; ++ig)
                                {
                                    ...
                                    vq[ig] = ModuleBase::PolyInt::Polynomial_Interpolation (this->tab,
                                                                                            ...
                                                                                            PARAM.globalv.dq,
                                                                                            gnorm);
                                    dvq[ig] = ModuleBase::PolyInt::Polynomial_Interpolation (this->tab_dq,
                                                                                             it,
                                                                                             nb,
                                                                                             PARAM.globalv.nqx,
                                                                                             PARAM.globalv.dq,
                                    ...
                                    if (ggnorm < 1e-8)
                                        {
                                            if (lm == 0 || lm > 3)
                                                ...
                                                {
                                                    // q \to 0 : \nabla(f(q)Y(\hat{q})) =
                                                    // f(q)/q*sqrt(3/4/pi)*vec(-delta(lm,2), -delta(lm,3), delta(lm,1))
                                                    //  tmpgradvkb(id, ih, ig) = 0.0;
                                                    tmpgradvkb (id, ih, ig)
                                                        = dvq[ig] * sqrt (3.0 / 4.0 / M_PI) * lmmat[(lm - 1) * 3 + id];
                                                }
                                        }
                                    else
                                        {
                                            tmpgradvkb (id, ih, ig)
                                                = ylm (lm, ig) * dvq[ig] * gg[id] / ggnorm
                                    ...
                                    for (int ig = 0; ig < npw; ++ig)
                                        {
                                            ...
                                            // std::complex<double> dskig = ModuleBase::NEG_IMAG_UNIT *
                                            // (ucell.atoms[it].tau[ia][id] * this->wfcpw->lat0) * skig; pgvkb[ig] =
                                            // tmpgradvkb(id, ih, ig) * skig * pref +  tmpvkb(ih, ig) * dskig * pref;
                                            // The second term will be eliminate when doing <psi|beta>Dij<beta|psi> or
                                            // we can say (\nabla_q+\nabla_q')S(q'-q) = 0
