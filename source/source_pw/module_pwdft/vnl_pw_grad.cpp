...
void pseudopot_cell_vnl::getgradq_vnl(const UnitCell& ucell, const int ik) {
            ...
            for (int id = 0; id < 3; ++id) {
                ...
                for (int ig = 0; ig < npw; ++ig) {
                    ...
                    if (ggnorm < 1e-8) {
                        ...
                        {
                            // q \to 0 : \nabla(f(q)Y(\hat{q})) = f(q)/q*sqrt(3/4/pi)*vec(-delta(lm,2), -delta(lm,3),
                            // delta(lm,1))
                            //  tmpgradvkb(id, ih, ig) = 0.0;
            ...
            for (int ih = 0; ih < nh; ++ih) {
                ...
                for (int id = 0; id < 3; ++id) {
                    ...
                    for (int ig = 0; ig < npw; ++ig) {
                        ...
                        // std::complex<double> dskig = ModuleBase::NEG_IMAG_UNIT * (ucell.atoms[it].tau[ia][id] *
                        // this->wfcpw->lat0) * skig; pgvkb[ig] = tmpgradvkb(id, ih, ig) * skig * pref +  tmpvkb(ih, ig)
                        // * dskig * pref; The second term will be eliminate when doing <psi|beta>Dij<beta|psi> or we
                        // can say (\nabla_q+\nabla_q')S(q'-q) = 0
