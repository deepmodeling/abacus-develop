...
void
    Stress_Func<FPTYPE, Device>::stress_nl (ModuleBase::matrix& sigma,
...
template <typename FPTYPE, typename Device>
void
    Stress_Func<FPTYPE, Device>::get_dvnl1 (ModuleBase::ComplexMatrix& vkb,
                                            const int ik,
                                            const int ipol,
                                            Structure_Factor* p_sf,
                                            ModulePW::PW_Basis_K* wfc_basis)
...
template <typename FPTYPE, typename Device>
void
    Stress_Func<FPTYPE, Device>::get_dvnl2 (ModuleBase::ComplexMatrix& vkb,
                                            const int ik,
                                            Structure_Factor* p_sf,
                                            ModulePW::PW_Basis_K* wfc_basis)
{
                    ...
                    for (int ig = 0; ig < npw; ig++)
                        {
                            const FPTYPE gnorm = gk[ig].norm () * this->ucell->tpiba;
                            vq[ig]
                                = hamilt::Nonlocal_maths<FPTYPE, Device>::Polynomial_Interpolation_nl (nlpp->tab,
                                                                                                       it,
                                                                                                       nb,
                                                                                                       PARAM.globalv.dq,
                                                                                                       gnorm);
                    ...
                    for (int ih = 0; ih < nh; ih++)
                        {
                            for (int ig = 0; ig < npw; ig++)
                                {
                                    std::complex<FPTYPE> pref
                                        = pref_tab[int (nlpp->nhtol (it, ih)) % imag_pow_period]; //?
                                    vkb (jkb + ih, ig) = vkb1 (ih, ig) * sk[ig] * pref;
                                }
...
template <typename FPTYPE, typename Device>
FPTYPE
    Stress_Func<FPTYPE, Device>::Polynomial_Interpolation_nl (const ModuleBase::realArray& table,
                                                              const int& dim1,
                                                              const int& dim2,
                                                              const int& dim3,
