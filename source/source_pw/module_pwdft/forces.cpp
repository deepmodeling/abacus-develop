...
void
    Forces<FPTYPE, Device>::cal_force (UnitCell& ucell,
            ...
            for (int it = 0; it < ucell.ntype; it++)
                {
                    for (int ia = 0; ia < ucell.atoms[it].na; ia++)
                        {
                            force (iat, ipol) = forcelc (iat, ipol) + forceion (iat, ipol) + forcenl (iat, ipol)
                                                + forcecc (iat, ipol) + forcescc (iat, ipol);
...
template <typename FPTYPE, typename Device>
void
    Forces<FPTYPE, Device>::cal_force_loc (const UnitCell& ucell,
    ...
    if (this->device == base_device::GpuDevice)
        {
            ...
            hamilt::cal_force_loc_op<FPTYPE, Device> () (this->nat,
                                                         rho_basis->npw,
...
template <typename FPTYPE, typename Device>
void
    Forces<FPTYPE, Device>::cal_force_ew (const UnitCell& ucell,
    ...
    do
        {
            ...
            upperbound = 2.0 * charge * charge * sqrt (2.0 * alpha / ModuleBase::TWO_PI)
                         * ModuleBase::truncated_erfc (sqrt (ucell.tpiba2 * rho_basis->ggecut / 4.0 / alpha));
        }
    ...
    for (int ig = 0; ig < rho_basis->npw; ig++)
        {
            ...
            aux[ig] *= ModuleBase::truncated_exp (-1.0 * rho_basis->gg[ig] * ucell.tpiba2 / alpha / 4.0)
    ...
    if (this->device == base_device::GpuDevice)
        {
            ...
            for (int it = 0; it < ucell.ntype; ++it)
                {
                    it_fact_h[it] = ucell.atoms[it].ncpp.zv * ModuleBase::e2 * ucell.tpiba * ModuleBase::TWO_PI
                                    / ucell.omega * fact;
                }
            ...
            hamilt::cal_force_ew_op<FPTYPE, Device> () (this->nat,
                                                        rho_basis->npw,
                                                        rho_basis->ig_gge0,
                                                        iat2it_d,
                                                        gcar_d,
            ...
            for (int iat = 0; iat < this->nat; ++iat)
                {
                    ...
                    double it_fact = ucell.atoms[it].ncpp.zv * ModuleBase::e2 * ucell.tpiba * ModuleBase::TWO_PI
                                     / ucell.omega * fact;
                                        ...
                                        for (int n = 0; n < nrm; n++)
                                            {
                                                ...
                                                double factor = 0.0;
                                                {
                                                    factor = ucell.atoms[T1].ncpp.zv * ucell.atoms[T2].ncpp.zv
                                                             * ModuleBase::e2 / (rr * rr)
                                                             * (erfc (sqa * rr) / rr
                                                                + sq8a_2pi * ModuleBase::libm::exp (-alpha * rr * rr))
                                                             * ucell.lat0;
                                                }
