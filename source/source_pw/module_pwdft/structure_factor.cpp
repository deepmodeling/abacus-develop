...
// called in input.cpp
void
    Structure_Factor::set (const ModulePW::PW_Basis* rho_basis_in, const int& nbspline_in)
{
...
void
    Structure_Factor::setup (const UnitCell* Ucell, const Parallel_Grid& pgrid, const ModulePW::PW_Basis* rho_basis)
{
...
#pragma omp parallel for
    ...
    ModuleBase::Memory::record ("SF::eigts123",
...
//
void
    Structure_Factor::bspline_sf (const int norder,
...
#pragma omp atomic
#endif
                                            r[icz * rho_basis->ny * rho_basis->nx + icx * rho_basis->ny + icy]
...
void
    Structure_Factor::bsplinecoef (std::complex<double>* b1,
        ...
        for (int ix = 0; ix < nx; ++ix)
            {
                ...
                for (int io = 0; io < norder - 1; ++io)
                    {
                        fracx += bsp.bezier_ele (io)
                                 * ModuleBase::libm::exp (ci_tpi * double (ix) / double (nx) * double (io));
                    }
        ...
        for (int iy = 0; iy < ny; ++iy)
            {
                ...
                for (int io = 0; io < norder - 1; ++io)
                    {
                        fracy += bsp.bezier_ele (io)
                                 * ModuleBase::libm::exp (ci_tpi * double (iy) / double (ny) * double (io));
                    }
        ...
        for (int iz = 0; iz < nz; ++iz)
            {
                ...
                for (int io = 0; io < norder - 1; ++io)
                    {
                        fracz += bsp.bezier_ele (io)
                                 * ModuleBase::libm::exp (ci_tpi * double (iz) / double (nz) * double (io));
                    }
...
template <>
std::complex<float>*
    Structure_Factor::get_eigts1_data () const
{
...
template <>
std::complex<float>*
    Structure_Factor::get_eigts2_data () const
{
...
template <>
std::complex<float>*
    Structure_Factor::get_eigts3_data () const
{
