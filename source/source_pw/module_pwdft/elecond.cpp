...
template <typename FPTYPE, typename Device>
void
    EleCond<FPTYPE, Device>::KG (const int& smear_type,
                                 const double& fwhmin,
                                 const double& wcut,
                                 const double& dw_in,
                                 const double& dt_in,
...
template <typename FPTYPE, typename Device>
void
    EleCond<FPTYPE, Device>::jjresponse_ks (const int ik,
                                            const int nt,
                                            const double dt,
                                            const double decut,
                                            ModuleBase::matrix& wg,
...
template <typename FPTYPE, typename Device>
void
    EleCond<FPTYPE, Device>::calcondw (const int nt,
                                       const double dt,
    ...
    for (int iw = 0; iw < nw; ++iw)
        {
            ...
            cw12[iw] *= double (2) / ndim / this->p_ucell->omega * factor * 2.17987092759e-18
                        / 1.6021766208e-19; // unit in Am^-1
            ...
            kappa[iw] = (cw22[iw] - pow (cw12[iw], 2) / cw11[iw]) / Occupy::gaussian_parameter / ModuleBase::Ry_to_eV
                        / 11604.518026;
            ofscond << std::setw (8) << (iw + 0.5) * dw * ModuleBase::Ry_to_eV << std::setw (20) << cw11[iw]
                    << std::setw (20) << kappa[iw] << std::setw (20) << cw12[iw] << std::setw (20) << cw22[iw]
                    << std::endl;
        }
