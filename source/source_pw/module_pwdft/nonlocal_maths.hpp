...
template <typename FPTYPE, typename Device>
std::vector<FPTYPE>
    Nonlocal_maths<FPTYPE, Device>::cal_gk (int ik, const ModulePW::PW_Basis_K* pw_basis)
{
...
template <typename FPTYPE, typename Device>
void
    Nonlocal_maths<FPTYPE, Device>::cal_ylm (int lmax, int npw, const FPTYPE* q, FPTYPE* ylm)
{
    ...
    if (this->device == base_device::GpuDevice)
        {
            ...
            using syncmem_var_h2d_op
                = base_device::memory::synchronize_memory_op<FPTYPE, Device, base_device::DEVICE_CPU>;
            // allocate
...
template <typename FPTYPE, typename Device>
void
    Nonlocal_maths<FPTYPE, Device>::cal_ylm_deri (int lmax, int npw, const FPTYPE* q, FPTYPE* out)
{
    const int ntot_ylm = (lmax + 1) * (lmax + 1);
...
std::vector<std::complex<FPTYPE>>
    Nonlocal_maths<FPTYPE, Device>::cal_pref (int it, const int nh)
{
...
template <typename FPTYPE, typename Device>
void
    Nonlocal_maths<FPTYPE, Device>::cal_vkb (int it,
                                             int ia,
                                             int npw,
                                             const FPTYPE* vq_in,
                                             const FPTYPE* ylm_in,
...
template <typename FPTYPE, typename Device>
void
    Nonlocal_maths<FPTYPE, Device>::cal_vkb_deri (int it,
                                                  int ia,
                                                  int npw,
                                                  int ipol,
                                                  int jpol,
                    ...
                    for (int ig = 0; ig < npw; ig++)
                        {
                            vkb_ptr[ig] -= 2.0 * ylm_ptr[ig] * vq_deri_ptr[ig] * sk_in[ig] * pref_in[ih]
                                           * gk_in[ig * 3 + ipol] * gk_in[ig * 3 + jpol] * qnorm[ig];
                        }
                    ih++;
                }
        }
...
template <typename FPTYPE, typename Device>
void
    Nonlocal_maths<FPTYPE, Device>::prepare_vkb_ptr (int nbeta,
                                                     double* nhtol,
                                                     int nhtol_nc,
                                                     int npw,
                                                     int it,
...
template <typename FPTYPE, typename Device>
void
    Nonlocal_maths<FPTYPE, Device>::cal_dvkb_index (const int nbeta,
                                                    const double* nhtol,
                                                    const int nhtol_nc,
                                                    const int npw,
                                                    const int it,
    ...
    for (int nb = 0; nb < nbeta; nb++)
        {
            ...
            for (int m = 0; m < 2 * l + 1; m++)
                {
                    // std::cout << "in function cal_dvkb_index, nhtol(" << it << ", " << ih << ") = " << l <<
                    // std::endl;
                    int lm = l * l + m;
...
template <typename FPTYPE, typename Device>
void
    Nonlocal_maths<FPTYPE, Device>::dylmr2 (const int nylm,
                                            const int ngy,
                                            const FPTYPE* gk,
                                            FPTYPE* dylm,
                                            const int ipol)
...
template <typename FPTYPE, typename Device>
FPTYPE
    Nonlocal_maths<FPTYPE, Device>::Polynomial_Interpolation_nl (const ModuleBase::realArray& table,
                                                                 const int& dim1,
                                                                 const int& dim2,
                                                                 const FPTYPE& table_interval,
