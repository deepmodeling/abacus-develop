...
void
    Stress_Func<FPTYPE, Device>::stress_onsite (ModuleBase::matrix& sigma,
    ...
    for (int ik = 0; ik < nks; ik++)
        {
            ...
            if (nbands_occ < 0)
                {
                    ModuleBase::WARNING_QUIT ("stress_onsite.cpp",
                                              "Number of bands is negative, cannot calculate stress");
                }
                            ...
                            if (PARAM.inp.dft_plus_u)
                                {
                                    ...
                                    double dftu_stress = fs_tools->cal_stress_dftu (ik,
                                                                                    num_occupied_bands,
                            ...
                            if (PARAM.inp.sc_mag_switch)
                                {
                                    ...
                                    const std::vector<ModuleBase::Vector3<double>>& lambda
                                        = spin_constrain.get_sc_lambda ();

                                    // Calculate spin constraint stress contribution
                                    double dspin_stress
                                        = fs_tools->cal_stress_dspin (ik, num_occupied_bands, lambda.data (), wg.c);
