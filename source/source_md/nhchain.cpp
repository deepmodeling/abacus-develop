             ...
             */
            else if (mdp.md_pmode == "tri")
                {
                    if (ucell.latvec.e12 || ucell.latvec.e13 || ucell.latvec.e23)
                        {
                            ModuleBase::WARNING_QUIT ("Nose_Hoover",
                                                      "the lattice must be lower-triangular when md_pmode == tri!");
                        }
                    pstart[0] = pstart[1] = pstart[2] = md_pfirst;
                    pstop[0] = pstop[1] = pstop[2] = md_plast;
                    pfreq[0] = pfreq[1] = pfreq[2] = md_pfreq;
...
void
    Nose_Hoover::setup (ModuleESolver::ESolver* p_esolver, const std::string& global_readin_dir)
{
...
void
    Nose_Hoover::first_half (std::ofstream& ofs)
{
...
void
    Nose_Hoover::print_md (std::ofstream& ofs, const bool& cal_stress)
{
    MD_base::print_md (ofs, cal_stress);
}

void
    Nose_Hoover::write_restart (const std::string& global_out_dir)
{
    if (!my_rank)
        {
...
void
    Nose_Hoover::restart (const std::string& global_readin_dir)
{
    bool ok = true;
    bool ok2 = true;
    bool ok3 = true;
...
void
    Nose_Hoover::update_volume (std::ofstream& ofs)
{
    double factor = 0.0;
