...
double
    kinetic_energy (const int& natom, const ModuleBase::Vector3<double>* vel, const double* allmass)
{
    double ke = 0;
...
void
    compute_stress (const UnitCell& unit_in,
...
void
    read_vel (const UnitCell& unit_in, ModuleBase::Vector3<double>* vel)
{
    int iat = 0;
...
void
    init_vel (const UnitCell& unit_in,
              const int& my_rank,
              const bool& restart,
              double& temperature,
              double* allmass,
...
void
    print_stress (std::ofstream& ofs, const ModuleBase::matrix& virial, const ModuleBase::matrix& stress)
{
    ...
    for (int it = 0; it < unit_in.ntype; ++it)
        {
            for (int ia = 0; ia < unit_in.atoms[it].na; ++ia)
                {
                    ofs << "  " << index << "  " << unit_in.atom_label[it] << "  "
                        << unit_in.atoms[it].tau[ia].x * unit_pos << "  " << unit_in.atoms[it].tau[ia].y * unit_pos
...
void
    get_mass_mbl (const UnitCell& unit_in,
                  double* allmass,
                  ModuleBase::Vector3<int>& frozen,
                  ModuleBase::Vector3<int>* ionmbl)
{
...
double
    target_temp (const int& istep, const int& nstep, const double& tfirst, const double& tlast)
{
    assert (nstep > 0);
