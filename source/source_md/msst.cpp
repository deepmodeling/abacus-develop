...
void
    MSST::setup (ModuleESolver::ESolver* p_esolver, const std::string& global_readin_dir)
{
            ...
            if (kinetic > 0 && mdp.msst_tscale > 0)
                {
                    ...
                    std::cout << "initial strain rate = " << fac2 << "    msst_tscale = " << mdp.msst_tscale
                              << std::endl;
...
void
    MSST::first_half (std::ofstream& ofs)
{
...
void
    MSST::print_md (std::ofstream& ofs, const bool& cal_stress)
{
...
void
    MSST::write_restart (const std::string& global_out_dir)
{
    if (!my_rank)
        {
...
void
    MSST::restart (const std::string& global_readin_dir)
{
    bool ok = true;
...
void
    MSST::rescale (std::ofstream& ofs, const double& volume)
{
    int sd = mdp.msst_direction;
                            ...
                            if (fabs (dthalf * const_D[k]) > 1e-6)
                                {
                                    ...
                                    vel[i][k]
                                        = expd * (const_C[k] + const_D[k] * vel[i][k] - const_C[k] / expd) / const_D[k];
                                }
                            else
                                {
                                    vel[i][k] += (const_C[k] + const_D[k] * vel[i][k]) * dthalf
                                                 + 0.5 * (const_D[k] * const_D[k] * vel[i][k] + const_C[k] * const_D[k])
                                                       * dthalf * dthalf;
                                }
                        }
                }
