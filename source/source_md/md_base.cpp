...
void
    MD_base::setup (ModuleESolver::ESolver* p_esolver, const std::string& global_readin_dir)
{
    if (mdp.md_restart)
        {
...
void
    MD_base::first_half (std::ofstream& ofs)
{
    update_vel (force);
    update_pos ();
...
void
    MD_base::update_vel (const ModuleBase::Vector3<double>* force)
{
    if (my_rank == 0)
        {
            for (int i = 0; i < ucell.nat; ++i)
...
void
    MD_base::print_md (std::ofstream& ofs, const bool& cal_stress)
{
    if (my_rank != 0)
        {
            return;
    ...
    ofs << std::endl;
    ofs << " " << std::left << std::setw (20) << 2 * (potential + kinetic) << std::left << std::setw (20)
        << 2 * potential << std::left << std::setw (20) << 2 * kinetic << std::left << std::setw (20)
        << t_current * ModuleBase::Hartree_to_K;
...
void
    MD_base::write_restart (const std::string& global_out_dir)
{
    if (!my_rank)
        {
...
void
    MD_base::restart (const std::string& global_readin_dir)
{
