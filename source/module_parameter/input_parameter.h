...
struct Input_supplement {
    ...
    double bessel_nao_rcut
        = 6.0; ///< radial cutoff for spherical bessel functions(a.u.)
    ...
    bool double_grid
        = false; ///< true if "ndx,ndy,ndz" is larger than "nx,ny,nz"
    double uramping = -10.0 / 13.6; /// U-Ramping method (Ry)
    std::vector<double> hubbard_u
        = {}; ///< Hubbard Coulomb interaction parameter U (Ry)
    std::string global_calculation
        = "scf"; ///< global calculation type decided by "calculation"
};
...
struct Input_para {
    ...
    std::string kpoint_file
        = "KPT"; ///< file contains k-points -- xiaohui modify 2015-02-01
    ...
    double symmetry_prec
        = 1.0e-6; ///< LiuXh add 2021-08-12, accuracy for symmetry
    ...
    int out_freq_elec
        = 0; ///< the frequency ( >= 0) of electronic iter to output charge
             ///< density and wavefunction. 0: output only when converged
    bool dft_plus_dmft
        = false; ///< true:DFT+DMFT; false: standard DFT calcullation(default)
    bool rpa = false;  ///< rpa calculation
    int printe = 100;  ///< mohan add 2011-03-16
    int mem_saver = 0; ///< 1: save psi when nscf calculation.
    int diago_proc
        = 0; ///< the number of procs used to diag. mohan add 2012-01-13
    int nbspline
        = -1; ///< the order of B-spline basis(>=0) if it is -1 (default)
    ...
    int out_freq_ion
        = 0; ///< the frequency ( >= 0 ) of ionic step to output charge density
    ...
    double erf_height
        = 0; ///< the height of the energy step for reciprocal vectors
    double erf_sigma
        = 0.1; ///< the width of the energy step for reciprocal vectors
    int fft_mode
        = 0; ///< fftw mode 0: estimate, 1: measure, 2: patient, 3: exhaustive
    int pw_diag_nmax = 50;
    int diago_cg_prec = 1; ///< mohan add 2012-03-31
    int pw_diag_ndim
        = 4; ///< dimension of workspace for Davidson diagonalization
    ...
    int scf_thr_type
        = -1; ///< type of the criterion of scf_thr, 1: reci drho, 2: real drho
    ...
    int pw_seed
        = 1; ///< random seed for initializing wave functions qianrui 2021-8-12
    ...
    int initsto_freq
        = 0; ///< frequency to init stochastic orbitals when running md
    ...
    int cond_dtbatch
        = 0; ///< exp(iH*dt*cond_dtbatch) is expanded with Chebyshev expansion.
    int cond_smear
        = 1; ///< smearing method for conductivities 1: Gaussian 2: Lorentzian
    ...
    double relax_cg_thr
        = 0.5; ///< threshold when cg to bfgs, pengfei add 2011-08-15
    double stress_thr
        = 0.5; ///< Pengfei Li 2017-11-01 ///<LiuXh update 20180515
    double press1 = 0;
    double press2 = 0;
    double press3 = 0;
    ...
    bool deepks_out_labels
        = false; ///< (need libnpy) prints energy and force labels and
                 ///< descriptors for training, wenfei 2022-1-12
    bool deepks_scf
        = false; ///< (need libnpy and libtorch) if set to true, a trained model
    ...
    bool deepks_out_unittest
        = false; ///< if set to true, prints intermediate quantities that shall
                 ///< be used for making unit test
    std::string deepks_model = ""; ///< needed when deepks_scf=1

    // ==============   #Parameters (5.LCAO) ===========================
    std::string basis_type
        = "pw"; ///< xiaohui add 2013-09-01, for structural adjustment
    ...
    std::vector<int> out_mat_hs
        = {0, 8};             ///< output H matrix and S matrix in local basis.
    ...
    int out_interval = 1;
    bool out_app_flag
        = true; ///< whether output r(R), H(R), S(R), T(R), and dH(R) matrices
                ///< in an append manner during MD liuyu 2023-03-20
    int out_ndigits
        = 8; ///< Assuming 8 digits precision is needed for matrices output
    ...
    double dmax
        = 0.01; ///< maximum displacement of all atoms in one step (bohr)
    ...
    int efield_dir
        = 2; ///< the direction of the electric field or dipole correction
    ...
    double efield_pos_dec
        = -1; ///< zone in the unit cell where the saw-like potential decreases
    ...
    std::string vdw_method
        = "none"; ///< the method of calculating vdw (none; d2; d3_0; d3_bj)
    ...
    std::string vdw_cutoff_radius
        = "default"; ///< radius cutoff for periodic structure
    std::string vdw_radius_unit
        = "Bohr";             ///< unit of radius cutoff for periodic structure
    ...
    ModuleBase::Vector3<int> vdw_cutoff_period
        = {3, 3, 3}; ///< periods of periodic structure
    ...
    std::string exx_hybrid_alpha
        = "default"; ///< fraction of Fock exchange in hybrid functionals
    double exx_hse_omega
        = 0.11; ///< range-separation parameter in HSE functional
    bool exx_separate_loop
        = true; ///< if 1, a two-step method is employed, else it will start
    ...
    double exx_mixing_beta
        = 1.0; ///< mixing_beta for outer-loop when exx_separate_loop=1
    double exx_lambda
        = 0.3; ///< used to compensate for divergence points at G=0 in the
    ...
    double exx_pca_threshold
        = 0.0001; ///< threshold to screen on-site ABFs in exx
    ...
    double exx_dm_threshold
        = 0.0001; ///< threshold to screen density matrix in exx
    double exx_schwarz_threshold
        = 0; ///< threshold to screen exx using Cauchy-Schwartz inequality
    double exx_cauchy_threshold
        = 1e-07; ///< threshold to screen exx using Cauchy-Schwartz inequality
    double exx_c_grad_threshold
        = 0.0001; ///< threshold to screen nabla C matrix in exx
    double exx_v_grad_threshold
        = 0.1; ///< threshold to screen nabla V matrix in exx
    double exx_cauchy_force_threshold
        = 1e-07; ///< threshold to screen exx force using Cauchy-Schwartz
                 ///< inequality
    double exx_cauchy_stress_threshold
        = 1e-07; ///< threshold to screen exx stress using Cauchy-Schwartz
                 ///< inequality
    std::string exx_ccp_rmesh_times
        = "1"; ///< how many times larger the radial mesh required for
               ///< calculating Columb potential is to that of atomic orbitals
    std::string exx_distribute_type
        = "htime"; ///< distribute type (assuming default as no specific value
                   ///< provided)
    int exx_opt_orb_lmax
        = 0; ///< the maximum l of the spherical Bessel functions for opt ABFs
    double exx_opt_orb_ecut
        = 0.0; ///< the cut-off of plane wave expansion for opt ABFs
    double exx_opt_orb_tolerence
        = 0.0; ///< the threshold when solving for the zeros of spherical Bessel
               ///< functions for opt ABFs
    double rpa_ccp_rmesh_times
        = 10.0; ///< how many times larger the radial mesh required for
                ///< calculating Columb potential is to that of atomic orbitals
    ...
    int td_stype
        = 0; ///< type of space domain  0 : length gauge  1: velocity gauge
    ...
    bool berry_phase
        = false;  ///< berry phase calculation: calculate berry phase or not
    ...
    std::string nnkpfile
        = "seedname.nnkp"; ///< add by jingan for wannier90: the wannier90 code
                           ///< nnkp file name
    std::string wannier_spin = "up"; ///< add by jingan for wannier90: calculate
                                     ///< spin in wannier90 code interface
    int wannier_method
        = 1; ///< different implementation methods under Lcao basis set
    bool out_wannier_mmn
        = true; ///< add by renxi for wannier90: output .mmn file or not
    ...
    bool out_wannier_wvfn_formatted
        = true; ///< output UNK. file in text format or in binary format
    ...
    std::string of_kinetic
        = "wt"; ///< Kinetic energy functional, such as TF, VW, WT, TF+
    std::string of_method
        = "tn"; ///< optimization method, include cg1, cg2, tn (default), bfgs
    ...
    bool of_hold_rho0
        = false; ///< If set to 1, the rho0 will be fixed even if the volume of
                 ///< system has changed, it will be set to 1 automatically if
                 ///< of_wt_rho0 is not zero.
    double of_lkt_a = 1.3; ///< parameter a of LKT KEDF
    bool of_full_pw
        = true; ///< If set to 1, ecut will be ignored while collecting
                ///< planewaves, so that all planewaves will be used.
    int of_full_pw_dim
        = 0; ///< If of_full_pw = 1, the dimension of FFT will be restricted to
    ...
    std::string bessel_nao_ecut
        = "default"; ///< energy cutoff for spherical bessel functions(Ry)
    double bessel_nao_tolerence
        = 1e-12; ///< tolerance for spherical bessel root
     ...
     */
    bool sc_mag_switch
        = false; ///< the switch to open the DeltaSpin function, 0: no
                 ///< spin-constrained DFT; 1: constrain atomic magnetization
    bool decay_grad_switch
        = false; ///< the switch to use the local approximation of gradient
    ...
    int sc_scf_nmin
        = 2; ///< minimum number of outer scf loop before initial lambda loop
    double alpha_trial
        = 0.01;         ///< initial trial step size for lambda in eV/uB^2
    ...
    std::string qo_basis
        = "szv"; ///< type of QO basis function: hydrogen: hydrogen-like basis,
