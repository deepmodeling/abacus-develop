...
void surchem::gauss_charge(const UnitCell &cell, ModulePW::PW_Basis* rho_basis, complex<double> *N)
{
    ...
    {
        double RCS = GetAtom.atom_RCS[cell.atoms[it].atom_pp.psd];
        ...
        for (int ig = 0; ig < rho_basis->npw; ig++)
        {
            ...
            N[ig].real(N[ig].real()
                       + (GetAtom.atom_Z[cell.atoms[it].atom_pp.psd] - cell.atoms[it].atom_pp.zv) * GlobalC::sf.strucFac(it, ig).real()
                             * exp(-0.5 * gg * (sigma_rc_k * sigma_rc_k)));
            N[ig].imag(N[ig].imag()
                       + (GetAtom.atom_Z[cell.atoms[it].atom_pp.psd] - cell.atoms[it].atom_pp.zv) * GlobalC::sf.strucFac(it, ig).imag()
                             * exp(-0.5 * gg * (sigma_rc_k * sigma_rc_k)));
        }
    }
    for (int ig = 0; ig < rho_basis->npw; ig++)
