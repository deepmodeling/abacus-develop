    ...
    for (int it = 0;it < cell.ntype;it++)
    {
        for (int ia = 0;ia < cell.atoms[it].na ; ia++)
        {
            ...
            {   
                ...
                {
                    N[ig] = GlobalC::ucell.atoms[it].atom_pp.zv / GlobalC::ucell.omega;
                }
                else
                {
...
void force_cor_two(const UnitCell &cell, ModulePW::PW_Basis* rho_basis , ModuleBase::matrix& forcesol)
{
    ...
    for (int it = 0;it < cell.ntype;it++)
    {
        double RCS = GlobalC::solvent_model.GetAtom.atom_RCS[cell.atoms[it].atom_pp.psd];
        ...
        for (int ia = 0;ia < cell.atoms[it].na;ia++)
        {
            ...
            for (int ig = 0; ig < rho_basis->npw; ig++)
            {
                ...
                n_pseudo[ig].real((GlobalC::solvent_model.GetAtom.atom_Z[cell.atoms[it].atom_pp.psd] - cell.atoms[it].atom_pp.zv) * phase.real()
                             * exp(-0.5 * gg * (sigma_rc_k * sigma_rc_k)));
                n_pseudo[ig].imag((GlobalC::solvent_model.GetAtom.atom_Z[cell.atoms[it].atom_pp.psd] - cell.atoms[it].atom_pp.zv) * phase.imag()
                             * exp(-0.5 * gg * (sigma_rc_k * sigma_rc_k)));
            }
