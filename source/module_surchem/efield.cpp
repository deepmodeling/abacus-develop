...
double Efield::cal_ion_dipole(const UnitCell &cell, const double &bmod)
{
    ...
    for(int it=0; it<cell.ntype; ++it)
    {
        ...
        ion_dipole += sum * cell.atoms[it].atom_pp.zv;
    }
    ...
    if(GlobalV::GATE_FLAG && GlobalV::DIP_COR_FLAG)
    {
        ...
        for(int it=0; it<cell.ntype; ++it)
        {
            ion_charge += cell.atoms[it].na * cell.atoms[it].atom_pp.zv;
        }
    ...
    if(GlobalV::DIP_COR_FLAG)
    {
        ...
        for(int it=0; it<cell.ntype; ++it)
        {
            for(int ia=0; ia<cell.atoms[it].na; ++ia)
            {
                ...
                {
                    fdip(iat, jj) = ModuleBase::e2 * (efield_amp  - tot_dipole) * cell.atoms[it].atom_pp.zv * bvec[jj] / bmod;
                }
                ++iat;
            }
        }
    ...
    else
    {
        ...
        for(int it=0; it<cell.ntype; ++it)
        {
            for(int ia=0; ia<cell.atoms[it].na; ++ia)
            {
                for(int jj=0; jj<3; ++jj)
                {
                    fdip(iat, jj) = ModuleBase::e2 * efield_amp  * cell.atoms[it].atom_pp.zv * bvec[jj] / bmod;
                }
                ++iat;
            }
        }
