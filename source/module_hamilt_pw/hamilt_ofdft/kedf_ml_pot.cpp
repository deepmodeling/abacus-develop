...
void KEDF_ML::get_potential_(const double* const* prho, ModulePW::PW_Basis* pw_rho, ModuleBase::matrix& rpotential)
{
    ...
    for (int ir = 0; ir < this->nx; ++ir)
    {
        pauli_potential[ir] += this->cTF * std::pow(prho[0][ir], 5. / 3.) / prho[0][ir]
                               * (5. / 3. * this->enhancement_cpu_ptr[ir] + this->potGammaTerm(ir) + this->potPTerm1(ir)
                                  + this->potQTerm1(ir) + this->potXiTerm1(ir) + this->potTanhxiTerm1(ir)
...
void KEDF_ML::potGammanlTerm(const double* const* prho, ModulePW::PW_Basis* pw_rho, std::vector<double>& rGammanlTerm)
{
    ...
    for (int ik = 0; ik < this->descriptor2kernel["gammanl"].size(); ++ik)
    {
        ...
        {
            dFdgammanl[ir]
                = this->cTF * std::pow(prho[0][ir], 5. / 3.) * this->gradient_cpu_ptr[ir * this->ninput + d2i];
        }
...
void KEDF_ML::potTanhxinlTerm(const double* const* prho, ModulePW::PW_Basis* pw_rho, std::vector<double>& rTanhxinlTerm)
{
    ...
    for (int ik = 0; ik < this->descriptor2kernel["tanhxi"].size(); ++ik)
    {
        ...
        for (int ir = 0; ir < this->nx; ++ir)
        {
            dFdtanhxi[ir] = this->cTF * std::pow(prho[0][ir], 4. / 3.)
                               ...
                               std::vector<double>& rTanhxi_nlTerm)
{
    ...
    for (int ik = 0; ik < this->descriptor2kernel["tanhxi_nl"].size(); ++ik)
    {
        ...
        {
            dFdtanhxi_nl[ir]
                *= this->cal_tool->dtanh(this->tanhxi[d2k][ir], this->chi_xi[d2k]) / std::pow(prho[0][ir], 1. / 3.);
        }
...
void KEDF_ML::potPPnlTerm(const double* const* prho, ModulePW::PW_Basis* pw_rho, std::vector<double>& rPPnlTerm)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        for (int ir = 0; ir < this->nx; ++ir)
        {
            tempP[i][ir] = (this->ml_p)
                               ? -3. / 20. * this->gradient_cpu_ptr[ir * this->ninput + this->descriptor2index["p"][0]]
                                     * nablaRho[i][ir] / prho[0][ir] * /*Ha2Ry*/ 2.
                               : 0.;
            if (this->ml_pnl)
            {
                tempP[i][ir]
                    += -this->pqcoef * 2. * this->nablaRho[i][ir] / std::pow(prho[0][ir], 8. / 3.) * dFdpnl_tot[ir];
            }
        }
    }
...
void KEDF_ML::potQQnlTerm(const double* const* prho, ModulePW::PW_Basis* pw_rho, std::vector<double>& rQQnlTerm)
{
    ...
    for (int ir = 0; ir < this->nx; ++ir)
    {
        tempQ[ir] = (this->ml_q) ? 3. / 40. * this->gradient_cpu_ptr[ir * this->ninput + this->descriptor2index["q"][0]]
                                       * /*Ha2Ry*/ 2.
                                 : 0.;
        if (this->ml_qnl)
        {
                                   ...
                                   std::vector<double>& rTanhpTanh_pnlTerm)
{
    ...
    for (int ik = 0; ik < this->descriptor2kernel["tanh_pnl"].size(); ++ik)
    {
        ...
        for (int ir = 0; ir < this->nx; ++ir)
        {
            dFdpnl[ir] = this->cTF * std::pow(prho[0][ir], 5. / 3.)
                         * this->cal_tool->dtanh(this->tanh_pnl[d2k][ir], this->chi_pnl[d2k])
                         * this->gradient_cpu_ptr[ir * this->ninput + d2i];
        }
        this->cal_tool->multiKernel(d2k, dFdpnl, pw_rho, dFdpnl);
        for (int ir = 0; ir < this->nx; ++ir)
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        for (int ir = 0; ir < this->nx; ++ir)
        {
            tempP[i][ir] = (this->ml_tanhp)
                               ? -3. / 20. * this->cal_tool->dtanh(this->tanhp[ir], this->chi_p)
                                     * this->gradient_cpu_ptr[ir * this->ninput + this->descriptor2index["tanhp"][0]]
                                     * nablaRho[i][ir] / prho[0][ir] * /*Ha2Ry*/ 2.
                               : 0.;
            if (this->ml_tanh_pnl)
                                   ...
                                   std::vector<double>& rTanhqTanh_qnlTerm)
{
    ...
    for (int ik = 0; ik < this->descriptor2kernel["tanh_qnl"].size(); ++ik)
    {
        ...
        for (int ir = 0; ir < this->nx; ++ir)
        {
            dFdqnl[ir] = this->cTF * std::pow(prho[0][ir], 5. / 3.)
                         * this->cal_tool->dtanh(this->tanh_qnl[d2k][ir], this->chi_qnl[d2k])
                         * this->gradient_cpu_ptr[ir * this->ninput + d2i];
        }
        this->cal_tool->multiKernel(d2k, dFdqnl, pw_rho, dFdqnl);
        for (int ir = 0; ir < this->nx; ++ir)
    ...
    for (int ir = 0; ir < this->nx; ++ir)
    {
        tempQ[ir] = (this->ml_tanhq)
                        ? 3. / 40. * this->cal_tool->dtanh(this->tanhq[ir], this->chi_q)
                              * this->gradient_cpu_ptr[ir * this->ninput + this->descriptor2index["tanhq"][0]]
                              * /*Ha2Ry*/ 2.
                        : 0.;
        if (this->ml_tanh_qnl)
