...
void KEDF_WT::set_para(double dV, double alpha, double beta, double nelec, double tf_weight, double vw_weight, double of_wt_rho0, bool of_hold_rho0, bool read_kernel, std::string kernel_file, ModulePW::PW_Basis *pw_rho)
{
    ...
    this->wt_coef_ = 5./(9. * this->alpha_ * this->beta_ * std::pow(this->rho0_, this->alpha_ + this->beta_ - 5./3.));
 ...
 * \f[ V_{WT} = c_{TF} * [\alpha \rho^{\alpha-1} \int{W(r - r')\rho^{\beta}(r') dr'} + \beta \rho^{\beta-1} \int{W(r' - r)\rho^{\alpha}(r') dr'}] \f]
 * 
 * @param prho charge density
 * @param pw_rho pw basis
...
void KEDF_WT::get_stress(const double * const * prho, ModulePW::PW_Basis *pw_rho, double vw_weight)
{
    ...
    for (int a = 0; a < 3; ++a)
    {
        for (int b = a; b < 3; ++b)
        {
            ...
            {
                this->stress(a,b) *= - std::pow(ModuleBase::PI,2) / (this->alpha_ * this->beta_ * this->kf_ * std::pow(this->rho0_, this->alpha_ + this->beta_ - 2.)) * 2.; // multiply by 2 to convert Hartree to Ry
            }
            else
            {
                this->stress(a,b) *= - std::pow(ModuleBase::PI,2) / (2. * this->alpha_ * this->beta_ * this->kf_ * std::pow(this->rho0_, this->alpha_ + this->beta_ - 2.)) * 2.; // multiply by 2 to convert Hartree to Ry
            }
        }
    }
