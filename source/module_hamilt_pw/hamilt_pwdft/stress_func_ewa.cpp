        ...
        for (; ig < ig_end; ig++)
        {
            ...
            for (int it = 0; it < ucell.ntype; it++)
            {
                for (int i = 0; i < ucell.atoms[it].na; i++)
                {
                    ...
                    rhostar = rhostar
                              + std::complex<FPTYPE>(ucell.atoms[it].ncpp.zv * cosp, ucell.atoms[it].ncpp.zv * sinp);
                }
            }
            rhostar /= ucell.omega;
            ...
            for (int l = 0; l < 3; l++)
            {
                for (int m = 0; m < l + 1; m++)
                {
                    local_sigma(l, m) += sewald * ucell.tpiba2 * 2.0 * rho_basis->gcar[ig][l] * rho_basis->gcar[ig][m]
                                         / g2 * (g2a + 1);
                }
            }
        }
