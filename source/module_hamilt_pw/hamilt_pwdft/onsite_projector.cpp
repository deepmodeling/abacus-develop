...
template <typename T, typename Device>
void projectors::OnsiteProjector<T, Device>::overlap_proj_psi(const int npm, const std::complex<double>* ppsi)
{
...
template <typename T, typename Device>
void projectors::OnsiteProjector<T, Device>::cal_occupations(const psi::Psi<std::complex<T>, Device>* psi_in,
                                                             const ModuleBase::matrix& wg_in)
{
    ...
    GlobalV::ofs_running
        << "-------------------------------------------------------------------------------------------" << std::endl;
    GlobalV::ofs_running << "Orbital Charge Analysis      Charge         Mag(x)         Mag(y)         Mag(z)"
                         << std::endl;
    GlobalV::ofs_running
        << "-------------------------------------------------------------------------------------------" << std::endl;
    ...
    for (int iat = 0; iat < this->ucell->nat; iat++)
    {
        ...
        for (int ih = 0; ih < this->iat_nh[iat]; ih++)
        {
            ...
            if (ih == current_l * current_l - 1)
            {
                ...
                GlobalV::ofs_running << FmtCore::format("%20s", orb_names[current_l - 1])
                                     ...
                                     << fmt_of_chg.format(charge_mag[2]) << fmt_of_chg.format(charge_mag[3])
                                     << std::endl;
                current_l++;
        ...
        mag_y[iat] = sum[2];
        mag_z[iat] = sum[3];
        GlobalV::ofs_running << FmtCore::format("%20s", std::string("Sum")) << "" << fmt_of_chg.format(sum[0])
                             << fmt_of_chg.format(sum[1]) << fmt_of_chg.format(sum[2]) << fmt_of_chg.format(sum[3])
                             << std::endl;
    }
    GlobalV::ofs_running
        << "-------------------------------------------------------------------------------------------" << std::endl;
