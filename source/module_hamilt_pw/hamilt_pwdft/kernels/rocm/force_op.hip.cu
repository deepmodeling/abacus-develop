...
__device__ __forceinline__ void sincos_(float x, float* s, float* c) {
    sincosf(x, s, c);
}

__device__ __forceinline__ void sincos_(double x, double* s, double* c) {
    sincos(x, s, c);
}
...
namespace hamilt {
    ...
    template <typename FPTYPE>
        void cal_force_loc_op<FPTYPE, base_device::DEVICE_GPU>::operator()(
                ...
                FPTYPE* forcelc)
        {
            hipLaunchKernelGGL(HIP_KERNEL_NAME(force_loc_kernel<FPTYPE>),
                    dim3(nat), dim3(THREADS_PER_BLOCK), 0, 0,
                    nat,
                    npw,
                    tpiba_omega,
                    iat2it,
    ...
    template <typename FPTYPE>
        void cal_force_ew_op<FPTYPE, base_device::DEVICE_GPU>::operator()(
                ...
                FPTYPE* forceion)
        {
            hipLaunchKernelGGL(HIP_KERNEL_NAME(force_ew_kernel<FPTYPE>),
                    dim3(nat), dim3(THREADS_PER_BLOCK), 0, 0,
                    nat,
                    npw,
                    ig_gge0,
