...
InitController<T, Device>::InitController(const std::string& init_wfc_in,
...
template <typename T, typename Device>
void InitController<T, Device>::prepare_init(Structure_Factor* p_sf, //< structure factor
...
#endif
{
    ...
    ModuleBase::timer::tick("InitController", "prepare_init");
    ...
    {
        ModuleBase::WARNING_QUIT("InitController::prepare_init",
...
template <typename T, typename Device>
void InitController<T, Device>::allocate_psi(Psi<std::complex<double>>*& psi,
                                             const int nkstot,
                                             const int nks,
                                             const int* ngk,
                                             const int npwx,
                                             Structure_Factor* p_sf)
{
    ...
    // Because the calling relationship between InitController and derived class is
    ...
    // LCAOINPW also temporarily uses InitController workflow, but in principle, it
...
template <typename T, typename Device>
void InitController<T, Device>::make_table(const int nks, Structure_Factor* p_sf)
{
...
template <typename T, typename Device>
void InitController<T, Device>::initialize_psi(Psi<std::complex<double>>* psi,
                                               ...
                                               std::ofstream& ofs_running)
{
    if (!this->use_psiinitializer)
        return;
    ModuleBase::timer::tick("InitController", "initialize_psi");
    for (int ik = 0; ik < this->pw_wfc->nks; ik++)
    {
        ...
        if (psig.expired())
        {
            ModuleBase::WARNING_QUIT("InitController::initialize_psi", "psig lifetime is expired");
        }
...
template class InitController<std::complex<float>, base_device::DEVICE_CPU>;
template class InitController<std::complex<double>, base_device::DEVICE_CPU>;
#if ((defined __CUDA) || (defined __ROCM))
template class InitController<std::complex<float>, base_device::DEVICE_GPU>;
template class InitController<std::complex<double>, base_device::DEVICE_GPU>;
#endif
} // namespace psi