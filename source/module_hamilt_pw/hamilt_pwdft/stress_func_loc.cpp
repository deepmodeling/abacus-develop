...
#pragma omp parallel for collapse(2) reduction(+ : evloc)
        for (int it = 0; it < ucell.ntype; it++)
        {
            for (int ig = 0; ig < rho_basis->npw; ig++)
            {
                ...
                {
                    evloc += vloc(it, rho_basis->ig2igg[ig]) * (p_sf->strucFac(it, ig) * conj(aux[ig])).real();
                }
                else
                {
                    evloc += vloc(it, rho_basis->ig2igg[ig]) * (p_sf->strucFac(it, ig) * conj(aux[ig]) * fact).real();
                }
            }
        }
    }
    ...
    for (int it = 0; it < ucell.ntype; ++it)
    {
        ...
        else
        {
            ...
            this->dvloc_of_g(atom->ncpp.msh,
                             atom->ncpp.rab.data(),
                             atom->ncpp.r.data(),
                             atom->ncpp.vloc_at.data(),
                             atom->ncpp.zv,
                             dvloc.data(),
                             rho_basis,
                             ucell);
                ...
                for (int m = 0; m < l + 1; m++)
                {
                    local_sigma(l, m) = local_sigma(l, m)
                                        + (conj(aux[ig]) * p_sf->strucFac(it, ig)).real() * 2.0
                                              * dvloc[rho_basis->ig2igg[ig]] * ucell.tpiba2 * rho_basis->gcar[ig][l]
                                              * rho_basis->gcar[ig][m] * fact;
                }
            }
        }
#ifdef _OPENMP
...
#pragma omp parallel for
    ...
    if (this->device == base_device::GpuDevice)
    {
        ...
        syncmem_var_h2d_op()(gx_arr_d, gx_arr.data(), rho_basis->ngg + 1);
...
#pragma omp parallel for
    ...
    {
        dvloc[i]
            = ModuleBase::FOUR_PI * zp * ModuleBase::e2 / ucell.omega / pow((ucell.tpiba2 * rho_basis->gg_uniq[i]), 2);
    }
