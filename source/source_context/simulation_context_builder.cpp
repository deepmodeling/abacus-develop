...
bool field_is(const char* field, const char* expected)
{
    return std::strcmp(field, expected) == 0;
}

template <typename Domain, typename T>
void project_input_field(Domain& domain, const char* field, const T& value)
{
    domain.project(field, value);
}

template <typename T>
void project_input_field(RunControl& domain, const char* field, const T& value)
{
    if (field_is(field, "calculation") || field_is(field, "esolver_type") || field_is(field, "out_level")
        || field_is(field, "cal_force") || field_is(field, "cal_stress"))
    {
        return;
    }
    domain.project(field, value);
...
template <typename T>
void project_input_field(SpinConfig& domain, const char* field, const T& value)
{
    if (field_is(field, "nspin") || field_is(field, "lspinorb") || field_is(field, "noncolin"))
    {
        return;
    }
    domain.project(field, value);
...
template <typename T>
void project_input_field(SolverConfig& domain, const char* field, const T& value)
{
    if (field_is(field, "ks_solver") || field_is(field, "basis_type"))
    {
        return;
    }
    domain.project(field, value);
...
template <typename T>
void project_input_field(MatrixOutputConfig& domain, const char* field, const T& value)
{
    if (field_is(field, "out_mat_hs") || field_is(field, "out_mat_hs2")
        || field_is(field, "out_mat_tk") || field_is(field, "out_mat_t") || field_is(field, "out_mat_r")
        || field_is(field, "out_mat_dh") || field_is(field, "out_mat_ds")
        || field_is(field, "out_mat_h_t") || field_is(field, "out_mat_h_vnl")
        || field_is(field, "out_mat_h_vl") || field_is(field, "out_mat_h_vh")
        || field_is(field, "out_mat_h_vxc") || field_is(field, "out_mat_h_exx")
        || field_is(field, "out_mat_dh_t") || field_is(field, "out_mat_dh_vnl")
        || field_is(field, "out_mat_dh_vl") || field_is(field, "out_mat_dh_vh")
        || field_is(field, "out_mat_dh_vxc") || field_is(field, "out_mat_dh_exx")
        || field_is(field, "out_mat_xc2") || field_is(field, "out_mat_xc")
        || field_is(field, "out_eband_terms") || field_is(field, "out_app_flag")
        || field_is(field, "out_ndigits"))
    {
        return;
    }
    domain.project(field, value);
...
template <typename T>
void project_input_field(GeneralOutputConfig& domain, const char* field, const T& value)
{
    if (field_is(field, "out_freq_elec") || field_is(field, "out_freq_ion")
        || field_is(field, "out_freq_td") || field_is(field, "out_alllog"))
    {
        return;
    }
    domain.project(field, value);
...
template <typename T>
void project_input_field(DftUConfig& domain, const char* field, const T& value)
{
    if (field_is(field, "dft_plus_u") || field_is(field, "dft_plus_dmft"))
    {
        return;
    }
    domain.project(field, value);
...
template <typename T>
void project_input_field(ExactExchangeConfig& domain, const char* field, const T& value)
{
    if (field_is(field, "exx_separate_loop") || field_is(field, "exx_hybrid_step")
        || field_is(field, "exx_mixing_beta") || field_is(field, "exx_symmetry_realspace"))
    {
        return;
    }
    domain.project(field, value);
...
template <typename T>
void project_exx_state_field(ExactExchangeState& state, const char* field, const T& value)
{
    if (field_is(field, "info_global.cal_exx") || field_is(field, "info_global.hybrid_alpha")
        || field_is(field, "info_global.hse_omega") || field_is(field, "info_ri.real_number"))
    {
        return;
    }
...
#define MODULE_CONTEXT_EXX_STATE_FIELD(section, field) \
        project_exx_state_field(state, #section "." #field, GlobalC::exx_info.section.field);
...
{
#define MODULE_CONTEXT_INPUT_FIELD(domain, field) project_input_field(context.domain, #field, input.field);
...
{
#define MODULE_CONTEXT_SYSTEM_FIELD(domain, field) static_cast<void>(system.field);
...
SimulationContext SimulationContextBuilder::finalize(const System_para& system)
{
    if (finalized_)
    {
    ...
    capture_system(context_, system);
#define MODULE_CONTEXT_GLOBALV_FIELD(domain, field) static_cast<void>(GlobalV::field);
