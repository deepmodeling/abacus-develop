...
class SimulationContextBuilderTest : public ::testing::Test
{
  protected:
    ...
    ModuleContext::SimulationContext freeze(Input_para& input, System_para& system)
    {
        ...
        return builder.finalize(input, system);
    }
};
...
#define MODULE_CONTEXT_INPUT_FIELD(domain, field)                                                        \
    ...
    {                                                                                                    \
        ...
        EXPECT_EQ(1u, context.domain.mapped_values.count(#field)) << "missing INPUT field " << #field;  \
    } while (false);
...
TEST_F(SimulationContextBuilderTest, RequiresRuntimeCaptureBeforeFreeze)
{
    ...
    EXPECT_THROW(builder.finalize(input, system), std::logic_error);
    ...
    const ModuleContext::SimulationContext context = builder.finalize(input, system);
...
#define MODULE_CONTEXT_EXX_STATE_FIELD(section, field)                                      \
    ...
    {                                                                                        \
        ...
        EXPECT_EQ(1u, exx.mapped_values.count(name)) << "missing EXX state field " << name; \
    } while (false);
