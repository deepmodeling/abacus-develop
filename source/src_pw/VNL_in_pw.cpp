...
//----------------------------------------------------------
	...
	for (it = 0;it < ntype; it++)
	{
		...
		for (int ibeta = 0; ibeta < GlobalC::ucell.atoms[it].atom_pp.nbeta; ibeta++) 
		{
			GlobalV::ofs_running << " projector " << ibeta+1 << " L=" << GlobalC::ucell.atoms[it].atom_pp.lll[ibeta] <<  std::endl;
			this->lmaxkb = std::max( this->lmaxkb, GlobalC::ucell.atoms[it].atom_pp.lll[ibeta]);
		}
	}
...
//----------------------------------------------------------
	...
	for (it=0;it<ntype;it++)
	{	
		this->nhm = std::max(nhm, GlobalC::ucell.atoms[it].atom_pp.nh);
	}
...
//----------------------------------------------------------
	...
	for (it=0; it<ntype; it++)
	{
		this->nkb += GlobalC::ucell.atoms[it].atom_pp.nh * GlobalC::ucell.atoms[it].na;
	}
...
void pseudopot_cell_vnl::getvnl(const int &ik, ModuleBase::ComplexMatrix& vkb_in)const
{
	...
	for(int it = 0;it < GlobalC::ucell.ntype;it++)
	{
		...
		const int nbeta = GlobalC::ucell.atoms[it].atom_pp.nbeta;
		const int nh = GlobalC::ucell.atoms[it].atom_pp.nh;
...
void pseudopot_cell_vnl::init_vnl(UnitCell_pseudo &cell)
{
	...
	{
		int BetaIndex=0;
		const int Nprojectors = cell.atoms[it].atom_pp.nh;
		for (int ib=0; ib<cell.atoms[it].atom_pp.nbeta; ib++)
		{
			const int l = cell.atoms[it].atom_pp.lll [ib];
			const double j = cell.atoms[it].atom_pp.jjj [ib];
		...
		if(cell.atoms[it].atom_pp.has_so )
		{
			Soc soc;
...
//
			for(int ip = 0;ip<Nprojectors; ++ip)
			{
				...
				for(int ip2=0; ip2<Nprojectors; ++ip2)
				{
					...
					for(int is1=0;is1<2;++is1)
					{
						for(int is2=0;is2<2;++is2)
						{
							this->dvan_so(ijs,it,ip,ip2) = cell.atoms[it].atom_pp.dion(ir, is) * soc.fcoef(it,is1,is2,ip,ip2);
							++ijs;
		...
		for (int ip=0; ip<Nprojectors; ip++)
		{
			for (int ip2=0; ip2<Nprojectors; ip2++)
			{
				     ...
				     this->nhtolm(it, ip) == nhtolm(it, ip2) )
				{
					...
					if(GlobalV::LSPINORB)
					{
						this->dvan_so(0,it,ip,ip2) = cell.atoms[it].atom_pp.dion(ir, is);
						this->dvan_so(3,it,ip,ip2) = cell.atoms[it].atom_pp.dion(ir, is);
					}
					else
					{
						this->dvan(it, ip, ip2) = cell.atoms[it].atom_pp.dion(ir, is);
					}
				}
			} 
		} 
	...
	************************************************************/
	...
	for (int it = 0;it < cell.ntype;it++)  
	{
		const int nbeta = cell.atoms[it].atom_pp.nbeta;
		int kkbeta = cell.atoms[it].atom_pp.kkbeta;
		...
		for (int ib = 0;ib < nbeta;ib++)
		{
			...
			for (int iq=0; iq<GlobalV::NQX; iq++)  
			{
				...
				ModuleBase::Sphbes::Spherical_Bessel(kkbeta, cell.atoms[it].atom_pp.r, q, l, jl);

				for (int ir = 0;ir < kkbeta;ir++)
				{
					aux[ir] = cell.atoms[it].atom_pp.betar(ib, ir) *
					          jl[ir] * cell.atoms[it].atom_pp.r[ir];
				} 
				double vqint;
				ModuleBase::Integral::Simpson_Integral(kkbeta, aux, cell.atoms[it].atom_pp.rab, vqint);
...
void pseudopot_cell_vnl::init_vnl_alpha(void)          // pengfei Li 2018-3-23
{
	...
	for(int it=0;it<GlobalC::ucell.ntype;it++)
	{
		...
		for (int ib=0; ib<GlobalC::ucell.atoms[it].atom_pp.nbeta; ib++)
		{
			const int l = GlobalC::ucell.atoms[it].atom_pp.lll [ib];
			for(int m=0; m<2*l+1; m++)
			{
				this->nhtol(it,BetaIndex) = l;
				this->nhtolm(it,BetaIndex) = l*l + m;
	...
	{
		const int nbeta = GlobalC::ucell.atoms[it].atom_pp.nbeta;
		int kkbeta = GlobalC::ucell.atoms[it].atom_pp.kkbeta;
		...
		for (int ib = 0;ib < nbeta;ib++)
		{
			for (int L = 0; L <= lmaxkb+1; L++)
			{
				...
				{
					const double q = iq * GlobalV::DQ;
					ModuleBase::Sphbes::Spherical_Bessel(kkbeta, GlobalC::ucell.atoms[it].atom_pp.r, q, L, jl);
					
					for (int ir = 0;ir < kkbeta;ir++)
					{
						aux[ir] = GlobalC::ucell.atoms[it].atom_pp.betar(ib, ir) * jl[ir] * 
								  GlobalC::ucell.atoms[it].atom_pp.r[ir] * GlobalC::ucell.atoms[it].atom_pp.r[ir];
					}
					double vqint;
					ModuleBase::Integral::Simpson_Integral(kkbeta, aux, GlobalC::ucell.atoms[it].atom_pp.rab, vqint);
