 ...
 * Currently, this test is very much involved due to the use of extensively
 * called global classes. It will be adjusted to test the corresponding
 * new interfaces in HSolver soon.
 * The Inputs/Outputs are taken from an integrated test:
...
class ENVEnvironment : public ::testing::Environment
{
  public:
    ...
    void SetUp() override
    {
        ...
        INPUT.latname = env->latname_;
        INPUT.ntype = env->ntype_;
...
TEST_F(CHGTest, SumBand)
{
    ...
    // step 1: parse info. of latname, ntype, lmaxmax, init_vel, fixed_axes
    GlobalC::ucell.setup(INPUT.latname, INPUT.ntype, INPUT.lmaxmax, INPUT.init_vel, INPUT.fixed_axes);
    EXPECT_EQ(GlobalC::ucell.latName, INPUT.latname);
    EXPECT_EQ(GlobalC::ucell.ntype, INPUT.ntype);
    EXPECT_EQ(GlobalC::ucell.lmaxmax, INPUT.lmaxmax);
    EXPECT_EQ(GlobalC::ucell.init_vel, INPUT.init_vel);
    ...
    // calculate total number of electrons and NBANDS
    ...
    // std::cout <<INPUT.ecutrho<<std::endl; ecutrho is still 0 after this step
    ...
    // we need to supply cell_factor here
    GlobalC::ppcell.cell_factor = INPUT.cell_factor;
    ...
    // we need to supply init_chg here
    // GlobalC::pot.init_pot(0, GlobalC::pw.strucFac); //atomic_rho, v_of_rho, set_vrs
    GlobalC::pot.newd();
    ///////
    // we need to supply GlobalC::wf.init_wfc here
    ...
    //====== output wavefunction (electrons.cpp line:365) ===============
    // WF_io::write_wfc2( ssw.str(), GlobalC::wf.evc, GlobalC::pw.gcar);

    //====== read band energy ===========================================
    std::stringstream ssb;
    ssb << "BANDS_1.dat";
    std::ifstream ifsb(ssb.str());
    for (int ik = 0; ik < GlobalC::kv.nkstot; ik++)
    {
        int order;
        double path;
