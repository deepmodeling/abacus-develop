...
void Stress_Func::stress_cc(ModuleBase::matrix& sigma, ModulePW::PW_Basis* rho_basis, const bool is_pw)
{
	...
	{
		if(GlobalC::ucell.atoms[nt].atom_pp.nlcc) 
		{
			judge++;
		}
	}
	...
	for(int nt=0;nt<GlobalC::ucell.ntype;nt++)
	{
		if(GlobalC::ucell.atoms[nt].atom_pp.nlcc)
		{
			...
			GlobalC::CHR.non_linear_core_correction(
				...
				GlobalC::ucell.atoms[nt].atom_pp.msh,
				GlobalC::ucell.atoms[nt].atom_pp.r,
				GlobalC::ucell.atoms[nt].atom_pp.rab,
				GlobalC::ucell.atoms[nt].atom_pp.rho_atc,
				rhocg,
				rho_basis);
