...
//----------------------------------------------------------
    ...
    {
        charge += cell.atoms[it].na * cell.atoms[it].atom_pp.zv;//mohan modify 2007-11-7
    }
    ...
    for (int ig = 0; ig < rho_basis->npw; ig++)
    {
        ...
        for (int it=0; it<cell.ntype; it++)
        {
            rhon += static_cast<double>( cell.atoms[it].atom_pp.zv ) * conj( GlobalC::sf.strucFac(it, ig));
        }
...
//	std::cout << "\n ewaldg = " << ewaldg;
	...
	if (rho_basis->ig_gge0 >= 0)
	{
    	for (int it = 0; it < cell.ntype;it++)
    	{
        	ewaldg = ewaldg - cell.atoms[it].na * cell.atoms[it].atom_pp.zv * cell.atoms[it].atom_pp.zv * sqrt(8.0 / ModuleBase::TWO_PI * alpha);
		}
                ...
                for (na = 0; na < cell.atoms[nt1].na; na++)
                {
                    for (nb = 0; nb < cell.atoms[nt2].na; nb++)
                    {
                        ...
                        for (nr = 0;nr < nrm;nr++)
                        {
                            ...
                            ewaldr = ewaldr + cell.atoms[nt1].atom_pp.zv * cell.atoms[nt2].atom_pp.zv *
