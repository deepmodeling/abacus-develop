...
DistBCDMatrix::~DistBCDMatrix () { delete[] prowpcol2pnum; }

int
    DistBCDMatrix::globalRow (const int localRow)
{
    return (localRow / nblk * nprows + myprow) * nblk + localRow % nblk;
}

int
    DistBCDMatrix::globalCol (const int localCol)
{
    return (localCol / nblk * npcols + mypcol) * nblk + localCol % nblk;
}

int
    DistBCDMatrix::localRow (const int globalRow, int& myprow)
{
...
int
    DistBCDMatrix::localCol (const int globalCol, int& mypcol)
{
...
int
    DistBCDMatrix::pnum (const int prow, const int pcol)
{
