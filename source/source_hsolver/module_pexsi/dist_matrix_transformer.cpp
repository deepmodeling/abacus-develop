...
inline int
    DistMatrixTransformer::MinimumIndexPosition (const bool isFirst,
                                                 const int nprocs,
                                                 int* size_process,
...
inline void
    DistMatrixTransformer::buildCCSParameter (const int size,
                                              const int nprocs,
...
inline void
    DistMatrixTransformer::buffer2CCSvalue (int nnzLocal, int* buffer2ccsIndex, double* buffer, double* nzvalLocal)
{
...
inline void
    DistMatrixTransformer::countMatrixDistribution (int N, double* A, std::map<int, int>& P)
{
    for (int i = 0; i < N; ++i)
        {
            int key = 0;
...
inline int
    DistMatrixTransformer::getNonZeroIndex (char layout,
                                            const int nrow,
                                            const int ncol,
                                            double* H_2d,
                                            double* S_2d,
...
int
    DistMatrixTransformer::buildTransformParameter (DistBCDMatrix& SRC_Matrix,
                                                    DistCCSMatrix& DST_Matrix,
    ...
    if (myproc == 0)
        {
            ...
            for (int i = 0; i < nproc_data; ++i)
                {
                    MPI_Group_translate_ranks (DST_Matrix.get_group_data (),
                                               1,
                                               &i,
                                               GROUP_TRANS,
                                               &proc_map_data_trans[i]);
                }
...
int
    DistMatrixTransformer::newGroupCommTrans (DistBCDMatrix& SRC_Matrix,
                                              DistCCSMatrix& DST_Matrix,
                                              MPI_Group& GROUP_TRANS,
                                              MPI_Comm& COMM_TRANS)
{
...
int
    DistMatrixTransformer::deleteGroupCommTrans (MPI_Group& GROUP_TRANS, MPI_Comm& COMM_TRANS)
{
...
int
    DistMatrixTransformer::transformBCDtoCCS (DistBCDMatrix& SRC_Matrix,
                                              double* H_2d,
                                              double* S_2d,
                                              const double ZERO_Limit,
                                              DistCCSMatrix& DST_Matrix,
...
int
    DistMatrixTransformer::transformCCStoBCD (DistCCSMatrix& SRC_Matrix,
                                              double* DMnzvalLocal,
    ...
    if (COMM_TRANS != MPI_COMM_NULL)
        {
            ...
            if (myproc_trans == 0)
                {
                    ...
                    for (int i = 0; i < nproc_bcd; ++i)
                        {
                            MPI_Group_translate_ranks (DST_Matrix.get_group (),
                                                       1,
                                                       &i,
                                                       GROUP_TRANS,
                                                       &proc_map_bcd_trans[i]);
                        }
            ...
            for (int i = 1; i < NPROC_TRANS; ++i)
                {
                    sender_displacement_process[i] = sender_displacement_process[i - 1] + sender_size_process[i - 1];
                    receiver_displacement_process[i]
                        = receiver_displacement_process[i - 1] + receiver_size_process[i - 1];
                    receiver_size += receiver_size_process[i];
                }
