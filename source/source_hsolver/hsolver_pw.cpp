...
void
    HSolverPW<T, Device>::cal_smooth_ethr (const double& wk,
                                           const double* wg,
                                           const double& ethr,
    ...
    if (wk > 0.0)
        {
            ...
            // In ABACUS, We applied a smoothing process to this truncation to avoid abrupt changes in energy errors
...
template <typename T, typename Device>
void
    HSolverPW<T, Device>::solve (hamilt::Hamilt<T, Device>* pHamilt,
                    ...
                    if (skip_charge)
                        {
                            GlobalV::ofs_running << " Average iterative diagonalization steps for k-points " << ik
                                                 ...
                                                 << "\n current threshold of diagonalization is " << this->diag_thr
                                                 << std::endl;
...
template <typename T, typename Device>
void
    HSolverPW<T, Device>::hamiltSolvePsiK (hamilt::Hamilt<T, Device>* hm,
                                           psi::Psi<T, Device>& psi,
                                           ...
                                           const int& nk_nums)
{
    ...
    if (npw_total < nbands)
        {
            const std::string msg = "npw_total < nbands (" + std::to_string (npw_total) + " < "
                                    + std::to_string (nbands)
                                    + "): the total number of plane waves across all MPI processes "
                                    + "is less than the number of bands, " + "which leads to a rank-deficient problem. "
    ...
    if (this->method == "cg")
        {
            ...
            auto subspace_func
                = [hm, cur_nbasis] (T* psi_in, T* psi_out, const int ld_psi, const int nband, const bool S_orth)
                {
            ...
            DiagoIterAssist<T, Device>::avg_iter += static_cast<double> (cg.diag (hpsi_func,
                                                                                  spsi_func,
                                                                                  psi.get_nbasis (),
                                                                                  psi.get_nbands (),
                                                                                  psi.get_current_ngk (),
    ...
    else if (this->method == "dav_subspace")
        {
            ...
            DiagoIterAssist<T, Device>::avg_iter += static_cast<double> (dav_subspace.diag (hpsi_func,
                                                                                            spsi_func,
                                                                                            psi.get_pointer (),
                                                                                            psi.get_nbasis (),
                                                                                            eigenvalue,
...
template <typename T, typename Device>
void
    HSolverPW<T, Device>::update_precondition (std::vector<Real>& h_diag,
                                               const int ik,
                                               const int npw,
                                               const Real vl_of_0)
{
...
template <typename T, typename Device>
void
    HSolverPW<T, Device>::output_iterInfo ()
{
...
template <typename T, typename Device>
void
    HSolverPW<T, Device>::build_k_neighbors ()
{
    ...
    struct KPoint
    {
        ...
        KPoint (const ModuleBase::Vector3<double>& v, int i) : kvec (v), index (i), norm (v.norm ()) {}
    };
    ...
    std::sort (klist.begin (), klist.end (), [] (const KPoint& a, const KPoint& b) { return a.norm < b.norm; });
...
template <typename T, typename Device>
void
    HSolverPW<T, Device>::propagate_psi (psi::Psi<T, Device>& psi, const int from_ik, const int to_ik)
{
    const int nbands = psi.get_nbands ();
