...
void
    HSolverLCAO<TK, Device>::solve (hamilt::Hamilt<TK>* pHamilt,
                                    psi::Psi<TK>& psi,
...
void
    HSolverLCAO<T, Device>::hamiltSolvePsiK (hamilt::Hamilt<T>* hm, psi::Psi<T>& psi, double* eigenvalue)
{
...
template <typename T, typename Device>
void
    HSolverLCAO<T, Device>::parakSolve (hamilt::Hamilt<T>* pHamilt,
                                        psi::Psi<T>& psi,
                                        elecstate::ElecState* pes,
                                        int kpar)
{
    ...
    for (int ik = 0; ik < k2d.get_pKpoints ()->get_max_nks_pool (); ++ik)
        {
            ...
            auto psi_pool
                = psi::Psi<T> (1, ncol_bands_pool, k2d.get_p2D_pool ()->nrow, k2d.get_p2D_pool ()->nrow, true);
            ...
            if (ik_global < psi.get_nk () && ik < k2d.get_pKpoints ()->nks_pool[k2d.get_my_pool ()])
                {
                    /// local psi in pool
                    psi_pool.fix_k (0);
                    hamilt::MatrixBlock<T> hk_pool
                        = hamilt::MatrixBlock<T>{k2d.hk_pool.data (),
                                                 (size_t)k2d.get_p2D_pool ()->get_row_size (),
                                                 (size_t)k2d.get_p2D_pool ()->get_col_size (),
                                                 k2d.get_p2D_pool ()->desc};
                    hamilt::MatrixBlock<T> sk_pool
                        = hamilt::MatrixBlock<T>{k2d.sk_pool.data (),
                                                 (size_t)k2d.get_p2D_pool ()->get_row_size (),
                                                 (size_t)k2d.get_p2D_pool ()->get_col_size (),
                                                 k2d.get_p2D_pool ()->desc};
                    /// solve eigenvector and eigenvalue for H(k)
...
template <typename T, typename Device>
void
    HSolverLCAO<T, Device>::parakSolve_cusolver (hamilt::Hamilt<T>* pHamilt,
                                                 psi::Psi<T>& psi,
                                                 elecstate::ElecState* pes)
{
    ModuleBase::timer::start ("HSolverLCAO", "parakSolve");
