...
void
    pxxxgvx_post_processing (const int info,
...
void
    get_lwork (int& lwork, std::vector<double>& work)
{
    lwork = work[0];
}

void
    get_lwork (int& lwork, std::vector<float>& work)
{
    lwork = work[0];
}

void
    get_lwork (int& lwork, std::vector<std::complex<double>>& work)
{
    lwork = work[0].real ();
}

void
    get_lwork (int& lwork, std::vector<std::complex<float>>& work)
{
    lwork = work[0].real ();
}
...
template <typename T>
void
    pxxxgvx_diag (const int* const desc,
                  const int ncol,
                  const int nrow,
                  const int nbands,
                  const T* const h_mat,
    ...
    while (true)
        {
            ...
            std::vector<typename GetTypeReal<T>::type> rwork (
                3,
                0); // only used for complex<float> and std::complex<double>
