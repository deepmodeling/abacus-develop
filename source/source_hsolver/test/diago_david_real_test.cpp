...
void
    lapackEigen (int& npw, std::vector<double>& hm, double* e, bool outtime = false)
{
    int info = 0;
    auto tmp = hm;
    clock_t start, end;
...
class DiagoDavPrepare
{
  public:
    ...
    void
        CompareEigen (psi::Psi<double>& phi, double* precondition)
    {
        ...
        if (mypnum == 0)
            {
                lapackEigen (npw, DIAGOTEST::hmatrix_d, e_lapack, DETAILINFO);
            }
        ...
        auto hpsi_func = [phm] (double* psi_in, double* hpsi_out, const int ld_psi, const int nvec)
            {
