...
{
void
    process_2d (int& nprows, int& npcols, int& myprow, int& mypcol, int& icontxt) // map the processes to 2D matrix
{
    int nprocs, mypnum;
...
inline int
    na_rc (int n, int nblk, int nprows, int myprow) // calculate the row/column number in each process
{
    int i, na = 0;
    for (i = 0; i < n; i++)
...
template <class T>
void
    distribute_data (T* hmatrix, T* new_matrix, int& nFull, int& nblk, int& na_rows, int& na_cols, int& icontxt)
{
    int mypnum;
    MPI_Comm_rank (MPI_COMM_WORLD, &mypnum);
                            ...
                            if (myprow == recv_prow && mypcol == recv_pcol)
                                {
                                    for (int i = 0; i < pass_length; i++)
                                        {
                                            new_matrix[(nm_col + i) * na_rows + nm_row]
                                                = hmatrix[nFull * row + col + i];
                                        }
                                }
                            else
                                {
...
template <class T>
inline void
    print_matrix (std::ofstream& fp, T* matrix, int& nrow, int& ncol, bool row_first)
{
...
template <class T>
bool
    read_hs (std::string fname, T& matrix)
{
    int ndim;
...
void
    lapack_diago (double* hmatrix, double* smatrix, double* e, int& nFull)
{
