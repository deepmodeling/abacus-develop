...
class HamiltTEST : public hamilt::Hamilt<T>
{
  public:
    ...
    void
        matrix (hamilt::MatrixBlock<T>& hk_in, hamilt::MatrixBlock<T>& sk_in)
    {
    ...
    void
        constructHamilt (const int iter, const hamilt::MatrixBlock<double> rho)
    {
    }
    void
        updateHk (const int ik)
    {
    }
};
...
class PexsiPrepare
{
  public:
        ...
        read_ref ()
    {
        ...
        if (nread != nlocal)
            {
                std::cout << "Error: the number of global orbitals in the reference file is not equal to the current "
                             "calculation"
                          << std::endl;
                return false;
            }
    ...
    bool
        compare_ref (std::stringstream& out_info)
    {
        double maxerror = 0.0;
        int iindex = 0;
        bool pass = true;
                        ...
                        if (myprow == recv_prow && mypcol == recv_pcol)
                            {
                                ...
                                for (int i = 0; i < pass_length; i++)
                                    {
                                        diff = std::abs (dm_local[0][(nm_col + i) * hmtest.nrow + nm_row]
                                                         - dm[nlocal * row + col + i]);
                                        if (diff > maxerror)
                                            {
                                                maxerror = diff;
...
INSTANTIATE_TEST_SUITE_P (DiagoTest,
                          PexsiGammaOnlyTest,
                          ::testing::Values ( // int nlocal, int nbands, int nb2d, int sparsity, std::string
                                              // ks_solver_in, std::string hfname, std::string sfname
                              PexsiPrepare<double> (0,
                                                    0,
                                                    2,
                                                    0,
