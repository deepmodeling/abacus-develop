...
void
    lapackEigen (int& npw, std::vector<std::complex<float>>& hm, float* e, bool outtime = false)
{
    int lwork = 2 * npw;
...
class DiagoDavPrepare
{
  public:
    ...
    void
        CompareEigen (psi::Psi<std::complex<float>>& phi, float* precondition)
    {
        ...
        if (mypnum == 0)
            {
                lapackEigen (npw, DIAGOTEST::hmatrix_f, e_lapack, DETAILINFO);
            }
        ...
        auto hpsi_func
            = [phm] (std::complex<float>* psi_in, std::complex<float>* hpsi_out, const int ld_psi, const int nvec)
            {
        ...
        auto spsi_func =
            [phm] (const std::complex<float>* psi_in, std::complex<float>* spsi_out, const int ld_psi, const int nbands)
        ...
        if (mypnum == 0)
            {
                if (DETAILINFO)
                    {
                        std::cout << "diag Run time: " << use_time << std::endl;
                    }
...
TEST_P (DiagoDavTest, RandomHamilt)
{
    ...
    if (DETAILINFO && ddp.mypnum == 0)
        {
            std::cout << "npw=" << ddp.npw << ", nband=" << ddp.nband << ", sparsity=" << ddp.sparsity
                      << ", eps=" << ddp.eps << std::endl;
        }
    ...
    for (int i = 0; i < DIAGOTEST::npw; i++)
        precondition_local[i] = (hpsi.precond ())[i];
#endif
