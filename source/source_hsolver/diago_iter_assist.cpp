...
void
    DiagoIterAssist<T, Device>::diag_subspace (
...
template <typename T, typename Device>
void
    DiagoIterAssist<T, Device>::diag_subspace_init (
        ...
        const std::function<void (const T* const, const int, const int)>& export_vcc)
{
    ...
    if (pHamilt->ops == nullptr)
        {
            ModuleBase::WARNING ("DiagoIterAssist::diag_subspace_init",
    ...
    if (base_device::get_device_type (ctx) == base_device::GpuDevice)
        {
            ...
            for (int i = 0; i < nstart; i++)
                {
                    // psi_temp is one band psi, psi is all bands psi, the range always is 1 for the only band in
                    // psi_temp
...
template <typename T, typename Device>
void
    DiagoIterAssist<T, Device>::diag_heevx (const int matrix_size,
                                            ...
                                            T* v)
{
    ...
    // (const Device *d, const int matrix_size, const int lda, const T *A, const int num_eigenpairs, Real *eigenvalues,
    // T *eigenvectors);
...
template <typename T, typename Device>
void
    DiagoIterAssist<T, Device>::diag_hegvd (const int nstart,
                                            const int nbands,
                                            const T* hcc,
                                            T* scc,
...
template <typename T, typename Device>
void
    DiagoIterAssist<T, Device>::cal_hs_subspace (
...
template <typename T, typename Device>
void
    DiagoIterAssist<T, Device>::diag_responce (const T* hcc,
                                               T* scc,
                                               const int nbands,
...
template <typename T, typename Device>
void
    DiagoIterAssist<T, Device>::diag_subspace_psi (const T* hcc,
                                                   T* scc,
...
template <typename T, typename Device>
bool
    DiagoIterAssist<T, Device>::test_exit_cond (const int& ntry, const int& notconv)
{
    //================================================================
