...
template <>
void
    hegvd_op<double, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* ctx,
                                                           const int nstart,
                                                           const int ldh,
                                                           const double* _hcc,
                                                           double* _scc,
                                                           ...
                                                           double* _vcc)
{
    ...
    if (nstart > N_DCU)
        {
            ...
            hipsolverErrcheck (hipsolverDnDsygvd_bufferSize (hipsolver_H,
            ...
            hipsolverErrcheck (hipsolverDnDsygvd (hipsolver_H,
                                                  HIPSOLVER_EIG_TYPE_1,
                                                  HIPSOLVER_EIG_MODE_VECTOR,
                                                  uplo,
                                                  nstart,
    ...
    // if(fail_info != nullptr) *fail_info = info_gpu;
    else
        {
            ...
            hipErrcheck (hipMemcpy (_eigenvalue,
                                    eigenvalue.data (),
                                    sizeof (double) * eigenvalue.size (),
...
template <>
void
    hegvd_op<std::complex<float>, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* ctx,
                                                                        const int nstart,
                                                                        const int ldh,
    ...
    if (nstart > N_DCU)
        {
            ...
            // calculate the sizes needed for pre-allocated buffer.
            hipsolverErrcheck (hipsolverDnChegvd_bufferSize (hipsolver_H,
                                                             HIPSOLVER_EIG_TYPE_1,
                                                             HIPSOLVER_EIG_MODE_VECTOR,
                                                             uplo,
                                                             nstart,
                                                             reinterpret_cast<const float2*> (_vcc),
                                                             ldh,
                                                             reinterpret_cast<const float2*> (_scc),
                                                             ldh,
                                                             _eigenvalue,
                                                             &lwork));
            ...
            // compute eigenvalues and eigenvectors.
            hipsolverErrcheck (hipsolverDnChegvd (hipsolver_H,
                                                  HIPSOLVER_EIG_TYPE_1,
                                                  HIPSOLVER_EIG_MODE_VECTOR,
                                                  uplo,
                                                  nstart,
                                                  reinterpret_cast<float2*> (_vcc),
                                                  ldh,
                                                  const_cast<float2*> (reinterpret_cast<const float2*> (_scc)),
                                                  ldh,
                                                  _eigenvalue,
                                                  work,
                                                  lwork,
...
template <>
void
    hegvd_op<std::complex<double>, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* ctx,
                                                                         const int nstart,
                                                                         const int ldh,
    ...
    if (nstart > N_DCU)
        {
            ...
            // calculate the sizes needed for pre-allocated buffer.
            hipsolverErrcheck (hipsolverDnZhegvd_bufferSize (hipsolver_H,
                                                             HIPSOLVER_EIG_TYPE_1,
                                                             HIPSOLVER_EIG_MODE_VECTOR,
                                                             uplo,
                                                             nstart,
                                                             reinterpret_cast<const double2*> (_vcc),
                                                             ldh,
                                                             reinterpret_cast<const double2*> (_scc),
                                                             ldh,
                                                             _eigenvalue,
                                                             &lwork));
            ...
            // compute eigenvalues and eigenvectors.
            hipsolverErrcheck (hipsolverDnZhegvd (hipsolver_H,
                                                  HIPSOLVER_EIG_TYPE_1,
                                                  HIPSOLVER_EIG_MODE_VECTOR,
                                                  uplo,
                                                  nstart,
                                                  reinterpret_cast<double2*> (_vcc),
                                                  ldh,
                                                  const_cast<double2*> (reinterpret_cast<const double2*> (_scc)),
                                                  ldh,
                                                  _eigenvalue,
                                                  work,
                                                  lwork,
...
template <>
void
    heevx_op<double, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* ctx,
                                                           const int nstart,
                                                           const int ldh,
                                                           const double* _hcc,
                                                           const int m,
                                                           ...
                                                           double* _vcc)
{
    ...
    hipErrcheck (hipMemcpy (hcc.data (), _hcc, sizeof (double) * hcc.size (), hipMemcpyDeviceToHost));
    base_device::DEVICE_CPU* cpu_ctx = {};
    heevx_op<double, base_device::DEVICE_CPU> () (cpu_ctx,
                                                  nstart,
                                                  ldh,
                                                  hcc.data (),
                                                  m,
...
template <>
void
    heevx_op<std::complex<float>, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* ctx,
                                                                        const int nstart,
                                                                        const int ldh,
                                                                        const std::complex<float>* _hcc,
                                                                        const int m,
...
template <>
void
    heevx_op<std::complex<double>, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* ctx,
                                                                         const int nstart,
                                                                         const int ldh,
                                                                         const std::complex<double>* _hcc,
                                                                         const int m,
...
template <>
void
    hegvx_op<std::complex<float>, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* d,
                                                                        const int nbase,
                                                                        const int ldh,
...
template <>
void
    hegvx_op<std::complex<double>, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* d,
                                                                         const int nbase,
                                                                         const int ldh,
...
template <>
void
    hegvx_op<double, base_device::DEVICE_GPU>::operator() (const base_device::DEVICE_GPU* d,
                                                           const int nbase,
                                                           const int ldh,
                                                           double* hcc,
                                                           double* scc,
