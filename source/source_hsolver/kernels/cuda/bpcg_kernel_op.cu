...
__global__ void
    line_minimize_with_block (thrust::complex<Real>* grad,
...
template <typename Real>
__global__ void
    calc_grad_with_block (const Real* prec,
                          Real* err,
                          Real* beta,
                          thrust::complex<Real>* psi,
                          thrust::complex<Real>* hpsi,
...
template <typename Real>
__global__ void
    apply_eigenvalues_kernel (const thrust::complex<Real>* vectors,
...
template <typename Real>
__global__ void
    precondition_kernel (thrust::complex<Real>* psi_iter,
...
template <typename Real>
__device__ Real
    blockReduceSum (Real val, volatile Real* shared)
{
...
template <typename Real>
__global__ void
    normalize_kernel (thrust::complex<Real>* psi_iter,
                      Real* psi_norm,
                      const int dim,
                      const int nbase,
                      const int notconv)
...
__global__ void
    refresh_hcc_scc_vcc_kernel (const int n, T* hcc, T* scc, T* vcc, const int ldh, const Real* eigenvalue, const T one)
{
...
template <typename T>
void
    line_minimize_with_block_op<T, base_device::DEVICE_GPU>::operator() (T* grad_out,
                                                                         T* hgrad_out,
                                                                         T* psi_out,
                                                                         T* hpsi_out,
                                                                         const int& n_basis,
                                                                         ...
                                                                         const int& n_band)
{
    ...
    line_minimize_with_block<Real><<<n_band, thread_per_block>>> (A, B, C, D, n_basis, n_basis_max);
...
template <typename T>
void
    calc_grad_with_block_op<T, base_device::DEVICE_GPU>::operator() (const Real* prec_in,
                                                                     Real* err_out,
                                                                     Real* beta_out,
                                                                     T* psi_out,
                                                                     T* hpsi_out,
                                                                     ...
                                                                     const int& n_band)
{
    ...
    calc_grad_with_block<Real>
        <<<n_band, thread_per_block>>> (prec_in, err_out, beta_out, A, B, C, D, n_basis, n_basis_max);
...
template <typename T>
void
    apply_eigenvalues_op<T, base_device::DEVICE_GPU>::operator() (const int& nbase,
                                                                  const int& nbase_x,
                                                                  const int& notconv,
                                                                  T* result,
                                                                  const T* vectors,
                                                                  const Real* eigenvalues)
{
    ...
    apply_eigenvalues_kernel<Real>
        <<<grid, threads_per_block>>> (vec_complex, res_complex, eigenvalues, nbase, nbase_x, notconv);
...
template <typename T>
void
    precondition_op<T, base_device::DEVICE_GPU>::operator() (const int& dim,
                                                             T* psi_iter,
                                                             const int& nbase,
                                                             const int& notconv,
                                                             const Real* precondition,
                                                             const Real* eigenvalues)
{
    ...
    precondition_kernel<Real>
        <<<grid, threads_per_block>>> (psi_complex, precondition, eigenvalues, dim, nbase, notconv);
...
template <typename T>
void
    normalize_op<T, base_device::DEVICE_GPU>::operator() (const int& dim,
                                                          T* psi_iter,
                                                          const int& nbase,
                                                          const int& notconv,
                                                          Real* psi_norm)
{
    ...
    normalize_kernel<Real>
        <<<notconv, thread_per_block, sharedMemSize, 0>>> (psi_complex, psi_norm, dim, nbase, notconv);
...
template <>
void
    refresh_hcc_scc_vcc_op<double, base_device::DEVICE_GPU>::operator() (const int& n,
                                                                         double* hcc,
                                                                         double* scc,
                                                                         double* vcc,
                                                                         const int& ldh,
...
template <>
void
    refresh_hcc_scc_vcc_op<std::complex<float>, base_device::DEVICE_GPU>::operator() (const int& n,
                                                                                      ...
                                                                                      const std::complex<float>& one)
{
    int thread = 512;
    int block = (n + thread - 1) / thread;
    refresh_hcc_scc_vcc_kernel<thrust::complex<float>, float>
        <<<block, thread>>> (n,
                             reinterpret_cast<thrust::complex<float>*> (hcc),
                             reinterpret_cast<thrust::complex<float>*> (scc),
                             reinterpret_cast<thrust::complex<float>*> (vcc),
                             ldh,
                             eigenvalue,
...
template <>
void
    refresh_hcc_scc_vcc_op<std::complex<double>, base_device::DEVICE_GPU>::operator() (const int& n,
                                                                                       ...
                                                                                       const std::complex<double>& one)
{
    int thread = 512;
    int block = (n + thread - 1) / thread;
    refresh_hcc_scc_vcc_kernel<thrust::complex<double>, double>
        <<<block, thread>>> (n,
                             reinterpret_cast<thrust::complex<double>*> (hcc),
                             reinterpret_cast<thrust::complex<double>*> (scc),
                             reinterpret_cast<thrust::complex<double>*> (vcc),
                             ldh,
                             eigenvalue,
