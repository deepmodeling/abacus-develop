...
#ifdef __LCAO
static inline void
    xhegvd_wrapper (const cublasFillMode_t& uplo,
                    const int& n,
                    double* A,
                    const int& lda,
                    double* B,
...
#ifdef __LCAO
static inline void
    xheevd_wrapper (const cublasFillMode_t& uplo, const int& n, double* A, const int& lda, double* W)
{
    // prepare some values for cusolverDnZhegvd_bufferSize
    int* devInfo = nullptr;
    int lwork = 0, info_gpu = 0;
...
struct hegvd_op<T, base_device::DEVICE_GPU>
{
    ...
    void
        operator() (const base_device::DEVICE_GPU* d,
                    const int nstart,
                    const int ldh,
                    const T* A, // hcc
                    T* B,       // scc
