...
///
/// The diag_cusolver contains subroutines for generalized eigenvector problems of dense matrice in lcao on a single
/// GPU. Cusolver apis, i.e. cusolverDnDsygvd and cusolverDnZhegvd, are wrapped with respect to different data types for
/// easy calling. For details of low-level methods relevent, one can refer to [cuSOLVER
/// documentation](https://docs.nvidia.com/cuda/cusolver/index.html#cuSolverDN-lt-t-gt-sygvd).
///
...
class Diag_Cusolver_gvd {
  ...
  public:
    ...
    void
    Dngvd_complex(int N, int M, std::complex<double>* A, std::complex<double>* B, double* W, std::complex<double>* V);

    void Dngvd(int N, int M, double* A, double* B, double* W, double* V) { return Dngvd_double(N, M, A, B, W, V); };
