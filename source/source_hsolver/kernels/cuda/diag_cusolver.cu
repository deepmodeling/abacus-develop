...
void
    Diag_Cusolver_gvd::init_double (int N)
{
...
void
    Diag_Cusolver_gvd::init_complex (int N)
{
    // step 2: Malloc A and B on device
    m = lda = N;
...
void
    Diag_Cusolver_gvd::Dngvd_double (int N, int M, double* A, double* B, double* W, double* V)
{
    ...
    CHECK_CUSOLVER (cusolverDnDsygvd_bufferSize (cusolverH, itype, jobz, uplo, m, d_A, lda, d_B, lda, d_W, &lwork));
    CHECK_CUDA (cudaMalloc ((void**)&d_work, sizeof (double) * lwork));

    // compute spectrum of (A,B)
    CHECK_CUSOLVER (
        cusolverDnDsygvd (cusolverH, itype, jobz, uplo, m, d_A, lda, d_B, lda, d_W, d_work, lwork, devInfo));
...
void
    Diag_Cusolver_gvd::Dngvd_complex (int N,
                                      int M,
                                      ...
                                      std::complex<double>* V)
{
    ...
    // The S and D data types are real valued single and double precision, respectively.
    // The C and Z data types are complex valued single and double precision, respectively.
    CHECK_CUSOLVER (cusolverDnZhegvd_bufferSize (cusolverH, itype, jobz, uplo, m, d_A2, lda, d_B2, lda, d_W, &lwork));
    CHECK_CUDA (cudaMalloc ((void**)&d_work2, sizeof (cuDoubleComplex) * lwork));

    // compute spectrum of (A,B)
    CHECK_CUSOLVER (
        cusolverDnZhegvd (cusolverH, itype, jobz, uplo, m, d_A2, lda, d_B2, lda, d_W, d_work2, lwork, devInfo));
    CHECK_CUDA (cudaDeviceSynchronize ());
