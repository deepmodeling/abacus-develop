...
template <>
MPI_Comm
    DiagoElpa<std::complex<double>>::setmpicomm ()
{
    if (this->elpa_num_thread == -1)
        {
            return MPI_COMM_WORLD;
...
template <>
void
    DiagoElpa<std::complex<double>>::diag (hamilt::Hamilt<std::complex<double>>* phm_in,
...
template <>
void
    DiagoElpa<double>::diag (hamilt::Hamilt<double>* phm_in, psi::Psi<double>& psi, Real* eigenvalue_in)
{
    ModuleBase::TITLE ("DiagoElpa", "diag");
#ifdef __MPI
    matd h_mat, s_mat;
...
template <>
void
    DiagoElpa<std::complex<double>>::diag_pool (hamilt::MatrixBlock<std::complex<double>>& h_mat,
...
template <>
void
    DiagoElpa<double>::diag_pool (hamilt::MatrixBlock<double>& h_mat,
...
template <typename T>
bool
    DiagoElpa<T>::ifElpaHandle (const bool& newIteration, const bool& ifNSCF) const
{
    int doHandle = false;
    if (newIteration)
        {
