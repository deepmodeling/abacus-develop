...
template <typename T, typename Device>
void
    PLinearTransform<T, Device>::set_dimension (const int nrowA,
                                                const int ncolA,
                                                const int ncolB,
                                                const int LDA,
#ifdef __MPI
...
template <typename T, typename Device>
void
    PLinearTransform<T, Device>::act (const T alpha, const T* A, const T* U, const T beta, T* B)
{
            ...
            for (int ip = 0; ip < nproc_col; ++ip)
                {
                    if (rank_col != ip)
                        {
                            int size = LDA * ncolA;
                            Parallel_Common::isend_dev<T, Device> (A,
                                                                   size,
                                                                   ip,
                                                                   0,
                                                                   col_world,
                    ...
                    if (ip != rank_col)
                        {
                            ...
                            MPI_Status status;
                            Parallel_Common::recv_dev<T, Device> (Atmp_device,
                                                                  size,
                                                                  ip,
                                                                  0,
                                                                  col_world,
