...
void
    Parallel_K2D<TK>::set_para_env (int nks,
                                    const int& nw,
                                    const int& nb2d,
                                    const int& nproc,
                                    const int& my_rank,
...
template <typename TK>
void
    Parallel_K2D<TK>::distribute_hsk (hamilt::Hamilt<TK>* pHamilt, const std::vector<int>& ik_kpar, const int& nw)
{
#ifdef __MPI
...
template <typename TK>
void
    Parallel_K2D<TK>::unset_para_env ()
{
...
template <typename TK>
void
    Parallel_K2D<TK>::set_kpar (int kpar)
{
    if (kpar < 1)
        {
