...
Diago_DavSubspace<T, Device>::Diago_DavSubspace (const std::vector<Real>& precondition_in,
    ...
    : precondition (precondition_in), n_band (nband_in), dim (nbasis_in), nbase_x (nband_in * david_ndim_in),
      diag_thr (diag_thr_in), iter_nmax (diag_nmax_in), diag_comm (diag_comm_in), diag_subspace (diag_subspace_in),
      diago_subspace_bs (diago_subspace_bs_in)
{
...
template <typename T, typename Device>
int
    Diago_DavSubspace<T, Device>::diag_once (const HPsiFunc& hpsi_func,
            ...
            if ((this->notconv == 0) || (nbase + this->notconv + 1 > this->nbase_x) || (dav_iter == this->iter_nmax))
                {
...
#ifdef __DSP
                    ModuleBase::gemm_op_mt<T, Device> () // In order to not coding another whole template, using this
                                                         // method to minimize the code change.
#else
                    ...
                    if (!this->notconv || (dav_iter == this->iter_nmax))
                    ...
                    else
                        {
                            ...
                            syncmem_complex_2d_op () (this->psi_in_iter,
                                                      this->dim,
                                                      psi_in,
                                                      psi_in_dmax,
                                                      this->dim,
...
template <typename T, typename Device>
void
    Diago_DavSubspace<T, Device>::cal_grad (const HPsiFunc& hpsi_func,
                                            const HPsiFunc& spsi_func,
                                            const int& dim,
                                            const int& nbase,
                                            const int& notconv,
                                            ...
                                            std::vector<Real>* eigenvalue_iter)
{
    ...
    // vcc = - vcc * eigenvalue
    ModuleBase::matrix_mul_vector_op<T, Device> () (nbase,
                                                    notconv,
                                                    vcc,
                                                    this->nbase_x,
                                                    e_temp_hd,
    ...
    if (this->device == base_device::GpuDevice)
        {
            precondition_op<T, Device> () (this->dim, psi_iter, nbase, notconv, d_precondition, this->d_eigenvalue);
        }
    else
#endif
        {
                    ...
                    if (psi_norm_host[i] <= 1.0e-12)
                        {
                            ...
                            std::cout << "This may be due to npwx < nbands: the number of plane waves is less than"
                                      << std::endl;
...
template <typename T, typename Device>
void
    Diago_DavSubspace<T, Device>::cal_elem (const int& dim,
                                            int& nbase,
                                            const int& notconv,
                                            const T* psi_iter,
                                            const T* spsi,
...
template <typename T, typename Device>
void
    Diago_DavSubspace<T, Device>::diag_zhegvx (const int& nbase,
                                               const int& nband,
                                               T* hcc,
                                               T* scc,
                                               const int& nbase_x,
            ...
            if (this->diag_comm.rank == 0)
                {
                    ...
                    ct::kernels::lapack_hegvd<T, ct_Device> () (nbase,
                                                                this->nbase_x,
                                                                this->hcc,
                                                                this->d_scc,
                                                                this->d_eigenvalue,
...
template <typename T, typename Device>
void
    Diago_DavSubspace<T, Device>::refresh (const int& dim,
                                           const int& nband,
                                           int& nbase,
...
template <typename T, typename Device>
int
    Diago_DavSubspace<T, Device>::diag (const HPsiFunc& hpsi_func,
                                        const HPsiFunc& spsi_func,
                                        T* psi_in,
                                        const int psi_in_dmax,
                                        Real* eigenvalue_in_hsolver,
...
template <typename T, typename Device>
bool
    Diago_DavSubspace<T, Device>::test_exit_cond (const int& ntry, const int& notconv, const bool& scf)
{
    // scf = true; // scf
    // scf = false; // nscf
