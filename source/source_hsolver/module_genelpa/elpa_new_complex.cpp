...
int
    ELPA_Solver::eigenvector (std::complex<double>* A, double* EigenValue, std::complex<double>* EigenVector)
{
    int info = 0;
    int allinfo = 0;
    double t = 0.0;
...
int
    ELPA_Solver::generalized_eigenvector (std::complex<double>* A,
...
int
    ELPA_Solver::decomposeRightMatrix (std::complex<double>* B,
                                       double* EigenValue,
                                       std::complex<double>* EigenVector,
                                       int& DecomposedState)
{
...
int
    ELPA_Solver::composeEigenVector (int DecomposedState, std::complex<double>* B, std::complex<double>* EigenVector)
{
    double t = 0.0;
    if (DecomposedState == 1 || DecomposedState == 2)
        {
...
void
    ELPA_Solver::verify (std::complex<double>* A,
                         double* EigenValue,
                         std::complex<double>* EigenVector,
                         double& maxError,
                         double& meanError)
{
    ...
    if (loglevel > 2)
        saveMatrix ("VD.dat", nFull, R, desc, cblacs_ctxt);
    // R=A*V-V*D=A*V-R
    ScalapackConnector::hemm ('L', 'U', nFull, 1.0, A, V, -1.0, R, desc);
    if (loglevel > 2)
        saveMatrix ("AV-VD.dat", nFull, R, desc, cblacs_ctxt);
                         ...
                         double& meanError)
{
    ...
    if (loglevel > 2)
        saveMatrix ("BV.dat", nFull, zwork.data (), desc, cblacs_ctxt);
    // R=B*V*D=zwork*D
    ScalapackConnector::hemm ('L', 'U', nFull, 1.0, B, V, 0.0, zwork.data (), desc);
    if (loglevel > 2)
        saveMatrix ("BVD.dat", nFull, R, desc, cblacs_ctxt);
    // R=A*V-B*V*D=A*V-R
    ScalapackConnector::hemm ('L', 'U', nFull, 1.0, B, V, 0.0, zwork.data (), desc);
    if (loglevel > 2)
        saveMatrix ("AV-BVD.dat", nFull, R, desc, cblacs_ctxt);
    // calculate the maximum and mean value of sum_i{R(:,i)*R(:,i)}
    double sumError = 0;
    maxError = 0;
    for (int i = 1; i <= nev; ++i)
