...
void
    loadMatrix (const char FileName[], int nFull, double* a, int* desca, int blacs_ctxt)
{
    ...
    int M
        = std::max (1,
                    std::min (nFull, (int)(MAX_BUFFER_SIZE / nFull / sizeof (double)))); // at lease 1 row, max size 1GB
    if (myid == ROOT_PROC)
        b = new double[M * N];
    else
        b = new double[1];
...
void
    saveLocalMatrix (const char filePrefix[], int narows, int nacols, double* a)
{
    char FileName[80];
    int myid = 0;
...
void
    saveMatrix (const char FileName[], int nFull, double* a, int* desca, int blacs_ctxt)
{
    int nprows, npcols, myprow, mypcol;
    Cblacs_gridinfo (blacs_ctxt, &nprows, &npcols, &myprow, &mypcol);
    int myid = Cblacs_pnum (blacs_ctxt, myprow, mypcol);
...
// load matrix from the file
void
    loadMatrix (const char FileName[], int nFull, std::complex<double>* a, int* desca, int blacs_ctxt)
{
    int nprows, npcols, myprow, mypcol;
    Cblacs_gridinfo (blacs_ctxt, &nprows, &npcols, &myprow, &mypcol);
    int myid = Cblacs_pnum (blacs_ctxt, myprow, mypcol);
...
void
    saveLocalMatrix (const char filePrefix[], int narows, int nacols, std::complex<double>* a)
{
    char FileName[80];
    int myid;
    std::ofstream matrixFile;
...
// and save to one completed matrix file
void
    saveMatrix (const char FileName[], int nFull, std::complex<double>* a, int* desca, int blacs_ctxt)
{
    int nprows, npcols, myprow, mypcol;
    Cblacs_gridinfo (blacs_ctxt, &nprows, &npcols, &myprow, &mypcol);
    int myid = Cblacs_pnum (blacs_ctxt, myprow, mypcol);
