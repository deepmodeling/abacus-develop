...
double
    set_diagethr_ks (const std::string basis_type,
            ...
            if (calculation_in == "nscf")
                {
                    if (res_diag_ethr - 1e-2 > -1e-5)
                        {
                            res_diag_ethr
                                = std::max (1e-13, 0.1 * std::min (1e-2, PARAM.inp.scf_thr / PARAM.inp.nelec));
                        }
                }
            else if (iter == 1)
                {
...
double
    set_diagethr_sdft (const std::string basis_type,
                       const std::string esolver_type,
                       const std::string calculation_in,
                       const std::string init_chg_in,
                       const int istep,
    ...
    if (basis_type == "pw" && esolver_type == "sdft")
        {
            if (calculation_in == "nscf")
                {
                    res_diag_ethr
                        = std::max (std::min (1e-5, 0.1 * PARAM.inp.scf_thr / std::max (1.0, PARAM.inp.nelec)), 1e-12);
                }
            else if (iter == 1)
                {
                    if (istep == 0)
...
double
    reset_diag_ethr (std::ofstream& ofs_running,
...
double
    cal_hsolve_error (const std::string basis_type,
                      const std::string esolver_type,
                      const double diag_ethr_in,
                      const double nelec_in)
{
