...
template <>
void
    DiagoScalapack<double>::diag (hamilt::Hamilt<double>* phm_in, psi::Psi<double>& psi, Real* eigenvalue_in)
{
...
template <>
void
    DiagoScalapack<std::complex<double>>::diag (hamilt::Hamilt<std::complex<double>>* phm_in,
...
template <>
void
    DiagoScalapack<double>::diag_pool (hamilt::MatrixBlock<double>& h_mat,
...
template <>
void
    DiagoScalapack<std::complex<double>>::diag_pool (hamilt::MatrixBlock<std::complex<double>>& h_mat,
...
std::pair<int, std::vector<int>>
    DiagoScalapack<T>::pdsygvx_once (const int* const desc,
                                     const int ncol,
                                     const int nrow,
...
template <typename T>
std::pair<int, std::vector<int>>
    DiagoScalapack<T>::pzhegvx_once (const int* const desc,
                                     const int ncol,
                                     const int nrow,
...
template <typename T>
void
    DiagoScalapack<T>::pdsygvx_diag (const int* const desc,
                                     const int ncol,
                                     const int nrow,
                                     const double* const h_mat,
                                     const double* const s_mat,
                                     ...
                                     psi::Psi<double>& wfc_2d)
{
    while (true)
        {
            const std::pair<int, std::vector<int>> info_vec
                = pdsygvx_once (desc, ncol, nrow, h_mat, s_mat, ekb, wfc_2d);
...
template <typename T>
void
    DiagoScalapack<T>::pzhegvx_diag (const int* const desc,
                                     const int ncol,
                                     const int nrow,
                                     const std::complex<double>* const h_mat,
                                     const std::complex<double>* const s_mat,
                                     ...
                                     psi::Psi<std::complex<double>>& wfc_2d)
{
    while (true)
        {
            const std::pair<int, std::vector<int>> info_vec
                = pzhegvx_once (desc, ncol, nrow, h_mat, s_mat, ekb, wfc_2d);
            post_processing (info_vec.first, info_vec.second);
            if (info_vec.first == 0)
                {
                    break;
...
template <typename T>
void
    DiagoScalapack<T>::post_processing (const int info, const std::vector<int>& vec)
{
    ...
    if (info == 0)
    ...
    else if (info / 2 % 2)
        {
            ...
            const std::string str_need
                = "degeneracy_need = " + ModuleBase::GlobalFunc::TO_STRING (degeneracy_need) + ".\n";
        ...
        {
            const std::string str_npos
                = "not positive definite = " + ModuleBase::GlobalFunc::TO_STRING (vec[0]) + ".\n";
