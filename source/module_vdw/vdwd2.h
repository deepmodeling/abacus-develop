        ...
        for (int it1 = 0; it1 != ucell_.ntype; ++it1)
        {
            for (int it2 = 0; it2 != ucell_.ntype; ++it2)
            {
                const double C6_product
                    = sqrt(para_.C6().at(ucell_.atoms[it1].atom_pp.psd) * para_.C6().at(ucell_.atoms[it2].atom_pp.psd))
                      / pow(ucell_.lat0, 6);
                const double R0_sum
                    = (para_.R0().at(ucell_.atoms[it1].atom_pp.psd) + para_.R0().at(ucell_.atoms[it2].atom_pp.psd)) / ucell_.lat0;
                if (!R0_sum)
                {
