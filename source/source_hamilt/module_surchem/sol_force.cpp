...
void
    surchem::force_cor_one (const UnitCell& cell,
            ...
            for (int ia = 0; ia < cell.atoms[it].na; ia++)
                {
                    for (int ig = 0; ig < rho_basis->npw; ig++)
                        {
                            std::complex<double> phase = exp (ModuleBase::NEG_IMAG_UNIT * ModuleBase::TWO_PI
                    ...
                    // cout<<"Force1"<<iat<<":"<<" "<<forcesol(iat, 0)<<" "<<forcesol(iat, 1)<<" "<<forcesol(iat,
                    // 2)<<endl;
...
void
    surchem::force_cor_two (const UnitCell& cell, const ModulePW::PW_Basis* rho_basis, ModuleBase::matrix& forcesol)
{
            ...
            for (int ia = 0; ia < cell.atoms[it].na; ia++)
                {
                    ...
                    for (int ig = 0; ig < rho_basis->npw; ig++)
                        {
                            forcesol (iat, 0)
                                -= rho_basis->gcar[ig][0] * imag (conj (Vcav_g[ig] + Vel_g[ig]) * n_pseudo[ig]);
                            forcesol (iat, 1)
                                -= rho_basis->gcar[ig][1] * imag (conj (Vcav_g[ig] + Vel_g[ig]) * n_pseudo[ig]);
                            forcesol (iat, 2)
                                -= rho_basis->gcar[ig][2] * imag (conj (Vcav_g[ig] + Vel_g[ig]) * n_pseudo[ig]);
                        }
                    ...
                    // cout<<"Force2"<<iat<<":"<<" "<<forcesol(iat, 0)<<" "<<forcesol(iat, 1)<<" "<<forcesol(iat,
                    // 2)<<endl;
...
void
    surchem::cal_force_sol (const UnitCell& cell,
                            const ModulePW::PW_Basis* rho_basis,
                            const ModuleBase::matrix& vloc,
                            ModuleBase::matrix& forcesol)
{
