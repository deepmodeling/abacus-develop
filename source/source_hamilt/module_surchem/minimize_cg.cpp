...
void
    surchem::minimize_cg (const UnitCell& ucell,
                          ...
                          int& ncgsol)
{
    ...
    Leps2 (ucell, rho_basis, phi, d_eps, gradphi_G_work, lp, aux_grad_phi, aux_grad_grad_phi_real);
    ...
    while (count < 20000 && sqrt (r2) > 1e-5 && sqrt (rinvLr) > 1e-10)
        {
            ...
            Leps2 (ucell, rho_basis, d, d_eps, gradphi_G_work, lp, aux_grad_phi, aux_grad_grad_phi_real);
...
void
    surchem::Leps2 (const UnitCell& ucell,
                    const ModulePW::PW_Basis* rho_basis,
                    std::complex<double>* phi,
