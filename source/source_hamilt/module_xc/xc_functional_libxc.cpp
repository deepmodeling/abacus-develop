...
int xc_func_type_classifier(const std::string& xc_func,
                            const std::map<std::string, int>& mymap = {{"LDA", 1},
                                                                       {"GGA", 2},
                                                                       {"MGGA", 3},
                                                                       {"HYB_LDA", 4},
                                                                       {"HYB_GGA", 4},
...
std::pair<int, std::vector<int>> XC_Functional_Libxc::set_xc_type_libxc(const std::string& xc_func_in) {
    ...
    std::transform(xcfunc_words_.begin(), xcfunc_words_.end(), xcfunc_type_.begin(), [](const std::string& func) {
    ...
    std::transform(xcfunc_words_.begin(), xcfunc_words_.end(), func_id.begin(), [](const std::string& func) {
...
const std::vector<double> in_built_xc_func_ext_params(const int id) {
    ...
    case XC_HYB_GGA_XC_LRC_WPBEH: // Long-range corrected short-range hybrid PBE (LRC-wPBEh) by Rohrdanz, Martins and
                                  // Herbert
    {
...
const std::vector<double> external_xc_func_ext_params(const int id) {
    const std::map<int, std::vector<double>> mymap = {
        {PARAM.inp.xc_exch_ext[0], std::vector<double>(PARAM.inp.xc_exch_ext.begin() + 1, PARAM.inp.xc_exch_ext.end())},
