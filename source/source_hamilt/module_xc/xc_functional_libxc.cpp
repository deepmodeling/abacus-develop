...
bool
    not_supported_xc_with_laplacian (const std::string& xc_func_in)
{
    ...
    const std::vector<std::string> not_supported = {"MGGA_XC_CC06", "MGGA_C_CS", "MGGA_X_BR89", "MGGA_X_MK00"};
...
bool
    not_supported_xc_with_nonlocal_vdw (const std::string& xc_func_in)
{
...
int
    xc_func_type_classifier (const std::string& xc_func,
                             const std::map<std::string, int>& mymap
                             = {{"LDA", 1}, {"GGA", 2}, {"MGGA", 3}, {"HYB_LDA", 4}, {"HYB_GGA", 4}, {"HYB_MGGA", 5}})
{
    ...
    XC_Functional_Libxc::set_xc_type_libxc (const std::string& xc_func_in)
{
    ...
    if (std::adjacent_find (xcfunc_type_.begin (), xcfunc_type_.end (), [] (int a, int b) { return a != b; })
        != xcfunc_type_.end ())
        {
            ModuleBase::WARNING_QUIT ("XC_Functional::set_xc_type_libxc",
    ...
    const bool not_recognized_xc = std::any_of (func_id.begin (), func_id.end (), [] (int id) { return id == -1; });
...
const std::vector<double>
    in_built_xc_func_ext_params (const int id)
{
    switch (id)
        {
        ...
        case XC_GGA_C_LYPR:
            return {0.04918, 0.132, 0.2533, 0.349, 0.35 / 2.29, 2.0 / 2.29, GlobalC::exx_info.info_global.hse_omega};
        ...
        case XC_HYB_GGA_XC_LRC_WPBEH: // Long-range corrected short-range hybrid PBE (LRC-wPBEh) by Rohrdanz, Martins
                                      // and Herbert
            {
...
const std::vector<double>
    external_xc_func_ext_params (const int id)
{
    const std::map<int, std::vector<double>> mymap
        = {{PARAM.inp.xc_exch_ext[0],
            std::vector<double> (PARAM.inp.xc_exch_ext.begin () + 1, PARAM.inp.xc_exch_ext.end ())},
           {PARAM.inp.xc_corr_ext[0],
            std::vector<double> (PARAM.inp.xc_corr_ext.begin () + 1, PARAM.inp.xc_corr_ext.end ())}};
    auto it = mymap.find (id);
    return (it != mymap.end ()) ? it->second : std::vector<double>{};
}

std::vector<xc_func_type>
    XC_Functional_Libxc::init_func (const std::vector<int>& func_id, const int xc_polarized)
{
    ...
    for (int id: func_id)
        {
            ...
            const double* xc_func_ext_params = (n2 > 0)   ? external_ext_params.data ()
...
void
    XC_Functional_Libxc::finish_func (std::vector<xc_func_type>& funcs)
{
