...
template <typename T>
__global__ void
    xc_functional_grad_wfc (const int ik,
                            const int pol,
                            const int npw,
                            const int npwx,
                            const T tpiba,
...
template <typename T>
__global__ void
    xc_functional_grad_wfc (const int ipol, const int nrxx, const T* porter, T* grad)
{
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx >= nrxx)
        {
...
void
    xc_functional_grad_wfc_op<T, Device>::operator() (const int& ik,
                                                      const int& pol,
                                                      const int& npw,
                                                      const int& npwx,
                                                      const Real& tpiba,
                                                      ...
                                                      T* porter)
{
    ...
    xc_functional_grad_wfc<Real>
        <<<block, THREADS_PER_BLOCK>>> (ik, pol, npw, npwx, tpiba, gcar, kvec_c, rhog_, porter_);
...
template <typename T, typename Device>
void
    xc_functional_grad_wfc_op<T, Device>::operator() (const int& ipol, const int& nrxx, const T* porter, T* grad)
{
