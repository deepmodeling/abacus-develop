...
template <typename T>
__global__ void xc_functional_grad_wfc(const int ik,
                                       const int pol,
                                       const int npw,
                                       const int npwx,
                                       const T tpiba,
    ...
    T kplusg = (gcar[(ik * npwx + idx) * 3 + pol] + kvec_c[ik * 3 + pol]) * tpiba;
    // calculate the charge density gradient in reciprocal space.
    porter[idx] = thrust::complex<T>(0.0, kplusg) * rhog[idx];
}

template <typename T>
__global__ void xc_functional_grad_wfc(const int ipol, const int nrxx, const T* porter, T* grad) {
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx >= nrxx) {
        return;
    }
...
void xc_functional_grad_wfc_op<T, Device>::operator()(const int& ik,
                                                      const int& pol,
                                                      const int& npw,
                                                      const int& npwx,
                                                      const Real& tpiba,
    ...
    xc_functional_grad_wfc<Real><<<block, THREADS_PER_BLOCK>>>(ik, pol, npw, npwx, tpiba, gcar, kvec_c, rhog_, porter_);

    CHECK_CUDA_SYNC();
}

template <typename T, typename Device>
void xc_functional_grad_wfc_op<T, Device>::operator()(const int& ipol, const int& nrxx, const T* porter, T* grad) {
    ...
    xc_functional_grad_wfc<<<block, THREADS_PER_BLOCK>>>(ipol, nrxx, porter_, grad_);
