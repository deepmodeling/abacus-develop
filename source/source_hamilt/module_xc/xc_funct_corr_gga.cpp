...
void
    XC_Functional::perdew86 (const double rho, const double grho, double& sc, double& v1c, double& v2c)
{
...
void
    XC_Functional::ggac (const double& rho, const double& grho, double& sc, double& v1c, double& v2c)
{
...
void
    XC_Functional::pbec (const double& rho, const double& grho, const int& iflag, double& sc, double& v1c, double& v2c)
{
...
void
    XC_Functional::glyp (const double& rho, const double& grho, double& sc, double& v1c, double& v2c)
{
    //-----------------------------------------------------------------------
    ...
    double x;
    x = 1.0 + d * rhom13;
    dxl = (7.0 / 3.0) * (c + d + 2.0 * c * d * rhom13 + c * d * d * rhom13 * rhom13) / (x * x);
    rhom43 = pow (rhom13, 4);
...
//-----------------------------------------------------------------------
void
    XC_Functional::perdew86_spin (double rho,
                                  double zeta,
                                  double grho,
                                  double& sc,
                                  double& v1cup,
                                  ...
                                  double& v2c)
{
    ...
    // SdG: in the original paper 1.745*0.11=0.19195 is used
    dd = pow ((2.0), third)
         * sqrt (pow (((1.0 + zeta) * 0.50), (5.0 / 3.0)) + pow (((1.0 - zeta) * 0.50), (5.0 / 3.0)));
    ddd = pow ((2.0), (-4.0 / 3.0)) * 5.0
          * (pow (((1.0 + zeta) * 0.50), (2.0 / 3.0)) - pow (((1.0 - zeta) * 0.50), (2.0 / 3.0))) / (3.0 * dd);
    ephi = exp (-phi);
    sc = grho / rho43 * cn * ephi / dd;
    v1cup
        = sc * ((1.0 + phi) * dcn / cn - ((4.0 / 3.0) - (7.0 / 6.0) * phi) / rho) - sc * ddd / dd * (1.0 - zeta) / rho;
    v1cdw
        = sc * ((1.0 + phi) * dcn / cn - ((4.0 / 3.0) - (7.0 / 6.0) * phi) / rho) + sc * ddd / dd * (1.0 + zeta) / rho;
...
//---------------------------------------------------------------
void
    XC_Functional::pbec_spin (double rho,
                              double zeta,
                              double grho,
                              const int& iflag,
                              double& sc,
                              ...
                              double& v2c)
{
    ...
    kf = xkf / rs;
    ks = xks * sqrt (kf);
    fz = 0.50 * (pow ((1.0 + zeta), (2.0 / 3.0)) + pow ((1.0 - zeta), (2.0 / 3.0)));
    fz2 = fz * fz;
    fz3 = fz2 * fz;
    // fz4 = fz3 * fz;
    dfz = (pow ((1.0 + zeta), (-1.0 / 3.0)) - pow ((1.0 - zeta), (-1.0 / 3.0))) / 3.0;
    t = sqrt (grho) / (2.0 * fz * ks * rho);
    expe = exp (-ec / (fz3 * ga));
    ...
    dh0zup = (3.0 * h0 / fz
              - be[iflag] * t * t * fz2 / s1 * (2.0 * xy - qy * (3.0 * af * expe * ec / fz3 / be[iflag] + 2.0)))
             * dfz * (1.0 - zeta);
    dh0zdw = -(3.0 * h0 / fz
               - be[iflag] * t * t * fz2 / s1 * (2.0 * xy - qy * (3.0 * af * expe * ec / fz3 / be[iflag] + 2.0)))
             * dfz * (1.0 + zeta);
