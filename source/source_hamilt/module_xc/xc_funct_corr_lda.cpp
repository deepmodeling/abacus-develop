...
void
    XC_Functional::pw (const double& rs, const int& iflag, double& ec, double& vc)
{
...
void
    XC_Functional::pz (const double& rs, const int& iflag, double& ec, double& vc)
{
    ...
    if (rs < 1.0)
    ...
    else
        {
            ...
            const double dox = 1.0 + 7.0 / 6.0 * b1[iflag] * rs12 + 4.0 / 3.0 * b2[iflag] * rs;
            ec = gc[iflag] / ox;
            vc = ec * dox / ox;
        }
    return;
...
// LDA part only
void
    XC_Functional::lyp (const double& rs, double& ec, double& vc)
{
...
void
    XC_Functional::vwn (const double& rs, double& ec, double& vc)
{
    ...
    double x = 0.0;
    x = (rs12 - x0);
    ec = a * (log (rs / fx) + f1 * qx - f2 * (log ((x * x) / fx) + f3 * qx));
    tx = 2.0 * rs12 + b;
    tt = tx * tx + q * q;
...
void
    XC_Functional::wigner (const double& rs, double& ec, double& vc)
{
...
void
    XC_Functional::hl (const double& rs, double& ec, double& vc)
{
    double a, x;
    a = log (1.00 + 21.0 / rs);
    x = rs / 21.00;
...
void
    XC_Functional::gl (const double& rs, double& ec, double& vc)
{
    ...
    double x = rs / r;
    vc = -c * log (1.0 + 1.0 / x);
    ec = -c * ((1.0 + pow (x, 3)) * log (1.0 + 1.0 / x) - 1.00 / 3.00 + x * (0.50 - x));
    return;
}

// J.P. Perdew and Y. Wang, PRB 45, 13244 (1992)
void
    XC_Functional::pw_spin (const double& rs, const double& zeta, double& ec, double& vcup, double& vcdw)
{
    const double a = 0.0310910;
    ...
    dfz = (pow ((1.0 + zeta), (1.0 / 3.0)) - pow ((1.0 - zeta), (1.0 / 3.0))) * 4.0
          / (3.0 * (pow (2.0, (4.0 / 3.0)) - 2.0));

    ec = epwc + alpha * fz * (1.0 - zeta4) / fz0 + (epwcp - epwc) * fz * zeta4;

    vcup
        = vpwc + vpwca * fz * (1.0 - zeta4) / fz0 + (vpwcp - vpwc) * fz * zeta4
          + (alpha / fz0 * (dfz * (1.0 - zeta4) - 4.0 * fz * zeta3) + (epwcp - epwc) * (dfz * zeta4 + 4.0 * fz * zeta3))
                * (1.0 - zeta);
...
// J.P. Perdew and Y. Wang, PRB 45, 13244 (1992)
void
    XC_Functional::pz_spin (const double& rs, const double& zeta, double& ec, double& vcup, double& vcdw)
{
...
void
    XC_Functional::pz_polarized (const double& rs, double& ec, double& vc)
{
