...
void
    XC_Functional::hcth (const double rho, const double grho, double& sx, double& v1x, double& v2x)
{
    //     ===============================================================
    ...
    double r3q2, r3pi, gr, rho_o3, rho_o34, xa, xa2, ra, rab, dra_drho, drab_drho, g, dg, era1, dera1_dra, erab0,
        derab0_drab, ex, dex_drho, uaa, uab, ux, ffaa, ffab, dffaa_drho, dffab_drho, denaa, denab, denx, f83rho, bygr,
        gaa, gab, gx, taa, tab, txx, dgaa_drho, dgab_drho, dgx_drho, dgaa_dgr, dgab_dgr, dgx_dgr;
    ...
    taa = denaa * uaa * (caa[2] + uaa * (2.0 * caa[3] + uaa * (3.0 * caa[4] + uaa * 4.0 * caa[5])));
    tab = denab * uab * (cab[2] + uab * (2.0 * cab[3] + uab * (3.0 * cab[4] + uab * 4.0 * cab[5])));
    txx = denx * ux * (cx[2] + ux * (2.0 * cx[3] + ux * (3.0 * cx[4] + ux * 4.0 * cx[5])));
    ...
    v1x = dex_drho * gx + ex * dgx_drho + dffaa_drho * gaa + ffaa * dgaa_drho + dffab_drho * gab + ffab * dgab_drho;
    v2x = (ex * dgx_dgr + ffaa * dgaa_dgr + ffab * dgab_dgr) / gr;
    return;
} // end subroutine hcth

void
    XC_Functional::pwcorr (const double r, const double c[], double& g, double& dg)
{
    // USE kinds
    // implicit none
