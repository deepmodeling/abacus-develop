...
std::vector<double>
    XC_Functional_Libxc::convert_rho (const int nspin, const std::size_t nrxx, const Charge* const chr)
{
    ...
    for (int ir = 0; ir < nrxx; ++ir)
        {
            ...
            amag[ir] = std::sqrt (std::pow (chr->rho[1][ir], 2) + std::pow (chr->rho[2][ir], 2)
...
std::vector<std::vector<ModuleBase::Vector3<double>>>
    XC_Functional_Libxc::cal_gdr (const int nspin,
...
std::vector<double>
    XC_Functional_Libxc::cal_sgn (const double rho_threshold,
...
double
    XC_Functional_Libxc::convert_etxc (const int nspin,
                                       const std::size_t nrxx,
...
#pragma omp parallel for schedule(static, 1024)
            ...
            for (std::size_t ir = 0; ir < nrxx; ++ir)
                {
                    h[0][ir] = 2.0
                               * (gdr[0][ir] * vsigma[ir * 3] * sgn[ir * 2] * 2.0
                                  + gdr[1][ir] * vsigma[ir * 3 + 1] * sgn[ir * 2] * sgn[ir * 2 + 1]);
                    h[1][ir] = 2.0
                               * (gdr[1][ir] * vsigma[ir * 3 + 2] * sgn[ir * 2 + 1] * 2.0
...
ModuleBase::matrix
    XC_Functional_Libxc::convert_v_nspin4 (const std::size_t nrxx,
                                           const Charge* const chr,
