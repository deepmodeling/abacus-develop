...
{
void
    WARNING_QUIT (const std::string& file, const std::string& description)
{
    exit (1);
}
} // namespace ModuleBase
...
TEST_F (XCTest_BP_SPN, set_xc_type)
{
    ...
    std::vector<double> e_gga_ref
        = {-2.527553387e-14, -0.005791315733, -0.09840317072, -0.004480017364, -0.00633879446};
...
TEST_F (XCTest_revPBE_SPN, set_xc_type)
{
    ...
    std::vector<double> v2_lda_ref = {-1.259358955, -1.186042827, -1.011084744, -0.3739376001, -0.6598499399};
    std::vector<double> e_gga_ref
        = {-5.063224319e-14, -0.01154733753, -0.2341920935, -0.006370370784, -0.0001006666667};
...
TEST_F (XCTest_PZ_SPN, set_xc_type)
{
    ...
    std::vector<double> e_lda_ref
        = {-0.95651735756525513, -0.96317186117476516, -0.95998479865326902, -0.41690422190075471, -11.392818565518368};
    std::vector<double> v1_lda_ref
        = {-1.2588131365004438, -1.3213867444561467, -1.3486549996482768, -0.56794400635336095, -15.170993784117668};
    std::vector<double> v2_lda_ref
        = {-1.2588131365004438, -1.1877924103859072, -1.0155330910438201, -0.37092937962238148, -0.51758342826640091};
...
{
    std::vector<double> e_lda_ref
        = {0.0, -0.880392474033, -0.888247554199, -0.892602327244, -0.378797005763, -9.99791043021};
    std::vector<double> v1_lda_ref
        = {0.0, -1.17442450837, -1.24798175209, -1.28947632654, -0.532758325459, -14.0193907595};
    std::vector<double> v2_lda_ref
        = {0.0, -1.17442450837, -1.09028491017, -0.894235359765, -0.2561411064, -0.688843519257};
...
{
    std::vector<double> e_gga_ref
        = {1.69759930415e-14, 0.00308117598269, 0.0408000476851, 0.00290513793266, 8.49954060062e-08};
    std::vector<double> v1_gga_ref
        = {-1.32642497169e-14, -0.0022804667616, -0.0166964664575, -0.00685850787795, -6.30224371228e-11};
    std::vector<double> v2_gga_ref
        = {-1.32642497169e-14, -0.00188528133486, -0.00317100883673, 0.0160558001336, -2.36727121199e-11};
    std::vector<double> v3_gga_ref
        = {0.00419160260597, 0.00338159500585, 0.00145591122701, 0.0336591369313, 3.09070721877e-07};
...
TEST_F (XCTest_PBE0_SPN, set_xc_type)
{
    ...
    std::vector<double> e_gga_ref
        = {-3.387102535e-14, -0.007743108067, -0.1276936518, -0.003065679956, -4.873559704e-05};
        ...
        for (int i = 0; i < 5; i++)
            {
                ...
                double r1 = rho[i] * (1 + zeta[i]) / 2.0;
                double r2 = rho[i] * (1 - zeta[i]) / 2.0;
                XC_Functional_Libxc::gcxc_spin_libxc (XC_Functional::get_func_id (),
                                                      r1,
                                                      r2,
                                                      gdr[i],
                                                      gdr[i],
