...
template <typename T, typename Device>
typename Operator<T, Device>::hpsi_info
    Operator<T, Device>::hPsi (hpsi_info& input) const
{
...
template <typename T, typename Device>
void
    Operator<T, Device>::init (const int ik_in)
{
    this->ik = ik_in;
...
template <typename T, typename Device>
void
    Operator<T, Device>::add (Operator* next)
{
    if (next == nullptr)
        {
            return;
...
template <typename T, typename Device>
T*
    Operator<T, Device>::get_hpsi (const hpsi_info& info) const
{
