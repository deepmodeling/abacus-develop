...
int
    H_Ewald_pw::estimate_mxr (const double& rmax, const ModuleBase::Matrix3& bg)
{
    double bg1[3];
    bg1[0] = bg.e11;
    bg1[1] = bg.e12;
...
double
    H_Ewald_pw::compute_ewald (const UnitCell& cell,
    ...
    for (int ig = 0; ig < rho_basis->npw; ig++)
        {
            ...
            ewaldg += fact * std::abs (rhon) * std::abs (rhon) * exp (-rho_basis->gg[ig] * cell.tpiba2 / alpha / 4.0)
                      / rho_basis->gg[ig] / cell.tpiba2;
        }
            ...
            for (int it = 0; it < cell.ntype; it++)
                {
                    {
                        ewaldg = ewaldg
                                 - cell.atoms[it].na * cell.atoms[it].ncpp.zv * cell.atoms[it].ncpp.zv
                    ...
                    for (nr = 0; nr < nrm; nr++)
                        {
                            ...
                            {
                                ewaldr = ewaldr
                                         + cell.atoms[it1].ncpp.zv * cell.atoms[it2].ncpp.zv * erfc (sqrt (alpha) * rr)
                                               / rr;
                            }
                        }
                    if (PARAM.inp.test_energy > 1)
                                            ...
                                            for (nr = 0; nr < nrm; nr++)
                                                {
                                                    ...
                                                    {
                                                        ewaldr = ewaldr
                                                                 + cell.atoms[nt1].ncpp.zv * cell.atoms[nt2].ncpp.zv
...
void
    H_Ewald_pw::rgen (const ModuleBase::Vector3<double>& dtau,
                      const double& rmax,
                      int* irr,
                            ...
                            if (tt <= rmax * rmax && std::abs (tt) > 1.e-10)
                                {
                                    if (nrm >= mxr)
                                        {
                                            ModuleBase::WARNING_QUIT ("rgen",
                                                                      "too many r-vectors (nrm=" + std::to_string (nrm)
                                                                          + ", mxr=" + std::to_string (mxr)
