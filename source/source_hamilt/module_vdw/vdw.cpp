...
std::string
    parse_xcname (const std::string& xc_input, const std::vector<std::string>& xc_psp)
{
    if (xc_input != "default")
        {
            return xc_input;
    ...
    std::cout << " ***WARNING*** ModuleHamiltGeneral::ModuleVDW::parse_xcname: "
              << "XC name is automatically inferred from pseudopotential as `" << xc << "`" << std::endl;
    return xc;
}
...
std::unique_ptr<Vdw>
    make_vdw (const UnitCell& ucell, const Input_para& input, std::ofstream* plog)
{
