        ...
        for (int it1 = 0; it1 < ucell_.ntype; ++it1)
            {
                for (int it2 = 0; it2 < ucell_.ntype; ++it2)
                    {
                        const double C6_product = sqrt (para_.C6 ().at (ucell_.atoms[it1].ncpp.psd)
                                                        * para_.C6 ().at (ucell_.atoms[it2].ncpp.psd))
                                                  / pow (ucell_.lat0, 6);
                        const double R0_sum = (para_.R0 ().at (ucell_.atoms[it1].ncpp.psd)
                                               + para_.R0 ().at (ucell_.atoms[it2].ncpp.psd))
                                              / ucell_.lat0;
                        if (!R0_sum)
                            {
                                                ...
                                                for (ilat_loop.y = -yidx; ilat_loop.y <= yidx; ++ilat_loop.y)
                                                    {
                                                        for (ilat_loop.z = -zidx; ilat_loop.z <= zidx; ++ilat_loop.z)
                                                            {
                                                                if ((!(ilat_loop.x || ilat_loop.y || ilat_loop.z))
                                                                    && (it1 == it2) && (ia1 == ia2))
                                                                    {
                                                                        continue;
                                                                    }
                                                                const ModuleBase::Vector3<double> tau2
                                                                    = ucell_.atoms[it2].tau[ia2]
                                                                      + ilat_loop * ucell_.latvec;
