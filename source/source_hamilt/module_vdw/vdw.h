...
class Vdw
{
  public:
    ...
    inline const std::vector<ModuleBase::Vector3<double>>&
        get_force (bool cal = true)
    {
        if (cal)
            {
                cal_force ();
    ...
    inline const ModuleBase::Matrix3&
        get_stress (bool cal = true)
    {
        if (cal)
            {
                cal_stress ();
 ...
 */
std::unique_ptr<Vdw> make_vdw (const UnitCell& ucell, const Input_para& input, std::ofstream* plog = nullptr);
