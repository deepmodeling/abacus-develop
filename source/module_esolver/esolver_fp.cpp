...
ESolver_FP::ESolver_FP() {
    ...
    pw_rho = new ModulePW::PW_Basis_Big(GlobalV::device_flag,
                                        GlobalV::precision_flag);

    if (GlobalV::double_grid) {
        pw_rhod = new ModulePW::PW_Basis_Big(GlobalV::device_flag,
                                             GlobalV::precision_flag);
    } else {
        pw_rhod = pw_rho;
    }
    ...
    GlobalC::ucell.symm.epsilon = GlobalC::ucell.symm.epsilon_input
        = PARAM.get().symmetry_prec;
}
...
void ESolver_FP::before_all_runners(Input& inp, UnitCell& cell) {
...
#ifdef __MPI
    this->pw_rho->initmpi(GlobalV::NPROC_IN_POOL,
    ...
    if (inp.nx * inp.ny * inp.nz == 0) {
        this->pw_rho->initgrids(inp.ref_cell_factor * cell.lat0,
                                cell.latvec,
                                4.0 * inp.ecutwfc);
    } else {
        this->pw_rho->initgrids(inp.ref_cell_factor * cell.lat0,
    ...
    if (GlobalV::double_grid) {
        ModulePW::PW_Basis_Sup* pw_rhod_sup
            = static_cast<ModulePW::PW_Basis_Sup*>(pw_rhod);
#ifdef __MPI
        this->pw_rhod->initmpi(GlobalV::NPROC_IN_POOL,
                               GlobalV::RANK_IN_POOL,
                               POOL_WORLD);
#endif
        if (this->classname == "ESolver_OF") {
        ...
        if (inp.ndx * inp.ndy * inp.ndz == 0) {
            this->pw_rhod->initgrids(inp.ref_cell_factor * cell.lat0,
                                     cell.latvec,
                                     inp.ecutrho);
        } else {
            this->pw_rhod->initgrids(inp.ref_cell_factor * cell.lat0,
...
void ESolver_FP::init_after_vc(Input& inp, UnitCell& cell) {
    ...
    if (GlobalV::md_prec_level == 2) {
        if (inp.nx * inp.ny * inp.nz == 0) {
            ...
            this->pw_rho->initgrids(cell.lat0,
                                    cell.latvec,
                                    inp.nx,
                                    inp.ny,
                                    inp.nz);
        ...
        if (GlobalV::double_grid) {
            ...
            if (inp.ndx * inp.ndy * inp.ndz == 0) {
                ...
                this->pw_rhod->initgrids(cell.lat0,
                                         cell.latvec,
                                         inp.ndx,
                                         inp.ndy,
                                         inp.ndz);
...
void ESolver_FP::print_rhofft(Input& inp, std::ofstream& ofs) {
    std::cout << " UNIFORM GRID DIM        : " << pw_rho->nx << " * "
              << pw_rho->ny << " * " << pw_rho->nz << std::endl;
    std::cout << " UNIFORM GRID DIM(BIG)   : " << pw_big->nbx << " * "
              << pw_big->nby << " * " << pw_big->nbz << std::endl;
    if (GlobalV::double_grid) {
        std::cout << " UNIFORM GRID DIM(DENSE) : " << pw_rhod->nx << " * "
                  << pw_rhod->ny << " * " << pw_rhod->nz << std::endl;
    }
    ...
    ModuleBase::GlobalFunc::OUT(ofs,
                                "energy cutoff for charge/potential (unit:Ry)",
                                ecut);
    ...
    ModuleBase::GlobalFunc::OUT(ofs,
                                ...
                                pw_big->bz);
    ModuleBase::GlobalFunc::OUT(ofs,
                                "big fft grid for charge/potential",
                                pw_big->nbx,
                                pw_big->nby,
                                pw_big->nbz);
    ...
    if (GlobalV::double_grid) {
        ...
        for (int i = 0; i < GlobalV::NPROC_IN_POOL; ++i) {
            ofs << " " << std::setw(8) << i + 1 << std::setw(15)
                << this->pw_rhod->nst_per[i] << std::setw(15)
                << this->pw_rhod->npw_per[i] << std::endl;
        }
        ofs << " --------------- sum -------------------" << std::endl;
        ofs << " " << std::setw(8) << GlobalV::NPROC_IN_POOL << std::setw(15)
            << this->pw_rhod->nstot << std::setw(15) << this->pw_rhod->npwtot
            << std::endl;
        ...
        ModuleBase::GlobalFunc::OUT(
            ofs,
            "max |g|",
            this->pw_rhod->gg_uniq[this->pw_rhod->ngg - 1]);
