 ...
 */
void ESolver_OF::kinetic_potential(double** prho,
                                   double** pphi,
...
void ESolver_OF::kinetic_stress(ModuleBase::matrix& kinetic_stress_) {
    ...
    if (this->of_kinetic_ == "wt") {
        this->wt_->get_stress(pelec->charge->rho,
                              this->pw_rho,
                              INPUT.of_vw_weight);
 ...
 * @param ucell
 */
void ESolver_OF::get_step_length(double* dEdtheta,
                                 double** ptemp_phi,
                                 UnitCell& ucell) {
    ...
    if (GlobalV::NSPIN == 1) {
        ...
        while (true) {
            ...
            this->opt_dcsrch_->dcSrch(temp_energy,
                                      dEdtheta[0],
                                      this->theta_[0],
                                      this->task_);
            numDC++;
            ...
            if (strncmp(this->task_, "FG", 2) == 0) // continue line search
            {
                ...
                for (int i = 0; i < this->pw_rho->nrxx; ++i) {
                    ptemp_phi[0][i]
                        = this->pphi_[0][i] * cos(this->theta_[0])
                          + this->pdirect_[0][i] * sin(this->theta_[0]);
                    this->ptemp_rho_->rho[0][i]
                        = ptemp_phi[0][i] * ptemp_phi[0][i];
                }
            ...
            } else if (strncmp(this->task_, "CO", 2)
                       == 0) // convergence achieved
            {
                break;
            } else if (strncmp(this->task_, "WA", 2)
                       == 0) // warning of line search
            {
                GlobalV::ofs_warning << "ESolver_OF linesearch: WARNING "
                                     << this->task_ << std::endl;
                std::cout << this->task_ << std::endl;
                break;
            } else if (strncmp(this->task_, "ER", 2)
                       == 0) // ERROR in line search
            {
