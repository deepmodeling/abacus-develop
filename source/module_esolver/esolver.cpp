...
void ESolver::printname() { std::cout << classname << std::endl; }
...
std::string determine_type() {
    ...
    if (GlobalV::BASIS_TYPE == "pw") {
        ...
        ModuleBase::WARNING_QUIT(
            "ESolver",
            "LCAO basis type must be compiled with __LCAO");
#endif
    ...
    if (GlobalV::ESOLVER_TYPE == "lj") {
        ...
        ModuleBase::WARNING_QUIT(
            "ESolver",
            "No such esolver_type combined with basis_type");
    }

    GlobalV::ofs_running << " The esolver type has been set to : "
                         << esolver_type << std::endl;
    ...
    if (esolver_type == "ksdft_pw") {
        ...
        if (GlobalV::device_flag == "gpu") {
            if (GlobalV::precision_flag == "single") {
                p_esolver = new ESolver_KS_PW<std::complex<float>,
        ...
        if (GlobalV::GAMMA_ONLY_LOCAL) {
            ...
            p_esolver = new ESolver_KS_LCAO<std::complex<double>,
                                            std::complex<double>>();
        }
...
void clean_esolver(ESolver*& pesolver) { delete pesolver; }
