...
void Exx_Lip<T, Device>::cal_exx()
{
    ...
    for (int ik = 0; ik < this->k_pack->kv_ptr->get_nks(); ++ik)
    {
        ...
        for (int iq_tmp = this->iq_vecik; iq_tmp < this->iq_vecik + this->q_pack->kv_ptr->get_nks() / PARAM.inp.nspin;
             ++iq_tmp) // !!! k_point
                    ...
                    for (int iy = 0; iy < this->rho_basis->ny; ++iy)
                    {
                             ...
                             ++iz)
                        {
                            const Treal phase_xyz
                                = phase_xy + this->q_pack->kv_ptr->kvec_d[iq].z * iz / this->rho_basis->nz;
                            const T exp_tmp = std::exp(phase_xyz * this->two_pi_i);
                    ...
                    this->exx_matrix[ik][iw_l][iw_r]
                        += spin_fac
                           * (-1 / (Treal)sqrt(info.lambda * ModuleBase::PI)
                              * (Treal)(this->q_pack->kv_ptr->get_nks() / PARAM.inp.nspin) * this->sum3[iw_l][iw_r]);
                }
            }
        }
    }
    ...
    for (int ik = 0; ik < this->k_pack->kv_ptr->get_nks(); ++ik)
    {
        for (int iw_l = 0; iw_l < PARAM.globalv.nlocal; ++iw_l)
        {
            for (int iw_r = 0; iw_r < PARAM.globalv.nlocal; ++iw_r)
            {
                ...
                {
                    exx_energy_tmp
                        += (this->exx_matrix[ik][iw_l][iw_r] * conj((*this->k_pack->hvec_array)(ik, ib, iw_l))
...
void Exx_Lip<T, Device>::write_q_pack() const
{
    ...
    if (!GlobalV::RANK_IN_POOL)
    {
        ...
        const std::string command_mkdir = "test -d " + PARAM.globalv.global_out_dir + exx_q_pack + " || mkdir "
                                          + PARAM.globalv.global_out_dir + exx_q_pack;
        assert(system(command_mkdir.c_str()) == 0);

        const std::string command_kpoint = "test -f " + PARAM.globalv.global_out_dir + exx_q_pack
                                           + PARAM.inp.kpoint_file + " || cp " + PARAM.inp.kpoint_file + " "
                                           + PARAM.globalv.global_out_dir + exx_q_pack + PARAM.inp.kpoint_file;
