...
void Exx_Abfs::Jle::init_jle(const double kmesh_times, const UnitCell& ucell, const LCAO_Orbitals& orb)
{
    ...
    for (int T = 0; T < ucell.ntype; T++)
    {
        ...
        for (int L = 0; L <= Lmax; ++L)
        {
            ...
            ModuleBase::Sphbes::Spherical_Bessel_Roots(ecut_number,
                                                       L,
                                                       tolerence,
                                                       ModuleBase::GlobalFunc::VECTOR_TO_PTR(en),
                                                       orb.Phi[T].getRcut());

            for (size_t E = 0; E != ecut_number; ++E)
            {
                std::vector<double> jle_r(orb.Phi[T].PhiLN(0, 0).getNr());
                ModuleBase::Sphbes::Spherical_Bessel(orb.Phi[T].PhiLN(0, 0).getNr(),
                                                     orb.Phi[T].PhiLN(0, 0).getRadial(),
                                                     en[E],
                                                     L,
                                                     ModuleBase::GlobalFunc::VECTOR_TO_PTR(jle_r));
                jle[T][L][E].set_orbital_info(orb.Phi[T].PhiLN(0, 0).getLabel(),
