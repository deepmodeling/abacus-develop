...
std::vector<double> cal_psi_fock_spencer(const std::vector<double>& psif,
...
std::vector<double> cal_psi_erfc_limits(const std::vector<double>& psif,
                                        const std::vector<double>& k_radial,
    ...
    for (const auto& param_list: coulomb_param)
    {
        ...
        case Conv_Coulomb_Pot_K::Coulomb_Type::Fock: {
            for (const auto& param: param_list.second)
            {
                ...
                {
                    throw std::invalid_argument("Rcut_type = " + param.at("Rcut_type") + " in " + std::string(__FILE__)
