...
auto RI_2D_Comm::split_m2D_ktoR(const K_Vectors& kv,
    ...
    const double SPIN_multiple
        = std::map<int, double>{{1, 0.5}, {2, 1}, {4, 1}}.at(
            GlobalV::NSPIN); // why?
        ...
        for (const TC& cell: RI_Util::get_Born_von_Karmen_cells(period)) {
            ...
            for (const int ik: ik_list) {
                ...
                RI::Tensor<Tdata_m> mk_2D
                    = RI_Util::Vector_to_Tensor<Tdata_m>(*mks_2D[ik],
                                                         pv.get_col_size(),
                                                         pv.get_row_size());
                const Tdata_m frac
                    = SPIN_multiple
                      * RI::Global_Func::convert<Tdata_m>(
                          std::exp(-ModuleBase::TWO_PI * ModuleBase::IMAG_UNIT
                                   * (kv.kvec_c[ik]
                                      * (RI_Util::array3_to_Vector3(cell)
            ...
            for (int iwt0_2D = 0; iwt0_2D != mR_2D.shape[0]; ++iwt0_2D) {
                ...
                std::tie(iat0, iw0_b, is0_b)
                    = RI_2D_Comm::get_iat_iw_is_block(iwt0);
                ...
                for (int iwt1_2D = 0; iwt1_2D != mR_2D.shape[1]; ++iwt1_2D) {
                    ...
                    std::tie(iat1, iw1_b, is1_b)
                        = RI_2D_Comm::get_iat_iw_is_block(iwt1);
                    const int it1 = GlobalC::ucell.iat2it[iat1];

                    const int is_b
                        = RI_2D_Comm::get_is_block(is_k, is0_b, is1_b);
                    RI::Tensor<Tdata>& mR_a2D
                        = mRs_a2D[is_b][iat0][{iat1, cell}];
                    if (mR_a2D.empty())
            ...
            for (const auto& Hs_tmpB: Hs_tmpA.second) {
                ...
                for (size_t iw0_b = 0; iw0_b < H.shape[0]; ++iw0_b) {
                    ...
                    for (size_t iw1_b = 0; iw1_b < H.shape[1]; ++iw1_b) {
                        const int iwt1
                            = RI_2D_Comm::get_iwt(iat1, iw1_b, is1_b);
                        if (pv.global2local_col(iwt1) < 0)
                            continue;
                        LCAO_Matrix::set_mat2d(
                            iwt0,
                            iwt1,
