...
#pragma omp for schedule(dynamic) nowait
        for (size_t i1 = 0; i1 < list_A1.size(); ++i1)
        {
            ...
            const double Rcut = std::min(orb_cutoff_[it0] * rmesh_times + orb_cutoff_[it1],
                 ...
                 std::array<std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>, 3>>
{
    ...
    std::map<TA, std::map<TAC, std::pair<RI::Tensor<Tdata>, std::array<RI::Tensor<Tdata>, 3>>>> Cs_dCs_tmp
        = this->cal_datas(ucell, list_A0, list_A1, flags, std::min(1.0, this->ccp_rmesh_times), func_DPcal_C_dC);
