                          ...
                          const LCAO_Orbitals& orb)
{
    ...
    const std::unordered_map<Conv_Coulomb_Pot_K::Coulomb_Type, std::vector<std::map<std::string, std::string>>>
        coulomb_param_updated
        = RI_Util::update_coulomb_param(this->info.coulomb_param, ucell.omega, this->p_kv->get_nkstot_full());
    ...
    if (PARAM.inp.cal_force || PARAM.inp.cal_stress)
    {
        ...
        if (PARAM.inp.cal_stress)
        {
            std::array<std::array<std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>, 3>, 3> dVRs
                = LRI_CV_Tools::cal_dMRs(ucell, dVs);
    ...
    if (PARAM.inp.cal_force || PARAM.inp.cal_stress)
    {
        ...
        if (PARAM.inp.cal_stress)
        {
            std::array<std::array<std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>, 3>, 3> dCRs
                = LRI_CV_Tools::cal_dMRs(ucell, dCs);
