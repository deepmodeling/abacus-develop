						  ...
						  const LCAO_Orbitals& orb)
{
	...
	const std::map<std::string,double> ccp_parameter = RI_Util::get_ccp_parameter(this->info, ucell, this->p_kv, nspin0);
...
void Exx_LRI<Tdata>::cal_exx_ions(const UnitCell& ucell, const bool write_cv)
{
    ...
    const std::array<Tcell, Ndim> period_Cs
        = LRI_CV_Tools::cal_latvec_range<Tcell>(2, ucell,orb_cutoff_);
    const std::pair<
        std::vector<TA>,
	...
	const ModuleSymmetry::Symmetry_rotation* p_symrot)
{
    ...
    const std::vector<std::tuple<std::set<TA>, std::set<TA>>> judge
        = RI_2D_Comm::get_2D_judge(ucell,pv);
