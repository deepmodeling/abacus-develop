...
void
    Charge_Extra::Init_CE (const int& nspin, const int& natom, const int& nrxx, const std::string chg_extrap)
{
    if (chg_extrap == "none")
        {
            pot_order = 0;
...
void
    Charge_Extra::extrapolate_charge (Parallel_Grid* Pgrid,
                                      UnitCell& ucell,
                                      Charge* chr,
                                      Structure_Factor* sf,
            ...
            for (int is = 0; is < this->nspin; is++)
                {
                    for (int ir = 0; ir < chr->rhopw->nrxx; ir++)
                        {
                            chr->rho[is][ir] = one_add_alpha * delta_rho1[is][ir] + beta_alpha * delta_rho2[is][ir]
                                               - beta * delta_rho3[is][ir];
                        }
                }
        }
...
void
    Charge_Extra::find_alpha_and_beta (const int& natom, std::ofstream& ofs_running, std::ofstream& ofs_warning)
{
    if (istep < 3)
        {
            return;
    ...
    double det = 0.0;
#ifdef _OPENMP
#pragma omp parallel for schedule(static, 16) reduction(+ : a11) reduction(+ : a12) reduction(+ : a22)                 \
...
void
    Charge_Extra::update_all_dis (const UnitCell& ucell)
{
    istep++;
    if (pot_order == 3)
        {
...
void
    Charge_Extra::update_delta_rho (const UnitCell& ucell, const Charge* chr, const Structure_Factor* sf)
{
    if (pot_order == 0)
        {
            return;
