...
void
    Symmetry_rho::psymmg (std::complex<double>* rhog_part,
...
void
    Symmetry_rho::reduce_to_fullrhog (const ModulePW::PW_Basis* rho_basis,
                                      std::complex<double>* rhogtot,
    ...
    for (int proc = 0; proc < rho_basis->poolnproc; ++proc)
        {
            ...
            // from other processors
                ...
                {
                    MPI_Recv (rhog_piece,
                              rho_basis->npw_per[proc],
                              MPI_DOUBLE_COMPLEX,
                              proc,
                              proc,
                              POOL_WORLD,
                              &ierror);
                    MPI_Recv (ig2isz_piece,
                              rho_basis->npw_per[proc],
                              MPI_INT,
                              proc,
                              proc + rho_basis->poolnproc,
                              POOL_WORLD,
                              &ierror);
                }
...
void
    Symmetry_rho::rhog_piece_to_all (const ModulePW::PW_Basis* rho_basis,
                                     std::complex<double>* rhogtot,
                                     std::complex<double>* rhog_part) const
{
    ...
    if (GlobalV::RANK_IN_POOL == 0)
        ...
        {
            MPI_Recv (rhog_part,
                      rho_basis->npw_per[GlobalV::RANK_IN_POOL],
                      MPI_DOUBLE_COMPLEX,
                      0,
...
// only for MYRANK==0
void
    Symmetry_rho::get_ixyz2ipw (const ModulePW::PW_Basis* rho_basis,
                                const int* ig2isztot,
                                const int* fftixy2is,
                                int* ixyz2ipw) const
{
                    ...
                    for (int iz = 0; iz < rho_basis->fftnz; ++iz)
                        {
                            ...
                            if (ip == -1)
                                {
                                    continue; // not in any core
                                }
                            is = fftixy2is[ixy]; // stick-index on ip=proc core
                            if (is == -1)
                                {
                                    continue; // not on any stick
                                }
