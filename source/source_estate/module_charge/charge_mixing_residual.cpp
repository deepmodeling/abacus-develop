...
double
    Charge_Mixing::get_drho (Charge* chr, const double nelec)
{
...
double
    Charge_Mixing::get_dkin (Charge* chr, const double nelec)
{
...
double
    Charge_Mixing::inner_product_recip_rho (std::complex<double>* rho1, std::complex<double>* rho2)
{
                ...
                for (int ig = 0; ig < this->rhopw->npw; ++ig)
                    {
                        ...
                        sum += (conj (rhog1[0][ig] + rhog1[1][ig]) * (rhog2[0][ig] + rhog2[1][ig])).real ()
                               / this->rhopw->gg[ig];
                    }
                sum *= fac;
...
#pragma omp parallel for reduction(+ : sum)
                    ...
                    if (ig0 > 0)
                        {
                            sum += fac2
                                   * ((conj (rhog1[1][ig0]) * rhog2[1][ig0]).real ()
                    ...
                    for (int ig = 0; ig < this->rhopw->npw; ig++)
                        {
                            ...
                            sum += fac3
                                   * ((conj (rhog1[1][ig]) * rhog2[1][ig]).real ()
...
double
    Charge_Mixing::inner_product_recip_simple (std::complex<double>* rho1, std::complex<double>* rho2)
{
...
double
    Charge_Mixing::inner_product_recip_hartree (std::complex<double>* rhog1, std::complex<double>* rhog2)
{
...
double
    Charge_Mixing::inner_product_real (double* rho1, double* rho2)
{
    double rnorm = 0.0;
    // consider a resize for mixing_angle
    int resize_tmp = 1;
