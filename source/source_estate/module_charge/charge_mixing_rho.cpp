...
void
    Charge_Mixing::mix_rho_recip (Charge* chr)
{
    ...
    if (nspin == 1)
    ...
    else if (nspin == 2)
        {
            ...
            auto twobeta_mix
                = [this,
                   npw] (std::complex<double>* out, const std::complex<double>* in, const std::complex<double>* sres)
                {
#ifdef _OPENMP
...
#pragma omp parallel for schedule(static, 256)
    ...
    else if (nspin == 4 && PARAM.inp.mixing_angle > 0)
        {
            ...
            if (PARAM.globalv.double_grid)
                {
                    ModuleBase::WARNING_QUIT ("Charge_Mixing",
                                              "double_grid is not supported for new mixing method yet.");
                }
            ...
            for (int ir = 0; ir < nrxx; ir++)
                {
                    // |m| for rho
                    rho_magabs[ir] = std::sqrt (chr->rho[1][ir] * chr->rho[1][ir] + chr->rho[2][ir] * chr->rho[2][ir]
            ...
            for (int ig = 0; ig < npw; ig++)
                {
                    ...
                    double norm = std::sqrt (chr->rho[1][ig] * chr->rho[1][ig] + chr->rho[2][ig] * chr->rho[2][ig]
            ...
            for (int is = 0; is < nspin; is++)
                {
                    ...
                    this->rhodpw->recip_to_real<std::complex<double>, double, base_device::DEVICE_CPU> (chr->rhog[is],
                                                                                                        chr->rho[is]);
                }
        }
...
void
    Charge_Mixing::mix_rho_real (Charge* chr)
{
    ...
    if (nspin == 1)
    ...
    else if (nspin == 2)
        {
            ...
            rhor_out = rho_mag;
            auto screen = std::bind (&Charge_Mixing::Kerker_screen_real, this, std::placeholders::_1);
            auto twobeta_mix = [this, nrxx] (double* out, const double* in, const double* sres)
                {
#ifdef _OPENMP
#pragma omp parallel for schedule(static, 256)
#endif
            ...
            for (int ir = 0; ir < nrxx; ir++)
                {
                    ...
                    rho_magabs_save[ir] = chr->rho_save[0][ir]; // rho_save
                                                                // |m| for rho
                    rho_magabs[nrxx + ir]
                        = std::sqrt (chr->rho[1][ir] * chr->rho[1][ir] + chr->rho[2][ir] * chr->rho[2][ir]
                                     + chr->rho[3][ir] * chr->rho[3][ir]);
                    // |m| for rho_save
            ...
            for (int ir = 0; ir < nrxx; ir++)
                {
                    ...
                    double norm = std::sqrt (chr->rho[1][ir] * chr->rho[1][ir] + chr->rho[2][ir] * chr->rho[2][ir]
                                             + chr->rho[3][ir] * chr->rho[3][ir]);
...
void
    Charge_Mixing::mix_rho (Charge* chr)
{
