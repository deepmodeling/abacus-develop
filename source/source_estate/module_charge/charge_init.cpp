...
void
    Charge::init_rho (const UnitCell& ucell,
                    ...
                    for (int is = 0; is < nspin; ++is)
                        {
                            ...
                            if (ModuleIO::read_vdata_palgrid (
                                    ...
                                    ucell.nat))
                                {
                                    GlobalV::ofs_running << " Read electron density from file: " << ssc.str ()
                                                         << std::endl;
                                }
                            else if (is > 0) // nspin=2 or 4
                                {
                                    if (is == 1) // failed at the second spin
                                        ...
                                        {
                                            GlobalV::ofs_running
                                                << " Didn't read in the electron density but would rearrange it later. "
                                                << std::endl;
                                        }
                    ...
                    if (!read_error)
                        {
                            ...
                            if (ModuleIO::read_rhog (binary.str (), rhopw, kin_g.data ()))
                                {
                                    GlobalV::ofs_running << " Read in the kinetic energy density: " << binary.str ()
                                                         << std::endl;
                                    for (int is = 0; is < nspin; ++is)
                                        {
                                    ...
                                    for (int is = 0; is < nspin; is++)
                                        {
                                            ...
                                            if (ModuleIO::read_vdata_palgrid (pgrid,
                                                ...
                                                {
                                                    GlobalV::ofs_running
                                                        << " Read in the kinetic energy density: " << ssc.str ()
                                                        << std::endl;
                                                }
                                            else
                                                {
                                                    read_kin_error = true;
                                                    std::cout
                                                        << " WARNING: \"init_chg\" is enabled but ABACUS failed to "
                                                           "read kinetic energy "
                                                           "density from file.\n"
    ...
    if (PARAM.inp.init_chg == "wfc")
        {
            ...
            ModuleIO::read_wf2rho_pw (pw_wfc,
                                      symm,
                                      *this,
...
//==========================================================
void
    Charge::set_rho_core (const UnitCell& ucell, const ModuleBase::ComplexMatrix& structure_factor, const bool* numeric)
{
            ...
            if (ucell.atoms[it].ncpp.nlcc)
                {
                    ...
                    // each shell of g vec
                    //----------------------------------------------------------
                    this->non_linear_core_correction (numeric,
                                                      ucell.omega,
                                                      ucell.tpiba2,
...
void
    Charge::non_linear_core_correction (const bool& numeric,
                                        const double omega,
                                        const double tpiba2,
                                        const int mesh,
                                        const double* r,
