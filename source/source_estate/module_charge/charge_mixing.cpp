...
void
    Charge_Mixing::set_mixing (const std::string& mixing_mode_in,
    ...
    if (PARAM.inp.scf_thr_type == 1)
    ...
    else
        {
            if (PARAM.inp.nspin == 4 && PARAM.inp.mixing_angle > 0)
                ...
                {
                    this->mixing->init_mixing_data (this->rho_mdata,
                                                    this->rhopw->nrxx * PARAM.inp.nspin,
                                                    sizeof (double));
                }
        }
...
void
    Charge_Mixing::set_rhopw (ModulePW::PW_Basis* rhopw_in, ModulePW::PW_Basis* rhodpw_in)
{
    this->rhopw = rhopw_in;
...
bool
    Charge_Mixing::if_scf_oscillate (const int iteration,
                                     const double drho,
