...
void
    Charge::set_rhopw (ModulePW::PW_Basis* rhopw_in)
{
    this->rhopw = rhopw_in;
}
...
void
    Charge::allocate (const int& nspin_in, const bool kin_den)
{
...
//-------------------------------------------------------
void
    Charge::atomic_rho (const int spin_number_need,
                        const double& omega,
                        double** rho_in,
                                                ...
                                                if (atom->ncpp.tvanp)
                                                ...
                                                else
                                                    {
                                                        for (int ir = 0; ir < mesh; ++ir)
                                                            {
                                                                ...
                                                                if (r2 != 0)
                                                                    {
                                                                        rhoatm[ir] = atom->ncpp.rho_at[ir]
                                                                                     / ModuleBase::FOUR_PI / r2;
                                                                    }
                                                            }
                                                        rhoatm[0]
                                                            = pow ((rhoatm[2] / rhoatm[1]),
                                                                   atom->ncpp.r[1]
                                                                       / (atom->ncpp.r[2]
                                                                          - atom->ncpp.r[1])); // zws add, sunliang
                                                                                               // updated 2024-03-04
                                                        if (rhoatm[0] < 1e-12)
                                                            {
                                                                rhoatm[0] = rhoatm[1];
                                                        ...
                                                        assert (
                                                            charge != 0.0
                                                            || charge
                                                                   == atom->ncpp.zv); // Peize Lin add charge==atom->zv
                                                                                      // for bsse 2021.04.07
                                                        ...
                                                        for (int ir = 0; ir < mesh; ++ir)
                                                            {
                                                                rhoatm[ir] *= scale;
                                                                rhoatm[ir] *= (ModuleBase::FOUR_PI * atom->ncpp.r[ir]
                                                                               * atom->ncpp.r[ir]);
                                                            }
                                                    }
                                                return rhoatm;
                                                    ...
                                                    for (int ir = 0; ir < mesh; ir++)
                                                        {
                                                            if (atom->ncpp.r[ir] < 1.0e-8)
                                                            ...
                                                            else
                                                                {
                                                                    ...
                                                                    rho1d[ir] = rhoatm[ir] * ModuleBase::libm::sin (gxx)
                                                                                / gxx;
                                                                }
                                                        }
                                                    ModuleBase::Integral::Simpson_Integral (mesh,
...
#pragma omp parallel for
                                                ...
                                                for (int ig = 0; ig < this->rhopw->npw; ig++)
                                                    {
                                                        const std::complex<double> swap
                                                            = strucFac (it, ig) * rho_lgl[this->rhopw->ig2igg[ig]];
                                                        const double up
                                                            = 0.5 * (1 + ucell.magnet.start_mag[it] / atom->ncpp.zv);
                                                        const double dw
                                                            = 0.5 * (1 - ucell.magnet.start_mag[it] / atom->ncpp.zv);
                                                        ...
                                                        for (int ig = 0; ig < this->rhopw->npw; ig++)
                                                            {
                                                                ...
                                                                std::complex<double> swap
                                                                    = ModuleBase::libm::exp (ci_tpi * Gtau)
                                                                      * rho_lgl[this->rhopw->ig2igg[ig]];
                                                ...
                                                for (int ig = 0; ig < this->rhopw->npw; ig++)
                                                    {
                                                        ...
                                                        if (PARAM.globalv.domag)
                                                            { // will not be used now, will be deleted later
                                                                rho_g3d (1, ig)
                                                                    += swap
                                                                       * (ucell.magnet.start_mag[it] / atom->ncpp.zv)
                                                                       * sin_a1 * cos_a2;
                                                                rho_g3d (2, ig)
                                                                    += swap
                                                                       * (ucell.magnet.start_mag[it] / atom->ncpp.zv)
                                                ...
                                                for (int ia = 0; ia < atom->na; ia++)
                                                    {
                                                        ...
                                                        if (PARAM.globalv.domag || PARAM.globalv.domag_z)
                                                            {
                                                                ModuleBase::libm::sincos (atom->angle1[ia],
                                                                                          &sin_a1,
                                                                                          &cos_a1);
                                                            }
                                                        if (PARAM.globalv.domag)
                                                            {
                                                                ModuleBase::libm::sincos (atom->angle2[ia],
                                                                                          &sin_a2,
                                                                                          &cos_a2);
                                                            }
#ifdef _OPENMP
...
double
    Charge::cal_rho2ne (const double* rho_in) const
{
