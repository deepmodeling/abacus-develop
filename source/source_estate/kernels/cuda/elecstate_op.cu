...
__global__ void
elecstate_pw(const int spin, const int nrxx, const FPTYPE w1, FPTYPE* rho, const thrust::complex<FPTYPE>* wfcr) {
...
template <typename FPTYPE>
__global__ void elecstate_pw(const bool DOMAG,
                             const bool DOMAG_Z,
                             const int nrxx,
                             const FPTYPE w1,
                             FPTYPE* rho,
    ...
    if (DOMAG) {
        rho[1 * nrxx + idx] +=
            w1 * 2.0 *
            (wfcr[idx].real() * wfcr_another_spin[idx].real() + wfcr[idx].imag() * wfcr_another_spin[idx].imag());
        rho[2 * nrxx + idx] +=
            w1 * 2.0 *
            (wfcr[idx].real() * wfcr_another_spin[idx].imag() - wfcr_another_spin[idx].real() * wfcr[idx].imag());
...
void elecstate_pw_op<FPTYPE, base_device::DEVICE_GPU>::operator()(const base_device::DEVICE_GPU* ctx,
    ...
    elecstate_pw<FPTYPE>
        <<<block, THREADS_PER_BLOCK>>>(spin, nrxx, w1, rho[0], reinterpret_cast<const thrust::complex<FPTYPE>*>(wfcr));
