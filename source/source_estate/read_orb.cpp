...
{
bool
    read_orb_file (int it, std::string& orb_file, std::ofstream& ofs_running, Atom* atom)
{
    ...
    if (!ifs)
        {
            ...
            ModuleBase::WARNING ("elecstate::read_orb_file", "cannot open the ORBITAL file (NAO basis sets)");
            return false;
        }
    std::string word;
    atom->nw = 0;
