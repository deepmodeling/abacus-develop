...
{
void
    calEBand (const ModuleBase::matrix& ekb, const ModuleBase::matrix& wg, fenergy& f_en)
{
...
void
    calculate_weights (const ModuleBase::matrix& ekb,
    ...
    if (!(Occupy::use_gaussian_broadening || Occupy::fixed_occupations))
        {
            if (PARAM.globalv.two_fermi)
                {
                    Occupy::iweights (nks, klist->wk, nbands, nelec_spin[0], ekb, eferm.ef_up, wg, 0, klist->isk);
                    Occupy::iweights (nks, klist->wk, nbands, nelec_spin[1], ekb, eferm.ef_dw, wg, 1, klist->isk);
                    // ef = ( ef_up + ef_dw ) / 2.0_dp need??? mohan add 2012-04-16
                }
            else
                {
                    // -1 means don't need to consider spin.
                    Occupy::iweights (nks, klist->wk, nbands, PARAM.inp.nelec, ekb, eferm.ef, wg, -1, klist->isk);
                }
        }
...
void
    fixed_weights (const std::vector<double>& ocp_kb,
                   const int& nbands,
                   const double& nelec,
                   const K_Vectors* klist,
                   ModuleBase::matrix& wg,
