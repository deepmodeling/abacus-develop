...
class MockElecState : public ElecState
{
  public:
    void Set_GlobalV_Default()
    {
        ...
        TestParameters::input().nbands = GLOBAL_NBANDS;
        TestParameters::sys().nbands_l = 6;
...
TEST_F(ElecStateTest, InitKS)
{
    ...
    EXPECT_EQ(elecstate->ekb.nc, TestParameters::input().nbands);
    EXPECT_EQ(elecstate->wg.nr, nk);
    EXPECT_EQ(elecstate->wg.nc, TestParameters::input().nbands);
    delete klist;
    delete bigpw;
    delete charge;
}
...
TEST_F(ElecStateTest, GetRho)
{
    ...
    int nk = 1;
    int nrxx = 100;
    charge->rho = new double*[TestParameters::input().nspin];
    for (int i = 0; i < TestParameters::input().nspin; ++i)
    {
...
{
    EXPECT_EQ(TestParameters::input().nbands, 6);
    TestParameters::input().nelec = 30;
    K_Vectors* klist = new K_Vectors;
    klist->set_nks(5);
    klist->set_nkstot(5);
    klist->ik2iktot = {0, 1, 2, 3, 4};
    elecstate->klist = klist;
    elecstate->wg.create(klist->get_nks(), TestParameters::input().nbands);
    std::vector<double> ocp_kb;
    ocp_kb.resize(TestParameters::input().nbands * elecstate->klist->get_nks());
    for (int i = 0; i < ocp_kb.size(); ++i)
    {
        ocp_kb[i] = 1.0;
    }
    elecstate::fixed_weights(ocp_kb, TestParameters::input().nbands, TestParameters::input().nelec,klist,elecstate->wg,elecstate->skip_weights);
    EXPECT_EQ(elecstate->wg(0, 0), 1.0);
    EXPECT_EQ(elecstate->wg(klist->get_nks() - 1, TestParameters::input().nbands - 1), 1.0);
...
TEST_F(ElecStateDeathTest, FixedWeightsWarning1)
{
    ...
    elecstate->wg.create(klist->get_nks(), TestParameters::input().nbands);
    std::vector<double> ocp_kb;
    ocp_kb.resize(TestParameters::input().nbands * elecstate->klist->get_nks() - 1);
    for (int i = 0; i < ocp_kb.size(); ++i)
    {
        ocp_kb[i] = 1.0;
    }
    testing::internal::CaptureStdout();
    EXPECT_EXIT(elecstate::fixed_weights(ocp_kb, TestParameters::input().nbands, TestParameters::input().nelec,klist,elecstate->wg,elecstate->skip_weights),
...
TEST_F(ElecStateTest, CalEBand)
{
    EXPECT_EQ(TestParameters::input().nbands, 6);
    int nks = 5;
    elecstate->wg.create(nks, TestParameters::input().nbands);
    elecstate->ekb.create(nks, TestParameters::input().nbands);
...
TEST_F(ElecStateTest, CalculateWeightsIWeights)
{
    ...
    EXPECT_DOUBLE_EQ(elecstate->wg(nks - 1, TestParameters::input().nelec / 2 - 1), 2.0);
    EXPECT_DOUBLE_EQ(elecstate->wg(nks - 1, TestParameters::input().nbands - 1), 0.0);
...
{
    // get nelec_spin
    TestParameters::sys().two_fermi = true;
    TestParameters::input().nspin = 2;
    elecstate->init_nelec_spin();
    EXPECT_EQ(elecstate->nelec_spin[0], 5.0);
    EXPECT_EQ(elecstate->nelec_spin[1], 5.0);
    //
    EXPECT_FALSE(elecstate->skip_weights);
    int nks = 5 * TestParameters::input().nspin;
    K_Vectors* klist = new K_Vectors;
    klist->set_nks(nks);
    klist->wk.resize(nks);
    for (int ik = 0; ik < nks; ++ik)
    ...
    EXPECT_DOUBLE_EQ(elecstate->wg(nks - 1, TestParameters::input().nelec / 2 - 1), 1.0);
    EXPECT_DOUBLE_EQ(elecstate->wg(nks - 1, TestParameters::input().nbands - 1), 0.0);
...
TEST_F(ElecStateTest, CalculateWeightsGWeights)
{
    ...
    EXPECT_NEAR(elecstate->wg(nks - 1, TestParameters::input().nelec / 2 - 1), 0.33333333333, 1e-10);
    EXPECT_NEAR(elecstate->wg(nks - 1, TestParameters::input().nbands - 1), 0.33333333333, 1e-10);
...
TEST_F(ElecStateTest, CalculateWeightsGWeightsTwoFermi)
{
    ...
    EXPECT_NEAR(elecstate->wg(nks - 1, TestParameters::input().nelec / 2 - 1), 0.16666666666, 1e-10);
    EXPECT_NEAR(elecstate->wg(nks - 1, TestParameters::input().nbands - 1), 0.16666666666, 1e-10);
