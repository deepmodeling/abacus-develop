...
void
    ML_EXX::set_para (const Input_para& inp, const UnitCell* ucell_in, const ModulePW::PW_Basis* rho_basis_in)
{
            ...
            if (PARAM.inp.of_ml_feg != 0)
                {
                    ...
                    for (int i = 0; i < this->ninput; ++i)
                        {
                            if (this->descriptor_type[i] == "gamma")
                                feg_inpt[i] = 1.;
                        }
    ...
    if (PARAM.inp.ml_exx || PARAM.inp.of_ml_gene_data == 1)
        {
            ...
            this->cal_tool->set_para (this->nx,
                                      inp.nelec,
                                      inp.of_tf_weight,
                                      inp.of_vw_weight,
                                      this->chi_p,
 ...
 */
void
    ML_EXX::ml_potential (const double* const* prho, const ModulePW::PW_Basis* pw_rho, ModuleBase::matrix& rpotential)
{
 ...
 */
void
    ML_EXX::generateTrainData (const double* const* prho, const ModulePW::PW_Basis* pw_rho, const double* veff)
{
 ...
 */
void
    ML_EXX::localTest (const double* const* pprho, const ModulePW::PW_Basis* pw_rho)
{
    // for test =====================
