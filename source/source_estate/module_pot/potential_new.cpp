...
Potential::Potential (const ModulePW::PW_Basis* rho_basis_in,
                      ...
                      double* vtxc_in,
                      VSep* vsep_cell_in)
    : ucell_ (ucell_in), vloc_ (vloc_in), structure_factors_ (structure_factors_in), solvent_ (solvent_in),
      vsep_cell (vsep_cell_in), etxc_ (etxc_in), vtxc_ (vtxc_in)
{
...
void
    Potential::pot_register (const std::vector<std::string>& components_list)
{
...
void
    Potential::update_from_charge (const Charge* const chg, const UnitCell* const ucell)
{
...
void
    Potential::init_pot (const Charge* const chg)
{
...
void
    Potential::get_vnew (const Charge* chg, ModuleBase::matrix& vnew)
{
...
template <>
float*
    Potential::get_veff_smooth_data ()
{
...
template <>
float*
    Potential::get_vofk_smooth_data ()
{
...
double
    Potential::get_ml_exx_energy () const
{
#ifdef __MLALGO
    for (size_t i = 0; i < this->components.size (); i++)
        {
