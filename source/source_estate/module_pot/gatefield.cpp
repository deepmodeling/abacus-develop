...
void
    Gatefield::add_gatefield (double* vltot,
                              const UnitCell& cell,
    ...
    if (block)
        {
            if (PARAM.inp.dip_cor_flag)
                {
                    GlobalV::ofs_running << "Adding potential to prevent charge spilling into region of the gate"
                                         << std::endl;
                }
            else
                {
                    GlobalV::ofs_running
                        << "Adding potential to prevent interaction between lower and upper part of unit cell"
                        << std::endl;
                }
                    ...
                    if (block && gatepos >= block_down && gatepos <= block_up && !PARAM.inp.dip_cor_flag)
                        {
                            if (gatepos - zgate <= -block_size / 2.0 * 0.9) // smooth increase within the first 10%
                                {
                                    value += block_height * (gatepos - zgate + block_size / 2.0)
                                             / (0.1 * block_size / 2.0);
                                }
...
double
    Gatefield::mopopla (double& zgate, double z, bool flag)
{
    while (z > 1.0)
        {
            z -= 1.0;
...
void
    Gatefield::compute_force (const UnitCell& cell, ModuleBase::matrix& fgate)
{
    int iat = 0;
    for (int it = 0; it < cell.ntype; ++it)
        {
            ...
            for (int ia = 0; ia < cell.atoms[it].na; ++ia)
                {
                    ...
                    for (int jj = 0; jj < 3; ++jj)
                        {
                            fgate (iat, jj) = -zval * ModuleBase::e2 * rho_surface * Efield::bvec[jj] / Efield::bmod
                                              * (fac - 2.0 * pos);
                        }
                    ++iat;
                }
