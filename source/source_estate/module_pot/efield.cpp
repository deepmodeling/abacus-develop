...
//=======================================================
ModuleBase::matrix
    Efield::add_efield (const UnitCell& cell,
    ...
    if (PARAM.inp.dip_cor_flag)
        {
            ...
            // energy correction
            etotefield
                = -ModuleBase::e2 * (efield_amp - 0.5 * tot_dipole) * tot_dipole * cell.omega / ModuleBase::FOUR_PI;
        }
    else
        {
            ion_dipole = cal_ion_dipole (cell, bmod);
...
//=======================================================
double
    Efield::cal_ion_dipole (const UnitCell& cell, const double& bmod)
{
    double ion_dipole = 0;
    for (int it = 0; it < cell.ntype; ++it)
        {
    ...
    if (PARAM.inp.gate_flag && PARAM.inp.dip_cor_flag)
        {
            ...
            ion_dipole
                += (PARAM.inp.nelec - ion_charge) * saw_function (efield_pos_max, efield_pos_dec, Gatefield::zgate);
        }
...
double
    Efield::cal_elec_dipole (const UnitCell& cell,
                             const ModulePW::PW_Basis* rho_basis,
                             const int& nspin,
                             const double* const* const rho,
                             const double& bmod)
...
double
    Efield::cal_induced_dipole (const UnitCell& cell,
                                const ModulePW::PW_Basis* rho_basis,
                                const surchem& solvent,
                                const double& bmod)
{
...
double
    Efield::saw_function (const double& a, const double& b, const double& x)
{
    assert (x >= 0);
    assert (x <= 1);
...
void
    Efield::compute_force (const UnitCell& cell, ModuleBase::matrix& fdip)
{
    if (PARAM.inp.dip_cor_flag)
        {
            int iat = 0;
                    ...
                    for (int ia = 0; ia < cell.atoms[it].na; ++ia)
                        {
                            for (int jj = 0; jj < 3; ++jj)
                                {
                                    fdip (iat, jj) = ModuleBase::e2 * (efield_amp - tot_dipole) * cell.atoms[it].ncpp.zv
                                                     * bvec[jj] / bmod;
                                }
                            ++iat;
                        }
                    ...
                    for (int ia = 0; ia < cell.atoms[it].na; ++ia)
                        {
                            for (int jj = 0; jj < 3; ++jj)
                                {
                                    fdip (iat, jj)
                                        = ModuleBase::e2 * efield_amp * cell.atoms[it].ncpp.zv * bvec[jj] / bmod;
                                }
                            ++iat;
                        }
                }
...
void
    Efield::prepare (const UnitCell& cell, double& latvec, double& area)
{
    if (efield_dir == 0)
        {
            bvec[0] = cell.G.e11;
...
void
    Efield::autoset (std::vector<double>& pos)
{
