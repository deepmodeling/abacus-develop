...
Pot_Cosikr::Pot_Cosikr (const ModulePW::PW_Basis* rho_basis_in,
                        const ModuleBase::Vector3<double>& kvec_d_in,
                        const std::vector<double>& phase_in,
                        const std::vector<double>& amplitude_in)
    : kvec_d (kvec_d_in), phase (phase_in), amplitude (amplitude_in)
{
...
void
    Pot_Cosikr::cal_v_eff (const Charge* const chg, const UnitCell* const ucell, ModuleBase::matrix& v_eff)
{
            ...
            for (int iy = 0; iy < this->rho_basis_->ny; ++iy)
                {
                    ...
                    for (int iz = this->rho_basis_->startz_current;
                         iz < this->rho_basis_->startz_current + this->rho_basis_->nplane;
                         ++iz)
                        {
                            const double phase_xyz = phase_xy + this->kvec_d.z * iz / this->rho_basis_->nz;
                            for (int is = 0; is < v_eff.nr; ++is)
                                {
                                    v_eff (is, ir) += this->amplitude[is]
                                                      * std::cos ((phase_xyz + this->phase[is]) * ModuleBase::TWO_PI);
                                }
                            ++ir;
                        }
                }
