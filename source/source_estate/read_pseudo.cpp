...
{
void
    read_pseudo (std::ofstream& ofs, UnitCell& ucell)
{
                                    ...
                                    for (int j = 0; j < cut_mesh; ++j)
                                        {
                                            ofs << std::setw (15) << atom->ncpp.r[j] << std::setw (15)
                                                << atom->ncpp.betar (i, j) << std::setw (15) << atom->ncpp.rab[j]
                                                << std::endl;
                                        }
                                    ofs << "</PP_BETA>" << std::endl;
                                }
...
//==========================================================
void
    read_cell_pseudopots (const std::string& pp_dir, std::ofstream& log, UnitCell& ucell)
{
                    ...
                    if (error == 0) // mohan add 2021-04-16
                        {
                            ...
                            error_ap = upf.average_p (PARAM.inp.soc_lambda,
                                                      ucell.atoms[i].ncpp); // added by zhengdy 2020-10-20
                        }
                            ...
                            if (xc_func1 != ucell.atoms[i].ncpp.xc_func)
                                {
                                    ...
                                    GlobalV::ofs_warning << " NAME OF ELEMENT      : " << ucell.atoms[i].label
                                                         << std::endl;
                                    GlobalV::ofs_warning << " DFT FUNC. (PSEUDO)   : " << ucell.atoms[i].ncpp.xc_func
                                                         << std::endl;
                                    GlobalV::ofs_warning << " DFT FUNC. (SET TO)   : " << xc_func1 << std::endl;
                                    GlobalV::ofs_warning << " MAKE SURE THIS DFT FUNCTIONAL IS WHAT YOU NEED"
                                                         << std::endl;
...
void
    print_unitcell_pseudo (const std::string& fn, UnitCell& ucell)
{
