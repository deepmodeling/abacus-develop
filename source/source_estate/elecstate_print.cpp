 ...
 */
void
    print_scf_iterinfo (const std::string& ks_solver,
                        const int& istep,
                        const int& witer,
...
void
    print_etot (const Magnetism& magnet,
                ...
                const bool print)
{
    ...
    std::transform (energies_Ry.begin (),
                    energies_Ry.end (),
                    energies_eV.begin (),
                    [] (double ener) { return ener * ModuleBase::Ry_to_eV; });
...
/// @param value: value
void
    print_format (const std::string& name, const double& value)
{
