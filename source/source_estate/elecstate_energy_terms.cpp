...
double
    ElecState::get_solvent_model_Ael ()
{
    return surchem::Ael;
}

double
    ElecState::get_solvent_model_Acav ()
{
    return surchem::Acav;
}

double
    ElecState::get_dftu_energy ()
{
    return Plus_U::get_energy ();
}

double
    ElecState::get_local_pp_energy ()
{
    double local_pseudopot_energy = 0.; // electron-ion interaction energy from local pseudopotential
    for (int is = 0; is < PARAM.inp.nspin; ++is)
        {
            local_pseudopot_energy += BlasConnector::dot (this->charge->rhopw->nrxx,
