    ...
    cal_ux (UnitCell& ucell)
{
            ...
            for (int ia = 0; ia < ucell.atoms[it].na; ia++)
                {
                    ...
                    amag = pow (ucell.atoms[it].m_loc_[ia].x, 2) + pow (ucell.atoms[it].m_loc_[ia].y, 2)
            ...
            for (int ia = 0; ia < ucell.atoms[it].na; ia++)
                {
                    if (it > starting_it || ia > starting_ia)
                        {
                            ucell.magnet.lsign_
                                = ucell.magnet.lsign_ && judge_parallel (ucell.magnet.ux_, ucell.atoms[it].m_loc_[ia]);
                        }
                }
        }
    ...
    if (ucell.magnet.lsign_)
        {
            uxmod = pow (ucell.magnet.ux_[0], 2) + pow (ucell.magnet.ux_[1], 2) + pow (ucell.magnet.ux_[2], 2);
...
bool
    judge_parallel (double a[3], ModuleBase::Vector3<double> b)
{
    bool jp = false;
    double cross = 0.0;
    cross
        = pow ((a[1] * b.z - a[2] * b.y), 2) + pow ((a[2] * b.x - a[0] * b.z), 2) + pow ((a[0] * b.y - a[1] * b.x), 2);
    jp = (fabs (cross) < 1e-6);
    return jp;
}
} // namespace elecstate
