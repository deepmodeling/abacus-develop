    ...
    fenergy::calculate_etot ()
{
    etot = eband + deband + (etxc - etxcc) + ewald_energy + hartree_energy + demet + descf + exx + efield + gatefield
           + evdw + esol_el + esol_cav + edftu + edeepks_scf + escon + ml_exx;
    return etot;
}
...
void
    Efermi::set_efval (const int& is, const double& ef_in)
{
    if (!two_efermi)
        {
            this->ef = ef_in;
...
double
    Efermi::get_efval (const int& is) const
{
    if (!two_efermi)
        {
            return this->ef;
