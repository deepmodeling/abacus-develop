...
{
void
    print_local_matrix (std::ostream& os,
...
void
    cal_edm_tddft (Parallel_Orbitals& pv,
...
void
    cal_edm_tddft_tensor (Parallel_Orbitals& pv,
                          LCAO_domain::Setup_DM<std::complex<double>>& dmat,
                          K_Vectors& kv,
                          hamilt::Hamilt<std::complex<double>>* p_hamilt)
{
    ...
    for (int ik = 0; ik < kv.get_nks (); ++ik)
        {
            ...
            ct::Tensor Htmp_tensor (ct::DataType::DT_COMPLEX_DOUBLE,
                                    ct::DeviceType::CpuDevice,
                                    ct::TensorShape ({nloc}));
            Htmp_tensor.zero ();

            ct::Tensor Sinv_tensor (ct::DataType::DT_COMPLEX_DOUBLE,
                                    ct::DeviceType::CpuDevice,
                                    ct::TensorShape ({nloc}));
            Sinv_tensor.zero ();

            ct::Tensor tmp1_tensor (ct::DataType::DT_COMPLEX_DOUBLE,
                                    ct::DeviceType::CpuDevice,
                                    ct::TensorShape ({nloc}));
            tmp1_tensor.zero ();

            ct::Tensor tmp2_tensor (ct::DataType::DT_COMPLEX_DOUBLE,
                                    ct::DeviceType::CpuDevice,
                                    ct::TensorShape ({nloc}));
            tmp2_tensor.zero ();

            ct::Tensor tmp3_tensor (ct::DataType::DT_COMPLEX_DOUBLE,
                                    ct::DeviceType::CpuDevice,
                                    ct::TensorShape ({nloc}));
            tmp3_tensor.zero ();

            ct::Tensor tmp4_tensor (ct::DataType::DT_COMPLEX_DOUBLE,
                                    ct::DeviceType::CpuDevice,
                                    ct::TensorShape ({nloc}));
            tmp4_tensor.zero ();
...
void
    cal_edm_tddft_tensor_lapack (Parallel_Orbitals& pv,
                                 LCAO_domain::Setup_DM<std::complex<double>>& dmat,
                                 K_Vectors& kv,
                                 hamilt::Hamilt<std::complex<double>>* p_hamilt)
{
            ...
            if (myid == root_proc)
                {
                    ...
                    H_dev = ct::Tensor (ct::DataType::DT_COMPLEX_DOUBLE,
                                        ct_device_type,
                                        ct::TensorShape ({nlocal, nlocal}));
                    syncmem_complex_h2d_op () (H_dev.template data<std::complex<double>> (), h_src, nlocal * nlocal);

                    S_dev = ct::Tensor (ct::DataType::DT_COMPLEX_DOUBLE,
                                        ct_device_type,
                                        ct::TensorShape ({nlocal, nlocal}));
                    syncmem_complex_h2d_op () (S_dev.template data<std::complex<double>> (), s_src, nlocal * nlocal);

                    DMK_dev = ct::Tensor (ct::DataType::DT_COMPLEX_DOUBLE,
                                          ct_device_type,
                                          ct::TensorShape ({nlocal, nlocal}));
                    syncmem_complex_h2d_op () (DMK_dev.template data<std::complex<double>> (),
                                               dmk_src,
                                               nlocal * nlocal);
                    ...
                    // tmp1 = H * Sinv
                    ct::Tensor tmp1_dev (ct::DataType::DT_COMPLEX_DOUBLE,
                                         ct_device_type,
                                         ct::TensorShape ({nlocal, nlocal}));
                    ...
                    // tmp2 = tmp1^T * DMK
                    ct::Tensor tmp2_dev (ct::DataType::DT_COMPLEX_DOUBLE,
                                         ct_device_type,
                                         ct::TensorShape ({nlocal, nlocal}));
                    ct::kernels::blas_gemm<std::complex<double>, ct_Device> () (
                        'T',
                    ...
                    // tmp3 = Sinv * H
                    ct::Tensor tmp3_dev (ct::DataType::DT_COMPLEX_DOUBLE,
                                         ct_device_type,
                                         ct::TensorShape ({nlocal, nlocal}));
                    ct::kernels::blas_gemm<std::complex<double>, ct_Device> () (
                        'N',
                    ...
                    // tmp4 = DMK * tmp3^T
                    ct::Tensor tmp4_dev (ct::DataType::DT_COMPLEX_DOUBLE,
                                         ct_device_type,
                                         ct::TensorShape ({nlocal, nlocal}));
                    ct::kernels::blas_gemm<std::complex<double>, ct_Device> () (
                        'N',
                    ...
                    if (num_procs == 1 || myid == root_proc)
                        {
                            syncmem_complex_d2h_op () (edm_dest,
                                                       tmp4_dev.template data<std::complex<double>> (),
                                                       nlocal * nlocal);
                        }
                }
