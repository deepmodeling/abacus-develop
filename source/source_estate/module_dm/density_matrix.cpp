...
DensityMatrix<TK, TR>::DensityMatrix (const Parallel_Orbitals* paraV_in,
                                      ...
                                      const int nk)
    : _paraV (paraV_in), _nspin (nspin), _kvec_d (kvec_d),
...
DensityMatrix<TK, TR>::DensityMatrix (const Parallel_Orbitals* paraV_in, const int nspin)
    : _paraV (paraV_in), _nspin (nspin), _kvec_d ({ModuleBase::Vector3<double> (0, 0, 0)}), _nk (1)
{
...
void
    DensityMatrix_Tools::cal_DMR (const DensityMatrix<TK, TR_in>& dm,
                    ...
                    for (int ik = 0; ik < dm._nk; ++ik)
                        {
                            ...
                            // copy column-major DMK to row-major DMK_mat_trans (for the purpose of computational
                            // efficiency)
                            const TK* const DMK_mat_ptr
                                = dm._DMK[ik + ik_begin].data () + col_ap * dm._paraV->nrow + row_ap;
...
template <>
void
    DensityMatrix<std::complex<double>, double>::cal_DMR (const int ik_in)
{
...
template <>
void
    DensityMatrix<std::complex<double>, std::complex<double>>::cal_DMR (const int ik_in)
{
    DensityMatrix_Tools::cal_DMR (*this, this->_DMR, ik_in);
}
...
template <typename TK, typename TR_in, typename TR_out>
void
    DensityMatrix_Tools::cal_DMR_td (const DensityMatrix<TK, TR_in>& dm,
                                     std::vector<hamilt::HContainer<TR_out>*>& dmR_out,
...
template <>
void
    DensityMatrix<double, double>::cal_DMR_td (const UnitCell& ucell,
                                               const ModuleBase::Vector3<double> At,
                                               const int ik_in)
{
    return;
...
template <>
void
    DensityMatrix<std::complex<double>, double>::cal_DMR_td (const UnitCell& ucell,
                                                             const ModuleBase::Vector3<double> At,
                                                             const int ik_in)
{
...
template <>
void
    DensityMatrix<std::complex<double>, std::complex<double>>::cal_DMR_td (const UnitCell& ucell,
                                                                           const ModuleBase::Vector3<double> At,
                                                                           const int ik_in)
{
    DensityMatrix_Tools::cal_DMR_td (*this, this->_DMR, ucell, At, ik_in);
...
template <typename TK, typename TR_in, typename TR_out>
void
    DensityMatrix_Tools::cal_DMR_full (const DensityMatrix<TK, TR_in>& dm,
            ...
            for (int ik = 0; ik < dm._nk; ++ik)
                {
                    ...
                    const TK* const DMK_mat_ptr = dm._DMK[ik].data () + col_ap * dm._paraV->nrow + row_ap;
                    for (int icol = 0; icol < col_size; ++icol)
                        {
                            for (int irow = 0; irow < row_size; ++irow)
                                {
                    ...
                    for (int iR = 0; iR < R_size; ++iR)
                        {
                            const TK kphase = kphase_vec[ik][iR];
                            BlasConnector::axpy (mat_size, kphase, DMK_mat_trans.data (), 1, target_DMR_mat_vec[iR], 1);
                        }
                }
        }
...
template <>
void
    DensityMatrix<double, double>::cal_DMR_full (hamilt::HContainer<std::complex<double>>* dmR_out,
                                                 const int ik_in) const
{
}
template <>
void
    DensityMatrix<std::complex<double>, double>::cal_DMR_full (hamilt::HContainer<std::complex<double>>* dmR_out,
                                                               const int ik_in) const
{
...
template <>
void
    DensityMatrix<double, double>::cal_DMR (const int ik_in)
{
    ModuleBase::TITLE ("DensityMatrix", "cal_DMR");
    using TK = double;
    using TR = double;
            ...
            for (int i = 0; i < target_DMR->size_atom_pairs (); ++i)
                {
                    ...
                    const TK* DMK_mat_ptr = this->_DMK[0 + ik_begin].data () + col_ap * this->_paraV->nrow + row_ap;
                    // set DMR element
                    TR* target_DMR_ptr = target_mat->get_pointer ();
                    for (int mu = 0; mu < row_size; ++mu)
                        {
                            BlasConnector::axpy (col_size, kphase, DMK_mat_ptr, ld_hk, target_DMR_ptr, 1);
                            DMK_mat_ptr += 1;
...
template <typename TK, typename TR>
void
    DensityMatrix<TK, TR>::switch_dmr (const int mode)
{
...
template <>
void
    DensityMatrix_Tools::func_exp_mul_dmk<double> (const std::complex<double> kphase,
                                                   ...
                                                   double* target_DMR_mat)
{
    ...
    for (std::size_t i = 0; i < mat_size; i++)
        {
            target_DMR_mat[i] += kphase.real () * DMK_mat_trans[i].real () - kphase.imag () * DMK_mat_trans[i].imag ();
        }
}

template <>
void
    DensityMatrix_Tools::func_exp_mul_dmk<std::complex<double>> (const std::complex<double> kphase,
                                                                 const std::vector<std::complex<double>>& DMK_mat_trans,
                                                                 std::complex<double>* target_DMR_mat)
{
    BlasConnector::axpy (DMK_mat_trans.size (), kphase, DMK_mat_trans.data (), 1, target_DMR_mat, 1);
}

template <>
void
    DensityMatrix_Tools::func_xyz_to_updown<double> (const std::complex<double> tmp[4],
                                                     const int icol,
                                                     ...
                                                     double* target_DMR_mat)
{
    ...
    target_DMR_mat[icol + step_trace[2]]
        = -tmp[1].imag () + tmp[2].imag (); // rho_y = (i * (rho_updown - rho_downup)).real()
    target_DMR_mat[icol + step_trace[3]] = tmp[0].real () - tmp[3].real (); // rho_z = (rho_upup - rho_downdown).real()
}

template <>
void
    DensityMatrix_Tools::func_xyz_to_updown<std::complex<double>> (const std::complex<double> tmp[4],
                                                                   const int icol,
                                                                   const int step_trace[4],
                                                                   std::complex<double>* target_DMR_mat)
{
    ...
    target_DMR_mat[icol + step_trace[2]]
        = ModuleBase::IMAG_UNIT * (tmp[1].imag () - tmp[2].imag ()); // rho_y = (i * (rho_updown - rho_downup))
