...
void DensityMatrix_Tools::cal_DMR(const DensityMatrix<TK, TR_in>& dm,
...
#pragma omp parallel for schedule(dynamic)
        ...
        for (int i = 0; i < target_DMR->size_atom_pairs(); ++i) {
            ...
            for (int ik = 0; ik < dm._nk; ++ik) {
                ...
                const TK* const DMK_mat_ptr = dm._DMK[ik + ik_begin].data() + col_ap * dm._paraV->nrow + row_ap;
                ...
                for (int iR = 0; iR < R_size; ++iR) {
                    ...
                    if (PARAM.inp.nspin != 4) // only save real kr*Dr-ki*Di
                    {
                        ...
                        BlasConnector::axpy(mat_size, kphase, DMK_mat_trans.data(), 1, &tmp_DMR[iR * mat_size], 1);
                    }
                }
            }
...
// calculate DMR from DMK using blas for multi-k calculation
template <typename TK, typename TR_in, typename TR_out>
void DensityMatrix_Tools::cal_DMR_td(const DensityMatrix<TK, TR_in>& dm,
                                     std::vector<hamilt::HContainer<TR_out>*>& dmR_out,
...
template <>
void DensityMatrix<double, double>::cal_DMR_td(const UnitCell& ucell,
                                               const ModuleBase::Vector3<double> At,
                                               const int ik_in) {
    return;
}
template <>
void DensityMatrix<std::complex<double>, double>::cal_DMR_td(const UnitCell& ucell,
                                                             const ModuleBase::Vector3<double> At,
                                                             const int ik_in) {
...
template <>
void DensityMatrix<std::complex<double>, std::complex<double>>::cal_DMR_td(const UnitCell& ucell,
                                                                           const ModuleBase::Vector3<double> At,
                                                                           const int ik_in) {
    DensityMatrix_Tools::cal_DMR_td(*this, this->_DMR, ucell, At, ik_in);
}
...
// calculate DMR from DMK using blas for multi-k calculation
template <typename TK, typename TR_in, typename TR_out>
void DensityMatrix_Tools::cal_DMR_full(const DensityMatrix<TK, TR_in>& dm,
...
template <>
void DensityMatrix<double, double>::cal_DMR_full(hamilt::HContainer<std::complex<double>>* dmR_out,
                                                 const int ik_in) const {}
template <>
void DensityMatrix<std::complex<double>, double>::cal_DMR_full(hamilt::HContainer<std::complex<double>>* dmR_out,
                                                               const int ik_in) const {
...
template <>
void DensityMatrix_Tools::func_exp_mul_dmk<double>(const std::complex<double> kphase,
...
template <>
void DensityMatrix_Tools::func_exp_mul_dmk<std::complex<double>>(const std::complex<double> kphase,
                                                                 const std::vector<std::complex<double>>& DMK_mat_trans,
...
template <>
void DensityMatrix_Tools::func_xyz_to_updown<double>(const std::complex<double> tmp[4],
                                                     const int icol,
...
template <>
void DensityMatrix_Tools::func_xyz_to_updown<std::complex<double>>(const std::complex<double> tmp[4],
                                                                   const int icol,
                                                                   const int step_trace[4],
                                                                   std::complex<double>* target_DMR_mat) {
