...
template <typename T, typename Device>
void
    ElecStatePW<T, Device>::init_rho_data ()
{
    if (this->init_rho)
        {
            return;
...
template <typename T, typename Device>
void
    ElecStatePW<T, Device>::updateRhoK (const psi::Psi<T, Device>& psi)
{
    this->rhoBandK (psi);
}
...
template <typename T, typename Device>
void
    ElecStatePW<T, Device>::parallelK ()
{
#ifdef __MPI
    this->charge->rho_mpi ();
#endif
...
template <typename T, typename Device>
void
    ElecStatePW<T, Device>::rhoBandK (const psi::Psi<T, Device>& psi)
{
                                            ...
                                            for (int ih = 0; ih < atom->ncpp.nh; ih++)
                                                {
                                                    ...
                                                    for (int ib = 0; ib < nbands; ib++)
                                                        {
                                                            auxk1[ih * nbands + ib]
                                                                = becp[ib * this->ppcell->nkb + ikb];
                                                            auxk2[ih * nbands + ib]
                                                                = becp[ib * this->ppcell->nkb + ikb]
                                                                  * static_cast<Real> (this->wg (ik, ib));
                                                        }
                                                }
                                                    ...
                                                    for (int jh = ih; jh < atom->ncpp.nh; jh++)
                                                        {
                                                            ...
                                                            if (ih == jh)
                                                                {
                                                                    becsum[index + ijh]
                                                                        += std::real (aux_gk[ih * atom->ncpp.nh + jh]);
                                                                }
                                                            else
                                                                {
                                                                    becsum[index + ijh]
...
template <typename T, typename Device>
void
    ElecStatePW<T, Device>::add_usrho (const psi::Psi<T, Device>& psi)
{
    if (PARAM.globalv.use_uspp)
        {
            this->cal_becsum (psi);
...
template <typename T, typename Device>
void
    ElecStatePW<T, Device>::addusdens_g (const Real* becsum, T** rhog)
{
    const T one{1, 0};
    const T zero{0, 0};
