...
void TwoCenterIntegrator::calculate(const int itype1,
    ...
    if (hess_out)
        std::fill(hess_out, hess_out + 9, 0.0);
    ...
    if (hess_out && (l1 + l2 > 6)) {
        ModuleBase::WARNING_QUIT("TwoCenterIntegrator::calculate", "Hessian computation not supported for l1+l2 > 6");
    }
    ...
    if (grad_out || hess_out)
        ModuleBase::Ylm::grad_rl_sph_harm(l1 + l2, vR[0], vR[1], vR[2], Rl_Y.data(), grad_Rl_Y.data());
    if (hess_out)
        ModuleBase::Ylm::hes_rl_sph_harm(l1 + l2, vR[0], vR[1], vR[2], hess_Rl_Y);
    ...
    for (int l = std::abs(l1 - l2); l <= l1 + l2; l += 2) {
        ...
        for (int m = -l; m <= l; ++m) {
            ...
            if (grad_out) {
                for (int i = 0; i < 3; ++i) {
                    grad_out[i] +=
                        sign * G * ((*d_S_by_Rl) * uR[i] * Rl_Y[lm_idx] + (*S_by_Rl) * grad_Rl_Y[lm_idx * 3 + i]);
                }
            }
            ...
            if (hess_out) {
                ...
                double H_full[9] = {hess_Rl_Y[lm_idx][0],
                                    hess_Rl_Y[lm_idx][1],
                                    hess_Rl_Y[lm_idx][2],
                                    hess_Rl_Y[lm_idx][1],
                                    hess_Rl_Y[lm_idx][3],
                ...
                for (int alpha = 0; alpha < 3; ++alpha) {
                    for (int beta = 0; beta < 3; ++beta) {
                        ...
                        double term2 = (*d_S_by_Rl) * (du_dR * Rl_Y[lm_idx] + uR[alpha] * grad_Rl_Y[lm_idx * 3 + beta] +
...
int TwoCenterIntegrator::ylm_index(const int l, const int m) const { return l * l + (m > 0 ? 2 * m - 1 : -2 * m); }
