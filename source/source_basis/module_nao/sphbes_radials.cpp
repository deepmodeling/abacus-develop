...
void
    SphbesRadials::build (const std::string& file,
                          const double dr,
                          const int itype,
...
void
    SphbesRadials::build (const int lmax,
                          const int nbes,
                          const double rcut,
                          const double sigma,
                          const double dr,
...
void
    SphbesRadials::read_coeff (std::ifstream& ifs, std::ofstream* ptr_log, const int rank)
{
    std::string info, tmp;
    if (rank == 0)
        {
    ...
    for (size_t i = 0; i < delim.size () - 1; ++i)
        {
            ...
            std::transform (v.begin () + delim[i] + 6,
                            v.begin () + delim[i + 1],
                            coeff_q.begin (),
...
std::string
    SphbesRadials::extract (std::string const& str, std::string const& keyword)
{
    std::smatch match;
...
std::vector<std::string>
    SphbesRadials::split (std::string const& str, const char* delim)
{
...
std::vector<double>
    SphbesRadials::sphbes_comb (const int l,
...
double
    SphbesRadials::smooth (double r, double rcut, double sigma)
{
    return (r < rcut) * (sigma == 0 ? 1.0 : 1.0 - std::exp (-0.5 * std::pow ((r - rcut) / sigma, 2)));
}

void
    SphbesRadials::build_radset (const bool normalize)
{
