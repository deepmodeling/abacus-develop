...
void
    RealGauntTable::build (const int lmax)
{
#ifdef __DEBUG
    assert (lmax >= 0);
#endif
                            ...
                            for (int m3 = 0; m3 <= l3; ++m3)
                                {
                                    ...
                                    for (int m2 = -l2; m2 <= m2_max; ++m2)
                                        {
                                            int m1 = -m2 - m3;
                                            gaunt_table_[std::array<int, 6> ({l1, l2, l3, m1, m2, m3})]
...
const double&
    RealGauntTable::operator() (const int l1, const int l2, const int l3, const int m1, const int m2, const int m3)
        const
{
#ifdef __DEBUG
...
double
    RealGauntTable::real_gaunt_lookup (const int l1,
                                       const int l2,
                                       const int l3,
                                       const int m1,
                                       const int m2,
...
double
    RealGauntTable::gaunt (const int l1, const int l2, const int l3, const int m1, const int m2, const int m3) const
{
    ...
    double wigner1
        = minus_1_pow (g) * tri * factorial (g) / factorial (g - l1) / factorial (g - l2) / factorial (g - l3);
    ...
    wigner2 *= tri * minus_1_pow (l1 - l2 - m3)
               * std::sqrt (factorial (l1 + m1) * factorial (l1 - m1) * factorial (l2 + m2) * factorial (l2 - m2)
...
bool
    RealGauntTable::is_valid_lm (const int l1, const int l2, const int l3, const int m1, const int m2, const int m3)
        const
{
...
bool
    RealGauntTable::gaunt_select_l (const int l1, const int l2, const int l3) const
{
    return l1 + l2 >= l3 && l1 + l3 >= l2 && l2 + l3 >= l1 && (l1 + l2 + l3) % 2 == 0;
}

bool
    RealGauntTable::real_gaunt_select_m (const int m1, const int m2, const int m3) const
{
    return (((m1 < 0) + (m2 < 0) + (m3 < 0)) % 2 == 0)
           && (std::abs (m1) + std::abs (m2) == std::abs (m3) || std::abs (m2) + std::abs (m3) == std::abs (m1)
               || std::abs (m3) + std::abs (m1) == std::abs (m2));
}

double
    RealGauntTable::gaunt_lookup (const int l1, const int l2, const int l3, const int m1, const int m2, const int m3)
        const
{
#ifdef __DEBUG
    assert (is_valid_lm (l1, l2, l3, m1, m2, m3));
...
std::array<int, 6>
    RealGauntTable::gaunt_key (const int l1, const int l2, const int l3, const int m1, const int m2, const int m3) const
{
#ifdef __DEBUG
    assert (is_valid_lm (l1, l2, l3, m1, m2, m3));
#endif
...
void
    RealGauntTable::arrange (int& l1, int& l2, int& m1, int& m2) const
{
    if (l1 < l2)
        {
            std::swap (l1, l2);
...
double
    RealGauntTable::factorial (const int n) const
{
#ifdef __DEBUG
    assert (n >= 0);
#endif
...
int
    RealGauntTable::index_map (int l, int m) const
{
#ifdef __DEBUG
    assert (std::abs (m) <= l);
#endif
