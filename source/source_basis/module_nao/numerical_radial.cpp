...
NumericalRadial::NumericalRadial (const NumericalRadial& other)
    : symbol_ (other.symbol_), itype_ (other.itype_), l_ (other.l_), izeta_ (other.izeta_), nr_ (other.nr_),
      nk_ (other.nk_), ircut_ (other.ircut_), ikcut_ (other.ikcut_), is_fft_compliant_ (other.is_fft_compliant_),
      pr_ (other.pr_), pk_ (other.pk_), sbt_ (other.sbt_)
{
    // deep copy
    if (other.rgrid ())
        {
...
NumericalRadial&
    NumericalRadial::operator= (const NumericalRadial& rhs)
{
    if (this == &rhs)
        {
            return *this;
...
void
    NumericalRadial::build (const int l,
...
void
    NumericalRadial::to_numerical_orbital_lm (Numerical_Orbital_Lm& orbital_lm,
                                              const int nk_legacy,
...
void
    NumericalRadial::set_transformer (ModuleBase::SphericalBesselTransformer sbt, int update)
{
    sbt_ = sbt;
...
void
    NumericalRadial::set_grid (const bool for_r_space, const int ngrid, const double* const grid, const char mode)
{
#ifdef __DEBUG
    ...
    if (mode == 't')
        ...
        { // obtain new values by interpolation in the current space
          // make sure an interpolation in the current space is available
            ...
            cubspl.eval (std::distance (grid_start, grid_end),
                         grid_start,
                         value_new + std::distance (grid_new, grid_start));
...
void
    NumericalRadial::set_uniform_grid (const bool for_r_space,
                                       const int ngrid,
                                       const double cutoff,
                                       const char mode,
...
void
    NumericalRadial::set_value (const bool for_r_space, const double* const value, const int p)
{
#ifdef __DEBUG
...
void
    NumericalRadial::wipe (const bool r_space, const bool k_space)
{
#ifdef __DEBUG
...
void
    NumericalRadial::radtab (const char op,
...
void
    NumericalRadial::normalize (bool for_r_space)
{
...
void
    NumericalRadial::transform (const bool forward)
{
#ifdef __DEBUG
...
void
    NumericalRadial::set_icut (const bool for_r_space, const bool for_k_space, const double tol)
{
    if (for_r_space)
        {
#ifdef __DEBUG
...
bool
    NumericalRadial::is_uniform (const int n, const double* const x, const double tol)
{
...
bool
    NumericalRadial::is_fft_compliant (const int nr,
