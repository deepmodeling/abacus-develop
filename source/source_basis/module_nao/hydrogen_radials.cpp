...
void
    HydrogenRadials::build (const int itype,
                            const double charge,
...
std::vector<double>
    HydrogenRadials::generate_hydrogen_radial_segment (const double charge,
                                                       const bool with_slater_screening,
                                                       const int n,
                                                       const int l,
                                                       const double rmin,
                                                       ...
                                                       std::ofstream* ptr_log)
{
    ...
    double norm_factor = sqrt (
        4.0 * std::pow (screened_charge, 3) * static_cast<double> (this->assoc_laguerre_.factorial (n - l - 1))
        / std::pow (double (n), 4) / static_cast<double> (this->assoc_laguerre_.factorial (n + l)) / std::pow (a0, 3));
...
double
    HydrogenRadials::radial_norm (const std::vector<double> rgrid, const std::vector<double> rvalue)
{
...
double
    HydrogenRadials::generate_hydrogen_radial_toconv (const double charge,
                                                      const bool with_slater_screening,
                                                      const int n,
                                                      const int l,
                                                      const double conv_thr,
    ...
    while ((std::fabs (delta_norm) > conv_thr))
        {
            ...
            std::vector<double> rvalue_segment = generate_hydrogen_radial_segment (charge,
...
std::vector<std::pair<int, int>>
    HydrogenRadials::unzip_strategy (const int nmax, const std::string strategy)
{
...
void
    HydrogenRadials::smooth (std::vector<double>& rgrid, std::vector<double>& rvalue, const double sigma)
{
...
std::map<std::pair<int, int>, std::pair<int, int>>
    HydrogenRadials::mapping_nl_lzeta (const int nmax, const std::string strategy)
{
...
void
    HydrogenRadials::hydrogen (const double charge,
                               const bool with_slater_screening,
                               const int nmax,
                               const double dr,
                               const double conv_thr,
...
double
    HydrogenRadials::slater_screening (const std::string symbol, const int n, const int l)
{
    atom_in atom_db;
