...
void
    TwoCenterTable::build (const RadialCollection& bra,
...
const double*
    TwoCenterTable::table (const int itype1,
                           const int l1,
                           const int izeta1,
                           const int itype2,
                           const int l2,
...
#endif
    return deriv
               ? dtable_.inner_most_ptr<double> (index_map_.get_value<int> (itype1, l1, izeta1, itype2, l2, izeta2, l))
               : table_.inner_most_ptr<double> (index_map_.get_value<int> (itype1, l1, izeta1, itype2, l2, izeta2, l));
}

void
    TwoCenterTable::lookup (const int itype1,
                            const int l1,
                            const int izeta1,
                            const int itype2,
                            const int l2,
...
int&
    TwoCenterTable::table_index (const NumericalRadial* it1, const NumericalRadial* it2, const int l)
{
    return index_map_
        .get_value<int> (it1->itype (), it1->l (), it1->izeta (), it2->itype (), it2->l (), it2->izeta (), l);
}

void
    TwoCenterTable::cleanup ()
{
    op_ = '\0';
    ntab_ = 0;
    nr_ = 0;
...
bool
    TwoCenterTable::is_present (const int itype1,
                                const int l1,
                                const int izeta1,
                                const int itype2,
                                const int l2,
                                ...
                                const int l) const
{
    ...
    return itype1 >= 0 && itype1 < index_map_.shape ().dim_size (0) && l1 >= 0 && l1 < index_map_.shape ().dim_size (1)
           ...
           && itype2 < index_map_.shape ().dim_size (3) && l2 >= 0 && l2 < index_map_.shape ().dim_size (4)
           && izeta2 >= 0 && izeta2 < index_map_.shape ().dim_size (5) && l >= 0
           && l <= index_map_.shape ().dim_size (6)
           && index_map_.get_value<int> (itype1, l1, izeta1, itype2, l2, izeta2, l) >= 0;
}

double
    TwoCenterTable::dfact (int l) const
{
    double result = 1.0;
    for (int i = l; i > 1; i -= 2)
        {
...
void
    TwoCenterTable::two_center_loop (const RadialCollection& bra, const RadialCollection& ket, looped_func f)
{
...
void
    TwoCenterTable::_indexing (const NumericalRadial* it1, const NumericalRadial* it2, const int l)
{
    table_index (it1, it2, l) = ntab_++;
}

void
    TwoCenterTable::_tabulate (const NumericalRadial* it1, const NumericalRadial* it2, const int l)
{
