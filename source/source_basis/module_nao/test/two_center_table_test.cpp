                                            ...
                                            for (int N2 = 0; N2 < orb (T2).nzeta (L2); N2++)
                                                {
                                                    for (int L = std::abs (L1 - L2); L <= (L1 + L2); L += 2)
                                                        {
                                                            const double* f
                                                                = S_tab.table (T1, L1, N1, T2, L2, N2, L, false);
                                                            const double* df
                                                                = S_tab.table (T1, L1, N1, T2, L2, N2, L, true);

                                                            for (int ir = 4; ir != S_tab.nr () - 4; ++ir)
                                                                {
                                                                    double df_fd
                                                                        = (-1.0 / 280 * (f[ir + 4] - f[ir - 4])
                                                                           + 4.0 / 105 * (f[ir + 3] - f[ir - 3])
                                                                           - 0.2 * (f[ir + 2] - f[ir - 2])
                                                                           + 0.8 * (f[ir + 1] - f[ir - 1]))
                                                                          / dr;

                                                                    // df is (d/dR)(S/R^l), it should be rescaled to
                                                                    // have the same unit as dS/dR in order to have
                                                                    // meaningful error comparison
                                                                    double err_abs = std::abs (df_fd - df[ir])
                                                                                     * std::pow (ir * dr, L);
                                                                    double err_rel
                                                                        = std::abs ((df_fd - df[ir]) / df[ir]);
                                                                    if (err_abs > tol_d_abs && err_rel > tol_d_rel)
                                                                        {
                                                                            printf (
                                                                                "T1 = %i   L1 = %i   N1 = %i   T2 = %i "
                                                                                "  L2 = %i   N2 = %i   L = %i   "
                                                                                "ir = %2i   df_fd = % 8.5e   df_tab = "
                                                                                "% 8.5e   err_abs = %8.5e   "
                                                                                "err_rel = %8.5e\n",
                                                                                T1,
                                                                                L1,
