                                            ...
                                            for (int mm3 = 0; mm3 <= 2 * l3; ++mm3)
                                                {
                                                    ...
                                                    EXPECT_NEAR (RealGauntTable::instance () (l1, l2, l3, m1, m2, m3),
                                                                 ogt.Gaunt_Coefficients (index1, index2, index3),
                                                                 tol);
                                                }
                                        }
                                }
...
TEST_F (RealGauntTableTest, SanityCheck)
{
    ...
    EXPECT_NEAR (RealGauntTable::instance () (2, 2, 2, 2, -1, -1),
                 -std::sqrt (15.0) / 7.0 * ModuleBase::SQRT_INVERSE_FOUR_PI,
                 tol);
    EXPECT_NEAR (RealGauntTable::instance () (2, 2, 2, -1, 2, -1),
                 -std::sqrt (15.0) / 7.0 * ModuleBase::SQRT_INVERSE_FOUR_PI,
                 tol);

    EXPECT_NEAR (RealGauntTable::instance () (3, 3, 2, 2, 1, 1),
                 ModuleBase::SQRT_INVERSE_FOUR_PI / std::sqrt (6.0),
                 tol);
    EXPECT_NEAR (RealGauntTable::instance () (2, 3, 3, 1, 1, 2),
                 ModuleBase::SQRT_INVERSE_FOUR_PI / std::sqrt (6.0),
                 tol);

    EXPECT_NEAR (RealGauntTable::instance () (4, 5, 7, 3, -2, -5),
                 ModuleBase::SQRT_INVERSE_FOUR_PI * std::sqrt (210.0) / 221.0,
                 tol);
}
...
TEST_F (RealGauntTableTest, Check3)
{
    ...
    for (int i = 0; i < len; i++)
        {
            infile >> ga_ref[i].l1 >> ga_ref[i].l2 >> ga_ref[i].l3 >> ga_ref[i].m1 >> ga_ref[i].m2 >> ga_ref[i].m3
                >> ga_ref[i].gaunt;
        }
                                    ...
                                    for (int m2 = -l2; m2 <= l2; m2++)
                                        {
                                            for (int m3 = 0; m3 <= l3; m3++)
                                                {
                                                    double gaunt
                                                        = RealGauntTable::instance ().gaunt (l1, l2, l3, m1, m2, m3);
                                                    double gaunt_symmetry[20];
                                                    double tmp;
                                                    int cnt_sym = 0;
                                                    if (gaunt > tol && cnt < len_gaunt)
                                                        {
                                                            ga_func[cnt].l1 = l1, ga_func[cnt].l2 = l2,
                                                            ga_func[cnt].l3 = l3;
                                                            ga_func[cnt].m1 = m1, ga_func[cnt].m2 = m2,
                                                            ga_func[cnt].m3 = m3;
                                                            ga_func[cnt++].gaunt = gaunt;

                                                            // Detects whether the values of functions omitted in the
                                                            // loop due to symmetry are equal
                                                            if (l3 <= 10)
                                                                {

                                                                    gaunt_symmetry[cnt_sym++]
                                                                        = RealGauntTable::instance ()
                                                                              .gaunt (l2, l1, l3, m2, m1, m3);
                                                                    gaunt_symmetry[cnt_sym++]
                                                                        = RealGauntTable::instance ()
                                                                              .gaunt (l2, l1, l3, -m2, -m1, -m3);
