...
void
    RadialCollection::build (const int ntype, Numerical_Nonlocal* const nls)
{
    cleanup ();
    ntype_ = ntype;
    radset_ = new RadialSet*[ntype_];
...
void
    RadialCollection::build (const RadialCollection* nls, const double radius)
{
    cleanup ();
...
void
    RadialCollection::build (const int nfile, const std::string* const file, const char ftype)
{
...
void
    RadialCollection::build (const int ntype,
...
void
    RadialCollection::build (const int lmax, const int nbes, const double rcut, const double sigma, const double dr)
{
    cleanup ();
    ntype_ = 1;
    radset_ = new RadialSet*[ntype_];
...
void
    RadialCollection::set_transformer (ModuleBase::SphericalBesselTransformer sbt, const int update)
{
    for (int itype = 0; itype < ntype_; ++itype)
        {
...
void
    RadialCollection::set_grid (const bool for_r_space, const int ngrid, const double* grid, const char mode)
{
    for (int itype = 0; itype < ntype_; ++itype)
        {
...
void
    RadialCollection::set_uniform_grid (const bool for_r_space,
                                        const int ngrid,
                                        const double cutoff,
                                        const char mode,
...
char
    RadialCollection::check_file_type (const std::string& file) const
{
...
void
    RadialCollection::to_file (const std::string& appendix, const std::string& format) const
{
    for (int itype = 0; itype < ntype_; ++itype)
        {
