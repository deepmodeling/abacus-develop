...
void
    PswfcRadials::build (const std::string& file,
                         const int itype,
...
bool
    PswfcRadials::startswith (std::string word, std::string pattern)
{
...
std::string
    PswfcRadials::steal_from_quotes (std::string word)
{
...
std::string
    PswfcRadials::steal_from_quotes (std::ifstream& ifs, std::string word)
{
    // concatenate all words until the second quote, no matter how many lines and spaces between
    std::string concatenated
        = word.substr (word.find_first_of ("\"") + 1, word.size () - word.find_first_of ("\"") - 1);
    int num_quote = 1;
    while (num_quote < 2)
        {
            std::string line;
...
std::string
    PswfcRadials::read_keyword_value (std::ifstream& ifs, std::string word)
{
...
double
    PswfcRadials::radial_norm (const std::vector<double> rgrid, const std::vector<double> rvalue)
{
...
double
    PswfcRadials::cut_to_convergence (const std::vector<double>& rgrid,
    ...
    while ((std::fabs (delta_norm) > conv_thr) && (ir_ <= ir_max_))
        {
            ...
            std::vector<double> rvalue_slice
                = std::vector<double> (rvalue.begin () + ir_min_, rvalue.begin () + ir_ + 1);
...
void
    PswfcRadials::smooth (std::vector<double>& rgrid, std::vector<double>& rvalue, const double sigma)
{
...
std::vector<double>
    PswfcRadials::pswfc_prepossess (std::map<std::pair<int, int>, std::vector<double>>& lzeta_rvalues,
                                    const double conv_thr)
{
    double nmax = 0.0;
...
void
    PswfcRadials::read_upf_pswfc (std::ifstream& ifs,
                                  const double screening_coeff,
                                  const double conv_thr,
                                  std::ofstream* ptr_log,
                                  const int rank)
    ...
    if (rank != 0)
        {
            ...
            // decomposed correlated container std::map<std::pair<int, int>, std::vector<double>> into three
            // std::vectors, additionally with rgrid the r values of pseudowavefunction
            ls.resize (nchi_);
            izetas.resize (nchi_);
            rgrid.resize (ngrid);
