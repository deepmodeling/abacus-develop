...
namespace ModulePW {
#ifdef __MPI
void PW_Basis::initmpi(const int poolnproc_in, const int poolrank_in, MPI_Comm pool_world_in) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        do {
            ...
            while (!done_factoring) {
                if (b % 2 == 0 &&
                    (!this->full_pw || this->full_pw_dim != 1)) // full_pw_dim = 1 means FFT dimension should be odd.
                {
                    n2++;
                    b /= 2;
                    continue;
...
void PW_Basis::initparameters(const bool gamma_only_in,
