...
namespace ModulePW {
PW_Basis::PW_Basis() { classname = "PW_Basis"; }

PW_Basis::PW_Basis(std::string device_, std::string precision_)
    : device(std::move(device_)), precision(std::move(precision_)) {
    classname = "PW_Basis";
...
void PW_Basis::collect_uniqgg() {
    ...
    int* sortindex = new int[this->npw];   // Reconstruct the mapping of the plane wave index ig according to the energy
                                           // size of the plane waves
    double* tmpgg = new double[this->npw]; // Ranking the plane waves by energy size while ensuring that the same energy
                                           // is preserved for each wave to correspond
...
void PW_Basis::set_device(std::string device_) { this->device = std::move(device_); }

void PW_Basis::set_precision(std::string precision_) { this->precision = std::move(precision_); }
