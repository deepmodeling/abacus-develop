...
{
    void
        operator() (const int npwk, const int* box_index, const std::complex<FPTYPE>* in, std::complex<FPTYPE>* out)
    {
...
struct set_recip_to_real_output_op<FPTYPE, base_device::DEVICE_CPU>
{
    ...
    void
        operator() (const int nrxx, const bool add, const FPTYPE factor, const std::complex<FPTYPE>* in, FPTYPE* out)
    {
        if (add)
            {
                for (int ir = 0; ir < nrxx; ++ir)
