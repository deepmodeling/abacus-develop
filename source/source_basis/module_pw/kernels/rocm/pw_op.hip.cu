...
template <class FPTYPE>
__global__ void
    set_recip_to_real_output (const int nrxx,
                              const bool add,
                              const FPTYPE factor,
                              const thrust::complex<FPTYPE>* in,
                              thrust::complex<FPTYPE>* out)
...
template <class FPTYPE>
__global__ void
    set_real_to_recip_output (const int npwk,
                              const int nxyz,
                              const bool add,
                              const FPTYPE factor,
                              const int* box_index,
...
void
    set_3d_fft_box_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const int npwk,
                                                                    const int* box_index,
                                                                    const std::complex<FPTYPE>* in,
                                                                    std::complex<FPTYPE>* out)
{
    const int block = (npwk + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
    hipLaunchKernelGGL (HIP_KERNEL_NAME (set_3d_fft_box<FPTYPE>),
                        dim3 (block),
...
template <typename FPTYPE>
void
    set_recip_to_real_output_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const int nrxx,
                                                                              const bool add,
                                                                              const FPTYPE factor,
                                                                              const std::complex<FPTYPE>* in,
                                                                              std::complex<FPTYPE>* out)
{
    ...
    hipLaunchKernelGGL (HIP_KERNEL_NAME (set_recip_to_real_output<FPTYPE>),
                        dim3 (block),
                        dim3 (THREADS_PER_BLOCK),
                        0,
                        0,
...
template <typename FPTYPE>
void
    set_real_to_recip_output_op<FPTYPE, base_device::DEVICE_GPU>::operator() (const int npwk,
                                                                              const int nxyz,
                                                                              const bool add,
                                                                              const FPTYPE factor,
                                                                              const int* box_index,
...
{
    const int block = (npwk + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
    hipLaunchKernelGGL (HIP_KERNEL_NAME (set_real_to_recip_output<FPTYPE>),
                        dim3 (block),
                        dim3 (THREADS_PER_BLOCK),
                        0,
                        0,
