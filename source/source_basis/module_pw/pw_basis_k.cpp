...
void
    PW_Basis_K::initparameters (const bool gamma_only_in,
            ...
            if (this->float_data_)
                {
                    ...
                    castmem_d2s_h2d_op () (this->s_kvec_c,
                                           reinterpret_cast<double*> (&this->kvec_c[0][0]),
                                           this->nks * 3);
                }
            ...
            if (this->float_data_)
                {
                    ...
                    castmem_d2s_h2h_op () (this->s_kvec_c,
                                           reinterpret_cast<double*> (&this->kvec_c[0][0]),
                                           this->nks * 3);
                }
    ...
    for (int ik = 0; ik < this->nks; ik++)
        {
            ...
            if (ng_global_k == 0)
                {
                    no_pw_message = "No plane waves are available for this k-point across the whole pool. Please "
                                    "increase ecutwfc or check KPT settings.";
                }
            ModuleBase::CHECK_WARNING_QUIT ((ng == 0), "pw_basis_k.cpp", PARAM.inp.calculation, no_pw_message);
            if (this->npwk_max < ng)
                {
                    this->npwk_max = ng;
                }
...
void
    PW_Basis_K::collect_local_pw (const double& erf_ecut_in, const double& erf_height_in, const double& erf_sigma_in)
{
                    ...
                    if (erf_height > 0)
                        {
                            this->gk2[ik * npwk_max + igl]
                                = temp_gk2
                                  + erf_height / tpiba2 * (1.0 + std::erf ((temp_gk2 * tpiba2 - erf_ecut) / erf_sigma));
                        }
                    else
                        {
...
ModuleBase::Vector3<double>
    PW_Basis_K::cal_GplusK_cartesian (const int ik, const int ig) const
{
...
double&
    PW_Basis_K::getgk2 (const int ik, const int igl) const
{
    return this->gk2[ik * this->npwk_max + igl];
}

ModuleBase::Vector3<double>&
    PW_Basis_K::getgcar (const int ik, const int igl) const
{
    return this->gcar[ik * this->npwk_max + igl];
}

ModuleBase::Vector3<double>
    PW_Basis_K::getgdirect (const int ik, const int igl) const
{
...
ModuleBase::Vector3<double>
    PW_Basis_K::getgpluskcar (const int ik, const int igl) const
{
    return this->gcar[ik * this->npwk_max + igl] + this->kvec_c[ik];
}
int&
    PW_Basis_K::getigl2isz (const int ik, const int igl) const
{
    return this->igl2isz_k[ik * this->npwk_max + igl];
}
int&
    PW_Basis_K::getigl2ig (const int ik, const int igl) const
{
    return this->igl2ig_k[ik * this->npwk_max + igl];
}

void
    PW_Basis_K::get_ig2ixyz_k ()
{
...
std::vector<int>
    PW_Basis_K::get_ig2ix (const int ik) const
{
...
std::vector<int>
    PW_Basis_K::get_ig2iy (const int ik) const
{
...
std::vector<int>
    PW_Basis_K::get_ig2iz (const int ik) const
{
...
template <>
float*
    PW_Basis_K::get_kvec_c_data () const
{
...
template <>
float*
    PW_Basis_K::get_gcar_data () const
{
    return this->s_gcar;
}
template <>
