...
void PW_Basis_K::setupIndGk() {
    ...
    for (int ik = 0; ik < this->nks; ik++) {
        ...
        if (ng_global_k == 0) {
            no_pw_message = "No plane waves are available for this k-point across the whole pool. Please increase "
                            "ecutwfc or check KPT settings.";
        }
...
double& PW_Basis_K::getgk2(const int ik, const int igl) const { return this->gk2[ik * this->npwk_max + igl]; }
...
int& PW_Basis_K::getigl2isz(const int ik, const int igl) const { return this->igl2isz_k[ik * this->npwk_max + igl]; }
int& PW_Basis_K::getigl2ig(const int ik, const int igl) const { return this->igl2ig_k[ik * this->npwk_max + igl]; }
