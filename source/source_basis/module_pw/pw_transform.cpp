...
#pragma omp parallel for schedule(static)
    ...
    if (add)
    {
        FPTYPE tmpfac = factor / FPTYPE(nxyz);
#ifdef _OPENMP
#pragma omp parallel for schedule(static)
#endif
        for (int ig = 0; ig < npw; ++ig)
        {
            out[ig] += tmpfac * this->fft_bundle.get_auxg_data<FPTYPE>()[ig2isz[ig]];
        }
    }
    else
    {
        FPTYPE tmpfac = 1.0 / FPTYPE(nxyz);
#ifdef _OPENMP
#pragma omp parallel for schedule(static)
#endif
        for (int ig = 0; ig < npw; ++ig)
...
#pragma omp parallel for schedule(static)
    ...
    for (int ig = 0; ig < npw; ++ig)
    {
        this->fft_bundle.get_auxg_data<FPTYPE>()[ig2isz[ig]] = in[ig];
    }
