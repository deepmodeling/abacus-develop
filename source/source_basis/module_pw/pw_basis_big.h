...
class PW_Basis_Big : public PW_Basis_Sup
{
  public:
    ...
    ~PW_Basis_Big () {};
    void
        setbxyz (const int bx_in, const int by_in, const int bz_in)
    {
        bx = bx_in;
        by = by_in;
        bz = bz_in;
    ...
    void
        autoset_big_cell_size (int& b_size, const int& nc_size, const int nproc = 0)
    {
    ...
    virtual void
        initgrids (const double lat0_in, const ModuleBase::Matrix3 latvec_in, const double gridecut)
    {
        // init lattice
        this->lat0 = lat0_in;
                ...
                do
                    {
                        ...
                        if ((this->full_pw && this->full_pw_dim == 2) && b % 2 != 0)
                            done_factoring = true; // full_pw_dim = 2 means FFT dimensions should be even.
                        while (!done_factoring)
                            {
                                if (b % 2 == 0
                                    && (!this->full_pw
                                        || this->full_pw_dim
                                               != 1)) // full_pw_dim = 1 means FFT dimension should be odd.
                                    {
                                        n2++;
                                        b /= 2;
                                        continue;
        ...
        if (ibox[0] % this->bx != 0)
            ibox[0] += (this->bx - ibox[0] % this->bx);
        if (ibox[1] % this->by != 0)
            ibox[1] += (this->by - ibox[1] % this->by);
        if (ibox[2] % this->bz != 0)
            ibox[2] += (this->bz - ibox[2] % this->bz);
    ...
    virtual void
        initgrids (const double lat0_in,
                   ...
                   int nz_in)
    {
        ...
        this->bxyz = this->bx * this->by * this->bz;
        if (this->nx % this->bx != 0)
            this->nx += (this->bx - this->nx % this->bx);
        if (this->ny % this->by != 0)
            this->ny += (this->by - this->ny % this->by);
        if (this->nz % this->bz != 0)
            this->nz += (this->bz - this->nz % this->bz);
        this->nbx = this->nx / bx;
        this->nby = this->ny / by;
        this->nbz = this->nz / bz;
        this->nxy = this->nx * this->ny;
                ...
                for (int igy = -ibox[1]; igy <= ibox[1]; ++igy)
                    {
                        for (int igx = -ibox[0]; igx <= ibox[0]; ++igx)
                            {
                                ++count;
                                if (count % this->poolnproc != this->poolrank)
                                    continue;
                                if (std::abs (igx) <= ibox[0] - 1 && std::abs (igy) <= ibox[1] - 1
                                    && std::abs (igz) <= ibox[2] - 1)
                                    continue;
                                ModuleBase::Vector3<double> f;
                                f.x = igx;
                                f.y = igy;
