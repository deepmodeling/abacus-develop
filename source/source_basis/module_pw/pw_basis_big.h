    ...
    void autoset_big_cell_size(int& b_size, const int& nc_size, const int nproc = 0) {
        ...
        if (nproc > 0) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (nc_size % tmp != 0) { // ignore candidates which can't be factored by nc_size
                    max_bz[i] = 0;
                    continue;
                }
                if (max_bz[i] % nproc == 0) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            do {
                ...
                while (!done_factoring) {
                    if (b % 2 == 0 && (!this->full_pw ||
                                       this->full_pw_dim != 1)) // full_pw_dim = 1 means FFT dimension should be odd.
                    {
                        n2++;
                        b /= 2;
                        continue;
            ...
            for (int igy = -ibox[1]; igy <= ibox[1]; ++igy) {
                for (int igx = -ibox[0]; igx <= ibox[0]; ++igx) {
                    ...
                    if (count % this->poolnproc != this->poolrank)
                        continue;
                    if (std::abs(igx) <= ibox[0] - 1 && std::abs(igy) <= ibox[1] - 1 && std::abs(igz) <= ibox[2] - 1)
                        continue;
                    ModuleBase::Vector3<double> f;
                    f.x = igx;
                    f.y = igy;
