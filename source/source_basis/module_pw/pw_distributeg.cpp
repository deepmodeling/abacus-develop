...
void
    PW_Basis::count_pw_st (int* st_length2D, // the number of planewaves that belong to the stick located on (x, y).
                            ...
                            if (modulus <= this->ggecut || this->full_pw)
                                {
                                    if (length == 0)
                                        {
                                            st_bottom2D[index]
                                                = iz; // length == 0 means this point is the bottom of stick (x, y).
                                        }
                                    ++this->npwtot;
                                    ++length;
                                    if (iy < this->riy)
