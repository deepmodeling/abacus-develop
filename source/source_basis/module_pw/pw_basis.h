...
class PW_Basis
{
  ...
  public:
    ...
    // Init mpi parameters
#ifdef __MPI
    void initmpi (const int poolnproc_in, // Number of processors in this pool
    ...
    virtual void initgrids (const double lat0_in,                // unit length (unit in bohr)
    ...
    // Init the grids for FFT
    virtual void initgrids (const double lat0_in,
  ...
  public:
    ...
    int *is2fftixy = nullptr,
        *d_is2fftixy = nullptr; // is2fftixy[is]: iy + ix * ny of is^th stick among sticks on current proc.
    int* fftixy2ip
        = nullptr;          // fftixy2ip[iy + ix * fftny]: ip of proc which contains stick on (ix, iy). if no stick: -1
  ...
  protected:
    int* startnsz_per
        = nullptr; // useless intermediate variable// startnsz_per[ip]: starting is * nz stick in the ip^th proc.
  ...
  public:
    ...
    bool full_pw
        = false;         ///< If set to 1, ecut will be ignored while collecting planewaves, so that all planewaves will
  ...
  protected:
    ...
    void divide_sticks_1 (int* st_i,     // x or x + fftnx (if x < 0) of stick.
    ...
    void count_pw_st (int* st_length2D, // the number of planewaves that belong to the stick located on (x, y).
                      int* st_bottom2D  // the z-coordinate of the bottom of stick on (x, y).
    );

    // get ig2isz and is2fftixy
    void get_ig2isz_is2fftixy (int* st_bottom, // minimum z of stick, stored in 1d array with tot_nst elements.
    ...
    void collect_st (int* st_length2D, // the number of planewaves that belong to the stick located on (x, y), stored in
                                       // 2d x-y plane.
    ...
    // get istot2ixy
    void get_istot2ixy (int* st_i, // x or x + fftnx (if x < 0) of stick.
                        int* st_j  // y or y + fftny (if y < 0) of stick.
    );
  ...
  public:
     ...
     *
     * @tparam FPTYPE The type of the input data, which can only be a compelx type (e.g., std::complex<float>,
     * std::complex<double>)
    ...
    template <typename TK,
              ...
              typename std::enable_if<!std::is_same<TK, typename GetTypeReal<TK>::type>::value
                                      ...
                                      int>::type = 0>
    void
        recip_to_real (TK* in, TR* out, const bool add = false, const typename GetTypeReal<TK>::type factor = 1.0) const
    {
    ...
    template <typename TR,
              ...
              typename std::enable_if<!std::is_same<TK, typename GetTypeReal<TK>::type>::value
                                      ...
                                      int>::type = 0>
    void
        real_to_recip (TR* in, TK* out, const bool add = false, const typename GetTypeReal<TK>::type factor = 1.0) const
    {
