...
{
    cout << "dividemthd 2, gamma_only: off, full_pw: true, full_pw_dim: 2, check gcar,gdirect,gg,istot2ixy,ig2isz"
         << endl;
    //--------------------------------------------------
                            ...
                            for (int ix = 0; ix < pwtest.nx; ++ix)
                                {
                                    ...
                                    f.y = iy;
                                    f.z = iz;
                                    if (iz >= int (pwtest.nz / 2) + 1)
                                        {
                                            f.z -= pwtest.nz;
                                        }
                                    if (iy >= int (pwtest.ny / 2) + 1)
                                        {
                                            f.y -= pwtest.ny;
                                        }
                                    if (ix >= int (pwtest.nx / 2) + 1)
                                        {
                                            f.x -= pwtest.nx;
                                        }
                                    ...
                                    if (modulus <= ggecut)
                                        {
                                            EXPECT_EQ (tmpx[iz + iy * pwtest.nz + ix * pwtest.ny * pwtest.nz],
                                                       int (f.x));
                                            EXPECT_EQ (tmpy[iz + iy * pwtest.nz + ix * pwtest.ny * pwtest.nz],
                                                       int (f.y));
                                            EXPECT_EQ (tmpz[iz + iy * pwtest.nz + ix * pwtest.ny * pwtest.nz],
                                                       int (f.z));
                                        }
                                }
                        }
    ...
    if (pwtest.ig_gge0 >= 0)
        {
            EXPECT_NEAR (0.0, pwtest.gg[pwtest.ig_gge0], 1e-8);
        }
    delete[] startnst;
    delete[] tmpx;
    delete[] tmpy;
    ...
    ModuleBase::Matrix3
        latvec2 (5.1358423233, 0.0, 0.0, 0.1578526541, 5.1334159104, 0.0, -2.646847675, -2.5667081359, 3.5753437737);
    gamma_only = false;
    wfcecut = 240;
    lat0 = 1.88972613;
    distribution_type = 1;
