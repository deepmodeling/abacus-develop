    ...
    for (int ik = 0; ik < nks; ++ik) {
                    ...
                    for (int iz = 0; iz < nz; ++iz) {
                        ...
                        if (modulusgk <= ggecut) {
                            tmp[ix * ny * nz + iy * nz + iz] =
                                1.0 / (modulusgk + 1) + ModuleBase::IMAG_UNIT / (std::abs(v.x + 1) + 1);
                        }
                    }
                }
            }
        ...
        for (int ig = 0; ig < npwk; ++ig) {
            rhog[ig] = 1.0 / (pwtest.getgk2(ik, ig) + 1) +
                       ModuleBase::IMAG_UNIT / (std::abs(pwtest.getgdirect(ik, ig).x + 1) + 1);
        ...
        for (int igl = 0; igl < npwk; ++igl) {
            ...
            EXPECT_NEAR(pwtest.getgpluskcar(ik, igl).norm2(),
                        ((pwtest.getgdirect(ik, igl) + kvec_d[ik]) * G).norm2(),
                        1e-8);
        }
