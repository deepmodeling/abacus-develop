...
{
    cout << "dividemthd 1, gamma_only: on, xprime: true, full_pw: true, full_pw_dim: 0, check "
            "gcar,gdirect,gg,istot2ixy,ig2isz"
         << endl;
    //--------------------------------------------------
                            ...
                            for (int ix = 0; ix < pwtest.fftnx; ++ix)
                                {
                                    ...
                                    if (modulus <= ggecut)
                                        {
                                            EXPECT_EQ (tmpx[iz + iy * pwtest.fftnz + ix * pwtest.fftny * pwtest.fftnz],
                                                       int (f.x));
                                            EXPECT_EQ (tmpy[iz + iy * pwtest.fftnz + ix * pwtest.fftny * pwtest.fftnz],
                                                       int (f.y));
                                            EXPECT_EQ (tmpz[iz + iy * pwtest.fftnz + ix * pwtest.fftny * pwtest.fftnz],
                                                       int (f.z));
                                        }
                                }
                        }
