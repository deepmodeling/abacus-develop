...
{
    cout << "dividemthd 1, gamma_only: off, xprime: true, full_pw: true, full_pw_dim: 2, check fft between complex and "
            "complex, reset ggecut to latecut"
         << endl;
    ...
    pwtest.initparameters (gamma_only,
                           2 * wfcecut,
                           distribution_type,
                           xprime); // pwtest will reset 2*wfcecut to wfcecut
    ...
    if (rank_in_pool == 0)
        {
                            ...
                            for (int iz = 0; iz < nz; ++iz)
                                {
                                    ...
                                    if (modulus <= ggecut || full_pw)
                                        {
                                            tmp[ix * ny * nz + iy * nz + iz]
                                                = 1.0 / (modulus + 1)
                                                  + ModuleBase::IMAG_UNIT / (std::abs (v.x + 1) + 1);
                                        }
                                }
                        }
                }
            ...
            ModuleBase::Vector3<double> delta_g ((double (int (nx / 2)) - 1.) / nx,
                                                 (double (int (ny / 2)) - 1.) / ny,
                                                 (double (int (nz / 2)) - 1.) / nz);
