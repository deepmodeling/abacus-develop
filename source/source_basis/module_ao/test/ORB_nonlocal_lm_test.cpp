...
void
    NumericalNonlocalLmTest::TearDown ()
{
    delete[] r_radial_;
    delete[] rab_;
...
std::string
    NumericalNonlocalLmTest::trim (std::string const& str)
{
...
double
    NumericalNonlocalLmTest::err_r2k2r (Numerical_Nonlocal_Lm& nnl_tmp)
{
...
size_t
    NumericalNonlocalLmTest::calc_nk (double const& ecutwfc, double const& dk)
{
...
void
    NumericalNonlocalLmTest::change_k (Numerical_Nonlocal_Lm& nnl_, double const& ecut, double const& dk)
{
...
bool
    NumericalNonlocalLmTest::check_file_match (size_t const& nline,
                                               double const* col1,
                                               double const* col2,
                                               double const& tol,
                                               std::string const& fname)
    ...
    for (size_t i: {0, 2})
        {
            ...
            EXPECT_EQ (true, this->check_file_match (nnl[i].nr, nnl[i].r_radial, nnl[i].beta_r, tol, betar_fname));
            EXPECT_EQ (true, this->check_file_match (nnl[i].nk, nnl[i].k_radial, nnl[i].beta_k, tol, betak_fname));
            ...
            EXPECT_EQ (
                true,
                this->check_file_match (nnl[i].nr_uniform, r_uniform_mesh, nnl[i].beta_uniform, tol, betaru_fname));
