...
size_t
    NumericalOrbitalLmTest::calc_nk (double const& ecutwfc, double const& dk)
{
...
size_t
    NumericalOrbitalLmTest::calc_nr_uniform (double const& rcut, double const& dr_uniform)
{
    return static_cast<int> (rcut / dr_uniform) + 10;
}

void
    NumericalOrbitalLmTest::SetUp ()
{
...
void
    NumericalOrbitalLmTest::TearDown ()
{
    delete[] r_radial_;
    delete[] rab_;
...
void
    NumericalOrbitalLmTest::init_with_different_k (double const& ecut, double const& dk)
{
...
bool
    NumericalOrbitalLmTest::check_file_match (size_t const& nline,
                                              double const* col1,
                                              double const* col2,
                                              double const& tol,
                                              std::string const& fname)
    ...
    for (size_t i = 0; i != nline; ++i)
        {
            ...
            if (std::abs (col1[i] - std::stod (tmp1.c_str ())) > tol
                || std::abs (col2[i] - std::stod (tmp2.c_str ())) > tol || ss.tellg () != -1)
                {
                    return false;
                }
        }
    ...
    for (size_t i = 0; i != nolm_.size (); ++i)
        {
            ...
            auto get_fname = [&] (std::string const& suffix) -> std::string
                {
                    return dir + "/O-" + orb[i] + std::to_string (nolm_[i].index_chi + 1) + "-orbital-" + suffix
                           + ".dat";
                };
            ...
            EXPECT_TRUE (
                this->check_file_match (nolm_[i].nr, nolm_[i].getRadial (), nolm_[i].getPsi (), tol, psi_fname));
            EXPECT_TRUE (
                this->check_file_match (nolm_[i].nk, nolm_[i].getKpoint (), nolm_[i].getPsi_k (), tol, psik_fname));
            ...
            EXPECT_TRUE (
                this->check_file_match (nolm_[i].nr_uniform, ru_mesh, nolm_[i].getPsiuniform (), tol, psiru_fname));
            EXPECT_TRUE (
                this->check_file_match (nolm_[i].nr_uniform, ru_mesh, nolm_[i].getDpsiuniform (), tol, psidru_fname));
