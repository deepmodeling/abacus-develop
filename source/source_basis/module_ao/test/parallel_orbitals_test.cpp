                    ...
                    for (auto mode: {0, 1})
                        {
                            ...
                            auto cal_lsize
                                = [] (const int& gsize, const int& nb, const int& np, const int& pcoord) -> int
                                {
                                    int nblock = gsize / nb;
                                    return nblock / np * nb
                                           + static_cast<int> (nblock % np > pcoord) * nb // full blocks' contribution
                                           + static_cast<int> (nblock % np == pcoord)
                                                 * (gsize % nb); // the last block's contribution
                                };
                            ...
                            for (int i = 0; i < lr; ++i)
                                for (int j = 0; j < lc; ++j)
                                    EXPECT_TRUE (
                                        po.in_this_processor (po.local2global_row (i), po.local2global_col (j)));
                                            ...
                                            for (int j = 0; j < atomic_row_array.size (); ++j)
                                                {
                                                    ...
                                                    EXPECT_EQ (atomic_row_array[j] + iat2iwt[i],
                                                               global_row_array[local_index_trace_row]);
                                                    // check local_index == local_index
                                                    EXPECT_EQ (local_index_trace_row,
                                                               po.global2local_row (atomic_row_array[j] + iat2iwt[i]));
                                                    local_index_trace_row++;
                                                }
                                            for (int j = 0; j < atomic_col_array.size (); ++j)
                                                {
                                                    // check global_index == global_index
                                                    EXPECT_EQ (atomic_col_array[j] + iat2iwt[i],
                                                               global_col_array[local_index_trace_col]);
                                                    // check local_index == local_index
                                                    EXPECT_EQ (local_index_trace_col,
                                                               po.global2local_col (atomic_col_array[j] + iat2iwt[i]));
