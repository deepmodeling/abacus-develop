...
void
    Numerical_Orbital_Lm::set_orbital_info (const std::string& label_in,
            ...
            if (flag_sbpool)
            ...
            else
                {
                    throw std::domain_error (
                        "flag_sbpool false not finished in Numerical_Orbital_Lm::set_orbital_info_k. "
...
void
    Numerical_Orbital_Lm::copy_parameter (const std::string& label_in,
                                          const int& index_atom_type_in,
                                          const int& angular_momentum_l_in,
                                          const int& index_chi_in,
                                          const int& nr_in,
...
void
    Numerical_Orbital_Lm::extra_uniform (const double& dr_uniform_in, const bool& force_flag)
{
...
#pragma omp parallel for schedule(static)
    ...
    switch (this->angular_momentum_l) // added by pengfei 13-8-8 different l has different  boundary conditions
        ...
        default:
            ...
            // std::endl; GlobalV::ofs_warning << " Check file " << __FILE__ << " line " << __LINE__ <<std::endl;
...
// use Sbt_new
void
    Numerical_Orbital_Lm::cal_kradial ()
{
    assert (this->nr > 0);
...
void
    Numerical_Orbital_Lm::cal_kradial_sbpool ()
{
    assert (this->nr > 0);
    assert (this->nr_uniform > 0);
...
void
    Numerical_Orbital_Lm::cal_rradial_sbpool ()
{
    // dr must be all the same for Sph_Bessel_Recursive_Pool
    const double dr = this->rab[0];
...
void
    Numerical_Orbital_Lm::plot () const
{
    ...
    if (GlobalV::MY_RANK == 0)
        {
            ...
            ssr << PARAM.globalv.global_out_dir << this->label << "/" << this->label << "-" << orbital_type
                << index_chi + 1 << "-orbital-r.dat";

            ssk << PARAM.globalv.global_out_dir << this->label << "/" << this->label << "-" << orbital_type
                << index_chi + 1 << "-orbital-k.dat";
            ...
            for (int i = 0; i < this->nr_uniform; i++)
                {
                    ofsdru << this->dr_uniform * i << " " << dpsi_uniform[i]
                           << std::endl; // output dphi/dr 2013-08-10  pengfei
                }
            ofsr.close ();
            ofsk.close ();
            ofsru.close ();
