...
Numerical_Nonlocal_Lm::~Numerical_Nonlocal_Lm () { this->freemem (); }

void
    Numerical_Nonlocal_Lm::renew ()
{
    assert (nr_uniform > 0);
    assert (nr > 0);
    assert (nk > 0);
...
void
    Numerical_Nonlocal_Lm::set_NL_proj (const std::string& label_in,
...
void
    Numerical_Nonlocal_Lm::get_kradial ()
{
    ...
    for (int ik = 0; ik < nk; ik++)
        {
            ...
            ModuleBase::Integral::Simpson_Integral (this->nr, integrated_func, this->rab, this->beta_k[ik]);
...
void
    Numerical_Nonlocal_Lm::plot (const int& my_rank) const
{
    ...
    switch (this->angular_momentum_l)
        ...
        case 4:
            ...
            break;
        default:
            ModuleBase::WARNING_QUIT ("Numerical_Orbital_Lm::plot", "Please check in functoin.");
        }
