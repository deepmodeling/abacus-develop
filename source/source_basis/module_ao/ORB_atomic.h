...
class Numerical_Orbital_AtomRelation
{
  public:
        ...
        get_distance ()
    {
        if (distance < 0.0)
            {
                ModuleBase::WARNING_QUIT ("NUMERICAL_ORBITAL", "distance should be above zero!");
            }
        return distance;
    }
    ...
    void
        set_position (const ModuleBase::Vector3<double>& R1_in, const ModuleBase::Vector3<double>& R2_in)
    {
        R1 = R1_in;
        R2 = R2_in;
        dR = R1 - R2;
...
class Numerical_Orbital
{
  ...
  public:
    ...
    const int&
        getTotal_nchi () const
    {
        return this->total_nchi;
    }
    const int&
        getNchi (const int l) const
    {
        return this->nchi[l];
    }
    const std::string&
    ...
    const inline Numerical_Orbital_Lm&
        PhiLN (const int& L, const int& N) const
    {
    ...
    static ModuleBase::Vector3<double>&
        getR1 ()
    {
        return NOAR.getR1 ();
    }
    ...
    ///
    void set_orbital_info (const int& type_in,
    ...
    std::vector<Numerical_Orbital_Lm>&
        chi ()
    {
        return this->phiLN;
    }
