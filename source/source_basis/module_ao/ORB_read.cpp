...
void
    LCAO_Orbitals::init (std::ofstream& ofs_in,
                         const int& ntype,
...
void
    LCAO_Orbitals::Read_Orbitals (std::ofstream& ofs_in,
                                  const int& ntype_in,
                                  const int& lmax_in,
                                  const bool& deepks_setorb,
                                  const int& out_mat_r,
                                  ...
                                  const int& my_rank)     // mohan add 2021-04-26
{
    ...
    if (!read_in_flag)
        {
            ModuleBase::WARNING_QUIT ("LCAO_Orbitals::Read_Orbitals",
                                      "Set the NUMERICAL_ORBITAL block in structure file.");
        }
...
//-------------------------------------------------------
void
    LCAO_Orbitals::Read_PAO (std::ofstream& ofs_in,
                             const int& it,
                             const bool& force_flag, // mohan add 2021-05-07
                             const int& my_rank)     // mohan add 2021-04-26
{
    ...
    ofs_in << " " << std::setw (12) << "ORBITAL" << std::setw (3) << "L" << std::setw (3) << "N" << std::setw (8)
           << "nr" << std::setw (8) << "dr" << std::setw (8) << "RCUT" << std::setw (12) << "CHECK_UNIT"
...
// caoyu add 2021-3-16
void
    LCAO_Orbitals::Read_Descriptor (std::ofstream& ofs_in,
                                    const bool& force_flag, // mohan add 2021-05-07
                                    const int& my_rank)     // read descriptor basis
{
    ...
    std::ifstream in_de;
    ofs_in << " " << std::setw (12) << "DESCRIPTOR" << std::setw (3) << "L" << std::setw (3) << "N" << std::setw (8)
           << "nr" << std::setw (8) << "dr" << std::setw (8) << "RCUT" << std::setw (12) << "CHECK_UNIT"
           << std::setw (12) << "NEW_UNIT" << std::endl;
...
void
    LCAO_Orbitals::read_orb_file (std::ofstream& ofs_in, // GlobalV::ofs_running
                                  std::ifstream& ifs,
                                  const int& it,
                                  int& lmax,
                                  int& nchimax,
                            ...
                            while (!find)
                                {
                                    if (ifs.eof ())
                                        {
                                            std::cout << " Can't find l=" << L << " n=" << N << " orbital."
                                                      << std::endl;
                                            break;
                                        }
