...
void
    BFGS::relax_step (const ModuleBase::matrix& _force, UnitCell& ucell)
{
    if (!is_initialized)
        {
            allocate (ucell.nat);
...
void
    BFGS::GetPos (UnitCell& ucell, std::vector<ModuleBase::Vector3<double>>& pos)
{
...
void
    BFGS::GetPostaud (UnitCell& ucell, std::vector<ModuleBase::Vector3<double>>& pos_taud)
{
...
void
    BFGS::PrepareStep (std::vector<ModuleBase::Vector3<double>>& force,
...
void
    BFGS::Update (std::vector<double>& pos,
...
void
    BFGS::DetermineStep (std::vector<double>& steplength,
                         std::vector<ModuleBase::Vector3<double>>& dpos,
                         double& maxstep)
{
...
void
    BFGS::CalculateLargestGrad (const ModuleBase::matrix& _force, UnitCell& ucell)
{
    ...
    if (PARAM.inp.out_level == "ie")
        {
            std::cout << " LARGEST GRAD (eV/Angstrom)  : "
                      << largest_grad * ModuleBase::Ry_to_eV / ModuleBase::BOHR_TO_A << std::endl;
        }
}
...
void
    BFGS::IsRestrain ()
{
    Ions_Move_Basic::converged = largest_grad * ModuleBase::Ry_to_eV / ModuleBase::BOHR_TO_A < PARAM.inp.force_thr_ev;
}
