...
void
    LBFGS::allocate (const int _size) // initialize H0、H、pos0、force0、force
{
...
void
    LBFGS::relax_step (const ModuleBase::matrix _force, UnitCell& ucell, const double& etot)
...
void
    LBFGS::get_pos (UnitCell& ucell, std::vector<ModuleBase::Vector3<double>>& pos)
{
    int k = 0;
    for (int i = 0; i < ucell.ntype; i++)
        {
...
void
    LBFGS::get_pos_taud (UnitCell& ucell, std::vector<ModuleBase::Vector3<double>>& pos_taud)
{
    int k = 0;
    for (int i = 0; i < ucell.ntype; i++)
        {
...
void
    LBFGS::prepare_step (std::vector<ModuleBase::Vector3<double>>& force,
...
void
    LBFGS::update (std::vector<ModuleBase::Vector3<double>>& pos_taud,
...
void
    LBFGS::determine_step (std::vector<double>& steplength,
                           std::vector<ModuleBase::Vector3<double>>& dpos,
                           double& maxstep)
{
...
void
    LBFGS::update_pos (UnitCell& ucell)
{
    double a[3 * size];
    for (int i = 0; i < size; i++)
        {
...
void
    LBFGS::is_restrain ()
{
    Ions_Move_Basic::converged
        = Ions_Move_Basic::largest_grad * ModuleBase::Ry_to_eV / ModuleBase::BOHR_TO_A < PARAM.inp.force_thr_ev;
}

void
    LBFGS::calculate_largest_grad (const ModuleBase::matrix& _force, UnitCell& ucell)
{
