...
void
    BFGS_Basic::update_inverse_hessian (const double& lat0)
{
...
void
    BFGS_Basic::new_step (const double& lat0)
{
    ...
    if (Ions_Move_Basic::update_iter == 1)
        {
            ...
            relax_bfgs_init = std::min (Ions_Move_Basic::best_xxx,
                                        relax_bfgs_init); // cg to bfgs initial trust_radius   13-8-10 pengfei
        }
    ...
    if (bfgs_ndim == 1)
        {
            ...
            GlobalV::ofs_running << " check the norm of new move " << dot_func (move, move, dim) << " (Bohr)"
                                 << std::endl;
        }
    else if (bfgs_ndim > 1)
        {
...
double
    BFGS_Basic::check_move (const double& lat0, const double& pos, const double& pos_p)
{
