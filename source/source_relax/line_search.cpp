...
bool
    Line_Search::line_search (const bool restart,
...
bool
    Line_Search::first_order (const double x, const double y, const double f, double& xnew)
{
...
bool
    Line_Search::third_order (const double x, const double y, const double f, double& xnew, const double conv_thr)
{
    double dmove = 0.0;
    double dmoveh = 0.0;
    double dmove1 = 0.0;
...
void
    Line_Search::init_brent (const double x, const double y, const double f)
{
    bracked = true;
...
void
    Line_Search::update_brent (const double x, const double y, const double f)
{
    xb = x;
    yb = y;
    fb = f;
...
bool
    Line_Search::brent (const double x, const double y, const double f, double& xnew, const double conv_thr)
{
    ls_step++;
    ...
    if (!bracked)
        {
            ...
            if (ls_step == 4) // I'm not sure if this is a good choice, but the idea is there should not be so many line
                              // search steps I feel if the line search does not converge, we'd better change the
                              // direction and restart line search
                {
