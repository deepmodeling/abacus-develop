...
void
    Ions_Move_Basic::setup_gradient (const UnitCell& ucell, const ModuleBase::matrix& force, double* pos, double* grad)
{
...
void
    Ions_Move_Basic::move_atoms (UnitCell& ucell, double* move, double* pos)
{
                    ...
                    for (int ia = 0; ia < ucell.atoms[it].na; ia++)
                        {
                            ...
                            GlobalV::ofs_running << " " << std::setw (12) << ss.str ().c_str () << std::setw (15)
                                                 << move[3 * iat + 0] << std::setw (15) << move[3 * iat + 1]
...
void
    Ions_Move_Basic::check_converged (const UnitCell& ucell, const double* grad)
{
    ...
    if (PARAM.inp.test_relax_method)
        {
            ...
            ModuleBase::GlobalFunc::OUT (GlobalV::ofs_running,
                                         "largest gradient (ry/bohr)",
                                         Ions_Move_Basic::largest_grad);
        }
    ...
    if (PARAM.inp.out_level == "ie")
        {
            ...
            std::cout << " LARGEST GRAD (eV/Angstrom)  : "
                      << Ions_Move_Basic::largest_grad * ModuleBase::Ry_to_eV / ModuleBase::BOHR_TO_A << std::endl;

            GlobalV::ofs_running << "\n Largest force is "
                                 << largest_grad * ModuleBase::Ry_to_eV / ModuleBase::BOHR_TO_A
                                 << " eV/Angstrom while threshold is " << PARAM.inp.force_thr_ev << " eV/Angstrom"
                                 << std::endl;
        }
    ...
    // mohan update 2011-04-21
    ...
    else
        {
            GlobalV::ofs_running << "\n Ion relaxation is not converged yet (threshold is "
                                 << PARAM.inp.force_thr * ModuleBase::Ry_to_eV / ModuleBase::BOHR_TO_A << ")"
                                 << std::endl;
...
void
    Ions_Move_Basic::terminate (const UnitCell& ucell)
{
...
void
    Ions_Move_Basic::setup_etot (const double& energy_in, const bool judgement)
{
...
double
    Ions_Move_Basic::dot_func (const double* a, const double* b, const int& dim_in)
{
    double result = 0.0;
