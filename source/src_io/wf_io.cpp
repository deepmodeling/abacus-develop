...
void WF_io::write_wfc(const std::string &fn,
    ...
    for (int ip = 0; ip < GlobalV::KPAR; ip++)
    {
        ...
        int id = 0;
                        ...
                        if (INPUT.out_wfc_pw == 1)
                        {
                            ...
                            if (id == 0)
                            {
                                ...
                                ofs2 << std::setw(10) << ikstot + 1 << std::setw(10) << nkstot << std::setw(10)
                                     << p_kv->kvec_c[ik].x << std::setw(10) << p_kv->kvec_c[ik].y << std::setw(10)
                                     << p_kv->kvec_c[ik].z << std::setw(10) << p_kv->wk[ik] << std::setw(10) << ikngtot
                                     << std::setw(10) << GlobalV::NBANDS << std::setw(10) << INPUT.ecutwfc
                                     << std::setw(10) << wfc_basis->lat0 << std::setw(10) << wfc_basis->tpiba
                                     << std::endl;
                                ofs2 << "\n<Reciprocal Lattice Vector>" << std::endl;
                                ofs2 << std::setw(10) << wfc_basis->G.e11 << std::setw(10) << wfc_basis->G.e12
                                     << std::setw(10) << wfc_basis->G.e13 << std::endl;
                                ofs2 << std::setw(10) << wfc_basis->G.e21 << std::setw(10) << wfc_basis->G.e22
                                     << std::setw(10) << wfc_basis->G.e23 << std::endl;
                                ofs2 << std::setw(10) << wfc_basis->G.e31 << std::setw(10) << wfc_basis->G.e32
                                     << std::setw(10) << wfc_basis->G.e33 << std::endl;
                                ofs2 << "<Reciprocal Lattice Vector>\n" << std::endl;
                                ofs2 << "<G vectors>" << std::endl;
                            }
                            for (int ig = 0; ig < p_kv->ngk[ik]; ig++)
                            {
                                ofs2 << std::setw(10) << wfc_basis->getgcar(ik, ig).x << std::setw(10)
                                     << wfc_basis->getgcar(ik, ig).y << std::setw(10) << wfc_basis->getgcar(ik, ig).z
                                     << std::endl;
                            }
                        ...
                        else if (INPUT.out_wfc_pw == 2)
                        {
                            ...
                            if (id == 0)
                            {
                                wfs2 << int(72) << ikstot + 1 << nkstot << p_kv->kvec_c[ik].x << p_kv->kvec_c[ik].y
                                     << p_kv->kvec_c[ik].z << p_kv->wk[ik] << ikngtot << GlobalV::NBANDS
                                     << INPUT.ecutwfc << wfc_basis->lat0 << wfc_basis->tpiba
                                     << 72; // 4 int + 7 double is 72B
                                wfs2 << 72 << wfc_basis->G.e11 << wfc_basis->G.e12 << wfc_basis->G.e13
                                     << wfc_basis->G.e21 << wfc_basis->G.e22 << wfc_basis->G.e23 << wfc_basis->G.e31
                                     << wfc_basis->G.e32 << wfc_basis->G.e33 << 72; // 9 double is 72B
                            }
                            if (id == 0)
                            {
                                wfs2 << ikngtot * 8 * 3;
                            ...
                            for (int ig = 0; ig < p_kv->ngk[ik]; ig++)
                            {
                                wfs2 << wfc_basis->getgcar(ik, ig).x << wfc_basis->getgcar(ik, ig).y
                                     << wfc_basis->getgcar(ik, ig).z;
                            }
                            if (id == GlobalV::NPROC_IN_POOL - 1)
                            {
                                wfs2 << ikngtot * 8 * 3;
...
// Note: The possibility of reading wfc files parallelly is supposed to happen outside of this function
void WF_io::read_wfc(const std::string &fn,
                     ...
                     int &nkstot)
{
    ModuleBase::TITLE("WF_io", "read_wfc");
    ...
    if (out_wfc_pw == 1)
    {
        if (ifs.good())
        {
            ...
            ifs >> ikstot >> nkstot >> p_kv->kvec_c[ik].x >> p_kv->kvec_c[ik].y >> p_kv->kvec_c[ik].z >> p_kv->wk[ik]
                >> ikngtot >> nbands >> ecut >> wfc_basis->lat0 >> wfc_basis->tpiba;
        }
        wfc_basis->npw = ikngtot;
        wfc_basis->gcar = new ModuleBase::Vector3<double>[wfc_basis->npw];
        psi.resize(ikstot, nbands, ikngtot);
        while (ifs.good())
        {
            getline(ifs, dummy);
            if (dummy == "<Reciprocal Lattice Vector>")
            {

                ifs >> wfc_basis->G.e11 >> wfc_basis->G.e12 >> wfc_basis->G.e13;
                ifs >> wfc_basis->G.e21 >> wfc_basis->G.e22 >> wfc_basis->G.e23;
                ifs >> wfc_basis->G.e31 >> wfc_basis->G.e32 >> wfc_basis->G.e33;
                getline(ifs, dummy);
                getline(ifs, dummy);
