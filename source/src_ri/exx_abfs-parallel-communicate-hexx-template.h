	...
	else
	{
		...
		else
		{
			...
			auto index = [&](const int i) -> int
			{
				...
				const int alpha_begin = GlobalC::CHR_MIX.idstep - alpha_size;
				return pos_mod( alpha_begin+i, GlobalC::CHR_MIX.dstep );
			};
			H_pulay = (1+GlobalC::CHR_MIX.alpha[index(H_seq.size()-2)]) * H_seq[H_seq.size()-1];
			for( int i=1; i<=H_seq.size()-2; ++i )
				H_pulay += ( GlobalC::CHR_MIX.alpha[index(i-1)] - GlobalC::CHR_MIX.alpha[index(i)] ) * H_seq[i];
			H_pulay -= GlobalC::CHR_MIX.alpha[index(0)] * H_seq[0];
		}
	}
	return H_pulay;
}
