...
template <typename TK, typename TR>
void
    ESolver_KS_LCAO<TK, TR>::before_all_runners (UnitCell& ucell, const Input_para& inp)
{
    ...
    if (this->pelec == nullptr)
        {
            ...
            this->pelec
                = new elecstate::ElecStateLCAO<TK> (&(this->chr), &(this->kv), this->kv.get_nks (), this->pw_big);
        }
...
#else
            ModuleBase::WARNING_QUIT ("ESolver_KS_LCAO::before_all_runners",
                                      "calculation=gen_opt_abfs must compile __EXX");
#endif
            return;
        }
...
template <typename TK, typename TR>
void
    ESolver_KS_LCAO<TK, TR>::before_scf (UnitCell& ucell, const int istep)
{
...
template <typename TK, typename TR>
double
    ESolver_KS_LCAO<TK, TR>::cal_energy ()
{
...
template <typename TK, typename TR>
void
    ESolver_KS_LCAO<TK, TR>::cal_force (UnitCell& ucell, ModuleBase::matrix& force)
{
...
template <typename TK, typename TR>
void
    ESolver_KS_LCAO<TK, TR>::cal_stress (UnitCell& ucell, ModuleBase::matrix& stress)
{
...
template <typename TK, typename TR>
void
    ESolver_KS_LCAO<TK, TR>::after_all_runners (UnitCell& ucell)
{
...
template <typename TK, typename TR>
void
    ESolver_KS_LCAO<TK, TR>::iter_init (UnitCell& ucell, const int istep, const int iter)
{
            ...
            if (PARAM.inp.gint_precision == "mix")
                {
                    ...
                    std::cout << " >> NOTICE: Gint grid-integration starts with fp32 (mixed-precision mode)"
                              << std::endl;
                }
...
template <typename TK, typename TR>
void
    ESolver_KS_LCAO<TK, TR>::hamilt2rho_single (UnitCell& ucell, int istep, int iter, double ethr)
{
...
template <typename TK, typename TR>
void
    ESolver_KS_LCAO<TK, TR>::iter_finish (UnitCell& ucell, const int istep, int& iter, bool& conv_esolver)
{
...
template <typename TK, typename TR>
void
    ESolver_KS_LCAO<TK, TR>::after_scf (UnitCell& ucell, const int istep, const bool conv_esolver)
{
