    ...
    if (PARAM.inp.basis_type == "pw")
    ...
    else if (PARAM.inp.basis_type == "lcao_in_pw")
        {
            ...
            ModuleBase::WARNING_QUIT ("ESolver",
                                      "Calculation involving numerical orbitals must be compiled with __LCAO");
#endif
        }
...
ESolver*
    init_esolver (const Input_para& inp, UnitCell& ucell)
{
            ...
            if (PARAM.globalv.gamma_only_local)
            ...
            else
                {
                    p_esolver_lr = new LR::ESolver_LR<std::complex<double>, double> (
                        std::move (
                            *dynamic_cast<ModuleESolver::ESolver_KS_LCAO<std::complex<double>, double>*> (p_esolver)),
                        inp,
                        ucell);
                }
