 ...
 */
void
    ESolver_OF::init_elecstate (UnitCell& ucell)
{
 ...
 */
void
    ESolver_OF::cal_potential (double* ptemp_phi, double* rdLdphi, UnitCell& ucell)
{
    ...
    this->kedf_manager_->get_potential (this->ptemp_rho_->rho, temp_phi, this->pw_rho,
 ...
 */
void
    ESolver_OF::cal_dEdtheta (double** ptemp_phi, Charge* temp_rho, UnitCell& ucell, double* ptheta, double* rdEdtheta)
{
    ...
    this->kedf_manager_->get_potential (temp_rho->rho, ptemp_phi, this->pw_rho,
                                        vr_eff); // KEDF potential
    for (int is = 0; is < PARAM.inp.nspin; ++is)
        {
            for (int ir = 0; ir < this->pw_rho->nrxx; ++ir)
                {
                    ...
                    dphi_dtheta[ir]
                        = -this->pphi_[is][ir] * sin (ptheta[is]) + this->pdirect_[is][ir] * cos (ptheta[is]);
                }
 ...
 */
double
    ESolver_OF::cal_mu (double* pphi, double* pdEdphi, double nelec)
{
            ...
            for (int i = 0; i < this->pw_rho->nrxx; ++i)
                {
                    ...
                    this->pdirect_[0][i]
                        = this->pdirect_[0][i] - this->pphi_[0][i] * inner_phi_direction / this->nelec_[0];
                }
            ...
            for (int is = 0; is < PARAM.inp.nspin; ++is)
                {
                    ...
                    for (int i = 0; i < this->pw_rho->nrxx; ++i)
                        {
                            this->pdirect_[is][i]
                                = std::sqrt (this->nelec_[is]) * this->pdirect_[is][i] / norm_direction;
                        }
                    this->theta_[is] = 0.;
                }
        }
 ...
 */
void
    ESolver_OF::check_direction (double* dEdtheta, double** ptemp_phi, UnitCell& ucell)
{
 ...
 */
void
    ESolver_OF::test_direction (double* dEdtheta, double** ptemp_phi, UnitCell& ucell)
{
            ...
            for (int i = -100; i < 100; ++i)
                {
                    ...
                    for (int ir = 0; ir < this->pw_rho->nrxx; ++ir)
                        {
                            ptemp_phi[0][ir] = this->pphi_[0][ir] * cos (this->theta_[0])
