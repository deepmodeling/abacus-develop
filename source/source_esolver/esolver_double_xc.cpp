...
template <typename TK, typename TR>
void
    ESolver_DoubleXC<TK, TR>::before_all_runners (UnitCell& ucell, const Input_para& inp)
{
            ...
            if (PARAM.globalv.gamma_only_local)
                {
                    ...
                    if (PARAM.inp.ks_solver == "genelpa" || PARAM.inp.ks_solver == "elpa"
                        || PARAM.inp.ks_solver == "lapack" || PARAM.inp.ks_solver == "pexsi"
...
template <typename TK, typename TR>
void
    ESolver_DoubleXC<TK, TR>::before_scf (UnitCell& ucell, const int istep)
{
...
template <typename TK, typename TR>
void
    ESolver_DoubleXC<TK, TR>::iter_finish (UnitCell& ucell, const int istep, int& iter, bool& conv_esolver)
{
            ...
            for (int is = 0; is < PARAM.inp.nspin; is++)
                {
                    ...
                    if (XC_Functional::get_ked_flag ())
                        {
                            ModuleBase::GlobalFunc::DCOPY (this->chr.kin_r[is],
                                                           this->chr_base.kin_r[is],
                                                           this->chr.rhopw->nrxx);
                        }
                }
        }
    ...
    if (output_iter)
        {
            ...
            this->pelec_base->cal_energies (2); // 2 means Kohn-Sham functional
                                                ...
                                                // std::cout<<"in double_xc------"<<std::endl;
            // GlobalV::ofs_running << std::setprecision(15) << " etot of base functional (Ry) " <<
            // pelec_base->f_en.etot << std::endl;
            ...
            if (PARAM.inp.deepks_v_delta > 0)
                {
                    if (PARAM.inp.vl_in_h)
                        ...
                        // which will change the hsolver::DiagoElpa<double>::DecomposedState, influencing the following
                        // SCF steps

#ifdef __MLALGO
                    using TH = std::
                        conditional_t<std::is_same<TK, double>::value, ModuleBase::matrix, ModuleBase::ComplexMatrix>;
                    ...
                    DeePKS_domain::get_h_tot<TK, TH, TR> (this->pv,
                                                          p_ham_deepks_base,
                                                          h_tot,
                                                          PARAM.globalv.nlocal,
                                                          nks,
                    ...
                    const std::string file_htot
                        = deepks_interface.get_filename ("hbase", PARAM.inp.deepks_out_labels, iter);
    ...
    if (PARAM.inp.cal_force)
        {
            if (!conv_esolver)
            ...
            else
                {
                    ...
                    for (int is = 0; is < PARAM.inp.nspin; is++)
                        {
                            ModuleBase::GlobalFunc::DCOPY (this->chr.rho[is],
                                                           this->chr_base.rho[is],
                                                           this->chr.rhopw->nrxx);
                            if (XC_Functional::get_ked_flag ())
                                {
                                    ModuleBase::GlobalFunc::DCOPY (this->chr.kin_r[is],
...
template <typename TK, typename TR>
void
    ESolver_DoubleXC<TK, TR>::cal_force (UnitCell& ucell, ModuleBase::matrix& force)
{
