...
void
    ESolver_OF::cal_potential_wrapper (double* ptemp_phi, double* rdLdphi)
{
 ...
 */
void
    ESolver_OF::get_direction (UnitCell& ucell)
{
 ...
 */
void
    ESolver_OF::get_step_length (double* dEdtheta, double** ptemp_phi, UnitCell& ucell)
{
                    ...
                    if (strncmp (this->task_, "FG", 2) == 0) // continue line search
                        {
                            ...
                            for (int i = 0; i < this->pw_rho->nrxx; ++i)
                                {
                                    ptemp_phi[0][i] = this->pphi_[0][i] * cos (this->theta_[0])
