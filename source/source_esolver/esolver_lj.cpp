...
void
    ESolver_LJ::before_all_runners (UnitCell& ucell, const Input_para& inp)
{
    lj_potential = 0;
...
void
    ESolver_LJ::runner (UnitCell& ucell, const int istep)
{
...
double
    ESolver_LJ::cal_energy ()
{
    return lj_potential;
}

void
    ESolver_LJ::cal_force (UnitCell& ucell, ModuleBase::matrix& force)
{
    force = lj_force;
...
void
    ESolver_LJ::cal_stress (UnitCell& ucell, ModuleBase::matrix& stress)
{
    stress = lj_virial;
...
void
    ESolver_LJ::after_all_runners (UnitCell& ucell)
{
    GlobalV::ofs_running << "\n --------------------------------------------" << std::endl;
...
double
    ESolver_LJ::LJ_energy (const double& d, const int& i, const int& j) const
{
...
ModuleBase::Vector3<double>
    ESolver_LJ::LJ_force (const ModuleBase::Vector3<double>& dr, const int& i, const int& j) const
{
...
void
    ESolver_LJ::LJ_virial (const ModuleBase::Vector3<double>& force, const ModuleBase::Vector3<double>& dtau)
{
    for (int i = 0; i < 3; ++i)
        {
            for (int j = 0; j < 3; ++j)
...
void
    ESolver_LJ::rcut_search_radius (const int& ntype, const std::vector<double>& rcut)
{
...
void
    ESolver_LJ::set_c6_c12 (const int& ntype,
                            const int& rule,
                            ...
                            const std::vector<double>& sigma)
{
    ...
    std::transform (begin (lj_epsilon),
                    end (lj_epsilon),
                    begin (lj_epsilon),
                    [] (double x) { return x / ModuleBase::Ry_to_eV; });
    std::transform (begin (lj_sigma),
                    end (lj_sigma),
                    begin (lj_sigma),
                    [] (double x) { return x * ModuleBase::ANGSTROM_AU; });
...
void
    ESolver_LJ::cal_en_shift (const int& ntype, const bool& is_shift)
{
