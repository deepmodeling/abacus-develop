        ...
        || PARAM.inp.calculation == "md")
    {
        if (GlobalC::exx_info.info_global.cal_exx)
        {
            ...
            this->exx_lip = std::unique_ptr<Exx_Lip<T>>(new Exx_Lip<T>(GlobalC::exx_info.info_lip,
                                                                       ...
                                                                       &ucell,
                                                                       this->pelec));
            // this->exx_lip.init(GlobalC::exx_info.info_lip, cell.symm, &this->kv, this->p_psi_init, this->kspw_psi,
            // this->pw_wfc, this->pw_rho, this->sf, &cell, this->pelec);
        }
    }
#endif
}
...
void ESolver_KS_LIP<T>::hamilt2rho_single(UnitCell& ucell, const int istep, const int iter, const double ethr)
{
    ...
    hsolver_lip_obj
        .solve(this->p_hamilt, this->kspw_psi[0], this->pelec, *this->psi_local, skip_charge, ucell.tpiba, ucell.nat);
