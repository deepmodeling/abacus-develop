...
template <typename T>
void
    psi_init_atomic<T>::allocate_ps_table ()
{
    ...
    if (dim2 == 0)
        {
            ModuleBase::WARNING_QUIT (
                "psi_init_atomic<T>::allocate_table",
...
template <typename T>
void
    psi_init_atomic<T>::initialize (const Structure_Factor* sf,         //< structure factor
    ...
    for (int it = 0; it < this->p_ucell_->ntype; it++)
        {
            ...
            GlobalV::ofs_running << "\n number of pseudo atomic orbitals for " << atom->label << " is "
                                 << atom->ncpp.nchi << std::endl;
            ...
            for (int ic = 0; ic < atom->ncpp.nchi; ic++)
                {
                    ...
                    if (unit < 1e-8)
                        {
                            ...
                            GlobalV::ofs_running << "WARNING: norm of atomic wavefunction # " << ic + 1
                                                 << " of atomic type " << atom->ncpp.psd << " is zero" << std::endl;
                        }
                    ...
                    if (std::abs (unit - 1.0) > 1e-6)
                        {
                            GlobalV::ofs_running << "WARNING: norm of atomic wavefunction # " << ic + 1
                                                 << " of atomic type " << atom->ncpp.psd << " is " << unit
                                                 << ", renormalized" << std::endl;
                            for (int ir = 0; ir < n_rgrid; ir++)
                                {
                    ...
                    for (int iq = startq; iq < PARAM.globalv.nqx; iq++)
                        {
                            ...
                            ModuleBase::Sphbes::Spherical_Bessel (atom->ncpp.msh,
                                                                  atom->ncpp.r.data (),
                                                                  q,
                                                                  l,
                                                                  aux.data ());
...
std::complex<double>
    phase_factor (double arg, int mode)
{
    if (mode == 1)
        {
            return std::complex<double> (cos (arg), 0);
        }
    else if (mode == -1)
        {
            return std::complex<double> (0, sin (arg));
        }
    else if (mode == 0)
        {
...
template <typename T>
void
    psi_init_atomic<T>::init_psig (T* psig, const int& ik)
{
                                    ...
                                    for (int ig = 0; ig < npw; ig++)
                                        {
                                            ovlp_pswfcjlg[ig] = ModuleBase::PolyInt::Polynomial_Interpolation (
                                                ...
                                                PARAM.globalv.nqx,
                                                PARAM.globalv.dq,
                                                gk[ig].norm () * this->p_ucell_->tpiba);
                                        }
                                    /* NSPIN == 4 */
                                            ...
                                            if (this->p_ucell_->atoms[it].ncpp.has_so)
                                                {
                                                    ...
                                                    if (!(PARAM.globalv.domag || PARAM.globalv.domag_z))
                                                            ...
                                                            for (int m = -l - 1; m < l + 1; m++)
                                                                {
                                                                    ...
                                                                    if (fabs (cg_coeffs[0]) > 1e-8
                                                                        || fabs (cg_coeffs[1]) > 1e-8)
                                                                        {
                                                                            for (int is = 0; is < 2; is++)
                                                                                {
                                                                                    if (fabs (cg_coeffs[is]) > 1e-8)
                                                                                        {
                                                                                            /* GET COMPLEX SPHERICAL
                                                                                             * HARMONIC FUNCTION */
                                                                                            const int ind
                                                                                                = this->p_pspot_nl_
                                                                                                      ->lmaxkb
                                                        ...
                                                        {
                                                            /* NONCONLINEAR CASE, will use [[cos(a/2)*exp(-ib/2),
                                                             * sin(a/2)*exp(ib/2)], [-sin(a/2)*exp(-ib/2),
                                                             * cos(a/2)*exp(ib/2)]] to rotate */
                                                            ...
                                                            if (l == 0)
                                                                {
                                                                    std::memcpy (chiaux.data (),
                                                                                 ovlp_pswfcjlg.data (),
                                                                                 npw * sizeof (double));
                                                                }
                                                            else
                                                                {
                                                                    /* L != 0, scan pswfcs that have the same L and
                                                                     * satisfy J(pswfc) = L - 0.5 */
                                                                    for (int jpsiwfc = 0;
                                                                         jpsiwfc < this->p_ucell_->atoms[it].ncpp.nchi;
                                                                         jpsiwfc++)
                                                                        {
                                                                            if ((this->p_ucell_->atoms[it]
                                                                                     .ncpp.lchi[jpsiwfc]
                                                                                 == l)
                                                                                && (fabs (this->p_ucell_->atoms[it]
                                                                                              .ncpp.jchi[jpsiwfc]
                                                                                          - l + 0.5)
                                                                                    < 1e-4))
                                                                                {
                                                                    ...
                                                                    for (int ig = 0; ig < npw; ig++)
                                                                        {
                                                                            /* average <pswfc_a|jl(q)> and
                                                                             * <pswfc_b(j=l-1/2)|jl(q)>, a and b seem
                                                            ...
                                                            double gamma = -1 * this->p_ucell_->atoms[it].angle2[ia]
                                                                           + 0.5 * ModuleBase::PI;
                                                                    ...
                                                                    if (index + 2 * l + 1 > this->p_ucell_->natomwfc)
                                                                        {
                                                                            std::cout << __FILE__ << __LINE__ << " "
                                                                                      << index << " "
                                                                                      << this->p_ucell_->natomwfc
                                                                                      << std::endl;
                                                                            // ModuleBase::WARNING_QUIT("psi_init_atomic<T>::init_psig()","error:
                                                                            // too many wfcs");
                                                                        }
                                                                    for (int ig = 0; ig < npw; ig++)
                                                                        {
                                                                            aux[ig]
                                                                                = sk[ig] * ylm (lm, ig) * chiaux[ig];
                                                                        }
                                                                    // rotate wfc as needed
                                                                    // first rotation with angle alpha around (OX)
                                                                    for (int ig = 0; ig < npw; ig++)
                                                                        {
                                                                            fup = phase_factor (0.5 * alpha, 1)
                                                                                  * aux[ig];
                                                                            fdw = phase_factor (0.5 * alpha, -1)
                                                                                  * aux[ig];
                                                                            // build the orthogonal wfc
                                                                            // first rotation with angle (alpha +
                                                                            // ModuleBase::PI) around (OX)
                                                                            psig[index * 2 * npwk_max + ig]
                                                                                = this->template cast_to_T<T> (
                                                                                    phase_factor (0.5 * gamma, 0)
                                                                                    * fup);
                                                                            psig[(index * 2 + 1) * npwk_max + ig]
                                                                                = this->template cast_to_T<T> (
                                                                                    phase_factor (-0.5 * gamma, 0)
                                                                                    * fdw);
                                                                            // second rotation with angle gamma
                                                                            // around(OZ)
                                                                            fup = phase_factor (
                                                                                      0.5 * (alpha + ModuleBase::PI),
                                                                                      1)
                                                                                  * aux[ig];
                                                                            fdw = phase_factor (
                                                                                      0.5 * (alpha + ModuleBase::PI),
                                                                                      -1)
                                                                                  * aux[ig];
                                                                            psig[(index + 2 * l + 1) * 2 * npwk_max
                                                                                 + ig]
                                                                                = this->template cast_to_T<T> (
                                                                                    phase_factor (0.5 * gamma, 0)
                                                                                    * fup);
                                                                            psig[((index + 2 * l + 1) * 2 + 1)
                                                                                     * npwk_max
                                                                                 + ig]
                                                                                = this->template cast_to_T<T> (
                                                                                    phase_factor (-0.5 * gamma, 0)
                                                ...
                                                { // atomic_wfc_nc
                                                    ...
                                                    // gamman = -this->p_ucell_->magnet.angle2_[it] +
                                                    // 0.5*ModuleBase::PI;
                                                    alpha = this->p_ucell_->atoms[it].angle1[ia];
                                                    gamman = -1 * this->p_ucell_->atoms[it].angle2[ia]
                                                             + 0.5 * ModuleBase::PI;
                                                    for (int m = 0; m < 2 * l + 1; m++)
                                                        {
                                                            const int lm = l * l + m;
                                                            ...
                                                            for (int ig = 0; ig < npw; ig++)
                                                                {
                                                                    ...
                                                                    fdown = ModuleBase::IMAG_UNIT * sin (0.5 * alpha)
                                                                            * aux[ig];
                                                                    // build the orthogonal wfc
                                                                    // first rotation with angle(alpha+ModuleBase::PI)
                                                                    // around(OX)
                                                                    psig[index * 2 * npwk_max + ig]
                                                                        = this->template cast_to_T<T> (
                                                                            (cos (0.5 * gamman)
                                                                    ...
                                                                    fup = cos (0.5 * (alpha + ModuleBase::PI))
                                                                          * aux[ig];
                                                                    fdown = ModuleBase::IMAG_UNIT
                                                                            * sin (0.5 * (alpha + ModuleBase::PI))
                                                                            * aux[ig];
                                            ...
                                            for (int m = 0; m < 2 * l + 1; m++)
                                                {
                                                    ...
                                                    for (int ig = 0; ig < npw; ig++)
                                                        {
                                                            psig[index * npwk_max + ig] = this->template cast_to_T<T> (
