...
void
    normalize (const std::vector<double>& r, std::vector<double>& flz)
{
    ...
    std::transform (r.begin (),
                    ...
                    flz.begin (),
                    flz2r2.begin (),
                    [] (double r, double flz) { return flz * flz * r * r; });
    double dr = r[1] - r[0];
...
template <typename T>
void
    psi_init_nao<T>::read_external_orbs (const std::string* orbital_files, const int& rank)
{
    ...
    for (int it = 0; it < this->p_ucell_->ntype; it++)
        {
            ...
            std::for_each (this->rgrid_[it].begin (),
                           this->rgrid_[it].end (),
                           [nr, dr] (std::vector<double>& rgrid)
                               {
                                   rgrid.resize (nr);
            ...
            std::for_each (this->chi_[it].begin (),
                           this->chi_[it].end (),
                           [nr] (std::vector<double>& chi) { chi.resize (nr); });
...
template <typename T>
void
    psi_init_nao<T>::allocate_ao_table ()
{
...
template <typename T>
void
    psi_init_nao<T>::initialize (const Structure_Factor* sf,
    ...
    psi_init_nao<T>::tabulate ()
{
    ...
    this->cubspl_
        = std::unique_ptr<ModuleBase::CubicSpline> (new ModuleBase::CubicSpline (qgrid.size (), qgrid.data ()));
...
template <typename T>
void
    psi_init_nao<T>::init_psig (T* psig, const int& ik)
{
                            ...
                            for (int N = 0; N < this->p_ucell_->atoms[it].l_nchi[L]; N++)
                                {
                                    ...
                                    /*
                                        for already using flattened 1d index of chi, which folds l and n, the spherical
                                       bessel transformation of numerical orbital function, is indiced by it and ic, is
                                       needed to interpolate everytime when ic updates, therefore everytime when present
                                       orbital is done
                                    */

                                    // use cublic spline instead of previous polynomial interpolation
                                    this->cubspl_->eval (npw,
                                                         qnorm.data (),
                                                         Jlfq.data (),
                                                         nullptr,
                                                         nullptr,
                                                         this->projmap_ (it, L, N));
                                                ...
                                                // for(int is_N = 0; is_N < 1; is_N++)
                                                {
                                                    if (L == 0 && is_N == 1)
                                                    ...
                                                    else
                                                        {
                                                            ...
                                                            gamma = -1 * this->p_ucell_->atoms[it].angle2[ia]
                                                                    + 0.5 * ModuleBase::PI;
...
#pragma omp parallel for
                                                    for (int ig = 0; ig < npw; ig++)
                                                        {
                                                            psig[ibasis * npwk_max + ig] = this->template cast_to_T<T> (
                                                                lphase * sk[ig] * ylm (lm, ig) * Jlfq[ig]);
                                                        }
                                                    ++ibasis;
                                                }
                                        }
