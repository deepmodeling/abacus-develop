...
template <typename T, typename Device>
void
    PSIPrepare<T, Device>::prepare_init (const int& random_seed)
{
    ...
    if (this->init_wfc == "random")
        {
            ...
            GlobalV::ofs_running << "\n Using RANDOM starting wave functions for all " << PARAM.inp.nbands
                                 << " bands\n";
        }
            ...
            if (this->ucell.natomwfc < PARAM.inp.nbands)
                {
                    ...
                    GlobalV::ofs_running << "\n Using ATOMIC starting wave functions with " << this->ucell.natomwfc
                                         << " atomic orbitals"
                ...
                {
                    GlobalV::ofs_running << "\n Using ATOMIC starting wave functions for all " << this->ucell.natomwfc
                                         << " atomic orbitals"
    ...
    else if (this->init_wfc == "atomic+random")
        {
            ...
            GlobalV::ofs_running << "\n Using ATOMIC+RANDOM starting wave functions with " << this->ucell.natomwfc
                                 << " atomic orbitals\n";
        }
...
template <typename T, typename Device>
void
    PSIPrepare<T, Device>::initialize_psi (Psi<std::complex<double>>* psi,
            ...
            if (fill)
                {
                    ...
                    if (psi_device->get_pointer () != psi_cpu->get_pointer ())
                        {
                            syncmem_h2d_op () (psi_device->get_pointer (),
                                               psi_cpu->get_pointer (),
                                               nbands_start * nbasis);
                        }
                    ...
                    if (this->ks_solver == "cg")
                        {
                            ...
                            if (not_equal)
                                {
                                    // for diagH_subspace_init, psi_device->get_pointer() and kspw_psi->get_pointer()
                                    // should be different
                                ...
                                {
                                    // for diagH_subspace, psi_device->get_pointer() and kspw_psi->get_pointer() can be
                                    // the same
                    ...
                    else // dav, bpcg
                        {
                            if (psi_device->get_pointer () != kspw_psi->get_pointer ())
                                {
                                    syncmem_complex_op () (kspw_psi->get_pointer (),
                                                           psi_device->get_pointer (),
                                                           nbands_l * nbasis);
                                }
                        }
                    ...
                    if (GlobalV::MY_BNDGROUP == 0)
                        {
                            for (int ip = 1; ip < PARAM.inp.bndpar; ++ip)
                                {
                                    Parallel_Common::send_data (psi_cpu->get_pointer () + displs[ip],
                                                                sendcounts[ip],
                                                                ip,
                                                                0,
                                                                BP_WORLD);
...
void
    allocate_psi (Psi<std::complex<double>>*& psi,
                  const int& nks,
