...
void Setup_Psi_pw::before_runner_impl(const UnitCell& ucell,
                                      const K_Vectors& kv,
...
void Setup_Psi_pw::before_runner(const UnitCell& ucell,
    ...
    if (is_gpu) {
        if (is_single) {
            before_runner_impl<std::complex<float>, base_device::DEVICE_GPU>(ucell, kv, sf, pw_wfc, ppcell, inp);
        } else {
            before_runner_impl<std::complex<double>, base_device::DEVICE_GPU>(ucell, kv, sf, pw_wfc, ppcell, inp);
        }
    } else
#endif
    {
        if (is_single) {
            before_runner_impl<std::complex<float>, base_device::DEVICE_CPU>(ucell, kv, sf, pw_wfc, ppcell, inp);
        } else {
            before_runner_impl<std::complex<double>, base_device::DEVICE_CPU>(ucell, kv, sf, pw_wfc, ppcell, inp);
        }
    }
}
...
void Setup_Psi_pw::castmem_d2h_impl(std::complex<double>* dst, const std::complex<double>* src, const size_t size) {
    base_device::memory::cast_memory_op<std::complex<double>, std::complex<double>, base_device::DEVICE_CPU, Device>()(
        dst,
        src,
        size);
}
