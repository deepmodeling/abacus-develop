...
void
    Setup_Psi_pw::before_runner_impl (const UnitCell& ucell,
                                      const K_Vectors& kv,
...
void
    Setup_Psi_pw::before_runner (const UnitCell& ucell,
                                 const K_Vectors& kv,
                                 const Structure_Factor& sf,
                                 const ModulePW::PW_Basis_K& pw_wfc,
                                 const pseudopot_cell_vnl& ppcell,
    ...
    if (is_gpu)
        {
            if (is_single)
                {
                    before_runner_impl<std::complex<float>, base_device::DEVICE_GPU> (ucell,
                                                                                      kv,
                                                                                      sf,
                                                                                      pw_wfc,
                                                                                      ppcell,
                ...
                {
                    before_runner_impl<std::complex<double>, base_device::DEVICE_GPU> (ucell,
                                                                                       kv,
                                                                                       sf,
                                                                                       pw_wfc,
                                                                                       ppcell,
...
#endif
        {
            if (is_single)
                {
                    before_runner_impl<std::complex<float>, base_device::DEVICE_CPU> (ucell,
                                                                                      kv,
                                                                                      sf,
                                                                                      pw_wfc,
                                                                                      ppcell,
                ...
                {
                    before_runner_impl<std::complex<double>, base_device::DEVICE_CPU> (ucell,
                                                                                       kv,
                                                                                       sf,
                                                                                       pw_wfc,
                                                                                       ppcell,
...
template <typename T, typename Device>
void
    Setup_Psi_pw::update_psi_d_impl ()
{
    ...
    if (this->precision_type_ == PrecisionType::ComplexFloat)
        {
            this->psi_d
                = static_cast<void*> (new psi::Psi<std::complex<double>, Device> (*this->get_psi_t<T, Device> ()));
        }
    else
        {
...
template <typename T, typename Device>
void
    Setup_Psi_pw::init_impl (hamilt::Hamilt<T, Device>* p_hamilt)
{
...
void
    Setup_Psi_pw::init (hamilt::HamiltBase* p_hamilt)
{
    if (this->already_initpsi)
        {
            return;
...
template <typename T, typename Device>
void
    Setup_Psi_pw::castmem_d2h_impl (std::complex<double>* dst, const std::complex<double>* src, const size_t size)
{
    base_device::memory::
        cast_memory_op<std::complex<double>, std::complex<double>, base_device::DEVICE_CPU, Device> () (dst, src, size);
}

template <typename T, typename Device>
void
    Setup_Psi_pw::castmem_d2h_impl (std::complex<double>* dst, const std::complex<float>* src, const size_t size)
{
    base_device::memory::cast_memory_op<std::complex<double>, std::complex<float>, base_device::DEVICE_CPU, Device> () (
        dst,
        src,
        size);
}
...
template void
    Setup_Psi_pw::before_runner_impl<std::complex<float>, base_device::DEVICE_CPU> (const UnitCell&,
                                                                                    const K_Vectors&,
...
template void
    Setup_Psi_pw::before_runner_impl<std::complex<double>, base_device::DEVICE_CPU> (const UnitCell&,
                                                                                     const K_Vectors&,
                                                                                     const Structure_Factor&,
                                                                                     const ModulePW::PW_Basis_K&,
                                                                                     const pseudopot_cell_vnl&,
...
template void Setup_Psi_pw::castmem_d2h_impl<std::complex<float>, base_device::DEVICE_CPU> (std::complex<double>*,
...
template void Setup_Psi_pw::castmem_d2h_impl<std::complex<double>, base_device::DEVICE_CPU> (std::complex<double>*,
                                                                                             const std::complex<float>*,
                                                                                             const size_t);

template void
    Setup_Psi_pw::castmem_d2h_impl<std::complex<double>, base_device::DEVICE_CPU> (std::complex<double>*,
                                                                                   const std::complex<double>*,
                                                                                   const size_t);
...
template void
    Setup_Psi_pw::before_runner_impl<std::complex<float>, base_device::DEVICE_GPU> (const UnitCell&,
                                                                                    const K_Vectors&,
                                                                                    const Structure_Factor&,
                                                                                    const ModulePW::PW_Basis_K&,
                                                                                    const pseudopot_cell_vnl&,
...
template void
    Setup_Psi_pw::before_runner_impl<std::complex<double>, base_device::DEVICE_GPU> (const UnitCell&,
                                                                                     const K_Vectors&,
                                                                                     const Structure_Factor&,
                                                                                     const ModulePW::PW_Basis_K&,
                                                                                     const pseudopot_cell_vnl&,
...
template void Setup_Psi_pw::castmem_d2h_impl<std::complex<float>, base_device::DEVICE_GPU> (std::complex<double>*,
                                                                                            const std::complex<float>*,
                                                                                            const size_t);
...
template void Setup_Psi_pw::castmem_d2h_impl<std::complex<double>, base_device::DEVICE_GPU> (std::complex<double>*,
                                                                                             const std::complex<float>*,
                                                                                             const size_t);

template void
    Setup_Psi_pw::castmem_d2h_impl<std::complex<double>, base_device::DEVICE_GPU> (std::complex<double>*,
                                                                                   const std::complex<double>*,
                                                                                   const size_t);
#endif
