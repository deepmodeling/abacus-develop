...
template <typename T>
void
    psi_initializer<T>::initialize (const Structure_Factor* sf,
...
template <typename T>
void
    psi_initializer<T>::random_t (T* psi, const int iw_start, const int iw_end, const int ik, const int mode)
{
                            ...
                            for (int ig = 0; ig < ng; ig++)
                                {
                                    ...
                                    psi_slice[ig] = this->template cast_to_T<T> (
                                        std::complex<double> (rr * cos (arg), rr * sin (arg)));
                                }
#ifdef _OPENMP
#pragma omp parallel for schedule(static)
#endif
    ...
    // It does not guarantee the same results using different number of processors.
    else
        {
            for (int iw = iw_start; iw < iw_end; iw++)
                {
                    T* psi_slice
                        = &(psi[iw * npwk_max * npol]); // get the memory to write directly. For nspin 4, nbasis*2
                            ...
                            for (int ig = npwk_max; ig < npwk_max + ng; ig++)
                                {
                                    ...
                                    psi_slice[ig] = this->template cast_to_T<T> (
                                        std::complex<double> (rr * cos (arg) / (gk2 + 1.0),
                                                              rr * sin (arg) / (gk2 + 1.0)));
                                }
                        }
                }
...
template <typename T>
void
    psi_initializer<T>::stick_to_pool (Real* stick, const int& ir, Real* out) const
{
