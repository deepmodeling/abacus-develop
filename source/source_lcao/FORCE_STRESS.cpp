...
template <>
void
    assign_dmk_ptr<double> (elecstate::DensityMatrix<double, double>* dm,
...
template <>
void
    assign_dmk_ptr<std::complex<double>> (elecstate::DensityMatrix<std::complex<double>, double>* dm,
                                          std::vector<std::vector<double>>*& dmk_d,
                                          std::vector<std::vector<std::complex<double>>>*& dmk_c,
                                          bool gamma_only_local)
{
                                          ...
                                          ModuleSymmetry::Symmetry* symm)
{
    ...
    elecstate::DensityMatrix<T, double> edm
        = flk.cal_edm (pelec, *psi, *dmat.dm, kv, pv, PARAM.inp.nspin, PARAM.inp.nbands, ucell, *this->RA);
    ...
    if (PARAM.inp.nspin == 1 || PARAM.inp.nspin == 2)
        {
            ...
            hamilt::Overlap<hamilt::OperatorLCAO<T, double>> tmp_overlap (nullptr,
                                                                          kv.kvec_d,
                                                                          nullptr,
                                                                          nullptr,
                                                                          &ucell,
            ...
            hamilt::Nonlocal<hamilt::OperatorLCAO<T, double>> tmp_nonlocal (nullptr,
                                                                            kv.kvec_d,
                                                                            nullptr,
                                                                            &ucell,
                                                                            orb.cutoffs (),
            ...
            if (PARAM.inp.t_in_h)
                {
                    hamilt::EKinetic<hamilt::OperatorLCAO<std::complex<double>, std::complex<double>>> tmp_ekinetic (
                        ...
                        &gd,
                        two_center_bundle.kinetic_orb.get ());
                    tmp_ekinetic.cal_force_stress (isforce,
                                                   isstress,
                                                   dmat.dm->get_DMR_pointer (1),
                                                   ftvnl_dphi,
                                                   stvnl_dphi);
                }
    ...
    if (PARAM.inp.dft_plus_u) // Quxin add for DFT+U on 20201029
        {
            ...
            if (PARAM.inp.dft_plus_u == 2)
                ...
                {
                    hamilt::DFTU<hamilt::OperatorLCAO<T, double>> tmpu (
    ...
    if (isforce)
        {
            ...
            if (PARAM.inp.gate_flag || PARAM.inp.efield_flag)
                {
                    GlobalV::ofs_running << "Atomic forces are not shifted if gate_flag or efield_flag == true!"
                                         << std::endl;
                }
            ...
            if (istestf)
                {
                    ...
                    GlobalV::ofs_running << " " << std::setw (8) << "atom" << std::setw (5) << "x" << std::setw (5)
                                         << "y" << std::setw (5) << "z" << std::endl;
                    for (int iat = 0; iat < ucell.nat; iat++)
                        {
            ...
            if (istests)
                {
                    ...
                    if (PARAM.inp.sc_mag_switch)
                        {
                            ModuleIO::print_stress ("DeltaSpin  STRESS",
                                                    stress_dspin,
                                                    screen,
                                                    ry,
                                                    GlobalV::ofs_running);
...
template <>
void
    Force_Stress_LCAO<double>::integral_part (
...
template <>
void
    Force_Stress_LCAO<std::complex<double>>::integral_part (
        const bool isGammaOnly,
        const bool isforce,
        const bool isstress,
        const UnitCell& ucell,
...
template <typename T>
void
    Force_Stress_LCAO<T>::forceSymmetry (const UnitCell& ucell, ModuleBase::matrix& fcs, ModuleSymmetry::Symmetry* symm)
{
    double d1, d2, d3;
    for (int iat = 0; iat < ucell.nat; iat++)
        {
