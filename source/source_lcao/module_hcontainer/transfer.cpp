...
template <typename T>
void
    HTransPara<T>::cal_orb_indexes (int irank, std::vector<int>* orb_indexes)
{
...
template <typename T>
void
    HTransPara<T>::receive_ap_indexes (int irank, const int* ap_indexes_in, const long& size_ap_indexes_in)
{
    ...
    if (ap_indexes_in == nullptr)
        {
            ...
            MPI_Recv (this->ap_indexes[irank].data (),
                      size_ap_indexes,
                      MPI_INT,
                      irank,
                      1,
...
template <typename T>
void
    HTransPara<T>::send_orb_indexes (int irank, MPI_Request* request)
{
...
template <typename T>
void
    HTransPara<T>::send_data (int irank, MPI_Request* request)
{
...
template <typename T>
void
    HTransPara<T>::receive_data (int irank, const T* values)
{
    // sender and receiver are not same process
    if (values == nullptr)
        {
...
template <typename T>
void
    HTransPara<T>::pack_data (int irank, T* values)
{
#ifdef __DEBUG
...
template <typename T>
void
    HTransPara<T>::unpack_data (int irank, const T* values)
{
#ifdef __DEBUG
    assert (values != nullptr);
    assert (this->size_values[irank] != 0);
...
template <typename T>
long
    HTransPara<T>::get_max_size () const
{
...
template <typename T>
void
    HTransPara<T>::get_value_size (int* out) const
{
...
template <typename T>
void
    HTransSerial<T>::cal_ap_indexes (int irank, std::vector<int>* ap_indexes)
{
...
template <typename T>
void
    HTransSerial<T>::send_ap_indexes (int irank, MPI_Request* request)
{
...
template <typename T>
void
    HTransSerial<T>::receive_orb_indexes (int irank, const int* orb_indexes_in, const long& size_orb_indexes_in)
{
    // sender and receiver are not same process
...
template <typename T>
void
    HTransSerial<T>::send_data (int irank, MPI_Request* request)
{
    std::vector<T> values (this->size_values[irank]);
    this->pack_data (irank, values.data ());
    if (request != nullptr)
...
template <typename T>
void
    HTransSerial<T>::receive_data (int irank, const T* values)
{
    // sender and receiver are not same process
    if (values == nullptr)
        {
...
template <typename T>
void
    HTransSerial<T>::pack_data (int irank, T* values)
{
#ifdef __DEBUG
    assert (values != nullptr);
    assert (this->size_values[irank] != 0);
...
template <typename T>
void
    HTransSerial<T>::unpack_data (int irank, const T* values)
{
#ifdef __DEBUG
    assert (values != nullptr);
    assert (this->size_values[irank] != 0);
...
template <typename T>
long
    HTransSerial<T>::get_max_size () const
{
    return *std::max_element (this->size_values.begin (), this->size_values.end ());
}
template <typename T>
void
    HTransSerial<T>::get_value_size (int* out) const
{
    for (int i = 0; i < this->size_values.size (); ++i)
        {
            out[i] = this->size_values[i];
