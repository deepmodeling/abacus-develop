...
AtomPair<T>::AtomPair (const AtomPair<T>& other, T* data_pointer)
    : R_index (other.R_index), paraV (other.paraV), current_R (other.current_R), atom_i (other.atom_i),
      atom_j (other.atom_j), row_ap (other.row_ap), col_ap (other.col_ap), row_size (other.row_size),
...
template <typename T>
void
    AtomPair<T>::allocate (T* data_array, bool is_zero)
{
...
template <typename T>
AtomPair<T>&
    AtomPair<T>::operator= (const AtomPair<T>& other)
{
    if (this != &other)
        {
...
AtomPair<T>::AtomPair (AtomPair<T>&& other) noexcept
    : R_index (std::move (other.R_index)), values (std::move (other.values)), paraV (other.paraV),
      current_R (other.current_R), atom_i (other.atom_i), atom_j (other.atom_j), row_ap (other.row_ap),
      col_ap (other.col_ap), row_size (other.row_size), col_size (other.col_size)
{
    other.paraV = nullptr;
}
...
template <typename T>
AtomPair<T>&
    AtomPair<T>::operator= (AtomPair<T>&& other) noexcept
{
    if (this != &other)
        {
...
template <typename T>
bool
    AtomPair<T>::operator< (const AtomPair<T>& other) const
{
...
template <typename T>
void
    AtomPair<T>::set_size (const int& col_size_in, const int& row_size_in)
{
...
template <typename T>
bool
    AtomPair<T>::identify (const AtomPair<T>& other) const
{
...
template <typename T>
bool
    AtomPair<T>::identify (const int& atom_i_, const int& atom_j_) const
{
...
template <typename T>
BaseMatrix<T>&
    AtomPair<T>::get_HR_values (int rx_in, int ry_in, int rz_in)
{
...
template <typename T>
const BaseMatrix<T>&
    AtomPair<T>::get_HR_values (int rx_in, int ry_in, int rz_in) const
{
...
template <typename T>
BaseMatrix<T>&
    AtomPair<T>::get_HR_values (const int& index) const
{
...
template <typename T>
int
    AtomPair<T>::find_R (const int& rx_in, const int& ry_in, const int& rz_in) const
{
...
template <typename T>
int
    AtomPair<T>::find_R (const ModuleBase::Vector3<int>& R_in) const
{
    for (int i = 0; i < this->R_index.size (); i++)
        {
            if (R_index[i] == R_in)
...
template <typename T>
const BaseMatrix<T>*
    AtomPair<T>::find_matrix (const int& rx_in, const int& ry_in, const int& rz_in) const
{
    const int r_index = this->find_R (rx_in, ry_in, rz_in);
    if (r_index == -1)
        {
...
template <typename T>
BaseMatrix<T>*
    AtomPair<T>::find_matrix (const int& rx_in, const int& ry_in, const int& rz_in)
{
    const int r_index = this->find_R (rx_in, ry_in, rz_in);
    if (r_index == -1)
        {
...
template <typename T>
const BaseMatrix<T>*
    AtomPair<T>::find_matrix (const ModuleBase::Vector3<int>& R_in) const
{
...
template <typename T>
BaseMatrix<T>*
    AtomPair<T>::find_matrix (const ModuleBase::Vector3<int>& R_in)
{
    const int r_index = this->find_R (R_in);
    if (r_index == -1)
        {
...
template <typename T>
void
    AtomPair<T>::convert_add (const BaseMatrix<T>& target, int rx_in, int ry_in, int rz_in)
{
...
template <typename T>
void
    AtomPair<T>::merge (const AtomPair<T>& other, bool skip_R)
{
...
template <typename T>
void
    AtomPair<T>::add_to_matrix (std::complex<T>* hk,
                                const int ld_hk,
...
template <typename T>
void
    AtomPair<T>::add_to_matrix (T* hk, const int ld_hk, const T& kphase, const int hk_type) const
{
    const BaseMatrix<T>& matrix = values[current_R];
    T* hr_tmp = matrix.get_pointer ();
    T* hk_tmp = hk;
...
template <typename T>
void
    AtomPair<T>::add_from_matrix (const std::complex<T>* hk,
                                  const int ld_hk,
                                  const std::complex<T>& kphase,
                                  const int hk_type)
{
...
template <typename T>
void
    AtomPair<T>::add_from_matrix (const T* hk, const int ld_hk, const T& kphase, const int hk_type)
{
    const BaseMatrix<T>& matrix = values[current_R];
    T* hr_tmp = matrix.get_pointer ();
    const T* hk_tmp = hk;
...
template <typename T>
void
    AtomPair<T>::add_to_array (T* array, const T& kphase) const
{
    const BaseMatrix<T>& matrix = values[current_R];
...
template <typename T>
void
    AtomPair<T>::add_to_array (std::complex<T>* array, const std::complex<T>& kphase) const
{
    const BaseMatrix<T>& matrix = values[current_R];
    for (int i = 0; i < this->row_size * this->col_size; i++)
        {
...
std::tuple<std::vector<int>, T*>
    AtomPair<T>::get_matrix_values (int ir) const
{
    if (ir < 0)
        {
            ir = this->current_R;
        }
    return std::tuple<std::vector<int>, T*> ({this->row_ap, this->row_size, this->col_ap, this->col_size},
                                             this->values[ir].get_pointer ());
}
...
ModuleBase::Vector3<int>
    AtomPair<T>::get_R_index (const int& index) const
{
...
template <typename T>
T&
    AtomPair<T>::get_value (const int& i) const
{
#ifdef __DEBUG
...
template <typename T>
T&
    AtomPair<T>::get_value (const int& row, const int& col) const
{
#ifdef __DEBUG
...
template <typename T>
T*
    AtomPair<T>::get_pointer (int ir) const
{
    if (ir < 0)
        {
            ir = current_R;
...
template <typename T>
size_t
    AtomPair<T>::get_memory_size () const
{
