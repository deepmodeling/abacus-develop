...
template <typename T>
void
    Output_HContainer<T>::write (bool write_empty)
{
    _ofs << " #----------------------------------------------------------------------#" << std::endl;
...
template <typename T>
void
    Output_HContainer<T>::write (int rx_in, int ry_in, int rz_in)
{
    int size_for_loop_R = this->_hcontainer->size_R_loop ();
    int rx = 0;
    int ry = 0;
...
template <typename T>
void
    Output_HContainer<T>::write_single_R (int rx, int ry, int rz)
{
    if (this->_hcontainer->get_paraV () == nullptr)
        {
            ModuleBase::WARNING_QUIT ("Output_HContainer::write_single_R",
                                      "paraV is nullptr! Unable to write the matrix.");
        }
