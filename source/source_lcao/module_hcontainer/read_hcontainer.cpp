                    ...
                    for (const auto& element: sparse_matrix.getElements ())
                        {
                            ...
                            if (ij_pair == nullptr)
                                {
                                    // insert new pair
                                    hamilt::AtomPair<T> new_pair (atom_i,
                                                                  atom_j,
                                                                  RCoord[0],
                                                                  RCoord[1],
                                                                  RCoord[2],
                    ...
                    for (const auto& element: sparse_matrix.getElements ())
                        {
                            ...
                            int atom_j = atom_index_col[col];
                            auto* matrix
                                = hcontainer_serial.find_matrix (atom_i, atom_j, RCoord[0], RCoord[1], RCoord[2]);
    ...
    if (my_rank != 0)
        {
            ...
            MPI_Recv (tmp_ijrs.data (), tmp_ijrs.size (), MPI_INT, 0, 1, MPI_COMM_WORLD, &status);
