...
template <typename T>
HContainer<T>&
    HContainer<T>::operator= (HContainer<T>&& HR_in) noexcept
{
    if (this != &HR_in)
        {
            this->atom_pairs = std::move (HR_in.atom_pairs);
                    ...
                    for (int j = 0; j < ucell_.nat; j++)
                        {
                            ...
                            this->atom_pairs[i * ucell_.nat + j]
                                = AtomPair<T> (i, j, atom_begin_row.data (), atom_begin_col.data (), ucell_.nat);
...
template <typename T>
void
    HContainer<T>::allocate (T* data_array, bool is_zero)
{
...
template <typename T>
AtomPair<T>*
    HContainer<T>::find_pair (int atom_i, int atom_j) const
{
    ...
    if (it != this->sparse_ap[atom_i].end () && *it == atom_j)
        {
            AtomPair<T>* tmp_pointer = const_cast<AtomPair<T>*> (
                &this->atom_pairs[this->sparse_ap_index[atom_i][it - this->sparse_ap[atom_i].begin ()]]);
            return tmp_pointer;
        }
    else
        {
...
template <typename T>
const BaseMatrix<T>*
    HContainer<T>::find_matrix (int atom_i, int atom_j, int rx, int ry, int rz) const
{
...
template <typename T>
BaseMatrix<T>*
    HContainer<T>::find_matrix (int atom_i, int atom_j, int rx, int ry, int rz)
{
    AtomPair<T>* tmp = this->find_pair (atom_i, atom_j);
    if (tmp == nullptr)
        {
...
template <typename T>
BaseMatrix<T>*
    HContainer<T>::find_matrix (int atom_i, int atom_j, const ModuleBase::Vector3<int>& R_index)
{
    AtomPair<T>* tmp = this->find_pair (atom_i, atom_j);
    if (tmp == nullptr)
        {
...
template <typename T>
const BaseMatrix<T>*
    HContainer<T>::find_matrix (int atom_i, int atom_j, const ModuleBase::Vector3<int>& R_index) const
{
    AtomPair<T>* tmp = this->find_pair (atom_i, atom_j);
    if (tmp == nullptr)
        {
...
template <typename T>
int
    HContainer<T>::find_matrix_offset (int atom_i, int atom_j, int rx, int ry, int rz) const
{
...
template <typename T>
int
    HContainer<T>::find_matrix_offset (int atom_i, int atom_j, const ModuleBase::Vector3<int>& R_index) const
{
...
template <typename T>
AtomPair<T>&
    HContainer<T>::get_atom_pair (int atom_i, int atom_j) const
{
    if (atom_i >= this->sparse_ap.size ())
        {
            ModuleBase::WARNING_QUIT ("HContainer::insert_pair", "atom_i out of range");
    ...
    if (it != this->sparse_ap[atom_i].end () && *it == atom_j)
        {
            AtomPair<T>* tmp = const_cast<AtomPair<T>*> (
                &this->atom_pairs[this->sparse_ap_index[atom_i][it - this->sparse_ap[atom_i].begin ()]]);
            return *tmp;
        }
    else
...
template <typename T>
AtomPair<T>&
    HContainer<T>::get_atom_pair (int index) const
{
#ifdef __DEBUG
    if (this->current_R > -1)
        {
...
template <typename T>
void
    HContainer<T>::add (const HContainer<T>& other)
{
...
template <typename T>
bool
    HContainer<T>::fix_R (int rx_in, int ry_in, int rz_in) const
{
...
template <typename T>
int
    HContainer<T>::find_R (const int& rx_in, const int& ry_in, const int& rz_in) const
{
    ...
    for (int i = 0; i < this->tmp_R_index.size (); i++)
        {
            if (this->tmp_R_index[i].x == rx_in && this->tmp_R_index[i].y == ry_in && this->tmp_R_index[i].z == rz_in)
                {
                    return i;
                }
        }
...
template <typename T>
int
    HContainer<T>::find_R (const ModuleBase::Vector3<int>& R_in) const
{
...
template <typename T>
size_t
    HContainer<T>::size_R_loop () const
{
    // R index is fixed
...
template <typename T>
void
    HContainer<T>::loop_R (const size_t& index, int& rx, int& ry, int& rz) const
{
#ifdef __DEBUG
...
template <typename T>
size_t
    HContainer<T>::size_atom_pairs () const
{
    // R index is fixed
    if (this->current_R > -1)
        {
...
template <typename T>
T*
    HContainer<T>::data (int atom_i, int atom_j) const
{
...
template <typename T>
T*
    HContainer<T>::data (int atom_i, int atom_j, int* R_pointer) const
{
    AtomPair<T>* atom_ij = this->find_pair (atom_i, atom_j);
    if (atom_ij != nullptr)
        {
...
template <typename T>
void
    HContainer<T>::insert_pair (const AtomPair<T>& atom_ij)
{
    ...
    if (it != this->sparse_ap[atom_i].end () && *it == atom_j)
        {
            // 2. merge atom_ij
            this->atom_pairs[this->sparse_ap_index[atom_i][it - this->sparse_ap[atom_i].begin ()]].merge (
                atom_ij,
                this->gamma_only);
        }
    else
        {
            ...
            if (this->paraV != atom_ij.get_paraV ())
                {
                    ModuleBase::WARNING_QUIT ("HContainer::insert_pair",
                                              "atom_ij has different paraV pointer as HContainer");
                }
            else
                    ...
                    if (it != this->sparse_ap[atom_i].end ())
                        {
                            ...
                            this->sparse_ap_index[atom_i].insert (this->sparse_ap_index[atom_i].begin () + index,
                                                                  this->atom_pairs.size () - 1);
                        }
                    else
                        {
...
template <typename T>
int
    HContainer<T>::get_current_R () const
{
...
template <typename T>
bool
    HContainer<T>::is_gamma_only () const
{
...
template <typename T>
size_t
    HContainer<T>::get_memory_size () const
{
...
template <typename T>
void
    HContainer<T>::shape_synchron (const HContainer<T>& other)
{
    // check paraV pointer
...
template <typename T>
std::vector<int>
    HContainer<T>::get_ijr_info () const
{
...
template <typename T>
void
    HContainer<T>::insert_ijrs (const std::vector<int>* ijrs)
{
    ...
    for (int i = 0; i < number_ap; ++i)
        {
            ...
            if (this->paraV->atom_begin_row[atom_i] == -1 || this->paraV->atom_begin_col[atom_j] == -1)
                {
...
template <typename T>
void
    HContainer<T>::insert_ijrs (const std::vector<int>* ijrs, const UnitCell& ucell, const int npol)
{
