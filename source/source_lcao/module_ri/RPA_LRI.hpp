...
void RPA_LRI<T, Tdata>::cal_postSCF_exx(const elecstate::DensityMatrix<T, Tdata>& dm,
    ...
    GlobalC::exx_info.info_global.ccp_type =
        Conv_Coulomb_Pot_K::Ccp_Type::Hf; // not used now, Hf/Ccp -> singularity_correction, see conv_coulomb_pot_k.cpp
    ...
    if (GlobalC::exx_info.info_ri.shrink_abfs_pca_thr >= 0.0) {
        ...
        if (this->info.files_shrink_abfs.empty()) {
            ...
            this->abfs_shrink =
                Exx_Abfs::IO::construct_abfs(abfs_same_atom, orb, this->info.files_shrink_abfs, this->info.kmesh_times);
        }
