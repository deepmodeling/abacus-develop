...
void
    Matrix_Orbs11::init (const std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>>& orb_A,
                         ...
                         const double kmesh_times)
{
    ...
    const int Lmax
        = std::max ({Exx_Abfs::Construct_Orbs::get_Lmax (orb_A), Exx_Abfs::Construct_Orbs::get_Lmax (orb_B)});
    ...
    const double rmax = Exx_Abfs::Construct_Orbs::get_Rmax (orb_A) + Exx_Abfs::Construct_Orbs::get_Rmax (orb_B);
                                            ...
                                            for (size_t NB = 0; NB != orb_B[TB][LB].size (); ++NB)
                                                {
                                                    center2_orb11_s[TA][TB][LA][NA][LB].insert (
                                                        std::make_pair (NB,
                                                                        Center2_Orb::Orb11 (orb_A[TA][LA][NA],
                                                                                            orb_B[TB][LB][NB],
                                                                                            psb_,
                                                                                            *this->MGT)));
                                                }
...
void
    Matrix_Orbs11::init_radial_table ()
{
...
void
    Matrix_Orbs11::init_radial_table (const std::map<size_t, std::map<size_t, std::set<double>>>& Rs)
{
            ...
            for (const auto& RsB: RsA.second)
                {
                    if (auto* const center2_orb11_sAB = static_cast<
                            std::map<int,
                                     std::map<size_t, std::map<int, std::map<size_t, Center2_Orb::Orb11>>>>* const> (
