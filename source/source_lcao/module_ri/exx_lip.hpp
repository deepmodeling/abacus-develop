    ...
    for (int ik = 0; ik < this->k_pack->kv_ptr->get_nks (); ++ik)
        {
            ...
            for (int iq_tmp = this->iq_vecik;
                 iq_tmp < this->iq_vecik + this->q_pack->kv_ptr->get_nks () / PARAM.inp.nspin;
                 ++iq_tmp) // !!! k_point
                           // parallel incompleted.need to loop iq in other pool
                {
                    const int iq = (ik < (this->k_pack->kv_ptr->get_nks () / PARAM.inp.nspin))
                                       ? (iq_tmp % (this->q_pack->kv_ptr->get_nks () / PARAM.inp.nspin))
                                       : (iq_tmp % (this->q_pack->kv_ptr->get_nks () / PARAM.inp.nspin)
                                          + (this->q_pack->kv_ptr->get_nks () / PARAM.inp.nspin));
                    this->qkg2_exp (ik, iq);
    ...
    : info (info_in)
{
    ...
    this->k_pack->hvec_array = new psi::Psi<T, Device> (this->k_pack->kv_ptr->get_nks (),
                                                        PARAM.inp.nbands,
...
Exx_Lip<T, Device>::~Exx_Lip ()
{
    if (this->k_pack)
        {
            delete this->k_pack->hvec_array;
            this->k_pack->hvec_array = nullptr;
        }
    delete this->k_pack;
    this->k_pack = nullptr;
    ...
    if (PARAM.inp.init_chg == "atomic")
        ...
        {
            delete this->q_pack->kv_ptr;
            this->q_pack->kv_ptr = nullptr;
...
template <typename T, typename Device>
void
    Exx_Lip<T, Device>::phi_cal (k_package* kq_pack, const int ikq)
{
                    ...
                    for (int iy = 0; iy < this->rho_basis->ny; ++iy)
                        {
                            ...
                            for (int iz = this->rho_basis->startz_current;
                                 iz < this->rho_basis->startz_current + this->rho_basis->nplane;
                                 ++iz)
                                {
                                    const Treal phase_xyz
                                        = phase_xy + kq_pack->kv_ptr->kvec_d[ikq].z * iz / this->rho_basis->nz;
            ...
            for (int iq = 0; iq < this->q_pack->kv_ptr->get_nks (); ++iq)
                {
                    for (int ib = 0; ib < PARAM.inp.nbands; ++ib)
                        {
                            this->wfc_basis->recip2real (&(this->q_pack->kspw_psi_ptr->operator() (iq, ib, 0)),
                                                         porter.data (),
                                                         iq);
                            ...
                            for (int ix = 0; ix < this->rho_basis->nx; ++ix)
                                {
                                    ...
                                    for (int iy = 0; iy < this->rho_basis->ny; ++iy)
                                        {
                                            const Treal phase_xy
                                                = phase_x
                                                  + this->q_pack->kv_ptr->kvec_d[iq].y * iy / this->rho_basis->ny;
                                            for (int iz = this->rho_basis->startz_current;
                                                 iz < this->rho_basis->startz_current + this->rho_basis->nplane;
                                                 ++iz)
                                                {
                                                    const Treal phase_xyz = phase_xy
                                                                            + this->q_pack->kv_ptr->kvec_d[iq].z * iz
                                                                                  / this->rho_basis->nz;
                                                    const T exp_tmp = std::exp (phase_xyz * this->two_pi_i);
                            ...
                            for (int iw = 0; iw < PARAM.globalv.nlocal; ++iw)
                                {
                                    for (int ir = 0; ir < this->rho_basis->nrxx; ++ir)
                                        {
                                            this->psi[iq][ib][ir]
                                                += (*this->q_pack->hvec_array) (iq, ib, iw) * this->phi[iw][ir];
                                        }
                                }
                        }
                }
...
template <typename T, typename Device>
void
    Exx_Lip<T, Device>::judge_singularity (const int ik)
{
    ...
    if (PARAM.inp.init_chg == "atomic")
    ...
    else if (PARAM.inp.init_chg == "file")
        {
            ...
            for (int iq = 0; iq < this->q_pack->kv_ptr->get_nks (); ++iq)
                {
                    const Treal q_minus_k (
                        (this->q_pack->kv_ptr->kvec_c[iq] - this->k_pack->kv_ptr->kvec_c[ik]).norm2 ());
...
template <typename T, typename Device>
void
    Exx_Lip<T, Device>::qkg2_exp (const int ik, const int iq)
{
    ModuleBase::timer::start ("Exx_Lip", "qkg2_exp");
    for (int ig = 0; ig < this->rho_basis->npw; ++ig)
        {
            const Treal qkg2
                = ((this->q_pack->kv_ptr->kvec_c[iq] - this->k_pack->kv_ptr->kvec_c[ik] + this->rho_basis->gcar[ig])
                   * (ModuleBase::TWO_PI / this->ucell_ptr->lat0))
                      .norm2 ();
            if (Conv_Coulomb_Pot_K::Ccp_Type::Ccp == info.ccp_type || Conv_Coulomb_Pot_K::Ccp_Type::Hf == info.ccp_type)
                {
                    if (std::abs (qkg2) < 1e-10)
                        {
                            this->recip_qkg2[ig] = 0.0;
                        } // 0 to ignore bb/qkg2 when qkg2==0
                    else
                        {
            ...
            else if (Conv_Coulomb_Pot_K::Ccp_Type::Erfc == info.ccp_type)
                {
                    if (std::abs (qkg2) < 1e-10)
                        ...
                        {
                            this->recip_qkg2[ig]
                                = sqrt ((1 - std::exp (-qkg2 / (4 * info.hse_omega * info.hse_omega))) / qkg2);
                        }
                }
            else
                {
...
template <typename T, typename Device>
void
    Exx_Lip<T, Device>::b_cal (const int ik, const int iq, const int ib)
{
            ...
            for (size_t iy = 0; iy < this->rho_basis->ny; ++iy)
                {
                    ...
                    for (size_t iz = this->rho_basis->startz_current;
                         iz < this->rho_basis->startz_current + this->rho_basis->nplane;
                         ++iz)
                        {
...
template <typename T, typename Device>
void
    Exx_Lip<T, Device>::sum3_cal (const int iq, const int ib)
{
            ...
            for (int iw_l = 0; iw_l < PARAM.globalv.nlocal; ++iw_l)
                {
                    for (int iw_r = 0; iw_r < PARAM.globalv.nlocal; ++iw_r)
                        {
                            this->sum3[iw_l][iw_r]
                                += this->b0[iw_l] * conj (this->b0[iw_r]) * (Treal)this->q_pack->wf_wg (iq, ib);
                        }
                }
        }
...
template <typename T, typename Device>
void
    Exx_Lip<T, Device>::b_sum (const int iq, const int ib) // Peize Lin change 2019-04-14
{
...
template <typename T, typename Device>
void
    Exx_Lip<T, Device>::sum_all (const int ik)
{
    ...
    for (size_t iw_l = 1; iw_l < PARAM.globalv.nlocal; ++iw_l)
        {
            for (size_t iw_r = 0; iw_r < iw_l; ++iw_r)
                {
                    this->sum1[iw_l * PARAM.globalv.nlocal + iw_r]
                        = conj (this->sum1[iw_r * PARAM.globalv.nlocal + iw_l]); // Peize Lin add conj 2019-04-14
                }
        }
    ...
    for (int iw_l = 0; iw_l < PARAM.globalv.nlocal; ++iw_l)
        {
            for (int iw_r = 0; iw_r < PARAM.globalv.nlocal; ++iw_r)
                {
                    this->exx_matrix[ik][iw_l][iw_r]
                        = -fourpi_div_omega * this->sum1[iw_l * PARAM.globalv.nlocal + iw_r] * spin_fac;
                    if (Conv_Coulomb_Pot_K::Ccp_Type::Ccp == info.ccp_type
                        || Conv_Coulomb_Pot_K::Ccp_Type::Hf == info.ccp_type)
                        {
                            if (gzero_rank_in_pool == GlobalV::RANK_IN_POOL)
                                {
                                    this->exx_matrix[ik][iw_l][iw_r]
                                        += spin_fac * (fourpi_div_omega * this->sum3[iw_l][iw_r] * sum2_factor_g);
                                    this->exx_matrix[ik][iw_l][iw_r]
                                        += spin_fac
                                           * (-1 / (Treal)sqrt (info.lambda * ModuleBase::PI)
                                              * (Treal)(this->q_pack->kv_ptr->get_nks () / PARAM.inp.nspin)
                                              * this->sum3[iw_l][iw_r]);
                                }
                        }
                }
...
template <typename T, typename Device>
void
    Exx_Lip<T, Device>::exx_energy_cal ()
{
                    ...
                    for (int iw_r = 0; iw_r < PARAM.globalv.nlocal; ++iw_r)
                        {
                            for (int ib = 0; ib < PARAM.inp.nbands; ++ib)
                                {
                                    exx_energy_tmp += (this->exx_matrix[ik][iw_l][iw_r]
                                                       * conj ((*this->k_pack->hvec_array) (ik, ib, iw_l))
                                                       * (*this->k_pack->hvec_array) (ik, ib, iw_r))
                                                          .real ()
                                                      * this->k_pack->wf_wg (ik, ib);
                                }
                        }
                }
        }
...
template <typename T, typename Device>
void
    Exx_Lip<T, Device>::write_q_pack () const
{
                    ...
                    for (int iw = 0; iw < PARAM.globalv.nlocal; ++iw)
                        {
                            for (int ib = 0; ib < PARAM.inp.nbands; ++ib)
                                {
                                    ofs_hvec << (*this->q_pack->hvec_array) (iq, ib, iw).real () << " "
                          ...
                          const Structure_Factor& sf)
{
    ...
    this->q_pack->kv_ptr->set( symm, exx_kpoint_card, PARAM.inp.nspin, this->ucell_ptr->G, this->ucell_ptr->latvec,
GlobalV::ofs_running ); this->q_pack->wf_ptr = new wavefunc();
...
this->q_pack->wf_ptr->init(this->q_pack->kv_ptr->get_nks(),this->q_pack->kv_ptr,this->ucell_ptr,old_pwptr,&ppcell,&GlobalC::ORB,&hm,&Pkpoints);
    ...
    // this->q_pack->wf_ptr->table_local.create(this->q_pack->wf_ptr->this->ucell_ptr->ntype,
this->q_pack->wf_ptr->this->ucell_ptr->nmax_total, PARAM.globalv.nqx); #ifdef __LCAO
    Wavefunc_in_pw::make_table_q(GlobalC::ORB.orbital_file, this->q_pack->wf_ptr->table_local);
    // Wavefunc_in_pw::make_table_q(this->q_pack->wf_ptr->ORB_ptr->orbital_file, this->q_pack->wf_ptr->table_local,
this->q_pack->wf_ptr); for(int iq=0; iq<this->q_pack->kv_ptr->get_nks(); ++iq)
    {
        ...
        //		Wavefunc_in_pw::produce_local_basis_in_pw(iq, this->q_pack->wf_ptr->wanf2[iq],
this->q_pack->wf_ptr->table_local,
        // this->q_pack->wf_ptr);
    }
  #endif
    ...
    for( int iq=0; iq<this->q_pack->kv_ptr->get_nks(); ++iq)
    {
        MPI_Bcast( this->q_pack->hvec_array[iq].c, PARAM.globalv.nlocal*PARAM.inp.nbands, MPI_DOUBLE_COMPLEX, 0,
POOL_WORLD);
    }
    #endif
    return;
