...
void Exx_Opt_Orb::generate_matrix(const Exx_Info::Exx_Info_Opt_ABFs& info,
    ...
    std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>> lcaos =
        Exx_Abfs::Construct_Orbs::change_orbs(orb, info.kmesh_times);
    Exx_Abfs::Construct_Orbs::filter_empty_orbs(lcaos);

    std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>> abfs =
        Exx_Abfs::Construct_Orbs::abfs_same_atom(ucell, orb, lcaos, info.kmesh_times, info.pca_threshold);
    ...
    std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>> jle =
        Exx_Abfs::Jle::init_jle(info, info.kmesh_times, ucell, orb);
    ...
    const ModuleBase::Element_Basis_Index::IndexLNM index_jys =
        ModuleBase::Element_Basis_Index::construct_index(range_jys);
    ...
    const auto ms_lcaoslcaos_lcaoslcaos =
        [&]() -> std::map<size_t, std::map<size_t, std::map<size_t, std::map<size_t, RI::Tensor<double>>>>> {
        if (judge_orbs_empty(lcaos)) {
            return {};
        }
...
#endif
        return m_lcaoslcaos_lcaoslcaos.cal_overlap_matrix_all<double>(ucell,
                                                                      index_lcaos,
                                                                      index_lcaos,
                                                                      index_lcaos,
                                                                      index_lcaos);
    ...
    // < lcaos lcaos | jys >
    const auto ms_lcaoslcaos_jys = [&]()
        -> std::map<size_t, std::map<size_t, std::map<size_t, std::map<size_t, std::vector<RI::Tensor<double>>>>>> {
        if (judge_orbs_empty(lcaos)) {
            return {};
        }
        if (judge_orbs_empty(jle)) {
            return {};
        }
            ...
            for (size_t TB = 0; TB != ucell.ntype; ++TB) {
                for (size_t IB = 0; IB != ucell.atoms[TB].na; ++IB) {
                    if (TA == TB && IA == IB) {
                        ...
                        if (!judge_orbs_empty(abfs)) {
                            // < abfs | abfs >.I
                            const std::vector<std::vector<RI::Tensor<double>>> ms_abfs_abfs_I =
                                cal_I(ms_abfs_abfs, T, I, T, I);
                            // < lcaos lcaos | lcaos lcaos > - < lcaos lcaos | abfs > * < abfs | abfs >.I * < abfs |
                            // lcaos lcaos >
                            const RI::Tensor<double> m_lcaoslcaos_lcaoslcaos_proj =
                                ms_lcaoslcaos_lcaoslcaos.at(T).at(I).at(T).at(I) -
                                cal_mul_22(ms_lcaoslcaos_abfs.at(T).at(I).at(T).at(I),
                                           ms_abfs_abfs_I,
                            ...
                            const std::vector<RI::Tensor<double>> m_lcaoslcaos_jys_proj = {
                                ...
                                cal_mul_21(ms_lcaoslcaos_abfs.at(T).at(I).at(T).at(I),
                                           ms_abfs_abfs_I,
                            ...
                            const std::vector<std::vector<RI::Tensor<double>>> m_jys_jys_proj = {
                                {ms_jys_jys.at(T).at(I).at(T).at(I) -
                                 cal_mul_11({ms_jys_abfs.at(T).at(I).at(T).at(I)},
                                            ms_abfs_abfs_I,
                                            {ms_jys_abfs.at(T).at(I).at(T).at(I)})}};
                            print_matrix(info,
                                         ucell,
                        ...
                        if (!judge_orbs_empty(abfs)) {
                            // < abfs | abfs >.I
                            const std::vector<std::vector<RI::Tensor<double>>> ms_abfs_abfs_I =
                                cal_I(ms_abfs_abfs, TA, IA, TB, IB);
                            // < lcaos lcaos | lcaos lcaos > - < lcaos lcaos | abfs > * < abfs | abfs >.I * < abfs |
                            // lcaos lcaos >
                            const RI::Tensor<double> m_lcaoslcaos_lcaoslcaos_proj =
                                ms_lcaoslcaos_lcaoslcaos.at(TA).at(IA).at(TB).at(IB) -
                                cal_mul_22(ms_lcaoslcaos_abfs.at(TA).at(IA).at(TB).at(IB),
                                           ms_abfs_abfs_I,
                                           ms_lcaoslcaos_abfs.at(TA).at(IA).at(TB).at(IB));
                            // < lcaos lcaos | jys > - < lcaos lcaos | abfs > * < abfs | abfs >.I * < abfs | jys >
                            const std::vector<RI::Tensor<double>> m_lcaoslcaos_jys_proj = {
                                ms_lcaoslcaos_jys.at(TA).at(IA).at(TB).at(IB)[0] -
                                    cal_mul_21(ms_lcaoslcaos_abfs.at(TA).at(IA).at(TB).at(IB),
                                               ms_abfs_abfs_I,
                                               {ms_jys_abfs.at(TA).at(IA).at(TA).at(IA),
                                                ms_jys_abfs.at(TA).at(IA).at(TB).at(IB)}),
                                ms_lcaoslcaos_jys.at(TA).at(IA).at(TB).at(IB)[1] -
                                    cal_mul_21(ms_lcaoslcaos_abfs.at(TA).at(IA).at(TB).at(IB),
                                               ms_abfs_abfs_I,
                                               {ms_jys_abfs.at(TB).at(IB).at(TA).at(IA),
                                                ms_jys_abfs.at(TB).at(IB).at(TB).at(IB)})};
                            // < jys | jys > - < jys | abfs > * < abfs | abfs >.I * < abfs | jys >
                            const std::vector<std::vector<RI::Tensor<double>>> m_jys_jys_proj = {
                                {ms_jys_jys.at(TA).at(IA).at(TA).at(IA) -
                                     cal_mul_11({ms_jys_abfs.at(TA).at(IA).at(TA).at(IA),
                                                 ms_jys_abfs.at(TA).at(IA).at(TB).at(IB)},
                                                ms_abfs_abfs_I,
                                                {ms_jys_abfs.at(TA).at(IA).at(TA).at(IA),
                                                 ms_jys_abfs.at(TA).at(IA).at(TB).at(IB)}),
                                ...
                                {ms_jys_jys.at(TB).at(IB).at(TA).at(IA) -
                                     cal_mul_11({ms_jys_abfs.at(TB).at(IB).at(TA).at(IA),
                                                 ms_jys_abfs.at(TB).at(IB).at(TB).at(IB)},
                                                ms_abfs_abfs_I,
                                                {ms_jys_abfs.at(TA).at(IA).at(TA).at(IA),
                                                 ms_jys_abfs.at(TA).at(IA).at(TB).at(IB)}),
...
RI::Tensor<double> Exx_Opt_Orb::cal_mul_22(const std::vector<RI::Tensor<double>>& m_left,
...
RI::Tensor<double> Exx_Opt_Orb::cal_mul_21(const std::vector<RI::Tensor<double>>& m_left,
                                           const std::vector<std::vector<RI::Tensor<double>>>& m_middle,
                                           const std::vector<RI::Tensor<double>>& m_right) const {
...
RI::Tensor<double> Exx_Opt_Orb::cal_mul_12(const std::vector<RI::Tensor<double>>& m_left,
                                           const std::vector<std::vector<RI::Tensor<double>>>& m_middle,
                                           const std::vector<RI::Tensor<double>>& m_right) const {
...
RI::Tensor<double> Exx_Opt_Orb::cal_mul_11(const std::vector<RI::Tensor<double>>& m_left,
                                           const std::vector<std::vector<RI::Tensor<double>>>& m_middle,
                                           const std::vector<RI::Tensor<double>>& m_right) const {
...
std::vector<std::vector<RI::Tensor<double>>>
Exx_Opt_Orb::cal_I(const std::map<size_t, std::map<size_t, std::map<size_t, std::map<size_t, RI::Tensor<double>>>>>& ms,
                   const size_t TA,
                   const size_t IA,
                   const size_t TB,
                   const size_t IB) const {
    ...
    if (TA == TB && IA == IB) {
        ...
        std::vector<std::vector<RI::Tensor<double>>> m_in{
            {ms.at(TA).at(IA).at(TA).at(IA), ms.at(TA).at(IA).at(TB).at(IB)},
            {ms.at(TB).at(IB).at(TA).at(IA), ms.at(TB).at(IB).at(TB).at(IB)}};
