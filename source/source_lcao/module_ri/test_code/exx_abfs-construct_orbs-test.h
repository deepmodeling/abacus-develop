...
static void print_orbs(const std::vector<std::vector<std::vector<std::vector<double>>>>& orbs,
...
static void print_orbs(const std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>>& orbs,
                       const std::string& file_name) {
    std::ofstream ofsN(file_name);
    for (size_t T = 0; T != orbs.size(); ++T)
        for (size_t L = 0; L != orbs[T].size(); ++L)
