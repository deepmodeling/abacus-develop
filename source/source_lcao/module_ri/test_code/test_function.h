...
#ifdef __MPI
static void
    MPI_RANK_OFSTREAM (const std::string& file_name, std::stringstream& content)
{
...
static std::ostream&
    operator<< (std::ostream& os, const std::array<T, N>& v)
{
    os << "[";
    for (const T& i: v)
        os << i << "\t";
...
template <typename T1, typename T2>
static std::ostream&
    operator<< (std::ostream& os, const std::map<T1, T2>& v)
{
    for (const auto& i: v)
        //		os<<"{"<<i.first<<":"<<i.second<<"}"<<"\t";
...
static double
    cal_time (const timeval& t_begin)
{
    timeval t_end;
