    ...
    for (auto& tmp1: HR_irreducible) {
        ...
        for (auto& tmp2: tmp1.second) {
            ...
            if (this->irs_.sector_stars_.find(irapR) != this->irs_.sector_stars_.end()) {
                ...
                std::cout << "Warning: not found: irreducible atom pair =(" << irap1 << "," << irap2 << "), irR=("
                          << irR[0] << "," << irR[1] << "," << irR[2] << ")\n";
            }
        }
    }
...
template <typename Tdata>
inline void set_block(const int starti,
                      const int startj,
                      const RI::Tensor<std::complex<double>>& block,
                      RI::Tensor<Tdata>& obj_tensor) { // no changing row/col order
...
template <typename Tdata>
RI::Tensor<Tdata> Symmetry_rotation::rotate_atompair_serial(
    const RI::Tensor<Tdata>& A,
    const int isym,
    const Atom& a1,
    const Atom& a2,
    const char mode,
    const bool output) const { // due to col-contiguous, actually what we know is T^T and H^T (or D^T),
...
template <typename Tdata>
void Symmetry_rotation::rotate_atompair_serial(
    Tdata* TAT,
    const Tdata* A,
    const int& nw1,
    const int& nw2,
    ...
    const Atom& a1,
    const Atom& a2,
    const char mode) const { // due to col-contiguous, actually what we know is T^T and H^T (or D^T),
    // and what we calculate is(H'^T = T ^ T * H ^ T * T^*) or (D'^T = T ^ \dagger * D ^ T * T)
    assert(mode == 'H' || mode == 'D');
    bool sametype = (a1.label == a2.label);
...
template <typename Tdata>
void Symmetry_rotation::print_HR(const std::map<int, std::map<std::pair<int, TC>, RI::Tensor<Tdata>>>& HR,
...
template <typename Tdata>
void Symmetry_rotation::test_HR_rotation(
    const Symmetry& symm,
    const Atom* atoms,
    const Statistics& st,
    const char mode,
...
template <typename Tdata>
void Symmetry_rotation::test_Cs_rotation(
    const Symmetry& symm,
    const Atom* atoms,
    const Statistics& st,
    ...
    for (auto& sector_pair: this->irs_.full_map_to_irreducible_sector_) {
        ...
        if (apR.first != irapR.first) {
            std::cout << "irapR=(" << irapR.first.first << "," << irapR.first.second << "), (" << irapR.second[0] << ","
                      << irapR.second[1] << "," << irapR.second[2] << "):\n";
