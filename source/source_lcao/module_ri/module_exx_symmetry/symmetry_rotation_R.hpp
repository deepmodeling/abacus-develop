...
inline void
    print_tensor (const RI::Tensor<Tdata>& t, const std::string& name, const double& threshold = 0.0)
{
...
template <typename Tdata>
inline void
    print_tensor3 (const RI::Tensor<Tdata>& t, const std::string& name, const double& threshold = 0.0)
{
    GlobalV::ofs_running << name << ":\n";
            ...
            for (int i = 0; i < t.shape[1]; ++i)
                {
                    for (int j = 0; j < t.shape[2]; ++j)
                        {
                            GlobalV::ofs_running
                                << ((std::abs (t (a, i, j)) > threshold) ? t (a, i, j) : static_cast<Tdata> (0)) << " ";
                        }
                    GlobalV::ofs_running << std::endl;
                }
            GlobalV::ofs_running << std::endl;
...
template <typename Tdata>
std::map<int, std::map<std::pair<int, TC>, RI::Tensor<Tdata>>>
    Symmetry_rotation::restore_HR (
        const Symmetry& symm,
        const Atom* atoms,
        const Statistics& st,
        const char mode,
        const std::map<int, std::map<std::pair<int, TC>, RI::Tensor<Tdata>>>& HR_irreducible) const
{
                            ...
                            for (auto& isym_apR: this->irs_.sector_stars_.at (irapR))
                                {
                                    ...
                                    HR_full[ap1][{ap2, R}] = rotate_atompair_serial (tmp2.second,
                                                                                     isym,
                                                                                     atoms[st.iat2it[irap1]],
                                                                                     atoms[st.iat2it[irap2]],
                                                                                     mode);
                                }
                        }
                    else
                        {
                            std::cout << "Warning: not found: irreducible atom pair =(" << irap1 << "," << irap2
                                      << "), irR=(" << irR[0] << "," << irR[1] << "," << irR[2] << ")\n";
                        }
                }
        }
    ...
    //     if (HR_irreducible.find(irap.first) != HR_irreducible.end() && HR_irreducible.at(irap.first).find({
    //     irap.second, irR }) != HR_irreducible.at(irap.first).end())
    //         HR_full[ap.first][{ap.second, R}] = rotate_atompair_serial(HR_irreducible.at(irap.first).at({
    //         irap.second, irR }),
    //             isym, atoms[st.iat2it[irap.first]], atoms[st.iat2it[irap.second]], mode);
    //     else
    //         std::cout << "not found: current atom pair =(" << ap.first << "," << ap.second << "), R=(" << R[0] << ","
    //         << R[1] << "," << R[2] << "), irreducible atom pair =(" << irap.first << "," << irap.second << "), irR=("
    //         << irR[0] << "," << irR[1] << "," << irR[2] << ")\n";
    // }
    ...
    //         print_tensor(tmp2.second, "HR_irreducible (" + std::to_string(a1) + ", " + std::to_string(a2) + "), R=("
    ...
    //         const int& a2 = tmp2.first.first;
    //         const TC& R = tmp2.first.second;
    //         print_tensor(tmp2.second, "HR_full (" + std::to_string(a1) + ", " + std::to_string(a2) + "), R=(" +
    //         std::to_string(R[0]) + " " + std::to_string(R[1]) + " " + std::to_string(R[2]) + ")");
    //     }
    // }
...
template <typename Tdata>
RI::Tensor<Tdata>
    Symmetry_rotation::set_rotation_matrix (const Atom& a, const int& isym) const
{
...
template <typename Tdata>
RI::Tensor<Tdata>
    Symmetry_rotation::rotate_atompair_serial (const RI::Tensor<Tdata>& A,
                                               const int isym,
                                               const Atom& a1,
                                               const Atom& a2,
                                               const char mode,
...
template <typename Tdata>
void
    Symmetry_rotation::rotate_atompair_serial (Tdata* TAT,
                                               const Tdata* A,
                                               const int& nw1,
                                               const int& nw2,
                                               const int isym,
...
template <typename Tdata>
RI::Tensor<Tdata>
    Symmetry_rotation::set_rotation_matrix_abf (const int& type, const int& isym) const
{
    int nabfs = 0;
    for (int l = 0; l < this->abfs_l_nchi_[type].size (); ++l)
        {
            nabfs += this->abfs_l_nchi_[type][l] * (2 * l + 1);
        }
...
template <typename Tdata>
RI::Tensor<Tdata>
    Symmetry_rotation::rotate_singleC_serial (const RI::Tensor<Tdata>& C,
                                              const int isym,
                                              const Atom& a1,
                                              const Atom& a2,
                                              const int& type1,
                                              bool output) const
{
    ...
    for (int iabf = 0; iabf < C.shape[0]; ++iabf)
        {
            this->rotate_atompair_serial (Cout.ptr () + iabf * slice_size,
                                          C.ptr () + iabf * slice_size,
                                          a1.nw,
                                          a2.nw,
                                          isym,
                                          a1,
...
template <typename Tdata>
void
    Symmetry_rotation::print_HR (const std::map<int, std::map<std::pair<int, TC>, RI::Tensor<Tdata>>>& HR,
...
template <typename Tdata>
void
    Symmetry_rotation::test_HR_rotation (const Symmetry& symm,
                                         const Atom* atoms,
                                         const Statistics& st,
                                         const char mode,
            ...
            for (auto& irR: irap_Rs.second)
                {
                    ...
                    HR_irreducible[irap.first][a2_irR]
                        = (HR_full.at (irap.first).count (a2_irR) != 0)
                              ? HR_full.at (irap.first).at (a2_irR)
                              : RI::Tensor<Tdata> (HR_full.at (irap.first).begin ()->second.shape);
                }
        }
    // 2. rotate
    std::map<int, std::map<std::pair<int, TC>, RI::Tensor<Tdata>>> HR_rotated
            ...
            for (auto& HR_ia12R: HR_ia1.second)
                {
                    ...
                    if (HR_full.at (iat1).count ({iat2, R}) == 0) // rot back but not found
                        {
                            std::cout << "R_rot not found in atom pair (" << iat1 << ", " << iat2 << "):  R=(" << R[0]
                                      << "," << R[1] << "," << R[2] << "):\n";
                            continue;
                        }
...
template <typename Tdata>
void
    Symmetry_rotation::test_Cs_rotation (
        const Symmetry& symm,
        const Atom* atoms,
        const Statistics& st,
    ...
    for (auto& sector_pair: this->irs_.full_map_to_irreducible_sector_)
        {
            ...
            if (apR.first != irapR.first)
                {
                    std::cout << "irapR=(" << irapR.first.first << "," << irapR.first.second << "), ("
                              << irapR.second[0] << "," << irapR.second[1] << "," << irapR.second[2] << "):\n";
                    std::cout << "apR=(" << apR.first.first << "," << apR.first.second << "), (" << apR.second[0] << ","
                              << apR.second[1] << "," << apR.second[2] << "):\n";
                    const RI::Tensor<Tdata>& Cs_ir
                        = Cs_full.at (irapR.first.first).at ({irapR.first.second, irapR.second});
                    const RI::Tensor<Tdata>& Cs_ref = Cs_full.at (apR.first.first).at ({apR.first.second, apR.second});
                    const RI::Tensor<Tdata>& Cs_rot = this->rotate_singleC_serial (Cs_ir,
                                                                                   isym,
                                                                                   atoms[st.iat2it[irapR.first.first]],
                                                                                   atoms[st.iat2it[irapR.first.second]],
                                                                                   irapR.first.first);
