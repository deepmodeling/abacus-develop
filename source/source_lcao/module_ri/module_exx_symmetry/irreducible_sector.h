...
class Irreducible_Sector
{
  public:
    ...
    {
        bool
            operator() (const Tap& lhs, const Tap& rhs) const
        {
    ...
    {
        bool
            operator() (const TapR& lhs, const TapR& rhs) const
        {
            if (lhs.first < rhs.first)
                {
                    return true;
    ...
    {
        int
            norm2 (const TC& R) const
        {
            return R[0] * R[0] + R[1] * R[1] + R[2] * R[2];
        }
        bool
            operator() (const TC& lhs, const TC& rhs) const
        {
            if (norm2 (lhs) < norm2 (rhs))
                {
                    return true;
                }
            else if (norm2 (lhs) > norm2 (rhs))
                {
                    return false;
                }
            else
    ...
    {
        return this->irreducible_sector_;
    }
  ...
  protected:
    ...
    TC rotate_R (const Symmetry& symm,
                 const int isym,
                 const int iat1,
                 const int iat2,
                 const TC& R,
                 const char gauge = 'R') const;
    ...
    TCdouble get_aRb_direct (const Atom* atoms,
                             const Statistics& st,
                             const int iat1,
                             const int iat2,
                             const TC& R,
    ...
    void gen_symmetry_BvK (const Symmetry& symm,
                           const Atom* atoms,
                           const Lattice& lat,
                           const Statistics& st,
                           const TC bvk_period);
