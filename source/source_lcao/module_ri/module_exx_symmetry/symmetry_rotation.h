...
class Symmetry_rotation
{
  public:
    ...
    //--------------------------------------------------------------------------------
    // getters
    const std::map<Tap, std::set<TC>>&
        get_irreducible_sector () const
    {
        return this->irs_.get_irreducible_sector ();
    }
    TCdouble
        get_return_lattice (const Symmetry& symm,
    ...
    TCdouble
        get_return_lattice (const int iat, const int isym) const
    {
    ...
    // setters
    void
        find_irreducible_sector (const Symmetry& symm,
                                 const Atom* atoms,
                                 const Statistics& st,
    ...
    void set_block_to_mat2d (const int starti,
                             const int startj,
    ...
    std::vector<std::complex<double>> contruct_2d_rot_mat_ao (const Symmetry& symm,
                                                              const Atom* atoms,
    ...
    std::vector<std::vector<RI::Tensor<std::complex<double>>>>&
        get_rotmat_Slm ()
    {
        return this->rotmat_Slm_;
    }
    ...
    //--------------------------------------------------------------------------------
    /// test functions
    /// test H(R) rotation: giver a full H(R), pick out H(R) in the irreducible sector, rotate it, and compare with the
    /// original full H(R)
    template <typename Tdata> // RI::Tensor type, using col-major implementation
    void test_HR_rotation (const Symmetry& symm,
                           const Atom* atoms,
                           const Statistics& st,
                           const char mode,
    ...
    void test_Cs_rotation (const Symmetry& symm,
                           const Atom* atoms,
                           const Statistics& st,
    ...
    void print_HR (const std::map<int, std::map<std::pair<int, TC>, RI::Tensor<Tdata>>>& HR,
  ...
  private:
    ...
    std::vector<TC>
        get_Rs_from_adjacent_list (const UnitCell& ucell, const Grid_Driver& gd, const Parallel_Orbitals& pv) const;
    //--------------------------------------------------------------------------------
    ...
    RI::Tensor<Tdata> rotate_atompair_serial (const RI::Tensor<Tdata>& A,
                                              const int isym,
                                              const Atom& a1,
                                              const Atom& a2,
                                              const char mode,
    ...
    void rotate_atompair_serial (Tdata* TAT,
                                 const Tdata* A,
                                 const int& nw1,
                                 const int& nw2,
                                 const int isym,
    ...
    void rotate_atompair_parallel (const TR* Alocal_in,
                                   const int isym,
                                   const Atom* atoms,
                                   const Statistics& st,
                                   const Tap& ap_in,
    ...
    RI::Tensor<Tdata> rotate_singleC_serial (const RI::Tensor<Tdata>& C,
                                             const int isym,
                                             const Atom& a1,
                                             const Atom& a2,
                                             const int& type1,
...
template <typename T>
std::string
    vec3_fmt (const T& x, const T& y, const T& z)
{
    return "(" + std::to_string (x) + " " + std::to_string (y) + " " + std::to_string (z) + ")";
}
template <typename T>
std::string
    vec3_fmt (const ModuleBase::Vector3<T>& v)
{
...
void print_symrot_info_R (const Symmetry_rotation& symrot,
                          const Symmetry& symm,
                          const int lmax_ao,
                          const std::vector<TC>& Rs);
} // namespace ModuleSymmetry
