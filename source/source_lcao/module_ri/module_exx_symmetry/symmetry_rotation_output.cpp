...
{
std::string
    mat3_fmt (const ModuleBase::Matrix3& m)
{
...
void
    print_symrot_info_R (const Symmetry_rotation& symrot,
                         const Symmetry& symm,
                         const int lmax_ao,
...
void
    print_symrot_info_k (const Symmetry_rotation& symrot, const K_Vectors& kv, const UnitCell& ucell)
{
                    ...
                    for (int iat1 = 0; iat1 < ucell.nat; ++iat1)
                        {
                            ...
                            const double arg
                                = 2 * ModuleBase::PI * isym_kvd.second * symrot.get_return_lattice (iat1, isym_space);
                            std::complex<double> phase_factor = std::complex<double> (std::cos (arg), std::sin (arg));
                            ofs << "atom " << iat1 + 1 << " -> " << iat2 + 1 << " of type " << it + 1
                                << " with Lmax= " << lmax << "\n";
