...
void Symmetry_rotation::cal_Ms(const K_Vectors& kv,
    ...
    auto restrict_kpt = [](const TCdouble& kvec, const double& symm_prec) -> TCdouble { // in (-0.5, 0.5]
        TCdouble kvec_res;
        ...
        if (std::abs(kvec_res.x) < symm_prec) {
            kvec_res.x = 0.0;
        }
        if (std::abs(kvec_res.y) < symm_prec) {
            kvec_res.y = 0.0;
        }
        if (std::abs(kvec_res.z) < symm_prec) {
            kvec_res.z = 0.0;
        }
        return kvec_res;
    };
...
Symmetry_rotation::restore_dm(const K_Vectors& kv,
    ...
    auto vec_conj = [](const std::vector<std::complex<double>>& z,
                       const double scal = 1.0) -> std::vector<std::complex<double>> {
        ...
        for (int ik_ibz = 0; ik_ibz < nk; ++ik_ibz) {
            for (auto& isym_kvd: kv.kstars[ik_ibz]) {
                if (isym_kvd.first == 0) {
                    ...
                    dm_k_full.push_back(
                        vec_conj(dm_k_ibz[ik_ibz + is * nk], 1.0 / static_cast<double>(kv.kstars[ik_ibz].size())));
...
double Symmetry_rotation::wigner_d(const double beta, const int l, const int m1, const int m2) const {
    ...
    for (int i = std::max(0, m2 - m1); i <= std::min(l - m1, l + m2); ++i) {
        result +=
            std::pow(-1, i) * std::sqrt(factorial(l + m1) * factorial(l - m1) * factorial(l + m2) * factorial(l - m2)) *
...
std::complex<double> Symmetry_rotation::wigner_D(const TCdouble& euler_angle,
                                                 const int l,
                                                 const int m1,
                                                 const int m2,
...
TCdouble Symmetry_rotation::get_euler_angle(const ModuleBase::Matrix3& gmatc) const {
    ...
    } else { // sin(beta)=0, beta = 0 or pi, only (alpha+gamma) or (alpha-gamma) is important. now assign this to alpha.
...
inline int m2im(int m) { return (m > 0 ? 2 * m - 1 : -2 * m); }
...
void Symmetry_rotation::cal_rotmat_Slm(const ModuleBase::Matrix3* gmatc, const int lmax) {
    ...
    for (int isym = 0; isym < nsym_; ++isym) {
        ...
        TCdouble euler_angle = get_euler_angle(
            gmatc[isym].Det() > 0 ? gmatc[isym] : gmatc[isym] * ModuleBase::Matrix3(-1, 0, 0, 0, -1, 0, 0, 0, -1));
...
void Symmetry_rotation::set_block_to_mat2d(const int starti,
                                           const int startj,
                                           const RI::Tensor<std::complex<double>>& block,
                                           std::vector<std::complex<double>>& obj_mat,
                                           const Parallel_2D& pv,
                                           const bool trans)
    const { // caution: ComplaxMatrix is row-major(col-continuous), but obj_mat is col-major(row-continuous)
...
std::vector<std::complex<double>> Symmetry_rotation::contruct_2d_rot_mat_ao(const Symmetry& symm,
                                                                            const Atom* atoms,
