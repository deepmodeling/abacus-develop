...
{
void
    Symmetry_rotation::set_Cs_rotation (const std::vector<std::vector<int>>& abfs_l_nchi)
{
    this->reduce_Cs_ = true;
    this->abfs_l_nchi_ = abfs_l_nchi;
    for (auto& abfs_T: abfs_l_nchi)
        {
            this->abfs_Lmax_ = std::max (this->abfs_Lmax_, static_cast<int> (abfs_T.size ()) - 1);
        }
}
void
    Symmetry_rotation::cal_Ms (const K_Vectors& kv,
                               ...
                               const Parallel_2D& pv)
{
    ...
    // 1. calculate the rotation matrix in real spherical harmonics representation for each symmetry operation: [T_l
    // (isym)]_mm'
    std::vector<ModuleBase::Matrix3> gmatc (nsym_);
    for (int i = 0; i < nsym_; ++i)
        {
            gmatc[i] = this->irs_.direct_to_cartesian (ucell.symm.gmatrix[i], ucell.latvec);
        }
    ...
    for (int ik_ibz = 0; ik_ibz < nks_ibz; ++ik_ibz)
        {
            // const TCdouble& kvec_d_ibz = restrict_kpt((*kstars[ik_ibz].begin()).second *
            // ucell.symm.kgmatrix[(*kstars[ik_ibz].begin()).first], ucell.symm.epsilon);
            for (auto& isym_kvd: kv.kstars[ik_ibz])
                {
                    if (isym_kvd.first < nsym_)
                        {
                            this->Ms_[ik_ibz][isym_kvd.first] = this->contruct_2d_rot_mat_ao (ucell.symm,
                                                                                              ucell.atoms,
                                                                                              ucell.st,
                                                                                              kv.kvec_d[ik_ibz],
                                                                                              isym_kvd.first,
...
std::vector<std::vector<std::complex<double>>>
    Symmetry_rotation::restore_dm (const K_Vectors& kv,
                                   ...
                                   const Parallel_2D& pv) const
{
    ...
    auto vec3_eq = [] (const TCdouble& v1, const TCdouble& v2, const double& prec) -> bool
        {
            return (std::abs (v1.x - v2.x) < prec) && (std::abs (v1.y - v2.y) < prec)
                   && (std::abs (v1.z - v2.z) < prec);
        };
    auto vec_conj
        = [] (const std::vector<std::complex<double>>& z, const double scal = 1.0) -> std::vector<std::complex<double>>
        {
            std::vector<std::complex<double>> z_conj (z.size ());
            for (int i = 0; i < z.size (); ++i)
                {
                    z_conj[i] = std::conj (z[i]) * scal;
                }
            return z_conj;
        };
                            ...
                            if (isym_kvd.first == 0)
                                {
                                    ...
                                    for (int i = 0; i < pv.get_local_size (); ++i)
                                        {
                                            dm_scaled[i] = factor * dm_k_ibz[ik_ibz + is * nk][i];
                                        }
                                ...
                                {
                                    dm_k_full.push_back (
                                        vec_conj (dm_k_ibz[ik_ibz + is * nk],
                                                  1.0 / static_cast<double> (kv.kstars[ik_ibz].size ())));
                                }
                            else if (isym_kvd.first < nsym_)
                                { // space group operations
                                    dm_k_full.push_back (this->rot_matrix_ao (dm_k_ibz[ik_ibz + is * nk],
                                                                              ik_ibz,
                                                                              kv.kstars[ik_ibz].size (),
                                                                              isym_kvd.first,
                                                                              pv));
                                }
                            else
    ...
    /*
            ...
            for (int ikibz = 0;ikibz < kv.get_nkstot() / nspin0;++ikibz)
                for (auto& isym_kvd : kv.kstars[ikibz])
                {
                    ...
                    ofs << " k = " << isym_kvd.second.x << " " << isym_kvd.second.y << " " << isym_kvd.second.z <<
       std::endl; ofs << "DM(k):" << std::endl; for (int i = 0;i < pv.get_row_size();++i)
                    {
                        for (int j = 0;j < pv.get_col_size();++j)
                        {
...
double
    Symmetry_rotation::wigner_d (const double beta, const int l, const int m1, const int m2) const
{
    ...
    for (int i = std::max (0, m2 - m1); i <= std::min (l - m1, l + m2); ++i)
        {
            result += std::pow (-1, i)
                      * std::sqrt (factorial (l + m1) * factorial (l - m1) * factorial (l + m2) * factorial (l - m2))
...
std::complex<double>
    Symmetry_rotation::wigner_D (const TCdouble& euler_angle, const int l, const int m1, const int m2, const bool inv)
        const
{
...
std::complex<double>
    Symmetry_rotation::ovlp_Ylm_Slm (const int l, const int m1, const int m2) const
{
    if (m1 == m2)
        {
            if (m1 == 0)
            ...
            if (m1 < 0)
                {
                    return std::pow (-1, m1) * ModuleBase::IMAG_UNIT / std::sqrt (2);
                }
        }
    else if (m1 == -m2)
        {
            if (m1 > 0)
                {
                    return -ModuleBase::IMAG_UNIT / std::sqrt (2);
                }
            if (m1 < 0)
                {
                    return std::pow (-1, m1) / std::sqrt (2);
                }
        }
    return 0.0;
}
...
TCdouble
    Symmetry_rotation::get_euler_angle (const ModuleBase::Matrix3& gmatc) const
{
    ...
    if (std::fabs (gmatc.e32) > threshold || std::fabs (gmatc.e31) > threshold) // sin(beta) is not zero
        {
            ...
            if (alpha < 0)
                {
                    alpha += 2 * ModuleBase::PI;
                }
            gamma = std::atan2 (gmatc.e23, -gmatc.e13);
            if (gamma < 0)
                {
                    gamma += 2 * ModuleBase::PI;
                }
...
void
    Symmetry_rotation::cal_rotmat_Slm (const ModuleBase::Matrix3* gmatc, const int lmax)
{
            ...
            for (int i = 0; i < mat.shape[0]; ++i)
                {
                    for (int j = 0; j < mat.shape[1]; ++j)
                        {
                            if (std::abs (mat (i, j).real () - std::round (mat (i, j).real ())) < zero_thres)
                                {
                                    mat (i, j).real (std::round (mat (i, j).real ()));
                                }
                            if (std::abs (mat (i, j).imag () - std::round (mat (i, j).imag ())) < zero_thres)
                                {
                                    mat (i, j).imag (std::round (mat (i, j).imag ()));
                                }
                        }
                }
        };
    ...
    for (int isym = 0; isym < nsym_; ++isym)
        {
            ...
            TCdouble euler_angle = get_euler_angle (
                gmatc[isym].Det () > 0 ? gmatc[isym]
                                       : gmatc[isym] * ModuleBase::Matrix3 (-1, 0, 0, 0, -1, 0, 0, 0, -1));
                    ...
                    for (int m1 = -l; m1 <= l; ++m1)
                        {
                            for (int m2 = -l; m2 <= l; ++m2)
                                {
                                    D_mm (m2im (m1), m2im (m2))
                                        = wigner_D (euler_angle, l, m1, m2, (gmatc[isym].Det () < 0));
                                }
                        }
    ...
    /*
            ...
            for (int isym = 0;isym < nsym_;++isym) euler_angles_test[isym] =
                get_euler_angle(gmatc[isym].Det() > 0 ? gmatc[isym] : gmatc[isym] * ModuleBase::Matrix3(-1, 0, 0, 0, -1,
       0, 0, 0, -1));
            ...
            auto test_Tmm = [&]()-> void
                {
                    ...
                    for (int isym = 0;isym < nsym_;++isym)
                    {
                        ...
                        ofs << ucell.symm.gmatrix[isym].e11 << " " << ucell.symm.gmatrix[isym].e12 << " " <<
       ucell.symm.gmatrix[isym].e13 << std::endl; ofs << ucell.symm.gmatrix[isym].e21 << " " <<
       ucell.symm.gmatrix[isym].e22 << " " << ucell.symm.gmatrix[isym].e23 << std::endl; ofs <<
       ucell.symm.gmatrix[isym].e31 << " " << ucell.symm.gmatrix[isym].e32 << " " << ucell.symm.gmatrix[isym].e33 <<
       std::endl; ofs << "kgmatrix_direct=" << std::endl; ofs << ucell.symm.kgmatrix[isym].e11 << " " <<
       ucell.symm.kgmatrix[isym].e12 << " " << ucell.symm.kgmatrix[isym].e13 << std::endl; ofs <<
       ucell.symm.kgmatrix[isym].e21 << " " << ucell.symm.kgmatrix[isym].e22 << " " << ucell.symm.kgmatrix[isym].e23 <<
       std::endl; ofs << ucell.symm.kgmatrix[isym].e31 << " " << ucell.symm.kgmatrix[isym].e32 << " " <<
       ucell.symm.kgmatrix[isym].e33 << std::endl; ofs << "euler_angle/pi: " << euler_angles_test[isym].x /
       ModuleBase::PI << " "
                            << euler_angles_test[isym].y / ModuleBase::PI << " " << euler_angles_test[isym].z /
       ModuleBase::PI << std::endl; for (int l = 0;l <= lmax;++l) for (int i = 0;i < 2 * l + 1;++i)
                            {
...
void
    Symmetry_rotation::set_block_to_mat2d (const int starti,
                                           const int startj,
                                           const RI::Tensor<std::complex<double>>& block,
                                           std::vector<std::complex<double>>& obj_mat,
                                           const Parallel_2D& pv,
                                           const bool trans) const
{ // caution: ComplaxMatrix is row-major(col-continuous), but obj_mat is col-major(row-continuous)
    for (int j = 0; j < block.shape[0]; ++j)
        { // outside dimension
            for (int i = 0; i < block.shape[1]; ++i)
                { // inside dimension
                    if (pv.in_this_processor (starti + i, startj + j))
                        {
                            int index = pv.global2local_col (startj + j) * pv.get_row_size ()
                                        + pv.global2local_row (starti + i);
...
std::vector<std::complex<double>>
    Symmetry_rotation::contruct_2d_rot_mat_ao (const Symmetry& symm,
                                               const Atom* atoms,
            ...
            while (iw < atoms[it].nw)
                {
                    ...
                    set_block_to_mat2d (iw2start + iw,
                                        iw1start + iw,
                                        phase_factor * this->rotmat_Slm_[isym][l],
                                        M_isym,
                                        pv,
                                        true);
                    iw += nm;
...
std::vector<std::complex<double>>
    Symmetry_rotation::rot_matrix_ao (const std::vector<std::complex<double>>& DMkibz,
                                      const int ik_ibz,
                                      const int kstar_size,
                                      const int isym,
                                      const Parallel_2D& pv,
                                      const bool TRS_conj) const
{
    ...
    if (TRS_conj)
        {
            ...
            ScalapackConnector::gemm (transpose,
                                      transpose,
                                      nbasis,
                                      nbasis,
                                      nbasis,
                                      alpha,
                                      this->Ms_[ik_ibz].at (isym).data (),
                                      i1,
                                      i1,
                                      pv.desc,
                                      DMkibz.data (),
        ...
        {
            // D^T = M^\daggger D^T M
            ScalapackConnector::gemm (dagger,
                                      notrans,
                                      nbasis,
                                      nbasis,
                                      nbasis,
                                      ...
                                      pv.desc);
            alpha.real (1.0 / static_cast<double> (kstar_size));
            ScalapackConnector::gemm (notrans,
                                      notrans,
                                      nbasis,
                                      nbasis,
                                      nbasis,
...
std::vector<TC>
    Symmetry_rotation::get_Rs_from_adjacent_list (const UnitCell& ucell,
...
std::vector<TC>
    Symmetry_rotation::get_Rs_from_BvK (const K_Vectors& kv) const
{
