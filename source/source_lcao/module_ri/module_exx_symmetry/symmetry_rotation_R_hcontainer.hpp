...
inline void print_global(const TR* tlocal, const int nr, const int nc, const std::string name) {
    ...
    for (int i = 0; i < nr; ++i) {
        for (int j = 0; j < nc; ++j)
            GlobalV::ofs_running << tlocal[j + i * nc] << " ";
        GlobalV::ofs_running << "\n";
    }
}
template <typename TR>
inline void print_local(const Parallel_Orbitals& pv, const TR* tlocal, const std::string name) {
    ...
    for (int i = 0; i < pv.get_row_size(); ++i) {
        for (int j = 0; j < pv.get_col_size(); ++j)
            GlobalV::ofs_running << tlocal[j + i * pv.get_col_size()] << " ";
        GlobalV::ofs_running << "\n";
    }
}
template <typename TR>
inline void print_atompair_local(const Parallel_Orbitals& pv,
                                 const int iat1,
                                 const int iat2,
                                 const TR* tlocal,
    ...
    for (int i = 0; i < pv.get_row_size(iat1); ++i) {
        for (int j = 0; j < pv.get_col_size(iat2); ++j)
            GlobalV::ofs_running << tlocal[j + i * pv.get_col_size(iat2)] << " ";
        GlobalV::ofs_running << "\n";
    }
}

template <typename TR> // HContainer type
void Symmetry_rotation::restore_HR(const Symmetry& symm,
                                   const Atom* atoms,
                                   const Statistics& st,
                                   const char mode,
...
inline void set_block(const int starti,
                      ...
                      double* obj) { // row-major (col-contiguous)
...
inline void set_block(const int starti,
                      ...
                      const int nr,
                      const ModuleBase::ComplexMatrix& block,
                      std::complex<double>* obj) { // row-major (col-contiguous)
    for (int i = 0; i < block.nr; ++i)
        for (int j = 0; j < block.nc; ++j)
            obj[starti + i + (startj + j) * nr] = block(j, i);
};
template <typename TR>
void Symmetry_rotation::rotate_atompair_parallel(const TR* Alocal_in,
                                                 const int isym,
                                                 const Atom* atoms,
                                                 const Statistics& st,
                                                 const Tap& ap_in,
    ...
    if (abr1 >= 0 && abc2 >= 0) { // "pv.local2global_row(i) - iw1start": global index in current atom pair
        for (int j = 0; j < pv.get_col_size(iat2); ++j)
            for (int i = 0; i < pv.get_row_size(iat1); ++i)
                A[(pv.local2global_row(i + abr1) - iw1start) * atoms[it2].nw +
                  (pv.local2global_col(j + abc2) - iw2start)] = Alocal_in[j + i * pv.get_col_size(iat2)];
    }
    if (output)
        print_global(A.data(), atoms[it1].nw, atoms[it2].nw, "A before allreduce");
    ...
    if (mode == 'H') { // H'=T1^\dagger * H * T2
        BlasConnector::gemm(notrans,
                            notrans,
                            atoms[it1].nw,
                            atoms[it2].nw,
                            atoms[it2].nw,
        ...
        BlasConnector::gemm(dagger,
                            ...
                            TAT.data(),
                            atoms[it2].nw);
    } else if (mode == 'D') { // D' = T1^T * D * T2^* = T1^T * [T2^\dagger * D^T]^T
        BlasConnector::gemm(dagger,
                            transpose,
                            atoms[it2].nw,
                            atoms[it1].nw,
        ...
        throw std::invalid_argument("Symmetry_rotation::rotate_atompair_tensor: invalid mode.");
    ...
    if (abr1 >= 0 && abc2 >= 0) { // ap_in index for TAT but ap_out index for Alocal_out
        for (int j = 0; j < pv.get_col_size(iat2); ++j)
            for (int i = 0; i < pv.get_row_size(iat1); ++i)
                Alocal_out[j + i * pv.get_col_size(iat2)] =
                    TAT[(pv.local2global_row(i + abr1) - iw1start) * atoms[it2].nw +
                        (pv.local2global_col(j + abc2) - iw2start)];
    }
}
...
void Symmetry_rotation::test_HR_rotation(const Symmetry& symm,
    ...
    for (auto& irap_irR: this->irs_.irreducible_sector_) {
        ...
        for (auto& R: irap_irR.second)
            if (irap.find_R(R[0], R[1], R[2]) >= 0) { // out-of-range R can be added to irreducible sector to be
                                                      // calculated, but not in this test for lack of reference
    ...
    for (int iat1 = 0; iat1 < st.nat; ++iat1) {
        for (int iat2 = 0; iat2 < st.nat; ++iat2) {
            ...
            for (int irR = 0; irR < ap_full.get_R_size(); ++irR) {
                ...
                GlobalV::ofs_running << "atom pair: (" << iat1 << ", " << iat2 << "),  R: (" << R[0] << " " << R[1]
                                     << " " << R[2] << ")\n";
