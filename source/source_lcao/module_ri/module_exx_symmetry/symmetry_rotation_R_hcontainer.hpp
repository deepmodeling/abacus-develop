...
inline void
    print_global (const TR* tlocal, const int nr, const int nc, const std::string name)
{
...
template <typename TR>
inline void
    print_local (const Parallel_Orbitals& pv, const TR* tlocal, const std::string name)
{
    GlobalV::ofs_running << name << std::endl;
...
template <typename TR>
inline void
    print_atompair_local (const Parallel_Orbitals& pv,
                          const int iat1,
                          const int iat2,
                          const TR* tlocal,
...
void
    Symmetry_rotation::restore_HR (const Symmetry& symm,
                                   const Atom* atoms,
                                   const Statistics& st,
                                   const char mode,
...
inline void
    set_block (const int starti, const int startj, const int nr, const ModuleBase::ComplexMatrix& block, double* obj)
...
template <typename TR>
void
    Symmetry_rotation::rotate_atompair_parallel (const TR* Alocal_in,
                                                 const int isym,
                                                 const Atom* atoms,
                                                 const Statistics& st,
                                                 const Tap& ap_in,
    ...
    for (auto& irap_irR: this->irs_.irreducible_sector_)
        {
            ...
            for (auto& R: irap_irR.second)
                if (irap.find_R (R[0], R[1], R[2]) >= 0)
                    { // out-of-range R can be added to irreducible sector to be calculated, but not in this test for
                      // lack of reference
                    ...
                    for (int irR = 0; irR < ap_full.get_R_size (); ++irR)
                        {
                            ...
                            GlobalV::ofs_running << "atom pair: (" << iat1 << ", " << iat2 << "),  R: (" << R[0] << " "
                                                 << R[1] << " " << R[2] << ")\n";
                            print_atompair_local (*pv,
                                                  iat1,
                                                  iat2,
                                                  rotated_ptr,
                                                  std::string ("R_rot").insert (0, 1, mode));
