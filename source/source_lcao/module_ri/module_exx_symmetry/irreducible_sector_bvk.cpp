...
{
ModuleBase::Matrix3
    Irreducible_Sector::direct_to_cartesian (const ModuleBase::Matrix3& d, const ModuleBase::Matrix3& latvec) const
{
    return latvec.Inverse () * d * latvec;
}

std::vector<int> inline get_isymbvk_to_isym_map (const std::vector<ModuleBase::Matrix3>& bvkgmat,
...
void
    Irreducible_Sector::gen_symmetry_BvK (const ModuleSymmetry::Symmetry& symm,
                                          const Atom* atoms,
                                          const Lattice& lat,
                                          const Statistics& st,
                                          const TC bvk_period)
{
    ...
    auto set_matrix3 = [] (const ModuleBase::Vector3<double>& a1,
                           const ModuleBase::Vector3<double>& a2,
                           const ModuleBase::Vector3<double>& a3) -> ModuleBase::Matrix3
        { return ModuleBase::Matrix3 (a1.x, a1.y, a1.z, a2.x, a2.y, a2.z, a3.x, a3.y, a3.z); };
    auto set_bvk_same_as_ucell = [&symm, this] () -> void
        {
            ...
            for (int isym = 0; isym < symm.nrotk; ++isym)
                {
                    this->isymbvk_to_isym_[isym] = isym;
                }
        };
    ...
    for (int it = 0; it < st.ntype; ++it)
        {
            ...
            if (it > 0)
                {
                    bvk_istart[it] = bvk_istart[it - 1] + bvk_na[it - 1];
                }
                            ...
                            for (int c3 = 0; c3 < bvk_min_period[2]; ++c3)
                                {
                                    for (int ia = 0; ia < atoms[it].na; ++ia)
                                        {
                                            bvk_dpos[3 * at] = (static_cast<double> (c1) + atoms[it].taud[ia].x)
                                                               / static_cast<double> (bvk_min_period[0]);
                                            bvk_dpos[3 * at + 1] = (static_cast<double> (c2) + atoms[it].taud[ia].y)
                                                                   / static_cast<double> (bvk_min_period[1]);
                                            bvk_dpos[3 * at + 2] = (static_cast<double> (c3) + atoms[it].taud[ia].z)
                                                                   / static_cast<double> (bvk_min_period[2]);
                                            for (int k = 0; k < 3; ++k)
                                                {
                                                    symm.check_translation (bvk_dpos[3 * at + k],
                                                                            -floor (bvk_dpos[3 * at + k]));
    ...
    // bvk_brav = symm.standard_lat(s1, s2, s3, cel_const); //not enough, optimal lattice may change after
    // cell-extension
...
// std::vector<bool> Irreducible_Sector::in_plain(const ModuleSymmetry::Symmetry& symm, const ModuleBase::Matrix3&
// latvec)const
// {
