...
ModuleBase::Matrix3 Irreducible_Sector::direct_to_cartesian(const ModuleBase::Matrix3& d,
...
void Irreducible_Sector::gen_symmetry_BvK(const ModuleSymmetry::Symmetry& symm,
                                          const Atom* atoms,
                                          const Lattice& lat,
                                          const Statistics& st,
                                          const TC bvk_period) {
    ...
    if (bvk_period[0] == bvk_period[1] &&
        bvk_period[0] == bvk_period[2]) { // the BvK supercell has the same symmetry as the original cell
            ...
            for (int c2 = 0; c2 < bvk_min_period[1]; ++c2) {
                for (int c3 = 0; c3 < bvk_min_period[2]; ++c3) {
                    for (int ia = 0; ia < atoms[it].na; ++ia) {
                        bvk_dpos[3 * at] =
                            (static_cast<double>(c1) + atoms[it].taud[ia].x) / static_cast<double>(bvk_min_period[0]);
                        bvk_dpos[3 * at + 1] =
                            (static_cast<double>(c2) + atoms[it].taud[ia].y) / static_cast<double>(bvk_min_period[1]);
                        bvk_dpos[3 * at + 2] =
                            (static_cast<double>(c3) + atoms[it].taud[ia].z) / static_cast<double>(bvk_min_period[2]);
                        for (int k = 0; k < 3; ++k) {
