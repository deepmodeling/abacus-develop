...
class SymmetryRotationTest : public testing::Test {
  protected:
    ...
    std::vector<std::complex<double>> wigerD_p_C41_ref =
        {ModuleBase::IMAG_UNIT, 0, 0, 0, 1, 0, 0, 0, -ModuleBase::IMAG_UNIT};
    std::vector<std::complex<double>> cmm_p_C41_ref =
        {-ModuleBase::IMAG_UNIT / sqrt(2), 0, -1 / sqrt(2), 0, 1, 0, -ModuleBase::IMAG_UNIT / sqrt(2), 0, 1 / sqrt(2)};
...
TEST_F(SymmetryRotationTest, WignerD) {
    ...
    for (int m1 = -l; m1 <= l; ++m1)
        for (int m2 = -l; m2 <= l; ++m2) {
            int i = m1 + l, j = m2 + l;
            wignerD_p_C41(i, j) =
                symrot.wigner_D(ModuleBase::Vector3<double>(0, 0, ModuleBase::PI / 2), 1, m1, m2, false);
            ...
            wignerD_p_C41(i, j) =
                symrot.wigner_D(ModuleBase::Vector3<double>(ModuleBase::PI / 2, 0, 0), 1, m1, m2, false);
            EXPECT_NEAR(wignerD_p_C41(i, j).real(), wigerD_p_C41_ref[i * 3 + j].real(), DOUBLETHRESHOLD);
            EXPECT_NEAR(wignerD_p_C41(i, j).imag(), wigerD_p_C41_ref[i * 3 + j].imag(), DOUBLETHRESHOLD);
        }
...
TEST_F(SymmetryRotationTest, GetReturnLattice) {
    ...
    ModuleBase::Vector3<double> return_lattice =
        symrot.get_return_lattice(ModuleSymmetry::Symmetry(), gmatd, gtransd, posd_a1, posd_a2);
...
TEST_F(SymmetryRotationTest, SetBlockToMat2d) {
    ...
    for (int j = 0; j < pv.get_col_size(); ++j)
        for (int i = 0; i < pv.get_row_size(); ++i)
            obj_mat[j * pv.get_row_size() + i] = std::complex<double>(static_cast<double>(pv.local2global_row(i)),
                                                                      static_cast<double>(pv.local2global_col(j)));
