...
{
TC
    Irreducible_Sector::rotate_R (const Symmetry& symm,
                                  const int isym,
                                  const int iat1,
                                  const int iat2,
                                  const TC& R,
                                  const char gauge) const
{
    auto round2int = [symm] (const double x) -> int
...
TapR
    Irreducible_Sector::rotate_apR_by_formula (const Symmetry& symm,
                                               const int isym,
                                               const TapR& apR,
                                               const char gauge) const
{
...
TCdouble
    Irreducible_Sector::get_aRb_direct (const Atom* atoms,
                                        const Statistics& st,
                                        const int iat1,
                                        const int iat2,
                                        const TCdouble& R,
                                        const char gauge) const
{
    return TCdouble (atoms[st.iat2it[iat1]].taud[st.iat2ia[iat1]] - atoms[st.iat2it[iat2]].taud[st.iat2ia[iat2]])
           + (gauge == 'L' ? R : TCdouble (-R));
}
...
inline void
    output_return_lattice (const std::vector<std::vector<TCdouble>>& return_lattice)
{
    ...
    for (int iat = 0; iat < return_lattice.size (); ++iat)
        {
            ...
            for (int isym = 0; isym < return_lattice[iat].size (); ++isym)
                {
                    std::cout << "isym=" << isym << ", return lattice=" << return_lattice[iat][isym].x << " "
                              << return_lattice[iat][isym].y << " " << return_lattice[iat][isym].z << std::endl;
                }
        }
}
...
TCdouble
    Irreducible_Sector::get_return_lattice (const Symmetry& symm,
                                            const ModuleBase::Matrix3& gmatd,
                                            const TCdouble gtransd,
                                            const TCdouble& posd_a1,
                                            const TCdouble& posd_a2) const
{
    ...
    auto restrict_center = [&symm] (const TCdouble& v) -> TCdouble
        {
            ...
            if (std::abs (vr.x) < symm.epsilon)
                {
                    vr.x = 0.0;
                }
            if (std::abs (vr.y) < symm.epsilon)
                {
                    vr.y = 0.0;
                }
            if (std::abs (vr.z) < symm.epsilon)
                {
                    vr.z = 0.0;
                }
            return vr;
        };
    auto check_integer = [&symm] (const double x) -> void { assert (symm.equal (x, std::round (x))); };
...
#endif
    return TCdouble (std::round (return_lattice_double.x),
                     std::round (return_lattice_double.y),
                     std::round (return_lattice_double.z));
}

void
    Irreducible_Sector::cal_return_lattice_all (const Symmetry& symm, const Atom* atoms, const Statistics& st)
{
            ...
            for (int isym = 0; isym < symm.nrotk; ++isym)
                {
                    ...
                    this->return_lattice_[iat1][isym] = get_return_lattice (symm,
                                                                            symm.gmatrix[isym],
                                                                            symm.gtrans[isym],
                                                                            atoms[it].taud[ia1],
                                                                            atoms[it].taud[ia2]);
...
void
    Irreducible_Sector::output_full_map_to_irreducible_sector (const int nat)
{
    ...
    for (auto& apR_isym_irapR: this->full_map_to_irreducible_sector_)
        {
            ...
            std::cout << "atompair (" << ap.first << ", " << ap.second << "), R=(" << R[0] << ", " << R[1] << ", "
                      << R[2] << ") -> "
                      << "isym=" << apR_isym_irapR.second.first << " -> irreducible atompair (" << irap.first << ", "
                      << irap.second << "), irreducible R=(" << irR[0] << ", " << irR[1] << ", " << irR[2] << ")"
                      << std::endl;
        }
}

void
    Irreducible_Sector::output_sector_star ()
{
    ...
    for (auto& irap_star: this->sector_stars_)
        {
            ...
            std::cout << "in star of irreducible atompair=(" << irap.first << ", " << irap.second << "), R=(" << irR[0]
                      << ", " << irR[1] << ", " << irR[2] << ") with size " << star.size () << ":\n";
            for (auto& isym_ap_R: star)
                {
                    std::cout << "isym=" << isym_ap_R.first << ", atompair=(" << isym_ap_R.second.first.first << ", "
                              << isym_ap_R.second.first.second << "), R=(" << isym_ap_R.second.second[0] << ", "
                              << isym_ap_R.second.second[1] << ", " << isym_ap_R.second.second[2] << ")" << std::endl;
                }
        }
    ...
    for (auto& irap_irR: this->irreducible_sector_)
        {
            for (auto& irR: irap_irR.second)
                {
                    std::cout << "atompair (" << irap_irR.first.first << ", " << irap_irR.first.second << "), R = ("
                              << irR[0] << ", " << irR[1] << ", " << irR[2] << ") \n";
                }
            std::cout << std::endl;
        }
            ...
            for (auto& irap_irR: this->irreducible_sector_)
                {
                    for (auto& irR: irap_irR.second)
                        {
                            ofs << "atompair (" << irap_irR.first.first << ", " << irap_irR.first.second << "), R = ("
                                << irR[0] << ", " << irR[1] << ", " << irR[2] << ") \n";
                        }
                }
            ofs.close ();
...
void
    Irreducible_Sector::find_irreducible_sector (const Symmetry& symm,
                                                 const Atom* atoms,
                                                 const Statistics& st,
                                                 ...
                                                 const Lattice& lat)
{
    ...
    if (this->return_lattice_.empty ())
        {
            this->cal_return_lattice_all (symm, atoms, st);
        }
