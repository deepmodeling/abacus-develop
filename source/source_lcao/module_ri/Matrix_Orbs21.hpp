...
RI::Tensor<Tdata> Matrix_Orbs21::cal_overlap_matrix(const size_t TA,
                                                    const size_t TB,
    ...
    switch (matrix_order) {
    case Matrix_Order::A1A2B:
        m = RI::Tensor<Tdata>({sizeA1, sizeA2, sizeB});
        break;
    case Matrix_Order::A1BA2:
        m = RI::Tensor<Tdata>({sizeA1, sizeB, sizeA2});
        break;
    case Matrix_Order::BA1A2:
        m = RI::Tensor<Tdata>({sizeB, sizeA1, sizeA2});
        break;
    case Matrix_Order::BA2A1:
        m = RI::Tensor<Tdata>({sizeB, sizeA2, sizeA1});
        break;
    case Matrix_Order::A2A1B:
        m = RI::Tensor<Tdata>({sizeA2, sizeA1, sizeB});
        break;
    case Matrix_Order::A2BA1:
        m = RI::Tensor<Tdata>({sizeA2, sizeB, sizeA1});
        break;
    default:
                            ...
                            for (const auto& co7: co6.second) {
                                ...
                                for (const auto& co8: co7.second) {
                                    ...
                                    for (size_t MB = 0; MB != 2 * LB + 1; ++MB) {
                                        const Tdata overlap =
                                            co8.second.cal_overlap(tauA * lat0, tauB * lat0, MA1, MA2, MB);
                                        const size_t iA1 = index_A1[TA][LA1][NA1][MA1];
                                        const size_t iA2 = index_A2[TA][LA2][NA2][MA2];
                                        const size_t iB = index_B[TB][LB][NB][MB];
                                        switch (matrix_order) {
                                        case Matrix_Order::A1A2B:
                                            m(iA1, iA2, iB) = overlap;
                                            break;
                                        case Matrix_Order::A1BA2:
                                            m(iA1, iB, iA2) = overlap;
                                            break;
                                        case Matrix_Order::A2A1B:
                                            m(iA2, iA1, iB) = overlap;
                                            break;
                                        case Matrix_Order::A2BA1:
                                            m(iA2, iB, iA1) = overlap;
                                            break;
                                        case Matrix_Order::BA1A2:
                                            m(iB, iA1, iA2) = overlap;
                                            break;
                                        case Matrix_Order::BA2A1:
                                            m(iB, iA2, iA1) = overlap;
                                            break;
                                        default:
...
Matrix_Orbs21::cal_grad_overlap_matrix(const size_t TA,
                                       const size_t TB,
                                       const ModuleBase::Vector3<double>& tauA,
                                       const ModuleBase::Vector3<double>& tauB,
                                       const ModuleBase::Element_Basis_Index::IndexLNM& index_A1,
    ...
    for (int i = 0; i < m.size(); ++i) {
        switch (matrix_order) {
        case Matrix_Order::A1A2B:
            m[i] = RI::Tensor<Tdata>({sizeA1, sizeA2, sizeB});
            break;
        case Matrix_Order::A1BA2:
            m[i] = RI::Tensor<Tdata>({sizeA1, sizeB, sizeA2});
            break;
        case Matrix_Order::BA1A2:
            m[i] = RI::Tensor<Tdata>({sizeB, sizeA1, sizeA2});
            break;
        case Matrix_Order::BA2A1:
            m[i] = RI::Tensor<Tdata>({sizeB, sizeA2, sizeA1});
            break;
        case Matrix_Order::A2A1B:
            m[i] = RI::Tensor<Tdata>({sizeA2, sizeA1, sizeB});
            break;
        case Matrix_Order::A2BA1:
            m[i] = RI::Tensor<Tdata>({sizeA2, sizeB, sizeA1});
            break;
        default:
            throw std::invalid_argument(std::string(__FILE__) + " line " + std::to_string(__LINE__));
        }
                                ...
                                for (const auto& co8: co7.second) {
                                    ...
                                    for (size_t MB = 0; MB != 2 * LB + 1; ++MB) {
                                        const std::array<double, 3> grad_overlap = RI_Util::Vector3_to_array3(
                                            co8.second.cal_grad_overlap(tauA * lat0, tauB * lat0, MA1, MA2, MB));
                                        const size_t iA1 = index_A1[TA][LA1][NA1][MA1];
                                        const size_t iA2 = index_A2[TA][LA2][NA2][MA2];
                                        const size_t iB = index_B[TB][LB][NB][MB];
                                        for (size_t i = 0; i < m.size(); ++i) {
                                            switch (matrix_order) {
                                            case Matrix_Order::A1A2B:
                                                m[i](iA1, iA2, iB) = grad_overlap[i];
                                                break;
                                            case Matrix_Order::A1BA2:
                                                m[i](iA1, iB, iA2) = grad_overlap[i];
                                                break;
                                            case Matrix_Order::A2A1B:
                                                m[i](iA2, iA1, iB) = grad_overlap[i];
                                                break;
                                            case Matrix_Order::A2BA1:
                                                m[i](iA2, iB, iA1) = grad_overlap[i];
                                                break;
                                            case Matrix_Order::BA1A2:
                                                m[i](iB, iA1, iA2) = grad_overlap[i];
                                                break;
                                            case Matrix_Order::BA2A1:
                                                m[i](iB, iA2, iA1) = grad_overlap[i];
                                                break;
                                            default:
                                                throw std::invalid_argument(std::string(__FILE__) + " line " +
                                                                            std::to_string(__LINE__));
...
Matrix_Orbs21::cal_overlap_matrix_all(const UnitCell& ucell,
                                      const ModuleBase::Element_Basis_Index::IndexLNM& index_A1,
                                      const ModuleBase::Element_Basis_Index::IndexLNM& index_A2,
            ...
            for (const auto& co2: co1.second) {
                ...
                for (size_t IB = 0; IB != ucell.atoms[TB].na; ++IB) {
                    ...
                    const RI::Tensor<Tdata>&& m =
                        cal_overlap_matrix<Tdata>(TA, TB, tauA, tauB, index_A1, index_A2, index_B, Matrix_Order::A2BA1);
                    matrixes[TA][IA][TB][IB].resize(2);
                    matrixes[TA][IA][TB][IB][0] = std::move(m);
                    const RI::Tensor<Tdata>&& n =
                        cal_overlap_matrix<Tdata>(TA, TB, tauA, tauB, index_A1, index_A2, index_B, Matrix_Order::BA2A1);
