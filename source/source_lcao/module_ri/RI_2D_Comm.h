...
// judge[is] = {s0, s1}
extern std::vector<std::tuple<std::set<TA>, std::set<TA>>> get_2D_judge(const UnitCell& ucell, const Parallel_2D& pv);

template <typename Tdata, typename TK>
extern void add_Hexx(const UnitCell& ucell,
                     const K_Vectors& kv,
                     const int ik,
                     const double alpha,
...
template <typename Tdata, typename TK>
extern void add_Hexx_td(const UnitCell& ucell,
                        const K_Vectors& kv,
                        const int ik,
                        const double alpha,
                        const std::vector<std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>>& Hs,
...
extern std::vector<std::vector<Tdata>>
Hexxs_to_Hk(const K_Vectors& kv,
            const Parallel_Orbitals& pv,
            const std::vector<std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>>& Hexxs,
            const int ik);
template <typename Tdata>
std::vector<std::vector<Tdata>> pulay_mixing(const Parallel_Orbitals& pv,
...
comm_map2(const MPI_Comm& mpi_comm, const std::map<TA, std::map<TAC, T>>& Ds_in, const Tjudge& judge);
...
extern void
set_value_add(std::tuple<Tkey0, Tkey1>&& key, Tvalue&& value, std::map<Tkey0, std::map<Tkey1, Tvalue>>& data);
template <typename Tkey, typename Tvalue>
