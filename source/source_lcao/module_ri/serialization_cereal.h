...
void
    serialize (Archive& ar, Abfs::Vector3_Order<T>& v)
{
    ar (v.x);
    ar (v.y);
    ar (v.z);
...
template <class Archive, typename T>
void
    serialize (Archive& ar, Vector3<T>& v)
{
    ar (v.x);
    ar (v.y);
    ar (v.z);
...
void
    save (Archive& ar, const matrix& m)
{
    ar (m.nr);
    ar (m.nc);
...
template <typename T>
void
    bcast_data_cereal (T& data, const MPI_Comm& mpi_comm, const int& rank_bcast)
{
    int my_rank;
