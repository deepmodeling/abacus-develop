...
inline std::map<Conv_Coulomb_Pot_K::Coulomb_Type, std::vector<std::map<std::string, std::string>>> update_coulomb_param(
    ...
    for (auto& param_list: coulomb_param_updated) {
        for (auto& param: param_list.second) {
            if (param.at("singularity_correction") == "spencer") {
                ...
                const double Rcut =
                    std::pow(0.75 * p_kv->get_nkstot_full() * ucell.omega / (ModuleBase::PI), 1.0 / 3.0);
