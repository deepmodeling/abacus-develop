...
inline std::map<Conv_Coulomb_Pot_K::Coulomb_Type, std::vector<std::map<std::string, std::string>>>
    update_coulomb_param (const std::map<Conv_Coulomb_Pot_K::Coulomb_Type,
                                         std::vector<std::map<std::string, std::string>>>& coulomb_param,
                          const UnitCell& ucell,
                          const K_Vectors* p_kv)
{
    std::map<Conv_Coulomb_Pot_K::Coulomb_Type, std::vector<std::map<std::string, std::string>>> coulomb_param_updated
        = coulomb_param;
                    ...
                    if (param.at ("singularity_correction") == "spencer")
                        {
                            ...
                            const double Rcut
                                = std::pow (0.75 * p_kv->get_nkstot_full () * ucell.omega / (ModuleBase::PI),
                                            1.0 / 3.0);
...
inline std::map<
    ...
    std::pair<bool, std::map<Conv_Coulomb_Pot_K::Coulomb_Type, std::vector<std::map<std::string, std::string>>>>>
    update_coulomb_settings (const std::map<Conv_Coulomb_Pot_K::Coulomb_Type,
                                            std::vector<std::map<std::string, std::string>>>& coulomb_param,
                             const UnitCell& ucell,
                             const K_Vectors* p_kv)
{
    ...
    for (auto& param_list: coulomb_param_updated)
        {
            switch (param_list.first)
                ...
                case Conv_Coulomb_Pot_K::Coulomb_Type::Fock:
                    {
                        for (auto& param: param_list.second)
                            {
                                if (param.at ("singularity_correction") == "spencer"
                                    || param.at ("singularity_correction") == "limits"
                ...
                case Conv_Coulomb_Pot_K::Coulomb_Type::Erfc:
                    {
                        coulomb_param_center2[param_list.first]
                            = param_list.second; // Erfc is always calculated with Center2 method.
                        break;
                    }
                default:
                    {
    ...
    if (cal_center)
        {
            coulomb_settings[Conv_Coulomb_Pot_K::Coulomb_Method::Center2]
                = std::make_pair (cal_center, coulomb_param_center2);
        }
    if (cal_ewald)
        {
            coulomb_settings[Conv_Coulomb_Pot_K::Coulomb_Method::Ewald]
                = std::make_pair (cal_ewald, coulomb_param_ewald);
        }
    if (cal_center && cal_ewald)
        {
            coulomb_settings[Conv_Coulomb_Pot_K::Coulomb_Method::Center2].first
                = false; // If both methods are available, only HF for C is needed.
        }
