...
{
std::vector<double>
    cal_psi_fock_limits (const std::vector<double>& psif)
{
...
std::vector<double>
    cal_psi_fock_spencer (const std::vector<double>& psif, const std::vector<double>& k_radial, const double rcut)
{
    std::vector<double> psik2_ccp (psif.size ());
    for (size_t ik = 0; ik < psif.size (); ++ik)
        {
...
std::vector<double>
    cal_psi_erfc_limits (const std::vector<double>& psif, const std::vector<double>& k_radial, const double erfc_omega)
{
    std::vector<double> psik2_ccp (psif.size ());
    for (size_t ik = 0; ik < psif.size (); ++ik)
        {
            psik2_ccp[ik] = ModuleBase::FOUR_PI * psif[ik]
                            * (1 - std::exp (-(k_radial[ik] * k_radial[ik]) / (4 * erfc_omega * erfc_omega)));
        }
    return psik2_ccp;
}

std::vector<double>
    cal_psi_erfc_spencer (const std::vector<double>& psif,
    ...
    for (size_t ik = 0; ik < psif.size (); ++ik)
        {
            ...
            if (temp1 >= eps)
                {
                    temp2 = ModuleBase::ErrorFunc::erf (
                        0.5 * (ModuleBase::IMAG_UNIT * k_radial[ik] + 2 * erfc_omega * erfc_omega * rcut) / erfc_omega);
                    temp3 = ModuleBase::NEG_IMAG_UNIT
                            * ModuleBase::ErrorFunc::erfi (0.5 * k_radial[ik] / erfc_omega
                                                           + ModuleBase::IMAG_UNIT * erfc_omega * rcut);
                }
...
Numerical_Orbital_Lm
    cal_orbs_ccp<Numerical_Orbital_Lm> (const Numerical_Orbital_Lm& orbs,
                                        const std::map<Conv_Coulomb_Pot_K::Coulomb_Type,
                                                       std::vector<std::map<std::string, std::string>>>& coulomb_param,
                        ...
                        for (const auto& param: param_list.second)
                            {
                                if (param.at ("singularity_correction") == "limits"
                                    ...
                                    || param.at ("singularity_correction") == "carrier")
                                    {
                                        psik2_ccp
                                            = psik2_ccp
                                              + std::stod (param.at ("alpha")) * cal_psi_fock_limits (orbs.get_psif ());
                                    }
                                ...
                                else
                                    {
                                        throw std::invalid_argument (
                                            "singularity_correction = " + param.at ("singularity_correction") + " in "
...
template <>
Numerical_Orbital_Lm
    cal_orbs_ccp_spencer<Numerical_Orbital_Lm> (
...
template <>
double
    get_rmesh_proportion (const Numerical_Orbital_Lm& orbs, const double psi_threshold)
{
