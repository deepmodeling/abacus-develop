...
std::vector<double> cal_psi_erfc_spencer(const std::vector<double>& psif,
    ...
    for (size_t ik = 0; ik < psif.size(); ++ik) {
        ...
        if (temp1 >= eps) {
            ...
            temp3 = ModuleBase::NEG_IMAG_UNIT * ModuleBase::ErrorFunc::erfi(0.5 * k_radial[ik] / erfc_omega +
        ...
        case Conv_Coulomb_Pot_K::Coulomb_Type::Fock: {
            for (const auto& param: param_list.second) {
                if (param.at("singularity_correction") == "limits" ||
                    ...
                    psik2_ccp =
                        psik2_ccp + std::stod(param.at("alpha")) * cal_psi_fock_spencer(orbs.get_psif(),
                                                                                        orbs.get_k_radial(),
        ...
        case Conv_Coulomb_Pot_K::Coulomb_Type::Erfc: {
            for (const auto& param: param_list.second) {
                if (param.at("singularity_correction") == "limits" ||
                    ...
                    param.at("singularity_correction") == "carrier") {
                    psik2_ccp =
                        psik2_ccp + std::stod(param.at("alpha")) * cal_psi_erfc_limits(orbs.get_psif(),
                                                                                       orbs.get_k_radial(),
                                                                                       std::stod(param.at("omega")));
                } else if (param.at("singularity_correction") == "spencer" ||
                           param.at("singularity_correction") == "revised_spencer") {
                    psik2_ccp =
                        psik2_ccp + std::stod(param.at("alpha")) * cal_psi_erfc_spencer(orbs.get_psif(),
                                                                                        orbs.get_k_radial(),
                                                                                        std::stod(param.at("omega")),
                                                                                        std::stod(param.at("Rcut")));
                } else {
            ...
            for (const auto& param: param_list.second) {
                psik2_ccp = psik2_ccp +
                            std::stod(param.at("alpha")) *
                                cal_psi_fock_spencer(orbs.get_psif(), orbs.get_k_radial(), std::stod(param.at("Rcut")));
            }
            break;
        }
        case Conv_Coulomb_Pot_K::Coulomb_Type::Erfc: {
