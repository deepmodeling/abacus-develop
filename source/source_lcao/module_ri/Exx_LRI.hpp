...
inline double
    default_spencer_rcut (const UnitCell& ucell, const K_Vectors& kv)
{
    return std::pow (0.75 * kv.get_nkstot_full () * ucell.omega / (ModuleBase::PI), 1.0 / 3.0);
}

inline CoulombParam
    build_center2_cut_coulomb_param (const CoulombParam& coulomb_param,
...
void
    Exx_LRI<Tdata>::init (const MPI_Comm& mpi_comm_in,
                          const UnitCell& ucell,
                          ...
                          const LCAO_Orbitals& orb)
{
    ...
    if (this->info.files_abfs.empty ())
        ...
        {
            this->abfs
                = Exx_Abfs::IO::construct_abfs (abfs_same_atom, orb, this->info.files_abfs, this->info.kmesh_times);
        }
    ...
    for (const auto& settings_list: this->coulomb_settings)
        {
            ...
            this->exx_objs[settings_list.first].cv.set_orbitals (ucell,
                                                                 orb,
                                                                 this->lcaos,
                                                                 this->abfs,
                                                                 this->exx_objs[settings_list.first].abfs_ccp,
            ...
            if (settings_list.first == Conv_Coulomb_Pot_K::Coulomb_Method::Ewald)
                {
                    this->exx_objs[settings_list.first].evq.init (ucell,
                                                                  orb,
                                                                  this->mpi_comm,
                                                                  this->p_kv,
                                                                  this->lcaos,
...
template <typename Tdata>
void
    Exx_LRI<Tdata>::init_spencer (const MPI_Comm& mpi_comm_in,
                                  const UnitCell& ucell,
                                  const K_Vectors& kv_in,
                                  const LCAO_Orbitals& orb)
{
    ...
    this->coulomb_settings[Conv_Coulomb_Pot_K::Coulomb_Method::Center2]
        = std::make_pair (true, ExxLriDetail::build_center2_cut_coulomb_param (this->info.coulomb_param, ucell, kv_in));
    ...
    this->exx_objs[Conv_Coulomb_Pot_K::Coulomb_Method::Center2].abfs_ccp
        = Conv_Coulomb_Pot_K::cal_orbs_ccp_spencer (this->abfs,
...
template <typename Tdata>
void
    Exx_LRI<Tdata>::cal_exx_ions (const UnitCell& ucell, const bool write_cv)
{
    ...
    for (const auto& settings_list: this->coulomb_settings)
        {
                    ...
                    for (const auto& param_list: coulomb_param)
                        {
                            ...
                            switch (param_list.first)
                                ...
                                case Conv_Coulomb_Pot_K::Coulomb_Type::Fock:
                                    {
                                        double chi = this->exx_objs[settings_list.first].evq.get_singular_chi (
                                            ucell,
                                            param_list.second,
                                            2.0);
                                        Vs_ewald_temp
            ...
            if (PARAM.inp.cal_force || PARAM.inp.cal_stress)
                {
                    std::map<TA, std::map<TAC, std::array<RI::Tensor<Tdata>, Ndim>>> dVs_temp
                        = this->exx_objs[settings_list.first].cv.cal_dVs (ucell,
                                                                          list_As_Vs.first,
                                                                          list_As_Vs.second[0],
                                                                          {{"writable_dVws", true}});
            ...
            if (settings_list.second.first)
                {
                    ...
                    if (PARAM.inp.cal_force || PARAM.inp.cal_stress)
                        {
                            std::map<TA, std::map<TAC, std::array<RI::Tensor<Tdata>, 3>>>& dCs_temp
                                = std::get<1> (Cs_dCs);
...
template <typename Tdata>
void
    Exx_LRI<Tdata>::cal_cut_coulomb_cs (std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>& Vs_cut,
                                        std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>& Cs,
                                        const UnitCell& ucell,
                                        const bool write_cv)
{
    ...
    Vs_cut = center2_obj_it->second.cv.cal_Vs (ucell, list_As_Vs.first, list_As_Vs.second[0], {{"writable_Vws", true}});
...
template <typename Tdata>
void
    Exx_LRI<Tdata>::cal_ewald_coulomb (std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>& Vs_full,
                                       std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>& Cs,
                                       const UnitCell& ucell,
                                       const bool write_cv)
{
...
template <typename Tdata>
void
    Exx_LRI<Tdata>::cal_exx_elec (const std::vector<std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>>& Ds,
                                  const UnitCell& ucell,
    ...
    for (int is = 0; is < PARAM.inp.nspin; ++is)
        {
            ...
            if (!p_symrot)
            ...
            else
                {
                    ...
                    this->exx_lri.energy
                        = this->exx_lri.post_2D.cal_energy (this->exx_lri.post_2D.saves["Ds_" + suffix],
...
template <typename Tdata>
void
    Exx_LRI<Tdata>::post_process_Hexx (std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>& Hexxs_io) const
{
...
template <typename Tdata>
double
    Exx_LRI<Tdata>::post_process_Eexx (const double& Eexx_in) const
{
...
template <typename Tdata>
void
    Exx_LRI<Tdata>::cal_exx_force (const int& nat)
{
...
template <typename Tdata>
void
    Exx_LRI<Tdata>::cal_exx_stress (const double& omega, const double& lat0)
{
