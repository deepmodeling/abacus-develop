...
void
    Exx_LRI_Interface<T, Tdata>::init (const MPI_Comm& mpi_comm,
...
template <typename T, typename Tdata>
void
    Exx_LRI_Interface<T, Tdata>::cal_exx_ions (const UnitCell& ucell, const bool write_cv)
{
    ModuleBase::TITLE ("Exx_LRI_Interface", "cal_exx_ions");
    if (!this->flag_finish.init)
        {
            throw std::runtime_error ("Exx init unfinished when " + std::string (__FILE__) + " line "
...
template <typename T, typename Tdata>
void
    Exx_LRI_Interface<T, Tdata>::cal_exx_elec (const std::vector<std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>>& Ds,
                                               const UnitCell& ucell,
...
template <typename T, typename Tdata>
void
    Exx_LRI_Interface<T, Tdata>::cal_exx_force (const int& nat)
{
    ...
    if (!this->flag_finish.elec)
        {
            throw std::runtime_error ("Exx Hamiltonian unfinished when " + std::string (__FILE__) + " line "
                                      + std::to_string (__LINE__));
        }
...
template <typename T, typename Tdata>
void
    Exx_LRI_Interface<T, Tdata>::cal_exx_stress (const double& omega, const double& lat0)
{
...
template <typename T, typename Tdata>
void
    Exx_LRI_Interface<T, Tdata>::exx_before_all_runners (const K_Vectors& kv,
                                                         const UnitCell& ucell,
    ...
    if (this->exx_spacegroup_symmetry)
        {
            ...
            this->symrot_.find_irreducible_sector (ucell.symm,
                                                   ucell.atoms,
                                                   ucell.st,
...
template <typename T, typename Tdata>
void
    Exx_LRI_Interface<T, Tdata>::exx_beforescf (const int istep,
                                                const K_Vectors& kv,
                                                ...
                                                const LCAO_Orbitals& orb)
{
    ...
    if (GlobalC::exx_info.info_global.cal_exx)
        {
            if ((GlobalC::restart.info_load.load_H_finish && !GlobalC::restart.info_load.restart_exx) || (istep > 0)
...
template <typename T, typename Tdata>
void
    Exx_LRI_Interface<T, Tdata>::exx_eachiterinit (const int istep,
                                                   const UnitCell& ucell,
                                                   ...
                                                   const int& iter)
{
    ...
    if (GlobalC::exx_info.info_global.cal_exx)
        {
            if (!GlobalC::exx_info.info_global.separate_loop
                    && (this->two_level_step || istep > 0 || PARAM.inp.init_wfc == "file") // non separate loop case
                || (GlobalC::exx_info.info_global.separate_loop && PARAM.inp.init_wfc == "file"
                    && this->two_level_step == 0 && iter == 1)) // the first iter in separate loop case
                {
                    ...
                    auto cal = [this, &ucell, &kv, &flag_restart] (const elecstate::DensityMatrix<T, double>& dm_in)
                        {
                            if (this->exx_spacegroup_symmetry)
                                {
                                    this->mix_DMk_2D.mix (
                                        symrot_.restore_dm (kv, dm_in.get_DMK_vector (), *dm_in.get_paraV_pointer ()),
                                        flag_restart);
                                }
                            else
                                {
...
template <typename T, typename Tdata>
void
    Exx_LRI_Interface<T, Tdata>::exx_hamilt2rho (elecstate::ElecState& elec,
                                                 const Parallel_Orbitals& pv,
                                                 const int iter)
{
                    ...
                    if (GlobalV::MY_RANK == 0)
                        {
                            try
                                ...
                                {
                                    std::cout << "WARNING: Cannot read Eexx from disk, the energy of the 1st loop will "
                                                 "be wrong, sbut it does not influence the subsequent loops."
                                              << std::endl;
                                }
                        }
...
template <typename T, typename Tdata>
void
    Exx_LRI_Interface<T, Tdata>::exx_iter_finish (const K_Vectors& kv,
                                                  const UnitCell& ucell,
                                                  ...
                                                  bool& conv_esolver)
{
    ...
    if (GlobalC::exx_info.info_global.cal_exx && conv_esolver)
        {
            ...
            conv_esolver = this->exx_after_converge (ucell,
                                                     hamilt,
                                                     *dm,
                                                     kv,
                                                     PARAM.inp.nspin,
    ...
    // else if ( PARAM.inp.rdmft && two_level_step ) { conv_esolver = true; }    // for RDMFT in the future to quit
    // after the first iter of the exx-loop
}

template <typename T, typename Tdata>
bool
    Exx_LRI_Interface<T, Tdata>::exx_after_converge (const UnitCell& ucell,
                                                     hamilt::Hamilt<T>& hamilt,
                                                     const elecstate::DensityMatrix<T, double>& dm,
                                                     const K_Vectors& kv,
                                                     const int& nspin,
        ...
        { // has separate_loop case
            ...
            if (this->two_level_step == GlobalC::exx_info.info_global.hybrid_step
            ...
            else
                {
                    ...
                    // if init_wfc == "file", DM is calculated in the 1st iter of the 1st two-level step, so we mix it
                    // here
                    const bool flag_restart
                        = (this->two_level_step == 0 && PARAM.inp.init_wfc != "file") ? true : false;

                    if (this->exx_spacegroup_symmetry)
                        {
                            this->mix_DMk_2D.mix (
                                symrot_.restore_dm (kv, dm.get_DMK_vector (), *dm.get_paraV_pointer ()),
                                flag_restart);
                        }
                    else
                        {
                    ...
                    const std::vector<std::map<int, std::map<std::pair<int, std::array<int, 3>>, RI::Tensor<Tdata>>>> Ds
                        = std::is_same<T, double>::value // gamma_only_local
                              ? RI_2D_Comm::split_m2D_ktoR<Tdata> (ucell,
                                                                   *this->exx_ptr->p_kv,
                                                                   this->mix_DMk_2D.get_DMk_gamma_out (),
                    ...
                    if (this->exx_spacegroup_symmetry && GlobalC::exx_info.info_ri.exx_symmetry_realspace)
                        ...
                        {
                            this->cal_exx_elec (Ds, ucell, *dm.get_paraV_pointer ());
                        } // restore DM but not Hexx
                    iter = 0;
                    this->two_level_step++;
