    ...
    if (GlobalC::exx_info.info_global.cal_exx) {
                ...
                this->two_level_step == 0 && iter == 1)) // the first iter in separate loop case
        {
            ...
            auto cal = [this, &ucell, &kv, &flag_restart](const elecstate::DensityMatrix<T, double>& dm_in) {
                if (this->exx_spacegroup_symmetry) {
                    this->mix_DMk_2D.mix(symrot_.restore_dm(kv, dm_in.get_DMK_vector(), *dm_in.get_paraV_pointer()),
                                         flag_restart);
                } else {
        ...
        if (GlobalC::restart.info_load.load_H_finish && !GlobalC::restart.info_load.restart_exx &&
            ...
            if (GlobalV::MY_RANK == 0) {
                try {
                    ...
                    std::cout << "WARNING: Cannot read Eexx from disk, the energy of the 1st loop will be wrong, sbut "
                                 "it does not influence the subsequent loops."
                              << std::endl;
                }
            }
...
void Exx_LRI_Interface<T, Tdata>::exx_iter_finish(const K_Vectors& kv,
    ...
    if (GlobalC::exx_info.info_global.cal_exx && conv_esolver) {
        ...
        conv_esolver =
            this->exx_after_converge(ucell, hamilt, *dm, kv, PARAM.inp.nspin, iter, istep, elec.f_en.etot, scf_ene_thr);
    }
...
bool Exx_LRI_Interface<T, Tdata>::exx_after_converge(
    ...
    const int& istep,
    const double& etot,
    const double& scf_ene_thr) { // only called if (GlobalC::exx_info.info_global.cal_exx)
    ModuleBase::TITLE("Exx_LRI_Interface", "exx_after_converge");
    auto restart_reset = [this]() { // avoid calling restart related procedure in the subsequent ion steps
    ...
    if (!GlobalC::exx_info.info_global.separate_loop) {
            ...
            || (ediff < scf_ene_thr && this->two_level_step != 0)) // energy convergence of outer loop
        {
            ...
            if (this->exx_spacegroup_symmetry) {
                this->mix_DMk_2D.mix(symrot_.restore_dm(kv, dm.get_DMK_vector(), *dm.get_paraV_pointer()),
                                     flag_restart);
            } else {
