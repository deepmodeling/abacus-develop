...
template <>
void
    tensor_syev<double> (char jobz, char uplo, RI::Tensor<double>& a, double* w, int& info)
{
...
template <>
void
    tensor_syev<float> (char jobz, char uplo, RI::Tensor<float>& a, float* w, int& info)
{
    assert (a.shape.size () == 2);
    assert (a.shape[0] == a.shape[1]);
...
template <>
void
    tensor_syev<std::complex<double>> (char jobz, char uplo, RI::Tensor<std::complex<double>>& a, double* w, int& info)
{
    assert (a.shape.size () == 2);
    assert (a.shape[0] == a.shape[1]);
...
template <>
void
    tensor_syev<std::complex<float>> (char jobz, char uplo, RI::Tensor<std::complex<float>>& a, float* w, int& info)
{
    assert (a.shape.size () == 2);
    assert (a.shape[0] == a.shape[1]);
...
RI::Tensor<double>
    get_sub_matrix (const RI::Tensor<double>& m, // size: (lcaos, lcaos, abfs)
                    const std::size_t& T,
                    const std::size_t& L,
...
RI::Tensor<double>
    get_column_mean0_matrix (const RI::Tensor<double>& m)
{
...
std::vector<std::vector<std::pair<std::vector<double>, RI::Tensor<double>>>>
    cal_PCA (const UnitCell& ucell,
             ...
             const double kmesh_times)
{
    ...
    for (std::size_t T = 0; T != abfs.size (); ++T)
        {
            const RI::Tensor<double> A
                = m_abfslcaos_lcaos.cal_overlap_matrix<double> (T,
                                                                T,
