...
class Exx_LRI_Interface
{
  public:
    ...
    Exx_LRI_Interface (const Exx_Info::Exx_Info_RI& info) { this->exx_ptr = std::make_shared<Exx_LRI<Tdata>> (info); }
    ...
    std::vector<std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>>&
        get_Hexxs () const
    {
        return this->exx_ptr->Hexxs;
    }
    double&
        get_Eexx () const
    {
        return this->exx_ptr->Eexx;
    }
    ModuleBase::matrix&
        get_force () const
    {
        return this->exx_ptr->force_exx;
    }
    ModuleBase::matrix&
        get_stress () const
    {
        return this->exx_ptr->stress_exx;
    }
    ...
    /// since k-points are not reduced again after the variation of the cell and exx-symmetry must be consistent with
    /// k-points. In the future, we will reduce k-points again during cell-relax, then this setting can be moved to
    /// `exx_beforescf`.
    ...
    void exx_beforescf (const int istep,
                        const K_Vectors& kv,
