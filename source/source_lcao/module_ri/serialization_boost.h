...
inline void
    serialize (Archive& ar, ModuleBase::Vector3<T>& v, const unsigned int version)
{
    ar & v.x;
    ar & v.y;
    ar & v.z;
...
template <typename Archive, typename T>
inline void
    serialize (Archive& ar, Abfs::Vector3_Order<T>& v, const unsigned int version)
{
    ar & v.x;
    ar & v.y;
    ar & v.z;
...
inline void
    save (Archive& ar, const ModuleBase::matrix& m, const unsigned int /*file_version*/)
{
...
inline void
    load (Archive& ar, ModuleBase::matrix& m, const unsigned int /*file_version*/)
{
...
template <class Archive>
inline void
    serialize (Archive& ar, ModuleBase::matrix& m, const unsigned int file_version)
{
...
template <typename Archive>
inline void
    save (Archive& ar, const ModuleBase::Matrix_Wrapper& m, const unsigned int /*file_version*/)
{
    const collection_size_type nr (m.nr), nc (m.nc);
    ar << BOOST_SERIALIZATION_NVP (nr) << BOOST_SERIALIZATION_NVP (nc);
    if (nr && nc)
...
template <class Archive>
inline void
    load (Archive& ar, ModuleBase::Matrix_Wrapper& m, const unsigned int /*file_version*/)
{
    collection_size_type nr, nc;
    ar >> BOOST_SERIALIZATION_NVP (nr) >> BOOST_SERIALIZATION_NVP (nc);
    m.create (nr, nc, false);
...
template <class Archive>
inline void
    serialize (Archive& ar, ModuleBase::Matrix_Wrapper& m, const unsigned int file_version)
{
    boost::serialization::split_free (ar, m, file_version);
}
} // namespace serialization
