...
template <typename Tdata>
RI::Tensor<Tdata>
    LRI_CV_Tools::transform_Rm (const RI::Tensor<Tdata>& V)
{
    return V.transpose ();
}
...
template <typename Tdata>
bool
    LRI_CV_Tools::exist (const RI::Tensor<Tdata>& V)
{
    return !V.empty ();
}
...
bool
    LRI_CV_Tools::exist (const std::array<T, N>& dV)
{
...
template <typename Tdata>
RI::Tensor<Tdata>
    LRI_CV_Tools::mul1 (const RI::Tensor<Tdata>& t1, const RI::Tensor<Tdata>& t2)
{
...
template <typename T>
std::array<T, 3>
    LRI_CV_Tools::mul1 (const std::array<T, 3>& t1, const T& t2)
{
...
template <typename T1, typename T2>
std::array<T2, 3>
    LRI_CV_Tools::mul2 (const T1& t1, const std::array<T2, 3>& t2)
{
...
template <typename T>
RI::Tensor<T>
    LRI_CV_Tools::mul2 (const T& t1, const RI::Tensor<T>& t2)
{
    return t1 * t2;
}
...
std::map<TkeyA, std::map<TkeyB, Tvalue>>
    LRI_CV_Tools::mul2 (const T& t1, const std::map<TkeyA, std::map<TkeyB, Tvalue>>& t2)
{
...
std::vector<std::array<T, N>>
    LRI_CV_Tools::minus (const std::vector<std::array<T, N>>& v1, const std::vector<std::array<T, N>>& v2)
{
    assert (v1.size () == v2.size ());
...
std::map<TkeyA, std::map<TkeyB, Tvalue>>
    LRI_CV_Tools::minus (std::map<TkeyA, std::map<TkeyB, Tvalue>>& v1, std::map<TkeyA, std::map<TkeyB, Tvalue>>& v2)
{
    assert (v1.size () == v2.size ());
...
template <typename T, std::size_t N>
std::vector<std::array<T, N>>
    LRI_CV_Tools::add (const std::vector<std::array<T, N>>& v1, const std::vector<std::array<T, N>>& v2)
{
    assert (v1.size () == v2.size ());
    std::vector<std::array<T, N>> v (v1.size ());
    for (std::size_t i = 0; i < v.size (); ++i)
...
template <typename TkeyA, typename TkeyB, typename Tvalue>
std::map<TkeyA, std::map<TkeyB, Tvalue>>
    LRI_CV_Tools::add (std::map<TkeyA, std::map<TkeyB, Tvalue>>& v1, std::map<TkeyA, std::map<TkeyB, Tvalue>>& v2)
{
    assert (v1.size () == v2.size ());
    using namespace RI::Map_Operator;
    using namespace RI::Array_Operator;
...
template <typename T, std::size_t N>
std::array<T, N>
    LRI_CV_Tools::negative (const std::array<T, N>& v_in)
{
    std::array<T, N> v_out;
    for (std::size_t i = 0; i < N; ++i)
        v_out[i] = -v_in[i];
...
template <typename Tdata>
RI::Tensor<Tdata>
    LRI_CV_Tools::transpose12 (const RI::Tensor<Tdata>& c_in)
{
...
template <typename T, std::size_t N>
std::array<T, N>
    LRI_CV_Tools::transpose12 (const std::array<T, N>& c_in)
{
    std::array<T, N> c_out;
    for (size_t i = 0; i < N; ++i)
...
template <typename T, std::size_t N>
void
    LRI_CV_Tools::init_elem (std::array<RI::Tensor<T>, N>& data, const size_t ndim0, const size_t ndim1)
{
    for (size_t i = 0; i < N; ++i)
        {
...
template <typename T, std::size_t N>
void
    LRI_CV_Tools::add_elem (std::array<T, N>& data, const T& val, const T& frac)
{
    for (size_t i = 0; i < N; ++i)
        data[i] += frac * val;
}
...
template <typename T, std::size_t N>
void
    LRI_CV_Tools::add_elem (std::array<RI::Tensor<T>, N>& data,
                            const int lmp,
                            const int lmq,
...
RI::Tensor<Tout>
    LRI_CV_Tools::convert (RI::Tensor<Tin>&& data)
{
