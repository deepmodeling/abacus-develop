    ...
    for (int i = 0; i < nr; ++i)
        {
            for (int j = 0; j < nc; ++j)
                {
                    EXPECT_DOUBLE_EQ (data_out (i, j),
                                      (1 - mixing_beta) * mix_data[0](i, j) + mixing_beta * mix_data[1](i, j));
                }
        }
    // vector<double>
            ...
            for (int j = 0; j < nc; ++j)
                {
                    ...
                    std::complex<double> ref
                        = second_step_result * mixing->coef[1] + first_step_result * mixing->coef[0];
