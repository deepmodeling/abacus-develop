...
std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>>
    Exx_Abfs::IO::construct_abfs (const LCAO_Orbitals& orbs,
...
std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>>
    Exx_Abfs::IO::construct_abfs (const std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>>& abfs_pre,
...
std::vector<std::vector<Numerical_Orbital_Lm>>
    Exx_Abfs::IO::construct_abfs_T (const std::string& file_name,
                                    const int& T,
                                    const int& nk,
                                    const double& dk,
            ...
            for (size_t N = 0; N != N_size[L]; ++N)
                {
                    if (psis.find (L) == psis.end () || psis[L].find (N) == psis[L].end ())
                        {
                            throw std::domain_error ("Can't find abf of L=" + std::to_string (L)
                                                     + " T=" + std::to_string (T) + " in " + file_name);
                        }
                }
        }
