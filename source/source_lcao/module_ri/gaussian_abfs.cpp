...
void
    Gaussian_Abfs::init (const UnitCell& ucell,
                         const int& Lmax,
    ...
    for (size_t ik = 0; ik != nks0; ++ik)
        {
            ...
            int total_cells = std::accumulate (n_supercells.begin (),
                                               n_supercells.end (),
                                               1,
...
auto
    Gaussian_Abfs::get_Vq (const int& lp_max,
...
auto
    Gaussian_Abfs::get_dVq (const int& lp_max,
                            const int& lq_max, // Maximum L for which to calculate interaction.
                            const size_t& ik,
                            const double& chi, // Singularity corrected value at q=0.
                            const ModuleBase::Vector3<double>& tau,
...
auto
    Gaussian_Abfs::DPcal_Vq_dVq (const double& omega,
                                 const int& lp_max,
                                 const int& lq_max, // Maximum L for which to calculate interaction.
                                 const size_t& ik,
                                 const double& chi, // Singularity corrected value at q=0.
    ...
    */
    ...
    for (int i_add_ksq = 0; i_add_ksq != n_add_ksq + 1; ++i_add_ksq) // integrate lp, lq, L to one index i_add_ksq, i.e.
                                                                     // (lp+lq-L)/2
        {
            ...
            const bool exclude_Gamma = (qvec.norm () < 1e-10 && i_add_ksq == 0); // only Gamma point and lq+lp-2>0 need
                                                                                 // to be corrected
...
Numerical_Orbital_Lm
    Gaussian_Abfs::Gauss (const Numerical_Orbital_Lm& orb, const double& lambda)
{
...
double
    Gaussian_Abfs::double_factorial (const int& n)
{
    double result = 1.0;
    for (int i = n; i > 0; i -= 2)
        {
...
auto
    Gaussian_Abfs::get_lattice_sum (const double& tpiba,
                                    const size_t& ik,
...
std::vector<int>
    Gaussian_Abfs::get_n_supercells (const double& lat0, const ModuleBase::Matrix3& G, const double& Gmax)
{
