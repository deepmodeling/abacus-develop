...
void
    Matrix_Orbs21::init (const std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>>& orb_A1,
                                                    ...
                                                    for (int LB = 0; LB != orb_B[TB].size (); ++LB)
                                                        {
                                                            for (size_t NB = 0; NB != orb_B[TB][LB].size (); ++NB)
                                                                {
                                                                    center2_orb21_s[TA][TB][LA1][NA1][LA2][NA2][LB]
...
void
    Matrix_Orbs21::init_radial_table (const std::map<size_t, std::map<size_t, std::set<double>>>& Rs)
{
            ...
            for (const auto& RsB: RsA.second)
                {
                    if (auto* const center2_orb21_sAB = static_cast<std::map<
                            ...
                            std::map<size_t,
                                     std::map<int,
                                              std::map<size_t,
                                                       std::map<int, std::map<size_t, Center2_Orb::Orb21>>>>>>* const> (
