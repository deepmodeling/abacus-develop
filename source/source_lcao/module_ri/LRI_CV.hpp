...
template <typename Tdata>
void
    LRI_CV<Tdata>::set_orbitals (const UnitCell& ucell,
                                 ...
                                 const bool& init_C)
{
    ...
    this->m_abfs_abfs.init (this->abfs_ccp, this->abfs, ucell, orb, kmesh_times);
    if (init_C)
        this->m_abfslcaos_lcaos.init (this->abfs_ccp, this->lcaos, this->lcaos, ucell, orb, kmesh_times);
...
template <typename Tdata>
double
    LRI_CV<Tdata>::cal_V_Rcut (const int it0, const int it1)
{
...
template <typename Tdata>
double
    LRI_CV<Tdata>::cal_C_Rcut (const int it0, const int it1)
{
...
auto
    LRI_CV<Tdata>::cal_datas (const UnitCell& ucell,
...
template <typename Tdata>
auto
    LRI_CV<Tdata>::cal_Vs (const UnitCell& ucell,
                           const std::vector<TA>& list_A0,
                           const std::vector<TAC>& list_A1,
...
template <typename Tdata>
auto
    LRI_CV<Tdata>::cal_dVs (const UnitCell& ucell,
                            const std::vector<TA>& list_A0,
                            const std::vector<TAC>& list_A1,
...
template <typename Tdata>
auto
    LRI_CV<Tdata>::cal_Cs_dCs (const UnitCell& ucell,
                               const std::vector<TA>& list_A0,
                               const std::vector<TAC>& list_A1,
...
To11
    LRI_CV<Tdata>::DPcal_o11 (const int it0,
                              const int it1,
            ...
            if (LRI_CV_Tools::exist (o11_transform_read))
                {
                    ...
                    if (flag_writable_o11ws) // such write may be deleted for memory saving with transform_Rm() every
                                             // time
                        {
...
template <typename Tdata>
RI::Tensor<Tdata>
    LRI_CV<Tdata>::DPcal_V (const int it0,
                            const int it1,
                            const Abfs::Vector3_Order<double>& R,
                            const std::map<std::string, bool>& flags) // "writable_Vws"
{
...
std::array<RI::Tensor<Tdata>, 3>
    LRI_CV<Tdata>::DPcal_dV (const int it0,
                             const int it1,
                             const Abfs::Vector3_Order<double>& R,
                             const std::map<std::string, bool>& flags) // "writable_dVws"
{
    if (ModuleBase::Vector3<double> (0, 0, 0) == R)
        {
            ...
            const std::array<RI::Tensor<Tdata>, 3> dV = {RI::Tensor<Tdata> ({size, size}),
...
std::pair<RI::Tensor<Tdata>, std::array<RI::Tensor<Tdata>, 3>>
    LRI_CV<Tdata>::DPcal_C_dC (const int it0,
                               const int it1,
                               const Abfs::Vector3_Order<double>& R,
                               const std::map<std::string, bool>&
                                   flags) // "cal_dC", "writable_Cws", "writable_dCws" + "writable_Vws", "writable_dVws"
    ...
    if (!C_read.empty () && flag_finish_dC)
    ...
    else
        {
            if ((ModuleBase::Vector3<double> (0, 0, 0) == R) && (it0 == it1))
                {
                    const RI::Tensor<Tdata> A = this->m_abfslcaos_lcaos.template cal_overlap_matrix<Tdata> (
                        it0,
                        it1,
                        {0, 0, 0},
                        {0, 0, 0},
                    ...
                    if (GlobalC::exx_info.info_ri.Cs_inv_thr > 0)
                        L = LRI_CV_Tools::cal_I (V,
                                                 Inverse_Matrix<Tdata>::Method::syev,
                                                 GlobalC::exx_info.info_ri.Cs_inv_thr);
                    else
                        L = LRI_CV_Tools::cal_I (V);
                    ...
                    if (flag_finish_dC)
                    ...
                    else
                        {
                            const RI::Shape_Vector sizes = {this->index_abfs[it0].count_size,
                                                            ...
                                                            this->index_lcaos[it0].count_size};
                            const std::array<RI::Tensor<Tdata>, 3> dC ({RI::Tensor<Tdata> ({sizes}),
                            ...
                            if (flags.at ("writable_dCws"))
                                {
                                    ...
                                    this->dCws[it1][it0][Rm]
                                        = LRI_CV_Tools::negative (LRI_CV_Tools::transpose12 (dC[1]));
                                    pthread_rwlock_unlock (&this->rwlock_dCw);
                                }
