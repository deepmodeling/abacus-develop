...
inline RI::Tensor<double>
    tensor_conj (const RI::Tensor<double>& t)
{
    return t;
}
inline RI::Tensor<std::complex<double>>
    tensor_conj (const RI::Tensor<std::complex<double>>& t)
{
...
auto
    RI_2D_Comm::split_m2D_ktoR (const UnitCell& ucell,
                                const K_Vectors& kv,
                    ...
                    for (const int ik: ik_list)
                        {
                            ...
                            if (!spgsym)
                                {
                                    RI::Tensor<Tdata_m> mk_2D = RI_Util::Vector_to_Tensor<Tdata_m> (*mks_2D[ik],
                                                                                                    pv.get_col_size (),
                                                                                                    pv.get_row_size ());
                                    const Tdata_m frac
                                        = SPIN_multiple
                                          * RI::Global_Func::convert<Tdata_m> (std::exp (
                                              ...
                                              * (kv.kvec_c[ik] * (RI_Util::array3_to_Vector3 (cell) * ucell.latvec))));
                                    if (static_cast<int> (
                                            std::round (SPIN_multiple * kv.wk[ik] * kv.get_nkstot_full ()))
                                        == 2)
                                        {
                                ...
                                { // traverse kstar, ik means ik_ibz
                                    for (auto& isym_kvd: kv.kstars[ik % ik_list.size ()])
                                        {
                                            RI::Tensor<Tdata_m> mk_2D = RI_Util::Vector_to_Tensor<Tdata_m> (
                                            ...
                                            const Tdata_m frac
                                                = SPIN_multiple
                                                  * RI::Global_Func::convert<Tdata_m> (std::exp (
                                                      -ModuleBase::TWO_PI * ModuleBase::IMAG_UNIT
                                                      * ((isym_kvd.second * ucell.G)
                                                         * (RI_Util::array3_to_Vector3 (cell) * ucell.latvec))));
                                            set_mR_2D (mk_2D * frac);
                                            ++ik_full;
                                        }
                                }
                            ...
                            for (int iwt1_2D = 0; iwt1_2D != mR_2D.shape[1]; ++iwt1_2D)
                                {
                                    ...
                                    if (mR_a2D.empty ())
                                        {
                                            mR_a2D = RI::Tensor<Tdata> ({static_cast<size_t> (ucell.atoms[it0].nw),
...
void
    RI_2D_Comm::add_Hexx (const UnitCell& ucell,
                          const K_Vectors& kv,
                          const int ik,
                          const double alpha,
...
template <typename Tdata, typename TK>
void
    RI_2D_Comm::add_Hexx_td (const UnitCell& ucell,
                             const K_Vectors& kv,
                             const int ik,
                             const double alpha,
                             const std::vector<std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>>& Hs,
...
int
    RI_2D_Comm::get_is_block (const int is_k, const int is_row_b, const int is_col_b)
{
    switch (PARAM.inp.nspin)
        {
        case 1:
...
int
    RI_2D_Comm::get_iwt (const UnitCell& ucell, const int iat, const int iw_b, const int is_b)
{
...
void
    RI_2D_Comm::add_HexxR (const int current_spin,
                           const double alpha,
                           const std::vector<std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>>& Hs,
                           const Parallel_Orbitals& pv,
                           const int npol,
                                    ...
                                    for (int lw1_b = 0; lw1_b < col_indexes.size (); lw1_b += npol)
                                        {
                                            ...
                                            HlocR->add_element (lw0,
                                                                lw1,
                                                                RI::Global_Func::convert<TR> (HexxR (gw0, gw1)));
                                        }
                                }
                        }
                }
...
std::map<TA, std::map<TAC, T>>
    RI_2D_Comm::comm_map2_first (const MPI_Comm& mpi_comm,
...
std::map<TA, std::map<TAC, T>>
    RI_2D_Comm::comm_map2 (const MPI_Comm& mpi_comm, const std::map<TA, std::map<TAC, T>>& Ds_in, const Tjudge& judge)
{
...
void
    RI_2D_Comm::set_value_add (Tkey&& key, Tvalue&& value, std::map<Tkey, Tvalue>& data)
{
...
void
    RI_2D_Comm::set_value_add (std::tuple<Tkey0, Tkey1>&& key,
                               Tvalue&& value,
...
template <typename Tkey, typename Tvalue>
void
    RI_2D_Comm::add_datas (std::map<Tkey, Tvalue>&& data_local, std::map<Tkey, Tvalue>& data_recv)
{
    using namespace RI::Array_Operator;
...
template <typename Tkey0, typename Tkey1, typename Tvalue>
void
    RI_2D_Comm::add_datas (std::map<Tkey0, std::map<Tkey1, Tvalue>>&& data_local,
