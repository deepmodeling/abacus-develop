        ...
        for (const TC& cell: RI_Util::get_Born_von_Karmen_cells(period)) {
            ...
            for (const int ik: ik_list) {
                ...
                if (!spgsym) {
                    ...
                    const Tdata_m frac =
                        SPIN_multiple * RI::Global_Func::convert<Tdata_m>(std::exp(
                ...
                } else { // traverse kstar, ik means ik_ibz
                    for (auto& isym_kvd: kv.kstars[ik % ik_list.size()]) {
                        ...
                        const Tdata_m frac =
                            ...
                            RI::Global_Func::convert<Tdata_m>(std::exp(
                                -ModuleBase::TWO_PI * ModuleBase::IMAG_UNIT *
                                ((isym_kvd.second * ucell.G) * (RI_Util::array3_to_Vector3(cell) * ucell.latvec))));
                        set_mR_2D(mk_2D * frac);
                        ++ik_full;
                    }
                }
                ...
                for (int iwt1_2D = 0; iwt1_2D != mR_2D.shape[1]; ++iwt1_2D) {
                    ...
                    if (mR_a2D.empty()) {
                        mR_a2D = RI::Tensor<Tdata>(
                            {static_cast<size_t>(ucell.atoms[it0].nw), static_cast<size_t>(ucell.atoms[it1].nw)});
                    }
        ...
        for (const auto& Hs_tmpA: Hs[is_b]) {
            ...
            for (const auto& Hs_tmpB: Hs_tmpA.second) {
                ...
                const std::complex<double> frac =
                    alpha * std::exp(ModuleBase::TWO_PI * ModuleBase::IMAG_UNIT *
    ...
    for (const int is_b: is_list.at(PARAM.inp.nspin)) {
        ...
        for (const auto& Hs_tmpA: Hs[is_b]) {
            ...
            for (const auto& Hs_tmpB: Hs_tmpA.second) {
                ...
                const std::complex<double> frac =
                    alpha * std::exp(ModuleBase::IMAG_UNIT *
