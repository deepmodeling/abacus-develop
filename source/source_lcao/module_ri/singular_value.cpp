...
double
    sum_for_solve_chi (const std::vector<ModuleBase::Vector3<double>>& kvec_c,
...
double
    solve_chi (const ModuleBase::Matrix3& G,
...
// for analytic integral of fq
double
    solve_chi (const std::vector<ModuleBase::Vector3<double>>& kvec_c,
               const T_cal_fq_type& func_cal_fq,
               const double& fq_int)
{
    return sum_for_solve_chi (kvec_c, func_cal_fq, fq_int);
...
double
    solve_chi (const int& nks, const T_cal_fq_type_no& func_cal_fq, const double& fq_int)
{
...
double
    cal_carrier (const UnitCell& ucell,
                 const std::vector<ModuleBase::Vector3<double>>& kvec_c,
                 const int& qdiv,
                 const double& qdense,
                 const int& niter,
...
double
    fq_massidda (const double& tpiba,
...
double
    cal_massidda (const UnitCell& ucell,
...
double
    Iter_Integral (const ModuleBase::Matrix3& G,
                   const T_cal_fq_type& func_cal_fq,
                   const std::array<int, 3>& nq_arr,
                   const int& niter,
                   const double& eps,
            ...
            for (int ig1 = -nq_arr[0]; ig1 != nq_arr[0] + 1; ++ig1)
                for (int ig2 = -nq_arr[1]; ig2 != nq_arr[1] + 1; ++ig2)
                    for (int ig3 = -nq_arr[2]; ig3 != nq_arr[2] + 1; ++ig3)
                        {
                            if (std::abs (ig1) <= nq_arr_in[0] && std::abs (ig2) <= nq_arr_in[1]
