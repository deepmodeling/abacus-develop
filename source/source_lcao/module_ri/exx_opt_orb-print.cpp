...
void Exx_Opt_Orb::print_matrix(const Exx_Info::Exx_Info_Opt_ABFs& info,
    ...
    auto print_header = [&](std::ofstream& ofs) {
        ...
        if (TA == TB) {
            ...
            if (IA == IB) {
                ofs << 1 << " na" << std::endl;
                ofs << ucell.atoms[TA].tau[IA].x << " " << ucell.atoms[TA].tau[IA].y << " " << ucell.atoms[TA].tau[IA].z
                    << std::endl;
            } else {
                ofs << 2 << " na" << std::endl;
                ofs << ucell.atoms[TA].tau[IA].x << " " << ucell.atoms[TA].tau[IA].y << " " << ucell.atoms[TA].tau[IA].z
                    << std::endl;
                ofs << ucell.atoms[TB].tau[IB].x << " " << ucell.atoms[TB].tau[IB].y << " " << ucell.atoms[TB].tau[IB].z
                    << std::endl;
            }
        } else {
                            ...
                            for (std::size_t im2 = 0; im2 != range_jles[it2][il2].M; ++im2) {
                                for (std::size_t iq1 = 0; iq1 != range_jles[it1][il1].N; ++iq1) {
                                    for (std::size_t iq2 = 0; iq2 != range_jles[it2][il2].N; ++iq2) {
                                        ofs << matrix_S[iat1][iat2](index_jles[it1][il1][iq1][im1],
    ...
    std::ofstream ofs(file_name + "_" + std::to_string(TA) + "_" + std::to_string(IA) + "_" + std::to_string(TB) + "_" +
                      std::to_string(IB));
    print_header(ofs);
