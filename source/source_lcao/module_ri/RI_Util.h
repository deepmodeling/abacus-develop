...
namespace RI_Util {
inline extern std::array<int, 3> get_Born_vonKarmen_period(const K_Vectors& kv);
...
inline std::array<Tcell, 3> Vector3_to_array3(const ModuleBase::Vector3<Tcell>& v) {
    return std::array<Tcell, 3>{v.x, v.y, v.z};
}
template <typename Tcell>
inline ModuleBase::Vector3<Tcell> array3_to_Vector3(const std::array<Tcell, 3>& v) {
...
std::map<Conv_Coulomb_Pot_K::Coulomb_Type, std::vector<std::map<std::string, std::string>>> update_coulomb_param(
