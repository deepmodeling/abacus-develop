...
std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>>
    Exx_Abfs::Jle::init_jle (const Exx_Info::Exx_Info_Opt_ABFs& info,
    ...
    for (int T = 0; T < ucell.ntype; ++T)
        {
            ...
            for (int L = 0; L <= info.abfs_Lmax; ++L)
                {
                    const size_t ecut_number = static_cast<size_t> (std::sqrt (info.ecut_exx) * orb.Phi[T].getRcut ()
                                                                    / ModuleBase::PI); // Rydberg Unit.
                    if (ecut_number <= 0)
                        {
                            continue;
                        }
