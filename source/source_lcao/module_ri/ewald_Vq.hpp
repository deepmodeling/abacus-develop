...
template <typename Tdata>
void
    Ewald_Vq<Tdata>::init (const UnitCell& ucell,
                           ...
                           const std::map<Conv_Coulomb_Pot_K::Coulomb_Type,
                                          std::vector<std::map<std::string, std::string>>>& coulomb_param_in,
                           ...
                           const double& kmesh_times_in)
{
    ...
    this->g_abfs_ccp = Conv_Coulomb_Pot_K::cal_orbs_ccp (this->g_abfs, this->coulomb_param, this->ccp_rmesh_times);
    ...
    const ModuleBase::Element_Basis_Index::Range range_abfs
        = ModuleBase::Element_Basis_Index::construct_range (abfs_in);
    this->index_abfs = ModuleBase::Element_Basis_Index::construct_index (range_abfs);

    this->cv.set_orbitals (ucell, orb, this->g_lcaos, this->g_abfs, this->g_abfs_ccp, kmesh_times_in, MGT_in, false);
...
template <typename Tdata>
void
    Ewald_Vq<Tdata>::init_ions (const UnitCell& ucell, const std::array<Tcell, Ndim>& period_Vs_NAO)
{
    ...
    this->gaussian_abfs.init (ucell,
                              2 * GlobalC::exx_info.info_ri.abfs_Lmax + 1,
                              neg_kvec,
                              ucell.G,
                              this->ewald_lambda);
...
template <typename Tdata>
double
    Ewald_Vq<Tdata>::get_singular_chi (const UnitCell& ucell,
...
template <typename Tdata>
auto
    Ewald_Vq<Tdata>::cal_Vs_gauss (const UnitCell& ucell,
...
template <typename Tdata>
auto
    Ewald_Vq<Tdata>::cal_dVs_gauss (const UnitCell& ucell,
                                    const std::vector<TA>& list_A0,
...
template <typename Tdata>
auto
    Ewald_Vq<Tdata>::cal_Vs_minus_gauss (const UnitCell& ucell,
                                         const std::vector<TA>& list_A0,
                                         const std::vector<TAC>& list_A1,
...
template <typename Tdata>
auto
    Ewald_Vq<Tdata>::cal_dVs_minus_gauss (const UnitCell& ucell,
                                          const std::vector<TA>& list_A0,
                                          const std::vector<TAC>& list_A1,
...
template <typename Tdata>
double
    Ewald_Vq<Tdata>::cal_V_Rcut (const int it0, const int it1)
{
...
template <typename Tdata>
double
    Ewald_Vq<Tdata>::get_Rcut_max (const int it0, const int it1)
{
...
auto
    Ewald_Vq<Tdata>::set_Vs_dVs_minus_gauss (const UnitCell& ucell,
                                             const std::vector<TA>& list_A0,
                                             const std::vector<TAC>& list_A1,
                                            ...
                                            for (size_t n0 = 0; n0 != this->g_abfs_ccp.at (it0).at (l0).size (); ++n0)
                                                {
                                                    ...
                                                    for (size_t n1 = 0; n1 != this->g_abfs.at (it1).at (l1).size ();
                                                         ++n1)
                                                        {
                                                            const double pB = this->multipole.at (it1).at (l1).at (n1);
                                                            Tin_convert pp
                                                                = RI::Global_Func::convert<Tin_convert> (pA * pB);
                                                            for (size_t m0 = 0; m0 != 2 * l0 + 1; ++m0)
                                                                {
                                                                    for (size_t m1 = 0; m1 != 2 * l1 + 1; ++m1)
                                                                        {
                                                                            const size_t index0
                                                                                = this->index_abfs.at (it0)
                                                                                      .at (l0)
                                                                                      .at (n0)
                                                                                      .at (m0);
                                                                            const size_t index1
                                                                                = this->index_abfs.at (it1)
                                                                                      .at (l1)
                                                                                      .at (n1)
                                                                                      .at (m1);
...
template <typename Tdata>
auto
    Ewald_Vq<Tdata>::cal_Vq_gauss (const UnitCell& ucell,
...
template <typename Tdata>
auto
    Ewald_Vq<Tdata>::cal_dVq_gauss (const UnitCell& ucell,
                                    const std::vector<TA>& list_A0_k,
                                    const std::vector<TAK>& list_A1_k,
                                    const double& chi,
                                    const int& shift_for_mpi)
...
template <typename Tresult>
auto
    Ewald_Vq<Tdata>::set_Vq_dVq_gauss (const UnitCell& ucell,
                                       const std::vector<TA>& list_A0_k,
                                       const std::vector<TAK>& list_A1_k,
                                       const int& shift_for_mpi,
                    ...
                    for (size_t i1 = 0; i1 < list_A1_k.size (); ++i1)
                        {
                            ...
                            auto data = func_DPget_Vq_dVq (this->g_abfs_ccp.at (it0).size () - 1,
                                                           this->g_abfs.at (it1).size () - 1,
                                                           ik,
                                                           tau);
...
template <typename Tdata>
auto
    Ewald_Vq<Tdata>::cal_Vq_minus_gauss (const UnitCell& ucell,
                                         const std::vector<TA>& list_A0,
                                         const std::vector<TAC>& list_A1,
                                         std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>& Vs_minus_gauss)
        -> std::map<TA, std::map<TAK, RI::Tensor<std::complex<double>>>>
...
template <typename Tdata>
auto
    Ewald_Vq<Tdata>::cal_dVq_minus_gauss (
...
auto
    Ewald_Vq<Tdata>::set_Vq_dVq_minus_gauss (const UnitCell& ucell,
                                             const std::vector<TA>& list_A0,
                                             const std::vector<TAC>& list_A1,
                        ...
                        for (size_t i1 = 0; i1 < list_A1.size (); ++i1)
                            {
                                ...
                                const ModuleBase::Vector3<double> tau1 = ucell.atoms[it1].tau[ia1];
                                const double Rcut
                                    = std::min (this->get_Rcut_max (it0, it1), this->get_Rcut_max (it1, it0));
                                ...
                                if (R_delta.norm () * ucell.lat0 < Rcut)
                                    {
                                        ...
                                        Tout Vs_dVs_tmp
                                            = LRI_CV_Tools::mul2 (phase,
                                                                  LRI_CV_Tools::convert<Tin_convert> (std::move (
                                                                      Vs_dVs_minus_gauss.at (iat0).at (list_A1[i1]))));
...
template <typename Tdata>
auto
    Ewald_Vq<Tdata>::cal_Vq (const UnitCell& ucell,
                             const double& chi,
                             std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>& Vs_in)
        -> std::map<TA, std::map<TAK, RI::Tensor<std::complex<double>>>>
{
...
template <typename Tdata>
auto
    Ewald_Vq<Tdata>::cal_dVq (const UnitCell& ucell,
                              const double& chi,
                              std::map<TA, std::map<TAC, std::array<RI::Tensor<Tdata>, Ndim>>>& dVs_in)
        -> std::map<TA, std::map<TAK, std::array<RI::Tensor<std::complex<double>>, Ndim>>>
{
...
template <typename Tout, typename Tin>
auto
    Ewald_Vq<Tdata>::set_Vq_dVq (const UnitCell& ucell,
                                            ...
                                            for (size_t n1 = 0; n1 != this->g_abfs.at (it1).at (l1).size (); ++n1)
                                                {
                                                    ...
                                                    for (size_t m0 = 0; m0 != 2 * l0 + 1; ++m0)
                                                        {
                                                            const size_t index0
                                                                = this->index_abfs.at (it0).at (l0).at (n0).at (m0);
                                                            const size_t lm0 = l0 * l0 + m0;
                                                            for (size_t m1 = 0; m1 != 2 * l1 + 1; ++m1)
                                                                {
                                                                    const size_t index1
                                                                        = this->index_abfs.at (it1).at (l1).at (n1).at (
                                                                            m1);
                                                                    ...
                                                                    LRI_CV_Tools::add_elem (
                                                                        ...
                                                                        index0,
                                                                        index1,
                                                                        Vq_dVq_gauss.at (list_A0_pair_k[i0])
                                                                            .at (list_A1_pair_k[i1]),
                                                                        lm0,
                                                                        lm1,
                                                                        frac);
                                                                }
...
#pragma omp critical(Ewald_Vq_set_Vq_dVq)
                    if (LRI_CV_Tools::exist (Vq_dVq_minus_gauss.at (list_A0_pair_k[i0]).at (re_index)))
                        Vq_dVq[list_A0_pair_k[i0]][re_index]
                            = Vq_dVq_minus_gauss.at (list_A0_pair_k[i0]).at (re_index) + data;
                }
        }
...
template <typename Tdata>
auto
    Ewald_Vq<Tdata>::cal_Vs (const UnitCell& ucell,
                             const double& chi,
...
template <typename Tout, typename Tin>
auto
    Ewald_Vq<Tdata>::set_Vs_dVs (const UnitCell& ucell,
...
std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>>
    Ewald_Vq<Tdata>::init_gauss (std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>>& orb_in)
{
