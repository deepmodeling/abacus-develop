...
void
    Moment_abfs<Tdata>::cal_multipole (const std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>>& orb_in)
{
                    ...
                    for (size_t N = 0; N != orb_in[T][L].size (); ++N)
                        {
                            ...
                            for (size_t ir = 0; ir != nr; ++ir)
                                integrated_func[ir]
                                    = orb_lm.getPsi (ir) * std::pow (orb_lm.getRadial (ir), 2 + L) / (2 * L + 1);

                            ModuleBase::Integral::Simpson_Integral (nr,
                                                                    integrated_func,
                                                                    orb_lm.getRab (),
                                                                    this->multipole[T][L][N]);
                        }
                }
        }
...
template <typename Tdata>
void
    Moment_abfs<Tdata>::rotate_abfs (std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>>& orb_in)
{
                                            ...
                                            if (N2 == N)
                                                {
                                                    orb_lm_mod += (1.0
                                                                   - this->multipole[T][L][N]
                                                                         * this->multipole[T][L][N2] / square)
                                                                  * orb_in[T][L][N2];
                                                }
                                            else
                                                {
                                                    orb_lm_mod += (-this->multipole[T][L][N] * this->multipole[T][L][N2]
                                                                   / square)
                                                                  * orb_in[T][L][N2];
                                                }
                                        }
                            ...
                            if (N == 0)
                                {
                                    ...
                                    std::cout << "Atom type " << T << ", L " << L << ", N " << N
                                              << ", multipole after rotation: " << this->multipole[T][L][N]
                                              << std::endl;
                                }
                            else
                                {
...
template <typename Tdata>
double
    Moment_abfs<Tdata>::dfact (const int& l) const
{
    double result = 1;
    for (int i = l; i > 1; i -= 2)
        {
...
template <typename Tdata>
int
    Moment_abfs<Tdata>::factorial (const int& n) const
{
    if (n == 0)
        {
            return 1;
...
template <typename Tdata>
double
    Moment_abfs<Tdata>::ln_factorial (int n) const
{
    double res = 0.0;
    for (int i = 2; i <= n; ++i)
        res += std::log (i);
...
template <typename Tdata>
double
    Moment_abfs<Tdata>::cal_cl1l2 (int l1, int l2) const
{
    double result = 0.0;
    // int overflow
...
template <typename Tdata>
double
    Moment_abfs<Tdata>::sum_triple_Y_YLM_real (int l1,
            ...
            for (size_t i1 = 0; i1 < list_A1.size (); ++i1)
                {
                                            ...
                                            for (int M2 = -L2; M2 <= L2; ++M2)
                                                {
                                                    ...
                                                    const double prefactor = std::pow (-1, L2)
                                                                             * std::pow (ModuleBase::TWO_PI, 1.5)
                                                                             / prefactor1;
                                                    const double clmlm = this->cal_cl1l2 (L1, L2);
                                                    // For real spherical harmonics, m order is: 0, 0, 1, -1, 0, 1, -1,
                                                    // 2, -2, ...
                                                    const double ylm
                                                        = sum_triple_Y_YLM_real (L1, M1, L2, M2, rly, MGT0, distance);

                                                    // Determine N1 and N2 loop ranges based on rotate_abfs
                                                    // When rotate_abfs=true: only N=0 has non-zero moment, calculate
                                                    // only N1=0 and N2=0 When rotate_abfs=false: all moments are
                                                    // non-zero, calculate all N1, N2
                                                    const int N1_max = GlobalC::exx_info.info_ri.rotate_abfs
                                                                           ? 1
                                                                           : orb_in[T1][L1].size ();
                                                    const int N2_max = GlobalC::exx_info.info_ri.rotate_abfs
                                                                           ? 1
                                                                           : orb_in[T2][L2].size ();
                                                            ...
                                                            for (int N2 = 0; N2 != N2_max; ++N2)
                                                                {
                                                                    ...
                                                                    // every L has only one moment!=0 after rotation
                                                                    // (N=0)
                                                                    const size_t iA = index[T1][L1][N1][index_M1];
                                                                    const size_t iB = index[T2][L2][N2][index_M2];

                                                                    const double value
                                                                        = prefactor * mom1 * mom2 * clmlm * ylm;

                                                                    // Apply smooth truncation using erfc function in
                                                                    // log space, exactly as implemented in FHI-aims to
                                                                    // ensure consistency with k-space 1-cos(qRc)
                                                                    // truncation scheme.
                                                                    //
                                                                    // FHI-aims formula (cut_coulomb_operator.f90 lines
                                                                    // 205, 220):
                                                                    ...
                                                                    //
                                                                    // The log-space erfc truncation and 1-cos(qRc) form
                                                                    // a Fourier transform pair, eliminating
                                                                    // high-frequency oscillations that cause aliasing
                                                                    // in coarse k-grids.
                                                                    //
                                                                    ...
                                                                    //   - runtime_choices.f90 line 2701: width_factor
                                                                    //   = 8.0
                                                                    //
                                                                    // Default width parameter from FHI-aims (can be
                                                                    // adjusted). Smaller width = sharper truncation
                                                                    // (better for coarse k-grids) For high-lying
                                                                    // states, stronger truncation is often needed.
                                                                    // width = 1.05: r = Rc*1.2 gives ~1.5% contribution
                                                                    // width = 1.03: r = Rc*1.1 gives ~0.5% contribution
                                                                    // width = 1.02: r = Rc*1.05 gives ~0.08%
                                                                    // contribution (nearly hard) width = 1.01:
                                                                    // essentially hard cutoff at Rc
                                                                    const double width = 0.9; // Width factor - tune
                                                                                              // this for high states!
                                                                    ...
                                                                    if (distance > 0.0 && width > 1.0)
                                                                        {
                                                                            // Log-space erfc truncation (FHI-aims
                                                                            // implementation)
                                                                            cutoff_factor
                                                                                = 0.5
                                                                                  * std::erfc (std::log (distance / Rc)
                                                                                               / std::log (width));

                                                                            // Debug output for high states (check if
                                                                            // truncation is working)
                                                                            static int debug_count = 0;
                                                                            if (debug_count < 10 && distance > Rc * 0.9
                                                                                && distance < Rc * 1.2)
                                                                                {
                                                    ...
                                                    // if (iat0 == 0 && iat0 == 0 && R[0] == 2 && R[1] == 2 && R[2] == 1
                                                    // && iat0 == 0 && iat1 == 0)
                                                    // {
                                                    //     std::cout << "iA: " << iA << ", iB: " << iB << ", L2: " << L2
                                                    //     << ", M2: " << M2
                                                    //               << ", V= " << value << ", prefactor:" << prefactor
                                                    //               << ",mom1: " << mom1
                                                    ...
                                                    // if (R[0] == 2 && R[1] == 2 && R[2] == 1 && iat0 == 0 && iat1 ==
                                                    // 0)
                                                    // {
                                                    //     out_pure_ri_tensor("Vs_tensor.txt", tmp_tensor, 0.0);
                                                    // }
                                                    // debug
                                                    // std::cout << "T1: " << T1 << ", L1: " << L1 << ", T2: " << T2 <<
                                                    // ", L2: " << L2
                                                    //           << ", delta_R: " << delta_R[0] << ", " << delta_R[1] <<
                                                    //           ", " << delta_R[2]
                                                    //           << ", distance: " << distance << ", outside V= " <<
                                                    //           value
                                                    //           << ", prefactor:" << prefactor << ",mom1: " << mom1 <<
                                                    //           ", mom2: " << mom2
                                                    //           << ", clmlm: " << clmlm << ", ylm_real: " << ylm_real
                                                    //           << std::endl;
                                                }
                                        }
                                }
                    ...
                    // otherwise, warning
                    else
                        {
                            ...
                            ModuleBase::WARNING_QUIT ("merge_VR",
                                                      "JR already exists in Vs_cut[I], which is unexpected.");
                        }
                }
        }
                                                            ...
                                                            for (int M2 = -L2; M2 <= L2; ++M2)
                                                                {
                                                                    ...
                                                                    for (int N1 = 1; N1 != orb_in[T1][L1].size (); ++N1)
                                                                        {
                                                                            const size_t iA
                                                                                = index[T1][L1][N1][index_M1];
                                                                            for (int N2 = 0;
                                                                                 N2 != orb_in[T2][L2].size ();
                                                                                 ++N2)
                                                                                {
                                                                                    const size_t iB
                                                                                        = index[T2][L2][N2][index_M2];
                                                                                    tensor (iA, iB)
                                                                                        = static_cast<Tdata> (0);
                                                                                }
                                                                        }
...
template <typename Tdata>
void
    Moment_abfs<Tdata>::out_pure_ri_tensor (const std::string fn, RI::Tensor<double>& olp, const double threshold)
{
    std::ofstream fs;
...
template <typename Tdata>
void
    Moment_abfs<Tdata>::out_pure_ri_tensor (const std::string fn,
