...
template <typename T>
TLRI<T>
    read_Cs_ao (const std::string& file_path, const double& threshold)
{
    IZ (natom)
    IZ (ncell) IZ (ia1) IZ (ia2) IZ (ic_1) IZ (ic_2) IZ (ic_3) SZ (nw1) SZ (nw2) SZ (nabf) std::ifstream infile;
    ...
    while (infile.peek () != EOF)
        {
            ...
            if (absmax (tensor_cs) >= threshold)
                {
                    Cs[ia1 - 1][{ia2 - 1, box}] = tensor_cs;
                }
        }
    infile.close ();
    return Cs;
...
template <typename T>
void
    write_Cs_ao (const TLRI<T>& Cs, const std::string& file_path)
{
            ...
            for (auto& it2: it1.second)
                {
                    ...
                    outfile << ia1 + 1 << " " << ia2 + 1 << " " << box[0] << " " << box[1] << " " << box[2]
                            << std::endl;
...
template <typename T>
TLRI<T>
    read_Vs_abf (const std::string& file_path, const double& threshold)
{
    IZ (natom) IZ (ncell) IZ (ia1) IZ (ia2) IZ (ic_1) IZ (ic_2) IZ (ic_3) SZ (nabf1) SZ (nabf2) std::ifstream infile;
    infile.open (file_path);
    infile >> natom >> ncell; // no use of ncell
    ...
    while (infile.peek () != EOF)
        {
            ...
            if (absmax (tensor_vs) >= threshold)
                {
                    Vs[ia1 - 1][{ia2 - 1, box}] = tensor_vs;
                }
            // else ++cs_discard;
        }
    infile.close ();
...
template <typename T>
void
    write_Vs_abf (const TLRI<T>& Vs, const std::string& file_path)
{
    std::ofstream outfile;
    outfile.open (file_path);
