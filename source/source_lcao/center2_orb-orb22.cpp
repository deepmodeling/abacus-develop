...
void
    Center2_Orb::Orb22::init_radial_table ()
{
    ...
    for (int LB = std::abs (LB1 - LB2); LB <= LB1 + LB2; ++LB)
        {
            if ((LB - std::abs (LB1 - LB2)) % 2 == 1)
                { // if LA+LB-LAB == odd, then Gaunt_Coefficients = 0
                    continue;
                }
...
void
    Center2_Orb::Orb22::init_radial_table (const std::set<size_t>& radials)
{
    const Numerical_Orbital_Lm& nB_short = (nB1.getNr () <= nB2.getNr ()) ? nB1 : nB2;
...
double
    Center2_Orb::Orb22::cal_overlap (const ModuleBase::Vector3<double>& RA,
...
ModuleBase::Vector3<double>
    Center2_Orb::Orb22::cal_grad_overlap (const ModuleBase::Vector3<double>& RA,
                                          const ModuleBase::Vector3<double>& RB,
                                          const int& mA1,
                                          const int& mA2,
                                          const int& mB1,
