...
void
    sparse_format::cal_dS (const UnitCell& ucell,
...
void
    sparse_format::cal_dH (const UnitCell& ucell,
                           const Parallel_Orbitals& pv,
                           LCAO_HS_Arrays& HS_Arrays,
                           const Grid_Driver& grid,
                           const TwoCenterBundle& two_center_bundle,
                           ...
                           const ModuleBase::matrix& v_eff)
{
    ...
    if (PARAM.inp.nspin == 2)
        {
            const double* vr_eff1 = v_eff.nc * v_eff.nr > 0 ? &(v_eff (current_spin, 0)) : nullptr;
            if (!PARAM.globalv.gamma_only_local)
                {
                    ModuleGint::cal_dvlocal_R_sparseMatrix (PARAM.inp.nspin,
                                                            PARAM.globalv.npol,
                                                            current_spin,
...
void
    sparse_format::set_R_range (std::set<Abfs::Vector3_Order<int>>& all_R_coor, const Grid_Driver& grid)
{
...
void
    sparse_format::cal_dSTN_R (const UnitCell& ucell,
                               const Parallel_Orbitals& pv,
                               LCAO_HS_Arrays& HS_Arrays,
                                                    ...
                                                    if (PARAM.inp.nspin != 4)
                                                        {
                                                            ...
                                                            if (std::abs (temp_value_double) > sparse_thr)
                                                                {
                                                                    HS_Arrays
                                                                        .dHRx_sparse[current_spin][dR][iw1_all][iw2_all]
                                                                        = temp_value_double;
                                                                }
                                                            ...
                                                            if (std::abs (temp_value_double) > sparse_thr)
                                                                {
                                                                    HS_Arrays
                                                                        .dHRy_sparse[current_spin][dR][iw1_all][iw2_all]
                                                                        = temp_value_double;
                                                                }
                                                            ...
                                                            if (std::abs (temp_value_double) > sparse_thr)
                                                                {
                                                                    HS_Arrays
                                                                        .dHRz_sparse[current_spin][dR][iw1_all][iw2_all]
                                                                        = temp_value_double;
                                                                }
                                                        }
                                                    else
                                                        {
                                                            ModuleBase::WARNING_QUIT (
                                                                "cal_dSTN_R",
                                                                "nspin=4 with SOC is not supported yet.");
                                                        }
                                                    ++index;
                                                }
                                        }
...
void
    sparse_format::destroy_dH_R_sparse (LCAO_HS_Arrays& HS_Arrays)
{
