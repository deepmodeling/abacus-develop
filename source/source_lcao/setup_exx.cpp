...
template <typename TK>
void Exx_NAO<TK>::before_runner(UnitCell& ucell,             // unitcell
    ...
    if (inp.calculation == "scf" || inp.calculation == "relax" || inp.calculation == "cell-relax" ||
        ...
        if (GlobalC::exx_info.info_global.cal_exx) {
            if (inp.init_wfc != "file") { // if init_wfc==file, directly enter the EXX loop
...
template <typename TK>
void Exx_NAO<TK>::before_scf(const UnitCell& ucell, // unitcell
                             const K_Vectors& kv,
                             const LCAO_Orbitals& orb, // orbital info
