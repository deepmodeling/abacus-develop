...
class PauliToMomentTest : public ::testing::Test
{
  protected:
    ...
    void SetUp() override
    {
        for (int i = 0; i < 4; i++) occ[i] = {0.0, 0.0};
    }
};
...
TEST_F(PauliToMomentTest, ComplexPhase_SpinY)
{
    // |psi> = 1/sqrt(2) * (|up> + i|down>)
    // For proper Pauli decomposition:
    // occ[0] = |a|^2, occ[1] = a*conj(b), occ[2] = b*conj(a), occ[3] = |b|^2
    double a = 1.0 / std::sqrt(2.0);
    double b = 1.0 / std::sqrt(2.0);
    occ[0] = {a * a, 0.0};
    occ[1] = {0.0, -a * b}; // a * conj(i*b) = a * (-i*b) = -i*a*b
    occ[2] = {0.0, a * b};  // i*b * conj(a) = i*b*a
    occ[3] = {b * b, 0.0};

    auto M = pauli_to_moment(occ, 1.0);
    // Mx = (-iab + iab).real = 0
    // My = (-iab - iab).imag = (-2iab).imag = -2ab = -1
    // Mz = (a^2 - b^2) = 0
    EXPECT_NEAR(M.x, 0.0, 1e-15);
    EXPECT_NEAR(M.y, -1.0, 1e-15);
    EXPECT_NEAR(M.z, 0.0, 1e-15);
}

TEST_F(PauliToMomentTest, MixedState_PartialPolarization)
{
    occ[0] = {0.7, 0.0};
    occ[3] = {0.3, 0.0};
    auto M = pauli_to_moment(occ, 1.0);
    EXPECT_NEAR(M.z, 0.4, 1e-15);
}
...
TEST_F(PauliToMomentTest, WeightScaling)
{
    occ[0] = {1.0, 0.0};
    EXPECT_NEAR(pauli_to_moment(occ, 0.5).z, 0.5, 1e-15);
    EXPECT_NEAR(pauli_to_moment(occ, 2.0).z, 2.0, 1e-15);
}

TEST_F(PauliToMomentTest, HermitianProperty)
{
    // For valid density matrix: occ[2] = conj(occ[1])
    // This ensures Mx, My, Mz are real
    occ[0] = {0.5, 0.0};
    occ[1] = {0.2, 0.3};
    occ[2] = {0.2, -0.3}; // conj
    occ[3] = {0.5, 0.0};
    ...
    auto M = pauli_to_moment(occ, 1.0);
    // Verify all components are real (imaginary parts should be exactly 0)
    // Mx = (0.2+0.3i + 0.2-0.3i).real = 0.4
    // My = (0.2+0.3i - (0.2-0.3i)).imag = (0+0.6i).imag = 0.6
    // Mz = 0
    EXPECT_NEAR(M.x, 0.4, 1e-15);
    EXPECT_NEAR(M.y, 0.6, 1e-15);
    EXPECT_NEAR(M.z, 0.0, 1e-15);
}
...
TEST_F(DeltaHCCTest, Npol2_PureZ)
{
    ...
    becp[0] = {1.0, 0.0};       // becp_up
    becp[1] = {0.0, 0.0};       // becp_dn
    ...
    // c0 = (2,0), c3 = (-2,0)
    // ps_up = c0 * becp_up + c2 * becp_dn = (2,0)*(1,0) + 0 = (2,0)
    // ps_dn = c1 * becp_up + c3 * becp_dn = 0 + (-2,0)*0 = 0
...
TEST_F(DeltaHCCTest, Npol2_PureX)
{
    ...
    // c0 = (0,0), c1 = (3,0), c2 = (3,0), c3 = (0,0)
    // ps_up = 0 * becp_up + 3 * becp_dn = 3
    // ps_dn = 3 * becp_up + 0 * becp_dn = 3
...
TEST_F(DeltaHCCTest, Npol1_SpinDownNegative)
{
    const int nkb = 1, nbands = 1;
    std::vector<std::complex<double>> becp = {{1.0, 0.0}};
    std::vector<std::complex<double>> ps(nkb, {0.0, 0.0});
...
TEST_F(DeltaHCCTest, Npol2_MultiBand_MultiProj)
{
    const int nkb = 3, nbands = 2, npol = 2;
    Vec3 lambda = {1.0, 2.0, 3.0};

    std::vector<std::complex<double>> becp(nbands * npol * nkb);
    for (size_t i = 0; i < becp.size(); i++)
        becp[i] = {static_cast<double>(i + 1), static_cast<double>(i % 3) * 0.1};

    std::vector<std::complex<double>> ps(becp.size(), {0.0, 0.0});

    compute_delta_hcc_npol2(ps, becp, lambda, nbands, nkb, npol);

    // Verify structure: each band's up/down are coupled via lambda
    for (int ib = 0; ib < nbands; ib++)
    {
        for (int ip = 0; ip < nkb; ip++)
        {
            int up_idx = ib * npol * nkb + ip;
            int dn_idx = up_idx + nkb;
            // ps should be non-zero (coupling applied)
            EXPECT_NE(ps[up_idx].real(), 0.0) << "band " << ib << " proj " << ip;
        }
    }
}
...
class AccumulateMiTest : public ::testing::Test
{
  protected:
    static double accumulate_mi_npol1(
        ...
        const double* weights, const int* isk,
        int nproj)
    {
        double Mi_z = 0.0;
        ...
        const double* weights, int nproj)
    {
        ...
        for (int ik = 0; ik < nk; ik++)
        {
            int offset = ik * nbands * nkb * 2; // npol=2: 2*nkb per band
...
TEST_F(AccumulateMiTest, Npol2_MultiK_Sum)
{
    const int nbands = 1, nkb = 1, nk = 2, nproj = 1;
    double weights[] = {1.0, 0.5};
    ...
    // k=0: pure z-up
    // k=1: pure x
    std::vector<std::complex<double>> becp(nbands * nkb * 2 * nk, {0.0, 0.0});
    ...
    // k=0: Mz = 0.64, Mx = My = 0
    // k=1: Mx = 1.0, My = Mz = 0, weight = 0.5
    EXPECT_NEAR(Mi.x, 0.5, 1e-15);
    EXPECT_NEAR(Mi.y, 0.0, 1e-15);
    EXPECT_NEAR(Mi.z, 0.64, 1e-15);
}
...
TEST_F(AdaptiveThresholdTest, DefaultCase)
{
    double thr = calc_adaptive(0.5, 1e-6, 1e-3);
    EXPECT_NEAR(thr, 5e-4, 1e-15);
}

TEST_F(AdaptiveThresholdTest, TightThreshold)
{
    double thr = calc_adaptive(0.1, 1e-4, 1e-3);
    EXPECT_NEAR(thr, 1e-4, 1e-15);
}
...
TEST_F(AdaptiveThresholdTest, VeryLargeInitialRms)
{
    double thr = calc_adaptive(100.0, 1e-6, 1e-3);
    EXPECT_NEAR(thr, 0.1, 1e-15);
}
...
class GradientDecayTest : public ::testing::Test
{
  protected:
    static bool check_gradient_decay(
        const std::vector<Vec3>& new_spin,
        const std::vector<Vec3>& old_spin,
        const std::vector<Vec3>& new_lambda,
        const std::vector<Vec3>& old_lambda,
        const std::vector<Vec3i>& constrain,
        double decay_grad, int nat)
    {
        for (int iat = 0; iat < nat; iat++)
        {
            for (int ic = 0; ic < 3; ic++)
            {
                ...
                double gradient = std::abs(dM / dL);
                if (gradient > decay_grad) return false;
            }
        }
        return true;
    }
...
TEST_F(GradientDecayTest, UnconstrainedIgnored)
{
    std::vector<Vec3> new_spin = {{100.0, 0.0, 0.1}};
    std::vector<Vec3> old_spin = {{0.0, 0.0, 0.2}};
    std::vector<Vec3> new_lambda = {{100.0, 0.0, 1.0}};
    std::vector<Vec3> old_lambda = {{0.0, 0.0, 2.0}};
    std::vector<Vec3i> constrain = {{0, 0, 1}}; // only z constrained

    // x has large gradient but is unconstrained, so ignored
    bool decayed = check_gradient_decay(new_spin, old_spin, new_lambda, old_lambda, constrain, 0.9, 1);
    EXPECT_TRUE(decayed);
}
...
TEST_F(DirectionOnlyTest, PerpendicularLambda_Unchanged)
{
    Vec3 lambda = {1.0, 0.0, 0.0};
    Vec3 target = {0.0, 1.0, 0.0};
    auto perp = project_lambda_perpendicular(lambda, target);
    EXPECT_NEAR(perp.x, 1.0, 1e-14);
    EXPECT_NEAR(perp.y, 0.0, 1e-14);
    EXPECT_NEAR(perp.z, 0.0, 1e-14);
}
...
TEST_F(DirectionOnlyTest, ZeroTarget_NoProjection)
{
    Vec3 lambda = {1.0, 2.0, 3.0};
    Vec3 target = {0.0, 0.0, 0.0};
    auto perp = project_lambda_perpendicular(lambda, target);
    EXPECT_NEAR(perp.x, 1.0, 1e-14);
    EXPECT_NEAR(perp.y, 2.0, 1e-14);
    EXPECT_NEAR(perp.z, 3.0, 1e-14);
}
...
class ConstraintEnergyTest : public ::testing::Test
{
  protected:
    static double calc_escon(
        const std::vector<Vec3>& lambda,
        const std::vector<Vec3>& Mi,
        const std::vector<Vec3i>& constrain,
        int nat)
    {
        double escon = 0.0;
...
TEST_F(ConstraintEnergyTest, PerfectConstraint_ZeroLambda)
{
    std::vector<Vec3> lambda = {{0.0, 0.0, 0.0}};
    std::vector<Vec3> Mi = {{1.0, 2.0, 3.0}};
    std::vector<Vec3i> constrain = {{1, 1, 1}};

    double escon = calc_escon(lambda, Mi, constrain, 1);
    EXPECT_DOUBLE_EQ(escon, 0.0);
}
...
TEST_F(RmsErrorTest, Converged)
{
    std::vector<Vec3> Mi = {{1.0, 2.0, 3.0}};
    std::vector<Vec3> target = {{1.0, 2.0, 3.0}};
    std::vector<Vec3i> constrain = {{1, 1, 1}};
...
TEST_F(RmsErrorTest, NoConstrained)
{
    std::vector<Vec3> Mi = {{1.0, 2.0, 3.0}};
    std::vector<Vec3> target = {{0.0, 0.0, 0.0}};
    std::vector<Vec3i> constrain = {{0, 0, 0}};
