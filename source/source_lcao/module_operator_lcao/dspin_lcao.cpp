...
inline void
    cal_coeff_lambda (const std::vector<double>& current_lambda, std::vector<double>& coefficients)
{
    coefficients[0] = current_lambda[0];
    coefficients[1] = -current_lambda[0];
}
inline void
    cal_coeff_lambda (const std::vector<double>& current_lambda, std::vector<std::complex<double>>& coefficients)
...
template <typename TK, typename TR>
void
    hamilt::DeltaSpin<hamilt::OperatorLCAO<TK, TR>>::contributeHR ()
{
                    ...
                    for (int ir = 0; ir < tmp.get_R_size (); ++ir)
                        {
                            ...
                            TR* dhr_data = this->hR->find_matrix (iat1, iat2, r_index[0], r_index[1], r_index[2])
                                               ->get_pointer ();
                            // TR== double: axpy for dhr_data += current_lambda * pre_hr_data
                            // TR!= double: call cal_lambda_hr_IJR
                            if (this->nspin == 2)
                                {
                                    // BlasConnector::axpy(row_size*col_size, coefficients[this->current_spin],
                                    // pre_hr_data, dhr_data);
...
template <typename TK, typename TR>
void
    hamilt::DeltaSpin<hamilt::OperatorLCAO<TK, TR>>::pre_coeff_array (const std::vector<TR>& coeff,
                                                                      const int row_size,
                                                                      const int col_size)
{
...
template <typename TK, typename TR>
void
    hamilt::DeltaSpin<hamilt::OperatorLCAO<TK, TR>>::cal_constraint_atom_list (
...
template <typename TK, typename TR>
void
    hamilt::DeltaSpin<hamilt::OperatorLCAO<TK, TR>>::cal_pre_HR ()
{
                    ...
                    for (int ad2 = 0; ad2 < adjs.adj_num + 1; ++ad2)
                        {
                            ...
                            int r_vector[3]
                                = {R_index2.x - R_index1.x, R_index2.y - R_index1.y, R_index2.z - R_index1.z};
...
template <typename TK, typename TR>
void
    hamilt::DeltaSpin<hamilt::OperatorLCAO<TK, TR>>::cal_HR_IJR (
        const int& iat1,
        const int& iat2,
...
template <typename TK, typename TR>
std::vector<double>
    hamilt::DeltaSpin<hamilt::OperatorLCAO<TK, TR>>::cal_moment (const HContainer<double>* dmR,
                    ...
                    for (int ir = 0; ir < tmp.get_R_size (); ++ir)
                        {
                            const ModuleBase::Vector3<int> r_index = tmp.get_R_index (ir);
                            double* dmr_data
                                = dmR->find_matrix (iat1, iat2, r_index[0], r_index[1], r_index[2])->get_pointer ();
...
template <typename TK, typename TR>
void
    hamilt::DeltaSpin<hamilt::OperatorLCAO<TK, TR>>::cal_moment_IJR (const double* dmR,
                                                                     const TR* hr,
                                                                     const int row_size,
                                                                     const int col_size,
                                                                     double* moment)
...
template <>
void
    hamilt::DeltaSpin<hamilt::OperatorLCAO<std::complex<double>, std::complex<double>>>::cal_moment_IJR (
        const double* dmR,
