...
template <typename TK, typename TR>
void
    hamilt::DFTU<hamilt::OperatorLCAO<TK, TR>>::initialize_HR (const Grid_Driver* GridD)
{
...
template <typename TK, typename TR>
void
    hamilt::DFTU<hamilt::OperatorLCAO<TK, TR>>::cal_nlm_all (const Parallel_Orbitals* paraV)
{
...
template <typename TK, typename TR>
void
    hamilt::DFTU<hamilt::OperatorLCAO<TK, TR>>::contributeHR ()
{
                    ...
                    for (int ad2 = 0; ad2 < adjs.adj_num + 1; ++ad2)
                        {
                            ...
                            hamilt::BaseMatrix<TR>* tmp
                                = this->hR->find_matrix (iat1, iat2, R_vector[0], R_vector[1], R_vector[2]);
                            // if not found , skip this pair of atoms
                            if (tmp != nullptr)
                                {
...
template <typename TK, typename TR>
void
    hamilt::DFTU<hamilt::OperatorLCAO<TK, TR>>::cal_HR_IJR (
        const int& iat1,
        const int& iat2,
...
template <typename TK, typename TR>
void
    hamilt::DFTU<hamilt::OperatorLCAO<TK, TR>>::cal_occ (const int& iat1,
                                                         const int& iat2,
                                                         const Parallel_Orbitals* paraV,
                                                         const std::unordered_map<int, std::vector<double>>& nlm1_all,
                                                         const std::unordered_map<int, std::vector<double>>& nlm2_all,
...
template <typename TK, typename TR>
void
    hamilt::DFTU<hamilt::OperatorLCAO<TK, TR>>::transfer_vu (std::vector<double>& vu_tmp, std::vector<TR>& vu)
{
#ifdef __DEBUG
...
template <>
void
    hamilt::DFTU<hamilt::OperatorLCAO<std::complex<double>, std::complex<double>>>::transfer_vu (
...
template <typename TK, typename TR>
void
    hamilt::DFTU<hamilt::OperatorLCAO<TK, TR>>::cal_v_of_u (const std::vector<double>& occ,
                                                            const int m_size,
                            ...
                            for (int m2 = 0; m2 < m_size; m2++)
                                {
                                    vu[start + m1 * m_size + m2]
                                        = u_value * (0.5 * (m1 == m2) - occ[start + m2 * m_size + m1]);
