...
template <>
void Nonlocal<OperatorLCAO<std::complex<double>, std::complex<double>>>::cal_force_IJR(
    const int& iat1,
    const int& iat2,
    const int& T0,
...
template <>
void Nonlocal<OperatorLCAO<std::complex<double>, std::complex<double>>>::cal_stress_IJR(
    const int& iat1,
    const int& iat2,
    const int& T0,
    const Parallel_Orbitals* paraV,
    ...
    for (int iw1l = 0; iw1l < row_indexes.size(); iw1l += npol) {
        ...
        for (int iw2l = 0; iw2l < col_indexes.size(); iw2l += npol) {
            ...
            // calculate the force, transfer nlm_tmp to pauli matrix
            for (int i = 0; i < 6; i++) {
                stress[i] += (dm_pointer[step_trace[0]] * nlm_tmp[i] + dm_pointer[step_trace[1]] * nlm_tmp[i + 6] +
        ...
        for (int iw2l = 0; iw2l < col_indexes.size(); iw2l++) {
            ...
            for (int no = 0; no < this->ucell->atoms[T0].ncpp.non_zero_count_soc[0]; no++) {
                ...
                nlm_tmp[2] +=
                    (nlm1[p1 + length] * dis1.z * nlm2[p2] + nlm1[p1] * nlm2[p2 + length] * dis2.z) * (*tmp_d);
                nlm_tmp[3] +=
                    (nlm1[p1 + length * 2] * dis1.y * nlm2[p2] + nlm1[p1] * nlm2[p2 + length * 2] * dis2.y) * (*tmp_d);
                nlm_tmp[4] +=
                    (nlm1[p1 + length * 2] * dis1.z * nlm2[p2] + nlm1[p1] * nlm2[p2 + length * 2] * dis2.z) * (*tmp_d);
                nlm_tmp[5] +=
                    (nlm1[p1 + length * 3] * dis1.z * nlm2[p2] + nlm1[p1] * nlm2[p2 + length * 3] * dis2.z) * (*tmp_d);
            }
            // calculate the force, transfer nlm_tmp to pauli matrix
            for (int i = 0; i < 6; i++) {
