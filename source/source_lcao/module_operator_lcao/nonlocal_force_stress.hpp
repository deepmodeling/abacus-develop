...
void
    Nonlocal<OperatorLCAO<TK, TR>>::cal_force_stress (const bool cal_force,
...
template <>
void
    Nonlocal<OperatorLCAO<std::complex<double>, std::complex<double>>>::cal_force_IJR (
        const int& iat1,
        const int& iat2,
        const int& T0,
            ...
            for (int iw2l = 0; iw2l < col_indexes.size (); iw2l += npol)
                {
                    ...
                    for (int i = 0; i < 3; i++)
                        {
                            double tmp
                                = (dm_pointer[step_trace[0]] * nlm_tmp[i] + dm_pointer[step_trace[1]] * nlm_tmp[i + 3]
                                   + dm_pointer[step_trace[2]] * nlm_tmp[i + 6]
                                   + dm_pointer[step_trace[3]] * nlm_tmp[i + 9])
                                      .real ();
                            force1[i] += tmp;
                            force2[i] -= tmp;
                        }
...
template <>
void
    Nonlocal<OperatorLCAO<std::complex<double>, std::complex<double>>>::cal_stress_IJR (
        const int& iat1,
        const int& iat2,
        const int& T0,
        const Parallel_Orbitals* paraV,
            ...
            for (int iw2l = 0; iw2l < col_indexes.size (); iw2l += npol)
                {
                            ...
                            for (int no = 0; no < this->ucell->atoms[T0].ncpp.non_zero_count_soc[is]; no++)
                                {
                                    ...
                                    const int p2 = this->ucell->atoms[T0].ncpp.index2_soc[is][no];
                                    this->ucell->atoms[T0].ncpp.get_d (is, p1, p2, tmp_d);
                                    nlm_tmp[is * 6] += (nlm1[p1 + length] * dis1.x * nlm2[p2]
                                                        + nlm1[p1] * nlm2[p2 + length] * dis2.x)
                                                       * (*tmp_d);
                                    nlm_tmp[is * 6 + 1] += (nlm1[p1 + length] * dis1.y * nlm2[p2]
                                                            + nlm1[p1] * nlm2[p2 + length] * dis2.y)
                                                           * (*tmp_d);
                                    nlm_tmp[is * 6 + 2] += (nlm1[p1 + length] * dis1.z * nlm2[p2]
                                                            + nlm1[p1] * nlm2[p2 + length] * dis2.z)
                                                           * (*tmp_d);
                                    nlm_tmp[is * 6 + 3] += (nlm1[p1 + length * 2] * dis1.y * nlm2[p2]
                                                            + nlm1[p1] * nlm2[p2 + length * 2] * dis2.y)
                                                           * (*tmp_d);
                                    nlm_tmp[is * 6 + 4] += (nlm1[p1 + length * 2] * dis1.z * nlm2[p2]
                                                            + nlm1[p1] * nlm2[p2 + length * 2] * dis2.z)
                                                           * (*tmp_d);
                                    nlm_tmp[is * 6 + 5] += (nlm1[p1 + length * 3] * dis1.z * nlm2[p2]
                                                            + nlm1[p1] * nlm2[p2 + length * 3] * dis2.z)
                                                           * (*tmp_d);
                                }
                        }
                    // calculate the force, transfer nlm_tmp to pauli matrix
                    for (int i = 0; i < 6; i++)
                        {
                            stress[i]
                                += (dm_pointer[step_trace[0]] * nlm_tmp[i] + dm_pointer[step_trace[1]] * nlm_tmp[i + 6]
                                    + dm_pointer[step_trace[2]] * nlm_tmp[i + 12]
                                    + dm_pointer[step_trace[3]] * nlm_tmp[i + 18])
                                       .real ();
                        }
                    dm_pointer += npol;
                }
...
template <typename TK, typename TR>
void
    Nonlocal<OperatorLCAO<TK, TR>>::cal_force_IJR (const int& iat1,
                                                   const int& iat2,
                                                   const int& T0,
                                                   const Parallel_Orbitals* paraV,
                                                   const std::unordered_map<int, std::vector<double>>& nlm1_all,
...
template <typename TK, typename TR>
void
    Nonlocal<OperatorLCAO<TK, TR>>::cal_stress_IJR (const int& iat1,
                                                    const int& iat2,
                                                    const int& T0,
                                                    const Parallel_Orbitals* paraV,
                                                    const std::unordered_map<int, std::vector<double>>& nlm1_all,
                    ...
                    for (int no = 0; no < this->ucell->atoms[T0].ncpp.non_zero_count_soc[0]; no++)
                        {
                            ...
                            this->ucell->atoms[T0].ncpp.get_d (0, p1, p2, tmp_d);
                            nlm_tmp[0]
                                += (nlm1[p1 + length] * dis1.x * nlm2[p2] + nlm1[p1] * nlm2[p2 + length] * dis2.x)
                                   * (*tmp_d);
                            nlm_tmp[1]
                                += (nlm1[p1 + length] * dis1.y * nlm2[p2] + nlm1[p1] * nlm2[p2 + length] * dis2.y)
                                   * (*tmp_d);
                            nlm_tmp[2]
                                += (nlm1[p1 + length] * dis1.z * nlm2[p2] + nlm1[p1] * nlm2[p2 + length] * dis2.z)
                                   * (*tmp_d);
                            nlm_tmp[3] += (nlm1[p1 + length * 2] * dis1.y * nlm2[p2]
                                           + nlm1[p1] * nlm2[p2 + length * 2] * dis2.y)
                                          * (*tmp_d);
                            nlm_tmp[4] += (nlm1[p1 + length * 2] * dis1.z * nlm2[p2]
                                           + nlm1[p1] * nlm2[p2 + length * 2] * dis2.z)
                                          * (*tmp_d);
                            nlm_tmp[5] += (nlm1[p1 + length * 3] * dis1.z * nlm2[p2]
                                           + nlm1[p1] * nlm2[p2 + length * 3] * dis2.z)
                                          * (*tmp_d);
                        }
                    // calculate the force, transfer nlm_tmp to pauli matrix
