    ...
    for (auto& Htmp1: Hexxs[0]) {
        ...
        for (auto& Htmp2: Htmp1.second) {
            ...
            if (pv->get_row_size(iat0) > 0 && pv->get_col_size(iat1) > 0) {
                ...
                if (HlocR == nullptr) { // add R to HContainer
                    need_allocate = true;
...
OperatorEXX<OperatorLCAO<TK, TR>>::OperatorEXX(
    ...
    if (PARAM.inp.calculation == "nscf" &&
        GlobalC::exx_info.info_global
            .cal_exx) { // if nscf, read HexxR first and reallocate hR according to the read-in HexxR
    ...
    } else { // if scf and Add_Hexx_Type::R, init cell_nearest and reallocate hR according to BvK cells
        if (this->add_hexx_type == Add_Hexx_Type::R) {
        ...
        if (this->restart) { ///  Now only Hexx depends on DM, so we can directly read Hexx to reduce the computational
                             ///  cost.
