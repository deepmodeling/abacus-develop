...
void
    reallocate_hcontainer (const std::vector<std::map<int, std::map<TAC, RI::Tensor<Tdata>>>>& Hexxs,
                           HContainer<TR>* hR,
...
void
    reallocate_hcontainer (const int nat,
                           HContainer<TR>* hR,
                    ...
                    if (pv->get_row_size (iat0) > 0 && pv->get_col_size (iat1) > 0)
                        {
                            for (auto& cell: Rs)
                                {
                                    const Abfs::Vector3_Order<int>& R = RI_Util::array3_to_Vector3 (
                                        (cell_nearest ? cell_nearest->get_cell_nearest_discrete (iat0, iat1, cell)
                                                      : cell));
                                    BaseMatrix<TR>* HlocR = hR->find_matrix (iat0, iat1, R.x, R.y, R.z);
...
OperatorEXX<OperatorLCAO<TK, TR>>::OperatorEXX (
    ...
    : OperatorLCAO<TK, TR> (hsk_in, kv_in.kvec_d, hR_in), ucell (ucell_in), kv (kv_in), Hexxd (Hexxd_in),
      Hexxc (Hexxc_in), add_hexx_type (add_hexx_type_in), istep (istep), two_level_step (two_level_step_in),
      restart (restart_in)
{
                    ...
                    for (int irank = 0; irank < PARAM.globalv.nproc; ++irank)
                        {
                            for (int is = 0; is < PARAM.inp.nspin; ++is)
                                {
                                    file_name_list.push_back (PARAM.globalv.global_readin_dir + "HexxR"
                                                              + std::to_string (irank) + "_" + std::to_string (is)
                                                              + ".csr");
                                }
                        }
            ...
            if (check_exist (file_name_list_csr ()))
                {
                    ...
                    if (GlobalC::exx_info.info_ri.real_number)
                        {
                            ModuleIO::read_Hexxs_csr (file_name_exx_csr,
                                                      ucell,
                                                      PARAM.inp.nspin,
            ...
            else if (check_exist (file_name_list_cereal ()))
                {
                    ...
                    if (!ifs)
                        {
                            ModuleBase::WARNING_QUIT ("OperatorEXX",
                                                      "Can't open EXX file < " + file_name_exx_cereal + " >.");
                        }
                    if (GlobalC::exx_info.info_ri.real_number)
                        {
                ...
                {
                    ModuleBase::WARNING_QUIT ("OperatorEXX",
                                              "Can't open EXX file in " + PARAM.globalv.global_readin_dir);
                }
        ...
        { // if scf and Add_Hexx_Type::R, init cell_nearest and reallocate hR according to BvK cells
            ...
            if (this->restart)
                { ///  Now only Hexx depends on DM, so we can directly read Hexx to reduce the computational cost.
                    /// If other operators depends on DM, we can also read DM and then calculate the operators to save
                    /// the memory to store operator terms.
                            ...
                            if (all_exist)
                                {
                                    ...
                                    if (GlobalC::exx_info.info_ri.real_number)
                                        {
                                            ModuleIO::read_Hexxs_csr (restart_HR_path,
                                                                      ucell,
                                                                      PARAM.inp.nspin,
                                                                      PARAM.globalv.nlocal,
                                                                      *Hexxd);
...
template <typename TK, typename TR>
void
    OperatorEXX<OperatorLCAO<TK, TR>>::contributeHR ()
{
    ModuleBase::TITLE ("OperatorEXX", "contributeHR");
    // Peize Lin add 2016-12-03
    if (this->istep == 0 && PARAM.inp.calculation != "nscf" && this->two_level_step != nullptr
        && *this->two_level_step == 0 && PARAM.inp.init_wfc != "file" && !this->restart)
        {
            return;
...
template <typename TK, typename TR>
void
    OperatorEXX<OperatorLCAO<TK, TR>>::contributeHk (int ik)
{
    ModuleBase::TITLE ("OperatorEXX", "constributeHk");
    // Peize Lin add 2016-12-03
    if (PARAM.inp.calculation != "nscf" && this->two_level_step != nullptr && *this->two_level_step == 0
        && !this->restart)
        {
            return;
        } // in the non-exx loop, do nothing
    ...
    if (this->add_hexx_type == Add_Hexx_Type::R)
        {
            throw std::invalid_argument ("Set Add_Hexx_Type::k sto call OperatorEXX::contributeHk().");
        }
