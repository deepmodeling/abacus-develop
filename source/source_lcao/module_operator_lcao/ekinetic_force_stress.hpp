...
void
    EKinetic<OperatorLCAO<TK, TR>>::cal_force_stress (const bool cal_force,
                                                      ...
                                                      ModuleBase::matrix& stress)
{
    ...
    auto integral_calc = [this] (int T1,
                                 int L1,
                                 int N1,
                                 int M1,
                                 int T2,
                                 ...
                                 double* olm)
        { this->intor_->calculate (T1, L1, N1, M1, T2, L2, N2, M2, dtau * this->ucell->lat0, &olm[0], &olm[1]); };

    // Use unified template with ForceSign=+1, StressSign=-1 for kinetic operator
    OperatorForceStress::cal_force_stress_2center<TK, TR, decltype (integral_calc), +1, -1> (cal_force,
                                                                                             cal_stress,
                                                                                             dmR,
                                                                                             this->ucell,
                                                                                             this->gridD,
...
template <typename TK, typename TR>
void
    EKinetic<OperatorLCAO<TK, TR>>::cal_force_IJR (const int& iat1,
                                                   const int& iat2,
...
template <typename TK, typename TR>
void
    EKinetic<OperatorLCAO<TK, TR>>::cal_stress_IJR (const int& iat1,
                                                    const int& iat2,
                                                    const Parallel_Orbitals* paraV,
                                                    const std::unordered_map<int, std::vector<double>>& nlm1_all,
                                                    const std::unordered_map<int, std::vector<double>>& nlm2_all,
