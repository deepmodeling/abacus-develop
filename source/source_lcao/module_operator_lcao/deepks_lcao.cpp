...
template <typename TK, typename TR>
void
    hamilt::DeePKS<hamilt::OperatorLCAO<TK, TR>>::initialize_HR (const Grid_Driver* GridD)
{
...
template <typename TK, typename TR>
void
    hamilt::DeePKS<hamilt::OperatorLCAO<TK, TR>>::contributeHR ()
{
#ifdef __MLALGO
    ...
    if (this->ld->get_hr_cal ())
        {
            ...
            DeePKS_domain::cal_descriptor (this->ucell->nat, this->ld->deepks_param, this->ld->pdm, descriptor);
...
template <typename TK, typename TR>
void
    hamilt::DeePKS<hamilt::OperatorLCAO<TK, TR>>::calculate_HR ()
{
            ...
            for (int ad1 = 0; ad1 < adjs.adj_num + 1; ++ad1)
                {
                    ...
                    for (int irow = 0; irow < row_size; irow++)
                        {
                            const hamilt::BaseMatrix<double>* overlap_1
                                = this->ld->phialpha[0]->find_matrix (iat0, iat1, dR1);
                            const double* row_ptr
                                = overlap_1->get_pointer () + row_indexes[irow] * overlap_1->get_col_size ();
                    ...
                    for (int ad2 = 0; ad2 < adjs.adj_num + 1; ++ad2)
                        {
                            ...
                            for (int icol = 0; icol < col_size; icol++)
                                {
                                    const hamilt::BaseMatrix<double>* overlap_2
                                        = this->ld->phialpha[0]->find_matrix (iat0, iat2, dR2);
                                    const double* col_ptr
                                        = overlap_2->get_pointer () + col_indexes[icol] * overlap_2->get_col_size ();
...
template <typename TK, typename TR>
void
    hamilt::DeePKS<hamilt::OperatorLCAO<TK, TR>>::cal_HR_IJR (const double* hr_in,
                                                              const int& row_size,
                                                              const int& col_size,
                                                              TR* data_pointer)
{
...
template <typename TK, typename TR>
void
    hamilt::DeePKS<hamilt::OperatorLCAO<TK, TR>>::contributeHk (int ik)
{
