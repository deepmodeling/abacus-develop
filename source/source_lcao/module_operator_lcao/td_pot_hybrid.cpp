...
hamilt::TD_pot_hybrid<hamilt::OperatorLCAO<TK, TR>>::TD_pot_hybrid (HS_Matrix_K<TK>* hsk_in,
    ...
    : hamilt::OperatorLCAO<TK, TR> (hsk_in, kv_in->kvec_d, hR_in), SR (SR_in), orb_ (orb), orb_cutoff_ (orb_cutoff),
      intor_ (intor)
{
...
template <typename TK, typename TR>
void
    hamilt::TD_pot_hybrid<hamilt::OperatorLCAO<TK, TR>>::initialize_HR (const Grid_Driver* GridD)
{
...
template <typename TK, typename TR>
void
    hamilt::TD_pot_hybrid<hamilt::OperatorLCAO<TK, TR>>::calculate_HR ()
{
...
template <typename TK, typename TR>
void
    hamilt::TD_pot_hybrid<hamilt::OperatorLCAO<TK, TR>>::cal_HR_IJR (const int& iat1,
                                                                     const int& iat2,
            ...
            for (int iw2l = 0; iw2l < col_indexes.size (); iw2l += npol)
                {
                    ...
                    ModuleBase::Vector3<double> tmp_r = r_calculator->get_psi_r_psi (tau1 * this->ucell->lat0,
                                                                                     T1,
                                                                                     L1,
                                                                                     m1,
                                                                                     N1,
                                                                                     tau2 * this->ucell->lat0,
                                                                                     T2,
                                                                                     L2,
                                                                                     m2,
                                                                                     N2);
                    ...
                    for (int ipol = 0; ipol < npol; ipol++)
                        {
                            ...
                            hr_mat_p[ipol * step_trace]
                                -= ((dtau + tau1) * Et) * sr_p[ipol * step_trace] * this->ucell->lat0;
                        }
                    hr_mat_p += npol;
                    sr_p += npol;
                }
...
template <typename TK, typename TR>
void
    hamilt::TD_pot_hybrid<hamilt::OperatorLCAO<TK, TR>>::init_td ()
{
...
template <typename TK, typename TR>
void
    hamilt::TD_pot_hybrid<hamilt::OperatorLCAO<TK, TR>>::update_td ()
{
    // std::cout<<"hybrid gauge" <<std::endl;
    this->cart_At = TD_info::cart_At;
    // std::cout<<"At: "<< TD_info::td_vel_op->cart_At[0] <<" "<<TD_info::td_vel_op->cart_At[1]<<"
    // "<<TD_info::td_vel_op->cart_At[2]<<" "<<std::endl;
    Et = elecstate::H_TDDFT_pw::Et;
}
// set_HR_fixed()
template <typename TK, typename TR>
void
    hamilt::TD_pot_hybrid<hamilt::OperatorLCAO<TK, TR>>::set_HR_fixed (void* HR_fixed_in)
{
...
template <typename TK, typename TR>
void
    hamilt::TD_pot_hybrid<hamilt::OperatorLCAO<TK, TR>>::contributeHR ()
{
...
template <typename TK, typename TR>
void
    hamilt::TD_pot_hybrid<hamilt::OperatorLCAO<TK, TR>>::contributeHk (int ik)
{
    return;
}
