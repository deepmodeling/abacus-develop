...
void
    DFTU<OperatorLCAO<TK, TR>>::cal_force_stress (const bool cal_force,
        ...
        for (int iat0 = 0; iat0 < this->ucell->nat; iat0++)
            {
                                                        ...
                                                        for (int n = 0; n < 4; n++) // value, deri_x, deri_y, deri_z
                                                            {
                                                                ...
                                                                // if(dtau.norm2 == 0.0)
                                                                // std::cout<<__FILE__<<__LINE__<<" "<<m<<" "<<n<<"
                                                                // "<<(m+n*tlp1)<<" "<<iw+m<<" "<<nlm[n][iw+m]<<"
                                                                // "<<nlm_target[m + n * tlp1] << std::endl;
                                                            }
                                                    }
                                                break;
                                            }
                ...
                //     -1/Omega * DMR_{I,J,R'-R} * [ \frac{\partial <phi_{I,R}|chi_m>}{\partial
                //     \tau_{J,\alpha}}\tau_{J,\beta} U*(1/2*delta(m, m')-occ(m, m'))<chi_m'|phi_{J,R'}>
                //   + <phi_{I,R}|chi_m> U*(1/2*delta(m, m')-occ(m, m'))
                //     \frac{\partial <chi_m'|phi_{J,R'}>}{\partial \tau_{J,\alpha}}\tau_{J,\beta}] for each pair of
                //     <IJR> atoms
...
template <typename TK, typename TR>
void
    DFTU<OperatorLCAO<TK, TR>>::cal_force_IJR (const int& iat1,
                                               const int& iat2,
...
template <typename TK, typename TR>
void
    DFTU<OperatorLCAO<TK, TR>>::cal_stress_IJR (const int& iat1,
                                                const int& iat2,
                                                const Parallel_Orbitals* paraV,
                                                const std::unordered_map<int, std::vector<double>>& nlm1_all,
                                                const std::unordered_map<int, std::vector<double>>& nlm2_all,
                                    ...
                                    for (int m2 = 0; m2 < m_size; m2++)
                                        {
                                            ...
                                            // std::cout<<__FILE__<<__LINE__<<" "<<tmp<<" "<<m1<<" "<<m2<<" "<<nlm1[m1 +
                                            // m_size * 2]<<"
