...
#pragma omp for schedule(dynamic)
        for (int iat0 = 0; iat0 < this->ucell->nat; iat0++) {
            ...
            //     \frac{\partial <chi_m'|phi_{J,R'}>}{\partial \tau_{J,\alpha}}\tau_{J,\beta}] for each pair of <IJR>
            //     atoms
            ...
            for (int iw2l = 0; iw2l < col_indexes.size(); iw2l += npol) {
                ...
                for (int m1 = 0; m1 < m_size; m1++) {
                    for (int m2 = 0; m2 < m_size; m2++) {
                        tmp[0] = vu_in[m1 * m_size + m2 + is * m_size2] * nlm1[m1 + m_size] * nlm2[m2] *
                                 dm_pointer[step_trace[step_is]];
                        tmp[1] = vu_in[m1 * m_size + m2 + is * m_size2] * nlm1[m1 + m_size * 2] * nlm2[m2] *
                                 dm_pointer[step_trace[step_is]];
                        tmp[2] = vu_in[m1 * m_size + m2 + is * m_size2] * nlm1[m1 + m_size * 3] * nlm2[m2] *
                                 dm_pointer[step_trace[step_is]];
