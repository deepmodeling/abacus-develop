...
void
    DeltaSpin<OperatorLCAO<TK, TR>>::cal_force_stress (const bool cal_force,
...
template <typename TK, typename TR>
void
    DeltaSpin<OperatorLCAO<TK, TR>>::cal_force_IJR (const int& iat1,
                                                    const int& iat2,
                            ...
                            for (int l = 0; l < lmax; l++)
                                {
                                    for (int m = 0; m < 2 * l + 1; m++)
                                        {
                                            index = l * l + m;
                                            tmp[0] = lambda_tmp * nlm1[index + length] * nlm2[index]
                                                     * dm_pointer[step_trace[is]];
                                            tmp[1] = lambda_tmp * nlm1[index + length * 2] * nlm2[index]
                                                     * dm_pointer[step_trace[is]];
                                            tmp[2] = lambda_tmp * nlm1[index + length * 3] * nlm2[index]
                                                     * dm_pointer[step_trace[is]];
...
template <typename TK, typename TR>
void
    DeltaSpin<OperatorLCAO<TK, TR>>::cal_stress_IJR (const int& iat1,
                                                     const int& iat2,
                                                     const Parallel_Orbitals* paraV,
                                                     const std::unordered_map<int, std::vector<double>>& nlm1_all,
                                                     const std::unordered_map<int, std::vector<double>>& nlm2_all,
                                    ...
                                    for (int m = 0; m < 2 * l + 1; m++)
                                        {
                                            index = l * l + m;
                                            stress[0] += tmp
                                                         * (nlm1[index + length] * dis1.x * nlm2[index]
                                                            + nlm1[index] * nlm2[index + length] * dis2.x);
                                            stress[1] += tmp
                                                         * (nlm1[index + length] * dis1.y * nlm2[index]
                                                            + nlm1[index] * nlm2[index + length] * dis2.y);
                                            stress[2] += tmp
                                                         * (nlm1[index + length] * dis1.z * nlm2[index]
                                                            + nlm1[index] * nlm2[index + length] * dis2.z);
                                            stress[3] += tmp
