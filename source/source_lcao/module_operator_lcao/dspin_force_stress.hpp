        ...
        for (int iw1l = 0; iw1l < row_indexes.size(); iw1l += npol) {
            ...
            for (int iw2l = 0; iw2l < col_indexes.size(); iw2l += npol) {
                ...
                for (int l = 0; l < lmax; l++) {
                    for (int m = 0; m < 2 * l + 1; m++) {
                        index = l * l + m;
                        stress[0] += tmp * (nlm1[index + length] * dis1.x * nlm2[index] +
                                            nlm1[index] * nlm2[index + length] * dis2.x);
                        stress[1] += tmp * (nlm1[index + length] * dis1.y * nlm2[index] +
                                            nlm1[index] * nlm2[index + length] * dis2.y);
                        stress[2] += tmp * (nlm1[index + length] * dis1.z * nlm2[index] +
                                            nlm1[index] * nlm2[index + length] * dis2.z);
                        stress[3] += tmp * (nlm1[index + length * 2] * dis1.y * nlm2[index] +
                                            nlm1[index] * nlm2[index + length * 2] * dis2.y);
                        stress[4] += tmp * (nlm1[index + length * 2] * dis1.z * nlm2[index] +
                                            nlm1[index] * nlm2[index + length * 2] * dis2.z);
                        stress[5] += tmp * (nlm1[index + length * 3] * dis1.z * nlm2[index] +
