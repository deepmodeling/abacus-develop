...
template <typename TK, typename TR>
void
    RDMFT<TK, TR>::init (Parallel_Orbitals& ParaV_in,
                         UnitCell& ucell_in,
    ...
    for (int ik = 0; ik < nk_total; ++ik)
        {
            ...
            // exx-type functional HkPsi( ParaV, HK_XC[0], wfc(ik, 0, 0), H_wfc_XC(ik, 0, 0)); psiDotPsi( ParaV,
            // para_Eij, wfc(ik, 0, 0), H_wfc_XC(ik, 0, 0), Eij_XC, &(wfcHwfc_XC(ik, 0)) );
        }
...
template <typename TK, typename TR>
double
    RDMFT<TK, TR>::cal_E_grad_wfc_occ_num ()
{
...
template <typename TK, typename TR>
void
    RDMFT<TK, TR>::cal_Energy (const int cal_type)
{
...
template <typename TK, typename TR>
double
    RDMFT<TK, TR>::run (ModuleBase::matrix& E_gradient_occNum, psi::Psi<TK>& E_gradient_wfc)
{
