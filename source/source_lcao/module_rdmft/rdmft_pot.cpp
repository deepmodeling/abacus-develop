...
void
    RDMFT<TK, TR>::get_DM_XC (std::vector<std::vector<TK>>& DM_XC)
{
...
template <typename TK, typename TR>
void
    RDMFT<TK, TR>::cal_V_XC (const UnitCell& ucell)
{
    // // //test
    // DM_XC_pass = DM_XC;
    ...
    if (GlobalC::exx_info.info_global.cal_exx)
        {
            ...
            for (int ik = 0; ik < DM_XC.size (); ++ik)
                {
                    DM_XC_pointer[ik] = &DM_XC[ik];
                }
            ...
            if (GlobalC::exx_info.info_ri.real_number)
                {
                    ...
                    std::vector<std::map<int, std::map<std::pair<int, std::array<int, 3>>, RI::Tensor<double>>>> Ds_XC_d
                        = std::is_same<TK, double>::value // gamma_only_local
                              ? RI_2D_Comm::split_m2D_ktoR<double> (ucell, *kv, DM_XC_pointer, *ParaV, nspin)
                              : RI_2D_Comm::split_m2D_ktoR<double> (ucell,
                                                                    *kv,
                                                                    DM_XC_pointer,
                                                                    *ParaV,
                                                                    nspin,
                    ...
                    V_exx_XC = new hamilt::OperatorEXX<hamilt::OperatorLCAO<TK, TR>> (hsk_exx_XC,
                                                                                      HR_exx_XC,
                                                                                      ucell,
                                                                                      *kv,
                                                                                      &Vxc_fromRI_d->Hexxs,
            ...
            else
                {
                    ...
                    std::vector<
                        ...
                        Ds_XC_c
                        = std::is_same<TK, double>::value // gamma_only_local
                              ? RI_2D_Comm::split_m2D_ktoR<std::complex<double>> (ucell,
                                                                                  *kv,
                                                                                  DM_XC_pointer,
                                                                                  *ParaV,
                                                                                  nspin)
