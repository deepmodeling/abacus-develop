...
template <typename TK>
void cal_bra_op_ket(const Parallel_Orbitals* ParaV,
...
template <typename TK>
void occNum_MulPsi(const Parallel_Orbitals* ParaV,
    ...
    for (int ik = 0; ik < nk_local; ++ik) {
        ...
        {
            const double occNum_local =
                occNum_func(occ_number(ik, ParaV->local2global_col(ib_local)), symbol, XC_func_rdmft, alpha);
