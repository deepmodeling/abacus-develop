...
void
    conj_psi (psi::Psi<TK>& wfc)
{
    TK* pwfc = &wfc (0, 0, 0);
    for (int i = 0; i < wfc.size (); ++i)
        {
            pwfc[i] = std::conj (pwfc[i]);
        }
}
...
template <typename TK>
void
    HkPsi (const Parallel_Orbitals* ParaV, const TK& HK, const TK& wfc, TK& H_wfc)
{
...
template <typename TK>
void
    cal_bra_op_ket (const Parallel_Orbitals* ParaV,
...
template <typename TK>
void
    _diagonal_in_serial (const Parallel_2D& para_Eij_in, const std::vector<TK>& Dmn, double* wfcHwfc)
{
    const int nrow_bands = para_Eij_in.get_row_size ();
    const int ncol_bands = para_Eij_in.get_col_size ();
...
template <typename TK>
void
    occNum_MulPsi (const Parallel_Orbitals* ParaV,
                   ...
                   const double alpha = 1.0)
{
    ...
    for (int ik = 0; ik < nk_local; ++ik)
        {
            for (int ib_local = 0; ib_local < nbands_local;
                 ++ib_local) // ib_local < nbands_local , some problem, ParaV->ncol_bands
                {
                    const double occNum_local = occNum_func (occ_number (ik, ParaV->local2global_col (ib_local)),
                                                             symbol,
                                                             XC_func_rdmft,
                                                             alpha);
...
template <typename TK>
void
    add_psi (const Parallel_Orbitals* ParaV,
             const K_Vectors* kv,
             const ModuleBase::matrix& occ_number,
             psi::Psi<TK>& psi_TV,
            ...
            for (int inbn = 0; inbn < nbn_local; ++inbn)
                {
                    ...
                    for (int inbs = 0; inbs < nbs_local; ++inbs)
                        {
                            p_occNum_Hpsi[inbs] = psi_TV (ik, inbn, inbs) + psi_hartree (ik, inbn, inbs)
                                                  + psi_dft_XC (ik, inbn, inbs) + psi_exx_XC (ik, inbn, inbs);
                        }
 ...
 * @param symbol: When symbol = 0, 1, 2, 3, 4, occNum = occNum, 0.5*occNum, g(occNum), 0.5*g(occNum),
 * d_g(occNum)/d_occNum respectively. Default symbol=0.
 */
...
class Veff_rdmft : public hamilt::OperatorLCAO<TK, TR>
{
  public:
    ...
    Veff_rdmft (hamilt::HS_Matrix_K<TK>* hsk_in,
        ...
        : orb_cutoff_ (orb_cutoff), pot (pot_in), ucell (ucell_in), gd (GridD_in),
          hamilt::OperatorLCAO<TK, TR> (hsk_in, kvec_d_in, hR_in), charge_ (charge_in), rho_basis_ (rho_basis_in),
          vloc_ (vloc_in), sf_ (sf_in), potential_ (potential_in), etxc (etxc_in), vtxc (vtxc_in)
    {
