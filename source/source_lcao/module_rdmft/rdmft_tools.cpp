...
template <>
void
    conj_psi<double> (psi::Psi<double>& wfc)
{
}

template <>
void
    HkPsi<double> (const Parallel_Orbitals* ParaV, const double& HK, const double& wfc, double& H_wfc)
{
    const int one_int = 1;
    ...
    // because wfc(bands, basis'), H(basis, basis'), we do wfc*H^T(in the perspective of cpp, not in fortran). And get
    // H_wfc(bands, basis) is correct.
...
template <>
void
    cal_bra_op_ket<double> (const Parallel_Orbitals* ParaV,
...
// When symbol = 0, 1, 2, 3, 4, occNum = occNum, 0.5*occNum, g(occNum), 0.5*g(occNum), d_g(occNum)/d_occNum
// respectively. Default symbol=0.
void
    occNum_Mul_wfcHwfc (const ModuleBase::matrix& occ_number,
    ...
    for (int ir = 0; ir < occ_number.nr; ++ir)
        {
            for (int ic = 0; ic < occ_number.nc; ++ic)
                {
                    occNum_wfcHwfc (ir, ic)
                        += occNum_func (occ_number (ir, ic), symbol, XC_func_rdmft, alpha) * wfcHwfc (ir, ic);
                }
        }
}
...
void
    add_wfcHwfc (const ModuleBase::matrix& wg,
...
double
    getEnergy (const ModuleBase::matrix& occNum_wfcHwfc)
{
    double energy = 0.0;
...
double
    occNum_func (const double eta, const int symbol, const std::string XC_func_rdmft, double alpha)
{
    ...
    if (symbol == 0)
        ...
        {
            return std::pow (eta, alpha);
        }
    else if (symbol == 3)
        {
            return 0.5 * std::pow (eta, alpha);
        }
    else if (symbol == 4)
        {
            return alpha * std::pow (eta, alpha - 1.0);
        }
    else if (symbol == 5)
        {
            return 1.0;
...
void
    Veff_rdmft<TK, TR>::initialize_HR (const UnitCell* ucell_in, const Grid_Driver* GridD)
{
...
template <>
void
    Veff_rdmft<std::complex<double>, double>::contributeHR ()
{
...
template <>
void
    Veff_rdmft<std::complex<double>, std::complex<double>>::contributeHR ()
{
...
template <>
void
    Veff_rdmft<double, double>::contributeHR ()
{
    ModuleBase::TITLE ("Veff", "contributeHR");
    ModuleBase::timer::start ("Veff", "contributeHR");
