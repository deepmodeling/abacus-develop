 ...
 */
static void
    init_gauss_legendre_grid (int grid_size, std::vector<double>& gl_x, std::vector<double>& gl_w)
{
 ...
 */
void
    snap_psibeta_half_tddft (const LCAO_Orbitals& orb,
            ...
            for (int ir = 0; ir < radial_grid_num; ir++)
                {
                    ...
                    for (int ian = 0; ian < angular_grid_num; ian++)
                        {
                            ...
                            double interp_psi
                                = ModuleBase::PolyInt::Polynomial_Interpolation (psi_1, mesh_r1, dk_1, tnorm);
                    ...
                    // Interpolate Beta(|r|)
                    // Note: The original code implies beta_r stores values that might need scaling or are just the
                    // function values. Typically radial integration is \int f(r) r^2 dr. Here we have factor: beta_val
                    // * r_radial[ir] * w_radial[ir] w_radial includes the Jacobian for the change of variable from
                    // [-1,1] to [r_min, r_max]. The extra r_radial[ir] suggests either beta is stored as r*beta, or we
                    // are doing \int ... r dr (2D?), or Jacobian r^2 is split. Assuming original logic is correct.

                    double beta_val
                        = ModuleBase::PolyInt::Polynomial_Interpolation (beta_r, mesh_r0, dk_0, r_radial[ir]);
