...
template <typename TR>
void
    Velocity_op<TR>::initialize_vcomm_r (const Grid_Driver* GridD, const Parallel_Orbitals* paraV)
{
...
template <typename TR>
void
    Velocity_op<TR>::initialize_grad_term (const Grid_Driver* GridD, const Parallel_Orbitals* paraV)
{
...
template <typename TR>
void
    Velocity_op<TR>::calculate_vcomm_r ()
{
                        ...
                        for (int iw1l = 0; iw1l < all_indexes.size (); iw1l += npol)
                            {
                                ...
                                // nlm is a vector of vectors, but size of outer vector is only 1 when out_current is
                                // false and size of outer vector is 4 when out_current is true (3 for <psi|r_i *
                                // exp(-iAr)|beta>, 1 for <psi|exp(-iAr)|beta>) inner loop : all projectors (L0,M0)
                                ...
                                for (int i = 0; i < 3; i++)
                                    {
                                        hamilt::BaseMatrix<std::complex<double>>* matrix_ptr
                                            = this->current_term[i]->find_matrix (iat1,
                                                                                  iat2,
                                                                                  R_vector[0],
                                                                                  R_vector[1],
                                                                                  R_vector[2]);
...
template <typename TR>
void
    Velocity_op<TR>::cal_vcomm_r_IJR (const int& iat1,
                                      const int& iat2,
                                      const int& T0,
                                      const Parallel_Orbitals* paraV,
...
template <typename TR>
void
    Velocity_op<TR>::calculate_grad_term ()
{
...
template <typename TR>
void
    Velocity_op<TR>::cal_grad_IJR (const int& iat1,
                                   const int& iat2,
                                   const Parallel_Orbitals* paraV,
