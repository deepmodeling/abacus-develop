...
void
    TD_MovingGauge::init_DR (const hamilt::HContainer<T_sR>* sR_template,
                            ...
                            for (int iw2l = 0; iw2l < col_indexes.size (); iw2l += npol)
                                {
                                    ...
                                    // out stores the integral value in olm[0], grad_out stores the gradient in olm[1]
                                    // to olm[3]
                                    intor->calculate (T1,
                                                      L1,
                                                      N1,
...
template <typename T_sR>
void
    TD_MovingGauge::update_DR (const hamilt::HContainer<T_sR>* sR_template,
                               const UnitCell* ucell,
                               const Parallel_Orbitals* paraV,
                               TwoCenterIntegrator* intor)
{
...
void
    TD_MovingGauge::get_D_k (int K, const ModuleBase::Vector3<double>& kvec_d, TK* Dk_x, TK* Dk_y, TK* Dk_z, int hk_ld)
        const
{
...
template <typename TK>
void
    TD_MovingGauge::get_P_k (const UnitCell* ucell,
...
template void
    TD_MovingGauge::init_DR<std::complex<double>> (const hamilt::HContainer<std::complex<double>>* sR_template,
                                                   const UnitCell* ucell,
                                                   const Parallel_Orbitals* paraV,
                                                   TwoCenterIntegrator* intor);
...
template void
    TD_MovingGauge::update_DR<std::complex<double>> (const hamilt::HContainer<std::complex<double>>* sR_template,
                                                     const UnitCell* ucell,
                                                     const Parallel_Orbitals* paraV,
                                                     TwoCenterIntegrator* intor);
