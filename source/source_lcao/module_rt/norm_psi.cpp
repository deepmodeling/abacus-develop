...
void
    norm_psi (const Parallel_Orbitals* pv,
              const int nband,
              const int nlocal,
                                    ...
                                    for (int i = 0; i < naroc[0]; ++i)
                                        {
                                            ...
                                            if (igcol == igrow)
                                                {
                                                    Cij[j * naroc[0] + i]
                                                        = {1.0 / sqrt (Cij[j * naroc[0] + i].real ()), 0.0};
                                                }
                                            else
                                                {
...
void
    norm_psi_tensor (const Parallel_Orbitals* pv,
                     const int nband,
                     const int nlocal,
...
void
    norm_psi_tensor_lapack (const Parallel_Orbitals* pv,
                            const int nband,
                            const int nlocal,
                            const ct::Tensor& Stmp,
                            ct::Tensor& psi_k,
