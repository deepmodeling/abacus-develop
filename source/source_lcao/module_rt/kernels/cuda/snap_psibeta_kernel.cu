 ...
 */
__global__ void
    snap_psibeta_atom_batch_kernel (double3 R0,
                                    double3 A,
                    ...
                    if (tnorm <= r1_max)
                        {
                            ...
                            const double psi_val = interpolate_radial_gpu (psi_radial + norb.psi_offset,
                                                                           norb.psi_mesh,
                                                                           1.0 / norb.psi_dk,
                                                                           tnorm);

                            // Interpolate projector radial function
                            const double beta_val = interpolate_radial_gpu (beta_radial + proj.beta_offset,
                                                                            proj.beta_mesh,
                                                                            1.0 / proj.beta_dk,
                                                                            r_val);
