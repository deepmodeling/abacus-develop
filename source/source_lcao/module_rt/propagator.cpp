...
#ifdef __MPI
void
    Propagator::compute_propagator (const int nlocal,
...
void
    Propagator::compute_propagator_tensor (const int nlocal,
                                           ...
                                           CublasMpResources& cublas_res) const
{
    ...
    switch (ptype)
        ...
        case 0:
            if (!use_lapack)
                {
                    compute_propagator_cn2_tensor (nlocal,
                                                   Stmp,
                                                   Htmp,
                                                   U_operator,
                                                   ofs_running,
            ...
            else
                {
                    ...
                    if (myid == root_proc)
                        {
                            compute_propagator_cn2_tensor_lapack<Device> (nlocal,
                                                                          Stmp,
                                                                          Htmp,
                                                                          U_operator,
                                                                          ofs_running,
        ...
        default:
            ModuleBase::WARNING_QUIT (
                "Propagator::compute_propagator_tensor",
