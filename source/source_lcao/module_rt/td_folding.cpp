...
void
    folding_HR_td (const UnitCell& ucell,
...
template <typename TR>
void
    folding_partial_HR (const UnitCell& ucell,
                        const hamilt::HContainer<TR>& hR,
                        std::complex<double>* hk,
                        const ModuleBase::Vector3<double>& kvec_d_in,
                        const int ix,
            ...
            for (int ir = 0; ir < tmp.get_R_size (); ++ir)
                {
                    ...
                    tmp.find_R (r_index);
                    tmp.add_to_matrix (hk,
                                       ncol,
                                       kphase * ModuleBase::IMAG_UNIT * std::complex<double> (dR_car[ix]),
                                       hk_type);
                }
        }
}
...
template <typename TR>
void
    folding_partial_HR_td (const UnitCell& ucell,
                           const hamilt::HContainer<TR>& hR,
                           std::complex<double>* hk,
                           const ModuleBase::Vector3<double>& kvec_d_in,
                           const ModuleBase::Vector3<double>& cart_At,
...
template void folding_HR_td<double> (const UnitCell& ucell,
                                     ...
                                     const int ncol,
                                     const int hk_type);
template void folding_HR_td<std::complex<double>> (const UnitCell& ucell,
                                                   ...
                                                   const int ncol,
                                                   const int hk_type);
template void folding_partial_HR<std::complex<double>> (const UnitCell& ucell,
                                                        const hamilt::HContainer<std::complex<double>>& hR,
                                                        std::complex<double>* hk,
                                                        const ModuleBase::Vector3<double>& kvec_d_in,
                                                        const int ix,
                                                        const int ncol,
                                                        const int hk_type);
template void folding_partial_HR<double> (const UnitCell& ucell,
                                          const hamilt::HContainer<double>& hR,
                                          std::complex<double>* hk,
                                          const ModuleBase::Vector3<double>& kvec_d_in,
                                          const int ix,
                                          const int ncol,
                                          const int hk_type);
template void folding_partial_HR_td<std::complex<double>> (const UnitCell& ucell,
                                                           const hamilt::HContainer<std::complex<double>>& hR,
                                                           std::complex<double>* hk,
                                                           const ModuleBase::Vector3<double>& kvec_d_in,
                                                           const ModuleBase::Vector3<double>& cart_At,
                                                           ...
                                                           const int ncol,
                                                           const int hk_type);
template void folding_partial_HR_td<double> (const UnitCell& ucell,
                                             const hamilt::HContainer<double>& hR,
                                             std::complex<double>* hk,
                                             const ModuleBase::Vector3<double>& kvec_d_in,
                                             const ModuleBase::Vector3<double>& cart_At,
