...
void
    compute_ekb (const Parallel_Orbitals* pv,
                 const int nband,
                 const int nlocal,
                            ...
                            if (std::abs (aa) > 0.0 || std::abs (bb) > 0.0)
                                {
                                    ...
                                    ofs_running << "i = " << std::setw (2) << i << ", j = " << std::setw (2) << j
                                                << ", Eij = " << std::setw (12) << aa << " + " << std::setw (12) << bb
                                                << " i" << std::endl;
...
void
    compute_ekb_tensor (const Parallel_Orbitals* pv,
                        const int nband,
                        const int nlocal,
...
void
    compute_ekb_tensor_lapack (const Parallel_Orbitals* pv,
                               const int nband,
                               const int nlocal,
                               const ct::Tensor& Htmp,
                               const ct::Tensor& psi_k,
