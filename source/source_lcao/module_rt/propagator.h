...
template <typename T>
inline T
    init_value (typename std::enable_if<!std::is_same<T, std::complex<float>>::value
...
template <typename T>
ct::Tensor
    create_identity_matrix (const int n, ct::DeviceType device = ct::DeviceType::CpuDevice)
{
