...
void
    reset_matrix_boundary (const UnitCell& ucell,
                           const K_Vectors& kv,
                    ...
                    for (int ik = 0; ik < kv.get_nks (); ik++)
                        {
                            ...
                            module_rt::boundary_shift_mat (phase,
                                                           hk_last.template data<std::complex<double>> () + ik * len_hs,
                                                           pv,
                                                           iat);
                            module_rt::boundary_shift_mat (phase,
                                                           sk_last.template data<std::complex<double>> () + ik * len_hs,
                                                           pv,
                                                           iat);
                            psi_last->fix_k (ik);
...
void
    boundary_shift_mat (const std::complex<double>& phase,
    ...
    for (size_t nu = 0; nu < pv->get_col_size (iat); ++nu)
        {
            ...
            BlasConnector::scal (pv->get_row_size (), phase, p_matkc, 1);
            p_matkc += pv->get_row_size ();
        }
    std::complex<double>* p_matkr = matk + row0;
    for (size_t mu = 0; mu < pv->get_row_size (iat); ++mu)
        {
            BlasConnector::scal (pv->get_col_size (), phase_conj, p_matkr, pv->get_row_size ());
            p_matkr += 1;
        }
    return;
}
...
void
    boundary_shift_c (const std::complex<double>& phase,
                      ...
                      const size_t iat)
{
    ...
    for (size_t nu = 0; nu < pv->get_row_size (iat); ++nu)
        {
            BlasConnector::scal (pv->ncol_bands, phase_conj, p_ck, pv->get_row_size ());
            p_ck += 1;
        }
    return;
}
