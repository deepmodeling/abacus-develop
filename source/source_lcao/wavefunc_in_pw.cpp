...
void
    Wavefunc_in_pw::make_table_q (const UnitCell& ucell,
                            ...
                            for (int iq = 0; iq < PARAM.globalv.nqx; iq++)
                                {
                                    ...
                                    table_local (it, ic, iq)
                                        = table[iq]; //* Wavefunc_in_pw::smearing(energy_q,150,0.666666);
                                }
                            delete[] table;
                            delete[] radial;
                            delete[] rab;
...
double
    Wavefunc_in_pw::smearing (const double& energy_x, const double& ecut, const double& beta)
{
    double w = 0.0;
...
void
    Wavefunc_in_pw::integral (const UnitCell& ucell,
                              const int meshr,
                              const double* psir,
                              const double* r,
                              const double* rab,
...
void
    Wavefunc_in_pw::produce_local_basis_in_pw (const UnitCell& ucell,
                                               const int& ik,
                    ...
                    for (int L = 0; L < ucell.atoms[it].nwl + 1; L++)
                        {
                            ...
                            for (int N = 0; N < ucell.atoms[it].l_nchi[L]; N++)
                                {
                                    //					GlobalV::ofs_running << " it=" << it << " ia=" << ia << " L=" <<
                                    // L << " N=" << N << std::endl;
                                    ...
                                    if (PARAM.inp.nspin == 4)
                                        {
                                            /*						for(int is_N = 0; is_N < 2; is_N++)*/ // for rotate
                                                                                                          // base
                                                            ...
                                                            if (!(PARAM.globalv.domag || PARAM.globalv.domag_z))
                                                                { // atomic_wfc_so
                                                                    for (int m = 0; m < 2 * L + 1; m++)
                                                                        {
                                                                            std::cout << "iwall: " << iwall
                                                                                      << std::endl;
                                                                            const int lm = L * L + m;
                                                                            for (int ig = 0; ig < npw; ig++)
                                                                                {
                                                                                    // if(is_N==0)
                                                                                    psi (iwall, ig) = lphase * sk[ig]
                                                                                                      * ylm (lm, ig)
                                                                                                      * flq[ig];
                                                                                    // else
                                                                                    psi (iwall + 1,
                                                                                         ig + wfc_basis->npwk_max)
                                                                                        = lphase * sk[ig] * ylm (lm, ig)
                                                                                          * flq[ig];
                                                                                }
                                                                            iwall += 2;
                                                                        }
                                                                ...
                                                                { // atomic_wfc_so_mag
                                                                    ...
                                                                    // int nc;
                                                                    // This routine creates two functions only in the
                                                                    // case j=l+1/2 or exit in the other case
                                                                    ...
                                                                    if (L == 0)
                                                                        ...
                                                                        {
                                                                            /*for(int ib = 0;ib <
                                                                            ucell.atoms[it].nchi;ib++)
                                                                            {
                                                                                if((ucell.atoms[it].lchi[ib] ==
                                                                            L)&&(fabs(ucell.atoms[it].jjj[ib]-L+0.5)<1e-4))
                                                                                {
                                                                                nc=ib;
                                                                                break;
                                                                                }
                                                                    ...
                                                                    // and construct the starting wavefunctions as in
                                                                    // the noncollinear case. alpha =
                                                                    // ucell.magnet.angle1_[it]; gamma = -1 *
                                                                    // ucell.magnet.angle2_[it] + 0.5 * ModuleBase::PI;
                                                                    alpha = ucell.atoms[it].angle1[ia];
                                                                    gamma = -1 * ucell.atoms[it].angle2[ia]
                                                                            + 0.5 * ModuleBase::PI;
                                                                    for (int m = 0; m < 2 * L + 1; m++)
                                                                        {
                                                                            const int lm = L * L + m;
                                                                            ...
                                                                            for (int ig = 0; ig < npw; ig++)
                                                                                {
                                                                                    aux[ig] = sk[ig] * ylm (lm, ig)
                                                                                              * chiaux[ig];
                                                                                }
                                                                            // rotate wfc as needed
                                                                            // first rotation with angle alpha around
                                                                            // (OX)
                                                                            for (int ig = 0; ig < npw; ig++)
                                                                                {
                                                                    ...
                                                                    iwall += 2 * L + 1;
                                                                } // end else INPUT.starting_spin_angle ||
                                                                  // !PARAM.globalv.domag
                                                                    ...
                                                                    for (int ig = 0; ig < npw; ig++)
                                                                        {
                                                                            ...
                                                                            // build the orthogonal wfc
                                                                            // first rotation with
                                                                            // angle(alpha+ModuleBase::PI) around(OX)
                                                                            psi (iwall, ig)
                                                                                = (cos (0.5 * gamman)
                                                                                   + ModuleBase::IMAG_UNIT
                                                                                         * sin (0.5 * gamman))
