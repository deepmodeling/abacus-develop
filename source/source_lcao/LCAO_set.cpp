...
void
    LCAO_domain::set_psi_occ_dm_chg (const K_Vectors& kv,             // k-points
                                     ...
                                     const Input_para& inp)           // input parameters
{
    ...
    if (inp.ocp && inp.esolver_type != "tddft")
        {
            elecstate::fixed_weights (inp.ocp_kb, inp.nbands, inp.nelec, &kv, pelec->wg, pelec->skip_weights);
        }
...
template <typename TK>
void
    LCAO_domain::set_pot (UnitCell& ucell,      // not const because of dftu
...
template <typename TK>
void
    LCAO_domain::init_dm_from_file (const std::string& readin_dir,
                                    const int nspin,
    ...
    for (int is = 0; is < nspin_dm; ++is)
        {
            ...
            hamilt::Read_HContainer<double> reader_dm (dm_container, dmfile, PARAM.globalv.nlocal, &ucell);
            reader_dm.read ();
        }
    return;
}
...
template <typename TK>
void
    LCAO_domain::init_chg_dm (const std::string& readin_dir,
                              const int nspin,
                              LCAO_domain::Setup_DM<TK>& dmat,
                              const UnitCell& ucell,
                              const Parallel_Orbitals* pv,
...
void
    LCAO_domain::init_hr_from_file (const std::string hrfile,
...
void
    LCAO_domain::init_chg_hr (const std::string& readin_dir,
                              const int nspin,
...
template void LCAO_domain::set_psi_occ_dm_chg<double> (const K_Vectors& kv,         // k-points
...
template void LCAO_domain::set_pot<double> (UnitCell& ucell,
                                            K_Vectors& kv,
                                            Structure_Factor& sf,
                                            const ModulePW::PW_Basis& pw_rho,
                                            const ModulePW::PW_Basis& pw_rhod,
...
template void LCAO_domain::init_dm_from_file<double> (const std::string& readin_dir,
                                                      const int nspin,
                                                      LCAO_domain::Setup_DM<double>& dmat,
                                                      const UnitCell& ucell,
                                                      const Parallel_Orbitals* pv);
template void LCAO_domain::init_dm_from_file<std::complex<double>> (const std::string& readin_dir,
                                                                    const int nspin,
                                                                    LCAO_domain::Setup_DM<std::complex<double>>& dmat,
                                                                    const UnitCell& ucell,
                                                                    const Parallel_Orbitals* pv);

template void LCAO_domain::init_chg_dm<double> (const std::string& readin_dir,
                                                const int nspin,
                                                LCAO_domain::Setup_DM<double>& dmat,
                                                const UnitCell& ucell,
                                                const Parallel_Orbitals* pv,
                                                Charge* chr);
template void LCAO_domain::init_chg_dm<std::complex<double>> (const std::string& readin_dir,
                                                              const int nspin,
                                                              LCAO_domain::Setup_DM<std::complex<double>>& dmat,
                                                              const UnitCell& ucell,
                                                              const Parallel_Orbitals* pv,
...
template void LCAO_domain::init_hr_from_file<double> (const std::string hrfile,
                                                      hamilt::HContainer<double>* hmat,
                                                      const UnitCell& ucell,
                                                      const Parallel_Orbitals* pv);
template void LCAO_domain::init_hr_from_file<std::complex<double>> (const std::string hrfile,
                                                                    hamilt::HContainer<std::complex<double>>* hmat,
                                                                    const UnitCell& ucell,
                                                                    const Parallel_Orbitals* pv);

template void LCAO_domain::init_chg_hr<double, double> (const std::string& readin_dir,
                                                        const int nspin,
                                                        ...
                                                        const std::string& ks_solver);
template void
    LCAO_domain::init_chg_hr<std::complex<double>, double> (const std::string& readin_dir,
                                                            const int nspin,
