...
void
    Plus_U::cal_yukawa_lambda (double** rho, const int& nrxx)
{
...
void
    Plus_U::cal_slater_Fk (const UnitCell& ucell, const int L, const int T)
{
                                    ...
                                    for (int ir1 = 1; ir1 < mesh; ir1++)
                                        {
                                            ...
                                            this->Fk[T][L][chi][k] -= (4 * k + 1) * lambda * pow (R_L0, 2) * bslval
                                                                      * hnkval * pow (R_L1, 2) * pow (r0, 2)
                                                                      * pow (r1, 2) * rab0 * rab1;
                                        }
                                }
                        }
...
void
    Plus_U::cal_slater_UJ (const UnitCell& ucell, double** rho, const int& nrxx)
{
            ...
            for (int l = 0; l < NL; l++)
                {
                    ...
                    for (int n = 0; n < N; n++)
                        {
                            ModuleBase::GlobalFunc::ZEROS (ModuleBase::GlobalFunc::VECTOR_TO_PTR (this->Fk[it][l][n]),
                                                           l + 1);
                        }
                }
        }
                    ...
                    if (L >= PARAM.inp.orbital_corr[T] && PARAM.inp.orbital_corr[T] != -1)
                        {
                            ...
                            if (L == 1)
                            ...
                            else if (L == 3)
                                {
                                    this->U_Yukawa[T][L][0] = this->Fk[T][L][0][0];
                                    this->J_Yukawa[T][L][0]
                                        = (286.0 * this->Fk[T][L][0][1] + 195.0 * this->Fk[T][L][0][2]
...
double
    Plus_U::spherical_Bessel (const int k, const double r, const double lambda)
{
...
double
    Plus_U::spherical_Hankel (const int k, const double r, const double lambda)
{
    ModuleBase::TITLE ("Plus_U", "spherical_Bessel");
    ...
    if (k == 0)
    ...
    else if (k == 6)
        {
            if (x < 9.0e-1)
                {
                    val = 10395 / pow (x, 7) - 945 / (2 * pow (x, 5)) + 105 / (8 * pow (x, 3)) - 5 / (16 * x)
                          + x / 128.0 - pow (x, 3) / 3840.0 + pow (x, 5) / 46080.0 - pow (x, 6) / 135135.0;
                }
            else
                {
                    val = exp (-x)
                          * (10395 + 10395 * x + 4725 * pow (x, 2) + 1260 * pow (x, 3) + 210 * pow (x, 4)
                             + 21 * pow (x, 5) + pow (x, 6))
                          / pow (x, 7);
                }
        }
