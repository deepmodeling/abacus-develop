...
void Plus_U::cal_slater_Fk(const UnitCell& ucell, const int L, const int T) {
        ...
        for (int L = 0; L < NL; L++) {
            ...
            if (L >= PARAM.inp.orbital_corr[T] && PARAM.inp.orbital_corr[T] != -1) {
                ...
                if (L == 1) {
                    ...
                    this->U_Yukawa[T][L][0] = this->Fk[T][L][0][0];
                    this->J_Yukawa[T][L][0] =
                        (286.0 * this->Fk[T][L][0][1] + 195.0 * this->Fk[T][L][0][2] + 250.0 * this->Fk[T][L][0][3]) /
                        6435.0;
                }
...
double Plus_U::spherical_Hankel(const int k, const double r, const double lambda) {
    ...
    if (k == 0) {
        ...
        if (x < 5.0e-1) {
            val =
                -105 / pow(x, 5) + 15 / (2 * pow(x, 3)) - 3 / (8 * x) + x / 48 - pow(x, 3) / 384.0 + pow(x, 4) / 945.0;
        } else {
