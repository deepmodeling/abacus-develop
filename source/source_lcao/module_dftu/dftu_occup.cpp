...
void
    Plus_U::copy_locale (const UnitCell& ucell)
{
...
void
    Plus_U::zero_locale (const UnitCell& ucell)
{
...
void
    Plus_U::mix_locale (const UnitCell& ucell, const double& mixing_beta)
{
                            ...
                            for (int n = 0; n < N; n++)
                                {
                                    if (PARAM.inp.nspin == 4)
                                        {
                                            locale[iat][l][n][0] = locale[iat][l][n][0] * beta
                                    ...
                                    else if (PARAM.inp.nspin == 1 || PARAM.inp.nspin == 2)
                                        {
                                            locale[iat][l][n][0] = locale[iat][l][n][0] * beta
                                                                   + locale_save[iat][l][n][0] * (1.0 - beta);
                                            locale[iat][l][n][1] = locale[iat][l][n][1] * beta
...
void
    Plus_U::cal_occup_m_k (const int iter,
    ...
    for (int ik = 0; ik < kv.get_nks (); ik++)
        {
            ...
            if (PARAM.inp.nspin != 4)
                ...
                {
                    s_k_pointer = dynamic_cast<hamilt::HamiltLCAO<std::complex<double>, std::complex<double>>*> (p_ham)
                                      ->getSk ();
                }
                                                            ...
                                                            for (int m1 = 0; m1 < 2 * l + 1; m1++)
                                                                {
                                                                    for (int ipol1 = 0; ipol1 < PARAM.globalv.npol;
                                                                        ...
                                                                        {
                                                                            const int iwt1
                                                                                = this->iatlnmipol2iwt[iat][l][n][m1]
                                                                                                      [ipol1];
                                                                            const int nu
                                                                                = this->paraV->global2local_col (iwt1);
                                                                            const int nu_prime
                                                                                = this->paraV->global2local_row (iwt1);

                                                                            const int irc = nu * this->paraV->nrow + mu;
                                                                            const int irc_prime
                                                                                = mu_prime * this->paraV->nrow
                                                                                  + nu_prime;
                                    ...
                                    if (PARAM.inp.nspin == 1 || PARAM.inp.nspin == 4)
                                        {
                                            ...
                                            MPI_Allreduce (&temp (0, 0),
                                                           ...
                                                           (2 * l + 1) * PARAM.globalv.npol * (2 * l + 1)
                                                               * PARAM.globalv.npol,
                                                           MPI_DOUBLE,
                                                           MPI_SUM,
                                                           MPI_COMM_WORLD);
...
void
    Plus_U::cal_occup_m_gamma (const int iter,
                               const UnitCell& ucell,
