...
void
    Plus_U::cal_occ_pw (const int iter,
                        const void* psi_in,
                        ...
                        const double& mixing_beta)
{
    ...
    if (PARAM.inp.device == "cpu")
                                            ...
                                            for (int m2 = 0; m2 < tlp1; m2++)
                                                {
                                                    ...
                                                    occ[3]
                                                        = weight * conj (becp[index_m1 + nkb]) * becp[index_m2 + nkb];
                                                    this->locale[iat][target_l][0][0].c[ind_m1m2]
                                                        += (occ[0] + occ[3]).real ();
                                                    this->locale[iat][target_l][0][0].c[ind_m1m2 + tlp1_2]
                                                        += (occ[1] + occ[2]).real ();
                                                    this->locale[iat][target_l][0][0].c[ind_m1m2 + 2 * tlp1_2]
                                                        += (occ[1] - occ[2]).imag ();
                                                    this->locale[iat][target_l][0][0].c[ind_m1m2 + 3 * tlp1_2]
                                                        += (occ[0] - occ[3]).real ();
                                                    ind_m1m2++;
                                                }
                                        }
    ...
    else
        {
            ...
            const psi::Psi<std::complex<double>, base_device::DEVICE_GPU>* psi_p
                = (const psi::Psi<std::complex<double>, base_device::DEVICE_GPU>*)psi_in;
            // loop over k-points to calculate Mi of \sum_{k,i,l,m}<Psi_{k,i}|alpha_{l,m}><alpha_{l,m}|Psi_{k,i}>
            const int nbands = psi_p->get_nbands ();
            for (int ik = 0; ik < psi_p->get_nk (); ik++)
                {
            ...
            for (int m1 = 0; m1 < m_size; m1++)
                {
                    for (int m2 = 0; m2 < m_size; m2++)
                        {
                            vu_iat[m1 * m_size + m2]
                                = u_value * (1.0 * (m1 == m2) - this->locale[iat][target_l][0][0].c[m2 * m_size + m1]);
                    ...
                    for (int m1 = 0; m1 < m_size; m1++)
                        {
                            for (int m2 = 0; m2 < m_size; m2++)
                                {
                                    vu_iat[start + m1 * m_size + m2]
                                        = u_value * (0 - this->locale[iat][target_l][0][0].c[start + m2 * m_size + m1]);
...
void
    Plus_U::cal_VU_pot_pw (const int spin)
{
}
