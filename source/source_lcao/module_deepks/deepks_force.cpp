...
void
    DeePKS_domain::cal_f_delta (const hamilt::HContainer<double>* dmr,
                        ...
                        for (int ad2 = 0; ad2 < adjs.adj_num + 1; ad2++)
                            {
                                ...
                                const double* dm_current
                                    = dmr->find_matrix (ibt1, ibt2, dR.x, dR.y, dR.z)->get_pointer ();
                                                        ...
                                                        for (int L0 = 0; L0 <= orb.Alpha[0].getLmax (); ++L0)
                                                            {
                                                                for (int N0 = 0; N0 < orb.Alpha[0].getNchi (L0); ++N0)
                                                                    {
                                                                        const int inl
                                                                            = deepks_param.inl_index[T0](I0, L0, N0);
                                                                        const int nm = 2 * L0 + 1;
