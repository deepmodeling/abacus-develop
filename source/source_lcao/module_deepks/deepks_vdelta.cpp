...
void
    DeePKS_domain::cal_e_delta_band (const std::vector<std::vector<TK>>& dm,
                            ...
                            for (int is = 0; is < nspin; is++)
                                {
                                    for (int ik = 0; ik < nks / nspin; ik++)
                                        {
                                            e_delta_band_tmp
                                                += dm[ik + is * nks / nspin][nu * pv->nrow + mu] * V_delta[ik][iic];
                                        }
                                }
                        }
                }
...
void
    DeePKS_domain::collect_h_mat (const Parallel_Orbitals& pv,
...
void
    DeePKS_domain::get_h_tot (const Parallel_Orbitals& pv,
