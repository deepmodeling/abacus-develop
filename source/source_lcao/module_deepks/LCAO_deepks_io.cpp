...
void
    LCAO_deepks_io::print_dm (const int nks, const int nlocal, const int nrow, const std::vector<std::vector<TK>>& dm)
{
    std::stringstream ss;
...
void
    LCAO_deepks_io::load_npy_gedm (const int nat,
    ...
    if (rank == 0)
        {
            ...
            std::vector<unsigned long> dshape
                = {static_cast<unsigned long> (nat), static_cast<unsigned long> (des_per_atom)};
...
void
    LCAO_deepks_io::save_npy_d (const int nat,
...
void
    LCAO_deepks_io::save_npy_e (const double& e, const std::string& e_file, const int rank, const double unit_scale)
{
...
void
    LCAO_deepks_io::save_npy_h (const std::vector<TH>& hamilt,
...
void
    LCAO_deepks_io::save_matrix2npy (const std::string& file_name,
...
template <typename T>
void
    LCAO_deepks_io::save_tensor2npy (const std::string& file_name, const torch::Tensor& tensor, const int rank)
{
...
template void
    LCAO_deepks_io::save_tensor2npy<int> (const std::string& file_name, const torch::Tensor& tensor, const int rank);

template void
    LCAO_deepks_io::save_tensor2npy<double> (const std::string& file_name, const torch::Tensor& tensor, const int rank);
