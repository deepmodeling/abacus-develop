...
template <typename T>
void
    test_deepks<T>::read_dm (const int nks)
{
    dm.resize (nks);
    std::stringstream ss;
...
template <typename T>
void
    test_deepks<T>::check_descriptor (std::vector<torch::Tensor>& descriptor)
{
...
template <typename T>
void
    test_deepks<T>::check_gdmx (torch::Tensor& gdmx)
{
...
template <typename T>
void
    test_deepks<T>::check_gvx (torch::Tensor& gdmx)
{
...
template <typename T>
void
    test_deepks<T>::check_gdmepsl (torch::Tensor& gdmepsl)
{
...
template <typename T>
void
    test_deepks<T>::check_gvepsl (torch::Tensor& gdmepsl)
{
    std::vector<torch::Tensor> gevdm;
    DeePKS_domain::cal_gevdm (ucell.nat, this->ld.deepks_param, this->ld.pdm, gevdm);
    torch::Tensor gvepsl;
...
template <typename T>
void
    test_deepks<T>::check_edelta (std::vector<torch::Tensor>& descriptor)
{
...
template <typename T>
void
    test_deepks<T>::check_e_deltabands ()
{
    this->cal_V_delta ();
...
template <typename T>
void
    test_deepks<T>::check_f_delta_and_stress_delta ()
{
...
template <typename T>
void
    test_deepks<T>::compare_with_ref (const std::string f1, const std::string f2)
{
    this->total_check += 1;
