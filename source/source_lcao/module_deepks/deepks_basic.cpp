...
void
    DeePKS_domain::cal_gevdm (const int nat,
...
void
    DeePKS_domain::load_model (const std::string& model_file, torch::jit::script::Module& model)
{
    ...
    if (!ifs)
        {
            ...
            ModuleBase::WARNING_QUIT ("DeePKS_domain::load_model",
                                      "No model file named " + model_file + ", please check!");
            return;
        }
    ifs.close ();
    try
...
void
    DeePKS_domain::cal_edelta_gedm_equiv (const int nat,
                                          const DeePKS_Param& deepks_param,
            ...
            if (static_cast<int> (deepks_param.nchi_d_l.size ()) != deepks_param.lmaxd + 1)
                {
                    ModuleBase::WARNING_QUIT ("DeePKS_domain::cal_edelta_gedm_equiv",
                                              "Invalid nchi_d_l in DeePKS parameters: expected size lmaxd + 1 for "
                                              "equivariant shell construction.");
                }
            ...
            for (int l = 0; l <= deepks_param.lmaxd; ++l)
                {
                    ...
                    for (int n = 0; n < deepks_param.nchi_d_l[l]; ++n)
                        {
                            torch::Tensor po
                                = torch::zeros ({basis_size, 1, nm}, torch::TensorOptions ().dtype (torch::kFloat64));
...
void
    DeePKS_domain::cal_edelta_gedm (const int nat,
                                    const DeePKS_Param& deepks_param,
                                    const std::vector<torch::Tensor>& descriptor,
                                    const std::vector<torch::Tensor>& pdm,
                                    torch::jit::script::Module& model_deepks,
...
void
    DeePKS_domain::check_gedm (const DeePKS_Param& deepks_param, double** gedm)
{
...
void
    DeePKS_domain::prepare_atom (const UnitCell& ucell, torch::Tensor& atom_out)
{
    int nat = ucell.nat;
...
void
    DeePKS_domain::prepare_box (const UnitCell& ucell, torch::Tensor& box_out)
{
