...
void
    DeePKS_domain::read_pdm (bool read_pdm_file,
                             bool is_equiv,
                             bool& init_pdm,
                             const int nat,
...
void
    DeePKS_domain::update_dmr (const std::vector<ModuleBase::Vector3<double>>& kvec_d,
...
template <typename TK>
void
    DeePKS_domain::cal_pdm (bool& init_pdm,
                            const DeePKS_Param& deepks_param,
                    ...
                    for (int irow = 0; irow < row_size; irow++)
                        {
                            ...
                            for (int i = 0; i < trace_alpha_size; i++)
                                {
                                    s_1t[i * row_size + irow]
                                        = row_ptr->get_value (row_indexes[irow], trace_alpha_row[i]);
                                }
                        }
                                    ...
                                    for (int i = 0; i < trace_alpha_size; i++)
                                        {
                                            s_2t[i * col_size + icol]
                                                = col_ptr->get_value (col_indexes[icol], trace_alpha_col[i]);
                                        }
                                }
                            // prepare DM from DMR
                                            ...
                                            for (int m1 = 0; m1 < nm; ++m1) // m1 = 1 for s, 3 for p, 5 for d
                                                {
                                                    for (int m2 = 0; m2 < nm; ++m2) // m1 = 1 for s, 3 for p, 5 for d
                                                        {
                                                            accessor[m1][m2] += BlasConnector::dot (
                                                                row_size,
                                    ...
                                    for (int jproj = 0; jproj < nproj; jproj++)
                                        {
                                            ...
                                            accessor[iproj * nproj + jproj]
                                                += BlasConnector::dot (row_size,
                                                                       g_1dmt.data () + index * row_size,
                                                                       inc,
                                                                       s_1t.data () + index * row_size,
                                                                       inc);
...
void
    DeePKS_domain::check_pdm (const DeePKS_Param& deepks_param, const std::vector<torch::Tensor>& pdm)
{
...
template void
    DeePKS_domain::update_dmr<std::complex<double>> (const std::vector<ModuleBase::Vector3<double>>& kvec_d,
