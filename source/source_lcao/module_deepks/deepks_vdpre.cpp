...
void
    DeePKS_domain::cal_v_delta_precalc (const int nlocal,
                                        const int nat,
                                        const int nks,
                                ...
                                for (int ik = 0; ik < nks; ik++)
                                    {
                                        ...
                                        if (std::is_same<TK, std::complex<double>>::value)
                                            {
                                                const double arg
                                                    = (kvec_d[ik] * ModuleBase::Vector3<double> (dR1 - dR2))
                                                      * ModuleBase::TWO_PI;
    ...
    for (int nl = 0; nl < nlmax; ++nl)
        {
            ...
            v_delta_precalc_vector.push_back (
                at::einsum ("kxyamn, avmn->kxyav", {v_delta_pdm_vector[nl], gevdm_totype}));
        }
...
template <typename TK>
void
    DeePKS_domain::prepare_phialpha (const int nlocal,
                                     const int nat,
                                     const int nks,
                                     const DeePKS_Param& deepks_param,
                                     const std::vector<ModuleBase::Vector3<double>>& kvec_d,
...
void
    DeePKS_domain::prepare_gevdm (const int nat,
                                  const DeePKS_Param& deepks_param,
                                  const LCAO_Orbitals& orb,
                                  ...
                                  torch::Tensor& gevdm_out)
{
    ...
    for (int nl = 0; nl < nlmax; ++nl)
        {
            torch::Tensor gevdm_tmp
                = torch::zeros ({nat, mmax, mmax, mmax}, torch::TensorOptions ().dtype (torch::kFloat64));
...
template void DeePKS_domain::cal_v_delta_precalc<double> (const int nlocal,
                                                          ...
                                                          torch::Tensor& v_delta_precalc);
template void
    DeePKS_domain::cal_v_delta_precalc<std::complex<double>> (const int nlocal,
                                                              const int nat,
                                                              const int nks,
                                                              const DeePKS_Param& deepks_param,
                                                              const std::vector<ModuleBase::Vector3<double>>& kvec_d,
...
template void
    DeePKS_domain::prepare_phialpha<std::complex<double>> (const int nlocal,
                                                           const int nat,
                                                           const int nks,
                                                           const DeePKS_Param& deepks_param,
                                                           const std::vector<ModuleBase::Vector3<double>>& kvec_d,
