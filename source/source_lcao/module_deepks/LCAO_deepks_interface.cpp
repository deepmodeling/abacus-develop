...
std::string
    true_file_type (const std::string& file_type)
{
...
template <typename TK, typename TR>
std::string
    LCAO_Deepks_Interface<TK, TR>::get_filename (const std::string& file_type, const int& label_type, const int& iter)
{
...
template <typename TK, typename TR>
void
    LCAO_Deepks_Interface<TK, TR>::out_deepks_labels (const double& etot,
                                                      const int& nks,
                                                      const int& nat,
                                                      const int& nlocal,
                    ...
                    if (PARAM.inp.cal_force)
                        {
                            ...
                            if (output_precalc              // don't need these when deepks_out_labels == 2
                                && !PARAM.inp.deepks_equiv) // training with force label not supported by equivariant
                                                            // version now
                                {
                                    torch::Tensor gdmx;
                                    ...
                                    const std::string file_gradvx
                                        = get_filename ("gradvx", PARAM.inp.deepks_out_labels, iter);
                    ...
                    if (PARAM.inp.cal_stress)
                        {
                            // these items are not related to model, so can output without deepks_scf
                            if (output_precalc              // don't need these when deepks_out_labels == 2
                                && !PARAM.inp.deepks_equiv) // training with stress label not supported by equivariant
                                                            // version now
                                {
                                    ...
                                    const std::string file_gvepsl
                                        = get_filename ("gvepsl", PARAM.inp.deepks_out_labels, iter);
                    ...
                    for (int iks = 0; iks < nks; ++iks)
                        {
                            ...
                            if (PARAM.inp.deepks_bandgap == 1 || PARAM.inp.deepks_bandgap == 3)
                                ...
                                {
                                    for (int ir = PARAM.inp.deepks_band_range[0]; ir <= PARAM.inp.deepks_band_range[1];
                                         ++ir)
                                        {
                                            if (ir != -1)
                                                {
                    ...
                    if (output_precalc)
                        {
                            ...
                            for (int iks = 0; iks < nks; ++iks)
                                {
                                    ...
                                    if (PARAM.inp.deepks_bandgap == 1 || PARAM.inp.deepks_bandgap == 3)
                                        ...
                                        {
                                            for (int ir = PARAM.inp.deepks_band_range[0];
                                                 ir <= PARAM.inp.deepks_band_range[1];
                                                 ++ir)
                                                {
                                                    if (ir != -1)
                                                        {
                            ...
                            for (int ir = 0; ir < range; ++ir)
                                {
                                    ...
                                    if (PARAM.inp.deepks_scf)
                                        {
                                            DeePKS_domain::cal_o_delta<TK, TH> (dm_bandgap,
                                                                                *h_delta,
                                                                                o_delta_temp,
                                                                                *ParaV,
                                                                                nks,
                            ...
                            if (PARAM.inp.deepks_scf)
                                {
                                    const std::string file_obase
                                        = get_filename ("obase", PARAM.inp.deepks_out_labels, iter);
                                    LCAO_deepks_io::save_matrix2npy (file_obase,
                                                                     o_tot - o_delta,
                                                                     rank); // Unit: Hartree
                                }
                            else
                    ...
                    if (PARAM.inp.deepks_v_delta < 0)
                        {
                            ...
                            const std::string file_hrtot
                                = PARAM.globalv.global_out_dir
                                  + (PARAM.inp.deepks_out_labels == 1 ? "deepks_hrtot.csr"
                                                                      : "deepks_hamiltonian_r.csr");
                            ...
                            if (PARAM.inp.deepks_scf)
                                {
                                    if (PARAM.inp.deepks_out_labels == 1)
                                        {
                                            const std::string file_vdeltar
                                                = PARAM.globalv.global_out_dir + "deepks_hrdelta.csr";
                                            ...
                                            if (rank == 0)
                                                {
                                                    ...
                                                    ofs_hr
                                                        << "Matrix Dimension of H_delta(R): " << h_deltaR->get_nbasis ()
                                                        << std::endl;
                                                    ofs_hr
                                                        << "Matrix number of H_delta(R): " << h_deltaR->size_R_loop ()
                                                        << std::endl;
                                            ...
                                            if (PARAM.inp.deepks_v_delta == -1)
                                                {
                                                    ...
                                                    const std::string file_vdrpre
                                                        = PARAM.globalv.global_out_dir + "deepks_vdrpre.npy";
                                                    LCAO_deepks_io::save_tensor2npy<double> (file_vdrpre,
                                                                                             vdr_precalc,
                                                                                             rank);
                                                }
                                            else if (PARAM.inp.deepks_v_delta == -2)
                                                {
                                                    ...
                                                    DeePKS_domain::prepare_gevdm (nat,
                                                                                  deepks_param,
                                                                                  orb,
                                                                                  gevdm,
                                                                                  gevdm_out);
                                                    const std::string file_gevdm
                                                        = PARAM.globalv.global_out_dir + "deepks_gevdm.npy";
                                                    LCAO_deepks_io::save_tensor2npy<double> (file_gevdm,
                                                                                             gevdm_out,
                                                                                             rank);
                                                    ...
                                                    DeePKS_domain::prepare_phialpha_iRmat (nlocal,
                                                                                           ...
                                                                                           overlap_out,
                                                                                           iRmat);
                                                    const std::string file_overlap
                                                        = PARAM.globalv.global_out_dir + "deepks_phialpha_r.npy";
                                                    LCAO_deepks_io::save_tensor2npy<double> (file_overlap,
                                                                                             overlap_out,
                                                                                             rank);
                                                    const std::string file_iRmat
                                                        = PARAM.globalv.global_out_dir + "deepks_iRmat.npy";
                            ...
                            if (PARAM.inp.deepks_scf)
                                {
                                    ...
                                    const std::string file_hbase
                                        = get_filename ("hbase", PARAM.inp.deepks_out_labels, iter);
                                    for (int ik = 0; ik < nks; ik++)
                                        {
                                    ...
                                    const std::string file_vdelta
                                        = get_filename ("vdelta", PARAM.inp.deepks_out_labels, iter);
                            ...
                            if (PARAM.inp.deepks_v_delta == 1) // v_delta_precalc storage method 1
                                {
                                    ...
                                    const std::string file_vdpre
                                        = get_filename ("vdpre", PARAM.inp.deepks_out_labels, iter);
                            ...
                            else if (PARAM.inp.deepks_v_delta == 2) // v_delta_precalc storage method 2
                                {
                                    ...
                                    DeePKS_domain::prepare_phialpha<TK> (nlocal,
                                                                         ...
                                                                         *ParaV,
                                                                         GridD,
                                                                         phialpha_out);
                                    const std::string file_phialpha
                                        = get_filename ("phialpha", PARAM.inp.deepks_out_labels, iter);
                                    ...
                                    const std::string file_gevdm
                                        = get_filename ("gevdm", PARAM.inp.deepks_out_labels, iter);
                    ...
                    if (rank == 0)
                        {
                            ...
                            ofs_running << "  E_delta_band = " << std::setprecision (8) << e_delta_band << " Ry"
                                        << " = " << std::setprecision (8) << e_delta_band * ModuleBase::Ry_to_eV
                                        << " eV" << std::endl;
                            ofs_running << "  E_delta_NN = " << std::setprecision (8) << E_delta << " Ry"
                                        << " = " << std::setprecision (8) << E_delta * ModuleBase::Ry_to_eV << " eV"
                                        << std::endl;
                            ofs_running << " -----------------------------------------------" << std::endl;
                        }
                    if (PARAM.inp.deepks_out_unittest)
