...
class OutputHContainerTest : public testing::Test
{
  protected:
    ...
    hamilt::HContainer<double>*
        create_hcontainer (double scale)
    {
    ...
    void
        write_full_csr (const std::string& filename,
...
TEST_F (OutputHContainerTest, Nspin2TwoFileConsistency)
{
    ...
    EXPECT_GT (
        std::abs (hc_read_up->find_pair (0, 0)->get_pointer ()[0] - hc_read_down->find_pair (0, 0)->get_pointer ()[0]),
        1e-6);
