...
void Setup_DM<TK>::allocate_dm(const K_Vectors* kv, const Parallel_Orbitals* paraV, const int nspin)
{
    const int nspin_dm = nspin == 2 ? 2 : 1;
    this->dm = new elecstate::DensityMatrix<TK, double>(paraV, nspin_dm, kv->kvec_d, kv->get_nks() / nspin_dm);
}
