...
{
GintAtom::GintAtom (const Atom* atom,
                    int it,
                    int ia,
                    int iat,
                    Vec3i biggrid_idx,
    ...
    : atom_ (atom), it_ (it), ia_ (ia), iat_ (iat), biggrid_idx_ (biggrid_idx), unitcell_idx_ (unitcell_idx),
      tau_in_biggrid_ (tau_in_biggrid), orb_ (orb), ucell_ (ucell)
{
...
template <typename T>
void
    GintAtom::set_phi (const std::vector<Vec3d>& coords, const int stride, T* phi) const
{
                    ...
                    for (int ib = 0; ib < num_blocks; ++ib)
                        {
                            ...
                            const double psi = c1 * psi_uniform[ip] + c2 * dpsi_uniform[ip] + c3 * psi_uniform[ip + 1]
...
template <typename T>
void
    GintAtom::set_phi_dphi (const std::vector<Vec3d>& coords, const int stride, T* phi, T* dphi_x, T* dphi_y, T* dphi_z)
        const
{
    const int num_mgrids = coords.size ();
...
template void GintAtom::set_phi_dphi (const std::vector<Vec3d>& coords,
                                      const int stride,
                                      float* phi,
                                      float* dphi_x,
                                      float* dphi_y,
