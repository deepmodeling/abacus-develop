...
inline void
    finalize_hr_gint_gpu_ (HContainer<double>& hr_gint, HContainer<double>* hR)
{
...
void
    finalize_hr_gint_gpu_ (HContainer<Real>& hr_gint, HContainer<double>* hR)
{
    ...
    if (std::is_same<Real, double>::value)
        {
            ...
            CHECK_CUDA (cudaMemcpy (vr_eff_d.get_device_ptr (),
                                    reinterpret_cast<const Real*> (vr_eff_),
                                    local_mgrid_num * sizeof (Real),
    ...
    else
        {
            ...
            std::transform (vr_eff_,
                            vr_eff_ + local_mgrid_num,
                            vr_eff_buffer.begin (),
...
#pragma omp for schedule(dynamic)
        for (int i = 0; i < gint_info_->get_bgrid_batches_num (); ++i)
            {
                ...
                phi_op.phi_mul_vldr3 (vr_eff_d.get_device_ptr (),
                                      static_cast<Real> (dr3_),
                                      phi.get_device_ptr (),
                                      phi_vldr3.get_device_ptr ());
                phi_op.phi_mul_phi (phi.get_device_ptr (),
                                    phi_vldr3.get_device_ptr (),
                                    hr_gint,
                                    hr_gint_d.get_device_ptr ());
            }
    ...
    CHECK_CUDA (cudaMemcpy (hr_gint.get_wrapper (),
                            hr_gint_d.get_device_ptr (),
                            hr_gint.get_nnr () * sizeof (Real),
