...
void Gint_vl_gpu::cal_gint_impl_() {
    ...
    if (std::is_same<Real, double>::value) {
        ...
        std::transform(vr_eff_, vr_eff_ + local_mgrid_num, vr_eff_buffer.begin(), [](const double v) {
...
#pragma omp for schedule(dynamic)
        for (int i = 0; i < gint_info_->get_bgrid_batches_num(); ++i) {
            ...
            phi_op.phi_mul_phi(phi.get_device_ptr(), phi_vldr3.get_device_ptr(), hr_gint, hr_gint_d.get_device_ptr());
        }
