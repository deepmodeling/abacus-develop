...
template <typename T>
void
    PhiOperator::set_phi (T* phi) const
{
...
template <typename T>
void
    PhiOperator::phi_mul_dm (const T* const phi,      // phi(ir,iwt)
            ...
            if (is_symm)
                {
                    ...
                    BlasConnector::symm_cm ('L',
                                            ...
                                            dm_mat->get_pointer (),
                                            atoms_phi_len_[i],
                                            &phi[0 * cols_ + atoms_startidx_[i]],
                                            cols_,
                                            beta,
                                            &phi_dm[0 * cols_ + atoms_startidx_[i]],
                                            cols_);
                }
            ...
            for (int j = start; j < biggrid_->get_atoms_num (); ++j)
                {
                    ...
                    BlasConnector::gemm ('N',
                                         ...
                                         atoms_phi_len_[i],
                                         alpha,
                                         &phi[start_idx * cols_ + atoms_startidx_[i]],
                                         cols_,
                                         dm_mat->get_pointer (),
                                         atoms_phi_len_[j],
                                         beta,
                                         &phi_dm[start_idx * cols_ + atoms_startidx_[j]],
                                         cols_);
                }
        }
}
...
template <typename T>
void
    PhiOperator::phi_mul_vldr3 (const T* const vl, // vl(ir)
                                const T dr3,
...
template <typename T>
void
    PhiOperator::phi_mul_phi (const T* const phi_i, // phi_i(ir,iwt)
...
void
    PhiOperator::phi_dot_phi (const Tin* const phi_i, // phi_i(ir,iwt)
                              ...
                              Tout* const rho) const  // rho(ir)
{
    ...
    for (int i = 0; i < biggrid_->get_mgrids_num (); ++i)
        {
            rho[meshgrids_local_idx_[i]]
                += static_cast<Tout> (BlasConnector::dotc (cols_, phi_j + i * cols_, inc, phi_i + i * cols_, inc));
        }
}
