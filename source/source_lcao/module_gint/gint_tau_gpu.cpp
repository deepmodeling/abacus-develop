...
void
    Gint_tau_gpu::transfer_cpu_to_gpu_ ()
{
...
void
    Gint_tau_gpu::transfer_gpu_to_cpu_ ()
{
    for (int is = 0; is < nspin_; is++)
        {
            CHECK_CUDA (cudaMemcpy (kin_[is],
                                    kin_d_vec_[is].get_device_ptr (),
                ...
                for (int is = 0; is < nspin_; is++)
                    {
                        ...
                        phi_op.phi_mul_dm (dphi_x.get_device_ptr (),
                                           dm_gint_d_vec_[is].get_device_ptr (),
                                           dm_gint_vec_[is],
                                           is_symm,
                                           dphi_x_dm.get_device_ptr ());
                        phi_op.phi_mul_dm (dphi_y.get_device_ptr (),
                                           dm_gint_d_vec_[is].get_device_ptr (),
                                           dm_gint_vec_[is],
                                           is_symm,
                                           dphi_y_dm.get_device_ptr ());
                        phi_op.phi_mul_dm (dphi_z.get_device_ptr (),
                                           dm_gint_d_vec_[is].get_device_ptr (),
                                           dm_gint_vec_[is],
                                           is_symm,
                                           dphi_z_dm.get_device_ptr ());
                        phi_op.phi_dot_phi (dphi_x.get_device_ptr (),
                                            dphi_x_dm.get_device_ptr (),
                                            kin_d_vec_[is].get_device_ptr ());
                        phi_op.phi_dot_phi (dphi_y.get_device_ptr (),
                                            dphi_y_dm.get_device_ptr (),
                                            kin_d_vec_[is].get_device_ptr ());
                        phi_op.phi_dot_phi (dphi_z.get_device_ptr (),
                                            dphi_z_dm.get_device_ptr (),
                                            kin_d_vec_[is].get_device_ptr ());
                    }
            }
