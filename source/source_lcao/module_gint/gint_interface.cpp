...
void
    cal_gint_vl (const double* vr_eff, HContainer<double>* hR)
{
#ifdef __CUDA
...
// nspin == 4 case
void
    cal_gint_vl (std::vector<const double*> vr_eff, HContainer<std::complex<double>>* hR)
{
#ifdef __CUDA
    if (PARAM.inp.device == "gpu")
        {
...
void
    cal_gint_vl_metagga (const double* vr_eff, const double* vfork, HContainer<double>* hR)
{
#ifdef __CUDA
    if (PARAM.inp.device == "gpu")
        {
...
// nspin == 4 case
void
    cal_gint_vl_metagga (std::vector<const double*> vr_eff,
...
void
    cal_gint_rho (const std::vector<HContainer<double>*>& dm_vec, const int nspin, double** rho, bool is_dm_symm)
{
#ifdef __CUDA
    if (PARAM.inp.device == "gpu")
        {
...
void
    cal_gint_tau (const std::vector<HContainer<double>*>& dm_vec, const int nspin, double** tau)
{
#ifdef __CUDA
    if (PARAM.inp.device == "gpu")
        {
...
void
    cal_gint_fvl_meta (const int nspin,
                       const std::vector<const double*>& vr_eff,
...
void
    cal_dvlocal_R_sparseMatrix (const int nspin,
                                const int npol,
                                ...
                                LCAO_HS_Arrays& hs_arrays)
{
    ...
    gint_dvlocal.cal_dvlocal_R_sparseMatrix (nspin, current_spin, nlocal, sparse_thr, pv, ucell, gdriver, hs_arrays);
}
