...
void
    Gint_vl_metagga_nspin4_gpu::cal_gint ()
{
...
void
    Gint_vl_metagga_nspin4_gpu::init_hr_gint_ ()
{
...
void
    Gint_vl_metagga_nspin4_gpu::transfer_cpu_to_gpu_ ()
{
    ...
    for (int i = 0; i < nspin_; i++)
        {
            ...
            CHECK_CUDA (cudaMemcpy (vr_eff_d_[i].get_device_ptr (),
                                    vr_eff_[i],
                                    gint_info_->get_local_mgrid_num () * sizeof (double),
                                    cudaMemcpyHostToDevice));
            CHECK_CUDA (cudaMemcpy (vofk_d_[i].get_device_ptr (),
                                    vofk_[i],
                                    gint_info_->get_local_mgrid_num () * sizeof (double),
                                    cudaMemcpyHostToDevice));
        }
...
void
    Gint_vl_metagga_nspin4_gpu::transfer_gpu_to_cpu_ ()
{
    for (int i = 0; i < nspin_; i++)
        {
            CHECK_CUDA (cudaMemcpy (hr_gint_part_[i].get_wrapper (),
                                    hr_gint_part_d_[i].get_device_ptr (),
                                    hr_gint_part_[i].get_nnr () * sizeof (double),
                                    cudaMemcpyDeviceToHost));
        }
}

void
    Gint_vl_metagga_nspin4_gpu::cal_hr_gint_ ()
{
        ...
        for (int i = 0; i < gint_info_->get_bgrid_batches_num (); ++i)
            {
                ...
                for (int is = 0; is < nspin_; is++)
                    {
                        phi_op.phi_mul_vldr3 (vr_eff_d_[is].get_device_ptr (),
                                              dr3_,
                                              phi.get_device_ptr (),
                                              phi_vldr3.get_device_ptr ());
                        phi_op.phi_mul_vldr3 (vofk_d_[is].get_device_ptr (),
                                              dr3_,
                                              dphi_x.get_device_ptr (),
                        ...
                        phi_op.phi_mul_phi (phi.get_device_ptr (),
                                            phi_vldr3.get_device_ptr (),
                                            hr_gint_part_[is],
                                            hr_gint_part_d_[is].get_device_ptr ());
                        phi_op.phi_mul_phi (dphi_x.get_device_ptr (),
                                            dphi_x_vldr3.get_device_ptr (),
                                            hr_gint_part_[is],
                                            hr_gint_part_d_[is].get_device_ptr ());
                        phi_op.phi_mul_phi (dphi_y.get_device_ptr (),
                                            dphi_y_vldr3.get_device_ptr (),
                                            hr_gint_part_[is],
                                            hr_gint_part_d_[is].get_device_ptr ());
                        phi_op.phi_mul_phi (dphi_z.get_device_ptr (),
                                            dphi_z_vldr3.get_device_ptr (),
                                            hr_gint_part_[is],
                                            hr_gint_part_d_[is].get_device_ptr ());
                    }
