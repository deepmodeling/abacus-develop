...
void
    Gint_dvlocal::cal_dvlocal_R_sparseMatrix (const int nspin,
                                              const int cspin,
                                              const int nlocal,
...
void
    Gint_dvlocal::distribute_pvdpR_sparseMatrix (
        const int cspin,
        const int dim,
        const int nlocal,
                                                            ...
                                                            if (dim == 0)
                                                                {
                                                                    double& value
                                                                        = hs_arrays
                                                                              .dHRx_sparse[cspin][R_coor][row][col];
                                                                    value += tmp[col];
                                                                    if (std::abs (value) <= sparse_threshold)
                                                                        {
                                                                            hs_arrays.dHRx_sparse[cspin][R_coor][row]
                                                                                .erase (col);
                                                                        }
                                                                }
                                                            if (dim == 1)
                                                                {
                                                                    double& value
                                                                        = hs_arrays
                                                                              .dHRy_sparse[cspin][R_coor][row][col];
                                                                    value += tmp[col];
                                                                    if (std::abs (value) <= sparse_threshold)
                                                                        {
                                                                            hs_arrays.dHRy_sparse[cspin][R_coor][row]
                                                                                .erase (col);
                                                                        }
                                                                }
                                                            if (dim == 2)
                                                                {
                                                                    double& value
                                                                        = hs_arrays
                                                                              .dHRz_sparse[cspin][R_coor][row][col];
                                                                    value += tmp[col];
                                                                    if (std::abs (value) <= sparse_threshold)
                                                                        {
                                                                            hs_arrays.dHRz_sparse[cspin][R_coor][row]
                                                                                .erase (col);
                                                                        }
                                                                }
                                                        }
