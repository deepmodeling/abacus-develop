...
void
    Gint_vl_metagga_gpu::init_hr_gint_ ()
{
    hr_gint_ = gint_info_->get_hr<double> ();
}

void
    Gint_vl_metagga_gpu::transfer_cpu_to_gpu_ ()
{
    ...
    CHECK_CUDA (cudaMemcpy (vr_eff_d_.get_device_ptr (),
                            vr_eff_,
                            gint_info_->get_local_mgrid_num () * sizeof (double),
                            cudaMemcpyHostToDevice));
    CHECK_CUDA (cudaMemcpy (vofk_d_.get_device_ptr (),
                            vofk_,
                            gint_info_->get_local_mgrid_num () * sizeof (double),
                            cudaMemcpyHostToDevice));
}
...
void
    Gint_vl_metagga_gpu::transfer_gpu_to_cpu_ ()
{
    CHECK_CUDA (cudaMemcpy (hr_gint_.get_wrapper (),
                            hr_gint_d_.get_device_ptr (),
                            hr_gint_.get_nnr () * sizeof (double),
                            cudaMemcpyDeviceToHost));
}

void
    Gint_vl_metagga_gpu::cal_hr_gint_ ()
{
        ...
        for (int i = 0; i < gint_info_->get_bgrid_batches_num (); ++i)
            {
                ...
                phi_op.phi_mul_vldr3 (vr_eff_d_.get_device_ptr (),
                                      dr3_,
                                      phi.get_device_ptr (),
                                      phi_vldr3.get_device_ptr ());
                phi_op.phi_mul_vldr3 (vofk_d_.get_device_ptr (),
                                      dr3_,
                                      dphi_x.get_device_ptr (),
