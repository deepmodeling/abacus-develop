...
void
    PhiOperator::set_bgrid (std::shared_ptr<const BigGrid> biggrid)
{
    biggrid_ = biggrid;
            ...
            for (int j = 0; j < rows_; ++j)
                {
                    is_atom_on_mgrid_[i * rows_ + j]
                        = atoms_relative_coords_[i][j].norm () <= biggrid_->get_atom (i)->get_rcut ();
                }
        }
...
void
    PhiOperator::set_phi_dphi (double* phi, double* dphi_x, double* dphi_y, double* dphi_z) const
{
...
void
    PhiOperator::set_ddphi (double* ddphi_xx,
                            double* ddphi_xy,
                            double* ddphi_xz,
                            double* ddphi_yy,
                            double* ddphi_yz,
                            double* ddphi_zz) const
{
    for (int i = 0; i < biggrid_->get_atoms_num (); ++i)
        {
            const auto atom = biggrid_->get_atom (i);
            atom->set_ddphi (atoms_relative_coords_[i],
                             cols_,
                             ddphi_xx,
                             ddphi_xy,
                             ddphi_xz,
...
void
    PhiOperator::phi_dot_dphi (const double* phi,
                               const double* dphi_x,
                               const double* dphi_y,
                               const double* dphi_z,
...
void
    PhiOperator::phi_dot_dphi_r (const double* phi,
                                 const double* dphi_x,
                                 const double* dphi_y,
                                 const double* dphi_z,
...
void
    PhiOperator::cal_env_gamma (const double* phi, const double* wfc, const vector<int>& trace_lo, double* rho) const
{
    for (int i = 0; i < biggrid_->get_atoms_num (); ++i)
        {
            const auto atom = biggrid_->get_atom (i);
...
void
    PhiOperator::cal_env_k (const double* phi,
                    ...
                    if (is_atom_on_mgrid (i, j))
                        {
                            ...
                            if (nspin == 4) // is it a simple add of 2 spins?
                                    ...
                                    for (int is = 0; is < 2; ++is)
                                        {
                                            ...
                                            for (int iw = 0; iw < atom->get_nw (); ++iw, ++iw_lo)
                                                {
                                                    tmp += std::complex<double> (phi[phi_start_idx + iw], 0.0)
                                                           * wfc[iw_lo] * kphase;
                                                }
                                        }
                                }
                            else
                                {
                                    ...
                                    for (int iw = 0; iw < atom->get_nw (); ++iw, ++iw_lo)
                                        {
                                            tmp += std::complex<double> (phi[phi_start_idx + iw], 0.0) * wfc[iw_lo]
                                                   * kphase;
                                        }
                                }
...
//===============================
void
    PhiOperator::init_atom_pair_start_end_idx_ ()
{
