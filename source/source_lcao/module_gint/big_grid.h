...
class BigGrid
{
  public:
    ...
    static void
        init_localcell_info (std::shared_ptr<const LocalCellInfo> localcell_info)
    {
        localcell_info_ = localcell_info;
    }
    static void
        init_unitcell_info (std::shared_ptr<const UnitCellInfo> unitcell_info)
    {
        unitcell_info_ = unitcell_info;
    }
    static void
        init_bgrid_info (std::shared_ptr<const BigGridInfo> biggrid_info)
    {
        biggrid_info_ = biggrid_info;
    }
    ...
    static std::shared_ptr<const LocalCellInfo>
        get_localcell_info ()
    {
        return localcell_info_;
    }
    static std::shared_ptr<const UnitCellInfo>
        get_unitcell_info ()
    {
        return unitcell_info_;
    }
    static std::shared_ptr<const BigGridInfo>
        get_bgrid_info ()
    {
        return biggrid_info_;
    }
    const std::vector<const GintAtom*>&
        get_atoms () const
    {
        return atoms_;
    }
    const GintAtom*
        get_atom (int i) const
    {
        return atoms_[i];
    }
    ...
    {
        return biggrid_info_->get_mgrids_num ();
    }
     ...
     */
    void set_atom_relative_coords (const Vec3i bgrid_idx,
                                   const Vec3d tau_in_bgrid,
                                   std::vector<Vec3d>& atom_coord) const;
    ...
    // note when we say an atom affects a big grid, it does not mean that the atom affects all the meshgrid on the big
    // grid, it may only affect a part of them.
