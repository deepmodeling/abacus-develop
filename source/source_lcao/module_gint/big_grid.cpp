...
void
    BigGrid::add_atom (const GintAtom* atom)
{
    atoms_.push_back (atom);
}

int
    BigGrid::get_phi_len () const
{
    int len = 0;
...
void
    BigGrid::set_atoms_startidx (std::vector<int>& startidx) const
{
...
void
    BigGrid::set_atoms_phi_len (std::vector<int>& phi_len) const
{
...
void
    BigGrid::set_mgrids_coord (std::vector<Vec3d>& coord) const
{
...
void
    BigGrid::set_mgrids_local_idx (std::vector<int>& mgrids_idx) const
{
    auto index_3d = localcell_info_->bgrid_idx_1Dto3D (idx_);
    Vec3i startidx (index_3d.x * biggrid_info_->get_nmx (),
...
void
    BigGrid::set_atom_relative_coords (const GintAtom* atom, std::vector<Vec3d>& atom_coord) const
{
    set_atom_relative_coords (atom->get_bgrid_idx (), atom->get_tau_in_bgrid (), atom_coord);
}

void
    BigGrid::set_atom_relative_coords (const Vec3i bgrid_idx,
                                       ...
                                       std::vector<Vec3d>& atom_coord) const
{
    ...
    Vec3d bgrid_relative_coord = unitcell_info_->get_relative_coord (bgrid_idx, this_bgrid_idx) + tau_in_bgrid;
...
Vec3d
    BigGrid::get_bgrid_atom_rcoord (const GintAtom* atom) const
{
    Vec3i this_bgrid_idx = localcell_info_->get_bgrid_global_idx_3D (idx_);
...
bool
    BigGrid::is_atom_on_bgrid (const GintAtom* atom) const
{
