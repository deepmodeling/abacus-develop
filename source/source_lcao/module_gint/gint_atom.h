...
class GintAtom
{
  public:
    ...
    int
        get_start_iw () const
    {
        return ucell_->itiaiw2iwt (it_, ia_, 0);
    } // get the start index of global atomic orbitals
    const Vec3i&
        get_bgrid_idx () const
    {
        return biggrid_idx_;
    ...
    const Vec3i&
        get_unitcell_idx () const
    {
        return unitcell_idx_;
    }
    const Vec3i&
        get_R () const
    {
    ...
    const Vec3d&
        get_tau_in_bgrid () const
    {
        return tau_in_biggrid_;
    }
     ...
     */
    template <typename T>
    void set_phi_dphi (const std::vector<Vec3d>& coords, const int stride, T* phi, T* dphi_x, T* dphi_y, T* dphi_z)
        const;
     ...
     */
    template <typename T>
    void set_ddphi (const std::vector<Vec3d>& coords,
                    const int stride,
                    T* ddphi_xx,
                    T* ddphi_xy,
                    T* ddphi_xz,
