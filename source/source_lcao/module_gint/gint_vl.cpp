...
inline const double*
    get_vr_eff_data_ (const double* vr_eff, int /*local_mgrid_num*/, std::vector<double>& /*vr_eff_buffer*/)
{
    return vr_eff;
}
...
template <typename Real>
const Real*
    get_vr_eff_data_ (const double* vr_eff, int local_mgrid_num, std::vector<Real>& vr_eff_buffer)
{
    vr_eff_buffer.resize (local_mgrid_num);
    std::transform (vr_eff,
                    vr_eff + local_mgrid_num,
                    vr_eff_buffer.begin (),
...
inline void
    finalize_hr_gint_ (HContainer<double>& hr_gint, HContainer<double>* hR)
{
...
template <typename Real>
void
    finalize_hr_gint_ (HContainer<Real>& hr_gint, HContainer<double>* hR)
{
...
template <typename Real>
void
    Gint_vl::cal_gint_impl_ ()
{
    HContainer<Real> hr_gint = init_hr_gint_<Real> ();
    std::vector<Real> vr_eff_buffer;
    const Real* vr_eff = get_vr_eff_data_ (vr_eff_, gint_info_->get_local_mgrid_num (), vr_eff_buffer);
