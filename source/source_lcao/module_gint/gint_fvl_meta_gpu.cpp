...
void
    Gint_fvl_meta_gpu::init_dm_gint_ ()
{
...
void
    Gint_fvl_meta_gpu::transfer_cpu_to_gpu_ ()
{
    ...
    for (int is = 0; is < nspin_; is++)
        {
            ...
            CHECK_CUDA (cudaMemcpy (dm_gint_d_vec_[is].get_device_ptr (),
                                    dm_gint_vec_[is].get_wrapper (),
                                    dm_gint_vec_[is].get_nnr () * sizeof (double),
                                    cudaMemcpyHostToDevice));
            vr_eff_d_vec_[is] = CudaMemWrapper<double> (gint_info_->get_local_mgrid_num (), 0, false);
            CHECK_CUDA (cudaMemcpy (vr_eff_d_vec_[is].get_device_ptr (),
                                    vr_eff_[is],
                                    gint_info_->get_local_mgrid_num () * sizeof (double),
                                    cudaMemcpyHostToDevice));
            vofk_d_vec_[is] = CudaMemWrapper<double> (gint_info_->get_local_mgrid_num (), 0, false);
            CHECK_CUDA (cudaMemcpy (vofk_d_vec_[is].get_device_ptr (),
                                    vofk_[is],
                                    gint_info_->get_local_mgrid_num () * sizeof (double),
                                    cudaMemcpyHostToDevice));
        }
...
void
    Gint_fvl_meta_gpu::transfer_gpu_to_cpu_ ()
{
    if (isforce_)
        {
...
void
    Gint_fvl_meta_gpu::cal_fvl_svl_ ()
{
        ...
        for (int i = 0; i < gint_info_->get_bgrid_batches_num (); ++i)
            {
                ...
                phi_op.set_ddphi (ddphi_xx.get_device_ptr (),
                                  ddphi_xy.get_device_ptr (),
                                  ddphi_xz.get_device_ptr (),
                                  ddphi_yy.get_device_ptr (),
                                  ddphi_yz.get_device_ptr (),
                                  ddphi_zz.get_device_ptr ());
                for (int is = 0; is < nspin_; is++)
                    {
                        const bool is_symm = false;
                        phi_op.phi_mul_vldr3 (vr_eff_d_vec_[is].get_device_ptr (),
                                              dr3_,
                                              phi.get_device_ptr (),
                                              phi_vldr3.get_device_ptr ());
                        phi_op.phi_mul_vldr3 (vofk_d_vec_[is].get_device_ptr (),
                                              dr3_,
                                              dphi_x.get_device_ptr (),
                                              dphi_x_vldr3.get_device_ptr ());
                        phi_op.phi_mul_vldr3 (vofk_d_vec_[is].get_device_ptr (),
                                              dr3_,
                                              dphi_y.get_device_ptr (),
                                              dphi_y_vldr3.get_device_ptr ());
                        phi_op.phi_mul_vldr3 (vofk_d_vec_[is].get_device_ptr (),
                                              dr3_,
                                              dphi_z.get_device_ptr (),
                                              dphi_z_vldr3.get_device_ptr ());
                        phi_op.phi_mul_dm (phi_vldr3.get_device_ptr (),
                                           dm_gint_d_vec_[is].get_device_ptr (),
                                           dm_gint_vec_[is],
                                           is_symm,
                                           phi_vldr3_dm.get_device_ptr ());
                        phi_op.phi_mul_dm (dphi_x_vldr3.get_device_ptr (),
                                           dm_gint_d_vec_[is].get_device_ptr (),
                                           dm_gint_vec_[is],
                                           is_symm,
                                           dphi_x_vldr3_dm.get_device_ptr ());
                        phi_op.phi_mul_dm (dphi_y_vldr3.get_device_ptr (),
                                           dm_gint_d_vec_[is].get_device_ptr (),
                                           dm_gint_vec_[is],
                                           is_symm,
                                           dphi_y_vldr3_dm.get_device_ptr ());
                        phi_op.phi_mul_dm (dphi_z_vldr3.get_device_ptr (),
                                           dm_gint_d_vec_[is].get_device_ptr (),
                                           dm_gint_vec_[is],
                                           is_symm,
                                           dphi_z_vldr3_dm.get_device_ptr ());
                        if (isforce_)
                            {
