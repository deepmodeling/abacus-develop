 ...
 *
 * In fact, the variables and functions of this class could be placed in the BigGrid class, but the lifecycle of the
 * BigGrid class is relatively long. We do not want the BigGrid to contain too many member variables, as this could lead
 * to excessive memory usage. Therefore, we separate this class out, so it can be destroyed after use.
 */
class PhiOperator
{
  public:
    enum class Triangular_Matrix
    {
        Upper,
        Lower,
        Full
    ...
    void set_ddphi (double* ddphi_xx,
                    double* ddphi_xy,
                    double* ddphi_xz,
                    double* ddphi_yy,
                    double* ddphi_yz,
    ...
    template <typename T>
    void phi_mul_dm (const T* const phi,      // phi(ir,iwt)
    ...
    template <typename T>
    void phi_mul_vldr3 (const T* const vl, // vl(ir)
                        const T dr3,
    ...
    template <typename T>
    void phi_mul_phi (const T* const phi_i, // phi_i(ir,iwt)
    ...
    void phi_dot_phi (const Tin* const phi_i, // phi_i(ir,iwt)
                      const Tin* const phi_j, // phi_j(ir,iwt)
                      Tout* const rho) const; // rho(ir)

    void phi_dot_dphi (const double* phi,
                       const double* dphi_x,
                       const double* dphi_y,
                       const double* dphi_z,
                       ModuleBase::matrix* fvl) const;

    void phi_dot_dphi_r (const double* phi,
                         const double* dphi_x,
                         const double* dphi_y,
                         const double* dphi_z,
    ...
    void cal_env_k (const double* phi,
  ...
  private:
    ...
    // Note that atom_pair_start_end_idx_ only stores the cases where a <= b, so this function is needed to retrieve the
    // value
    const std::pair<int, int>&
        get_atom_pair_start_end_idx_ (int a, int b) const
    {
        int x = std::min (a, b);
        int y = std::abs (a - b);
    ...
    bool
        is_atom_on_mgrid (int atom_idx, int mgrid_idx) const
    {
