...
void
    cast_hcontainer_values (const HContainer<Tin>& src, HContainer<Tout>& dst)
{
    if (src.get_ijr_info () != dst.get_ijr_info () || src.is_gamma_only () != dst.is_gamma_only ()
        || src.get_nnr () != dst.get_nnr ())
        {
            ModuleBase::WARNING_QUIT ("cast_hcontainer_values",
                                      "Source and destination HContainer shapes do not match.");
        }
...
template <typename Tout, typename Tin>
HContainer<Tout>
    make_cast_hcontainer (const HContainer<Tin>& src)
{
...
template <typename T>
void
    compose_hr_gint (HContainer<T>& hr_gint)
{
...
template <typename T>
void
    transfer_hr_gint_to_hR (const HContainer<T>& hr_gint, HContainer<T>& hR)
{
...
void
    merge_hr_part_to_hR (const std::vector<hamilt::HContainer<double>>& hr_gint_tmp,
    ...
    for (int is = 0; is < 4; is++)
        {
            ...
            hR_tmp->set_zero ();
            hamilt::HContainer<std::complex<double>>* hRGint_tmpCd
                                            ...
                                            for (int icol = 0; icol < mat_nspin1->get_col_size (); ++icol)
                                                {
                                                    upper_mat->get_value (irow, icol)
                                                        = mat_nspin1->get_value (irow, icol)
                                                          + std::complex<double> (clx_i[is], clx_j[is])
                                                                * mat_nspin2->get_value (irow, icol);
                                                }
                                        }
...
typename std::enable_if<std::is_same<TGint, TDM>::value>::type
    gather_dm (const HContainer<TDM>& dm_src, HContainer<TGint>& dm_dst, const GintInfo& /*gint_info*/)
{
#ifdef __MPI
...
template <typename TGint, typename TDM>
void
    transfer_dm_2d_to_gint (const GintInfo& gint_info,
                                            ...
                                            for (int icol = 0; icol < ap.get_col_size () / 2; icol++)
                                                {
                                                    ...
                                                    int index_j = (irow * 2 + row_set[is]) * ap.get_col_size ()
                                                                  + icol * 2 + col_set[is];
...
int
    globalIndex (int localindex, int nblk, int nprocs, int myproc)
{
...
int
    localIndex (int globalindex, int nblk, int nprocs, int& myproc)
{
...
template void transfer_hr_gint_to_hR (const HContainer<std::complex<double>>& hr_gint,
...
template void transfer_dm_2d_to_gint (const GintInfo& gint_info,
...
template void wfc_2d_to_gint (const double* wfc_2d,
                              int nbands,
                              int nlocal,
                              const Parallel_Orbitals& pv,
                              double* wfc_grid,
                              const GintInfo& gint_info);
template void wfc_2d_to_gint (const std::complex<double>* wfc_2d,
                              int nbands,
                              int nlocal,
                              const Parallel_Orbitals& pv,
