    ...
    for (int is = 0; is < nspin_; is++)
        {
            ...
            CHECK_CUDA (cudaMemcpy (dm_gint_d_vec[is].get_device_ptr (),
                                    dm_gint_vec[is].get_wrapper (),
                                    dm_gint_vec[is].get_nnr () * sizeof (Real),
        ...
        for (int i = 0; i < gint_info_->get_bgrid_batches_num (); ++i)
            {
                ...
                for (int is = 0; is < nspin_; is++)
                    {
                        phi_op.phi_mul_dm (phi.get_device_ptr (),
                                           dm_gint_d_vec[is].get_device_ptr (),
                                           dm_gint_vec[is],
                                           is_dm_symm_,
                                           phi_dm.get_device_ptr ());
                        phi_op.phi_dot_phi (phi.get_device_ptr (),
                                            phi_dm.get_device_ptr (),
                                            rho_d_vec[is].get_device_ptr ());
                    }
            }
    ...
    for (int is = 0; is < nspin_; is++)
        {
            ...
            CHECK_CUDA (cudaMemcpy (rho_tmp.data (),
                                    rho_d_vec[is].get_device_ptr (),
