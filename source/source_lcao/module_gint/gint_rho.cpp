...
template <typename Real>
std::vector<HContainer<Real>>
    Gint_rho::init_dm_gint_ () const
{
...
inline double*
    get_rho_data (double* const* rho, int is, int /*local_mgrid_num*/, std::vector<std::vector<double>>& /*rho_cache*/)
{
    return rho[is];
}
...
template <typename Real>
Real*
    get_rho_data (double* const* rho, int is, int local_mgrid_num, std::vector<std::vector<Real>>& rho_cache)
{
    rho_cache[is].resize (local_mgrid_num);
    std::transform (rho[is],
                    rho[is] + local_mgrid_num,
                    rho_cache[is].begin (),
                    [] (const double value) { return static_cast<Real> (value); });
    return rho_cache[is].data ();
}

inline void
    transfer_rho_back (double* const* /*rho*/,
                       int /*nspin*/,
                       int /*local_mgrid_num*/,
...
template <typename Real>
Real*
    Gint_rho::get_rho_data_ (int is, std::vector<std::vector<Real>>& rho_cache) const
{
    return get_rho_data (rho_, is, gint_info_->get_local_mgrid_num (), rho_cache);
}

template <typename Real>
void
    Gint_rho::cal_rho_ (const std::vector<HContainer<Real>>& dm_gint_vec, const std::vector<Real*>& rho_data) const
{
#pragma omp parallel
    {
        PhiOperator phi_op;
...
template <typename Real>
void
    Gint_rho::transfer_rho_cache_ (const std::vector<std::vector<Real>>& rho_cache) const
{
    transfer_rho_back (rho_, nspin_, gint_info_->get_local_mgrid_num (), rho_cache);
}
