...
template <typename Real>
Real* get_rho_data(double* const* rho, int is, int local_mgrid_num, std::vector<std::vector<Real>>& rho_cache) {
...
inline void transfer_rho_back(double* const* /*rho*/,
                              int /*nspin*/,
                              int /*local_mgrid_num*/,
...
template <typename Real>
void Gint_rho::cal_rho_(const std::vector<HContainer<Real>>& dm_gint_vec, const std::vector<Real*>& rho_data) const {
#pragma omp parallel
    {
        PhiOperator phi_op;
        std::vector<Real> phi;
