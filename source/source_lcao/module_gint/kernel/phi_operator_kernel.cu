...
__global__ void set_phi_kernel(const int nwmax,
    ...
    for (int atom_id = threadIdx.x; atom_id < atoms_num; atom_id += blockDim.x) {
        ...
        const double3 rcoord =
            atoms_bgrids_rcoords[atom_id + pre_atoms_num]; // rcoord is the ralative coordinate of an atom and a biggrid
        const double3 coord =
            make_double3(mgrid_pos.x - rcoord.x, // coord is the relative coordinate of an atom and a meshgrid
        ...
        if (dist < rcut[atom_type]) {
            ...
            int phi_idx = atoms_phi_start[atom_id + pre_atoms_num] + bgrids_phi_len[bgrid_id] * mgrid_id;

            for (int iw = 0; iw < atom_nw[atom_type]; iw++, iw_nr += nrmax) {
                if (atom_iw2_new[it_nw + iw]) {
                    psi = c1 * psi_u[iw_nr] + c2 * dpsi_u[iw_nr] + c3 * psi_u[iw_nr + 1] + c4 * dpsi_u[iw_nr + 1];
                }
...
template __global__ void set_phi_kernel<double>(const int,
                                                const int,
                                                const int,
                                                const double,
                                                const int*,
                                                ...
                                                const int*,
                                                double*);
template __global__ void set_phi_kernel<float>(const int,
                                               const int,
                                               const int,
                                               const double,
                                               const int*,
...
__global__ void set_phi_dphi_kernel(const int nwmax,
                                    const int mgrids_num,
                                    const int nrmax,
                                    const double dr_uniform,
                                    const int* __restrict__ ucell_atom_nwl,
    ...
    for (int atom_id = threadIdx.x; atom_id < atoms_num; atom_id += blockDim.x) {
        ...
        const double3 coord = make_double3(mgrid_pos.x - rcoord.x, mgrid_pos.y - rcoord.y, mgrid_pos.z - rcoord.z);
        double dist = norm3d(coord.x, coord.y, coord.z);
        if (dist < rcut[atom_type]) {
            if (dist < 1.0E-9) {
                dist += 1.0E-9;
...
__global__ void set_ddphi_kernel(const int nwmax,
                                 const int mgrids_num,
                                 const int nrmax,
                                 const double dr_uniform,
                                 const int* __restrict__ ucell_atom_nwl,
    ...
    for (int atom_id = threadIdx.x; atom_id < atoms_num; atom_id += blockDim.x) {
        const int atom_type = iat2it[atoms_iat[atom_id + pre_atoms_num]];
        const double3 rcoord = atoms_bgrids_rcoords[atom_id + pre_atoms_num];
        double coord[3]{mgrid_pos.x - rcoord.x, mgrid_pos.y - rcoord.y, mgrid_pos.z - rcoord.z};
...
template <typename Real>
__global__ void phi_mul_vldr3_kernel(const Real* __restrict__ vl,
                                     const Real dr3,
...
template __global__ void phi_mul_vldr3_kernel<double>(const double*,
                                                      const double,
                                                      const double*,
                                                      const int,
                                                      const int*,
                                                      ...
                                                      const int*,
                                                      double*);
template __global__ void phi_mul_vldr3_kernel<float>(const float*,
                                                     const float,
                                                     const float*,
                                                     const int,
                                                     const int*,
...
template <typename Real>
__global__ void phi_dot_phi_kernel(const Real* __restrict__ phi_i,
...
phi_dot_phi_kernel<double>(const double*, const double*, const int, const int*, const int*, const int*, double*);
template __global__ void
phi_dot_phi_kernel<float>(const float*, const float*, const int, const int*, const int*, const int*, float*);

__global__ void phi_dot_dphi_kernel(const double* __restrict__ phi,
...
__global__ void phi_dot_dphi_r_kernel(const double* __restrict__ phi,
                                      const double* __restrict__ dphi_x,
                                      const double* __restrict__ dphi_y,
                                      const double* __restrict__ dphi_z,
                                      const int mgrids_per_bgrid,
    ...
    for (int mgrid_id = blockIdx.x; mgrid_id < mgrids_per_bgrid; mgrid_id += gridDim.x) {
        ...
        for (int atom_id = 0; atom_id < atoms_num; atom_id++) {
            ...
            const int nw = atom_nw[iat2it[iat]];
            const double3 rcoord =
                atoms_bgrids_rcoords[atom_id +
                                     pre_atoms_num]; // rcoord is the ralative coordinate of an atom and a biggrid
            const double3 coord =
                make_double3(mgrid_pos.x - rcoord.x, // coord is the relative coordinate of an atom and a meshgrid
                             mgrid_pos.y - rcoord.y,
                             mgrid_pos.z - rcoord.z);
