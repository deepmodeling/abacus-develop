...
static __global__ void
    vbatched_gemm_nt_kernel (const int* M,
                             const int* N,
                             const int* K,
...
template <typename T,
          ...
          int DIM_YB>
void
    vbatched_gemm_tn_impl (int max_m,
                           int max_n,
                           const int* m,
                           const int* n,
                           const int* k,
    ...
    for (int i = 0; i < batchCount; i += max_batch_count)
        {
            ...
            vbatched_gemm_nt_kernel<T, DIM_X, DIM_Y, BLK_M, BLK_N, BLK_K, DIM_XA, DIM_YA, DIM_XB, DIM_YB>
