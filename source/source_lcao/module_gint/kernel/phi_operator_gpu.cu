...
PhiOperatorGpu<Real>::PhiOperatorGpu (std::shared_ptr<const GintGpuVars> gint_gpu_vars, cudaStream_t stream)
    : gint_gpu_vars_ (gint_gpu_vars), stream_ (stream),
      ...
      mgrids_local_idx_batch_ (BatchBigGrid::get_max_batch_size () * BatchBigGrid::get_bgrid_info ()->get_mgrids_num (),
                               stream_,
                               true),
...
template <typename Real>
void
    PhiOperatorGpu<Real>::set_bgrid_batch (std::shared_ptr<BatchBigGrid> bgrid_batch)
{
    ...
    for (const auto& bgrid: bgrid_batch->get_bgrids ())
        {
            ...
            std::copy (mgrids_local_idx.begin (), mgrids_local_idx.end (), mgrids_local_idx_batch_h + i * mgrids_num_);
            int phi_len_bgrid = 0;
...
template <typename Real>
void
    PhiOperatorGpu<Real>::set_phi (Real* phi_d) const
{
    dim3 grid_dim (mgrids_num_, bgrid_batch_->get_batch_size ());
    dim3 threads_per_block (64);
    set_phi_kernel<Real><<<grid_dim, threads_per_block, 0, stream_>>> (gint_gpu_vars_->nwmax,
                                                                       mgrids_num_,
...
template <typename Real>
void
    PhiOperatorGpu<Real>::set_phi_dphi (double* phi_d, double* dphi_x_d, double* dphi_y_d, double* dphi_z_d) const
{
    dim3 grid_dim (mgrids_num_, bgrid_batch_->get_batch_size ());
    dim3 threads_per_block (64);
    set_phi_dphi_kernel<<<grid_dim, threads_per_block, 0, stream_>>> (gint_gpu_vars_->nwmax,
                                                                      mgrids_num_,
                                                                      gint_gpu_vars_->nr_max,
                                                                      gint_gpu_vars_->dr_uniform,
                                                                      gint_gpu_vars_->ucell_atom_nwl_d,
...
template <typename Real>
void
    PhiOperatorGpu<Real>::set_ddphi (double* ddphi_xx_d,
                                     double* ddphi_xy_d,
                                     double* ddphi_xz_d,
                                     double* ddphi_yy_d,
                                     double* ddphi_yz_d,
                                     double* ddphi_zz_d) const
{
    ...
    dim3 grid_dim (mgrids_num_, bgrid_batch_->get_batch_size ());
    dim3 threads_per_block (64);
    set_ddphi_kernel<<<grid_dim, threads_per_block, 0, stream_>>> (gint_gpu_vars_->nwmax,
                                                                   mgrids_num_,
                                                                   gint_gpu_vars_->nr_max,
                                                                   gint_gpu_vars_->dr_uniform,
                                                                   gint_gpu_vars_->ucell_atom_nwl_d,
...
template <typename Real>
void
    PhiOperatorGpu<Real>::phi_mul_vldr3 (const Real* vl_d, const Real dr3, const Real* phi_d, Real* result_d) const
{
    dim3 grid_dim (mgrids_num_, bgrid_batch_->get_batch_size ());
    dim3 threads_per_block (64);
    phi_mul_vldr3_kernel<Real><<<grid_dim, threads_per_block, 0, stream_>>> (vl_d,
                                                                             dr3,
                                                                             phi_d,
                                                                             mgrids_num_,
...
template <typename Real>
void
    PhiOperatorGpu<Real>::phi_mul_phi (const Real* phi_d,
...
template <typename Real>
void
    PhiOperatorGpu<Real>::phi_mul_dm (const Real* phi_d,
                                      const Real* dm_d,
...
template <typename Real>
void
    PhiOperatorGpu<Real>::phi_dot_phi (const Real* phi_i_d, const Real* phi_j_d, Real* rho_d) const
{
    dim3 grid_dim (mgrids_num_, bgrid_batch_->get_batch_size ());
    dim3 threads_per_block (64);
    phi_dot_phi_kernel<Real>
        <<<grid_dim, threads_per_block, sizeof (Real) * 32, stream_>>> (phi_i_d,
                                                                        phi_j_d,
                                                                        mgrids_num_,
                                                                        mgrids_local_idx_batch_.get_device_ptr (),
                                                                        bgrids_phi_len_.get_device_ptr (),
...
template <typename Real>
void
    PhiOperatorGpu<Real>::phi_dot_dphi (const double* phi_d,
                                        const double* dphi_x_d,
                                        const double* dphi_y_d,
                                        const double* dphi_z_d,
                                        double* fvl_d) const
{
    dim3 grid_dim (bgrid_batch_->get_max_atoms_num_per_bgrid (), bgrid_batch_->get_batch_size ());
...
template <typename Real>
void
    PhiOperatorGpu<Real>::phi_dot_dphi_r (const double* phi_d,
                                          const double* dphi_x_d,
                                          const double* dphi_y_d,
                                          const double* dphi_z_d,
                                          double* svl_d) const
