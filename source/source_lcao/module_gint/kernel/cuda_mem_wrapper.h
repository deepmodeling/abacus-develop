...
class CudaMemWrapper
{
  public:
    ...
    CudaMemWrapper&
        operator= (CudaMemWrapper&& other) noexcept
    {
        if (this != &other)
            {
                this->device_ptr_ = other.device_ptr_;
    ...
    void
        copy_host_to_device_sync (size_t size)
    {
    ...
    void
        copy_host_to_device_sync ()
    {
        copy_host_to_device_sync (size_);
    }

    void
        copy_host_to_device_async (size_t size)
    {
        if (host_ptr_ == nullptr)
            {
                ModuleBase::WARNING_QUIT ("cuda_mem_wrapper", "Host pointer is null, cannot copy to device.");
    ...
    void
        copy_device_to_host_sync (size_t size)
    {
        if (host_ptr_ == nullptr)
            {
    ...
    void
        copy_device_to_host_sync ()
    {
        copy_device_to_host_sync (size_);
    }

    void
        copy_device_to_host_async (size_t size)
    {
        if (host_ptr_ == nullptr)
            {
                ModuleBase::WARNING_QUIT ("cuda_mem_wrapper", "Host pointer is null, cannot copy to host.");
    ...
    void
        memset_device_sync (const size_t size, const int value = 0)
    {
        CHECK_CUDA (cudaMemset (device_ptr_, value, size * sizeof (T)));
    }

    void
        memset_device_sync (const int value = 0)
    {
        memset_device_sync (size_, value);
    }

    void
        memset_device_async (const size_t size, const int value = 0)
    {
        CHECK_CUDA (cudaMemsetAsync (device_ptr_, value, size * sizeof (T), stream_));
    }

    void
        memset_device_async (const int value = 0)
    {
        memset_device_async (size_, value);
    }

    void
        memset_host (const size_t size, const int value = 0)
    {
        if (host_ptr_ == nullptr)
            {
