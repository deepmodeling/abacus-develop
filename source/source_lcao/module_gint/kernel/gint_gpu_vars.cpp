...
// Just get the device_id from DeviceContext for use in destructor
    ...
    CHECK_CUDA (
        cudaMemcpy (atom_iw2_new_d, atom_iw2_new_h.data (), sizeof (bool) * ntype * nwmax, cudaMemcpyHostToDevice));
    CHECK_CUDA (cudaMalloc ((void**)&atom_iw2_ylm_d, sizeof (int) * ntype * nwmax));
    CHECK_CUDA (
        cudaMemcpy (atom_iw2_ylm_d, atom_iw2_ylm_h.data (), sizeof (int) * ntype * nwmax, cudaMemcpyHostToDevice));
    ...
    CHECK_CUDA (
        cudaMemcpy (psi_u_d, psi_u_h.data (), sizeof (double) * ntype * nwmax * nr_max, cudaMemcpyHostToDevice));
    CHECK_CUDA (cudaMalloc ((void**)&dpsi_u_d, sizeof (double) * ntype * nwmax * nr_max));
    CHECK_CUDA (
        cudaMemcpy (dpsi_u_d, dpsi_u_h.data (), sizeof (double) * ntype * nwmax * nr_max, cudaMemcpyHostToDevice));
    CHECK_CUDA (cudaMalloc ((void**)&d2psi_u_d, sizeof (double) * ntype * nwmax * nr_max));
    CHECK_CUDA (
        cudaMemcpy (d2psi_u_d, d2psi_u_h.data (), sizeof (double) * ntype * nwmax * nr_max, cudaMemcpyHostToDevice));
