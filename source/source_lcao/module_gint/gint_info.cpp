    ...
    : ucell_ (&ucell)
{
    ...
    unitcell_info_ = std::make_shared<UnitCellInfo> (ucell_->a1 * ucell_->lat0,
                                                     ucell_->a2 * ucell_->lat0,
                                                     ucell_->a3 * ucell_->lat0,
                                                     nbx,
                                                     nby,
                                                     nbz,
                                                     nmx,
    ...
    divide_info_ = std::make_shared<DivideInfo> (startidx_bx,
                                                 startidx_by,
                                                 startidx_bz,
                                                 nbx_local,
                                                 nby_local,
...
void
    GintInfo::init_atoms_ (int ntype, const Atom* atoms, const Numerical_Orbital* Phi)
{
            ...
            for (int j = 0; j < atom.na; j++)
                {
                    ...
                    for (int bgrid_x = atom_bgrid_idx.x - ext_bgrid.x; bgrid_x <= atom_bgrid_idx.x + ext_bgrid.x;
                         bgrid_x++)
                        {
                            for (int bgrid_y = atom_bgrid_idx.y - ext_bgrid.y;
                                 bgrid_y <= atom_bgrid_idx.y + ext_bgrid.y;
                                 bgrid_y++)
                                {
                                    for (int bgrid_z = atom_bgrid_idx.z - ext_bgrid.z;
                                         bgrid_z <= atom_bgrid_idx.z + ext_bgrid.z;
                                         bgrid_z++)
                                        {
                                            // get the extended biggrid idx of the affected biggrid
                                            const Vec3i ext_bgrid_idx (bgrid_x, bgrid_y, bgrid_z);
                                            const Vec3i normal_bgrid_idx
                                                = unitcell_info_->map_ext_idx_to_ucell (ext_bgrid_idx);
                                            if (localcell_info_->is_bgrid_in_lcell (normal_bgrid_idx) == false)
                                                {
                                                    continue;
                                                }
                                            const int bgrid_local_idx
                                                = localcell_info_->get_bgrid_local_idx_1D (normal_bgrid_idx);
                                            // get the unitcell idx of the big grid
                                            const Vec3i ucell_idx_bgrid
                                                = unitcell_info_->get_unitcell_idx (ext_bgrid_idx);

                                            // The index of the unitcell containing the biggrid relative to the unitcell
                                            // containing the atom.
                                            ...
                                            // it means this is the first time we find this atom may affect some
                                            // biggrids, add it to the r_to_atom map
                                            if (it == r_to_atom.end ())
                                                {
                                                    Vec3i ext_atom_bgrid_idx (
                                                        atom_bgrid_idx.x
                                                            - ucell_idx_bgrid.x * unitcell_info_->get_nbx (),
                                                        atom_bgrid_idx.y
                                                            - ucell_idx_bgrid.y * unitcell_info_->get_nby (),
                                                        atom_bgrid_idx.z
                                                            - ucell_idx_bgrid.z * unitcell_info_->get_nbz ());
                                            ...
                                            if (biggrids_[bgrid_local_idx]->is_atom_on_bgrid (
                                                    &r_to_atom.at (ucell_idx_relative)))
                                                {
                                                    biggrids_[bgrid_local_idx]->add_atom (
                                                        &r_to_atom.at (ucell_idx_relative));
...
void
    GintInfo::init_trace_lo_ (const UnitCell& ucell, const int nspin)
{
...
void
    GintInfo::init_ijr_info_ (const UnitCell& ucell, Grid_Driver& gd)
{
...
#ifdef __CUDA
void
    GintInfo::init_bgrid_batches_ (int batch_size)
{
