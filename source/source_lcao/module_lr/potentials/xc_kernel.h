...
class KernelXC {
  ...
  private:
    ...
    void f_xc_libxc(const int& nspin,
                    const double& omega,
                    const double& tpiba,
                    const double* const* const rho_gs,
                    const double* const rho_core = nullptr);
#endif
    ...
    // std::map<std::string, std::vector<ModuleBase::Vector3<double>>> grad_kernel_set_;// [kernel_type][nrxx][nspin],
    // intermediate terms for GGA for nspin=1, gga
    ...
    Tvec3
        v2rhosigma_drho_singlet_; ///< $2(f^{\rho_u\sigma_{uu}}+f^{\rho_u\sigma_{ud}}+f^{\rho_u\sigma_{dd}})*\nabla\rho)$
