...
class PotHxcLR
{
  public:
    ...
    /// S2_updown: K^Hartree + (K^xc_{upup}, K^xc_{updown},  K^xc_{downup} or K^xc_{downdown}), according to `ispin_op`
    ...
    /// XCType here is to determin the method of integration from kernel to potential, not the way calculating the
    /// kernel
    enum XCType
    {
        None = 0,
    ...
    PotHxcLR (const std::string& xc_kernel,
