...
template <typename T>
inline void
    add_op (const T* const src1, const T* const src2, T* const dst, const int size)
{
#ifdef _OPENMP
...
template <typename T>
inline void
    add_op (const std::vector<T>& src1, const std::vector<T>& src2, std::vector<T>& dst)
{
...
template <typename T>
inline void
    add_assign_op (const std::vector<T>& src, std::vector<T>& dst)
{
    add_op (src, dst, dst);
}
template <typename Telement, typename Tscalar>
inline void
    cutoff_grid_data_spin2 (std::vector<Telement>& func, const std::vector<Tscalar>& mask)
{
    ...
    for (int ir = 0; ir < nrxx; ++ir)
        {
            ...
            std::for_each (func.begin () + istart,
                           func.begin () + istart + n_component - 1,
                           [&] (Telement& f) { f *= mask[i2]; }); // spin-up
            std::for_each (func.begin () + istart + 1,
                           func.begin () + istart + n_component,
                           [&] (Telement& f) { f *= mask[i2 + 1]; }); // spin-down
        }
}

#ifdef USE_LIBXC
void
    LR::KernelXC::f_xc_libxc (const int& nspin,
                              const double& omega,
                              const double& tpiba,
...
#pragma omp parallel for collapse(2) schedule(static, 1024)
    ...
    // -----------------------------------------------------------------------------------
    // for GGA
    const bool is_gga = std::any_of (
        funcs.begin (),
        funcs.end (),
        [] (const xc_func_type& f) { return f.info->family == XC_FAMILY_GGA || f.info->family == XC_FAMILY_HYB_GGA; });
                ...
                case XC_FAMILY_HYB_GGA:
                    {
                        ...
                        // v2sigma2_tmp.end()) << std::endl; std::cout << "rho corresponding to max element of
                        // v2sigma2_tmp: " << rho[(std::max_element(v2sigma2_tmp.begin(), v2sigma2_tmp.end()) -
                        // v2sigma2_tmp.begin()) / 6] << std::endl; cut off by sgn
                            ...
                            for (int i = 0; i < nrxx; ++i)
                                {
                                    ...
                                    this->v2sigma2_drho_singlet_[i]
                                        = gradrho[0][i]
                                          * (v2s2[istart] * 2. + v2s2[istart + 1] * 3. + v2s2[istart + 2] * 2.
                                             + v2s2[istart + 3] + v2s2[istart + 4])
                                          * 2.;
                                    this->v2rhosigma_drho_triplet_[i]
                                        = gradrho[0][i] * (v2rs[istart] - v2rs[istart + 2]) * 2.;
                                    this->v2sigma2_drho_triplet_[i] = gradrho[0][i]
                                                                      * (v2s2[istart] * 2. + v2s2[istart + 1]
                                                                         - v2s2[istart + 2] * 2. - v2s2[istart + 4])
                                                                      * 2.;
                                }
                        }
                    else
...
#pragma omp parallel for schedule(static, 4096)
                            ...
                            for (int i = 0; i < nrxx; ++i)
                                {
                                    const int istart = i * 6;
                                    this->v2rhosigma_drho_uu_[i]
                                        = gradrho[0][i] * v2rs[istart] * 2. + gradrho[1][i] * v2rs[istart + 1];
                ...
                {
                    throw std::domain_error ("nspin =" + std::to_string (nspin) + " unfinished in "
