...
PotHxcLR::PotHxcLR (const std::string& xc_kernel,
    ...
    : xc_kernel_ (xc_kernel), tpiba_ (ucell.tpiba), spin_type_ (st), rho_basis_ (rho_basis), nrxx_ (chg_gs.nrxx),
      ...
      xc_kernel_components_ (rho_basis,
                             ...
                             xc_kernel,
                             lr_init_xc_kernel,
                             (st == SpinType::S2_updown)), // call XC_Functional::set_func_type and libxc
      xc_type_ (XCType (XC_Functional::get_func_type ()))
{
    if (std::set<std::string> ({"lda", "pwlda", "pbe", "hse"}).count (xc_kernel))
        {
                         ...
                         const std::vector<int>& ispin_op)
{
    ...
    switch (this->spin_type_)
        ...
        case SpinType::S2_updown:
            v_eff += elecstate::H_Hartree_pw::v_hartree (ucell,
        ...
        case SpinType::S2_singlet:
            v_eff += 2
                     * elecstate::H_Hartree_pw::v_hartree (ucell,
                                                           const_cast<ModulePW::PW_Basis*> (&this->rho_basis_),
                                                           1,
                                                           rho);
            break;
...
#else
    throw std::domain_error ("GlobalV::XC_Functional::get_func_type() ="
                             + std::to_string (XC_Functional::get_func_type ()) + " unfinished in "
...
void
    PotHxcLR::set_integral_func (const SpinType& s, const XCType& xc)
{
                    ...
                    funcs[s] = [this, &fxc] (FXC_PARA_TYPE) -> void
                        {
                            for (int ir = 0; ir < nrxx; ++ir)
                                {
                                    v_eff (0, ir) += ModuleBase::e2 * fxc.v2rho2.at (ir) * rho[ir];
                                }
                        };
                    break;
                case SpinType::S2_singlet:
                            ...
                            for (int ir = 0; ir < nrxx; ++ir)
                                {
                                    ...
                                    v_eff (0, ir)
                                        += ModuleBase::e2 * (fxc.v2rho2.at (irs0) + fxc.v2rho2.at (irs1)) * rho[ir];
                                }
                        };
                    break;
                    ...
                    funcs[s] = [this, &fxc] (FXC_PARA_TYPE) -> void
                        {
                            ...
                            for (int ir = 0; ir < nrxx; ++ir)
                                {
                                    v_eff (0, ir) += ModuleBase::e2 * fxc.v2rho2.at (3 * ir + is) * rho[ir];
                                }
                        };
                    break;
                default:
                    throw std::domain_error ("SpinType =" + std::to_string (static_cast<int> (s)) + " unfinished in "
                                             + std::string (__FILE__) + " line " + std::to_string (__LINE__));
                    break;
                }
        }
            ...
            switch (s)
                    ...
                    funcs[s] = [this, &fxc] (FXC_PARA_TYPE) -> void
                        {
                            ...
                            //     for (int ir = 0;ir < nrxx;++ir) if (std::abs(v.at(ir).x) > thr) std::cout <<
                            //     v.at(ir).x << " "; std::cout << std::endl; for (int ir = 0;ir < nrxx;++ir) if
                            //     (std::abs(v.at(ir).y) > thr) std::cout << v.at(ir).y << " "; std::cout << std::endl;
                            //     for (int ir = 0;ir < nrxx;++ir) if (std::abs(v.at(ir).z) > thr) std::cout <<
                            //     v.at(ir).z << " "; std::cout << std::endl;};
                            ...
                            for (int ir = 0; ir < nrxx; ++ir)
                                {
                                    vxc_tmp[ir]
                                        += (fxc.v2rho2.at (ir) * rho[ir] + fxc.v2rhosigma_2drho.at (ir) * drho.at (ir));
                                }
                            ...
                            for (int ir = 0; ir < nrxx; ++ir)
                                {
                                    // gdot terms in f_uu
                                    gdot_terms[ir] = -(
                                        rho[ir] * fxc.v2rhosigma_drho_singlet.at (ir)
                                        + (fxc.drho_gs.at (0).at (ir) * drho.at (ir))
                                              * fxc.v2sigma2_drho_singlet.at (ir)
                                        + drho.at (ir) * (fxc.vsigma.at (ir * 3) * 2. + fxc.vsigma.at (ir * 3 + 1)));
                                }
                            XC_Functional::grad_dot (gdot_terms.data (),
                                                     vxc_tmp.data (),
                                                     &this->rho_basis_,
                                                     this->tpiba_);
                            ...
                            for (int ir = 0; ir < nrxx; ++ir)
                                {
                                    // gdot terms in f_uu
                                    gdot_terms[ir] = -(
                                        rho[ir] * fxc.v2rhosigma_drho_triplet.at (ir)
                                        + (fxc.drho_gs.at (0).at (ir) * drho.at (ir))
                                              * fxc.v2sigma2_drho_triplet.at (ir)
                                        + drho.at (ir) * (fxc.vsigma.at (ir * 3) * 2. - fxc.vsigma.at (ir * 3 + 1)));
                                }
                            XC_Functional::grad_dot (gdot_terms.data (),
                                                     vxc_tmp.data (),
                    ...
                    funcs[s] = [this, &fxc] (FXC_PARA_TYPE) -> void
                        {
                                    ...
                                    for (int ir = 0; ir < nrxx; ++ir)
                                        {
                                            gdot_terms[ir] = -(rho[ir] * fxc.v2rhosigma_drho_uu.at (ir)
                                                               + (fxc.drho_gs.at (0).at (ir) * drho.at (ir))
                                                                     * fxc.v2sigma2_drho_uu_u.at (ir)
                                                               + (fxc.drho_gs.at (1).at (ir) * drho.at (ir))
                                                                     * fxc.v2sigma2_drho_uu_d.at (ir)
                                    ...
                                    for (int ir = 0; ir < nrxx; ++ir)
                                        {
                                            gdot_terms[ir] = -(rho[ir] * fxc.v2rhosigma_drho_du.at (ir) // rho_d, drho_u
                                                               + (fxc.drho_gs.at (0).at (ir) * drho.at (ir))
                                                                     * fxc.v2sigma2_drho_ud_u.at (ir)
                                                               + (fxc.drho_gs.at (1).at (ir) * drho.at (ir))
                                                                     * fxc.v2sigma2_drho_ud_d.at (ir)
                                    ...
                                    for (int ir = 0; ir < nrxx; ++ir)
                                        {
                                            gdot_terms[ir] = -(rho[ir] * fxc.v2rhosigma_drho_ud.at (ir) // rho_u, drho_d
                                                               + (fxc.drho_gs.at (0).at (ir) * drho.at (ir))
                                                                     * fxc.v2sigma2_drho_du_u.at (ir)
                                                               + (fxc.drho_gs.at (1).at (ir) * drho.at (ir))
                                                                     * fxc.v2sigma2_drho_du_d.at (ir)
                                                               + drho.at (ir) * fxc.vsigma.at (ir * 3 + 1));
                                        }
                                    break;
                                case 3: // (1,1)
                                    for (int ir = 0; ir < nrxx; ++ir)
                                        {
                                            gdot_terms[ir] = -(rho[ir] * fxc.v2rhosigma_drho_dd.at (ir)
                                                               + (fxc.drho_gs.at (0).at (ir) * drho.at (ir))
                                                                     * fxc.v2sigma2_drho_dd_u.at (ir)
                                                               + (fxc.drho_gs.at (1).at (ir) * drho.at (ir))
                                                                     * fxc.v2sigma2_drho_dd_d.at (ir)
                            ...
                            switch (ispin_op[0] << 1 | ispin_op[1])
                                ...
                                case 0:
                                    for (int ir = 0; ir < nrxx; ++ir)
                                        {
                                            vxc_tmp[ir] += rho[ir] * fxc.v2rho2.at (ir * 3)
                                                           + drho.at (ir) * fxc.v2rhosigma_drho_uu.at (ir);
                                        }
                                    break;
                                case 1:
                                    for (int ir = 0; ir < nrxx; ++ir)
                                        {
                                            vxc_tmp[ir] += rho[ir] * fxc.v2rho2.at (ir * 3 + 1)
                                                           + drho.at (ir) * fxc.v2rhosigma_drho_ud.at (ir);
                                        }
                                    break;
                                case 2:
                                    for (int ir = 0; ir < nrxx; ++ir)
                                        {
                                            vxc_tmp[ir] += rho[ir] * fxc.v2rho2.at (ir * 3 + 1)
                                                           + drho.at (ir) * fxc.v2rhosigma_drho_du.at (ir);
                                        }
                                    break;
                                case 3:
                                    for (int ir = 0; ir < nrxx; ++ir)
                                        {
                                            vxc_tmp[ir] += rho[ir] * fxc.v2rho2.at (ir * 3 + 2)
                                                           + drho.at (ir) * fxc.v2rhosigma_drho_dd.at (ir);
                                        }
                                    break;
                                default:
                                    throw std::runtime_error ("Invalid ispin_op");
                ...
                default:
                    throw std::domain_error ("SpinType =" + std::to_string (static_cast<int> (s))
                                             + "for GGA or HYB_GGA is unfinished in " + std::string (__FILE__)
