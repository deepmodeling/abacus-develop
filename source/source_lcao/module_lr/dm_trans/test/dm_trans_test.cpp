...
{
  public:
    std::vector<matsize> sizes{{2, 14, 9, 4}, {2, 20, 10, 7}};
    int nstate = 2;
    ...
    void
        set_ones (double* data, int size)
    {
        for (int i = 0; i < size; ++i)
            data[i] = 1.0;
    };
    void
        set_int (double* data, int size)
    {
        for (int i = 0; i < size; ++i)
            data[i] = static_cast<double> (i + 1);
    };
    void
        set_int (std::complex<double>* data, int size)
    {
        for (int i = 0; i < size; ++i)
            data[i] = std::complex<double> (i + 1, -i - 1);
    };
    void
        set_rand (double* data, int size)
    {
        for (int i = 0; i < size; ++i)
            data[i] = double (rand ()) / double (RAND_MAX) * 10.0 - 5.0;
    };
    void
        set_rand (std::complex<double>* data, int size)
    {
        for (int i = 0; i < size; ++i)
            data[i] = std::complex<double> (rand (), rand ()) / double (RAND_MAX) * 10.0 - 5.0;
    };
    void
        check_eq (double* data1, double* data2, int size)
    {
        for (int i = 0; i < size; ++i)
            EXPECT_NEAR (data1[i], data2[i], 1e-10);
    };
    void
        check_eq (std::complex<double>* data1, std::complex<double>* data2, int size)
    {
        for (int i = 0; i < size; ++i)
            {
            ...
            for (int istate = 0; istate < nstate; ++istate)
                {
                    ...
                    auto test = [&] (psi::Psi<double>& X, const LR::MO_TYPE type)
                        {
                            X.fix_b (istate);
                            const std::vector<container::Tensor>& dm_for
                                = LR::cal_dm_trans_forloop_serial (X.get_pointer (), c, s.nocc, s.nvirt, 1., type);
                            const std::vector<container::Tensor>& dm_blas
                                = LR::cal_dm_trans_blas (X.get_pointer (), c, s.nocc, s.nvirt, 1., type);
                            for (int isk = 0; isk < s.nks; ++isk)
                                check_eq (dm_for[isk].data<double> (), dm_blas[isk].data<double> (), s.naos * s.naos);
                        };
                    ...
                    auto test = [&] (psi::Psi<std::complex<double>>& X, const LR::MO_TYPE type)
                        {
                            X.fix_b (istate);
                            const std::vector<container::Tensor>& dm_for
                                = LR::cal_dm_trans_forloop_serial (X.get_pointer (),
                                                                   c,
                                                                   s.nocc,
                                                                   s.nvirt,
                                                                   ...
                                                                   type);
                            const std::vector<container::Tensor>& dm_blas
                                = LR::cal_dm_trans_blas (X.get_pointer (),
                                                         c,
                                                         s.nocc,
                                                         s.nvirt,
                                                         std::complex<double> (1., 0.),
                                                         type);
                            for (int isk = 0; isk < s.nks; ++isk)
                                check_eq (dm_for[isk].data<std::complex<double>> (),
                                          dm_blas[isk].data<std::complex<double>> (),
                                          s.naos * s.naos);
                        };
                    test (X_vo, LR::MO_TYPE::VO);
                    test (X_oo, LR::MO_TYPE::OO);
    ...
    for (auto s: this->sizes)
        {
            ...
            auto gather = [&] (const psi::Psi<double>& X,
                               psi::Psi<double>& X_full,
            ...
            for (int istate = 0; istate < nstate; ++istate)
                {
                    ...
                    auto test = [&] (psi::Psi<double>& X,
                                     ...
                                     const LR::MO_TYPE type)
                        {
                            X.fix_b (istate);
                            X_full.fix_b (istate);
                            std::vector<container::Tensor> dm_pblas_loc
                                = LR::cal_dm_trans_pblas (X.get_pointer (),
                                                          px,
                                                          c,
                                                          pc,
                                                          s.naos,
                                                          ...
                                                          s.nvirt,
                                                          pmat,
                                                          (double)1.0 / (double)s.nks,
                                                          type);
                            std::vector<container::Tensor> dm_gather (
                                s.nks,
                                container::Tensor (DAT::DT_DOUBLE, DEV::CpuDevice, {s.naos, s.naos}));
                            for (int isk = 0; isk < s.nks; ++isk)
                                {
                                    LR_Util::gather_2d_to_full (pmat,
                                                                dm_pblas_loc[isk].data<double> (),
                                                                dm_gather[isk].data<double> (),
                                                                false,
                                                                s.naos,
                                                                s.naos);
                                }
                            if (my_rank == 0)
                                {
                                    const std::vector<container::Tensor>& dm_full
                                        = LR::cal_dm_trans_blas (X_full.get_pointer (),
                                                                 c_full,
                                                                 s.nocc,
                                                                 s.nvirt,
                                                                 (double)1.0 / (double)s.nks,
                                                                 type);
                                    for (int isk = 0; isk < s.nks; ++isk)
                                        check_eq (dm_full[isk].data<double> (),
                                                  dm_gather[isk].data<double> (),
                                                  s.naos * s.naos);
                                }
                        };
    ...
    for (auto s: this->sizes)
        {
            ...
            psi::Psi<std::complex<double>> X_vo (s.nks,
                                                 nstate,
                                                 px_vo.get_local_size (),
                                                 px_vo.get_local_size (),
                                                 false);
            set_rand (X_vo.get_pointer (), nstate * s.nks * px_vo.get_local_size ());
            psi::Psi<std::complex<double>> X_oo (s.nks,
                                                 nstate,
                                                 px_oo.get_local_size (),
                                                 px_oo.get_local_size (),
                                                 false);
            set_rand (X_oo.get_pointer (), nstate * s.nks * px_oo.get_local_size ());
            psi::Psi<std::complex<double>> X_vv (s.nks,
                                                 nstate,
                                                 px_vv.get_local_size (),
                                                 px_vv.get_local_size (),
                                                 false);
            set_rand (X_vv.get_pointer (), nstate * s.nks * px_vv.get_local_size ());
            ...
            psi::Psi<std::complex<double>> X_full_vo (s.nks,
                                                      nstate,
                                                      s.nocc * s.nvirt,
                                                      s.nocc * s.nvirt,
                                                      false); // allocate X_full
            psi::Psi<std::complex<double>> X_full_oo (s.nks,
                                                      nstate,
                                                      s.nocc * s.nocc,
                                                      s.nocc * s.nvirt,
                                                      false); // allocate X_full
            psi::Psi<std::complex<double>> X_full_vv (s.nks,
                                                      nstate,
                                                      s.nvirt * s.nvirt,
                                                      s.nocc * s.nvirt,
                                                      false); // allocate X_full

            auto gather = [&] (const psi::Psi<std::complex<double>>& X,
                               psi::Psi<std::complex<double>>& X_full,
                               const Parallel_2D& px,
                               const int dim1,
                               const int dim2)
                {
            ...
            for (int istate = 0; istate < nstate; ++istate)
                {
                    ...
                    auto test = [&] (psi::Psi<std::complex<double>>& X,
                                     psi::Psi<std::complex<double>>& X_full,
                                     const Parallel_2D& px,
                                     const LR::MO_TYPE type)
                        {
                            ...
                            std::vector<container::Tensor> dm_pblas_loc
                                = LR::cal_dm_trans_pblas (X.get_pointer (),
                                                          ...
                                                          s.nvirt,
                                                          pmat,
                                                          std::complex<double> (1.0, 0.0) / (double)s.nks,
                                                          type);
                            std::vector<container::Tensor> dm_gather (
                                s.nks,
                                container::Tensor (DAT::DT_COMPLEX_DOUBLE, DEV::CpuDevice, {s.naos, s.naos}));
                            for (int isk = 0; isk < s.nks; ++isk)
                                {
                                    LR_Util::gather_2d_to_full (pmat,
                                                                dm_pblas_loc[isk].data<std::complex<double>> (),
                                                                dm_gather[isk].data<std::complex<double>> (),
                                                                false,
                                                                s.naos,
                                                                s.naos);
                                }
                            if (my_rank == 0)
                                {
                                    const std::vector<container::Tensor>& dm_full
                                        = LR::cal_dm_trans_blas (X_full.get_pointer (),
                                                                 ...
                                                                 type);
                                    for (int isk = 0; isk < s.nks; ++isk)
                                        check_eq (dm_full[isk].data<std::complex<double>> (),
                                                  dm_gather[isk].data<std::complex<double>> (),
                                                  s.naos * s.naos);
                                }
                        };
