...
class DMTransTest : public testing::Test {
  public:
    ...
    void TearDown() override { ofs_running.close(); }
#endif
            ...
            auto test = [&](psi::Psi<std::complex<double>>& X, const LR::MO_TYPE type) {
                ...
                const std::vector<container::Tensor>& dm_for =
                    LR::cal_dm_trans_forloop_serial(X.get_pointer(),
                                                    ...
                                                    type);
                const std::vector<container::Tensor>& dm_blas =
                    LR::cal_dm_trans_blas(X.get_pointer(), c, s.nocc, s.nvirt, std::complex<double>(1., 0.), type);
                for (int isk = 0; isk < s.nks; ++isk)
...
TEST_F(DMTransTest, DoubleParallel) {
    for (auto s: this->sizes) {
        ...
        for (int istate = 0; istate < nstate; ++istate) {
            ...
            auto test =
                [&](psi::Psi<double>& X, psi::Psi<double>& X_full, const Parallel_2D& px, const LR::MO_TYPE type) {
                    X.fix_b(istate);
                    X_full.fix_b(istate);
                    std::vector<container::Tensor> dm_pblas_loc = LR::cal_dm_trans_pblas(X.get_pointer(),
                                                                                         px,
                                                                                         c,
                                                                                         pc,
                                                                                         s.naos,
                    ...
                    if (my_rank == 0) {
                        const std::vector<container::Tensor>& dm_full =
                            LR::cal_dm_trans_blas(X_full.get_pointer(),
                                                  ...
                                                  type);
                        for (int isk = 0; isk < s.nks; ++isk)
                            check_eq(dm_full[isk].data<double>(), dm_gather[isk].data<double>(), s.naos * s.naos);
                    }
                };
