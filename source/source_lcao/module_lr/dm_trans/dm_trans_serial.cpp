...
namespace LR {
template <>
std::vector<container::Tensor> cal_dm_trans_forloop_serial(const double* const X_istate,
    ...
    for (auto& dm: dm_trans)
        ModuleBase::GlobalFunc::ZEROS(dm.data<double>(), naos * naos);
    // loop for AOs
            ...
            for (size_t nu = 0; nu < naos; ++nu) {
                ...
                for (size_t p = 0; p < nmo1; ++p) {
                    for (size_t q = 0; q < nmo2; ++q)
                        dm_trans[isk].data<double>()[mu * naos + nu] +=
                            c(imo1 + p, mu) * X_istate[x_start + p * nmo2 + q] * c(imo2 + q, nu) * factor;
                }
            }
        }
    }
...
template <>
std::vector<container::Tensor> cal_dm_trans_forloop_serial(const std::complex<double>* const X_istate,
    ...
    for (auto& dm: dm_trans)
        ModuleBase::GlobalFunc::ZEROS(dm.data<std::complex<double>>(), naos * naos);
    // loop for AOs
    for (size_t isk = 0; isk < nks; ++isk) {
        c.fix_k(isk);
        const int x_start = isk * nmo1 * nmo2;
...
template <>
std::vector<container::Tensor> cal_dm_trans_blas(const double* const X_istate,
                                                 const psi::Psi<double>& c,
                                                 const int& nocc,
                                                 const int& nvirt,
                                                 const double factor,
...
template <>
std::vector<container::Tensor> cal_dm_trans_blas(const std::complex<double>* const X_istate,
                                                 const psi::Psi<std::complex<double>>& c,
                                                 const int& nocc,
                                                 const int& nvirt,
                                                 const std::complex<double> factor,
