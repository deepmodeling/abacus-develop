...
class HamiltULR
{
  public:
    HamiltULR (std::string& xc_kernel,
        ...
        : nocc (nocc), nvirt (nvirt), pX (pX_in), nk (kv_in.get_nks () / nspin),
          ...
          gdim (nk * std::inner_product (nocc.begin (), nocc.end (), nvirt.begin (), 0))
    {
        ...
        auto newHxc = [&] (const int& sl, const int& sr)
            {
                return new OperatorLRHxc<T> (nspin,
                                             naos,
                                             nocc,
                                             nvirt,
                                             psi_ks_in,
        ...
        if (xc_kernel == "hf" || xc_kernel == "hse")
            {
                std::vector<psi::Psi<T>> psi_ks_spin
                    = {LR_Util::get_psi_spin (psi_ks_in, 0, nk), LR_Util::get_psi_spin (psi_ks_in, 1, nk)};
                for (int is: {0, 1})
                    {
                        this->ops[(is << 1) + is]->add (new OperatorLREXX<T> (nspin,
                                                                              naos,
                                                                              nocc[is],
                                                                              nvirt[is],
                                                                              ucell_in,
            ...
            // LR_Util::print_value(X, pX_in[is].get_local_size());
#ifdef __MPI
                std::vector<ct::Tensor> dm_trans_2d = cal_dm_trans_pblas (X,
                                                                          pX[is],
                                                                          psi_ks_is,
                                                                          pc_in,
                                                                          naos,
                ...
                if (this->tdm_sym)
                    for (auto& t: dm_trans_2d)
                        LR_Util::matsym (t.data<T> (), naos, pmat_in);
#else
                std::vector<ct::Tensor> dm_trans_2d
                    = cal_dm_trans_blas (X, psi_ks_is, nocc[is], nvirt[is], (T)1.0 / (T)nk);
                if (this->tdm_sym)
                    for (auto& t: dm_trans_2d)
                        LR_Util::matsym (t.data<T> (), naos);
#endif
                // LR_Util::print_tensor<T>(dm_trans_2d[0], "DMtrans(k=0)", &pmat_in);
                // tensor to vector, then set DMK
                for (int ik = 0; ik < nk; ++ik)
                    {
                        this->DM_trans->set_DMK_pointer (ik, dm_trans_2d[ik].data<T> ());
                    }
            };
    }
    ~HamiltULR ()
    ...
    void
        hPsi (const T* const psi_in, T* const hpsi, const int ld_psi, const int& nband) const
    {
                                ...
                                for (int b = 0; b < nv; ++b)
                                    {
                                        ...
                                        const int lcol = loffset_bj + ik_bj * px.get_local_size ()
                                                         + lj * px.get_row_size () + lb; // local
                                                ...
                                                while (node != nullptr)
                                                    {
                                                        node->act (1,
                                                                   ldim_is[is_bj],
                                                                   /*npol=*/1,
                                                                   X_bj.data () + loffset_bj,
                                                                   Aloc_col.data () + loffset_ai);
                                                        node = (hamilt::Operator<T>*)(node->next_op);
                                                    }
#ifdef __MPI
                                                for (int ik_ai = 0; ik_ai < this->nk; ++ik_ai)
                                                    {
                                                        LR_Util::gather_2d_to_full (
                                                            pax,
                                                            Aloc_col.data () + loffset_ai
                                                                + ik_ai * pax.get_local_size (),
                                                            Amat_full.data () + gcol * gdim /*col, bj*/ + goffset_ai
                                                                + ik_ai * npairs[is_ai] /*row, ai*/,
                                                            false,
                                                            nv,
                                                            no);
                                                    }
#else
                                                std::memcpy (Amat_full.data () + gcol * gdim + goffset_ai,
                                                             Aloc_col.data () + goffset_ai,
                                                             gdim_is[is_ai] * sizeof (T));
#endif
                                            }
                                    }
                            }
    ...
    void
        global2local (T* lvec, const T* gvec, const int& nband) const
    {
        const std::vector<int> npairs = {this->nocc[0] * this->nvirt[0], this->nocc[1] * this->nvirt[1]};
        const std::vector<int64_t> ldim_is = {nk * pX[0].get_local_size (), nk * pX[1].get_local_size ()};
        const std::vector<int> gdim_is = {nk * npairs[0], nk * npairs[1]};
                                        ...
                                        for (int lv = 0; lv < pX[is].get_row_size (); ++lv)
                                            {
                                                ...
                                                lvec[loffset + lo * pX[is].get_row_size () + lv]
                                                    = gvec[goffset + go * nvirt[is] + gv];
                                            }
                                    }
                            }
                    }
