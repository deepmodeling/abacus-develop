  ...
  public:
    HamiltULR(std::string& xc_kernel,
        ...
        if (xc_kernel == "hf" || xc_kernel == "hse") {
            std::vector<psi::Psi<T>> psi_ks_spin = {LR_Util::get_psi_spin(psi_ks_in, 0, nk),
                                                    LR_Util::get_psi_spin(psi_ks_in, 1, nk)};
            for (int is: {0, 1}) {
        ...
        this->cal_dm_trans = [&, this](const int& is, const T* const X) -> void {
            ...
            std::vector<ct::Tensor> dm_trans_2d =
                cal_dm_trans_pblas(X, pX[is], psi_ks_is, pc_in, naos, nocc[is], nvirt[is], pmat_in, (T)1.0 / (T)nk);
            if (this->tdm_sym)
            ...
            for (int ik_bj = 0; ik_bj < nk; ++ik_bj) {
                for (int j = 0; j < no; ++j) {
                    for (int b = 0; b < nv; ++b) {
                        ...
                        const int lcol =
                            loffset_bj + ik_bj * px.get_local_size() + lj * px.get_row_size() + lb; // local
                        ...
                        for (int is_ai: {0, 1}) {
                            ...
                            for (int ik_ai = 0; ik_ai < this->nk; ++ik_ai) {
                                LR_Util::gather_2d_to_full(pax,
                                                           Aloc_col.data() + loffset_ai + ik_ai * pax.get_local_size(),
