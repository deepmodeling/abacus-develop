        ...
        : nspin (nspin), nocc (nocc), nvirt (nvirt), pX (pX_in), nk (kv_in.get_nks () / nspin)
    {
        ...
        if (ri_hartree_benchmark != "none")
            {
                        ...
                        if (ri_hartree_benchmark == "aims")
                            {
                                ...
                                Vs_read
                                    = RI_Benchmark::read_coulomb_mat_general<T> (dir + "coulomb_mat_0.txt", Cs_read);
                            }
                        ...
                        if (!std::set<std::string> ({"rpa", "hf"}).count (xc_kernel))
                            {
                                throw std::runtime_error (
                                    "ri_hartree_benchmark is only supported for xc_kernel rpa and hf");
                            }
                        RI_Benchmark::OperatorRIHartree<T>* ri_hartree_op
                            = new RI_Benchmark::OperatorRIHartree<T> (ucell_in,
                                                                      naos,
                                                                      nocc[0],
                                                                      nvirt[0],
                                                                      psi_ks_in,
                ...
                else if (spin_type == "triplet")
                    {
                        std::cout << "f_Hxc based on grid integral is not needed." << std::endl;
                    }
#else
...
#endif
            {
                OperatorLRHxc<T>* lr_hxc = new OperatorLRHxc<T> (nspin,
                                                                 naos,
                                                                 nocc,
                                                                 nvirt,
                                                                 psi_ks_in,
        ...
        if (xc_kernel == "hf" || xc_kernel == "hse")
            { // add Exx operator
                ...
                // std::cout << "exx_alpha=" << exx_alpha << std::endl; // the default value of exx_alpha is 0.25 when
                // dft_functional is pbe or hse
                hamilt::Operator<T>* lr_exx = new OperatorLREXX<T> (nspin,
                                                                    naos,
                                                                    nocc[0],
                                                                    nvirt[0],
                                                                    ucell_in,
                                ...
                                for (int lv = 0; lv < pX[0].get_row_size (); ++lv)
                                    {
                                        ...
                                        lvec[loffset + lo * pX[0].get_row_size () + lv]
                                            = gvec[goffset + go * nvirt[0] + gv];
                                    }
                            }
                    }
            }
