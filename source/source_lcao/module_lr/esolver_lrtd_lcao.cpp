...
template <>
void
    LR::ESolver_LR<double>::move_exx_lri (std::shared_ptr<Exx_LRI<double>>& exx_ks)
{
...
template <>
void
    LR::ESolver_LR<std::complex<double>>::move_exx_lri (std::shared_ptr<Exx_LRI<std::complex<double>>>& exx_ks)
{
...
template <>
void
    LR::ESolver_LR<std::complex<double>>::move_exx_lri (std::shared_ptr<Exx_LRI<double>>& exx_ks)
{
    throw std::runtime_error (
        "ESolver_LR<std::complex<double>>::move_exx_lri: cannot move double to std::complex<double>");
}
template <>
void
    LR::ESolver_LR<double>::move_exx_lri (std::shared_ptr<Exx_LRI<std::complex<double>>>& exx_ks)
{
...
inline int
    cal_nupdown_form_occ (const ModuleBase::matrix& wg)
{ // only for nspin=2
    const int& nk = wg.nr / 2;
    auto occ_sum_k = [&] (const int& is, const int& ib) -> double
        {
            double o = 0.0;
...
inline void
    setup_2center_table (TwoCenterBundle& two_center_bundle, LCAO_Orbitals& orb, UnitCell& ucell)
{
...
void
    LR::ESolver_LR<T, TR>::parameter_check () const
{
...
template <typename T, typename TR>
void
    LR::ESolver_LR<T, TR>::set_dimension ()
{
    ...
    if (input.nvirt > this->nvirt_in)
        {
            GlobalV::ofs_warning
                << "ESolver_LR: input nvirt is too large to cover by nbands, set nvirt = nbands - nocc = "
                << this->nvirt_in << std::endl;
        }
    else if (input.nvirt > 0)
        {
            this->nvirt_in = input.nvirt;
        }
    ...
    for (int is = 0; is < nspin; ++is)
        {
            this->npairs.push_back (nocc[is] * nvirt[is]);
        }
...
template <typename T, typename TR>
void
    LR::ESolver_LR<T, TR>::reset_dim_spin2 ()
{
    if (nspin != 2)
        {
            return;
    ...
    if (nupdown == 0)
        ...
        {
            this->openshell = true;
            nupdown > 0 ? ((nocc[1] -= nupdown) && (nvirt[1] += nupdown))
                        : ((nocc[0] += nupdown) && (nvirt[0] -= nupdown));
    ...
    for (int is: {0, 1})
        {
            if (npairs[is] <= 0)
                {
                    throw std::invalid_argument (std::string ("ESolver_LR: npairs (nocc*nvirt) <= 0 for spin")
                                                 + std::string (is == 0 ? "up" : "down"));
                }
        }
...
#endif
{
    ...
    LR_Util::setup_2d_division (this->paraC_,
                                1,
                                this->nbasis,
                                this->nbands
#ifdef __MPI
...
template <typename T, typename TR>
LR::ESolver_LR<T, TR>::ESolver_LR (const Input_para& inp, UnitCell& ucell)
    : input (inp), ucell (ucell)
#ifdef __EXX
      ,
      exx_info (GlobalC::exx_info)
#endif
{
    ...
    if (input.ri_hartree_benchmark != "aims")
        {
...
template <typename T, typename TR>
void
    LR::ESolver_LR<T, TR>::runner (UnitCell& ucell, const int istep)
{
    ...
    auto efile = [&] (const std::string& label) -> std::string
        { return PARAM.globalv.global_out_dir + "Excitation_Energy_" + label + ".dat"; };
    auto vfile = [&] (const std::string& label) -> std::string
        {
            return PARAM.globalv.global_out_dir + "Excitation_Amplitude_" + label + "_"
                   + std::to_string (GlobalV::MY_RANK) + ".dat";
        };
    if (this->input.lr_solver != "spectrum")
        {
            auto write_states = [&] (const std::string& label,
                                     const Real<T>* e,
                                     const T* v,
                                     const int& dim,
                                     const int& nst,
                                     const int& prec = 8) -> void
                {
                    if (GlobalV::MY_RANK == 0)
                        {
                            assert (nst == LR_Util::write_value (efile (label), prec, e, nst));
                        }
            ...
            if (openshell)
                {
                    for (int is: {0, 1})
                        {
                            ...
                            OperatorLRDiag<double> pre_op (this->eig_ks.c + is * nk * (nocc[0] + nvirt[0]),
                                                           this->paraX_[is],
                                                           this->nk,
                                                           this->nocc[is],
                                                           this->nvirt[is]);
                    ...
                    HamiltULR<T> hulr (xc_kernel,
                                       ...
                                       *this->psi_ks,
                                       ...
                                       this->paraC_,
                                       this->paraMat_);
                    LR::HSolver::solve (hulr,
                                        this->X[0].template data<T> (),
                                        nloc_per_band,
                                        nstates,
                                        this->pelec->ekb.c,
                ...
                {
                    OperatorLRDiag<double> pre_op (this->eig_ks.c,
                                                   this->paraX_[0],
                                                   this->nk,
                                                   this->nocc[0],
                                                   this->nvirt[0]);
            ...
            auto read_states = [&] (const std::string& label, Real<T>* e, T* v, const int& dim, const int& nst) -> void
                {
                    if (GlobalV::MY_RANK == 0)
                        {
                            assert (nst == LR_Util::read_value (efile (label), e, nst));
                        }
...
template <typename T, typename TR>
void
    LR::ESolver_LR<T, TR>::after_all_runners (UnitCell& ucell)
{
    ModuleBase::TITLE ("ESolver_LR", "after_all_runners");
    if (input.ri_hartree_benchmark != "none")
        {
            return;
        } // no need to calculate the spectrum in the benchmark routine
    // cal spectrum
    std::vector<double> freq (100);
    std::vector<double> abs_wavelen_range ({20, 200}); // default range
    if (input.abs_wavelen_range.size () >= 2
        && std::abs (input.abs_wavelen_range[1] - input.abs_wavelen_range[0]) > 0.02)
        {
    ...
    for (int i = 0; i < freq.size (); ++i)
        {
            freq[i] = 91.126664 / (lambda_min + 0.01 * static_cast<double> (i + 1) * lambda_diff);
        }
    auto spin_types = (nspin == 2 && !openshell) ? std::vector<std::string> ({"singlet", "triplet"})
            ...
            if (spin_types[is] != "triplet") // triplets has no transition dipole and no contribution to the spectrum
                {
                    ...
                    // =============================================== for test
                    // ==================================================== spectrum.optical_absorption_method2(freq,
                    // input.abs_broadening); spectrum.test_transition_dipoles_velocity_ks(eig_ks.c);
...
template <typename T, typename TR>
void
    LR::ESolver_LR<T, TR>::setup_eigenvectors_X ()
{
    ModuleBase::TITLE ("ESolver_LR", "setup_eigenvectors_X");
    for (int is = 0; is < nspin; ++is)
        {
            Parallel_2D px;
            LR_Util::setup_2d_division (px,
                                        /*nb2d=*/1,
                                        this->nvirt[is],
                                        this->nocc[is]
#ifdef __MPI
    ...
    this->nloc_per_band
        = nk * (openshell ? paraX_[0].get_local_size () + paraX_[1].get_local_size () : paraX_[0].get_local_size ());
...
template <typename T, typename TR>
void
    LR::ESolver_LR<T, TR>::set_X_initial_guess ()
{
    ...
    for (int is = 0; is < this->nspin; ++is)
        {
            ...
            if (this->eig_ks.nc > no + 1 && no >= 2
                && eig_ks (is, no) - eig_ks (is, no - 2) - 1e-5 > eig_ks (is, no + 1) - eig_ks (is, no - 1))
                {
                    ix_mode = true;
                }
            GlobalV::ofs_running << "setting the initial guess of X of spin" << is << std::endl;
            if (no >= 2 && eig_ks.nc > no)
                {
                    GlobalV::ofs_running << "E_{lumo}-E_{homo-1}=" << eig_ks (is, no) - eig_ks (is, no - 2)
                                         << std::endl;
                }
            if (no >= 1 && eig_ks.nc > no + 1)
                {
                    GlobalV::ofs_running << "E_{lumo+1}-E{homo}=" << eig_ks (is, no + 1) - eig_ks (is, no - 1)
                                         << std::endl;
                }
            ...
            for (int ib = 0; ib < nstates; ++ib)
                {
                    ...
                    const int xstart_bs = (openshell && is == 1) ? xstart_b + nk * paraX_[0].get_local_size ()
                                                                 : xstart_b; // start index of band ib, spin is
                    ...
                    if (px.in_this_processor (virt_global, occ_global))
                        {
                            ...
                            const int ipair_loc = px.global2local_col (occ_global) * px.get_row_size ()
                                                  + px.global2local_row (virt_global);
                            X[is_in_x].data<T> ()[xstart_bs + xstart_pair + ipair_loc]
...
template <typename T, typename TR>
void
    LR::ESolver_LR<T, TR>::init_pot (const Charge& chg_gs)
{
    this->pot.resize (nspin, nullptr);
    if (this->input.ri_hartree_benchmark != "none")
        {
            return;
        } // no need to initialize potential for Hxc kernel in the RI-benchmark routine
    switch (nspin)
        {
            using ST = PotHxcLR::SpinType;
        case 1:
            this->pot[0] = std::make_shared<PotHxcLR> (xc_kernel,
                                                       *this->pw_rho,
                                                       ucell,
                                                       chg_gs,
                                                       Pgrid,
                                                       ST::S1,
                                                       input.lr_init_xc_kernel);
            break;
        case 2:
            this->pot[0] = std::make_shared<PotHxcLR> (xc_kernel,
                                                       *this->pw_rho,
                                                       ...
                                                       chg_gs,
                                                       Pgrid,
                                                       openshell ? ST::S2_updown : ST::S2_singlet,
                                                       input.lr_init_xc_kernel);
            this->pot[1] = std::make_shared<PotHxcLR> (xc_kernel,
                                                       *this->pw_rho,
                                                       ucell,
                                                       chg_gs,
                                                       Pgrid,
                                                       openshell ? ST::S2_updown : ST::S2_triplet,
                                                       input.lr_init_xc_kernel);
            break;
        default:
...
template <typename T, typename TR>
void
    LR::ESolver_LR<T, TR>::read_ks_wfc ()
{
...
#ifdef __EXX
            int ncore = 0;
            std::vector<double> eig_ks_vec
                = RI_Benchmark::read_aims_ebands<double> (PARAM.globalv.global_readin_dir + "band_out",
                                                          nocc_in,
                                                          nvirt_in,
                                                          ncore);
            std::cout << "ncore=" << ncore << ", nocc=" << nocc_in << ", nvirt=" << nvirt_in
                      << ", nbands=" << this->nbands << std::endl;
            std::cout << "eig_ks_vec.size()=" << eig_ks_vec.size () << std::endl;
            if (eig_ks_vec.size () != this->nbands)
                {
                    ModuleBase::WARNING_QUIT ("ESolver_LR", "read_aims_ebands failed.");
                };
            for (int i = 0; i < nbands; ++i)
                {
                    this->pelec->ekb (0, i) = eig_ks_vec[i];
                }
            RI_Benchmark::read_aims_eigenvectors<T> (*this->psi_ks,
                                                     PARAM.globalv.global_readin_dir + "KS_eigenvectors.out",
                                                     ncore,
                                                     nbands,
                                                     nbasis);
#else
    ...
    else if (!ModuleIO::read_wfc_nao (PARAM.globalv.global_readin_dir,
                                      this->paraMat_,
                                      *this->psi_ks,
                                      this->pelec->ekb,
                                      this->pelec->wg,
...
template <typename T, typename TR>
void
    LR::ESolver_LR<T, TR>::read_ks_chg (Charge& chg_gs)
{
    ...
    for (int is = 0; is < this->nspin; ++is)
        {
            ...
            if (ModuleIO::read_vdata_palgrid (Pgrid,
                ...
                { // prenspin for nspin=4 is not supported currently
                    ModuleBase::WARNING_QUIT ("init_rho",
                                              "!!! Couldn't find the charge file !!! The default directory \n of "
                                              "SPIN1_CHG.cube is OUT.suffix, "
