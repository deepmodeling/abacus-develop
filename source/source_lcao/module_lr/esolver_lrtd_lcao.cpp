...
inline int cal_nupdown_form_occ(const ModuleBase::matrix& wg) { // only for nspin=2
...
void LR::ESolver_LR<T, TR>::set_dimension() {
    ...
    if (input.nvirt > this->nvirt_in) {
        GlobalV::ofs_warning << "ESolver_LR: input nvirt is too large to cover by nbands, set nvirt = nbands - nocc = "
...
void LR::ESolver_LR<T, TR>::runner(UnitCell& ucell, const int istep) {
    ...
    auto vfile = [&](const std::string& label) -> std::string {
        return PARAM.globalv.global_out_dir + "Excitation_Amplitude_" + label + "_" + std::to_string(GlobalV::MY_RANK) +
               ".dat";
    };
    if (this->input.lr_solver != "spectrum") {
        ...
        if (openshell) {
            ...
            LR::HSolver::solve(hulr,
                               ...
                               precondition);
            if (input.out_wfc_lr) {
                write_states("openshell", this->pelec->ekb.c, this->X[0].template data<T>(), nloc_per_band, nstates);
            }
        } else {
...
void LR::ESolver_LR<T, TR>::set_X_initial_guess() {
    ...
    for (int is = 0; is < this->nspin; ++is) {
        ...
        if (no >= 2 && eig_ks.nc > no) {
            GlobalV::ofs_running << "E_{lumo}-E_{homo-1}=" << eig_ks(is, no) - eig_ks(is, no - 2) << std::endl;
        }
        if (no >= 1 && eig_ks.nc > no + 1) {
            GlobalV::ofs_running << "E_{lumo+1}-E{homo}=" << eig_ks(is, no + 1) - eig_ks(is, no - 1) << std::endl;
        }
        ...
        for (int ib = 0; ib < nstates; ++ib) {
            ...
            if (px.in_this_processor(virt_global, occ_global)) {
                ...
                const int ipair_loc =
                    px.global2local_col(occ_global) * px.get_row_size() + px.global2local_row(virt_global);
...
void LR::ESolver_LR<T, TR>::init_pot(const Charge& chg_gs) {
    ...
    case 1:
        this->pot[0] =
            std::make_shared<PotHxcLR>(xc_kernel, *this->pw_rho, ucell, chg_gs, Pgrid, ST::S1, input.lr_init_xc_kernel);
        break;
    case 2:
    ...
    if (input.ri_hartree_benchmark == "aims") // for aims benchmark
    {
        ...
        std::vector<double> eig_ks_vec =
            RI_Benchmark::read_aims_ebands<double>(PARAM.globalv.global_readin_dir + "band_out",
                                                   ...
                                                   nvirt_in,
                                                   ncore);
        std::cout << "ncore=" << ncore << ", nocc=" << nocc_in << ", nvirt=" << nvirt_in << ", nbands=" << this->nbands
                  << std::endl;
