...
inline Velocity_op<std::complex<double>>
    get_velocity_matrix_R (const UnitCell& ucell,
...
inline double
    lorentz_delta (const double dfreq_au, const double eta_au)
{
    return eta_au / (dfreq_au * dfreq_au + eta_au * eta_au) / M_PI;
}
inline double
    gauss_delta (const double dfreq_au, const double eta_au)
{
...
template <typename T>
inline ModuleBase::Vector3<T> convert_vector_to_vector3 (const std::vector<std::complex<double>>& vec);
template <>
inline ModuleBase::Vector3<double>
    convert_vector_to_vector3 (const std::vector<std::complex<double>>& vec)
{
    assert (vec.size () == 3);
    return ModuleBase::Vector3<double> (vec[0].real (), vec[1].real (), vec[2].real ());
}
template <>
inline ModuleBase::Vector3<std::complex<double>>
    convert_vector_to_vector3 (const std::vector<std::complex<double>>& vec)
{
    assert (vec.size () == 3);
...
template <typename T>
ModuleBase::Vector3<T>
    LR::LR_Spectrum<T>::cal_transition_dipole_istate_velocity_R (const int istate,
    ...
    for (int i = 0; i < 3; i++)
        {
            for (int is = 0; is < this->nspin_x; ++is)
                {
                    trans_dipole[i] += LR_Util::dot_R_matrix (*vR.get_current_term_pointer (i),
                                                              *DM_trans.get_DMR_pointer (is + 1),
                                                              ucell.nat)
                                       * fac;
...
template <typename T>
ModuleBase::Vector3<T>
    LR::LR_Spectrum<T>::cal_transition_dipole_istate_velocity_k (const int istate,
                                                                 const Velocity_op<std::complex<double>>& vR)
{
    // transition density matrix D(R)
                    ...
                    for (int ik = 0; ik < nk; ++ik)
                        {
                            ...
                            hamilt::folding_HR (*vR.get_current_term_pointer (i),
                                                vk.data (),
                                                kv.kvec_d[ik],
                                                pmat.get_row_size (),
                                                1);
                            trans_dipole[i] += std::inner_product (vk.begin (),
                                                                   vk.end (),
                                                                   DM_trans.get_DMK_pointer (is * nk + ik),
...
template <typename T>
void
    LR::LR_Spectrum<T>::cal_transition_dipoles_velocity ()
{
    const Velocity_op<std::complex<double>>& vR
        = get_velocity_matrix_R (ucell, gd_, pmat, two_center_bundle_); // velocity matrix v(R)
...
template <typename T>
void
    LR::LR_Spectrum<T>::optical_absorption_method2 (const std::vector<double>& freq, const double eta)
{
    ...
    for (int f = 0; f < freq.size (); ++f)
        {
            ...
            for (int i = 0; i < nstate; ++i)
                {
                    abs_value += this->mean_squared_transition_dipole_[i]
                                 * lorentz_delta ((freq[f] - eig[i]) / ModuleBase::e2,
                                                  eta / ModuleBase::e2); // e2: Ry to Hartree
                }
            abs_value *= fac;
            if (GlobalV::MY_RANK == 0)
                {
                    ofs << freq[f] * ModuleBase::Ry_to_eV << "\t" << 91.126664 / freq[f] << "\t" << abs_value
                        << std::endl;
                }
        }
}
...
inline void
    cal_eig_ks_diff (double* const eig_ks_diff,
                    ...
                    for (int iv = 0; iv < px.get_row_size (); ++iv) // nvirt_local
                        {
                            ...
                            eig_ks_diff[ik * px.get_local_size () + io * px.get_row_size () + iv]
                                = (eig_ks[start_k + nocc + iv_g] - eig_ks[start_k + io_g])
                                  / ModuleBase::e2; // eV to Hartree
                        }
                }
        }
}
...
template <typename T>
void
    LR::LR_Spectrum<T>::test_transition_dipoles_velocity_ks (const double* const ks_eig)
{
    // velocity matrix v(R)
    ...
    for (int is = 0; is < this->nspin_x; ++is)
        {
            cal_eig_ks_diff (eig_ks_diff.data () + is * nk * pX[0].get_local_size (),
                             ks_eig,
                             pX[is],
                             nk,
                             nocc[is],
    ...
    for (int istate = 0; istate < nstate; ++istate)
        {
            ...
            const elecstate::DensityMatrix<T, T>& DM_trans
                = this->cal_transition_density_matrix (istate, X_div_ks_eig.data ());
            std::vector<std::complex<double>> tmp_trans_dipole (3, 0.0);
            for (int i = 0; i < 3; i++)
                {
                    for (int is = 0; is < this->nspin_x; ++is)
                        {
                            tmp_trans_dipole[i] += LR_Util::dot_R_matrix (*vR.get_current_term_pointer (i),
                                                                          *DM_trans.get_DMR_pointer (is + 1),
                                                                          ucell.nat)
