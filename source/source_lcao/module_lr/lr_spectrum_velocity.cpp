...
template <>
inline ModuleBase::Vector3<double> convert_vector_to_vector3(const std::vector<std::complex<double>>& vec) {
    assert(vec.size() == 3);
...
LR::LR_Spectrum<T>::cal_transition_dipole_istate_velocity_R(const int istate,
    ...
    for (int i = 0; i < 3; i++) {
        for (int is = 0; is < this->nspin_x; ++is) {
            trans_dipole[i] +=
                LR_Util::dot_R_matrix(*vR.get_current_term_pointer(i), *DM_trans.get_DMR_pointer(is + 1), ucell.nat) *
                fac;
    ...
    for (int f = 0; f < freq.size(); ++f) {
        ...
        for (int i = 0; i < nstate; ++i) {
            abs_value += this->mean_squared_transition_dipole_[i] *
                         lorentz_delta((freq[f] - eig[i]) / ModuleBase::e2, eta / ModuleBase::e2); // e2: Ry to Hartree
        }
        abs_value *= fac;
        if (GlobalV::MY_RANK == 0) {
            ofs << freq[f] * ModuleBase::Ry_to_eV << "\t" << 91.126664 / freq[f] << "\t" << abs_value << std::endl;
        }
    }
}

inline void cal_eig_ks_diff(double* const eig_ks_diff,
        ...
        for (int io = 0; io < px.get_col_size(); ++io) // nocc_local
        {
            for (int iv = 0; iv < px.get_row_size(); ++iv) // nvirt_local
            {
                ...
                eig_ks_diff[ik * px.get_local_size() + io * px.get_row_size() + iv] =
                    (eig_ks[start_k + nocc + iv_g] - eig_ks[start_k + io_g]) / ModuleBase::e2; // eV to Hartree
            }
        }
    }
}
