                    ...
                    for (int b = 0; b < nv; ++b)
                        {                               // calculate A^{ai} for each bj
                            ...
                            if (px.in_this_processor (b, j))
                                {
                                    X_bj (0, 0, ik * px.get_local_size () + lj * px.get_row_size () + lb)
                                        = this->one ();
                                }
                            psi::Psi<T> A_aibj (1,
                                                1,
                            ...
                            for (int ik_ai = 0; ik_ai < this->nk; ++ik_ai)
                                {
                                    LR_Util::gather_2d_to_full (
                                        ...
                                        Amat_full.data ()
                                            + kbj * this->nk * npairs /*col, bj*/ + ik_ai * npairs /*row, ai*/,
                                        false,
                                        nv,
                                        no);
                                }
...
template <>
double
    HamiltLR<double>::one () const
{
    return 1.0;
}
template <>
std::complex<double>
    HamiltLR<std::complex<double>>::one () const
{
