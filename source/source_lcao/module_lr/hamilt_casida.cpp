...
std::vector<T> HamiltLR<T>::matrix() const {
    ...
    for (int ik = 0; ik < this->nk; ++ik) {
        for (int j = 0; j < no; ++j) {
            for (int b = 0; b < nv; ++b) {  // calculate A^{ai} for each bj
                ...
                psi::Psi<T> X_bj(1,
                                 ...
                                 this->nk * px.get_local_size(),
                                 true); // k1-first, like in iterative solver
                X_bj.zero_out();
                ...
                if (px.in_this_processor(b, j)) {
                    X_bj(0, 0, ik * px.get_local_size() + lj * px.get_row_size() + lb) = this->one();
                }
                psi::Psi<T> A_aibj(1,
                                   1,
                                   this->nk * px.get_local_size(),
                ...
                while (node != nullptr) { // act() on and return the k1-first type of psi
                ...
                for (int ik_ai = 0; ik_ai < this->nk; ++ik_ai) {
                    LR_Util::gather_2d_to_full(px,
                                               &A_aibj.get_pointer()[ik_ai * px.get_local_size()],
                                               Amat_full.data() + kbj * this->nk * npairs /*col, bj*/ +
                                                   ik_ai * npairs /*row, ai*/,
                                               false,
                                               nv,
                                               no);
...
template <>
double HamiltLR<double>::one() const {
    return 1.0;
}
template <>
std::complex<double> HamiltLR<std::complex<double>>::one() const {
