...
class OperatorLRDiag : public hamilt::Operator<T, Device> {
  public:
    OperatorLRDiag(const double* eig_ks,
                   const Parallel_2D& pX_in,
                   const int& nk_in,
                   const int& nocc_in,
                   const int& nvirt_in)
        : pX(pX_in), nk(nk_in), nocc(nocc_in), nvirt(nvirt_in) { // calculate the difference of eigenvalues
        ...
        for (int ik = 0; ik < nk; ++ik) {
            ...
            for (int io = 0; io < pX.get_col_size(); ++io) // nocc_local
            {
                for (int iv = 0; iv < pX.get_row_size(); ++iv) // nvirt_local
                {
                    ...
                    this->eig_ks_diff(ik, io * pX.get_row_size() + iv) =
                        eig_ks[istart + nocc + iv_g] - eig_ks[istart + io_g];
                }
            }
        }
    };
