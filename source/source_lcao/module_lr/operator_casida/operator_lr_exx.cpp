...
template <typename T>
void
    OperatorLREXX<T>::allocate_Ds_onebase ()
{
                    ...
                    for (auto cell: this->BvK_cells)
                        {
                            this->Ds_onebase[iat1][std::make_pair (iat2, cell)]
                                = aims_nbasis.empty () ? RI::Tensor<T> ({static_cast<size_t> (ucell.atoms[it1].nw),
                                                                         static_cast<size_t> (ucell.atoms[it2].nw)})
                                                       : RI::Tensor<T> ({static_cast<size_t> (aims_nbasis[it1]),
                                                                         static_cast<size_t> (aims_nbasis[it2])});
                        }
                }
        }
}
...
template <>
void
    OperatorLREXX<double>::cal_DM_onebase (const int io, const int iv, const int ik) const
{
                                    ...
                                    for (int iw2 = 0; iw2 < nw2; ++iw2)
                                        {
                                            ...
                                            if (this->pmat.in_this_processor (iwt1, iwt2))
                                                D2d (iw1, iw2) = this->psi_ks_full (ik, io, iwt1)
                                                                 * this->psi_ks_full (ik, nocc + iv, iwt2);
                                        }
                            }
        }
}
...
template <>
void
    OperatorLREXX<std::complex<double>>::cal_DM_onebase (const int io, const int iv, const int ik) const
{
    ModuleBase::TITLE ("OperatorLREXX", "cal_DM_onebase");
    // NOTICE: DM_onebase will be passed into `cal_energy` interface and conjugated by "zdotc".
    // So the formula should be the same as RHS. instead of LHS of the A-matrix,
    ...
    for (auto cell: this->BvK_cells)
        {
            std::complex<double> frac = RI::Global_Func::convert<std::complex<double>> (
                std::exp (-ModuleBase::TWO_PI * ModuleBase::IMAG_UNIT
                          * (this->kv.kvec_c.at (ik) * (RI_Util::array3_to_Vector3 (cell) * ucell.latvec))));
            for (int it1 = 0; it1 < ucell.ntype; ++it1)
                for (int ia1 = 0; ia1 < ucell.atoms[it1].na; ++ia1)
                    for (int it2 = 0; it2 < ucell.ntype; ++it2)
                        for (int ia2 = 0; ia2 < ucell.atoms[it2].na; ++ia2)
                                    ...
                                    for (int iw2 = 0; iw2 < nw2; ++iw2)
                                        {
                                            ...
                                            const int iwt2 = ucell.itiaiw2iwt (it2, ia2, iw2);
                                            if (this->pmat.in_this_processor (iwt1, iwt2))
                                                D2d (iw1, iw2) = frac * std::conj (this->psi_ks_full (ik, io, iwt2))
                                                                 * this->psi_ks_full (ik, nocc + iv, iwt1);
                                        }
                            }
        }
}
...
template <typename T>
void
    OperatorLREXX<T>::act (const int nbands,
                           const int nbasis,
                           const int npol,
                           const T* psi_in,
                           T* hpsi,
                           ...
                           const bool is_first_node) const
{
    ...
    lri->exx_lri.cal_Hs ();
    lri->Hexxs[0] = RI::Communicate_Tensors_Map_Judge::comm_map2_first (lri->mpi_comm,
                                                                        std::move (lri->exx_lri.Hs),
                                                                        std::get<0> (judge[0]),
                                                                        std::get<1> (judge[0]));
                    ...
                    for (int ik = 0; ik < nk; ++ik)
                        {
                            ...
                            this->cal_DM_onebase (io,
                                                  iv,
                                                  ik); // set Ds_onebase for all e-h pairs (not only on this processor)
                            // LR_Util::print_CV(Ds_onebase, "Ds_onebase of occ " + std::to_string(io) + ", virtual " +
                            // std::to_string(iv) + " in OperatorLREXX", 1e-10);
                            const T& ene = 2 * alpha
                                           * // minus for exchange(but here plus is right, why?), 2 for Hartree to Ry
                                           lri->exx_lri.post_2D.cal_energy (this->Ds_onebase, lri->Hexxs[0]);
                            if (this->pX.in_this_processor (iv, io))
                                {
                                    hpsi[xstart_bk + this->pX.global2local_col (io) * this->pX.get_row_size ()
                                         + this->pX.global2local_row (iv)] += ene;
                                }
                        }
                }
        }
