        ...
        for (int iat2 = 0; iat2 < ucell.nat; ++iat2) {
            ...
            for (auto cell: this->BvK_cells) {
                this->Ds_onebase[iat1][std::make_pair(iat2, cell)] =
                    aims_nbasis.empty()
                        ? RI::Tensor<T>(
                              {static_cast<size_t>(ucell.atoms[it1].nw), static_cast<size_t>(ucell.atoms[it2].nw)})
                        ...
                        for (int iw1 = 0; iw1 < nw1; ++iw1)
                            for (int iw2 = 0; iw2 < nw2; ++iw2) {
                                ...
                                if (this->pmat.in_this_processor(iwt1, iwt2))
                                    D2d(iw1, iw2) =
                                        this->psi_ks_full(ik, io, iwt1) * this->psi_ks_full(ik, nocc + iv, iwt2);
                            }
                    }
    }
}
...
void OperatorLREXX<T>::act(const int nbands,
    ...
    std::vector<std::map<TA, std::map<TAC, RI::Tensor<T>>>> Ds_trans =
        aims_nbasis.empty() ? RI_2D_Comm::split_m2D_ktoR<T>(ucell, this->kv, DMk_trans_pointer, this->pmat, 1)
