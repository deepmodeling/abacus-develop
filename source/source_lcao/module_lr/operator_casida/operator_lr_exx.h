  ...
  public:
    OperatorLREXX (const int& nspin,
        ...
        : nspin (nspin), naos (naos), nocc (nocc), nvirt (nvirt), nk (kv_in.get_nks () / nspin), psi_ks (psi_ks_in),
          DM_trans (DM_trans_in), exx_lri (exx_lri_in), kv (kv_in), pX (pX_in), pc (pc_in), pmat (pmat_in),
          ucell (ucell_in), alpha (alpha), aims_nbasis (aims_nbasis)
    {
        ...
        for (int ik = 0; ik < nk; ++ik)
            {
                LR_Util::gather_2d_to_full (this->pc,
                                            &this->psi_ks (ik, 0, 0),
                                            &this->psi_ks_full (ik, 0, 0),
                                            false,
                                            this->naos,
                                            nocc + nvirt);
            }
