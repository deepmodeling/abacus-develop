...
template <typename T>
elecstate::DensityMatrix<T, T>
    LR::LR_Spectrum<T>::cal_transition_density_matrix (const int istate, const T* X_in, const bool need_R)
{
    ...
    for (int is = 0; is < this->nspin_x; ++is)
        {
...
#ifdef __MPI
            std::vector<container::Tensor> dm_trans_2d = cal_dm_trans_pblas (X + offset_x,
                                                                             this->pX[is],
                                                                             psi_ks[is],
                                                                             this->pc,
                                                                             this->naos,
            ...
            std::vector<container::Tensor> dm_trans_2d
                = cal_dm_trans_blas (X + offset_x, this->psi_ks[is], this->nocc[is], this->nvirt[is], (T)1.0 / (T)nk);
...
inline void
    check_sum_rule (const double& osc_tot)
{
...
ModuleBase::Vector3<double>
    LR::LR_Spectrum<double>::cal_transition_dipole_istate_length (const int istate)
{
            ...
            for (int ir = 0; ir < rho_basis.nrxx; ++ir)
                {
                    ...
                    ModuleBase::Vector3<double> rd (static_cast<double> (i) / rho_basis.nx,
                                                    static_cast<double> (j) / rho_basis.ny,
                                                    static_cast<double> (k) / rho_basis.nz); //+1/2 better?
    ...
    if (this->nspin_x == 1)
        {
            trans_dipole *= sqrt (2.0);
        } // *2 for 2 spins, /sqrt(2) for the halfed dimension of X in the normalizaiton
...
template <>
ModuleBase::Vector3<std::complex<double>>
    LR::LR_Spectrum<std::complex<double>>::cal_transition_dipole_istate_length (const int istate)
{

    // 1. transition density matrix
    ModuleBase::Vector3<std::complex<double>> trans_dipole (0.0, 0.0, 0.0);
    const elecstate::DensityMatrix<std::complex<double>, std::complex<double>>& DM_trans
        = this->cal_transition_density_matrix (istate);
    for (int is = 0; is < this->nspin_x; ++is)
        {
            // 2. transition density
            ...
            elecstate::DensityMatrix<std::complex<double>, double> DM_trans_real_imag (&this->pmat,
                                                                                       1,
                                                                                       this->kv.kvec_d,
                                                                                       this->nk);
...
template <>
double
    LR::LR_Spectrum<double>::cal_mean_squared_dipole (ModuleBase::Vector3<double> dipole)
{
    return dipole.norm2 () / 3.;
}
template <>
double
    LR::LR_Spectrum<std::complex<double>>::cal_mean_squared_dipole (ModuleBase::Vector3<std::complex<double>> dipole)
{
    // return dipole.norm2().real() / 3.;       // ModuleBase::Vector3::norm2 calculates x*x + y*y + z*z, but here we
    // need x*x.conj() + y*y.conj() + z*z.conj()
    return (std::norm (dipole.x) + std::norm (dipole.y) + std::norm (dipole.z)) / 3.;
}

template <typename T>
void
    LR::LR_Spectrum<T>::cal_transition_dipoles_length ()
{
...
template <typename T>
void
    LR::LR_Spectrum<T>::oscillator_strength ()
{
...
template <typename T>
void
    LR::LR_Spectrum<T>::optical_absorption_method1 (const std::vector<double>& freq, const double eta)
{
    // ============test dipole================
    ...
    for (int f = 0; f < freq.size (); ++f)
        {
            ...
            double abs = 0.0;
            // for (int i = 0;i < osc.size();++i) { abs += (osc[i] / (f_complex * f_complex - eig[i] * eig[i])).imag() *
            // freq[f] * FourPI_div_c; }
            for (int i = 0; i < osc.size (); ++i)
                {
                    abs += (osc[i] / (f_complex * f_complex - eig[i] * eig[i])).imag () * fac;
                }
            if (GlobalV::MY_RANK == 0)
                {
                    ofs << freq[f] * ModuleBase::Ry_to_eV << "\t" << 91.126664 / freq[f] << "\t" << std::abs (abs)
                        << std::endl;
                }
        }
    ofs.close ();
...
template <typename T>
void
    LR::LR_Spectrum<T>::transition_analysis (const std::string& spintype)
{
    ...
    ofs << "==================================================================== " << std::endl;
    ofs << std::setw (8) << "State" << std::setw (30) << "Excitation Energy (Ry, eV)" << std::setw (90)
        << "Transition dipole x, y, z (a.u.)" << std::setw (30) << "Oscillator strength(a.u.)" << std::endl;
    ofs << "------------------------------------------------------------------------------------ " << std::endl;
    for (int istate = 0; istate < nstate; ++istate)
        ofs << std::setw (8) << istate << std::setw (15) << std::setprecision (6) << eig[istate] << std::setw (15)
            << eig[istate] * ModuleBase::Ry_to_eV << std::setprecision (4) << std::setw (30)
            << transition_dipole_[istate].x << std::setw (30) << transition_dipole_[istate].y << std::setw (30)
            << transition_dipole_[istate].z << std::setprecision (6) << std::setw (30) << oscillator_strength_[istate]
            << std::endl;
    ofs << "------------------------------------------------------------------------------------ " << std::endl;
    ofs << std::setw (8) << "State" << std::setw (20) << "Occupied orbital" << std::setw (20) << "Virtual orbital"
        << std::setw (30) << "Excitation amplitude" << std::setw (30) << "Excitation rate" << std::setw (10)
        << "k-point" << std::endl;
    ofs << "------------------------------------------------------------------------------------ " << std::endl;
    for (int istate = 0; istate < nstate; ++istate)
        {
            ...
            for (int i = 0; i < gdim; ++i)
                {
                    double abs = std::abs (X_full.at (i));
                    if (abs > ana_thr)
                        {
                            abs_order[abs] = i;
                        }
                    ...
                    for (auto it = abs_order.cbegin (); it != abs_order.cend (); ++it)
                        {
                            ...
                            ofs << std::setw (8) << (it == abs_order.cbegin () ? std::to_string (istate) : " ")
                                << std::setw (20) << std::to_string (pair_info["iocc"] + 1) + s << std::setw (20)
                                << std::to_string (pair_info["ivirt"] + nocc[is] + 1) + s // iocc and ivirt
                                << std::setw (30) << X_full.at (it->second) << std::setw (30)
                                << std::norm (X_full.at (it->second)) << std::setw (10) << pair_info["ik"] + 1
                                << std::endl;
                        }
                }
        }
...
std::map<std::string, int>
    LR::LR_Spectrum<T>::get_pair_info (const int i)
{
    assert (i >= 0 && i < gdim);
...
template <typename T>
void
    LR::LR_Spectrum<T>::write_transition_dipole (const std::string& filename)
{
    std::ofstream ofs (filename);
    ofs << "Transition dipole moment (a.u.)" << std::endl;
    ofs << std::setw (20) << "State" << std::setw (20) << "x" << std::setw (20) << "y" << std::setw (20) << "z"
        << std::setw (20) << "average" << std::endl;
    for (int istate = 0; istate < nstate; ++istate)
        {
            ofs << std::setw (20) << istate << std::setw (20) << transition_dipole_[istate].x << std::setw (20)
                << transition_dipole_[istate].y << std::setw (20) << transition_dipole_[istate].z << std::setw (20)
