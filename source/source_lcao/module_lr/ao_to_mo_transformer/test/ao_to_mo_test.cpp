...
class AO2MOTest : public testing::Test
{
  public:
        ...
        set_rand (std::complex<double>* data, int size)
    {
        for (int i = 0; i < size; ++i)
            {
                data[i] = std::complex<double> (rand01 * 10, rand01 * 10);
            }
    };
    void
            ...
            for (int istate = 0; istate < nstate; ++istate)
                {
                    ...
                    for (auto& v: V)
                        {
                            set_rand (v.data<std::complex<double>> (), size_v);
                        }
                    LR::ao_to_mo_forloop_serial (V, c, s.nocc, s.nvirt, &vo_for (istate, 0, 0));
                    LR::ao_to_mo_blas (V, c, s.nocc, s.nvirt, &vo_blas (istate, 0, 0), false);
                    LR::ao_to_mo_forloop_serial (V, c, s.nocc, s.nvirt, &oo_for (istate, 0, 0), LR::MO_TYPE::OO);
    ...
    for (auto s: this->sizes)
        {
            ...
            std::vector<container::Tensor> V (
                s.nks,
                container::Tensor (DAT::DT_DOUBLE, DEV::CpuDevice, {pV.get_col_size (), pV.get_row_size ()}));
            Parallel_2D pc;
            ...
            for (int istate = 0; istate < nstate; ++istate)
                {
                    ...
                    LR::ao_to_mo_pblas (V,
                                        ...
                                        s.nvirt,
                                        pvv,
                                        &vv_pblas_loc (istate, 0, 0),
                                        false,
                                        LR::MO_TYPE::VV);
                    // gather AX and output
                    for (int isk = 0; isk < s.nks; ++isk)
                        {
                            LR_Util::gather_2d_to_full (pvo,
                                                        &vo_pblas_loc (istate, isk, 0),
                                                        &vo_gather (istate, isk, 0),
                                                        false /*pblas: row first*/,
                                                        s.nvirt,
                                                        s.nocc);
                            LR_Util::gather_2d_to_full (poo,
                                                        &oo_pblas_loc (istate, isk, 0),
                                                        &oo_gather (istate, isk, 0),
                                                        false /*pblas: row first*/,
                                                        s.nocc,
                                                        s.nocc);
                            LR_Util::gather_2d_to_full (pvv,
                                                        &vv_pblas_loc (istate, isk, 0),
                                                        &vv_gather (istate, isk, 0),
                                                        false /*pblas: row first*/,
                                                        s.nvirt,
                                                        s.nvirt);
                        }
                    // compare to global AX
                    std::vector<container::Tensor> V_full (
                        s.nks,
                        container::Tensor (DAT::DT_DOUBLE, DEV::CpuDevice, {s.naos, s.naos}));
                    ...
                    for (int isk = 0; isk < s.nks; ++isk)
                        {
                            LR_Util::gather_2d_to_full (pV,
                                                        V.at (isk).data<double> (),
                                                        V_full.at (isk).data<double> (),
                                                        false,
                                                        s.naos,
                                                        s.naos);
                    ...
                    if (my_rank == 0)
                        {
                            ...
                            LR::ao_to_mo_blas (V_full,
                                               c_full,
                                               s.nocc,
                                               s.nvirt,
                                               &oo_full_istate (0, 0, 0),
                                               false,
                                               LR::MO_TYPE::OO);
    ...
    for (auto s: this->sizes)
        {
            ...
            std::vector<container::Tensor> V (
                s.nks,
                container::Tensor (DAT::DT_COMPLEX_DOUBLE, DEV::CpuDevice, {pV.get_col_size (), pV.get_row_size ()}));
            Parallel_2D pc;
            LR_Util::setup_2d_division (pc, s.nb, s.naos, s.nocc + s.nvirt, pV.blacs_ctxt);
            ...
            for (int istate = 0; istate < nstate; ++istate)
                {
                    ...
                    for (int isk = 0; isk < s.nks; ++isk)
                        {
                            LR_Util::gather_2d_to_full (pV,
                                                        V.at (isk).data<std::complex<double>> (),
                                                        V_full.at (isk).data<std::complex<double>> (),
                                                        false,
                                                        s.naos,
                                                        s.naos);
                            LR_Util::gather_2d_to_full (pc,
                    ...
                    if (my_rank == 0)
                        {
                            psi::Psi<std::complex<double>> vo_full_istate (s.nks,
                                                                           1,
                                                                           s.nocc * s.nvirt,
                                                                           s.nocc * s.nvirt,
                                                                           false);
                            LR::ao_to_mo_blas (V_full, c_full, s.nocc, s.nvirt, &vo_full_istate (0, 0, 0), false);
                            check_eq (&vo_full_istate (0, 0, 0), &vo_gather (istate, 0, 0), s.nks * s.nocc * s.nvirt);
                            psi::Psi<std::complex<double>> oo_full_istate (s.nks,
                                                                           1,
                                                                           s.nocc * s.nocc,
                                                                           s.nocc * s.nvirt,
                                                                           false);
                            LR::ao_to_mo_blas (V_full,
                                               ...
                                               LR::MO_TYPE::OO);
                            check_eq (&oo_full_istate (0, 0, 0), &oo_gather (istate, 0, 0), s.nks * s.nocc * s.nocc);
                            psi::Psi<std::complex<double>> vv_full_istate (s.nks,
                                                                           1,
                                                                           s.nvirt * s.nvirt,
                                                                           s.nocc * s.nvirt,
                                                                           false);
