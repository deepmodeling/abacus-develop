...
template <>
void
    ao_to_mo_forloop_serial (const std::vector<container::Tensor>& mat_ao,
                            ...
                            for (int nu = 0; nu < naos; ++nu)
                                {
                                    for (int mu = 0; mu < naos; ++mu)
                                        {
                                            mat_mo[start + p * nmo2 + q]
                                                += coeff (imo2 + q, mu) * mat_ao[isk].data<double> ()[nu * naos + mu]
                                                   * coeff (imo1 + p, nu);
                                        }
                                }
                        }
                            ...
                            for (int nu = 0; nu < naos; ++nu)
                                {
                                    for (int mu = 0; mu < naos; ++mu)
                                        {
                                            mat_mo[start + p * nmo2 + q]
                                                += std::conj (coeff (imo2 + q, mu))
                                                   * mat_ao[isk].data<std::complex<double>> ()[nu * naos + mu]
                                                   * coeff (imo1 + p, nu);
                                        }
                                }
                        }
...
template <>
void
    ao_to_mo_blas (const std::vector<container::Tensor>& mat_ao,
                   const psi::Psi<double>& coeff,
                   const int& nocc,
                   const int& nvirt,
                   double* mat_mo,
    ...
    for (int isk = 0; isk < nks; ++isk)
        {
            ...
            BlasConnector::gemm (transb,
                                 ...
                                 naos,
                                 alpha,
                                 coeff.get_pointer (imo1),
                                 naos,
                                 mat_ao[isk].data<double> (),
                                 naos,
                                 beta,
                                 Vc.data<double> (),
                                 naos);
    ...
    for (int isk = 0; isk < nks; ++isk)
        {
            ...
            const std::complex<double> beta
                = add_on ? std::complex<double> (1.0, 0.0) : std::complex<double> (0.0, 0.0);
            BlasConnector::gemm (transb,
                                 transa,
                                 nmo1,
                                 naos,
                                 ...
                                 coeff.get_pointer (imo1),
                                 naos,
                                 mat_ao[isk].data<std::complex<double>> (),
                                 naos,
                                 beta,
                                 Vc.data<std::complex<double>> (),
                                 naos);
