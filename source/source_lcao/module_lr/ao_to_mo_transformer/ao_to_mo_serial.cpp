...
namespace LR {
template <>
void ao_to_mo_forloop_serial(const std::vector<container::Tensor>& mat_ao,
                ...
                for (int nu = 0; nu < naos; ++nu) {
                    for (int mu = 0; mu < naos; ++mu) {
                        mat_mo[start + p * nmo2 + q] +=
                            coeff(imo2 + q, mu) * mat_ao[isk].data<double>()[nu * naos + mu] * coeff(imo1 + p, nu);
                    }
                }
            }
        }
    ...
    for (int isk = 0; isk < nks; ++isk) {
        ...
        for (int p = 0; p < nmo1; ++p) {
            for (int q = 0; q < nmo2; ++q) {
                for (int nu = 0; nu < naos; ++nu) {
                    for (int mu = 0; mu < naos; ++mu) {
                        mat_mo[start + p * nmo2 + q] += std::conj(coeff(imo2 + q, mu)) *
...
template <>
void ao_to_mo_blas(const std::vector<container::Tensor>& mat_ao,
                   const psi::Psi<double>& coeff,
                   const int& nocc,
                   const int& nvirt,
                   double* mat_mo,
