  ...
  public:
    LR_Spectrum (const int& nspin_global,
        ...
        : nspin_x (openshell ? 2 : 1), naos (naos), nocc (nocc), nvirt (nvirt), nk (kv_in.get_nks () / nspin_global),
          rho_basis (rho_basis), ucell (ucell), kv (kv_in), gd_ (gd), orb_cutoff_ (orb_cutoff),
          two_center_bundle_ (two_center_bundle_), pX (pX_in), pc (pc_in), pmat (pmat_in), eig (eig), X (X),
          nstate (nstate), ldim (nk
                                 * (nspin_x == 2 ? pX_in[0].get_local_size () + pX_in[1].get_local_size ()
                                                 : pX_in[0].get_local_size ())),
    ...
    //======================================================================================================
  private:
    /// $$2/3\Omega\sum_{ia\sigma} |\braket{\psi_{i}|\mathbf{r}|\psi_{a}} |^2\int \rho_{\alpha\beta}(\mathbf{r})
    /// \mathbf{r} d\mathbf{r}$$
    ...
    ModuleBase::Vector3<T> cal_transition_dipole_istate_velocity_R (const int istate,
                                                                    const Velocity_op<std::complex<double>>& vR);
    ModuleBase::Vector3<T> cal_transition_dipole_istate_velocity_k (const int istate,
                                                                    const Velocity_op<std::complex<double>>& vR);
    ...
    elecstate::DensityMatrix<T, T>
        cal_transition_density_matrix (const int istate, const T* X_in = nullptr, const bool need_R = true);
    ...
    std::vector<double> oscillator_strength_; ///< $2/3\Omega |\sum_{ia\sigma} \braket{\psi_{i}|\mathbf{r}|\psi_{a}}
                                              ///< |^2$, atomic unit (Hartree)
};
} // namespace LR
