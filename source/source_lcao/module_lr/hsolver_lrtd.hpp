...
template <typename T>
inline void
    print_eigs (const std::vector<T>& eigs, const std::string& label = "", const double factor = 1.0)
{
            ...
            if (method == "dav")
                {
                    ...
                    hsolver::DiagoDavid<T> david (precondition.data (), nband, dim, PARAM.inp.pw_diag_ndim, comm_info);
                    std::vector<double> ethr_band (nband, diag_ethr);
                    hsolver::DiagoIterAssist<T>::avg_iter += static_cast<double> (david.diag (hpsi_func,
                                                                                              spsi_func,
                                                                                              dim,
                                                                                              psi,
                                                                                              eigenvalue.data (),
            ...
            else if (method == "dav_subspace") // need refactor
                {
                    ...
                    hsolver::DiagoIterAssist<T>::avg_iter += static_cast<double> (
                        dav_subspace
                            .diag (hpsi_func, spsi_func, psi, dim, eigenvalue.data (), ethr_band, false /*scf*/));
                }
            else if (method == "cg")
                {
                    ...
                    // hsolver::DiagoCG<T> cg("lcao", "nscf", true, subspace_func, diag_ethr, maxiter,
                    // GlobalV::NPROC_IN_POOL);

                    auto subspace_func
                        = [] (T* psi_in, T* psi_out, const int ld_psi, const int nband, const bool S_orth) {};
                    hsolver::DiagoCG<T>
                        cg ("lcao", "nscf", false, subspace_func, diag_ethr, maxiter, GlobalV::NPROC_IN_POOL);
                    ...
                    auto spsi_func = [] (T* psi_in, T* spsi, const int ld_psi, const int nvec)
                        {
                            std::memcpy (spsi,
                                         psi_in,
                                         sizeof (T) * static_cast<size_t> (ld_psi) * static_cast<size_t> (nvec));
                        };
