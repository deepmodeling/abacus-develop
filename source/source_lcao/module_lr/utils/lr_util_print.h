...
template <typename T>
constexpr T
    filter (const T& v, const double& threshold = 1e-10)
{
...
template <typename T>
int
    read_value (std::ifstream& ifs, T* ptr, const int& size)
{
...
int
    read_value (std::ifstream& ifs, T* ptr, const int& size, Args&&... args)
{
    int size_now = 0;
    for (int i = 0; i < size; ++i)
        {
            size_now += read_value (ifs, ptr + size_now, args...);
        }
    return size_now;
}
template <typename T, typename... Args>
int
    read_value (const std::string& file, T* ptr, const int& size, Args&&... args)
{
...
template <typename T>
int
    write_value (std::ofstream& ofs, const T* ptr, const int& size)
{
    for (int i = 0; i < size; ++i)
        {
            ofs << filter (ptr[i]) << " ";
...
template <typename T, typename... Args>
int
    write_value (std::ofstream& ofs, const T* ptr, const int& size, Args&&... args)
{
    int size_now = 0;
    for (int i = 0; i < size; ++i)
        {
            size_now += write_value (ofs, ptr + size_now, args...);
        }
    ofs << std::endl;
    return size_now;
}
template <typename T, typename... Args>
int
    write_value (const std::string& file, const int& prec, const T* ptr, const int& size, Args&&... args)
{
...
template <typename T>
int
    print_value (const T* ptr, const int& size)
{
    for (int i = 0; i < size; ++i)
        {
            std::cout << filter (ptr[i]) << " ";
        }
    std::cout << std::endl;
    return size;
}
template <typename T, typename... Args>
int
    print_value (const T* ptr, const int& size, Args&&... args)
{
    int size_now = 0;
    for (int i = 0; i < size; ++i)
        {
            size_now += print_value (ptr + size_now, args...);
        }
    std::cout << std::endl;
    return size_now;
}
...
template <typename T>
void
    print_psi_bandfirst (const psi::Psi<T>& psi,
...
template <typename T>
void
    write_psi_bandfirst (const psi::Psi<T>& psi,
...
template <typename T>
void
    write_psi_bandfirst (const T* psi,
                         const int& nband,
                         const int& nk,
                         const int& nbasis,
                         const std::string& filename,
...
template <typename T>
psi::Psi<T>
    read_psi_bandfirst (const std::string& filename, const int& rank)
{
...
template <typename T>
void
    print_psi_kfirst (const psi::Psi<T>& psi, const std::string& label, const double& threshold = 1e-10)
{
...
template <typename T>
void
    print_tensor (const container::Tensor& t,
                  const std::string& label,
...
template <typename T>
void
    print_CsX (const TLRIX<T>& CsX, const int nvirt, const std::string& label, const double& threshold = 1e-10)
{
    std::cout << label << "\n";
...
template <typename T>
void
    print_CV (const TLRI<T>& Cs, const std::string& label, const double& threshold = 1e-10)
{
    std::cout << label << "\n";
