...
void
    print_HR (const hamilt::HContainer<TR>& HR,
              const int& nat,
                    ...
                    for (int iR = 0; iR < ap->get_R_size (); ++iR)
                        {
                            std::cout << "atom pair (" << ia << ", " << ja << "),  "
                                      << "R=(" << ap->get_R_index (iR)[0] << ", " << ap->get_R_index (iR)[1] << ", "
                                      << ap->get_R_index (iR)[2] << "): \n";
...
void
    print_DMR (const elecstate::DensityMatrix<TK, TR>& DMR,
               const int& nat,
               const std::string& label,
               const double& threshold = 1e-10)
{
...
void
    initialize_HR (hamilt::HContainer<TR>& hR,
            ...
            for (int ad = 0; ad < adjs.adj_num + 1; ++ad)
                {
                    ...
                    if (pmat.get_row_size (iat1) <= 0 || pmat.get_col_size (iat2) <= 0)
                        {
                            continue;
                        }
...
template <typename T, typename TR>
void
    initialize_DMR (elecstate::DensityMatrix<T, TR>& dm,
...
TR1
    dot_R_matrix (const hamilt::HContainer<TR1>& h1, const hamilt::HContainer<TR2>& h2, const int& nat)
{
                    ...
                    for (int iR = 0; iR < ap1->get_R_size (); ++iR)
                        {
                            ...
                            sum += std::inner_product (mat1.get_pointer (),
                                                       mat1.get_pointer () + mat1.get_memory_size (),
                                                       mat2.get_pointer (),
                                                       (TR1)0.0);
                        }
                }
