...
void print_HR(const hamilt::HContainer<TR>& HR,
              const int& nat,
    ...
    for (int ia = 0; ia < nat; ia++)
        for (int ja = 0; ja < nat; ja++) {
            ...
            for (int iR = 0; iR < ap->get_R_size(); ++iR) {
                std::cout << "atom pair (" << ia << ", " << ja << "),  " << "R=(" << ap->get_R_index(iR)[0] << ", "
                          << ap->get_R_index(iR)[1] << ", " << ap->get_R_index(iR)[2] << "): \n";
...
void print_DMR(const elecstate::DensityMatrix<TK, TR>& DMR,
               const int& nat,
               const std::string& label,
               const double& threshold = 1e-10) {
    std::cout << label << "\n";
