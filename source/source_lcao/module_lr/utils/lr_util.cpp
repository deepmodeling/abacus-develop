...
int
    cal_nocc (int nelec)
{
    return nelec / ModuleBase::DEGSPIN + nelec % static_cast<int> (ModuleBase::DEGSPIN);
}
...
template <>
void
    matsym<double> (const double* in, const int n, const Parallel_2D& pmat, double* out)
{
...
template <>
void
    matsym<double> (double* inout, const int n, const Parallel_2D& pmat)
{
...
template <>
void
    matsym<std::complex<double>> (const std::complex<double>* in,
                                  const int n,
...
template <>
void
    matsym<std::complex<double>> (std::complex<double>* inout, const int n, const Parallel_2D& pmat)
{
...
void
    setup_2d_division (Parallel_2D& pv, int nb, int gr, int gc)
{
...
void
    setup_2d_division (Parallel_2D& pv, int nb, int gr, int gc, const int& blacs_ctxt_in)
{
    ModuleBase::TITLE ("LR_Util", "setup_2d_division");
...
void
    diag_lapack (const int& n, double* mat, double* eig)
{
...
void
    diag_lapack (const int& n, std::complex<double>* mat, double* eig)
{
...
void
    diag_lapack_nh (const int& n, double* mat, std::complex<double>* eig)
{
    ...
    if (info)
        {
            std::cout << "ERROR: Lapack solver dgeev, info=" << info << std::endl;
        }
    for (int i = 0; i < n; ++i)
        {
            eig[i] = std::complex<double> (eig_real[i], eig_imag[i]);
        }
}

void
    diag_lapack_nh (const int& n, std::complex<double>* mat, std::complex<double>* eig)
{
    ...
    if (info)
        {
            std::cout << "ERROR: Lapack solver zgeev, info=" << info << std::endl;
        }
}
