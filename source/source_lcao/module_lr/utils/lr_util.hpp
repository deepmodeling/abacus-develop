...
template <typename T>
void
    _allocate_2order_nested_ptr (T**& p2, size_t size1, size_t size2)
{
    p2 = new T*[size1];
...
template <typename T>
void
    _deallocate_2order_nested_ptr (T** p2, size_t size)
{
    if (p2 != nullptr)
        {
...
inline double
    get_conj (const double& x)
{
    return x;
}
inline std::complex<double>
    get_conj (const std::complex<double>& x)
{
    return std::conj (x);
}
template <typename T>
void
    matsym (const T* in, const int n, T* out)
{
    for (int i = 0; i < n; ++i)
        {
...
template <typename T>
psi::Psi<T>
    get_psi_spin (const psi::Psi<T>& psi_in, const int& is, const int& nk)
{
    return psi::Psi<T> (&psi_in (is * nk, 0, 0), nk, psi_in.get_nbands (), psi_in.get_nbasis (), true);
}

/// psi(nk=1, nbands=nb, nk * nbasis) -> psi(nb, nk, nbasis) without memory copy
template <typename T, typename Device>
psi::Psi<T, Device>
    k1_to_bfirst_wrapper (const psi::Psi<T, Device>& psi_kfirst, int nk_in, int nbasis_in)
{
...
template <typename T, typename Device>
psi::Psi<T, Device>
    bfirst_to_k1_wrapper (const psi::Psi<T, Device>& psi_bfirst)
{
...
template <typename T>
void
    gather_2d_to_full (const Parallel_2D& pv,
                       const T* submat,
                       T* fullmat,
                       bool col_first,
                       int global_nrow,
    ...
    auto get_mpi_datatype = [] () -> MPI_Datatype
        {
            ...
            if (std::is_same<T, std::complex<float>>::value)
                ...
                {
                    throw std::runtime_error ("gather_2d_to_full: unsupported type");
                }
        };

    // zeros
    for (int i = 0; i < global_nrow * global_ncol; ++i)
        {
            fullmat[i] = 0.0;
        }
    // copy
            ...
            for (int j = 0; j < pv.get_col_size (); ++j)
                {
                    if (col_first)
                        {
                            fullmat[pv.local2global_row (i) * global_ncol + pv.local2global_col (j)]
                                = submat[i * pv.get_col_size () + j];
                        }
                    else
                        {
                            fullmat[pv.local2global_col (j) * global_nrow + pv.local2global_row (i)]
                                = submat[j * pv.get_row_size () + i];
                        }
                }
        }
