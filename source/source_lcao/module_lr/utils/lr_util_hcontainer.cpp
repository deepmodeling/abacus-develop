...
{
void
    get_DMR_real_imag_part (const elecstate::DensityMatrix<std::complex<double>, std::complex<double>>& DMR,
                    ...
                    for (int ja = 0; ja < nat; ja++)
                        {
                            ...
                            for (int iR = 0; iR < ap->get_R_size (); ++iR)
                                {
                                    // R index may be different between the two HContainers, find by R value instead of
                                    // R-index
                                    ...
                                    for (int i = 0; i < ap->get_size (); ++i)
                                        {
                                            ptr_real[i] = (get_imag ? ptr[i].imag () : ptr[i].real ());
                                        }
                                }
                        }
                }
...
void
    set_HR_real_imag_part (const hamilt::HContainer<double>& HR_real,
                    ...
                    for (int iR = 0; iR < ap->get_R_size (); ++iR)
                        {
                            ...
                            for (int i = 0; i < ap->get_size (); ++i)
                                {
                                    get_imag ? ptr[i].imag (ptr_real[i]) : ptr[i].real (ptr_real[i]);
                                }
                        }
                }
        }
