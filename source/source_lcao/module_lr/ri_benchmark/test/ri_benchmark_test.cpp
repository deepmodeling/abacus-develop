...
UnitCell::~UnitCell() { delete[] atoms; }
...
TEST(RI_Benchmark, CalCsMO) {
    ...
    for (int i = 0; i < nabf * nao * nao; ++i) {
        Cs_ao[0][{0, {0, 0, 0}}].ptr()[i] = static_cast<double>(i);
    }

    const UnitCell ucell;
    psi::Psi<double> psi_ks(1, 2, 2, 2, true);
    for (int i = 0; i < 4; ++i) {
        psi_ks.get_pointer()[i] = static_cast<double>(i);
    }
    ...
    for (int i = 0; i < 2; ++i) {
        EXPECT_DOUBLE_EQ(Cs_a_mo.at(0).at({0, {0, 0, 0}})(i, 0, 0), Cs_a_mo_ref[i]);
    }
    ...
    for (int i = 0; i < 2; ++i) {
        EXPECT_DOUBLE_EQ(Cs_b_mo.at(0).at({0, {0, 0, 0}})(i, 0, 0), Cs_b_mo_ref[i]);
    }
}
