...
void benchmark_driver_A(std::string& file_Cs,
                        std::string& file_Vs,
                        std::string& file_kswfc,
                        const int nocc,
                        const int nvirt);
template <typename TK, typename TR>
void benchmark_driver_AX(std::string& file_Cs,
                         std::string& file_Vs,
                         std::string& file_kswfc,
                         const int nocc,
                         const int nvirt);
...
template <typename TK, typename TR>
TLRI<TK> cal_CV(const TLRI<TK>& Cs_a, const TLRI<TR>& Vs);

/// AX=CV(CX), sum over atom quads
template <typename TK, typename TR>
void cal_AX(const TLRI<TK>& Cs_a, const TLRIX<TK>& Cs_bX, const TLRI<TR>& Vs, TK* AX, const double& scale = 2.0);
/// AX=（CV)(CX), sum over atom quads
template <typename TK>
void cal_AX(const TLRI<TK>& CV, const TLRIX<TK>& Cs_bX, TK* AX, const double& scale = 2.0);
