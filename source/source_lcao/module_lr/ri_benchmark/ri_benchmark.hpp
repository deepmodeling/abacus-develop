...
inline int
    count_nao_from_Cs (const TLRI<TR>& Cs_ao)
{
    int naos = 0;
...
inline std::vector<TK>
    slice_psi (const psi::Psi<TK>& wfc_ks, const int& ibstart, const int& nb, const int& iwstart, const int& nw)
{
...
TLRI<TK>
    cal_Cs_mo (const UnitCell& ucell,
               const TLRI<TR>& Cs_ao,
                    ...
                    for (int iabf = 0; iabf < nabf; ++iabf)
                        {
                            ...
                            if (occ_first)
                                {
                                    container::BlasConnector::gemm ('T',
                                                                    'N',
                                                                    nvirt,
                                                                    nw1,
                                                                    nw2,
                                                                    ...
                                                                    tmp.data (),
                                                                    nvirt);
                                    container::BlasConnector::gemm ('N',
                                                                    'N',
                                                                    nvirt,
                                                                    nocc,
                                                                    nw1,
                                                                    ...
                                                                    nw1,
                                                                    0.0,
                                                                    &Cs_mo[c1.first][c2.first](iabf, 0, 0),
                                                                    nvirt);
                                }
                            else
                                {
...
template <typename TK, typename TR>
std::vector<TK>
    cal_Amat_full (const TLRI<TK>& Cs_a, const TLRI<TK>& Cs_b, const TLRI<TR>& Vs)
{
    assert (Cs_a.size () > 0);
    assert (Cs_b.size () > 0);
    assert (Vs.size () > 0);
...
template <typename TK>
TLRIX<TK>
    cal_CsX (const TLRI<TK>& Cs_mo, const TK* X)
{
    TLRIX<TK> CsX;
    for (auto& it1: Cs_mo)
        {
...
template <typename TK, typename TR>
TLRI<TK>
    cal_CV (const TLRI<TK>& Cs_a_mo, const TLRI<TR>& Vs)
{
    TLRI<TK> CV;
                    ...
                    for (auto& it3: Vs.at (iat1))
                        {
                            ...
                            if (CV.count (iat2) && CV.at (iat2).count ({iat3, {0, 0, 0}})) // add-up, sum over iat1
                                ...
                                {
                                    RI::Tensor<TK> tmp (
                                        {nabf2, tensor_c.shape[1], tensor_c.shape[2]}); // (nabf2, nocc, nvirt)
                                    container::BlasConnector::gemm ('N',
                                                                    'T',
                                                                    npairs,
                                                                    nabf2,
...
template <typename TK, typename TR>
void
    cal_AX (const TLRI<TK>& Cs_a, const TLRIX<TK>& Cs_bX, const TLRI<TR>& Vs, TK* AX, const double& scale)
{
                            ...
                            for (auto& itcb2: Cs_bX.at (iat2))
                                {
                                    ...
                                    container::BlasConnector::gemv ('T',
                                                                    nabf1,
                                                                    nabf2,
                                                                    1.0,
                                                                    tensor_v.ptr (),
...
template <typename TK>
void
    cal_AX (const TLRI<TK>& CV, const TLRIX<TK>& Cs_bX, TK* AX, const double& scale)
{
    for (auto& it1: CV)
        {
            const int& iat1 = it1.first;
...
std::vector<FPTYPE>
    read_aims_ebands (const std::string& file, const int nocc, const int nvirt, int& ncore)
{
...
template <typename TK>
void
    read_aims_eigenvectors (psi::Psi<TK>& wfc_ks,
...
TLRI<TR>
    read_coulomb_mat (const std::string& file, const TLRI<TR>& Cs)
{ // for gamma_only, V(q)=V(R=0)
    ...
    if (nks > 1)
        {
            std::cout << "Warning: nks>1 is not supported yet!" << std::endl;
        }
    TLRI<TR> Vs;
...
TLRI<TR>
    read_coulomb_mat_general (const std::string& file, const TLRI<TR>& Cs)
{ // for gamma_only, V(q)=V(R=0)
    std::ifstream ifs;
    ifs.open (file);
    size_t nks = 0, nabf = 0, istart = 0, jstart = 0, iend = 0, jend = 0;
...
template <typename TR>
bool
    compare_Vs (const TLRI<TR>& Vs1, const TLRI<TR>& Vs2, const double thr)
{
    for (auto& tmp1: Vs1)
        {
                    ...
                    for (int i = 0; i < t1.shape[0]; ++i)
                        {
                            for (int j = 0; j < t1.shape[1]; ++j)
                                {
                                    if (std::abs (t1 (i, j) - t2 (i, j)) > thr)
                                        {
                                            std::cout << "element (" << i << ", " << j
                                                      << ") are differernt: " << t1 (i, j) << ", " << t2 (i, j)
                                                      << std::endl;
                                            return false;
                                        }
                                }
...
template <typename TR>
std::vector<TLRI<TR>>
    split_Ds (const std::vector<std::vector<TR>>& Ds, const std::vector<int>& aims_nbasis, const UnitCell& ucell)
{
    // Due to the hard-coded constructor of elecstate::DensityMatrix, singlet-triplet with nspin=2 cannot use DM_trans
    // with size 1 if(Ds.size()>1) { throw std::runtime_error("split_Ds only supports gamma-only spin-1 Ds now."); }
                            ...
                            for (int i = 0; i < nw1; ++i)
                                {
                                    for (int j = 0; j < nw2; ++j)
                                        {
                                            D_split[iat1][{iat2, {0, 0, 0}}](i, j)
                                                = D[(iw1_start + i) * nbasis + (iw2_start + j)]
                                                  * 0.5; // consistent with split_m2D_ktoR
                                        }
                                }
                            iw2_start += nw2;
                        }
