...
template <typename TK>
void read_aims_eigenvectors(psi::Psi<TK>& wfc_ks,
        ...
        for (int iw = 0; iw < nbasis; ++iw) {
            ...
            for (int ib = nbands_last; ib < nbands_file; ++ib) {
                ...
                if (ib >= ncore && ib < ncore + nbands) {
                    for (int is = 0; is < wfc_ks.get_nk(); ++is) { // only for gamma_only and spin degenerate
...
TLRI<TR> read_coulomb_mat(const std::string& file, const TLRI<TR>& Cs) { // for gamma_only, V(q)=V(R=0)
    std::ifstream ifs;
    ifs.open(file);
    ...
    for (int iat1 = 0; iat1 < nat; ++iat1) {
        ...
        for (int iat2 = 0; iat2 < nat; ++iat2) {
            if (iat1 > iat2) { // coulomb_mat has only the upper triangle part
...
TLRI<TR> read_coulomb_mat_general(const std::string& file, const TLRI<TR>& Cs) { // for gamma_only, V(q)=V(R=0)
    std::ifstream ifs;
    ifs.open(file);
    size_t nks = 0, nabf = 0, istart = 0, jstart = 0, iend = 0, jend = 0;
    std::string tmp;
            ...
            for (int i = 0; i < t1.shape[0]; ++i) {
                for (int j = 0; j < t1.shape[1]; ++j) {
                    if (std::abs(t1(i, j) - t2(i, j)) > thr) {
                        std::cout << "element (" << i << ", " << j << ") are differernt: " << t1(i, j) << ", "
                                  << t2(i, j) << std::endl;
                        return false;
                    }
                }
                ...
                for (int i = 0; i < nw1; ++i) {
                    for (int j = 0; j < nw2; ++j) {
                        D_split[iat1][{iat2, {0, 0, 0}}](i, j) =
                            D[(iw1_start + i) * nbasis + (iw2_start + j)] * 0.5; // consistent with split_m2D_ktoR
                    }
                }
                iw2_start += nw2;
            }
