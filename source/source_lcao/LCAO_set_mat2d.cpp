...
//------------------------------------------------------
template <typename T>
void set_mat2d(const int& global_ir, // index i for atomic orbital (row)
               ...
               T* HSloc) // input pointer for store the matrix
{
    ...
    const long index = ModuleBase::GlobalFunc::IS_COLUMN_MAJOR_KS_SOLVER(PARAM.inp.ks_solver)
...
template void set_mat2d<double>(const int& global_ir,
                                const int& global_ic,
                                const double& v,
                                const Parallel_Orbitals& pv,
                                double* HSloc);

template void set_mat2d<std::complex<double>>(const int& global_ir,
                                              const int& global_ic,
