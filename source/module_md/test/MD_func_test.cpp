...
TEST_F(MD_func_test, randomvel)
{
    ...
    EXPECT_DOUBLE_EQ(vel[0].x, 9.9105892783200826e-06);
    EXPECT_DOUBLE_EQ(vel[0].y, -3.343699576563167e-05);
    EXPECT_DOUBLE_EQ(vel[0].z, 9.385130426808701e-05);
    EXPECT_DOUBLE_EQ(vel[1].x, -0.00017919300771203808);
    EXPECT_DOUBLE_EQ(vel[1].y, 5.7074002254799079e-05);
    EXPECT_DOUBLE_EQ(vel[1].z, -3.1088136026582953e-05);
    EXPECT_DOUBLE_EQ(vel[2].x, 0.000141316492668737);
    EXPECT_DOUBLE_EQ(vel[2].y, -0.00015841124290501442);
    EXPECT_DOUBLE_EQ(vel[2].z, 1.900921882689748e-05);
    EXPECT_DOUBLE_EQ(vel[3].x, 2.7965925764981002e-05);
    EXPECT_DOUBLE_EQ(vel[3].y, 0.00013477423641584702);
    EXPECT_DOUBLE_EQ(vel[3].z, -8.177238706840153e-05);
}
