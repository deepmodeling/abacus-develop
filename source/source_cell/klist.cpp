...
void
    K_Vectors::set (const UnitCell& ucell,
    ...
    if (!berryphase::berry_phase_flag && ModuleSymmetry::Symmetry::symm_flag != -1)
        {
            ...
            if (!match)
                {
                    std::cout
                        << "Optimized lattice type of reciprocal lattice cannot match the optimized real lattice. "
                        << std::endl;
...
void
    K_Vectors::renew (const int& kpoint_number)
{
...
bool
    K_Vectors::read_kpoints (const UnitCell& ucell, const std::string& fn)
{
...
void
    K_Vectors::interpolate_k_between (std::ifstream& ifk, std::vector<ModuleBase::Vector3<double>>& kvec)
{
...
double
    K_Vectors::Monkhorst_Pack_formula (const int& k_type, const double& offset, const int& n, const int& dim)
{
...
// add by dwan
void
    K_Vectors::Monkhorst_Pack (const int* nmp_in, const double* koffset_in, const int k_type)
{
...
void
    K_Vectors::update_use_ibz (const int& nkstot_ibz,
...
void
    K_Vectors::normalize_wk (const int& degspin)
{
    if (GlobalV::MY_RANK != 0)
        {
            return;
