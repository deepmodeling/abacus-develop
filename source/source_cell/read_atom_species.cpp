...
{
bool
    read_atom_species (std::ifstream& ifa, std::ofstream& ofs_running, UnitCell& ucell)
{
            ...
            for (int i = 0; i < ntype; i++)
                {
                    ...
                    if (!end && ss >> one_string && one_string[0] != '#')
                        {
                            if (one_string == "auto" || one_string == "upf" || one_string == "vwr"
                                || one_string == "upf201" || one_string == "blps")
                                {
                            ...
                            else
                                {
                                    ...
                                    ModuleBase::WARNING_QUIT ("read_atom_species",
                                                              "unrecognized pseudopotential type.");
                                }
                        }

                    // Peize Lin test for bsse 2021.04.07
                    const std::string bsse_label = "empty";
                    ucell.atoms[i].flag_empty_element = (search (ucell.atom_label[i].begin (),
...
bool
    read_lattice_constant (std::ifstream& ifa, std::ofstream& ofs_running, Lattice& lat)
{
    //==========================
            ...
            if (ModuleBase::GlobalFunc::SCAN_LINE_BEGIN (ifa, "LATTICE_PARAMETERS", true, false))
                {
                    ModuleBase::WARNING_QUIT (
                        "unitcell::read_lattice_constant",
