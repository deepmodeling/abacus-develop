...
void
    check_atomic_stru (UnitCell& ucell, const double& factor)
{
                        ...
                        for (int it2 = it1; it2 < ntype; it2++)
                            {
                                ...
                                double covalent_length
                                    = (symbol1_covalent_radius + symbol2_covalent_radius) / bohr_to_a;
...
#pragma omp critical
                                                        {
                                                            no_warning = false;
                                                            all_pass
                                                                = all_pass && (sqrt_bon < factor_error ? false : true);
                                                            errorlog << std::setw (3) << ia1 + 1 << "-th " << label[it1]
                                                                     << ", " << std::setw (3) << ia2 + 1 << "-th "
                                                                     << label[it2] << cell[i] << std::setprecision (3)
                                                                     << sqrt_bon << " Bohr (" << sqrt_bon * bohr_to_a
                                                                     << " Angstrom)\n";
                                                        }
                                                    }
                                            }
