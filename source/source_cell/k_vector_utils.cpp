...
void set_both_kvec(K_Vectors& kv, const ModuleBase::Matrix3& G, const ModuleBase::Matrix3& R, std::string& skpt) {
    ...
    if (GlobalV::MY_RANK == 0) {
        std::stringstream ss;
        ss << " " << std::setw(40) << "nkstot now" << " = " << kv.get_nkstot() << std::endl;
        ss << table << std::endl;
        skpt = ss.str();
    }
    return;
...
#ifdef __EXX
    if (ModuleSymmetry::Symmetry::symm_flag == 1) { // bcast kstars
...
void kvec_ibz_kpoint(K_Vectors& kv,
    ...
    // output in kpoints file
    std::stringstream ss;
    ss << " " << std::setw(40) << "nkstot" << " = " << kv.get_nkstot() << std::setw(66) << "ibzkpt" << std::endl;
    std::string table;
