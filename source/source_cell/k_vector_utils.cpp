...
{
void
    kvec_d2c (K_Vectors& kv, const ModuleBase::Matrix3& reciprocal_vec)
{
...
void
    kvec_c2d (K_Vectors& kv, const ModuleBase::Matrix3& latvec)
{
    if (kv.kvec_d.size () != kv.kvec_c.size ())
        {
...
void
    set_both_kvec (K_Vectors& kv, const ModuleBase::Matrix3& G, const ModuleBase::Matrix3& R, std::string& skpt)
{
...
void
    set_after_vc (K_Vectors& kv, const int& nspin_in, const ModuleBase::Matrix3& reciprocal_vec)
{
...
void
    print_klists (const K_Vectors& kv, std::ofstream& ofs)
{
                     ...
                     bool& match)
{
    ...
    if (kv.get_is_mp ())
        {
            k_vec1 = ModuleBase::Vector3<double> (recip_vec1.x / kv.nmp[0],
                                                  recip_vec1.y / kv.nmp[0],
                                                  recip_vec1.z / kv.nmp[0]);
            k_vec2 = ModuleBase::Vector3<double> (recip_vec2.x / kv.nmp[1],
                                                  recip_vec2.y / kv.nmp[1],
                                                  recip_vec2.z / kv.nmp[1]);
            k_vec3 = ModuleBase::Vector3<double> (recip_vec3.x / kv.nmp[2],
                                                  recip_vec3.y / kv.nmp[2],
                                                  recip_vec3.z / kv.nmp[2]);
    ...
    if (use_symm)
        {
            ...
            if (!ibrav_match (recip_brav_type)) // if not match, exit and return
                {
                    GlobalV::ofs_running
                        << "Error: Bravais lattice type of reciprocal lattice is not compatible with that of "
                           "real space lattice:"
                        << std::endl;
    ...
    for (int i = 0; i < kv.get_nkstot (); ++i)
        {
            if (!kv.get_is_mp ())
                {
                    weight = kv.wk[i];
                } // use the input weight, instead of 1/nkstot
                    ...
                    if (!already_exist)
                        {
                            ...
                            kvec_rot
                                = kv.kvec_d[i] * kgmatrix[j]; // wrong for total energy, but correct for nonlocal force.
                            // kvec_rot = kgmatrix[j] * kvec_d[i]; //correct for total energy, but wrong for nonlocal
                            // force.
                            ...
                            if (kv.get_is_mp ())
                                {
                                    ...
                                    // std::cout << "\n kvec_rot (in recip) = " << kvec_rot.x << " " << kvec_rot.y << "
