    ...
    remake_cell (Lattice& lat)
{
    ...
    if (latName == "user_defined_lattice")
        ...
        {
            double celldm
                = std::sqrt (pow (latvec.e11, 2) + pow (latvec.e12, 2) + pow (latvec.e13, 2)) / std::sqrt (2.0);
        ...
        {
            double celldm
                = std::sqrt (pow (latvec.e11, 2) + pow (latvec.e12, 2) + pow (latvec.e13, 2)) / std::sqrt (3.0);
    ...
    else
        {
            ...
            ModuleBase::WARNING_QUIT ("unitcell::remake_cell", "latname type not supported!");
        }
}
...
// 20180515
void
    setup_cell_after_vc (UnitCell& ucell, std::ofstream& log)
{
    ModuleBase::TITLE ("unitcell", "setup_cell_after_vc");
    assert (ucell.lat0 > 0.0);
    ucell.omega = std::abs (ucell.latvec.Det ()) * pow (ucell.lat0, 3);
    if (ucell.omega <= 0)
        {
    ...
    else
        {
            ...
            ModuleBase::GlobalFunc::OUT (log, "Cell volume (A^3)", ucell.omega * pow (ModuleBase::BOHR_TO_A, 3));
        }
    ...
    log << std::endl;
    output::printM3 (log, "Lattice vectors: (Cartesian coordinate: in unit of a_0)", ucell.latvec);
    output::printM3 (log, "Reciprocal vectors: (Cartesian coordinate: in unit of 2 pi/a_0)", ucell.G);

    return;
}

void
    update_pos_tau (const Lattice& lat, const double* pos, const int ntype, const int nat, Atom* atoms)
{
    int iat = 0;
...
void
    update_pos_taud (const Lattice& lat, const double* posd_in, const int ntype, const int nat, Atom* atoms)
{
    int iat = 0;
    for (int it = 0; it < ntype; it++)
        {
...
void
    update_pos_taud (const Lattice& lat,
...
void
    update_vel (const ModuleBase::Vector3<double>* vel_in, const int ntype, const int nat, Atom* atoms)
{
    int iat = 0;
    for (int it = 0; it < ntype; ++it)
        {
...
void
    periodic_boundary_adjustment (Atom* atoms, const ModuleBase::Matrix3& latvec, const int ntype)
{
    //----------------------------------------------
            ...
            for (int ia = 0; ia < atom->na; ia++)
                {
                    ...
                    if (atom->taud[ia].x < -eps || atom->taud[ia].y < -eps || atom->taud[ia].z < -eps
                        || atom->taud[ia].x >= 1.0 + eps || atom->taud[ia].y >= 1.0 + eps
                        || atom->taud[ia].z >= 1.0 + eps)
                        {
                            GlobalV::ofs_warning << " atom type=" << it + 1 << " atom index=" << ia + 1 << std::endl;
                            GlobalV::ofs_warning << " direct coordinate=" << atom->taud[ia].x << " " << atom->taud[ia].y
