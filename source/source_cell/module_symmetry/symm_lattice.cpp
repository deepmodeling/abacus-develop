...
//---------------------------------------------------
int
    Symmetry::standard_lat (ModuleBase::Vector3<double>& a,
                            ...
                            double* cel_const) const
{
    ...
    //------------------------
    else if (equal (alpha - beta, 0))
        {
            ...
            // orthogonal to one another and (adjustment//): |A1+A3|=|A2+A3|/=|A1+A2|
                ...
                {
                    if (equal (norm_a, norm_b) && equal (norm_b, norm_c) && equal (cpa, bpc) && !equal (apb, cpa)
                    ...
                    else if (equal (norm_a, norm_b) && alpha < (-1.0 * small) && beta < (-1.0 * small))
                        {
                            type = 13;
                            cel_const[0] = apb;
                            cel_const[1] = amb / apb;
    ...
    //-------------------------------
    else
        {
            ...
            if (equal (norm_a, norm_b) && equal (norm_b, norm_c) && ((cpa - bpc) > small) && ((apb - cpa) > small)
                && equal (norm_c * norm_c + abc, 0))
                {
                    type = 9;
                    cel_const[0] = bpc;
            ...
            else if (equal (amb, norm_c) && equal (cma, norm_b) && equal (bmc, norm_a) && ((apbmc - cpamb) > small)
                     && ((cpamb - bpcma) > small))
                {
                    type = 10;
                    cel_const[0] = bpcma;
...
//---------------------------------------------------
void
    Symmetry::lattice_type (ModuleBase::Vector3<double>& v1,
