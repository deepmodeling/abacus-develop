...
void
    Symmetry::pricell (double* pos, const Atom* atoms)
{
    bool no_diff = false;
    ptrans.clear ();
    ...
    for (int i = itmin_start; i < itmin_start + na[itmin_type]; ++i)
        {
                    ...
                    for (int ia = istart[it]; ia < na[it] + istart[it]; ia++)
                        {
                            ...
                            if (!equal (diff.x, 0.0) || !equal (diff.y, 0.0) || !equal (diff.z, 0.0))
                                {
                                    no_diff = false;
                                    break;
                                }
            ...
            if (no_diff)
                {
                    ptrans.push_back (ModuleBase::Vector3<double> (tmp_ptrans[0], tmp_ptrans[1], tmp_ptrans[2]));
                }
    ...
    // 1. kplane for b3
    while (kplane < ntrans && std::abs (ptrans[kplane].z - ptrans[0].z) < this->epsilon)
        {
            ++kplane;
        }
    if (kplane == ntrans)
    ...
    b3 = kplane > 0 ? ModuleBase::Vector3<double> (ptrans[kplane].x, ptrans[kplane].y, ptrans[kplane].z)
                    : ModuleBase::Vector3<double> (0, 0, 1);
    // 2. jplane for b2 (not collinear with b3)
    jplane = kplane + 1;
    while (jplane < ntrans
           && (std::abs (ptrans[jplane].y - ptrans[0].y) < this->epsilon || equal ((ptrans[jplane] ^ b3).norm (), 0)))
        {
            ++jplane;
        }
    if (jplane == ntrans)
    ...
    b2 = jplane > kplane ? ModuleBase::Vector3<double> (ptrans[jplane].x, ptrans[jplane].y, ptrans[jplane].z)
                         : ModuleBase::Vector3<double> (0, 1, 0);
    // 3. iplane for b1 (not coplane with <b2, b3>)
    iplane = jplane + 1;
    while (iplane < ntrans
           && (std::abs (ptrans[iplane].x - ptrans[0].x) < this->epsilon || equal (ptrans[iplane] * (b2 ^ b3), 0)))
        {
            ++iplane;
        }
    ...
    auto reset_pcell = [this] () -> void
        ...
        but we have to treat it as a primitive cell. Use a larger `symmetry_prec`to avoid this warning."
                                 << std::endl;
        };
