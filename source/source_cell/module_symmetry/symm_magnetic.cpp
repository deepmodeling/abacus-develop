...
void
    Symmetry::analyze_magnetic_group (const Atom* atoms, const Statistics& st, int& nrot_out, int& nrotk_out)
{
            ...
            for (auto& mag_iat: mag_type_atoms.at (mag_it))
                {
                    ...
                    const ModuleBase::Vector3<double> direct_tmp
                        = atoms[st.iat2it[mag_iat]].tau[st.iat2ia[mag_iat]] * this->optlat.Inverse ();
...
bool
    Symmetry::magmom_same_check (const Atom* atoms) const
{
