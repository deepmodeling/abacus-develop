...
bool
    Symmetry::checksym (const ModuleBase::Matrix3& s,
                        ...
                        int* na) const
{
            ...
            for (int j = istart[it]; j < istart[it] + na[it]; ++j)
                {
                    ...
                    rotpos[xx] = pos[xx] * s.e11 + pos[yy] * s.e21 + pos[zz] * s.e31;

                    rotpos[yy] = pos[xx] * s.e12 + pos[yy] * s.e22 + pos[zz] * s.e32;

                    rotpos[zz] = pos[xx] * s.e13 + pos[yy] * s.e23 + pos[zz] * s.e33;
    ...
    ModuleBase::Vector3<double> sptmin (rotpos[itmin_start * 3],
                                        rotpos[itmin_start * 3 + 1],
                                        rotpos[itmin_start * 3 + 2]);
    ...
    for (int i = itmin_start; i < itmin_start + na[itmin_type]; ++i)
        {
            ...
            if (gtrans.x > trans.x + epsilon || gtrans.y > trans.y + epsilon || gtrans.z > trans.z + epsilon)
                {
                    continue;
                }
                    ...
                    for (int ia = istart[it]; ia < na[it] + istart[it]; ia++)
                        {
                            ...
                            if (no_diff == false || !equal (diff.x, 0.0) || !equal (diff.y, 0.0)
                                || !equal (diff.z, 0.0))
                                {
                                    no_diff = false;
                                }
