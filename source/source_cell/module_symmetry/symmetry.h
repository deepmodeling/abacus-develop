...
class Symmetry : public Symmetry_Basic
{
  ...
  public:
    ...
    /// @param ofs_running
    /// get the symmetry information of the system, gmatries (rotation 3*3 matrixs), gtrans (transfer a collections
    /// vector3), etc.
    ...
    ModuleBase::Vector3<double> a1, a2, a3; // primitive cell vectors(might be changed during the process of the
                                            // program)
    ...
    std::vector<ModuleBase::Vector3<double>>
        ptrans;    // the translation vectors of the primitive cell in the input structure
    ...
    double pre_const[6]
        = {0.0}; // cel_const of input configuration, first 3 is moduli of a1, a2, a3, last 3 is eular angle
    ...
    void rhog_symmetry (std::complex<double>* rhogtot,
                        int* ixyz2ipw,
                        const int& nx,
                        const int& ny,
                        const int& nz,
    ...
    void gmatrix_convert (const ModuleBase::Matrix3* sa,
                          ModuleBase::Matrix3* sb,
                          const int n,
                          const ModuleBase::Matrix3& a,
                          const ModuleBase::Matrix3& b) const;

    void gmatrix_convert_int (const ModuleBase::Matrix3* sa,
                              ModuleBase::Matrix3* sb,
                              const int n,
                              const ModuleBase::Matrix3& a,
                              const ModuleBase::Matrix3& b) const;
    ...
    void gtrans_convert (const ModuleBase::Vector3<double>* va,
    ...
    int
        get_rotated_atom (int isym, int iat) const
    {
        if (!this->isym_rotiat_.empty ())
            {
                return this->isym_rotiat_[isym][iat];
            }
        else
            {
                return -1;
  ...
  private:
    ...
    void get_optlat (ModuleBase::Vector3<double>& v1,
                     ModuleBase::Vector3<double>& v2,
                     ModuleBase::Vector3<double>& v3,
