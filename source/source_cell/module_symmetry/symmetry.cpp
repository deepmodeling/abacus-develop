    ...
    for (int it = 0; it < this->ntype; it++) {
        for (int ia = istart[it]; ia < istart[it] + na[it]; ++ia) {
            ...
            for (int k = 0; k < this->nrotk; ++k) {
                rotpos[xx] =
                    pos[xx] * gmatrix[k].e11 + pos[yy] * gmatrix[k].e21 + pos[zz] * gmatrix[k].e31 + gtrans[k].x;
                rotpos[yy] =
                    pos[xx] * gmatrix[k].e12 + pos[yy] * gmatrix[k].e22 + pos[zz] * gmatrix[k].e32 + gtrans[k].y;
                rotpos[zz] =
                    pos[xx] * gmatrix[k].e13 + pos[yy] * gmatrix[k].e23 + pos[zz] * gmatrix[k].e33 + gtrans[k].z;
...
void Symmetry::gmatrix_convert_int(const ModuleBase::Matrix3* sa,
...
void Symmetry::gmatrix_convert(const ModuleBase::Matrix3* sa,
                               ModuleBase::Matrix3* sb,
                               const int n,
                               const ModuleBase::Matrix3& a,
                               const ModuleBase::Matrix3& b) const {
...
void Symmetry::gtrans_convert(const ModuleBase::Vector3<double>* va,
...
void Symmetry::gmatrix_invmap(const ModuleBase::Matrix3* s, const int n, int* invmap) const {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = i; j < n; ++j) {
            tmp = s[i] * s[j];
            if (equal(tmp.e11, 1) && equal(tmp.e22, 1) && equal(tmp.e33, 1) && equal(tmp.e12, 0) && equal(tmp.e21, 0) &&
                equal(tmp.e13, 0) && equal(tmp.e31, 0) && equal(tmp.e23, 0) && equal(tmp.e32, 0)) {
                invmap[i] = j;
                invmap[j] = i;
                break;
            }
...
void Symmetry::get_optlat(ModuleBase::Vector3<double>& v1,
                                ...
                                for (int n12 = -2; n12 < 3; ++n12) {
                                    for (int n11 = -2; n11 < 3; ++n11) {
                                        ...
                                        if (equal(mat.Det(), 1.0)) {
                                            ...
                                            if (ibrav < real_brav ||
                                                (ibrav == real_brav && (fabs(cel_const[3]) < (cos1 - 1.0e-9)) &&
                                                 (fabs(cel_const[4]) < (cos2 - 1.0e-9)) &&
                                                 (fabs(cel_const[5]) <
                                                  (cos3 - 1.0e-9))) // mohan fix bug 2012-01-15, not <=
                                            ) {
                                                real_brav = ibrav;
