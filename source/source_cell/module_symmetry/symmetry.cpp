...
void
    Symmetry::set_atom_map (const Atom* atoms)
{
            ...
            for (int ia = istart[it]; ia < istart[it] + na[it]; ++ia)
                {
                    ...
                    for (int k = 0; k < this->nrotk; ++k)
                        {
                            rotpos[xx] = pos[xx] * gmatrix[k].e11 + pos[yy] * gmatrix[k].e21 + pos[zz] * gmatrix[k].e31
                                         + gtrans[k].x;
                            rotpos[yy] = pos[xx] * gmatrix[k].e12 + pos[yy] * gmatrix[k].e22 + pos[zz] * gmatrix[k].e32
                                         + gtrans[k].y;
                            rotpos[zz] = pos[xx] * gmatrix[k].e13 + pos[yy] * gmatrix[k].e23 + pos[zz] * gmatrix[k].e33
                                         + gtrans[k].z;
...
void
    Symmetry::symmetrize_vec3_nat (double* v) const // pengfei 2016-12-20
{
    ModuleBase::TITLE ("Symmetry", "symmetrize_vec3_nat");
    double* vtot = nullptr;
    int* n = nullptr;
    vtot = new double[nat * 3];
    ModuleBase::GlobalFunc::ZEROS (vtot, nat * 3);
    n = new int[nat];
    ModuleBase::GlobalFunc::ZEROS (n, nat);
            ...
            for (int k = 0; k < nrotk; ++k)
                {
                    ...
                    vtot[l * 3]
                        = vtot[l * 3] + v[jx] * gmatrix[k].e11 + v[jy] * gmatrix[k].e21 + v[jz] * gmatrix[k].e31;
                    vtot[l * 3 + 1]
                        = vtot[l * 3 + 1] + v[jx] * gmatrix[k].e12 + v[jy] * gmatrix[k].e22 + v[jz] * gmatrix[k].e32;
                    vtot[l * 3 + 2]
                        = vtot[l * 3 + 2] + v[jx] * gmatrix[k].e13 + v[jy] * gmatrix[k].e23 + v[jz] * gmatrix[k].e33;
                    n[l]++;
                }
        }
    for (int j = 0; j < nat; ++j)
...
void
    Symmetry::symmetrize_mat3 (ModuleBase::matrix& sigma, const Lattice& lat) const // zhengdy added 2017
{
...
void
    Symmetry::gmatrix_convert_int (const ModuleBase::Matrix3* sa,
                                   ModuleBase::Matrix3* sb,
                                   const int n,
                                   const ModuleBase::Matrix3& a,
                                   const ModuleBase::Matrix3& b) const
{
...
void
    Symmetry::gmatrix_convert (const ModuleBase::Matrix3* sa,
                               ModuleBase::Matrix3* sb,
                               const int n,
                               const ModuleBase::Matrix3& a,
                               const ModuleBase::Matrix3& b) const
...
void
    Symmetry::gtrans_convert (const ModuleBase::Vector3<double>* va,
...
void
    Symmetry::gmatrix_invmap (const ModuleBase::Matrix3* s, const int n, int* invmap) const
{
    ...
    for (int i = 0; i < n; ++i)
        {
            for (int j = i; j < n; ++j)
                {
                    tmp = s[i] * s[j];
                    if (equal (tmp.e11, 1) && equal (tmp.e22, 1) && equal (tmp.e33, 1) && equal (tmp.e12, 0)
                        && equal (tmp.e21, 0) && equal (tmp.e13, 0) && equal (tmp.e31, 0) && equal (tmp.e23, 0)
                        && equal (tmp.e32, 0))
                        {
                            invmap[i] = j;
                            invmap[j] = i;
...
void
    Symmetry::get_shortest_latvec (ModuleBase::Vector3<double>& a1,
                                   ModuleBase::Vector3<double>& a2,
                                   ModuleBase::Vector3<double>& a3) const
{
    double len1 = a1.norm ();
    double len2 = a2.norm ();
    double len3 = a3.norm ();
...
void
    Symmetry::get_optlat (ModuleBase::Vector3<double>& v1,
                                                                    ...
                                                                    for (int n11 = -2; n11 < 3; ++n11)
                                                                        {
                                                                            ...
                                                                            if (equal (mat.Det (), 1.0))
                                                                                {
                                                                                    r1.x = n11 * v1.x + n12 * v2.x
                                                                                           + n13 * v3.x;
                                                                                    r1.y = n11 * v1.y + n12 * v2.y
                                                                                           + n13 * v3.y;
                                                                                    r1.z = n11 * v1.z + n12 * v2.z
                                                                                           + n13 * v3.z;

                                                                                    r2.x = n21 * v1.x + n22 * v2.x
                                                                                           + n23 * v3.x;
                                                                                    r2.y = n21 * v1.y + n22 * v2.y
                                                                                           + n23 * v3.y;
                                                                                    r2.z = n21 * v1.z + n22 * v2.z
                                                                                           + n23 * v3.z;

                                                                                    r3.x = n31 * v1.x + n32 * v2.x
                                                                                           + n33 * v3.x;
                                                                                    r3.y = n31 * v1.y + n32 * v2.y
                                                                                           + n33 * v3.y;
                                                                                    r3.z = n31 * v1.z + n32 * v2.z
                                                                                           + n33 * v3.z;

                                                                                    ibrav = standard_lat (r1,
                                                                                                          r2,
                                                                                                          r3,
                                                                                                          cel_const);
...
bool
    Symmetry::is_all_movable (const Atom* atoms, const Statistics& st) const
{
    bool all_mbl = true;
