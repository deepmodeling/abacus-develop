    ...
    for (int stru = 0; stru < stru_lib.size(); stru++) {
        ...
        for (int i = 0; i < symm.nrotk; ++i) {
            ...
            if (!symm.equal(tmpG.e13, tmpG_int.e13) || !symm.equal(tmpG.e23, tmpG_int.e23) ||
                ...
                std::cout << symm.kgmatrix[i].e11 << " " << symm.kgmatrix[i].e12 << " " << symm.kgmatrix[i].e13
                          << std::endl;
                std::cout << symm.kgmatrix[i].e21 << " " << symm.kgmatrix[i].e22 << " " << symm.kgmatrix[i].e23
                          << std::endl;
                std::cout << symm.kgmatrix[i].e31 << " " << symm.kgmatrix[i].e32 << " " << symm.kgmatrix[i].e33
                          << std::endl;
                std::cout << "kgmatrix[i], nonint:" << std::endl;
                std::cout << kgmatrix_nonint[i].e11 << " " << kgmatrix_nonint[i].e12 << " " << kgmatrix_nonint[i].e13
                          << std::endl;
                std::cout << kgmatrix_nonint[i].e21 << " " << kgmatrix_nonint[i].e22 << " " << kgmatrix_nonint[i].e23
                          << std::endl;
                std::cout << kgmatrix_nonint[i].e31 << " " << kgmatrix_nonint[i].e32 << " " << kgmatrix_nonint[i].e33
                          << std::endl;
            }
...
TEST_F(SymmetryTest, AtomOrderingNew) {
    ...
    for (int i = 0; i < nat - 1; ++i) {
        ...
        if (symm.equal(new_pos[3 * i], new_pos[3 * (i + 1)])) { // y[i]<=y[i+1]
            EXPECT_LE(new_pos[3 * i + 1], new_pos[3 * (i + 1) + 1] + symm.epsilon);
            if (symm.equal(new_pos[3 * i + 1], new_pos[3 * (i + 1) + 1])) { // z[i]<=z[i+1]
