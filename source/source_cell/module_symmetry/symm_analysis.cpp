...
void Symmetry::analy_sys(const Lattice& lat, const Statistics& st, Atom* atoms, std::ofstream& ofs_running) {
    ...
    if (PARAM.inp.calculation == "cell-relax" && nrotk > 0) {
        ...
        if (eps_enlarged) {
            if (epsilon > MAX_EPS) {
                ofs_running << " WARNING: Symmetry cannot be kept due to the lost of accuracy with atom position "
                               "during cell-relax."
                            << std::endl;
        ...
        if (!eps_enlarged &&
            epsilon > epsilon_input * 1.001) // not "else" here. "eps_enlarged" can be set to false in the above "if"
    ...
    //-----------------------------
    if (!valid_group) { // select the operations that have the inverse
