...
void
    Symmetry::analy_sys (const Lattice& lat, const Statistics& st, Atom* atoms, std::ofstream& ofs_running)
{
    ...
    if (PARAM.inp.calculation == "cell-relax" && nrotk > 0)
        {
            ...
            if (eps_enlarged)
                {
                    if (epsilon > MAX_EPS)
                        {
                            ofs_running << " WARNING: Symmetry cannot be kept due to the lost of accuracy with atom "
                                           "position during cell-relax."
                                        << std::endl;
                            ...
                            while (valid_index > 0 && tmp_nrotk <= nrotks_try[valid_index - 1])
                                {
                                    --valid_index;
                                }
                            ...
                            if (valid_index > 0)
                                {
                                    ofs_running << " Enlarging `symmetry_prec` to " << epsilon << " ..." << std::endl;
                                }
                            else
                                {
                                    eps_enlarged = false;
            ...
            if (!eps_enlarged
                && epsilon
                       > epsilon_input * 1.001) // not "else" here. "eps_enlarged" can be set to false in the above "if"
                { // try a smaller symmetry_prec until the number of symmetry operations decreases
                    ...
                    this->nrotk = tmp_nrotk;
                    if (valid_index > 0)
                        { // epsilon is set smaller
                            ofs_running << " Narrowing `symmetry_prec` from " << eps_current << " to " << epsilon
                                        << " ..." << std::endl;
                        }
                }
        }
