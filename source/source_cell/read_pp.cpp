...
int
    Pseudopot_upf::init_pseudo_reader (const std::string& fn, std::string& type, Atom_pseudo& pp)
{
...
//----------------------------------------------------------
int
    Pseudopot_upf::set_pseudo_type (const std::string& fn, std::string& type) // zws add
{
...
std::string&
    Pseudopot_upf::trim (std::string& in_str)
{
...
std::string
    Pseudopot_upf::trimend (std::string& in_str)
{
...
int
    Pseudopot_upf::average_p (const double& lambda, Atom_pseudo& pp)
{
    int error = 0;
            ...
            for (int nb = 0; nb < pp.nbeta; nb++)
                {
                    new_nbeta++;
                    if (pp.lll[nb] != 0
                        && std::abs (pp.jjj[nb] - pp.lll[nb] - 0.5) < 1e-6) // two J = l +- 0.5 average to one
                        {
                            new_nbeta--;
                        }
                }
                    ...
                    if (l != 0)
                        {
                            ...
                            double vion1
                                = ((l + 1.0) * pp.dion (ind, ind) + l * pp.dion (ind1, ind1)) / (2.0 * l + 1.0);
                            ...
                            for (int ir = 0; ir < pp.mesh; ir++)
                                {
                                    double avera = 1.0 / (2.0 * l + 1.0)
                                                   * ((l + 1.0) * sqrtDplus * pp.betar (ind, ir)
                                                      + l * sqrtDminus * pp.betar (ind1, ir));
                                    double delta
                                        = 1.0 / (2.0 * l + 1.0)
                                          * (sqrtDplus * pp.betar (ind, ir) - sqrtDminus * pp.betar (ind1, ir));
                    ...
                    if (l != 0)
                        {
                            ...
                            for (int ir = 0; ir < pp.mesh; ir++)
                                {
                                    double avera = 0.5 * (pp.chi (ind, ir) + pp.chi (ind1, ir));
                                    double delta = 0.5 * (pp.chi (ind, ir) - pp.chi (ind1, ir));
...
void
    Pseudopot_upf::set_empty_element (Atom_pseudo& pp)
{
    pp.zv = 0;
 ...
 */
void
    Pseudopot_upf::set_upf_q (Atom_pseudo& pp)
{
...
void
    Pseudopot_upf::setqfnew (const int& nqf,
                             const int& mesh,
                             const int& l,
                             const int& n,
                             const double* qfcoef,
...
void
    Pseudopot_upf::skip_number (std::ifstream& ifs, bool mesh_changed)
{
    if (mesh_changed)
        {
            double temp = 0.;
