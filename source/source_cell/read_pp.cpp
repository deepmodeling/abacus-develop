    ...
    if (!PARAM.inp.lspinorb) {
            ...
            if (l != 0) {
                ...
                for (int ir = 0; ir < pp.mesh; ir++) {
                    pp.betar(nb, ir) = 1.0 / (2.0 * l + 1.0) *
                                       ((l + 1.0) * sqrt(std::abs(pp.dion(ind, ind) / vion1)) * pp.betar(ind, ir) +
                                        l * sqrt(std::abs(pp.dion(ind1, ind1) / vion1)) * pp.betar(ind1, ir));
                }
        ...
        for (int nb = 0; nb < pp.nchi; nb++) {
            ...
            if (l != 0) {
                ...
                // average chi
                for (int ir = 0; ir < pp.mesh; ir++) {
                    pp.chi(nb, ir) = 1.0 / (2.0 * l + 1.0) * ((l + 1.0) * pp.chi(ind, ir) + (l * pp.chi(ind1, ir)));
                }
                old_nwfc++;
            } else {
                for (int ir = 0; ir < pp.mesh; ir++) {
            ...
            if (l != 0) {
                ...
                for (int ir = 0; ir < pp.mesh; ir++) {
                    double avera = 1.0 / (2.0 * l + 1.0) *
                                   ((l + 1.0) * sqrtDplus * pp.betar(ind, ir) + l * sqrtDminus * pp.betar(ind1, ir));
                    double delta =
                        1.0 / (2.0 * l + 1.0) * (sqrtDplus * pp.betar(ind, ir) - sqrtDminus * pp.betar(ind1, ir));
