...
int
    CellIndex::get_ntype () const
{
    return this->atomCounts.size ();
}

int
    CellIndex::get_nat () const
{
    int nat = 0;
...
int
    CellIndex::get_nat (int it) const
{
    return this->atomCounts[it];
}

int
    CellIndex::get_nw () const
{
    int nw = 0;
...
int
    CellIndex::get_nw (int iat) const
{
...
int
    CellIndex::get_iwt (int iat, int orbital_index) const
{
...
int
    CellIndex::get_maxL (int iat) const
{
    int it = this->iat2it (iat);
...
/// @brief  get nchi
int
    CellIndex::get_nchi (int iat, int L) const
{
    int it = this->iat2it (iat);
...
std::string
    CellIndex::get_atom_label (int iat, bool order) const
{
    int it = this->iat2it (iat);
    int ia = this->iat2ia (iat);
...
int
    CellIndex::iat2it (int iat) const
{
    int running_iat = 0;
...
int
    CellIndex::iat2ia (int iat) const
{
    int it = this->iat2it (iat);
...
int
    CellIndex::iw2l (int iat, int iw) const
{
    int it = this->iat2it (iat);
...
int
    CellIndex::iw2z (int iat, int iw) const
{
    int it = this->iat2it (iat);
    int maxL = this->lnchiCounts[it].size () - 1;
    for (int L = 0; L <= maxL; ++L)
...
int
    CellIndex::iw2m (int iat, int iw) const
{
    int it = this->iat2it (iat);
    int maxL = this->lnchiCounts[it].size () - 1;
    for (int L = 0; L <= maxL; ++L)
...
bool
    CellIndex::check_nspin (int nspin)
{
...
void
    CellIndex::write_orb_info (const std::string& out_dir) const
{
    std::stringstream os;
    os << out_dir << "Orbital";
    std::ofstream out (os.str ().c_str ());
    out << std::setw (5) << "#io" << std::setw (8) << "spec" << std::setw (5) << "l" << std::setw (5) << "m"
        << std::setw (5) << "z" << std::setw (5) << "sym" << std::endl;
            ...
            for (int iw = 0; iw < this->get_nw (iat); ++iw)
                {
                    ...
                    out << std::setw (5) << iat << std::setw (8) << this->get_atom_label (iat) << std::setw (5) << L
                        << std::setw (5) << M << std::setw (5) << Z + 1 << std::setw (15)
