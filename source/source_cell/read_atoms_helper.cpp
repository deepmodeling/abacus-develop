...
void process_magnetization(Atom& atom,
    ...
    if (nspin == 4) {
        ...
        if (ia == 0 || (atom.m_loc_[ia].x != atom.m_loc_[0].x || atom.m_loc_[ia].y != atom.m_loc_[0].y ||
            ...
            ModuleBase::GlobalFunc::OUT(ofs_running, ss.str(), atom.m_loc_[ia].x, atom.m_loc_[ia].y, atom.m_loc_[ia].z);
        }
    ...
    while ((tmpid != "\n") && (ifpos.good()) && (tmpid != "#")) {
        ...
        if (tmpid == "m") {
            ...
            if ((tmp >= DIGIT_START && tmp <= DIGIT_END) or tmp == MINUS_SIGN) {
                ...
                atom.mag[ia] = sqrt(pow(atom.m_loc_[ia].x, 2) + pow(atom.m_loc_[ia].y, 2) + pow(atom.m_loc_[ia].z, 2));
                input_vec_mag = true;
