...
bool
    validate_coordinate_system (const std::string& Coordinate, std::ofstream& ofs_warning)
{
...
void
    allocate_atom_properties (Atom& atom, int na, double mass)
{
...
void
    set_atom_movement_flags (Atom& atom, int ia, const ModuleBase::Vector3<int>& mv)
{
...
void
    autoset_magnetization (UnitCell& ucell, int nspin, std::ofstream& ofs_running)
{
                            ...
                            for (int ia = 0; ia < ucell.atoms[it].na; ia++)
                                {
                                    ...
                                    ModuleBase::GlobalFunc::OUT (ofs_running,
                                                                 "Autoset magnetism for this atom",
                                                                 1.0,
                                                                 1.0,
                                                                 1.0);
                                }
...
bool
    finalize_atom_positions (UnitCell& ucell, std::ofstream& ofs_running, std::ofstream& ofs_warning)
{
...
ModuleBase::Vector3<double>
    calculate_lattice_center (const ModuleBase::Matrix3& latvec, const std::string& center_mode)
{
...
void
    transform_atom_coordinates (Atom& atom,
                                int ia,
                                ...
                                ModuleBase::Vector3<double>& latcenter)
{
    ...
    if (Coordinate == "Cartesian" || Coordinate == "Cartesian_angstrom" || Coordinate == "Cartesian_angstrom_center_xy"
        || Coordinate == "Cartesian_angstrom_center_xz" || Coordinate == "Cartesian_angstrom_center_yz"
        || Coordinate == "Cartesian_angstrom_center_xyz" || Coordinate == "Cartesian_au")
        {
            double dx = 0.0;
            double dy = 0.0;
            double dz = 0.0;
...
void
    process_magnetization (Atom& atom,
                           int it,
                           int ia,
                           int nspin,
                           bool input_vec_mag,
    ...
    if (input_angle_mag)
        {
            ...
            if (std::abs (sin (atom.angle1[ia])) > 1e-10)
                {
                    atom.m_loc_[ia].x = atom.mag[ia] * sin (atom.angle1[ia]) * cos (atom.angle2[ia]);
                    atom.m_loc_[ia].y = atom.mag[ia] * sin (atom.angle1[ia]) * sin (atom.angle2[ia]);
                }
        }
    ...
    if (nspin == 4)
        {
            ...
            if (ia == 0
                || (atom.m_loc_[ia].x != atom.m_loc_[0].x || atom.m_loc_[ia].y != atom.m_loc_[0].y
...
bool
    parse_atom_properties (std::ifstream& ifpos,
                           Atom& atom,
                           int ia,
                           ...
                           bool& set_element_mag_zero)
{
    ...
    if ((int)tmpid[0] < 0)
        {
            std::cout << "read_atom_positions, mismatch in atom number for atom type: " << atom.label << std::endl;
            exit (1);
        }
                    ...
                    if ((tmp >= DIGIT_START && tmp <= DIGIT_END) or tmp == MINUS_SIGN)
                        {
                            ...
                            atom.mag[ia] = sqrt (pow (atom.m_loc_[ia].x, 2) + pow (atom.m_loc_[ia].y, 2)
