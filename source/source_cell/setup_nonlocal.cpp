...
void
    InfoNonlocal::Set_NonLocal (const int& it,
                                Atom* atom,
                                int& n_projectors,
                                const int& kmesh,
                                const double& dk,
                                                    ...
                                                    for (int is2 = 0; is2 < 2; is2++)
                                                        {
                                                            if (atom->ncpp.has_so)
                                                            ...
                                                            else
                                                                {
                                                                    if (is1 == is2 && m1 == m2)
                                                                        {
                                                                            coefficient_D_nc_in (ip1 + nh * is1,
                                                                                                 ip2 + nh * is2)
                                                                                = atom->ncpp.dion (p1, p2);
                                                                        }
                                                                }
...
void
    InfoNonlocal::Read_NonLocal (const int& it,
                                 Atom* atom,
                                 int& n_projectors,
                                 const int& my_rank,
                                 const int& kmesh,
            ...
            if (!find_lmax)
                {
                    ...
                    for (int ib = 0; ib < atom->ncpp.nbeta; ++ib)
                        {
                            std::cout << " Max L Read in from pseudopotential file = " << atom->ncpp.lll[ib]
                                      << std::endl;
                        }
                            ...
                            if (iproj != p1)
                                {
                                    ...
                                    ModuleBase::WARNING_QUIT ("InfoNonlocal::Read_NonLocal",
                                                              "Check non-local projector index.");
                                }
...
void
    InfoNonlocal::setupNonlocal (const int& ntype, Atom* atoms, std::ofstream& log, LCAO_Orbitals& orb)
{
    //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    //~~~~~~~~~~~~~~~~~~~~~~   2    ~~~~~~~~~~~~~~~~~~~~~~~~~
