...
#ifdef __LCAO
void
    LCAO_Orbitals::bcast_files (const int& ntype_in, const int& my_rank)
{
    return;
}
#endif
    ...
    for (int i = 0; i < latname_in.size (); ++i)
        {
            ...
            if (latname_in[i] == "sc")
                {
                    double celldm = std::sqrt (pow (ucell->latvec.e11, 2) + pow (ucell->latvec.e12, 2)
                                               + pow (ucell->latvec.e13, 2));
                ...
                {
                    double celldm1 = std::sqrt (pow (ucell->latvec.e11, 2) + pow (ucell->latvec.e12, 2)
                                                + pow (ucell->latvec.e13, 2));
                    double celldm3 = std::sqrt (pow (ucell->latvec.e31, 2) + pow (ucell->latvec.e32, 2)
                                                + pow (ucell->latvec.e33, 2));
                ...
                {
                    double a1 = std::sqrt (pow (ucell->latvec.e11, 2) + pow (ucell->latvec.e12, 2)
                                           + pow (ucell->latvec.e13, 2));
                    double a2 = std::sqrt (pow (ucell->latvec.e21, 2) + pow (ucell->latvec.e22, 2)
                                           + pow (ucell->latvec.e23, 2));
            ...
            else if (latname_in[i] == "st")
                {
                    double a1 = std::sqrt (pow (ucell->latvec.e11, 2) + pow (ucell->latvec.e12, 2)
                                           + pow (ucell->latvec.e13, 2));
                    double a3 = std::sqrt (pow (ucell->latvec.e31, 2) + pow (ucell->latvec.e32, 2)
                                           + pow (ucell->latvec.e33, 2));
            ...
            else if (latname_in[i] == "triclinic")
                {
                    ...
                    double d1 = std::sqrt (1.0 + 2.0 * cosgamma * cosbeta * cosalpha - cosgamma * cosgamma
                                           - cosbeta * cosbeta - cosalpha * cosalpha)
                                / singamma;
                    EXPECT_DOUBLE_EQ (ucell->latvec.e11, a1);
                    EXPECT_DOUBLE_EQ (ucell->latvec.e12, 0.0);
                    EXPECT_DOUBLE_EQ (ucell->latvec.e13, 0.0);
                            ...
                            for (int ja = 0; ja < utp.natom[jt]; ++ja)
                                {
                                    ...
                                    if (it_test == utp.natom.size () - 1 && ia_test == utp.natom[it] - 1
                                        ...
                                        {
                                            EXPECT_FALSE (
                                                ucell->step_jajtiait (&ja_test, &jt_test, &ia_test, &it_test));
                                        }
                                }
                        }
                }
...
TEST_F (UcellTest, PeriodicBoundaryAdjustment2)
{
    ...
    PARAM.input.relax_new = utp.relax_new;
    ucell = utp.SetUcellInfo ();
    EXPECT_NO_THROW (unitcell::periodic_boundary_adjustment (ucell->atoms, ucell->latvec, ucell->ntype));
}
...
TEST_F (UcellTest, PrintSTRU)
{
     ...
     *
     */
    unitcell::print_stru_file (*ucell, ucell->atoms, ucell->latvec, fn, 1, false, false, false, false, false, 0);
    std::ifstream ifs;
    ifs.open ("C1H2_STRU");
    std::string str ((std::istreambuf_iterator<char> (ifs)), std::istreambuf_iterator<char> ());
     ...
     *
     */
    unitcell::print_stru_file (*ucell, ucell->atoms, ucell->latvec, fn, 2, true, true, false, false, false, 0);
    ifs.open ("C1H2_STRU");
    ...
    unitcell::print_stru_file (*ucell, ucell->atoms, ucell->latvec, fn, 2, true, false, true, true, true, 0);
    ifs.open ("C1H2_STRU");
    str = {(std::istreambuf_iterator<char> (ifs)), std::istreambuf_iterator<char> ()};
    EXPECT_THAT (str, testing::HasSubstr ("ATOMIC_SPECIES"));
    EXPECT_THAT (str, testing::HasSubstr ("C  12.0000 C.upf upf201"));
...
TEST_F (UcellTestReadStru, ReadAtomPositionsWarning4)
{
    ...
    ucell->magnet.start_mag = new double[ucell->ntype];
    testing::internal::CaptureStdout ();
    EXPECT_EXIT (unitcell::read_atom_positions (*ucell, ifa, ofs_running, ofs_warning),
                 ::testing::ExitedWithCode (1),
                 "");
    output = testing::internal::GetCapturedStdout ();
