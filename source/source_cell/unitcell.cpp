...
void
    UnitCell::print_cell (std::ofstream& ofs) const
{
...
std::map<int, int>
    UnitCell::get_atom_Counts () const
{
...
std::map<int, int>
    UnitCell::get_orbital_Counts () const
{
...
std::map<int, std::map<int, int>>
    UnitCell::get_lnchi_Counts () const
{
...
std::vector<std::vector<int>>
    UnitCell::get_lnchiCounts () const
{
...
//==============================================================
void
    UnitCell::setup_cell (const std::string& fn, std::ofstream& log)
{
                    ...
                    if (PARAM.inp.dfthalf_type > 0)
                        {
                            ...
                            // ok3 = GlobalC::sep_cell.read_sep_potentials(ifa, PARAM.inp.pseudo_dir,
                            // GlobalV::ofs_warning, this->atom_label);

                            sep_cell.init (this->ntype);
                            ok3 = sep_cell.read_sep_potentials (ifa,
...
void
    UnitCell::set_iat2iwt (const int& npol_in)
{
#ifdef __DEBUG
...
bool
    UnitCell::if_atoms_can_move () const
{
    for (int it = 0; it < this->ntype; it++)
        {
            Atom* atom = &atoms[it];
...
bool
    UnitCell::if_cell_can_change () const
{
...
void
    UnitCell::compare_atom_labels (const std::string& label1, const std::string& label2) const
{
