...
{
void
    bcast_atoms_tau (Atom* atoms, const int ntype)
{
#ifdef __MPI
...
void
    bcast_atoms_pseudo (Atom* atoms, const int ntype)
{
#ifdef __MPI
    MPI_Barrier (MPI_COMM_WORLD);
    for (int i = 0; i < ntype; i++)
...
void
    bcast_magnetism (Magnetism& magnet, const int ntype)
{
#ifdef __MPI
    MPI_Barrier (MPI_COMM_WORLD);
    ...
    bcast_unitcell (UnitCell& ucell)
{
    ...
    ModuleBase::bcast_data_cereal (GlobalC::exx_info.info_opt_abfs.files_abfs, MPI_COMM_WORLD, 0);
    ModuleBase::bcast_data_cereal (GlobalC::exx_info.info_opt_abfs.files_jles, MPI_COMM_WORLD, 0);
#endif
    return;
#endif
}
