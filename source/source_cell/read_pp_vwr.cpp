...
//-----------------------------------------------------------
int
    Pseudopot_upf::read_pseudo_vwr (std::ifstream& ifs, Atom_pseudo& pp)
{
    ...
    GlobalV::ofs_running << std::setw (15) << "OCCUPATION" << std::setw (15) << tmp_oc[0] << std::setw (15) << tmp_oc[1]
                         << std::setw (15) << tmp_oc[2] << std::endl;
    // (7) spin orbital
    ifs >> pp.has_so;
    ...
    GlobalV::ofs_running << std::setw (15) << "Vnl_USED" << std::setw (15) << iref_s << std::setw (15) << iref_p
                         << std::setw (15) << iref_d << std::endl;
    if (spd_loc == 1)
        {
            iref_s = 0;
        }
    ...
    GlobalV::ofs_running << std::setw (15) << "Orb_USED" << std::setw (15) << iTB_s << std::setw (15) << iTB_p
                         << std::setw (15) << iTB_d << std::endl;
    ...
    if (spd_loc > 0 && pp.nlcc == 0)
            ...
            for (int ir = 0; ir < pp.mesh; ++ir)
                {
                    ...
                    ifs >> pp.r[ir] >> vs[ir] >> vp[ir] >> vd[ir] >> ws[ir] >> wp[ir] >> wd[ir];
                    getline (ifs, line);
                }
        }
    else if (spd_loc == 0 && pp.nlcc == 0)
        {
            for (int ir = 0; ir < pp.mesh; ++ir)
                {
                    ifs >> pp.r[ir] >> vs[ir] >> vp[ir] >> vd[ir] >> ws[ir] >> wp[ir] >> wd[ir] >> pp.vloc_at[ir];
                    getline (ifs, line);
                }
        }
    else if (spd_loc > 0 && pp.nlcc == 1)
        {
            for (int ir = 0; ir < pp.mesh; ++ir)
                {
                    ifs >> pp.r[ir] >> vs[ir] >> vp[ir] >> vd[ir] >> ws[ir] >> wp[ir] >> wd[ir] >> pp.rho_atc[ir];
                    getline (ifs, line);
                }
        }
    else if (spd_loc == 0 && pp.nlcc == 1)
    ...
    GlobalV::ofs_running << std::setw (15) << "WFC_UNIT" << std::setw (15) << units << std::setw (15) << unitp
                         << std::setw (15) << unitd << std::endl;
    ...
    // mohan 2013-06-28
    if (std::abs (units - 1.0) > 0.2 && (iTB_s == 1 || iref_s == 1))
        {
            return 3;
        }
    if (std::abs (unitp - 1.0) > 0.2 && (iTB_p == 1 || iref_p == 1))
        {
            return 3;
        }
    if (std::abs (unitd - 1.0) > 0.2 && (iTB_d == 1 || iref_d == 1))
        {
            return 3;
        }
    ...
    for (int ir = 0; ir < pp.mesh; ++ir)
        {
            ...
            if (iTB_s == 1)
                {
                    pp.chi (iwfc, ir) = ws[ir] * pp.r[ir];
                    ++iwfc;
                }
            if (iTB_p == 1)
                {
                    pp.chi (iwfc, ir) = wp[ir] * pp.r[ir];
                    ++iwfc;
                }
            if (iTB_d == 1)
                {
                    pp.chi (iwfc, ir) = wd[ir] * pp.r[ir];
                    ++iwfc;
                }
        }
    // rho atom
    ...
    if (iref_d == 1)
    ...
    else
        {
            ...
            std::cout << " Could not decide which is the max angular momentum from .vwr pseudopotential file."
                      << std::endl;
    ...
    if (iref_s == 1)
        {
            ++pp.nbeta; // add one s projector
        }
    if (iref_p == 1)
        {
            ++pp.nbeta; // add one p projector
        }
    if (iref_d == 1)
        {
            ++pp.nbeta; // add one p projector
                    ...
                    if (pp.r[ir] < rcut)
                        {
                            coef
                                = coef
                                  + (vl[ir] - pp.vloc_at[ir]) * wlr[ir] * wlr[ir] * (pp.r[ir + 1] - pp.r[ir - 1]) / 2.0;
                        }
                }
