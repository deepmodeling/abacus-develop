...
class UnitCell
{
  ...
  public:
    ...
    inline bool
        iat2iait (const Tiat iat, Tiait* ia, Tiait* it) const
    {
        if (iat >= nat)
            {
                *ia = 0;
    ...
    template <typename Tiat, typename Tiait>
    inline bool
        ijat2iaitjajt (const Tiat ijat, Tiait* ia, Tiait* it, Tiait* ja, Tiait* jt) const
    {
        Tiat iat = ijat / nat;
        Tiat jat = ijat % nat;
        iat2iait (iat, ia, it);
    ...
    template <typename Tiait>
    inline bool
        step_ia (const Tiait it, Tiait* ia) const
    {
        if (++(*ia) >= atoms[it].na)
            {
                *ia = 0;
    ...
    template <typename Tiait>
    inline bool
        step_iait (Tiait* ia, Tiait* it) const
    {
        if (step_ia (*it, ia))
            {
                return step_it (it);
