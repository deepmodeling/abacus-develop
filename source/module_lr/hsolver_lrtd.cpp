        ...
        const bool skip_charge)
    {
        ...
        else
        {
            ...
            if (this->method == "dav")
            {
                ...
                hsolver::DiagoDavid<T, Device> david(precondition.data(), nband, GlobalV::PW_DIAG_NDIM, GlobalV::use_paw, comm_info);
                hsolver::DiagoIterAssist<T, Device>::avg_iter += static_cast<double>(david.diag(hpsi_func, spsi_func,
                    dim, dim, psi_k1_dav.get_pointer(), eigenvalue.data(), this->diag_ethr, david_maxiter, ntry_max, 0));
            }
