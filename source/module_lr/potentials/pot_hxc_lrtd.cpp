...
void PotHxcLR::cal_v_eff(double** rho,
                         const UnitCell& ucell,
                         ...
                         const std::vector<int>& ispin_op)
{
    ...
    case SpinType::S2_singlet:
        v_eff
            += 2
               * elecstate::H_Hartree_pw::v_hartree(ucell, const_cast<ModulePW::PW_Basis*>(&this->rho_basis_), 1, rho);
        break;
    default:
        break;
    }
    ...
    else if (xc == XCType::GGA || xc == XCType::HYB_GGA)
    {
        ...
        case SpinType::S1:
            funcs[s] = [this, &fxc](FXC_PARA_TYPE) -> void {
                ...
                for (int ir = 0; ir < nrxx; ++ir)
                {
                    vxc_tmp[ir] += (fxc.v2rho2.at(ir) * rho[ir] + fxc.v2rhosigma_2drho.at(ir) * drho.at(ir));
                }
        ...
        case SpinType::S2_updown:
            funcs[s] = [this, &fxc](FXC_PARA_TYPE) -> void {
                ...
                case 1:
                    ...
                    {
                        vxc_tmp[ir]
                            += rho[ir] * fxc.v2rho2.at(ir * 3 + 1) + drho.at(ir) * fxc.v2rhosigma_drho_ud.at(ir);
                    }
                    break;
                case 2:
                    for (int ir = 0; ir < nrxx; ++ir)
                    {
                        vxc_tmp[ir]
                            += rho[ir] * fxc.v2rho2.at(ir * 3 + 1) + drho.at(ir) * fxc.v2rhosigma_drho_du.at(ir);
                    }
                    break;
                case 3:
                    for (int ir = 0; ir < nrxx; ++ir)
                    {
                        vxc_tmp[ir]
                            += rho[ir] * fxc.v2rho2.at(ir * 3 + 2) + drho.at(ir) * fxc.v2rhosigma_drho_dd.at(ir);
                    }
                    break;
                default:
                    throw std::runtime_error("Invalid ispin_op");
        ...
        default:
            throw std::domain_error("SpinType =" + std::to_string(static_cast<int>(s))
                                    + "for GGA or HYB_GGA is unfinished in " + std::string(__FILE__) + " line "
