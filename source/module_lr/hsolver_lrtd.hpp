           ...
           const bool hermitian = true)
{
    ...
    if (method == "lapack")
    {
        ...
        if (hermitian)
        {
            LR_Util::diag_lapack(gdim, Amat_full.data(), eigenvalue.data());
        }
        else
        {
            ...
            {
                eigenvalue[i] = eig_complex[i].real();
            }
        }
        // copy eigenvectors
    ...
    else
    {
        ...
        auto hpsi_func
            = [&hm](T* psi_in, T* hpsi, const int ld_psi, const int nvec) { hm.hPsi(psi_in, hpsi, ld_psi, nvec); };
        ...
        if (method == "dav")
        {
            ...
            hsolver::DiagoDavid<T> david(precondition.data(), nband, dim, PARAM.inp.pw_diag_ndim, false, comm_info);
            std::vector<double> ethr_band(nband, diag_ethr);
            hsolver::DiagoIterAssist<T>::avg_iter += static_cast<double>(
                david.diag(hpsi_func, spsi_func, dim, psi, eigenvalue.data(), ethr_band, maxiter, ntry_max, 0));
        }
        else if (method == "dav_subspace") // need refactor
        {
            ...
            std::vector<double> ethr_band(nband, diag_ethr);
            hsolver::DiagoIterAssist<T>::avg_iter += static_cast<double>(
                dav_subspace.diag(hpsi_func, psi, dim, eigenvalue.data(), ethr_band, false /*scf*/));
        }
        else if (method == "cg")
        {
            ...
            //     hsolver::DiagoIterAssist<T>::diagH_subspace(hm, psi_in_wrapper, psi_out_wrapper,
            //     eigen.data<Real<T>>());
            //     };
            ...
            auto psi_tensor = ct::TensorMap(psi,
                                            ct::DataTypeToEnum<T>::value,
                                            ct::DeviceType::CpuDevice,
                                            ct::TensorShape({nband, dim}));
            auto eigen_tensor = ct::TensorMap(eigenvalue.data(),
                                              ct::DataTypeToEnum<Real<T>>::value,
                                              ct::DeviceType::CpuDevice,
                                              ct::TensorShape({nband}));
            std::vector<Real<T>> precondition_(precondition); // since TensorMap does not support const pointer
            auto precon_tensor = ct::TensorMap(precondition_.data(),
                                               ct::DataTypeToEnum<Real<T>>::value,
                                               ct::DeviceType::CpuDevice,
                                               ct::TensorShape({dim}));
            auto hpsi_func = [&hm](const ct::Tensor& psi_in, ct::Tensor& hpsi) {
                hm.hPsi(psi_in.data<T>(),
                        hpsi.data<T>(),
                        psi_in.shape().dim_size(0) /*nbasis_local*/,
                        1 /*band-by-band*/);
            };
