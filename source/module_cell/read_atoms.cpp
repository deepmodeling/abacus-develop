...
#endif
{
	...
	{	
		...
		for (int i = 0;i < ntype;i++)
		{
			...
			this->atoms[i].atom_pp.flag_empty_element = 
...
#endif
{
		...
		for (int it = 0;it < ntype; it++)
		{
			...
			if (na > 0)
			{
				...
				atoms[it].vel = new ModuleBase::Vector3<double>[na];
       			atoms[it].atom_pp.mbl = new ModuleBase::Vector3<int>[na];
				...
				atoms[it].atom_pp.mass = this->atom_mass[it]; //mohan add 2011-11-07 
 ...
 // modify the reading of frozen ions and velocities  -- Yuanbo Li 2021/8/20
					...
					{
						atoms[it].atom_pp.mbl[ia] = mv;
					}
					else
					{
						atoms[it].atom_pp.mbl[ia] = 0.0;
						atoms[it].atom_pp.mbl[ia].print();
					}
		...
		for(int it=0; it<ntype; it++)
		{
			...
			for(int ia=0; ia<atoms[it].na; ia++)
			{
				ofs << atoms[it].tau[ia].x << "  " << atoms[it].tau[ia].y << "  " << atoms[it].tau[ia].z
					<< "  m  " << atoms[it].atom_pp.mbl[ia].x << "  " << atoms[it].atom_pp.mbl[ia].y << "  " << atoms[it].atom_pp.mbl[ia].z;
