 ...
 *   - PrintCell
 *     - print_cell(ofs): print basic info into ofs
 *   - PrintCellCif
	...
	std::unique_ptr<UnitCell> SetUcellInfo()
	{
		...
		for(int it=0;it<ucell->ntype;++it)
		{
			...
			for(int ia=0; ia<ucell->atoms[it].na; ++ia)
			{
				...
				{
					ucell->atoms[it].mbl[ia].x = this->velocity[this->atomic_index*3+0];
					ucell->atoms[it].mbl[ia].y = this->velocity[this->atomic_index*3+1];
					ucell->atoms[it].mbl[ia].z = this->velocity[this->atomic_index*3+2];
				}
				else
				{
