 ...
 *     - ReadKpointsKspacing1kpoint: automatically set GlobalV::GAMMA_ONLY_LOCAL = 1 when there is only one kpoint
 * generating KPT from kspacing parameter
...
std::vector<stru_> stru_lib{stru_{1, "O_h", "m-3m", "Pm-3m", std::vector<double>{1., 0., 0., 0., 1., 0., 0., 0., 1.},
                                  std::vector<atomtype_>{atomtype_{"C", std::vector<std::vector<double>>{
                                                                            {0., 0., 0.},
                                                                        }}}}};
        ...
        for (int i = 0; i < coord.size(); i++)
        {
            ...
            for (int j = 0; j < ucell.atoms[i].na; j++)
            {
                ...
                ModuleBase::Mathzone::Cartesian_to_Direct(
                    ucell.atoms[i].tau[j].x, ucell.atoms[i].tau[j].y, ucell.atoms[i].tau[j].z, ucell.a1.x, ucell.a1.y,
                    ucell.a1.z, ucell.a2.x, ucell.a2.y, ucell.a2.z, ucell.a3.x, ucell.a3.y, ucell.a3.z,
                    ucell.atoms[i].taud[j].x, ucell.atoms[i].taud[j].y, ucell.atoms[i].taud[j].z);
            }
...
TEST_F(KlistTest, ReadKpointsWarning6)
{
    ...
    std::string str((std::istreambuf_iterator<char>(ifs)), std::istreambuf_iterator<char>());
    EXPECT_THAT(str,
                testing::HasSubstr("Line mode of k-points is open, automatically set symmetry to 0 to continue ..."));
    ifs.close();
