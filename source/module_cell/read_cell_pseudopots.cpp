...
//xiaohui add 2015-03-24
		...
		if(GlobalV::MY_RANK==0)
		{
			if (atoms[i].ncpp.psd != upf.psd){
				std::cout << " Pseudopotential directory now is : " << pp_address << std::endl;
				GlobalV::ofs_warning << " Pseudopotential directory now is : " << pp_address << std::endl;
				ModuleBase::WARNING_QUIT("read_pseudopot","Different kinds of atoms between atom in STRU in file and psedopotention file");
			}
			else{
	//			upf.print_pseudo_upf( ofs );
				...
				ModuleBase::GlobalFunc::OUT(log,"pseudopotential atom type",atoms[i].ncpp.psd);
