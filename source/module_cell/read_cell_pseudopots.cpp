...
void UnitCell_pseudo::read_cell_pseudopots(const std::string &pp_dir, std::ofstream &log)
{
	...
	for (int i = 0;i < ntype;i++)
	{
		...
		if(GlobalV::MY_RANK==0)
		{
			...
			{
				if(this->atoms[i].atom_pp.flag_empty_element)	// Peize Lin add for bsse 2021.04.07
				{
...
//			upf.print_pseudo_upf( ofs );
			atoms[i].atom_pp.set_pseudo_nc( upf );

			log << "\n Read in pseudopotential file is " << pseudo_fn[i] << std::endl;
			ModuleBase::GlobalFunc::OUT(log,"pseudopotential type",atoms[i].atom_pp.pp_type);
			ModuleBase::GlobalFunc::OUT(log,"exchange-correlation functional", atoms[i].atom_pp.xc_func);
			ModuleBase::GlobalFunc::OUT(log,"nonlocal core correction", atoms[i].atom_pp.nlcc);
//			ModuleBase::GlobalFunc::OUT(log,"spin orbital",atoms[i].has_so);
			ModuleBase::GlobalFunc::OUT(log,"valence electrons", atoms[i].atom_pp.zv);
			ModuleBase::GlobalFunc::OUT(log,"lmax", atoms[i].atom_pp.lmax);
			ModuleBase::GlobalFunc::OUT(log,"number of zeta", atoms[i].atom_pp.nchi);
			ModuleBase::GlobalFunc::OUT(log,"number of projectors", atoms[i].atom_pp.nbeta);
			for(int ib=0; ib<atoms[i].atom_pp.nbeta; ib++)
			{
				ModuleBase::GlobalFunc::OUT(log,"L of projector", atoms[i].atom_pp.lll[ib]);
			}
//			ModuleBase::GlobalFunc::OUT(log,"Grid Mesh Number", atoms[i].mesh);
		...
		if(upf.functional_error == 1)
		{
			...
			atoms[i].atom_pp.xc_func = GlobalV::DFT_FUNCTIONAL;
			transform(atoms[i].atom_pp.xc_func.begin(), atoms[i].atom_pp.xc_func.end(), atoms[i].atom_pp.xc_func.begin(), (::toupper));
			if(GlobalV::MY_RANK==0)
			{
