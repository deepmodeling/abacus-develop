...
void LambdaLoop::run_lambda_loop(int outer_step)
{
    ...
    for (int i_step = 0; i_step < num_step; i_step++)
    {
        ...
        else
        {
            ...
            */
            ...
            for (int it = 0; it < ntype; it++)
            {
                ...
                {
                    std::cout << "Reach limitation of current step ( maximum gradient < " << bound_gradient[it] << " in atom type " << it << " ), exit." << std::endl;
        ...
        if (debug)
        {
            ...
            print_2d("(Debug) target spin:", target_spin);
        }
        ...
        //
        where_fill_scalar_else_2d(constrain, 0, 0.0, target_spin, target_spin_mask);
