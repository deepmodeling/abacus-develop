...
void fill_scalar_2d(double scalar,
                    std::vector<std::vector<double>> &result)
{
    for (auto &row : result)
    {
...
void where_fill_scalar_2d(const std::vector<std::vector<int>> &array_mask,
                          ...
                          std::vector<std::vector<double>> &result)
{
    int size_1 = result.size();
    int size_2 = result[0].size();
    for (int i = 0; i < size_1; i++)
    {
        for (int j = 0; j < size_2; j++)
        {
...

{
    ...
    for (int i_step = 0; i_step < num_step; i_step++)
    {
        ...
        else
        {
            ...
            {
                std::cout << "(" << max_gradient_index[0][i] << "," << max_gradient_index[1][i] << ")" << std::endl;
            }
            ...
            for (int i = 0; i < NTYP; i++)
            {
                ...
                {
                    std::cout << "Reach limitation of current step ( maximum gradient < " << bound_gradient[i] << " in atom type " << i << " ), exit." << std::endl;
        ...
        std::cout << "Step (Outer -- Inner) =  " << N << " -- " << i_step + 1 << "       RMS =", rms_error << std::endl;
        ...
        for (int i = 0; i < 3; i++)
        {
            for (int j = 0; j < NIONS; j++)
            {
                temp_1[i][j] = (target_spin_mask[i][j] - spin_mask[i][j]) * (spin_plus_mask[i][j] - spin_mask[i][j]);
                temp_2[i][j] = pow(spin_mask[i][j] - spin_plus_mask[i][j], 2)
            }
        }
        sum_k = sum_2d(temp_1);
