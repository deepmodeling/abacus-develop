...
void Grid_MeshCell::set_grid_dim(const int& ncx_in, const int& ncy_in, const int& ncz_in, const int& bx_in,
                                 ...
                                 const int& nbz_in, const int& nbxx_in, const int& nbzp_start_in, const int& nbzp_in)
{
    ...
    if (GlobalV::OUT_LEVEL != "m")
    {
        ModuleBase::GlobalFunc::OUT(GlobalV::ofs_running, "big cell numbers in grid", nbx, nby,
                                    nbz); // reduced by BIG_CELL
    }

    if (GlobalV::OUT_LEVEL != "m")
    {
        ModuleBase::GlobalFunc::OUT(GlobalV::ofs_running, "meshcell numbers in big cell", bx, by,
...
void Grid_MeshCell::init_latvec(const UnitCell& ucell)
{
    ...
    if (GlobalV::test_gridt)
    {
        ...
        GlobalV::ofs_running << " vec1( " << std::setw(15) << meshcell_vec1[0] << std::setw(15) << meshcell_vec1[1]
                             << std::setw(15) << meshcell_vec1[2] << ")" << std::endl;

        GlobalV::ofs_running << " vec2( " << std::setw(15) << meshcell_vec2[0] << std::setw(15) << meshcell_vec2[1]
                             << std::setw(15) << meshcell_vec2[2] << ")" << std::endl;

        GlobalV::ofs_running << " vec3( " << std::setw(15) << meshcell_vec3[0] << std::setw(15) << meshcell_vec3[1]
                             << std::setw(15) << meshcell_vec3[2] << ")" << std::endl;
    }
