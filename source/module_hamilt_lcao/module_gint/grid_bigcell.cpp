...
void Grid_BigCell::init_big_latvec(const UnitCell& ucell)
{
    ...
    if (GlobalV::test_gridt)
    {
        ...
        GlobalV::ofs_running << " vec1( " << std::setw(15) << bigcell_vec1[0] << std::setw(15) << bigcell_vec1[1]
                             << std::setw(15) << bigcell_vec1[2] << ")" << std::endl;

        GlobalV::ofs_running << " vec2( " << std::setw(15) << bigcell_vec2[0] << std::setw(15) << bigcell_vec2[1]
                             << std::setw(15) << bigcell_vec2[2] << ")" << std::endl;

        GlobalV::ofs_running << " vec3( " << std::setw(15) << bigcell_vec3[0] << std::setw(15) << bigcell_vec3[1]
                             << std::setw(15) << bigcell_vec3[2] << ")" << std::endl;
    }
    return;
}
...
void Grid_BigCell::init_grid_expansion(const UnitCell& ucell, double* rcut)
{
    ...
    if (GlobalV::test_gridt)
        ModuleBase::GlobalFunc::OUT(GlobalV::ofs_running, "rmax of periodic grid (bohr)", orbital_rmax);
    ...
    double g1
        = sqrt(bigcell_GT.e11 * bigcell_GT.e11 + bigcell_GT.e21 * bigcell_GT.e21 + bigcell_GT.e31 * bigcell_GT.e31);

    double g2
        = sqrt(bigcell_GT.e12 * bigcell_GT.e12 + bigcell_GT.e22 * bigcell_GT.e22 + bigcell_GT.e32 * bigcell_GT.e32);

    double g3
        = sqrt(bigcell_GT.e13 * bigcell_GT.e13 + bigcell_GT.e23 * bigcell_GT.e23 + bigcell_GT.e33 * bigcell_GT.e33);
    ...
    if (GlobalV::OUT_LEVEL != "m")
        ModuleBase::GlobalFunc::OUT(GlobalV::ofs_running, "extended fft grid", dxe, dye, dze);
    ...
    if (GlobalV::OUT_LEVEL != "m")
        ModuleBase::GlobalFunc::OUT(GlobalV::ofs_running, "dimension of extened grid", nxe, nye, nze);
    return;
}
    ...
    for (int it = 0; it < ucell.ntype; it++)
    {
        for (int ia = 0; ia < ucell.atoms[it].na; ia++)
        {
            ...
            if (fraction.x < 0 || fraction.y < 0 || fraction.z < 0)
            {
                ...
                std::cout << fraction.z << " ";
                std::cout << std::endl;
                ModuleBase::WARNING_QUIT("Grid_BigCell::init_tau_in_bigcell",
            ...
            */
            ...
            {
                this->tau_in_bigcell[iat][ic] = delta[0] * this->bigcell_vec1[ic] + delta[1] * this->bigcell_vec2[ic]
