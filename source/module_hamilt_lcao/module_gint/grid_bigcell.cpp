...
void Grid_BigCell::init_big_latvec(void)
{
    ...
    {
        GlobalV::ofs_running << " the VECTORS of BIGCELL are (Bohr): "
                             << std::endl;
...
void Grid_BigCell::init_tau_in_bigcell(void)
{
    ...
    for (int it = 0; it < GlobalC::ucell.ntype; it++)
    {
        for (int ia = 0; ia < GlobalC::ucell.atoms[it].na; ia++)
        {
            ...
            // not used because the factrion may be <0 (although very small,
            // such as -1.0e-15) mohan note 2012-07-03 fraction = (
            // GlobalC::ucell.atoms[it].tau[ia] * GlobalC::ucell.lat0 )*
            // this->bigcell_GT;
            ...
            //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            fraction.x
                = GlobalC::ucell.atoms[it].taud[ia].x / (1.0 / (double)nbx);
            fraction.y
                = GlobalC::ucell.atoms[it].taud[ia].y / (1.0 / (double)nby);
            fraction.z
                = GlobalC::ucell.atoms[it].taud[ia].z / (1.0 / (double)nbz);
