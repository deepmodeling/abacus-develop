...
void Grid_Technique::init_gpu_gint_variables(const UnitCell& ucell,
    ...
    // 2];
    double* psi_u_now = (double*)malloc(ucell.ntype * ucell.nwmax * nr_max * 2
                                        * sizeof(double));
    memset(psi_u_now,
           0,
           ucell.ntype * ucell.nwmax * nr_max * 2 * sizeof(double));
    bool* atom_iw2_new_now
        = (bool*)malloc(ucell.ntype * ucell.nwmax * sizeof(bool));
    ...
    checkCudaErrors(
        cudaMalloc((void**)&atom_l_g, ucell.ntype * ucell.nwmax * sizeof(int)));
