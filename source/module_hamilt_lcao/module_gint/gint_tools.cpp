...
int *get_vindex(const int bxyz, const int bx, const int by, const int bz,
                const int nplane, const int start_ind, const int ncyz) {
...
int *get_vindex(const int bxyz, const int bx, const int by, const int bz,
                const int nplane, const int ncyz, const int ibx, const int jby,
                const int kbz) {
  int *vindex = new int[bxyz];
  int bindex = 0;
  // z is the fastest,
...
double *get_vldr3(const double *const vlocal, // vlocal[ir]
                  const int bxyz, const int bx, const int by, const int bz,
                  const int nplane, const int ncyz, const int ibx,
                  const int jby, const int kbz, const double dv) {
  // set the index for obtaining local potentials
  int *vindex =
      Gint_Tools::get_vindex(bxyz, bx, by, bz, nplane, ncyz, ibx, jby, kbz);
...
double *get_vldr3(const double *const vlocal, // vlocal[ir]
  ...
  // set the index for obtaining local potentials
  int *vindex =
      Gint_Tools::get_vindex(bxyz, bx, by, bz, nplane, start_ind, ncyz);
  double *vldr3 = new double[bxyz];
  for (int ib = 0; ib < bxyz; ib++) {
    vldr3[ib] = vlocal[vindex[ib]] * dv;
  }
...
void get_block_info(const Grid_Technique &gt, const int bxyz, const int na_grid,
                    const int grid_index, int *&block_iw, int *&block_index,
                    int *&block_size, bool **&cal_flag) {
  ...
  for (int id = 0; id < na_grid; id++) {
    ...
    const int ia =
        GlobalC::ucell.iat2ia[iat]; // index of atoms within each type
    const int start = GlobalC::ucell.itiaiw2iwt(
        it, ia, 0); // the index of the first wave function for atom (it,ia)
    ...
    for (int ib = 0; ib < bxyz; ib++) {
      ...
      const double distance =
          std::sqrt(dr[0] * dr[0] + dr[1] * dr[1] +
                    dr[2] * dr[2]); // distance between atom and grid
...
void cal_psir_ylm(
    ...
    double *const *const psir_ylm) // cal_flag[bxyz][na_grid],	whether the
                                   // atom-grid distance is larger than cutoff
{
  ...
  for (int id = 0; id < na_grid; id++) {
    ...
    // the std::vector from the grid which is now being operated to the atom
    // position. in meshball language, is the std::vector from imcell to the
    // center cel, plus tau_in_bigcell.
    const int imcell = gt.which_bigcell[mcell_index];
    const double mt[3] = {
        gt.meshball_positions[imcell][0] - gt.tau_in_bigcell[iat][0],
        gt.meshball_positions[imcell][1] - gt.tau_in_bigcell[iat][1],
      ...
      if (!cal_flag[ib][id]) {
        ...
        for (int iw = 0; iw < atom->nw; ++iw) {
          if (atom->iw2_new[iw]) {
            ...
            phi = c1 * psi_uniform[ip] +
                  c2 * dpsi_uniform[ip] // radial wave functions
...
void cal_dpsir_ylm(
    ...
    const int *const
        block_size, // block_size[na_grid],	number of columns of a band
    const bool *const
        *const cal_flag, // cal_flag[bxyz][na_grid],	whether the atom-grid
                         // distance is larger than cutoff
    double *const *const psir_ylm, double *const *const dpsir_ylm_x,
    double *const *const dpsir_ylm_y, double *const *const dpsir_ylm_z) {
  ...
  for (int id = 0; id < na_grid; id++) {
    ...
    for (int ib = 0; ib < bxyz; ib++) {
      ...
      if (!cal_flag[ib][id]) {
        ...
        ModuleBase::Ylm::grad_rl_sph_harm(GlobalC::ucell.atoms[it].nwl, dr[0],
                                          dr[1], dr[2], rly, grly);
        if (distance < 1e-9)
          distance = 1e-9;
        ...
        for (int iw = 0; iw < atom->nw; ++iw) {
          ...
          if (atom->iw2_new[iw]) {
            const Numerical_Orbital_Lm &philn =
                GlobalC::ORB.Phi[it].PhiLN(atom->iw2l[iw], atom->iw2n[iw]);
          ...
          const double tmpdphi_rly =
              (dtmp - tmp * ll / distance) / rl * rly[idx_lm] / distance;
...
void cal_ddpsir_ylm(
    ...
    const bool *const
        *const cal_flag, // cal_flag[bxyz][na_grid],	whether the atom-grid
                         // distance is larger than cutoff
    double *const *const ddpsir_ylm_xx, double *const *const ddpsir_ylm_xy,
    double *const *const ddpsir_ylm_xz, double *const *const ddpsir_ylm_yy,
    double *const *const ddpsir_ylm_yz, double *const *const ddpsir_ylm_zz) {
      ...
      if (!cal_flag[ib][id]) {
        ...
        if (/*distance < 1e-9*/ true) {
          ...
          for (int i = 0; i < 6; i++) {
            ...
            std::vector<double> rly;
            std::vector<std::vector<double>> grly;
            ModuleBase::Ylm::grad_rl_sph_harm(GlobalC::ucell.atoms[it].nwl,
                                              dr1[0], dr1[1], dr1[2], rly,
                                              grly);

            double distance1 =
                std::sqrt(dr1[0] * dr1[0] + dr1[1] * dr1[1] + dr1[2] * dr1[2]);
            if (distance1 < 1e-9)
              distance1 = 1e-9;
            ...
            for (int iw = 0; iw < atom->nw; ++iw) {
              ...
              // derivative of wave functions with respect to atom positions.
              const double tmpdphi_rly =
                  (dtmp - tmp * ll / distance1) / rl * rly[idx_lm] / distance1;
              const double tmprl = tmp / rl;
        ...
        // it is not used currently
        {
          ...
          ModuleBase::Ylm::grad_rl_sph_harm(GlobalC::ucell.atoms[it].nwl, dr[0],
                                            dr[1], dr[2], rly, grly);
          ModuleBase::Ylm::hes_rl_sph_harm(GlobalC::ucell.atoms[it].nwl, dr[0],
                                           dr[1], dr[2], hrly);
          const double position = distance / delta_r;
          ...
          for (int iw = 0; iw < atom->nw; ++iw) {
            ...
            const double term3 =
                ll * (ll + 2) * tmp / r_lp2 / distance / distance;
...
void cal_dpsirr_ylm(
    ...
    const bool *const
        *const cal_flag, // cal_flag[bxyz][na_grid],	whether the atom-grid
                         // distance is larger than cutoff
    double *const *const dpsir_ylm_x, double *const *const dpsir_ylm_y,
    double *const *const dpsir_ylm_z, double *const *const dpsir_ylm_xx,
    double *const *const dpsir_ylm_xy, double *const *const dpsir_ylm_xz,
    double *const *const dpsir_ylm_yy, double *const *const dpsir_ylm_yz,
...
Gint_Tools::Array_Pool<double>
get_psir_vlbr3(const int bxyz,
               const int na_grid, // how many atoms on this (i,j,k) grid
               const int LD_pool,
               const int *const block_index, // block_index[na_grid+1], count
                                             // total number of atomis orbitals
               const bool *const
                   *const cal_flag, // cal_flag[bxyz][na_grid],	whether the
                                    // atom-grid distance is larger than cutoff
...
void mult_psi_DM(
    ...
    const bool if_symm) // true: density, use dsymv; false: potential/transition
                        // density use dgemv
{
  ...
  for (int ia1 = 0; ia1 < na_grid; ia1++) {
    ...
    if (if_symm) // density
    {
      ...
      if (cal_num > ib_length / 4) {
        dsymm_(&side, &uplo, &block_size[ia1], &ib_length, &alpha_symm,
               &DM[iw1_lo][iw1_lo], &gt.lgd, &psi[first_ib][block_index[ia1]],
               &LD_pool, &beta, &psi_DM[first_ib][block_index[ia1]], &LD_pool);
      } else {
        // int k=1;
        for (int ib = first_ib; ib < last_ib; ++ib) {
          if (cal_flag[ib][ia1]) {
            dsymv_(&uplo, &block_size[ia1], &alpha_symm, &DM[iw1_lo][iw1_lo],
                   &gt.lgd, &psi[ib][block_index[ia1]], &inc, &beta,
                   &psi_DM[ib][block_index[ia1]], &inc);
          }
        }
      }
    }
    ...
    for (int ia2 = start; ia2 < na_grid; ia2++) {
      ...
      if (cal_pair_num > ib_length / 4) {
        dgemm_(&transa, &transb, &block_size[ia2], &ib_length, &block_size[ia1],
               ...
               &psi[first_ib][block_index[ia1]], &LD_pool, &beta,
               &psi_DM[first_ib][block_index[ia2]], &LD_pool);
      } else {
        for (int ib = first_ib; ib < last_ib; ++ib) {
          if (cal_flag[ib][ia1] && cal_flag[ib][ia2]) {
            dgemv_(&transa, &block_size[ia2], &block_size[ia1], &alpha_gemm,
                   &DM[iw1_lo][iw2_lo], &gt.lgd, &psi[ib][block_index[ia1]],
                   &inc, &beta, &psi_DM[ib][block_index[ia2]], &inc);
          }
        }
      }
    } // ia2
    ...
    const bool if_symm) // 1: density, 2: force
{
  ...
  for (int ia1 = 0; ia1 < na_grid; ia1++) {
    ...
    if (if_symm) // density
    {
      ...
      if (cal_num > ib_length / 4) {
        dsymm_(&side, &uplo, &block_size[ia1], &ib_length, &alpha_symm,
               tmp_matrix, &block_size[ia1], &psi[first_ib][block_index[ia1]],
               &LD_pool, &beta, &psi_DM[first_ib][block_index[ia1]], &LD_pool);
      } else {
        // int k=1;
        for (int ib = first_ib; ib < last_ib; ++ib) {
          if (cal_flag[ib][ia1]) {
            dsymv_(&uplo, &block_size[ia1], &alpha_symm, tmp_matrix,
                   &block_size[ia1], &psi[ib][block_index[ia1]], &inc, &beta,
                   &psi_DM[ib][block_index[ia1]], &inc);
          }
        }
      }
    ...
    for (int ia2 = start; ia2 < na_grid; ia2++) {
      ...
      if (cal_pair_num > ib_length / 4) {
        ...
        for (int ib = first_ib; ib < last_ib; ++ib) {
          if (cal_flag[ib][ia1] && cal_flag[ib][ia2]) {
            dgemv_(&transa, &block_size[ia2], &block_size[ia1], &alpha_gemm,
                   tmp_matrix, &block_size[ia2], &psi[ib][block_index[ia1]],
                   &inc, &beta, &psi_DM[ib][block_index[ia2]], &inc);
          }
        }
      }
...
void mult_psi_DMR(const Grid_Technique &gt, const int bxyz,
                  const int &grid_index, const int &na_grid,
                  const int *const block_index, const int *const block_size,
                  bool **cal_flag, double **psi, double **psi_DMR, double *DMR,
                  const hamilt::HContainer<double> *DM, const bool if_symm) {
  ...
  for (int ia1 = 0; ia1 < na_grid; ia1++) {
    ...
    const double *tmp_matrix =
        DM->find_matrix(iat, iat, 0, 0, 0)->get_pointer();
    if (if_symm) // density
    {
      ...
      if (cal_num > 0) {
        ...
        for (int *find = find_start; find < find_end; find++) {
          ...
          if (find[0] == index) {
            offset =
                find - find_start; // start positions of adjacent atom of 'iat'
            break;
          }
        }
      ...
      //	{
      //		std::cout << i <<" "<<j<<" " <<
      // DMR[DM_start+i*block_size[ia1]+j]<<" " <<
      ...
      if (cal_num > bxyz / 4) {
        const int DM_start = gt.nlocstartg[iat] + gt.find_R2st[iat][offset];
        dgemm_(&trans, &trans, &block_size[ia1], &bxyz, &block_size[ia1],
               &alpha, tmp_matrix, &block_size[ia1], &psi[0][idx1], &LD_pool,
        ...
        for (int ib = 0; ib < bxyz; ++ib) {
          if (cal_flag[ib][ia1]) {
            dgemv_(&trans, &block_size[ia1], &block_size[ia1], &alpha,
                   tmp_matrix, &block_size[ia1], &psi[ib][idx1], &inc, &beta,
                   &psi_DMR[ib][idx1], &inc);
          }
        }
      }
    ...
    for (int ia2 = start; ia2 < na_grid; ia2++) {
      ...
      // get AtomPair
      const double *tmp_matrix =
          DM->find_matrix(iat, iat2, dRx, dRy, dRz)->get_pointer();
      int offset = -1;
      ...
      if (cal_num > bxyz / 4) {
        ...
        const int DM_start = gt.nlocstartg[iat] + gt.find_R2st[iat][offset];
        dgemm_(&trans, &trans, &block_size[ia2], &bxyz, &block_size[ia1],
               &alpha1, tmp_matrix, &block_size[ia2], &psi[0][idx1], &LD_pool,
        ...
        for (int ib = 0; ib < bxyz; ++ib) {
          if (cal_flag[ib][ia1] && cal_flag[ib][ia2]) {
            dgemv_(&trans, &block_size[ia2], &block_size[ia1], &alpha1,
                   tmp_matrix, &block_size[ia2], &psi[ib][idx1], &inc, &beta,
                   &psi_DMR[ib][idx2], &inc);
          }
        }
      } // cal_num
