                    ...
                    bool**& cal_flag)
{
    ...
    for (int id = 0; id < na_grid; id++)
    {
        ...
        const int start = GlobalC::ucell.itiaiw2iwt(
            it,
            ia,
            0); // the index of the first wave function for atom (it,ia)
                ...
                for (int iw = 0; iw < atom->nw; ++iw)
                {
                    ...
                    if (atom->iw2_new[iw])
                    {
                        const Numerical_Orbital_Lm& philn
                            = GlobalC::ORB.Phi[it].PhiLN(atom->iw2l[iw],
                                                         atom->iw2n[iw]);
                        ...
                        else
                        {
                            ...
                            tmp = x12
                                      * (philn.psi_uniform[iq] * x3
                                         + philn.psi_uniform[iq + 3] * x0)
                                  + x03
                                        * (philn.psi_uniform[iq + 1] * x2
                                           - philn.psi_uniform[iq + 2] * x1);

                            dtmp = x12
                                       * (philn.dpsi_uniform[iq] * x3
                                          + philn.dpsi_uniform[iq + 3] * x0)
                                   + x03
                                         * (philn.dpsi_uniform[iq + 1] * x2
                    ...
                    // derivative of wave functions with respect to atom
                    // positions.
                    const double tmpdphi_rly = (dtmp - tmp * ll / distance) / rl
                                               * rly[idx_lm] / distance;
    ...
    for (int id = 0; id < na_grid; id++)
    {
        ...
        for (int ib = 0; ib < bxyz; ib++)
        {
            ...
            else
            {
                ...
                // for some unknown reason, the finite difference between dpsi
                // and ddpsi using analytical expression is always wrong; as a
                // result, I switch to explicit finite difference method for
                // evaluating the second derivatives of the orbitals
                if (/*distance < 1e-9*/ true)
                {
                    ...
                    for (int i = 0; i < 6; i++)
                    {
                        ...
                        // array to store spherical harmonics and its
                        // derivatives
                        std::vector<double> rly;
                        std::vector<std::vector<double>> grly;
                        ModuleBase::Ylm::grad_rl_sph_harm(
                            GlobalC::ucell.atoms[it].nwl,
                            dr1[0],
                            dr1[1],
                            dr1[2],
                            rly,
                        ...
                        double distance1
                            = std::sqrt(dr1[0] * dr1[0] + dr1[1] * dr1[1]
                                        + dr1[2] * dr1[2]);
                        if (distance1 < 1e-9)
                            distance1 = 1e-9;
                        ...
                        for (int iw = 0; iw < atom->nw; ++iw)
                        {
                            ...
                            if (atom->iw2_new[iw])
                            {
                                ...
                                {
                                    // use Polynomia Interpolation method to get
                                    // the wave functions

                                    tmp = x12
                                              * (philn.psi_uniform[iq] * x3
                                                 + philn.psi_uniform[iq + 3]
                                                       * x0)
                                          + x03
                                                * (philn.psi_uniform[iq + 1]
                                                       * x2
                                                   - philn.psi_uniform[iq + 2]
                                                         * x1);

                                    dtmp = x12
                                               * (philn.dpsi_uniform[iq] * x3
                                                  + philn.dpsi_uniform[iq + 3]
                                                        * x0)
                                           + x03
                                                 * (philn.dpsi_uniform[iq + 1]
                                                        * x2
                                                    - philn.dpsi_uniform[iq + 2]
                                                          * x1);
                                }
                            } // new l is used.
                            ...
                            const double tmpdphi_rly
                                = (dtmp - tmp * ll / distance1) / rl
                                  * rly[idx_lm] / distance1;
                            const double tmprl = tmp / rl;

                            dpsi[iw][i][0] = tmpdphi_rly * dr1[0]
                                             + tmprl * grly[idx_lm][0];
                            dpsi[iw][i][1] = tmpdphi_rly * dr1[1]
                                             + tmprl * grly[idx_lm][1];
                            dpsi[iw][i][2] = tmpdphi_rly * dr1[2]
                                             + tmprl * grly[idx_lm][2];
                        } // end iw
                    }     // end i = 0-6
                    ...
                    {
                        p_ddpsi_xx[iw]
                            = (dpsi[iw][0][0] - dpsi[iw][1][0]) / 0.0002;
                        ...
                        p_ddpsi_xz[iw] = ((dpsi[iw][4][0] - dpsi[iw][5][0])
                                         ...
                                         / 0.0004;
                        p_ddpsi_yy[iw]
                            = (dpsi[iw][2][1] - dpsi[iw][3][1]) / 0.0002;
                        p_ddpsi_yz[iw] = ((dpsi[iw][4][1] - dpsi[iw][5][1])
                                          + (dpsi[iw][2][2] - dpsi[iw][3][2]))
                                         / 0.0004;
                        p_ddpsi_zz[iw]
                            = (dpsi[iw][4][2] - dpsi[iw][5][2]) / 0.0002;
                    }
                ...
                // it is not used currently
                {
                    ...
                    ModuleBase::Ylm::grad_rl_sph_harm(
                        ...
                        rly,
                        grly);
                    ModuleBase::Ylm::hes_rl_sph_harm(
                        GlobalC::ucell.atoms[it].nwl,
                        dr[0],
                        dr[1],
                        dr[2],
                    ...
                    for (int iw = 0; iw < atom->nw; ++iw)
                    {
                        ...
                        if (atom->iw2_new[iw])
                        {
                            ...
                            else
                            {
                                ...
                                ddtmp
                                    = x12
                                          * (philn.ddpsi_uniform[iq] * x3
                                             + philn.ddpsi_uniform[iq + 3] * x0)
                                      + x03
                                            * (philn.ddpsi_uniform[iq + 1] * x2
                                               - philn.ddpsi_uniform[iq + 2]
                                                     * x1);
                            }
                        } // new l is used.
                        ...
                        // d/dr (R_l / r^l)
                        const double tmpdphi
                            = (dtmp - tmp * ll / distance) / rl;
                        const double term1 = ddtmp / r_lp2;
                        const double term2
                            = (2 * ll + 1) * dtmp / r_lp2 / distance;
                        const double term3
                        ...
                        p_ddpsi_xy[iw] = term_xy * rly[idx_lm]
                                         + term_1x * grly[idx_lm][1]
                                         + term_1y * grly[idx_lm][0]
                                         + tmp / rl * hrly[idx_lm][1];
                        p_ddpsi_xz[iw] = term_xz * rly[idx_lm]
                                         + term_1x * grly[idx_lm][2]
                                         + term_1z * grly[idx_lm][0]
                                         + tmp / rl * hrly[idx_lm][2];
                        p_ddpsi_yy[iw] = term_yy * rly[idx_lm]
                                         + 2.0 * term_1y * grly[idx_lm][1]
                                         + tmp / rl * hrly[idx_lm][3];
                        p_ddpsi_yz[iw] = term_yz * rly[idx_lm]
                                         + term_1y * grly[idx_lm][2]
...
Gint_Tools::Array_Pool<double> get_psir_vlbr3(
    ...
    const bool* const* const
        cal_flag,              // cal_flag[bxyz][na_grid],	whether the
                               // atom-grid distance is larger than cutoff
                  ...
                  const bool if_symm)
{
    ...
    for (int ia1 = 0; ia1 < na_grid; ia1++)
    {
        ...
        if (if_symm) // density
        {
            ...
            if (cal_num > 0)
            {
                ...
                for (int* find = find_start; find < find_end; find++)
                {
                    ...
                    if (find[0] == index)
                    {
                        offset = find - find_start; // start positions of
                                                    // adjacent atom of 'iat'
                        break;
                    }
                }
            ...
            // gt.find_R2st[iat][offset]; for (int i = 0; i < block_size[ia1];
            // i++)
            //{
            ...
            {
                const int DM_start
                    = gt.nlocstartg[iat] + gt.find_R2st[iat][offset];
                dgemm_(&trans,
                       &trans,
                       &block_size[ia1],
                       &bxyz,
        ...
        for (int ia2 = start; ia2 < na_grid; ia2++)
        {
            ...
            //---------------------------------------------------------------
            // what I do above is to get 'offset' for atom std::pair (iat1,
            // iat2) if I want to simplify this searching for offset, I should
            // take advantage of gt.which_unitcell.
            //---------------------------------------------------------------
