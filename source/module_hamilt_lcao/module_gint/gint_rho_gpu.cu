                        ...
                        double *rho)
{
    ...
    for (int i = 0; i < gridt.nbx; i++)
    {
        for (int j = 0; j < gridt.nby; j++)
        {
            ...
            // The suffix "_g" indicates that the data is stored in the GPU, otherwise it is stored in the host.
            ...
            // atom_pair_alpha represents the coefficient alpha in the expression alpha * mat_DM * mat_psir.
            double *atom_pair_alpha = &gridt.alpha_global[gridt.atom_pair_nbz * stream_num];
            double *atom_pair_alpha_g = &gridt.alpha_global_g[gridt.atom_pair_nbz * stream_num];
            ...
            // psir_ylm_right_g is used to store psir_dm, which is the product of mat_DM * mat_psir.
            ...
            checkCuda(cudaMemcpyAsync(atom_pair_alpha_g, atom_pair_alpha, gridt.atom_pair_nbz * sizeof(double), cudaMemcpyHostToDevice, gridt.streams[stream_num]));
