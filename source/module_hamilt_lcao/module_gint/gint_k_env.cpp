...
void Gint_k::cal_env_k(int ik, const std::complex<double> *psi_k, double *rho,
  ...
  if (max_size != 0) {
    ...
    for (int grid_index = 0; grid_index < this->nbxx; grid_index++) {
      ...
      bool **cal_flag;
      Gint_Tools::get_block_info(*this->gridt, this->bxyz, size, grid_index,
                                 block_iw, block_index, block_size, cal_flag);

      // evaluate psi on grids
      Gint_Tools::Array_Pool<double> psir_ylm(this->bxyz, LD_pool);
      Gint_Tools::cal_psir_ylm(*this->gridt, this->bxyz, size, grid_index,
      ...
      for (int ia1 = 0; ia1 < size; ia1++) {
        ...
        const int Rx =
            this->gridt->ucell_index2x[id_ucell] + this->gridt->minu1;
        const int Ry =
            this->gridt->ucell_index2y[id_ucell] + this->gridt->minu2;
        const int Rz =
            this->gridt->ucell_index2z[id_ucell] + this->gridt->minu3;
        ModuleBase::Vector3<double> R((double)Rx, (double)Ry, (double)Rz);
        // std::cout << "kvec_d: " << kvec_d[ik].x << " " << kvec_d[ik].y << " "
        // << kvec_d[ik].z << std::endl; std::cout << "kvec_c: " << kvec_c[ik].x
        // << " " << kvec_c[ik].y << " " << kvec_c[ik].z << std::endl; std::cout
        // << "R: " << R.x << " " << R.y << " " << R.z << std::endl;
        const double arg = (kvec_d[ik] * R) * ModuleBase::TWO_PI;
        const double arg1 =
            (kvec_c[ik] * (R.x * GlobalC::ucell.a1 + R.y * GlobalC::ucell.a2 +
                           R.z * GlobalC::ucell.a3)) *
            ModuleBase::TWO_PI;
            ...
            if (GlobalV::NSPIN == 4) // is it a simple add of 2 spins?
            {
              for (int is = 0; is < 2; ++is) {
                ...
                for (int iw = 0; iw < atom1->nw; ++iw, ++iw1_lo)
                  tmp += std::complex<double>(psi1[iw], 0.0) * psi_k[iw1_lo] *
                         kphase;
              }
            } else {
