                       ...
                       const std::vector<ModuleBase::Vector3<double>>& kvec_d, LCAO_Orbitals& orb, UnitCell& ucell)
{
    ...
    if (max_size != 0)
    {
        ...
        for (int grid_index = 0; grid_index < this->nbxx; grid_index++)
        {
            ...
            bool** cal_flag;
            Gint_Tools::get_block_info(*this->gridt, this->bxyz, size, grid_index, block_iw, block_index, block_size,
                                       cal_flag);

            // evaluate psi on grids
            Gint_Tools::Array_Pool<double> psir_ylm(this->bxyz, LD_pool);
            Gint_Tools::cal_psir_ylm(*this->gridt, this->bxyz, size, grid_index, delta_r, block_index, block_size,
                                     cal_flag, psir_ylm.ptr_2D);

            int* vindex = Gint_Tools::get_vindex(this->bxyz, this->bx, this->by, this->bz, this->nplane,
