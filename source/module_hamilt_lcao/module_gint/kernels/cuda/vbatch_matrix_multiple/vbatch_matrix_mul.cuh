...
/*
 * Why do we need to implement our own matrix multiplication based on the magma code? 
 * There are two main reasons. 
 * First is when we are doing batch matrix multiplication, since we need to accumulate 
 * the results of the multiplications, it is necessary to pass the same memory address 
 * of matrix C to different multiplications. This way, the accumulation can be done 
 * directly through atomic operations during the matrix multiplication, avoiding the 
 * reduction operations after the multiplication. Secondly, when calculating the charge 
 * density, where C = alpha * A * B + C, the value of alpha might be different for the 
 * same batch of matrices. Using the standard matrix multiplication interface would 
 * require breaking down the batch matrix multiplication into smaller batches. In 
 * practice, it is difficult to accumulate a batch.
 * 
 * Moreover, taking into account the specific requirements of our application, especially 
 * the fact that we can relatively easily control the arrangement of the matrix elements, 
 * we have only implemented one type of requirement for matrix transposition. That is, we 
 * have implemented the operation C = alpha * trans(A) * B + C under the constraint of 
 * column-major order.
 * 
 * Finally, we would like to thank Magma for its contributions to the field of scientific computing.
*/
