...
#pragma omp for schedule(dynamic)
        for (int grid_index = 0; grid_index < this->nbxx; grid_index++)
        {
            ...
            for (int is = 0; is < inout->nspin_rho; ++is)
            {
                ...
                const ModuleBase::Array_Pool<double>& psir_ylm_1 = (!this->psir_func_1)
                                                                       ? psir_ylm
                                                                       : this->psir_func_1(psir_ylm,
                                                                                           *this->gridt,
                                                                                           ...
                                                                                           block_index,
                                                                                           cal_flag);
                const ModuleBase::Array_Pool<double>& psir_ylm_2 = (!this->psir_func_2)
                                                                       ? psir_ylm
                ...
                this->cal_meshball_rho(na_grid,
                                       block_index.data(),
                                       vindex.data(),
                                       psir_ylm_2.get_ptr_2D(),
                                       psir_DM.get_ptr_2D(),
                                       inout->rho[is]);
            }
        }
