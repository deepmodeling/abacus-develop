...
void Gint::gint_kernel_vlocal(const int na_grid, const int grid_index,
                              const double delta_r, double *vldr3,
                              const int LD_pool, double *pvpR_in,
  ...
  Gint_Tools::cal_psir_ylm(*this->gridt, this->bxyz, na_grid, grid_index,
  ...
  if (GlobalV::GAMMA_ONLY_LOCAL) {
    if (hR == nullptr)
      hR = this->hRGint;
    this->cal_meshball_vlocal_gamma(na_grid, LD_pool, block_iw, block_size,
                                    block_index, grid_index, cal_flag,
                                    psir_ylm.ptr_2D, psir_vlbr3.ptr_2D, hR);
  } else {
    this->cal_meshball_vlocal_k(na_grid, LD_pool, grid_index, block_size,
                                block_index, block_iw, cal_flag,
...
void Gint::gint_kernel_dvlocal(const int na_grid, const int grid_index,
                               const double delta_r, double *vldr3,
                               const int LD_pool, double *pvdpRx,
                               double *pvdpRy, double *pvdpRz) {
  // prepare block information
  int *block_iw, *block_index, *block_size;
  bool **cal_flag;
  Gint_Tools::get_block_info(*this->gridt, this->bxyz, na_grid, grid_index,
...
void Gint::gint_kernel_vlocal_meta(const int na_grid, const int grid_index,
                                   const double delta_r, double *vldr3,
  ...
  const Gint_Tools::Array_Pool<double> dpsix_vlbr3 =
      Gint_Tools::get_psir_vlbr3(this->bxyz, na_grid, LD_pool, block_index,
                                 cal_flag, vkdr3, dpsir_ylm_x.ptr_2D);
  const Gint_Tools::Array_Pool<double> dpsiy_vlbr3 =
      Gint_Tools::get_psir_vlbr3(this->bxyz, na_grid, LD_pool, block_index,
                                 cal_flag, vkdr3, dpsir_ylm_y.ptr_2D);
  const Gint_Tools::Array_Pool<double> dpsiz_vlbr3 =
      Gint_Tools::get_psir_vlbr3(this->bxyz, na_grid, LD_pool, block_index,
  ...
  if (GlobalV::GAMMA_ONLY_LOCAL) {
    ...
    this->cal_meshball_vlocal_k(
        ...
        cal_flag, dpsir_ylm_x.ptr_2D, dpsix_vlbr3.ptr_2D, pvpR_in);
    this->cal_meshball_vlocal_k(
        na_grid, LD_pool, grid_index, block_size, block_index, block_iw,
        cal_flag, dpsir_ylm_y.ptr_2D, dpsiy_vlbr3.ptr_2D, pvpR_in);
    this->cal_meshball_vlocal_k(
        na_grid, LD_pool, grid_index, block_size, block_index, block_iw,
        cal_flag, dpsir_ylm_z.ptr_2D, dpsiz_vlbr3.ptr_2D, pvpR_in);
  }
...
void Gint::cal_meshball_vlocal_gamma(
    ...
    const bool *const
        *const cal_flag, // cal_flag[this->bxyz][na_grid],	whether the
    ...
    hamilt::HContainer<double> *hR) // this->hRGint is the container of <phi_0 |
  ...
  for (int ia1 = 0; ia1 < na_grid; ++ia1) {
    ...
    for (int ia2 = 0; ia2 < na_grid; ++ia2) {
      ...
      if (iw1_lo <= iw2_lo) {
        ...
        // std::cout<<__FILE__<<__LINE__<<" "<<n<<" "<<m<<"
        // "<<tmp_ap->get_row_size()<<" "<<tmp_ap->get_col_size()<<std::endl;
        if (cal_pair_num > ib_length / 4) {
          dgemm_(&transa, &transb, &n, &m, &ib_length, &alpha,
                 &psir_vlbr3[first_ib][block_index[ia2]], &LD_pool,
                 &psir_ylm[first_ib][block_index[ia1]], &LD_pool, &beta,
                 tmp_ap->get_pointer(0), &n);
          ...
          for (int ib = first_ib; ib < last_ib; ++ib) {
            if (cal_flag[ib][ia1] && cal_flag[ib][ia2]) {
              ...
              dgemm_(&transa, &transb, &n, &m, &k, &alpha,
                     ...
                     &psir_ylm[ib][block_index[ia1]], &LD_pool, &beta,
                     tmp_ap->get_pointer(0), &n);
            }
          }
        }
        // std::cout<<__FILE__<<__LINE__<<"
        // "<<tmp_ap->get_pointer(0)[2]<<std::endl;
      }
    }
  }
}

void Gint::cal_meshball_vlocal_k(int na_grid, int LD_pool, int grid_index,
                                 int *block_size, int *block_index,
                                 int *block_iw, bool **cal_flag,
                                 double **psir_ylm, double **psir_vlbr3,
                                 double *pvpR) {
  auto find_offset = [&](const int id1, const int id2, const int iat1,
                         const int iat2) -> int {
    ...
    for (int ia2 = 0; ia2 < na_grid; ++ia2) {
      ...
      if (iat1 <= iat2) {
        ...
        if (cal_num > this->bxyz / 4) {
          k = this->bxyz;
          dgemm_(&transa, &transb, &n, &m, &k, &alpha, &psir_vlbr3[0][idx2],
                 &LD_pool, &psir_ylm[0][idx1], &LD_pool, &beta, &pvpR[iatw],
                 &n);
        } else {
          for (int ib = 0; ib < this->bxyz; ++ib) {
            if (cal_flag[ib][ia1] && cal_flag[ib][ia2]) {
              k = 1;
              dgemm_(&transa, &transb, &n, &m, &k, &alpha,
                     &psir_vlbr3[ib][idx2], &LD_pool, &psir_ylm[ib][idx1],
                     &LD_pool, &beta, &pvpR[iatw], &n);
            }
          }
        }
      }
