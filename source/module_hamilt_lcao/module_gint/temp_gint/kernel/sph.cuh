...
)
{
	...
	***************************/
	...
	/***************************
			 L = 1
	***************************/
	rly[1] = ylmcoef[1]*dr[2]; //l=1, m=0
	grly[3] = grly[4] = 0.0;
	grly[5] = ylmcoef[1];

	rly[2] = -ylmcoef[1]*dr[0]; //l=1, m=1
	grly[7] = grly[8] = 0.0;
	grly[6] = -ylmcoef[1];

	rly[3] = -ylmcoef[1]*dr[1]; //l=1, m=-1
	grly[9] = grly[11] = 0.0;
	grly[10] = -ylmcoef[1];
	...
	/***************************
			 L = 2
	***************************/
	rly[4] = ylmcoef[2]*dr[2]*rly[1]-ylmcoef[3]*rly[0]*r2;//l=2, m=0
	grly[12] = ylmcoef[2]*dr[2]*grly[3]-ylmcoef[3]*(grly[0]*r2+rly[0]*tx);//l=2, m=0
	grly[13] = ylmcoef[2]*dr[2]*grly[4]-ylmcoef[3]*(grly[1]*r2+rly[0]*ty);//l=2, m=0
	grly[14] = ylmcoef[2]*(dr[2]*grly[5]+rly[1])-ylmcoef[3]*(grly[2]*r2+rly[0]*tz);//l=2, m=0
	...
	grly[15] = tmp0*grly[6];
	grly[16] = tmp0*grly[7];
	grly[17] = ylmcoef[4]*(rly[2]+dr[2]*grly[8]);

	rly[6] = tmp0*rly[3];//l=2,m=-1
	grly[18] = tmp0*grly[9];
	grly[19] = tmp0*grly[10];
	grly[20] = ylmcoef[4]*(rly[3]+dr[2]*grly[11]);

	double tmp2 = ylmcoef[4]*dr[0];
	rly[7]= ylmcoef[5]*rly[4]-ylmcoef[6]*rly[0]*r2 - tmp2*rly[2];//l=2,m=2
	grly[21] = ylmcoef[5]*grly[12]-ylmcoef[6]*(rly[0]*tx+grly[0]*r2)-ylmcoef[4]*(dr[0]*grly[6]+rly[2]);

//	std::cout << "\np1 = "<< ylmcoef[5]*grly[12] << " p2 = " << -ylmcoef[6]*rly[0]*tx
//						<< " p3 = " << -ylmcoef[4]*dr[0]*grly[6] << " p4 = " << -ylmcoef[4]*rly[2] << std::endl;
	...
	rly[8] = -tmp2*rly[3];
	grly[24] = -ylmcoef[4]*(rly[3]+dr[0]*grly[9]);
...
//	rly[8] = tmp1+tmp2*rly[3];//l=2,m=-2
	...
	/***************************
			 L = 3
	***************************/
	rly[9] = ylmcoef[7]*dr[2]*rly[4]-ylmcoef[8]*rly[1]*r2; //l=3, m=0
	grly[27] = ylmcoef[7]*dr[2]*grly[12]-ylmcoef[8]*(rly[1]*tx+grly[3]*r2);
	grly[28] = ylmcoef[7]*dr[2]*grly[13]-ylmcoef[8]*(rly[1]*ty+grly[4]*r2);
	grly[29] = ylmcoef[7]*(rly[4]+dr[2]*grly[14])-ylmcoef[8]*(rly[1]*tz+grly[5]*r2);

	double tmp3 = ylmcoef[9]*dr[2];
	...
	grly[32] = ylmcoef[9]*(dr[2]*grly[17]+rly[5])-ylmcoef[10]*(grly[8]*r2+rly[2]*tz);
	...
	grly[35] = ylmcoef[9]*(dr[2]*grly[20]+rly[6])-ylmcoef[10]*(grly[11]*r2+rly[3]*tz);

	double tmp4 = ylmcoef[11]*dr[2];
	rly[12] = tmp4*rly[7];//l=3,m=2
	grly[36] = tmp4*grly[21];
	grly[37] = tmp4*grly[22];
	grly[38] = ylmcoef[11]*(dr[2]*grly[23]+rly[7]);

	rly[13] = tmp4*rly[8];//l=3,m=-2
	grly[39] = tmp4*grly[24];
	grly[40] = tmp4*grly[25];
	grly[41] = ylmcoef[11]*(dr[2]*grly[26]+rly[8]);

	double tmp5 = ylmcoef[14]*dr[0];
	rly[14] = ylmcoef[12]*rly[10]-ylmcoef[13]*rly[2]*r2-tmp5*rly[7];//l=3,m=3
	grly[42] = ylmcoef[12]*grly[30]-ylmcoef[13]*(rly[2]*tx+grly[6]*r2)-ylmcoef[14]*(rly[7]+dr[0]*grly[21]);
	...
	grly[45] = ylmcoef[12]*grly[33]-ylmcoef[13]*(rly[3]*tx+grly[9]*r2)-ylmcoef[14]*(rly[8]+dr[0]*grly[24]);
	...
	/***************************
			 L = 4
	***************************/
	rly[16] = ylmcoef[15]*dr[2]*rly[9]-ylmcoef[16]*rly[4]*r2;//l=4,m=0
	grly[48] = ylmcoef[15]*dr[2]*grly[27]-ylmcoef[16]*(rly[4]*tx+grly[12]*r2);
	grly[49] = ylmcoef[15]*dr[2]*grly[28]-ylmcoef[16]*(rly[4]*ty+grly[13]*r2);
	grly[50] = ylmcoef[15]*(dr[2]*grly[29]+rly[9])-ylmcoef[16]*(rly[4]*tz+grly[14]*r2);

	double tmp6 = ylmcoef[17]*dr[2];
	...
	grly[53] = ylmcoef[17]*(dr[2]*grly[32]+rly[10])-ylmcoef[18]*(rly[5]*tz+grly[17]*r2);
	...
	grly[56] = ylmcoef[17]*(dr[2]*grly[35]+rly[11])-ylmcoef[18]*(rly[6]*tz+grly[20]*r2);

	double tmp7 = ylmcoef[19]*dr[2];
	...
	grly[59] = ylmcoef[19]*(dr[2]*grly[38]+rly[12])-ylmcoef[20]*(rly[7]*tz+grly[23]*r2);
	...
	grly[62] = ylmcoef[19]*(dr[2]*grly[41]+rly[13])-ylmcoef[20]*(rly[8]*tz+grly[26]*r2);

	double tmp8 = 3.0*dr[2];
	rly[21] = tmp8*rly[14];//l=4,m=3
	grly[63] = tmp8*grly[42];
	grly[64] = tmp8*grly[43];
	grly[65] = 3.0*(dr[2]*grly[44]+rly[14]);


	rly[22] = tmp8*rly[15];//l=4,m=-3
	grly[66] = tmp8*grly[45];
	grly[67] = tmp8*grly[46];
	grly[68] = 3.0*(dr[2]*grly[47]+rly[15]);

	double tmp9 = ylmcoef[23]*dr[0];
	rly[23] = ylmcoef[21]*rly[19]-ylmcoef[22]*rly[7]*r2-tmp9*rly[14];//l=4,m=4
	grly[69] = ylmcoef[21]*grly[57]-ylmcoef[22]*(rly[7]*tx+grly[21]*r2)-ylmcoef[23]*(dr[0]*grly[42]+rly[14]);
	...
	grly[72] = ylmcoef[21]*grly[60]-ylmcoef[22]*(rly[8]*tx+grly[24]*r2)-ylmcoef[23]*(dr[0]*grly[45]+rly[15]);
	...
	/***************************
			 L = 5
	***************************/
	rly[25] = ylmcoef[24]*dr[2]*rly[16]-ylmcoef[25]*rly[9]*r2;//l=5,m=0
	grly[75] = ylmcoef[24]*dr[2]*grly[48]-ylmcoef[25]*(rly[9]*tx+grly[27]*r2);
	grly[76] = ylmcoef[24]*dr[2]*grly[49]-ylmcoef[25]*(rly[9]*ty+grly[28]*r2);
	grly[77] = ylmcoef[24]*(dr[2]*grly[50]+rly[16])-ylmcoef[25]*(rly[9]*tz+grly[29]*r2);

	double tmp10 = ylmcoef[26]*dr[2];
	...
	grly[80] = ylmcoef[26]*(dr[2]*grly[53]+rly[17])-ylmcoef[27]*(rly[10]*tz+grly[32]*r2);
	...
	grly[83] = ylmcoef[26]*(dr[2]*grly[56]+rly[18])-ylmcoef[27]*(rly[11]*tz+grly[35]*r2);

	double tmp11 = ylmcoef[28]*dr[2];
	...
	grly[86] = ylmcoef[28]*(dr[2]*grly[59]+rly[19])-ylmcoef[29]*(rly[12]*tz+grly[38]*r2);
	...
	grly[89] = ylmcoef[28]*(dr[2]*grly[62]+rly[20])-ylmcoef[29]*(rly[13]*tz+grly[41]*r2);

	double tmp12 = ylmcoef[30]*dr[2];
	...
	grly[92] = ylmcoef[30]*(dr[2]*grly[65]+rly[21])-ylmcoef[31]*(grly[44]*r2+rly[14]*tz);
	...
	grly[95] = ylmcoef[30]*(dr[2]*grly[68]+rly[22])-ylmcoef[31]*(grly[47]*r2+rly[15]*tz);

	double tmp13 = ylmcoef[32]*dr[2];
	rly[32] = tmp13*rly[23];//l=5,m=4
	grly[96] = tmp13*grly[69];
	grly[97] = tmp13*grly[70];
	grly[98] = ylmcoef[32]*(rly[23]+dr[2]*grly[71]);

	rly[33] = tmp13*rly[24];//l=5,m=-4
	grly[99] = tmp13*grly[72];
	grly[100] = tmp13*grly[73];
	grly[101] = ylmcoef[32]*(rly[24]+dr[2]*grly[74]);

	double tmp14 = ylmcoef[35]*dr[0];
	rly[34] = ylmcoef[33]*rly[30]-ylmcoef[34]*rly[14]*r2-tmp14*rly[23];//l=5,m=5
	grly[102] = ylmcoef[33]*grly[90]-ylmcoef[34]*(rly[14]*tx+grly[42]*r2)-ylmcoef[35]*(dr[0]*grly[69]+rly[23]);
	...
	grly[105] = ylmcoef[33]*grly[93]-ylmcoef[34]*(rly[15]*tx+grly[45]*r2)-ylmcoef[35]*(dr[0]*grly[72]+rly[24]);
