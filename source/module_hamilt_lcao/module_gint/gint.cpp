      ...
      if (inout->job == Gint_Tools::job_type::vlocal) {
        ...
        for (int is = 0; is < GlobalV::NSPIN; ++is) {
          ...
          GintKernel::gint_gamma_force_gpu(
              ...
              force, stress, this->nplane,dr,rcut,
               *this->gridt, GlobalC::ucell);
...
#pragma omp parallel
#endif
      {
        ...
        // it's a uniform grid to save orbital values, so the delta_r is a
        // constant.
        ...
        if (inout->job == Gint_Tools::job_type::dvlocal) {
          ...
          ModuleBase::GlobalFunc::ZEROS(this->pvdpRx_reduced[inout->ispin],
                                        nnrg);
          ModuleBase::GlobalFunc::ZEROS(this->pvdpRy_reduced[inout->ispin],
                                        nnrg);
          ModuleBase::GlobalFunc::ZEROS(this->pvdpRz_reduced[inout->ispin],
                                        nnrg);
        }
        ...
        hamilt::HContainer<double> *hRGint_thread =
            nullptr; // auxiliary pointer for multi-threading
        if (inout->job == Gint_Tools::job_type::vlocal ||
            inout->job == Gint_Tools::job_type::vlocal_meta) {
          if (!GlobalV::GAMMA_ONLY_LOCAL) {
...
#pragma omp for
        ...
        for (int grid_index = 0; grid_index < this->nbxx; grid_index++) {
          ...
          if (inout->job == Gint_Tools::job_type::rho) {
            ...
            int *vindex = Gint_Tools::get_vindex(
                this->bxyz, this->bx, this->by, this->bz, this->nplane,
                this->gridt->start_ind[grid_index], ncyz);
            this->gint_kernel_rho(na_grid, grid_index, delta_r, vindex, LD_pool,
                                  inout);
            delete[] vindex;
            ...
            int *vindex = Gint_Tools::get_vindex(
                this->bxyz, this->bx, this->by, this->bz, this->nplane,
                this->gridt->start_ind[grid_index], ncyz);
            this->gint_kernel_tau(na_grid, grid_index, delta_r, vindex, LD_pool,
                                  inout);
            delete[] vindex;
          } else if (inout->job == Gint_Tools::job_type::force) {
            double *vldr3 = Gint_Tools::get_vldr3(
                inout->vl, this->bxyz, this->bx, this->by, this->bz,
            ...
            if (!GlobalV::GAMMA_ONLY_LOCAL)
              DM_in = inout->DM_R;
#ifdef _OPENMP
            this->gint_kernel_force(na_grid, grid_index, delta_r, vldr3,
                                    LD_pool, DM_in, inout->ispin,
                                    inout->isforce, inout->isstress,
            ...
            if ((GlobalV::GAMMA_ONLY_LOCAL && lgd > 0) ||
                !GlobalV::GAMMA_ONLY_LOCAL) {
              this->gint_kernel_vlocal(na_grid, grid_index, delta_r, vldr3,
                                       LD_pool, pvpR_thread, hRGint_thread);
            }
#else
            if (GlobalV::GAMMA_ONLY_LOCAL && lgd > 0) {
              this->gint_kernel_vlocal(na_grid, grid_index, delta_r, vldr3,
            ...
            double *vldr3 = Gint_Tools::get_vldr3(
                ...
                this->nplane, this->gridt->start_ind[grid_index], ncyz, dv);
#ifdef _OPENMP
            this->gint_kernel_dvlocal(na_grid, grid_index, delta_r, vldr3,
                                      LD_pool, pvdpRx_thread, pvdpRy_thread,
                                      pvdpRz_thread);
#else
            this->gint_kernel_dvlocal(na_grid, grid_index, delta_r, vldr3,
                                      LD_pool,
            ...
            double *vldr3 = Gint_Tools::get_vldr3(
                inout->vl, this->bxyz, this->bx, this->by, this->bz,
                this->nplane, this->gridt->start_ind[grid_index], ncyz, dv);
            double *vkdr3 = Gint_Tools::get_vldr3(
                inout->vofk, this->bxyz, this->bx, this->by, this->bz,
                this->nplane, this->gridt->start_ind[grid_index], ncyz, dv);
#ifdef _OPENMP
            if ((GlobalV::GAMMA_ONLY_LOCAL && lgd > 0) ||
                !GlobalV::GAMMA_ONLY_LOCAL) {
              this->gint_kernel_vlocal_meta(na_grid, grid_index, delta_r, vldr3,
                                            vkdr3, LD_pool, pvpR_thread,
                                            hRGint_thread);
            }
#else
            if (GlobalV::GAMMA_ONLY_LOCAL && lgd > 0) {
              this->gint_kernel_vlocal_meta(na_grid, grid_index, delta_r, vldr3,
                                            vkdr3, LD_pool, nullptr);
            }
            if (!GlobalV::GAMMA_ONLY_LOCAL) {
              this->gint_kernel_vlocal_meta(na_grid, grid_index, delta_r, vldr3,
                                            vkdr3, LD_pool,
            ...
            if (!GlobalV::GAMMA_ONLY_LOCAL)
              DM_in = inout->DM_R;
#ifdef _OPENMP
            this->gint_kernel_force_meta(na_grid, grid_index, delta_r, vldr3,
                                         vkdr3, LD_pool, DM_in, inout->ispin,
                                         inout->isforce, inout->isstress,
                                         &fvl_dphi_thread, &svl_dphi_thread);
#else
            this->gint_kernel_force_meta(na_grid, grid_index, delta_r, vldr3,
...
#pragma omp critical(gint_gamma)
            {
              BlasConnector::axpy(this->hRGint->get_nnr(), 1.0,
                                  hRGint_thread->get_wrapper(), 1,
                                  this->hRGint->get_wrapper(), 1);
            }
...
#pragma omp critical(gint_k)
            {
              BlasConnector::axpy(nnrg, 1.0, pvpR_thread, 1,
                                  pvpR_reduced[inout->ispin], 1);
            }
            delete[] pvpR_thread;
          }
        }
...
void Gint::prep_grid(const Grid_Technique &gt, const int &nbx_in,
                     const int &nby_in, const int &nbz_in,
                     const int &nbz_start_in, const int &ncxyz_in,
                     const int &bx_in, const int &by_in, const int &bz_in,
                     const int &bxyz_in, const int &nbxx_in, const int &ny_in,
                     const int &nplane_in, const int &startz_current_in) {
...
void Gint::initialize_pvpR(const UnitCell &ucell_in, Grid_Driver *gd) {
  ...
  // prepare the row_index and col_index for construct AtomPairs, they are same,
  // name as orb_index
    ...
    for (int I1 = 0; I1 < atom1->na; ++I1) {
      ...
      if (this->gridt->in_this_processor[iat1]) {
        for (int ad = 0; ad < gd->getAdjacentNum() + 1; ++ad) {
          ...
          // if only upper triangle is saved, the lower triangle will be lost in
          // 2D-block parallelization. if the adjacent atom is in this
          // processor.
          if (this->gridt->in_this_processor[iat2]) {
            ...
            double rcut =
                GlobalC::ORB.Phi[T1].getRcut() + GlobalC::ORB.Phi[T2].getRcut();

            // if(distance < rcut)
            //  mohan reset this 2013-07-02 in Princeton
            //  we should make absolutely sure that the distance is smaller than
            //  GlobalC::ORB.Phi[it].getRcut this should be consistant with
            //  LCAO_nnr::cal_nnrg function typical example : 7 Bohr cutoff Si
            //  orbital in 14 Bohr length of cell. distance = 7.0000000000000000
            ...
            if (distance < rcut - 1.0e-15) {
              ...
              if (npol == 1) {
                hamilt::AtomPair<double> tmp_atom_pair(
                    iat1, iat2, R_index.x, R_index.y, R_index.z,
                    orb_index.data(), orb_index.data(), ucell_in.nat);
                this->hRGint->insert_pair(tmp_atom_pair);
              } else {
                // HR is complex and size is nw * npol
                hamilt::AtomPair<std::complex<double>> tmp_atom_pair(
                    iat1, iat2, R_index.x, R_index.y, R_index.z,
                    orb_index_npol.data(), orb_index_npol.data(), ucell_in.nat);
    ...
    for (int iap = 0; iap < this->DMRGint_full->size_atom_pairs(); ++iap) {
      ...
      for (int ir = 0; ir < ap.get_R_size(); ++ir) {
        ...
        for (int is = 0; is < 4; is++) {
          tmp_pointer[is] =
              this->DMRGint[is]
                  ->find_matrix(iat1, iat2, r_index[0], r_index[1], r_index[2])
                  ->get_pointer();
        }
