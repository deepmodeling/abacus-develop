...
void Gint::initialize_pvpR(const UnitCell& ucell_in, const Grid_Driver* gd, const int& nspin)
{
    ...
    else
    {
        ...
        ModuleBase::Memory::record("Gint::hRGint_tmp", this->hRGint_tmp[0]->get_memory_size() * nspin);
        ModuleBase::Memory::record("Gint::DMRGint", this->DMRGint[0]->get_memory_size() * this->DMRGint.size() * nspin);
#ifdef __MPI
...
void Gint::reset_DMRGint(const int& nspin)
{
    if (this->hRGint)
    {
        ...
        for (auto& d: this->DMRGint)
        {
            d = new hamilt::HContainer<double>(*this->hRGint);
        }
        if (nspin == 4)
        {
            for (auto& d: this->DMRGint)
            {
                d->allocate(nullptr, false);
            }
#ifdef __MPI
            delete this->DMRGint_full;
