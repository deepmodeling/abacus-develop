...
void Grid_MeshBall::init_meshball(void)
{
    ...
    if (GlobalV::test_gridt)
        ModuleBase::GlobalFunc::OUT(GlobalV::ofs_running, "how many cells in meshball", this->meshball_ncells);
    ...
    for (int i = -1; i <= 1; i++)
    {
        for (int j = -1; j <= 1; j++)
        {
            for (int k = -1; k <= 1; k++)
            {
                ...
                {
                    // change to cartesian coordinates.
                    cell[ip] = i * this->bigcell_vec1[ip] + j * this->bigcell_vec2[ip] + k * this->bigcell_vec3[ip];
