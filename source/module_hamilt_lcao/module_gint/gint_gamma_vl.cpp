...
extern "C" {
void Cblacs_gridinfo(int icontxt, int *nprow, int *npcol, int *myprow,
                     int *mypcol);
void Cblacs_pinfo(int *myid, int *nprocs);
void Cblacs_pcoord(int icontxt, int pnum, int *prow, int *pcol);
}

void Gint_Gamma::cal_vlocal(Gint_inout *inout, LCAO_Matrix *lm,
...
//------------------------------------------------------------------
inline int setBufferParameter(const Grid_Technique &gt, MPI_Comm comm_2D,
                              int blacs_ctxt, int nblk, int &s_index_siz,
                              int *&s_local_index, int *&s_siz_pro,
                              int *&s_dis_pro, int &s_siz, double *&s_buffer,
                              int &r_index_siz, int *&r_global_index,
                              int *&r_siz_pro, int *&r_dis_pro, int &r_siz,
                              double *&r_buffer) {
  //-----------------------------------------
  ...
  MPI_Alltoall(s_siz_pro, 1, MPI_INT, r_siz_pro, 1, MPI_INT, comm_2D);
  ...
  MPI_Alltoallv(s_global_index, s_siz_pro, s_dis_pro, MPI_INT, r_global_index,
                r_siz_pro, r_dis_pro, MPI_INT, comm_2D);
...
void Gint_Gamma::vl_grid_to_2D(
    const double *vl_grid, const Parallel_2D &p2d, const int loc_grid_dim,
    const bool new_e_iteration, double *vl_2d,
    std::function<void(const int &, const int &, const double &, double *)>
        setfunc) {
  ModuleBase::timer::tick("Gint_Gamma", "distri_vl");
  // setup send buffer and receive buffer size
  // OUT(GlobalV::ofs_running, "Start transforming vlocal from grid distribute
  // to 2D block");
  if (new_e_iteration) {
    ...
    setBufferParameter(*this->gridt, p2d.comm_2D, p2d.blacs_ctxt, p2d.nb,
                       ...
                       this->sender_displacement_process, this->sender_size,
                       this->sender_buffer, this->receiver_index_size,
                       this->receiver_global_index, this->receiver_size_process,
                       this->receiver_displacement_process, this->receiver_size,
    ...
    ModuleBase::GlobalFunc::OUT(GlobalV::ofs_running, "buffer size(M):",
                                (this->sender_size + this->receiver_size) *
                                    sizeof(double) / 1024 / 1024);
    ModuleBase::GlobalFunc::OUT(
        GlobalV::ofs_running, "buffer index size(M):",
        (this->sender_index_size + this->receiver_index_size) * sizeof(int) /
            1024 / 1024);
#endif
    ModuleBase::timer::tick("Gint_Gamma", "distri_vl_index");
  }
...
void Gint_Gamma::transfer_pvpR(hamilt::HContainer<double> *hR) {
  ...
  for (int iap = 0; iap < this->hRGint->size_atom_pairs(); iap++) {
    ...
    if (iat1 > iat2) {
      ...
      const hamilt::AtomPair<double> *upper_ap =
          this->hRGint->find_pair(iat2, iat1);
#ifdef __DEBUG
