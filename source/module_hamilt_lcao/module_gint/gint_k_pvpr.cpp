...
void Gint_k::allocate_pvpR(void)
{
    ...
    ModuleBase::Memory::record("pvpR_reduced",
                               sizeof(double) * this->gridt->nnrg
                                   * GlobalV::NSPIN);
        ...
        for (int iat = 0; iat < GlobalC::ucell.nat; ++iat)
        {
            ...
            const int I1 = GlobalC::ucell.iat2ia[iat];
            {
                ...
                if (this->gridt->in_this_processor[iat])
                {
                    ...
                    GlobalC::GridD.Find_atom(GlobalC::ucell,
                                             tau1,
                                             T1,
                                             I1,
                                             &adjs);
                    ...
                    for (int ad = 0; ad < adjs.adj_num + 1; ad++)
                    {
                        ...
                        if (this->gridt->in_this_processor[iat2])
                        {
                            ...
                            // for the local part, only need to calculate <phi_i
                            // | phi_j> within range mohan note 2012-07-06
                            if (distance < rcut)
                            {
                                const int start2
                                    = GlobalC::ucell.itiaiw2iwt(T2, I2, 0);
                            ...
                            if (nug >= 0)
                            {
                                if (mug <= nug)
                                {
                                    if (ij_same)
                                    {
                                        // spin = 0;
                                        // spin = 3;
                                        tmp[j] = pvp_nc[0][mug0][nug0]
                                                 + sign * pvp_nc[3][mug0][nug0];
                                    }
                                    else
                                    {
                                        // spin = 1;
                ...
                for (int ad = 0; ad < adjs.adj_num + 1; ad++)
                {
                    ...
                    if (this->gridt->in_this_processor[iat2])
                    {
                        ...
                        if (distance < rcut)
                        {
                            ...
                            int ixxx
                                = DM_start + this->gridt->find_R2st[iat][nad];

                            hamilt::BaseMatrix<double>* tmp_matrix
                                = this->hRGint->find_matrix(iat,
                                                            iat2,
                                                            dR.x,
                                                            dR.y,
                                                            dR.z);
                ...
                for (int ad = 0; ad < adjs.adj_num + 1; ad++)
                {
                    ...
                    if (this->gridt->in_this_processor[iat2])
                    {
                        ...
                        if (distance < rcut)
                        {
                            ...
                            assert(tmp_matrix != nullptr);
#endif
                            std::complex<double>* tmp_pointer
                            ...
                            for (int iw = 0; iw < atom1->nw; iw++)
                            {
                                for (int iw2 = 0; iw2 < atom2->nw; ++iw2)
                                {
                                    tmp_pointer[step_trace[0]]
                                        = *vijR[0] + *vijR[3];
                                    tmp_pointer[step_trace[3]]
                                        = *vijR[0] - *vijR[3];
                                    tmp_pointer += 2;
                                    vijR[0]++;
                                    vijR[3]++;
                            ...
                            if (iat < iat2)
                            {
                                hamilt::BaseMatrix<std::complex<double>>*
                                    conj_matrix
                                    = this->hRGintCd->find_matrix(iat2,
                                                                  iat,
                                                                  -dR.x,
                                                                  -dR.y,
                                                                  -dR.z);
                                ...
                                {
                                    for (int iw2 = 0; iw2 < atom2->nw * 2;
                                         ++iw2)
                                    {
                                        conj_matrix->get_value(iw2, iw1)
                                            = conj(*tmp_pointer);
                                        tmp_pointer++;
                                    }
                                }
