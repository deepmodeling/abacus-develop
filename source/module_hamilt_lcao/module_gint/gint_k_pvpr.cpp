...
void Gint_k::allocate_pvpR(void) {
  ...
  ModuleBase::Memory::record(
      "pvpR_reduced", sizeof(double) * this->gridt->nnrg * GlobalV::NSPIN);
...
void Gint_k::folding_vl_k(
    const int &ik, LCAO_Matrix *LM,
    const std::vector<ModuleBase::Vector3<double>> &kvec_d) {
  ModuleBase::TITLE("Gint_k", "folding_vl_k");
  ModuleBase::timer::tick("Gint_k", "folding_vl_k");

  if (!pvpR_alloc_flag) {
    ModuleBase::WARNING_QUIT("Gint_k::destroy_pvpR",
                             "pvpR hasnot been allocated yet!");
  }
  ...
  auto init_pvp = [&](int num_threads, int thread_id) {
    int beg, len;
    ModuleBase::BLOCK_TASK_DIST_1D(num_threads, thread_id, lgd * lgd, 256, beg,
                                   len);
        ...
        if (this->gridt->in_this_processor[iat]) {
          ...
          for (int ad = 0; ad < adjs.adj_num + 1; ad++) {
            ...
            if (this->gridt->in_this_processor[iat2]) {
              ...
              // for the local part, only need to calculate <phi_i | phi_j>
              // within range mohan note 2012-07-06
              if (distance < rcut) {
                const int start2 = GlobalC::ucell.itiaiw2iwt(T2, I2, 0);

                // calculate the distance between iat1 and iat2.
                // ModuleBase::Vector3<double> dR =
                // GlobalC::GridD.getAdjacentTau(ad) - tau1;
                dR.x = adjs.box[ad].x;
                dR.y = adjs.box[ad].y;
                dR.z = adjs.box[ad].z;
                ...
                if (GlobalV::NSPIN != 4) {
                  for (int iw = 0; iw < atom1->nw; iw++) {
                    std::complex<double> *vij =
                        pvp[this->gridt->trace_lo[start1 + iw]];
                    ...
                    for (int iw2 = 0; iw2 < atom2->nw; ++iw2) {
                      ...
                      //     "<<iat<<" "<<iat2<<" "<<ixxx<<" "<<start1+iw<<"
                      //     "<<start2+iw2<<" "<<vijR[iw2]<<" "<<iw2_lo[iw2]<<"
                  ...
                  for (int iw = 0; iw < atom1->nw; iw++) {
                    ...
                    for (int spin = 0; spin < 4; spin++) {
                      auto vij = pvp_nc[spin][this->gridt->trace_lo[start1] /
                                                  GlobalV::NPOL +
                                              iw];
              ...
              if (nug >= 0) {
                if (mug <= nug) {
                  if (ij_same) {
                    ...
                    // spin = 3;
                    tmp[j] =
                        pvp_nc[0][mug0][nug0] + sign * pvp_nc[3][mug0][nug0];
                  } else {
                    // spin = 1;
                    // spin = 2;
                    tmp[j] = pvp_nc[1][mug0][nug0] +
                             sign * std::complex<double>(0.0, 1.0) *
                                 pvp_nc[2][mug0][nug0];
                  }
                } else {
                  if (ij_same) {
                    // spin = 0;
                    // spin = 3;
                    tmp[j] = conj(pvp_nc[0][nug0][mug0] +
                                  sign * pvp_nc[3][nug0][mug0]);
                  } else {
                    // spin = 1;
                    // spin = 2;
      ...
      if (this->gridt->in_this_processor[iat]) {
        ...
        for (int ad = 0; ad < adjs.adj_num + 1; ad++) {
          ...
          if (this->gridt->in_this_processor[iat2]) {
            ...
            if (distance < rcut) {
              if (iat > iat2) { // skip the lower triangle.
                nad++;
                continue;
              }
              // calculate the distance between iat1 and iat2.
              ...
              hamilt::BaseMatrix<double> *tmp_matrix =
                  this->hRGint->find_matrix(iat, iat2, dR.x, dR.y, dR.z);
#ifdef __DEBUG
              ...
              {
                hamilt::BaseMatrix<double> *conj_matrix =
                    this->hRGint->find_matrix(iat2, iat, -dR.x, -dR.y, -dR.z);
#ifdef __DEBUG
...
void Gint_k::transfer_pvpR(hamilt::HContainer<std::complex<double>> *hR) {
          ...
          if (this->gridt->in_this_processor[iat2]) {
            ...
            if (distance < rcut) {
              ...
              hamilt::BaseMatrix<std::complex<double>> *tmp_matrix =
                  this->hRGintCd->find_matrix(iat, iat2, dR.x, dR.y, dR.z);
#ifdef __DEBUG
              assert(tmp_matrix != nullptr);
#endif
              ...
              if (iat < iat2) {
                hamilt::BaseMatrix<std::complex<double>> *conj_matrix =
                    this->hRGintCd->find_matrix(iat2, iat, -dR.x, -dR.y, -dR.z);
#ifdef __DEBUG
                assert(conj_matrix != nullptr);
#endif
                tmp_pointer = tmp_matrix->get_pointer();
  ...
  // ===================================
  // transfer HR from Gint to Veff<OperatorLCAO<std::complex<double>,
  // std::complex<double>>>
  // ===================================
#ifdef __MPI
  int size;
  ...
  if (size == 1) {
    ...
    hamilt::transferSerials2Parallels<std::complex<double>>(*this->hRGintCd,
                                                            hR);
  }
#else
  hR->add(*this->hRGintCd);
