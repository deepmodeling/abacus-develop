                ...
                if (LM->ParaV->global2local_row(row) >= 0)
                {
                    for (int col = 0; col < GlobalV::NLOCAL; ++col)
                    {
                        if (LM->ParaV->global2local_col(col) >= 0)
                        {
                            ...
                            {
                                double& value = LM->HR_sparse[current_spin]
                                                             [R_coor][row][col];
                                value += tmp[col];
                                if (std::abs(value) <= sparse_threshold)
                                {
                                    LM->HR_sparse[current_spin][R_coor][row]
                                        .erase(col);
                                }
                            }
                        }
                        ...
                        if (LM->ParaV->global2local_col(col) >= 0)
                        {
                            if (std::abs(tmp_soc[col]) > sparse_threshold)
                            {
                                std::complex<double>& value
                                    = LM->HR_soc_sparse[R_coor][row][col];
                                value += tmp_soc[col];
                                if (std::abs(value) <= sparse_threshold)
                                {
                                       ...
                                       LCAO_Matrix* LM)
{
                        ...
                        if (distance < rcut)
                        {
                                 ...
                                 iw++)
                            {
                                     ...
                                     iw2++)
                                {
                                    ...
                                    if (GlobalV::NSPIN == 4)
                                    {
                                        ...
                                        if (iw % 2 == 0 && iw2 % 2 == 0)
                                        {
                                            ...
                                            if (std::abs(temp_value_complex)
                                                > sparse_threshold)
                                            {
                                                pvpR_soc_sparseMatrix[dR][start1
                                                                          + iw]
                                                                     [start2
                                                                      + iw2]
                                                    = temp_value_complex;
                                    ...
                                    {
                                        temp_value_double
                                            = pvpR_reduced[current_spin]
                                                          [iw_nowg];
                                        if (std::abs(temp_value_double)
                                            > sparse_threshold)
                                        {
                                            pvpR_sparseMatrix[dR][start1 + iw]
                                                             [start2 + iw2]
                                                = temp_value_double;
                                        }
    ...
    if (GlobalV::NSPIN != 4)
    {
        distribute_pvpR_sparseMatrix(current_spin,
                                     sparse_threshold,
                                     pvpR_sparseMatrix,
                                     LM);
    }
    ...
    {
        distribute_pvpR_soc_sparseMatrix(sparse_threshold,
                                         pvpR_soc_sparseMatrix,
                                         LM);
    }
