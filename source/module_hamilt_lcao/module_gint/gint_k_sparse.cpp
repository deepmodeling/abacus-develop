...
void Gint_k::distribute_pvpR_sparseMatrix(
    const int current_spin, const double &sparse_threshold,
...
void Gint_k::cal_vlocal_R_sparseMatrix(const int &current_spin,
                  ...
                  if (GlobalV::NSPIN == 4) {
                    ...
                    if (iw % 2 == 0 && iw2 % 2 == 0) {
                      ...
                      if (!GlobalV::DOMAG) {
                        ...
                        temp_value_complex = pvpR_reduced[1][iw_nowg] -
                                             std::complex<double>(0.0, 1.0) *
                                                 pvpR_reduced[2][iw_nowg];
                        if (std::abs(temp_value_complex) > sparse_threshold) {
                          pvpR_soc_sparseMatrix[dR][start1 + iw][start2 + iw2] =
                              temp_value_complex;
                        }
                    ...
                    if (std::abs(temp_value_double) > sparse_threshold) {
                      pvpR_sparseMatrix[dR][start1 + iw][start2 + iw2] =
                          temp_value_double;
                    }
  ...
  if (GlobalV::NSPIN != 4) {
    distribute_pvpR_sparseMatrix(current_spin, sparse_threshold,
                                 pvpR_sparseMatrix, LM);
  } else {
