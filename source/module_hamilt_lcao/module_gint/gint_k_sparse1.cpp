...
void Gint_k::distribute_pvdpR_sparseMatrix(
    const int current_spin, const int dim, const double &sparse_threshold,
          ...
          for (int col = 0; col < GlobalV::NLOCAL; ++col) {
            if (LM->ParaV->global2local_col(col) >= 0) {
              if (std::abs(tmp[col]) > sparse_threshold) {
                if (dim == 0) {
                  double &value =
                      LM->dHRx_sparse[current_spin][R_coor][row][col];
                  value += tmp[col];
                ...
                if (dim == 1) {
                  double &value =
                      LM->dHRy_sparse[current_spin][R_coor][row][col];
                  value += tmp[col];
                  if (std::abs(value) <= sparse_threshold) {
                ...
                if (dim == 2) {
                  double &value =
                      LM->dHRz_sparse[current_spin][R_coor][row][col];
                  value += tmp[col];
                  if (std::abs(value) <= sparse_threshold) {
...
void Gint_k::distribute_pvdpR_soc_sparseMatrix(
    const int dim, const double &sparse_threshold,
    const std::map<Abfs::Vector3_Order<int>,
              ...
              if (std::abs(tmp_soc[col]) > sparse_threshold) {
                if (dim == 0) {
                  std::complex<double> &value =
                      LM->dHRx_soc_sparse[R_coor][row][col];
                  value += tmp_soc[col];
                  if (std::abs(value) <= sparse_threshold) {
                ...
                if (dim == 1) {
                  std::complex<double> &value =
                      LM->dHRy_soc_sparse[R_coor][row][col];
                  value += tmp_soc[col];
                  if (std::abs(value) <= sparse_threshold) {
                ...
                if (dim == 2) {
                  std::complex<double> &value =
                      LM->dHRz_soc_sparse[R_coor][row][col];
                  value += tmp_soc[col];
                  if (std::abs(value) <= sparse_threshold) {
...
void Gint_k::cal_dvlocal_R_sparseMatrix(const int &current_spin,
                  ...
                  if (GlobalV::NSPIN == 4) {
                    ...
                    if (iw % 2 == 0 && iw2 % 2 == 0) {
                      ...
                      if (!GlobalV::DOMAG) {
                        ...
                        if (std::abs(temp_value_complex) > sparse_threshold) {
                          pvdpRx_soc_sparseMatrix[dR][start1 + iw]
                                                 [start2 + iw2] =
                                                     temp_value_complex;
                        }
                        ...
                        if (std::abs(temp_value_complex) > sparse_threshold) {
                          pvdpRy_soc_sparseMatrix[dR][start1 + iw]
                                                 [start2 + iw2] =
                                                     temp_value_complex;
                        }
                        ...
                        if (std::abs(temp_value_complex) > sparse_threshold) {
                          pvdpRz_soc_sparseMatrix[dR][start1 + iw]
                                                 [start2 + iw2] =
                                                     temp_value_complex;
                        }
                      }
  ...
  if (GlobalV::NSPIN != 4) {
    distribute_pvdpR_sparseMatrix(current_spin, 0, sparse_threshold,
                                  pvdpRx_sparseMatrix, LM);
    distribute_pvdpR_sparseMatrix(current_spin, 1, sparse_threshold,
                                  pvdpRy_sparseMatrix, LM);
    distribute_pvdpR_sparseMatrix(current_spin, 2, sparse_threshold,
                                  pvdpRz_sparseMatrix, LM);
  } else {
    distribute_pvdpR_soc_sparseMatrix(0, sparse_threshold,
                                      pvdpRx_soc_sparseMatrix, LM);
    distribute_pvdpR_soc_sparseMatrix(1, sparse_threshold,
                                      pvdpRy_soc_sparseMatrix, LM);
    distribute_pvdpR_soc_sparseMatrix(2, sparse_threshold,
                                      pvdpRz_soc_sparseMatrix, LM);
  }
