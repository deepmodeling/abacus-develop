                    ...
                    for (int col = 0; col < GlobalV::NLOCAL; ++col)
                    {
                        if (LM->ParaV->global2local_col(col) >= 0)
                        {
                            if (std::abs(tmp[col]) > sparse_threshold)
                            {
                                ...
                                {
                                    double& value
                                        = LM->dHRx_sparse[current_spin][R_coor]
                                                         [row][col];
                                    value += tmp[col];
                                ...
                                {
                                    double& value
                                        = LM->dHRy_sparse[current_spin][R_coor]
                                                         [row][col];
                                    value += tmp[col];
                                    if (std::abs(value) <= sparse_threshold)
                                    {
                                ...
                                {
                                    double& value
                                        = LM->dHRz_sparse[current_spin][R_coor]
                                                         [row][col];
                                    value += tmp[col];
                                    if (std::abs(value) <= sparse_threshold)
                                    {
                        ...
                        if (LM->ParaV->global2local_col(col) >= 0)
                        {
                            if (std::abs(tmp_soc[col]) > sparse_threshold)
                            {
                                if (dim == 0)
                                {
                                    ...
                                    if (std::abs(value) <= sparse_threshold)
                                    {
                                        LM->dHRx_soc_sparse[R_coor][row].erase(
                                            col);
                                    }
                                }
                                if (dim == 1)
                                {
                                    ...
                                    if (std::abs(value) <= sparse_threshold)
                                    {
                                        LM->dHRy_soc_sparse[R_coor][row].erase(
                                            col);
                                    }
                                }
                                if (dim == 2)
                                {
                                    ...
                                    if (std::abs(value) <= sparse_threshold)
                                    {
                                        LM->dHRz_soc_sparse[R_coor][row].erase(
                                            col);
                                    }
                                }
                            }
                                        ...
                                        LCAO_Matrix* LM)
{
                ...
                for (int ad = 0; ad < GlobalC::GridD.getAdjacentNum() + 1; ad++)
                {
                    ...
                    if (this->gridt->in_this_processor[iat2])
                    {
                        ...
                        if (distance < rcut)
                        {
                            ...
                            int ixxx
                                = DM_start + this->gridt->find_R2st[iat][nad2];
                            for (int iw = 0; iw < atom1->nw * GlobalV::NPOL;
                                 iw++)
                            {
                                for (int iw2 = 0;
                                     iw2 < atom2->nw * GlobalV::NPOL;
                                     iw2++)
                                {
                                    ...
                                    {
                                        temp_value_double
                                            = pvdpRx_reduced[current_spin]
                                                            [iw_nowg];
                                        if (std::abs(temp_value_double)
                                            > sparse_threshold)
                                        {
                                            pvdpRx_sparseMatrix[dR][start1 + iw]
                                                               [start2 + iw2]
                                                = temp_value_double;
                                        }
                                        temp_value_double
                                            = pvdpRy_reduced[current_spin]
                                                            [iw_nowg];
                                        if (std::abs(temp_value_double)
                                            > sparse_threshold)
                                        {
                                            pvdpRy_sparseMatrix[dR][start1 + iw]
                                                               [start2 + iw2]
                                                = temp_value_double;
                                        }
                                        temp_value_double
                                            = pvdpRz_reduced[current_spin]
                                                            [iw_nowg];
                                        if (std::abs(temp_value_double)
                                            > sparse_threshold)
                                        {
                                            pvdpRz_sparseMatrix[dR][start1 + iw]
                                                               [start2 + iw2]
                                                = temp_value_double;
                                        }
                                    } // endif normal
    ...
    if (GlobalV::NSPIN != 4)
    {
        distribute_pvdpR_sparseMatrix(current_spin,
                                      0,
                                      sparse_threshold,
                                      pvdpRx_sparseMatrix,
                                      LM);
    ...
    {
        distribute_pvdpR_soc_sparseMatrix(0,
                                          sparse_threshold,
                                          pvdpRx_soc_sparseMatrix,
                                          LM);
        distribute_pvdpR_soc_sparseMatrix(1,
                                          sparse_threshold,
                                          pvdpRy_soc_sparseMatrix,
                                          LM);
        distribute_pvdpR_soc_sparseMatrix(2,
                                          sparse_threshold,
                                          pvdpRz_soc_sparseMatrix,
                                          LM);
    }
