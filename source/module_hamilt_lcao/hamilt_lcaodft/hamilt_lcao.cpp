...
HamiltLCAO<TK, TR>::HamiltLCAO(LCAO_Matrix* LM_in,
    ...
    this->getOperator()
        = new OverlapNew<OperatorLCAO<TK, TR>>(LM_in,
                                               this->kv->kvec_d,
                                               this->hR,
                                               &(this->getHk(LM_in)),
                                               this->sR,
...
HamiltLCAO<TK, TR>::HamiltLCAO(Gint_Gamma* GG_in,
    ...
    const std::size_t row_col_size
        = static_cast<std::size_t>(LM_in->ParaV->get_row_size())
          * LM_in->ParaV->get_col_size();
    this->getSk(LM_in).resize(row_col_size);
    this->getHk(LM_in).resize(row_col_size);

    // Effective potential term (\sum_r <psi(r)|Veff(r)|psi(r)>) is registered
    // without template
    ...
    if (std::is_same<TK, double>::value) {
        ...
        // in Gamma_only case, target SR is LCAO_Matrix::Sloc, which is same as
        // SK
        ...
        // in Gamma_only case, target HR is LCAO_Matrix::Hloc_fixed, while
        // target HK is LCAO_Matrix::Hloc LCAO_Matrix::Hloc_fixed2 is used for
        // storing
        if (GlobalV::T_IN_H) {
        ...
        // in general case, target HR is this->hR, while target HK is
        // LCAO_Matrix::Hloc
        if (GlobalV::VNL_IN_H) {
        ...
        // in general case, target HR is Gint::hRGint, while target HK is
        // LCAO_Matrix::Hloc
        if (GlobalV::VL_IN_H) {
        ...
        if (GlobalV::deepks_scf) {
            Operator<TK>* deepks = new DeePKS<OperatorLCAO<TK, TR>>(
                loc_in,
                LM_in,
                this->kv->kvec_d,
                this->hR, // no explicit call yet
    ...
    // multi-k-points case to initialize HamiltLCAO, ops will be used
        ...
        // in general case, target HR is Gint::pvpR_reduced, while target HK is
        // LCAO_Matrix::Hloc2
        if (GlobalV::VL_IN_H) {
            // only Potential is not empty, Veff and Meta are available
            if (pot_register_in.size() > 0) {
                ...
                // Veff term
                this->getOperator()
                    = new Veff<OperatorLCAO<TK, TR>>(GK_in,
                                                     LM_in,
                                                     kv->kvec_d,
                                                     pot_in,
                                                     this->hR,
        ...
        if (GlobalV::VNL_IN_H) {
            Operator<TK>* nonlocal = new NonlocalNew<OperatorLCAO<TK, TR>>(
                ...
                two_center_bundle.overlap_orb_beta.get(),
                LM_in->ParaV);
            // TDDFT velocity gague will calculate full non-local potential
            // including the original one and the correction on its own. So the
            // original non-local potential term should be skipped
            if (GlobalV::ESOLVER_TYPE != "tddft"
                || elecstate::H_TDDFT_pw::stype != 1) {
                this->getOperator()->add(nonlocal);
            } else {
                delete nonlocal;
            }
        ...
        if (TD_Velocity::tddft_velocity) {
            ...
            Operator<TK>* td_ekinetic = new TDEkinetic<OperatorLCAO<TK, TR>>(
                LM_in,
                this->hR,
                &(this->getHk(LM_in)),
                this->sR,
            ...
            Operator<TK>* td_nonlocal
                = new TDNonlocal<OperatorLCAO<TK, TR>>(LM_in,
                                                       this->kv->kvec_d,
                                                       this->hR,
                                                       &(this->getHk(LM_in)),
                                                       &GlobalC::ucell,
    ...
    if (GlobalC::exx_info.info_global.cal_exx) {
        Operator<TK>* exx = new OperatorEXX<OperatorLCAO<TK, TR>>(
            LM_in,
            this->hR,
            &(this->getHk(LM_in)),
            *this->kv,
...
#endif
    // if NSPIN==2, HR should be separated into two parts, save HR into
    // this->hRS2
    int memory_fold = 1;
    if (GlobalV::NSPIN == 2) {
    ...
    ModuleBase::Memory::record("HamiltLCAO::hR",
                               this->hR->get_memory_size() * memory_fold);
...
void HamiltLCAO<TK, TR>::updateHk(const int ik) {
    ...
    if (GlobalV::NSPIN == 2) {
        ...
        if (GlobalV::VL_IN_H && this->kv->isk[ik] != this->current_spin) {
            ...
            this->hR->allocate(this->hRS2.data()
                                   + this->hRS2.size() / 2 * this->kv->isk[ik],
                               0);
            if (this->refresh_times > 0) {
                this->refresh_times--;
                dynamic_cast<hamilt::OperatorLCAO<TK, TR>*>(this->ops)
                    ->set_hr_done(false);
            }
        }
...
std::vector<std::complex<double>>&
    HamiltLCAO<std::complex<double>, double>::getHk(LCAO_Matrix* LM) {
    return LM->Hloc2;
}
template <>
std::vector<std::complex<double>>&
    HamiltLCAO<std::complex<double>, std::complex<double>>::getHk(
        LCAO_Matrix* LM) {
    return LM->Hloc2;
}
...
template <>
std::vector<std::complex<double>>&
    HamiltLCAO<std::complex<double>, double>::getSk(LCAO_Matrix* LM) {
    return LM->Sloc2;
}
template <>
std::vector<std::complex<double>>&
    HamiltLCAO<std::complex<double>, std::complex<double>>::getSk(
        LCAO_Matrix* LM) {
    return LM->Sloc2;
}
