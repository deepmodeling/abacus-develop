    ...
    elecstate::ElecState* pelec)
{
	...
	else if(GlobalC::wf.init_wfc == "file")
	{
		...
		for(int ik=0; ik<GlobalC::kv.nkstot; ++ik)
		{
            ...
            error = ModuleIO::read_psi_nao_complex(ctot, ik, this->ParaV, psi, pelec);
#ifdef __MPI
    ...
    const ModuleBase::matrix& wg)
{
    ...
    if(out_wfc_lcao && myid == 0)
    {
        ...
        ModuleIO::write_psi_nao(ss.str(), ctot, ekb, wg);
        for (int i = 0; i < GlobalV::NBANDS; i++)
        {
            delete[] ctot[i];
        }
    ...
    const ModuleBase::matrix& wg)
{
    ...
    if (out_wfc_lcao && myid == 0)
    {
        ...
        ModuleIO::write_psi_nao_complex(ss.str(), ctot, ik, ekb, wg);
        for (int i = 0; i < GlobalV::NBANDS; i++)
        {
            delete[] ctot[i];
        }
