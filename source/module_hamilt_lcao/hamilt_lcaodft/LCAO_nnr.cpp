...
void Grid_Technique::cal_nnrg(Parallel_Orbitals* pv) {
    ...
    for (int T1 = 0; T1 < GlobalC::ucell.ntype; ++T1) {
        ...
        for (int I1 = 0; I1 < atom1->na; ++I1) {
            ...
            if (this->in_this_processor[iat]) {
                ...
                int count = 0;
                for (int ad = 0; ad < GlobalC::GridD.getAdjacentNum() + 1;
                     ++ad) {
                    ...
                    if (this->in_this_processor[iat2]) {
                        ...
                        //  smaller than GlobalC::ORB.Phi[it].getRcut this
                        //  should be consistant with LCAO_nnr::cal_nnrg
                        //  function typical example : 7 Bohr cutoff Si orbital
                        //  in 14 Bohr length of cell. distance
                        //  = 7.0000000000000000 GlobalC::ORB.Phi[it].getRcut
                        //  = 7.0000000000000008
                        if (distance < rcut - 1.0e-15) {
                            ...
                            this->nnrg_index.push_back(gridIntegral::gridIndex{
                                this->nnrg,
                                mu,
                                nu,
                                GlobalC::GridD.getBox(ad),
                                atom1->nw,
                                atom2->nw});
                      ...
                      //				GlobalV::ofs_running << " iat=" << iat << "
                      //nlocstartg=" << nlocstartg[iat] << "
    ...
    for (int iat = 0; iat < GlobalC::ucell.nat; iat++) {
        ...
        std::fill(this->find_R2_sorted_index[iat].begin(),
                  this->find_R2_sorted_index[iat].end(),
                  0);
    ...
    for (int T1 = 0; T1 < GlobalC::ucell.ntype; T1++) {
        for (int I1 = 0; I1 < GlobalC::ucell.atoms[T1].na; I1++) {
            ...
            //			std::cout << " Number of adjacent = " <<
            //GlobalC::GridD.getAdjacentNum()+1 << std::endl;
            ...
            for (int ad = 0; ad < GlobalC::GridD.getAdjacentNum() + 1; ad++) {
                ...
                if (this->in_this_processor[iat]) {
                    if (this->in_this_processor[iat2]) {
                        ...
                        // mohan reset this 2013-07-02 in Princeton
                        // we should make absolutely sure that the distance is
                        // smaller than GlobalC::ORB.Phi[it].getRcut this should
                        // be consistant with LCAO_nnr::cal_nnrg function
                        // typical example : 7 Bohr cutoff Si orbital in 14 Bohr
                        // length of cell. distance = 7.0000000000000000
                        ...
                        if (distance < rcut - 1.0e-15) {
                            ...
                            //--------------------------------------------------------------
                            find_R2[iat][count]
                                = this->cal_RindexAtom(b1, b2, b3, iat2);
                            ...
                            if (count + 1 < nad[iat]) {
                                find_R2st[iat][count + 1]
                                    = find_R2st[iat][count]
                                      + GlobalC::ucell.atoms[T1].nw
                                            * GlobalC::ucell.atoms[T2]
                                                  .nw; // modified by
                                                       // zhengdy-soc
                            }
                            ++count;
            ...
            std::stable_sort(find_R2_sorted_index[iat].begin(),
                             find_R2_sorted_index[iat].begin() + nad[iat],
                             [&](int pos1, int pos2) {
                                 return find_R2[iat][pos1] < find_R2[iat][pos2];
                             });
        }
    }
...
int Grid_Technique::cal_RindexAtom(const int& u1,
    ...
    if (x1 < 0 || x2 < 0 || x3 < 0) {
        ...
        ModuleBase::WARNING_QUIT("LCAO_nnr::cal_Rindex",
                                 "x1<0 || x2<0 || x3<0 !");
    }
    ...
    return (iat2
            + (x3 + x2 * this->nB3 + x1 * this->nB2 * this->nB3)
                  * GlobalC::ucell.nat);
}
