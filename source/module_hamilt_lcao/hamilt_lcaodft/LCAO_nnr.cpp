            ...
            if (this->in_this_processor[iat])
            {
                ...
                for (int ad = 0; ad < GlobalC::GridD.getAdjacentNum() + 1; ++ad)
                {
                    ...
                    if (this->in_this_processor[iat2])
                    {
                        ...
                        //  GlobalC::ORB.Phi[it].getRcut this should be consistant with LCAO_nnr::cal_nnrg function
                        ...
                        if (distance < rcut - 1.0e-15)
                        {
                            ...
                            this->nnrg_index.push_back(gridIntegral::gridIndex{
                                this->nnrg, mu, nu, GlobalC::GridD.getBox(ad), atom1->nw, atom2->nw});
                                ...
                                find_R2st[iat][count + 1]
                                    = find_R2st[iat][count]
                                      + GlobalC::ucell.atoms[T1].nw
                                            * GlobalC::ucell.atoms[T2].nw; // modified by zhengdy-soc
                            }
                            ++count;
                        }
                    }
...
void LCAO_Matrix::folding_fixedH(const int& ik, const std::vector<ModuleBase::Vector3<double>>& kvec_d, bool cal_syns)
{
                ...
                for (int ad = 0; ad < adjs.adj_num + 1; ++ad)
                {
                    ...
                    else if (distance >= rcut)
                    {
                        for (int ad0 = 0; ad0 < adjs.adj_num + 1; ++ad0)
                        {
                            ...
                            double rcut1
                                = GlobalC::ORB.Phi[T1].getRcut() + GlobalC::ucell.infoNL.Beta[T0].get_rcut_max();
                            double rcut2
                                = GlobalC::ORB.Phi[T2].getRcut() + GlobalC::ucell.infoNL.Beta[T0].get_rcut_max();
