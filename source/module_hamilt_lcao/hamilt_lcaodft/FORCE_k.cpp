...
void Force_LCAO<std::complex<double>>::allocate(
    ...
    const auto init_DSloc_Rxyz
        = [this, nnr, &fsr](int num_threads, int thread_id) {
              int beg = 0;
              int len = 0;
    ...
    const auto init_DHloc_fixedR_xyz
        = [this, nnr, &fsr](int num_threads, int thread_id) {
              int beg = 0;
              int len = 0;
              ModuleBase::BLOCK_TASK_DIST_1D(num_threads,
...
template <>
void Force_LCAO<std::complex<double>>::test(Parallel_Orbitals& pv,
                                            double* mmm,
