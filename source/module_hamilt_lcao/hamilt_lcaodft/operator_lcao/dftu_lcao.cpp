...
// initialize_HR()
template <typename TK, typename TR>
void hamilt::DFTU<hamilt::OperatorLCAO<TK, TR>>::initialize_HR(
    Grid_Driver* GridD) {
...
template <typename TK, typename TR>
void hamilt::DFTU<hamilt::OperatorLCAO<TK, TR>>::cal_nlm_all(
    ...
    for (int iat0 = 0; iat0 < ucell->nat; iat0++) {
        ...
        for (int ad = 0; ad < adjs.adj_num + 1; ++ad) {
            ...
            // insert col_indexes into all_indexes to get universal set with no
            // repeat elements
            ...
            for (int iw1l = 0; iw1l < all_indexes.size(); iw1l += npol) {
                ...
                // nlm is a vector of vectors, but size of outer vector is only
                // 1 here If we are calculating force, we need also to store the
                // gradient and size of outer vector is then 4 inner loop : all
                // projectors (L0,M0)
...
void hamilt::DFTU<hamilt::OperatorLCAO<TK, TR>>::contributeHR() {
    ...
    if (this->dftu->get_dmr(0) == nullptr
        && this->dftu->initialed_locale
               == false) { // skip the calculation if dm_in_dftu is nullptr
        return;
    } else {
    ...
    for (int iat0 = 0; iat0 < this->ucell->nat; iat0++) {
        ...
        if (this->dftu->initialed_locale == false) {
            const hamilt::HContainer<double>* dmR_current
                = this->dftu->get_dmr(this->current_spin);
            for (int ad1 = 0; ad1 < adjs.adj_num + 1; ++ad1) {
                const int T1 = adjs.ntype[ad1];
                const int I1 = adjs.natom[ad1];
                const int iat1 = ucell->itia2iat(T1, I1);
            ...
            for (int i = 0; i < occ.size(); i++) {
                if (this->nspin == 1)
                    occ[i] *= 0.5;
                this->dftu->locale[iat0][target_L][0][this->current_spin].c[i]
                    = occ[i];
            }
        } else // use readin locale to calculate occupation matrix
        {
            for (int i = 0; i < occ.size(); i++) {
                occ[i]
                    = this->dftu->locale[iat0][target_L][0][this->current_spin]
                          .c[i];
            }
            // set initialed_locale to false to avoid using readin locale in
            // next iteration
        }
        ModuleBase::timer::tick("DFTU", "cal_occ");
        ...
        // calculate <psi_I|beta_m> U*(1/2*delta(m, m')-occ(m, m'))
        // <beta_m'|psi_{J,R}> for each pair of <IJR> atoms
    ...
    if (this->nspin == 1)
        this->dftu->EU *= 2.0;
    // for readin onsite_dm, set initialed_locale to false to avoid using readin
    // locale in next iteration
...
void hamilt::DFTU<hamilt::OperatorLCAO<TK, TR>>::cal_HR_IJR(
    ...
    for (int iw1l = 0; iw1l < row_indexes.size(); iw1l += npol) {
        const std::vector<double>& nlm1
            = nlm1_all.find(row_indexes[iw1l])->second;
        for (int iw2l = 0; iw2l < col_indexes.size(); iw2l += npol) {
            const std::vector<double>& nlm2
                = nlm2_all.find(col_indexes[iw2l])->second;
#ifdef __DEBUG
            ...
            for (int is = 0; is < npol * npol; ++is) {
                ...
                for (int m1 = 0; m1 < m_size; m1++) {
                    for (int m2 = 0; m2 < m_size; m2++) {
                        nlm_tmp += nlm1[m1] * nlm2[m2]
                                   * VU[m1 * m_size + m2 + start];
                    }
                }
                    ...
                    for (int m1 = 0; m1 < m_size; ++m1) {
                        for (int m2 = 0; m2 < m_size; ++m2) {
                            occ[m1 * m_size + m2 + (is1 * npol + is2) * m_size2]
                                += nlm1[m1] * nlm2[m2]
                                   * dm_pointer[step_trace[is1 * npol + is2]];
                        }
                    }
                }
            }
...
template <typename TK, typename TR>
void hamilt::DFTU<hamilt::OperatorLCAO<TK, TR>>::transfer_vu(
...
void hamilt::DFTU<
    ...
    transfer_vu(std::vector<double>& vu_tmp,
    ...
    for (int m1 = 0; m1 < m_size; m1++) {
        for (int m2 = 0; m2 < m_size; m2++) {
            ...
            // vu should be complex<double> type, but here we use double type
            // for test
            vu[index[1]]
                = 0.5
                  * (vu_tmp[index[1]]
                     + std::complex<double>(0.0, 1.0) * vu_tmp[index[2]]);
            vu[index[2]]
                = 0.5
                  * (vu_tmp[index[1]]
                     - std::complex<double>(0.0, 1.0) * vu_tmp[index[2]]);
...
void hamilt::DFTU<hamilt::OperatorLCAO<TK, TR>>::cal_v_of_u(
    ...
    if (spin_fold < 4)
        for (int is = 0; is < spin_fold; ++is) {
            ...
            for (int m1 = 0; m1 < m_size; m1++) {
                for (int m2 = 0; m2 < m_size; m2++) {
                    vu[start + m1 * m_size + m2]
                        = u_value
                          * (0.5 * (m1 == m2) - occ[start + m2 * m_size + m1]);
                    eu += u_value * 0.5 * occ[start + m2 * m_size + m1]
        ...
        for (int m1 = 0; m1 < m_size; m1++) {
            for (int m2 = 0; m2 < m_size; m2++) {
                vu[m1 * m_size + m2]
                    = u_value * (1.0 * (m1 == m2) - occ[m2 * m_size + m1]);
                eu += u_value * 0.25 * occ[m2 * m_size + m1]
                      * occ[m1 * m_size + m2];
            }
        }
        for (int is = 1; is < spin_fold; ++is) {
            ...
            for (int m1 = 0; m1 < m_size; m1++) {
                for (int m2 = 0; m2 < m_size; m2++) {
                    vu[start + m1 * m_size + m2]
                        = u_value * (0 - occ[start + m2 * m_size + m1]);
                    eu += u_value * 0.25 * occ[start + m2 * m_size + m1]
                          * occ[start + m1 * m_size + m2];
                }
            }
        }
