...
// term A^2*S
template <typename TK, typename TR>
void TDEkinetic<OperatorLCAO<TK, TR>>::td_ekinetic_scalar(
...
// term A^2*S
template <>
void TDEkinetic<OperatorLCAO<std::complex<double>, double>>::td_ekinetic_scalar(
    std::complex<double>* Hloc,
    double* Sloc,
    int nnr) {
...
// term A dot ∇
template <typename TK, typename TR>
void TDEkinetic<OperatorLCAO<TK, TR>>::td_ekinetic_grad(
    std::complex<double>* Hloc,
    int nnr,
    ModuleBase::Vector3<double> grad_overlap) {
    // the correction term -iA dot ∇r
    //∇ refer to the integral ∫𝜙(𝑟)𝜕/𝜕𝑟𝜙(𝑟−𝑅)𝑑𝑟,but abacus only provide the
    //integral of ∫𝜙(𝑟)𝜕/𝜕R𝜙(𝑟−𝑅)𝑑𝑟. An extra
...
void TDEkinetic<OperatorLCAO<TK, TR>>::calculate_HR() {
    ...
    if (this->hR_tmp == nullptr || this->hR_tmp->size_atom_pairs() <= 0) {
        ModuleBase::WARNING_QUIT("TDEkinetic::calculate_HR",
                                 "hR_tmp is nullptr or empty");
    }
    ...
    for (int iat1 = 0; iat1 < this->ucell->nat; iat1++) {
        ...
        for (int ad = 0; ad < adjs.adj_num + 1; ++ad) {
            ...
            ModuleBase::Vector3<double> dtau
                = this->ucell->cal_dtau(iat1, iat2, R_index2);

            hamilt::BaseMatrix<std::complex<double>>* tmp
                = this->hR_tmp->find_matrix(iat1, iat2, R_index2);
            hamilt::BaseMatrix<TR>* tmps
                = this->SR->find_matrix(iat1, iat2, R_index2);
            if (tmp != nullptr) {
                if (TD_Velocity::out_current) {
                    std::complex<double>* tmp_c[3]
                        = {nullptr, nullptr, nullptr};
                    for (int i = 0; i < 3; i++) {
                        tmp_c[i] = td_velocity.get_current_term_pointer(i)
...
template <typename TK, typename TR>
void TDEkinetic<OperatorLCAO<TK, TR>>::cal_HR_IJR(
    const int& iat1,
    const int& iat2,
    const Parallel_Orbitals* paraV,
        ...
        for (int iw2l = 0; iw2l < col_indexes.size(); iw2l += npol) {
            ...
            if (data_pointer_c != nullptr) {
                for (int dir = 0; dir < 3; dir++) {
                    for (int ipol = 0; ipol < npol; ipol++) {
                        // part of Momentum operator, -i∇r,used to calculate the
                        // current here is actually i∇R
                        data_pointer_c[dir][ipol * step_trace]
                            += std::complex<double>(0, grad_overlap[dir]);
                    }
...
void TDEkinetic<OperatorLCAO<TK, TR>>::init_td(void) {
    ...
    td_velocity.cal_cart_At(this->ucell->a1,
                            this->ucell->a2,
                            this->ucell->a3,
                            elecstate::H_TDDFT_pw::At);
    this->cart_At = td_velocity.cart_At;
    std::cout << "cart_At: " << cart_At[0] << " " << cart_At[1] << " "
              << cart_At[2] << std::endl;
}

template <typename TK, typename TR>
void hamilt::TDEkinetic<hamilt::OperatorLCAO<TK, TR>>::set_HR_fixed(
    void* hR_tmp_in) {
    this->hR_tmp
...
template <typename TK, typename TR>
void TDEkinetic<OperatorLCAO<TK, TR>>::initialize_HR(
    Grid_Driver* GridD,
    const Parallel_Orbitals* paraV) {
    ...
    for (int iat1 = 0; iat1 < ucell->nat; iat1++) {
        ...
        for (int ad1 = 0; ad1 < adjs.adj_num + 1; ++ad1) {
            ...
            if (this->ucell->cal_dtau(iat1, iat2, R_index2).norm()
                    * this->ucell->lat0
...
void TDEkinetic<OperatorLCAO<TK, TR>>::contributeHR() {
    ...
    if (!this->hR_tmp_done) {
        const Parallel_Orbitals* paraV = this->hR->get_atom_pair(0).get_paraV();
        // if this Operator is the first node of the sub_chain, then hR_tmp is
        // nullptr
        if (this->hR_tmp == nullptr) {
...
template <>
void TDEkinetic<OperatorLCAO<std::complex<double>, double>>::contributeHk(
    int ik) {
    if (TD_Velocity::tddft_velocity == false) {
        ...
        const Parallel_Orbitals* paraV
            = this->hR_tmp->get_atom_pair(0).get_paraV();
