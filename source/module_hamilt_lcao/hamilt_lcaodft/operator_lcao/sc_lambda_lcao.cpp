...
std::vector<int>& OperatorScLambda<OperatorLCAO<std::complex<double>>>::get_iwt2iat()
{
...
// get lambda
std::vector<ModuleBase::Vector3<double>>& OperatorScLambda<OperatorLCAO<std::complex<double>>>::get_lambda()
{
    return this->lambda_;
}
...
void OperatorScLambda<OperatorLCAO<std::complex<double>>>::cal_weight_func(const std::vector<std::complex<double>>& Sloc2)
{
    ...
    this->W_i_.reserve(this->nloc_ * this->nloc_);
    for (int i = 0; i < this->nloc_; i++)
    {
        int iat = iwt2iat_[i];
