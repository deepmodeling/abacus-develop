...
/// and fold it to k-space HR = <psi_{mu, 0}|beta_p1>D_{p1, p2}<beta_p2|psi_{nu,
/// R}> HK = <psi_{mu, k}|beta_p1>D_{p1, p2}<beta_p2|psi_{nu, k}> = \sum_{R}
/// e^{ikR} HR Template parameters:
...
class DFTU<OperatorLCAO<TK, TR>> : public OperatorLCAO<TK, TR> {
  ...
  private:
    ...
    /// @brief the number of spin components, 1 for no-spin, 2 for collinear
    /// spin case and 4 for non-collinear spin case
    int nspin = 0;
    /// @brief the current spin index for nspin==2 to calculate spin-up and
    /// spin-down separately
    int current_spin = 0;

    /**
     * @brief search the nearest neighbor atoms and save them into
     * this->adjs_all the size of HR will not change in DFTU, because I don't
     * want to expand HR larger than Nonlocal operator caused by DFTU
     */
    void initialize_HR(Grid_Driver* gridD_in);

    /**
     * @brief calculate the <phi|alpha^I> overlap values and save them in
     * this->nlm_tot it will be reused in the calculation of calculate_HR()
     */
    void cal_nlm_all(const Parallel_Orbitals* paraV);

    /**
     * @brief calculate the occ_mm' = \sum_R
     * DMR*<phi_0|alpha^I_m'><alpha^I_m'|phi_R> matrix for each atom to add U
     */
    ...
    /// transfer VU format from pauli matrix to normal for non-collinear spin
    /// case
    ...
    void cal_v_of_u(const std::vector<double>& occ,
                    const int m_size,
    ...
    /// @brief the overlap values for all [atoms][nerghbors][orb_index(iw) in
    /// NAOs][m of target_l in Projectors]
