...
/// it is used to calculate the Deep Potential Kohn-Sham correction from DeePKS
...
class DeePKS<OperatorLCAO<TK, TR>> : public OperatorLCAO<TK, TR> {
...
#ifdef __DEEPKS
    /**
     * @brief contribute the DeePKS correction for each k-point to ld.H_V_delta
     * or ld.H_V_delta_k this function is not used for update hK, but for DeePKS
     * module
  ...
  private:
     ...
     * HContainer is used to store the DeePKS real space Hamiltonian correction
     * with specific <I,J,R> atom-pairs the size of HR will be fixed after
     * initialization
     */
    void initialize_HR(Grid_Driver* GridD);

    /**
     * @brief calculate the DeePKS correction matrix with specific <I,J,R>
     ...
     * used for calculate the DeePKS real space Hamiltonian correction with
     * specific <I,J,R> atom-pairs
     */
