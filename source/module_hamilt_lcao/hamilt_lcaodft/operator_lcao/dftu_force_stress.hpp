...
void DFTU<OperatorLCAO<TK, TR>>::cal_force_stress(const bool cal_force,
    ...
    // try to get the density matrix, if the density matrix is empty, skip the
    ...
    for (int iat0 = 0; iat0 < this->ucell->nat; iat0++) {
        ...
        //     DMR_{I,J,R'-R} * <phi_{I,R}|chi_m> U*(1/2*delta(m, m')-occ(m,
        //     m')) \frac{\partial <chi_m'|phi_{J,R'}>}{\partial \tau_J} for
        //     each pair of <IJR> atoms
        // calculate Stress for strain tensor \varepsilon_{\alpha\beta}
        //     -1/Omega * DMR_{I,J,R'-R} * [ \frac{\partial
        //     <phi_{I,R}|chi_m>}{\partial \tau_{J,\alpha}}\tau_{J,\beta}
        //     U*(1/2*delta(m, m')-occ(m, m'))<chi_m'|phi_{J,R'}>
        //   + <phi_{I,R}|chi_m> U*(1/2*delta(m, m')-occ(m, m'))
        //     \frac{\partial <chi_m'|phi_{J,R'}>}{\partial
        //     \tau_{J,\alpha}}\tau_{J,\beta}] for each pair of <IJR> atoms
        for (int ad1 = 0; ad1 < adjs.adj_num + 1; ++ad1) {
            ...
            for (int ad2 = 0; ad2 < adjs.adj_num + 1; ++ad2) {
                ...
                ModuleBase::Vector3<double> dis2
                    = adjs.adjacent_tau[ad2] - tau0;
                ModuleBase::Vector3<int> R_vector(R_index2[0] - R_index1[0],
                                                  R_index2[1] - R_index1[1],
                                                  R_index2[2] - R_index1[2]);
                const hamilt::BaseMatrix<double>* tmp[this->nspin];
                tmp[0] = dmR_tmp[0]->find_matrix(iat1,
                                                 iat2,
                                                 R_vector[0],
                                                 R_vector[1],
                                                 R_vector[2]);
                if (this->nspin == 2) {
                    tmp[1] = dmR_tmp[1]->find_matrix(iat1,
                                                     iat2,
                                                     R_vector[0],
                                                     R_vector[1],
                                                     R_vector[2]);
            ...
            for (int iw2l = 0; iw2l < col_indexes.size(); iw2l += npol) {
                ...
                for (int m1 = 0; m1 < m_size; m1++) {
                    for (int m2 = 0; m2 < m_size; m2++) {
                        tmp[0] = vu_in[m1 * m_size + m2 + is * m_size2]
                                 * nlm1[m1 + m_size] * nlm2[m2] * dm_pointer[0];
                        tmp[1] = vu_in[m1 * m_size + m2 + is * m_size2]
                                 * nlm1[m1 + m_size * 2] * nlm2[m2]
                                 * dm_pointer[0];
                        tmp[2] = vu_in[m1 * m_size + m2 + is * m_size2]
                                 * nlm1[m1 + m_size * 3] * nlm2[m2]
                                 * dm_pointer[0];
                        // force1 = - VU * <d phi_{I,R1}/d R1|chi_m> *
                        // <chi_m'|phi_{J,R2}> force2 = - VU * <phi_{I,R1}|d
                        // chi_m/d R0> * <chi_m'|phi_{J,R2>}
                        force1[0] += tmp[0];
                        force1[1] += tmp[1];
                        force1[2] += tmp[2];
                        force2[0] -= tmp[0];
    ...
    for (int is = 0; is < nspin; is++) {
        ...
        for (int iw1l = 0; iw1l < row_indexes.size(); iw1l += npol) {
            ...
            for (int iw2l = 0; iw2l < col_indexes.size(); iw2l += npol) {
                ...
                for (int m1 = 0; m1 < m_size; m1++) {
                    for (int m2 = 0; m2 < m_size; m2++) {
                        double tmp = vu_in[m1 * m_size + m2 + is * m_size2]
                                     * dm_pointer[0];
                        // std::cout<<__FILE__<<__LINE__<<" "<<tmp<<" "<<m1<<"
                        // "<<m2<<" "<<nlm1[m1 + m_size * 2]<<"
                        // "<<nlm2[m2 + m_size * 2]<<" "<<dis1.y<<"
                        // "<<dis2.y<<std::endl;
                        stress[0]
                            += tmp
                               * (nlm1[m1 + m_size] * dis1.x * nlm2[m2]
                                  + nlm1[m1] * nlm2[m2 + m_size] * dis2.x);
                        stress[1]
                            += tmp
                               * (nlm1[m1 + m_size] * dis1.y * nlm2[m2]
                                  + nlm1[m1] * nlm2[m2 + m_size] * dis2.y);
                        stress[2]
                            += tmp
                               * (nlm1[m1 + m_size] * dis1.z * nlm2[m2]
