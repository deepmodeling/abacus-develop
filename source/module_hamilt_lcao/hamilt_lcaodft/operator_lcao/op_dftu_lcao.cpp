...
void OperatorDFTU<OperatorLCAO<double, double>>::contributeHk(int ik) {
    ...
    // Effective potential of DFT+U is added to total Hamiltonian here; Quxin
    // adds on 20201029
    std::vector<double> eff_pot(this->hsk->get_pv()->nloc);
    GlobalC::dftu.cal_eff_pot_mat_real(ik,
                                       &eff_pot[0],
                                       isk,
                                       this->hsk->get_sk());
...
template <>
void OperatorDFTU<OperatorLCAO<std::complex<double>, double>>::contributeHk(
    int ik) {
    ModuleBase::TITLE("OperatorDFTU", "contributeHk");
    ModuleBase::timer::tick("OperatorDFTU", "contributeHk");
    // Effective potential of DFT+U is added to total Hamiltonian here; Quxin
    ...
    GlobalC::dftu.cal_eff_pot_mat_complex(ik,
                                          &eff_pot[0],
                                          isk,
                                          this->hsk->get_sk());
...
template <>
void OperatorDFTU<OperatorLCAO<std::complex<double>,
