...
// EkineticNew and NonlocalNew are derivative classes of Operator, used to
// calculate the T+VNL matrix It use HContainer to store the real space HR
// matrix In this test, we test the correctness and time consuming of 6
// functions in T+VNL class
...
class TNLTest : public ::testing::Test {
  protected:
    void SetUp() override {
        ...
        // set up a unitcell, with one element and test_size atoms, each atom
        // has test_nw orbitals
        ucell.ntype = 1;
        ucell.nat = test_size;
...
TEST_F(TNLTest, testTVNLcd2cd) {
    ...
    hamilt::Operator<std::complex<double>>* op = new hamilt::EkineticNew<
        ...
        &gd,
        &intor_);
    hamilt::Operator<std::complex<double>>* op1 = new hamilt::NonlocalNew<
        hamilt::OperatorLCAO<std::complex<double>, std::complex<double>>>(
        &hsk,
        kvec_d_in,
        HR,
    ...
    std::chrono::duration<double> elapsed_time2
        = std::chrono::duration_cast<std::chrono::duration<double>>(
            end_time - start_time);
    std::cout << "Test terms:   " << std::setw(15) << "constructor"
              << std::setw(15) << "init(HR+HK)" << std::setw(15)
              << "2nd-init(HK)" << std::endl;
    std::cout << "Elapsed time: " << std::setw(15) << elapsed_time0.count()
              << std::setw(15) << elapsed_time1.count() << std::setw(15)
              << elapsed_time2.count() << " seconds." << std::endl;
