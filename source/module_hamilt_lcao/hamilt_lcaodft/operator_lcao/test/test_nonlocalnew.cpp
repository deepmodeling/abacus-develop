...
// NonlocalNew is a derivative class of Operator, it is used to calculate the
// kinetic matrix It use HContainer to store the real space HR matrix In this
// test, we test the correctness and time consuming of 3 functions in
// NonlocalNew class
...
TEST_F(NonlocalNewTest, constructHRd2d) {
    ...
    hamilt::NonlocalNew<hamilt::OperatorLCAO<double, double>> op(&hsk,
                                                                 kvec_d_in,
                                                                 HR,
                                                                 &ucell,
                                                                 &gd,
    ...
    std::cout << "Test terms:   " << std::setw(15) << "initialize_HR"
              << std::setw(15) << "contributeHR" << std::setw(15)
              << "contributeHk" << std::setw(15) << "2nd-calHR" << std::endl;
    std::cout << "Elapsed time: " << std::setw(15) << elapsed_time.count()
              << std::setw(15) << elapsed_time1.count() << std::setw(15)
              << elapsed_time2.count() << std::setw(15) << elapsed_time3.count()
              << " seconds." << std::endl;
}
