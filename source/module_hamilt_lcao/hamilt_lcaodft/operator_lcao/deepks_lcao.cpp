...
DeePKS<OperatorLCAO<TK, TR>>::DeePKS(
    ...
    : nks(nks_in),
      ucell(ucell_in), OperatorLCAO<TK, TR>(hsk_in, kvec_d_in, hR_in),
      DM(DM_in), intor_orb_alpha_(intor_orb_alpha) {
...
// initialize_HR()
template <typename TK, typename TR>
void hamilt::DeePKS<hamilt::OperatorLCAO<TK, TR>>::initialize_HR(
    Grid_Driver* GridD) {
    ...
    if (ucell->nat
        < 100) // less than 100 atom , cost memory for high performance
    {          // pre calculate nlm in initialization
        this->nlm_tot.resize(ucell->nat);
        pre_cal_nlm = true;
    } else { // calculate nlm on the fly
    ...
    for (int iat0 = 0; iat0 < ucell->nat; iat0++) {
        ...
        for (int ad1 = 0; ad1 < adjs.adj_num + 1; ++ad1) {
            ...
            for (int ad2 = 0; ad2 < adjs.adj_num + 1; ++ad2) {
                ...
                if (paraV->get_col_size(iat2) <= 0
                    || paraV->get_row_size(iat1) <= 0) {
                    continue;
                }
    ...
    if (std::is_same<TK, double>::value) {
        ...
        // multi-k keep same size of nonlocal operator, H_V_delta will be
        // allocated by hR
...
void DeePKS<OperatorLCAO<std::complex<double>, double>>::contributeHR() {
    ...
    // this operator should be informed that DM_K has changed and HR need to
    // recalculate.
    if (GlobalC::ld.get_hr_cal()) {
        ModuleBase::timer::tick("DeePKS", "contributeHR");
...
template <>
void DeePKS<
    OperatorLCAO<std::complex<double>, std::complex<double>>>::contributeHR() {
#ifdef __DEEPKS
    ModuleBase::TITLE("DeePKS", "contributeHR");
    // if DM_K changed, HR of DeePKS need to refresh.
    // the judgement is based on the status of HR in GlobalC::ld
    ...
    if (GlobalC::ld.get_hr_cal()) {
        ...
        if (this->H_V_delta == nullptr) {
            this->H_V_delta
                = new hamilt::HContainer<std::complex<double>>(*this->hR);
        }
        this->H_V_delta->set_zero();
        this->calculate_HR();
...
void hamilt::DeePKS<hamilt::OperatorLCAO<TK, TR>>::pre_calculate_nlm(
    ...
    for (int ad = 0; ad < adjs.adj_num + 1; ++ad) {
        ...
        all_indexes.erase(std::unique(all_indexes.begin(), all_indexes.end()),
                          all_indexes.end());
        for (int iw1l = 0; iw1l < all_indexes.size(); iw1l += npol) {
            const int iw1 = all_indexes[iw1l] / npol;
            std::vector<std::vector<double>> nlm;
            // nlm is a vector of vectors, but size of outer vector is only 1
            // here If we are calculating force, we need also to store the
        ...
        for (int ad1 = 0; ad1 < adjs.adj_num + 1; ++ad1) {
            ...
            for (int ad2 = 0; ad2 < adjs.adj_num + 1; ++ad2) {
                ...
                hamilt::BaseMatrix<TR>* tmp
                    = this->H_V_delta->find_matrix(iat1,
                                                   iat2,
                                                   R_vector[0],
                                                   R_vector[1],
                                                   R_vector[2]);
                ...
                for (int icol = 0; icol < col_size; icol++) {
                    const double* col_ptr
                        = nlm_iat[ad2][col_indexes[icol]].data();
                    ...
                    for(int icol=0;icol<col_size;icol++)
                    {
                        for(int ialpha=0;ialpha<trace_alpha_size;ialpha++)
                        {
                            tmp->get_pointer()[irow*col_size+icol] +=
                                s_1t[irow*trace_alpha_size+ialpha] *
                s_2t[icol*trace_alpha_size+ialpha];
                        }
                    }
                }*/
                ...
                this->cal_HR_IJR(hr_current.data(),
                                 row_size,
                                 col_size,
                                 tmp->get_pointer());
            }
...
void hamilt::DeePKS<hamilt::OperatorLCAO<TK, TR>>::contributeHk(int ik) {
    ...
    if (ModuleBase::GlobalFunc::IS_COLUMN_MAJOR_KS_SOLVER()) {
        ...
        hamilt::folding_HR(*this->H_V_delta,
                           h_delta_k,
                           this->kvec_d[ik],
                           nrow,
                           1);
