...
// initialize_HR()
template <typename TK, typename TR>
void hamilt::TDNonlocal<hamilt::OperatorLCAO<TK, TR>>::initialize_HR(
    Grid_Driver* GridD,
    const Parallel_Orbitals* paraV) {
    ...
    for (int iat0 = 0; iat0 < ucell->nat; iat0++) {
        ...
        for (int ad1 = 0; ad1 < adjs.adj_num + 1; ++ad1) {
            ...
            if (this->ucell->cal_dtau(iat0, iat1, R_index1).norm()
                    * this->ucell->lat0
                < orb.Phi[T1].getRcut()
                      + this->ucell->infoNL.Beta[T0].get_rcut_max()) {
                is_adj[ad1] = true;
            }
        }
...
// initialize_HR_tmp()
template <typename TK, typename TR>
void hamilt::TDNonlocal<hamilt::OperatorLCAO<TK, TR>>::initialize_HR_tmp(
    const Parallel_Orbitals* paraV) {
    if (elecstate::H_TDDFT_pw::stype != 1) {
        return;
    }
    ...
    for (int i = 0; i < this->hR->size_atom_pairs(); ++i) {
        ...
        for (int ir = 0; ir < tmp.get_R_size(); ++ir) {
            ...
            hamilt::AtomPair<std::complex<double>> tmp1(iat1,
                                                        iat2,
                                                        R_index,
                                                        paraV);
...
#pragma omp for
        ...
        for (int iat0 = 0; iat0 < this->ucell->nat; iat0++) {
            ...
            for (int ad = 0; ad < adjs.adj_num + 1; ++ad) {
                ...
                // insert col_indexes into all_indexes to get universal set with
                // no repeat elements
                ...
                all_indexes.erase(
                    std::unique(all_indexes.begin(), all_indexes.end()),
                    all_indexes.end());
                for (int iw1l = 0; iw1l < all_indexes.size(); iw1l += npol) {
                    const int iw1 = all_indexes[iw1l] / npol;
                    std::vector<std::vector<std::complex<double>>> nlm;
                    // nlm is a vector of vectors, but size of outer vector is
                    // only 1 when out_current is false and size of outer vector
                    // is 4 when out_current is true (3 for <psi|r_i *
                    // exp(-iAr)|beta>, 1 for <psi|exp(-iAr)|beta>) inner loop :
                    // all projectors (L0,M0)

                    // snap_psibeta_half_tddft() are used to calculate
                    // <psi|exp(-iAr)|beta> and <psi|rexp(-iAr)|beta> as well if
                    // current are needed
            ...
            // 2. calculate <psi_I|beta>D<beta|psi_{J,R}> for each pair of <IJR>
            // atoms
            for (int ad1 = 0; ad1 < adjs.adj_num + 1; ++ad1) {
                const int T1 = adjs.ntype[ad1];
                const int I1 = adjs.natom[ad1];
                ...
                for (int ad2 = 0; ad2 < adjs.adj_num + 1; ++ad2) {
                    ...
                    if (tmp != nullptr) {
                        if (TD_Velocity::out_current) {
                            std::complex<double>* tmp_c[3]
                                = {nullptr, nullptr, nullptr};
                            for (int i = 0; i < 3; i++) {
                                tmp_c[i]
                                    = TD_Velocity::td_vel_op
                                          ->get_current_term_pointer(i)
...
void hamilt::TDNonlocal<hamilt::OperatorLCAO<TK, TR>>::cal_HR_IJR(
    ...
    const Parallel_Orbitals* paraV,
    const std::vector<
        std::unordered_map<int, std::vector<std::complex<double>>>>& nlm1_all,
    const std::vector<
        std::unordered_map<int, std::vector<std::complex<double>>>>& nlm2_all,
    ...
    for (int iw1l = 0; iw1l < row_indexes.size(); iw1l += npol) {
        // const std::vector<std::complex<double>>* nlm1 =
        // &(nlm1_all[0].find(row_indexes[iw1l])->second);
        ...
        for (int iw2l = 0; iw2l < col_indexes.size(); iw2l += npol) {
            ...
            for (int dir = 0; dir < nlm_dim; dir++) {
                nlm2.push_back(
                    &(nlm2_all[dir].find(col_indexes[iw2l])->second));
            }
#ifdef __DEBUG
            ...
            for (int is = 0; is < npol * npol; ++is) {
                ...
                for (int no = 0;
                     ...
                     no++) {
                    const int p1
                        = this->ucell->atoms[T0].ncpp.index1_soc[is][no];
                    const int p2
                        = this->ucell->atoms[T0].ncpp.index2_soc[is][no];
                    this->ucell->atoms[T0].ncpp.get_d(is, p1, p2, tmp_d);
                    nlm_tmp += nlm1[0]->at(p1) * std::conj(nlm2[0]->at(p2))
                               * (*tmp_d);
                }
                data_pointer[step_trace[is]] += nlm_tmp;
                if (data_pointer_c != nullptr) {
                    for (int dir = 0; dir < 3; dir++) {
                        std::complex<double> nlm_r_tmp
                            = std::complex<double>{0, 0};
                        std::complex<double> imag_unit
                            = std::complex<double>{0, 1};
                        for (int no = 0; no < this->ucell->atoms[T0]
                                                  .ncpp.non_zero_count_soc[is];
                             no++) {
                            const int p1 = this->ucell->atoms[T0]
                                               .ncpp.index1_soc[is][no];
                            const int p2 = this->ucell->atoms[T0]
                                               .ncpp.index2_soc[is][no];
                            this->ucell->atoms[T0].ncpp.get_d(is,
                                                              p1,
                                                              p2,
                                                              tmp_d);
                            ...
                            nlm_r_tmp
                                += (nlm1[dir + 1]->at(p1)
                                        * std::conj(nlm2[0]->at(p2))
                                    - nlm1[0]->at(p1)
                                          * std::conj(nlm2[dir + 1]->at(p2)))
                                   * (*tmp_d);
                        }
                        // -i[r,Vnl], 2.0 due to the unit transformation
                        data_pointer_c[dir][step_trace[is]]
                            -= imag_unit * nlm_r_tmp / 2.0;
                    }
                }
            }
            data_pointer += npol;
...
// set_hR_tmp()
template <typename TK, typename TR>
void hamilt::TDNonlocal<hamilt::OperatorLCAO<TK, TR>>::set_HR_fixed(
    void* hR_tmp_in) {
    this->hR_tmp
        = static_cast<hamilt::HContainer<std::complex<double>>*>(hR_tmp_in);
...
void hamilt::TDNonlocal<hamilt::OperatorLCAO<TK, TR>>::contributeHR() {
    ...
    if (!this->hR_tmp_done) {
        if (this->hR_tmp == nullptr) {
            this->hR_tmp
                = new hamilt::HContainer<std::complex<double>>(this->LM->ParaV);
        ...
        if (this->next_sub_op != nullptr) {
            ...
            static_cast<OperatorLCAO<TK, TR>*>(this->next_sub_op)
                ->set_HR_fixed(this->hR_tmp);
        }
...
template <>
void hamilt::TDNonlocal<
    hamilt::OperatorLCAO<std::complex<double>, double>>::contributeHk(int ik) {
...
template class hamilt::TDNonlocal<
