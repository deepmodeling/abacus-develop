...
/// @brief Effective potential class, used for calculating Hamiltonian with grid
/// integration tools If user want to separate the contribution of V_{eff} into
/// V_{H} and V_{XC} and V_{local pseudopotential} and so on, the user can
/// separate the Potential class into different parts, and construct different
/// Veff class for each part.
/// @tparam TK
/// @tparam TR
...
class Veff<OperatorLCAO<TK, TR>> : public OperatorLCAO<TK, TR> {
  public:
    ...
    Veff<OperatorLCAO<TK, TR>>(
        ...
        Grid_Driver* GridD_in)
        : GG(GG_in),
          pot(pot_in), OperatorLCAO<TK, TR>(hsk_in, kvec_d_in, hR_in) {
        this->cal_type = calculation_type::lcao_gint;
        this->initialize_HR(ucell_in, GridD_in);
     ...
     * the contribution of V_{eff} is calculated by the contribution of V_{H}
     * and V_{XC} and V_{local pseudopotential} and so on. grid integration is
     * used to calculate the contribution Hamiltonian of effective potential
     */
  ...
  private:
    ...
    // Charge calculating method in LCAO base and contained grid base
    // calculation: DM_R, DM, pvpR_reduced
