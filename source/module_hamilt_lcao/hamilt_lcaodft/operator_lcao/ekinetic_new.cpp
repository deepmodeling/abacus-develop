...
// initialize_HR()
template <typename TK, typename TR>
void hamilt::EkineticNew<hamilt::OperatorLCAO<TK, TR>>::initialize_HR(
    Grid_Driver* GridD) {
...
void hamilt::EkineticNew<hamilt::OperatorLCAO<TK, TR>>::calculate_HR() {
    ...
    if (this->HR_fixed == nullptr || this->HR_fixed->size_atom_pairs() <= 0) {
        ModuleBase::WARNING_QUIT("hamilt::EkineticNew::calculate_HR",
                                 "HR_fixed is nullptr or empty");
    }
    ...
    for (int iat1 = 0; iat1 < this->ucell->nat; iat1++) {
        ...
        for (int ad = 0; ad < adjs.adj_num + 1; ++ad) {
            ...
            hamilt::BaseMatrix<TR>* tmp
                = this->HR_fixed->find_matrix(iat1, iat2, R_index2);
            if (tmp != nullptr) {
...
// cal_HR_IJR()
template <typename TK, typename TR>
void hamilt::EkineticNew<hamilt::OperatorLCAO<TK, TR>>::cal_HR_IJR(
    const int& iat1,
    const int& iat2,
    const Parallel_Orbitals* paraV,
...
// set_HR_fixed()
template <typename TK, typename TR>
void hamilt::EkineticNew<hamilt::OperatorLCAO<TK, TR>>::set_HR_fixed(
    void* HR_fixed_in) {
...
template class hamilt::EkineticNew<
