...
template <typename T>
Force_Stress_LCAO<T>::Force_Stress_LCAO(Record_adj& ra, const int nat_in)
    : RA(&ra), f_pw(nat_in), nat(nat_in) {}
template <typename T>
Force_Stress_LCAO<T>::~Force_Stress_LCAO() {}
template <typename T>
void Force_Stress_LCAO<T>::getForceStress(
    const bool isforce,
    const bool isstress,
    const bool istestf,
    const bool istests,
    ...
    fvl_dphi.create(nat,
                    3); // must do it now, update it later, noted by zhengdy
    ...
    if (GlobalV::dft_plus_u) // Quxin add for DFT+U on 20201029
    {
        ...
        if (GlobalV::dft_plus_u == 2) {
            ...
            hamilt::DFTU<hamilt::OperatorLCAO<T, double>> tmp_dftu(
                nullptr, // HK and SK are not used for force&stress
                kv.kvec_d,
            ...
            tmp_dftu.cal_force_stress(isforce,
                                      isstress,
                                      force_dftu,
                                      stress_dftu);
        }
    ...
    if (GlobalC::exx_info.info_global.cal_exx) {
        if (isforce) {
            if (GlobalC::exx_info.info_ri.real_number) {
                ...
                force_exx = GlobalC::exx_info.info_global.hybrid_alpha
                            * exx_lri_double.force_exx;
            } else {
                exx_lri_complex.cal_exx_force();
                force_exx = GlobalC::exx_info.info_global.hybrid_alpha
                            * exx_lri_complex.force_exx;
            }
        }
        if (isstress) {
            if (GlobalC::exx_info.info_ri.real_number) {
                exx_lri_double.cal_exx_stress();
                stress_exx = GlobalC::exx_info.info_global.hybrid_alpha
                             * exx_lri_double.stress_exx;
            } else {
                exx_lri_complex.cal_exx_stress();
                stress_exx = GlobalC::exx_info.info_global.hybrid_alpha
                             * exx_lri_complex.stress_exx;
            }
        }
    }
#endif
    ...
    if (isforce) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            for (int iat = 0; iat < nat; iat++) {
                fcs(iat, i)
                    += foverlap(iat, i) + ftvnl_dphi(iat, i)
                       + fvnl_dbeta(iat, i) + fvl_dphi(iat, i)
                       + fvl_dvl(iat,
                                 i) // derivative of local potential force (pw)
            ...
            if (GlobalV::OUT_LEVEL != "m") {
                GlobalV::ofs_running
                    << " correction force for each atom along direction "
                    << i + 1 << " is " << sum / nat << std::endl;
            }
        }

        if (GlobalV::GATE_FLAG || GlobalV::EFIELD_FLAG) {
            GlobalV::ofs_running << "Atomic forces are not shifted if "
                                    "gate_flag or efield_flag == true!"
                                 << std::endl;
        }
        ...
        {
            GlobalC::ld.save_npy_f(fcs,
                                   "f_tot.npy",
                                   GlobalC::ucell.nat); // Ty/Bohr, F_tot
            if (GlobalV::deepks_scf) {
                GlobalC::ld.save_npy_f(fcs - GlobalC::ld.F_delta,
                                       "f_base.npy",
                                       GlobalC::ucell.nat); // Ry/Bohr, F_base

                if (!GlobalV::deepks_equiv) // training with force label not
                                            // supported by equivariant version
                                            // now
                {
                    if (GlobalV::GAMMA_ONLY_LOCAL) {
                        const std::vector<std::vector<double>>& dm_gamma
                            = dynamic_cast<
                                  const elecstate::ElecStateLCAO<double>*>(
                                  pelec)
                                  ->get_DM()
                                  ->get_DMK_vector();
                        ...
                        const std::vector<std::vector<std::complex<double>>>&
                            dm_k
                            = dynamic_cast<const elecstate::ElecStateLCAO<
                                std::complex<double>>*>(pelec)
                                  ->get_DM()
                                  ->get_DMK_vector();
                    ...
                    GlobalC::ld.save_npy_gvx(
                        GlobalC::ucell.nat); //  /Bohr, grad_vx
                }
            } else {
                GlobalC::ld.save_npy_f(
                    fcs,
                    "f_base.npy",
                    GlobalC::ucell.nat); // no scf, F_base=F_tot
            }
        }
#endif
        ...
        if (istestf) {
            ...
            ModuleIO::print_force(GlobalV::ofs_running,
                                  ...
                                  foverlap,
                                  0);
            //  this->print_force("TVNL_DPHI
            //  force",ftvnl_dphi,GlobalV::TEST_FORCE);
            //  this->print_force("VNL_DBETA
            //  force",fvnl_dbeta,GlobalV::TEST_FORCE);
            ...
            if (GlobalV::deepks_scf) {
                ModuleIO::print_force(GlobalV::ofs_running,
                                      ...
                                      GlobalC::ld.F_delta,
                                      1);
                // this->print_force("DeePKS 	FORCE", GlobalC::ld.F_delta, 1,
                // ry);
            }
#endif
        }
        ...
        if (istestf) {
            ...
            GlobalV::ofs_running << " " << std::setw(8) << "atom"
                                 << std::setw(5) << "x" << std::setw(5) << "y"
    ...
    if (isstress) {
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                scs(i, j)
                    += soverlap(i, j) + stvnl_dphi(i, j) + svnl_dbeta(i, j)
                       + svl_dphi(i, j)
                       + sigmadvl(
                           i,
                           j) // derivative of local potential stress (pw)
        ...
        if (GlobalV::deepks_out_labels) // not parallelized yet
        {
            GlobalC::ld.save_npy_s(
                ...
                "s_base.npy",
                GlobalC::ucell
                    .omega); // change to energy unit Ry when printing, S_base;
        }
        if (GlobalV::deepks_scf) {
            if (ModuleSymmetry::Symmetry::symm_flag == 1) {
        ...
        if (GlobalV::deepks_out_labels) // not parallelized yet
        {
            ...
            if (GlobalV::deepks_scf) {
                GlobalC::ld.save_npy_s(
                    scs,
                    "s_tot.npy",
                    GlobalC::ucell.omega);  // change to energy unit Ry when
                                            // printing, S_tot, w/ model
                if (!GlobalV::deepks_equiv) // training with stress label not
                                            // supported by equivariant version
                                            // now
                {
                    GlobalC::ld.cal_gvepsl(GlobalC::ucell.nat);
                    GlobalC::ld.save_npy_gvepsl(
                        GlobalC::ucell.nat); //  unitless, grad_vepsl
                }
            } else {
                GlobalC::ld.save_npy_s(
                    scs,
        ...
        if (istests) {
            ...
            ModuleIO::print_stress("OVERLAP  STRESS",
                                   soverlap,
                                   GlobalV::TEST_STRESS,
                                   ry);
            // test
            ModuleIO::print_stress("T        STRESS",
                                   stvnl_dphi,
                                   GlobalV::TEST_STRESS,
                                   ry);
            ModuleIO::print_stress("VNL      STRESS",
                                   svnl_dbeta,
                                   GlobalV::TEST_STRESS,
                                   ry);

            ModuleIO::print_stress("T_VNL    STRESS",
                                   stvnl,
                                   GlobalV::TEST_STRESS,
                                   ry);

            ModuleIO::print_stress("VL_dPHI  STRESS",
                                   svl_dphi,
                                   GlobalV::TEST_STRESS,
                                   ry);
            ModuleIO::print_stress("VL_dVL   STRESS",
                                   sigmadvl,
                                   GlobalV::TEST_STRESS,
                                   ry);
            ModuleIO::print_stress("HAR      STRESS",
                                   sigmahar,
                                   GlobalV::TEST_STRESS,
                                   ry);

            ModuleIO::print_stress("EWALD    STRESS",
                                   sigmaewa,
                                   GlobalV::TEST_STRESS,
                                   ry);
            ModuleIO::print_stress("cc       STRESS",
                                   sigmacc,
                                   GlobalV::TEST_STRESS,
                                   ry);
            //		ModuleIO::print_stress("NLCC
            //STRESS",sigmacc,GlobalV::TEST_STRESS,ry);
            ModuleIO::print_stress("XC       STRESS",
                                   sigmaxc,
                                   GlobalV::TEST_STRESS,
                                   ry);
            if (vdw_solver != nullptr) {
                ModuleIO::print_stress("VDW      STRESS",
                                       sigmaxc,
                                       GlobalV::TEST_STRESS,
                                       ry);
            }
            if (GlobalV::dft_plus_u) {
                ModuleIO::print_stress("DFTU     STRESS",
                                       stress_dftu,
                                       GlobalV::TEST_STRESS,
                                       ry);
            }
            ModuleIO::print_stress("TOTAL    STRESS",
                                   scs,
                                   GlobalV::TEST_STRESS,
                                   ry);
        ...
        unit_transform = ModuleBase::RYDBERG_SI
                         / pow(ModuleBase::BOHR_RADIUS_SI, 3) * 1.0e-8;
...
// overlap, kinetic, nonlocal pseudopotential, Local potential terms in force
// and stress
template <>
...
template <typename T>
void Force_Stress_LCAO<T>::forceSymmetry(ModuleBase::matrix& fcs,
                                         ModuleSymmetry::Symmetry* symm) {
    double d1, d2, d3;
    for (int iat = 0; iat < GlobalC::ucell.nat; iat++) {
