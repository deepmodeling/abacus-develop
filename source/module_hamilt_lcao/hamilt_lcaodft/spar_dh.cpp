...
void sparse_format::cal_dH(LCAO_Matrix& lm,
    ...
    gint_k.cal_dvlocal_R_sparseMatrix(current_spin,
                                      sparse_thr,
                                      &lm,
                                      HS_Arrays,
                                      lm.ParaV,
    ...
    for (int ix = 0; ix < Rx; ix++) {
        for (int iy = 0; iy < Ry; iy++) {
            for (int iz = 0; iz < Rz; iz++) {
                Abfs::Vector3_Order<int> temp_R(ix + RminX,
                                                iy + RminY,
                                                iz + RminZ);
                        ...
                        for (int jj = 0; jj < atom2->nw * GlobalV::NPOL; jj++) {
                            ...
                            if (GlobalV::NSPIN != 4) {
                                ...
                                if (std::abs(temp_value_double) > sparse_thr) {
                                    HS_Arrays.dHRx_sparse[current_spin][dR]
                                                         [iw1_all][iw2_all]
                                        = temp_value_double;
                                }
                                temp_value_double = fsr.DHloc_fixedR_y[index];
                                if (std::abs(temp_value_double) > sparse_thr) {
                                    HS_Arrays.dHRy_sparse[current_spin][dR]
                                                         [iw1_all][iw2_all]
                                        = temp_value_double;
                                }
                                temp_value_double = fsr.DHloc_fixedR_z[index];
                                if (std::abs(temp_value_double) > sparse_thr) {
                                    HS_Arrays.dHRz_sparse[current_spin][dR]
                                                         [iw1_all][iw2_all]
                                        = temp_value_double;
                                }
                            } else {
...
void sparse_format::destroy_dH_R_sparse(LCAO_HS_Arrays& HS_Arrays) {
    ...
    if (GlobalV::NSPIN != 4) {
        std::map<Abfs::Vector3_Order<int>,
            ...
            empty_dHRx_sparse_up;
        std::map<Abfs::Vector3_Order<int>,
                 std::map<size_t, std::map<size_t, double>>>
            empty_dHRx_sparse_down;
        std::map<Abfs::Vector3_Order<int>,
                 std::map<size_t, std::map<size_t, double>>>
            empty_dHRy_sparse_up;
        std::map<Abfs::Vector3_Order<int>,
                 std::map<size_t, std::map<size_t, double>>>
            empty_dHRy_sparse_down;
        std::map<Abfs::Vector3_Order<int>,
                 std::map<size_t, std::map<size_t, double>>>
            empty_dHRz_sparse_up;
        std::map<Abfs::Vector3_Order<int>,
                 std::map<size_t, std::map<size_t, double>>>
            empty_dHRz_sparse_down;
