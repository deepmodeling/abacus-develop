...
void Local_Orbital_Charge::gamma_file(psi::Psi<double>* psid,
                                      elecstate::ElecState* pelec) {
    ...
    if (GlobalV::KS_SOLVER == "genelpa" || GlobalV::KS_SOLVER == "lapack"
    ...
    if (psid == nullptr) {
        ModuleBase::WARNING_QUIT("gamma_file",
                                 "psid should be allocated first!");
    } else {
    ...
    for (int is = 0; is < GlobalV::NSPIN; ++is) {
        ...
        switch (error) {
        case 1:
            std::cout << "Can't find the wave function file: WFC_NAO_GAMMA"
                      << is + 1 << ".txt" << std::endl;
            break;
        case 2:
            std::cout << "In wave function file, band number doesn't match"
                      << std::endl;
            break;
        case 3:
            std::cout << "In wave function file, nlocal doesn't match"
                      << std::endl;
            break;
        case 4:
            std::cout
            ...
            break;
        default:
            std::cout << " Successfully read in wave functions " << is
                      << std::endl;
        }
        if (error) {
            ModuleBase::WARNING_QUIT("Local_Orbital_wfc::gamma_file",
                                     "Failed to read in wavefunction.");
        }
    } // loop ispin
}

void Local_Orbital_Charge::allocate_k(
    const int& lgd,
    psi::Psi<std::complex<double>>* psi,
    elecstate::ElecState* pelec,
    const int& nks,
    ...
    if (GlobalV::NLOCAL < GlobalV::NBANDS) {
        ModuleBase::WARNING_QUIT("Local_Orbital_wfc::allocate",
                                 "NLOCAL<NBANDS");
    }
    ...
    if (lgd != 0) {
        ...
        this->wfc_k_grid2
            = new std::complex<double>[nks * page]; // wfc_k_grid2 stores nks *
                                                    // nbands * lgd number of
                                                    // basis coefficients
        ModuleBase::GlobalFunc::ZEROS(wfc_k_grid2, nks * page);
        for (int ik = 0; ik < nks; ik++) {
            for (int ib = 0; ib < GlobalV::NBANDS; ib++) {
                this->wfc_k_grid[ik][ib]
                    = &wfc_k_grid2[ik * page + ib * lgd + 0];
            }
    ...
    if (INPUT.init_wfc
        == "file") // init_wfc can also be "atomic" but actually do nothing.
    {
        // confusing, seems istep to be the index of scf step. if not the first
        // scf, why call this function?
        if (istep > 0) {
            return;
        }
        std::cout << " Read in wave functions files: " << nkstot << std::endl;
        if (psi == nullptr) {
            ModuleBase::WARNING_QUIT("allocate_k",
                                     "psi should be allocated first!");
        } else {
        ...
        for (int ik = 0; ik < nkstot; ++ik) {
            ...
            switch (error) {
            case 1:
                std::cout << "Can't find the wave function file: WFC_NAO_K"
                          << ik + 1 << ".txt" << std::endl;
                break;
            case 2:
                std::cout << "In wave function file, band number doesn't match"
            ...
            case 3:
                ...
                break;
            case 4:
                std::cout << "In k-dependent wave function file, k point is "
                             "not correct"
                          << std::endl;
                break;
            default:
                std::cout << " Successfully read in wave functions " << ik + 1
                          << std::endl;
            }
            if (error) {
                ModuleBase::WARNING_QUIT("Local_Orbital_wfc::allocate_k",
                                         "Failed to read in wavefunction.");
            }
        }
    }
