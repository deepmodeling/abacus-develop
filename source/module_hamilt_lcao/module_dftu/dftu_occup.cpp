        ...
        for (int I = 0; I < GlobalC::ucell.atoms[T].na; I++) {
            ...
            for (int l = 0; l < GlobalC::ucell.atoms[T].nwl + 1; l++) {
                ...
                for (int n = 0; n < N; n++) {
                    if (GlobalV::NSPIN == 4) {
                        locale[iat][l][n][0]
                            = locale[iat][l][n][0] * beta
                              + locale_save[iat][l][n][0] * (1.0 - beta);
                    } else if (GlobalV::NSPIN == 1 || GlobalV::NSPIN == 2) {
                        locale[iat][l][n][0]
                            = locale[iat][l][n][0] * beta
                              + locale_save[iat][l][n][0] * (1.0 - beta);
                        locale[iat][l][n][1]
                            = locale[iat][l][n][1] * beta
                              + locale_save[iat][l][n][1] * (1.0 - beta);
                    }
                }
            }
        }
...
void DFTU::cal_occup_m_k(
    ...
    //=================Part 1======================
    // call SCALAPACK routine to calculate the product of the S and density
    // matrix
                ...
                for (int l = 0; l < NL; l++) {
                    ...
                    for (int n = 0; n < N; n++) {
                        ...
                        for (int m0 = 0; m0 < 2 * l + 1; m0++) {
                            for (int ipol0 = 0; ipol0 < GlobalV::NPOL;
                                 ipol0++) {
                                const int iwt0
                                    = this->iatlnmipol2iwt[iat][l][n][m0]
                                                          [ipol0];
                                const int mu
                                    = this->LM->ParaV->global2local_row(iwt0);
                                const int mu_prime
                                    = this->LM->ParaV->global2local_col(iwt0);

                                for (int m1 = 0; m1 < 2 * l + 1; m1++) {
                                    for (int ipol1 = 0; ipol1 < GlobalV::NPOL;
                                         ipol1++) {
                                        const int iwt1
                                            = this->iatlnmipol2iwt[iat][l][n]
                                                                  [m1][ipol1];
                                        const int nu
                                        ...
                                        const int irc
                                            = nu * this->LM->ParaV->nrow + mu;
                                        const int irc_prime
                                            = mu_prime * this->LM->ParaV->nrow
                                              + nu_prime;
        ...
        for (int ia = 0; ia < GlobalC::ucell.atoms[it].na; ia++) {
            ...
            for (int l = 0; l < NL; l++) {
                ...
                for (int n = 0; n < N; n++) {
                    ...
                    if (n != 0)
                        continue;
                        // set the local occupation mumber matrix of spin up and
                        // down zeros
                    ...
                    default:
                        std::cout << "Not supported NSPIN parameter"
                                  << std::endl;
                        exit(0);
                    }
                } // end n
...
void DFTU::cal_occup_m_gamma(const int iter,
    ...
    for (int is = 0; is < GlobalV::NSPIN; is++) {
        ...
        double* s_gamma_pointer
            = dynamic_cast<hamilt::HamiltLCAO<double, double>*>(p_ham)->getSk();
