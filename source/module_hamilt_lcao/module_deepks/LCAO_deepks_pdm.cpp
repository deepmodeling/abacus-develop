...
void LCAO_Deepks::read_projected_DM(bool read_pdm_file, bool is_equiv, const Numerical_Orbital& alpha)
{
    if (read_pdm_file && !this->init_pdm) // for DeePKS NSCF calculation
    {
        ...
        if (!ifs)
        {
            ModuleBase::WARNING_QUIT("LCAO_Deepks::cal_projected_DM",
                                     "Can not find the file pdm.dat . Please do DeePKS SCF calculation first.");
        }
        ...
        for (int I0 = 0; I0 < atom0->na; I0++)
        {
            ...
            for (int ad1 = 0; ad1 < GridD.getAdjacentNum() + 1; ++ad1)
            {
                ...
                for (int ad2 = 0; ad2 < GridD.getAdjacentNum() + 1; ad2++)
                {
                    ...
                    for (int is = 0; is < dm->get_DMR_vector().size(); is++)
                    {
                        auto tmp_matrix = dm->get_DMR_vector()[is]->find_matrix(ibt1,
                                                                                ibt2,
                                                                                (dR2 - dR1).x,
                                                                                (dR2 - dR1).y,
                                                                                (dR2 - dR1).z);
                                ...
                                for (int N0 = 0; N0 < orb.Alpha[0].getNchi(L0); ++N0)
                                {
                                    ...
                                    for (int m1 = 0; m1 < nm; ++m1)
                                    {
                                        for (int m2 = 0; m2 < nm; ++m2)
                                        {
                                            ...
                                            gdmx[iat][inl][m1 * nm + m2] += nlm2[1][ib + m2] * nlm1[ib + m1]
                                                                            * *dm_current; // dm(iw1_local,iw2_local);
                                            gdmy[iat][inl][m1 * nm + m2] += nlm2[2][ib + m2] * nlm1[ib + m1]
                                                                            * *dm_current; // dm(iw1_local,iw2_local);
                                            gdmz[iat][inl][m1 * nm + m2] += nlm2[3][ib + m2] * nlm1[ib + m1]
                                                                            * *dm_current; // dm(iw1_local,iw2_local);

                                            //(<d/dX chi_nu|alpha_m'>)<chi_mu|alpha_m>
                                            gdmx[iat][inl][m2 * nm + m1] += nlm2[1][ib + m2] * nlm1[ib + m1]
                                                                            * *dm_current; // dm(iw1_local,iw2_local);
                                            gdmy[iat][inl][m2 * nm + m1] += nlm2[2][ib + m2] * nlm1[ib + m1]
                                                                            * *dm_current; // dm(iw1_local,iw2_local);
                                            gdmz[iat][inl][m2 * nm + m1] += nlm2[3][ib + m2] * nlm1[ib + m1]
                                                                            * *dm_current; // dm(iw1_local,iw2_local);

                                            //(<chi_mu|d/dX alpha_m>)<chi_nu|alpha_m'> = -(<d/dX
                                            //chi_mu|alpha_m>)<chi_nu|alpha_m'>
                                            gdmx[ibt2][inl][m1 * nm + m2] -= nlm2[1][ib + m2] * nlm1[ib + m1]
                                                                             * *dm_current; // dm(iw1_local,iw2_local);
                                            gdmy[ibt2][inl][m1 * nm + m2] -= nlm2[2][ib + m2] * nlm1[ib + m1]
                                                                             * *dm_current; // dm(iw1_local,iw2_local);
                                            gdmz[ibt2][inl][m1 * nm + m2] -= nlm2[3][ib + m2] * nlm1[ib + m1]
                                                                             * *dm_current; // dm(iw1_local,iw2_local);

                                            //(<chi_nu|d/dX alpha_m'>)<chi_mu|alpha_m> = -(<d/dX
                                            //chi_nu|alpha_m'>)<chi_mu|alpha_m>
                                            gdmx[ibt2][inl][m2 * nm + m1] -= nlm2[1][ib + m2] * nlm1[ib + m1]
                                                                             * *dm_current; // dm(iw1_local,iw2_local);
                                            gdmy[ibt2][inl][m2 * nm + m1] -= nlm2[2][ib + m2] * nlm1[ib + m1]
                                                                             * *dm_current; // dm(iw1_local,iw2_local);
                                            gdmz[ibt2][inl][m2 * nm + m1] -= nlm2[3][ib + m2] * nlm1[ib + m1]
                                                                             * *dm_current; // dm(iw1_local,iw2_local);
                                            ...
                                            if (isstress)
                                            {
                                                ...
                                                for (int ipol = 0; ipol < 3; ipol++)
                                                {
                                                    ...
                                                    {
                                                        // gdm_epsl[mm][inl][m2*nm+m1] += ucell.lat0 * dm(iw1_local,
                                                        // iw2_local) * (nlm2[jpol+1][ib+m2] * nlm1[ib+m1] * r0[ipol]);
                                                        gdm_epsl[mm][inl][m2 * nm + m1]
                                                            += ucell.lat0 * *dm_current
                                                               * (nlm2[jpol + 1][ib + m2] * nlm1[ib + m1] * r0[ipol]);
                                                        mm++;
                                                    }
                                                }
                                            }
                                            ...
                                            for (int m2 = 0; m2 < nm; ++m2)
                                            {
                                                ...
                                                for (int ipol = 0; ipol < 3; ipol++)
                                                {
                                                    for (int jpol = ipol; jpol < 3; jpol++)
                                                    {
                                                        gdm_epsl[mm][inl][m2 * nm + m1]
                                                            += ucell.lat0 * *dm_current
                                                               * (nlm1[ib + m1] * nlm2[jpol + 1][ib + m2] * r1[ipol]);
                                                        mm++;
                                                    }
                                                }
                                            }
                                    ...
                                    for (int m1 = 0; m1 < 2 * L0 + 1; ++m1)
                                    {
                                        for (int m2 = 0; m2 < 2 * L0 + 1; ++m2)
                                        {
                                            ...
                                            gdmx[iat][inl][m1 * nm + m2]
                                                += nlm2[1][ib + m2] * nlm1[ib + m1] * *dm_current;
                                            gdmy[iat][inl][m1 * nm + m2]
                                                += nlm2[2][ib + m2] * nlm1[ib + m1] * *dm_current;
                                            gdmz[iat][inl][m1 * nm + m2]
                                                += nlm2[3][ib + m2] * nlm1[ib + m1] * *dm_current;
