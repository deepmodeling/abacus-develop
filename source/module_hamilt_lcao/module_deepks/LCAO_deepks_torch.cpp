...
// calculates v_delta_precalc[nks,NAt,NDscrpt] = gvdm * v_delta_pdm_shell;
// v_delta_pdm_shell[nks,Inl,nm*nm] = overlap * overlap;
    ...
    Grid_Driver &GridD)
{
    ...
    for(int inl = 0; inl < this->inlmax; inl++)
    {
        for(int mu = 0; mu < nlocal ; mu++)
        {
            ...
            {
                Parallel_Reduce::reduce_double_all(this->v_delta_pdm_shell[0][mu][nu][inl],(2 * this->lmaxd + 1) * (2 * this->lmaxd + 1));
            }
        }
    }
#endif    
...
#include <npy.hpp>
void LCAO_Deepks::check_v_delta_precalc(const int nlocal,const int nat)
{
    //forward
    std::vector<torch::jit::IValue> inputs;
    ...
    //cal gev
    std::vector<torch::Tensor> gev_shell;
    gev_shell.push_back(torch::ones_like(ec[0]));
    std::vector<torch::Tensor> gev_tensor;
    gev_tensor = torch::autograd::grad(ec, this->d_tensor, gev_shell, /*retain_grad=*/true, /*create_graph=*/false, /*allow_unused=*/true);

    torch::Tensor gev=torch::cat(gev_tensor, 0).reshape({ nat, this->des_per_atom });
    torch::Tensor v_delta_tensor=at::einsum("kxyap, ap->kxy", {this->v_delta_precalc_tensor, gev});

    std::ofstream ofs_gev("gev.dat");
    ofs_gev<<std::setprecision(10);
    for(int x=0;x<nat;x++)
    {
        for(int y=0;y<this->des_per_atom;y++)
        {
            ofs_gev<<gev.index({ x,y }).item().toDouble() * 2<<" ";
        }
        ofs_gev<<std::endl;
    }
    ofs_gev.close();
    std::ofstream ofs_v_delta("v_delta.dat");
    ofs_v_delta<<std::setprecision(10);
    for(int x=0;x<nlocal;x++)
    {
        for(int y=0;y<nlocal;y++)
        {
            ofs_v_delta<<v_delta_tensor.index({ 0,x,y }).item().toDouble() * 2<<" ";
        }
        ofs_v_delta<<std::endl;
    }
    ofs_v_delta.close();

    if(GlobalV::MY_RANK!=0) return;
    const long unsigned gevshape[] = {static_cast<unsigned long>(nat), static_cast<unsigned long>(this->des_per_atom)};
    vector<double> npy_gev;
    for (int ibt = 0;ibt < nat;++ibt)
    {
        for(int p=0;p<this->des_per_atom;++p)
        {
            npy_gev.push_back(gev.index({ ibt, p }).item().toDouble());
        }

    }
    npy::SaveArrayAsNumpy("gev.npy", false, 2, gevshape, npy_gev);
}
    ...
    for(int iat=0; iat< nat ; iat++)
    {
        for(int nl = 0; nl < nlmax; nl++)
        {
            for(int mu = 0; mu < nlocal ; mu++)
            {
                ...
                Parallel_Reduce::reduce_double_all(msg,mmax);
