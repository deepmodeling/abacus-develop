                         ...
                         const int print_matrix)
{
    ...
    ScalapackConnector::geadd('N', nlocal, nlocal, alpha, H_laststep.data<std::complex<double>>(), 1, 1, pv->desc, beta, Htmp.data<std::complex<double>>(), 1, 1, pv->desc);

    // Perform the operation Stmp = alpha * S_laststep + beta * Stmp
    ScalapackConnector::geadd('N', nlocal, nlocal, alpha, S_laststep.data<std::complex<double>>(), 1, 1, pv->desc, beta, Stmp.data<std::complex<double>>(), 1, 1, pv->desc);
