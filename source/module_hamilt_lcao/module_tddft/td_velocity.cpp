...
void TD_Velocity::output_cart_At(const std::string& out_dir) {
    if (GlobalV::MY_RANK == 0) {
        ...
        if (istep == 0) {
            ...
            ofs << std::left << std::setw(8) << "#istep" << std::setw(15)
                << "A_x" << std::setw(15) << "A_y" << std::setw(15) << "A_z"
                << std::endl;
        } else {
        ...
        for (int i = 0; i < 3; i++) {
            ofs << std::scientific << std::setprecision(4) << std::setw(15)
                << cart_At[i] / 2.0;
        }
        ofs << std::endl;
        ofs.close();
...
void TD_Velocity::cal_cart_At(const ModuleBase::Vector3<double>& a0,
    ...
    if (init_vecpot_file) {
        ...
        this->cart_At = a0 * At[0] / l_norm[0] + a1 * At[1] / l_norm[1]
                        + a2 * At[2] / l_norm[2];
    }
...
void TD_Velocity::read_cart_At(void) {
    ...
    if (!ifs) {
        ModuleBase::WARNING_QUIT("TD_Velocity::read_cart_At",
                                 "Cannot open Vector potential file!");
    }
    std::string line;
        ...
        for (int i = 0; i < 3; i++) {
            if (!(iss >> component)) {
                ModuleBase::WARNING_QUIT(
                    "TD_Velocity::read_cart_At",
                    "Error reading component " + std::to_string(i + 1)
                        + " for istep " + std::to_string(tmp) + "!");
            }
            At[i] = component;
        }
...
void TD_Velocity::initialize_current_term(
    ...
    for (int dir = 0; dir < 3; dir++) {
        if (this->current_term[dir] == nullptr)
            this->current_term[dir]
                = new hamilt::HContainer<std::complex<double>>(paraV);
    }
