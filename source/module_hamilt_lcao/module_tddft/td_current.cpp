...
#pragma omp parallel
{
    ...
    for (int iat0 = 0; iat0 < this->ucell->nat; iat0++)
    {
        ...
        std::vector<std::vector<std::unordered_map<int, std::vector<std::complex<double>>>>> nlm_tot(4);
        for(int dir=0;dir < 4;dir++)
        {
            nlm_tot[dir].resize(adjs.adj_num + 1);
        }
        ...
        for (int ad = 0; ad < adjs.adj_num + 1; ++ad)
        {
            ...
            for (int iw1l = 0; iw1l < all_indexes.size(); iw1l += npol)
            {
                ...
                for(int dir=0;dir<4;dir++)
                {
                    nlm_tot[dir][ad].insert({all_indexes[iw1l], nlm[dir]});
                }
            }
        }
// 2. calculate <psi_I|beta>D<beta|psi_{J,R}> for each pair of <IJR> atoms
        for (int ad1 = 0; ad1 < adjs.adj_num + 1; ++ad1)
        {
            ...
            for (int ad2 = 0; ad2 < adjs.adj_num + 1; ++ad2)
            {
                ...
                hamilt::BaseMatrix<std::complex<double>>* tmpx = this->current_term[0]->find_matrix(iat1, iat2, R_vector[0], R_vector[1], R_vector[2]);
                hamilt::BaseMatrix<std::complex<double>>* tmpy = this->current_term[1]->find_matrix(iat1, iat2, R_vector[0], R_vector[1], R_vector[2]);
                hamilt::BaseMatrix<std::complex<double>>* tmpz = this->current_term[2]->find_matrix(iat1, iat2, R_vector[0], R_vector[1], R_vector[2]);
    ...
    std::complex<double>* data_pointerz)
{
    ...
    for (int iw1l = 0; iw1l < row_indexes.size(); iw1l += npol)
    {
        ...
        const std::vector<std::complex<double>>& nlm1x = nlm1x_all.find(row_indexes[iw1l])->second;
        const std::vector<std::complex<double>>& nlm1y = nlm1y_all.find(row_indexes[iw1l])->second;
        const std::vector<std::complex<double>>& nlm1z = nlm1z_all.find(row_indexes[iw1l])->second;
        for (int iw2l = 0; iw2l < col_indexes.size(); iw2l += npol)
        {
            const std::vector<std::complex<double>>& nlm2 = nlm2_all.find(col_indexes[iw2l])->second;
            const std::vector<std::complex<double>>& nlm2x = nlm2x_all.find(col_indexes[iw2l])->second;
            const std::vector<std::complex<double>>& nlm2y = nlm2y_all.find(col_indexes[iw2l])->second;
            const std::vector<std::complex<double>>& nlm2z = nlm2z_all.find(col_indexes[iw2l])->second;
#ifdef __DEBUG
            assert(nlm1.size() == nlm2.size());
#endif
            for (int is = 0; is < npol * npol; ++is)
            {
                std::complex<double> nlm_tmpx = std::complex<double>{0,0};
                std::complex<double> nlm_tmpy = std::complex<double>{0,0};
                std::complex<double> nlm_tmpz = std::complex<double>{0,0};
                ...
                for (int no = 0; no < this->ucell->atoms[T0].ncpp.non_zero_count_soc[is]; no++)
                {
                    ...
                    nlm_tmpx += (nlm1x[p1] * std::conj(nlm2[p2])-nlm1[p1] * std::conj(nlm2x[p2])) * (*tmp_d);
                    nlm_tmpy += (nlm1y[p1] * std::conj(nlm2[p2])-nlm1[p1] * std::conj(nlm2y[p2])) * (*tmp_d);
                    nlm_tmpz += (nlm1z[p1] * std::conj(nlm2[p2])-nlm1[p1] * std::conj(nlm2z[p2])) * (*tmp_d);
                }
                data_pointerx[step_trace[is]] -= imag_unit*nlm_tmpx/2.0;
                data_pointery[step_trace[is]] -= imag_unit*nlm_tmpy/2.0;
                data_pointerz[step_trace[is]] -= imag_unit*nlm_tmpz/2.0;
            }
            data_pointerx += npol;
            data_pointery += npol;
            data_pointerz += npol;
        }
        data_pointerx += (npol - 1) * col_indexes.size();
        data_pointery += (npol - 1) * col_indexes.size();
        data_pointerz += (npol - 1) * col_indexes.size();
    }
}
...
#pragma omp parallel for
    ...
    for (int iat1 = 0; iat1 < this->ucell->nat; iat1++)
    {
        ...
        for (int ad = 0; ad < adjs.adj_num + 1; ++ad)
        {
            ...
            hamilt::BaseMatrix<std::complex<double>>* tmpx = this->current_term[0]->find_matrix(iat1, iat2, R_index2.x, R_index2.y, R_index2.z);
            hamilt::BaseMatrix<std::complex<double>>* tmpy = this->current_term[1]->find_matrix(iat1, iat2, R_index2.x, R_index2.y, R_index2.z);
            hamilt::BaseMatrix<std::complex<double>>* tmpz = this->current_term[2]->find_matrix(iat1, iat2, R_index2.x, R_index2.y, R_index2.z);
            if (tmpx != nullptr)
            {
                this->cal_grad_IJR(iat1, iat2, paraV, dtau, tmpx->get_pointer(), tmpy->get_pointer(), tmpz->get_pointer());
            }
            else
            {
                            ...
                            std::complex<double>* data_pointerz)
{
    ...
    for(int iw1l = 0; iw1l < row_indexes.size(); iw1l += npol)
    {
        ...
        for (int iw2l = 0; iw2l < col_indexes.size(); iw2l += npol)
        {
            ...
            {
                data_pointerx[ipol * step_trace]+= std::complex<double>(0,grad[0]);
                data_pointery[ipol * step_trace]+= std::complex<double>(0,grad[1]);
                data_pointerz[ipol * step_trace]+= std::complex<double>(0,grad[2]);
            }
            data_pointerx += npol;
            data_pointery += npol;
            data_pointerz += npol;
