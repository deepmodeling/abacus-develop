...
void snap_psibeta_half_tddft(
    ...
    // in our calculation, we always put orbital phi at the left side of
    // <phi|beta> because <phi|beta> = <beta|phi>
    ...
    auto Polynomial_Interpolation = [](const int& mesh_r,
                                       const double* psi_r,
                                       const double* r_radial,
        ...
        y = (x - x1) * (x - x2) * (x - x3) / (x0 - x1) / (x0 - x2) / (x0 - x3)
                * y0
    ...
    for (int nb = 0; nb < nproj; nb++) {
        ...
        double A_phase = A * R0;
        std::complex<double> exp_iAR0
            = std::exp(ModuleBase::IMAG_UNIT * A_phase);
        ...
        for (int ir = 0; ir < ridial_grid_num; ir++) {
            ...
            for (int ian = 0; ian < angular_grid_num; ian++) {
                ...
                double weights_angular
                    = ModuleBase::Integral::Lebedev_Laikov_grid110_w[ian];
                ModuleBase::Vector3<double> r_angular_tmp(x, y, z);

                ModuleBase::Vector3<double> r_coor
                    = r_ridial[ir] * r_angular_tmp;
                ...
                std::complex<double> exp_iAr
                    = std::exp(ModuleBase::IMAG_UNIT * phase);

                ModuleBase::Vector3<double> tmp_r_coor_r_commu = r_coor + R0;

                for (int m0 = 0; m0 < 2 * L0 + 1; m0++) {
                    double temp_interpolation_value
                        = Polynomial_Interpolation(mesh_r1,
                                                   psi_1,
                                                   radial1,
                                                   tmp_r_coor_norm);

                    result_angular[m0]
                        += exp_iAr * rly0[L0 * L0 + m0] * rly1[L1 * L1 + m1]
                           * temp_interpolation_value * weights_angular;

                    if (calc_r) {
                        result_angular_r_commu_x[m0]
                            += exp_iAr * tmp_r_coor_r_commu.x
                               * rly0[L0 * L0 + m0] * rly1[L1 * L1 + m1]
                               * temp_interpolation_value * weights_angular;

                        result_angular_r_commu_y[m0]
                            += exp_iAr * tmp_r_coor_r_commu.y
                               * rly0[L0 * L0 + m0] * rly1[L1 * L1 + m1]
                               * temp_interpolation_value * weights_angular;

                        result_angular_r_commu_z[m0]
                            += exp_iAr * tmp_r_coor_r_commu.z
                               * rly0[L0 * L0 + m0] * rly1[L1 * L1 + m1]
                               * temp_interpolation_value * weights_angular;
                    }
                }
            ...
            int index_tmp = index;
            double temp = Polynomial_Interpolation(mesh_r0,
                                                   beta_r,
                                                   radial0,
                                                   r_ridial[ir])
            ...
            if (!calc_r) {
                ...
                for (int m0 = 0; m0 < 2 * L0 + 1; m0++) {
                    nlm[0][index_tmp] += temp * result_angular[m0] * exp_iAR0;
                    nlm[1][index_tmp]
                        += temp * result_angular_r_commu_x[m0] * exp_iAR0;
                    nlm[2][index_tmp]
                        += temp * result_angular_r_commu_y[m0] * exp_iAR0;
                    nlm[3][index_tmp]
                        += temp * result_angular_r_commu_z[m0] * exp_iAR0;
                    index_tmp++;
                }
            }
        }
