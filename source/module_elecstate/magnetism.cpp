...
void Magnetism::compute_magnetization(const Charge* const chr, double* nelec_spin)
{
    if (GlobalV::NSPIN==2)
    {
        ...
        for (int ir=0; ir<this->get_rhopw_nrxx(); ir++)
        {
...
#endif
        this->tot_magnetization *= this->get_ucell_omega() / this->get_rhopw_nxyz();
        this->abs_magnetization *= this->get_ucell_omega() / this->get_rhopw_nxyz();
	...
	else if(GlobalV::NSPIN==4)
	{
		...
		Parallel_Reduce::reduce_double_pool( this->abs_magnetization );
#endif
		for(int i=0;i<3;i++)this->tot_magnetization_nc[i] *= this->get_ucell_omega() / this->get_rhopw_nxyz();
		this->abs_magnetization *= this->get_ucell_omega() / this->get_rhopw_nxyz();
