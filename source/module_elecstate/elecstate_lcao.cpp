...
void ElecStateLCAO<double>::psiToRho(const psi::Psi<double>& psi) {
    ...
    if (GlobalV::KS_SOLVER == "genelpa" || GlobalV::KS_SOLVER == "scalapack_gvx"
        ...
        elecstate::cal_dm_psi(this->DM->get_paraV_pointer(),
                              ...
                              *(this->DM));
        this->DM->cal_DMR();
        if (this->loc->out_dm) // keep interface for old Output_DM until new one
                               // is ready
        {
        ...
        for (int ik = 0; ik < psi.get_nk(); ++ik) {
            ...
            // old 2D-to-Grid conversion has been replaced by new Gint Refactor
            // 2023/09/25
            if (this->loc->out_dm) // keep interface for old Output_DM until new
                                   // one is ready
            {
...
void ElecStateLCAO<double>::dmToRho(std::vector<double*> pexsi_DM,
    ...
    // 2023/09/25
    if (this->loc
            ->out_dm) // keep interface for old Output_DM until new one is ready
    {
