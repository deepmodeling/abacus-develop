...
def read_esolver_type_from_running_log(src: str | Path | List[str]) -> str:
    ...
    lines = [re.match(r"#ENERGY SOLVER#\s+(\S+)", l) for l in raw]
...
def read_band_from_eig_occ(src: str | Path | List[str]) -> List[Dict[str, np.ndarray]]:
    ...
    ekb_leading_pat = (
        r"spin=(\d)\s+k-point=(\d+)/(\d+)\s+Cartesian=\s*"
        + r"(-?\d(\.\d+)?(e-\d+)?)\s+"
        + r"(-?\d(\.\d+)?(e-\d+)?)\s+"
        + r"(-?\d(\.\d+)?(e-\d+)?)\s+"
        + r"\(\d+\s+plane wave\)"
...
def read_traj_from_running_log(
    ...
    nframe = len(icoord)
    coord_raw = np.array(
        [l.split() for i in icoord for l in raw[i + 1 : i + 1 + natoms]]
    )
...
def read_abacus_out(
    ...
    # of the running log
    trajectory = (
        read_traj_from_running_log(abacus_lines)
        if fileobj.name != "running_md.log"
        else read_traj_from_md_dump(fileobj.parent / "MD_dump")
    )
...
class TestLatestIO(unittest.TestCase):
    ...
    def test_read_traj_from_running_log(self):
        traj = read_traj_from_running_log(
            self.testfiles / "lcao-symm0-nspin2-multik-cellrelax"
        )
        ...
        reference = [
            ...
            np.array(
                [
                    [4.17550947, 2.05376104, 2.05376104],
            ...
            np.array(
                [
                    [4.17667347, 2.04296382, 2.04296382],
        ...
        taud = np.array(
            [[0.0, 1.0, 1.0], [0.5, 0.5, 0.5], [0.25, 0.25, 0.25], [0.75, 0.75, 0.75]]
        )
        for t, c in zip(traj, reference):
            ...
            self.assertTrue(
                all(
                    e1 == e2
                    for e1, e2 in zip(t["elem"], np.array(["Ni1", "Ni2", "O", "O"]))
                )
            )
    ...
    def test_read_forces_from_running_log(self):
        forces = read_forces_from_running_log(
            self.testfiles / "lcao-symm0-nspin2-multik-relax"
        )
    ...
    def test_read_pw_symm0_nspin4_gamma_md(self):
        ...
        shutil.copy(
            self.testfiles / "nspin4-gamma-eigocc", self.testfiles / "eig_occ.txt"
        )
        shutil.copy(self.testfiles / "nspin4-gamma-mddump", self.testfiles / "MD_dump")
