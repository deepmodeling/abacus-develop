...
def load_pseudo(pseudo_dir: str) -> Dict[str, str]:
    ...
    return {
        re.match(UPFPAT_, f.name).group(1): f.name
...
def load_orbital(orbital_dir: str, efficiency: bool = True) -> Dict[str, str]:
    ...
    return {
        elem: sorted(basis, key=lambda b: (b[3], b[1], b[2]))[ib][0]
...
def file_safe_backup(fn: Path, suffix: str = "bak"):
    ...
    fbak = sorted(
        list(where.glob(f"{fn.name}.{suffix}.*")),
...
def _write_stru(job_dir, stru, fname="STRU"):
    ...
    with open(Path(job_dir) / fname, "w") as f:
        ...
        width = {
            key + "_w": max([len(str(s[key])) for s in stru["species"]])
            for key in ["symbol", "mass", "pp_file"]
        }
        for s in stru["species"]:
            f.write(
                "{symbol:<{symbol_w}}  {mass:>{mass_w}}  {pp_file:>{pp_file_w}}".format(
                    **s, **width
                )
            )
            if "pp_type" in s:
        ...
        for s in stru["species"]:
            ...
            for atom in s["atom"]:
                ...
                for key in ["m", "v"]:  # frozen atom / initial velocity
                    if key in atom:
                        f.write(f" {key}" + "".join(f" {x}" for x in atom[key]))
...
def write_stru(
    ...
    """
    from ase.units import Bohr as __BOHR__, Angstrom as __ANGSTROM__
    ...
    magmoms = np.array([stru[i].magmom for i in ind]).reshape(
        len(stru), -1
    )  # ncol in [1, 3]
    magmoms = [
        (
            {}
            if abs(np.linalg.norm(m)) <= 1e-10
...
def _write_ksampl_mp(data: Dict[str, Any], f: TextIOWrapper):
    ...
    f.write(
        f"{data['nk'][0]} {data['nk'][1]} {data['nk'][2]} "
...
def write_kpt(data: Dict[str, Any], fn: str) -> str:
    ...
    assert data["mode"] in mywriter, (
        f'Invalid KPT mode {data["mode"]}, must be one of ' f"{list(mywriter.keys())}"
    )
    with open(fn, "w") as f:
...
def read_input(fn: str) -> Dict[str, Any]:
    ...
    raw = [l for l in raw if l and not re.match(r"^(INPUT_PARAMETERS|#|//|!)", l)]
...
def _read_kline(raw: List[str]) -> Dict[str, Any]:
    ...
    """
    assert len(raw) >= 4, f"Invalid KPT file, expected at least 4 lines, got {len(raw)}"
    assert re.match(r"^\d+$", raw[1]), (
        f"Invalid KPT file, expected the second line to be an integer, " f"got {raw[1]}"
    )
...
def _read_ksampl_mp(raw: List[str]) -> Dict[str, Any]:
    ...
    """
    assert len(raw) == 4, f"Invalid KPT file, expected 4 lines, got {len(raw)}"
...
def read_kpt(fn: str) -> Dict[str, Any]:
    ...
    parser_switch = {
        "line": _read_kline,
        "gamma": _read_ksampl_mp,
        "mp": _read_ksampl_mp,
        "direct": _read_kpoint,
        "cartesian": _read_kpoint,
    }
    assert raw[0] == "K_POINTS", f'Invalid KPT file {fn}, first line must be "K_POINTS"'
...
class TestAbacusCalculatorIOUtil(unittest.TestCase):
    ...
    def test_stru_io(self):
        ...
        write_stru(
            ...
            pp_file={"Na": "Na.pz-bhs.UPF", "Cl": "Cl.pz-bhs.UPF"},
            orb_file={
    ...
    def test_write_stru_with_magmom(self):
        ...
        atoms = Atoms(
            symbols=["Co", "Cr", "Ni", "Co"], cell=np.eye(3), magmoms=[1, 2, 3, 1]
        )
        with tempfile.NamedTemporaryFile(mode="w") as f:
            write_stru(
                stru=atoms,
        ...
        self.assertEqual(stru["species"][0]["mag_each"], 0.0)
        self.assertEqual(
            stru["species"][0]["atom"][0]["mag"], ("Cartesian", [1.0, 0.0, 0.0])
        )
        self.assertEqual(
            stru["species"][0]["atom"][1]["mag"], ("Cartesian", [1.0, 0.0, 0.0])
        )
        # Cr
        self.assertEqual(stru["species"][1]["mag_each"], 0.0)
        self.assertEqual(
            stru["species"][1]["atom"][0]["mag"], ("Cartesian", [0.0, 2.0, 0.0])
        )
        # Ni
        self.assertEqual(stru["species"][2]["mag_each"], 0.0)
        self.assertEqual(
            stru["species"][2]["atom"][0]["mag"], ("Cartesian", [0.0, 0.0, 3.0])
        )
