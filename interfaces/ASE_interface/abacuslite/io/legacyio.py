...
from ase.calculators.singlepoint import SinglePointKPoint, SinglePointDFTCalculator
...
def parse_kpoints_table(
    ...
    assert with_ibz or all(
        re.match(WOPAT_, l) for l in raw
...
def read_kpoints_from_running_log(
...
def read_esolver_type_from_running_log(src: str | Path | List[str]) -> str:
    """
    ...
    lines = [re.match(r"The esolver type has been set to : (\S+)", l) for l in raw]
    eslvtyp = [m.group(1) for m in lines if m is not None]
    assert len(set(eslvtyp)) == 1, f"Inconsistent esolver type: {set(eslvtyp)}"
    return eslvtyp[0]


def read_band_from_running_log(
    src: str | Path | List[str],
    ...
    assert len(set(nspin)) == 1, f"Inconsistent number of spins: {set(nspin)}"
    nspin = nspin[0]
    # r'NBANDS\s+=\s+(\d+)'
    nbnd = [re.search(r"NBANDS\s+=\s+(\d+)", l) for l in raw]
    nbnd = [int(n.group(1)) for n in nbnd if n]
    assert len(set(nbnd)) == 1, f"Inconsistent number of bands: {set(nbnd)}"
    nbnd = nbnd[0]
    # r'nkstot\snow\s=\s(\d+)'
    ...
    nframe = len(iekb) // (
        masknspin(nspin) * nk
    )  # number of frames, for MD or relax tasks
    ...
    ekbpat += r"(\d+(\.\d+)?(e[+-]\d+)?)"
    assert all(
        re.match(ekbpat, l) for l in ekb_raw
...
def read_traj_from_running_log(
    src: str | Path | List[str],
    ...
    assert len(set(natoms)) == 1, f"Inconsistent number of atoms: {set(natoms)}"
    natoms = natoms[0]
    ...
    a0 = [
        re.search(
            r"lattice constant \(Angstrom\)\s*=\s*(-?\d+(\.\d+)?)", l, re.IGNORECASE
        )
        for l in raw
    ]
    ...
    cell = [
        np.array([list(map(float, l.split())) for l in c]) * a0 for c in cell_raw
    ]  # convert to Angstrom
    assert all(
        c.shape == (3, 3) for c in cell
    ...
    coords = np.array(
        [[float(m.group(2)), float(m.group(4)), float(m.group(6))] for m in m_tau]
    ).reshape(-1, natoms, 3)
    ...
    assert len(elem) == natoms * nframe, (
        f"Unexpected number of elements: {len(elem)}, " f"expected {natoms * nframe}"
    )
    ...
    assert len(cell) == nframe, (
        f"Unexpected number of cells: {len(cell)}, " f"expected {nframe}"
    )
    return [
        {
...
def read_traj_from_md_dump(src: str | Path | List[str]) -> List[Dict[str, np.ndarray]]:
    """
    ...
    cell_raw = [raw[i + 1 : i + 1 + 3] for i in icell]
    cell = [np.array([list(map(float, l.split())) for l in c]) for c in cell_raw]
    assert all(
        c.shape == (3, 3) for c in cell
    ), f"Unexpected shape of cell: {[c.shape for c in cell]}"
    nframe = len(cell)
    ...
    assert len(elem) == nframe, (
        f"Unexpected number of elements: {len(elem)}, " f"expected {nframe}"
    )
...
def read_forces_from_running_log(src: str | Path | List[str]) -> List[np.ndarray]:
    """"""
    if isinstance(src, (str, Path)):
        with open(src) as f:
            raw = f.readlines()
...
def read_stress_from_running_log(src: str | Path | List[str]) -> List[np.ndarray]:
    """"""
    if isinstance(src, (str, Path)):
        with open(src) as f:
            raw = f.readlines()
    ...
    while istart < len(raw):
        ...
        stress = np.array([list(map(float, l.split())) for l in stress_raw]).reshape(
            3, 3
        )
        # unit: kbar -> GPa
...
def read_energies_from_running_log(
    src: str | Path | List[str],
) -> List[Dict[str, float]]:
    """"""
    if isinstance(src, (str, Path)):
...
def read_magmom_from_running_log(src: str | Path | List[str]) -> List[np.ndarray]:
    """
    ...
    while istart < len(raw):
        ...
        for i, l in enumerate(raw[istart:]):
            if re.match(
                r"\s*Total\sMagnetism\s\(uB\)(\s+x\s+y\s+z)?\s*$", l, re.IGNORECASE
            ):
                ith = i
                break
        ...
        if len(res) == 1:  # colinear case
            ...
            magmom.append(
                np.array(
                    [list(map(float, (mx, my, mz))) for mx, my, mz in zip(mx, my, mz)]
                )
            )
        # update the istart
        istart += ith + itb + jtb + 1
...
def read_iter_header_from_running_log(
    src: str | Path | List[str],
) -> List[Tuple[int, int]]:
    """
...
def find_final_info_with_iter_header(
    info: List[Any], headers: List[Tuple[int, int]]
) -> List[Any]:
...
def read_abacus_out(
    ...
    sort_atoms_with: Optional[List[int]] = None,
) -> Atoms | List[Atoms]:
    ...
    for frame, estat, mag, frs, strs, ener in zip(
        ...
        atoms = Atoms(
            symbols=np.array(frame["elem"])[ind].tolist(),
        ...
        calc.kpts = []
        for ispn, (ekb, occ) in enumerate(
            zip(estat["e"], estat["occ"])
        ):  # loop over the spin
            calc.kpts += [
                SinglePointKPoint(
                    weight=wk[ik], s=ispn, k=kvecd[ik], eps_n=ekb[ik, :], f_n=occ[ik, :]
                )
...
class TestLegacyIO(unittest.TestCase):
    ...
    def test_read_traj_from_running_log(self):
        ...
        self.assertTrue(
            all(
                "coordinate" in d and "cell" in d and "elem" in d and "coords" in d
                for d in data
            )
        )
        for d in data:
    ...
    def test_read_energies_from_running_log(self):
        ...
        self.assertTrue(
            all(isinstance(e, dict) for e in energies_ry)
        )  # dict of energies
    ...
    def test_read_magmom_from_running_log(self):
        ...
        for magmom in magmoms:
            self.assertTrue(
                np.allclose(
                    magmom, np.array([[0.0, 0.0, 3.62032142], [0.0, 0.0, 3.62032142]])
                )
            )
