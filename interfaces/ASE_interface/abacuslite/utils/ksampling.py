...
def convert_kspacing_to_kpts(
    cell: np.ndarray, kspacing: float | Tuple[float, float, float]
) -> Tuple[int, int, int]:
    """
    ...
    kspacing = (
        (kspacing, kspacing, kspacing) if isinstance(kspacing, float) else kspacing
    )
...
def get_kpath(
    ...
    assert (
        len(kvec) == len(kdist) == len(kname)
    ), f"{len(kvec)}, {len(kdist)}, {len(kname)}"
    return kvec, kdist, kname


def interpolate_kpath(
    knodes: np.ndarray, knames: List[str], n_interpl: int | List[int] = 10
) -> Tuple[np.ndarray, np.ndarray, List[str]]:
    """
    ...
    kname = [[kn] + [""] * (n_interpl[i] - 1) for i, kn in enumerate(knames[:-1])] + [
        [knames[-1]]
    ]
    kname = [n for kn in kname for n in kn]
...
def make_kstring(klabels: List[str], is_brkpt: List[bool]) -> str:
    """
...
def make_klines(
    kpts, is_brkpt, n_interpl, klabels
    ...
    return [
        fspawnk(c, 1 if is_brkpt[i] else n_interpl, klbl)
