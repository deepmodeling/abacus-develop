...
def _parse_coord_line(line):
    ...
    if len(fields) < 3:
        raise ValueError(
            f"Invalid coordinate line: '{line}'. Expected at least 3 coordinates."
        )
...
def _atomic_positions_gen(lines):
    ...
    while current_pos < len(lines):
        ...
        if pos_end > len(lines):
            raise ValueError(
                f"Not enough coordinate lines for {symbol}. Expected {natom}, found {len(lines) - pos_start}"
            )
        ...
        yield {"symbol": symbol, "mag_each": mag_each, "natom": natom, "atom": atoms}
...
def read_stru(fpath: str) -> Dict[str, Any]:
    ...
    # ============ ATOMIC_SPECIES ============
    stru["species"] = [
        _atomic_species_from_file(line) for line in blocks["ATOMIC_SPECIES"]
    ]
        ...
        for s in stru["species"]:
            ...
            f.write(
                "{symbol:<{symbol_w}}  {mass:>{mass_w}}  {pp_file:>{pp_file_w}}".format(
                    symbol=s["symbol"], mass=s["mass"], pp_file=pp_path, **width
                )
            )
