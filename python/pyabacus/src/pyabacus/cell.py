...
class Cell:
    ...
    def _set_auto_parameters(self) -> None:
        ...
        if self._lattice is not None:
            ...
            self._mesh = [
                max(1, int(np.ceil(length / self._precision))) for length in lengths
            ]
        else:
    ...
    def _load_stru(self, stru_file: str) -> None:
        ...
        for species in stru_dict["species"]:
            ...
            if "pp_type" in species:
                self.pseudo_potentials[species["symbol"]]["pp_type"] = species[
                    "pp_type"
                ]
    ...
    def _save_stru(self, file_path: str) -> None:
        ...
        stru_dict = {
            "lat": {"const": self.lattice_constant, "vec": self.lattice.tolist()},
