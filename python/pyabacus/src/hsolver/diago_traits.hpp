...
class RawPointerStorage
{
  public:
    ...
    RawPointerStorage&
        operator= (RawPointerStorage&& other) noexcept
    {
        if (this != &other)
            {
                cleanup ();
...
class TensorStorage
{
  public:
    ...
    TensorStorage&
        operator= (TensorStorage&& other) noexcept
    {
        if (this != &other)
            {
                cleanup ();
    ...
    void
        set_preconditioner (py::array_t<double> prec_in)
    {
...
template <typename T>
auto
    make_hpsi_func_fstyle (std::function<py::array_t<T> (py::array_t<T>)> mm_op)
{
...
template <typename T>
auto
    make_hpsi_func_tensor (std::function<py::array_t<T> (py::array_t<T>)> mm_op)
{
...
template <typename Traits>
auto
    make_spsi_func_tensor_identity ()
{
    using T = typename Traits::T;
    using syncmem_op = typename Traits::syncmem_op;
