...
class PyDiagoCG
{
  public:
    ...
    void
        set_psi (py::array_t<std::complex<double>> psi_in)
    {
    ...
    void
        set_prec (py::array_t<double> prec_in)
    {
        py::buffer_info prec_buf = prec_in.request ();
        double* prec_ptr = static_cast<double*> (prec_buf.ptr);

        prec
            = new ct::TensorMap (prec_ptr, ct::DataType::DT_DOUBLE, ct::DeviceType::CpuDevice, ct::TensorShape ({dim}));
    }

    void
        diag (std::function<py::array_t<std::complex<double>> (py::array_t<std::complex<double>>)> mm_op,
              int diag_ndim,
              double tol,
