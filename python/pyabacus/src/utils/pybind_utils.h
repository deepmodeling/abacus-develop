...
template <typename T>
void
    check_1d_array (const py::array_t<T>& arr, const std::string& name = "array")
{
    py::buffer_info info = arr.request ();
    if (info.ndim != 1)
        {
            throw std::runtime_error (name + " must be 1-dimensional, got " + std::to_string (info.ndim)
                                      + " dimensions");
        }
}
...
template <typename T>
void
    check_2d_array (const py::array_t<T>& arr, const std::string& name = "array")
{
    py::buffer_info info = arr.request ();
    if (info.ndim != 2)
        {
            throw std::runtime_error (name + " must be 2-dimensional, got " + std::to_string (info.ndim)
                                      + " dimensions");
        }
}
...
template <typename T>
void
    check_array_size (const py::array_t<T>& arr, size_t expected_size, const std::string& name = "array")
{
    if (static_cast<size_t> (arr.size ()) != expected_size)
        {
            throw std::runtime_error (name + " must have size " + std::to_string (expected_size) + ", got "
...
template <typename T>
void
    check_array_shape (const py::array_t<T>& arr,
...
void
    check_same_size (const py::array_t<T1>& arr1,
                     ...
                     const std::string& name2 = "array2")
{
    if (arr1.size () != arr2.size ())
        {
            throw std::runtime_error (name1 + " and " + name2 + " must have the same size");
        }
}
...
template <typename T>
T*
    get_array_ptr (py::array_t<T>& arr)
{
    py::buffer_info info = arr.request ();
...
template <typename T>
const T*
    get_array_ptr (const py::array_t<T>& arr)
{
    py::buffer_info info = arr.request ();
...
template <typename T>
T*
    get_1d_array_ptr (py::array_t<T>& arr, const std::string& name = "array")
{
...
template <typename T>
const T*
    get_1d_array_ptr (const py::array_t<T>& arr, const std::string& name = "array")
{
    check_1d_array (arr, name);
    return get_array_ptr (arr);
}
...
template <typename T>
py::array_t<T>
    numpy_from_vector_copy (const std::vector<T>& vec)
{
...
template <typename T>
py::array_t<T>
    numpy_from_ptr_copy (const T* ptr, size_t size)
{
...
template <typename T>
py::array_t<T>
    numpy_from_ptr_copy_2d (const T* ptr, size_t nrow, size_t ncol)
{
...
template <typename T>
py::array_t<T>
    numpy_from_ptr_copy_nd (const T* ptr, const std::vector<ssize_t>& shape)
{
    py::array_t<T> result (shape);
    size_t total_size = 1;
    for (auto dim: shape)
...
py::array_t<T>
    numpy_view_with_keepalive (T* ptr, const std::vector<ssize_t>& shape, std::shared_ptr<Owner> owner_ptr)
{
    ...
    py::capsule free_when_done (owner_ptr.get (),
                                [] (void*)
                                    {
...
template <typename T>
py::array_t<T>
    numpy_view_1d (T* ptr, size_t size)
{
    return py::array_t<T> ({static_cast<ssize_t> (size)}, {sizeof (T)}, ptr, py::none ());
}
...
template <typename T>
py::array_t<T>
    numpy_view_2d (T* ptr, size_t nrow, size_t ncol)
{
    return py::array_t<T> ({static_cast<ssize_t> (nrow), static_cast<ssize_t> (ncol)},
...
Ret
    call_python_safe (const py::function& func, Args&&... args)
{
    try
        {
...
void
    call_python_safe_void (const py::function& func, Args&&... args)
{
    try
        {
...
struct BufferHelper
{
    ...
    explicit BufferHelper (py::array_t<T>& arr)
        : info (arr.request ()), ptr (static_cast<T*> (info.ptr)), size (static_cast<size_t> (arr.size ()))
    {
    }
     ...
     */
    void
        require_1d (const std::string& name = "array") const
    {
        if (info.ndim != 1)
            {
     ...
     */
    void
        require_2d (const std::string& name = "array") const
    {
        if (info.ndim != 2)
            {
     ...
     */
    void
        require_size (size_t expected, const std::string& name = "array") const
    {
        if (size != expected)
            {
                throw std::runtime_error (name + " must have size " + std::to_string (expected) + ", got "
                                          + std::to_string (size));
            }
    }
     ...
     */
    void
        require_shape (const std::vector<ssize_t>& expected_shape, const std::string& name = "array") const
    {
        if (info.ndim != static_cast<ssize_t> (expected_shape.size ()))
            {
                throw std::runtime_error (name + " has wrong number of dimensions");
