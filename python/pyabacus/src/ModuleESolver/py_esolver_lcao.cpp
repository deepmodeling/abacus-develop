...
void
    PyChargeAccessor::set_from_charge (const Charge* chr)
{
    if (chr == nullptr)
        {
            chr_ptr_ = nullptr;
...
void
    PyChargeAccessor::set_data (const double* rho_ptr, int nspin, int nrxx)
{
...
py::array_t<double>
    PyChargeAccessor::get_rho () const
{
    if (!is_valid ())
        {
...
py::array_t<std::complex<double>>
    PyChargeAccessor::get_rhog () const
{
...
py::array_t<double>
    PyChargeAccessor::get_rho_core () const
{
...
void
    PyEnergyAccessor::set_from_fenergy (const elecstate::fenergy* f_en)
{
    if (f_en == nullptr)
        {
            etot_ = 0.0;
...
void
    PyEnergyAccessor::set_energies (double etot,
                                    double eband,
                                    double hartree,
                                    double etxc,
                                    double ewald,
...
py::dict
    PyEnergyAccessor::get_all_energies () const
{
    py::dict result;
    result["etot"] = etot_;
    result["eband"] = eband_;
...
void
    PyHamiltonianAccessor<TK, TR>::set_from_hamilt (hamilt::HamiltLCAO<TK, TR>* hamilt_lcao,
                                                    int nks,
...
template <typename TK, typename TR>
void
    PyHamiltonianAccessor<TK, TR>::set_dimensions (int nbasis, int nks)
{
    nbasis_ = nbasis;
    nks_ = nks;
    hk_ptrs_.resize (nks, nullptr);
...
template <typename TK, typename TR>
void
    PyHamiltonianAccessor<TK, TR>::set_Hk_data (int ik, const TK* data, int nrow, int ncol)
{
    if (ik >= 0 && ik < nks_)
        {
            hk_ptrs_[ik] = data;
...
template <typename TK, typename TR>
void
    PyHamiltonianAccessor<TK, TR>::set_Sk_data (int ik, const TK* data, int nrow, int ncol)
{
    if (ik >= 0 && ik < nks_)
        {
            sk_ptrs_[ik] = data;
...
template <typename TK, typename TR>
py::array_t<TK>
    PyHamiltonianAccessor<TK, TR>::get_Hk (int ik) const
{
...
template <typename TK, typename TR>
py::array_t<TK>
    PyHamiltonianAccessor<TK, TR>::get_Sk (int ik) const
{
    if (!is_valid () || ik < 0 || ik >= nks_)
        {
...
template <typename TK, typename TR>
py::dict
    PyHamiltonianAccessor<TK, TR>::get_HR () const
{
...
template <typename TK, typename TR>
py::dict
    PyHamiltonianAccessor<TK, TR>::get_SR () const
{
    // Placeholder: will be implemented when full ABACUS integration is available
    py::dict result;
    return result;
...
template <typename TK, typename TR>
void
    PyDensityMatrixAccessor<TK, TR>::set_from_dm (elecstate::DensityMatrix<TK, TR>* dm)
{
    dm_ptr_ = dm;
...
template <typename TK, typename TR>
void
    PyDensityMatrixAccessor<TK, TR>::set_dimensions (int nks, int nrow, int ncol)
{
    nks_ = nks;
    nrow_ = nrow;
    ncol_ = ncol;
...
template <typename TK, typename TR>
void
    PyDensityMatrixAccessor<TK, TR>::set_DMK_data (int ik, const TK* data)
{
    if (ik >= 0 && ik < nks_)
        {
            dmk_ptrs_[ik] = data;
...
template <typename TK, typename TR>
py::array_t<TK>
    PyDensityMatrixAccessor<TK, TR>::get_DMK (int ik) const
{
    if (!is_valid () || ik < 0 || ik >= nks_)
        {
...
template <typename TK, typename TR>
std::vector<py::array_t<TK>>
    PyDensityMatrixAccessor<TK, TR>::get_DMK_all () const
{
...
template <typename TK, typename TR>
py::dict
    PyDensityMatrixAccessor<TK, TR>::get_DMR () const
{
    // Placeholder: will be implemented when full ABACUS integration is available
    py::dict result;
    return result;
...
template <typename TK, typename TR>
void
    PyESolverLCAO<TK, TR>::initialize (const std::string& input_dir)
{
...
template <typename TK, typename TR>
void
    PyESolverLCAO<TK, TR>::before_all_runners ()
{
    if (!initialized_)
        {
...
template <typename TK, typename TR>
void
    PyESolverLCAO<TK, TR>::before_scf (int istep)
{
    if (!initialized_)
        {
            throw std::runtime_error ("ESolver not initialized. Call initialize() first.");
...
template <typename TK, typename TR>
void
    PyESolverLCAO<TK, TR>::run_scf_iteration (int iter)
{
    if (!scf_started_)
        {
...
template <typename TK, typename TR>
void
    PyESolverLCAO<TK, TR>::run_scf (int max_iter)
{
    before_scf (istep_);
...
template <typename TK, typename TR>
void
    PyESolverLCAO<TK, TR>::after_scf (int istep)
{
    if (!scf_started_)
        {
            throw std::runtime_error ("SCF not started. Call before_scf() first.");
...
template <typename TK, typename TR>
PyChargeAccessor
    PyESolverLCAO<TK, TR>::get_charge () const
{
...
template <typename TK, typename TR>
PyEnergyAccessor
    PyESolverLCAO<TK, TR>::get_energy () const
{
...
template <typename TK, typename TR>
PyHamiltonianAccessor<TK, TR>
    PyESolverLCAO<TK, TR>::get_hamiltonian () const
{
...
template <typename TK, typename TR>
PyDensityMatrixAccessor<TK, TR>
    PyESolverLCAO<TK, TR>::get_density_matrix () const
{
...
template <typename TK, typename TR>
py::array_t<TK>
    PyESolverLCAO<TK, TR>::get_psi (int ik) const
{
...
template <typename TK, typename TR>
py::array_t<double>
    PyESolverLCAO<TK, TR>::get_eigenvalues (int ik) const
{
...
template <typename TK, typename TR>
py::array_t<double>
    PyESolverLCAO<TK, TR>::get_occupations (int ik) const
{
...
template <typename TK, typename TR>
int
    PyESolverLCAO<TK, TR>::get_nks () const
{
...
template <typename TK, typename TR>
py::array_t<double>
    PyESolverLCAO<TK, TR>::get_kvec_d (int ik) const
{
...
template <typename TK, typename TR>
py::array_t<double>
    PyESolverLCAO<TK, TR>::get_wk () const
{
...
template <typename TK, typename TR>
int
    PyESolverLCAO<TK, TR>::get_nbasis () const
{
...
template <typename TK, typename TR>
int
    PyESolverLCAO<TK, TR>::get_nbands () const
{
...
template <typename TK, typename TR>
int
    PyESolverLCAO<TK, TR>::get_nspin () const
{
...
template <typename TK, typename TR>
int
    PyESolverLCAO<TK, TR>::get_nat () const
{
...
void
    bind_charge_accessor (py::module& m)
{
    py::class_<py_esolver::PyChargeAccessor> (m,
                                              "ChargeAccessor",
                                              R"pbdoc(
        ...
        .def (py::init<> ())
        .def ("get_rho",
              &py_esolver::PyChargeAccessor::get_rho,
              R"pbdoc(
        ...
        .def ("get_rhog",
              &py_esolver::PyChargeAccessor::get_rhog,
              R"pbdoc(
        ...
        .def ("get_rho_core",
              &py_esolver::PyChargeAccessor::get_rho_core,
              R"pbdoc(
        ...
        .def_property_readonly ("nspin", &py_esolver::PyChargeAccessor::get_nspin, "Number of spin channels")
        .def_property_readonly ("nrxx", &py_esolver::PyChargeAccessor::get_nrxx, "Number of real-space grid points")
        .def_property_readonly ("ngmc",
                                &py_esolver::PyChargeAccessor::get_ngmc,
                                "Number of G-vectors for charge density")
        .def ("is_valid", &py_esolver::PyChargeAccessor::is_valid, "Check if charge data is available");
}

void
    bind_energy_accessor (py::module& m)
{
    py::class_<py_esolver::PyEnergyAccessor> (m,
                                              "EnergyAccessor",
                                              R"pbdoc(
        ...
        .def (py::init<> ())
        .def_property_readonly ("etot", &py_esolver::PyEnergyAccessor::get_etot, "Total energy (Ry)")
        .def_property_readonly ("eband", &py_esolver::PyEnergyAccessor::get_eband, "Band energy (Ry)")
        .def_property_readonly ("hartree_energy",
                                &py_esolver::PyEnergyAccessor::get_hartree_energy,
                                "Hartree energy (Ry)")
        .def_property_readonly ("etxc", &py_esolver::PyEnergyAccessor::get_etxc, "Exchange-correlation energy (Ry)")
        .def_property_readonly ("ewald_energy", &py_esolver::PyEnergyAccessor::get_ewald_energy, "Ewald energy (Ry)")
        .def_property_readonly ("demet", &py_esolver::PyEnergyAccessor::get_demet, "-TS term for metals (Ry)")
        .def_property_readonly ("exx", &py_esolver::PyEnergyAccessor::get_exx, "Exact exchange energy (Ry)")
        .def_property_readonly ("evdw", &py_esolver::PyEnergyAccessor::get_evdw, "van der Waals energy (Ry)")
        .def ("get_all_energies", &py_esolver::PyEnergyAccessor::get_all_energies, "Get all energies as a dictionary");
}

template <typename TK>
void
    bind_hamiltonian_accessor (py::module& m, const std::string& suffix)
{
    using HamiltAccessor = py_esolver::PyHamiltonianAccessor<TK>;

    std::string class_name = "HamiltonianAccessor" + suffix;

    py::class_<HamiltAccessor> (m,
                                class_name.c_str (),
                                R"pbdoc(
        ...
        .def (py::init<> ())
        .def_property_readonly ("nbasis", &HamiltAccessor::get_nbasis, "Number of basis functions")
        .def_property_readonly ("nks", &HamiltAccessor::get_nks, "Number of k-points")
        .def ("get_Hk",
              &HamiltAccessor::get_Hk,
              R"pbdoc(
        ...
        .def ("get_Sk",
            ...
            numpy.ndarray
              ...
              "ik"_a)
        .def ("get_HR", &HamiltAccessor::get_HR, "Get H(R) in sparse format")
        .def ("get_SR", &HamiltAccessor::get_SR, "Get S(R) in sparse format")
        .def ("is_valid", &HamiltAccessor::is_valid, "Check if Hamiltonian data is available");
}

template <typename TK>
void
    bind_density_matrix_accessor (py::module& m, const std::string& suffix)
{
    using DMAccessor = py_esolver::PyDensityMatrixAccessor<TK>;

    std::string class_name = "DensityMatrixAccessor" + suffix;

    py::class_<DMAccessor> (m,
                            class_name.c_str (),
                            R"pbdoc(
        ...
        .def (py::init<> ())
        .def_property_readonly ("nks", &DMAccessor::get_nks, "Number of k-points")
        .def_property_readonly ("nrow", &DMAccessor::get_nrow, "Number of rows in density matrix")
        .def_property_readonly ("ncol", &DMAccessor::get_ncol, "Number of columns in density matrix")
        .def ("get_DMK",
              &DMAccessor::get_DMK,
              R"pbdoc(
            ...
            numpy.ndarray
              ...
              "ik"_a)
        .def ("get_DMK_all", &DMAccessor::get_DMK_all, "Get all DM(k) matrices as a list")
        .def ("get_DMR", &DMAccessor::get_DMR, "Get DM(R) in sparse format")
        .def ("is_valid", &DMAccessor::is_valid, "Check if density matrix data is available");
}

template <typename TK, typename TR>
void
    bind_esolver_lcao (py::module& m, const std::string& suffix)
{
    ...
    py::class_<ESolver> (m,
        ...
        .def ("initialize",
            ...
            input_dir : str
              ...
              "input_dir"_a)
        .def ("before_all_runners", &ESolver::before_all_runners, "Initialize calculation environment")
        ...
        .def ("before_scf",
            ...
            istep : int, optional
              ...
              "istep"_a = 0)
        .def ("run_scf_iteration",
              &ESolver::run_scf_iteration,
              R"pbdoc(
        ...
        // Status
        .def ("is_converged", &ESolver::is_converged, "Check if SCF is converged")
        .def_property_readonly ("niter", &ESolver::get_niter, "Current iteration number")
        .def_property_readonly ("drho", &ESolver::get_drho, "Charge density difference")
        .def_property_readonly ("istep", &ESolver::get_istep, "Current ion step")

        // Data Accessors
        .def ("get_charge", &ESolver::get_charge, "Get charge density accessor")
        .def ("get_energy", &ESolver::get_energy, "Get energy accessor")
        .def ("get_hamiltonian", &ESolver::get_hamiltonian, "Get Hamiltonian accessor")
        .def ("get_density_matrix", &ESolver::get_density_matrix, "Get density matrix accessor")

        // Wave functions
        .def ("get_psi", &ESolver::get_psi, "Get wave function coefficients for k-point ik", "ik"_a)
        .def ("get_eigenvalues", &ESolver::get_eigenvalues, "Get eigenvalues for k-point ik", "ik"_a)
        .def ("get_occupations", &ESolver::get_occupations, "Get occupation numbers for k-point ik", "ik"_a)

        // K-points
        .def_property_readonly ("nks", &ESolver::get_nks, "Number of k-points")
        .def ("get_kvec_d", &ESolver::get_kvec_d, "Get k-vector in direct coordinates", "ik"_a)
        .def ("get_wk", &ESolver::get_wk, "Get k-point weights")

        // System info
        .def_property_readonly ("nbasis", &ESolver::get_nbasis, "Number of basis functions")
        .def_property_readonly ("nbands", &ESolver::get_nbands, "Number of bands")
        .def_property_readonly ("nspin", &ESolver::get_nspin, "Number of spin channels")
        .def_property_readonly ("nat", &ESolver::get_nat, "Number of atoms");
}
