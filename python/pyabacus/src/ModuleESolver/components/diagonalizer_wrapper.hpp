...
class DiagonalizerWrapper : public IDiagonalizer<TK>
{
  public:
                     ...
                     const py::array_t<TK>& psi_init) override
    {
        ...
        auto hpsi_func
            = [&Hk, this] (const py::array_t<TK>& psi) -> py::array_t<TK> { return matrix_vector_multiply (Hk, psi); };

        auto spsi_func
            = [&Sk, this] (const py::array_t<TK>& psi) -> py::array_t<TK> { return matrix_vector_multiply (Sk, psi); };
    ...
    void
        set_config (const DiagConfig& config) override
    {
        config_ = config;
    }
    ...
    void
        set_tolerance (double tol) override
    {
        config_.tolerance = tol;
    }

    void
        set_max_iterations (int max_iter) override
    {
    ...
    int
        get_nbands () const override
    {
        return nbands_;
    }

    void
        set_nbands (int nbands) override
    {
        nbands_ = nbands;
    }

    void
        set_nbasis (int nbasis)
    {
        nbasis_ = nbasis;
    }
  ...
  private:
    ...
    DiagResult<TK>
        diagonalize_davidson (std::function<py::array_t<TK> (const py::array_t<TK>&)> hpsi_func,
                              const py::array_t<TK>& psi_init,
    ...
    DiagResult<TK>
        diagonalize_dav_subspace (std::function<py::array_t<TK> (const py::array_t<TK>&)> hpsi_func,
                                  const py::array_t<TK>& psi_init,
                                  const py::array_t<double>& precond)
    {
        DiagResult<TK> result;
    ...
    // CG diagonalization
    DiagResult<TK>
        diagonalize_cg (std::function<py::array_t<TK> (const py::array_t<TK>&)> hpsi_func,
                        const py::array_t<TK>& psi_init,
                        const py::array_t<double>& precond)
    {
        DiagResult<TK> result;
    ...
    py::array_t<TK>
        matrix_vector_multiply (const py::array_t<TK>& matrix, const py::array_t<TK>& vec)
    {
    ...
    py::array_t<double>
        compute_preconditioner (const py::array_t<TK>& Hk)
    {
        auto buf = Hk.request ();
