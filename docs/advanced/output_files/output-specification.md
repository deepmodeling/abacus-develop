...
ABACUS is an integrated software package designed to provide a cohesive user experience. To achieve
this goal, **we are establishing unified output file standards** that all developers should follow.

Recent versions of ABACUS have been working on standardizing all output file naming conventions.
This work is ongoing, and if there are discrepancies between actual file names and this document,
please refer to the latest documentation.

All output file naming conventions can be found in the online documentation (with corresponding
3.10-LTS file names):
[ABACUS Input Documentation](https://abacus.deepmodeling.com/en/latest/advanced/input_files/input-main.html)
...
**Rule 3:** For special output quantities (e.g., wavefunctions), add `_pw` or `_nao` to distinguish
between plane wave basis and numerical atomic orbital basis.
...
**Avoid sparse format with single value per line.** Instead, output 6-8 values per line with proper
alignment:
...
1. **Add `#` comment lines for data meaning, units, and source module**
1. **Use compact format: 6-8 values per line**
1. **Make precision controllable via input parameter**
1. **Use `std::setw()` for column alignment**
...
1. Add documentation in `docs/advanced/output_files/`
1. Submit PR with output sample for review
