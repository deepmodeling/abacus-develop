...
- **Type**: Boolean
- **Availability**: NAO basis
- **Description**: print energy and force labels and descriptors for DeePKS training. In `LCAO` calculation, the path of a numerical descriptor (an `orb` file) is needed to be specified under the `NUMERICAL_DESCRIPTOR` tag in the `STRU` file. For example:
...
- **Type**: Boolean
- **Availability**: NAO basis
- **Description**: perform self-consistent field iteration in DeePKS method. A trained, traced model file is needed.
- **Default**: False

### deepks_model

- **Type**: String
- **Availability**: NAO basis and `deepks_scf` is true
...
- **Description**: the maximum angular momentum of the Bessel functions generated as the projectors in DeePKS. To generate such projectors, set calculation type to `gen_bessel` in ABACUS. See also [calculation](#calculation).
- **Default**: 2
...
- **Type**: Boolean
- **Description**: generate files for constructing DeePKS unit test. Not relevant when running actual calculations. When set to 1, ABACUS needs to be run with only 1 process.
- **Default**: False
...
- **Type**: Real
- **Description**: the effective surface tension parameter, which describes the cavitation, the dispersion, and the repulsion interaction between the solute and the solvent that are not captured by the electrostatic terms.
- **Default**: 1.0798e-05
- **Unit**: $Ry/Bohr^{2}$
