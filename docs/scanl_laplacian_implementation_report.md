...
#### 方案 3：V 和 vtxc 均不含 vlapl，stress 保留 vlapl 项
...
**ecutwfc=200 结果**（此前测试，单位转换有误，仅参考趋势）：FD/AB = 1.018（1.82% 误差）

**ecutwfc=400 结果**：待验证（需在对应分支上重新构建和测试）。
...
注：方案 3、4、5 的 FD/AB 值来自 ecutwfc=200 Ry 的测试，由于此前存在单位转换错误，这些数值仅反映趋势，需用正确转换因子重新验证。
