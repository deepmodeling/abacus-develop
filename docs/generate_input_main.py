...
Usage:
    # Generate markdown from an ABACUS parameter YAML dump:
    abacus --generate-parameters-yaml | python docs/generate_input_main.py - [--output FILE]

Can also be imported from conf.py:
    from generate_input_main import generate_from_data
    generate_from_data(data, output)
"""
...
def generate_from_data(data: Dict, output: Path, verbose: bool = False):
    """
    Generate markdown from loaded parameter metadata.
    """
    output = Path(output)
...
def generate(yaml_path: Path, output: Path, verbose: bool = False):
    """
    Core generation logic. Can be called from the CLI.
    """
    ...
    generate_from_data(data, output=output, verbose=verbose)
...
def main():
    ...
    parser.add_argument(
        'yaml_file',
        type=str,
        help='Path to parameter YAML, or - to read from standard input'
    )
    parser.add_argument(
        '--output',
        type=Path,
