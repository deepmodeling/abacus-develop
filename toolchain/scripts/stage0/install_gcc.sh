...
case "${with_gcc}" in
  __INSTALL__)
    ...
    if verify_checksums "${install_lock_file}"; then
      ...
      # Download prerequisites from ABACUS.org because gcc.gnu.org returns 403 when queried from GCP.
      sed -i 's|http://gcc.gnu.org/pub/gcc/infrastructure/|https://ABACUS.org/static/downloads/|' ./contrib/download_prerequisites
