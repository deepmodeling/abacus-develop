...
def cal_weight(info_weight, flag_same_band, stru_file_list=None):
    ...
    if "bands_file" in info_weight.keys():
        ...
        for weight_stru, file_name in zip(
            info_weight["stru"], info_weight["bands_file"]
        ):
        ...
        for weight_stru, bands_range, nbands_ist in zip(
            info_weight["stru"], info_weight["bands_range"], nbands
        ):
...
def read_k_weight(stru_file_list):
    ...
    for file_name in stru_file_list:
        ...
        with open(file_name, "r") as file:
            data = (
                re.compile(r"<WEIGHT_OF_KPOINTS>(.+)</WEIGHT_OF_KPOINTS>", re.S)
                .search(file.read())
                .group(1)
                .split("\n")
            )
...
def read_nbands(stru_file_list):
    ...
    for file_name in stru_file_list:
        with open(file_name, "r") as file:
            nbands.append(
                int(re.compile(r"(\d+)\s+nbands").search(file.read()).group(1))
            )
    return nbands
