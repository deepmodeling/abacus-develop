...
def read_file_head(info, file_list):
    ...
    repeat_Nk = lambda x: list(
        itertools.chain.from_iterable(
            map(lambda x: itertools.repeat(*x), zip(x, info_true.Nk))
        )
    )
    ...
    for ist_true, file_name in enumerate(file_list):
        ...
        with open(file_name, "r") as file:
            ...
            for ik in range(Nk):
                ...
                QI.append(qi)
        with open(file_name, "r") as file:
            data = re.compile(r"<OVERLAP_Sq>(.+)</OVERLAP_Sq>", re.S).search(
                file.read()
            )
            data = map(float, data.group(1).split())
            for ik in range(Nk):
        ...
        if V_info["init_from_file"]:
            with open(file_name, "r") as file:
                data = re.compile(r"<OVERLAP_V>(.+)</OVERLAP_V>", re.S).search(
                    file.read()
                )
                data = map(float, data.group(1).split())
        else:
...
def read_QI(info_stru, info_element, data):
    ...
    for it in info_stru.Na.keys():
        ...
        for il in range(info_element[it].Nl):
            QI[it][il] = torch.zeros(
                (info_stru.Nb, info_stru.Na[it], util.Nm(il), info_element[it].Ne),
    ...
    for it in info_stru.Na.keys():
        for il in range(info_element[it].Nl):
            QI[it][il] = (
                QI[it][il][: info_stru.Nb_true, :, :, :]
                .view(-1, info_element[it].Ne)
                .conj()
            )
    return QI
...
def read_SI(info_stru, info_element, data):
    ...
    for it1, it2 in itertools.product(info_stru.Na.keys(), info_stru.Na.keys()):
        ...
        for il1, il2 in itertools.product(
            range(info_element[it1].Nl), range(info_element[it2].Nl)
        ):
            SI[it1, it2][il1][il2] = torch.zeros(
                (
                    info_stru.Na[it1],
                    util.Nm(il1),
                    info_element[it1].Ne,
                                ...
                                for im2 in range(util.Nm(il2)):
                                    for ie1 in range(info_element[it1].Ne):
                                        for ie2 in range(info_element[it2].Ne):
                                            SI[it1, it2][il1][il2][
                                                ia1, im1, ie1, ia2, im2, ie2
                                            ] = complex(next(data), next(data))
