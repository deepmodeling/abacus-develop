...
def main():
    ...
    file_list, info_true, weight_info, C_init_info, V_info = IO.read_json.read_json(
        "INPUT"
    )

    weight = IO.cal_weight.cal_weight(
        weight_info, V_info["same_band"], file_list["origin"]
    )
    ...
    print(
        "info_element:",
        pprint.pformat(info_element, width=40),
        sep="\n",
        end="\n" * 2,
        flush=True,
    )
    print(
        "info_opt:",
        pprint.pformat(info_opt, width=40),
        sep="\n",
        end="\n" * 2,
        flush=True,
    )
    ...
    QI, SI, VI_origin = IO.read_QSV.read_QSV(
        info_stru, info_element, file_list["origin"], V_info
    )
    if "linear" in file_list.keys():
        QI_linear, SI_linear, VI_linear = list(
            zip(
                *(
                    IO.read_QSV.read_QSV(info_stru, info_element, file, V_info)
    ...
    if C_init_info["init_from_file"]:
        C, C_read_index = IO.func_C.read_C_init(
            C_init_info["C_init_file"], info_element
        )
    else:
    ...
    with open("Spillage.dat", "w") as S_file:
        ...
        for istep in range(10000):
            ...
            for ist in range(len(info_stru)):
                ...
                Q = opt_orb.change_index_Q(
                    opt_orb.cal_Q(QI[ist], C, info_stru[ist], info_element),
                    info_stru[ist],
                )
                S = opt_orb.change_index_S(
                    opt_orb.cal_S(SI[ist], C, info_stru[ist], info_element),
                    info_stru[ist],
                    info_element,
                )
                ...
                if "linear" in file_list.keys():
                    ...
                    for i in range(len(file_list["linear"])):
                        Q_linear = opt_orb.change_index_Q(
                            opt_orb.cal_Q(
                                QI_linear[i][ist], C, info_stru[ist], info_element
                            ),
                            info_stru[ist],
                        )
                        S_linear = opt_orb.change_index_S(
                            opt_orb.cal_S(
                        ...
                        V_linear[i] = opt_orb.cal_V_linear(
                            coef, Q_linear, S_linear, V, V_info
                        )
            ...
            if info_opt.cal_T:
                T = opt_orb.cal_T(C, E)
                if not "TSrate" in vars():
                    TSrate = torch.abs(0.002 * Spillage / T).data[0]
    ...
    orbital.orth(orb, {it: info_element[it].dr for it in info_element})
