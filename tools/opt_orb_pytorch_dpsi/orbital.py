            ...
            for iu in range(info_element[it].Nu[il]):
                ...
                for ir in range(Nr):
                    ...
                    for ie in range(info_element[it].Ne):
                        orb[it][il][iu][ir] += C[it][il][ie, iu].item() * spherical_jn(
                            il, E[it][il, ie].item() * r
                        )
    return orb
        ...
        for orb_tl in orb_t:
            for orb_tlu in orb_tl:
                for ir in range(orb_tlu.shape[0]):
                    ...
                    r = ir * dr[it]
                    orb_tlu[ir] *= 1 - np.exp(
                        -((r - Rcut[it]) ** 2) / (2 * smearing_sigma**2)
                    )
    ...
    for it, orb_t in orb.items():
        for il, orb_tl in enumerate(orb_t):
            for iu, orb_tlu in enumerate(orb_tl):
                ...
                if flag_norm_orb:
                    orb_tlu[:] = orb_tlu / norm
                if flag_norm_C:
                    C[it][il].data[:, iu] = C[it][il].data[:, iu] / norm
        ...
        for il, orb_tl in enumerate(orb_t):
            for iu1, orb_tlu1 in enumerate(orb_tl):
                for iu2 in range(iu1):
                    orb_tlu1[:] -= orb_tl[iu2] * inner_product(
                        orb_tlu1, orb_tl[iu2], dr[it]
                    )
                orb_tlu1[:] = orb_tlu1 / np.sqrt(
                    inner_product(orb_tlu1, orb_tlu1, dr[it])
                )
...
def set_E(info_element):
    """E[it][il,ie]"""
    eigenvalue = {
        it: find_eigenvalue(info_element[it].Nl, info_element[it].Ne)
        for it in info_element
    }
    E = dict()
    for it in info_element:
        E[it] = torch.from_numpy(
            (eigenvalue[it] / info_element[it].Rcut).astype("float64")
        )
    return E
