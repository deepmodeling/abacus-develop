...
def submit_jobs():
    ...
    indices1, indices2, pp, basis = split_stru_with_index(
        origin_structure, selected_indices, "A.stru", "B.stru"
    )
    ...
    result = subprocess.run(
        "phonopy setting.conf --abacus -d -c A.stru",
        shell=True,
        capture_output=True,
        text=True,
    )
    ...
    for batch in range(1, TOTAL_BATCHES + 1):
        ...
        for task in range(1, TASKS_PER_BATCH + 1):
            ...
            merge_stru_by_index(
                perturbed_stru, "../B.stru", indices1, indices2, pp, basis, "STRU"
            )
            ...
            try:
                result = subprocess.run(
                    ["sbatch", "../job.sh"], check=True, capture_output=True, text=True
                )
...
def postprocess():
 ...
 -------------------------------------------------------------------------\n"""
    ...
    result = subprocess.run(
        "phonopy -t mesh.conf", shell=True, check=True, capture_output=True, text=True
    )
