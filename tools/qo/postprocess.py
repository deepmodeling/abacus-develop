...
def cal_tb_hs(
    h_k: np.ndarray,
    s_k: np.ndarray,
    denmat_k_tilde: np.ndarray,
    ao_proj_k: np.ndarray,
    norm_thr: float = 1e-10,
):
...
def kR_transform(dst, mats_in: list, srcs: list, direction: str = "R->k"):
    ...
    if len(mats_in) != len(srcs):
        print(
            f"Number of input matrices and source vectors are not equal: {len(mats_in)} (num of mats) != {len(srcs)} (num of srcs)."
        )
...
def k_extrapolation(mats_k: list, kvecs_in: list, Rs: list, kvecs_out: list):
    ...
    """
    mats_R = [
        kR_transform(Rs[i], mats_k, kvecs_in, direction="k->R") for i in range(len(Rs))
    ]
    mats_kpath = [
        kR_transform(kvecs_out[i], mats_R, Rs, direction="R->k")
        for i in range(len(kvecs_out))
    ]
    return mats_kpath
...
def norm_filter_k(
    ...
    norms = [
        np.sqrt(ao_k[:, iao].conj().T @ s_k @ ao_k[:, iao])
        for iao in range(ao_k.shape[1])
    ]
...
def mats_hs_k(path: str, nk: int, ib_min: int, ib_max: int = -1, qo: bool = True):
    ...
    lowfs_k = (
        lowfs_k if ib_max == -1 else [lowfs_k[ik][:, ib_min:ib_max] for ik in range(nk)]
    )
    ...
    if not qo:
        ...
        lowfs_bar_k = [
            lowdin_onW(
                s_k=ss_k[ik], denmat_k=denmats_k[ik], ao_proj_k=aos_proj_k[ik], m=m
            )
            for ik in range(nk)
        ]
        lowfs_tilde_k = [
            np.concatenate((lowfs_k[ik], lowfs_bar_k[ik]), axis=1) for ik in range(nk)
        ]
        _one = [
            lowf_tilde_k.conj().T @ ss_k[ik] @ lowf_tilde_k
            for ik, lowf_tilde_k in enumerate(lowfs_tilde_k)
        ]
        ...
        for ik in range(nk):
            for i, j in it.product(range(nband + m), repeat=2):
                if i != j:
                    # print(_one[ik][i, j])
                    assert (
                        abs(_one[ik][i, j].real) < 1e-6
                        and abs(_one[ik][i, j].imag) < 1e-6
                    )
                else:
                    # print(_one[ik][i, j])
                    assert (
                        abs(_one[ik][i, j].real - 1) < 1e-5
                        and abs(_one[ik][i, j].imag) < 1e-6
                    )
        # go on
        ...
        hqos_k, sqos_k = [], []
        for ik in range(nk):
            hqo_k, sqo_k = cal_tb_hs(
                hs_k[ik], ss_k[ik], denmats_tilde_k[ik], aos_proj_k[ik]
            )
            hqos_k.append(hqo_k)
            sqos_k.append(sqo_k)
...
def production(
    path: str,  # path of OUT.${suffix} folder
    ...
    error_estimation_with_lcao: bool = False,
):  # whether to estimate errors with LCAO
    ...
    eigs_qo = [
        bad_condition_diag(hqos_k[ik], sqos_k[ik], diag_thr) for ik in range(nkpts)
    ]
    ...
    if error_estimation_with_lcao:
        ...
        eigs_ao = [
            bad_condition_diag(haos_k[ik], saos_k[ik], diag_thr) for ik in range(nkpts)
        ]
        ...
        exclude_kvecs = [
            ik for ik, e_error_k in enumerate(e_errors_k) if e_error_k > 1e-8
        ]
        print(
            "Number of kpoints excluded due to large error:",
            len(exclude_kvecs),
            "\nTheir indices:",
            exclude_kvecs,
        )
...
class QOUnittest(unittest.TestCase):
    ...
    def test_read_lowf_k(self):
        for ik in range(ndim**3):
            ...
            self.assertEqual(
                nrows, nlocal
            )  # should be overlap matrix of basis functions
            ...
            for i in range(nrows):
                for j in range(ncols):
                    self.assertAlmostEqual(
                        denmat_k[i, j], denmat_dagger_k[i, j], delta=1e-5
                    )
    ...
    def test_lowdin_onW(self):
        for ik in range(ndim**3):
            ...
            lowf_bar = lowdin_onW(
                s_k=s_k, denmat_k=denmat_k, ao_proj_k=ao_proj_k, m=ndim_test
            )
            ...
            for i in range(len(D_tilde_k)):
                for j in range(len(D_tilde_k[0])):
                    self.assertAlmostEqual(
                        D_tilde_k[i][j], DSD_tilde_k[i][j], delta=1e-5
                    )
    ...
    def test_kR_transform(self):
        for nk in [1, 3, 5]:
            ...
            mats_k = [
                kR_transform(kpoints[i], mats_0, Rs, "R->k")
                for i in range(len(kpoints))
            ]
            mats_kR = [
                kR_transform(Rs[i], mats_k, kpoints, "k->R")
                for i in range(len(kpoints))
            ]
            for ik in range(len(kpoints)):
                for i in range(ndim):
                    for j in range(ndim):
                        self.assertAlmostEqual(
                            mats_kR[ik][i, j].real, mats_0[ik][i, j].real, delta=1e-12
                        )
                        self.assertAlmostEqual(
                            mats_kR[ik][i, j].imag, mats_0[ik][i, j].imag, delta=1e-12
                        )
        ...
        for nk in [1, 3, 5]:
            ...
            for ik in range(len(kpoints)):
                for i in range(ndim):
                    for j in range(ndim):
                        self.assertAlmostEqual(
                            mats_k2R2k[ik][i, j].real,
                            mats_0[ik][i, j].real,
                            delta=1e-12,
                        )
                        self.assertAlmostEqual(
                            mats_k2R2k[ik][i, j].imag,
                            mats_0[ik][i, j].imag,
                            delta=1e-12,
                        )
...
if __name__ == "__main__":
    ...
    if run_type == "production":
        ...
        hqos_k, sqos_k, kvecs_d = production(
            path, nkpts, ib_min, ib_max, eig_thr, error_estimation_with_lcao=True
        )
