...
V_ANGSTROM_TO_AU_EFIELD = (
    sc.e
    / (4 * sc.pi * sc.epsilon_0 * sc.physical_constants["Bohr radius"][0] ** 2)
    * 1e-10
)
...
def validate_inputs(direc: List[int], efield_path: List[str]) -> None:
    if len(direc) != len(efield_path):
        raise ValueError(
            f"Number of --direc ({len(direc)}) != --efield_path ({len(efield_path)})"
        )
...
def zero_pad_and_mask(
    signal: np.ndarray, dt: float, pad_factor: int = 10, decay_beta: float = 5.0
) -> np.ndarray:
...
def load_dipole(
    filename: str, step_start: int, step_end: int, remove_dc: bool
) -> np.ndarray:
...
def load_efield(
    file_groups: List[List[str]], step_start: int, step_end: int, remove_dc: bool
) -> np.ndarray:
    ...
    for i, files in enumerate(file_groups):
        for f in files:
            arr = np.loadtxt(f)
            efield[i] += (
                arr[step_start:step_end, 1] / V_ANGSTROM_TO_AU_EFIELD
            )  # V/Å → a.u.
        # Remove DC (zero-frequency) component by subtracting mean
...
class AbsorptionSpectrum:
    ...
    def compute_alpha(self, direction: int) -> np.ndarray:
        ...
        d_pad = zero_pad_and_mask(
            self.dipole[direction], self.dt_au, self.pad_factor, self.decay_beta
        )
        e_pad = zero_pad_and_mask(
            self.efield[direction], self.dt_au, self.pad_factor, self.decay_beta
        )
        ...
        if self.system_type == "dipole_epsilon":
            ...
            omega = 2 * np.pi * freqs_full  # ω in a.u.^{-1}
            alpha = np.divide(
                ratio.real, omega, out=np.zeros_like(ratio.real), where=omega != 0
            )
            return np.abs(alpha)
            ...
            raise ValueError(
                "system_type must be 'dipole_epsilon', 'dipole_sigma', 'current_epsilon', or 'current_sigma'"
            )
...
def plot_absorption(
    ...
    if x_range:
        ...
        if np.any(mask):
            ax.set_ylim(
                0, 1.2 * max(max(a[mask]) for a in alphas if len(a) == len(x_vals))
            )
...
def compute_response_function(
    dipole_fft: np.ndarray, efield_fft: np.ndarray
) -> np.ndarray:
    """Compute complex response χ(ω) = D(ω) / E(ω)"""
    return np.divide(
        dipole_fft, efield_fft, out=np.zeros_like(dipole_fft), where=efield_fft != 0
    )
...
def kk_hilbert_transform(
    signal_pos: np.ndarray, omega: np.ndarray, is_odd: bool = True
) -> np.ndarray:
    """
    ...
    interp = interp1d(
        omega, signal_pos, kind="linear", bounds_error=False, fill_value=0.0
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot RT-TDDFT absorption spectrum from ABACUS output."
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="Field directions: 0=x,1=y,2=z",
    )
    parser.add_argument(
        "--efield_path", type=str, nargs="+", required=True, help="E-field file paths"
    )
    parser.add_argument("--dipolefile", type=str, default="./OUT.ABACUS/dipole_s1.txt")
    # parser.add_argument("--dipolefile", type=str, default="./OUT.ABACUS/dipole_s1.txt")
    parser.add_argument(
        "--material_name", type=str, default=r"CH$_3$COOH", help="Name for plot title"
    )
    parser.add_argument("--step_start", type=int, default=0)
    parser.add_argument("--step_end", type=int, default=8000)
    parser.add_argument(
        "--system_type",
        choices=["dipole_epsilon", "dipole_sigma", "current_epsilon", "current_sigma"],
        default="dipole_sigma",
        help="Type of system and response for absorption calculation",
    )
    parser.add_argument(
        "--pad_factor", type=int, default=10, help="Zero-padding factor for FFT"
    )
    parser.add_argument(
        "--decay_beta",
        type=float,
        default=5,
        help="Exponential decay parameter for windowing",
    )
    parser.add_argument(
        "--remove_dc",
        type=bool,
    ...
    # Load data
    dipole_data = load_dipole(
        args.dipolefile, args.step_start, args.step_end, args.remove_dc
    )
    efield_data = load_efield(
        Efile_groups, args.step_start, args.step_end, args.remove_dc
    )
    ...
    with np.errstate(divide="ignore"):
        wavelengths = np.where(
            energies > 0, (sc.h * sc.c / sc.eV * 1e9) / energies, np.inf
        )
    ...
    np.savetxt(
        "abs_dat.txt",
        data_out,
        header="Energy(eV) Wavelength(nm) alpha_X alpha_Y alpha_Z",
    )
    ...
    plot_time_series(
        ax, time, dipole_data, directions, ["$J_x$", "$J_y$", "$J_z$"], "Current (a.u.)"
    )
    ...
    plot_absorption(
        ax, energies, alphas_to_plot, directions, xlabel="Energy (eV)", x_range=(4, 20)
    )
    ...
    plot_absorption(
        ...
        x_range=(wl_min, wl_max),
    )

    all_alpha = np.concatenate(
        [a for a in alphas_to_plot if len(a) == len(wavelengths)]
    )
    if len(all_alpha) > 0:
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    efield_ffts = [
        np.fft.fft(zero_pad_and_mask(efield_data[d], args.td_dt, args.pad_factor))
        for d in directions
    ]
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    dipole_ffts = [
        np.fft.fft(zero_pad_and_mask(dipole_data[d], args.td_dt, args.pad_factor))
        for d in directions
    ]
    ...
    for idx, d in enumerate(directions):
        ...
        d_pad = zero_pad_and_mask(
            dipole_data[d], args.td_dt, args.pad_factor, args.decay_beta
        )
        e_pad = zero_pad_and_mask(
            efield_data[d], args.td_dt, args.pad_factor, args.decay_beta
        )
        d_fft = np.fft.fft(d_pad)
        e_fft = np.fft.fft(e_pad)
        ...
        chi_real_KK = kk_hilbert_transform(
            chi_imag, omega, is_odd=True
        )  # χ'' is odd → use odd extension

        # --- Reconstruct imag part from real ---
        chi_imag_KK = -kk_hilbert_transform(
            chi_real, omega, is_odd=False
        )  # χ' is even → use even extension

        # --- Plot: Real part comparison ---
        ax1 = axes_kk[idx, 0]
        ax1.plot(omega, chi_real, "b-", lw=1.5, label=r"$\chi'(\omega)$ (FFT)")
        ax1.plot(
            omega, chi_real_KK, "r--", lw=1.5, label=r"$\mathcal{H}[\chi''](\omega)$"
        )
        ax1.set_xlabel("Energy (eV)")
        ax1.set_ylabel(r"$\chi'(\omega)$")
        ax1.set_title(
            f"Direction {['$x$','$y$','$z$'][d]}: Real part KK check",
            fontsize=18,
            y=1.02,
        )
        ax1.legend()
        ...
        ax2.plot(
            omega, chi_imag_KK, "m--", lw=1.5, label=r"$-\mathcal{H}[\chi'](\omega)$"
        )
        ax2.set_xlabel("Energy (eV)")
        ax2.set_ylabel(r"$\chi''(\omega)$")
        ax2.set_title(
            f"Direction {['$x$','$y$','$z$'][d]}: Imaginary part KK check",
            fontsize=18,
            y=1.02,
        )
        ax2.legend()
