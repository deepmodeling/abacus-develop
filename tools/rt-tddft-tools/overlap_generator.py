...
class overlap_R:
    def __init__(self, orb_file_dir, atoms0, atomst):
        ...
        assert orb_file_num == len(
            unique_elements
        ), f"The number of orbitals{orb_file_num}does not match the number of unique elements{len(unique_elements)}!"
                ...
                for iL in range(self.orb.lmax_(it) + 1):
                    for iN in range(self.orb.nzeta(it, iL)):
                        for im in range(2 * iL + 1):
                            self.iw2it[count] = it
                            self.iw2positions_c[count] = self.atom_positions_c[
                                count_atom
                            ]
                            self.iw2positions_ct[count] = self.atom_positions_ct[
                                count_atom
                            ]
                            self.iw2iL[count] = iL
                            self.iw2iN[count] = iN
    ...
    def cal_R_direct_coor(self):
        ...
        print(rcut)
        Ncell = NeighbourCell(
            self.lattice_vector, self.atom_positions_c, self.atom_positions_ct, rcut
        )
    ...
    def cal_overlap(self):
        ...
        self.SR = np.zeros(
            (self.R_direct_coor.shape[0], basis_num, basis_num), dtype=float
        )
...
class NeighbourCell:
    def __init__(self, lattice_vectors, atom_positions0, atom_positionst, cutoff_radii):
        ...
        """
        if (
            len(lattice_vectors) != 3
            or len(atom_positions0) == 0
            or len(cutoff_radii) != len(atom_positions0)
        ):
    ...
    def calculate_distance_to_neighbour(
        self, atom_index1, atom_index2, translation_vector
    ):
        """
        ...
        if (
            atom_index1 < 0
            or atom_index1 >= self.natoms
            or atom_index2 < 0
            or atom_index2 >= self.natoms
        ):
            raise IndexError("Atom index out of range")

        displacement = (
            self.atom_positionst[atom_index2]
            - self.atom_positions0[atom_index1]
            + (self.lattice_vectors @ np.array(translation_vector).T).T
        )
    ...
    def check_interaction_neighbours(self):
        ...
        interactions = (
            []
        )  # List to store translation vectors of neighbouring cells with interactions
                ...
                for v in range(-5, 5):
                    ...
                    for i in range(self.natoms):
                        for j in range(self.natoms):
                            distance = self.calculate_distance_to_neighbour(
                                i, j, neighbour_translation
                            )
                            # if overlap exists, add the translation vector to the list
                            if distance <= (
                                self.cutoff_radii[i] + self.cutoff_radii[j]
                            ):
...
def overlap_gen(
    ...
    klist,
    steps,
    stru_dir="./OUT.ABACUS/STRU",
    orb_file_dir="",
    kpoits_dir="./OUT.ABACUS",
):
