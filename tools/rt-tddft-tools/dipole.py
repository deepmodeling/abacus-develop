...
class Dipole:
    ...
    def plot_dipole(
        ...
        for direc in directions:
            ax.plot(
                self._indices * self.dt, self.dipole_data[direc], label=labels[direc]
            )
...
class Absorption(Dipole, Efield):
    ...
    def __init__(
        self, dipolefile: PathLike, Efile: list[list[PathLike]], dt: float
    ) -> None:
    ...
    def plot_abs(
        ...
        x_data = (
            energies if unit == "eV" else sc.nu2lambda(sc.eV / sc.h * energies) * 1e9
        )
        ...
        for direc in directions:
            ...
            adsorption_spectra_data = np.concatenate(
                (adsorption_spectra_data, alpha[:, np.newaxis]), axis=1
            )
...
if __name__ == "__main__":
    ...
    x_range = [0, 400]
    fig2, ax2 = Abs.plot_abs(fig2, ax2, x_range=x_range, unit="nm")
