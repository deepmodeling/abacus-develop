...
|---|---|---|
| Window closes immediately after "Installing Ubuntu-22.04 …" | Legacy issue where `if errorlevel 1` was parse-time evaluated inside a nested block (fixed). Update to the latest scripts. | Pull latest `install-abacus.bat`. |
