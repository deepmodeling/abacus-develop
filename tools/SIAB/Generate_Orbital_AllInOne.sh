...
for name in $targets; do
  ...
  RestartSTRU[0]=0
  ListSTRU[0]=" "
  for ((iSTRU = 1; iSTRU <= $nSTRU; iSTRU++)); do
  ...
  count_r=1
  while [ $count_r -le $rcut_number ]; do
    ### (1.4.1)
    ...
    for ((iSTRU = 1; iSTRU <= $nSTRU; iSTRU++)); do
      ...
      count=1
      while [ $count -le ${BL_number[iSTRU]} ]; do
...
EOF
        ...
        count_files=1
        while [ $count_files -le ${BL_number[iSTRU]} ]; do
...
EOF
        ...
        for ((i = 1; i <= ${EndLevel[iSTRU]}; i++)); do
          if [ $i -ge ${BeginLevel[iSTRU]} -a $i -le ${EndLevel[iSTRU]} ]; then
            leveltype="new "
          else
            leveltype="skip"
          fi
...
EOF
        ...
        [ "${EXE_orbital:0-12:12}" == "dpsi/main.py" ]
      then # begin from if [ "${EXE_orbital:0-3:3}" != ".py" ];
...
EOF
        ...
        count_files=1
        while [ $count_files -lt ${BL_number[iSTRU]} ]; do
          BL=$(echo "${BLSTRU[iSTRU]}" | awk '{print $'$count_files'}')
          cat >> INPUT << EOF
            "../OUT.$element-$rcut-$BL/orb_matrix.0.dat",
EOF
...
EOF
        ...
        for ((LValue = 0; LValue < ${LValueMax}; LValue++)); do
          cat >> INPUT << EOF
                ${numL[$LValue]}, 
EOF
        done
...
EOF
        ...
        for ((i_BL = 1; i_BL < ${BL_number[iSTRU]}; i_BL++)); do
          cat >> INPUT << EOF
            1, 
EOF
        done
...
EOF
        ...
        # begin from: if [ "${EXE_orbital:0-3:3}" != ".py" ];
