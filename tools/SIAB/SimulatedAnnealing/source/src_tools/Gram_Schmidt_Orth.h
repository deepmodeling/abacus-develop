...
class Gram_Schmidt_Orth
{
  ...
  public:
    ...
    Gram_Schmidt_Orth&
        set_coordinate (size_t coordinate_in)
    {
        if (0 != coordinate_in && 1 != coordinate_in)
            {
                throw GS_Orth_Coordinate ();
            }
    ...
    Gram_Schmidt_Orth&
        set_func_num (size_t func_num_in)
    {
        func_num = func_num_in;
        return *this;
    }

    size_t
        get_variable_num () const
    {
        return variable_num;
    }
    Gram_Schmidt_Orth&
        set_variable_num (size_t variable_num_in)
    {
        variable_num = variable_num_in;
        return *this;
    }
    ...
    Gram_Schmidt_Orth&
        set_rab_element (const R_Type& rab_element_in)
    {
        if (NULL == rab)
            {
...
void
    Gram_Schmidt_Orth<Func_Type, R_Type>::init ()
{
    if (this->finish_init)
        {
            return;
...
template <typename Func_Type, typename R_Type>
Func_Type**
    Gram_Schmidt_Orth<Func_Type, R_Type>::orth ()
{
    init ();
...
template <typename Func_Type, typename R_Type>
Func_Type
    Gram_Schmidt_Orth<Func_Type, R_Type>::cal_norm (Func_Type* func_in)
{
    Func_Type norm (0.0);
    switch (coordinate)
        {
