...
double
    Mathzone::BESSJY (double x, double xnu, double* rj, double* ry, double* rjp, double* ryp)
{
    const int XMIN = 2;
...
void
    Mathzone::BESCHB (double x, double* gam1, double* gam2, double* gampl, double* gammi)
{
    const int NUSE1 = 7;
    const int NUSE2 = 8;
    double xx;
...
double
    Mathzone::CHEBEV (double a, double b, double c[], int m, double x)
{
...
double
    Mathzone::Spherical_Bessel_7 (const int n, const double& x)
{
    if (x == 0)
        {
            if (n != 0)
...
void
    Mathzone::Spherical_Bessel_Roots (const int& num,
                                      const int& l,
                                      ...
                                      const double& rcut)
{
    ...
    const double max = 2 * PI + (num + (l + 0.5) / 2 + 0.75) * PI / 2
                       + sqrt ((num + (l + 0.5) / 2 + 0.75) * (num + (l + 0.5) / 2 + 0.75) * PI * PI / 4
                               - (l + 0.5) * (l + 0.5) / 2);
...
// from sph_bes.f90
void
    Mathzone::Spherical_Bessel (const int& msh,  // number of grid points
            ...
            if (l == -1)
            ...
            else if (l == 3)
                {
                    for (ir = ir0; ir < msh; ir++)
                        {
                            x1 = q * r[ir];
                            jl[ir] = (sin (x1) * (15.0 / x1 - 6.0 * x1) + cos (x1) * (x1 * x1 - 15.0))
                                     / pow (x1, 3); // mohan modify 2007-10-13
                        }
                }
            else if (l == 4)
                {
                    for (ir = ir0; ir < msh; ir++)
                        {
                            ...
                            jl[ir] = (sin (x1) * (105.0 - 45.0 * x2 + x4) + cos (x1) * (10.0 * x3 - 105.0 * x1))
                    ...
                    for (ir = ir0; ir < msh; ir++)
                        {
                            ...
                            if (x1 < 0.14)
                            ...
                            else
                                {
                                    double cx1 = cos (x1);
                                    double sx1 = sin (x1);
                                    jl[ir] = (-cx1 - (945.0 * cx1) / pow (x1, 4) + (105.0 * cx1) / (x1 * x1)
                                              + (945.0 * sx1) / pow (x1, 5) - (420.0 * sx1) / pow (x1, 3)
                                              + (15.0 * sx1) / x1)
                                             / x1;
                                }
                        }
                    ...
                    for (ir = ir0; ir < msh; ir++)
                        {
                            ...
                            if (x1 < 0.29)
                            ...
                            else
                                {
                                    double cx1 = cos (x1);
                                    double sx1 = sin (x1);
                                    jl[ir] = ((-10395.0 * cx1) / pow (x1, 5) + (1260.0 * cx1) / pow (x1, 3)
                                              - (21.0 * cx1) / x1 - sx1 + (10395.0 * sx1) / pow (x1, 6)
                                              - (4725.0 * sx1) / pow (x1, 4) + (210.0 * sx1) / (x1 * x1))
                                             / x1;
                                }
                        }
...
void
    Mathzone::Pointwise_Product (const size_t mesh, const double* func1, const double* func2, double* mul_func)
{
...
void
    Mathzone::Simpson_Integral (const int mesh, const double* func, const double* rab, double& asum)
{
    ...
    if (mesh % 2 == 0)
        {
            ...
            cout << "\n routine assumes mesh is odd but mesh = " << mesh << endl;
            return;
        }
...
double
    Mathzone::Boltzmann_dist (const double& value, const double& t)
{
