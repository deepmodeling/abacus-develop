...
inline void
    Inverse_Matrix_Iterate::init (size_t dim_in)
{
...
inline Inverse_Matrix_Iterate&
    Inverse_Matrix_Iterate::set_X0 (const ComplexMatrix& X0_in)
{
...
inline void
    Inverse_Matrix_Iterate::iterate (const ComplexMatrix& old_X, ComplexMatrix& new_X)
{
    // I - A * X_n
    if (this->matrix_info.symmetry)
        {
            this->matrix_tmp_1.minus (this->identity,
                                      this->matrix_tmp_2.multiply_special (this->matrix_info.symmetry,
                                                                           'L',
                                                                           this->matrix_info.uplo,
                                                                           *(this->A_ptr),
                                                                           old_X));
        }
    else
    ...
    if (error_norm2 < this->tolerate_error2)
        ...
        {
            this->last_error2 = error_norm2;
        }

    // X_{n+1} = X_n (I + (I-A*X_n) ) = X_n ( 2I - A*X_n )
    for (size_t i (0); i < this->matrix_info.dim; ++i)
        {
            this->matrix_tmp_1 (i, i) += 1.0;
        }
...
inline const ComplexMatrix&
    Inverse_Matrix_Iterate::cal_inverse ()
{
    try
        {
