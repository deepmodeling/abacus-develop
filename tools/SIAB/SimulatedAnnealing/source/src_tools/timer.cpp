    ...
    timer::cpu_time ()
{
    ...
    if (t1 < 0)
        {
            return 0; // mohan add, abandon the cross point time 2^32 ~ -2^32 .
        }
    else
        {
...
void
    timer::tick (const string& tagc, const string& tag)
{
    //----------------------------------------------------------
    ...
    for (int i = 0; i < n_clock; i++)
        {
            ...
            if ((cpu_second[k] >= 0 && cpu_second[k] < small) || (cpu_second[k] <= 0 && cpu_second[k] > -small))
                {
                    continue;
                }
            ...
            cout << "\n"
                 << setw (20) << class_name[k] << setw (20) << name[k] << setw (15) << spend_time << setw (10)
                 << calls[k] << setw (10) << setprecision (2) << average_spend_time << setw (10)
                 << spend_time / cpu_second[0] * 100 << "%";
        }
    cout << "\n--------------------------------------------------------------------------------------" << endl;
    delete[] print_flag;
    return;
