...
inline void
    ComplexMatrix::init (const int nrows, const int ncols)
{
    this->nr = nrows;
    this->nc = ncols;
    this->size = nr * nc;
...
// deconstructor
inline ComplexMatrix::~ComplexMatrix () { this->freemem (); }

// Free up memory for ComplexMatrix
inline void
    ComplexMatrix::freemem ()
{
    delete[] c;
    c = nullptr;
}
...
inline void
    ComplexMatrix::create (const int nrow, const int ncol)
{
...
inline void
    ComplexMatrix::set_as_identity_matrix ()
{
    /*	for(int i=0; i<nr; i++)
        {
            for(int j=0; j<nc; j++)
...
inline ComplexMatrix
    operator+ (const ComplexMatrix& m1, const ComplexMatrix& m2)
{
    ComplexMatrix tm (m1);
    tm += m2;
    return tm;
    ...
    operator* (const ComplexMatrix& m1, const ComplexMatrix& m2)
{
    ...
    */
    ...
    LapackConnector::zgemm ('N', 'N', m1.nr, m2.nc, m1.nc, 1.0, m1.c, m1.nc, m2.c, m2.nc, 0.0, mprod.c, mprod.nc);
    return mprod;
}
...
inline ComplexMatrix
    multiply_special (const char Symmetry,
                      const char Side,
                      const char Uplo,
                      ...
                      const ComplexMatrix& m2)
{
    ...
    if ('T' == Symmetry)
        {
            LapackConnector::zsymm (Side, Uplo, m1.nr, m2.nc, 1.0, m1.c, m1.nc, m2.c, m2.nc, 0.0, mprod.c, mprod.nc);
        }
    else if ('H' == Symmetry)
        {
            LapackConnector::zhemm (Side, Uplo, m1.nr, m2.nc, 1.0, m1.c, m1.nc, m2.c, m2.nc, 0.0, mprod.c, mprod.nc);
        }
    else
        {
...
inline ComplexMatrix
    operator* (const std::complex<double>& c, const ComplexMatrix& m)
{
    ComplexMatrix sm (m);
...
inline ComplexMatrix
    operator* (const ComplexMatrix& m, const std::complex<double>& c)
{
    ComplexMatrix sm (m);
    for (int i = 0; i < m.size; i++)
        {
...
inline ComplexMatrix
    operator* (const double& r, const ComplexMatrix& m)
{
    ComplexMatrix sm (m);
    for (int i = 0; i < m.size; i++)
        {
...
inline ComplexMatrix
    operator* (const ComplexMatrix& m, const double& r)
{
    ComplexMatrix sm (m);
    for (int i = 0; i < m.size; i++)
        {
...
inline ComplexMatrix&
    ComplexMatrix::add (const ComplexMatrix& m1, const ComplexMatrix& m2)
{
    for (int i = 0; i < size; ++i)
        {
            this->c[i] = m1.c[i] + m2.c[i];
        }
    return *this;
}

inline ComplexMatrix&
    ComplexMatrix::minus (const ComplexMatrix& m1, const ComplexMatrix& m2)
{
    for (int i = 0; i < size; ++i)
        {
            this->c[i] = m1.c[i] - m2.c[i];
...
inline ComplexMatrix&
    ComplexMatrix::multiply (const ComplexMatrix& m1, const ComplexMatrix& m2)
{
    LapackConnector::zgemm ('N', 'N', m1.nr, m2.nc, m1.nc, 1.0, m1.c, m1.nc, m2.c, m2.nc, 0.0, this->c, this->nc);
    return *this;
}
...
// m1 stored in 'U'pper or 'L'ower
inline ComplexMatrix&
    ComplexMatrix::multiply_special (const char Symmetry,
                                     const char Side,
                                     const char Uplo,
                                     const ComplexMatrix& m1,
                                     const ComplexMatrix& m2)
{
    if ('T' == Symmetry)
        {
            LapackConnector::zsymm (Side, Uplo, m1.nr, m2.nc, 1.0, m1.c, m1.nc, m2.c, m2.nc, 0.0, this->c, this->nc);
        }
    else if ('H' == Symmetry)
        {
            LapackConnector::zhemm (Side, Uplo, m1.nr, m2.nc, 1.0, m1.c, m1.nc, m2.c, m2.nc, 0.0, this->c, this->nc);
        }
    else
        {
            throw invalid_argument ("Symmetry must be 'T' or 'H'");
...
// Do mout += s*min
inline void
    scale_accumulate (const std::complex<double>& s, const ComplexMatrix& min, ComplexMatrix& mout)
{
...
// Do mout[i] += s*min[i]
inline void
    scale_accumulate (const int& nmat, const std::complex<double>& s, ComplexMatrix** min, ComplexMatrix** mout)
{
    assert (nmat >= 0);
...
// Do mout = s1*m1 + s2*m2
inline void
    scaled_sum (const std::complex<double>& s1,
                const ComplexMatrix& m1,
...
inline double
    abs2_row (const ComplexMatrix& m, const int ir)
{
    double r = 0.0;
    complex<double> z;
...
inline double
    abs2_column (const ComplexMatrix& m, const int ic)
{
    double r = 0.0;
    complex<double> z;
...
inline double
    abs2 (const int nmat, ComplexMatrix** m)
{
    double r = 0.0;
    for (int i = 0; i < nmat; i++)
        {
...
inline double
    abs2_triangle_matrix (const char Uplo, ComplexMatrix& m)
{
    double r (0.0);
    complex<double> z;
...
inline ComplexMatrix
    transpose (const ComplexMatrix& m, const bool& conjugate)
{
