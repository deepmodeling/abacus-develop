...
class Mathzone
{
  public:
    ...
    // Spherical Bessel
    //========================================================
    static void Spherical_Bessel (const int& msh,  // number of grid points
                                  const double* r, // radial grid
                                  const double& q, //
                                  const int& l,    // angular momentum
                                  double* jl       // jl(1:msh) = j_l(q*r(i)),spherical bessel function
    );
    static void Spherical_Bessel_Roots (const int& num,
                                        const int& l,
                                        const double& epsilon,
                                        double* eigenvalue,
                                        const double& rcut);
    static void Pointwise_Product (const size_t mesh, const double* func1, const double* func2, double* mul_func);
    static void Simpson_Integral (const int mesh, const double* func, const double* rab, double& asum);
