...
ReadData::ReadData () : test (0) { weight = new double[1]; }
...
//==========================================================
void
    ReadData::OverlapQandS (const string& name)
{
    ...
    if (MY_RANK == 0)
        {
            ...
            if (SCAN_BEGIN (ifs, "<WEIGHT_OF_KPOINTS>", false))
                {
                    ...
                    for (int ik = 0; ik < NKSTOT; ik++)
                        {
                            ...
                            ifs >> carkx[ik] >> carky[ik] >> carkz[ik] >> weighttmp[ik];
                            ofs_running << " " << carkx[ik] << " " << carky[ik] << " " << carkz[ik] << " "
                                        << weighttmp[ik] << endl;
                        }
...
void
    ReadData::OverlapSq1q2 (ifstream& ifs)
{
                            ...
                            if (pool == 0)
                                {
                                    ...
                                    for (int id = 0; id < ndata; id++)
                                        {
                                            this->Sq1q2[iknow].ptr[id + startdata]
                                                = std::complex<double> (buffera[id], bufferb[id]);
                                        }
                                    //		ofs_running << "\n local data copy ";
                                }
                            else
...
void
    ReadData::OverlapQ (ifstream& ifs)
{
    if (CALSPI == 0)
        {
            return;
    ...
    if (MY_RANK == 0)
        {
            if (SCAN_BEGIN (ifs, "<OVERLAP_Q>", false))
                {
                    begin = true;
                }
        }
...
//==========================================================
void
    ReadData::OverlapSinv (const string& name)
{
