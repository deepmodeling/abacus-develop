                    ...
                    for (int N = 0; N < mz.l_nchi (it, L); N++)
                        {
                            ...
                            mz.metro.Psi2.ofsk << "\n" << "Type=" << it + 1 << " L=" << L << " N=" << N + 1;
                            mz.metro.Psi2.ofsk << "\n"
                                               << setw (5) << "nq" << setw (20) << "Coefficient" << setw (20)
                                               << "Eigen(Ry)" << setw (20) << "Kinetic_Energy";
                            double kin_sum = 0.0;
                            for (int ie = 0; ie < ne; ie++)
                                {
                                    // double kin = abs(c4[ie])*pow((ie+1)*PI/input.inputs.get_rcut(),2);//In Rydberg
                                    // Unit. double kin = abs(c4[ie])*pow(eigen1[ie]*PI/input.inputs.get_rcut(),2);//In
                                    // Rydberg Unit.
                                    double kin = pow (c4[ie] * eigen1[ie], 2) * mz.metro.Psi2.jjnorm (L, ie);
                                    kin_sum += kin;
                                    mz.metro.Psi2.ofsk
                                        << "\n"
                                        << setw (5) << ie << setiosflags (ios::fixed) << setprecision (10)
                                        << setiosflags (ios::showpoint)
                                        //<< setiosflags(ios::scientific)
                                        << setw (20) << c4[ie] << setw (20) << eigen1[ie] * eigen1[ie] << setw (20)
                                        << kin << " (Rydberg)" << setprecision (6);
                            ...
                            this->get_psi (meshU, rU, L, psi1, c4, eigen1, false);
                            this->get_psi (meshL, rL, L, psi2, c4, eigen1, true);
...
void
    Plot_Psi::print_orbitals_plot () const
{
    ofstream f1 ("ORBITAL_PLOTU.dat");
    ofstream f2 ("ORBITAL_PLOTL.dat");
    ofstream f3 ("ORBITAL_PLOTUK.dat");

    f1 << setprecision (12) << setiosflags (ios::fixed);
    f2 << setprecision (12) << setiosflags (ios::fixed);
    f3 << setprecision (12) << setiosflags (ios::fixed);
    for (int ir = 0; ir < meshU; ir++)
        {
            f1 << ir * dr;
...
void
    Plot_Psi::print_orbitalsU_used () const
{
    //====================================
    // (5)
    //====================================
...
void
    Plot_Psi::print_orbitalsL_used () const
{
    //====================================
    // (5)
    //====================================
...
void
    Plot_Psi::get_psi (const int& mesh,
                       const double* r,
                       const int& l,
                       double* psi,
                       double* c4,
...
void
    Plot_Psi::establish_ecut (ofstream& ofs, const int& ichi, double* psi, const int& L)
{
...
// Peize Lin add 2015-11-20
void
    Plot_Psi::orthogonalization (const int& mesh, double* r, const matrix& psi)
{
