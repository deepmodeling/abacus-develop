...
void
    Out_Orbital::version_information (ofstream& ofs)
{
    ofs << "\n <VERSION>";
    ofs << "\n AUTHOR : Mohan Chen";
    ofs << "\n StartDate : 2009-4-01";
...
void
    Out_Orbital::INPUTs_information (ofstream& ofs)
{
    ofs << "\n\n<INPUTS>";
...
void
    Out_Orbital::spillage_information (ofstream& ofs, const int& ilevel)
{
    ofs << "\n\n<Spillage>";
    ...
    for (int k = 0; k < ilevel; k++)
        {
            ...
            ofs << "\n " << setw (8) << k + 1 << setiosflags (ios::scientific) << setprecision (6) << setw (20)
                << average / STRNUM;
        }
...
void
    Out_Orbital::metropolis_information (ofstream& ofs)
{
    ofs << "\n\n<Metropolis>";
    ofs << "\n"
    ...
    ofs << "\n"
        << setw (20) << mz.metro.get_kin_ini_temp ()
        << " Start temperature (Kelvin) for kinetical energy minimization.";
...
void
    Out_Orbital::c4_information (ofstream& ofs)
{
    ofs << "\n\n<Coefficient>";
            ...
            for (int L = 0; L < mz.lmax_type[it] + 1; L++)
                {
                    ...
                    for (int N = 0; N < mz.l_nchi (it, L); N++)
                        {
                            ofs << "\n" << setw (20) << "Type" << setw (20) << "L" << setw (20) << "Zeta-Orbital";
                            ofs << "\n" << setw (20) << it + 1 << setw (20) << L << setw (20) << N + 1;
                            for (int ie = 0; ie < mz.Level[0].ne; ie++)
                                {
                                    ...
                                    ofs << setw (25) << setiosflags (ios::showpoint) << setiosflags (ios::scientific)
                                        << setprecision (15) << input.Coef.C4_old (it, L, N, ie);
                                }
                        }
                }
        }
...
void
    Out_Orbital::mkb_information (ofstream& ofs)
{
            ...
            if (MY_RANK == 0)
                {
                    ...
                    ofs << "\n"
                        << setw (5) << "BANDS" << setw (20) << "New Fill" << setw (20) << "Total Fill" << setw (20)
                        << "Left Spillage";
                    ofs << setprecision (10);
                }
            double conb = 0.0;
            ...
            for (int ib = bands_start; ib < bands_end; ib++)
                {
                            ...
                            for (int ik = 0; ik < mz.Level[il].data[is].nks; ik++)
                                {
                                    mm += mz.Level[il].data[is].Mkb (ik, ib) * mz.Level[il].data[is].weight[ik]
                                          / (double)bands_number / (double)STRNUM;
                                }
#ifdef __MPI
                    ...
                    if (MY_RANK == 0)
                        {
                            ofs << "\n"
                                << setw (5) << ib + 1 << setw (20) << mm << setw (20) << sum1[ib] << setw (20)
                                << 1.0 / (double)bands_number - sum1[ib];
                        }
