                ...
                for (int n = 0; n < info[it].n[l]; n++) {
                    ...
                    if (info[it].fa[l] == "f") // full
                    {
                        for (int m = 0; m < 2 * l + 1; m++) {
                            ...
                            if (test == 2) {
                                cout << "\n"
                                     << " L=" << l << " N=" << wayd[iw2].N << " m=" << m << " iw2iw[" << iw2
                                     << "]=" << copy;
                            }
            ...
            for (int iw = 0; iw < this->data[istr].nwfc2; iw++) {
                ...
                for (int ie = 0; ie < this->data[istr].ne; ie++) {
                    ...
                    if (this->wayd[iw].average == false) {
                        data[istr].Qoverlap[ik](iw, ib) += input.Coef.C4(T, L, N, ie) * data[istr].Qin(ik, ib, jw, ie);
                        ...
                        for (int ie2 = 0; ie2 < ne; ie2++) {
                            ...
                            if (!wayd[iw].average && !wayd[iw2].average) {
                                this->data[istr].Soverlap[ik](iw, iw2) +=
                                    input.Coef.C4(T1, L1, N1, ie) * input.Coef.C4(T2, L2, N2, ie2) *
                            ...
                            // case 2
                                ...
                                this->data[istr].Soverlap[ik](iw, iw2) +=
                                    input.Coef.C4(T1, L1, N1, ie) * input.Coef.C4(T2, L2, N2, ie2) * avalue;
                                //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                            }
                        } // ie2
                    } // ie
...
double SpillageStep::get_spillage(const int& istr, // index of structure
...
void SpillageStep::newQ(const int& istr,
                        const int& ic,
                        const int& ie,
                        const int& ik,
                        const double& c4_now,
    ...
    for (int iw = 0; iw < nwfc2; iw++) {
        ...
        if (this->wayd[iw].ic == ic) {
            ...
            if (!wayd[iw].average) {
                ...
                for (int ib = 0; ib < this->data[istr].nbands; ib++) {
                    this->data[istr].Qtrial[ik](iw, ib) += delta * this->data[istr].Qin(ik, ib, jw, ie);
                }
...
void SpillageStep::newS(const int& istr,
                        const int& ic,
                        const int& ie,
                        const int& ik,
                        const double& c4_now,
            ...
            if (ie1 != ie || ic1 != ic) {
                for (int iw2 = iw; iw2 < this->data[istr].nwfc2; iw2++) {
                    ...
                    if (this->wayd[iw2].ic == ic) {
                        ...
                        // case 1
                        if (!wayd[iw].average && !wayd[iw2].average) {
                            this->data[istr].Strial[ik](iw, iw2) += (c4_now * input.Coef.C4(T1, L1, N1, ie1) -
                ...
                for (int iw2 = iw; iw2 < this->data[istr].nwfc2; iw2++) {
                    ...
                    for (int ie2 = 0; ie2 < this->data[istr].ne; ie2++) {
                        // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                        if (!wayd[iw].average && !wayd[iw2].average) {
                            this->data[istr].Strial[ik](iw, iw2) += (c4_now * input.Coef.C4(T2, L2, N2, ie2) -
