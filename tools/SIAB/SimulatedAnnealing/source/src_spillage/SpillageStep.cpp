...
void
    SpillageStep::set_info (const int& ntype_in)
{
    this->ntype = ntype_in;
    cout << "\n Element type = " << ntype;
    assert (ntype > 0);
    ...
    for (int i = 0; i < ntype; i++)
        {
            ...
            nwfc2 += info[i].nw2;
            cout << " Type=" << info[i].id << " Readin_Orbital=" << info[i].nw << " Generate_Orbital=" << info[i].nw2
                 << endl;
        }
                                    ...
                                    for (int n = 0; n < info[it].n[l]; n++)
                                        {
                                            if (info[it].fa[l] == "f")
                                                ...
                                                {
                                                    this->wayd[iw2].iw00
                                                        = iw00; // just the start position to get overlap.
                                                    ++iw2;
                                                }
                                        }
                                }
...
void
    SpillageStep::set_nchi ()
{
    if (test == 1)
        {
            TITLE ("SpillageStep", "set_nchi");
        }
    assert (ntype > 0);
    //=========================
    // (1) generate nchi
                    ...
                    for (int j = 0; j < info[i].n[l]; j++)
                        {
                            ...
                            if (l == 0)
                                ...
                                {
                                    wayc[ichi].spd = 'h'; // right?
                                }
                            else if (l == 6)
                                {
                                    wayc[ichi].spd = 'i'; // right?
                                }
                            else
                                {
                                    wayc[ichi].spd = 'x'; // !!!
...
void
    SpillageStep::allocate_data (const int& istep)
{
    if (test == 1)
        {
            TITLE ("SpillageStep", "allocate_data");
        }
    assert (nwfc > 0);
                                    ...
                                    for (int ie = 0; ie < data[is].ne; ie++)
                                        {
                                            for (int iw = 0; iw < NWFCALL; iw++)
                                                {
                                                    data[is].Qin (ik, ib, iw, ie)
                                                        = input.QS_data[is].Qin (ik, ib, iw, ie);
                                                }
                                        }
                                }
                        }
                                ...
                                if( info[it].n[l] > 0 )
                                {
                                    for(int ik=0; ik< data[is].nks; ik++)
                                    {
                                        for(int ib=0; ib< data[is].nbands; ib++)
                                        {
                                            for(int ie=0; ie< data[is].ne; ie++)
                                            {
                                                data[is].Qin(ik, ib, iw_step, ie) = input.QS_data[is].Qin(ik, ib, iw,
            ie);
                                            }
                                        }
                                    }
...
void
    SpillageStep::init_QS_matrix (const int& istr)
{
    if (test == 1)
        {
            TITLE ("SpillageStep", "init_QS_matrix");
        }
                                    ...
                                    if (this->wayd[iw].average == false)
                                        {
                                            data[istr].Qoverlap[ik](iw, ib)
                                                += input.Coef.C4 (T, L, N, ie) * data[istr].Qin (ik, ib, jw, ie);
                                        }
                                    else // if use 'average orbitals'
                                        {
                                            ...
                                            data[istr].Qoverlap[ik](iw, ib) += input.Coef.C4 (T, L, N, ie) * avalue;
                                        }
                                    //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                                }
                        }
                                            ...
                                            for (int ie2 = 0; ie2 < ne; ie2++)
                                                {
                                                    ...
                                                    if (!wayd[iw].average && !wayd[iw2].average)
                                                        {
                                                            this->data[istr].Soverlap[ik](iw, iw2)
                                                                += input.Coef.C4 (T1, L1, N1, ie)
                                                                   ...
                                                                   * input.QS_data[istr].Sq1q2[ik](iw001,
                                                                                                   iw002,
                                                                                                   ie,
                                                                                                   ie2);
                                                        }
                                                    // case 2
                                                    else
                                                        {
                                                            ...
                                                            if (!wayd[iw].average && wayd[iw2].average)
                                                                {
                                                                    for (int m2 = 0; m2 < 2 * L2 + 1; m2++)
                                                                        {
                                                                            avalue += input.QS_data[istr].Sq1q2[ik](
                                                                                iw001,
                                                                                iw002 + m2,
                                                                                ie,
                                                                                ie2);
                                                            ...
                                                            this->data[istr].Soverlap[ik](iw, iw2)
                                                                += input.Coef.C4 (T1, L1, N1, ie)
                                                                   * input.Coef.C4 (T2, L2, N2, ie2) * avalue;
                                                            //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                                                        }
                                                } // ie2
                                        } // ie
...
double
    SpillageStep::get_spillage (const int& istr, // index of structure
    ...
    )
{
    ...
    const double c4_new = input.Coef.C4 (this->wayc[ic].type, this->wayc[ic].L, this->wayc[ic].N, ie);

    // (3) get old c4 coefficient.
    const double c4_old = input.Coef.C4_old (this->wayc[ic].type, this->wayc[ic].L, this->wayc[ic].N, ie);
                            ...
                            for (int index = 0; index < count; index++)
                                {
                                    ...
                                    //	ofs_running << "\n mu=" << pos[index*2] << " nu=" << pos[index*2+1] << " save="
                                    //<< save[index];
                                }
                        }
                    timer::tick ("PW_Basis", "calculateS");
                                    ...
                                    if (iw <= iw2)
                                        {
                                            Stru->Mkb_used (ik, ib) += (conj (Stru->Qtrial[ik](iw, ib))
                                                                        * Stru->inverse_S[ik].get_inverse () (
                                                                            iw,
                                                                            iw2) // Peize Lin update 2015-12-05
                                                                        * Stru->Qtrial[ik](iw2, ib))
                                                                           .real ();
                                        }
                                    else
                                        {
                                            Stru->Mkb_used (ik, ib) += (conj (Stru->Qtrial[ik](iw, ib))
                                                                        * conj (Stru->inverse_S[ik].get_inverse () (
                                                                            iw2,
                                                                            iw)) // Peize Lin update 2015-12-05
                                                                        * Stru->Qtrial[ik](iw2, ib))
                                                                           .real ();
                                        }
                                }
    ...
    if (spillage < 0.0 || spillage > 1.0 || unstable)
        {
            ...
            cout << "\n Type=" << this->wayc[ic].type << " L=" << this->wayc[ic].L << " N=" << this->wayc[ic].N;
...
void
    SpillageStep::newQ (const int& istr,
                        const int& ic,
                        const int& ie,
                        const int& ik,
                        const double& c4_now,
                    ...
                    if (!wayd[iw].average)
                        {
                            ...
                            for (int ib = 0; ib < this->data[istr].nbands; ib++)
                                {
                                    this->data[istr].Qtrial[ik](iw, ib)
                                        += delta * this->data[istr].Qin (ik, ib, jw, ie);
                                }
                        }
...
void
    SpillageStep::newS (const int& istr,
                        const int& ic,
                        const int& ie,
                        const int& ik,
                        const double& c4_now,
                                                    ...
                                                    if (wayd[iw].average && !wayd[iw2].average)
                                                        {
                                                            for (int m1 = 0; m1 < 2 * L1 + 1; m1++)
                                                                {
                                                                    avalue += input.QS_data[istr].Sq1q2[ik](jw + m1,
                                                                                                            kw,
                                                                                                            ie1,
                                                                                                            ie);
                                                                }
                                                    ...
                                                    if (!wayd[iw].average && wayd[iw2].average)
                                                        {
                                                            for (int m2 = 0; m2 < 2 * L2 + 1; m2++)
                                                                {
                                                                    avalue += input.QS_data[istr].Sq1q2[ik](jw,
                                                                                                            kw + m2,
                                                                                                            ie1,
                                                                                                            ie);
                                                                }
...
void
    SpillageStep::updateQS (const int& istr)
{
