...
void Orthogonal::start(SpillageStep& step1, SpillageStep& step2) {
    ...
    for (int istr = 0; istr < STRNUM; istr++) {
        ...
        if (test == 1) {
            cout << "\n"
                 << setw(10) << "nks" << setw(10) << "nbands" << setw(10) << "ne" << setw(10) << "nwfc_all" << setw(10)
                 << "nwfc2";
                ...
                for (int ie = 0; ie < ne; ie++) {
                    ...
                    for (int mu = 0; mu < nwfc2; mu++) {
                        ...
                        if (USEPW) {
                            ...
                            //        {
                            //            cout << "\n iw=" << iw << " ie=" << ie << " mu=" << mu << " first_part=" <<
                            //            first_part; BLOCK_HERE("haha");
                            //        }
                        } else {
                    ...
                    for (int ib = 0; ib < nbands; ib++) {
                        step2.data[istr].Qin(ik, ib, iw, ie) = step1.data[istr].Qin(ik, ib, iw, ie) - sum[ib];
                    }
                }
            }
            delete[] sum;
