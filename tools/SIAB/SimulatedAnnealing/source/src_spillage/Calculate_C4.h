...
class Calculate_C4 {
  public:
    ...
    static void norm_ic(realArray& C4,
    ...
    static void
    Find_Eigenvalues(const int& Enumber, matrix& en, const int& lmax, const double& rcut, const double& tolerence);

  private:
    void Test_Orthogonal(const double& rcut,
                         const double& dr,
                         const int& L,
                         const matrix& en,
                         const int& Enumber,
    ...
    void key_solver(const string& name, const int& L, const matrix& en, const int& Enumber, double* C4);
