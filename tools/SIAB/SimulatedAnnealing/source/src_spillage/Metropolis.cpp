...
void
    Metropolis::reset_temperature (const int& istep)
{
...
void
    Metropolis::move_various_temperature (SpillageStep& Level)
{
    ...
    sap.set_temperature_num (this->spi_ntemp).set_step_num (this->spi_nsteps);
    try
        {
            sap.cal_all_temperature ();
        }
    catch (File_Finish_Metropolis ())
        {
            return;
        }

    Simulated_Annealing_Orbital_Kinetic sak (this, Level);
    sak.set_temperature_num (this->kin_ntemp).set_step_num (this->kin_nsteps);
    try
        {
