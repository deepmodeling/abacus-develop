...
void
    Metropolis::preprocess_all_temperature (SpillageStep& Level)
{
...
void
    Metropolis::file_finish_metropolis (size_t istep)
{
...
void
    Metropolis::change_accept_rate (size_t istep, SpillageStep& Level)
{
                            ...
                            for (int ie2 = 0; ie2 < Level.ne; ie2++)
                                {
                                    ...
                                    const double rate
                                        = (double)input.Coef.accept_number (T, L, N, ie2) / (double)ACCEPTANCE_STEPS;
...
void
    Metropolis::reset_high_kinetic_energy (size_t itemperature, size_t istep)
{
    ...
    if (0 == itemperature && 50 == istep)
        {
            ...
            for (int ic = 0; ic < this->nchi; ic++)
                {
                    this->kin_temp[ic] = high;
                    ofs_running << " Notice: Change temperature of orbital " << ic + 1 << " to " << kin_temp[ic]
                                << endl;
                }
        }
}
...
void
    Metropolis::init_spi_states (size_t itemperature)
{
    ofs_running << "\n" << " ---> SpillageTemp " << setw (3) << itemperature + 1 << endl;
    ofs_running << "      Temperature  " << setw (6) << this->temperature << endl;
    this->info.states = "Spi";
    ofs_running << setw (5) << "STEPS" << setw (20) << "SPILLAGE" << setw (10) << "UPDATES" << endl;

    if (0 == itemperature)
        {
            cout << setw (5) << "STEP" << setw (10) << "TEMP" << setw (20) << "SPILLAGE" << endl;
        }
}
...
void
    Metropolis::init_temperature_kinetic (size_t itemperature, SpillageStep& Level)
{
    // set the initial temperature and kinetic energy value.
    if (0 == itemperature)
        {
            this->min_spillage = input.SV.cal_defined_value (false); // save the spillage value.
            ...
            for (int ic = 0; ic < this->nchi; ic++)
                {
                    ...
                    if (this->info.states == "Kin")
                        {
                            for (int ie2 = 0; ie2 < Level.ne; ie2++)
                                {
                                    this->kin_last[ic]
                                        += pow (c4_last[ie2] * (ie2 + 1) * PI / RCUT, 2) * this->Psi2.jjnorm (L, ie2);
                                }
                        }
                    ...
                    ofs_running << " Orbital " << ic + 1 << " Initial E_kin " << setw (15) << this->kin_last[ic]
                                << endl;
...
void
    Metropolis::norm_C4 (size_t istep, SpillageStep& Level, size_t nsteps_now)
{
    ...
    if ((istep % 50 == 0 && istep != 0) || istep == nsteps_now - 1) // Peize Lin test
        //	if ( (istep % 1 == 0 && istep !=0 ) || istep == nsteps_now-1) 		// Peize Lin test
        {
            ...
            Calculate_C4::norm_ic (input.Coef.C4,
...
double
    Metropolis::cal_spillage (SpillageStep& Level)
{
    const int ie = this->info.ie;
    ...
    return input.SV.cal_defined_value (true);
}

double
    Metropolis::cal_kinetic_energy (SpillageStep& Level)
{
    ...
    if (this->info.states == "Kin")
        {
            ...
            for (int ie2 = 0; ie2 < Level.ne; ie2++)
                {
                    value2
                        += pow (c4tmp[ie2] * (ie2 + 1) * PI / RCUT, 2) * this->Psi2.jjnorm (L, ie2); // In Rydberg Unit.
                }
        }
    else if (this->info.states == "Ecut")
        {
...
void
    Metropolis::accept_process (SpillageStep& Level)
{
...
void
    Metropolis::ofs_1 (size_t istep, SpillageStep& Level)
{
    if (0 == (istep % this->output_each_istep))
        {
            if (this->info.states == "Spi")
                {
                    ofs_running << setw (5) << istep + 1 << setprecision (10) << setiosflags (ios::fixed) << setw (20)
                                << input.SV.cal_defined_value (false) << setw (10) << this->info.update_number << endl;
                    ...
                    for (int ic = 0; ic < this->nchi; ic++)
                        {
                            ...
                            ofs_running << setprecision (3) << setiosflags (ios::scientific) << setw (10)
                                        << this->kin_temp[ic] << resetiosflags (ios::scientific);

                            ofs_running << setprecision (10) << setiosflags (ios::fixed) << setw (20)
                                        << this->kin_last[ic] << endl
...
void
    Metropolis::cout_1 (size_t itemperature)
{
    cout << setw (5) << itemperature + 1 << setiosflags (ios::showpoint) << setiosflags (ios::scientific)
         << setprecision (3) << setw (10) << this->temperature << resetiosflags (ios::scientific)
         << setiosflags (ios::fixed) << setprecision (10) << setw (19) << input.SV.cal_defined_value (false) * 100
         << "%" << resetiosflags (ios::fixed) << resetiosflags (ios::showpoint) << endl;
}

void
    Metropolis::cout_2 (size_t itemperature, SpillageStep& Level)
{
    //-------------------------------------------
    ...
    for (int ic = 0; ic < this->nchi; ic++)
        {
            ...
            cout << setiosflags (ios::showpoint) << setiosflags (ios::scientific) << setprecision (3) << setw (10)
                 << this->kin_temp[ic] << resetiosflags (ios::scientific);
            cout << setprecision (10) << setiosflags (ios::fixed) << setw (20) << this->kin_last[ic]
                 << resetiosflags (ios::fixed) << resetiosflags (ios::showpoint) << endl;
        }
            ...
            if (this->kin_last[ic] > KINETIC_MAX)
                {
                    ...
                    cout << " There maybe 2 reasons, first: The starting temperature for kinetical energy is too small."
                         << endl;
