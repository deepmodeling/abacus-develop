    ...
    if (0 == itemperature) {
        ...
        for (int ic = 0; ic < this->nchi; ic++) {
            ...
            if (this->info.states == "Kin") {
                for (int ie2 = 0; ie2 < Level.ne; ie2++) {
                    this->kin_last[ic] += pow(c4_last[ie2] * (ie2 + 1) * PI / RCUT, 2) * this->Psi2.jjnorm(L, ie2);
                }
...
double Metropolis::cal_kinetic_energy(SpillageStep& Level) {
    ...
    if (this->info.states == "Kin") {
        ...
        for (int ie2 = 0; ie2 < Level.ne; ie2++) {
            value2 += pow(c4tmp[ie2] * (ie2 + 1) * PI / RCUT, 2) * this->Psi2.jjnorm(L, ie2); // In Rydberg Unit.
        }
    } else if (this->info.states == "Ecut") {
    ...
    if (0 == (istep % this->output_each_istep)) {
        if (this->info.states == "Spi") {
            ofs_running << setw(5) << istep + 1 << setprecision(10) << setiosflags(ios::fixed) << setw(20)
                        << input.SV.cal_defined_value(0) << setw(10) << this->info.update_number << endl;
            ...
            for (int ic = 0; ic < this->nchi; ic++) {
                ...
                ofs_running << setprecision(3) << setiosflags(ios::scientific) << setw(10) << this->kin_temp[ic]
                            << resetiosflags(ios::scientific);

                ofs_running << setprecision(10) << setiosflags(ios::fixed) << setw(20) << this->kin_last[ic] << endl
...
void Metropolis::cout_1(size_t itemperature) {
    cout << setw(5) << itemperature + 1 << setiosflags(ios::showpoint) << setiosflags(ios::scientific)
         ...
         << setiosflags(ios::fixed) << setprecision(10) << setw(19) << input.SV.cal_defined_value(0) * 100 << "%"
