...
class Metropolis::Simulated_Annealing_Orbital : public Simulated_Annealing
{
  public:
    Simulated_Annealing_Orbital (Metropolis* metropolis_in, SpillageStep& Level_in)
        : Simulated_Annealing (), metropolis (metropolis_in), Level (Level_in)
    {
    }
  ...
  protected:
    ...
    virtual void
        cal_each_step (size_t itemperature, size_t istep)
    {
        ...
        for (int ic = 0; ic < metropolis->nchi; ic++)
            {
                ...
                if (Level.info[T].state == "skip")
                    continue; // if "skip", the C4 will not changed in this 'Big step' // mohan add 2009-01-27
    ...
    virtual void
        preprocess_each_step_beyondloop (size_t itemperature, size_t istep)
    {
    }
    virtual void
        reprocess_each_step_beyondloop (size_t itemperature, size_t istep)
    {
        metropolis->ofs_1 (istep, this->Level);
        metropolis->norm_C4 (istep, this->Level, this->step_num);
    }
    virtual void
        move_variable (size_t itemperature, size_t istep)
    {
        metropolis->trial_c4 ();
    }
    virtual void
        accept_process (double new_function, size_t itemperature, size_t istep)
    {
...
class Metropolis::Simulated_Annealing_Orbital_Spillage : public Simulated_Annealing_Orbital
{
  ...
  private:
    ...
    virtual double
        cal_new_function (size_t itemperature, size_t istep)
    {
