    ...
    Parallel_Kpoints::init ()
{
    ...
    if (MY_RANK == 0)
        {
            cout << "\n    " << setw (8) << "MY_RANK" << setw (8) << "MY_POOL" << setw (13) << "RANK_IN_POOL"
                 << setw (6) << "NPROC" << setw (6) << "KPAR" << setw (14) << "NPROC_IN_POOL" << endl;
        }

    ofs_running << "\n    " << setw (8) << "MY_RANK" << setw (8) << "MY_POOL" << setw (13) << "RANK_IN_POOL" << setw (6)
                << "NPROC" << setw (6) << "KPAR" << setw (14) << "NPROC_IN_POOL" << endl;

    for (int i = 0; i < NPROC; i++)
        {
            if (MY_RANK == i)
                {
                    cout << " I'm" << setw (8) << MY_RANK << setw (8) << MY_POOL << setw (13) << RANK_IN_POOL
                         << setw (6) << NPROC << setw (6) << KPAR << setw (14) << NPROC_IN_POOL << endl;

                    ofs_running << " I'm" << setw (8) << MY_RANK << setw (8) << MY_POOL << setw (13) << RANK_IN_POOL
                                << setw (6) << NPROC << setw (6) << KPAR << setw (14) << NPROC_IN_POOL << endl;
                }
...
void
    Parallel_Kpoints::kinfo (int& nkstot)
{
#ifdef __MPI
...
void
    Parallel_Kpoints::get_whichpool (const int& nkstot)
{
    delete[] whichpool;
...
void
    Parallel_Kpoints::get_nks_pool (const int& nkstot)
{
    delete[] nks_pool;
...
void
    Parallel_Kpoints::get_startk_pool (const int& nkstot)
{
    delete[] startk_pool;
...
void
    Parallel_Kpoints::pool_collection (double& value, const double* wk, const int& ik)
{
#ifdef __MPI
...
void
    Parallel_Kpoints::pool_collection (double* value, const realArray& overlap, const int& ik)
{
