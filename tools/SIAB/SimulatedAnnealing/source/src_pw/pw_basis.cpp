    ...
    PW_Basis::setup_gg ()
{
    ...
    // (1.3) get ggwfc2
    assert (NKS > 0);
    assert (CARKX != nullptr);
    assert (CARKY != nullptr);
    assert (CARKZ != nullptr);
...
#ifdef __MPI
void
    PW_Basis::get_MPI_GVectors (void)
{
...
void
    PW_Basis::setup_structure_factor ()
{
                    ...
                    for (int ia = 0; ia < NA[it]; ia++)
                        {
                            ...
                            const double theta = TWO_PI
                                                 * (gcar[ig].x * CARPOSX[it][ia] + gcar[ig].y * CARPOSY[it][ia]
...
complex<double>*
    PW_Basis::get_sk (const int ik, const int it, const int ia) const
{
    timer::tick ("PW_Basis", "get_sk");
    const double arg
        = (CARKX[ik] * CARPOSX[it][ia] + CARKY[ik] * CARPOSY[it][ia] + CARKZ[ik] * CARPOSZ[it][ia]) * TWO_PI;
    ...
    for (int ig = 0; ig < this->ngk[ik]; ig++)
        {
            ...
            sk[ig] = kphase * this->eigts1 (iat, this->ig1[iig]) * this->eigts2 (iat, this->ig2[iig])
...
Vector3<double>
    PW_Basis::get_1qvec_cartesian (const int ik, const int ig) const
{
...
void
    PW_Basis::allocate_psi1d (const int& il)
{
                            ...
                            for (int ie = 0; ie < NE; ie++)
                                {
                                    for (int ikm = 0; ikm < kmesh; ikm++)
                                        {
                                            this->psi1d (it, l, n, ikm)
                                                += input.Coef.C4 (it, l, n, ie)
...
void
    PW_Basis::allocate_psi3d (const int& level)
{
...
void
    PW_Basis::calculate_psi3d (const int& ilevel, const int& ik)
{
...
complex<double>
    PW_Basis::calculateS (const int& iw, const int& iw2, const int& ik)
{
...
void
    PW_Basis::update_psi1d (const int& il, const int& ic, const int& ie, const double& c4_now, const double& c4_old)
{
...
void
    PW_Basis::update_psi3d (const int& il, const int& ic, const int& ik)
{
                    ...
                    for (int ig = 0; ig < npw; ig++)
                        {
                            ...
                            //				if(ig==35)
                            //				{
                            //					cout << "\n samepart=" << samepart[ig] << " haha=" << lphase * sk[ig] *
                            // flq[ig]; 					int ok; cin >> ok;
                            //				}
                        }
...
double
    PW_Basis::Polynomial_Interpolation (const int& it, const int& l, const int& n, const double& gnorm) const
{
    ...
    const double y = this->psi1d (it, l, n, iq) * x1 * x2 * x3 / 6.0 + this->psi1d (it, l, n, iq) * x0 * x2 * x3 / 2.0
...
void
    PW_Basis::calculate_Jlq (const int& ik, const int& iw, const int& ie)
{
...
complex<double>
    PW_Basis::calculate_Jlq_Phi (const int& ik, const int& mu)
{
    const int npw = this->ngk[ik];
