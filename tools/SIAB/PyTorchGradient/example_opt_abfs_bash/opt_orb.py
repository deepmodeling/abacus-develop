...
def cp_matrix(folders_matrix):
    pathlib.Path(utils.folder_opt + "/" + utils.folder_opt_matrix).mkdir(
    ...
    for folder_matrix in folders_matrix:
        ...
        os.system(
            f"cp {folder_matrix}/{matrix_file} {utils.folder_opt}/{utils.folder_opt_matrix}/{folder_matrix}"
        )
...
def add_nbands(folders_matrix):
    for folder_matrix in folders_matrix:
        with open(
            f"{utils.folder_opt}/{utils.folder_opt_matrix}/{folder_matrix}", "r"
        ) as file:
...
def set_input(folders_matrix):
    ...
    input = {
        ...
        "V_info": {"init_from_file": True, "same_band": False},
    }
