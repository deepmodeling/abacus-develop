        ...
        for ist in range(info.Nst):
            for it in info.Nt[ist]:
                for il in range(info.Nl[it]):
                    Q[ist][it][il] = torch_complex.mm(QI[ist][it][il], C[it][il]).view(
                        info.Nb[ist], -1
                    )
        return Q
        ...
        for ist in range(info.Nst):
            for it1, it2 in itertools.product(info.Nt[ist], info.Nt[ist]):
                for il1, il2 in itertools.product(
                    range(info.Nl[it1]), range(info.Nl[it2])
                ):
                    # SI_C[ia1*im1*ie1*ia2*im2,iu2]
                    SI_C = torch_complex.mm(
                        SI[ist][it1, it2][il1][il2].view(-1, info.Ne[it2]), C[it2][il2]
                    )
                    # SI_C[ia1*im1,ie1,ia2*im2*iu2]
        ...
        for ist in range(info.Nst):
            ...
            if V_info["same_band"]:
                ...
                for ib in range(info.Nb[ist]):
                    ...
                    V_s[ib] = torch_complex.dot(
                        Q_cat.conj(), torch_complex.mv(S_I, Q_cat)
                    ).real.view(-1)
                # V[ist][ib]
                V[ist] = torch.cat(V_s)
            else:
                ...
                # V[ist][ib1,ib2]
                V[ist] = torch_complex.mm(
                    Q_cat.conj(), torch_complex.mm(S_I, Q_cat.t())
                ).real
