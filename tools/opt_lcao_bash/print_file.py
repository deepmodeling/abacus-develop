...
def print_file_opt(info, dis):
    ...
    with open("INPUT", "w") as file:
        input = {
            "file_list": {
                "origin": [
                    f'../{info["input"]["element"]}-{info["input"]["rcut"]}-{distance}/orb_matrix/orb_matrix.0.dat'
                    for distance in dis[info["input"]["element"]]
                ],
                "linear": [
                    [
                        f'../{info["input"]["element"]}-{info["input"]["rcut"]}-{distance}/orb_matrix/orb_matrix.1.dat'
                        for distance in dis[info["input"]["element"]]
                    ]
                ],
            },
            ...
            "weight": {
                ...
                "bands_file": [
                    f'../{info["input"]["element"]}-{info["input"]["rcut"]}-{distance}/OUT.ABACUS/istate.info'
                    for distance in dis[info["input"]["element"]]
                ],
            },
            "C_init_info": {"init_from_file": False},
            "V_info": {"same_band": True, "init_from_file": True},
        }
