...
def get_nw():
    ...
    lcao = get_lcao()
    for T in lcao:
        nw[T] = functools.reduce(
            operator.add, ((2 * l + 1) * n for l, n in enumerate(lcao[T]))
        )
    return nw
...
def round_dis(dis, precision):
    ...
    for T1, T2 in dis:
        ...
        for i_dis, num in dis[T1, T2].items():
            i_dis = float(
                decimal.Decimal(i_dis).quantize(
...
def cut_dis(dis):
    ...
    for T1, T2 in dis:
        ...
        dis_cut[T1, T2] = {
            i_dis: num for i_dis, num in dis[T1, T2].items() if i_dis < Rcut_sum
        }
    return dis_cut
