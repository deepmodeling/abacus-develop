...
def cal_ABACUS(T1, T2, i_dis):
    ...
    with open(folder / "INPUT", "w") as file:
        ...
        input_dict["nbands"] = (
            read_stru.get_nw()[T1]
            if abs(i_dis) < 1e-10
            else read_stru.get_nw()[T1] + read_stru.get_nw()[T2]
        )
        input_dict["nspin"] = 1
    ...
    for T1, T2 in dis:
        ...
        if len(dis_tmp) <= info["dimer_num"]:
            ...
            if opt_mode == "linspace":
                dis_opt[T1, T2] = np.linspace(
                    min(dis_tmp), max(dis_tmp), info["dimer_num"]
                )
...
def cal_dis_weight(dis_opt, dis_all):
    ...
    for T1, T2 in dis_opt:
        ...
        for index, i_dis_opt in enumerate(dis_opt_TT):
            ...
            i_dis_up = (
                dis_opt_TT[index + 1] if index < len(dis_opt_TT) - 1 else np.infty
            )
            for i_dis, num in dis_all[T1, T2].items():
                if i_dis_low < i_dis < i_dis_opt:
                    i_weight += (
                        weight_func(i_dis_opt - i_dis, i_dis_opt - i_dis_low) * num
                    )
                elif i_dis == i_dis_opt:
                    i_weight += num
                elif i_dis_opt < i_dis < i_dis_up:
                    i_weight += (
                        weight_func(i_dis_opt - i_dis, i_dis_opt - i_dis_up) * num
                    )
