...
class GTORadials:
    ...
    def register_cgto(self, a, c, l, elem=None, mode="a"):
        ...
        if len(self.NumericalRadials[it]) <= l:
            ...
            self.NumericalRadials[it] += [
                [] for i in range(l - len(self.NumericalRadials[it]) + 1)
            ]
        if mode == "w":
    ...
    def build(self, rgrid, normalize=True):
        ...
        ]  # the output, indexed by [it][l][ic][r] to get grid value
        for it in range(ntype):
            ...
            for l in range(lmax + 1):
                for i in range(len(self.NumericalRadials[it][l])):  # for each CGTO
                    ...
                    for a, c in self.NumericalRadials[it][l][
                        i
                    ]:  # for each primitive GTO
    ...
    def _cgto_parse(data):
        ...
        ****
        ...
        data = [
            d.strip() for d in data.split("****")
        ]  # the separator of different elements
        data = [d for d in data if d]  # remove empty data
        nelem = len(data)
        out = [
            [
                []  # CGTO, because the number is still uncertain, leave as a list
        ...
        for d in data:  # for each element...
            ...
            ]  # annotation from Basis Set Exchange
            ...
            cgtopat = (
                r"^([A-Z]+)\s+(\d+)\s+(\d+\.\d+)$"  # the header of one contracted GTOs
            )
            ...
            while i < len(d):
                if re.match(elempat, d[i]):
                    ...
                    l_ = [
                        spectra.index(s_) for s_ in spec_
                    ]  # the angular momentum of this CGTO, for Pople basis
                    # it is possible to be not only one angular momentum
                    lmax = max(
                        lmax, max(l_)
                    )  # refresh the maximal angular momentum for this atom type
                    ngto = int(ngto)
                    # then read the coefficients and exponents by ngto lines:
                    ac_ = np.array(
                        [re.split(r"\s+", line) for line in d[i + 1 : i + 1 + ngto]]
                    )
                    a_, c_ = ac_[:, 0], ac_[:, 1:]
                    a_ = [float(a.upper().replace("D", "E")) for a in a_]
                    c_ = [
                        [float(c.upper().replace("D", "E")) for c in ci] for ci in c_
                    ]  # convert to float
        ...
        for it in range(ntype):  # for each type
            ...
            for l in range(len(NumericalRadial)):
                ...
                for ic in range(ncgto):
                    ngto = len(
                        NumericalRadial[l][ic]
                    )  # number of primitive GTOs for this l and ic
...
def fit_radial_with_gto(nao, ngto, l, r, rel_r=2):
    ...
    # bounds for c and a
    bounds = [(0, np.inf) for i in range(ngto)] + [
        (-np.inf, np.inf) for i in range(ngto)
    ]
    ...
    print(
        f"""NAO2GTO: Angular momentum {l}, with {ngto} superposition to fit numerical atomic orbitals on given grid, 
...
def read_nao(fpath):
    ...
    nzeta = [
        int(data[data.index(spec_symbol[l] + "orbital-->") + 1])
        for l in range(lmax + 1)
    ]
    ...
    chi = [
        [
            np.array(
                data[delim[iorb(l, zeta)] + 6 : delim[iorb(l, zeta) + 1]], np.float64
            )
    ...
    for it in range(len(out)):
        for l in range(len(out[it])):
            for ic in range(len(out[it][l])):
                plt.plot(
                    rgrid, out[it][l][ic], label=f"element {symbol}, l={l}, ic={ic}"
                )
    plt.legend()

    fgto = os.path.basename(fnao).replace(".orb", "") + ".gto" if fgto is None else fgto
    fgto = fnao.replace(
        os.path.basename(fnao), fgto
    )  # make sure that only the file name is changed
...
def read_stru(fpath):
    ...
    blocks = {
        lines[delim[i]]: lines[delim[i] + 1 : delim[i + 1]]
    ...
    if "LATTICE_VECTORS" in blocks:
        stru["lat"]["vec"] = [
            [float(x) for x in line.split()] for line in blocks["LATTICE_VECTORS"]
        ]
    ...
    # ============ ATOMIC_SPECIES ============
    stru["species"] = [
        dict(zip(["symbol", "mass", "pp_file", "pp_type"], line.split()))
...
def moldengen(folder: str, ndigits=3, ngto=7, rel_r=2, fmolden="ABACUS.molden"):
    ...
    assert (
        ("STRU" in files) and ("INPUT" in files) and ("KPT" in files)
    ...
    mo_files = (
        [mo_files] if nspin == 1 else [mo_files, mo_files.replace("1.txt", "2.txt")]
    )
...
"""
        ...
        self.assertEqual(
            len(cgtos[0][0][0]), 6
        )  # 6 primitive GTOs for the first CGTO of s
        self.assertEqual(
            len(cgtos[0][0][1]), 3
        )  # 3 primitive GTOs for the second CGTO of s
        self.assertEqual(
            len(cgtos[0][0][2]), 1
        )  # 1 primitive GTOs for the third CGTO of s
        self.assertEqual(
            len(cgtos[0][0][3]), 1
        )  # 1 primitive GTOs for the fourth CGTO of s
        # thus it is 6-311G basis for Li
        self.assertEqual(len(cgtos[0][1]), 3)  # 2 CGTOs for p
        self.assertEqual(
            len(cgtos[0][1][0]), 3
        )  # 3 primitive GTOs for the first CGTO of p
        self.assertEqual(
            len(cgtos[0][1][1]), 1
        )  # 1 primitive GTOs for the second CGTO of p
        self.assertEqual(
            len(cgtos[0][1][2]), 1
        )  # 1 primitive GTOs for the third CGTO of p
...
"""
        ...
        rgrid = np.linspace(
            0, ngrid * dr, ngrid
        )  # the evenly spaced grid points, the simplest case
...
Once meet any problem, please submit an issue at: https://github.com/deepmodeling/abacus-develop/issues
    ...
    parser.add_argument(
        "-f", "--folder", type=str, help="the folder of the ABACUS calculation"
    )
    parser.add_argument(
        "-n",
        "--ndigits",
        type=int,
        default=3,
        help="the number of digits for the MO coefficients",
    )
    parser.add_argument(
        "-g",
        "--ngto",
        type=int,
        default=7,
        help="the number of GTOs to fit ABACUS NAOs",
    )
    parser.add_argument(
        "-r",
        "--rel_r",
        type=int,
        default=2,
        help="the relative cutoff radius for the GTOs",
    )
    parser.add_argument(
        "-o",
        "--output",
        type=str,
        default="ABACUS.molden",
        help="the output Molden file name",
    )
...
if __name__ == "__main__":
    ...
    print(
        f"""MOLDEN: Generated Molden file {args.output} from ABACUS calculation in folder {args.folder}.
