...
class DOS:
    ...
    def _plot(self, dosplot, energy_f, dos, label):
        if self._nsplit == 1:
            dosplot.ax.plot(energy_f, dos, lw=dosplot._lw, label=label)
        elif self._nsplit == 2:
    ...
    @classmethod
    def set_vcband(
        cls, energy: Sequence, dos: Sequence, prec=0.01
        ...
        ecbm = energy[np.where(dos[band_cbm_index] > prec)[0][0] + band_cbm_index[0]][0]
...
class DOSPlot:
    ...
    def __init__(
        self, fig: Figure = None, ax: axes.Axes = None, nspin: int = 1, **kwargs
    ) -> None:
        self.nspin = nspin
        self.fig = fig
        self.ax = ax
    ...
    def _set_figure(
        self, energy_range: Sequence = [], dos_range: Sequence = [], notes: Dict = {}
    ):
        ...
        if "legend_prop" in self.plot_params.keys():
            self.ax.legend(
                by_label.values(), by_label.keys(), prop=self.plot_params["legend_prop"]
            )
        else:
    ...
    def plot(
        ...
        **kwargs,
        ...
        if "notes" in dosplot.plot_params.keys():
            dosplot._set_figure(
                energy_range, dos_range, notes=dosplot.plot_params["notes"]
            )
        else:
...
class PDOS(DOS):
    ...
    def _read(self):
        ...
        e_list = root.xpath("//energy_values")[0].text.replace(" ", "").split("\n")
        remove_empty(e_list)
        self.orbitals = []
        for i in range(norbitals):
            orb = OrderedDict()
            orb["index"] = int(root.xpath("//orbital/@index")[i].replace(" ", ""))
            orb["atom_index"] = int(
        ...
        if isinstance(species, (list, tuple)):
            for elem in dos.keys():
                ...
                with open(file_dir / f"{keyname}-{elem}.dat", "w") as f:
                    header_list.append(f"Partial DOS for {keyname}: {elem}")
                    header_list.append("")
                    for orb in self.orbitals:
                        if orb[keyname] == elem:
                            header_list.append(
            ...
            for elem in dos.keys():
                ...
                for ang in dos[elem].keys():
                    ...
                    if isinstance(dos[elem][ang], dict):
                        ...
                        header_list = [""]
                        data = np.hstack((self.energy.reshape(-1, 1), dos[elem][ang]))
                        with open(
        ...
        **kwargs,
        ...
        if isinstance(species, (list, tuple)):
            ...
            if "xlabel_params" in dosplot.plot_params.keys():
                dosplot.ax.set_xlabel(
                    "Energy(eV)", **dosplot.plot_params["xlabel_params"]
                )
            else:
            ...
            dosplots = []
            assert len(ax) >= len(dos.keys()), "There must be enough `axes` to plot."
            for i, elem in enumerate(dos.keys()):
                ...
                if "notes" in dosplot.plot_params.keys():
                    dosplot._set_figure(
                        energy_range, dos_range, notes=dosplot.plot_params["notes"][i]
                    )
                else:
                    dosplot._set_figure(energy_range, dos_range)
