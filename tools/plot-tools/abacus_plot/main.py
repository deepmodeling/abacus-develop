...
class Show:
    ...
    def show_cmdline(cls, args):
        ...
        if args.band and args.projected:
            ...
            index = key2int(
                text.pop("index", None)
            )  # keys of json must be dict, I convert them to int for `index` and `atom_index`
        ...
        if args.band and args.out:
            ...
            pdos.write(
                index=index, atom_index=atom_index, species=species, outdir=outdir
            )
        ...
        if args.dos and args.projected:
            ...
            figsize = text.pop("figsize", (12, 10))
            if index:
                fig, ax = plt.subplots(len(index), 1, sharex=True, figsize=figsize)
            if atom_index:
                fig, ax = plt.subplots(len(atom_index), 1, sharex=True, figsize=figsize)
            if species:
                fig, ax = plt.subplots(len(species), 1, sharex=True, figsize=figsize)
            prec = text.pop("prec", 0.01)
            pdos = PDOS(pdosfile)
            pdos.plot(
                fig=fig,
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="plot band structure and show band information.",
    )
    parser.add_argument(
        "-d",
        "--dos",
        ...
        type=int,
        default=None,
        help="plot density of state(DOS).",
    )
    parser.add_argument(
        "-p",
        "--projected",
        ...
        type=int,
        default=None,
        help="plot projected band structure or partial density of state(PDOS), should be used with `-b` or `-d`.",
    )
    parser.add_argument(
        "-o",
        "--out_parsed_data",
        ...
        type=int,
        default=None,
        help="output projected band structure or partial density of state(PDOS) to files, should be used with `-b` or `-d`.",
    )
