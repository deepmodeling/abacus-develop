...
def write_gaussian_cube(data: dict, fcube: str, **kwargs):
    ...
    with open(fcube, "w") as f:
        ...
        for i in range(0, len(data["data"]), 6):
            f.write(
                " ".join([f"%{width}.{ndigits}e" % x for x in data["data"][i : i + 6]])
            )
            f.write("\n")
    return
...
def slice2d(data: dict, prompt: str):
    ...
    assert re.match(
        r"[xyz]=\d+(\.\d+)?", prompt
    ), "the prompt should be like 'x=0.0', 'y=0.0', 'z=0.0'."
...
def check(args):
    ...
    if args.p1d is not None:
        assert args.p1d in [
            ...
            "y",
            "z",
        ], "the axis for 2D integration should be 'x', 'y' or 'z'."
    ...
    if args.minus is not None:
        assert os.path.exists(
            args.minus
        ), f"the input file {args.minus} does not exist."
    return
...
def _argparse():
    ...
    parser = argparse.ArgumentParser(
        description="manipulate the Gaussian cube format volumetric data."
    )
...
"""
    ...
    parser.add_argument(
        "-s", "--scale", type=float, help="scale the Gaussian cube file by a factor."
    )
    parser.add_argument(
        "--p1d",
        type=str,
        help="integrate the Gaussian cube file in 2D, followed by the axis: 'x', ...",
    )
    parser.add_argument(
        "--s2d",
        type=str,
        help="slice the Gaussian cube file along one axis, followed by string like 'x=0.0', 'y=0.0', 'z=0.0'. Note: should be fractional coodinate.",
    )
    parser.add_argument(
        "-p", "--plus", type=str, help="plus the two Gaussian cube files."
    )
    parser.add_argument(
        "-m", "--minus", type=str, help="minus the two Gaussian cube files."
    )
