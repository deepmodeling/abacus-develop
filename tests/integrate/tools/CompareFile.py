...
def is_two_data_diff(data1, data2, epsilon, compareAbsoluteDiff):
    if compareAbsoluteDiff:
        ...
        if abs(data1) < 1e-8 and abs(data2) < 1e-8:
            return False
        if abs(data1 - data2) / max(abs(data1), abs(data2)) > epsilon:
            return True
    return False
...
def ExitError(iline, line1, line2, jnumber=-1):
    if jnumber < 0:
        print(
            "Compare Error: line %d\n %s:%s %s:%s"
            % (iline + 1, file1, line1, file2, line2)
        )
    else:
        print(
            "Compare Error: line %d, column %d \n %s: %s\n %s: %s"
            % (iline + 1, jnumber + 1, file1, line1, file2, line2)
        )
    sys.exit(1)
