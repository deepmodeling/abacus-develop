...
for case in allcase:
    ...
    with open("%s/OUT.%s/running_%s.log" % (case, suffix, calculation)) as f1:
        for i, line in enumerate(f1.readlines()):
            ...
            elif line[4:23] == "Processor Number is":
                nproc = int(line.split()[-1])
            elif line[31:41] == "nkstot_ibz":
                kpt_ibz = int(line.split()[-1])

    scftime = []
    all_time = {}
    with open("%s/%s" % (case, logfile)) as f1:
        lines = f1.readlines()
    for i, line in enumerate(lines):
        if line[1:5] == "ITER":
            for j in range(i + 1, len(lines)):
                if lines[j][1:3] in ["CG", "DA", "GE", "GV"]:
                    scftime.append(float(lines[j].split()[-1]))
            ...
            for j in range(i + 1, len(lines)):
                if lines[j][1:5] == "----":
                    ...
                    if lines[j][23:44] == "build_Nonlocal_mu_new":
                        ...
                        print(
                            "WARNING: the time of below line (%d) in %s/%s is ignored\n%s"
                            % (j + 1, case, logfile, lines[j])
                        )
                        continue
                    else:
                        key = sline[1]
                    ...
                    if float(sline[-2]) >= time_ratio * 100:
                        if basis == "pw" and key not in added_time_pw + key_unnece_pw:
                            ...
                            basis == "lcao"
                            and key not in added_time_lcao + key_unnece_lcao
                        ):
    ...
    if os.path.isfile("%s/time.log" % case):
        with open("%s/time.log" % case) as f1:
            lines = f1.readlines()
        for line in lines:
    ...
    if basis == "pw":
        data_pw.append(
            [
                ...
                "%.2f" % all_time["total"],
                "%.2f" % scftime[0],
                "%.2f" % (np.array(scftime[1:]).sum() / (len(scftime) - 1)),
            ]
        )
        ...
        data_lcao.append(
            [
                ...
                str(len(scftime)),
                "%.2f" % all_time["total"],
                "%.2f" % (np.array(scftime).sum() / (len(scftime))),
            ]
        )
